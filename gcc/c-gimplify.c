...
void c_genericize(tree fndecl) {
  ...
  if (dump_orig) {
    ...
    if (local_dump_flags & TDF_RAW)
      dump_node(DECL_SAVED_TREE(fndecl), TDF_SLIM | local_dump_flags,
                dump_orig);
    else
...
static void add_block_to_enclosing(tree block) {
  unsigned i;
  tree enclosing;
  gimple bind;
...
tree c_build_bind_expr(tree block, tree body) {
  tree decls, bind;
...
static enum gimplify_status gimplify_compound_literal_expr(tree *expr_p,
  ...
  if ((TREE_CODE(TREE_TYPE(decl)) == COMPLEX_TYPE ||
      ...
      !TREE_THIS_VOLATILE(decl) && !needs_to_live_in_memory(decl))
...
static tree optimize_compound_literals_in_ctor(tree orig_ctor) {
  ...
  for (idx = 0; idx < num; idx++) {
    ...
    if (TREE_CODE(value) == CONSTRUCTOR)
      ...
      if (!TREE_ADDRESSABLE(value) && !TREE_ADDRESSABLE(decl) && init)
        newval = init;
    }
    if (newval == value)
      continue;
...
int c_gimplify_expr(tree *expr_p, gimple_seq *pre_p,
  ...
  switch (code) {
    ...
    case MODIFY_EXPR:
      if (TREE_CODE(TREE_OPERAND(*expr_p, 1)) == COMPOUND_LITERAL_EXPR) {
        ...
        if (!TREE_ADDRESSABLE(complit) && !TREE_ADDRESSABLE(decl) && init) {
