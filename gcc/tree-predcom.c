...
void dump_dref(FILE *file, dref ref) {
  if (ref->ref) {
    fprintf(file, "    ");
...
void dump_chain(FILE *file, chain_p chain) {
  dref a;
  ...
  if (chain->type == CT_COMBINATION) {
    fprintf(file, "  equal to %p %s %p in type ", (void *)chain->ch1,
            op_symbol_code(chain->op), (void *)chain->ch2);
...
void dump_chains(FILE *file, VEC(chain_p, heap) * chains) {
  chain_p chain;
  unsigned i;
...
void dump_component(FILE *file, struct component *comp) {
  dref a;
  unsigned i;
...
void dump_components(FILE *file, struct component *comps) {
...
static void release_chain(chain_p chain) {
  dref ref;
  unsigned i;
...
static void release_chains(VEC(chain_p, heap) * chains) {
  unsigned i;
  chain_p chain;
...
static void release_component(struct component *comp) {
...
static unsigned component_of(unsigned fathers[], unsigned a) {
  unsigned root, n;
...
static void merge_comps(unsigned fathers[], unsigned sizes[], unsigned a,
                        unsigned b) {
...
static bool suitable_reference_p(struct data_reference *a,
  ...
  if (!step || !is_gimple_reg_type(TREE_TYPE(ref)) || tree_could_throw_p(ref))
    return false;
...
static void aff_combination_dr_offset(struct data_reference *dr,
                                      aff_tree *offset) {
  aff_tree delta;
...
static bool determine_offset(struct data_reference *a, struct data_reference *b,
                             double_int *off) {
  ...
  tree_to_aff_combination_expand(DR_STEP(a), sizetype, &step, &name_expansions);
...
static basic_block last_always_executed_block(struct loop *loop) {
  unsigned i;
...
static struct component *split_data_refs_to_components(
    struct loop *loop, VEC(data_reference_p, heap) * datarefs,
  ...
  for (i = 0; VEC_iterate(ddr_p, depends, i, ddr); i++) {
    ...
    if (DR_IS_READ(dra) && DR_IS_READ(drb) &&
        (ia == bad || ib == bad || !determine_offset(dra, drb, &dummy_off)))
      continue;
  ...
  for (i = 0; VEC_iterate(data_reference_p, datarefs, i, dr); i++) {
    ...
    dataref->always_accessed = dominated_by_p(
        CDI_DOMINATORS, last_always_executed, gimple_bb(dataref->stmt));
...
static bool suitable_component_p(struct loop *loop, struct component *comp) {
  unsigned i;
  dref a, first;
...
static struct component *filter_suitable_components(struct loop *loop,
                                                    struct component *comps) {
...
static inline dref get_chain_root(chain_p chain) {
...
static void add_ref_to_chain(chain_p chain, dref ref) {
  ...
  if (ref->distance == chain->length && ref->pos > root->pos)
...
static bool valid_initializer_p(struct data_reference *ref, unsigned distance,
...
static gimple find_looparound_phi(struct loop *loop, dref ref, dref root) {
  ...
  if (!REFERENCE_CLASS_P(init_ref) && !DECL_P(init_ref))
    return NULL;
...
static void insert_looparound_copy(chain_p chain, dref ref, gimple phi) {
...
static void add_looparound_copies(struct loop *loop, chain_p chain) {
  unsigned i;
...
static void determine_roots_comp(struct loop *loop, struct component *comp,
...
static void determine_roots(struct loop *loop, struct component *comps,
                            VEC(chain_p, heap) * *chains) {
  struct component *comp;
...
static void replace_ref_with(gimple stmt, tree new_tree, bool set,
                             bool in_lhs) {
  tree val;
  gimple new_stmt;
...
static tree ref_at_iteration(struct loop *loop, tree ref, int iter) {
  ...
  if (integer_zerop(iv.step))
    *idx_p = unshare_expr(iv.base);
    ...
    if (POINTER_TYPE_P(type)) {
      val = fold_build2(MULT_EXPR, sizetype, iv.step, size_int(iter));
      val = fold_build2(POINTER_PLUS_EXPR, type, iv.base, val);
    } else {
      val =
          fold_build2(MULT_EXPR, type, iv.step, build_int_cst_type(type, iter));
...
void mark_virtual_ops_for_renaming(gimple stmt) {
  ssa_op_iter iter;
  tree var;
...
static void mark_virtual_ops_for_renaming_list(gimple_seq list) {
...
static void initialize_root_vars(struct loop *loop, chain_p chain,
                                 bitmap tmp_vars) {
  unsigned i;
...
static void initialize_root(struct loop *loop, chain_p chain, bitmap tmp_vars) {
  dref root = get_chain_root(chain);
  bool in_lhs = (chain->type == CT_STORE_LOAD || chain->type == CT_COMBINATION);

  initialize_root_vars(loop, chain, tmp_vars);
  replace_ref_with(root->stmt, VEC_index(tree, chain->vars, chain->length),
                   true, in_lhs);
}
...
static void initialize_root_vars_lm(struct loop *loop, dref root, bool written,
                                    VEC(tree, heap) * *vars,
                                    VEC(tree, heap) * inits, bitmap tmp_vars) {
  unsigned i;
...
static void execute_load_motion(struct loop *loop, chain_p chain,
                                bitmap tmp_vars) {
  VEC(tree, heap) * vars;
  dref a;
  ...
  initialize_root_vars_lm(loop, get_chain_root(chain), n_writes > 0, &vars,
                          chain->inits, tmp_vars);
  ...
  for (i = 0; VEC_iterate(dref, chain->refs, i, a); i++) {
    ...
    replace_ref_with(a->stmt, VEC_index(tree, vars, ridx), !is_read, !is_read);
  }
...
static gimple single_nonlooparound_use(tree name) {
  use_operand_p use;
  imm_use_iterator it;
  ...
  FOR_EACH_IMM_USE_FAST(use, it, name) {
    ...
    if (gimple_code(stmt) == GIMPLE_PHI) {
      ...
      if (bitmap_bit_p(looparound_phis, SSA_NAME_VERSION(PHI_RESULT(stmt))))
        continue;
...
static void remove_stmt(gimple stmt) {
  tree name;
  gimple next;
  gimple_stmt_iterator psi;
  ...
  if (gimple_code(stmt) == GIMPLE_PHI) {
    ...
    if (!next || !gimple_assign_ssa_name_copy_p(next) ||
...
static void execute_pred_commoning_chain(struct loop *loop, chain_p chain,
                                         bitmap tmp_vars) {
  unsigned i;
  dref a, root;
  tree var;
...
static unsigned determine_unroll_factor(VEC(chain_p, heap) * chains) {
  chain_p chain;
...
static void replace_phis_by_defined_names(VEC(chain_p, heap) * chains) {
  chain_p chain;
  dref a;
  unsigned i, j;
...
static void replace_names_by_phis(VEC(chain_p, heap) * chains) {
  chain_p chain;
  dref a;
  unsigned i, j;
...
static void execute_pred_commoning_cbck(struct loop *loop, void *data) {
...
static bool should_unroll_loop_p(struct loop *loop, unsigned factor,
...
static void base_names_in_chain_on(struct loop *loop, tree name, tree var) {
  gimple stmt, phi;
...
static void eliminate_temp_copies(struct loop *loop, bitmap tmp_vars) {
  edge e;
  gimple phi, stmt;
  tree name, use, var;
  gimple_stmt_iterator psi;
...
static gimple find_use_stmt(tree *name) {
  gimple stmt;
  tree rhs, lhs;
...
static bool may_reassociate_p(tree type, enum tree_code code) {
  ...
  return (commutative_tree_code(code) && associative_tree_code(code));
}
...
static gimple find_associative_operation_root(gimple stmt, unsigned *distance) {
  tree lhs;
  gimple next;
  ...
  while (1) {
    ...
    if (!next || gimple_assign_rhs_code(next) != code)
      break;
...
static gimple find_common_use_stmt(tree *name1, tree *name2) {
  gimple stmt1, stmt2;
...
static bool combinable_refs_p(dref r1, dref r2, enum tree_code *code,
                              bool *swap, tree *rslt_type) {
  ...
  aswap = (!commutative_tree_code(acode) && gimple_assign_rhs1(stmt) != name1);
  ...
  return (*code == acode && *swap == aswap && *rslt_type == atype);
}
...
static gimple reassociate_to_the_same_stmt(tree name1, tree name2) {
  ...
  tmp_stmt = gimple_build_assign_with_ops(gimple_assign_rhs_code(s1), tmp_name,
...
static gimple stmt_combining_refs(dref r1, dref r2) {
  gimple stmt1, stmt2;
...
static chain_p combine_chains(chain_p ch1, chain_p ch2) {
  dref r1, r2, nw;
  ...
  for (i = 0; (VEC_iterate(dref, ch1->refs, i, r1) &&
               VEC_iterate(dref, ch2->refs, i, r2));
       i++) {
...
static void try_combine_chains(VEC(chain_p, heap) * *chains) {
  unsigned i, j;
  chain_p ch1, ch2, cch;
...
static void set_alias_info(tree ref, struct data_reference *dr) {
  tree var;
...
static bool prepare_initializers_chain(struct loop *loop, chain_p chain) {
...
static void prepare_initializers(struct loop *loop,
                                 VEC(chain_p, heap) * chains) {
  chain_p chain;
  unsigned i;
...
static bool tree_predictive_commoning_loop(struct loop *loop) {
  ...
  if (!chains) {
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file, "Predictive commoning failed: no suitable chains\n");
    goto end;
  }
  ...
  if (unroll) {
    ...
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file, "Executing predictive commoning without unrolling.\n");
