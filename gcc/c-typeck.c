...
static bool null_pointer_constant_p(const_tree expr) {
  ...
  return (TREE_CODE(expr) == INTEGER_CST && !TREE_OVERFLOW(expr) &&
          integer_zerop(expr) &&
...
void c_incomplete_type_error(const_tree value, const_tree type) {
  ...
  retry:
    ...
    if (TREE_CODE(TYPE_NAME(type)) == IDENTIFIER_NODE)
      error("invalid use of undefined type %<%s %E%>", type_code_string,
            TYPE_NAME(type));
    else
...
bool c_vla_type_p(const_tree t) {
...
tree composite_type(tree t1, tree t2) {
  ...
  switch (code1) {
    ...
    case ARRAY_TYPE: {
      ...
      d1_variable =
          (!d1_zero && (TREE_CODE(TYPE_MIN_VALUE(d1)) != INTEGER_CST ||
                        TREE_CODE(TYPE_MAX_VALUE(d1)) != INTEGER_CST));
      d2_variable =
          (!d2_zero && (TREE_CODE(TYPE_MIN_VALUE(d2)) != INTEGER_CST ||
         ...
         If both do, merge the arg types.  Also merge the return types.  */
      {
        ...
        for (; p1;
          ...
          if (TREE_CODE(TREE_VALUE(p1)) == UNION_TYPE &&
            ...
            for (memb = TYPE_FIELDS(TREE_VALUE(p1)); memb;
                 memb = TREE_CHAIN(memb)) {
          ...
          if (TREE_CODE(TREE_VALUE(p2)) == UNION_TYPE &&
            ...
            for (memb = TYPE_FIELDS(TREE_VALUE(p2)); memb;
                 memb = TREE_CHAIN(memb)) {
              tree mv3 = TREE_TYPE(memb);
              if (mv3 && mv3 != error_mark_node && TREE_CODE(mv3) != ARRAY_TYPE)
                mv3 = TYPE_MAIN_VARIANT(mv3);
...
static tree c_common_type(tree t1, tree t2) {
  ...
  if (code1 == FIXED_POINT_TYPE || code2 == FIXED_POINT_TYPE) {
    ...
    if ((TYPE_UNSIGNED(t1) && TYPE_UNSIGNED(t2) && code1 == FIXED_POINT_TYPE &&
         code2 == FIXED_POINT_TYPE) ||
...
static int comptypes_internal(const_tree type1, const_tree type2) {
  const_tree t1 = type1;
  const_tree t2 = type2;
  int attrval, val;
  ...
  if (t1 == t2 || !t1 || !t2 || TREE_CODE(t1) == ERROR_MARK ||
...
int same_translation_unit_p(const_tree t1, const_tree t2) {
...
static struct tagged_tu_seen_cache *alloc_tagged_tu_seen_cache(const_tree t1,
                                                               const_tree t2) {
...
static void free_all_tagged_tu_seen_up_to(
    const struct tagged_tu_seen_cache *tu_til) {
  ...
  switch (TREE_CODE(t1)) {
    ...
    case RECORD_TYPE: {
      ...
      for (s1 = TYPE_FIELDS(t1), s2 = TYPE_FIELDS(t2); s1 && s2;
           s1 = TREE_CHAIN(s1), s2 = TREE_CHAIN(s2)) {
        int result;
        if (TREE_CODE(s1) != TREE_CODE(s2) || DECL_NAME(s1) != DECL_NAME(s2))
          break;
        result = comptypes_internal(TREE_TYPE(s1), TREE_TYPE(s2));
        if (result == 0)
          break;
...
static int function_types_compatible_p(const_tree f1, const_tree f2) {
  tree args1, args2;
  /* 1 if no need for warning yet, 2 if warning cause has been seen.  */
  int val = 1;
  int val1;
...
static int type_lists_compatible_p(const_tree args1, const_tree args2) {
  /* 1 if no need for warning yet, 2 if warning cause has been seen.  */
  int val = 1;
  int newval = 0;
  ...
  while (1) {
    ...
    else if (TREE_CODE(a1) == ERROR_MARK || TREE_CODE(a2) == ERROR_MARK)
      ;
    else if (!(newval = comptypes_internal(mv1, mv2))) {
      /* Allow  wait (union {union wait *u; int *i} *)
         and  wait (union wait *)  to be compatible.  */
      if (TREE_CODE(a1) == UNION_TYPE &&
          (TYPE_NAME(a1) == 0 || TYPE_TRANSPARENT_UNION(a1)) &&
          TREE_CODE(TYPE_SIZE(a1)) == INTEGER_CST &&
          tree_int_cst_equal(TYPE_SIZE(a1), TYPE_SIZE(a2))) {
        tree memb;
        for (memb = TYPE_FIELDS(a1); memb; memb = TREE_CHAIN(memb)) {
          tree mv3 = TREE_TYPE(memb);
          if (mv3 && mv3 != error_mark_node && TREE_CODE(mv3) != ARRAY_TYPE)
            mv3 = TYPE_MAIN_VARIANT(mv3);
        ...
        if (memb == 0)
          return 0;
                 ...
                 (TYPE_NAME(a2) == 0 || TYPE_TRANSPARENT_UNION(a2)) &&
                 TREE_CODE(TYPE_SIZE(a2)) == INTEGER_CST &&
                 tree_int_cst_equal(TYPE_SIZE(a2), TYPE_SIZE(a1))) {
        tree memb;
        for (memb = TYPE_FIELDS(a2); memb; memb = TREE_CHAIN(memb)) {
          tree mv3 = TREE_TYPE(memb);
          if (mv3 && mv3 != error_mark_node && TREE_CODE(mv3) != ARRAY_TYPE)
            mv3 = TYPE_MAIN_VARIANT(mv3);
...
tree decl_constant_value(tree decl) {
  if (/* Don't change a variable array bound or initial value to a constant
      ...
      current_function_decl != 0 && TREE_CODE(decl) != PARM_DECL &&
      !TREE_THIS_VOLATILE(decl) && TREE_READONLY(decl) &&
      DECL_INITIAL(decl) != 0 &&
...
static tree function_to_pointer_conversion(tree exp) {
  tree orig_exp = exp;
...
struct c_expr default_function_array_conversion(struct c_expr exp) {
...
tree perform_integral_promotions(tree exp) {
  tree type = TREE_TYPE(exp);
  enum tree_code code = TREE_CODE(type);
  ...
  if (TREE_CODE(type) == POINTER_TYPE) {
    if (CONVERT_EXPR_P(pointer) || TREE_CODE(pointer) == VIEW_CONVERT_EXPR) {
      ...
      if (warn_strict_aliasing > 2)
        if (strict_aliasing_warning(TREE_TYPE(TREE_OPERAND(pointer, 0)), type,
    ...
    error_at(loc, "invalid type argument of %qs (have %qT)", errorstring, type);
  return error_mark_node;
}
...
tree build_array_ref(tree array, tree index, location_t loc) {
  tree ret;
  bool swapped = false;
  ...
  if (TREE_CODE(TREE_TYPE(array)) == ARRAY_TYPE) {
    ...
    /* Array ref is const/volatile if the array elements are
       or if the array is.  */
    TREE_READONLY(rval) |=
        (TYPE_READONLY(TREE_TYPE(TREE_TYPE(array))) | TREE_READONLY(array));
    ...
    return build_indirect_ref(
        loc, build_binary_op(loc, PLUS_EXPR, ar, index, 0), "array indexing");
  }
}
...
tree build_external_ref(tree id, int fun, location_t loc) {
  tree ref;
  ...
  if (TREE_CODE(ref) == FUNCTION_DECL && !in_alignof) {
    if (!in_sizeof && !in_typeof)
      C_DECL_USED(ref) = 1;
    else if (DECL_INITIAL(ref) == 0 && DECL_EXTERNAL(ref) && !TREE_PUBLIC(ref))
  ...
  if (TREE_CODE(ref) == CONST_DECL) {
    ...
    TREE_CONSTANT(ref) = 1;
             ...
             (TREE_CODE(ref) == VAR_DECL || TREE_CODE(ref) == PARM_DECL ||
  ...
  /* C99 6.7.4p3: An inline definition of a function with external
     ...
     internal linkage.  */
           ...
           !TREE_PUBLIC(ref) && DECL_CONTEXT(ref) != current_function_decl)
    pedwarn(loc, 0,
            "%qD is static but used in inline function %qD "
            "which is not static",
            ref, current_function_decl);
...
static void record_maybe_used_decl(tree decl) {
...
struct c_expr c_expr_sizeof_expr(struct c_expr expr) {
  struct c_expr ret;
  if (expr.value == error_mark_node) {
    ...
    if (c_vla_type_p(TREE_TYPE(expr.value))) {
      ...
      ret.value =
          build2(COMPOUND_EXPR, TREE_TYPE(ret.value), expr.value, ret.value);
    }
...
struct c_expr c_expr_sizeof_type(struct c_type_name *t) {
  tree type;
  struct c_expr ret;
  type = groktypename(t);
  ret.value = c_sizeof(type);
  ret.original_code = ERROR_MARK;
  pop_maybe_used(type != error_mark_node ? C_TYPE_VARIABLE_SIZE(type) : false);
  return ret;
}
...
tree build_function_call(tree function, tree params) {
  ...
  nargs = convert_arguments(nargs, argarray, TYPE_ARG_TYPES(fntype), params,
                            function, fundecl);
  if (nargs < 0)
    return error_mark_node;
  ...
  if (CONVERT_EXPR_P(function) &&
    ...
    tree trap = build_function_call(built_in_decls[BUILT_IN_TRAP], NULL_TREE);
    int i;
  ...
  if (require_constant_value) {
    result = fold_build_call_array_initializer(TREE_TYPE(fntype), function,
                                               nargs, argarray);
    ...
    result =
        fold_build_call_array(TREE_TYPE(fntype), function, nargs, argarray);
...
static int convert_arguments(int nargs, tree *argarray, tree typelist,
                             tree values, tree function, tree fundecl) {
      ...
      if (type == error_mark_node || !COMPLETE_TYPE_P(type)) {
        ...
        if (warn_traditional_conversion || warn_traditional) {
          ...
          if (INTEGRAL_TYPE_P(type) && TREE_CODE(TREE_TYPE(val)) == REAL_TYPE)
            warning(0,
                    "passing argument %d of %qE as integer "
          ...
          if (INTEGRAL_TYPE_P(type) &&
            ...
            warning(0,
                    "passing argument %d of %qE as complex "
                    "rather than floating due to prototype",
                    argnum, rname);
          else if (TREE_CODE(type) == REAL_TYPE &&
                   INTEGRAL_TYPE_P(TREE_TYPE(val)))
            warning(0,
                    "passing argument %d of %qE as floating "
          ...
          /* ??? At some point, messages should be written about
            ...
            /* Warn if mismatch between argument and prototype
               ...
               prototype. */
                     ...
                     (formal_prec <= TYPE_PRECISION(TREE_TYPE(val)) ||
                      (type == dfloat128_type_node &&
                       (TREE_TYPE(val) != dfloat64_type_node &&
                        (TREE_TYPE(val) != dfloat32_type_node))) ||
                      (type == dfloat64_type_node &&
        ...
        parmval = convert_for_assignment(type, val, ic_argpass, fundecl,
                                         function, parmnum + 1);
...
struct c_expr parser_build_unary_op(enum tree_code code, struct c_expr arg,
                                    location_t loc) {
...
struct c_expr parser_build_binary_op(location_t location, enum tree_code code,
  ...
  result.value = build_binary_op(location, code, arg1.value, arg2.value, 1);
  result.original_code = code;
  ...
  if (TREE_OVERFLOW_P(result.value) && !TREE_OVERFLOW_P(arg1.value) &&
...
static tree pointer_diff(tree op0, tree op1) {
  ...
  if (CONVERT_EXPR_P(op0) && (TYPE_PRECISION(TREE_TYPE(op0)) ==
                              TYPE_PRECISION(TREE_TYPE(TREE_OPERAND(op0, 0)))))
    con0 = TREE_OPERAND(op0, 0);
  else
    con0 = op0;
  if (CONVERT_EXPR_P(op1) && (TYPE_PRECISION(TREE_TYPE(op1)) ==
  ...
  op0 = build_binary_op(input_location, MINUS_EXPR, convert(restype, op0),
...
tree build_unary_op(location_t location, enum tree_code code, tree xarg,
                    int flag) {
  ...
  switch (code) {
    ...
    case BIT_NOT_EXPR:
      ...
      if (typecode == INTEGER_TYPE ||
          (typecode == VECTOR_TYPE && !VECTOR_FLOAT_TYPE_P(TREE_TYPE(arg)))) {
        if (!noconvert)
          arg = default_conversion(arg);
    ...
    case TRUTH_NOT_EXPR:
      if (typecode != INTEGER_TYPE && typecode != FIXED_POINT_TYPE &&
          ...
          typecode != COMPLEX_TYPE) {
        error_at(location, "wrong type argument to unary exclamation mark");
        return error_mark_node;
      }
...

      {
        ...
        if (typecode == POINTER_TYPE) {
          ...
          if (!COMPLETE_OR_VOID_TYPE_P(TREE_TYPE(argtype))) {
            if (code == PREINCREMENT_EXPR || code == POSTINCREMENT_EXPR)
              error_at(location, "increment of pointer to unknown structure");
            else
              error_at(location, "decrement of pointer to unknown structure");
        ...
        if (!lvalue_or_else(
                arg, ((code == PREINCREMENT_EXPR || code == POSTINCREMENT_EXPR)
                          ? lv_increment
                          : lv_decrement)))
          return error_mark_node;
    ...
    case ADDR_EXPR:
      ...
      if ((DECL_P(arg) || REFERENCE_CLASS_P(arg)) &&
        ...
        argtype = c_build_type_variant(argtype, TREE_READONLY(arg),
...
static int lvalue_p(const_tree ref) {
...
static void readonly_error(tree arg, enum lvalue_use use) {
...
static int lvalue_or_else(const_tree ref, enum lvalue_use use) {
    ...
    switch (TREE_CODE(x)) {
      case COMPONENT_REF:
        if (DECL_C_BIT_FIELD(TREE_OPERAND(x, 1))) {
          error("cannot take address of bit-field %qD", TREE_OPERAND(x, 1));
          return false;
        }
      ...
      case RESULT_DECL:
        if (C_DECL_REGISTER(x) && DECL_NONLOCAL(x)) {
          if (TREE_PUBLIC(x) || TREE_STATIC(x) || DECL_EXTERNAL(x)) {
            error("global register variable %qD used in nested function", x);
            return false;
          }
          pedwarn(input_location, 0,
                  "register variable %qD used in nested function", x);
...
tree build_conditional_expr(tree ifexp, tree op1, tree op2) {
  ...
  if (TYPE_MAIN_VARIANT(type1) == TYPE_MAIN_VARIANT(type2)) {
    ...
    if (warn_sign_compare && !skip_evaluation) {
      ...
      if (unsigned_op1 ^ unsigned_op2) {
        ...
        else if ((unsigned_op2 && tree_expr_nonnegative_warnv_p(op1, &ovf)) ||
                 (unsigned_op1 && tree_expr_nonnegative_warnv_p(op2, &ovf)))
          /* OK */;
        else
    ...
    if (comp_target_types(type1, type2))
      ...
      result_type =
          build_pointer_type(qualify_type(TREE_TYPE(type1), TREE_TYPE(type2)));
      ...
      if (TREE_CODE(TREE_TYPE(type1)) == FUNCTION_TYPE)
        pedwarn(input_location, OPT_pedantic,
                ...
                "%<void *%> and function pointer");
      result_type =
          build_pointer_type(qualify_type(TREE_TYPE(type2), TREE_TYPE(type1)));
    } else {
      if (!objc_ok)
        pedwarn(input_location, 0,
  ...
  result_type =
      build_type_variant(result_type, TREE_READONLY(op1) || TREE_READONLY(op2),
...
tree build_compound_expr(tree expr1, tree expr2) {
  if (!TREE_SIDE_EFFECTS(expr1)) {
    /* The left-hand operand of a comma expression is like an expression
       statement: with -Wunused, we should warn if it doesn't have
       any side-effects, unless it was explicitly cast to (void).  */
    if (warn_unused_value) {
      if (VOID_TYPE_P(TREE_TYPE(expr1)) && CONVERT_EXPR_P(expr1))
        ; /* (void) a, b */
...
tree build_c_cast(tree type, tree expr) {
  ...
  if (type == TYPE_MAIN_VARIANT(TREE_TYPE(value))) {
    ...
    if (field) {
      ...
      t = digest_init(type, build_constructor_single(type, field, value), true,
                      0);
    ...
    if (warn_cast_qual && TREE_CODE(type) == POINTER_TYPE &&
    ...
    if (STRICT_ALIGNMENT && TREE_CODE(type) == POINTER_TYPE &&
        TREE_CODE(otype) == POINTER_TYPE &&
    ...
    if (TREE_CODE(value) == CALL_EXPR && TREE_CODE(type) != TREE_CODE(otype))
      warning(OPT_Wbad_function_cast,
              "cast from function call of type %qT "
              "to non-matching type %qT",
              otype, type);
    ...
    if (TREE_CODE(type) == POINTER_TYPE && TREE_CODE(otype) == INTEGER_TYPE &&
        ...
        && !TREE_CONSTANT(value))
      warning(OPT_Wint_to_pointer_cast,
    ...
    if (pedantic && TREE_CODE(type) == POINTER_TYPE &&
        TREE_CODE(otype) == POINTER_TYPE &&
        TREE_CODE(TREE_TYPE(otype)) == FUNCTION_TYPE &&
    ...
    if (TREE_CODE(value) == INTEGER_CST) {
      if (CONSTANT_CLASS_P(ovalue) && TREE_OVERFLOW(ovalue)) {
        ...
        value = build_int_cst_wide(TREE_TYPE(value), TREE_INT_CST_LOW(value),
...
tree c_cast_expr(struct c_type_name *type_name, tree expr) {
  tree type;
...
tree build_modify_expr(location_t location, tree lhs, enum tree_code modifycode,
                       tree rhs) {
  tree result;
  tree newrhs;
  ...
  if (modifycode != NOP_EXPR) {
    ...
    newrhs = build_binary_op(location, modifycode, lhs, rhs, 1);
  }
  ...
  if (TREE_CODE(lhs) == COMPONENT_REF &&
      (TREE_CODE(lhstype) == INTEGER_TYPE ||
       TREE_CODE(lhstype) == BOOLEAN_TYPE || TREE_CODE(lhstype) == REAL_TYPE ||
  ...
  newrhs = convert_for_assignment(lhstype, newrhs, ic_assign, NULL_TREE,
                                  NULL_TREE, 0);
  ...
  result = convert_for_assignment(olhstype, result, ic_assign, NULL_TREE,
                                  NULL_TREE, 0);
  protected_set_expr_location(result, location);
  return result;
}
...
static tree convert_for_assignment(tree type, tree rhs, enum impl_conv errtype,
...
#define WARN_FOR_ASSIGNMENT(LOCATION, OPT, AR, AS, IN, RE)                   \
  ...
  /* Arithmetic types all interconvert, and enum is treated like int.  */
            ...
            codel == FIXED_POINT_TYPE || codel == ENUMERAL_TYPE ||
            codel == COMPLEX_TYPE || codel == BOOLEAN_TYPE) &&
           (coder == INTEGER_TYPE || coder == REAL_TYPE ||
            coder == FIXED_POINT_TYPE || coder == ENUMERAL_TYPE ||
            coder == COMPLEX_TYPE || coder == BOOLEAN_TYPE))
    return convert_and_check(type, rhs);

  /* Aggregates in different TUs might need conversion.  */
  if ((codel == RECORD_TYPE || codel == UNION_TYPE) && codel == coder &&
  ...
  /* Conversions among pointers */
    ...
    if (VOID_TYPE_P(ttl) || VOID_TYPE_P(ttr) ||
        (target_cmp = comp_target_types(type, rhstype)) || is_opaque_pointer ||
        (c_common_unsigned_type(mvl) == c_common_unsigned_type(mvr))) {
      if (pedantic && ((VOID_TYPE_P(ttl) && TREE_CODE(ttr) == FUNCTION_TYPE) ||
                       (VOID_TYPE_P(ttr) && !null_pointer_constant_p(rhs) &&
                        TREE_CODE(ttl) == FUNCTION_TYPE)))
  ...
  switch (errtype) {
    case ic_argpass:
      ...
      break;
    case ic_assign:
      error("incompatible types when assigning to type %qT from type %qT", type,
            rhstype);
      break;
    case ic_init:
...
void store_init_value(tree decl, tree init) {
  ...
  if (!in_system_header && AGGREGATE_TYPE_P(TREE_TYPE(decl)) &&
      !TREE_STATIC(decl))
...
#define PUSH_SPELLING(KIND, VALUE, MEMBER)                           \
  {                                                                  \
    ...
    if (depth >= spelling_size) {                                    \
      spelling_size += 10;                                           \
      spelling_base =                                                \
          XRESIZEVEC(struct spelling, spelling_base, spelling_size); \
...
static void push_array_bounds(unsigned HOST_WIDE_INT bounds) {
...
void pedwarn_init(location_t location, int opt, const char *msgid) {
  char *ofwhat;
...
static void warning_init(int opt, const char *msgid) {
  char *ofwhat;
...
void maybe_warn_string_init(tree type, struct c_expr expr) {
  if (pedantic && TREE_CODE(type) == ARRAY_TYPE &&
      TREE_CODE(expr.value) == STRING_CST && expr.original_code != STRING_CST)
...
static tree digest_init(tree type, tree init, bool strict_string,
                        int require_constant) {
  enum tree_code code = TREE_CODE(type);
  tree inside_init = init;

  if (type == error_mark_node || !init || init == error_mark_node ||
  ...
  if (code == ARRAY_TYPE && inside_init &&
    ...
    /* Note that an array could be both an array of character type
       ...
       char.  */
    bool char_array =
        (typ1 == char_type_node || typ1 == signed_char_type_node ||
    ...
    if (char_array || wchar_array || char16_array || char32_array) {
      ...
      if (char_array) {
        ...
        if (typ2 == char_type_node) {
          error_init(
              "wide character array initialized from non-wide "
              "string");
          return error_mark_node;
        } else if (!comptypes(typ1, typ2)) {
          error_init(
              "wide character array initialized from "
      ...
      if (TYPE_DOMAIN(type) != 0 && TYPE_SIZE(type) != 0 &&
  ...
  if (code == VECTOR_TYPE && TREE_CODE(TREE_TYPE(inside_init)) == VECTOR_TYPE &&
  ...
  if (inside_init && TREE_TYPE(inside_init) != 0 &&
      (comptypes(TYPE_MAIN_VARIANT(TREE_TYPE(inside_init)),
       ...
       (code == VECTOR_TYPE && comptypes(TREE_TYPE(inside_init), type)) ||
       (code == POINTER_TYPE &&
    ...
    if (require_constant && (code == VECTOR_TYPE || !flag_isoc99) &&
        TREE_CODE(inside_init) == COMPOUND_LITERAL_EXPR) {
  ...
  if (code == INTEGER_TYPE || code == REAL_TYPE || code == FIXED_POINT_TYPE ||
    ...
    inside_init =
        convert_for_assignment(type, init, ic_init, NULL_TREE, NULL_TREE, 0);
...
void start_init(tree decl, tree asmspec_tree ATTRIBUTE_UNUSED, int top_level) {
  const char *locus;
...
void really_start_incremental_init(tree type) {
  ...
  if (TREE_CODE(constructor_type) == RECORD_TYPE ||
    ...
    if (TYPE_DOMAIN(constructor_type)) {
      ...
      if (constructor_max_index == NULL_TREE && TYPE_SIZE(constructor_type))
      ...
      constructor_index =
          convert(bitsizetype, TYPE_MIN_VALUE(TYPE_DOMAIN(constructor_type)));
    } else {
...
struct c_expr pop_init_level(int implicit) {
  struct constructor_stack *p;
  struct c_expr ret;
  ret.value = 0;
  ret.original_code = ERROR_MARK;
  ...
  if (p->replacement_value.value)
    ...
    if (constructor_erroneous)
      ...
      ret.value = build_constructor(constructor_type, constructor_elements);
...
static void add_pending_init(tree purpose, tree value, bool implicit) {
      ...
      if (bitpos < HOST_BITS_PER_WIDE_INT) {
        ...
        if (val[1] < 0)
          val[0] = -1;
                 ...
                 (((HOST_WIDE_INT)1) << (bitpos - 1 - HOST_BITS_PER_WIDE_INT)))
        val[0] |= ((HOST_WIDE_INT)-1) << (bitpos - HOST_BITS_PER_WIDE_INT);
    }
...
static tree find_init_member(tree field) {
  ...
  if (TREE_CODE(constructor_type) == ARRAY_TYPE) {
    ...
    if (constructor_incremental &&
        (!constructor_unfilled_fields ||
         tree_int_cst_lt(bitpos, bit_position(constructor_unfilled_fields))))
      set_nonincremental_init();
    ...
    if (!VEC_empty(constructor_elt, constructor_elements) &&
        (VEC_last(constructor_elt, constructor_elements)->index == field))
...
static void output_init_element(tree value, bool strict_string, tree type,
  ...
  if (TREE_CODE(value) == COMPOUND_LITERAL_EXPR && require_constant_value &&
      !flag_isoc99 && pending) {
    /* As an extension, allow initializing objects with static storage
       duration with compound literals (which are then treated just as
  ...
  if (field &&
      (TREE_TYPE(field) == error_mark_node ||
       (COMPLETE_TYPE_P(TREE_TYPE(field)) &&
        ...
        (TREE_CODE(constructor_type) == ARRAY_TYPE || TREE_CHAIN(field)))))
    return;
  ...
  if (TREE_CODE(constructor_type) == ARRAY_TYPE &&
    ...
    if (!implicit) {
      if (TREE_SIDE_EFFECTS(
              VEC_last(constructor_elt, constructor_elements)->value))
        warning_init(0, "initialized field with side-effects overwritten");
      else if (warn_override_init)
        warning_init(OPT_Woverride_init, "initialized field overwritten");
    }
  ...
  while (elt) {
    if (TREE_CODE(constructor_type) == ARRAY_TYPE) {
      if (tree_int_cst_equal(elt->purpose, constructor_unfilled_index))
        output_init_element(elt->value, true, TREE_TYPE(constructor_type),
        ...
        if (elt->right)
          ...
          elt = elt->parent;
          if (elt &&
              tree_int_cst_lt(constructor_unfilled_index, elt->purpose)) {
            next = elt->purpose;
            break;
          }
        }
      ...
      if (tree_int_cst_equal(elt_bitpos, ctor_unfilled_bitpos)) {
        ...
        if (elt->right)
          ...
          while (elt->parent && elt->parent->right == elt)
            elt = elt->parent;
          elt = elt->parent;
          if (elt && (tree_int_cst_lt(ctor_unfilled_bitpos,
...
void process_init_element(struct c_expr value, bool implicit) {
  ...
  /* Handle superfluous braces around string cst as in
     char x[] = {"foo"}; */
  if (string_flag && constructor_type &&
      TREE_CODE(constructor_type) == ARRAY_TYPE &&
  ...
  while (1) {
    if (TREE_CODE(constructor_type) == RECORD_TYPE) {
      ...
      if (fieldcode == ARRAY_TYPE && !require_constant_value &&
      ...
      if (value.value != 0 && fieldcode == ARRAY_TYPE &&
          INTEGRAL_TYPE_P(TREE_TYPE(fieldtype)) && string_flag)
        value.value = orig_value;
      /* Otherwise, if we have come to a subaggregate,
         and we don't have an element of its type, push into it.  */
      else if (value.value != 0 && value.value != error_mark_node &&
      ...
      if (value.value) {
        push_member_name(constructor_fields);
        output_init_element(value.value, strict_string, fieldtype,
         ...
         directly output as a constructor.  */
      {
        ...
        if (DECL_SIZE(constructor_fields))
          constructor_bit_index =
              size_binop(PLUS_EXPR, bit_position(constructor_fields),
      ...
      if (constructor_fields == 0) {
        pedwarn_init(input_location, 0, "excess elements in union initializer");
        break;
      }
      ...
      if (!in_system_header && !constructor_designated &&
          !(value.value &&
            (integer_zerop(value.value) || real_zerop(value.value))))
        warning(OPT_Wtraditional,
      ...
      /* Accept a string constant to initialize a subarray.  */
      if (value.value != 0 && eltcode == ARRAY_TYPE &&
          INTEGRAL_TYPE_P(TREE_TYPE(elttype)) && string_flag)
        value.value = orig_value;
      /* Otherwise, if we have come to a subaggregate,
         and we don't have an element of its type, push into it.  */
      else if (value.value != 0 && value.value != error_mark_node &&
      ...
      if (constructor_max_index != 0 &&
        ...
        pedwarn_init(input_location, 0, "excess elements in array initializer");
        break;
      }
      ...
      if (value.value) {
        push_array_bounds(tree_low_cst(constructor_index, 1));
        output_init_element(value.value, strict_string, elttype,
    ...
    else if (constructor_type != error_mark_node && constructor_fields == 0) {
      pedwarn_init(input_location, 0, "excess elements in scalar initializer");
      break;
    } else {
      if (value.value)
        output_init_element(value.value, strict_string, constructor_type,
...
/* Build a complete asm-statement, whose components are a CV_QUALIFIER
   ...
   an ASM_EXPR node).  */
tree build_asm_stmt(tree cv_qualifier, tree args) {
...
tree build_asm_expr(tree string, tree outputs, tree inputs, tree clobbers,
                    bool simple) {
  tree tail;
  tree args;
  int i;
  ...
  for (i = 0, tail = outputs; tail; ++i, tail = TREE_CHAIN(tail)) {
    ...
    if (output != error_mark_node &&
        (TREE_READONLY(output) || TYPE_READONLY(TREE_TYPE(output)) ||
    ...
    if (parse_output_constraint(&constraint, i, ninputs, noutputs, &allows_mem,
                                &allows_reg, &is_inout)) {
...
tree c_finish_goto_label(tree label) {
...
tree c_finish_goto_ptr(tree expr) {
...
tree c_finish_return(tree retval) {
  ...
  if (!retval) {
    ...
    if ((warn_return_type || flag_isoc99) && valtype != 0 &&
    ...
    tree t = convert_for_assignment(valtype, retval, ic_return, NULL_TREE,
                                    NULL_TREE, 0);
    ...
    while (1) {
      ...
      case POINTER_PLUS_EXPR:
        ...
        case MINUS_EXPR:
             ...
             don't give a warning.  */
          {
            ...
            while (!POINTER_TYPE_P(TREE_TYPE(op1)) &&
                   (CONVERT_EXPR_P(op1) || TREE_CODE(op1) == NON_LVALUE_EXPR))
        ...
        case ADDR_EXPR:
          ...
          while (REFERENCE_CLASS_P(inner) && TREE_CODE(inner) != INDIRECT_REF)
            inner = TREE_OPERAND(inner, 0);
...
tree do_case(tree low_value, tree high_value) {
  ...
  if (c_switch_stack && !c_switch_stack->blocked_stmt_expr &&
    ...
    if (low_value)
      error(
          "case label in statement expression not containing "
          "enclosing switch statement");
    else
      error(
          "%<default%> label in statement expression not containing "
          "enclosing switch statement");
  } else if (c_switch_stack && c_switch_stack->blocked_vm) {
    if (low_value)
      error(
          "case label in scope of identifier with variably modified "
          "type not containing enclosing switch statement");
    else
      error(
          "%<default%> label in scope of identifier with variably "
...
void c_finish_case(tree body) {
  ...
  c_do_switch_warnings(cs->cases, switch_location, TREE_TYPE(cs->switch_expr),
...
void c_finish_if_stmt(location_t if_locus, tree cond, tree then_block,
...
void c_finish_loop(location_t start_locus, tree cond, tree incr, tree body,
...
tree c_finish_bc_stmt(tree *label_p, bool is_break) {
  bool skip;
  tree label = *label_p;
...
static void emit_side_effect_warnings(tree expr) {
...
tree c_process_expr_stmt(tree expr) {
  if (!expr)
    return NULL_TREE;
  ...
  if (!STATEMENT_LIST_STMT_EXPR(cur_stmt_list) && warn_unused_value)
...
tree c_finish_expr_stmt(tree expr) {
  if (expr)
...
tree c_begin_stmt_expr(void) {
  ...
  for (glist = label_context_stack_se->labels_used; glist != NULL;
       glist = glist->next) {
...
tree c_finish_stmt_expr(tree body) {
  ...
  for (dlist = label_context_stack_se->labels_def; dlist != NULL;
       dlist = dlist->next) {
    C_DECL_UNJUMPABLE_STMT_EXPR(dlist->label) = 1;
  }
  /* It is again possible to define labels with a goto just outside
     this statement expression.  */
  for (glist = label_context_stack_se->next->labels_used; glist != NULL;
       glist = glist->next) {
...
continue_searching:
  ...
  if (last == error_mark_node ||
      (last == BIND_EXPR_BODY(body) && BIND_EXPR_VARS(body) == NULL)) {
...
void c_begin_vm_scope(unsigned int scope) {
  ...
  for (glist = label_context_stack_vm->labels_used; glist != NULL;
       glist = glist->next) {
...
void c_end_vm_scope(unsigned int scope) {
  if (label_context_stack_vm == NULL)
    return;
  ...
  while (label_context_stack_vm->scope == scope) {
    ...
    /* It is no longer possible to jump to labels defined within this
       scope.  */
    for (dlist = label_context_stack_vm->labels_def; dlist != NULL;
         dlist = dlist->next) {
      C_DECL_UNJUMPABLE_VM(dlist->label) = 1;
    }
    /* It is again possible to define labels with a goto just outside
       this scope.  */
    for (glist = label_context_stack_vm->next->labels_used; glist != NULL;
         glist = glist->next) {
...
tree c_begin_compound_stmt(bool do_scope) {
...
tree c_end_compound_stmt(tree stmt, bool do_scope) {
  tree block = NULL;
  ...
  /* If this compound statement is nested immediately inside a statement
     ...
     was really last.  */
  if (cur_stmt_list && STATEMENT_LIST_STMT_EXPR(cur_stmt_list) &&
...
void push_cleanup(tree ARG_UNUSED(decl), tree cleanup, bool eh_only) {
  enum tree_code code;
  tree stmt, list;
  bool stmt_expr;
...
tree build_binary_op(location_t location, enum tree_code code, tree orig_op0,
  ...
  if ((invalid_op_diag = targetm.invalid_binary_op(code, type0, type1))) {
    error_at(location, invalid_op_diag);
    return error_mark_node;
  }
  ...
  switch (code) {
    ...
    case BIT_XOR_EXPR:
      ...
      /* Allow vector types which are not floating point types.   */
               ...
               !VECTOR_FLOAT_TYPE_P(type0) && !VECTOR_FLOAT_TYPE_P(type1))
        common = 1;
      break;
  ...
  if ((code0 == INTEGER_TYPE || code0 == REAL_TYPE || code0 == COMPLEX_TYPE ||
    ...
    if (shorten && none_complex) {
      ...
      result_type = shorten_binary_op(result_type, op0, op1, shorten == -1);
    }
    ...
    if (short_shift) {
      ...
      if (TYPE_PRECISION(TREE_TYPE(arg0)) < TYPE_PRECISION(result_type)
        ...
        result_type =
            c_common_signed_or_unsigned_type(unsigned_arg, TREE_TYPE(arg0));
...
tree c_objc_common_truthvalue_conversion(location_t location, tree expr) {
  switch (TREE_CODE(TREE_TYPE(expr))) {
    case ARRAY_TYPE:
      error_at(location,
               "used array that cannot be converted to pointer where scalar is "
               "required");
      return error_mark_node;
...
tree c_expr_to_decl(tree expr, bool *tc ATTRIBUTE_UNUSED, bool *se) {
...
tree c_begin_omp_parallel(void) {
  tree block;
...
tree c_finish_omp_parallel(tree clauses, tree block) {
  tree stmt;
...
tree c_finish_omp_task(tree clauses, tree block) {
  tree stmt;
  ...
  for (pc = &clauses, c = clauses; c; c = *pc) {
    ...
    switch (OMP_CLAUSE_CODE(c)) {
      ...
      case OMP_CLAUSE_REDUCTION:
        ...
        if (AGGREGATE_TYPE_P(TREE_TYPE(t)) || POINTER_TYPE_P(TREE_TYPE(t))) {
          ...
          if (r_name) {
            error("%qE has invalid type for %<reduction(%s)%>", t, r_name);
            remove = true;
          }
        }
        goto check_dup_generic;
    ...
    if (!remove) {
      ...
      if (need_implicitly_determined) {
        ...
        if (share_name) {
          error("%qE is predetermined %qs for %qs", t, share_name, name);
          remove = true;
        }
      }
    }
...
tree c_build_qualified_type(tree type, int type_quals) {
  ...
  if (TREE_CODE(type) == ARRAY_TYPE) {
    tree t;
    tree element_type = c_build_qualified_type(TREE_TYPE(type), type_quals);
