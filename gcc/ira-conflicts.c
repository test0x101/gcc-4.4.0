...
#define CONFLICT_ALLOCNO_P(A1, A2)                                           \
  (ALLOCNO_MIN(A1) <= ALLOCNO_CONFLICT_ID(A2) &&                             \
   ...
   TEST_ALLOCNO_SET_BIT(conflicts[ALLOCNO_NUM(A1)], ALLOCNO_CONFLICT_ID(A2), \
                        ALLOCNO_MIN(A1), ALLOCNO_MAX(A1)))
...
static bool build_conflict_bit_table(void) {
  ...
  conflicts =
      (IRA_INT_TYPE **)ira_allocate(sizeof(IRA_INT_TYPE *) * ira_allocnos_num);
  allocated_words_num = 0;
  FOR_EACH_ALLOCNO(allocno, ai) {
    ...
    conflict_bit_vec_words_num =
        ((ALLOCNO_MAX(allocno) - ALLOCNO_MIN(allocno) + IRA_INT_BITS) /
         IRA_INT_BITS);
    allocated_words_num += conflict_bit_vec_words_num;
    conflicts[num] = (IRA_INT_TYPE *)ira_allocate(sizeof(IRA_INT_TYPE) *
                                                  conflict_bit_vec_words_num);
    ...
    for (r = ira_start_point_ranges[i]; r != NULL; r = r->start_next) {
      ...
      EXECUTE_IF_SET_IN_SPARSESET(allocnos_live, j) {
        ...
        if (ira_reg_classes_intersect_p[cover_class]
            ...
            && num != (int)j) {
          SET_ALLOCNO_SET_BIT(conflicts[num], ALLOCNO_CONFLICT_ID(live_a),
                              ALLOCNO_MIN(allocno), ALLOCNO_MAX(allocno));
          SET_ALLOCNO_SET_BIT(conflicts[j], id, ALLOCNO_MIN(live_a),
                              ALLOCNO_MAX(live_a));
        }
      }
    }
...
static bool commutative_constraint_p(const char *str) {
  bool ignore_p;
  int c;
...
static int get_dup_num(int op_num, bool use_commut_op_p) {
  ...
  if (use_commut_op_p) {
    if (commutative_constraint_p(recog_data.constraints[op_num]))
      op_num++;
             ...
             commutative_constraint_p(recog_data.constraints[op_num - 1]))
      op_num--;
    else
    ...
    if (c == '#')
      ...
      switch (c) {
        ...
        case 'Z': {
          ...
          cl = (c == 'r' ? GENERAL_REGS : REG_CLASS_FROM_CONSTRAINT(c, str));
          if (cl != NO_REGS)
            return -1;
...
/* Return the operand which should be, in any case, the same as
   ...
   this.  */
static rtx get_dup(int op_num, bool use_commut_op_p) {
...
static rtx go_through_subreg(rtx x, int *offset) {
  ...
  *offset = 0;
  ...
  if (REGNO(reg) < FIRST_PSEUDO_REGISTER)
    *offset = subreg_regno_offset(REGNO(reg), GET_MODE(reg), SUBREG_BYTE(x),
                                  GET_MODE(x));
  else
...
/* Process registers REG1 and REG2 in move INSN with execution
   ...
   FALSE.  */
static bool process_regs_for_copy(rtx reg1, rtx reg2, bool constraint_p,
                                  rtx insn, int freq) {
  ...
  if (HARD_REGISTER_P(reg1)) {
    ...
    cp = ira_add_allocno_copy(ira_curr_regno_allocno_map[REGNO(reg1)],
                              ira_curr_regno_allocno_map[REGNO(reg2)], freq,
                              constraint_p, insn, ira_curr_loop_tree_node);
  ...
  for (;;) {
    ...
    ira_allocate_and_set_costs(&ALLOCNO_CONFLICT_HARD_REG_COSTS(a), cover_class,
                               0);
...
/* Process all of the output registers of the current insn and
   ...
   FREQ.  */
static void process_reg_shuffles(rtx reg, int op_num, int freq) {
  int i;
  rtx another_reg;
...
/* Process INSN and create allocno copies if necessary.  For example,
   ...
   operand insn.  */
static void add_insn_allocno_copies(rtx insn) {
  rtx set, operand, dup;
  const char *str;
  ...
  if ((set = single_set(insn)) != NULL_RTX && REG_SUBREG_P(SET_DEST(set)) &&
    ...
    for (i = 0; i < recog_data.n_operands; i++) {
      ...
      if (REG_SUBREG_P(operand) &&
          find_reg_note(insn, REG_DEAD,
                        REG_P(operand) ? operand : SUBREG_REG(operand)) !=
              NULL_RTX) {
        ...
        for (j = 0, commut_p = false; j < 2; j++, commut_p = true)
          if ((dup = get_dup(i, commut_p)) != NULL_RTX && REG_SUBREG_P(dup) &&
...
static void propagate_copies(void) {
  ...
  FOR_EACH_COPY(cp, ci) {
    ...
    if (!CONFLICT_ALLOCNO_P(parent_a1, parent_a2))
      ira_add_allocno_copy(parent_a1, parent_a2, cp->freq, cp->constraint_p,
...
static void build_allocno_conflicts(ira_allocno_t a) {
  int i, px, parent_num;
  ...
  px = 0;
  FOR_EACH_ALLOCNO_IN_SET(allocno_conflicts, ALLOCNO_MIN(a), ALLOCNO_MAX(a), i,
                          asi) {
    another_a = ira_conflict_id_allocno_map[i];
    ira_assert(ira_reg_classes_intersect_p[ALLOCNO_COVER_CLASS(a)]
  ...
  FOR_EACH_ALLOCNO_IN_SET(allocno_conflicts, ALLOCNO_MIN(a), ALLOCNO_MAX(a), i,
    ...
    if ((another_parent_a = ALLOCNO_CAP(another_a)) == NULL &&
        (another_parent_a =
             (parent->regno_allocno_map[ALLOCNO_REGNO(another_a)])) == NULL)
      continue;
    ...
    SET_ALLOCNO_SET_BIT(conflicts[parent_num],
                        ...
                        ALLOCNO_MIN(parent_a), ALLOCNO_MAX(parent_a));
  }
}
...
static void print_hard_reg_set(FILE *file, const char *title,
                               HARD_REG_SET set) {
  int i, start;
...
static void print_conflicts(FILE *file, bool reg_p) {
  ...
  FOR_EACH_ALLOCNO(a, ai) {
    ...
    COPY_HARD_REG_SET(conflicting_hard_regs, ALLOCNO_CONFLICT_HARD_REGS(a));
    AND_COMPL_HARD_REG_SET(conflicting_hard_regs, ira_no_alloc_regs);
    AND_HARD_REG_SET(conflicting_hard_regs,
                     reg_class_contents[ALLOCNO_COVER_CLASS(a)]);
    print_hard_reg_set(file,
                       ";;     conflict hard regs:", conflicting_hard_regs);
  }
  fprintf(file, "\n");
}
  ...
  FOR_EACH_ALLOCNO(a, ai) {
    ...
    if ((!flag_caller_saves && ALLOCNO_CALLS_CROSSED_NUM(a) != 0)
        ...
        || (optimize <= 1 &&
            ...
            (decl = attrs->decl) != NULL && VAR_OR_FUNCTION_DECL_P(decl) &&
      ...
      IOR_HARD_REG_SET(ALLOCNO_TOTAL_CONFLICT_HARD_REGS(a), temp_hard_reg_set);
      IOR_HARD_REG_SET(ALLOCNO_CONFLICT_HARD_REGS(a), no_caller_save_reg_set);
      IOR_HARD_REG_SET(ALLOCNO_CONFLICT_HARD_REGS(a), temp_hard_reg_set);
    }
  }
  if (optimize && ira_conflicts_p && internal_flag_ira_verbose > 2 &&
      ira_dump_file != NULL)
