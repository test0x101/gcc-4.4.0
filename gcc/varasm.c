...
static GTY((if_marked("tree_map_marked_p"),
            param_is(struct tree_map))) htab_t emutls_htab;
...
static tree get_emutls_object_name(tree name) {
  const char *prefix =
      (targetm.emutls.var_prefix ? targetm.emutls.var_prefix
                                 : "__emutls_v" EMUTLS_SEPARATOR);
  return prefix_name(prefix, name);
}

tree default_emutls_var_fields(tree type, tree *name ATTRIBUTE_UNUSED) {
...
static tree get_emutls_init_templ_addr(tree decl) {
  ...
  if (!targetm.emutls.tmpl_prefix || targetm.emutls.tmpl_prefix[0]) {
    const char *prefix =
        (targetm.emutls.tmpl_prefix ? targetm.emutls.tmpl_prefix
...
static int emutls_common_1(void **loc, void *xstmts) {
...
static int section_entry_eq(const void *p1, const void *p2) {
...
static hashval_t section_entry_hash(const void *p) {
...
static int object_block_entry_eq(const void *p1, const void *p2) {
...
static hashval_t object_block_entry_hash(const void *p) {
...
section *get_unnamed_section(unsigned int flags, void (*callback)(const void *),
                             const void *data) {
  section *sect;
...
section *get_section(const char *name, unsigned int flags, tree decl) {
  section *sect, **slot;

  slot = (section **)htab_find_slot_with_hash(section_htab, name,
...
static struct object_block *get_block_for_section(section *sect) {
  ...
  slot = htab_find_slot_with_hash(object_block_htab, sect, hash_section(sect),
                                  INSERT);
  block = (struct object_block *)*slot;
  if (block == NULL) {
    block =
        (struct object_block *)ggc_alloc_cleared(sizeof(struct object_block));
    block->sect = sect;
    *slot = block;
  }
  return block;
...
static rtx create_block_symbol(const char *label, struct object_block *block,
...
section *unlikely_text_section(void) {
  if (cfun) {
    ...
    return get_named_section(NULL, crtl->subsections.unlikely_text_section_name,
                             0);
  } else
...
bool unlikely_text_section_p(section *sect) {
  ...
  return (name && sect && SECTION_STYLE(sect) == SECTION_NAMED &&
...
section *get_named_section(tree decl, const char *name, int reloc) {
  unsigned int flags;
...
void resolve_unique_section(tree decl, int reloc ATTRIBUTE_UNUSED,
                            int flag_function_or_data_sections) {
  if (DECL_SECTION_NAME(decl) == NULL_TREE && targetm.have_named_sections &&
...
static void asm_output_bss(FILE *file, tree decl ATTRIBUTE_UNUSED,
                           const char *name,
...
static void asm_output_aligned_bss(FILE *file, tree decl ATTRIBUTE_UNUSED,
                                   const char *name,
...
section *function_section(tree decl) {
  ...
  if (decl != NULL_TREE && DECL_SECTION_NAME(decl) != NULL_TREE)
    return reloc ? unlikely_text_section() : get_named_section(decl, NULL, 0);
  else
...
section *current_function_section(void) {
  ...
  if (current_function_decl != NULL_TREE &&
    ...
    return in_cold_section_p
               ...
               : get_named_section(current_function_decl, NULL, 0);
  else
...
section *default_no_function_rodata_section(tree decl ATTRIBUTE_UNUSED) {
  return readonly_data_section;
}

/* Return the section to use for string merging.  */

static section *mergeable_string_section(
  ...
  if (HAVE_GAS_SHF_MERGE && flag_merge_constants &&
      ...
      TREE_CODE(TREE_TYPE(decl)) == ARRAY_TYPE && align <= 256 &&
    ...
    if (modesize >= 8 && modesize <= 256 && (modesize & (modesize - 1)) == 0) {
      if (align < modesize)
        align = modesize;
      ...
      if (i == len - unit) {
        sprintf(name, ".rodata.str%d.%d", modesize / 8, (int)(align / 8));
...
section *mergeable_constant_section(enum machine_mode mode ATTRIBUTE_UNUSED,
                                    unsigned HOST_WIDE_INT align
                                        ATTRIBUTE_UNUSED,
                                    unsigned int flags ATTRIBUTE_UNUSED) {
  unsigned int modesize = GET_MODE_BITSIZE(mode);

  if (HAVE_GAS_SHF_MERGE && flag_merge_constants && mode != VOIDmode &&
      mode != BLKmode && modesize <= align && align >= 8 && align <= 256 &&
      (align & (align - 1)) == 0) {
    char name[24];
...
static bool bss_initializer_p(const_tree decl) {
  return (DECL_INITIAL(decl) == NULL || DECL_INITIAL(decl) == error_mark_node ||
          (flag_zero_initialized_in_bss
           /* Leave constant zeroes in .rodata so they
              can be shared.  */
           && !TREE_READONLY(decl) && initializer_zerop(DECL_INITIAL(decl))));
}
...
void align_variable(tree decl, bool dont_output_data) {
  ...
  if (align > MAX_OFILE_ALIGNMENT) {
    warning(0,
            "alignment of %q+D is greater than maximum object "
            "file alignment.  Using %d",
  ...
  if (!DECL_USER_ALIGN(decl)) {
    ...
    if (DECL_INITIAL(decl) != 0 && DECL_INITIAL(decl) != error_mark_node) {
      unsigned int const_align = CONSTANT_ALIGNMENT(DECL_INITIAL(decl), align);
      /* Don't increase alignment too much for TLS variables - TLS space
         is too precious.  */
...
static section *get_variable_section(tree decl, bool prefer_noswitch_p) {
  int reloc;
...
static struct object_block *get_block_for_decl(tree decl) {
  section *sect;
...
void make_decl_rtl(tree decl) {
  ...
  gcc_assert(TREE_CODE(decl) != VAR_DECL || TREE_STATIC(decl) ||
  ...
  if (DECL_RTL_SET_P(decl)) {
    ...
    if (MEM_P(x) && GET_CODE(XEXP(x, 0)) == SYMBOL_REF &&
...
void default_stabs_asm_out_destructor(rtx symbol ATTRIBUTE_UNUSED,
...
section *get_cdtor_priority_section(int priority, bool constructor_p) {
  char buf[16];

  /* ??? This only works reliably with the GNU linker.  */
  sprintf(buf, "%s.%.5u", constructor_p ? ".ctors" : ".dtors",
...
void default_stabs_asm_out_constructor(rtx symbol ATTRIBUTE_UNUSED,
                                       int priority ATTRIBUTE_UNUSED) {
#if defined DBX_DEBUGGING_INFO || defined XCOFF_DEBUGGING_INFO
  /* Tell GNU LD that this is part of the static destructor set.
     This will work for any system that uses stabs, most usefully
...
void assemble_start_function(tree decl, const char *fnname) {
  ...
  if (!DECL_USER_ALIGN(decl) && align_functions_log > align &&
    ...
    ASM_OUTPUT_MAX_SKIP_ALIGN(asm_out_file, align_functions_log,
                              align_functions - 1);
#else
...
void assemble_end_function(tree decl, const char *fnname ATTRIBUTE_UNUSED) {
...
static bool emit_local(tree decl ATTRIBUTE_UNUSED,
...
static bool emit_common(tree decl ATTRIBUTE_UNUSED,
                        const char *name ATTRIBUTE_UNUSED,
                        unsigned HOST_WIDE_INT size ATTRIBUTE_UNUSED,
                        unsigned HOST_WIDE_INT rounded ATTRIBUTE_UNUSED) {
...
static bool emit_tls_common(tree decl ATTRIBUTE_UNUSED,
                            const char *name ATTRIBUTE_UNUSED,
                            unsigned HOST_WIDE_INT size ATTRIBUTE_UNUSED,
                            unsigned HOST_WIDE_INT rounded ATTRIBUTE_UNUSED) {
...
static void assemble_noswitch_variable(tree decl, const char *name,
                                       section *sect) {
  ...
  if (!sect->noswitch.callback(decl, name, size, rounded) &&
    ...
    warning(0,
            "requested alignment for %q+D is greater than "
            "implemented alignment of %wu",
            decl, rounded);
}
...
static void assemble_variable_contents(tree decl, const char *name,
  ...
  if (!dont_output_data) {
    if (DECL_INITIAL(decl) && DECL_INITIAL(decl) != error_mark_node &&
      ...
      output_constant(DECL_INITIAL(decl), tree_low_cst(DECL_SIZE_UNIT(decl), 1),
                      DECL_ALIGN(decl));
    else
      /* Leave space for it.  */
...
void assemble_variable(tree decl, int top_level ATTRIBUTE_UNUSED,
  ...
  if (!targetm.have_tls && TREE_CODE(decl) == VAR_DECL &&
    ...
    if (!DECL_EXTERNAL(to) &&
      ...
      DECL_INITIAL(to) =
          targetm.emutls.var_init(to, decl, get_emutls_init_templ_addr(decl));
...
/* Actually do the tests to determine if this is necessary, and invoke
   ASM_OUTPUT_EXTERNAL.  */
static void assemble_external_real(tree decl) {
  rtx rtl = DECL_RTL(decl);

  if (MEM_P(rtl) && GET_CODE(XEXP(rtl, 0)) == SYMBOL_REF &&
      !SYMBOL_REF_USED(XEXP(rtl, 0)) && !incorporeal_function_p(decl)) {
...
void assemble_external(tree decl ATTRIBUTE_UNUSED) {
...
#ifdef ASM_OUTPUT_EXTERNAL
  pending_assemble_externals = tree_cons(0, decl, pending_assemble_externals);
#endif
}
...
bool default_assemble_integer(rtx x ATTRIBUTE_UNUSED,
...
static GTY((param_is(struct constant_descriptor_tree))) htab_t const_desc_htab;

static struct constant_descriptor_tree *build_constant_desc(tree);
static void maybe_output_constant_def_contents(
    struct constant_descriptor_tree *, int);
...
static hashval_t const_hash_1(const tree exp) {
  const char *p;
  hashval_t hi;
  int len, i;
...
static int const_desc_eq(const void *p1, const void *p2) {
...
static int compare_constant(const tree t1, const tree t2) {
...
static tree copy_constant(tree exp) {
  switch (TREE_CODE(exp)) {
    ...
    case COMPLEX_CST:
      return build_complex(TREE_TYPE(exp), copy_constant(TREE_REALPART(exp)),
    ...
    case CONSTRUCTOR: {
      ...
      v = VEC_alloc(constructor_elt, gc,
                    VEC_length(constructor_elt, CONSTRUCTOR_ELTS(exp)));
...
static section *get_constant_section(tree exp) {
  if (IN_NAMED_SECTION(exp))
    ...
    return targetm.asm_out.select_section(exp, compute_reloc_for_constant(exp),
...
static struct constant_descriptor_tree *build_constant_desc(tree exp) {
  rtx symbol;
  rtx rtl;
  char label[256];
  int labelno;
  ...
  if (use_object_blocks_p()) {
    ...
    symbol =
        create_block_symbol(ggc_strdup(label), get_block_for_section(sect), -1);
  } else
...
static void assemble_constant_contents(tree exp, const char *label,
                                       unsigned int align) {
  HOST_WIDE_INT size;
...
static void output_constant_def_contents(rtx symbol) {
...
rtx lookup_constant_def(tree exp) {
  ...
  key.value = exp;
  key.hash = const_hash_1(exp);
  desc = (struct constant_descriptor_tree *)htab_find_with_hash(const_desc_htab,
                                                                &key, key.hash);
...
static hashval_t const_desc_rtx_hash(const void *ptr) {
  const struct constant_descriptor_rtx *const desc =
      (const struct constant_descriptor_rtx *)ptr;
  return desc->hash;
}

static int const_desc_rtx_eq(const void *a, const void *b) {
...
static int const_rtx_hash_1(rtx *xp, void *data) {
...
static struct rtx_constant_pool *create_constant_pool(void) {
  ...
  pool->const_rtx_htab =
      htab_create_ggc(31, const_desc_rtx_hash, const_desc_rtx_eq, NULL);
  pool->first = NULL;
  pool->last = NULL;
  pool->offset = 0;
  return pool;
...
rtx force_const_mem(enum machine_mode mode, rtx x) {
  ...
  pool = (targetm.use_blocks_for_constant_p(mode, x) ? shared_constant_pool
                                                     : crtl->varasm.pool);
...
enum machine_mode get_pool_mode(const_rtx addr) {
...
static void output_constant_pool_2(enum machine_mode mode, rtx x,
                                   unsigned int align) {
...
static void output_constant_pool_1(struct constant_descriptor_rtx *desc,
  ...
  switch (GET_CODE(tmp)) {
    ...
    case LABEL_REF:
      ...
      gcc_assert(!NOTE_P(tmp) || NOTE_KIND(tmp) != NOTE_INSN_DELETED);
      break;
  ...
  ASM_OUTPUT_SPECIAL_POOL_ENTRY(asm_out_file, x, desc->mode, align,
                                desc->labelno, done);
#endif
  ...
  if (align > GET_MODE_BITSIZE(desc->mode) && in_section &&
...
static int mark_constant(rtx *current_rtx, void *data ATTRIBUTE_UNUSED) {
  rtx x = *current_rtx;
...
static void mark_constants(rtx insn) {
  if (!INSN_P(insn))
    return;
...
static void output_constant_pool(const char *fnname ATTRIBUTE_UNUSED,
...
static tree narrowing_initializer_constant_valid_p(tree value, tree endtype) {
  tree op0, op1;
  ...
  while (CONVERT_EXPR_P(op0) || TREE_CODE(op0) == NON_LVALUE_EXPR) {
  ...
  while (CONVERT_EXPR_P(op1) || TREE_CODE(op1) == NON_LVALUE_EXPR) {
  ...
  if (op0 && op1) {
    if (op0 == op1 &&
        (op0 == null_pointer_node || TREE_CODE(value) == MINUS_EXPR))
      return null_pointer_node;

    /* Support differences between labels.  */
    if (TREE_CODE(op0) == LABEL_DECL && TREE_CODE(op1) == LABEL_DECL)
      return null_pointer_node;

    if (TREE_CODE(op0) == STRING_CST && TREE_CODE(op1) == STRING_CST &&
  ...
  switch (TREE_CODE(value)) {
    ...
    case FDESC_EXPR: {
      ...
      if (op0) {
        ...
        if (TREE_CODE(op0) == FUNCTION_DECL && decl_function_context(op0) &&
            !DECL_NO_STATIC_CHAIN(op0) && !TREE_NO_TRAMPOLINE(value))
          return NULL_TREE;
    ...
    CASE_CONVERT : {
      ...
      /* Likewise conversions from int to pointers, but also allow
         conversions from 0.  */
      if ((POINTER_TYPE_P(dest_type) || TREE_CODE(dest_type) == OFFSET_TYPE) &&
    ...
    case MINUS_EXPR:
      if (!INTEGRAL_TYPE_P(endtype) ||
        ...
        if (valid0 && TREE_CODE(valid0) == STRING_CST && valid1 &&
            TREE_CODE(valid1) == STRING_CST &&
...
void output_constant(tree exp, unsigned HOST_WIDE_INT size,
                     unsigned int align) {
  enum tree_code code;
  ...
  switch (code) {
    ...
    case FIXED_POINT_TYPE:
      if (!assemble_integer(
              expand_expr(exp, NULL_RTX, VOIDmode, EXPAND_INITIALIZER),
    ...
    case VECTOR_TYPE:
      switch (TREE_CODE(exp)) {
        case CONSTRUCTOR:
          ...
          return;
        case STRING_CST:
          thissize = MIN((unsigned HOST_WIDE_INT)TREE_STRING_LENGTH(exp), size);
...
static void output_constructor(tree exp, unsigned HOST_WIDE_INT size,
                               unsigned int align) {
  ...
  /* As LINK goes through the elements of the constant,
     ...
     more one).  */
  for (cnt = 0; VEC_iterate(constructor_elt, CONSTRUCTOR_ELTS(exp), cnt, ce);
    ...
    if (field && flag_verbose_asm)
      fprintf(asm_out_file, "%s %s:\n", ASM_COMMENT_START,
              DECL_NAME(field) ? IDENTIFIER_POINTER(DECL_NAME(field))
                               : "<anonymous>");
#endif
    ...
    if (index && TREE_CODE(index) == RANGE_EXPR) {
      unsigned HOST_WIDE_INT fieldsize = int_size_in_bytes(TREE_TYPE(type));
      ...
      while (next_offset < end_offset) {
        ...
        if (BYTES_BIG_ENDIAN) {
          ...
          byte |=
              (((value >> shift) & (((HOST_WIDE_INT)2 << (this_time - 1)) - 1))
...
static void mark_weak(tree decl) {
  ...
  if (DECL_RTL_SET_P(decl) && MEM_P(DECL_RTL(decl)) &&
...
void merge_weak(tree newdecl, tree olddecl) {
  ...
  if (DECL_WEAK(newdecl)) {
    ...
    if (TREE_ASM_WRITTEN(olddecl))
      error("weak declaration of %q+D must precede definition", newdecl);
    ...
    /* If we've already generated rtl referencing OLDDECL, we may
      ...
      warning(0,
              "weak declaration of %q+D after first use results "
...
void weak_finish(void) {
  ...
  for (t = weakref_targets; t; t = TREE_CHAIN(t)) {
    ...
    if (!TREE_SYMBOL_REFERENCED(DECL_ASSEMBLER_NAME(alias_decl)))
      ...
      if (!decl) {
        decl = build_decl(TREE_CODE(alias_decl), target, TREE_TYPE(alias_decl));
...
static void do_assemble_alias(tree decl, tree target) {
  if (TREE_ASM_WRITTEN(decl))
    return;
...
#else
  ASM_OUTPUT_DEF(asm_out_file, IDENTIFIER_POINTER(DECL_ASSEMBLER_NAME(decl)),
                 IDENTIFIER_POINTER(target));
#endif
  ...
  for (i = 0; VEC_iterate(alias_pair, alias_pairs, i, p); i++) {
    ...
    if (target_decl == NULL) {
      if (!lookup_attribute("weakref", DECL_ATTRIBUTES(p->decl)))
        error("%q+D aliased to undefined symbol %qs", p->decl,
              IDENTIFIER_POINTER(p->target));
    } else if (DECL_EXTERNAL(target_decl) &&
               !lookup_attribute("weakref", DECL_ATTRIBUTES(p->decl)))
      error("%q+D aliased to external symbol %qs", p->decl,
            IDENTIFIER_POINTER(p->target));
  }
}
...
void default_assemble_visibility(tree decl, int vis) {
  static const char *const visibility_types[] = {NULL, "protected", "hidden",
                                                 "internal"};
...
int maybe_assemble_visibility(tree decl) {
...
void init_varasm_once(void) {
  section_htab =
      htab_create_ggc(31, section_entry_hash, section_entry_eq, NULL);
  object_block_htab =
      htab_create_ggc(31, object_block_entry_hash, object_block_entry_eq, NULL);
  const_desc_htab = htab_create_ggc(1009, const_desc_hash, const_desc_eq, NULL);
...
#ifdef CTORS_SECTION_ASM_OP
  ctors_section =
      get_unnamed_section(0, output_section_asm_op, CTORS_SECTION_ASM_OP);
#endif

#ifdef DTORS_SECTION_ASM_OP
  dtors_section =
      get_unnamed_section(0, output_section_asm_op, DTORS_SECTION_ASM_OP);
#endif

#ifdef BSS_SECTION_ASM_OP
  bss_section = get_unnamed_section(SECTION_WRITE | SECTION_BSS,
                                    output_section_asm_op, BSS_SECTION_ASM_OP);
#endif

#ifdef SBSS_SECTION_ASM_OP
  sbss_section = get_unnamed_section(
      SECTION_WRITE | SECTION_BSS, output_section_asm_op, SBSS_SECTION_ASM_OP);
#endif

  tls_comm_section = get_noswitch_section(
      SECTION_WRITE | SECTION_BSS | SECTION_COMMON, emit_tls_common);
  lcomm_section = get_noswitch_section(
      SECTION_WRITE | SECTION_BSS | SECTION_COMMON, emit_local);
  comm_section = get_noswitch_section(
      SECTION_WRITE | SECTION_BSS | SECTION_COMMON, emit_common);

#if defined ASM_OUTPUT_ALIGNED_BSS || defined ASM_OUTPUT_BSS
  bss_noswitch_section =
      get_noswitch_section(SECTION_WRITE | SECTION_BSS, emit_bss);
#endif
...
unsigned int default_section_type_flags(tree decl, const char *name,
                                        int reloc) {
  unsigned int flags;
  ...
  if (decl && TREE_CODE(decl) == FUNCTION_DECL)
    ...
    flags = 0;
  else if (current_function_decl && cfun &&
           crtl->subsections.unlikely_text_section_name &&
           strcmp(name, crtl->subsections.unlikely_text_section_name) == 0)
    flags = SECTION_CODE;
  else if (!decl && (!current_function_decl || !cfun) &&
  ...
  if (strcmp(name, ".tdata") == 0 || strncmp(name, ".tdata.", 7) == 0 ||
      strncmp(name, ".gnu.linkonce.td.", 17) == 0)
    flags |= SECTION_TLS;

  if (strcmp(name, ".tbss") == 0 || strncmp(name, ".tbss.", 6) == 0 ||
...
void default_no_named_section(const char *name ATTRIBUTE_UNUSED,
                              unsigned int flags ATTRIBUTE_UNUSED,
...
void default_elf_asm_named_section(const char *name, unsigned int flags,
                                   tree decl ATTRIBUTE_UNUSED) {
  ...
  *f = '\0';
  ...
  if (!(flags & SECTION_NOTYPE)) {
    ...
    if (HAVE_COMDAT_GROUP && (flags & SECTION_LINKONCE))
      fprintf(asm_out_file, ",%s,comdat", lang_hooks.decls.comdat_group(decl));
  }

  putc('\n', asm_out_file);
}

void default_coff_asm_named_section(const char *name, unsigned int flags,
                                    tree decl ATTRIBUTE_UNUSED) {
...
void default_pe_asm_named_section(const char *name, unsigned int flags,
                                  tree decl) {
...
section *default_select_section(tree decl, int reloc,
...
enum section_category categorize_decl_for_section(const_tree decl, int reloc) {
  ...
  if (TREE_CODE(decl) == FUNCTION_DECL)
    ...
    if ((reloc & targetm.asm_out.reloc_rw_mask()) || TREE_SIDE_EFFECTS(decl) ||
        !TREE_CONSTANT(decl))
      ret = SECCAT_DATA;
    else
      ret = SECCAT_RODATA;
  ...
  if (TREE_CODE(decl) == VAR_DECL && DECL_TLS_MODEL(decl)) {
    ...
    else if (ret == SECCAT_BSS || (flag_zero_initialized_in_bss &&
                                   initializer_zerop(DECL_INITIAL(decl))))
      ret = SECCAT_TBSS;
    else
      ret = SECCAT_TDATA;
...
section *default_elf_select_section(tree decl, int reloc,
                                    unsigned HOST_WIDE_INT align) {
  const char *sname;
  switch (categorize_decl_for_section(decl, reloc)) {
    case SECCAT_TEXT:
...
static int compute_reloc_for_rtx_1(rtx *xp, void *data) {
  int *preloc = (int *)data;
  rtx x = *xp;
...
static int compute_reloc_for_rtx(rtx x) {
  int reloc;
...
section *default_select_rtx_section(
...
section *default_elf_select_rtx_section(enum machine_mode mode, rtx x,
                                        unsigned HOST_WIDE_INT align) {
...
void default_encode_section_info(tree decl, rtx rtl,
...
void default_asm_output_anchor(rtx symbol) {
  char buffer[100];
...
bool default_binds_local_p_1(const_tree exp, int shlib) {
  ...
  /* Uninitialized COMMON variable may be unified with symbols
     resolved from other modules.  */
           ...
           (DECL_INITIAL(exp) == NULL || DECL_INITIAL(exp) == error_mark_node))
    local_p = false;
...
bool default_valid_pointer_mode(enum machine_mode mode) {
...
void default_emit_unwind_label(FILE *stream ATTRIBUTE_UNUSED,
                               tree decl ATTRIBUTE_UNUSED,
...
void default_emit_except_table_label(FILE *stream ATTRIBUTE_UNUSED) {}

/* This is how to output an internal numbered label where PREFIX is
   the class of label and LABELNO is the number within the class.  */

void default_internal_label(FILE *stream, const char *prefix,
...
void output_section_asm_op(const void *directive) {
...
void switch_to_section(section *new_section) {
  ...
  switch (SECTION_STYLE(new_section)) {
    case SECTION_NAMED:
      if (cfun && !crtl->subsections.unlikely_text_section_name &&
...
rtx get_section_anchor(struct object_block *block, HOST_WIDE_INT offset,
...
static int output_object_block_htab(void **slot, void *data ATTRIBUTE_UNUSED) {
...
int elf_record_gcc_switches(print_switch_type type, const char *name) {
...
void default_elf_asm_output_external(FILE *file ATTRIBUTE_UNUSED, tree decl,
                                     const char *name ATTRIBUTE_UNUSED) {
