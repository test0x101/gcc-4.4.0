...
static tree tree_may_unswitch_on(basic_block bb, struct loop *loop) {
  gimple stmt, def;
  tree cond, use;
  basic_block def_bb;
  ssa_op_iter iter;
...
static tree simplify_using_entry_checks(struct loop *loop, tree cond) {
  ...
  while (1) {
    ...
    if (stmt && gimple_code(stmt) == GIMPLE_COND &&
        ...
        operand_equal_p(gimple_cond_lhs(stmt), TREE_OPERAND(cond, 0), 0) &&
        operand_equal_p(gimple_cond_rhs(stmt), TREE_OPERAND(cond, 1), 0))
      return (e->flags & EDGE_TRUE_VALUE ? boolean_true_node
                                         : boolean_false_node);
...
static bool tree_unswitch_single_loop(struct loop *loop, int num) {
  basic_block *bbs;
  struct loop *nloop;
  unsigned i;
  tree cond = NULL_TREE;
...
static struct loop *tree_unswitch_loop(struct loop *loop,
  ...
  return loop_version(loop, unshare_expr(cond), NULL, prob_true, prob_true,
