  ...
  if ((TREE_CODE(new_tree) == VAR_DECL || TREE_CODE(new_tree) == PARM_DECL) &&
    ...
    if (gimple_nop_p(SSA_NAME_DEF_STMT(name))) {
      ...
      if (id->entry_bb && is_gimple_reg(SSA_NAME_VAR(name)) &&
        ...
        init_stmt = gimple_build_assign(
            new_tree, fold_convert(TREE_TYPE(new_tree), integer_zero_node));
    ...
    if (cfun && gimple_in_ssa_p(cfun) &&
      ...
      if (TREE_CODE(decl) != PARM_DECL && def) {
        ...
        /* Watch out RESULT_DECLs whose SSA names map directly
           to them.  */
        if (TREE_CODE(map) == SSA_NAME && gimple_nop_p(SSA_NAME_DEF_STMT(map)))
...
static tree remapped_type(tree type, copy_body_data *id) {
  tree *node;
...
static bool can_be_nonlocal(tree decl, copy_body_data *id) {
  ...
  /* Local static vars must be non-local or we get multiple declaration
     problems.  */
  if (TREE_CODE(decl) == VAR_DECL && !auto_var_in_fn_p(decl, id->src_fn))
    return true;
...
static tree remap_decls(tree decls, VEC(tree, gc) * *nonlocalized_list,
                        copy_body_data *id) {
  tree old_var;
  ...
  for (old_var = decls; old_var; old_var = TREE_CHAIN(old_var)) {
    ...
    if (can_be_nonlocal(old_var, id)) {
      if (TREE_CODE(old_var) == VAR_DECL &&
        ...
        cfun->local_decls = tree_cons(NULL_TREE, old_var, cfun->local_decls);
      if (debug_info_level > DINFO_LEVEL_TERSE && !DECL_IGNORED_P(old_var) &&
          nonlocalized_list)
...
static void remap_block(tree *block, copy_body_data *id) {
  tree old_block;
  tree new_block;
  tree fn;
  ...
  *block = new_block;
  ...
  BLOCK_VARS(new_block) = remap_decls(BLOCK_VARS(old_block),
                                      &BLOCK_NONLOCALIZED_VARS(new_block), id);
...
static void copy_bind_expr(tree *tp, int *walk_subtrees, copy_body_data *id) {
...
gimple_seq remap_gimple_seq(gimple_seq body, copy_body_data *id) {
...
static gimple copy_gimple_bind(gimple stmt, copy_body_data *id) {
  gimple new_bind;
  ...
  if (TREE_CODE(*tp) == SSA_NAME) {
    ...
    *walk_subtrees = 0;
    ...
    gcc_unreachable();
           ...
           (!DECL_CONTEXT(*tp) || decl_function_context(*tp) == id->src_fn))
...
tree copy_tree_body_r(tree *tp, int *walk_subtrees, void *data) {
  ...
  /* Otherwise, just copy the node.  Note that copy_tree_r already
    ...
    if (EXPR_P(*tp)) {
      ...
      if (TREE_BLOCK(*tp)) {
        tree *n;
        n = (tree *)pointer_map_contains(id->decl_map, TREE_BLOCK(*tp));
        gcc_assert(n);
        new_block = *n;
      }
    ...
    if (TREE_CODE(*tp) == RESX_EXPR && id->eh_region_offset)
      TREE_OPERAND(*tp, 0) =
          build_int_cst(NULL_TREE, id->eh_region_offset +
...
static gimple remap_gimple_stmt(gimple stmt, copy_body_data *id) {
  gimple copy = NULL;
  ...
  if (gimple_code(stmt) == GIMPLE_RETURN && id->transform_return_to_modify) {
    ...
    switch (gimple_code(stmt)) {
      ...
      case GIMPLE_OMP_PARALLEL:
        ...
        copy = gimple_build_omp_parallel(s1, gimple_omp_parallel_clauses(stmt),
      ...
      case GIMPLE_OMP_TASK:
        ...
        copy = gimple_build_omp_task(
            s1, gimple_omp_task_clauses(stmt), gimple_omp_task_child_fn(stmt),
            gimple_omp_task_data_arg(stmt), gimple_omp_task_copy_fn(stmt),
            gimple_omp_task_arg_size(stmt), gimple_omp_task_arg_align(stmt));
        break;
      ...
      case GIMPLE_OMP_FOR:
                                    ...
                                    gimple_omp_for_collapse(stmt), s2);
        {
          ...
          for (i = 0; i < gimple_omp_for_collapse(stmt); i++) {
            ...
            gimple_omp_for_set_final(copy, i, gimple_omp_for_final(stmt, i));
            gimple_omp_for_set_incr(copy, i, gimple_omp_for_incr(stmt, i));
      ...
      case GIMPLE_OMP_SECTIONS:
        s1 = remap_gimple_seq(gimple_omp_body(stmt), id);
        copy = gimple_build_omp_sections(s1, gimple_omp_sections_clauses(stmt));
        break;

      case GIMPLE_OMP_SINGLE:
        s1 = remap_gimple_seq(gimple_omp_body(stmt), id);
        copy = gimple_build_omp_single(s1, gimple_omp_single_clauses(stmt));
        break;

      case GIMPLE_OMP_CRITICAL:
        s1 = remap_gimple_seq(gimple_omp_body(stmt), id);
        copy = gimple_build_omp_critical(s1, gimple_omp_critical_name(stmt));
        break;
  ...
  if (gimple_code(stmt) == GIMPLE_RESX && id->eh_region_offset) {
    gimple_resx_set_region(copy,
                           gimple_resx_region(stmt) + id->eh_region_offset);
  }
  return copy;
}
...
static basic_block copy_bb(copy_body_data *id, basic_block bb,
                           int frequency_scale, gcov_type count_scale) {
  ...
  copy_basic_block =
      create_basic_block(NULL, (void *)0, (basic_block)bb->prev_bb->aux);
  copy_basic_block->count = bb->count * count_scale / REG_BR_PROB_BASE;

  /* We are going to rebuild frequencies from scratch.  These values
     have just small importance to drive canonicalize_loop_headers.  */
  copy_basic_block->frequency =
      ((gcov_type)bb->frequency * frequency_scale / REG_BR_PROB_BASE);
  ...
  for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    if (is_gimple_assign(stmt) && gimple_assign_rhs_code(stmt) == NOP_EXPR &&
        !is_gimple_val(gimple_assign_rhs1(stmt))) {
      tree new_rhs;
      new_rhs = force_gimple_operand_gsi(&seq_gsi, gimple_assign_rhs1(stmt),
    ...
    do {
      ...
      if (is_gimple_call(stmt) && gimple_call_va_arg_pack_p(stmt) &&
          id->gimple_call) {
        ...
        memcpy(VEC_address(tree, argarray), gimple_call_arg_ptr(stmt, 0),
        ...
        new_call = gimple_build_call_vec(gimple_call_fn(stmt), argarray);
        ...
        stmt = new_call;
      } else if (is_gimple_call(stmt) && id->gimple_call &&
      ...
      if (is_gimple_call(stmt)) {
        ...
        switch (id->transform_call_graph_edges) {
          case CB_CGE_DUPLICATE:
            ...
            if (edge)
              cgraph_clone_edge(edge, id->dst_node, stmt, REG_BR_PROB_BASE, 1,
                                edge->frequency, true);
            break;

          case CB_CGE_MOVE_CLONES:
            for (node = id->dst_node->next_clone; node;
      ...
      if (stmt_could_throw_p(stmt)
        ...
        if (lookup_stmt_eh_region_fn(id->src_cfun, orig_stmt) <= 0 &&
            id->eh_region > 0 && stmt_could_throw_p(stmt))
...
static void update_ssa_across_abnormal_edges(basic_block bb, basic_block ret_bb,
                                             bool can_throw,
                                             bool nonlocal_goto) {
  edge e;
  edge_iterator ei;
  ...
  if (!e->dest->aux || ((basic_block)e->dest->aux)->index == ENTRY_BLOCK) {
    gimple phi;
    gimple_stmt_iterator si;
...
static void copy_edges_for_bb(basic_block bb, gcov_type count_scale,
                              basic_block ret_bb) {
  ...
  for (si = gsi_start_bb(new_bb); !gsi_end_p(si);) {
    ...
    if ((can_throw || nonlocal_goto) && gimple_in_ssa_p(cfun))
      update_ssa_across_abnormal_edges(gimple_bb(copy_stmt), ret_bb, can_throw,
                                       nonlocal_goto);
  }
}
    ...
    if (is_gimple_reg(res)) {
      ...
      FOR_EACH_EDGE(new_edge, ei, new_bb->preds) {
        ...
        if (TREE_CODE(new_arg) != SSA_NAME &&
            TREE_CODE(new_arg) != FUNCTION_DECL && !is_gimple_val(new_arg)) {
...
static void initialize_cfun(tree new_fndecl, tree callee_fndecl,
  ...
  if (src_cfun->ipa_transforms_to_apply)
    cfun->ipa_transforms_to_apply =
        VEC_copy(ipa_opt_pass, heap, src_cfun->ipa_transforms_to_apply);
  ...
  ENTRY_BLOCK_PTR->count = (ENTRY_BLOCK_PTR_FOR_FUNCTION(src_cfun)->count *
                            count_scale / REG_BR_PROB_BASE);
  ENTRY_BLOCK_PTR->frequency =
      (ENTRY_BLOCK_PTR_FOR_FUNCTION(src_cfun)->frequency * frequency_scale /
       REG_BR_PROB_BASE);
  EXIT_BLOCK_PTR->count = (EXIT_BLOCK_PTR_FOR_FUNCTION(src_cfun)->count *
                           count_scale / REG_BR_PROB_BASE);
  EXIT_BLOCK_PTR->frequency =
      (EXIT_BLOCK_PTR_FOR_FUNCTION(src_cfun)->frequency * frequency_scale /
       REG_BR_PROB_BASE);
  if (src_cfun->eh)
...
static tree copy_cfg_body(copy_body_data *id, gcov_type count, int frequency,
  ...
  if (cfun->eh) {
    id->eh_region_offset =
        duplicate_eh_regions(cfun_to_copy, remap_decl_1, id, 0, id->eh_region);
  }
...
static tree copy_body(copy_body_data *id, gcov_type count, int frequency,
...
static void insert_init_stmt(basic_block bb, gimple init_stmt) {
  ...
  if (init_stmt) {
    ...
    if (!is_gimple_reg(gimple_assign_lhs(init_stmt)) &&
      ...
      tree rhs =
          build1(gimple_assign_rhs_code(init_stmt), gimple_expr_type(init_stmt),
...
/* Initialize parameter P with VALUE.  If needed, produce init statement
   ...
   output later.  */
static gimple setup_one_parameter(copy_body_data *id, tree p, tree value,
                                  tree fn, basic_block bb, tree *vars) {
  gimple init_stmt = NULL;
  tree var;
  tree rhs = value;
  tree def =
      (gimple_in_ssa_p(cfun) ? gimple_default_def(id->src_cfun, p) : NULL);
  ...
  if (TREE_READONLY(p) && !TREE_ADDRESSABLE(p) && value &&
      !TREE_SIDE_EFFECTS(value) && !def) {
  ...
  *vars = var;
  ...
  if (gimple_in_ssa_p(cfun) && rhs && def && is_gimple_reg(p) &&
      (optimize || (TREE_READONLY(p) && is_gimple_min_invariant(rhs))) &&
...
static void initialize_inlined_parameters(copy_body_data *id, gimple stmt,
...
static tree declare_return_variable(copy_body_data *id, tree return_slot,
  ...
  if (return_slot) {
    ...
    if (DECL_BY_REFERENCE(result)) {
      ...
      if (gimple_in_ssa_p(cfun)) {
        ...
        int volatilep;
        tree base;
        base = get_inner_reference(return_slot, &bitsize, &bitpos, &offset,
    ...
    if ((TREE_CODE(TREE_TYPE(result)) == COMPLEX_TYPE ||
        ...
        !DECL_GIMPLE_REG_P(result) && DECL_P(var))
  ...
  if (modify_dest && TREE_CODE(modify_dest) != SSA_NAME) {
    bool use_it = false;
    ...
    /* If the callee cannot possibly modify MODIFY_DEST, then we can
      ...
      if (!DECL_P(base_m))
        ...
        use_it = false;
                ...
                TREE_CODE(TREE_TYPE(result)) == VECTOR_TYPE) &&
               !DECL_GIMPLE_REG_P(result) && DECL_GIMPLE_REG_P(base_m))
        use_it = false;
  ...
  DECL_STRUCT_FUNCTION(caller)->local_decls =
      tree_cons(NULL_TREE, var, DECL_STRUCT_FUNCTION(caller)->local_decls);
...
static tree inline_forbidden_p_op(tree *nodep,
...
static tree inline_forbidden_p_2(tree *nodep, int *walk_subtrees, void *fnp) {
  tree node = *nodep;
  tree fn = (tree)fnp;
...
static bool inline_forbidden_p(tree fndecl) {
  ...
  FOR_EACH_BB_FN(bb, fun) {
    ...
    ret = walk_gimple_seq(seq, inline_forbidden_p_stmt, inline_forbidden_p_op,
                          &wi);
...
static bool inlinable_function_p(tree fn) {
  bool inlinable = true;
  bool do_warning;
  tree always_inline;
  ...
  do_warning = (warn_inline && DECL_DECLARED_INLINE_P(fn) &&
                !DECL_NO_INLINE_WARNING_P(fn) && !DECL_IN_SYSTEM_HEADER(fn));

  always_inline = lookup_attribute("always_inline", DECL_ATTRIBUTES(fn));

  if (flag_no_inline && always_inline == NULL) {
    if (do_warning)
      warning(OPT_Winline,
  ...
  /* Don't auto-inline anything that might not be bound within
  ...
  else if (!DECL_DECLARED_INLINE_P(fn) && DECL_REPLACEABLE_P(fn))
    inlinable = false;
    ...
    if (do_warning)
      warning(OPT_Winline,
              "function %q+F can never be inlined because it "
              "uses attributes conflicting with inlining",
              fn);
    inlinable = false;
  }
...
static int estimate_operator_cost(enum tree_code code, eni_weights *weights) {
  switch (code) {
...
int count_insns_seq(gimple_seq seq, eni_weights *weights) {
  gimple_stmt_iterator gsi;
  int n = 0;
...
static tree get_indirect_callee_fndecl(struct cgraph_node *node, gimple stmt) {
...
static bool expand_call_inline(basic_block bb, gimple stmt,
                               copy_body_data *id) {
  ...
  /* If FN is a declaration of a function in a nested scope that was
     ...
     gimple_body.  */
  if (!DECL_INITIAL(fn) && DECL_ABSTRACT_ORIGIN(fn) &&
  ...
  if (!cgraph_inline_p(cg_edge, &reason)) {
    ...
    if (lookup_attribute("always_inline", DECL_ATTRIBUTES(fn))
      ...
      sorry("called from here");
               ...
               !DECL_IN_SYSTEM_HEADER(fn) && strlen(reason) &&
               !lookup_attribute("noinline", DECL_ATTRIBUTES(fn))
               /* Avoid warnings during early inline pass. */
               && cgraph_global_info_ready) {
      warning(OPT_Winline, "inlining failed in call to %q+F: %s", fn, reason);
  ...
  if (lookup_attribute("cold", DECL_ATTRIBUTES(fn))) {
    gimple_stmt_iterator si = gsi_last_bb(bb);
    gsi_insert_after(&si, gimple_build_predict(PRED_COLD_FUNCTION, NOT_TAKEN),
                     GSI_NEW_STMT);
  }
  ...
  for (; t_step; t_step = TREE_CHAIN(t_step)) {
    ...
    if (TREE_STATIC(var) && !TREE_ASM_WRITTEN(var)) {
      ...
      cfun->local_decls =
          tree_cons(NULL_TREE, remap_decl(var, id), cfun->local_decls);
  }
  ...
  if (use_retvar && gimple_call_lhs(stmt)) {
    ...
    if (gimple_call_lhs(stmt) && TREE_CODE(gimple_call_lhs(stmt)) == SSA_NAME) {
...
static bool gimple_expand_calls_inline(basic_block bb, copy_body_data *id) {
  ...
  for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    if (is_gimple_call(stmt) && expand_call_inline(bb, stmt, id))
      return true;
  }
...
static void fold_marked_statements(int first,
                                   struct pointer_set_t *statements) {
  for (; first < n_basic_blocks; first++)
    if (BASIC_BLOCK(first)) {
      gimple_stmt_iterator gsi;

      for (gsi = gsi_start_bb(BASIC_BLOCK(first)); !gsi_end_p(gsi);
           gsi_next(&gsi))
...
static inline bool has_abnormal_outgoing_edge_p(basic_block bb) {
  edge e;
  edge_iterator ei;
...
tree copy_tree_r(tree *tp, int *walk_subtrees, void *data ATTRIBUTE_UNUSED) {
  ...
  if (IS_EXPR_CODE_CLASS(cl) || code == TREE_LIST || code == TREE_VEC ||
      code == TYPE_DECL || code == OMP_CLAUSE) {
    ...
    *tp = new_tree;
    ...
    if (code == PARM_DECL || code == TREE_LIST || code == OMP_CLAUSE)
      TREE_CHAIN(*tp) = chain;
    ...
    CONSTRUCTOR_ELTS(new_tree) =
        VEC_copy(constructor_elt, gc, CONSTRUCTOR_ELTS(*tp));
    *tp = new_tree;
...
static void remap_save_expr(tree *tp, void *st_, int *walk_subtrees) {
...
static tree unsave_r(tree *tp, int *walk_subtrees, void *data) {
  copy_body_data *id = (copy_body_data *)data;
...
static tree replace_locals_op(tree *tp, int *walk_subtrees, void *data) {
  ...
  /* Only a local declaration (variable or label).  */
  if ((TREE_CODE(expr) == VAR_DECL && !TREE_STATIC(expr)) ||
    ...
    *walk_subtrees = 0;
             ...
             TREE_CODE(expr) == BIND_EXPR || TREE_CODE(expr) == SAVE_EXPR)
    gcc_unreachable();
...
static tree debug_find_tree_1(tree *tp, int *walk_subtrees ATTRIBUTE_UNUSED,
                              void *data) {
  if (*tp == data)
    return (tree)data;
  else
...
static void declare_inline_vars(tree block, tree vars) {
  tree t;
...
static tree copy_decl_to_var(tree decl, copy_body_data *id) {
  tree copy, type;
...
static tree copy_result_decl_to_var(tree decl, copy_body_data *id) {
  tree copy, type;
...
void tree_function_versioning(tree old_decl, tree new_decl,
                              varray_type tree_map, bool update_clones,
                              bitmap args_to_skip) {
  ...
  if (p)
    DECL_STRUCT_FUNCTION(new_decl)->static_chain_decl = copy_static_chain(
        DECL_STRUCT_FUNCTION(old_decl)->static_chain_decl, &id);
  ...
  if (tree_map)
    for (i = 0; i < VARRAY_ACTIVE_SIZE(tree_map); i++) {
      ...
      if (replace_info->replace_p) {
        ...
        init =
            setup_one_parameter(&id, replace_info->old_tree,
                                replace_info->new_tree, id.src_fn, NULL, &vars);
        if (init)
  ...
  if (DECL_ARGUMENTS(old_decl) != NULL_TREE)
    DECL_ARGUMENTS(new_decl) = copy_arguments_for_versioning(
        DECL_ARGUMENTS(old_decl), &id, args_to_skip, &vars);
  ...
  if (DECL_STRUCT_FUNCTION(old_decl)->local_decls != NULL_TREE)
    /* Add local vars.  */
    for (t_step = DECL_STRUCT_FUNCTION(old_decl)->local_decls; t_step;
...
bool tree_can_inline_p(tree caller, tree callee) {
#if 0
