...
extern void omega_constrain_variable_value(omega_pb, enum omega_eqn_color, int,
                                           int);
extern bool omega_query_variable(omega_pb, int, int *, int *);
extern int omega_query_variable_signs(omega_pb, int, int, int, int, int, int,
                                      bool *, int *);
...
static inline bool omega_wildcard_p(omega_pb pb, int i) {
  return (pb->var[i] < 0);
}

/* Return true when variable I in problem PB is a safe variable.  */

static inline bool omega_safe_var_p(omega_pb pb, int i) {
...
static inline void omega_print_eq(FILE *file, omega_pb pb, eqn e) {
  omega_print_eqn(file, pb, e, false, 0);
}

/* Print to FILE inequality E from PB.  */

static inline void omega_print_geq(FILE *file, omega_pb pb, eqn e) {
  omega_print_eqn(file, pb, e, true, 0);
}

/* Print to FILE inequality E from PB.  */

static inline void omega_print_geq_extra(FILE *file, omega_pb pb, eqn e) {
  omega_print_eqn(file, pb, e, true, 1);
}

/* E1 = E2, make a copy of E2 into E1.  Equations contain S variables.  */

static inline void omega_copy_eqn(eqn e1, eqn e2, int s) {
  e1->key = e2->key;
...
static inline void omega_init_eqn_zero(eqn e, int s) {
  e->key = 0;
  e->touched = 0;
  e->color = omega_black;
...
static inline eqn omega_alloc_eqns(int s, int n) {
  int i;
...
static inline void omega_free_eqns(eqn eq, int n) {
  int i;
...
static inline bool single_var_geq(eqn e, int nv ATTRIBUTE_UNUSED) {
  return (e->key != 0 && -OMEGA_MAX_VARS <= e->key && e->key <= OMEGA_MAX_VARS);
}

/* Allocate a new equality with all coefficients 0, and tagged with
   COLOR.  Return the index of this equality in problem PB.  */

static inline int omega_add_zero_eq(omega_pb pb, enum omega_eqn_color color) {
  int idx = pb->num_eqs++;
...
static inline int omega_add_zero_geq(omega_pb pb, enum omega_eqn_color color) {
  int idx = pb->num_geqs;
...
static inline void omega_initialize_variables(omega_pb pb) {
  int i;
...
static inline void omega_free_problem(omega_pb pb) {
  free(pb->var);
...
static inline void omega_copy_problem(omega_pb p1, omega_pb p2) {
  int e, i;
