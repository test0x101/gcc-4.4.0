...
static histogram_value gimple_alloc_histogram_value(
    struct function *fun ATTRIBUTE_UNUSED, enum hist_type type, gimple stmt,
    tree value) {
...
static void set_histogram_value(struct function *fun, gimple stmt,
                                histogram_value hist) {
  void **loc;
  if (!hist && !VALUE_HISTOGRAMS(fun))
    return;
  if (!VALUE_HISTOGRAMS(fun))
    VALUE_HISTOGRAMS(fun) = htab_create(1, histogram_hash, histogram_eq, NULL);
...
void gimple_add_histogram_value(struct function *fun, gimple stmt,
                                histogram_value hist) {
...
void gimple_remove_histogram_value(struct function *fun, gimple stmt,
                                   histogram_value hist) {
  ...
  switch (hist->type) {
    case HIST_TYPE_INTERVAL:
      fprintf(dump_file, "Interval counter range %d -- %d",
              ...
              (hist->hdata.intvl.int_start + hist->hdata.intvl.steps - 1));
...
void dump_histograms_for_stmt(struct function *fun, FILE *dump_file,
                              gimple stmt) {
  histogram_value hist;
  for (hist = gimple_histogram_value(fun, stmt); hist; hist = hist->hvalue.next)
...
void gimple_duplicate_stmt_histograms(struct function *fun, gimple stmt,
                                      struct function *ofun, gimple ostmt) {
  histogram_value val;
  for (val = gimple_histogram_value(ofun, ostmt); val != NULL;
       val = val->hvalue.next) {
    histogram_value new_val =
        gimple_alloc_histogram_value(fun, val->type, NULL, NULL);
    memcpy(new_val, val, sizeof(*val));
    new_val->hvalue.stmt = stmt;
    new_val->hvalue.counters = XNEWVAR(
        gcov_type, sizeof(*new_val->hvalue.counters) * new_val->n_counters);
    memcpy(new_val->hvalue.counters, val->hvalue.counters,
           sizeof(*new_val->hvalue.counters) * new_val->n_counters);
...
void gimple_move_stmt_histograms(struct function *fun, gimple stmt,
                                 gimple ostmt) {
  ...
  for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    for (hist = gimple_histogram_value(cfun, stmt); hist;
      ...
      if (hist->hvalue.stmt != stmt) {
        error(
            "Histogram value statement does not correspond to "
...
static int free_hist(void **slot, void *data ATTRIBUTE_UNUSED) {
  histogram_value hist = *(histogram_value *)slot;
  free(hist->hvalue.counters);
#ifdef ENABLE_CHECKING
  memset(hist, 0xab, sizeof(*hist));
...
static bool check_counter(gimple stmt, const char *name, gcov_type *count,
                          gcov_type *all, gcov_type bb_count) {
  if (*all != bb_count || *count > *all) {
    ...
    if (flag_profile_correction) {
      ...
      *all = bb_count;
      ...
      error(
          "%HCorrupted value profile: %s profiler overall count (%d) "
          "does not match BB count (%d)",
...
static bool gimple_value_profile_transformations(void) {
  basic_block bb;
  gimple_stmt_iterator gsi;
  bool changed = false;
    ...
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (flag_value_profile_transformations &&
          (gimple_mod_subtract_transform(&gsi) ||
           ...
           gimple_stringops_transform(&gsi) || gimple_ic_transform(stmt))) {
        stmt = gsi_stmt(gsi);
        changed = true;
...
static tree gimple_divmod_fixed_value(gimple stmt, tree value, int prob,
  ...
  stmt1 = gimple_build_assign_with_ops(gimple_assign_rhs_code(stmt), tmp2, op1,
                                       tmpv);
  gsi_insert_before(&gsi, stmt1, GSI_SAME_STMT);
  bb2end = stmt1;
...
static bool gimple_divmod_fixed_value_transform(gimple_stmt_iterator *si) {
  ...
  histogram =
      gimple_histogram_value_of_type(cfun, stmt, HIST_TYPE_SINGLE_VALUE);
  if (!histogram)
    return false;
  ...
  if (simple_cst_equal(gimple_assign_rhs2(stmt), value) != 1 ||
      2 * count < all || optimize_bb_for_size_p(gimple_bb(stmt)))
    return false;
  ...
  tree_val = build_int_cst_wide(get_gcov_type(), (unsigned HOST_WIDE_INT)val,
...
static tree gimple_mod_pow2(gimple stmt, int prob, gcov_type count,
                            gcov_type all) {
  ...
  stmt4 = gimple_build_cond(NE_EXPR, tmp3, build_int_cst(optype, 0), NULL_TREE,
                            NULL_TREE);
  gsi_insert_before(&gsi, stmt2, GSI_SAME_STMT);
  gsi_insert_before(&gsi, stmt3, GSI_SAME_STMT);
...
static bool gimple_mod_pow2_value_transform(gimple_stmt_iterator *si) {
  histogram_value histogram;
  enum tree_code code;
  ...
  if (simple_cst_equal(gimple_assign_rhs2(stmt), value) != 1 ||
      count < wrong_values || optimize_bb_for_size_p(gimple_bb(stmt)))
    return false;
...
/* Generate code for transformations 3 and 4 (with parent gimple assign STMT,
   and NCOUNTS the number of cases to support.  Currently only NCOUNTS==0 or 1
   is supported and this is built into this interface.  The probabilities of
   taking the optimal paths are PROB1 and PROB2, which are equivalent to
   COUNT1/ALL and COUNT2/ALL respectively within roundoff error).  This
   generates the result into a temp and returns the temp; it does not replace or
   alter the original STMT.  */
/* FIXME: Generalize the interface to handle NCOUNTS > 1.  */

static tree gimple_mod_subtract(gimple stmt, int prob1, int prob2, int ncounts,
                                gcov_type count1, gcov_type count2,
                                gcov_type all) {
  gimple stmt1, stmt2, stmt3;
  tree tmp1;
...
static bool gimple_mod_subtract_transform(gimple_stmt_iterator *si) {
  histogram_value histogram;
  enum tree_code code;
  gcov_type count, wrong_values, all;
  tree lhs_type, result, value;
  ...
  if (i == steps || optimize_bb_for_size_p(gimple_bb(stmt)))
    return false;
...
static void init_pid_map(void) {
  struct cgraph_node *n;

  if (pid_map != NULL)
    return;

  pid_map = (struct cgraph_node **)xmalloc(sizeof(struct cgraph_node *) *
                                           cgraph_max_pid);
...
static inline struct cgraph_node *find_func_by_pid(int pid) {
  init_pid_map();
...
static gimple gimple_ic(gimple stmt, gimple call,
                        struct cgraph_node *direct_call, int prob,
                        gcov_type count, gcov_type all) {
  gimple stmt1, stmt2, stmt3;
  tree tmp1, tmpv, tmp;
  gimple bb1end, bb2end, bb3end;
...
static bool gimple_ic_transform(gimple stmt) {
  histogram_value histogram;
...
static bool interesting_stringop_to_profile_p(tree fndecl, gimple call) {
...
*/
static void gimple_stringop_fixed_value(gimple stmt, tree value, int prob,
                                        gcov_type count, gcov_type all) {
  gimple stmt1, stmt2, stmt3;
  tree tmp1, tmpv;
...
static bool gimple_stringops_transform(gimple_stmt_iterator *gsi) {
  gimple stmt = gsi_stmt(*gsi);
  tree fndecl;
  tree blck_size;
  ...
  switch (fcode) {
    ...
    case BUILT_IN_MEMSET:
      if (!can_store_by_pieces(val, builtin_memset_read_str,
                               gimple_call_arg(stmt, 1), dest_align, true))
        return false;
      break;
    case BUILT_IN_BZERO:
      if (!can_store_by_pieces(val, builtin_memset_read_str, integer_zero_node,
                               dest_align, true))
        return false;
      break;
    default:
  ...
  tree_val = build_int_cst_wide(get_gcov_type(), (unsigned HOST_WIDE_INT)val,
                                val >> (HOST_BITS_PER_WIDE_INT - 1) >> 1);
  if (dump_file) {
    fprintf(dump_file, "Single value %i stringop transformation on ", (int)val);
    print_gimple_stmt(dump_file, stmt, 0, TDF_SLIM);
  }
...
void stringop_block_profile(gimple stmt, unsigned int *expected_align,
  ...
  if (!histogram)
    ...
    *expected_size = -1;
    ...
    gcov_type size;
    size =
        ((histogram->hvalue.counters[0] + histogram->hvalue.counters[1] / 2) /
  ...
  if (!histogram)
    ...
    *expected_align = 0;
    ...
    alignment = 1;
    while (!(count & alignment) && (alignment * 2 * BITS_PER_UNIT))
      alignment <<= 1;
  ...
  switch (gimple_assign_rhs_code(stmt)) {
    ...
    case TRUNC_MOD_EXPR:
      ...
      if (is_gimple_reg(divisor))
        ...
        VEC_quick_push(histogram_value, *values,
                       gimple_alloc_histogram_value(
                           cfun, HIST_TYPE_SINGLE_VALUE, stmt, divisor));
      ...
      if (gimple_assign_rhs_code(stmt) == TRUNC_MOD_EXPR &&
        ...
        VEC_quick_push(
            histogram_value, *values,
            gimple_alloc_histogram_value(cfun, HIST_TYPE_POW2, stmt, divisor));

        val = build2(TRUNC_DIV_EXPR, type, op0, divisor);
        hist =
            gimple_alloc_histogram_value(cfun, HIST_TYPE_INTERVAL, stmt, val);
...
static void gimple_indirect_call_to_profile(gimple stmt,
  ...
  if (gimple_code(stmt) != GIMPLE_CALL || gimple_call_fndecl(stmt) != NULL_TREE)
    return;
  ...
  VEC_quick_push(
      histogram_value, *values,
      gimple_alloc_histogram_value(cfun, HIST_TYPE_INDIR_CALL, stmt, callee));
...
/* Find values inside STMT for that we want to measure histograms for
   string operations.  */
static void gimple_stringops_values_to_profile(gimple stmt,
                                               histogram_values *values) {
  tree fndecl;
  tree blck_size;
  tree dest;
  ...
  if (TREE_CODE(blck_size) != INTEGER_CST) {
    VEC_safe_push(histogram_value, heap, *values,
                  gimple_alloc_histogram_value(cfun, HIST_TYPE_SINGLE_VALUE,
                                               stmt, blck_size));
    VEC_safe_push(
        histogram_value, heap, *values,
        gimple_alloc_histogram_value(cfun, HIST_TYPE_AVERAGE, stmt, blck_size));
  }
  if (TREE_CODE(blck_size) != INTEGER_CST)
    VEC_safe_push(
        histogram_value, heap, *values,
        gimple_alloc_histogram_value(cfun, HIST_TYPE_IOR, stmt, dest));
}
...
static void gimple_find_values_to_profile(histogram_values *values) {
  basic_block bb;
  gimple_stmt_iterator gsi;
  unsigned i;
...
static struct value_prof_hooks gimple_value_prof_hooks = {
    gimple_find_values_to_profile, gimple_value_profile_transformations};

void gimple_register_value_prof_hooks(void) {
  gcc_assert(current_ir_type() == IR_GIMPLE);
  value_prof_hooks = &gimple_value_prof_hooks;
}

/* IR-independent entry points.  */
void find_values_to_profile(histogram_values *values) {
