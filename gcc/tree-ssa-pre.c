...
static int pre_expr_eq(const void *p1, const void *p2) {
...
static hashval_t pre_expr_hash(const void *p1) {
...
static inline unsigned int alloc_expression_id(pre_expr expr) {
  void **slot;
...
static inline unsigned int get_expression_id(const pre_expr expr) {
  return expr->id;
}

static inline unsigned int lookup_expression_id(const pre_expr expr) {
  void **slot;
...
static inline unsigned int get_or_alloc_expression_id(pre_expr expr) {
...
static inline pre_expr expression_for_id(unsigned int id) {
...
static void clear_expression_ids(void) {
...
static hashval_t expr_pred_trans_hash(const void *p) {
...
static int expr_pred_trans_eq(const void *p1, const void *p2) {
...
static inline pre_expr phi_trans_lookup(pre_expr e, basic_block pred) {
  void **slot;
...
static inline void phi_trans_add(pre_expr e, pre_expr v, basic_block pred) {
  void **slot;
  expr_pred_trans_t new_pair = XNEW(struct expr_pred_trans_d);
  new_pair->e = e;
  new_pair->pred = pred;
  new_pair->v = v;
  new_pair->hashcode = iterative_hash_hashval_t(pre_expr_hash(e), pred->index);
...
void add_to_value(unsigned int v, pre_expr e) {
  bitmap_set_t set;

  gcc_assert(get_expr_value_id(e) == v);

  if (v >= VEC_length(bitmap_set_t, value_expressions)) {
    VEC_safe_grow_cleared(bitmap_set_t, heap, value_expressions, v + 1);
  }
...
static unsigned int get_expr_value_id(pre_expr expr) {
  switch (expr->kind) {
    case CONSTANT: {
      unsigned int id;
...
static void bitmap_remove_from_set(bitmap_set_t set, pre_expr expr) {
...
static void bitmap_insert_into_set_1(bitmap_set_t set, pre_expr expr,
...
static void bitmap_insert_into_set(bitmap_set_t set, pre_expr expr) {
  bitmap_insert_into_set_1(set, expr, false);
}

/* Copy a bitmapped set ORIG, into bitmapped set DEST.  */

static void bitmap_set_copy(bitmap_set_t dest, bitmap_set_t orig) {
...
static VEC(pre_expr, heap) * sorted_array_from_bitmap_set(bitmap_set_t set) {
  unsigned int i, j;
...
static void bitmap_set_and(bitmap_set_t dest, bitmap_set_t orig) {
  bitmap_iterator bi;
  unsigned int i;
...
static bitmap_set_t bitmap_set_subtract(bitmap_set_t dest, bitmap_set_t orig) {
  bitmap_set_t result = bitmap_set_new();
  bitmap_iterator bi;
  unsigned int i;

  bitmap_and_compl(result->expressions, dest->expressions, orig->expressions);
...
static bool bitmap_set_contains_value(bitmap_set_t set, unsigned int value_id) {
...
static inline bool bitmap_set_contains_expr(bitmap_set_t set,
                                            const pre_expr expr) {
...
static void bitmap_set_replace_value(bitmap_set_t set, unsigned int lookfor,
                                     const pre_expr expr) {
...
static void bitmap_value_replace_in_set(bitmap_set_t set, pre_expr expr) {
  unsigned int val = get_expr_value_id(expr);
...
static void bitmap_value_insert_into_set(bitmap_set_t set, pre_expr expr) {
  unsigned int val = get_expr_value_id(expr);
...
void debug_pre_expr(pre_expr e) {
...
static void print_bitmap_set(FILE *outfile, bitmap_set_t set,
...
void print_value_expressions(FILE *outfile, unsigned int val) {
...
void debug_value_expressions(unsigned int val) {
...
static tree get_constant_for_value_id(unsigned int v) {
...
static pre_expr get_or_alloc_expr_for(tree t) {
...
static pre_expr fully_constant_expression(pre_expr e) {
  switch (e->kind) {
      ...
      switch (TREE_CODE_CLASS(nary->opcode)) {
        case tcc_expression:
          if (nary->opcode == TRUTH_NOT_EXPR)
            goto do_unary;
          if (nary->opcode != TRUTH_AND_EXPR && nary->opcode != TRUTH_OR_EXPR &&
        ...
        case tcc_comparison: {
          ...
          result = fold_binary(nary->opcode, nary->type, naryop0, naryop1);
        ...
        case tcc_reference:
          if (nary->opcode != REALPART_EXPR && nary->opcode != IMAGPART_EXPR &&
    ...
    case REFERENCE: {
      ...
      if (op->opcode == CALL_EXPR && TREE_CODE(op->op0) == ADDR_EXPR &&
        ...
        if (TREE_CODE_CLASS(arg0->opcode) == tcc_constant ||
            (arg0->opcode == ADDR_EXPR && is_gimple_min_invariant(arg0->op0)))
          anyconst = true;
        if (arg1 &&
            (TREE_CODE_CLASS(arg1->opcode) == tcc_constant ||
             (arg1->opcode == ADDR_EXPR && is_gimple_min_invariant(arg1->op0))))
          anyconst = true;
        if (anyconst) {
          tree folded = build_call_expr(TREE_OPERAND(op->op0, 0), arg1 ? 2 : 1,
                                        arg0->op0, arg1 ? arg1->op0 : NULL);
          if (folded && TREE_CODE(folded) == NOP_EXPR)
...
static VEC(tree, gc) * translate_vuses_through_block(VEC(tree, gc) * vuses,
  ...
  for (i = 0; VEC_iterate(tree, vuses, i, oldvuse); i++) {
    ...
    if (gimple_code(phi) == GIMPLE_PHI && gimple_bb(phi) == phiblock) {
...
static inline pre_expr find_leader_in_sets(unsigned int val, bitmap_set_t set1,
                                           bitmap_set_t set2) {
  pre_expr result;
...
static tree get_expr_type(const pre_expr e) {
  switch (e->kind) {
    ...
    case REFERENCE: {
      ...
      vro = VEC_index(vn_reference_op_s, PRE_EXPR_REFERENCE(e)->operands, 0);
...
static tree get_representative_for(const pre_expr e) {
  ...
  switch (e->kind) {
    ...
    case REFERENCE: {
      ...
      unsigned int i;
      bitmap_iterator bi;
      bitmap_set_t exprs = VEC_index(bitmap_set_t, value_expressions, value_id);
...
static pre_expr phi_translate_1(pre_expr expr, bitmap_set_t set1,
                                bitmap_set_t set2, basic_block pred,
                                basic_block phiblock, bitmap seen) {
  ...
  switch (expr->kind) {
    ...
    case NARY: {
      ...
      memcpy(&newnary, nary,
             (sizeof(struct vn_nary_op_s) - sizeof(tree) * (4 - nary->length)));
      ...
      for (i = 0; i < newnary.length; i++) {
        if (TREE_CODE(newnary.op[i]) != SSA_NAME)
          ...
          pre_expr result =
              phi_translate_1(leader, set1, set2, pred, phiblock, seen);
      ...
      if (changed) {
        ...
        tree result = vn_nary_op_lookup_pieces(
            newnary.length, newnary.opcode, newnary.type, newnary.op[0],
            newnary.op[1], newnary.op[2], newnary.op[3], &nary);
        ...
        if (nary) {
          ...
          VEC_safe_grow_cleared(bitmap_set_t, heap, value_expressions,
                                get_max_value_id() + 1);
          nary = vn_nary_op_insert_pieces(
              newnary.length, newnary.opcode, newnary.type, newnary.op[0],
              newnary.op[1], newnary.op[2], newnary.op[3], result, new_val_id);
          PRE_EXPR_NARY(expr) = nary;
          constant = fully_constant_expression(expr);
          if (constant != expr)
            return constant;
    ...
    case REFERENCE: {
      ...
      if (changed) {
        ...
        tree result =
            vn_reference_lookup_pieces(newvuses, newoperands, &newref, true);
        if (newref)
          VEC_free(vn_reference_op_s, heap, newoperands);
        ...
        if (newref) {
          ...
          VEC_safe_grow_cleared(bitmap_set_t, heap, value_expressions,
                                get_max_value_id() + 1);
          newref = vn_reference_insert_pieces(newvuses, newoperands, result,
                                              new_val_id);
          newoperands = NULL;
          PRE_EXPR_REFERENCE(expr) = newref;
          constant = fully_constant_expression(expr);
...
static pre_expr phi_translate(pre_expr expr, bitmap_set_t set1,
                              bitmap_set_t set2, basic_block pred,
                              basic_block phiblock) {
...
static void phi_translate_set(bitmap_set_t dest, bitmap_set_t set,
...
static pre_expr bitmap_find_leader(bitmap_set_t set, unsigned int val,
                                   gimple stmt) {
  if (value_id_constant_p(val)) {
    unsigned int i;
    bitmap_iterator bi;
  ...
  if (bitmap_set_contains_value(set, val)) {
    ...
    EXECUTE_IF_AND_IN_BITMAP(exprset->expressions, set->expressions, 0, i, bi) {
 ...
 */
static bool vro_valid_in_sets(bitmap_set_t set1, bitmap_set_t set2,
                              vn_reference_op_t vro) {
  if (vro->op0 && TREE_CODE(vro->op0) == SSA_NAME) {
    ...
    if (temp.id == 0)
      return false;
    if (!union_contains_value(set1, set2, get_expr_value_id(&temp)))
      return false;
  }
...
static void dependent_clean(bitmap_set_t set1, bitmap_set_t set2,
                            basic_block block) {
...
static void clean(bitmap_set_t set, basic_block block) {
...
static bool defer_or_phi_translate_block(bitmap_set_t dest, bitmap_set_t source,
                                         basic_block block,
                                         basic_block phiblock) {
...
static bool compute_antic_aux(basic_block block,
   ...
   */
    ...
    if (!defer_or_phi_translate_block(ANTIC_OUT, ANTIC_IN(succ_bb), block,
                                      succ_bb)) {
      changed = true;
      goto maybe_dump_sets;
    }
  ...
  ANTIC_IN(block) = bitmap_set_subtract(EXP_GEN(block), TMP_GEN(block));

  /* Then union in the ANTIC_OUT - TMP_GEN values,
     to get ANTIC_OUT U EXP_GEN - TMP_GEN */
  FOR_EACH_EXPR_ID_IN_SET(S, bii, bi)
  bitmap_value_insert_into_set(ANTIC_IN(block), expression_for_id(bii));
...
maybe_dump_sets:
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    if (!BB_DEFERRED(block) || BB_VISITED(block)) {
      ...
      fprintf(dump_file, "Block %d was deferred for a future iteration.\n",
              block->index);
    }
  }
  if (old)
  ...
  /* If we have multiple successors, we take the union of all of
    ...
    if (VEC_length(basic_block, worklist) > 0) {
      for (i = 0; VEC_iterate(basic_block, worklist, i, bprime); i++) {
        ...
        bitmap_value_insert_into_set(PA_OUT, expression_for_id(i));
        if (phi_nodes(bprime)) {
...
static void compute_antic(void) {
  ...
  while (changed) {
    ...
    for (i = 0; i < n_basic_blocks - NUM_FIXED_BLOCKS; i++) {
      if (TEST_BIT(changed_blocks, postorder[i])) {
        ...
        changed |= compute_antic_aux(
            block, TEST_BIT(has_abnormal_preds, block->index));
      }
    }
  ...
  statistics_histogram_event(cfun, "compute_antic iterations", num_iterations);
...
static bool can_value_number_call(gimple stmt) {
...
static bool is_exception_related(gimple stmt) {
...
static bool can_PRE_operation(tree op) {
  return UNARY_CLASS_P(op) || BINARY_CLASS_P(op) || COMPARISON_CLASS_P(op) ||
         TREE_CODE(op) == INDIRECT_REF || TREE_CODE(op) == COMPONENT_REF ||
         TREE_CODE(op) == VIEW_CONVERT_EXPR || TREE_CODE(op) == CALL_EXPR ||
...
static tree create_component_ref_by_pieces_1(basic_block block,
                                             ...
                                             gimple_seq *stmts,
                                             gimple domstmt) {
  vn_reference_op_t currop =
      VEC_index(vn_reference_op_s, ref->operands, *operand);
  tree genop;
  ++*operand;
  switch (currop->opcode) {
    case CALL_EXPR: {
      ...
      tree *args =
          XNEWVEC(tree, VEC_length(vn_reference_op_s, ref->operands) - 1);
      while (*operand < VEC_length(vn_reference_op_s, ref->operands)) {
        args[nargs] = create_component_ref_by_pieces_1(block, ref, operand,
                                                       stmts, domstmt);
        nargs++;
      }
    ...
    case VIEW_CONVERT_EXPR: {
      tree folded;
      tree genop0 =
          create_component_ref_by_pieces_1(block, ref, operand, stmts, domstmt);
      if (!genop0)
        return NULL_TREE;
      folded = fold_build1(currop->opcode, currop->type, genop0);
      return folded;
    } break;
    ...
    case INDIRECT_REF: {
      ...
      if (!genop1)
        return NULL_TREE;
      genop1 = fold_convert(build_pointer_type(currop->type), genop1);

      if (currop->opcode == MISALIGNED_INDIRECT_REF)
        folded = fold_build2(currop->opcode, currop->type, genop1, currop->op1);
      else
        folded = fold_build1(currop->opcode, currop->type, genop1);
      return folded;
    } break;
    case BIT_FIELD_REF: {
      tree folded;
      ...
      if (!genop2)
        return NULL_TREE;
      folded = fold_build3(BIT_FIELD_REF, currop->type, genop0, genop1, genop2);
      return folded;
    }
    ...
    case ARRAY_REF: {
      ...
      genop3 = currop->op2;
      return build4(currop->opcode, currop->type, genop0, genop1, genop2,
                    genop3);
    }
    case COMPONENT_REF: {
      tree op0;
...
static tree find_or_generate_expression(basic_block block, pre_expr expr,
                                        gimple_seq *stmts, gimple domstmt) {
  pre_expr leader =
      bitmap_find_leader(AVAIL_OUT(block), get_expr_value_id(expr), domstmt);
  tree genop = NULL;
  if (leader) {
    if (leader->kind == NAME)
  ...
  if (genop == NULL && !in_fre) {
    ...
    FOR_EACH_EXPR_ID_IN_SET(exprset, i, bi) {
      ...
      if (temp->kind != NAME) {
        handled = true;
        genop = create_expression_by_pieces(block, temp, stmts, domstmt,
                                            get_expr_type(expr));
        break;
      }
    }
...
static tree create_expression_by_pieces(basic_block block, pre_expr expr,
                                        gimple_seq *stmts, gimple domstmt,
                                        tree type) {
  tree temp, name;
  tree folded, newexpr;
    ...
    case NARY: {
      ...
      switch (nary->length) {
        case 2: {
          ...
          folded = fold_build2(nary->opcode, nary->type, genop1, genop2);
        } break;
        case 1: {
          pre_expr op1 = get_or_alloc_expr_for(nary->op[0]);
          tree genop1 = find_or_generate_expression(block, op1, stmts, domstmt);
          ...
          folded = fold_build1(nary->opcode, nary->type, genop1);
        } break;
        default:
          return NULL_TREE;
      }
  ...
  newexpr =
      force_gimple_operand(unshare_expr(folded), &forced_stmts, false, NULL);
  ...
  if (TREE_CODE(exprtype) == COMPLEX_TYPE || TREE_CODE(exprtype) == VECTOR_TYPE)
...
static bool insert_into_preds_of_block(basic_block block, unsigned int exprnum,
                                       pre_expr *avail) {
  ...
  if (block->loop_depth > 0 && EDGE_COUNT(block->preds) == 2 &&
    ...
    firstinsideloop =
        flow_bb_inside_loop_p(block->loop_father, EDGE_PRED(block, 0)->src);
    secondinsideloop =
        flow_bb_inside_loop_p(block->loop_father, EDGE_PRED(block, 1)->src);
    /* Induction variables only have one edge inside the loop.  */
    if (firstinsideloop ^ secondinsideloop) {
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf(dump_file,
                "Skipping insertion of phi for partial redundancy: Looks like "
  ...
  FOR_EACH_EDGE(pred, ei, block->preds) {
    ...
    if (eprime->kind == NARY && vn_nary_may_trap(PRE_EXPR_NARY(eprime)))
      return false;
  }
  ...
  FOR_EACH_EDGE(pred, ei, block->preds) {
    ...
    if (eprime->kind != NAME && eprime->kind != CONSTANT) {
      builtexpr =
          create_expression_by_pieces(bprime, eprime, &stmts, NULL, type);
      ...
      */
      ...
      if (!useless_type_conversion_p(type, TREE_TYPE(constant))) {
        ...
        if (!is_gimple_min_invariant(builtexpr)) {
          tree forcedexpr = force_gimple_operand(builtexpr, &stmts, true, NULL);
      ...
      if (!useless_type_conversion_p(type, TREE_TYPE(name))) {
        ...
        forcedexpr = force_gimple_operand(builtexpr, &stmts, true, NULL);
  ...
  */
  ...
  bitmap_value_replace_in_set(AVAIL_OUT(block), newphi);
...
static bool do_regular_insertion(basic_block block, basic_block dom) {
  ...
  for (i = 0; VEC_iterate(pre_expr, exprs, i, expr); i++) {
    if (expr->kind != NAME) {
      ...
      FOR_EACH_EDGE(pred, ei, block->preds) {
        ...
        bprime = pred->src;
        eprime = phi_translate(expr, ANTIC_IN(block), NULL, bprime, block);
        ...
        edoubleprime = bitmap_find_leader(AVAIL_OUT(bprime), vprime, NULL);
      ...
      if (!cant_insert && !all_same && by_some && dbg_cnt(treepre_insert)) {
        if (insert_into_preds_of_block(block, get_expression_id(expr), avail))
          new_stuff = true;
      }
      /* If all edges produce the same value and that value is
         ...
         edges.  Note this.  */
               ...
               (edoubleprime->kind == CONSTANT || edoubleprime->kind == NAME) &&
...
static bool do_partial_partial_insertion(basic_block block, basic_block dom) {
  bool new_stuff = false;
  ...
  for (i = 0; VEC_iterate(pre_expr, exprs, i, expr); i++) {
    if (expr->kind != NAME) {
      ...
      FOR_EACH_EDGE(pred, ei, block->preds) {
        ...
        bprime = pred->src;
        eprime =
            phi_translate(expr, ANTIC_IN(block), PA_IN(block), bprime, block);
...
static bool insert_aux(basic_block block) {
  basic_block son;
  bool new_stuff = false;
  ...
  for (son = first_dom_son(CDI_DOMINATORS, block); son;
...
static void add_to_exp_gen(basic_block block, tree op) {
  if (!in_fre) {
    pre_expr result;
...
static void make_values_for_phi(gimple phi, basic_block block) {
...
static void compute_avail(void) {
  ...
  for (son = first_dom_son(CDI_DOMINATORS, ENTRY_BLOCK_PTR); son;
       son = next_dom_son(CDI_DOMINATORS, son))
    worklist[sp++] = son;
  ...
  while (sp) {
    ...
    for (gsi = gsi_start_bb(block); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (gimple_has_volatile_ops(stmt) || stmt_could_throw_p(stmt))
        continue;
      ...
      switch (gimple_code(stmt)) {
        ...
        case GIMPLE_CALL: {
          ...
          vn_reference_lookup_pieces(shared_vuses_from_stmt(stmt), ops, &ref,
                                     false);
          VEC_free(vn_reference_op_s, heap, ops);
          if (!ref)
            continue;

          for (i = 0; VEC_iterate(vn_reference_op_s, ref->operands, i, vro);
               i++) {
            if (vro->op0 && TREE_CODE(vro->op0) == SSA_NAME)
          ...
          switch (TREE_CODE_CLASS(gimple_assign_rhs_code(stmt))) {
            ...
            case tcc_comparison: {
              ...
              vn_nary_op_lookup_pieces(
                  gimple_num_ops(stmt) - 1, gimple_assign_rhs_code(stmt),
                  gimple_expr_type(stmt), gimple_assign_rhs1(stmt),
                  gimple_assign_rhs2(stmt), NULL_TREE, NULL_TREE, &nary);
            ...
            case tcc_reference: {
              ...
              vn_reference_lookup(gimple_assign_rhs1(stmt),
                                  shared_vuses_from_stmt(stmt), false, &ref);
              if (!ref)
                continue;
...
static tree do_SCCVN_insertion(gimple stmt, tree ssa_vn) {
...
static unsigned int eliminate(void) {
  basic_block b;
  unsigned int todo = 0;
  ...
  FOR_EACH_BB(b) {
    ...
    for (i = gsi_start_bb(b); !gsi_end_p(i); gsi_next(&i)) {
      ...
      if (gimple_has_lhs(stmt) && TREE_CODE(gimple_get_lhs(stmt)) == SSA_NAME &&
        ...
        sprimeexpr =
            bitmap_find_leader(AVAIL_OUT(b), get_expr_value_id(lhsexpr), NULL);
        ...
        if (!sprime && is_gimple_min_invariant(VN_INFO(lhs)->valnum)) {
          sprime = fold_convert(TREE_TYPE(lhs), VN_INFO(lhs)->valnum);
      ...
      /* Visit COND_EXPRs and fold the comparison with the
        ...
        result =
            fold_binary(gimple_cond_code(stmt), boolean_type_node, op0, op1);
...
static inline gimple mark_operand_necessary(tree op) {
  gimple stmt;
  ...
  if (gimple_plf(stmt, NECESSARY) || gimple_nop_p(stmt))
    return NULL;
...
static void remove_dead_inserted_code(void) {
...
static void init_pre(bool do_fre) {
  basic_block bb;
  ...
  phi_translate_table =
      htab_create(5110, expr_pred_trans_hash, expr_pred_trans_eq, free);
  expression_to_id =
      htab_create(num_ssa_names * 3, pre_expr_hash, pre_expr_eq, NULL);
  seen_during_translate = BITMAP_ALLOC(&grand_bitmap_obstack);
  bitmap_set_pool =
      create_alloc_pool("Bitmap sets", sizeof(struct bitmap_set), 30);
  pre_expr_pool =
      create_alloc_pool("pre_expr nodes", sizeof(struct pre_expr_d), 30);
  FOR_ALL_BB(bb) {
...
static void fini_pre(bool do_fre) {
  basic_block bb;
...
static unsigned int execute_pre(bool do_fre ATTRIBUTE_UNUSED) {
  unsigned int todo = 0;
...
static unsigned int execute_fre(void) {
  return execute_pre(true);
}

static bool gate_fre(void) {
  return flag_tree_fre != 0;
}

struct gimple_opt_pass pass_fre = {{
    GIMPLE_PASS, "fre",               /* name */
    gate_fre,                         /* gate */
    execute_fre,                      /* execute */
    NULL,                             /* sub */
    NULL,                             /* next */
