...
void dump_iv_info(FILE *file, struct rtx_iv *iv) {
  if (!iv->base) {
  ...
  if (iv->mode != iv->extend_mode)
    fprintf(file, " %s to %s", rtx_name[iv->extend],
...
rtx lowpart_subreg(enum machine_mode outer_mode, rtx expr,
...
static enum iv_grd_result iv_get_reaching_def(rtx insn, rtx reg, df_ref *def) {
  df_ref use, adef;
...
static bool iv_constant(struct rtx_iv *iv, rtx cst, enum machine_mode mode) {
  if (mode == VOIDmode)
    mode = GET_MODE(cst);
...
static bool iv_subreg(struct rtx_iv *iv, enum machine_mode mode) {
  ...
  iv->base = simplify_gen_binary(
      PLUS, iv->extend_mode, iv->delta,
      simplify_gen_binary(MULT, iv->extend_mode, iv->base, iv->mult));
...
static bool iv_extend(struct rtx_iv *iv, enum rtx_code extend,
  ...
  if (iv->extend != UNKNOWN && iv->extend != extend)
    return false;
...
static bool iv_neg(struct rtx_iv *iv) {
  if (iv->extend == UNKNOWN) {
    iv->base =
        simplify_gen_unary(NEG, iv->extend_mode, iv->base, iv->extend_mode);
    iv->step =
        simplify_gen_unary(NEG, iv->extend_mode, iv->step, iv->extend_mode);
  } else {
    iv->delta =
        simplify_gen_unary(NEG, iv->extend_mode, iv->delta, iv->extend_mode);
    iv->mult =
        simplify_gen_unary(NEG, iv->extend_mode, iv->mult, iv->extend_mode);
  }
...
static bool iv_add(struct rtx_iv *iv0, struct rtx_iv *iv1, enum rtx_code op) {
  enum machine_mode mode;
  rtx arg;

  /* Extend the constant to extend_mode of the other operand if necessary.  */
  if (iv0->extend == UNKNOWN && iv0->mode == iv0->extend_mode &&
      iv0->step == const0_rtx &&
      GET_MODE_SIZE(iv0->extend_mode) < GET_MODE_SIZE(iv1->extend_mode)) {
    iv0->extend_mode = iv1->extend_mode;
    iv0->base =
        simplify_gen_unary(ZERO_EXTEND, iv0->extend_mode, iv0->base, iv0->mode);
  }
  if (iv1->extend == UNKNOWN && iv1->mode == iv1->extend_mode &&
      iv1->step == const0_rtx &&
      GET_MODE_SIZE(iv1->extend_mode) < GET_MODE_SIZE(iv0->extend_mode)) {
    iv1->extend_mode = iv0->extend_mode;
    iv1->base =
        simplify_gen_unary(ZERO_EXTEND, iv1->extend_mode, iv1->base, iv1->mode);
  }
  ...
  if (iv1->extend == UNKNOWN && iv1->mode == mode && iv1->step == const0_rtx) {
    iv0->delta = simplify_gen_binary(op, mode, iv0->delta, iv1->base);
    return true;
  }

  if (iv0->extend == UNKNOWN && iv0->mode == mode && iv0->step == const0_rtx) {
    arg = iv0->base;
    *iv0 = *iv1;
    if (op == MINUS && !iv_neg(iv0))
      return false;
...
static bool iv_mult(struct rtx_iv *iv, rtx mby) {
...
static bool iv_shift(struct rtx_iv *iv, rtx mby) {
  enum machine_mode mode = iv->extend_mode;
...
static bool get_biv_step_1(df_ref def, rtx reg, rtx *inner_step,
  ...
  } else if (!get_biv_step_1(next_def, reg, inner_step, inner_mode, extend,
  ...
  if (GET_CODE(next) == SUBREG) {
    ...
    *inner_mode = amode;
    *inner_step =
        simplify_gen_binary(PLUS, outer_mode, *inner_step, *outer_step);
    *outer_step = const0_rtx;
    *extend = UNKNOWN;
  }
  ...
  switch (code) {
    ...
    case MINUS:
      if (*inner_mode == outer_mode
        ...
        *inner_step = simplify_gen_binary(code, outer_mode, *inner_step, op1);
      else
        *outer_step = simplify_gen_binary(code, outer_mode, *outer_step, op1);
      break;
...
static bool get_biv_step(df_ref last_def, rtx reg, rtx *inner_step,
                         enum machine_mode *inner_mode, enum rtx_code *extend,
                         enum machine_mode *outer_mode, rtx *outer_step) {
  *outer_mode = GET_MODE(reg);

  if (!get_biv_step_1(last_def, reg, inner_step, inner_mode, extend,
                      *outer_mode, outer_step))
    return false;
...
static void record_iv(df_ref def, struct rtx_iv *iv) {
...
static bool analyzed_for_bivness_p(rtx def, struct rtx_iv *iv) {
...
static void record_biv(rtx def, struct rtx_iv *iv) {
...
static bool iv_analyze_biv(rtx def, struct rtx_iv *iv) {
...
bool iv_analyze_expr(rtx insn, rtx rhs, enum machine_mode mode,
                     struct rtx_iv *iv) {
  ...
  if (op0 && !iv_analyze_expr(insn, op0, omode, &iv0))
    return false;

  if (op1 && !iv_analyze_expr(insn, op1, omode, &iv1))
    return false;
...
static bool iv_analyze_def(df_ref def, struct rtx_iv *iv) {
  rtx insn = DF_REF_INSN(def);
...
static bool iv_analyze_op(rtx insn, rtx op, struct rtx_iv *iv) {
  df_ref def = NULL;
  enum iv_grd_result res;
...
bool iv_analyze(rtx insn, rtx val, struct rtx_iv *iv) {
  rtx reg;
...
bool iv_analyze_result(rtx insn, rtx def, struct rtx_iv *iv) {
  df_ref adef;
...
rtx get_iv_value(struct rtx_iv *iv, rtx iteration) {
  rtx val;
  ...
  if (iv->step != const0_rtx && iteration != const0_rtx)
    val = simplify_gen_binary(
        PLUS, iv->extend_mode, iv->base,
        simplify_gen_binary(MULT, iv->extend_mode, iv->step, iteration));
  else
    val = iv->base;
  ...
  val = simplify_gen_binary(
      PLUS, iv->extend_mode, iv->delta,
      simplify_gen_binary(MULT, iv->extend_mode, iv->mult, val));
...
static unsigned HOST_WIDEST_INT inverse(unsigned HOST_WIDEST_INT x, int mod) {
...
static int altered_reg_used(rtx *reg, void *alt) {
  if (!REG_P(*reg))
    return 0;
...
static bool simple_rhs_p(rtx rhs) {
  ...
  if (CONSTANT_P(rhs) || (REG_P(rhs) && !HARD_REGISTER_P(rhs)))
    return true;
...
static void simplify_using_assignment(rtx insn, rtx *expr, regset altered) {
  ...
  if (set) {
    lhs = SET_DEST(set);
    if (!REG_P(lhs) || altered_reg_used(&lhs, altered))
      ret = true;
  } else
    ret = true;
...
static bool implies_p(rtx a, rtx b) {
  ...
  if ((GET_CODE(a) == GT || GET_CODE(a) == LT) &&
    ...
    if (SCALAR_INT_MODE_P(mode) && rtx_equal_p(op1, opb1) &&
  ...
  if (GET_CODE(b) == NE && (GET_CODE(a) == GT || GET_CODE(a) == GTU ||
  ...
  if (GET_CODE(a) == NE && op1 == const0_rtx) {
    if ((GET_CODE(b) == GTU && opb1 == const0_rtx) ||
        (GET_CODE(b) == GEU && opb1 == const1_rtx))
      return rtx_equal_p(op0, opb0);
  }

  /* A != N is equivalent to A - (N + 1) <u -1.  */
  if (GET_CODE(a) == NE && GET_CODE(op1) == CONST_INT && GET_CODE(b) == LTU &&
      opb1 == constm1_rtx && GET_CODE(opb0) == PLUS &&
  ...
  if (GET_CODE(a) == NE && GET_CODE(op1) == CONST_INT) {
    if (GET_CODE(b) == GTU && GET_CODE(opb0) == PLUS && opb1 == const0_rtx &&
        GET_CODE(XEXP(opb0, 1)) == CONST_INT
        /* Avoid overflows.  */
        && ((unsigned HOST_WIDE_INT)INTVAL(XEXP(opb0, 1)) !=
    ...
    if (GET_CODE(b) == GEU && GET_CODE(opb0) == PLUS && opb1 == const1_rtx &&
        GET_CODE(XEXP(opb0, 1)) == CONST_INT
        /* Avoid overflows.  */
        && ((unsigned HOST_WIDE_INT)INTVAL(XEXP(opb0, 1)) !=
            ((unsigned HOST_WIDE_INT)1 << (HOST_BITS_PER_WIDE_INT - 1))) &&
  ...
  if ((GET_CODE(a) == GT || GET_CODE(a) == GE) && GET_CODE(op1) == CONST_INT &&
      ((GET_CODE(a) == GT && op1 == constm1_rtx) || INTVAL(op1) >= 0) &&
      GET_CODE(b) == LTU && GET_CODE(opb1) == CONST_INT &&
...
rtx canon_condition(rtx cond) {
  ...
  if (GET_CODE(op1) == CONST_INT && GET_MODE_CLASS(mode) != MODE_CC &&
  ...
  if (op0 != XEXP(cond, 0) || op1 != XEXP(cond, 1) || code != GET_CODE(cond) ||
...
void simplify_using_condition(rtx cond, rtx *expr, regset altered) {
  ...
  if (altered && for_each_rtx(&cond, altered_reg_used, altered))
    return;
...
static void eliminate_implied_conditions(enum rtx_code op, rtx *head,
                                         rtx tail) {
  rtx elt;
...
static void simplify_using_initial_values(struct loop *loop, enum rtx_code op,
                                          rtx *expr) {
  rtx head, tail, insn;
  rtx neutral, aggr;
  regset altered;
  ...
  while (1) {
    ...
    if (!single_pred_p(e->src) || single_pred(e->src) == ENTRY_BLOCK_PTR)
      break;
...
static void shorten_into_mode(struct rtx_iv *iv, enum machine_mode mode,
                              enum rtx_code cond, bool signed_p,
                              struct niter_desc *desc) {
  rtx mmin, mmax, cond_over, cond_under;

  get_mode_bounds(mode, signed_p, iv->extend_mode, &mmin, &mmax);
  cond_under =
      simplify_gen_relational(LT, SImode, iv->extend_mode, iv->base, mmin);
  cond_over =
      simplify_gen_relational(GT, SImode, iv->extend_mode, iv->base, mmax);
...
static bool canonicalize_iv_subregs(struct rtx_iv *iv0, struct rtx_iv *iv1,
                                    enum rtx_code cond,
                                    struct niter_desc *desc) {
  ...
  switch (cond) {
    case LE:
    case LT:
      if (iv0->extend == ZERO_EXTEND || iv1->extend == ZERO_EXTEND)
        return false;
      signed_p = true;
      break;
    ...
    case LEU:
    case LTU:
      if (iv0->extend == SIGN_EXTEND || iv1->extend == SIGN_EXTEND)
        return false;
      signed_p = false;
      break;
...
static unsigned HOST_WIDEST_INT determine_max_iter(struct loop *loop,
                                                   struct niter_desc *desc) {
  rtx niter = desc->niter_expr;
  rtx mmin, mmax, cmp;
  unsigned HOST_WIDEST_INT nmax, inc;

  if (GET_CODE(niter) == AND && GET_CODE(XEXP(niter, 0)) == CONST_INT) {
...
static void iv_number_of_iterations(struct loop *loop, rtx insn, rtx condition,
                                    struct niter_desc *desc) {
  ...
  switch (cond) {
    ...
    case LTU:
      ...
      if (iv0.step == const0_rtx) {
        ...
        assumption = simplify_gen_relational(EQ, SImode, mode, tmp, mode_mmax);
        if (assumption == const_true_rtx)
          goto zero_iter_simplify;
        iv0.base = simplify_gen_binary(PLUS, comp_mode, iv0.base, const1_rtx);
      } else {
        tmp = lowpart_subreg(mode, iv1.base, comp_mode);
        assumption = simplify_gen_relational(EQ, SImode, mode, tmp, mode_mmin);
        if (assumption == const_true_rtx)
          goto zero_iter_simplify;
        iv1.base = simplify_gen_binary(PLUS, comp_mode, iv1.base, constm1_rtx);
      }
  ...
  if (cond != NE) {
    ...
    if (GET_CODE(delta) == CONST_INT) {
      if (was_sharp && INTVAL(delta) == INTVAL(step) - 1) {
        ...
        tmp = lowpart_subreg(mode, iv0.base, comp_mode);
        may_xform = simplify_gen_relational(cond, SImode, mode, bound, tmp);
        may_not_xform = simplify_gen_relational(reverse_condition(cond), SImode,
                                                mode, bound, tmp);
      } else {
        ...
        bound = lowpart_subreg(mode, bound, comp_mode);
        tmp = lowpart_subreg(mode, iv1.base, comp_mode);
        may_xform = simplify_gen_relational(cond, SImode, mode, tmp, bound);
        may_not_xform = simplify_gen_relational(reverse_condition(cond), SImode,
                                                mode, tmp, bound);
      }
    }
    ...
    if (may_xform != const0_rtx) {
      ...
      down = INTVAL(GET_CODE(iv0.base) == CONST_INT ? iv0.base : mode_mmin);
  ...
  if (cond == NE) {
       ...
       but it was already taken into account during LE -> NE transform).  */
    {
      ...
      if (step_is_pow2) {
        ...
        assumption = simplify_gen_relational(cond, SImode, mode, tmp1, bound);
      ...
      if (step_is_pow2) {
        ...
        assumption = simplify_gen_relational(cond, SImode, mode, bound, tmp0);
        desc->assumptions = alloc_EXPR_LIST(0, assumption, desc->assumptions);
      }
...
static void check_simple_exit(struct loop *loop, edge e,
                              struct niter_desc *desc) {
  basic_block exit_bb;
  rtx condition, at;
  edge ein;
...
void find_simple_exit(struct loop *loop, struct niter_desc *desc) {
  unsigned i;
  basic_block *body;
  edge e;
