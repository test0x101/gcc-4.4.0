...
typedef struct ipa_reference_global_vars_info_d
...
/* This splay tree contains all of the static variables that are
   ...
   from this list.  */
static GTY((param1_is(int),
            param2_is(tree))) splay_tree reference_vars_to_consider;
...
static inline ipa_reference_vars_info_t get_reference_vars_info(
    struct cgraph_node *node) {
  if (!ipa_reference_vars_vector ||
      VEC_length(ipa_reference_vars_info_t, ipa_reference_vars_vector) <=
...
/* Return the ipa_reference_vars structure starting from the cgraph NODE.  */
static inline void set_reference_vars_info(struct cgraph_node *node,
  ...
  VEC_replace(ipa_reference_vars_info_t, ipa_reference_vars_vector, node->uid,
              info);
}

/* Get a bitmap that contains all of the locally referenced static
   variables for function FN.  */
static ipa_reference_local_vars_info_t get_local_reference_vars_info(
...
static ipa_reference_global_vars_info_t get_global_reference_vars_info(
    struct cgraph_node *fn) {
  ipa_reference_vars_info_t info = get_reference_vars_info(fn);
...
static inline void add_static_var(tree var) {
  int uid = DECL_UID(var);
  gcc_assert(TREE_CODE(var) == VAR_DECL);
  if (!bitmap_bit_p(all_module_statics, uid)) {
    splay_tree_insert(reference_vars_to_consider, uid, (splay_tree_value)var);
...
static inline bool has_proper_scope_for_analysis(tree t) {
...
static void mark_address_taken(tree x) {
  if (TREE_CODE(x) == VAR_DECL && module_statics_escape &&
...
static void mark_load(ipa_reference_local_vars_info_t local, tree t) {
...
static void mark_store(ipa_reference_local_vars_info_t local, tree t) {
  if (TREE_CODE(t) == VAR_DECL && has_proper_scope_for_analysis(t)) {
    if (local)
...
static void check_asm_memory_clobber(ipa_reference_local_vars_info_t local,
                                     gimple stmt) {
  size_t i;
  tree op;
...
static void check_call(ipa_reference_local_vars_info_t local, gimple stmt) {
...
static tree scan_stmt_for_static_refs(gimple_stmt_iterator *gsip,
                                      struct cgraph_node *fn) {
  ...
  switch (gimple_code(stmt)) {
    ...
    case GIMPLE_ASM:
      ...
      /* We used to check nonlocal labels here and set them as potentially
         modifying everything.  This is not needed, since we can get to nonlocal
         label only from callee and thus we will get info propagated.  */
...
static tree scan_initializer_for_static_refs(tree *tp, int *walk_subtrees,
...
static tree get_static_decl(int index) {
  splay_tree_node stn = splay_tree_lookup(reference_vars_to_consider, index);
  if (stn)
...
/* Or in all of the bits from every callee of X into X_GLOBAL, the caller's
   cycle, bit vector.  There are several cases to check to avoid the sparse
   bitmap oring.  */
  ...
  for (e = x->callees; e; e = e->next_callee) {
    ...
    if (cgraph_function_body_availability(e->callee) > AVAIL_OVERWRITABLE) {
      if (get_reference_vars_info(y)) {
        ...
        if (x_global->statics_read != all_module_statics) {
          if (y_global->statics_read == all_module_statics) {
            BITMAP_FREE(x_global->statics_read);
            x_global->statics_read = all_module_statics;
          }
          /* Skip bitmaps that are pointer equal to node's bitmap
             (no reason to spin within the cycle).  */
          else if (x_global->statics_read != y_global->statics_read)
            bitmap_ior_into(x_global->statics_read, y_global->statics_read);
        }

        if (x_global->statics_written != all_module_statics) {
          if (y_global->statics_written == all_module_statics) {
            BITMAP_FREE(x_global->statics_written);
            x_global->statics_written = all_module_statics;
          }
          /* Skip bitmaps that are pointer equal to node's bitmap
             (no reason to spin within the cycle).  */
          else if (x_global->statics_written != y_global->statics_written)
...
static void ipa_init(void) {
  ...
  reference_vars_to_consider = splay_tree_new_ggc(splay_tree_compare_ints);
...
static void analyze_variable(struct varpool_node *vnode) {
  struct walk_stmt_info wi;
  tree global = vnode->decl;

  memset(&wi, 0, sizeof(wi));
  wi.pset = visited_nodes;
  walk_tree(&DECL_INITIAL(global), scan_initializer_for_static_refs, &wi,
            wi.pset);
}
...
static ipa_reference_local_vars_info_t init_function_info(
    struct cgraph_node *fn) {
  ipa_reference_vars_info_t info = XCNEW(struct ipa_reference_vars_info_d);
...
static void analyze_function(struct cgraph_node *fn) {
  ...
  for (step = DECL_STRUCT_FUNCTION(decl)->local_decls; step;
       step = TREE_CHAIN(step)) {
    tree var = TREE_VALUE(step);
    if (TREE_CODE(var) == VAR_DECL && DECL_INITIAL(var) && !TREE_STATIC(var))
      gcc_unreachable();
  }
#endif
  pop_cfun();
...
static void clean_function_local_data(struct cgraph_node *fn) {
  ipa_reference_vars_info_t info = get_reference_vars_info(fn);
  ipa_reference_local_vars_info_t l = info->local;
  if (l) {
    if (l->statics_read && l->statics_read != all_module_statics)
      BITMAP_FREE(l->statics_read);
    if (l->statics_written && l->statics_written != all_module_statics)
...
static void clean_function(struct cgraph_node *fn) {
  ipa_reference_vars_info_t info = get_reference_vars_info(fn);
  ipa_reference_global_vars_info_t g = info->global;

  clean_function_local_data(fn);
  if (g) {
    if (g->statics_read && g->statics_read != all_module_statics)
      BITMAP_FREE(g->statics_read);

    if (g->statics_written && g->statics_written != all_module_statics)
      BITMAP_FREE(g->statics_written);

    if (g->statics_not_read && g->statics_not_read != all_module_statics)
      BITMAP_FREE(g->statics_not_read);

    if (g->statics_not_written && g->statics_not_written != all_module_statics)
...
static bitmap copy_local_bitmap(bitmap src) {
  bitmap dst;
  if (!src)
    return NULL;
...
static bitmap copy_global_bitmap(bitmap src) {
  bitmap dst;
  if (!src)
    return NULL;
  if (src == all_module_statics)
...
static void duplicate_node_data(struct cgraph_node *src,
  ...
  if (ginfo) {
    get_reference_vars_info(dst)->global =
    ...
    dst_ginfo->statics_not_written =
        copy_global_bitmap(ginfo->statics_not_written);
  }
}
...
static void generate_summary(void) {
  ...
  */
  ...
  if (dump_file)
    EXECUTE_IF_SET_IN_BITMAP(all_module_statics, 0, index, bi) {
      fprintf(dump_file, "\nPromotable global:%s", get_static_name(index));
    }
  ...
  for (node = cgraph_nodes; node; node = node->next)
    if (cgraph_function_body_availability(node) >= AVAIL_OVERWRITABLE) {
      ...
      /* Any variables that are not in all_module_statics are
         ...
         scanning.  */
      bitmap_and_into(l->statics_read, all_module_statics);
      bitmap_and_into(l->statics_written, all_module_statics);
    }
  ...
  if (dump_file)
    for (node = cgraph_nodes; node; node = node->next)
      if (cgraph_function_body_availability(node) >= AVAIL_OVERWRITABLE) {
        ...
        l = get_reference_vars_info(node)->local;
        fprintf(dump_file, "\nFunction name:%s/%i:", cgraph_node_name(node),
                node->uid);
        fprintf(dump_file, "\n  locals read: ");
...
static unsigned int propagate(void) {
  ...
  for (i = 0; i < order_pos; i++) {
    ...
    w = w_info->next_cycle;
    while (w) {
      ipa_reference_local_vars_info_t w_l = get_reference_vars_info(w)->local;
    ...
    if (read_all)
      ...
      bitmap_copy(node_g->statics_read, node_l->statics_read);
    }
    ...
    if (write_all)
      ...
      bitmap_copy(node_g->statics_written, node_l->statics_written);
    }
    ...
    while (w) {
      ...
      /* These global bitmaps are initialized from the local info
         ...
         all_module_statics.  */
      if (!read_all)
        bitmap_ior_into(node_g->statics_read, w_l->statics_read);
      if (!write_all)
        bitmap_ior_into(node_g->statics_written, w_l->statics_written);
    ...
    while (w) {
      ...
      w_ri->global->statics_written =
          copy_global_bitmap(node_g->statics_written);
  ...
  if (dump_file) {
    for (i = 0; i < order_pos; i++) {
      ...
      fprintf(dump_file, "\nFunction name:%s/%i:", cgraph_node_name(node),
              node->uid);
      fprintf(dump_file, "\n  locals read: ");
      EXECUTE_IF_SET_IN_BITMAP(node_l->statics_read, 0, index, bi) {
        fprintf(dump_file, "%s ", get_static_name(index));
      }
      fprintf(dump_file, "\n  locals written: ");
      EXECUTE_IF_SET_IN_BITMAP(node_l->statics_written, 0, index, bi) {
        fprintf(dump_file, "%s ", get_static_name(index));
      }
      ...
      while (w) {
        ipa_reference_vars_info_t w_ri = get_reference_vars_info(w);
        ipa_reference_local_vars_info_t w_l = w_ri->local;
        fprintf(dump_file, "\n  next cycle: %s/%i ", cgraph_node_name(w),
                w->uid);
        fprintf(dump_file, "\n    locals read: ");
        EXECUTE_IF_SET_IN_BITMAP(w_l->statics_read, 0, index, bi) {
          fprintf(dump_file, "%s ", get_static_name(index));
        }

        fprintf(dump_file, "\n    locals written: ");
        EXECUTE_IF_SET_IN_BITMAP(w_l->statics_written, 0, index, bi) {
          fprintf(dump_file, "%s ", get_static_name(index));
        }
      ...
      fprintf(dump_file, "\n  globals read: ");
      EXECUTE_IF_SET_IN_BITMAP(node_g->statics_read, 0, index, bi) {
        fprintf(dump_file, "%s ", get_static_name(index));
      }
      fprintf(dump_file, "\n  globals written: ");
      EXECUTE_IF_SET_IN_BITMAP(node_g->statics_written, 0, index, bi) {
        fprintf(dump_file, "%s ", get_static_name(index));
      }
    }
  }
  ...
  for (i = 0; i < order_pos; i++) {
    ...
    if (node_g->statics_read != all_module_statics)
      bitmap_and_compl(node_g->statics_not_read, all_module_statics,
                       node_g->statics_read);

    if (node_g->statics_written != all_module_statics)
      bitmap_and_compl(node_g->statics_not_written, all_module_statics,
...
static bool gate_reference(void) {
