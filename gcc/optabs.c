...
__extension__ struct convert_optab convert_optab_table[COI_MAX] = {
    [0 ... COI_MAX - 1]
        .handlers[0 ... NUM_MACHINE_MODES - 1][0 ... NUM_MACHINE_MODES - 1]
...
static hashval_t hash_libfunc(const void *p) {
  const struct libfunc_entry *const e = (const struct libfunc_entry *)p;

  return (((int)e->mode1 + (int)e->mode2 * NUM_MACHINE_MODES) ^ e->optab);
}

/* Used for optab_hash.  */

static int eq_libfunc(const void *p, const void *q) {
  ...
  return (e1->optab == e2->optab && e1->mode1 == e2->mode1 &&
          e1->mode2 == e2->mode2);
}
...
rtx convert_optab_libfunc(convert_optab optab, enum machine_mode mode1,
  ...
  if (!slot) {
    if (optab->libcall_gen) {
      ...
      slot =
          (struct libfunc_entry **)htab_find_slot(libfunc_hash, &e, NO_INSERT);
      if (slot)
        return (*slot)->libfunc;
      else
        return NULL;
...
rtx optab_libfunc(optab optab, enum machine_mode mode) {
  ...
  if (!slot) {
    if (optab->libcall_gen) {
      optab->libcall_gen(optab, optab->libcall_basename, optab->libcall_suffix,
                         mode);
      slot =
          (struct libfunc_entry **)htab_find_slot(libfunc_hash, &e, NO_INSERT);
      if (slot)
...
static int add_equal_note(rtx insns, rtx target, enum rtx_code code, rtx op0,
                          rtx op1) {
  ...
  if (GET_RTX_CLASS(code) != RTX_COMM_ARITH &&
      ...
      GET_RTX_CLASS(code) != RTX_COMPARE && GET_RTX_CLASS(code) != RTX_UNARY)
    return 1;

  if (GET_CODE(target) == ZERO_EXTRACT)
    return 1;

  for (last_insn = insns; NEXT_INSN(last_insn) != NULL_RTX;
...
static rtx widen_operand(rtx op, enum machine_mode mode,
                         enum machine_mode oldmode, int unsignedp,
                         int no_extend) {
  rtx result;
  ...
  if (!no_extend || (GET_CODE(op) == SUBREG && SUBREG_PROMOTED_VAR_P(op) &&
...
optab optab_for_tree_code(enum tree_code code, const_tree type,
  ...
  switch (code) {
    ...
    case VEC_WIDEN_MULT_HI_EXPR:
      return TYPE_UNSIGNED(type) ? vec_widen_umult_hi_optab
                                 : vec_widen_smult_hi_optab;

    case VEC_WIDEN_MULT_LO_EXPR:
      return TYPE_UNSIGNED(type) ? vec_widen_umult_lo_optab
    ...
    case VEC_UNPACK_FLOAT_HI_EXPR:
      ...
      return TYPE_UNSIGNED(type) ? vec_unpacku_float_hi_optab
                                 : vec_unpacks_float_hi_optab;

    case VEC_UNPACK_FLOAT_LO_EXPR:
      /* The signedness is determined from input operand.  */
      return TYPE_UNSIGNED(type) ? vec_unpacku_float_lo_optab
...
rtx expand_widen_pattern_expr(tree exp, rtx op0, rtx op1, rtx wide_op,
  ...
  if (!target || !(*insn_data[icode].operand[0].predicate)(target, wmode))
  ...
  if (GET_MODE(op0) != xmode0 && xmode0 != VOIDmode)
    xop0 = convert_modes(xmode0,
                         GET_MODE(op0) != VOIDmode ? GET_MODE(op0) : tmode0,
                         xop0, unsignedp);

  if (op1)
    if (GET_MODE(op1) != xmode1 && xmode1 != VOIDmode)
      xop1 = convert_modes(xmode1,
                           GET_MODE(op1) != VOIDmode ? GET_MODE(op1) : tmode1,
                           xop1, unsignedp);

  if (wide_op)
    if (GET_MODE(wide_op) != wxmode && wxmode != VOIDmode)
      wxop = convert_modes(
          wxmode, GET_MODE(wide_op) != VOIDmode ? GET_MODE(wide_op) : wmode,
          wxop, unsignedp);
...
rtx expand_ternary_op(enum machine_mode mode, optab ternary_optab, rtx op0,
  ...
  gcc_assert(optab_handler(ternary_optab, mode)->insn_code != CODE_FOR_nothing);
  ...
  if (GET_MODE(op0) != mode0 && mode0 != VOIDmode)
    xop0 =
        convert_modes(mode0, GET_MODE(op0) != VOIDmode ? GET_MODE(op0) : mode,
                      xop0, unsignedp);

  if (GET_MODE(op1) != mode1 && mode1 != VOIDmode)
    xop1 =
        convert_modes(mode1, GET_MODE(op1) != VOIDmode ? GET_MODE(op1) : mode,
                      xop1, unsignedp);

  if (GET_MODE(op2) != mode2 && mode2 != VOIDmode)
    xop2 =
        convert_modes(mode2, GET_MODE(op2) != VOIDmode ? GET_MODE(op2) : mode,
                      xop2, unsignedp);

  /* Now, if insn's predicates don't allow our operands, put them into
     pseudo regs.  */

  if (!insn_data[icode].operand[1].predicate(xop0, mode0) && mode0 != VOIDmode)
    xop0 = copy_to_mode_reg(mode0, xop0);

  if (!insn_data[icode].operand[2].predicate(xop1, mode1) && mode1 != VOIDmode)
    xop1 = copy_to_mode_reg(mode1, xop1);

  if (!insn_data[icode].operand[3].predicate(xop2, mode2) && mode2 != VOIDmode)
...
static rtx simplify_expand_binop(enum machine_mode mode, optab binoptab,
...
bool force_expand_binop(enum machine_mode mode, optab binoptab, rtx op0,
                        rtx op1, rtx target, int unsignedp,
                        enum optab_methods methods) {
  rtx x = simplify_expand_binop(mode, binoptab, op0, op1, target, unsignedp,
                                methods);
  if (x == 0)
    return false;
  if (x != target)
...
rtx expand_vec_shift_expr(tree vec_shift_expr, rtx target) {
...
static bool expand_superword_shift(optab binoptab, rtx outof_input,
                                   rtx superword_op1, rtx outof_target,
                                   rtx into_target, int unsignedp,
                                   enum optab_methods methods) {
  if (into_target != 0)
  ...
  if (outof_target != 0) {
    ...
    else if (!force_expand_binop(word_mode, binoptab, outof_input,
                                 GEN_INT(BITS_PER_WORD - 1), outof_target,
                                 unsignedp, methods))
      return false;
  }
  return true;
...
static bool expand_subword_shift(enum machine_mode op1_mode, optab binoptab,
  ...
  if (CONSTANT_P(op1) || shift_mask >= BITS_PER_WORD) {
    ...
    tmp =
        simplify_expand_binop(op1_mode, sub_optab, tmp, op1, 0, true, methods);
  } else {
    ...
    carries = expand_binop(word_mode, reverse_unsigned_shift, outof_input,
                           const1_rtx, 0, unsignedp, methods);
    if (shift_mask == BITS_PER_WORD - 1) {
      tmp = immed_double_const(-1, -1, op1_mode);
      tmp = simplify_expand_binop(op1_mode, xor_optab, op1, tmp, 0, true,
                                  methods);
    } else {
      tmp = immed_double_const(BITS_PER_WORD - 1, 0, op1_mode);
      tmp = simplify_expand_binop(op1_mode, sub_optab, tmp, op1, 0, true,
                                  methods);
    }
  }
  if (tmp == 0 || carries == 0)
    return false;
  carries = expand_binop(word_mode, reverse_unsigned_shift, carries, tmp, 0,
                         unsignedp, methods);
  if (carries == 0)
    return false;
  ...
  tmp = expand_binop(word_mode, unsigned_shift, into_input, op1, into_target,
                     unsignedp, methods);
  if (tmp == 0)
    return false;

  /* Now OR in the bits carried over from OUTOF_INPUT.  */
  if (!force_expand_binop(word_mode, ior_optab, tmp, carries, into_target,
                          unsignedp, methods))
    return false;

  /* Use a standard word_mode shift for the out-of half.  */
  if (outof_target != 0)
    if (!force_expand_binop(word_mode, binoptab, outof_input, op1, outof_target,
                            unsignedp, methods))
      return false;
...
static bool expand_doubleword_shift_condmove(
    enum machine_mode op1_mode, optab binoptab, enum rtx_code cmp_code,
    rtx cmp1, rtx cmp2, rtx outof_input, rtx into_input, rtx subword_op1,
    rtx superword_op1, rtx outof_target, rtx into_target, int unsignedp,
    enum optab_methods methods, unsigned HOST_WIDE_INT shift_mask) {
  ...
  if (outof_target != 0 && subword_op1 == superword_op1) {
    ...
    if (!expand_superword_shift(binoptab, outof_input, superword_op1,
                                outof_superword, into_superword, unsignedp,
                                methods))
      return false;
  }

  /* Put the subword version directly in OUTOF_TARGET and INTO_TARGET.  */
  if (!expand_subword_shift(op1_mode, binoptab, outof_input, into_input,
                            subword_op1, outof_target, into_target, unsignedp,
                            methods, shift_mask))
    return false;
  ...
  if (outof_target != 0)
    if (!emit_conditional_move(outof_target, cmp_code, cmp1, cmp2, op1_mode,
                               outof_target, outof_superword, word_mode, false))
      return false;
...
static bool expand_doubleword_shift(enum machine_mode op1_mode, optab binoptab,
                                    rtx outof_input, rtx into_input, rtx op1,
                                    rtx outof_target, rtx into_target,
                                    int unsignedp, enum optab_methods methods,
                                    unsigned HOST_WIDE_INT shift_mask) {
  ...
  if (shift_mask >= BITS_PER_WORD && outof_target != 0 && !CONSTANT_P(op1)) {
    if (!expand_doubleword_shift(op1_mode, binoptab, outof_input, into_input,
                                 op1, 0, into_target, unsignedp, methods,
                                 shift_mask))
      return false;
    if (!force_expand_binop(word_mode, binoptab, outof_input, op1, outof_target,
                            unsignedp, methods))
  ...
  if (!CONSTANT_P(op1) && shift_mask == BITS_PER_WORD - 1) {
    ...
    cmp1 =
        simplify_expand_binop(op1_mode, and_optab, op1, tmp, 0, true, methods);
    ...
    cmp1 =
        simplify_expand_binop(op1_mode, sub_optab, op1, tmp, 0, true, methods);
    cmp2 = CONST0_RTX(op1_mode);
    cmp_code = LT;
    superword_op1 = cmp1;
  }
  ...
  if (tmp != 0 && GET_CODE(tmp) == CONST_INT) {
    if (tmp == const0_rtx)
      return expand_superword_shift(binoptab, outof_input, superword_op1,
                                    outof_target, into_target, unsignedp,
                                    methods);
    else
      return expand_subword_shift(op1_mode, binoptab, outof_input, into_input,
                                  op1, outof_target, into_target, unsignedp,
                                  methods, shift_mask);
  }
  ...
  /* Try using conditional moves to generate straight-line code.  */
  {
    ...
    if (expand_doubleword_shift_condmove(
            op1_mode, binoptab, cmp_code, cmp1, cmp2, outof_input, into_input,
            op1, superword_op1, outof_target, into_target, unsignedp, methods,
            shift_mask))
      return true;
  ...
  NO_DEFER_POP;
  do_compare_rtx_and_jump(cmp1, cmp2, cmp_code, false, op1_mode, 0, 0,
                          subword_label);
  OK_DEFER_POP;
  ...
  if (!expand_subword_shift(op1_mode, binoptab, outof_input, into_input, op1,
                            outof_target, into_target, unsignedp, methods,
                            shift_mask))
    return false;
...
static rtx expand_doubleword_mult(enum machine_mode mode, rtx op0, rtx op1,
                                  rtx target, bool umulp,
                                  enum optab_methods methods) {
  ...
  if (!umulp) {
    ...
    temp = expand_binop(word_mode, lshr_optab, op0_low, wordm1, NULL_RTX, 1,
                        methods);
    if (temp)
      op0_high = expand_binop(word_mode, add_optab, op0_high, temp, NULL_RTX, 0,
                              OPTAB_DIRECT);
    else {
      temp = expand_binop(word_mode, ashr_optab, op0_low, wordm1, NULL_RTX, 0,
                          methods);
      if (!temp)
        return NULL_RTX;
      op0_high = expand_binop(word_mode, sub_optab, op0_high, temp, NULL_RTX, 0,
                              OPTAB_DIRECT);
    }
  ...
  adjust = expand_binop(word_mode, smul_optab, op0_high, op1_low, NULL_RTX, 0,
                        OPTAB_DIRECT);
  if (!adjust)
    return NULL_RTX;
  ...
  if (!umulp) {
    /* ??? This could be done with emit_store_flag where available.  */
    temp = expand_binop(word_mode, lshr_optab, op1_low, wordm1, NULL_RTX, 1,
                        methods);
    if (temp)
      op1_high = expand_binop(word_mode, add_optab, op1_high, temp, NULL_RTX, 0,
                              OPTAB_DIRECT);
    else {
      temp = expand_binop(word_mode, ashr_optab, op1_low, wordm1, NULL_RTX, 0,
                          methods);
      if (!temp)
        return NULL_RTX;
      op1_high = expand_binop(word_mode, sub_optab, op1_high, temp, NULL_RTX, 0,
                              OPTAB_DIRECT);
    }
  ...
  temp = expand_binop(word_mode, smul_optab, op1_high, op0_low, NULL_RTX, 0,
                      OPTAB_DIRECT);
  if (!temp)
    return NULL_RTX;
  ...
  adjust =
      expand_binop(word_mode, add_optab, adjust, temp, adjust, 0, OPTAB_DIRECT);

  if (target && !REG_P(target))
    target = NULL_RTX;

  if (umulp)
    product = expand_binop(mode, umul_widen_optab, op0_low, op1_low, target, 1,
                           OPTAB_DIRECT);
  else
    product = expand_binop(mode, smul_widen_optab, op0_low, op1_low, target, 1,
                           OPTAB_DIRECT);
  ...
  adjust = expand_binop(word_mode, add_optab, product_high, adjust,
                        REG_P(product_high) ? product_high : adjust, 0,
                        OPTAB_DIRECT);
...
rtx expand_simple_binop(enum machine_mode mode, enum rtx_code code, rtx op0,
                        rtx op1, rtx target, int unsignedp,
                        enum optab_methods methods) {
...
static bool swap_commutative_operands_with_target(rtx target, rtx op0,
                                                  rtx op1) {
...
static rtx avoid_expensive_constant(enum machine_mode mode, optab binoptab,
...
static rtx expand_binop_directly(enum machine_mode mode, optab binoptab,
                                 rtx op0, rtx op1, rtx target, int unsignedp,
                                 enum optab_methods methods, rtx last) {
  ...
  if (commutative_p && GET_MODE(xop0) != mode0 && GET_MODE(xop1) != mode1 &&
  ...
  if (GET_MODE(xop0) != mode0 && mode0 != VOIDmode)
    xop0 =
        convert_modes(mode0, GET_MODE(xop0) != VOIDmode ? GET_MODE(xop0) : mode,
                      xop0, unsignedp);

  if (GET_MODE(xop1) != mode1 && mode1 != VOIDmode)
    xop1 =
        convert_modes(mode1, GET_MODE(xop1) != VOIDmode ? GET_MODE(xop1) : mode,
                      xop1, unsignedp);
  ...
  if (binoptab == vec_pack_trunc_optab || binoptab == vec_pack_usat_optab ||
  ...
  if (pat) {
    ...
    if (INSN_P(pat) && NEXT_INSN(pat) != NULL_RTX &&
      ...
      return expand_binop(mode, binoptab, op0, op1, NULL_RTX, unsignedp,
                          methods);
    }
...
rtx expand_binop(enum machine_mode mode, optab binoptab, rtx op0, rtx op1,
                 rtx target, int unsignedp, enum optab_methods methods) {
  enum optab_methods next_methods =
      (methods == OPTAB_LIB || methods == OPTAB_LIB_WIDEN ? OPTAB_WIDEN
                                                          : methods);
  ...
  if (methods != OPTAB_MUST_WIDEN &&
    ...
    temp = expand_binop_directly(mode, binoptab, op0, op1, target, unsignedp,
                                 methods, last);
    if (temp)
      return temp;
  }
  ...
  if (((binoptab == rotl_optab &&
    ...
    if (GET_CODE(op1) == CONST_INT)
      ...
      newop1 = expand_binop(mode, sub_optab, GEN_INT(bits), op1, NULL_RTX,
                            unsignedp, OPTAB_DIRECT);

    temp = expand_binop_directly(mode, otheroptab, op0, newop1, target,
  ...
  if (binoptab == smul_optab && GET_MODE_WIDER_MODE(mode) != VOIDmode &&
    ...
    temp = expand_binop(GET_MODE_WIDER_MODE(mode),
                        ...
                        op1, NULL_RTX, unsignedp, OPTAB_DIRECT);
  ...
  if (CLASS_HAS_WIDER_MODES_P(mclass) && methods != OPTAB_DIRECT &&
      methods != OPTAB_LIB)
    for (wider_mode = GET_MODE_WIDER_MODE(mode); wider_mode != VOIDmode;
      ...
      if (optab_handler(binoptab, wider_mode)->insn_code != CODE_FOR_nothing ||
        ...
        if ((binoptab == ior_optab || binoptab == and_optab ||
             binoptab == xor_optab || binoptab == add_optab ||
             binoptab == sub_optab || binoptab == smul_optab ||
             binoptab == ashl_optab) &&
            mclass == MODE_INT) {
          no_extend = 1;
          xop0 = avoid_expensive_constant(mode, binoptab, xop0, unsignedp);
          if (binoptab != ashl_optab)
            xop1 = avoid_expensive_constant(mode, binoptab, xop1, unsignedp);
        }
  ...
  if ((binoptab == and_optab || binoptab == ior_optab ||
    ...
    for (i = 0; i < GET_MODE_BITSIZE(mode) / BITS_PER_WORD; i++) {
      ...
      rtx x =
          expand_binop(word_mode, binoptab, operand_subword_force(op0, i, mode),
                       operand_subword_force(op1, i, mode), target_piece,
    ...
    if (i == GET_MODE_BITSIZE(mode) / BITS_PER_WORD) {
      if (binoptab->code != UNKNOWN)
        equiv_value =
            gen_rtx_fmt_ee(binoptab->code, mode, copy_rtx(op0), copy_rtx(op1));
      else
        equiv_value = 0;
  ...
  if ((binoptab == rotl_optab || binoptab == rotr_optab) &&
    ...
    if (shift_count == BITS_PER_WORD) {
      ...
      reverse_unsigned_shift =
          (left_shift ^ (shift_count < BITS_PER_WORD) ? lshr_optab
                                                      : ashl_optab);

      unsigned_shift =
          (left_shift ^ (shift_count < BITS_PER_WORD) ? ashl_optab
                                                      : lshr_optab);
      ...
      into_temp1 =
          expand_binop(word_mode, unsigned_shift, outof_input,
                       first_shift_count, NULL_RTX, unsignedp, next_methods);
      into_temp2 =
          expand_binop(word_mode, reverse_unsigned_shift, into_input,
                       second_shift_count, NULL_RTX, unsignedp, next_methods);
      ...
      outof_temp1 =
          expand_binop(word_mode, unsigned_shift, into_input, first_shift_count,
                       NULL_RTX, unsignedp, next_methods);
      outof_temp2 =
          expand_binop(word_mode, reverse_unsigned_shift, outof_input,
                       second_shift_count, NULL_RTX, unsignedp, next_methods);

      if (inter != 0 && outof_temp1 != 0 && outof_temp2 != 0)
        inter = expand_binop(word_mode, ior_optab, outof_temp1, outof_temp2,
  ...
  if ((binoptab == add_optab || binoptab == sub_optab) && mclass == MODE_INT &&
    ...
    for (i = 0; i < nwords; i++) {
      ...
      x = expand_binop(word_mode, binoptab, op0_piece, op1_piece, target_piece,
                       unsignedp, next_methods);
      if (x == 0)
        break;
      ...
      if (i + 1 < nwords) {
        ...
        carry_out =
            emit_store_flag_force(carry_out, (binoptab == add_optab ? LT : GT),
                                  x, op0_piece, word_mode, 1, normalizep);
      }
      ...
      if (i > 0) {
        ...
        newx = expand_binop(word_mode, normalizep == 1 ? binoptab : otheroptab,
        ...
        if (i + 1 < nwords) {
          ...
          carry_tmp = emit_store_flag_force(carry_tmp,
                                            (binoptab == add_optab ? LT : GT),
                                            newx, x, word_mode, 1, normalizep);

          /* Logical-ior the two poss. carry together.  */
          carry_out = expand_binop(word_mode, ior_optab, carry_out, carry_tmp,
    ...
    if (i == GET_MODE_BITSIZE(mode) / (unsigned)BITS_PER_WORD) {
      if (optab_handler(mov_optab, mode)->insn_code != CODE_FOR_nothing ||
        ...
        set_unique_reg_note(temp, REG_EQUAL,
                            gen_rtx_fmt_ee(binoptab->code, mode, copy_rtx(xop0),
                                           copy_rtx(xop1)));
      } else
        target = xtarget;
  ...
  if (binoptab == smul_optab && mclass == MODE_INT &&
      GET_MODE_SIZE(mode) == 2 * UNITS_PER_WORD &&
    ...
    if (optab_handler(umul_widen_optab, mode)->insn_code != CODE_FOR_nothing) {
      product = expand_doubleword_mult(mode, op0, op1, target, true, methods);
      if (!product)
        delete_insns_since(last);
    }

    if (product == NULL_RTX &&
        optab_handler(smul_widen_optab, mode)->insn_code != CODE_FOR_nothing) {
      product = expand_doubleword_mult(mode, op0, op1, target, false, methods);
      if (!product)
        delete_insns_since(last);
    }
  ...
  if (libfunc && (methods == OPTAB_LIB || methods == OPTAB_LIB_WIDEN)) {
    ...
    value = emit_library_call_value(libfunc, NULL_RTX, LCT_CONST, mode, 2, op0,
                                    mode, op1x, op1_mode);
  ...
  if (CLASS_HAS_WIDER_MODES_P(mclass)) {
    for (wider_mode = GET_MODE_WIDER_MODE(mode); wider_mode != VOIDmode;
      ...
      if ((optab_handler(binoptab, wider_mode)->insn_code !=
           CODE_FOR_nothing) ||
          (methods == OPTAB_LIB && optab_libfunc(binoptab, wider_mode))) {
        rtx xop0 = op0, xop1 = op1;
        int no_extend = 0;
...
rtx sign_expand_binop(enum machine_mode mode, optab uoptab, optab soptab,
                      rtx op0, rtx op1, rtx target, int unsignedp,
                      enum optab_methods methods) {
  rtx temp;
  ...
  temp = expand_binop(mode, direct_optab, op0, op1, target, unsignedp,
                      OPTAB_DIRECT);
  ...
  temp = expand_binop(mode, &wide_soptab, op0, op1, target, unsignedp,
                      OPTAB_WIDEN);
...
int expand_twoval_unop(optab unoptab, rtx op0, rtx targ0, rtx targ1,
                       int unsignedp) {
...
int expand_twoval_binop(optab binoptab, rtx op0, rtx op1, rtx targ0, rtx targ1,
                        int unsignedp) {
  enum machine_mode mode = GET_MODE(targ0 ? targ0 : targ1);
  enum mode_class mclass;
  enum machine_mode wider_mode;
  ...
  if (CLASS_HAS_WIDER_MODES_P(mclass)) {
    for (wider_mode = GET_MODE_WIDER_MODE(mode); wider_mode != VOIDmode;
      ...
      if (optab_handler(binoptab, wider_mode)->insn_code != CODE_FOR_nothing) {
        ...
        if (expand_twoval_binop(binoptab, cop0, cop1, t0, t1, unsignedp)) {
          convert_move(targ0, t0, unsignedp);
          convert_move(targ1, t1, unsignedp);
          return 1;
        } else
...
bool expand_twoval_binop_libfunc(optab binoptab, rtx op0, rtx op1, rtx targ0,
                                 rtx targ1, enum rtx_code code) {
  ...
  libval_mode = smallest_mode_for_size(2 * GET_MODE_BITSIZE(mode), MODE_INT);
  start_sequence();
...
/* Wrapper around expand_unop which takes an rtx code to specify
   the operation to perform, not an optab pointer.  All other
   arguments are the same.  */
rtx expand_simple_unop(enum machine_mode mode, enum rtx_code code, rtx op0,
                       rtx target, int unsignedp) {
...
static rtx widen_clz(enum machine_mode mode, rtx op0, rtx target) {
  ...
  if (CLASS_HAS_WIDER_MODES_P(mclass)) {
    ...
    for (wider_mode = GET_MODE_WIDER_MODE(mode); wider_mode != VOIDmode;
         wider_mode = GET_MODE_WIDER_MODE(wider_mode)) {
      if (optab_handler(clz_optab, wider_mode)->insn_code != CODE_FOR_nothing) {
        rtx xop0, temp, last;
        ...
        if (temp != 0)
          temp = expand_binop(
              ...
              GEN_INT(GET_MODE_BITSIZE(wider_mode) - GET_MODE_BITSIZE(mode)),
...
static rtx expand_doubleword_clz(enum machine_mode mode, rtx op0, rtx target) {
  ...
  emit_cmp_and_jump_insns(subhi, CONST0_RTX(word_mode), EQ, 0, word_mode, true,
                          hi0_label);
  ...
  temp = expand_binop(word_mode, add_optab, temp,
                      GEN_INT(GET_MODE_BITSIZE(word_mode)), result, true,
                      OPTAB_DIRECT);
  if (!temp)
    goto fail;
  if (temp != result)
...
static rtx widen_bswap(enum machine_mode mode, rtx op0, rtx target) {
  enum mode_class mclass = GET_MODE_CLASS(mode);
  enum machine_mode wider_mode;
  rtx x, last;
...
found:
  ...
  if (x != 0)
    x = expand_shift(
        RSHIFT_EXPR, wider_mode, x,
        size_int(GET_MODE_BITSIZE(wider_mode) - GET_MODE_BITSIZE(mode)),
        NULL_RTX, true);
...
static rtx expand_doubleword_bswap(enum machine_mode mode, rtx op, rtx target) {
  rtx t0, t1;

  t1 = expand_unop(word_mode, bswap_optab, operand_subword_force(op, 0, mode),
                   NULL_RTX, true);
  t0 = expand_unop(word_mode, bswap_optab, operand_subword_force(op, 1, mode),
                   NULL_RTX, true);
    ...
    for (wider_mode = mode; wider_mode != VOIDmode;
      ...
      if (optab_handler(popcount_optab, wider_mode)->insn_code !=
        ...
        if (target == 0)
          target = gen_reg_rtx(mode);
        xop0 = widen_operand(op0, wider_mode, mode, true, false);
        temp = expand_unop(wider_mode, popcount_optab, xop0, NULL_RTX, true);
        if (temp != 0)
          temp = expand_binop(wider_mode, and_optab, temp, const1_rtx, target,
                              true, OPTAB_DIRECT);
        if (temp == 0)
          delete_insns_since(last);
...
static rtx expand_ctz(enum machine_mode mode, rtx op0, rtx target) {
  rtx seq, temp;
  ...
  if (temp)
    temp =
        expand_binop(mode, and_optab, op0, temp, NULL_RTX, true, OPTAB_DIRECT);
  if (temp)
...
static rtx expand_ffs(enum machine_mode mode, rtx op0, rtx target) {
  HOST_WIDE_INT val = 0;
  ...
  if (defined_at_zero && val == -1)
    ...
    emit_cmp_and_jump_insns(op0, CONST0_RTX(mode), NE, 0, mode, true,
                            nonzero_label);
  ...
  temp = expand_binop(mode, add_optab, temp, GEN_INT(1), target, false,
                      OPTAB_DIRECT);
  if (!temp)
    goto fail;
...
static rtx lowpart_subreg_maybe_copy(enum machine_mode omode, rtx val,
...
static rtx expand_absneg_bit(enum rtx_code code, enum machine_mode mode,
                             rtx op0, rtx target) {
  ...
  if (nwords > 1) {
    ...
    for (i = 0; i < nwords; ++i) {
      ...
      if (i == word) {
        temp = expand_binop(imode, code == ABS ? and_optab : xor_optab,
                            op0_piece, immed_double_const(lo, hi, imode),
    ...
    temp =
        expand_binop(imode, code == ABS ? and_optab : xor_optab,
                     gen_lowpart(imode, op0), immed_double_const(lo, hi, imode),
...
static rtx expand_unop_direct(enum machine_mode mode, optab unoptab, rtx op0,
                              rtx target, int unsignedp) {
  if (optab_handler(unoptab, mode)->insn_code != CODE_FOR_nothing) {
    int icode = (int)optab_handler(unoptab, mode)->insn_code;
    enum machine_mode mode0 = insn_data[icode].operand[1].mode;
...
rtx expand_unop(enum machine_mode mode, optab unoptab, rtx op0, rtx target,
                int unsignedp) {
  enum mode_class mclass = GET_MODE_CLASS(mode);
  enum machine_mode wider_mode;
  rtx temp;
  ...
  if (CLASS_HAS_WIDER_MODES_P(mclass))
    for (wider_mode = GET_MODE_WIDER_MODE(mode); wider_mode != VOIDmode;
      ...
      if (optab_handler(unoptab, wider_mode)->insn_code != CODE_FOR_nothing) {
        ...
        xop0 =
            widen_operand(xop0, wider_mode, mode, unsignedp,
                          (unoptab == neg_optab || unoptab == one_cmpl_optab) &&
                              mclass == MODE_INT);
  ...
  /* These can be done a word at a time.  */
  if (unoptab == one_cmpl_optab && mclass == MODE_INT &&
    ...
    for (i = 0; i < GET_MODE_BITSIZE(mode) / BITS_PER_WORD; i++) {
      rtx target_piece = operand_subword(target, i, 1, mode);
      rtx x =
          expand_unop(word_mode, unoptab, operand_subword_force(op0, i, mode),
  ...
  if (unoptab->code == NEG) {
    ...
    if (!HONOR_SIGNED_ZEROS(mode)) {
      temp =
          expand_binop(mode, (unoptab == negv_optab ? subv_optab : sub_optab),
                       CONST0_RTX(mode), op0, target, unsignedp, OPTAB_DIRECT);
      if (temp)
        return temp;
    }
  }
...
try_libcall:
  ...
  if (libfunc) {
    ...
    if (unoptab == ffs_optab || unoptab == clz_optab || unoptab == ctz_optab ||
      ...
      outmode = GET_MODE(hard_libcall_value(TYPE_MODE(integer_type_node)));

    start_sequence();

    /* Pass 1 for NO_QUEUE so we don't lose any increments
       if the libcall is cse'd or moved.  */
    value = emit_library_call_value(libfunc, NULL_RTX, LCT_CONST, outmode, 1,
                                    op0, mode);
    insns = get_insns();
    end_sequence();
...
rtx expand_abs_nojump(enum machine_mode mode, rtx op0, rtx target,
  ...
  temp = expand_unop(mode, result_unsignedp ? abs_optab : absv_optab, op0,
                     target, 0);
  if (temp != 0)
    return temp;
  ...
  if (optab_handler(smax_optab, mode)->insn_code != CODE_FOR_nothing &&
    ...
    if (temp != 0)
      temp = expand_binop(mode, smax_optab, op0, temp, target, 0, OPTAB_WIDEN);
  ...
  if (GET_MODE_CLASS(mode) == MODE_INT &&
    ...
    rtx extended =
        expand_shift(RSHIFT_EXPR, mode, op0,
                     size_int(GET_MODE_BITSIZE(mode) - 1), NULL_RTX, 0);

    temp = expand_binop(mode, xor_optab, extended, op0, target, 0,
                        OPTAB_LIB_WIDEN);
    if (temp != 0)
      temp = expand_binop(mode, result_unsignedp ? sub_optab : subv_optab, temp,
                          extended, target, 0, OPTAB_LIB_WIDEN);
...
rtx expand_abs(enum machine_mode mode, rtx op0, rtx target,
  ...
  if (op0 == target && REG_P(op0) && REGNO(op0) >= FIRST_PSEUDO_REGISTER)
    safe = 1;
  ...
  do_compare_rtx_and_jump(target, CONST0_RTX(mode), GE, 0, mode, NULL_RTX,
                          NULL_RTX, op1);

  op0 = expand_unop(mode, result_unsignedp ? neg_optab : negv_optab, target,
                    target, 0);
  if (op0 != target)
    emit_move_insn(target, op0);
  emit_label(op1);
...
static rtx expand_copysign_absneg(enum machine_mode mode, rtx op0, rtx op1,
                                  rtx target, int bitpos, bool op0_is_abs) {
  enum machine_mode imode;
  int icode;
  rtx sign, label;
  ...
  if (icode != CODE_FOR_nothing) {
    ...
    sign = gen_reg_rtx(imode);
    sign =
        expand_binop(imode, and_optab, op1, immed_double_const(lo, hi, imode),
...
static rtx expand_copysign_bit(enum machine_mode mode, rtx op0, rtx op1,
                               rtx target, int bitpos, bool op0_is_abs) {
  enum machine_mode imode;
  HOST_WIDE_INT hi, lo;
  int word, nwords, i;
  ...
  if (nwords > 1) {
    ...
    for (i = 0; i < nwords; ++i) {
      ...
      if (i == word) {
        ...
        op1 = expand_binop(
            imode, and_optab, operand_subword_force(op1, i, mode),
            immed_double_const(lo, hi, imode), NULL_RTX, 1, OPTAB_LIB_WIDEN);

        temp = expand_binop(imode, ior_optab, op0_piece, op1, targ_piece, 1,
                            OPTAB_LIB_WIDEN);
        if (temp != targ_piece)
          emit_move_insn(targ_piece, temp);
      } else
    ...
    op1 = expand_binop(imode, and_optab, gen_lowpart(imode, op1),
                       immed_double_const(lo, hi, imode), NULL_RTX, 1,
                       OPTAB_LIB_WIDEN);
    ...
    temp = expand_binop(imode, ior_optab, op0, op1, gen_lowpart(imode, target),
                        1, OPTAB_LIB_WIDEN);
    target = lowpart_subreg_maybe_copy(mode, temp, imode);
  }
...
rtx expand_copysign(rtx op0, rtx op1, rtx target) {
  ...
  temp = expand_binop(mode, copysign_optab, op0, op1, target, 0, OPTAB_DIRECT);
  if (temp)
    return temp;
  ...
  if (fmt->signbit_ro >= 0 &&
    ...
    temp = expand_copysign_absneg(mode, op0, op1, target, fmt->signbit_ro,
                                  op0_is_abs);
    if (temp)
      return temp;
  }
  ...
  if (fmt->signbit_rw < 0)
    return NULL_RTX;
  return expand_copysign_bit(mode, op0, op1, target, fmt->signbit_rw,
                             op0_is_abs);
}
...
bool maybe_emit_unop_insn(int icode, rtx target, rtx op0, enum rtx_code code) {
  rtx temp;
  enum machine_mode mode0 = insn_data[icode].operand[1].mode;
  rtx pat;
  rtx last = get_last_insn();
...
void emit_libcall_block(rtx insns, rtx target, rtx result, rtx equiv) {
  ...
  if (optab_handler(mov_optab, GET_MODE(target))->insn_code != CODE_FOR_nothing)
...
int can_compare_p(enum rtx_code code, enum machine_mode mode,
...
static void prepare_cmp_insn(rtx *px, rtx *py, enum rtx_code *pcomparison,
                             rtx size, enum machine_mode *pmode,
                             int *punsignedp,
                             enum can_compare_purpose purpose) {
  ...
  if (CONSTANT_P(x) && optimize &&
      (rtx_cost(x, COMPARE, optimize_insn_for_speed_p()) > COSTS_N_INSNS(1)))
    x = force_reg(mode, x);

  if (CONSTANT_P(y) && optimize &&
      (rtx_cost(y, COMPARE, optimize_insn_for_speed_p()) > COSTS_N_INSNS(1)))
    y = force_reg(mode, y);
  ...
  if (mode == BLKmode) {
    ...
    rtx libfunc;
    rtx result;
    rtx opalign = GEN_INT(MIN(MEM_ALIGN(x), MEM_ALIGN(y)) / BITS_PER_UNIT);

    gcc_assert(size);

    /* Try to use a memory block compare insn - either cmpstr
       or cmpmem will do.  */
    for (cmp_mode = GET_CLASS_NARROWEST_MODE(MODE_INT); cmp_mode != VOIDmode;
      ...
      if ((GET_CODE(size) == CONST_INT &&
          ...
          (GET_MODE_BITSIZE(GET_MODE(size)) > GET_MODE_BITSIZE(cmp_mode)))
        continue;
    ...
    result = emit_library_call_value(libfunc, 0, LCT_PURE, result_mode, 3,
                                     XEXP(x, 0), Pmode, XEXP(y, 0), Pmode, size,
                                     cmp_mode);
    *px = result;
    *py = const0_rtx;
    *pmode = result_mode;
  ...
  *py = y;
  ...
  if (libfunc && !SCALAR_FLOAT_MODE_P(mode)) {
    ...
    result = emit_library_call_value(libfunc, NULL_RTX, LCT_CONST,
                                     targetm.libgcc_cmp_return_mode(), 2, x,
                                     mode, y, mode);
...
static rtx prepare_operand(int icode, rtx x, int opnum, enum machine_mode mode,
...
static void emit_cmp_and_jump_insn_1(rtx x, rtx y, enum machine_mode mode,
                                     enum rtx_code comparison, int unsignedp,
                                     rtx label) {
...
void emit_cmp_and_jump_insns(rtx x, rtx y, enum rtx_code comparison, rtx size,
  ...
  prepare_cmp_insn(&op0, &op1, &comparison, size, &mode, &unsignedp, ccp_jump);
  emit_cmp_and_jump_insn_1(op0, op1, mode, comparison, unsignedp, label);
}

/* Like emit_cmp_and_jump_insns, but generate only the comparison.  */

void emit_cmp_insn(rtx x, rtx y, enum rtx_code comparison, rtx size,
...
static void prepare_float_lib_cmp(rtx *px, rtx *py, enum rtx_code *pcomparison,
  ...
  if (comparison == UNORDERED) {
    ...
    equiv = simplify_gen_ternary(IF_THEN_ELSE, cmp_mode, cmp_mode, temp,
    ...
    if (!FLOAT_LIB_COMPARE_RETURNS_BOOL(mode, comparison)) {
      ...
      equiv = simplify_gen_ternary(IF_THEN_ELSE, cmp_mode, cmp_mode, equiv,
                                   true_rtx, false_rtx);
    }
  }

  start_sequence();
  value = emit_library_call_value(libfunc, NULL_RTX, LCT_CONST, cmp_mode, 2, x,
                                  mode, y, mode);
  insns = get_insns();
  end_sequence();
...
void emit_indirect_jump(rtx loc) {
  if (!insn_data[(int)CODE_FOR_indirect_jump].operand[0].predicate(loc, Pmode))
...
rtx emit_conditional_move(rtx target, enum rtx_code code, rtx op0, rtx op1,
  ...
  if (!insn_data[icode].operand[0].predicate(subtarget,
                                             insn_data[icode].operand[0].mode))
    subtarget = gen_reg_rtx(insn_data[icode].operand[0].mode);

  if (!insn_data[icode].operand[2].predicate(op2,
                                             insn_data[icode].operand[2].mode))
    op2 = copy_to_mode_reg(insn_data[icode].operand[2].mode, op2);

  if (!insn_data[icode].operand[3].predicate(op3,
...
rtx emit_conditional_add(rtx target, enum rtx_code code, rtx op0, rtx op1,
                         enum machine_mode cmode, rtx op2, rtx op3,
                         enum machine_mode mode, int unsignedp) {
  rtx tem, subtarget, comparison, insn;
  enum insn_code icode;
  ...
  if (!insn_data[icode].operand[0].predicate(target,
                                             insn_data[icode].operand[0].mode))
    subtarget = gen_reg_rtx(insn_data[icode].operand[0].mode);
  else
    subtarget = target;
...
enum insn_code can_extend_p(enum machine_mode to_mode,
                            enum machine_mode from_mode, int unsignedp) {
  convert_optab tab;
...
rtx gen_extend_insn(rtx x, rtx y, enum machine_mode mto,
...
static enum insn_code can_fix_p(enum machine_mode fixmode,
                                enum machine_mode fltmode, int unsignedp,
                                int *truncp_ptr) {
  convert_optab tab;
  enum insn_code icode;
...
static enum insn_code can_float_p(enum machine_mode fltmode,
                                  enum machine_mode fixmode, int unsignedp) {
  convert_optab tab;
...
void expand_float(rtx to, rtx from, int unsignedp) {
  ...
  if (unsignedp && can_do_signed) {
    ...
    if (fmode == VOIDmode) {
      ...
      if ((significand_size(fmode) + 1) < GET_MODE_BITSIZE(GET_MODE(from))) {
        ...
        emit_cmp_and_jump_insns(from, const0_rtx, LT, NULL_RTX, imode, 0,
                                neglabel);
        ...
        emit_label(neglabel);
        temp = expand_binop(imode, and_optab, from, const1_rtx, NULL_RTX, 1,
                            OPTAB_LIB_WIDEN);
        ...
        temp = expand_binop(fmode, add_optab, target, target, target, 0,
                            OPTAB_LIB_WIDEN);
        if (temp != target)
          emit_move_insn(target, temp);
    ...
    do_pending_stack_adjust();
    emit_cmp_and_jump_insns(from, const0_rtx, GE, NULL_RTX, GET_MODE(from), 0,
                            label);

    real_2expN(&offset, GET_MODE_BITSIZE(GET_MODE(from)), fmode);
    temp = expand_binop(fmode, add_optab, target,
                        CONST_DOUBLE_FROM_REAL_VALUE(offset, fmode), target, 0,
                        OPTAB_LIB_WIDEN);
    if (temp != target)
      emit_move_insn(target, temp);
  ...
  /* No hardware instruction available; call a library routine.  */
  {
    ...
    value = emit_library_call_value(libfunc, NULL_RTX, LCT_CONST, GET_MODE(to),
    ...
    for (imode = GET_MODE(to); imode != VOIDmode;
      ...
      if (icode != CODE_FOR_nothing) {
        ...
        if (must_trunc) {
          ...
          from = expand_unop(GET_MODE(from), ftrunc_optab, from, temp, 0);
        }
  ...
  if (unsignedp && GET_MODE_BITSIZE(GET_MODE(to)) <= HOST_BITS_PER_WIDE_INT)
    for (fmode = GET_MODE(from); fmode != VOIDmode;
      ...
      if (CODE_FOR_nothing != can_fix_p(GET_MODE(to), fmode, 0, &must_trunc) &&
        ...
        do_pending_stack_adjust();
        emit_cmp_and_jump_insns(from, limit, GE, NULL_RTX, GET_MODE(from), 0,
                                lab1);
        ...
        emit_label(lab1);
        target = expand_binop(GET_MODE(from), sub_optab, from, limit, NULL_RTX,
                              0, OPTAB_LIB_WIDEN);
        expand_fix(to, target, 0);
        target = expand_binop(
            GET_MODE(to), xor_optab, to,
            gen_int_mode((HOST_WIDE_INT)1 << (bitsize - 1), GET_MODE(to)), to,
            1, OPTAB_LIB_WIDEN);
...
void expand_fixed_convert(rtx to, rtx from, int uintp, int satp) {
  ...
  start_sequence();
  value = emit_library_call_value(libfunc, NULL_RTX, LCT_CONST, to_mode, 1,
                                  from, from_mode);
  insns = get_insns();
  end_sequence();

  emit_libcall_block(insns, to, value, gen_rtx_fmt_e(tab->code, to_mode, from));
}
...
static void init_insn_codes(void) {
  unsigned int i;
...
static inline void init_optab(optab op, enum rtx_code code) {
  op->code = code;
...
static inline void init_optabv(optab op, enum rtx_code code) {
  op->code = code;
}

/* Conversion optabs never go in the code_to_optab table.  */
static void init_convert_optab(convert_optab op, enum rtx_code code) {
  op->code = code;
}
...
static void gen_libfunc(optab optable, const char *opname, int suffix,
                        enum machine_mode mode) {
...
static void gen_int_libfunc(optab optable, const char *opname, char suffix,
                            enum machine_mode mode) {
  ...
  if (GET_MODE_CLASS(mode) != MODE_INT || mode < word_mode ||
...
static void gen_fp_libfunc(optab optable, const char *opname, char suffix,
                           enum machine_mode mode) {
  char *dec_opname;
...
static void gen_fixed_libfunc(optab optable, const char *opname, char suffix,
                              enum machine_mode mode) {
...
static void gen_signed_fixed_libfunc(optab optable, const char *opname,
                                     char suffix, enum machine_mode mode) {
...
static void gen_unsigned_fixed_libfunc(optab optable, const char *opname,
                                       char suffix, enum machine_mode mode) {
...
static void gen_int_fp_libfunc(optab optable, const char *name, char suffix,
                               enum machine_mode mode) {
...
static void gen_intv_fp_libfunc(optab optable, const char *name, char suffix,
                                enum machine_mode mode) {
  if (DECIMAL_FLOAT_MODE_P(mode) || GET_MODE_CLASS(mode) == MODE_FLOAT)
    gen_fp_libfunc(optable, name, suffix, mode);
  if (GET_MODE_CLASS(mode) == MODE_INT) {
...
static void gen_int_fp_fixed_libfunc(optab optable, const char *name,
                                     char suffix, enum machine_mode mode) {
  if (DECIMAL_FLOAT_MODE_P(mode) || GET_MODE_CLASS(mode) == MODE_FLOAT)
    gen_fp_libfunc(optable, name, suffix, mode);
  if (INTEGRAL_MODE_P(mode))
...
static void gen_int_fp_signed_fixed_libfunc(optab optable, const char *name,
                                            char suffix,
                                            enum machine_mode mode) {
  if (DECIMAL_FLOAT_MODE_P(mode) || GET_MODE_CLASS(mode) == MODE_FLOAT)
    gen_fp_libfunc(optable, name, suffix, mode);
...
static void gen_int_fixed_libfunc(optab optable, const char *name, char suffix,
                                  enum machine_mode mode) {
  if (INTEGRAL_MODE_P(mode))
    gen_int_libfunc(optable, name, suffix, mode);
  if (ALL_FIXED_POINT_MODE_P(mode))
...
static void gen_int_signed_fixed_libfunc(optab optable, const char *name,
                                         char suffix, enum machine_mode mode) {
  if (INTEGRAL_MODE_P(mode))
    gen_int_libfunc(optable, name, suffix, mode);
  if (SIGNED_FIXED_POINT_MODE_P(mode))
...
static void gen_int_unsigned_fixed_libfunc(optab optable, const char *name,
                                           char suffix,
                                           enum machine_mode mode) {
  if (INTEGRAL_MODE_P(mode))
    gen_int_libfunc(optable, name, suffix, mode);
...
static void gen_interclass_conv_libfunc(convert_optab tab, const char *opname,
...
static void gen_int_to_fp_conv_libfunc(convert_optab tab, const char *opname,
                                       enum machine_mode tmode,
                                       enum machine_mode fmode) {
...
static void gen_ufloat_conv_libfunc(convert_optab tab,
...
static void gen_int_to_fp_nondecimal_conv_libfunc(convert_optab tab,
                                                  const char *opname,
                                                  enum machine_mode tmode,
                                                  enum machine_mode fmode) {
  if (GET_MODE_CLASS(fmode) != MODE_INT)
...
static void gen_fp_to_int_conv_libfunc(convert_optab tab, const char *opname,
                                       enum machine_mode tmode,
                                       enum machine_mode fmode) {
...
static void gen_intraclass_conv_libfunc(convert_optab tab, const char *opname,
                                        enum machine_mode tmode,
                                        enum machine_mode fmode) {
  size_t opname_len = strlen(opname);
  size_t mname_len = 0;
...
static void gen_trunc_conv_libfunc(convert_optab tab, const char *opname,
                                   enum machine_mode tmode,
                                   enum machine_mode fmode) {
  if (GET_MODE_CLASS(tmode) != MODE_FLOAT && !DECIMAL_FLOAT_MODE_P(tmode))
    return;
...
static void gen_extend_conv_libfunc(convert_optab tab,
                                    const char *opname ATTRIBUTE_UNUSED,
                                    enum machine_mode tmode,
                                    enum machine_mode fmode) {
  if (GET_MODE_CLASS(tmode) != MODE_FLOAT && !DECIMAL_FLOAT_MODE_P(tmode))
...
static void gen_fract_conv_libfunc(convert_optab tab, const char *opname,
                                   enum machine_mode tmode,
                                   enum machine_mode fmode) {
  if (tmode == fmode)
    return;
...
static void gen_fractuns_conv_libfunc(convert_optab tab, const char *opname,
                                      enum machine_mode tmode,
                                      enum machine_mode fmode) {
  if (tmode == fmode)
    return;
  ...
  if (!((ALL_FIXED_POINT_MODE_P(tmode) && GET_MODE_CLASS(fmode) == MODE_INT) ||
        (ALL_FIXED_POINT_MODE_P(fmode) && GET_MODE_CLASS(tmode) == MODE_INT)))
    return;
...
static void gen_satfract_conv_libfunc(convert_optab tab, const char *opname,
                                      enum machine_mode tmode,
                                      enum machine_mode fmode) {
  if (tmode == fmode)
    return;
...
static void gen_satfractuns_conv_libfunc(convert_optab tab, const char *opname,
                                         enum machine_mode tmode,
                                         enum machine_mode fmode) {
  if (tmode == fmode)
    return;
...
static hashval_t libfunc_decl_hash(const void *entry) {
  return htab_hash_string(IDENTIFIER_POINTER(DECL_NAME((const_tree)entry)));
}

static int libfunc_decl_eq(const void *entry1, const void *entry2) {
...
rtx init_one_libfunc(const char *name) {
  ...
  if (libfunc_decls == NULL)
    libfunc_decls =
        htab_create_ggc(37, libfunc_decl_hash, libfunc_decl_eq, NULL);
...
void set_optab_libfunc(optab optable, enum machine_mode mode,
                       const char *name) {
  rtx val;
  struct libfunc_entry e;
  struct libfunc_entry **slot;
...
void set_conv_libfunc(convert_optab optable, enum machine_mode tmode,
...
void init_optabs(void) {
  ...
  unwind_sjlj_unregister_libfunc = init_one_libfunc("_Unwind_SjLj_Unregister");

  /* For function entry/exit instrumentation.  */
  profile_function_entry_libfunc = init_one_libfunc("__cyg_profile_func_enter");
  profile_function_exit_libfunc = init_one_libfunc("__cyg_profile_func_exit");
      ...
      for (k = 0; k < NUM_MACHINE_MODES; ++k) {
        ...
        if (l) {
          ...
          fprintf(stderr, "%s\t%s\t%s:\t%s\n", GET_RTX_NAME(o->code),
                  GET_MODE_NAME(j), GET_MODE_NAME(k), XSTR(l, 0));
        }
      }
}
...
rtx gen_cond_trap(enum rtx_code code ATTRIBUTE_UNUSED, rtx op1,
...
static inline enum insn_code get_vcond_icode(tree expr,
                                             enum machine_mode mode) {
...
rtx expand_vec_cond_expr(tree vec_cond_expr, rtx target) {
  ...
  comparison =
      vector_compare_rtx(TREE_OPERAND(vec_cond_expr, 0), unsignedp, icode);
  cc_op0 = XEXP(comparison, 0);
  cc_op1 = XEXP(comparison, 1);
  /* Expand both operands and force them in reg, if required.  */
  rtx_op1 = expand_normal(TREE_OPERAND(vec_cond_expr, 1));
  if (!insn_data[icode].operand[1].predicate(rtx_op1, mode) && mode != VOIDmode)
    rtx_op1 = force_reg(mode, rtx_op1);

  rtx_op2 = expand_normal(TREE_OPERAND(vec_cond_expr, 2));
  if (!insn_data[icode].operand[2].predicate(rtx_op2, mode) && mode != VOIDmode)
    rtx_op2 = force_reg(mode, rtx_op2);

  /* Emit instruction! */
  emit_insn(
      GEN_FCN(icode)(target, rtx_op1, rtx_op2, comparison, cc_op0, cc_op1));
...
static rtx expand_val_compare_and_swap_1(rtx mem, rtx old_val, rtx new_val,
...
rtx expand_val_compare_and_swap(rtx mem, rtx old_val, rtx new_val, rtx target) {
  enum machine_mode mode = GET_MODE(mem);
...
rtx expand_bool_compare_and_swap(rtx mem, rtx old_val, rtx new_val,
                                 rtx target) {
  enum machine_mode mode = GET_MODE(mem);
  enum insn_code icode;
  ...
  switch (icode) {
    default:
      subtarget =
          expand_val_compare_and_swap_1(mem, old_val, new_val, NULL_RTX, icode);
...
static bool expand_compare_and_swap_loop(rtx mem, rtx old_reg, rtx new_reg,
                                         rtx seq) {
  enum machine_mode mode = GET_MODE(mem);
  enum insn_code icode;
  ...
  switch (icode) {
    default:
      subtarget =
          expand_val_compare_and_swap_1(mem, old_reg, new_reg, cmp_reg, icode);
      if (subtarget != NULL_RTX) {
...
rtx expand_sync_operation(rtx mem, rtx val, enum rtx_code code) {
  enum machine_mode mode = GET_MODE(mem);
  enum insn_code icode;
  rtx insn;
  ...
  if (sync_compare_and_swap[mode] != CODE_FOR_nothing) {
    ...
    t1 = t0;
    if (code == NOT) {
      t1 = expand_simple_binop(mode, AND, t1, val, NULL_RTX, true,
                               OPTAB_LIB_WIDEN);
...
rtx expand_sync_fetch_operation(rtx mem, rtx val, enum rtx_code code,
  ...
  if (icode != CODE_FOR_nothing) {
    ...
    if (insn) {
      ...
      if (compensate) {
        ...
        if (code == NOT) {
          target = expand_simple_binop(mode, AND, target, val, NULL_RTX, true,
                                       OPTAB_LIB_WIDEN);
          target = expand_simple_unop(mode, code, target, NULL_RTX, true);
        } else
          target = expand_simple_binop(mode, code, target, val, NULL_RTX, true,
                                       OPTAB_LIB_WIDEN);
      }
...
rtx expand_sync_lock_test_and_set(rtx mem, rtx val, rtx target) {
  enum machine_mode mode = GET_MODE(mem);
  enum insn_code icode;
  rtx insn;
