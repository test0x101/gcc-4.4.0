...
void ipa_utils_print_order(FILE *out, const char *note,
                           struct cgraph_node **order, int count) {
  int i;
...
static void searchc(struct searchc_env *env, struct cgraph_node *v) {
  ...
  for (edge = v->callees; edge; edge = edge->next_callee) {
    ...
    if (w->aux &&
        cgraph_function_body_availability(edge->callee) > AVAIL_OVERWRITABLE) {
      w_info = (struct ipa_dfs_info *)w->aux;
      if (w_info->new_node) {
        searchc(env, w);
        v_info->low_link = (v_info->low_link < w_info->low_link)
                               ? v_info->low_link
                               : w_info->low_link;
      } else if ((w_info->dfn_number < v_info->dfn_number) &&
                 (w_info->on_stack))
        v_info->low_link = (w_info->dfn_number < v_info->low_link)
                               ? w_info->dfn_number
                               : v_info->low_link;
    }
  }
...
int ipa_utils_reduced_inorder(struct cgraph_node **order, bool reduce,
  ...
  for (node = cgraph_nodes; node; node = node->next) {
    ...
    if (avail > AVAIL_OVERWRITABLE ||
        (allow_overwritable && (avail == AVAIL_OVERWRITABLE))) {
      ...
      splay_tree_insert(env.nodes_marked_new, (splay_tree_key)node->uid,
...
tree get_base_var(tree t) {
  ...
  while (!SSA_VAR_P(t) && (!CONSTANT_CLASS_P(t)) &&
         TREE_CODE(t) != LABEL_DECL && TREE_CODE(t) != FUNCTION_DECL &&
