...
static char *affix_data_type(const char *param) {
  ...
  *p = saved;
  return reconcat(qualifiers_then_data_type, qualifiers_then_data_type, " ", p,
                  NULL);
}
...
static const char *gen_type(const char *ret_val, tree t, formals_style style) {
  ...
  if (TYPE_NAME(t) && TREE_CODE(TYPE_NAME(t)) == TYPE_DECL)
    ...
    switch (TREE_CODE(t)) {
      ...
      case FUNCTION_TYPE:
        ret_val =
            gen_type(concat(ret_val, gen_formal_list_for_type(t, style), NULL),
                     TREE_TYPE(t), style);
        break;
...
static const char *gen_decl(tree decl, int is_func_definition,
                            formals_style style) {
  const char *ret_val;
...
void gen_aux_info_record(tree fndecl, int is_definition, int is_implicit,
                         int is_prototyped) {
  if (flag_gen_aux_info) {
    ...
    fprintf(aux_info_file, "/* %s:%d:%c%c */ %s;", xloc.file, xloc.line,
            (is_implicit)     ? 'I'
            : (is_prototyped) ? 'N'
                              : 'O',
