...
char *file_name_as_prefix(const char *f) {
...
void diagnostic_set_info_translated(diagnostic_info *diagnostic,
                                    ...
                                    location_t location, diagnostic_t kind) {
...
/* Initialize DIAGNOSTIC, where the message GMSGID has not yet been
   translated.  */
void diagnostic_set_info(diagnostic_info *diagnostic, const char *gmsgid,
...
#undef DEFINE_DIAGNOSTIC_KIND
  ...
  return (
      s.file == NULL ? build_message_string("%s: %s", progname, text)
...
void diagnostic_report_current_module(diagnostic_context *context) {
  ...
  if (map && diagnostic_last_module_changed(context, map)) {
    ...
    if (!MAIN_FILE_P(map)) {
      ...
      pp_verbatim(context->printer, "In file included from %s:%d", map->to_file,
...
diagnostic_t diagnostic_classify_diagnostic(diagnostic_context *context,
  ...
  if (option_index <= 0 || option_index >= N_OPTS ||
...
bool diagnostic_report_diagnostic(diagnostic_context *context,
  ...
  /* If the user requested that warnings be treated as errors, so be
     ...
     -Wno-error=*.  */
  if (context->warning_as_error_requested && diagnostic->kind == DK_WARNING) {
  ...
  if (diagnostic->option_index) {
    ...
    /* This tests if the user provided the appropriate -Werror=foo
       option.  */
    if (context->classify_diagnostic[diagnostic->option_index] !=
        DK_UNSPECIFIED) {
  ...
  if (context->issue_warnings_are_errors_message &&
    ...
    pp_verbatim(context->printer, "%s: warnings being treated as errors\n",
                progname);
...
bool emit_diagnostic(diagnostic_t kind, location_t location, int opt,
                     const char *gmsgid, ...) {
  diagnostic_info diagnostic;
  va_list ap;
...
void fatal_error(const char *gmsgid, ...) {
  diagnostic_info diagnostic;
  va_list ap;
