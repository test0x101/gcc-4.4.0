...
static void alloc_mem(void) {
  ...
  /* Allocate the available expressions hash table.  We don't want to
     ...
     reasonable choice.  */
  expr_table =
      htab_create(MAX(i / 4, 13), hash_expr_for_htab, expr_equiv_p, NULL);
  ...
  modifies_mem_obstack_bottom = (struct modifies_mem *)obstack_alloc(
      &modifies_mem_obstack, sizeof(struct modifies_mem));
}
...
static hashval_t hash_expr(rtx x, int *do_not_record_p) {
  *do_not_record_p = 0;
  return hash_rtx(x, GET_MODE(x), do_not_record_p, NULL,
                  /*have_reg_qty=*/false);
}
...
static hashval_t hash_expr_for_htab(const void *expp) {
...
static int expr_equiv_p(const void *exp1p, const void *exp2p) {
...
static void insert_expr_in_table(rtx x, rtx insn) {
  int do_not_record_p;
  hashval_t hash;
  ...
  cur_expr = (struct expr *)obstack_alloc(&expr_obstack, sizeof(struct expr));
  cur_expr->expr = x;
  cur_expr->hash = hash;
  cur_expr->avail_occr = NULL;

  slot = (struct expr **)htab_find_slot_with_hash(expr_table, cur_expr, hash,
                                                  INSERT);
  ...
  if (avail_occr)
    ...
    avail_occr =
        (struct occr *)obstack_alloc(&occr_obstack, sizeof(struct occr));
...
static struct expr *lookup_expr_in_table(rtx pat) {
  int do_not_record_p;
  ...
  tmp_expr = (struct expr *)obstack_alloc(&expr_obstack, sizeof(struct expr));
  tmp_expr->expr = pat;
  tmp_expr->hash = hash;
  tmp_expr->avail_occr = NULL;

  slot = (struct expr **)htab_find_slot_with_hash(expr_table, tmp_expr, hash,
                                                  INSERT);
...
static int dump_hash_table_entry(void **slot, void *filep) {
...
static void dump_hash_table(FILE *file) {
  ...
  fprintf(file, "size %ld, %ld elements, %f collision/search ratio\n",
          (long)htab_size(expr_table), (long)htab_elements(expr_table),
...
static bool oprs_unchanged_p(rtx x, rtx insn, bool after_insn) {
  int i, j;
  enum rtx_code code;
  const char *fmt;
...
static void find_mem_conflicts(rtx dest, const_rtx setter ATTRIBUTE_UNUSED,
                               void *data) {
  rtx mem_op = (rtx)data;
  ...
  if (true_dependence(dest, GET_MODE(dest), mem_op, rtx_addr_varies_p))
    mems_conflict_p = 1;
}
...
static int load_killed_in_block_p(int uid_limit, rtx x, bool after_insn) {
  ...
  while (list_entry) {
    ...
    if ((after_insn && INSN_CUID(setter) < uid_limit) ||
        (!after_insn && INSN_CUID(setter) > uid_limit)) {
...
static inline void record_last_reg_set_info(rtx insn, rtx reg) {
  unsigned int regno, end_regno;
...
static inline void record_last_reg_set_info_regno(rtx insn, int regno) {
  reg_avail_info[regno] = INSN_CUID(insn);
}
...
static void record_opr_changes(rtx insn) {
  rtx note;
...
static void hash_scan_set(rtx insn) {
...
static bool reg_used_on_edge(rtx reg, edge e) {
  rtx insn;
...
static rtx get_avail_load_store_reg(rtx insn) {
...
static bool bb_has_well_behaved_predecessors(basic_block bb) {
  edge pred;
  edge_iterator ei;
...
static struct occr *get_bb_avail_insn(basic_block bb, struct occr *occr) {
...
static void eliminate_partially_redundant_load(basic_block bb, rtx insn,
                                               struct expr *expr) {
  edge pred;
  ...
  FOR_EACH_EDGE(pred, ei, bb->preds) {
    ...
    for (a_occr = get_bb_avail_insn(pred_bb, expr->avail_occr); a_occr;
      ...
      if (!constrain_operands(1) || reg_killed_on_edge(avail_reg, pred) ||
    ...
    if (avail_insn != NULL_RTX) {
      ...
      ok_count += pred->count;
      if (!set_noop_p(
              PATTERN(gen_move_insn(copy_rtx(dest), copy_rtx(avail_reg))))) {
  ...
  for (occr = avail_occrs; occr; occr = occr->next) {
    ...
    insert_insn_on_edge(gen_move_insn(copy_rtx(dest), copy_rtx(avail_reg)),
                        pred);
    stats.moves_inserted++;

    if (dump_file)
      fprintf(
          dump_file, "generating move from %d to %d on edge from %d to %d\n",
  ...
  for (unoccr = unavail_occrs; unoccr; unoccr = unoccr->next) {
    ...
    if (dump_file) {
      fprintf(dump_file, "generating on edge from %d to %d a copy of load: ",
  ...
  FOR_BB_BETWEEN(bb, ENTRY_BLOCK_PTR->next_bb->next_bb, EXIT_BLOCK_PTR,
    ...
    FOR_BB_INSNS(bb, insn) {
      ...
      if (NONJUMP_INSN_P(insn) && GET_CODE(PATTERN(insn)) == SET &&
          REG_P(SET_DEST(PATTERN(insn))) && MEM_P(SET_SRC(PATTERN(insn)))) {
        rtx pat = PATTERN(insn);
        rtx src = SET_SRC(pat);
        struct expr *expr;

        if (!MEM_VOLATILE_P(src) && GET_MODE(src) != BLKmode &&
            general_operand(src, GET_MODE(src))
...
struct rtl_opt_pass pass_gcse2 = {{
    ...
    0,                    /* todo_flags_start */
    TODO_dump_func | TODO_verify_rtl_sharing | TODO_verify_flow |
