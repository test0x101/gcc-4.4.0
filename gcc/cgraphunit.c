   ...
   The front-end is supposed to use following functionality:
    ...
    - analyze_expr callback
      ...
      need for these hooks (replacing them by genericizing hook)
        ...
        Finally the call-graph is topologically sorted and all reachable
   functions that has not been completely inlined or are not external are
   output.
...
static void record_cdtor_fn(tree fndecl) {
  struct cgraph_node *node;
  if (targetm.have_ctors_dtors ||
      (!DECL_STATIC_CONSTRUCTOR(fndecl) && !DECL_STATIC_DESTRUCTOR(fndecl)))
    return;
...
static void build_cdtor(bool ctor_p, tree *cdtors, size_t len) {
  size_t i;
  ...
  while (i < len) {
    ...
    do {
      ...
      if (!body)
        ...
        break;
      append_to_statement_list(build_function_call_expr(fn, 0), &body);
      ++i;
    } while (i < len);
...
static void cgraph_build_cdtor_fns(void) {
  if (!VEC_empty(tree, static_ctors)) {
    gcc_assert(!targetm.have_ctors_dtors);
    qsort(VEC_address(tree, static_ctors), VEC_length(tree, static_ctors),
          sizeof(tree), compare_ctor);
    build_cdtor(/*ctor_p=*/true, VEC_address(tree, static_ctors),
  ...
  if (!VEC_empty(tree, static_dtors)) {
    gcc_assert(!targetm.have_ctors_dtors);
    qsort(VEC_address(tree, static_dtors), VEC_length(tree, static_dtors),
          sizeof(tree), compare_dtor);
    build_cdtor(/*ctor_p=*/false, VEC_address(tree, static_dtors),
...
static bool decide_is_function_needed(struct cgraph_node *node, tree decl) {
  if (MAIN_NAME_P(DECL_NAME(decl)) && TREE_PUBLIC(decl)) {
  ...
  if (flag_keep_inline_functions && DECL_DECLARED_INLINE_P(decl) &&
      !DECL_EXTERNAL(decl) &&
  ...
  if (((TREE_PUBLIC(decl) ||
        (!optimize && !node->local.disregard_inline_limits &&
         !DECL_DECLARED_INLINE_P(decl) && !node->origin)) &&
       !flag_whole_program) &&
...
void cgraph_mark_if_needed(tree decl) {
  struct cgraph_node *node = cgraph_node(decl);
...
void verify_cgraph_node(struct cgraph_node *node) {
  ...
  for (e = node->callees; e; e = e->next_callee)
    if (e->aux) {
      error("aux field set for edge %s->%s", cgraph_node_name(e->caller),
  ...
  for (e = node->callers; e; e = e->next_caller) {
    ...
    if (!e->inline_failed) {
      if (node->global.inlined_to != (e->caller->global.inlined_to
  ...
  if (node->analyzed && !TREE_ASM_WRITTEN(node->decl) &&
...
static void process_function_and_variable_attributes(
    struct cgraph_node *first, struct varpool_node *first_var) {
  struct cgraph_node *node;
    ...
    if (lookup_attribute("externally_visible", DECL_ATTRIBUTES(decl))) {
      if (!TREE_PUBLIC(node->decl))
        warning(OPT_Wattributes,
                "%J%<externally_visible%> attribute have effect only on public "
                "objects",
                node->decl);
      else {
        if (node->local.finalized)
...
static void cgraph_analyze_functions(void) {
  ...
  process_function_and_variable_attributes(first_processed, first_analyzed_var);
...
static void cgraph_mark_functions_to_output(void) {
  ...
  for (node = cgraph_nodes; node; node = node->next) {
    ...
    if (node->analyzed && !node->global.inlined_to &&
        (node->needed || (e && node->reachable)) && !TREE_ASM_WRITTEN(decl) &&
        !DECL_EXTERNAL(decl))
      node->output = 1;
    else {
      /* We should've reclaimed all functions that are not needed.  */
#ifdef ENABLE_CHECKING
      if (!node->global.inlined_to && gimple_has_body_p(decl) &&
          !DECL_EXTERNAL(decl)) {
        dump_cgraph_node(stderr, node);
        internal_error("failed to reclaim unneeded function");
      }
#endif
      gcc_assert(node->global.inlined_to || !gimple_has_body_p(decl) ||
                 DECL_EXTERNAL(decl));
    }
  }
}
...
static void cgraph_expand_function(struct cgraph_node *node) {
  tree decl = node->decl;
...
void cgraph_optimize(void) {
  ...
  if (!(sorrycount || errorcount)) {
    ...
    for (node = cgraph_nodes; node; node = node->next)
      if (node->analyzed &&
          (node->global.inlined_to || gimple_has_body_p(node->decl))) {
        error_found = true;
        dump_cgraph_node(stderr, node);
      }
    if (error_found)
...
static struct cgraph_node *cgraph_copy_node_for_versioning(
    struct cgraph_node *old_version, tree new_decl,
...
struct cgraph_node *cgraph_function_versioning(
    struct cgraph_node *old_version_node,
    VEC(cgraph_edge_p, heap) * redirect_callers, varray_type tree_map,
    bitmap args_to_skip) {
  ...
  new_version_node = cgraph_copy_node_for_versioning(old_version_node, new_decl,
                                                     redirect_callers);
