...
static struct csa_memlist *record_one_stack_memref(rtx, rtx *,
                                                   struct csa_memlist *);
static int try_apply_stack_adjustment(rtx, struct csa_memlist *, HOST_WIDE_INT,
                                      HOST_WIDE_INT);
...
static int stack_memref_p(rtx x) {
  if (!MEM_P(x))
    return 0;
  x = XEXP(x, 0);
  ...
  if (x == stack_pointer_rtx)
    return 1;
  if (GET_CODE(x) == PLUS && XEXP(x, 0) == stack_pointer_rtx &&
...
static rtx single_set_for_csa(rtx insn) {
  int i;
  ...
  for (i = 1; i < XVECLEN(tmp, 0); ++i) {
    ...
    if (GET_CODE(this_rtx) == SET && SET_SRC(this_rtx) == SET_DEST(this_rtx))
      ;
    else if (GET_CODE(this_rtx) != CLOBBER && GET_CODE(this_rtx) != USE)
      return NULL_RTX;
  }
...
static void free_csa_memlist(struct csa_memlist *memlist) {
  struct csa_memlist *next;
...
static struct csa_memlist *record_one_stack_memref(
    rtx insn, rtx *mem, struct csa_memlist *next_memlist) {
...
static int try_apply_stack_adjustment(rtx insn, struct csa_memlist *memlist,
  ...
  for (ml = memlist; ml; ml = ml->next)
    validate_change(
        ...
        replace_equiv_address_nv(
            *ml->mem, plus_constant(stack_pointer_rtx, ml->sp_offset - delta)),
        1);
...
static int record_stack_memrefs(rtx *xp, void *data) {
  rtx x = *xp;
...
static void adjust_frame_related_expr(rtx last_sp_set, rtx insn,
  ...
  if (note && GET_CODE(XEXP(note, 0)) == SEQUENCE &&
    ...
    new_expr = gen_rtx_SEQUENCE(VOIDmode, rtvec_alloc(XVECLEN(expr, 0) + 1));
  ...
  if (note)
    ...
    REG_NOTES(last_sp_set) = gen_rtx_EXPR_LIST(REG_FRAME_RELATED_EXPR, new_expr,
                                               REG_NOTES(last_sp_set));
}

/* Subroutine of combine_stack_adjustments, called for each basic block.  */

static void combine_stack_adjustments_for_block(basic_block bb) {
  ...
  for (insn = BB_HEAD(bb); !end_of_block; insn = next) {
    ...
    if (set) {
      ...
      if (dest == stack_pointer_rtx && GET_CODE(src) == PLUS &&
        ...
        if (STACK_GROWS_DOWNWARD ? this_adjust <= 0 : this_adjust >= 0) {
          if (try_apply_stack_adjustment(last_sp_set, memlist,
            ...
            if (RTX_FRAME_RELATED_P(last_sp_set))
              adjust_frame_related_expr(last_sp_set, insn, this_adjust);
            /* It worked!  */
            delete_insn(insn);
            last_sp_adjust += this_adjust;
            continue;
        ...
        else if (STACK_GROWS_DOWNWARD ? last_sp_adjust >= 0
                                      : last_sp_adjust <= 0) {
      ...
      if (memlist == NULL && MEM_P(dest) &&
          ((GET_CODE(XEXP(dest, 0)) == PRE_DEC &&
            (last_sp_adjust == (HOST_WIDE_INT)GET_MODE_SIZE(GET_MODE(dest)))) ||
           (GET_CODE(XEXP(dest, 0)) == PRE_MODIFY &&
            GET_CODE(XEXP(XEXP(dest, 0), 1)) == PLUS &&
            XEXP(XEXP(XEXP(dest, 0), 1), 0) == stack_pointer_rtx &&
            (GET_CODE(XEXP(XEXP(XEXP(dest, 0), 1), 1)) == CONST_INT) &&
            (INTVAL(XEXP(XEXP(XEXP(dest, 0), 1), 1)) == -last_sp_adjust))) &&
          XEXP(XEXP(dest, 0), 0) == stack_pointer_rtx &&
          !reg_mentioned_p(stack_pointer_rtx, src) &&
          memory_address_p(GET_MODE(dest), stack_pointer_rtx) &&
          validate_change(insn, &SET_DEST(set),
                          replace_equiv_address(dest, stack_pointer_rtx), 0)) {
        delete_insn(last_sp_set);
        free_csa_memlist(memlist);
        memlist = NULL;
    ...
    if (last_sp_set &&
        (CALL_P(insn) || reg_mentioned_p(stack_pointer_rtx, PATTERN(insn)))) {
      if (last_sp_set && last_sp_adjust == 0)
        delete_insn(last_sp_set);
      free_csa_memlist(memlist);
      memlist = NULL;
...
static bool gate_handle_stack_adjustments(void) {
  return (optimize > 0);
}

static unsigned int rest_of_handle_stack_adjustments(void) {
