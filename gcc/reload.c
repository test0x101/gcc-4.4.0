...
#define CONST_POOL_OK_P(X) \
  (CONSTANT_P(X) && GET_CODE(X) != HIGH && !targetm.cannot_force_const_mem(X))
...
static struct replacement
    replacements[MAX_RECOG_OPERANDS * ((MAX_REGS_PER_ADDRESS * 2) + 1)];
...
/* Compare two RTX's.  */
#define MATCHES(x, y)                                                \
  (x == y || (x != 0 && (REG_P(x) ? REG_P(y) && REGNO(x) == REGNO(y) \
...
#define MERGE_TO_OTHER(when1, when2, op1, op2)                              \
  ((when1) != (when2) || !((op1) == (op2) || (when1) == RELOAD_FOR_INPUT || \
                           (when1) == RELOAD_FOR_OPERAND_ADDRESS ||         \
                           (when1) == RELOAD_FOR_OTHER_ADDRESS))
...
#define ADDR_TYPE(type)                                                    \
  ((type) == RELOAD_FOR_INPUT_ADDRESS                                      \
       ...
       : ((type) == RELOAD_FOR_OUTPUT_ADDRESS ? RELOAD_FOR_OUTADDR_ADDRESS \
                                              : (type)))
...
static int find_reusable_reload(rtx *, rtx, enum reg_class, enum reload_type,
                                int, int);
...
static int find_reloads_address(enum machine_mode, rtx *, rtx, rtx *, int,
                                enum reload_type, int, rtx);
static rtx subst_reg_equivs(rtx, rtx);
static rtx subst_indexed_address(rtx);
static void update_auto_inc_notes(rtx, int, int);
static int find_reloads_address_1(enum machine_mode, rtx, int, enum rtx_code,
                                  enum rtx_code, rtx *, int, enum reload_type,
                                  int, rtx);
static void find_reloads_address_part(rtx, rtx *, enum reg_class,
                                      enum machine_mode, int, enum reload_type,
                                      int);
static rtx find_reloads_subreg_address(rtx, int, int, enum reload_type, int,
                                       rtx);
...
static void push_reg_equiv_alt_mem(int regno, rtx mem) {
  ...
  reg_equiv_alt_mem_list[regno] =
      alloc_EXPR_LIST(REG_EQUIV, mem, reg_equiv_alt_mem_list[regno]);
}
...
static int push_secondary_reload(int in_p, rtx x, int opnum, int optional,
                                 ...
                                 enum reload_type type, enum insn_code *picode,
  ...
  *picode = CODE_FOR_nothing;
  ...
  for (s_reload = 0; s_reload < n_reloads; s_reload++)
    if (rld[s_reload].secondary_p &&
      ...
      if (MERGE_TO_OTHER(secondary_type, rld[s_reload].when_needed, opnum,
                         rld[s_reload].opnum))
...
enum reg_class secondary_reload_class(bool in_p, enum reg_class rclass,
...
rtx get_secondary_mem(rtx x ATTRIBUTE_UNUSED, enum machine_mode mode, int opnum,
  ...
  if (!mem_valid) {
    ...
    find_reloads_address(mode, &loc, XEXP(loc, 0), &XEXP(loc, 0), opnum, type,
                         0, 0);
  }
...
static enum reg_class find_valid_class(
    enum machine_mode outer ATTRIBUTE_UNUSED,
...
static int find_reusable_reload(rtx *p_in, rtx out, enum reg_class rclass,
                                enum reload_type type, int opnum,
                                int dont_share) {
  rtx in = *p_in;
  int i;
  ...
  for (i = 0; i < n_reloads; i++)
    if ((reg_class_subset_p(rclass, rld[i].rclass) ||
        ...
        ((REG_P(in) && GET_RTX_CLASS(GET_CODE(rld[i].in)) == RTX_AUTOINC &&
          MATCHES(XEXP(rld[i].in, 0), in)) ||
         (REG_P(rld[i].in) && GET_RTX_CLASS(GET_CODE(in)) == RTX_AUTOINC &&
...
static int reload_inner_reg_of_subreg(rtx x, enum machine_mode mode,
                                      int output) {
  rtx inner;
  ...
  return (GET_MODE_SIZE(mode) <= UNITS_PER_WORD && output &&
...
int push_reload(rtx in, rtx out, rtx *inloc, rtx *outloc, enum reg_class rclass,
                enum machine_mode inmode, enum machine_mode outmode,
                int strict_low, int optional, int opnum,
                enum reload_type type) {
  int i;
  int dont_share = 0;
  ...
  if (in != 0 && GET_CODE(in) == SUBREG && (subreg_lowpart_p(in) || strict_low)
#ifdef CANNOT_CHANGE_MODE_CLASS
      && !CANNOT_CHANGE_MODE_CLASS(GET_MODE(SUBREG_REG(in)), inmode, rclass)
#endif
      &&
      (CONSTANT_P(SUBREG_REG(in)) || GET_CODE(SUBREG_REG(in)) == PLUS ||
       strict_low ||
       (((REG_P(SUBREG_REG(in)) &&
        ...
        ((GET_MODE_SIZE(inmode) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(in))))
         ...
         ((GET_MODE_SIZE(inmode) < GET_MODE_SIZE(GET_MODE(SUBREG_REG(in)))) &&
          ((GET_MODE_SIZE(inmode) - 1) / UNITS_PER_WORD ==
           ((GET_MODE_SIZE(GET_MODE(SUBREG_REG(in))) - 1) / UNITS_PER_WORD)))
#endif
             )) ||
       (REG_P(SUBREG_REG(in)) &&
        REGNO(SUBREG_REG(in)) < FIRST_PSEUDO_REGISTER
       ...
       || (REG_P(SUBREG_REG(in)) &&
           REGNO(SUBREG_REG(in)) < FIRST_PSEUDO_REGISTER &&
           REG_CANNOT_CHANGE_MODE_P(REGNO(SUBREG_REG(in)),
  ...
  if (in != 0 && reload_inner_reg_of_subreg(in, inmode, 0)) {
    ...
    if (REG_P(SUBREG_REG(in)))
      in_class = find_valid_class(
          inmode, GET_MODE(SUBREG_REG(in)),
          subreg_regno_offset(REGNO(SUBREG_REG(in)), GET_MODE(SUBREG_REG(in)),
    ...
    push_reload(SUBREG_REG(in), NULL_RTX, &SUBREG_REG(in), (rtx *)0, in_class,
        ...
        ((GET_MODE_SIZE(outmode) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(out))))
         ...
         ((GET_MODE_SIZE(outmode) < GET_MODE_SIZE(GET_MODE(SUBREG_REG(out)))) &&
          ((GET_MODE_SIZE(outmode) - 1) / UNITS_PER_WORD ==
           ((GET_MODE_SIZE(GET_MODE(SUBREG_REG(out))) - 1) / UNITS_PER_WORD)))
#endif
             )) ||
       (REG_P(SUBREG_REG(out)) &&
        REGNO(SUBREG_REG(out)) < FIRST_PSEUDO_REGISTER &&
  ...
  if (out != 0 && reload_inner_reg_of_subreg(out, outmode, 1)) {
    ...
    dont_remove_subreg = 1;
    push_reload(
        SUBREG_REG(out), SUBREG_REG(out), &SUBREG_REG(out), &SUBREG_REG(out),
        find_valid_class(outmode, GET_MODE(SUBREG_REG(out)),
                         subreg_regno_offset(REGNO(SUBREG_REG(out)),
                                             GET_MODE(SUBREG_REG(out)),
                                             SUBREG_BYTE(out), GET_MODE(out)),
                         REGNO(SUBREG_REG(out))),
        VOIDmode, VOIDmode, 0, 0, opnum, RELOAD_OTHER);
  }
  ...
  /* Similarly for OUT.  */
  if (out != 0 && GET_CODE(out) == SUBREG && REG_P(SUBREG_REG(out)) &&
     ...
     desirable on this machine for efficiency.  */
  {
    ...
    if (preferred_class != NO_REGS || (optional && type == RELOAD_FOR_OUTPUT))
  ...
  gcc_assert(rclass != NO_REGS || (optional != 0 && type == RELOAD_FOR_OUTPUT));
  ...
  if (i == n_reloads) {
    ...
    if (out != 0 && GET_CODE(out) != SCRATCH)
      secondary_out_reload =
          push_secondary_reload(0, out, opnum, optional, rclass, outmode, type,
                                &secondary_out_icode, NULL);
    ...
    if (in != 0 &&
        (REG_P(in) || (GET_CODE(in) == SUBREG && REG_P(SUBREG_REG(in)))) &&
        reg_or_subregno(in) < FIRST_PSEUDO_REGISTER &&
        SECONDARY_MEMORY_NEEDED(REGNO_REG_CLASS(reg_or_subregno(in)), rclass,
                                inmode))
...
#ifdef SECONDARY_MEMORY_NEEDED
    if (out != 0 &&
        (REG_P(out) || (GET_CODE(out) == SUBREG && REG_P(SUBREG_REG(out)))) &&
        reg_or_subregno(out) < FIRST_PSEUDO_REGISTER &&
        SECONDARY_MEMORY_NEEDED(rclass, REGNO_REG_CLASS(reg_or_subregno(out)),
                                outmode))
    ...
    if (in != 0) {
      ...
      /* When emitting reloads we don't necessarily look at the in-
         ...
         See also PR33613.  */
      if (!rld[i].in ||
          GET_MODE_SIZE(GET_MODE(in)) > GET_MODE_SIZE(GET_MODE(rld[i].in)))
        rld[i].in = in;
      if (!rld[i].in_reg ||
          (in_reg && GET_MODE_SIZE(GET_MODE(in_reg)) >
                         GET_MODE_SIZE(GET_MODE(rld[i].in_reg))))
        rld[i].in_reg = in_reg;
    }
    if (out != 0) {
      if (!rld[i].out || (out && GET_MODE_SIZE(GET_MODE(out)) >
                                     GET_MODE_SIZE(GET_MODE(rld[i].out))))
        rld[i].out = out;
      if (outloc &&
          (!rld[i].out_reg || GET_MODE_SIZE(GET_MODE(*outloc)) >
  ...
  if (in != 0 && out != 0 && in != out && rld[i].reg_rtx == 0) {
    rld[i].reg_rtx =
        find_dummy_reload(in, out, inloc, outloc, inmode, outmode,
                          rld[i].rclass, i, earlyclobber_operand_p(out));

    /* If the outgoing register already contains the same value
       as the incoming one, we can dispense with loading it.
       The easiest way to tell the caller that is to give a phony
       value for the incoming operand (same as outgoing one).  */
    if (rld[i].reg_rtx == out && (REG_P(in) || CONSTANT_P(in)) &&
        0 != find_equiv_reg(in, this_insn, 0, REGNO(out), static_reload_reg_p,
                            i, inmode))
      rld[i].in = out;
  }
  ...
  if (rld[i].reg_rtx == 0 && in != 0 && hard_regs_live_known) {
    ...
    for (note = REG_NOTES(this_insn); note; note = XEXP(note, 1))
      if (REG_NOTE_KIND(note) == REG_DEAD && REG_P(XEXP(note, 0)) &&
          ...
          !refers_to_regno_for_reload_p(regno, end_hard_regno(rel_mode, regno),
                                        PATTERN(this_insn), inloc)
          /* If this is also an output reload, IN cannot be used as
             the reload register if it is set in this insn unless IN
             is also OUT.  */
          && (out == 0 || in == out ||
              !hard_reg_set_here_p(regno, end_hard_regno(rel_mode, regno),
                                   PATTERN(this_insn)))
          ...
          &&
          (GET_MODE_SIZE(rel_mode) <= GET_MODE_SIZE(GET_MODE(XEXP(note, 0)))) &&
        ...
        for (offs = 0; offs < nregs; offs++)
          if (fixed_regs[regno + offs] ||
              !TEST_HARD_REG_BIT(reg_class_contents[(int)rclass], regno + offs))
            break;
    ...
    if (rld[i].in && !rld[i].optional &&
        ...
        && (secondary_memlocs_elim[(int)rld[output_reload].outmode]
            ...
            rtx_equal_p(secondary_memlocs_elim[(int)rld[output_reload].outmode]
                                              [rld[i].opnum],
                        secondary_memlocs_elim[(int)rld[output_reload].outmode]
                                              [rld[output_reload].opnum]))
#endif
        &&
        (SMALL_REGISTER_CLASSES
             ? (rld[i].rclass == rld[output_reload].rclass)
             : (reg_class_subset_p(rld[i].rclass, rld[output_reload].rclass) ||
        ...
        (MATCHES(rld[i].in, rld[output_reload].out)
         ...
         ||
         (!reg_overlap_mentioned_for_reload_p(rld[output_reload].out, rld[i].in)
          ...
          && !(REG_P(rld[i].in) && reg_overlap_mentioned_for_reload_p(
                                       rld[i].in, rld[output_reload].out)))) &&
        !reload_inner_reg_of_subreg(rld[i].in, rld[i].inmode,
                                    rld[i].when_needed != RELOAD_FOR_INPUT) &&
        (reg_class_size[(int)rld[i].rclass] || SMALL_REGISTER_CLASSES)
      ...
      if (reg_class_subset_p(rld[output_reload].rclass, rld[i].rclass))
        rld[i].rclass = rld[output_reload].rclass;
...
static rtx find_dummy_reload(rtx real_in, rtx real_out, rtx *inloc, rtx *outloc,
                             ...
                             enum machine_mode outmode, enum reg_class rclass,
                             int for_real, int earlyclobber) {
  rtx in = real_in;
  rtx out = real_out;
  int in_offset = 0;
  int out_offset = 0;
  ...
  while (GET_CODE(out) == SUBREG) {
    if (REG_P(SUBREG_REG(out)) &&
        REGNO(SUBREG_REG(out)) < FIRST_PSEUDO_REGISTER)
      out_offset +=
          subreg_regno_offset(REGNO(SUBREG_REG(out)), GET_MODE(SUBREG_REG(out)),
                              SUBREG_BYTE(out), GET_MODE(out));
    out = SUBREG_REG(out);
  }
  while (GET_CODE(in) == SUBREG) {
    if (REG_P(SUBREG_REG(in)) && REGNO(SUBREG_REG(in)) < FIRST_PSEUDO_REGISTER)
      in_offset +=
          subreg_regno_offset(REGNO(SUBREG_REG(in)), GET_MODE(SUBREG_REG(in)),
                              SUBREG_BYTE(in), GET_MODE(in));
    in = SUBREG_REG(in);
  ...
  /* See if OUT will do.  */
  if (REG_P(out) && REGNO(out) < FIRST_PSEUDO_REGISTER) {
    ...
    *inloc = const0_rtx;
    ...
    if (regno < FIRST_PSEUDO_REGISTER && HARD_REGNO_MODE_OK(regno, outmode) &&
        !refers_to_regno_for_reload_p(regno, regno + nwords, PATTERN(this_insn),
                                      outloc)) {
      unsigned int i;
  ...
  if (hard_regs_live_known && REG_P(in) && REGNO(in) < FIRST_PSEUDO_REGISTER &&
      ...
      (ORIGINAL_REGNO(in) < FIRST_PSEUDO_REGISTER
       /* However only do this if we can be sure that this input
          ...
          pseudo.  See PR 20973.  */
       || (!bitmap_bit_p(DF_LR_OUT(ENTRY_BLOCK_PTR), ORIGINAL_REGNO(in))
...
static int hard_reg_set_here_p(unsigned int beg_regno, unsigned int end_regno,
                               rtx x) {
  if (GET_CODE(x) == SET || GET_CODE(x) == CLOBBER) {
    ...
    if (REG_P(op0)) {
      ...
      if (r < end_regno && end_hard_regno(GET_MODE(op0), r) > beg_regno)
        return 1;
    }
...
int strict_memory_address_p(enum machine_mode mode ATTRIBUTE_UNUSED, rtx addr) {
  ...
  if ((code == REG || (code == SUBREG && REG_P(SUBREG_REG(x)))) &&
    ...
    if (GET_CODE(y) == SUBREG) {
      ...
      if (j >= FIRST_PSEUDO_REGISTER)
        goto slow;
      j += subreg_regno_offset(REGNO(SUBREG_REG(y)), GET_MODE(SUBREG_REG(y)),
...
static int immune_p(rtx x, rtx y, struct decomposition ydata) {
  ...
  if (!rtx_equal_p(xdata.base, ydata.base)) {
    ...
    if (CONSTANT_P(xdata.base) && (ydata.base == frame_pointer_rtx ||
                                   ydata.base == hard_frame_pointer_rtx ||
                                   ydata.base == stack_pointer_rtx))
      return 1;
    if (CONSTANT_P(ydata.base) && (xdata.base == frame_pointer_rtx ||
...
int find_reloads(rtx insn, int replace, int ind_levels, int live_known,
  ...
  if (GET_CODE(body) == SET && REG_P(SET_DEST(body)) &&
      REGNO(SET_DEST(body)) < FIRST_PSEUDO_REGISTER && REG_P(SET_SRC(body)) &&
  ...
  memcpy(constraints, recog_data.constraints, noperands * sizeof(const char *));
    ...
    while ((c = *p)) {
      ...
      switch (c) {
        ...
        case '9': {
          ...
          operands_match[c][i] =
              operands_match_p(recog_data.operand[c], recog_data.operand[i]);
          ...
          if (commutative >= 0) {
            if (c == commutative || c == commutative + 1) {
              ...
              operands_match[other][i] = operands_match_p(
                  recog_data.operand[other], recog_data.operand[i]);
            }
            if (i == commutative || i == commutative + 1) {
              int other = i + (i == commutative ? 1 : -1);
              operands_match[c][other] = operands_match_p(
                  recog_data.operand[c], recog_data.operand[other]);
            }
  ...
  for (i = 0; i < noperands; i++) {
    ...
    operand_type[i] = (modified[i] == RELOAD_READ    ? RELOAD_FOR_INPUT
                       : modified[i] == RELOAD_WRITE ? RELOAD_FOR_OUTPUT
                                                     : RELOAD_OTHER);
    address_type[i] = (modified[i] == RELOAD_READ    ? RELOAD_FOR_INPUT_ADDRESS
    ...
    if (*constraints[i] == 0)
      ...
      address_operand_reloaded[i] = find_reloads_address(
          recog_data.operand_mode[i], (rtx *)0, recog_data.operand[i],
          recog_data.operand_loc[i], i, operand_type[i], ind_levels, insn);
      ...
      address_reloaded[i] = find_reloads_address(
          GET_MODE(recog_data.operand[i]), recog_data.operand_loc[i],
          XEXP(recog_data.operand[i], 0), &XEXP(recog_data.operand[i], 0), i,
          address_type[i], ind_levels, insn);
      recog_data.operand[i] = *recog_data.operand_loc[i];
      substed_operand[i] = recog_data.operand[i];
    } else if (code == SUBREG) {
      rtx reg = SUBREG_REG(recog_data.operand[i]);
      rtx op = find_reloads_toplev(
          recog_data.operand[i], i, address_type[i], ind_levels,
          set != 0 && &SET_DEST(set) == recog_data.operand_loc[i], insn,
          &address_reloaded[i]);
      ...
      if (replace && MEM_P(op) && REG_P(reg) &&
          (GET_MODE_SIZE(GET_MODE(reg)) >= GET_MODE_SIZE(GET_MODE(op))) &&
          reg_equiv_constant[REGNO(reg)] == 0)
        set_unique_reg_note(emit_insn_before(gen_rtx_USE(VOIDmode, reg), insn),
    ...
    preferred_class[i] =
        ((code == REG && REGNO(recog_data.operand[i]) >= FIRST_PSEUDO_REGISTER)
    ...
    for (i = 0; i < noperands; i++) {
      ...
      while (GET_CODE(operand) == SUBREG) {
        ...
        if (CONSTANT_P(operand) ||
            ...
            ||
            ((MEM_P(operand) ||
              (REG_P(operand) && REGNO(operand) >= FIRST_PSEUDO_REGISTER))
#ifndef WORD_REGISTER_OPERATIONS
             && (((GET_MODE_BITSIZE(GET_MODE(operand)) < BIGGEST_ALIGNMENT) &&
        ...
        switch ((c = *p, len = CONSTRAINT_LEN(c, p)), c) {
          ...
          case '9':
            ...
            if ((swapped && (m != commutative || i != commutative + 1))
                    ...
                    ? (operands_match[(m == commutative || m == commutative + 1)
                                          ? 2 * commutative + 1 - m
                                          : m]
              ...
              if (swapped && m == commutative)
                ...
                loc2 = m;
              value = find_dummy_reload(
                  recog_data.operand[i], recog_data.operand[m],
                  recog_data.operand_loc[loc1], recog_data.operand_loc[loc2],
                  operand_mode[i], operand_mode[m], this_alternative[m], -1,
          ...
          case '<':
            if (MEM_P(operand) && !address_reloaded[i] &&
          ...
          case 'o':
            ...
            if ((MEM_P(operand)
              ...
              win = 1;
            if (CONST_POOL_OK_P(operand) || MEM_P(operand))
              badop = 0;
            constmemok = 1;
            offmemok = 1;
            break;
          ...
          case 'F':
            if (GET_CODE(operand) == CONST_DOUBLE ||
                (GET_CODE(operand) == CONST_VECTOR &&
                 (GET_MODE_CLASS(GET_MODE(operand)) == MODE_VECTOR_FLOAT)))
              win = 1;
            break;
          ...
          case 'g':
            if (!force_reload
                ...
                (!CONSTANT_P(operand) || !flag_pic ||
                 LEGITIMATE_PIC_OPERAND_P(operand)) &&
                (GENERAL_REGS == ALL_REGS || !REG_P(operand) ||
              ...
              if (EXTRA_MEMORY_CONSTRAINT(c, p)) {
                ...
                /* Likewise if the address will be reloaded because
                   ...
                   we have to check.  */
                         ...
                         reg_renumber[REGNO(operand)] < 0 &&
                         ((reg_equiv_mem[REGNO(operand)] != 0 &&
                           EXTRA_CONSTRAINT_STR(reg_equiv_mem[REGNO(operand)],
                                                c, p)) ||
                          (reg_equiv_address[REGNO(operand)] != 0)))
                  win = 1;
          ...
          reg:
            ...
            winreg = 1;
            if (REG_P(operand) &&
                reg_fits_class_p(operand, this_alternative[i], offset,
      ...
      if (win && !force_reload)
        ...
        if (REG_P(operand) && this_alternative[i] == (int)NO_REGS &&
        ...
        if (CONST_POOL_OK_P(operand) &&
            ((PREFERRED_RELOAD_CLASS(
                  operand, (enum reg_class)this_alternative[i]) == NO_REGS) ||
             no_input_reloads) &&
      ...
      if (!win && !did_match && this_alternative[i] != (int)NO_REGS &&
          GET_MODE_SIZE(operand_mode[i]) <= UNITS_PER_WORD &&
    ...
    for (i = 0; i < noperands; i++)
      if (this_alternative_earlyclobber[i] &&
        ...
        for (j = 0; j < noperands; j++)
          ...
          if ((MEM_P(recog_data.operand[j]) || modified[j] != RELOAD_WRITE) &&
              j != i
              /* Ignore things like match_operator operands.  */
              ...
              && !(this_alternative_matches[j] == i &&
                   rtx_equal_p(recog_data.operand[i], recog_data.operand[j]))
...
finish:
  ...
  for (i = 0; i < noperands; i++) {
    ...
    if (goal_alternative_earlyclobber[i] && operand_type[i] != RELOAD_OTHER)
      operand_type[i] = (find_reg_note(insn, REG_UNUSED, recog_data.operand[i])
                             ? RELOAD_FOR_INSN
                             : RELOAD_OTHER);
  }
  ...
  for (i = 0; i < noperands; i++)
    if (!goal_alternative_win[i]) {
      ...
      if (CONST_POOL_OK_P(op) &&
          ((PREFERRED_RELOAD_CLASS(op, (enum reg_class)goal_alternative[i]) ==
            NO_REGS) ||
           no_input_reloads) &&
          mode != VOIDmode) {
        ...
        substed_operand[i] = recog_data.operand[i] =
            find_reloads_toplev(tem, i, address_type[i], ind_levels, 0, insn,
        ...
        if (plus == NULL_RTX && subreg == NULL_RTX &&
            alternative_allows_const_pool_ref(
                this_address_reloaded == 0 ? substed_operand[i] : NULL,
                recog_data.constraints[i], goal_alternative_number))
    ...
    if (!goal_alternative_win[i]) {
      ...
      /* Handle an operand with a nonoffsettable address
         ...
         so we don't bother with it.  It may not be worth doing.  */
               ...
               goal_alternative_offmemok[i] && MEM_P(recog_data.operand[i])) {
        ...
        operand_reloadnum[i] =
            push_reload(XEXP(recog_data.operand[i], 0), NULL_RTX,
                        ...
                        base_reg_class(VOIDmode, MEM, SCRATCH), address_mode,
        ...
        if (modified[i] == RELOAD_WRITE) {
          for (j = 0; j < n_reloads; j++) {
            if (rld[j].opnum == i) {
              ...
              else if (rld[j].when_needed == RELOAD_FOR_OUTADDR_ADDRESS)
                rld[j].when_needed = RELOAD_FOR_INPADDR_ADDRESS;
            }
          }
        }
      } else if (goal_alternative_matched[i] == -1) {
        operand_reloadnum[i] = push_reload(
            (modified[i] != RELOAD_WRITE ? recog_data.operand[i] : 0),
            (modified[i] != RELOAD_READ ? recog_data.operand[i] : 0),
            (modified[i] != RELOAD_WRITE ? recog_data.operand_loc[i] : 0),
            (modified[i] != RELOAD_READ ? recog_data.operand_loc[i] : 0),
      ...
      /* In a matching pair of operands, one must be input only
        ...
        operand_reloadnum[goal_alternative_matched[i]] = push_reload(
            ...
            recog_data.operand[i],
            recog_data.operand_loc[goal_alternative_matched[i]],
            recog_data.operand_loc[i], (enum reg_class)goal_alternative[i],
            operand_mode[goal_alternative_matched[i]], operand_mode[i], 0, 0, i,
            RELOAD_OTHER);
        ...
        return 0;
               ...
               address_operand_reloaded[i] != 1 && optimize) {
        ...
        if (REG_P(operand)) {
          if (modified[i] != RELOAD_WRITE)
            /* We mark the USE with QImode so that we recognize
               ...
               of reload.  */
            PUT_MODE(emit_insn_before(gen_rtx_USE(VOIDmode, operand), insn),
                     QImode);
      ...
      if ((MEM_P(operand) ||
        ...
        operand_reloadnum[i] = operand_reloadnum[goal_alternative_matches[i]] =
            push_reload(
                recog_data.operand[goal_alternative_matches[i]],
                recog_data.operand[i],
                recog_data.operand_loc[goal_alternative_matches[i]],
                recog_data.operand_loc[i],
                (enum reg_class)goal_alternative[goal_alternative_matches[i]],
                operand_mode[goal_alternative_matches[i]], operand_mode[i], 0,
                1, goal_alternative_matches[i], RELOAD_OTHER);
    }
    ...
    if (replace) {
      ...
      *recog_data.operand_loc[i] = substitution;
      ...
      if (GET_CODE(substitution) == LABEL_REF &&
          !find_reg_note(insn, REG_LABEL_OPERAND, XEXP(substitution, 0))
          /* For a JUMP_P, if it was a branch target it must have
             already been recorded as such.  */
          && (!JUMP_P(insn) ||
              !label_is_jump_target_p(XEXP(substitution, 0), insn)))
        ...
        if (first_num < 0)
          ...
          for (j = n_reloads - 1; j > first_num; j--) {
            if (rld[j].when_needed == type &&
                (rld[i].secondary_p ? rld[j].secondary_in_reload == i
    ...
    if (rld[i].in != 0 && rld[i].out == 0 &&
      ...
      for (j = 0; j < n_reloads; j++)
        if (i != j && rld[j].in != 0 && rld[j].out == 0 &&
            ...
            MATCHES(rld[i].in, rld[j].in) && rld[i].rclass == rld[j].rclass &&
  ...
  for (i = 0; i < n_reloads; i++) {
    rld[i].mode = (rld[i].inmode == VOIDmode || (GET_MODE_SIZE(rld[i].outmode) >
  ...
  for (i = 0; i < n_reloads; i++)
    if (rld[i].when_needed == RELOAD_FOR_INPUT &&
        GET_CODE(PATTERN(insn)) == SET && REG_P(SET_DEST(PATTERN(insn))) &&
      ...
      if (regno < FIRST_PSEUDO_REGISTER &&
        ...
        for (nri = 1; nri < nr; nri++)
          if (!TEST_HARD_REG_BIT(reg_class_contents[rld[i].rclass],
                                 regno + nri))
            ok = 0;
...
static bool alternative_allows_const_pool_ref(rtx mem, const char *constraint,
                                              int altnum) {
  int c;
...
static rtx find_reloads_toplev(rtx x, int opnum, enum reload_type type,
      ...
      if (reg_equiv_address[regno] || !rtx_equal_p(mem, reg_equiv_mem[regno])) {
        ...
        if (replace_reloads && recog_data.operand[opnum] != x)
          /* We mark the USE with QImode so that we recognize it
             ...
             reload.  */
          PUT_MODE(emit_insn_before(gen_rtx_USE(VOIDmode, x), insn), QImode);
        x = mem;
  ...
  if (code == MEM) {
    ...
    i = find_reloads_address(GET_MODE(x), &tem, XEXP(x, 0), &XEXP(x, 0), opnum,
    ...
    if (regno >= FIRST_PSEUDO_REGISTER && reg_renumber[regno] < 0 &&
      ...
      if (CONSTANT_P(tem) && !LEGITIMATE_CONSTANT_P(tem)) {
        ...
        i = find_reloads_address(GET_MODE(tem), &tem, XEXP(tem, 0),
                                 &XEXP(tem, 0), opnum, type, ind_levels, insn);
        if (address_reloaded)
          *address_reloaded = i;
      }
      return tem;
  ...
  for (copied = 0, i = GET_RTX_LENGTH(code) - 1; i >= 0; i--) {
    if (fmt[i] == 'e') {
      rtx new_part = find_reloads_toplev(XEXP(x, i), opnum, type, ind_levels,
                                         is_set_dest, insn, address_reloaded);
...
static int find_reloads_address(enum machine_mode mode, rtx *memrefloc, rtx ad,
    ...
    if (tem != 0) {
      if (reg_equiv_address[regno] != 0 || num_not_at_initial_offset) {
        ...
        if (!strict_memory_address_p(GET_MODE(tem), XEXP(tem, 0))) {
          ...
          find_reloads_address(GET_MODE(tem), &tem, XEXP(tem, 0), &XEXP(tem, 0),
        ...
        if (ind_levels > 0 && strict_memory_address_p(mode, tem) &&
            (REG_P(XEXP(tem, 0)) ||
             (GET_CODE(XEXP(tem, 0)) == PLUS && REG_P(XEXP(XEXP(tem, 0), 0)) &&
          ...
          if (replace_reloads && num_not_at_initial_offset &&
            ...
            *loc = tem;
            ...
            PUT_MODE(emit_insn_before(gen_rtx_USE(VOIDmode, ad), insn), QImode);
  ...
  if (strict_memory_address_p(mode, ad)) {
    ...
    if (GET_CODE(ad) == PLUS && GET_CODE(XEXP(ad, 1)) == CONST_INT &&
        REG_P(XEXP(ad, 0)) && reg_equiv_constant[REGNO(XEXP(ad, 0))] == 0)
      return 0;
  ...
  if (MEM_P(ad)) {
    ...
    tem = ad;
    find_reloads_address(GET_MODE(ad), &tem, XEXP(ad, 0), &XEXP(ad, 0), opnum,
                         ADDR_TYPE(type), ind_levels == 0 ? 0 : ind_levels - 1,
                         insn);
    ...
    if (ind_levels == 0 ||
      ...
      push_reload(tem, NULL_RTX, loc, (rtx *)0,
                  base_reg_class(mode, MEM, SCRATCH), GET_MODE(tem), VOIDmode,
                  0, 0, opnum, type);
      return !removed_and;
    } else
      return 0;
  ...
  else if (GET_CODE(ad) == PLUS && REG_P(XEXP(ad, 0)) &&
           REGNO(XEXP(ad, 0)) < FIRST_PSEUDO_REGISTER &&
           GET_CODE(XEXP(ad, 1)) == CONST_INT &&
           regno_ok_for_base_p(REGNO(XEXP(ad, 0)), mode, PLUS, CONST_INT))

  {
    ...
    if (double_reg_address_ok) {
      ...
      *loc = ad = copy_rtx(ad);
      ...
      find_reloads_address_part(XEXP(ad, 1), &XEXP(ad, 1), INDEX_REG_CLASS,
                                GET_MODE(ad), opnum, type, ind_levels);
      return 0;
    } else {
  ...
  for (op_index = 0; op_index < 2; ++op_index) {
    ...
    if ((regno_ok_for_base_p(REGNO(operand), mode, inner_code,
        ...
        !maybe_memory_address_p(mode, ad, &XEXP(XEXP(ad, 0), 1 - op_index))) {
      rtx offset_reg;
      enum reg_class cls;
      ...
      if (GET_CODE(XEXP(ad, 0)) == PLUS)
        ad = gen_rtx_PLUS(GET_MODE(ad), op_index == 0 ? offset_reg : addend,
                          op_index == 0 ? addend : offset_reg);
      else
        ad = gen_rtx_LO_SUM(GET_MODE(ad), op_index == 0 ? offset_reg : addend,
                            op_index == 0 ? addend : offset_reg);
      *loc = ad;

      cls = base_reg_class(mode, MEM, GET_CODE(addend));
      find_reloads_address_part(XEXP(ad, op_index), &XEXP(ad, op_index), cls,
                                GET_MODE(ad), opnum, type, ind_levels);
      find_reloads_address_1(mode, XEXP(ad, 1 - op_index), 1, GET_CODE(ad),
                             GET_CODE(XEXP(ad, op_index)),
                             &XEXP(ad, 1 - op_index), opnum, type, 0, insn);
...
static rtx subst_indexed_address(rtx addr) {
  ...
  if (GET_CODE(addr) == PLUS) {
    ...
    if (REG_P(op0) && (regno = REGNO(op0)) >= FIRST_PSEUDO_REGISTER &&
...
static void update_auto_inc_notes(rtx insn ATTRIBUTE_UNUSED,
  ...
  for (link = REG_NOTES(insn); link; link = XEXP(link, 1))
    if (REG_NOTE_KIND(link) == REG_INC && (int)REGNO(XEXP(link, 0)) == regno)
...
static int find_reloads_address_1(enum machine_mode mode, rtx x, int context,
                                  enum rtx_code outer_code,
                                  enum rtx_code index_code, rtx *loc, int opnum,
                                  enum reload_type type, int ind_levels,
                                  rtx insn) {
#define REG_OK_FOR_CONTEXT(CONTEXT, REGNO, MODE, OUTER, INDEX)     \
  ((CONTEXT) == 0 ? regno_ok_for_base_p(REGNO, MODE, OUTER, INDEX) \
  ...
  switch (code) {
    case PLUS: {
      ...
      if (GET_CODE(op0) == SUBREG) {
        ...
        if (code0 == REG && REGNO(op0) < FIRST_PSEUDO_REGISTER)
          op0 = gen_rtx_REG(
              word_mode,
              (REGNO(op0) + subreg_regno_offset(REGNO(SUBREG_REG(orig_op0)),
      ...
      if (GET_CODE(op1) == SUBREG) {
        ...
        if (code1 == REG && REGNO(op1) < FIRST_PSEUDO_REGISTER)
          ...
          op1 = gen_rtx_REG(
              GET_MODE(op1),
              (REGNO(op1) + subreg_regno_offset(REGNO(SUBREG_REG(orig_op1)),
      ...
      if (context == 1) {
        find_reloads_address(GET_MODE(x), loc, XEXP(x, 0), &XEXP(x, 0), opnum,
      ...
      if (code0 == MULT || code0 == SIGN_EXTEND || code0 == TRUNCATE ||
        ...
        find_reloads_address_1(mode, orig_op0, 1, PLUS, SCRATCH, &XEXP(x, 0),
                               opnum, type, ind_levels, insn);
        find_reloads_address_1(mode, orig_op1, 0, PLUS, code0, &XEXP(x, 1),
                               opnum, type, ind_levels, insn);
      }

      else if (code1 == MULT || code1 == SIGN_EXTEND || code1 == TRUNCATE ||
               code1 == ZERO_EXTEND || code0 == MEM) {
        find_reloads_address_1(mode, orig_op0, 0, PLUS, code1, &XEXP(x, 0),
                               opnum, type, ind_levels, insn);
        find_reloads_address_1(mode, orig_op1, 1, PLUS, SCRATCH, &XEXP(x, 1),
                               opnum, type, ind_levels, insn);
      }

      else if (code0 == CONST_INT || code0 == CONST || code0 == SYMBOL_REF ||
               code0 == LABEL_REF)
        find_reloads_address_1(mode, orig_op1, 0, PLUS, code0, &XEXP(x, 1),
                               opnum, type, ind_levels, insn);

      else if (code1 == CONST_INT || code1 == CONST || code1 == SYMBOL_REF ||
               code1 == LABEL_REF)
        find_reloads_address_1(mode, orig_op0, 0, PLUS, code1, &XEXP(x, 0),
                               opnum, type, ind_levels, insn);
        ...
        if (REGNO_OK_FOR_INDEX_P(REGNO(op1)) &&
          ...
          find_reloads_address_1(mode, orig_op0, 0, PLUS, REG, &XEXP(x, 0),
                                 opnum, type, ind_levels, insn);
        else if (regno_ok_for_base_p(REGNO(op1), mode, PLUS, REG))
          find_reloads_address_1(mode, orig_op0, 1, PLUS, SCRATCH, &XEXP(x, 0),
                                 opnum, type, ind_levels, insn);
        else if (REGNO_OK_FOR_INDEX_P(REGNO(op0)))
          find_reloads_address_1(mode, orig_op1, 0, PLUS, REG, &XEXP(x, 1),
                                 opnum, type, ind_levels, insn);
        else {
          find_reloads_address_1(mode, orig_op0, 0, PLUS, REG, &XEXP(x, 0),
                                 opnum, type, ind_levels, insn);
        ...
        find_reloads_address_1(mode, orig_op0, 1, PLUS, SCRATCH, &XEXP(x, 0),
                               opnum, type, ind_levels, insn);
        find_reloads_address_1(mode, orig_op1, 0, PLUS, REG, &XEXP(x, 1), opnum,
                               type, ind_levels, insn);
      }
        ...
        find_reloads_address_1(mode, orig_op1, 1, PLUS, SCRATCH, &XEXP(x, 1),
                               opnum, type, ind_levels, insn);
        find_reloads_address_1(mode, orig_op0, 0, PLUS, REG, &XEXP(x, 0), opnum,
                               type, ind_levels, insn);
      }
    }
    ...
    case PRE_MODIFY: {
      ...
      if ((REG_P(XEXP(op1, 1)) && !REGNO_OK_FOR_INDEX_P(REGNO(XEXP(op1, 1)))) ||
          GET_CODE(XEXP(op1, 1)) == PLUS)
        find_reloads_address_1(mode, XEXP(op1, 1), 1, code, SCRATCH,
                               &XEXP(op1, 1), opnum, RELOAD_OTHER, ind_levels,
                               insn);
        ...
        if (reg_equiv_address[regno] ||
          ...
          reloadnum =
              push_reload(tem, tem, &XEXP(x, 0), &XEXP(op1, 0),
                          base_reg_class(mode, code, index_code), GET_MODE(x),
      ...
      if (!regno_ok_for_base_p(regno, GET_MODE(x), code, index_code)) {
        reloadnum =
            push_reload(XEXP(op1, 0), XEXP(x, 0), &XEXP(op1, 0), &XEXP(x, 0),
                        base_reg_class(mode, code, index_code), GET_MODE(x),
                        GET_MODE(x), 0, 0, opnum, RELOAD_OTHER);
    ...
    case PRE_DEC:
      if (REG_P(XEXP(x, 0))) {
        ...
        if (reg_renumber[regno] >= 0)
          regno = reg_renumber[regno];
        if (regno >= FIRST_PSEUDO_REGISTER ||
            !REG_OK_FOR_CONTEXT(context, regno, mode, code, index_code)) {
          int reloadnum;
          ...
          rtx equiv = (MEM_P(XEXP(x, 0)) ? XEXP(x, 0) : reg_equiv_mem[regno]);
          ...
          if (insn && NONJUMP_INSN_P(insn) && equiv &&
              ...
              && !(icode != CODE_FOR_nothing &&
                   ((*insn_data[icode].operand[0].predicate)(equiv, Pmode)) &&
                   ((*insn_data[icode].operand[1].predicate)(equiv, Pmode)))) {
            ...
            x = XEXP(x, 0);
            reloadnum =
                push_reload(x, x, loc, loc, context_reg_class, GET_MODE(x),
                            GET_MODE(x), 0, 0, opnum, RELOAD_OTHER);
          } else {
            reloadnum =
                push_reload(x, x, loc, (rtx *)0, context_reg_class, GET_MODE(x),
          ...
          update_auto_inc_notes(this_insn, REGNO(XEXP(x_orig, 0)), reloadnum);
        }
        return value;
      }
      return 0;
    ...
    case ZERO_EXTEND:
      ...
      find_reloads_address_1(GET_MODE(XEXP(x, 0)), XEXP(x, 0), context, code,
                             SCRATCH, &XEXP(x, 0), opnum, type, ind_levels,
                             insn);
      push_reload(x, NULL_RTX, loc, (rtx *)0, context_reg_class, GET_MODE(x),
    ...
    case MEM:
      ...
      find_reloads_address(GET_MODE(x), loc, XEXP(x, 0), &XEXP(x, 0), opnum,
                           ADDR_TYPE(type), ind_levels, insn);
      push_reload(*loc, NULL_RTX, loc, (rtx *)0, context_reg_class, GET_MODE(x),
                  VOIDmode, 0, 0, opnum, type);
      return 1;
    ...
    case REG: {
      ...
      if (reg_equiv_constant[regno] != 0) {
        find_reloads_address_part(reg_equiv_constant[regno], loc,
                                  context_reg_class, GET_MODE(x), opnum, type,
                                  ind_levels);
        return 1;
      }
	...
	if (reg_equiv_mem[regno] != 0)
      ...
      if (regno >= FIRST_PSEUDO_REGISTER ||
          !REG_OK_FOR_CONTEXT(context, regno, mode, outer_code, index_code)) {
        push_reload(x, NULL_RTX, loc, (rtx *)0, context_reg_class, GET_MODE(x),
                    VOIDmode, 0, 0, opnum, type);
        return 1;
      }
      ...
      if (REG_P(SUBREG_REG(x))) {
        ...
        /* If this is a SUBREG of a pseudo-register, and the pseudo-register
          ...
          if ((unsigned)CLASS_MAX_NREGS(rclass, GET_MODE(SUBREG_REG(x))) >
            ...
            x = find_reloads_subreg_address(x, 0, opnum, ADDR_TYPE(type),
                                            ind_levels, insn);
            push_reload(x, NULL_RTX, loc, (rtx *)0, rclass, GET_MODE(x),
                        VOIDmode, 0, 0, opnum, type);
            return 1;
          }
        }
...
static void find_reloads_address_part(rtx x, rtx *loc, enum reg_class rclass,
  ...
  if (CONSTANT_P(x) && (!LEGITIMATE_CONSTANT_P(x) ||
    ...
    find_reloads_address(mode, &XEXP(x, 1), XEXP(tem, 0), &XEXP(tem, 0), opnum,
                         type, ind_levels, 0);
  }

  push_reload(x, NULL_RTX, loc, (rtx *)0, rclass, mode, VOIDmode, 0, 0, opnum,
              type);
}
...
static rtx find_reloads_subreg_address(rtx x, int force_replace, int opnum,
                                       enum reload_type type, int ind_levels,
                                       rtx insn) {
  int regno = REGNO(SUBREG_REG(x));
  ...
  if (reg_equiv_memory_loc[regno]) {
    ...
    if (!force_replace && (reg_equiv_address[regno] ||
                           !offsettable_memref_p(reg_equiv_mem[regno])))
      force_replace = 1;
    ...
    if (force_replace || num_not_at_initial_offset) {
      ...
      if (force_replace || !rtx_equal_p(tem, reg_equiv_mem[regno])) {
        ...
        if (outer_size > inner_size) {
          ...
          if (!REG_P(base) ||
              (REGNO_POINTER_ALIGN(REGNO(base)) < outer_size * BITS_PER_UNIT))
            return x;
        }
        ...
        /* For some processors an address may be valid in the
           ...
           here.  */
        if (reloaded == 0 &&
            !strict_memory_address_p(GET_MODE(tem), XEXP(tem, 0)))
...
#define CHECK_MODF(ARRAY)           \
  gcc_assert(!ARRAY[check_regno] || \
             !loc_mentioned_in_p(r->where, ARRAY[check_regno]))
...
#undef CHECK_MODF
      ...
      /* If we're replacing a LABEL_REF with a register, there must
         ...
         register refers to.  */
      gcc_assert(GET_CODE(*r->where) != LABEL_REF || !JUMP_P(insn) ||
                 find_reg_note(insn, REG_LABEL_OPERAND, XEXP(*r->where, 0)) ||
      ...
      if (r->subreg_loc != 0 && GET_CODE(reloadreg) == SUBREG) {
        if (GET_MODE(*r->subreg_loc) == GET_MODE(SUBREG_REG(reloadreg)))
          *r->subreg_loc = SUBREG_REG(reloadreg);
          ...
          final_offset =
              (final_offset / GET_MODE_SIZE(GET_MODE(*r->subreg_loc)));
          final_offset =
              (final_offset * GET_MODE_SIZE(GET_MODE(*r->subreg_loc)));
  ...
  for (r = &replacements[0]; r < &replacements[n_replacements]; r++) {
    ...
    if (reloadreg && r->where == loc) {
      ...
      if (REG_P(reloadreg))
        return gen_rtx_REG(
            GET_MODE(*loc),
            (REGNO(reloadreg) + subreg_regno_offset(REGNO(SUBREG_REG(*loc)),
    ...
    case REG:
      ...
      if (r >= FIRST_PSEUDO_REGISTER) {
        if (reg_equiv_memory_loc[r])
          return refers_to_regno_for_reload_p(
              regno, endregno, reg_equiv_memory_loc[r], (rtx *)0);
    ...
    case SUBREG:
      ...
      if (REG_P(SUBREG_REG(x)) &&
        ...
        unsigned int inner_endregno =
            inner_regno +
            (inner_regno < FIRST_PSEUDO_REGISTER ? subreg_nregs(x) : 1);
    ...
    case SET:
      if (&SET_DEST(x) != loc
          ...
          ((GET_CODE(SET_DEST(x)) == SUBREG &&
            loc != &SUBREG_REG(SET_DEST(x)) && REG_P(SUBREG_REG(SET_DEST(x))) &&
           ...
           /* If the output is an earlyclobber operand, this is
              a conflict.  */
           ||
           ((!REG_P(SET_DEST(x)) || earlyclobber_operand_p(SET_DEST(x))) &&
    ...
    if (fmt[i] == 'e' && loc != &XEXP(x, i)) {
      ...
      for (j = XVECLEN(x, i) - 1; j >= 0; j--)
        if (loc != &XVECEXP(x, i, j) &&
            refers_to_regno_for_reload_p(regno, endregno, XVECEXP(x, i, j),
                                         loc))
          return 1;
    }
  }
...
int reg_overlap_mentioned_for_reload_p(rtx x, rtx in) {
  ...
  if (CONSTANT_P(x) || CONSTANT_P(in))
    ...
    if (regno < FIRST_PSEUDO_REGISTER)
      regno +=
          subreg_regno_offset(REGNO(SUBREG_REG(x)), GET_MODE(SUBREG_REG(x)),
                              SUBREG_BYTE(x), GET_MODE(x));
    endregno = regno + (regno < FIRST_PSEUDO_REGISTER ? subreg_nregs(x) : 1);
...
static int refers_to_mem_for_reload_p(rtx x) {
  ...
  for (i = GET_RTX_LENGTH(GET_CODE(x)) - 1; i >= 0; i--)
    if (fmt[i] == 'e' &&
        (MEM_P(XEXP(x, i)) || refers_to_mem_for_reload_p(XEXP(x, i))))
      return 1;
...
rtx find_equiv_reg(rtx goal, rtx insn, enum reg_class rclass, int other,
  ...
  while (1) {
    ...
    if (NONJUMP_INSN_P(p)
        ...
        && (!(reload_reg_p != 0 && reload_reg_p != (short *)(HOST_WIDE_INT)1)
      ...
      if (pat != 0 &&
          ((regno >= 0 && true_regnum(SET_SRC(pat)) == regno &&
           ...
           (goal_const && REG_NOTES(p) != 0 &&
            ...
            ((rtx_equal_p(XEXP(tem, 0), goal) &&
              (valueno = true_regnum(valtry = SET_DEST(pat))) >= 0) ||
             (REG_P(SET_DEST(pat)) && GET_CODE(XEXP(tem, 0)) == CONST_DOUBLE &&
        ...
        if (other >= 0) {
          ...
          continue;
        else if (!in_hard_reg_set_p(reg_class_contents[(int)rclass], mode,
                                    valueno))
          continue;
        value = valtry;
        where = p;
  ...
  if (goal_mem && value == SET_DEST(single_set(where)) &&
      refers_to_regno_for_reload_p(valueno, end_hard_regno(mode, valueno), goal,
                                   (rtx *)0))
    return 0;
  ...
  if (reload_reg_p != 0) {
    ...
    for (i = 0; i < n_reloads; i++)
      if (rld[i].reg_rtx != 0 && rld[i].in) {
        ...
        int nregs1 = hard_regno_nregs[regno1][GET_MODE(rld[i].reg_rtx)];
    ...
    if (INSN_P(p)) {
         ...
         and this insn P increments a register used in GOAL, return 0.  */
      {
        ...
        for (link = REG_NOTES(p); link; link = XEXP(link, 1))
          if (REG_NOTE_KIND(link) == REG_INC && REG_P(XEXP(link, 0))) {
...
#ifdef AUTO_INC_DEC
static int reg_inc_found_and_valid_p(unsigned int regno, unsigned int endregno,
                                     rtx insn) {
  rtx link;
...
int regno_clobbered_p(unsigned int regno, rtx insn, enum machine_mode mode,
                      int sets) {
...
void debug_reload_to_stream(FILE *f) {
  ...
  for (r = 0; r < n_reloads; r++) {
    ...
    fprintf(f, "%s (opnum = %d)",
            reload_when_needed_name[(int)rld[r].when_needed], rld[r].opnum);
    ...
    if (rld[r].secondary_in_reload != -1) {
      fprintf(f, "%ssecondary_in_reload = %d", prefix,
              rld[r].secondary_in_reload);
      prefix = ", ";
    }

    if (rld[r].secondary_out_reload != -1)
      fprintf(f, "%ssecondary_out_reload = %d\n", prefix,
              rld[r].secondary_out_reload);
