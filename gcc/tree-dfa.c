...
static unsigned int find_referenced_vars(void) {
  basic_block bb;
...
var_ann_t create_var_ann(tree t) {
  var_ann_t ann;
...
function_ann_t create_function_ann(tree t) {
  function_ann_t ann;
...
tree_ann_common_t create_tree_common_ann(tree t) {
...
tree make_rename_temp(tree type, const char *prefix) {
...
void dump_referenced_vars(FILE *file) {
  tree var;
...
void dump_variable(FILE *file, tree var) {
  ...
  if (is_call_clobbered(var)) {
    ...
    if (escape_mask & ESCAPE_STORED_IN_GLOBAL) {
      fprintf(file, "%sstored in global", s);
      s = ", ";
    }
    if (escape_mask & ESCAPE_TO_ASM) {
      fprintf(file, "%sgoes through ASM", s);
      s = ", ";
    }
    if (escape_mask & ESCAPE_TO_CALL) {
      fprintf(file, "%spassed to call", s);
      s = ", ";
    }
    if (escape_mask & ESCAPE_BAD_CAST) {
      fprintf(file, "%sbad cast", s);
      s = ", ";
    }
    if (escape_mask & ESCAPE_TO_RETURN) {
      fprintf(file, "%sreturned from func", s);
      s = ", ";
    }
    if (escape_mask & ESCAPE_TO_PURE_CONST) {
      fprintf(file, "%spassed to pure/const", s);
      s = ", ";
    }
    if (escape_mask & ESCAPE_IS_GLOBAL) {
      fprintf(file, "%sis global var", s);
      s = ", ";
    }
    if (escape_mask & ESCAPE_IS_PARM) {
      fprintf(file, "%sis incoming pointer", s);
      s = ", ";
    }
    if (escape_mask & ESCAPE_UNKNOWN) {
      fprintf(file, "%sunknown escape", s);
      s = ", ";
    }
    fprintf(file, ")");
  }
...
void dump_dfa_stats(FILE *file) {
  ...
  total += size;
  fprintf(file, fmt_str_1, "Referenced variables",
          (unsigned long)num_referenced_vars, SCALE(size), LABEL(size));
  ...
  total += size;
  fprintf(file, fmt_str_1, "USE operands", dfa_stats.num_uses, SCALE(size),
          LABEL(size));

  size = dfa_stats.num_defs * sizeof(tree *);
  total += size;
  fprintf(file, fmt_str_1, "DEF operands", dfa_stats.num_defs, SCALE(size),
          LABEL(size));

  size = dfa_stats.num_vuses * sizeof(tree *);
  total += size;
  fprintf(file, fmt_str_1, "VUSE operands", dfa_stats.num_vuses, SCALE(size),
          LABEL(size));

  size = dfa_stats.num_vdefs * sizeof(tree *);
  total += size;
  fprintf(file, fmt_str_1, "VDEF operands", dfa_stats.num_vdefs, SCALE(size),
          LABEL(size));

  size = dfa_stats.num_phis * sizeof(struct gimple_statement_phi);
  total += size;
  fprintf(file, fmt_str_1, "PHI nodes", dfa_stats.num_phis, SCALE(size),
          LABEL(size));

  size = dfa_stats.num_phi_args * sizeof(struct phi_arg_d);
  total += size;
  fprintf(file, fmt_str_1, "PHI arguments", dfa_stats.num_phi_args, SCALE(size),
          LABEL(size));
...
static void collect_dfa_stats(
    struct dfa_stats_d *dfa_stats_p ATTRIBUTE_UNUSED) {
  basic_block bb;
...
static tree find_vars_r(tree *tp, int *walk_subtrees,
...
tree referenced_var_lookup(unsigned int uid) {
  tree h;
...
bool referenced_var_check_and_insert(tree to) {
  tree h, *loc;
  struct tree_decl_minimal in;
  ...
  loc = (tree *)htab_find_slot_with_hash(gimple_referenced_vars(cfun), &in, uid,
                                         INSERT);
  *loc = to;
  return true;
}
...
tree gimple_default_def(struct function *fn, tree var) {
...
void set_default_def(tree var, tree def) {
  struct tree_decl_minimal ind;
  struct tree_ssa_name in;
  void **loc;
  ...
  ind.uid = DECL_UID(var);
  if (!def) {
    loc = htab_find_slot_with_hash(DEFAULT_DEFS(cfun), &in, DECL_UID(var),
                                   INSERT);
    gcc_assert(*loc);
  ...
  loc =
      htab_find_slot_with_hash(DEFAULT_DEFS(cfun), &in, DECL_UID(var), INSERT);
...
bool add_referenced_var(tree var) {
  var_ann_t v_ann;
...
void remove_referenced_var(tree var) {
  var_ann_t v_ann;
  struct tree_decl_minimal in;
  void **loc;
...
void mark_symbols_for_renaming(gimple stmt) {
  tree op;
  ssa_op_iter iter;
...
static tree find_new_referenced_vars_1(tree *tp, int *walk_subtrees,
                                       void *data ATTRIBUTE_UNUSED) {
  tree t = *tp;
...
void find_new_referenced_vars(gimple stmt) {
...
tree get_ref_base_and_extent(tree exp, HOST_WIDE_INT *poffset,
                             HOST_WIDE_INT *psize, HOST_WIDE_INT *pmax_size) {
  ...
  while (1) {
    switch (TREE_CODE(exp)) {
      ...
      case ARRAY_RANGE_REF: {
        ...
        if (host_integerp(index, 0) && host_integerp(low_bound, 0) &&
...
bool refs_may_alias_p(tree ref1, tree ref2) {
  tree base1, base2;
  ...
  gcc_assert((SSA_VAR_P(ref1) || handled_component_p(ref1) ||
              INDIRECT_REF_P(ref1) || TREE_CODE(ref1) == TARGET_MEM_REF) &&
             (SSA_VAR_P(ref2) || handled_component_p(ref2) ||
              INDIRECT_REF_P(ref2) || TREE_CODE(ref2) == TARGET_MEM_REF));
  ...
  if (SSA_VAR_P(base1) && SSA_VAR_P(base2)) {
  ...
  strict_aliasing_applies =
      (flag_strict_aliasing &&
       (!INDIRECT_REF_P(base1) || get_alias_set(base1) != 0) &&
       (!INDIRECT_REF_P(base2) || get_alias_set(base2) != 0));
  ...
  if (strict_aliasing_applies &&
      ((SSA_VAR_P(ref2) && !AGGREGATE_TYPE_P(TREE_TYPE(ref2)) &&
        !INDIRECT_REF_P(ref1) && TREE_CODE(TREE_TYPE(base1)) != UNION_TYPE) ||
       (SSA_VAR_P(ref1) && !AGGREGATE_TYPE_P(TREE_TYPE(ref1)) &&
        !INDIRECT_REF_P(ref2) && TREE_CODE(TREE_TYPE(base2)) != UNION_TYPE)))
    return false;
  ...
  if ((strict_aliasing_applies && (TYPE_MAIN_VARIANT(TREE_TYPE(base1)) ==
                                   TYPE_MAIN_VARIANT(TREE_TYPE(base2)))) ||
      (TREE_CODE(base1) == INDIRECT_REF && TREE_CODE(base2) == INDIRECT_REF &&
       operand_equal_p(TREE_OPERAND(base1, 0), TREE_OPERAND(base2, 0), 0)))
    return ranges_overlap_p(offset1, max_size1, offset2, max_size2);
  ...
  if (strict_aliasing_applies &&
    ...
    refp = &ref1;
    while (handled_component_p(*refp) && (TYPE_MAIN_VARIANT(TREE_TYPE(*refp)) !=
                                          TYPE_MAIN_VARIANT(TREE_TYPE(base2))))
      refp = &TREE_OPERAND(*refp, 0);
    if (TYPE_MAIN_VARIANT(TREE_TYPE(*refp)) ==
        TYPE_MAIN_VARIANT(TREE_TYPE(base2))) {
...
gimple get_single_def_stmt(gimple stmt) {
  ...
  FOR_EACH_SSA_TREE_OPERAND(use, stmt, iter, SSA_OP_VIRTUAL_USES) {
    ...
    if (def_stmt && def_stmt != tmp &&
        !(gimple_nop_p(def_stmt) && gimple_nop_p(tmp)))
      return NULL;
...
gimple get_single_def_stmt_from_phi(tree ref, gimple phi) {
...
gimple get_single_def_stmt_with_phi(tree ref, gimple stmt) {
  switch (NUM_SSA_OPERANDS(stmt, SSA_OP_VIRTUAL_USES)) {
    ...
    case 1: {
      gimple def_stmt =
          SSA_NAME_DEF_STMT(SINGLE_SSA_TREE_OPERAND(stmt, SSA_OP_VIRTUAL_USES));
