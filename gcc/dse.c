...
static int clear_alias_mode_eq(const void *p1, const void *p2) {
...
static hashval_t clear_alias_mode_hash(const void *p) {
...
static int invariant_group_base_eq(const void *p1, const void *p2) {
...
static hashval_t invariant_group_base_hash(const void *p) {
...
static group_info_t get_group_info(rtx base) {
  ...
  if (base) {
    ...
    if (!clear_alias_group) {
      clear_alias_group = gi = (group_info_t)pool_alloc(rtx_group_info_pool);
...
static void dse_step0(void) {
  ...
  rtx_store_info_pool =
      create_alloc_pool("rtx_store_info_pool", sizeof(struct store_info), 100);
  read_info_pool =
      create_alloc_pool("read_info_pool", sizeof(struct read_info), 100);
  insn_info_pool =
      create_alloc_pool("insn_info_pool", sizeof(struct insn_info), 100);
  bb_info_pool = create_alloc_pool("bb_info_pool", sizeof(struct bb_info), 100);
  rtx_group_info_pool =
      create_alloc_pool("rtx_group_info_pool", sizeof(struct group_info), 100);
  deferred_change_pool = create_alloc_pool("deferred_change_pool",
                                           sizeof(struct deferred_change), 10);

  rtx_group_table =
      htab_create(11, invariant_group_base_hash, invariant_group_base_eq, NULL);
...
static int replace_inc_dec(rtx *r, void *d) {
  rtx x = *r;
  ...
  switch (GET_CODE(x)) {
    ...
    case POST_INC: {
      ...
      emit_insn_before(gen_rtx_SET(Pmode, r1, gen_rtx_PLUS(Pmode, r1, c)),
                       data->insn);
      return -1;
    }
...
static int replace_inc_dec_mem(rtx *r, void *d) {
  rtx x = *r;
...
static void check_for_inc_dec(rtx insn) {
...
static void delete_dead_store_insn(insn_info_t insn_info) {
  ...
  if (dump_file) {
    fprintf(dump_file, "Locally deleting insn %d ", INSN_UID(insn_info->insn));
...
static void set_usage_bits(group_info_t group, HOST_WIDE_INT offset,
                           HOST_WIDE_INT width) {
  HOST_WIDE_INT i;
...
static bool canon_address(rtx mem, alias_set_type *alias_set_out, int *group_id,
                          HOST_WIDE_INT *offset, cselib_val **base) {
  ...
  if (clear_alias_sets) {
    ...
    if (bitmap_bit_p(clear_alias_sets, alias_set)) {
      struct clear_alias_mode_holder *entry = clear_alias_set_lookup(alias_set);

      /* If the modes do not match, we cannot process this set.  */
      if (entry->mode != GET_MODE(mem)) {
        if (dump_file)
          fprintf(dump_file, "disqualifying alias set %d, (%s) != (%s)\n",
...
static inline void set_position_unneeded(store_info_t s_info, int pos) {
...
static inline void set_all_positions_unneeded(store_info_t s_info) {
  if (__builtin_expect(s_info->is_large, false)) {
...
static inline bool any_positions_needed_p(store_info_t s_info) {
  if (__builtin_expect(s_info->is_large, false))
    return (s_info->positions_needed.large.count < s_info->end - s_info->begin);
  else
...
static inline bool all_positions_needed_p(store_info_t s_info, int start,
                                          int width) {
  if (__builtin_expect(s_info->is_large, false)) {
    int end = start + width;
    while (start < end)
...
static int record_store(rtx body, bb_info_t bb_info) {
  ...
  if (GET_MODE(mem) == BLKmode) {
    ...
    /* Handle (set (mem:BLK (addr) [... S36 ...]) (const_int 0))
       as memset (addr, 0, 36);  */
    else if (!MEM_SIZE(mem) || !CONST_INT_P(MEM_SIZE(mem)) ||
             GET_CODE(body) != SET || INTVAL(MEM_SIZE(mem)) <= 0 ||
  ...
  if (spill_alias_set) {
    ...
    if (!base_term ||
        (GET_CODE(base_term) == ADDRESS && GET_MODE(base_term) == Pmode &&
  ...
  if (GET_CODE(body) == SET
      ...
      && !reload_completed &&
      (REG_P(SET_SRC(body)) || GET_CODE(SET_SRC(body)) == SUBREG ||
  ...
  while (ptr) {
    ...
    if (s_info->alias_set != spill_alias_set)
    ...
    } else if ((s_info->group_id == group_id) && (s_info->cse_base == base)) {
      HOST_WIDE_INT i;
      if (dump_file)
        fprintf(dump_file, "    trying store in insn=%d gid=%d[%d..%d)\n",
                INSN_UID(ptr->insn), s_info->group_id, (int)s_info->begin,
                (int)s_info->end);
      ...
      if (s_info->const_rhs && const_rhs && offset >= s_info->begin &&
          offset + width <= s_info->end &&
          all_positions_needed_p(s_info, offset - s_info->begin, width)) {
        if (GET_MODE(mem) == BLKmode) {
          ...
          rtx val;
          start_sequence();
          val = get_stored_val(s_info, GET_MODE(mem), offset, offset + width,
                               BLOCK_FOR_INSN(insn_info->insn), true);
          if (get_insns() != NULL)
            val = NULL_RTX;
          end_sequence();
    ...
    {
      if (canon_true_dependence(s_info->mem, GET_MODE(s_info->mem),
                                s_info->mem_addr, mem, rtx_varies_p)) {
        s_info->rhs = NULL;
    ...
    /* An insn can be deleted if every position of every one of
       its s_infos is zero.  */
    if (any_positions_needed_p(s_info) || ptr->cannot_delete)
      del = false;
...
static rtx find_shift_sequence(int access_size, store_info_t store_info,
                               enum machine_mode read_mode, int shift,
                               bool speed, bool require_cst) {
  ...
  for (new_mode = smallest_mode_for_size(access_size * BITS_PER_UNIT, MODE_INT);
    ...
    if (!CONSTANT_P(store_info->rhs) && !MODES_TIEABLE_P(new_mode, store_mode))
      continue;
    ...
    target = expand_binop(new_mode, lshr_optab, new_reg, GEN_INT(shift),
    ...
    new_lhs = extract_low_bits(new_mode, store_mode, copy_rtx(store_info->rhs));
    if (new_lhs == NULL_RTX)
      continue;
...
static void look_for_hardregs(rtx x, const_rtx pat ATTRIBUTE_UNUSED,
                              void *data) {
...
static rtx get_stored_val(store_info_t store_info, enum machine_mode read_mode,
  ...
  if (shift)
    ...
    if (store_mode == BLKmode)
      ...
      unsigned HOST_WIDE_INT c =
          INTVAL(store_info->rhs) & (((HOST_WIDE_INT)1 << BITS_PER_UNIT) - 1);
    ...
    read_reg =
        extract_low_bits(read_mode, store_mode, copy_rtx(store_info->rhs));
...
static bool replace_read(store_info_t store_info, insn_info_t store_insn,
  ...
  read_reg = get_stored_val(store_info, read_mode, read_info->begin,
                            read_info->end, bb, false);
  ...
  if (insns != NULL_RTX) {
    ...
    for (this_insn = insns; this_insn != NULL_RTX;
...
static int check_mem_read_rtx(rtx *loc, void *data) {
  rtx mem = *loc;
  bb_info_t bb_info;
  ...
  if (spill_alias_set) {
    ...
    while (i_ptr) {
      ...
      if (store_info->group_id < 0)
        /* We have a cselib store followed by a read from a
           const base. */
        remove =
            canon_true_dependence(store_info->mem, GET_MODE(store_info->mem),
                                  store_info->mem_addr, mem, rtx_varies_p);
        ...
        /* If this read is just reading back something that we just
          ...
          if (store_info->rhs && offset >= store_info->begin &&
              offset + width <= store_info->end &&
              all_positions_needed_p(store_info, offset - store_info->begin,
                                     width) &&
    ...
    while (i_ptr) {
      ...
      /* If this read is just reading back something that we just
         stored, rewrite the read.  */
      if (store_info->rhs && store_info->group_id == -1 &&
          store_info->cse_base == base && offset >= store_info->begin &&
          offset + width <= store_info->end &&
          all_positions_needed_p(store_info, offset - store_info->begin,
                                 width) &&
          replace_read(store_info, i_ptr, read_info, insn_info, loc,
...
static bool get_call_args(rtx call_insn, tree fn, rtx *args, int nargs) {
  ...
  for (idx = 0; arg != void_list_node && idx < nargs;
    ...
    for (link = CALL_INSN_FUNCTION_USAGE(call_insn); link; link = XEXP(link, 1))
      if (GET_CODE(XEXP(link, 0)) == USE) {
        ...
        if (REG_P(args[idx]) && REGNO(args[idx]) == REGNO(reg) &&
            (GET_MODE(args[idx]) == mode ||
             (GET_MODE_CLASS(GET_MODE(args[idx])) == MODE_INT &&
              (GET_MODE_SIZE(GET_MODE(args[idx])) <= UNITS_PER_WORD) &&
              (GET_MODE_SIZE(GET_MODE(args[idx])) > GET_MODE_SIZE(mode)))))
          break;
      }
    if (!link)
      return false;
...
static void scan_insn(bb_info_t bb_info, rtx insn) {
  rtx body;
  ...
  /* Cselib clears the table for this case, so we have to essentially
     do the same.  */
  if (NONJUMP_INSN_P(insn) && GET_CODE(PATTERN(insn)) == ASM_OPERANDS &&
  ...
  if (CALL_P(insn)) {
    ...
    if (const_call || memset_call) {
      ...
      if (memset_call) {
        rtx args[3];
        if (get_call_args(insn, memset_call, args, 3) && CONST_INT_P(args[1]) &&
            CONST_INT_P(args[2]) && INTVAL(args[2]) > 0) {
  ...
  /* Assuming that there are sets in these insns, we cannot delete
     them.  */
  if ((GET_CODE(PATTERN(insn)) == CLOBBER) || volatile_refs_p(PATTERN(insn)) ||
  ...
  if (dump_file)
    fprintf(dump_file, "mems_found = %d, cannot_delete = %s\n", mems_found,
            insn_info->cannot_delete ? "true" : "false");
...
static void dse_step1(void) {
  ...
  FOR_ALL_BB(bb) {
    ...
    if (bb->index >= NUM_FIXED_BLOCKS) {
      ...
      cse_store_info_pool = create_alloc_pool("cse_store_info_pool",
      ...
      while (ptr) {
        if (ptr->contains_cselib_groups) {
          ...
          while (s_info && !s_info->is_set)
            s_info = s_info->next;
          if (s_info && s_info->redundant_reason &&
              s_info->redundant_reason->insn && !ptr->cannot_delete) {
            if (dump_file)
              fprintf(dump_file,
...
static bool dse_step2_nospill(void) {
  unsigned int i;
  group_info_t group;
...
static bool dse_step2_spill(void) {
  unsigned int j;
  ...
  if (dump_file) {
    bitmap_print(dump_file, clear_alias_sets, "clear alias sets              ",
                 "\n");
...
void dse_record_singleton_alias_set(alias_set_type alias_set,
                                    enum machine_mode mode) {
  struct clear_alias_mode_holder tmp_holder;
  struct clear_alias_mode_holder *entry;
  void **slot;
  ...
  if (!clear_alias_sets) {
    ...
    clear_alias_mode_table =
        htab_create(11, clear_alias_mode_hash, clear_alias_mode_eq, NULL);
    clear_alias_mode_pool = create_alloc_pool(
        "clear_alias_mode_pool", sizeof(struct clear_alias_mode_holder), 100);
  }
...
void dse_invalidate_singleton_alias_set(alias_set_type alias_set) {
  if ((!gate_dse()) || !alias_set)
    return;
...
static int get_bitmap_index(group_info_t group_info, HOST_WIDE_INT offset) {
  if (offset < 0) {
...
static void scan_stores_nospill(store_info_t store_info, bitmap gen,
                                bitmap kill) {
  while (store_info) {
    HOST_WIDE_INT i;
...
static void scan_stores_spill(store_info_t store_info, bitmap gen,
                              bitmap kill) {
  while (store_info) {
    if (store_info->alias_set) {
      int index = get_bitmap_index(clear_alias_group, store_info->alias_set);
      if (index != 0) {
        bitmap_set_bit(gen, index);
        if (kill)
          bitmap_clear_bit(kill, index);
...
static void scan_reads_nospill(insn_info_t insn_info, bitmap gen, bitmap kill) {
      ...
      if (group->process_globally) {
        if (i == read_info->group_id) {
          ...
          if ((read_info->group_id < 0) &&
              canon_true_dependence(group->base_mem, QImode,
                                    group->canon_base_mem, read_info->mem,
                                    rtx_varies_p)) {
            if (kill)
              bitmap_ior_into(kill, group->group_kill);
            bitmap_and_compl_into(gen, group->group_kill);
...
static void scan_reads_spill(read_info_t read_info, bitmap gen, bitmap kill) {
  while (read_info) {
    if (read_info->alias_set) {
      int index = get_bitmap_index(clear_alias_group, read_info->alias_set);
      if (index != 0) {
        if (kill)
          bitmap_set_bit(kill, index);
        bitmap_clear_bit(gen, index);
...
static void dse_step3_scan(bool for_spills, basic_block bb) {
...
static void dse_step3(bool for_spills) {
  basic_block bb;
...
static void dse_confluence_n(edge e) {
...
static bool dse_transfer_function(int bb_index) {
  ...
  if (bb_info->kill) {
    if (bb_info->out) {
      /* Case 3 above.  */
      if (bb_info->in)
        return bitmap_ior_and_compl(bb_info->in, bb_info->gen, bb_info->out,
                                    bb_info->kill);
      else {
        bb_info->in = BITMAP_ALLOC(NULL);
        bitmap_ior_and_compl(bb_info->in, bb_info->gen, bb_info->out,
                             bb_info->kill);
        return true;
      }
    } else
...
static void dse_step4(void) {
  df_simple_dataflow(DF_BACKWARD, NULL, dse_confluence_0, dse_confluence_n,
                     dse_transfer_function, all_blocks,
...
static void dse_step5_nospill(void) {
  basic_block bb;
  FOR_EACH_BB(bb) {
    bb_info_t bb_info = bb_table[bb->index];
    insn_info_t insn_info = bb_info->last_insn;
    ...
    while (insn_info) {
      ...
      /* There may have been code deleted by the dce pass run before
         this phase.  */
      if (insn_info->insn && INSN_P(insn_info->insn) &&
          (!insn_info->cannot_delete) && (!bitmap_empty_p(v))) {
        store_info_t store_info = insn_info->store_rec;
...
static void dse_step5_spill(void) {
  basic_block bb;
  FOR_EACH_BB(bb) {
    bb_info_t bb_info = bb_table[bb->index];
    insn_info_t insn_info = bb_info->last_insn;
    ...
    while (insn_info) {
      ...
      if (insn_info->insn && INSN_P(insn_info->insn) &&
        ...
        while (store_info) {
          if (store_info->alias_set) {
            int index =
                get_bitmap_index(clear_alias_group, store_info->alias_set);
            if (index == 0 || !bitmap_bit_p(v, index)) {
              deleted = false;
              break;
            }
        ...
        if (s_info && s_info->redundant_reason &&
          ...
          if (dump_file)
            fprintf(dump_file,
                    ...
                    "same value and couldn't be "
                    "eliminated\n",
                    INSN_UID(insn_info->insn), INSN_UID(rinsn));
...
static void dse_step7(bool global_done) {
  unsigned int i;
  group_info_t group;
  basic_block bb;
...
static unsigned int rest_of_handle_dse(void) {
  ...
  if (dump_file)
    fprintf(dump_file,
            "dse: local deletions = %d, global deletions = %d, spill deletions "
            "= %d\n",
