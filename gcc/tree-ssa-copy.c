...
bool may_propagate_copy(tree dest, tree orig) {
  tree type_d = TREE_TYPE(dest);
  tree type_o = TREE_TYPE(orig);

  /* If ORIG flows in from an abnormal edge, it cannot be propagated.  */
  if (TREE_CODE(orig) == SSA_NAME && SSA_NAME_OCCURS_IN_ABNORMAL_PHI(orig))
    return false;

  /* If DEST is an SSA_NAME that flows from an abnormal edge, then it
     cannot be replaced.  */
  if (TREE_CODE(dest) == SSA_NAME && SSA_NAME_OCCURS_IN_ABNORMAL_PHI(dest))
    return false;
  ...
  if (TREE_CODE(dest) == SSA_NAME && TREE_CODE(orig) == SSA_NAME &&
    ...
    if (mt_dest && mt_orig && mt_dest != mt_orig)
      ...
      return false;
    else if (!MTAG_P(SSA_NAME_VAR(dest)) && !MTAG_P(SSA_NAME_VAR(orig)) &&
    ...
    if (SSA_NAME_PTR_INFO(orig) && SSA_NAME_PTR_INFO(dest)) {
      ...
      if (orig_ptr_info->name_mem_tag && dest_ptr_info->name_mem_tag &&
          orig_ptr_info->pt_vars && dest_ptr_info->pt_vars &&
          !bitmap_intersect_p(dest_ptr_info->pt_vars, orig_ptr_info->pt_vars))
        return false;
    }
  }
  ...
  if (TREE_CODE(dest) == SSA_NAME && !is_gimple_reg(dest)) {
    ...
    if (TREE_CODE(orig) == SSA_NAME && !is_gimple_reg(orig))
      return true;
...
void merge_alias_info(tree orig_name, tree new_name) {
  ...
  gcc_assert(
      useless_type_conversion_p(TREE_TYPE(orig_name), TREE_TYPE(new_name)));
  ...
  if (SSA_NAME_PTR_INFO(orig_name) && SSA_NAME_PTR_INFO(new_name)) {
    ...
    if (orig_ptr_info->name_mem_tag && new_ptr_info->name_mem_tag &&
        orig_ptr_info->pt_vars && new_ptr_info->pt_vars)
      gcc_assert(
          bitmap_intersect_p(new_ptr_info->pt_vars, orig_ptr_info->pt_vars));
  }
#endif
...
#if defined ENABLE_CHECKING
  gcc_assert(!(for_propagation && TREE_CODE(op) == SSA_NAME &&
               TREE_CODE(val) == SSA_NAME && !may_propagate_copy(op, val)));
#endif
...
#if defined ENABLE_CHECKING
  gcc_assert(!(TREE_CODE(val) == SSA_NAME && *op_p &&
               TREE_CODE(*op_p) == SSA_NAME &&
               !may_propagate_copy(*op_p, val)));
#endif

  if (TREE_CODE(val) == SSA_NAME) {
    if (*op_p && TREE_CODE(*op_p) == SSA_NAME &&
...
static inline prop_value_t *get_copy_of_val(tree var) {
...
static tree get_last_copy_of(tree var) {
  tree last;
  int i;
...
static inline bool set_copy_of_val(tree dest, tree first) {
...
static enum ssa_prop_result copy_prop_visit_stmt(gimple stmt,
                                                 edge *taken_edge_p,
                                                 tree *result_p) {
...
static enum ssa_prop_result copy_prop_visit_phi_node(gimple phi) {
  enum ssa_prop_result retval;
  unsigned i;
...
static void fini_copy_prop(void) {
  size_t i;
  prop_value_t *tmp;
...
static bool gate_copy_prop(void) {
  return flag_tree_copy_prop != 0;
}

struct gimple_opt_pass pass_copy_prop = {{
    GIMPLE_PASS, "copyprop", /* name */
    gate_copy_prop,          /* gate */
