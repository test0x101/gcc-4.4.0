...
bool for_each_index(tree *addr_p, bool (*cbck)(tree, tree *, void *),
                    void *data) {
  tree *nxt, *idx;
...
enum move_pos movement_possibility(gimple stmt) {
  tree lhs;
  enum move_pos ret = MOVE_POSSIBLE;

  if (flag_unswitch_loops && gimple_code(stmt) == GIMPLE_COND) {
  ...
  if (stmt_ends_bb_p(stmt) || gimple_has_volatile_ops(stmt) ||
      gimple_has_side_effects(stmt) || stmt_could_throw_p(stmt))
    return MOVE_IMPOSSIBLE;
  ...
  if (TREE_CODE(lhs) != SSA_NAME || gimple_could_trap_p(stmt))
...
static struct loop *outermost_invariant_loop(tree def, struct loop *loop) {
  gimple def_stmt;
  basic_block def_bb;
  ...
  if (lim_data != NULL && lim_data->max_loop != NULL)
    max_loop = find_common_loop(max_loop, loop_outer(lim_data->max_loop));
  if (max_loop == loop)
    return NULL;
...
static bool add_dependency(tree def, struct lim_aux_data *data,
                           struct loop *loop, bool add_cost) {
...
static struct loop *outermost_indep_loop(struct loop *outer, struct loop *loop,
                                         mem_ref_p ref) {
  struct loop *aloop;
  ...
  for (aloop = outer; aloop != loop;
       aloop = superloop_at_depth(loop, loop_depth(aloop) + 1))
    if (!bitmap_bit_p(ref->stored, aloop->num) && ref_indep_loop_p(aloop, ref))
      return aloop;
...
static mem_ref_p mem_ref_in_stmt(gimple stmt) {
  bool store;
...
static bool determine_max_movement(gimple stmt, bool must_preserve_exec) {
  ...
  if (!ZERO_SSA_OPERANDS(stmt, SSA_OP_VIRTUAL_USES)) {
    ...
    if (ref) {
      lim_data->max_loop = outermost_indep_loop(lim_data->max_loop, loop, ref);
...
static void set_level(gimple stmt, struct loop *orig_loop, struct loop *level) {
  ...
  if (lim_data != NULL && lim_data->tgt_loop != NULL)
    stmt_loop = find_common_loop(stmt_loop, loop_outer(lim_data->tgt_loop));
...
static gimple rewrite_reciprocal(gimple_stmt_iterator *bsi) {
  ...
  stmt1 = gimple_build_assign_with_ops(RDIV_EXPR, var, real_one,
...
static gimple rewrite_bittest(gimple_stmt_iterator *bsi) {
  ...
  if (TREE_CODE(lhs) != SSA_NAME || !single_imm_use(lhs, &use, &use_stmt) ||
  ...
  if (CONVERT_EXPR_CODE_P(gimple_assign_rhs_code(stmt1))) {
    ...
    if (TREE_CODE(t) != SSA_NAME || !has_single_use(t))
      return stmt;
  ...
  if (outermost_invariant_loop(b, loop_containing_stmt(stmt1)) != NULL &&
    ...
    add_referenced_var(var);
    t = fold_build2(LSHIFT_EXPR, TREE_TYPE(a), build_int_cst(TREE_TYPE(a), 1),
                    b);
...
static void determine_invariantness_stmt(
    struct dom_walk_data *dw_data ATTRIBUTE_UNUSED, basic_block bb) {
  enum move_pos pos;
  ...
  for (bsi = gsi_start_bb(bb); !gsi_end_p(bsi); gsi_next(&bsi)) {
    ...
    if (is_gimple_assign(stmt) && (get_gimple_rhs_class(gimple_assign_rhs_code(
                                       stmt)) == GIMPLE_BINARY_RHS)) {
      tree op0 = gimple_assign_rhs1(stmt);
      tree op1 = gimple_assign_rhs2(stmt);
      struct loop *ol1 =
          outermost_invariant_loop(op1, loop_containing_stmt(stmt));

      /* If divisor is invariant, convert a/b to a*(1/b), allowing reciprocal
         to be hoisted out of loop, saving expensive divide.  */
      if (pos == MOVE_POSSIBLE && gimple_assign_rhs_code(stmt) == RDIV_EXPR &&
          flag_unsafe_math_optimizations && !flag_trapping_math &&
          ol1 != NULL && outermost_invariant_loop(op0, ol1) == NULL)
      ...
      if (pos == MOVE_POSSIBLE &&
          gimple_assign_rhs_code(stmt) == BIT_AND_EXPR && integer_onep(op1) &&
          TREE_CODE(op0) == SSA_NAME && has_single_use(op0))
    ...
    if (dump_file && (dump_flags & TDF_DETAILS)) {
      ...
      fprintf(dump_file, "  invariant up to level %d, cost %d.\n\n",
              loop_depth(lim_data->max_loop), lim_data->cost);
    }
...
static void determine_invariantness(void) {
...
static void move_computations_stmt(
    struct dom_walk_data *dw_data ATTRIBUTE_UNUSED, basic_block bb) {
  struct loop *level;
  gimple_stmt_iterator bsi;
  gimple stmt;
...
static void move_computations(void) {
  struct dom_walk_data walk_data;
...
static void force_move_till_op(tree op, struct loop *orig_loop,
                               struct loop *loop) {
  gimple stmt;
...
static hashval_t memref_hash(const void *obj) {
...
static int memref_eq(const void *obj1, const void *obj2) {
...
static void free_mem_ref_locs(mem_ref_locs_p accs) {
  unsigned i;
  mem_ref_loc_p loc;
...
static void memref_free(void *obj) {
...
static mem_ref_p mem_ref_alloc(tree mem, unsigned hash, unsigned id) {
...
static mem_ref_locs_p mem_ref_locs_alloc(void) {
...
static void record_mem_ref_loc(mem_ref_p ref, struct loop *loop, gimple stmt,
                               tree *loc) {
  mem_ref_loc_p aref = XNEW(struct mem_ref_loc);
  mem_ref_locs_p accs;
  bitmap ril = VEC_index(bitmap, memory_accesses.refs_in_loop, loop->num);

  if (VEC_length(mem_ref_locs_p, ref->accesses_in_loop) <= (unsigned)loop->num)
...
static void mark_ref_stored(mem_ref_p ref, struct loop *loop) {
  for (; loop != current_loops->tree_root &&
...
static void gather_mem_refs_stmt(struct loop *loop, gimple stmt) {
  tree *mem = NULL;
  hashval_t hash;
  PTR *slot;
  mem_ref_p ref;
...
static void gather_mem_refs_in_loops(void) {
  gimple_stmt_iterator bsi;
  basic_block bb;
  struct loop *loop;
  loop_iterator li;
...
static hashval_t vtoe_hash(const void *obj) {
...
static int vtoe_eq(const void *obj1, const void *obj2) {
  const struct vop_to_refs_elt *const vtoe =
...
static void vtoe_free(void *obj) {
  struct vop_to_refs_elt *const vtoe = (struct vop_to_refs_elt *)obj;
...
static void record_vop_access(htab_t vop_to_refs, unsigned vop, unsigned ref,
                              bool stored) {
...
static bitmap get_vop_accesses(htab_t vop_to_refs, unsigned vop) {
...
static bitmap get_vop_stores(htab_t vop_to_refs, unsigned vop) {
  struct vop_to_refs_elt *const vtoe =
      (struct vop_to_refs_elt *)htab_find_with_hash(vop_to_refs, &vop, vop);
...
static void add_vop_ref_mapping(struct loop *loop, mem_ref_p ref) {
  htab_t map = VEC_index(htab_t, memory_accesses.vop_ref_map, loop->num);
  bool stored = bitmap_bit_p(ref->stored, loop->num);
  bitmap clobbers =
      VEC_index(bitmap, memory_accesses.clobbered_vops, loop->num);
  bitmap_iterator bi;
  unsigned vop;
...
static void create_vop_ref_mapping_loop(struct loop *loop) {
...
static void create_vop_ref_mapping(void) {
  loop_iterator li;
  struct loop *loop;
...
static void analyze_memory_references(void) {
  unsigned i;
  bitmap empty;
  htab_t hempty;

  memory_accesses.refs = htab_create(100, memref_hash, memref_eq, memref_free);
  memory_accesses.refs_list = NULL;
  memory_accesses.refs_in_loop = VEC_alloc(bitmap, heap, number_of_loops());
  memory_accesses.all_refs_in_loop = VEC_alloc(bitmap, heap, number_of_loops());
  memory_accesses.clobbered_vops = VEC_alloc(bitmap, heap, number_of_loops());
  memory_accesses.vop_ref_map = VEC_alloc(htab_t, heap, number_of_loops());
...
static bool cannot_overlap_p(aff_tree *diff, double_int size1,
                             double_int size2) {
  double_int d, bound;
...
static bool mem_refs_may_alias_p(tree mem1, tree mem2,
                                 struct pointer_map_t **ttae_cache) {
...
static void rewrite_mem_ref_loc(mem_ref_loc_p loc, tree tmp_var) {
...
static void get_all_locs_in_loop(struct loop *loop, mem_ref_p ref,
                                 VEC(mem_ref_loc_p, heap) * *locs) {
  mem_ref_locs_p accs;
  unsigned i;
  mem_ref_loc_p loc;
  bitmap refs = VEC_index(bitmap, memory_accesses.all_refs_in_loop, loop->num);
  struct loop *subloop;
...
static void rewrite_mem_refs(struct loop *loop, mem_ref_p ref, tree tmp_var) {
  unsigned i;
  mem_ref_loc_p loc;
...
char *get_lsm_tmp_name(tree ref, unsigned n) {
  char ns[2];
...
static void execute_sm(struct loop *loop, VEC(edge, heap) * exits,
                       mem_ref_p ref) {
  tree tmp_var;
  unsigned i;
  gimple load, store;
  ...
  tmp_var =
      make_rename_temp(TREE_TYPE(ref->mem), get_lsm_tmp_name(ref->mem, ~0));
...
static void hoist_memory_references(struct loop *loop, bitmap mem_refs,
                                    VEC(edge, heap) * exits) {
  mem_ref_p ref;
  unsigned i;
  bitmap_iterator bi;
...
static bool ref_always_accessed_p(struct loop *loop, mem_ref_p ref) {
  VEC(mem_ref_loc_p, heap) *locs = NULL;
  unsigned i;
  mem_ref_loc_p loc;
  bool ret = false;
...
static bool refs_independent_p(mem_ref_p ref1, mem_ref_p ref2) {
  if (ref1 == ref2 || bitmap_bit_p(ref1->indep_ref, ref2->id))
    return true;
  if (bitmap_bit_p(ref1->dep_ref, ref2->id))
    return false;

  if (dump_file && (dump_flags & TDF_DETAILS))
    fprintf(dump_file, "Querying dependency of refs %u and %u: ", ref1->id,
            ref2->id);

  if (mem_refs_may_alias_p(ref1->mem, ref2->mem, &memory_accesses.ttae_cache)) {
...
static void record_indep_loop(struct loop *loop, mem_ref_p ref, bool indep) {
  if (indep)
...
static bool ref_indep_loop_p_1(struct loop *loop, mem_ref_p ref) {
...
static bool ref_indep_loop_p(struct loop *loop, mem_ref_p ref) {
  bool ret;
...
static bool can_sm_ref_p(struct loop *loop, mem_ref_p ref) {
  ...
  if (tree_could_trap_p(ref->mem) && !ref_always_accessed_p(loop, ref))
    return false;
...
static void find_refs_for_sm(struct loop *loop, bitmap sm_executed,
                             bitmap refs_to_sm) {
  bitmap refs = VEC_index(bitmap, memory_accesses.all_refs_in_loop, loop->num);
  unsigned i;
  bitmap_iterator bi;
...
static bool loop_suitable_for_sm(struct loop *loop ATTRIBUTE_UNUSED,
                                 VEC(edge, heap) * exits) {
  unsigned i;
  edge ex;
...
static void store_motion_loop(struct loop *loop, bitmap sm_executed) {
...
static void fill_always_executed_in(struct loop *loop, sbitmap contains_call) {
...
static void tree_ssa_lim_initialize(void) {
