  ...
  switch (TREE_CODE(TREE_TYPE(expr))) {
    ...
    case BOOLEAN_TYPE:
      if (TYPE_PRECISION(TREE_TYPE(expr)) != POINTER_SIZE)
        expr = fold_build1(
            NOP_EXPR, lang_hooks.types.type_for_size(POINTER_SIZE, 0), expr);
...
tree convert_to_real(tree type, tree expr) {
  ...
  if (optimize && (TYPE_MODE(type) == TYPE_MODE(double_type_node) ||
...
#undef CASE_MATHFN
  ...
  if (optimize &&
      (((fcode == BUILT_IN_FLOORL || fcode == BUILT_IN_CEILL ||
         fcode == BUILT_IN_ROUNDL || fcode == BUILT_IN_RINTL ||
         fcode == BUILT_IN_TRUNCL || fcode == BUILT_IN_NEARBYINTL) &&
        (TYPE_MODE(type) == TYPE_MODE(double_type_node) ||
         TYPE_MODE(type) == TYPE_MODE(float_type_node))) ||
       ((fcode == BUILT_IN_FLOOR || fcode == BUILT_IN_CEIL ||
         fcode == BUILT_IN_ROUND || fcode == BUILT_IN_RINT ||
         fcode == BUILT_IN_TRUNC || fcode == BUILT_IN_NEARBYINT) &&
        (TYPE_MODE(type) == TYPE_MODE(float_type_node))))) {
  ...
  if (itype != type && FLOAT_TYPE_P(type))
    switch (TREE_CODE(expr)) {
      ...
      case NEGATE_EXPR:
        if (!flag_rounding_math &&
          ...
          return build1(TREE_CODE(expr), type,
                        fold(convert_to_real(type, TREE_OPERAND(expr, 0))));
        break;
      ...
      case RDIV_EXPR: {
        ...
        if (FLOAT_TYPE_P(TREE_TYPE(arg0)) && FLOAT_TYPE_P(TREE_TYPE(arg1)) &&
            DECIMAL_FLOAT_TYPE_P(itype) == DECIMAL_FLOAT_TYPE_P(type)) {
          tree newtype = type;

          if (TYPE_MODE(TREE_TYPE(arg0)) == SDmode ||
              TYPE_MODE(TREE_TYPE(arg1)) == SDmode || TYPE_MODE(type) == SDmode)
            newtype = dfloat32_type_node;
          if (TYPE_MODE(TREE_TYPE(arg0)) == DDmode ||
              TYPE_MODE(TREE_TYPE(arg1)) == DDmode || TYPE_MODE(type) == DDmode)
            newtype = dfloat64_type_node;
          if (TYPE_MODE(TREE_TYPE(arg0)) == TDmode ||
              TYPE_MODE(TREE_TYPE(arg1)) == TDmode || TYPE_MODE(type) == TDmode)
            newtype = dfloat128_type_node;
          if (newtype == dfloat32_type_node || newtype == dfloat64_type_node ||
              newtype == dfloat128_type_node) {
  ...
  switch (TREE_CODE(TREE_TYPE(expr))) {
    case REAL_TYPE:
      ...
      return build1((flag_float_store || DECIMAL_FLOAT_TYPE_P(type) ||
    ...
    case COMPLEX_TYPE:
      return convert(
          type, fold_build1(REALPART_EXPR, TREE_TYPE(TREE_TYPE(expr)), expr));
  ...
  if (optimize && (flag_unsafe_math_optimizations ||
    ...
    switch (fcode) {
      ...
      CASE_FLT_FN(BUILT_IN_ROUND)
          : if (outprec < TYPE_PRECISION(long_integer_type_node) ||
                (outprec == TYPE_PRECISION(long_integer_type_node) &&
                 !TYPE_UNSIGNED(type))) fn =
                mathfn_built_in(s_intype, BUILT_IN_LROUND);
      else if (outprec == TYPE_PRECISION(long_long_integer_type_node) &&
               !TYPE_UNSIGNED(type)) fn =
          mathfn_built_in(s_intype, BUILT_IN_LLROUND);
      break;

      CASE_FLT_FN(BUILT_IN_NEARBYINT)
          : /* Only convert nearbyint* if we can ignore math exceptions.  */
            if (flag_trapping_math) break;
      /* ... Fall through ...  */
      CASE_FLT_FN(BUILT_IN_RINT)
          : if (outprec < TYPE_PRECISION(long_integer_type_node) ||
                (outprec == TYPE_PRECISION(long_integer_type_node) &&
                 !TYPE_UNSIGNED(type))) fn =
                mathfn_built_in(s_intype, BUILT_IN_LRINT);
      else if (outprec == TYPE_PRECISION(long_long_integer_type_node) &&
               !TYPE_UNSIGNED(type)) fn =
          mathfn_built_in(s_intype, BUILT_IN_LLRINT);
      break;
  ...
  switch (TREE_CODE(intype)) {
    ...
    case REFERENCE_TYPE:
      ...
      expr = fold_build1(CONVERT_EXPR,
                         lang_hooks.types.type_for_size(POINTER_SIZE, 0), expr);
          ...
          if (outprec >= BITS_PER_WORD ||
            ...
            if (TYPE_PRECISION(typex) != inprec) {
              ...
              if (TYPE_UNSIGNED(TREE_TYPE(expr)) ||
                  (TYPE_UNSIGNED(TREE_TYPE(arg0)) &&
                   (TYPE_UNSIGNED(TREE_TYPE(arg1)) || ex_form == LSHIFT_EXPR ||
                    ex_form == RSHIFT_EXPR || ex_form == LROTATE_EXPR ||
              ...
              return convert(type,
                             fold_build2(ex_form, typex, convert(typex, arg0),
                                         convert(typex, arg1)));
            }
          }
        } break;
  ...
  switch (TREE_CODE(TREE_TYPE(expr))) {
    ...
    case COMPLEX_TYPE: {
      ...
      if (TYPE_MAIN_VARIANT(elt_type) == TYPE_MAIN_VARIANT(subtype))
        ...
        return fold_build2(
            COMPLEX_EXPR, type,
            convert(subtype, fold_build1(REALPART_EXPR,
                                         TREE_TYPE(TREE_TYPE(expr)), expr)),
            convert(subtype, fold_build1(IMAGPART_EXPR,
                                         TREE_TYPE(TREE_TYPE(expr)), expr)));
      }
    }
