...
static inline gimple allocate_phi_node(size_t len) {
  gimple phi;
  ...
  if (bucket < NUM_BUCKETS - 2 &&
      gimple_phi_capacity(VEC_index(gimple, free_phinodes[bucket], 0)) >= len) {
    free_phinode_count--;
...
static int ideal_phi_node_len(int len) {
...
gimple make_phi_node(tree var, int len) {
  gimple phi;
  int capacity, i;
  ...
  memset(phi, 0,
         (sizeof(struct gimple_statement_phi) - sizeof(struct phi_arg_d) +
...
void release_phi_node(gimple phi) {
  size_t bucket;
...
static void resize_phi_node(gimple *phi, size_t len) {
  size_t old_size, i;
  gimple new_phi;
...
gimple create_phi_node(tree var, basic_block bb) {
...
void add_phi_arg(gimple phi, tree def, edge e) {
...
static void remove_phi_arg_num(gimple phi, int i) {
