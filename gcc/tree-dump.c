...
static unsigned int queue(dump_info_p di, const_tree t, int flags) {
  ...
  dq->node =
      splay_tree_insert(di->nodes, (splay_tree_key)t, (splay_tree_value)dni);
...
void queue_and_dump_index(dump_info_p di, const char *field, const_tree t,
                          int flags) {
  unsigned int index;
  splay_tree_node n;
...
static void dump_real(dump_info_p di, const char *field,
...
static void dump_fixed(dump_info_p di, const char *field,
...
static void dequeue_and_dump(dump_info_p di) {
  ...
  if (dni->binfo_p) {
    ...
    for (ix = 0; BINFO_BASE_ITERATE(t, ix, base); ix++) {
      tree access =
          (accesses ? VEC_index(tree, accesses, ix) : access_public_node);
  ...
  switch (code) {
    ...
    case CONSTRUCTOR: {
      ...
      tree index, value;
      dump_int(di, "lngt", VEC_length(constructor_elt, CONSTRUCTOR_ELTS(t)));
...
void dump_node(const_tree t, int flags, FILE *stream) {
  struct dump_info di;
  dump_queue_p dq;
...
static const struct dump_option_value_info dump_options[] = {
    ...
    {"all",
     ~(TDF_RAW | TDF_SLIM | TDF_LINENO | TDF_TREE | TDF_RTL | TDF_IPA |
       TDF_STMTADDR | TDF_GRAPH | TDF_DIAGNOSTIC | TDF_VERBOSE | TDF_RHS_ONLY)},
    {NULL, 0}};

unsigned int dump_register(const char *suffix, const char *swtch,
                           const char *glob, int flags) {
  ...
  if (count >= extra_dump_files_alloced) {
    ...
    extra_dump_files = XRESIZEVEC(struct dump_file_info, extra_dump_files,
...
struct dump_file_info *get_dump_file_info(enum tree_dump_index phase) {
  if (phase < TDI_end)
...
char *get_dump_file_name(enum tree_dump_index phase) {
  char dump_id[10];
...
FILE *dump_begin(enum tree_dump_index phase, int *flag_ptr) {
  char *name;
  struct dump_file_info *dfi;
  FILE *stream;
...
int dump_initialized_p(enum tree_dump_index phase) {
  struct dump_file_info *dfi = get_dump_file_info(phase);
  return dfi->state > 0;
}
...
void dump_end(enum tree_dump_index phase ATTRIBUTE_UNUSED, FILE *stream) {
  fclose(stream);
}

/* Enable all tree dumps.  Return number of enabled tree dumps.  */

static int dump_enable_all(int flags) {
...
static int dump_switch_p_1(const char *arg, struct dump_file_info *dfi,
                           bool doglob) {
  ...
  while (*ptr) {
    ...
    warning(0, "ignoring unknown option %q.*s in %<-fdump-%s%>", length, ptr,
            dfi->swtch);
  found:;
    ptr = end_ptr;
  }
...
void dump_function(enum tree_dump_index phase, tree fn) {
  FILE *stream;
  int flags;
