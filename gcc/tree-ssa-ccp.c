...
static void dump_lattice_value(FILE *outf, const char *prefix,
                               prop_value_t val) {
...
tree get_symbol_constant_value(tree sym) {
  if (TREE_STATIC(sym) && TREE_READONLY(sym) && !MTAG_P(sym)) {
    ...
    if (!val && !DECL_EXTERNAL(sym) && targetm.binds_local_p(sym) &&
...
static inline void set_value_varying(tree var) {
...
static void canonicalize_float_value(prop_value_t *val) {
  ...
  if (val->lattice_val != CONSTANT || TREE_CODE(val->value) != REAL_CST)
    return;
  ...
  if (!HONOR_SIGNED_ZEROS(mode) && REAL_VALUE_MINUS_ZERO(d)) {
    val->value = build_real(type, dconst0);
    return;
  }

  if (!HONOR_NANS(mode) && REAL_VALUE_ISNAN(d)) {
    val->lattice_val = UNDEFINED;
    val->value = NULL;
    return;
  }
...
static ccp_lattice_t likely_value(gimple stmt) {
  ...
  gcc_assert(code == GIMPLE_ASSIGN || code == GIMPLE_CALL ||
             code == GIMPLE_COND || code == GIMPLE_SWITCH);
  ...
  if (code == GIMPLE_COND && is_gimple_min_invariant(gimple_cond_lhs(stmt)) &&
...
static bool surely_varying_stmt_p(gimple stmt) {
  ...
  if (gimple_code(stmt) != GIMPLE_ASSIGN && gimple_code(stmt) != GIMPLE_COND &&
      gimple_code(stmt) != GIMPLE_SWITCH && gimple_code(stmt) != GIMPLE_CALL)
    return true;
...
static void ccp_lattice_meet(prop_value_t *val1, prop_value_t *val2) {
  if (val1->lattice_val == UNDEFINED) {
    ...
    *val1 = *val2;
    ...
    ;
  } else if (val1->lattice_val == VARYING || val2->lattice_val == VARYING) {
    /* any M VARYING = VARYING.  */
    val1->lattice_val = VARYING;
    val1->value = NULL_TREE;
  } else if (val1->lattice_val == CONSTANT && val2->lattice_val == CONSTANT &&
...
bool may_propagate_address_into_dereference(tree addr, tree deref) {
  gcc_assert(INDIRECT_REF_P(deref) && TREE_CODE(addr) == ADDR_EXPR);
  ...
  if (is_gimple_min_invariant(addr) &&
      (!TREE_THIS_VOLATILE(deref) || TYPE_VOLATILE(TREE_TYPE(addr))))
  ...
  switch (gimple_code(stmt)) {
      ...
      switch (get_gimple_rhs_class(subcode)) {
        case GIMPLE_SINGLE_RHS: {
          ...
          /* Handle propagating invariant addresses into address operations.
            ...
            if (TREE_CODE(*base) == INDIRECT_REF &&
              ...
              if (val->lattice_val == CONSTANT &&
                  ...
                  may_propagate_address_into_dereference(val->value, *base)) {
          ...
          if (kind == tcc_reference) {
            if (TREE_CODE(rhs) == VIEW_CONVERT_EXPR &&
              ...
              if (val->lattice_val == CONSTANT)
                return fold_unary(VIEW_CONVERT_EXPR, TREE_TYPE(rhs),
                                  val->value);
            }
        ...
        case GIMPLE_UNARY_RHS: {
          ...
          /* Conversions are useless for CCP purposes if they are
             ...
             allowed places.  */
          if (CONVERT_EXPR_CODE_P(subcode) && POINTER_TYPE_P(TREE_TYPE(lhs)) &&
            ...
            if (!useless_type_conversion_p(TREE_TYPE(lhs), TREE_TYPE(op0)) &&
                ((tem = maybe_fold_offset_to_address(
                      op0, integer_zero_node, TREE_TYPE(lhs))) != NULL_TREE))
              return tem;
            return op0;
          }

          return fold_unary_ignore_overflow(subcode, gimple_expr_type(stmt),
                                            op0);
        }
        ...
        case GIMPLE_BINARY_RHS: {
          ...
          if (gimple_assign_rhs_code(stmt) == POINTER_PLUS_EXPR &&
            ...
            tree lhs = gimple_assign_lhs(stmt);
            tree tem = maybe_fold_offset_to_address(op0, op1, TREE_TYPE(lhs));
            if (tem != NULL_TREE)
              return tem;
          }
    ...
    case GIMPLE_CALL: {
      ...
      if (TREE_CODE(fn) == ADDR_EXPR &&
        ...
        call = build_call_array(gimple_call_return_type(stmt), fn,
                                gimple_call_num_args(stmt), args);
...
tree fold_const_aggregate_ref(tree t) {
  prop_value_t *value;
  ...
  switch (TREE_CODE(t)) {
    case ARRAY_REF:
      ...
      if (ctor == NULL_TREE ||
          (TREE_CODE(ctor) != CONSTRUCTOR && TREE_CODE(ctor) != STRING_CST) ||
          !TREE_STATIC(ctor))
        return NULL_TREE;
      ...
      switch (TREE_CODE(idx)) {
        case SSA_NAME:
          if ((value = get_value(idx)) && value->lattice_val == CONSTANT &&
      ...
      if (TREE_CODE(ctor) == STRING_CST) {
        if ((TYPE_MODE(TREE_TYPE(t)) ==
          ...
          return build_int_cst_type(
              TREE_TYPE(t), (TREE_STRING_POINTER(ctor)[TREE_INT_CST_LOW(idx)]));
        return NULL_TREE;
      }
    ...
    case COMPONENT_REF:
      ...
      if (ctor == NULL_TREE || TREE_CODE(ctor) != CONSTRUCTOR ||
          !TREE_STATIC(ctor))
        return NULL_TREE;
    ...
    case INDIRECT_REF: {
      ...
      if (TREE_CODE(base) == SSA_NAME && (value = get_value(base)) &&
...
static prop_value_t evaluate_stmt(gimple stmt) {
  prop_value_t val;
...
static tree maybe_fold_offset_to_array_ref(tree base, tree offset,
                                           tree orig_type,
  ...
  if (integer_zerop(offset)) {
    ...
    if (TREE_CODE(elt_size) != INTEGER_CST ||
        div_and_round_double(TRUNC_DIV_EXPR, 0, soffset.low, soffset.high,
                             TREE_INT_CST_LOW(elt_size),
                             TREE_INT_CST_HIGH(elt_size), &lquo, &hquo, &lrem,
                             &hrem) ||
        lrem || hrem)
      return NULL_TREE;
  ...
  if (domain_type && TYPE_MIN_VALUE(domain_type)) {
    if (!allow_negative_idx &&
      ...
      return NULL_TREE;
  } else if (!allow_negative_idx && compare_tree_int(idx, 0) < 0)
    return NULL_TREE;
...
static tree maybe_fold_offset_to_component_ref(tree record_type, tree base,
                                               tree offset, tree orig_type,
                                               bool base_is_ptr) {
  ...
  for (f = TYPE_FIELDS(record_type); f; f = TREE_CHAIN(f)) {
    ...
    if (cmp == 0 && useless_type_conversion_p(orig_type, field_type)) {
      if (base_is_ptr)
    ...
    if (!DECL_SIZE_UNIT(f) || TREE_CODE(DECL_SIZE_UNIT(f)) != INTEGER_CST)
      continue;
    ...
    if (ret)
      return ret;
    ret = maybe_fold_offset_to_component_ref(field_type, new_base, t, orig_type,
                                             false);
    if (ret)
      return ret;
  }
  ...
  if (t)
    return t;
  return maybe_fold_offset_to_component_ref(field_type, base, offset, orig_type,
                                            false);
}
...
tree maybe_fold_offset_to_reference(tree base, tree offset, tree orig_type) {
  tree ret;
  tree type;
  ...
  if (TREE_CODE(base) == ADDR_EXPR) {
    ...
    if (handled_component_p(base)) {
      ...
      tree newbase;
      newbase = get_ref_base_and_extent(base, &sub_offset, &size, &maxsize);
      gcc_assert(newbase);
      if (size == maxsize && size != -1 &&
          !(sub_offset & (BITS_PER_UNIT - 1))) {
        base = newbase;
        if (sub_offset)
          offset = int_const_binop(
              PLUS_EXPR, offset,
              build_int_cst(TREE_TYPE(offset), sub_offset / BITS_PER_UNIT), 1);
      }
    }
...
tree maybe_fold_offset_to_address(tree addr, tree offset, tree orig_type) {
  tree t;

  gcc_assert(POINTER_TYPE_P(TREE_TYPE(addr)) && POINTER_TYPE_P(orig_type));
  ...
  if (t != NULL_TREE) {
    ...
    if ((TREE_CODE(orig) == ARRAY_REF ||
        ...
        (TREE_CODE(t) == ARRAY_REF || TREE_CODE(t) == COMPONENT_REF) &&
        !operand_equal_p(
            TREE_CODE(orig) == ARRAY_REF ? TREE_OPERAND(orig, 0) : orig,
            TREE_CODE(t) == ARRAY_REF ? TREE_OPERAND(t, 0) : t, 0))
      return NULL_TREE;
...
static tree maybe_fold_stmt_indirect(tree expr, tree base, tree offset) {
  ...
  if (TREE_CODE(base) == POINTER_PLUS_EXPR) {
    ...
    offset =
        fold_convert(sizetype, int_const_binop(PLUS_EXPR, offset, offset2, 1));
  }
  ...
  if (TREE_CODE(base) == ADDR_EXPR) {
    ...
    t = maybe_fold_offset_to_reference(base_addr, offset, TREE_TYPE(expr));
    if (t) {
    ...
    if (POINTER_TYPE_P(TREE_TYPE(base))) {
      t = maybe_fold_offset_to_reference(base, offset, TREE_TYPE(expr));
      if (t)
        return t;
    }
  }
...
tree maybe_fold_stmt_addition(tree res_type, tree op0, tree op1) {
  tree ptd_type;
  tree t;
  ...
  if (VOID_TYPE_P(ptd_type) && TREE_CODE(TREE_TYPE(op0)) == ARRAY_TYPE)
    ptd_type = TREE_TYPE(TREE_TYPE(op0));

  /* At which point we can try some of the same things as for indirects.  */
  t = maybe_fold_offset_to_array_ref(op0, op1, ptd_type, true);
  if (!t)
    t = maybe_fold_offset_to_component_ref(TREE_TYPE(op0), op0, op1, ptd_type,
                                           false);
  if (t)
  ...
  switch (TREE_CODE(expr)) {
    case INDIRECT_REF:
      ...
      *walk_subtrees = 0;
      ...
      if (wi->is_lhs && t && TREE_CODE(t) == INTEGER_CST)
        t = NULL_TREE;
      if (!t && TREE_CODE(TREE_OPERAND(expr, 0)) == ADDR_EXPR)
    ...
    case POINTER_PLUS_EXPR:
      ...
      *walk_subtrees = 0;
      ...
      t = maybe_fold_stmt_addition(TREE_TYPE(expr), TREE_OPERAND(expr, 0),
                                   TREE_OPERAND(expr, 1));
      break;
    ...
    case COND_EXPR:
      if (COMPARISON_CLASS_P(TREE_OPERAND(expr, 0))) {
        ...
        fold_defer_overflow_warnings();
        tem = fold_binary(TREE_CODE(op0), TREE_TYPE(op0), TREE_OPERAND(op0, 0),
                          TREE_OPERAND(op0, 1));
...
static bool get_maxval_strlen(tree arg, tree *length, bitmap visited,
                              int type) {
  tree var, val;
  gimple def_stmt;
  ...
  if (TREE_CODE(arg) != SSA_NAME) {
    ...
    /* We can end up with &(*iftmp_1)[0] here as well, so handle it.  */
      ...
      if (TREE_CODE(aop0) == INDIRECT_REF &&
        ...
        return get_maxval_strlen(TREE_OPERAND(aop0, 0), length, visited, type);
    }

    if (type == 2) {
      val = arg;
      if (TREE_CODE(val) != INTEGER_CST || tree_int_cst_sgn(val) < 0)
        return false;
    } else
      val = c_strlen(arg, 1);
    if (!val)
    ...
    if (*length) {
      if (type > 0) {
        if (TREE_CODE(*length) != INTEGER_CST || TREE_CODE(val) != INTEGER_CST)
          return false;
...
static tree ccp_fold_builtin(gimple stmt) {
  tree result, val[3];
  tree callee, a;
  int arg_idx, type;
  bitmap visited;
  ...
  switch (DECL_FUNCTION_CODE(callee)) {
    case BUILT_IN_STRLEN:
      if (val[0] && nargs == 1) {
        tree new_val = fold_convert(TREE_TYPE(gimple_call_lhs(stmt)), val[0]);

        /* If the result is not a valid gimple value, or not a cast
           of a valid gimple value, then we can not use the result.  */
        if (is_gimple_val(new_val) || (is_gimple_cast(new_val) &&
    ...
    case BUILT_IN_STRCPY:
      if (val[1] && is_gimple_val(val[1]) && nargs == 2)
        result = fold_builtin_strcpy(callee, gimple_call_arg(stmt, 0),
                                     gimple_call_arg(stmt, 1), val[1]);
      break;

    case BUILT_IN_STRNCPY:
      if (val[1] && is_gimple_val(val[1]) && nargs == 3)
        result = fold_builtin_strncpy(callee, gimple_call_arg(stmt, 0),
                                      gimple_call_arg(stmt, 1),
                                      gimple_call_arg(stmt, 2), val[1]);
      break;

    case BUILT_IN_FPUTS:
      if (nargs == 2)
        result =
            fold_builtin_fputs(gimple_call_arg(stmt, 0),
                               gimple_call_arg(stmt, 1), ignore, false, val[0]);
      break;
    ...
    case BUILT_IN_FPUTS_UNLOCKED:
      if (nargs == 2)
        result =
            fold_builtin_fputs(gimple_call_arg(stmt, 0),
                               gimple_call_arg(stmt, 1), ignore, true, val[0]);
      break;
    ...
    case BUILT_IN_MEMSET_CHK:
      if (val[2] && is_gimple_val(val[2]) && nargs == 4)
        result = fold_builtin_memory_chk(
            callee, gimple_call_arg(stmt, 0), gimple_call_arg(stmt, 1),
            gimple_call_arg(stmt, 2), gimple_call_arg(stmt, 3), val[2], ignore,
    ...
    case BUILT_IN_STPCPY_CHK:
      if (val[1] && is_gimple_val(val[1]) && nargs == 3)
        result = fold_builtin_stxcpy_chk(callee, gimple_call_arg(stmt, 0),
                                         gimple_call_arg(stmt, 1),
                                         gimple_call_arg(stmt, 2), val[1],
                                         ignore, DECL_FUNCTION_CODE(callee));
      break;

    case BUILT_IN_STRNCPY_CHK:
      if (val[2] && is_gimple_val(val[2]) && nargs == 4)
        result = fold_builtin_strncpy_chk(
            gimple_call_arg(stmt, 0), gimple_call_arg(stmt, 1),
            gimple_call_arg(stmt, 2), gimple_call_arg(stmt, 3), val[2]);
      break;
...
static tree fold_gimple_assign(gimple_stmt_iterator *si) {
  ...
  switch (get_gimple_rhs_class(subcode)) {
    ...
    case GIMPLE_UNARY_RHS: {
      ...
      if (result) {
        ...
        if (CONVERT_EXPR_CODE_P(subcode) && TREE_CODE(result) == INTEGER_CST &&
    ...
    case GIMPLE_BINARY_RHS:
      ...
      if (gimple_assign_rhs_code(stmt) == POINTER_PLUS_EXPR) {
        ...
        result = maybe_fold_stmt_addition(type, gimple_assign_rhs1(stmt),
                                          gimple_assign_rhs2(stmt));
      }

      if (!result)
        result =
            fold_binary(subcode, TREE_TYPE(gimple_assign_lhs(stmt)),
...
static bool fold_gimple_cond(gimple stmt) {
  tree result = fold_binary(gimple_cond_code(stmt), boolean_type_node,
...
static bool fold_gimple_call(gimple_stmt_iterator *gsi) {
  ...
  if (callee && DECL_BUILT_IN(callee)) {
    ...
    if (TREE_CODE(callee) == OBJ_TYPE_REF && lang_hooks.fold_obj_type_ref &&
...
bool fold_stmt_inplace(gimple stmt) {
  tree res;
  struct fold_stmt_r_data fold_stmt_r_data;
  struct walk_stmt_info wi;
...
static tree optimize_stack_restore(gimple_stmt_iterator i) {
  ...
  if (gimple_code(call) != GIMPLE_CALL || gimple_call_num_args(call) != 1 ||
  ...
  if (gsi_end_p(i) &&
      (!single_succ_p(bb) || single_succ_edge(bb)->dest != EXIT_BLOCK_PTR))
    return NULL_TREE;
...
static tree optimize_stdarg_builtin(gimple call) {
  ...
  switch (DECL_FUNCTION_CODE(callee)) {
    case BUILT_IN_VA_START:
      if (!va_list_simple_ptr || targetm.expand_builtin_va_start != NULL ||
      ...
      rhs = build_call_expr(built_in_decls[BUILT_IN_NEXT_ARG], 1,
                            integer_zero_node);
    ...
    case BUILT_IN_VA_COPY:
      ...
      if (TYPE_MAIN_VARIANT(TREE_TYPE(rhs)) != TYPE_MAIN_VARIANT(cfun_va_list))
        return NULL_TREE;
...
static void gimplify_and_update_call_from_tree(gimple_stmt_iterator *si_p,
                                               tree expr) {
  tree lhs;
