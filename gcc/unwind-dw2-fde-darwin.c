...
static inline const fde *examine_objects(void *pc, struct dwarf_eh_bases *bases,
                                         int dont_alloc) {
    ...
    if ((image->examined_p & EXAMINED_IMAGE_MASK) == 0) {
...
#else
        fde = getsectdatafromheader(image->mh, "__TEXT", "__eh_frame", &sz);
#endif
        if (fde != NULL)
      ...
      if (fde != NULL && sz > 0) {
        ...
        if (!dont_alloc) {
          ...
          image->examined_p |=
              (EXAMINED_IMAGE_MASK | DESTRUCTOR_MAY_BE_CALLED_LIVE);
...
const fde *_Unwind_Find_FDE(void *pc, struct dwarf_eh_bases *bases) {
  ...
  the_obj_info = _keymgr_get_and_lock_processwide_ptr(KEYMGR_GCC3_DW2_OBJ_LIST);
  if (!the_obj_info)
