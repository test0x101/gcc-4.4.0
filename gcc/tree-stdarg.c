...
static unsigned HOST_WIDE_INT va_list_counter_bump(struct stdarg_info *si,
  ...
  while (lhs) {
    ...
    if ((rhs_code == POINTER_PLUS_EXPR || rhs_code == PLUS_EXPR) &&
        TREE_CODE(gimple_assign_rhs1(stmt)) == SSA_NAME &&
...
static tree find_va_list_reference(tree *tp,
  ...
  if (TREE_CODE(var) == VAR_DECL && bitmap_bit_p(va_list_vars, DECL_UID(var)))
    return var;
...
static void va_list_counter_op(struct stdarg_info *si, tree ap, tree var,
                               bool gpr_p, bool write_p) {
...
static bool va_list_counter_struct_op(struct stdarg_info *si, tree ap, tree var,
                                      bool write_p) {
  tree base;
...
static bool va_list_ptr_read(struct stdarg_info *si, tree ap, tree tem) {
  ...
  if (TREE_CODE(tem) != SSA_NAME ||
      bitmap_bit_p(si->va_list_vars, DECL_UID(SSA_NAME_VAR(tem))) ||
  ...
  bitmap_set_bit(si->va_list_escape_vars, DECL_UID(SSA_NAME_VAR(tem)));
  return true;
}
...
static void check_va_list_escapes(struct stdarg_info *si, tree lhs, tree rhs) {
  ...
  if (TREE_CODE(rhs) != SSA_NAME ||
      !bitmap_bit_p(si->va_list_escape_vars, DECL_UID(SSA_NAME_VAR(rhs))))
    return;
  ...
  bitmap_set_bit(si->va_list_escape_vars, DECL_UID(SSA_NAME_VAR(lhs)));
}
  ...
  FOR_EACH_BB(bb) {
    ...
    for (i = gsi_start_bb(bb); !gsi_end_p(i); gsi_next(&i)) {
      ...
      FOR_EACH_SSA_TREE_OPERAND(use, stmt, iter, SSA_OP_ALL_USES) {
        if (!bitmap_bit_p(si->va_list_escape_vars, DECL_UID(SSA_NAME_VAR(use))))
          continue;
        ...
        if (is_gimple_assign(stmt)) {
          ...
          if (gimple_assign_rhs_code(stmt) == INDIRECT_REF &&
              TREE_OPERAND(rhs, 0) == use && TYPE_SIZE_UNIT(TREE_TYPE(rhs)) &&
          ...
          if (rhs == use &&
              ((rhs_code == POINTER_PLUS_EXPR &&
               ...
               gimple_assign_cast_p(stmt) ||
               (get_gimple_rhs_class(rhs_code) == GIMPLE_SINGLE_RHS))) {
            ...
            if (TREE_CODE(lhs) == VAR_DECL &&
                bitmap_bit_p(si->va_list_vars, DECL_UID(lhs)))
              continue;
          }
        }
...
static bool gate_optimize_stdarg(void) {
...
static unsigned int execute_optimize_stdarg(void) {
  basic_block bb;
  ...
  if (!va_list_simple_ptr && va_list_gpr_counter_field == NULL_TREE &&
  ...
  FOR_EACH_BB(bb) {
    ...
    for (i = gsi_start_bb(bb); !gsi_end_p(i) && !va_list_escapes;
         gsi_next(&i)) {
      gimple stmt = gsi_stmt(i);
  ...
  if (!va_list_escapes && va_list_simple_ptr &&
      !bitmap_empty_p(si.va_list_escape_vars) && check_all_va_list_escapes(&si))
    va_list_escapes = true;
...
finish:
  ...
  free(si.offsets);
  if (dump_file) {
    fprintf(dump_file, "%s: va_list escapes %d, needs to save ", funcname,
            (int)va_list_escapes);
