      ...
      if (!inf->ignore && !inf->on_tree) {
        ...
        if (dump_file)
          fprintf(dump_file, "Edge %d to %d instrumented%s\n", e->src->index,
                  e->dest->index, EDGE_CRITICAL_P(e) ? " (and split)" : "");
...
static bool is_edge_inconsistent(VEC(edge, gc) * edges) {
  edge e;
  edge_iterator ei;
  FOR_EACH_EDGE(e, ei, edges) {
    if (!EDGE_INFO(e)->ignore) {
      if (e->count < 0 &&
          (!(e->flags & EDGE_FAKE) || !block_ends_with_call_p(e->src))) {
        if (dump_file) {
          fprintf(
              dump_file,
...
static bool is_inconsistent(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    if (bb->count < 0) {
      if (dump_file) {
        fprintf(dump_file, "BB %i count is negative " HOST_WIDEST_INT_PRINT_DEC,
                bb->index, bb->count);
    ...
    if (bb->count != sum_edge_counts(bb->preds)) {
      if (dump_file) {
        fprintf(dump_file,
                "BB %i count does not match sum of incomming "
                "edges " HOST_WIDEST_INT_PRINT_DEC
                " should be " HOST_WIDEST_INT_PRINT_DEC,
                bb->index, bb->count, sum_edge_counts(bb->preds));
        dump_bb(bb, dump_file, 0);
      }
      inconsistent = true;
    }
    if (bb->count != sum_edge_counts(bb->succs) &&
      ...
      if (dump_file) {
        fprintf(dump_file,
                "BB %i count does not match sum of outgoing "
                "edges " HOST_WIDEST_INT_PRINT_DEC
                " should be " HOST_WIDEST_INT_PRINT_DEC,
                bb->index, bb->count, sum_edge_counts(bb->succs));
        dump_bb(bb, dump_file, 0);
      }
      inconsistent = true;
    }
    ...
    if (!EDGE_INFO(e)->ignore && !EDGE_INFO(e)->on_tree) {
      ...
      if (exec_counts) {
        ...
        if (e->count > profile_info->sum_max) {
          error(
              "corrupted profile info: edge from %i to %i exceeds maximal "
              "count",
      ...
      if (dump_file) {
        fprintf(dump_file, "\nRead edge from %i to %i, count:", bb->index,
                e->dest->index);
...
static void compute_branch_probabilities(void) {
  ...
  FOR_BB_BETWEEN(bb, ENTRY_BLOCK_PTR, NULL, next_bb) {
    ...
    if (bb->count < 0) {
      error(
          "corrupted profile info: number of iterations for basic block %d "
          "thought to be %i",
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      /* Function may return twice in the cased the called function is
         ...
         point.  */
      if ((e->count < 0 && e->dest == EXIT_BLOCK_PTR) ||
      ...
      if (e->count < 0 || e->count > bb->count) {
        error(
            "corrupted profile info: number of executions for edge %d-%d "
            "thought to be %i",
    ...
    if (bb->count) {
      ...
      if (bb->index >= NUM_FIXED_BLOCKS && block_ends_with_condjump_p(bb) &&
...
static void output_location(char const *file_name, int line,
...
void branch_prob(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      ...
      if (last && gimple_has_location(last) &&
          e->goto_locus != UNKNOWN_LOCATION && !single_succ_p(bb) &&
    ...
    FOR_EACH_BB(bb) {
      ...
      for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
        ...
        if (gimple_has_location(stmt))
          output_location(gimple_filename(stmt), gimple_lineno(stmt), &offset,
                          bb);
      }
...
static void find_spanning_tree(struct edge_list *el) {
  ...
  for (i = 0; i < num_edges; i++) {
    edge e = INDEX_EDGE(el, i);
    if (!EDGE_INFO(e)->ignore && find_group(e->src) != find_group(e->dest)) {
      if (dump_file)
...
void tree_register_profile_hooks(void) {
