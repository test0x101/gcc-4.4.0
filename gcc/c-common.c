...
static tree handle_no_instrument_function_attribute(tree *, tree, tree, int,
                                                    bool *);
...
const struct attribute_spec c_common_attribute_table[] = {
    ...
    {"gnu_inline", 0, 0, true, false, false, handle_gnu_inline_attribute},
    {"artificial", 0, 0, true, false, false, handle_artificial_attribute},
    {"flatten", 0, 0, true, false, false, handle_flatten_attribute},
    ...
    {"constructor", 0, 1, true, false, false, handle_constructor_attribute},
    ...
    {"weakref", 0, 1, true, false, false, handle_weakref_attribute},
    {"no_instrument_function", 0, 0, true, false, false,
     handle_no_instrument_function_attribute},
    {"malloc", 0, 0, true, false, false, handle_malloc_attribute},
    {"returns_twice", 0, 0, true, false, false, handle_returns_twice_attribute},
    ...
    {"visibility", 1, 1, false, false, false, handle_visibility_attribute},
    ...
    {"alloc_size", 1, 2, false, true, true, handle_alloc_size_attribute},
    {"cold", 0, 0, true, false, false, handle_cold_attribute},
    {"hot", 0, 0, true, false, false, handle_hot_attribute},
    {"warning", 1, 1, true, false, false, handle_error_attribute},
    {"error", 1, 1, true, false, false, handle_error_attribute},
    {"target", 1, -1, true, false, false, handle_target_attribute},
...
void start_fname_decls(void) {
  ...
  if (saved || saved_function_name_decls)
    ...
    saved_function_name_decls =
        tree_cons(saved, NULL_TREE, saved_function_name_decls);
}
...
void constant_expression_error(tree value) {
...
void warn_logical_operator(enum tree_code code, tree arg1, tree arg2) {
  switch (code) {
    case TRUTH_ANDIF_EXPR:
    case TRUTH_ORIF_EXPR:
    case TRUTH_OR_EXPR:
    case TRUTH_AND_EXPR:
      if (!TREE_NO_WARNING(arg1) && INTEGRAL_TYPE_P(TREE_TYPE(arg1)) &&
          !CONSTANT_CLASS_P(arg1) && TREE_CODE(arg2) == INTEGER_CST &&
          !integer_zerop(arg2)) {
...
bool strict_aliasing_warning(tree otype, tree type, tree expr) {
  if (!(flag_strict_aliasing && POINTER_TYPE_P(type) && POINTER_TYPE_P(otype) &&
  ...
  if ((warn_strict_aliasing > 1) && TREE_CODE(expr) == ADDR_EXPR &&
    ...
    if (!COMPLETE_TYPE_P(TREE_TYPE(type)) && warn_strict_aliasing == 2) {
      ...
      alias_set_type set1 = get_alias_set(TREE_TYPE(TREE_OPERAND(expr, 0)));
    ...
    alias_set_type set2 = get_alias_set(TREE_TYPE(type));
    if (!COMPLETE_TYPE_P(type) || !alias_sets_must_conflict_p(set1, set2)) {
      warning(OPT_Wstrict_aliasing,
              "dereferencing type-punned "
              "pointer might break strict-aliasing rules");
      return true;
...
void check_main_parameter_types(tree decl) {
  tree args;
  int argct = 0;

  for (args = TYPE_ARG_TYPES(TREE_TYPE(decl)); args; args = TREE_CHAIN(args)) {
  ...
  if (argct > 0 && (argct < 2 || argct > 3))
    pedwarn(input_location, OPT_Wmain, "%q+D takes only zero or two arguments",
            decl);
}
...
bool vector_types_convertible_p(const_tree t1, const_tree t2,
                                bool emit_lax_note) {
  ...
  convertible_lax =
      (tree_int_cst_equal(TYPE_SIZE(t1), TYPE_SIZE(t2)) &&
       ...
       (INTEGRAL_TYPE_P(TREE_TYPE(t1)) == INTEGRAL_TYPE_P(TREE_TYPE(t2))));
  ...
  if ((TYPE_PRECISION(TREE_TYPE(arg0)) < TYPE_PRECISION(result_type)) &&
    ...
    return c_common_signed_or_unsigned_type(
        unsigned0, common_type(TREE_TYPE(arg0), TREE_TYPE(arg1)));
           ...
           (type =
                c_common_signed_or_unsigned_type(unsigned1, TREE_TYPE(arg1))) &&
           !POINTER_TYPE_P(type) && int_fits_type_p(arg0, type))
    return type;

  else if (TREE_CODE(arg1) == INTEGER_CST && (unsigned0 || !uns) &&
           (TYPE_PRECISION(TREE_TYPE(arg0)) < TYPE_PRECISION(result_type)) &&
           (type =
                c_common_signed_or_unsigned_type(unsigned0, TREE_TYPE(arg0))) &&
           !POINTER_TYPE_P(type) && int_fits_type_p(arg1, type))
    return type;
  ...
  switch (TREE_CODE(expr)) {
    ...
    case TRUTH_NOT_EXPR:
      ...
      if (TYPE_PRECISION(type) == 1 && !TYPE_UNSIGNED(type))
        warning(OPT_Wconversion, "conversion to %qT from boolean expression",
                type);
      return;
    ...
    case INTEGER_CST:
      ...
      if (give_warning)
        warning(OPT_Wconversion, "conversion to %qT alters %qT constant value",
                type, expr_type);
    ...
    default: /* 'expr' is not a constant.  */
      ...
      if (TREE_CODE(expr_type) == REAL_TYPE && TREE_CODE(type) == INTEGER_TYPE)
        ...
        if (TREE_CODE(expr) == BIT_AND_EXPR ||
          ...
          expr_type = shorten_binary_op(expr_type, TREE_OPERAND(expr, 0),
                                        TREE_OPERAND(expr, 1),
                                        /* bitwise */ 1);
          ...
          if (TREE_CODE(expr) == BIT_AND_EXPR) {
            ...
            else if ((TREE_CODE(op0) == INTEGER_CST && unsigned0 &&
                      int_fits_type_p(op0, type)) ||
                     (TREE_CODE(op1) == INTEGER_CST && unsigned1 &&
...
void warnings_for_convert_and_check(tree type, tree expr, tree result) {
...
static struct tlist *new_tlist(struct tlist *next, tree t, tree writer) {
  struct tlist *l;
...
static void add_tlist(struct tlist **to, struct tlist *add, tree exclude_writer,
                      int copy) {
  while (add) {
...
static void warn_for_collisions_1(tree written, tree writer, struct tlist *list,
                                  int only_writes) {
  struct tlist *tmp;
  ...
  while (list) {
    if (list->expr == written && list->writer != writer &&
        (!only_writes || list->writer) && DECL_NAME(list->expr)) {
      warned_ids = new_tlist(warned_ids, written, NULL_TREE);
      warning_at(
          EXPR_HAS_LOCATION(writer) ? EXPR_LOCATION(writer) : input_location,
...
static void verify_tree(tree x, struct tlist **pbefore_sp,
                        struct tlist **pno_sp, tree writer) {
...
static bool check_case_bounds(tree type, tree orig_type, tree *case_low_p,
                              tree *case_high_p) {
  ...
  if (tree_int_cst_compare(case_high, min_value) >= 0 &&
    ...
    warning(0,
            "lower value in case label range"
  ...
  if (tree_int_cst_compare(case_low, max_value) <= 0 &&
      tree_int_cst_compare(case_high, max_value) > 0) {
    warning(0,
            "upper value in case label range"
...
tree c_common_fixed_point_type_for_size(unsigned int ibit, unsigned int fbit,
  ...
  if (mode == VOIDmode || !targetm.scalar_mode_supported_p(mode)) {
    sorry(
        "GCC cannot support operators with integer types and "
...
tree c_common_type_for_mode(enum machine_mode mode, int unsignedp) {
  ...
  if (mode == TYPE_MODE(build_pointer_type(char_type_node)))
    return (unsignedp ? make_unsigned_type(GET_MODE_PRECISION(mode))
...
#define C_COMMON_FIXED_TYPES(NAME)                                       \
  ...
  if (type1 == NAME##_type_node || type1 == unsigned_##NAME##_type_node) \
    return unsignedp ? unsigned_##NAME##_type_node : NAME##_type_node;   \
...
#define C_COMMON_FIXED_MODE_TYPES(NAME)                          \
  if (type1 == NAME##_type_node || type1 == u##NAME##_type_node) \
    return unsignedp ? u##NAME##_type_node : NAME##_type_node;
...
#define C_COMMON_FIXED_MODE_TYPES_SAT(NAME)                                  \
  if (type1 == sat_##NAME##_type_node || type1 == sat_##u##NAME##_type_node) \
    return unsignedp ? sat_##u##NAME##_type_node : sat_##NAME##_type_node;
...
void binary_op_error(location_t location, enum tree_code code, tree type0,
                     tree type1) {
  const char *opname;
  ...
  error_at(location, "invalid operands to binary %s (have %qT and %qT)", opname,
           type0, type1);
}
...
tree shorten_compare(tree *op0_ptr, tree *op1_ptr, tree *restype_ptr,
  ...
  if (TREE_CONSTANT(primop0) && !integer_zerop(primop1) &&
      !real_zerop(primop1) && !fixed_zerop(primop1)) {
    tree tem = primop0;
    int temi = unsignedp0;
    primop0 = primop1;
    primop1 = tem;
  ...
  if (!real1 && !real2 && TREE_CODE(TREE_TYPE(primop0)) != FIXED_POINT_TYPE &&
    ...
    type = c_common_signed_or_unsigned_type(unsignedp0, TREE_TYPE(primop0));
    ...
    if (TREE_TYPE(primop1) != *restype_ptr) {
      ...
      primop1 = force_fit_type_double(*restype_ptr, TREE_INT_CST_LOW(primop1),
  ...
  /* If args' natural types are both narrower than nominal type
    ...
    type = c_common_signed_or_unsigned_type(
        unsignedp0 || TYPE_UNSIGNED(*restype_ptr), type);
    /* Make sure shorter operand is extended the right way
       to match the longer operand.  */
    primop0 = convert(
        c_common_signed_or_unsigned_type(unsignedp0, TREE_TYPE(primop0)),
        primop0);
    primop1 = convert(
        c_common_signed_or_unsigned_type(unsignedp1, TREE_TYPE(primop1)),
        primop1);
  } else {
    ...
    if (!real1 && !real2 && integer_zerop(primop1) &&
        ...
        case GE_EXPR:
          ...
          if (warn_type_limits && !in_system_header &&
              !(TREE_CODE(primop0) == INTEGER_CST &&
                !TREE_OVERFLOW(convert(c_common_signed_type(type), primop0))))
            warning(OPT_Wtype_limits,
      ...
      if (value != 0) {
        /* Don't forget to evaluate PRIMOP0 if it has side effects.  */
        if (TREE_SIDE_EFFECTS(primop0))
          return build2(COMPOUND_EXPR, TREE_TYPE(value), primop0, value);
        return value;
      }
    }
  }
...
tree pointer_int_sum(enum tree_code resultcode, tree ptrop, tree intop) {
  ...
  if ((TREE_CODE(intop) == PLUS_EXPR || TREE_CODE(intop) == MINUS_EXPR) &&
      !TREE_CONSTANT(intop) && TREE_CONSTANT(TREE_OPERAND(intop, 1)) &&
    ...
    ptrop =
        build_binary_op(EXPR_LOCATION(TREE_OPERAND(intop, 1)), subcode, ptrop,
  ...
  intop = convert(sizetype,
                  build_binary_op(EXPR_LOCATION(intop), MULT_EXPR, intop,
...
bool decl_with_nonnull_addr_p(const_tree expr) {
  return (DECL_P(expr) && (TREE_CODE(expr) == PARM_DECL ||
                           TREE_CODE(expr) == LABEL_DECL || !DECL_WEAK(expr)));
}
...
tree c_common_truthvalue_conversion(location_t location, tree expr) {
  switch (TREE_CODE(expr)) {
    ...
    case TRUTH_XOR_EXPR:
      if (TREE_TYPE(expr) == truthvalue_type_node)
        return expr;
      return build2(
          TREE_CODE(expr), truthvalue_type_node,
          c_common_truthvalue_conversion(location, TREE_OPERAND(expr, 0)),
          c_common_truthvalue_conversion(location, TREE_OPERAND(expr, 1)));
    ...
    case ADDR_EXPR: {
      ...
      if (TREE_SIDE_EFFECTS(inner))
        return build2(COMPOUND_EXPR, truthvalue_type_node, inner,
                      truthvalue_true_node);
      else
        return truthvalue_true_node;
    }
    ...
    case COMPLEX_EXPR:
      return build_binary_op(
          EXPR_LOCATION(expr),
          (TREE_SIDE_EFFECTS(TREE_OPERAND(expr, 1)) ? TRUTH_OR_EXPR
                                                    : TRUTH_ORIF_EXPR),
          c_common_truthvalue_conversion(location, TREE_OPERAND(expr, 0)),
          c_common_truthvalue_conversion(location, TREE_OPERAND(expr, 1)), 0);
    ...
    case RROTATE_EXPR:
      ...
      if (TREE_SIDE_EFFECTS(TREE_OPERAND(expr, 1)))
        return build2(
            COMPOUND_EXPR, truthvalue_type_node, TREE_OPERAND(expr, 1),
            c_common_truthvalue_conversion(location, TREE_OPERAND(expr, 0)));
      else
        return c_common_truthvalue_conversion(location, TREE_OPERAND(expr, 0));
    ...
    case COND_EXPR:
      ...
      return fold_build3(
          ...
          c_common_truthvalue_conversion(location, TREE_OPERAND(expr, 1)),
          c_common_truthvalue_conversion(location, TREE_OPERAND(expr, 2)));
    ...
    case MODIFY_EXPR:
      if (!TREE_NO_WARNING(expr) && warn_parentheses) {
  ...
  if (TREE_CODE(TREE_TYPE(expr)) == COMPLEX_TYPE) {
    ...
    return (build_binary_op(
        EXPR_LOCATION(expr),
        (TREE_SIDE_EFFECTS(expr) ? TRUTH_OR_EXPR : TRUTH_ORIF_EXPR),
        c_common_truthvalue_conversion(
            location, build_unary_op(location, REALPART_EXPR, t, 0)),
        c_common_truthvalue_conversion(
            location, build_unary_op(location, IMAGPART_EXPR, t, 0)),
        0));
  }

  if (TREE_CODE(TREE_TYPE(expr)) == FIXED_POINT_TYPE) {
    tree fixed_zero_node =
        build_fixed(TREE_TYPE(expr), FCONST0(TYPE_MODE(TREE_TYPE(expr))));
    return build_binary_op(EXPR_LOCATION(expr), NE_EXPR, expr, fixed_zero_node,
                           1);
  }

  return build_binary_op(EXPR_LOCATION(expr), NE_EXPR, expr, integer_zero_node,
                         1);
}

static void def_builtin_1(enum built_in_function fncode, const char *name,
                          enum built_in_class fnclass, tree fntype,
                          tree libtype, bool both_p, bool fallback_p,
                          bool nonansi_p, tree fnattrs, bool implicit_p);
...
alias_set_type c_common_get_alias_set(tree t) {
  tree u;
  PTR *slot;
  ...
  for (u = t; TREE_CODE(u) == COMPONENT_REF || TREE_CODE(u) == ARRAY_REF;
       u = TREE_OPERAND(u, 0))
  ...
  if (t == char_type_node || t == signed_char_type_node ||
  ...
  if (TREE_CODE(t) == POINTER_TYPE) {
    ...
    for (t2 = TREE_TYPE(t); TREE_CODE(t2) == POINTER_TYPE; t2 = TREE_TYPE(t2))
      ;
    if (TREE_CODE(t2) != RECORD_TYPE && TREE_CODE(t2) != ENUMERAL_TYPE &&
        TREE_CODE(t2) != QUAL_UNION_TYPE && TREE_CODE(t2) != UNION_TYPE)
      return -1;
    if (TYPE_SIZE(t2) == 0)
      return -1;
  }
  /* These are the only cases that need special handling.  */
  if (TREE_CODE(t) != RECORD_TYPE && TREE_CODE(t) != ENUMERAL_TYPE &&
      TREE_CODE(t) != QUAL_UNION_TYPE && TREE_CODE(t) != UNION_TYPE &&
  ...
  if (!type_hash_table)
    type_hash_table = htab_create_ggc(
        1021, c_type_hash, (htab_eq)lang_hooks.types_compatible_p, NULL);
...
tree c_sizeof_or_alignof_type(tree type, bool is_sizeof, int complain) {
  ...
  if (type_code == FUNCTION_TYPE) {
    ...
    if (type_code == VOID_TYPE && complain && (pedantic || warn_pointer_arith))
      pedwarn(input_location, pedantic ? OPT_pedantic : OPT_Wpointer_arith,
    ...
    if (complain)
      error("invalid application of %qs to incomplete type %qT ", op_name,
            type);
    ...
    if (is_sizeof)
      ...
      value =
          size_binop(CEIL_DIV_EXPR, TYPE_SIZE_UNIT(type),
...
static void c_define_builtins(tree va_list_ref_type_node,
...
#define DEF_FUNCTION_TYPE_7(ENUM, RETURN, ARG1, ARG2, ARG3, ARG4, ARG5, ARG6, \
                            ARG7)                                             \
...
#define DEF_BUILTIN(ENUM, NAME, CLASS, TYPE, LIBTYPE, BOTH_P, FALLBACK_P,     \
  ...
  if (NAME && COND)                                                           \
    def_builtin_1(ENUM, NAME, CLASS, builtin_types[(int)TYPE],                \
                  builtin_types[(int)LIBTYPE], BOTH_P, FALLBACK_P, NONANSI_P, \
...
void c_common_nodes_and_builtins(void) {
  ...
  lang_hooks.decls.pushdecl(build_decl(TYPE_DECL, NULL_TREE, intQI_type_node));
  lang_hooks.decls.pushdecl(build_decl(TYPE_DECL, NULL_TREE, intHI_type_node));
  lang_hooks.decls.pushdecl(build_decl(TYPE_DECL, NULL_TREE, intSI_type_node));
  lang_hooks.decls.pushdecl(build_decl(TYPE_DECL, NULL_TREE, intDI_type_node));
#if HOST_BITS_PER_WIDE_INT >= 64
  if (targetm.scalar_mode_supported_p(TImode))
    lang_hooks.decls.pushdecl(
        build_decl(TYPE_DECL, get_identifier("__int128_t"), intTI_type_node));
#endif
  lang_hooks.decls.pushdecl(
      build_decl(TYPE_DECL, NULL_TREE, unsigned_intQI_type_node));
  lang_hooks.decls.pushdecl(
      build_decl(TYPE_DECL, NULL_TREE, unsigned_intHI_type_node));
  lang_hooks.decls.pushdecl(
      build_decl(TYPE_DECL, NULL_TREE, unsigned_intSI_type_node));
  lang_hooks.decls.pushdecl(
      build_decl(TYPE_DECL, NULL_TREE, unsigned_intDI_type_node));
#if HOST_BITS_PER_WIDE_INT >= 64
  if (targetm.scalar_mode_supported_p(TImode))
    lang_hooks.decls.pushdecl(build_decl(
        TYPE_DECL, get_identifier("__uint128_t"), unsigned_intTI_type_node));
#endif
  ...
  lang_hooks.decls.pushdecl(
      build_decl(TYPE_DECL, NULL_TREE, widest_integer_literal_type_node));

  widest_unsigned_literal_type_node =
      make_unsigned_type(HOST_BITS_PER_WIDE_INT * 2);
  lang_hooks.decls.pushdecl(
      build_decl(TYPE_DECL, NULL_TREE, widest_unsigned_literal_type_node));
  ...
  pid_type_node = TREE_TYPE(identifier_global_value(get_identifier(PID_TYPE)));
  ...
  if (targetm.fixed_point_supported_p()) {
    ...
    record_builtin_type(RID_MAX, "long long _Fract", long_long_fract_type_node);
    record_builtin_type(RID_MAX, "unsigned short _Fract",
                        unsigned_short_fract_type_node);
    record_builtin_type(RID_MAX, "unsigned _Fract", unsigned_fract_type_node);
    ...
    record_builtin_type(RID_MAX, "_Sat long _Fract", sat_long_fract_type_node);
    ...
    record_builtin_type(RID_MAX, "long long _Accum", long_long_accum_type_node);
    record_builtin_type(RID_MAX, "unsigned short _Accum",
                        unsigned_short_accum_type_node);
    record_builtin_type(RID_MAX, "unsigned _Accum", unsigned_accum_type_node);
    ...
    record_builtin_type(RID_MAX, "_Sat long _Accum", sat_long_accum_type_node);
  ...
  lang_hooks.decls.pushdecl(build_decl(TYPE_DECL, get_identifier("complex int"),
                                       complex_integer_type_node));
  lang_hooks.decls.pushdecl(build_decl(
      TYPE_DECL, get_identifier("complex float"), complex_float_type_node));
  lang_hooks.decls.pushdecl(build_decl(
      TYPE_DECL, get_identifier("complex double"), complex_double_type_node));
...
#ifdef TARGET_ENUM_VA_LIST
  {
    ...
    for (l = 0; TARGET_ENUM_VA_LIST(l, &pname, &ptype); ++l) {
      lang_hooks.decls.pushdecl(
          build_decl(TYPE_DECL, get_identifier(pname), ptype));
    }
  }
#endif
...
void set_builtin_user_assembler_name(tree decl, const char *asmspec) {
  ...
  gcc_assert(TREE_CODE(decl) == FUNCTION_DECL &&
             DECL_BUILT_IN_CLASS(decl) == BUILT_IN_NORMAL && asmspec != 0);
...
void set_compound_literal_name(tree decl) {
  char *name;
  ASM_FORMAT_PRIVATE_NAME(name, "__compound_literal", compound_literal_number);
...
static bool builtin_function_disabled_p(const char *name) {
  disabled_builtin *p;
...
static void def_builtin_1(enum built_in_function fncode, const char *name,
  ...
  gcc_assert((!both_p && !fallback_p) ||
             !strncmp(name, "__builtin_", strlen("__builtin_")));

  libname = name + strlen("__builtin_");
  decl = add_builtin_function(name, fntype, fncode, fnclass,
                              (fallback_p ? libname : NULL), fnattrs);
  if (both_p && !flag_no_builtin && !builtin_function_disabled_p(libname) &&
...
int self_promoting_args_p(const_tree parms) {
  const_tree t;
...
tree c_add_case_label(splay_tree cases, tree cond, tree orig_type,
  ...
  if (low_value && !check_case_bounds(type, orig_type, &low_value,
                                      high_value ? &high_value : NULL))
    return NULL_TREE;
  ...
  if (!node && (low_value || high_value)) {
    ...
    if (low_bound && CASE_HIGH((tree)low_bound->value) &&
        tree_int_cst_compare(CASE_HIGH((tree)low_bound->value), low_value) >= 0)
      node = low_bound;
    /* Check to see if the HIGH_BOUND overlaps.  The low end of that
       range is bigger than the low end of the current range, so we
       are only interested if the current range is a real range, and
       not an ordinary case label.  */
    else if (high_bound && high_value &&
             (tree_int_cst_compare((tree)high_bound->key, high_value) <= 0))
      node = high_bound;
  }
  ...
  splay_tree_insert(cases, (splay_tree_key)low_value,
...
static void match_case_to_enum_1(tree key, tree type, tree label) {
  ...
  else if (!TYPE_UNSIGNED(type) && TREE_INT_CST_HIGH(key) == -1 &&
  ...
  if (TYPE_NAME(type) == 0)
    warning(warn_switch ? OPT_Wswitch : OPT_Wswitch_enum,
            "%Jcase value %qs not in enumerated type", CASE_LABEL(label), buf);
  else
    warning(warn_switch ? OPT_Wswitch : OPT_Wswitch_enum,
            "%Jcase value %qs not in enumerated type %qT", CASE_LABEL(label),
            buf, type);
}
...
void c_do_switch_warnings(splay_tree cases, location_t switch_location,
                          tree type, tree cond) {
  ...
  if (!warn_switch_enum && !(warn_switch && !default_node))
    return;
  ...
  for (chain = TYPE_VALUES(type); chain; chain = TREE_CHAIN(chain)) {
    ...
    warning((default_node || !warn_switch) ? OPT_Wswitch_enum : OPT_Wswitch,
            "%Henumeration value %qE not handled in switch", &switch_location,
            TREE_PURPOSE(chain));
  }
...
rtx c_expand_expr(tree exp, rtx target, enum machine_mode tmode,
...
tree boolean_increment(enum tree_code code, tree arg) {
  ...
  switch (code) {
    ...
    case POSTINCREMENT_EXPR:
      ...
      break;
    case PREDECREMENT_EXPR:
      val = build2(MODIFY_EXPR, TREE_TYPE(arg), arg, invert_truthvalue(arg));
      break;
    case POSTDECREMENT_EXPR:
      val = build2(MODIFY_EXPR, TREE_TYPE(arg), arg, invert_truthvalue(arg));
      arg = save_expr(arg);
...
#define DEF_ATTR_TREE_LIST(ENUM, PURPOSE, VALUE, CHAIN)                   \
  built_in_attributes[(int)ENUM] = tree_cons(                             \
      built_in_attributes[(int)PURPOSE], built_in_attributes[(int)VALUE], \
...
static tree handle_packed_attribute(tree *node, tree name,
  ...
  if (TYPE_P(*node)) {
    ...
    if (TYPE_ALIGN(TREE_TYPE(*node)) <= BITS_PER_UNIT
        ...
        && !DECL_INITIAL(*node))
      warning(OPT_Wattributes, "%qE attribute ignored for field of type %qT",
              name, TREE_TYPE(*node));
    else
      DECL_PACKED(*node) = 1;
  }
...
static tree handle_nocommon_attribute(tree *node, tree name,
...
static tree handle_common_attribute(tree *node, tree name,
                                    tree ARG_UNUSED(args),
...
static tree handle_noreturn_attribute(tree *node, tree name,
                                      tree ARG_UNUSED(args),
                                      int ARG_UNUSED(flags),
                                      bool *no_add_attrs) {
  ...
  if (TREE_CODE(*node) == FUNCTION_DECL)
    ...
    TREE_TYPE(*node) = build_pointer_type(
        build_type_variant(TREE_TYPE(type), TYPE_READONLY(TREE_TYPE(type)), 1));
  else {
    warning(OPT_Wattributes, "%qE attribute ignored", name);
    *no_add_attrs = true;
  }
...
static tree handle_hot_attribute(tree *node, tree name, tree ARG_UNUSED(args),
                                 int ARG_UNUSED(flags), bool *no_add_attrs) {
  if (TREE_CODE(*node) == FUNCTION_DECL) {
...
static tree handle_cold_attribute(tree *node, tree name, tree ARG_UNUSED(args),
                                  int ARG_UNUSED(flags), bool *no_add_attrs) {
  if (TREE_CODE(*node) == FUNCTION_DECL) {
...
static tree handle_noinline_attribute(tree *node, tree name,
                                      tree ARG_UNUSED(args),
                                      int ARG_UNUSED(flags),
                                      bool *no_add_attrs) {
  if (TREE_CODE(*node) == FUNCTION_DECL)
...
static tree handle_always_inline_attribute(tree *node, tree name,
                                           tree ARG_UNUSED(args),
                                           int ARG_UNUSED(flags),
                                           bool *no_add_attrs) {
  if (TREE_CODE(*node) == FUNCTION_DECL) {
...
static tree handle_gnu_inline_attribute(tree *node, tree name,
                                        tree ARG_UNUSED(args),
                                        int ARG_UNUSED(flags),
                                        bool *no_add_attrs) {
...
static tree handle_artificial_attribute(tree *node, tree name,
                                        tree ARG_UNUSED(args),
                                        int ARG_UNUSED(flags),
                                        bool *no_add_attrs) {
  if (TREE_CODE(*node) == FUNCTION_DECL && DECL_DECLARED_INLINE_P(*node)) {
...
static tree handle_flatten_attribute(tree *node, tree name,
...
static tree handle_error_attribute(tree *node, tree name, tree args,
                                   int ARG_UNUSED(flags), bool *no_add_attrs) {
  if (TREE_CODE(*node) == FUNCTION_DECL ||
...
static tree handle_used_attribute(tree *pnode, tree name, tree ARG_UNUSED(args),
                                  int ARG_UNUSED(flags), bool *no_add_attrs) {
  tree node = *pnode;
...
static tree handle_unused_attribute(tree *node, tree name,
                                    tree ARG_UNUSED(args), int flags,
                                    bool *no_add_attrs) {
  if (DECL_P(*node)) {
    tree decl = *node;
...
static tree handle_externally_visible_attribute(tree *pnode, tree name,
                                                tree ARG_UNUSED(args),
                                                int ARG_UNUSED(flags),
                                                bool *no_add_attrs) {
  tree node = *pnode;
...
static tree handle_const_attribute(tree *node, tree name, tree ARG_UNUSED(args),
                                   int ARG_UNUSED(flags), bool *no_add_attrs) {
  tree type = TREE_TYPE(*node);
...
static tree handle_transparent_union_attribute(tree *node, tree name,
                                               tree ARG_UNUSED(args), int flags,
                                               bool *no_add_attrs) {
  tree type;
...
static priority_type get_priority(tree args, bool is_destructor) {
  HOST_WIDE_INT pri;
  tree arg;
  ...
  arg = TREE_VALUE(args);
  if (!host_integerp(arg, /*pos=*/0) || !INTEGRAL_TYPE_P(TREE_TYPE(arg)))
    goto invalid;
...
static tree handle_constructor_attribute(tree *node, tree name, tree args,
                                         int ARG_UNUSED(flags),
                                         bool *no_add_attrs) {
  tree decl = *node;
  tree type = TREE_TYPE(decl);
...
static tree handle_destructor_attribute(tree *node, tree name, tree args,
                                        int ARG_UNUSED(flags),
                                        bool *no_add_attrs) {
  tree decl = *node;
  tree type = TREE_TYPE(decl);
...
static tree handle_mode_attribute(tree *node, tree name, tree args,
                                  int ARG_UNUSED(flags), bool *no_add_attrs) {
  tree type = *node;
  ...
  *no_add_attrs = true;
  ...
  if (TREE_CODE(TREE_VALUE(args)) != IDENTIFIER_NODE)
    ...
    if (len > 4 && p[0] == '_' && p[1] == '_' && p[len - 1] == '_' &&
        p[len - 2] == '_') {
    ...
    if (POINTER_TYPE_P(type)) {
      ...
      typefm = lang_hooks.types.type_for_mode(mode, ALL_FIXED_POINT_MODE_P(mode)
...
static tree handle_section_attribute(tree *node, tree ARG_UNUSED(name),
  ...
  if (targetm.have_named_sections) {
    ...
    if ((TREE_CODE(decl) == FUNCTION_DECL || TREE_CODE(decl) == VAR_DECL) &&
      ...
      if (TREE_CODE(decl) == VAR_DECL && current_function_decl != NULL_TREE &&
          !TREE_STATIC(decl)) {
        error(
            "%Jsection attribute cannot be specified for "
            "local variables",
            decl);
        *no_add_attrs = true;
      }
      ...
      /* The decl may have already been given a section attribute
        ...
        *no_add_attrs = true;
      } else if (TREE_CODE(decl) == VAR_DECL && !targetm.have_tls &&
                 targetm.emutls.tmpl_section && DECL_THREAD_LOCAL_P(decl)) {
...
static tree handle_aligned_attribute(tree *node, tree ARG_UNUSED(name),
  ...
  if (TREE_CODE(align_expr) != INTEGER_CST) {
  ...
  } else if (!VAR_OR_FUNCTION_DECL_P(decl) && TREE_CODE(decl) != FIELD_DECL) {
    ...
    *no_add_attrs = true;
    ...
    if (DECL_USER_ALIGN(decl))
      error(
          "alignment for %q+D was previously specified as %d "
...
static tree handle_weak_attribute(tree *node, tree name, tree ARG_UNUSED(args),
                                  int ARG_UNUSED(flags),
                                  bool *ARG_UNUSED(no_add_attrs)) {
  if (TREE_CODE(*node) == FUNCTION_DECL && DECL_DECLARED_INLINE_P(*node)) {
    error("inline function %q+D cannot be declared weak", *node);
    *no_add_attrs = true;
  } else if (TREE_CODE(*node) == FUNCTION_DECL || TREE_CODE(*node) == VAR_DECL)
    declare_weak(*node);
  else
    warning(OPT_Wattributes, "%qE attribute ignored", name);
...
static tree handle_alias_attribute(tree *node, tree name, tree args,
                                   int ARG_UNUSED(flags), bool *no_add_attrs) {
  tree decl = *node;
...
static tree handle_weakref_attribute(tree *node, tree ARG_UNUSED(name),
                                     tree args, int flags, bool *no_add_attrs) {
  tree attr = NULL_TREE;
  ...
  if (args) {
    ...
    *no_add_attrs = true;
    ...
    if (lookup_attribute("alias", DECL_ATTRIBUTES(*node)))
      error("%Jweakref attribute must appear before alias attribute", *node);
...
static tree handle_visibility_attribute(tree *node, tree name, tree args,
                                        int ARG_UNUSED(flags),
                                        bool *ARG_UNUSED(no_add_attrs)) {
  tree decl = *node;
  ...
  if (TYPE_P(*node)) {
    if (TREE_CODE(*node) == ENUMERAL_TYPE)
      ...
      error("%qE attribute ignored because %qT is already defined", name,
            *node);
      return NULL_TREE;
    }
  ...
  if (strcmp(TREE_STRING_POINTER(id), "default") == 0)
    ...
    error(
        "visibility argument must be one of \"default\", \"hidden\", "
  ...
  if (DECL_VISIBILITY_SPECIFIED(decl) && vis != DECL_VISIBILITY(decl)) {
    tree attributes =
        (TYPE_P(*node) ? TYPE_ATTRIBUTES(*node) : DECL_ATTRIBUTES(decl));
    if (lookup_attribute("visibility", attributes))
      error("%qD redeclared with different visibility", decl);
    else if (TARGET_DLLIMPORT_DECL_ATTRIBUTES &&
             lookup_attribute("dllimport", attributes))
      error("%qD was declared %qs which implies default visibility", decl,
            "dllimport");
    else if (TARGET_DLLIMPORT_DECL_ATTRIBUTES &&
...
static tree handle_tls_model_attribute(tree *node, tree name, tree args,
                                       int ARG_UNUSED(flags),
                                       bool *no_add_attrs) {
  tree id;
  tree decl = *node;
  ...
  *no_add_attrs = true;
  ...
  if (!strcmp(TREE_STRING_POINTER(id), "local-exec"))
    ...
    error(
        "tls_model argument must be one of \"local-exec\", \"initial-exec\", "
...
static tree handle_no_instrument_function_attribute(tree *node, tree name,
                                                    tree ARG_UNUSED(args),
                                                    int ARG_UNUSED(flags),
                                                    bool *no_add_attrs) {
  tree decl = *node;
...
static tree handle_malloc_attribute(tree *node, tree name,
                                    tree ARG_UNUSED(args),
                                    int ARG_UNUSED(flags), bool *no_add_attrs) {
  if (TREE_CODE(*node) == FUNCTION_DECL &&
...
static tree handle_alloc_size_attribute(tree *node, tree ARG_UNUSED(name),
                                        tree args, int ARG_UNUSED(flags),
                                        bool *no_add_attrs) {
  ...
  for (; args; args = TREE_CHAIN(args)) {
    ...
    if (TREE_CODE(position) != INTEGER_CST || TREE_INT_CST_HIGH(position) ||
        TREE_INT_CST_LOW(position) < 1 ||
        TREE_INT_CST_LOW(position) > arg_count) {
      warning(OPT_Wattributes, "alloc_size parameter outside range");
      *no_add_attrs = true;
      return NULL_TREE;
    }
  }
...
static tree handle_returns_twice_attribute(tree *node, tree name,
                                           tree ARG_UNUSED(args),
                                           int ARG_UNUSED(flags),
                                           bool *no_add_attrs) {
  if (TREE_CODE(*node) == FUNCTION_DECL)
...
static tree handle_no_limit_stack_attribute(tree *node, tree name,
                                            tree ARG_UNUSED(args),
                                            int ARG_UNUSED(flags),
                                            bool *no_add_attrs) {
  tree decl = *node;
...
static tree handle_pure_attribute(tree *node, tree name, tree ARG_UNUSED(args),
                                  int ARG_UNUSED(flags), bool *no_add_attrs) {
  if (TREE_CODE(*node) == FUNCTION_DECL)
    DECL_PURE_P(*node) = 1;
  /* ??? TODO: Support types.  */
...
static tree handle_deprecated_attribute(tree *node, tree name,
                                        tree ARG_UNUSED(args), int flags,
                                        bool *no_add_attrs) {
  tree type = NULL_TREE;
  int warn = 0;
...
static tree handle_vector_size_attribute(tree *node, tree name, tree args,
                                         int ARG_UNUSED(flags),
                                         bool *no_add_attrs) {
  ...
  *no_add_attrs = true;
  ...
  while (POINTER_TYPE_P(type) || TREE_CODE(type) == FUNCTION_TYPE ||
  ...
  for (attr_arg_num = 1; args; args = TREE_CHAIN(args)) {
    ...
    if (argument) {
      ...
      if (!argument || TREE_CODE(TREE_VALUE(argument)) == VOID_TYPE) {
        error(
            "nonnull argument with out-of-range operand number (argument %lu, "
            "operand %lu)",
      ...
      if (TREE_CODE(TREE_VALUE(argument)) != POINTER_TYPE) {
        error(
            "nonnull argument references non-pointer operand (argument %lu, "
            "operand %lu)",
            (unsigned long)attr_arg_num, (unsigned long)arg_num);
        *no_add_attrs = true;
        return NULL_TREE;
...
static void check_function_nonnull(tree attrs, int nargs, tree *argarray) {
  tree a, args;
  int i;
...
static void check_function_sentinel(tree attrs, int nargs, tree *argarray,
                                    tree typelist) {
  ...
  if (attr) {
    ...
    if ((nargs - 1 - pos) < len) {
      warning(OPT_Wformat, "not enough variable arguments to fit a sentinel");
      return;
    }
...
static void check_nonnull_arg(void *ARG_UNUSED(ctx), tree param,
...
static tree handle_nothrow_attribute(tree *node, tree name,
                                     tree ARG_UNUSED(args),
                                     int ARG_UNUSED(flags),
                                     bool *no_add_attrs) {
  if (TREE_CODE(*node) == FUNCTION_DECL)
...
static tree handle_cleanup_attribute(tree *node, tree name, tree args,
                                     int ARG_UNUSED(flags),
                                     bool *no_add_attrs) {
  tree decl = *node;
...
static tree handle_warn_unused_result_attribute(tree *node, tree name,
                                                tree ARG_UNUSED(args),
                                                int ARG_UNUSED(flags),
                                                bool *no_add_attrs) {
...
static tree handle_sentinel_attribute(tree *node, tree name, tree args,
                                      int ARG_UNUSED(flags),
                                      bool *no_add_attrs) {
  ...
  if (args) {
    ...
    if (TREE_CODE(position) != INTEGER_CST) {
      warning(OPT_Wattributes, "requested position is not an integer constant");
      *no_add_attrs = true;
    } else {
...
static tree handle_target_attribute(tree *node, tree name, tree args, int flags,
                                    bool *no_add_attrs) {
  /* Ensure we have a function type.  */
  if (TREE_CODE(*node) != FUNCTION_DECL) {
    warning(OPT_Wattributes, "%qE attribute ignored", name);
    ...
    if (TREE_CODE(value) == INTEGER_CST) {
      ...
      while (next_p != NULL) {
        ...
        if (*p == '-' && p[1] != 'O' && p[1] != 'f') {
          ret = false;
          if (attr_p)
            warning(OPT_Wattributes, "Bad option %s to optimize attribute.", p);
          else
            warning(OPT_Wpragmas, "Bad option %s to pragma attribute", p);
          continue;
        }
...
static tree handle_optimize_attribute(tree *node, tree name, tree args,
                                      int ARG_UNUSED(flags),
                                      bool *no_add_attrs) {
  /* Ensure we have a function type.  */
  if (TREE_CODE(*node) != FUNCTION_DECL) {
    ...
    *no_add_attrs = true;
    ...
    DECL_FUNCTION_SPECIFIC_OPTIMIZATION(*node) = build_optimization_node();
 ...
 */
void check_function_arguments(tree attrs, int nargs, tree *argarray,
                              tree typelist) {
...
void check_function_arguments_recurse(void (*callback)(void *, tree,
  ...
  if (CONVERT_EXPR_P(param) &&
    ...
    /* Strip coercion.  */
    check_function_arguments_recurse(callback, ctx, TREE_OPERAND(param, 0),
                                     param_num);
    return;
  }
    ...
    for (attrs = TYPE_ATTRIBUTES(type); attrs; attrs = TREE_CHAIN(attrs))
      if (is_attribute_p("format_arg", TREE_PURPOSE(attrs))) {
        ...
        for (inner_arg = first_call_expr_arg(param, &iter), i = 1;
             inner_arg != 0; inner_arg = next_call_expr_arg(&iter), i++)
          if (i == format_num) {
            check_function_arguments_recurse(callback, ctx, inner_arg,
                                             param_num);
  ...
  if (TREE_CODE(param) == COND_EXPR) {
    ...
    check_function_arguments_recurse(callback, ctx, TREE_OPERAND(param, 1),
                                     param_num);
    check_function_arguments_recurse(callback, ctx, TREE_OPERAND(param, 2),
                                     param_num);
    return;
  }
...
static bool validate_nargs(tree fndecl, int nargs, int required) {
  if (nargs < required) {
...
bool check_builtin_function_arguments(tree fndecl, int nargs, tree *args) {
    ...
    case BUILT_IN_ISNORMAL:
      if (validate_nargs(fndecl, nargs, 1)) {
        if (TREE_CODE(TREE_TYPE(args[0])) != REAL_TYPE) {
          error(
              "non-floating-point argument in call to "
              "function %qE",
              fndecl);
          return false;
        }
    ...
    case BUILT_IN_FPCLASSIFY:
      if (validate_nargs(fndecl, nargs, 6)) {
        ...
        if (TREE_CODE(TREE_TYPE(args[5])) != REAL_TYPE) {
          error("non-floating-point argument in call to function %qE", fndecl);
          return false;
        }
        return true;
      }
...
static int resort_field_decl_cmp(const void *x_p, const void *y_p) {
  const tree *const x = (const tree *const)x_p;
  const tree *const y = (const tree *const)y_p;
...
void resort_sorted_fields(void *obj, void *ARG_UNUSED(orig_obj),
...
static char *catenate_strings(const char *lhs, const char *rhs_start,
                              int rhs_size) {
...
#define catenate_messages(M1, M2) catenate_strings((M1), (M2), sizeof(M2))
  ...
  if (token == CPP_EOF)
    ...
    message = NULL;
             ...
             token == CPP_STRING16 || token == CPP_STRING32)
  ...
  switch (TREE_CODE(expr)) {
    ...
    case COMPONENT_REF:
      ...
      if (DECL_C_BIT_FIELD(t)) {
        error(
            "attempt to take address of bit-field structure "
            "member %qD",
            t);
        return error_mark_node;
      }
      off = size_binop(
          PLUS_EXPR, DECL_FIELD_OFFSET(t),
          size_int(tree_low_cst(DECL_FIELD_BIT_OFFSET(t), 1) / BITS_PER_UNIT));
      break;
...
int complete_array_type(tree *ptype, tree initial_value, bool do_default) {
    ...
    if (TREE_CODE(initial_value) == STRING_CST) {
      ...
      if (VEC_empty(constructor_elt, v)) {
        ...
        if (VEC_index(constructor_elt, v, 0)->index)
          maxindex =
              fold_convert(sizetype, VEC_index(constructor_elt, v, 0)->index);
        curindex = maxindex;

        for (cnt = 1; VEC_iterate(constructor_elt, v, cnt, ce); cnt++) {
  ...
  hashcode = iterative_hash_object(TYPE_HASH(TYPE_DOMAIN(main_type)), hashcode);
  ...
  if (TYPE_STRUCTURAL_EQUALITY_P(TREE_TYPE(main_type)) ||
    ...
    SET_TYPE_STRUCTURAL_EQUALITY(main_type);
           ...
           (TYPE_CANONICAL(TYPE_DOMAIN(main_type)) != TYPE_DOMAIN(main_type)))
  ...
  if (COMPLETE_TYPE_P(type) && TREE_CODE(TYPE_SIZE_UNIT(type)) == INTEGER_CST &&
...
static bool sync_resolve_params(tree orig_function, tree function,
                                tree params) {
...
bool same_scalar_type_ignoring_signedness(tree t1, tree t2) {
  ...
  gcc_assert(
      (c1 == INTEGER_TYPE || c1 == REAL_TYPE || c1 == FIXED_POINT_TYPE) &&
      (c2 == INTEGER_TYPE || c2 == REAL_TYPE || c2 == FIXED_POINT_TYPE));

  /* Equality works here because c_common_signed_type uses
     TYPE_MAIN_VARIANT.  */
  return c_common_signed_type(t1) == c_common_signed_type(t2);
}
...
void warn_about_parentheses(enum tree_code code, enum tree_code code_left,
                            tree arg_left, enum tree_code code_right,
                            tree arg_right) {
  if (!warn_parentheses)
    return;
...
#define APPEARS_TO_BE_BOOLEAN_EXPR_P(CODE, ARG)                      \
  (truth_value_p(TREE_CODE(ARG)) ||                                  \
   TREE_CODE(TREE_TYPE(ARG)) ==                                      \
       BOOLEAN_TYPE /* Folding may create 0 or 1 integers from other \
                       expressions.  */                              \
   || ((CODE) != INTEGER_CST && (integer_onep(ARG) || integer_zerop(ARG))))
  ...
  switch (code) {
    ...
    case BIT_XOR_EXPR:
      if (code_left == BIT_AND_EXPR || code_left == PLUS_EXPR ||
          code_left == MINUS_EXPR || code_right == BIT_AND_EXPR ||
          code_right == PLUS_EXPR || code_right == MINUS_EXPR)
        warning(OPT_Wparentheses,
                "suggest parentheses around arithmetic in operand of %<^%>");
      /* Check cases like x^y==z */
...
void warn_for_unused_label(tree label) {
  if (!TREE_USED(label)) {
...
void warn_for_div_by_zero(location_t loc, tree divisor) {
  /* If DIVISOR is zero, and has integral or fixed-point type, issue a warning
     about division by zero.  Do not issue a warning if DIVISOR has a
     floating-point type, since we consider 0.0/0.0 a valid way of
     generating a NaN.  */
  if (skip_evaluation == 0 && (integer_zerop(divisor) || fixed_zerop(divisor)))
...
void warn_for_sign_compare(location_t location, tree orig_op0, tree orig_op1,
  ...
  if (c_dialect_cxx() && TREE_CODE(TREE_TYPE(orig_op0)) == ENUMERAL_TYPE &&
      TREE_CODE(TREE_TYPE(orig_op1)) == ENUMERAL_TYPE &&
      TYPE_MAIN_VARIANT(TREE_TYPE(orig_op0)) !=
          TYPE_MAIN_VARIANT(TREE_TYPE(orig_op1))) {
    warning_at(location, OPT_Wsign_compare,
               "comparison between types %qT and %qT", TREE_TYPE(orig_op0),
               TREE_TYPE(orig_op1));
  }
  ...
  /* Do not warn if both operands are unsigned.  */
    ...
    STRIP_TYPE_NOPS(sop);
    STRIP_TYPE_NOPS(uop);
    base_type = (TREE_CODE(result_type) == COMPLEX_TYPE ? TREE_TYPE(result_type)
                                                        : result_type);
  ...
  if ((TREE_CODE(op0) == BIT_NOT_EXPR) ^ (TREE_CODE(op1) == BIT_NOT_EXPR)) {
    ...
    if (host_integerp(op0, 0) || host_integerp(op1, 0)) {
      ...
      if (bits < TYPE_PRECISION(result_type) && bits < HOST_BITS_PER_LONG &&
          unsignedp) {
        mask = (~(HOST_WIDE_INT)0) << bits;
        if ((mask & constant) != mask) {
          if (constant == 0)
            warning(OPT_Wsign_compare, "promoted ~unsigned is always non-zero");
          else
            warning_at(location, OPT_Wsign_compare,
               ...
               (TYPE_PRECISION(TREE_TYPE(op0)) < TYPE_PRECISION(result_type)) &&
               (TYPE_PRECISION(TREE_TYPE(op1)) < TYPE_PRECISION(result_type)))
      warning_at(location, OPT_Wsign_compare,
