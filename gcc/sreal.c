...
void dump_sreal(FILE *file, sreal *x) {
#if SREAL_PART_BITS < 32
  fprintf(file,
          "((" HOST_WIDE_INT_PRINT_UNSIGNED
          " * 2^16 + " HOST_WIDE_INT_PRINT_UNSIGNED ") * 2^%d)",
...
static inline void copy(sreal *r, sreal *a) {
#if SREAL_PART_BITS < 32
  r->sig_lo = a->sig_lo;
  r->sig_hi = a->sig_hi;
#else
...
static inline void shift_right(sreal *x, int s) {
  gcc_assert(s > 0);
...
static void normalize(sreal *x) {
  ...
  if (x->sig_lo == 0 && x->sig_hi == 0) {
    ...
    x->sig_lo >>= shift;
    x->sig_lo +=
        ((x->sig_hi & (((uhwi)1 << shift) - 1)) << (SREAL_PART_BITS - shift));
    x->sig_hi >>= shift;
    x->exp += shift;
    if (x->sig_lo & ((uhwi)1 << SREAL_PART_BITS)) {
      x->sig_lo -= (uhwi)1 << SREAL_PART_BITS;
...
sreal *sreal_init(sreal *r, unsigned HOST_WIDE_INT sig, signed int exp) {
#if SREAL_PART_BITS < 32
  r->sig_lo = 0;
  r->sig_hi = sig;
  r->exp = exp - 16;
...
sreal *sreal_add(sreal *r, sreal *a, sreal *b) {
  int dexp;
  sreal tmp;
  sreal *bb;
...
sreal *sreal_sub(sreal *r, sreal *a, sreal *b) {
  int dexp;
  sreal tmp;
  sreal *bb;
...
sreal *sreal_mul(sreal *r, sreal *a, sreal *b) {
#if SREAL_PART_BITS < 32
...
sreal *sreal_div(sreal *r, sreal *a, sreal *b) {
#if SREAL_PART_BITS < 32
