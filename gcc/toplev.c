...
const char *get_random_seed(bool noinit) {
...
const char *set_random_seed(const char *val) {
...
int read_integral_parameter(const char *p, const char *pname,
                            const int defval) {
  const char *endp = p;
...
void check_global_declaration_1(tree decl) {
  ...
  if (TREE_CODE(decl) == FUNCTION_DECL && DECL_INITIAL(decl) == 0 &&
      DECL_EXTERNAL(decl) && !DECL_ARTIFICIAL(decl) && !TREE_NO_WARNING(decl) &&
      !TREE_PUBLIC(decl) &&
  ...
  if (((warn_unused_function && TREE_CODE(decl) == FUNCTION_DECL)
    ...
    warning((TREE_CODE(decl) == FUNCTION_DECL) ? OPT_Wunused_function
...
void warn_deprecated_use(tree node) {
  ...
  if (DECL_P(node)) {
    ...
    warning(OPT_Wdeprecated_declarations,
            "%qD is deprecated (declared at %s:%d)", node, xloc.file,
            xloc.line);
  } else if (TYPE_P(node)) {
    ...
    if (decl) {
      ...
      if (what)
        warning(OPT_Wdeprecated_declarations,
                "%qs is deprecated (declared at %s:%d)", what, xloc.file,
                xloc.line);
      else
        warning(OPT_Wdeprecated_declarations,
                "type is deprecated (declared at %s:%d)", xloc.file, xloc.line);
    } else {
      if (what)
...
const char *const debug_type_names[] = {"none",    "stabs", "coff",
                                        "dwarf-2", "xcoff", "vms"};
...
#endif
      ;
  static const char fmt2[] = N_("GMP version %s, MPFR version %s.\n");
  static const char fmt3[] = N_(
      "%s%swarning: %s header version %s differs from library version %s.\n");
  static const char fmt4[] =
      N_("%s%sGGC heuristics: --param ggc-min-expand=%d --param "
         "ggc-min-heapsize=%d\n");
#ifndef __VERSION__
#define __VERSION__ "[?]"
#endif
...
#define GCC_GMP_STRINGIFY_VERSION              \
  ...
  "." GCC_GMP_STRINGIFY_VERSION2(              \
      __GNU_MP_VERSION_MINOR) "." GCC_GMP_STRINGIFY_VERSION2(__GNU_MP_VERSION_PATCHLEVEL)
#endif
  ...
  fprintf(file, file == stderr ? _(fmt4) : fmt4, indent,
          *indent != 0 ? " " : "", PARAM_VALUE(GGC_MIN_EXPAND),
          PARAM_VALUE(GGC_MIN_HEAPSIZE));
}

#ifdef ASM_COMMENT_START
static int print_to_asm_out_file(print_switch_type type, const char *text) {
...
static int print_to_stderr(print_switch_type type, const char *text) {
  switch (type) {
    case SWITCH_TYPE_LINE_END:
      putc('\n', stderr);
      return 1;
...
static int print_single_switch(print_switch_fn_type print_fn, int pos,
                               print_switch_type type, const char *text) {
...
static void print_switch_values(print_switch_fn_type print_fn) {
  int pos = 0;
  size_t j;
  const char **p;
  ...
  pos = print_single_switch(print_fn, pos, SWITCH_TYPE_DESCRIPTIVE,
                            _("options passed: "));

  for (p = &save_argv[1]; *p != NULL; p++) {
    if (**p == '-') {
      /* Ignore these.  */
      if (strcmp(*p, "-o") == 0 || strcmp(*p, "-dumpbase") == 0 ||
          strcmp(*p, "-auxbase") == 0) {
        if (p[1] != NULL)
          p++;
        continue;
      ...
      if (strcmp(*p, "-quiet") == 0 || strcmp(*p, "-version") == 0)
        continue;
  ...
  /* Print the -f and -m options that have been enabled.
     ...
     should suffice.  */
  pos = print_single_switch(print_fn, 0, SWITCH_TYPE_DESCRIPTIVE,
                            _("options enabled: "));

  for (j = 0; j < cl_options_count; j++)
    if ((cl_options[j].flags & CL_REPORT) && option_enabled(j) > 0)
      pos = print_single_switch(print_fn, pos, SWITCH_TYPE_ENABLED,
...
static inline bool option_affects_pch_p(int option,
...
static const char *pch_option_mismatch(const char *option) {
  char *r;
...
static bool default_tree_printer(pretty_printer *pp, text_info *text,
                                 const char *spec, int precision, bool wide,
                                 bool set_locus, bool hash) {
  tree t;
  ...
  if (DECL_P(t)) {
    const char *n =
        DECL_NAME(t) ? lang_hooks.decl_printable_name(t, 2) : "<anonymous>";
    pp_string(pp, n);
  } else
...
static void init_alignments(void) {
  if (align_loops <= 0)
    align_loops = 1;
...
static void process_options(void) {
  ...
  if (flag_section_anchors && !target_supports_section_anchors_p()) {
    warning(OPT_fsection_anchors, "this target does not support %qs",
            "-fsection-anchors");
...
#ifndef HAVE_cloog
  if (flag_graphite || flag_loop_block || flag_loop_interchange ||
      flag_loop_strip_mine || flag_graphite_identity)
  ...
  if (debug_info_level < DINFO_LEVEL_NORMAL ||
    ...
    if (flag_var_tracking == 1 || flag_var_tracking_uninit == 1) {
      if (debug_info_level < DINFO_LEVEL_NORMAL)
        warning(0,
  ...
  if (flag_unwind_tables && !ACCUMULATE_OUTGOING_ARGS &&
    ...
    warning(0,
            "unwind tables currently require a frame pointer "
            "for correctness");
...
void dump_memory_report(bool final) {
