...
#define gcc_obstack_init(OBSTACK)                                       \
  _obstack_begin((OBSTACK), OBSTACK_CHUNK_SIZE, 0, obstack_chunk_alloc, \
                 obstack_chunk_free)
...
/* If none of these macros are defined, the port must use the new
   ...
   still uses.  */
#if !defined CONSTRAINT_LEN && !defined REG_CLASS_FROM_LETTER &&              \
    !defined REG_CLASS_FROM_CONSTRAINT && !defined CONST_OK_FOR_LETTER_P &&   \
    !defined CONST_OK_FOR_CONSTRAINT_P &&                                     \
    !defined CONST_DOUBLE_OK_FOR_LETTER_P &&                                  \
    !defined CONST_DOUBLE_OK_FOR_CONSTRAINT_P && !defined EXTRA_CONSTRAINT && \
    !defined EXTRA_CONSTRAINT_STR && !defined EXTRA_MEMORY_CONSTRAINT &&      \
...
#if defined(CONST_DOUBLE_OK_FOR_LETTER_P) && \
