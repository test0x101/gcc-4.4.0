...
static void dbg_cnt_set_limit_by_index(enum debug_counter index, int value) {
  limit[index] = value;

  fprintf(stderr, "dbg_cnt '%s' set to %d\n", map[index].name, value);
}

static bool dbg_cnt_set_limit_by_name(const char *name, int len, int value) {
  int i;
...
static const char *dbg_cnt_process_single_pair(const char *arg) {
...
void dbg_cnt_list_all_counters(void) {
  ...
  for (i = 0; i < debug_counter_number_of_counters; i++)
    printf("  %-30s %5d %5u\n", map[i].name, limit[map[i].counter],
           count[map[i].counter]);
  printf("\n");
}
