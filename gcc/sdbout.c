...
#define PUT_SDB_VAL(a)                                                    \
  (fputs("\t.val\t", asm_out_file), output_addr_const(asm_out_file, (a)), \
...
#define PUSH_DERIVED_LEVEL(DT_type, PREV)         \
  ((((PREV) & ~(int)N_BTMASK) << (int)N_TSHIFT) | \
   ((int)DT_type << (int)N_BTSHFT) | ((PREV) & (int)N_BTMASK))
...
static int plain_type(tree type) {
...
static int template_name_p(tree name) {
...
static void sdbout_record_type_name(tree type) {
  const char *name = 0;
  int no_name;
...
static int plain_type_1(tree type, int level) {
  if (type == 0)
    type = void_type_node;
  ...
  switch (TREE_CODE(type)) {
    ...
    case INTEGER_TYPE: {
      ...
      if (TYPE_NAME(type) != 0 && TREE_CODE(TYPE_NAME(type)) == TYPE_DECL &&
          DECL_NAME(TYPE_NAME(type)) != 0 &&
          TREE_CODE(DECL_NAME(TYPE_NAME(type))) == IDENTIFIER_NODE) {
        const char *const name = IDENTIFIER_POINTER(DECL_NAME(TYPE_NAME(type)));
    ...
    case ARRAY_TYPE: {
      ...
      if (sdb_n_dims < SDB_MAX_DIM)
        sdb_dims[sdb_n_dims++] =
            (TYPE_DOMAIN(type) && TYPE_MIN_VALUE(TYPE_DOMAIN(type)) != 0 &&
...
static void sdbout_block(tree block) {
  while (block) {
...
static void sdbout_syms(tree syms) {
  while (syms) {
...
void sdbout_symbol(tree decl, int local) {
  ...
  switch (TREE_CODE(decl)) {
    ...
    case VAR_DECL:
      ...
      else if (MEM_P(value) && CONSTANT_P(XEXP(value, 0)) && !TREE_STATIC(decl))
        return;
      ...
      if (!local && MEM_P(value) && DECL_INITIAL(decl))
        return;
      ...
      if (MEM_P(value) && GET_CODE(XEXP(value, 0)) == SYMBOL_REF) {
        PUT_SDB_DEF(name);
        if (TREE_PUBLIC(decl)) {
         ...
         so all we can do is output the variable as a pointer.  */
      {
        ...
        if (REG_P(XEXP(value, 0))) {
          ...
          PUT_SDB_INT_VAL(DEBUGGER_AUTO_OFFSET(XEXP(XEXP(value, 0), 0)));
          PUT_SDB_SCL(C_AUTO);
        }
...
static void sdbout_toplevel_data(tree decl) {
  tree type = TREE_TYPE(decl);
...
static void sdbout_queue_anonymous_type(tree type) {
...
static void sdbout_type(tree type) {
...
static void sdbout_field_types(tree type) {
  tree tail;

  for (tail = TYPE_FIELDS(type); tail; tail = TREE_CHAIN(tail))
    /* This condition should match the one for emitting the actual
       members below.  */
    if (TREE_CODE(tail) == FIELD_DECL && DECL_NAME(tail) && DECL_SIZE(tail) &&
...
static void sdbout_one_type(tree type) {
      ...
      /* Output a structure type.  */
      {
          ...
          for (binfo = TYPE_BINFO(type), i = 0;
            ...
            if (TREE_CODE(TYPE_NAME(child_type)) == IDENTIFIER_NODE)
              ...
              if (child_type_name && template_name_p(child_type_name))
                child_type_name = DECL_ASSEMBLER_NAME(TYPE_NAME(child_type));
            } else
              continue;
        ...
        if (TREE_CODE(type) == ENUMERAL_TYPE) {
          ...
          for (tem = TYPE_FIELDS(type); tem; tem = TREE_CHAIN(tem))
            ...
            if (TREE_CODE(tem) == FIELD_DECL && DECL_NAME(tem) &&
...
static void sdbout_parms(tree parms) {
    ...
    if (DECL_NAME(parms)) {
      ...
      if (PARM_PASSED_IN_MEMORY(parms)) {
        ...
        if (REG_P(DECL_RTL(parms)) &&
          ...
          if (BYTES_BIG_ENDIAN && TREE_TYPE(parms) != DECL_ARG_TYPE(parms))
            current_sym_value +=
        ...
        if (REG_P(XEXP(DECL_RTL(parms), 0)) || MEM_P(XEXP(DECL_RTL(parms), 0)))
          current_sym_value = 0;
        else
        ...
        PUT_SDB_DEF(name);
        PUT_SDB_INT_VAL(
            DEBUGGER_ARG_OFFSET(current_sym_value, XEXP(DECL_RTL(parms), 0)));
        PUT_SDB_SCL(C_ARG);
        PUT_SDB_TYPE(plain_type(TREE_TYPE(parms)));
        PUT_SDB_ENDEF;
      }
...
static void sdbout_reg_parms(tree parms) {
  for (; parms; parms = TREE_CHAIN(parms))
    if (DECL_NAME(parms)) {
      const char *name = IDENTIFIER_POINTER(DECL_NAME(parms));
      ...
      /* Report parms that live in memory but not where they were passed.  */
...
#endif
        {
          if (name == 0 || *name == 0)
            name = gen_fake_label();
          PUT_SDB_DEF(name);
          PUT_SDB_INT_VAL(DEBUGGER_AUTO_OFFSET(XEXP(DECL_RTL(parms), 0)));
          PUT_SDB_SCL(C_AUTO);
          PUT_SDB_TYPE(plain_type(TREE_TYPE(parms)));
          PUT_SDB_ENDEF;
        }
...
static void sdbout_global_decl(tree decl) {
...
static void sdbout_finish(const char *main_filename ATTRIBUTE_UNUSED) {
  size_t i;
...
static void sdbout_begin_block(unsigned int line, unsigned int n) {
...
static void sdbout_end_block(unsigned int line,
...
static void sdbout_source_line(unsigned int line,
  ...
  if ((int)line > sdb_begin_function_line) {
    ...
    fprintf(
        asm_out_file, "\t.ln\t%d\n",
        ((sdb_begin_function_line > -1) ? line - sdb_begin_function_line : 1));
#endif
  }
}
...
static void sdbout_begin_function(tree decl ATTRIBUTE_UNUSED) {
...
#ifndef MIPS_DEBUGGING_INFO
static void sdbout_begin_prologue(unsigned int line,
                                  const char *file ATTRIBUTE_UNUSED) {
  sdbout_end_prologue(line, file);
}
#endif

static void sdbout_end_prologue(unsigned int line,
                                const char *file ATTRIBUTE_UNUSED) {
...
static void sdbout_end_function(unsigned int line) {
#ifdef SDB_ALLOW_FORWARD_REFERENCES
  sdbout_dequeue_anonymous_types();
#endif
...
static void sdbout_end_epilogue(unsigned int line ATTRIBUTE_UNUSED,
                                const char *file ATTRIBUTE_UNUSED) {
...
static void sdbout_label(rtx insn) {
...
static void sdbout_start_source_file(unsigned int line ATTRIBUTE_UNUSED,
                                     const char *filename ATTRIBUTE_UNUSED) {
#ifdef MIPS_DEBUGGING_INFO
...
static void sdbout_end_source_file(unsigned int line ATTRIBUTE_UNUSED) {
#ifdef MIPS_DEBUGGING_INFO
  struct sdb_file *next;
...
static void sdbout_init(const char *input_file_name ATTRIBUTE_UNUSED) {
  tree t;
