...
static inline tree mf_make_builtin(enum tree_code category, const char *name,
                                   tree type) {
...
static inline tree mf_make_mf_cache_struct_type(tree field_type) {
...
#define build_function_type_3(rtype, arg1, arg2, arg3) \
  build_function_type(                                 \
      rtype,                                           \
      tree_cons(0, arg1,                               \
                tree_cons(0, arg2, tree_cons(0, arg3, void_list_node))))
#define build_function_type_4(rtype, arg1, arg2, arg3, arg4) \
  build_function_type(                                       \
      ...
      tree_cons(                                             \
          0, arg1,                                           \
          tree_cons(0, arg2,                                 \
                    tree_cons(0, arg3, tree_cons(0, arg4, void_list_node)))))
...
void mudflap_init(void) {
  ...
  mf_unregister_fntype = build_function_type_3(
      void_type_node, ptr_type_node, size_type_node, integer_type_node);
  mf_init_fntype = build_function_type_0(void_type_node);
  ...
  mf_cache_shift_decl =
      mf_make_builtin(VAR_DECL, "__mf_lc_shift", unsigned_char_type_node);
  mf_cache_mask_decl =
      mf_make_builtin(VAR_DECL, "__mf_lc_mask", mf_uintptr_type);
  ...
  mf_check_fndecl =
      mf_make_builtin(FUNCTION_DECL, "__mf_check", mf_check_register_fntype);
  mf_register_fndecl =
      mf_make_builtin(FUNCTION_DECL, "__mf_register", mf_check_register_fntype);
  mf_unregister_fndecl =
      mf_make_builtin(FUNCTION_DECL, "__mf_unregister", mf_unregister_fntype);
  mf_init_fndecl = mf_make_builtin(FUNCTION_DECL, "__mf_init", mf_init_fntype);
  mf_set_options_fndecl =
      mf_make_builtin(FUNCTION_DECL, "__mf_set_options", mf_set_options_fntype);
}
...
static void mf_decl_cache_locals(void) {
  ...
  mf_cache_shift_decl_l = mf_mark(
      create_tmp_var(TREE_TYPE(mf_cache_shift_decl), "__mf_lookup_shift_l"));

  mf_cache_mask_decl_l = mf_mark(
      create_tmp_var(TREE_TYPE(mf_cache_mask_decl), "__mf_lookup_mask_l"));
...
static void mf_build_check_statement_for(tree base, tree limit,
  ...
  /* Build: __mf_elem = &__mf_lookup_cache [(__mf_base >> __mf_shift)
                                            & __mf_mask].  */
  t = build2(
      RSHIFT_EXPR, mf_uintptr_type, mf_base,
      flag_mudflap_threads ? mf_cache_shift_decl : mf_cache_shift_decl_l);
  t = build2(BIT_AND_EXPR, mf_uintptr_type, t,
             flag_mudflap_threads ? mf_cache_mask_decl : mf_cache_mask_decl_l);
  t = build4(ARRAY_REF, TREE_TYPE(TREE_TYPE(mf_cache_array_decl)),
  ...
  g = gimple_build_cond(NE_EXPR, cond, integer_zero_node, NULL_TREE, NULL_TREE);
  gimple_set_location(g, location);
  gimple_seq_add_stmt(&seq, g);
...
static void mf_xform_derefs_1(gimple_stmt_iterator *iter, tree *tp,
                              location_t location, tree dirflag) {
  ...
  do {
    ...
    for (i = gsi_start_bb(bb); !gsi_end_p(i); gsi_next(&i)) {
      ...
      switch (gimple_code(s)) {
        case GIMPLE_ASSIGN:
          mf_xform_derefs_1(&i, gimple_assign_lhs_ptr(s), gimple_location(s),
                            integer_one_node);
          mf_xform_derefs_1(&i, gimple_assign_rhs1_ptr(s), gimple_location(s),
                            integer_zero_node);
          grhs_class = get_gimple_rhs_class(gimple_assign_rhs_code(s));
          if (grhs_class == GIMPLE_BINARY_RHS)
            mf_xform_derefs_1(&i, gimple_assign_rhs2_ptr(s), gimple_location(s),
                              integer_zero_node);
          break;
...
gimple_seq mx_register_decls(tree decl, gimple_seq seq, location_t location) {
  ...
  while (decl != NULL_TREE) {
    if (mf_decl_eligible_p(decl)
      ...
      unregister_fncall_param =
          mf_mark(build1(ADDR_EXPR, build_pointer_type(TREE_TYPE(decl)), decl));
      /* __mf_unregister (&VARIABLE, sizeof (VARIABLE), __MF_TYPE_STACK) */
      unregister_fncall =
          gimple_build_call(mf_unregister_fndecl, 3, unregister_fncall_param,
                            size, build_int_cst(NULL_TREE, 3));
      ...
      /* __mf_register (&VARIABLE, sizeof (VARIABLE), __MF_TYPE_STACK,
                        "name") */
      register_fncall =
          gimple_build_call(mf_register_fndecl, 4, register_fncall_param, size,
                            build_int_cst(NULL_TREE, 3), variable_name);
      ...
      if (gsi_end_p(initially_stmts)) {
        if (!DECL_ARTIFICIAL(decl))
          warning(OPT_Wmudflap, "mudflap cannot track %qs in stub function",
    ...
    case GIMPLE_BIND: {
      ...
      if (d->param_decls) {
        gimple_bind_set_body(
            stmt, mx_register_decls(d->param_decls, gimple_bind_body(stmt),
                                    gimple_location(stmt)));
        d->param_decls = NULL_TREE;
      }

      gimple_bind_set_body(stmt, mx_register_decls(gimple_bind_vars(stmt),
...
static void mf_xform_decls(gimple_seq fnbody, tree fnparams) {
...
static void mudflap_register_call(tree obj, tree object_size, tree varname) {
  ...
  call_stmt = build_call_expr(mf_register_fndecl, 4, arg,
                              convert(size_type_node, object_size),
                              /* __MF_TYPE_STATIC */
                              build_int_cst(NULL_TREE, 4), varname);
  ...
  if (deferred_static_decls) {
    ...
    for (i = 0; VEC_iterate(tree, deferred_static_decls, i, obj); i++) {
      ...
      if (!COMPLETE_TYPE_P(TREE_TYPE(obj))) {
        warning(OPT_Wmudflap, "mudflap cannot track unknown size extern %qs",
                IDENTIFIER_POINTER(DECL_NAME(obj)));
        continue;
      }

      mudflap_register_call(obj, size_in_bytes(TREE_TYPE(obj)),
                            mf_varname_tree(obj));
    }
