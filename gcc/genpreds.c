...
static bool validate_exp(rtx exp, const char *name, int lineno) {
  if (exp == 0) {
  ...
  switch (GET_CODE(exp)) {
    ...
    default:
      message_with_line(lineno, "%s: cannot use '%s' in a predicate expression",
...
static void process_define_predicate(rtx defn, int lineno) {
...
static void write_predicate_subfunction(struct pred_data *p) {
  ...
  obstack_grow(rtl_obstack, "_1 (op, mode)", sizeof "_1 (op, mode)");
...
static void mark_mode_tests(rtx exp) {
  switch (GET_CODE(exp)) {
    ...
    case MATCH_CODE:
      if (XSTR(exp, 1)[0] != '\0' || (!strstr(XSTR(exp, 0), "const_int") &&
    ...
    case AND:
      ...
      NO_MODE_TEST(exp) =
          (NO_MODE_TEST(XEXP(exp, 0)) && NO_MODE_TEST(XEXP(exp, 1)));
      break;
...
static bool generate_switch_p(rtx exp) {
  return GET_CODE(exp) == MATCH_CODE && strchr(XSTR(exp, 0), ',');
}
...
static const char *mangle(const char *name) {
  for (; *name; name++)
    switch (*name) {
      case '_':
        obstack_grow(rtl_obstack, "__", 2);
        break;
      case '<':
        obstack_grow(rtl_obstack, "_l", 2);
        break;
      case '>':
        obstack_grow(rtl_obstack, "_g", 2);
        break;
      default:
...
static void add_constraint(const char *name, const char *regclass, rtx exp,
                           bool is_memory, bool is_address, int lineno) {
  ...
  if (strchr(generic_constraint_letters, name[0])) {
    if (name[1] == '\0')
      message_with_line(lineno,
                        ...
                        "redefined by the machine description",
                        name);
    else
      message_with_line(lineno,
  ...
  if (is_const_int || is_const_dbl) {
    enum rtx_code appropriate_code = is_const_int ? CONST_INT : CONST_DOUBLE;

    /* Consider relaxing this requirement in the future.  */
    if (regclass || GET_CODE(exp) != AND ||
        GET_CODE(XEXP(exp, 0)) != MATCH_CODE ||
        strcmp(XSTR(XEXP(exp, 0), 0), GET_RTX_NAME(appropriate_code))) {
      if (name[1] == '\0')
        message_with_line(lineno,
...
static void process_define_constraint(rtx c, int lineno) {
  add_constraint(XSTR(c, 0), 0, XEXP(c, 2),
                 GET_CODE(c) == DEFINE_MEMORY_CONSTRAINT,
                 GET_CODE(c) == DEFINE_ADDRESS_CONSTRAINT, lineno);
}

/* Process a DEFINE_REGISTER_CONSTRAINT expression, C.  */
static void process_define_register_constraint(rtx c, int lineno) {
...
static void write_lookup_constraint(void) {
  unsigned int i;
  puts(
      "enum constraint_num\n"
  ...
  for (i = 0; i < ARRAY_SIZE(constraints_by_letter_table); i++) {
       ...
       length.  */
    {
      ...
      while (c2) {
        if (c2->namelen != len)
          error(
              "Multi-letter constraints with first letter '%c' "
...
static void write_regclass_for_constraint(void) {
  struct constraint_data *c;
...
#define GCC_TM_CONSTRS_H\n");
  ...
  if (!c->is_register) {
    ...
    bool needs_mode = (needs_variable(c->exp, "mode") || needs_hval ||
                       needs_lval || needs_rval);
    bool needs_op = (needs_variable(c->exp, "op") || needs_ival || needs_mode);

    printf(
        "static inline bool\n"
        "satisfies_constraint_%s (rtx %s)\n"
        "{\n",
        c->c_name, needs_op ? "op" : "ARG_UNUSED (op)");
    if (needs_mode)
    ...
    if (needs_hval)
      puts(
          "  if (GET_CODE (op) == CONST_DOUBLE && mode == VOIDmode)"
...
/* Write out the wrapper function, constraint_satisfied_p, that maps
   ...
   above.  */
static void write_constraint_satisfied_p(void) {
  struct constraint_data *c;
...
/* Write out the function which computes whether a given constraint is
   a memory constraint.  */
static void write_insn_extra_memory_constraint(void) {
  struct constraint_data *c;
...
static void write_insn_extra_address_constraint(void) {
  struct constraint_data *c;
