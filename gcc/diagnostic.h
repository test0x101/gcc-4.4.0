...
#define diagnostic_last_function_changed(DC, DI) \
  ((DC)->last_function !=                        \
   ((DI)->abstract_origin ? (DI)->abstract_origin : current_function_decl))

/* Remember the current function as being the last one in which we report
   a diagnostic.  */
#define diagnostic_set_last_function(DC, DI)                   \
  (DC)->last_function =                                        \
      (((DI) && (DI)->abstract_origin) ? (DI)->abstract_origin \
...
#define diagnostic_report_warnings_p(LOC) \
  (!inhibit_warnings && !(in_system_header_at(LOC) && !warn_system_headers))
...
extern void diagnostic_set_info_translated(diagnostic_info *, const char *,
                                           va_list *, location_t, diagnostic_t)
    ATTRIBUTE_GCC_DIAG(2, 0);
