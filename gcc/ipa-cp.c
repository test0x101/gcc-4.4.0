...
static inline struct cgraph_node *ipcp_get_orig_node(struct cgraph_node *node) {
  return IPA_NODE_REF(node)->ipcp_orig_node;
}

/* Return true if NODE describes a cloned/versioned function.  */
static inline bool ipcp_node_is_clone(struct cgraph_node *node) {
...
static void ipcp_init_cloned_node(struct cgraph_node *orig_node,
...
static void ipcp_analyze_node(struct cgraph_node *node) {
...
static void ipcp_update_cloned_node(struct cgraph_node *new_node) {
...
static inline gcov_type ipcp_get_node_scale(struct cgraph_node *node) {
  return IPA_NODE_REF(node)->count_scale;
}

/* Set COUNT as scale for NODE.  */
static inline void ipcp_set_node_scale(struct cgraph_node *node,
                                       gcov_type count) {
  IPA_NODE_REF(node)->count_scale = count;
}

/* Return whether LAT is a constant lattice.  */
static inline bool ipcp_lat_is_const(struct ipcp_lattice *lat) {
...
static inline bool ipcp_lat_is_insertable(struct ipcp_lattice *lat) {
  return lat->type == IPA_CONST_VALUE;
}

/* Return true if LAT1 and LAT2 are equal.  */
static inline bool ipcp_lats_are_equal(struct ipcp_lattice *lat1,
...
static void ipa_lattice_meet(struct ipcp_lattice *res,
                             struct ipcp_lattice *lat1,
                             struct ipcp_lattice *lat2) {
...
/* Return the lattice corresponding to the Ith formal parameter of the function
   described by INFO.  */
static inline struct ipcp_lattice *ipcp_get_lattice(
    struct ipa_node_params *info, int i) {
...
static void ipcp_lattice_from_jfunc(struct ipa_node_params *info,
...
static bool ipcp_lattice_changed(struct ipcp_lattice *old_lat,
...
static void ipcp_print_all_lattices(FILE *f) {
...
static void ipcp_initialize_node_lattices(struct cgraph_node *node) {
  int i;
...
/* build INTEGER_CST tree with type TREE_TYPE and value according to LAT.
   Return the tree.  */
static tree build_const_val(struct ipcp_lattice *lat, tree tree_type) {
  tree val;
...
static void ipcp_compute_node_scale(struct cgraph_node *node) {
  gcov_type sum;
  struct cgraph_edge *cs;
...
static bool ipcp_change_tops_to_bottom(void) {
  int i, count;
  struct cgraph_node *node;
  bool prop_again;
...
/* Check conditions to forbid constant insertion to function described by
   NODE.  */
static inline bool ipcp_node_modifiable_p(struct cgraph_node *node) {
...
static void ipcp_function_scale_print(FILE *f) {
  struct cgraph_node *node;
...
static void ipcp_print_func_profile_counts(FILE *f) {
  struct cgraph_node *node;
...
static void ipcp_print_call_profile_counts(FILE *f) {
  struct cgraph_node *node;
  struct cgraph_edge *cs;
...
static void ipcp_print_edge_profiles(FILE *f) {
  struct cgraph_node *node;
  basic_block bb;
  edge_iterator ei;
  edge e;
  ...
  for (node = cgraph_nodes; node; node = node->next) {
    ...
    if (node->analyzed) {
      bb = ENTRY_BLOCK_PTR_FOR_FUNCTION(DECL_STRUCT_FUNCTION(node->decl));
      fprintf(f, "ENTRY: ");
      fprintf(f, " " HOST_WIDE_INT_PRINT_DEC " %d\n", (HOST_WIDE_INT)bb->count,
              bb->frequency);
      ...
      FOR_EACH_BB_FN(bb, DECL_STRUCT_FUNCTION(node->decl)) {
        ...
        FOR_EACH_EDGE(e, ei, bb->succs) {
          if (e->dest ==
            ...
            fprintf(f, "edge %d -> %d,  Count", e->src->index, e->dest->index);
          fprintf(f, " " HOST_WIDE_INT_PRINT_DEC " Prob %d\n",
                  (HOST_WIDE_INT)e->count, e->probability);
        }
      }
...
static void ipcp_print_bb_profiles(FILE *f) {
  basic_block bb;
  struct cgraph_node *node;
  ...
  for (node = cgraph_nodes; node; node = node->next) {
    ...
    if (node->analyzed) {
      ...
      fprintf(f, " " HOST_WIDE_INT_PRINT_DEC " Frequency  %d\n",
              (HOST_WIDE_INT)bb->count, bb->frequency);
      ...
      bb = EXIT_BLOCK_PTR_FOR_FUNCTION(DECL_STRUCT_FUNCTION(node->decl));
...
static void ipcp_print_profile_data(FILE *f) {
  fprintf(f, "\nNODE COUNTS :\n");
...
/* Build and initialize ipa_replace_map struct according to LAT. This struct is
   ...
   constant.  */
static struct ipa_replace_map *ipcp_create_replace_map(
    tree parm_tree, struct ipcp_lattice *lat) {
...
static bool ipcp_need_redirect_p(struct cgraph_edge *cs) {
    ...
    if (node->analyzed && ipcp_node_is_clone(node)) {
      ...
      for (cs = node->callers; cs; cs = next) {
        ...
        if (ipcp_node_is_clone(cs->caller) || !ipcp_need_redirect_p(cs)) {
          ...
          new_stmt = gimple_call_copy_skip_args(cs->call_stmt, args_to_skip);
...
static void ipcp_update_bb_counts(struct cgraph_node *node, gcov_type scale) {
  basic_block bb;
...
static void ipcp_update_edges_counts(struct cgraph_node *node,
                                     gcov_type scale) {
  basic_block bb;
  edge_iterator ei;
  edge e;
...
static void ipcp_update_profiling(void) {
  ...
  for (node = cgraph_nodes; node; node = node->next) {
    if (ipcp_node_is_clone(node)) {
      ...
      orig_node->count = orig_node->count * scale_complement / REG_BR_PROB_BASE;
      for (cs = node->callees; cs; cs = cs->next_callee)
...
static long ipcp_estimate_growth(struct cgraph_node *node) {
  struct cgraph_edge *cs;
...
static long ipcp_estimate_cloning_cost(struct cgraph_node *node) {
  int freq_sum = 1;
  ...
  for (e = node->callers; e; e = e->next_caller)
    if (!bitmap_bit_p(dead_nodes, e->caller->uid) && !ipcp_need_redirect_p(e)) {
      count_sum += e->count;
...
static int ipcp_const_param_count(struct cgraph_node *node) {
  int const_param = 0;
  struct ipa_node_params *info = IPA_NODE_REF(node);
...
static void ipcp_insert_stage(void) {
  ...
  while (!fibheap_empty(heap)) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (lat->type == IPA_CONST_VALUE) {
        replace_param = ipcp_create_replace_map(parm_tree, lat);
    ...
    for (cs = node->callees; cs; cs = cs->next_callee)
      if (cs->callee->aux) {
        ...
        cs->callee->aux = fibheap_insert(
            heap, ipcp_estimate_cloning_cost(cs->callee), cs->callee);
      }
  }
...
static bool cgraph_gate_cp(void) {
  return flag_ipa_cp;
}
