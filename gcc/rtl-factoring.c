...
static void match_seqs(p_hash_elem e0, p_hash_elem e1) {
  ...
  if (!pattern_seqs || pattern_seqs->insn != e0->insn) {
    pattern_seq pseq = (pattern_seq)xmalloc(sizeof(struct pattern_seq_def));
    pseq->insn = e0->insn;
    pseq->idx = e0->idx;
...
static void collect_pattern_seqs(void) {
  ...
  FOR_EACH_HTAB_ELEMENT(hash_bucket->seq_candidates, e1, p_hash_elem, hti2)
  if (e0 != e1
#if defined STACK_REGS || defined HAVE_cc0
...
static void recompute_gain_for_pattern_seq(pattern_seq pseq) {
  ...
  for (mseq = pseq->matching_seqs; mseq; mseq = mseq->next_matching_seq) {
    ...
    if (mseq->next_matching_seq)
      mseq->abstracted_length = (int)(mseq->next_matching_seq->idx - mseq->idx);
    else
    ...
    if (mseq->cost > seq_call_cost) {
      clear_regs_live_in_seq(&linkregs, mseq->insn, mseq->abstracted_length);
...
static void erase_from_pattern_seqs(rtx insn, int len) {
  ...
  while (len > 0) {
    for (pseq = &pattern_seqs; *pseq;) {
      plen = 0;
      pcost = 0;
      for (x = (*pseq)->insn; x && (x != insn); x = prev_insn_in_block(x)) {
        plen++;
      ...
      if (pcost <= seq_call_cost) {
        ...
        *pseq = temp->next_pattern_seq;
        ...
        for (mseq = &(*pseq)->matching_seqs; *mseq;) {
          mlen = 0;
          mcost = 0;
          for (x = (*mseq)->insn; x && (x != insn) && (mlen < plen) &&
    ...
    if (!seq_blocks || ((*mseq)->abstracted_length < seq_blocks->length)) {
      ...
      for (sb = seq_blocks; sb; sb = sb->next_seq_block) {
        ...
        if (!sb->next_seq_block ||
            ((*mseq)->abstracted_length < sb->next_seq_block->length)) {
          seq_block temp = (seq_block)xmalloc(sizeof(struct seq_block_def));
...
static rtx gen_symbol_ref_rtx_for_label(const_rtx label) {
  char name[20];
  rtx sym;
...
static basic_block split_block_and_df_analyze(basic_block bb, rtx insn) {
  basic_block next;
...
static void split_pattern_seq(void) {
  ...
  /* Emit an insn saving the return address to the link register before the
     sequence.  */
  saveinsn =
      emit_insn_after(gen_move_insn(pattern_seqs->link_reg,
                                    gen_symbol_ref_rtx_for_label(retlabel)),
                      BB_END(bb));
  /* Update liveness info.  */
  SET_REGNO_REG_SET(df_get_live_out(bb), REGNO(pattern_seqs->link_reg));
}
...
static void erase_matching_seqs(void) {
  ...
  for (sb = seq_blocks; sb; sb = sb->next_seq_block) {
    for (mseq = sb->matching_seqs; mseq; mseq = mseq->next_matching_seq) {
      ...
      make_edge(BLOCK_FOR_INSN(seq_blocks->label), BLOCK_FOR_INSN(retlabel),
                EDGE_ABNORMAL);
    }
  }
}
  ...
  FOR_EACH_BB(bb) {
    FOR_BB_INSNS_REVERSE(bb, insn) {
      ...
      if (!bucket) {
        ...
        bucket = (p_hash_bucket)xcalloc(1, sizeof(struct hash_bucket_def));
      ...
      if (!bucket->seq_candidates)
        bucket->seq_candidates =
            htab_create(HASH_INIT, htab_hash_elem, htab_eq_elem, htab_del_elem);
...
static void rtl_seqabstr(void) {
  ...
  hash_buckets =
      htab_create(HASH_INIT, htab_hash_bucket, htab_eq_bucket, htab_del_bucket);
  fill_hash_bucket();
