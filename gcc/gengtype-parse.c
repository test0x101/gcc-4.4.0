...
static inline const char *advance(void) {
  T.valid = false;
  return T.value;
}
...
static const char *print_token(int code, const char *value) {
  if (code < CHAR_TOKEN_OFFSET)
    ...
    return xasprintf(token_value_format[code - FIRST_TOKEN_WITH_VALUE], value);
}

/* Convenience wrapper around print_token which produces the printable
   representation of the current token.  */
static inline const char *print_cur_token(void) {
...
static const char *require(int t) {
  ...
  if (u != t) {
    parse_error("expected %s, have %s", print_token(t, 0), print_token(u, v));
    return 0;
  }
  return v;
}
...
static const char *require2(int t1, int t2) {
  ...
  const char *v = advance();
  if (u != t1 && u != t2) {
    parse_error("expected %s or %s, have %s", print_token(t1, 0),
                print_token(t2, 0), print_token(u, v));
    return 0;
  }
  return v;
}
...
static void consume_until_semi(bool immediate) {
  if (immediate && token() != ';')
    require(';');
  for (;;)
    switch (token()) {
...
static bool consume_until_comma_or_semi(bool immediate) {
...
static type_p direct_declarator(type_p ty, const char **namep,
                                options_p *optsp) {
...
static type_p inner_declarator(type_p ty, const char **namep,
                               options_p *optsp) {
  if (token() == '*') {
    type_p inner;
    advance();
...
static type_p declarator(type_p ty, const char **namep, options_p *optsp) {
  *namep = 0;
  *optsp = 0;
  while (token() == '*') {
    advance();
...
static void def_vec_alloc(void) {
