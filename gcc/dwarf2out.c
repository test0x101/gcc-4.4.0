...
static inline dw_fde_ref current_fde(void) {
...
static struct dw_loc_descr_struct *build_cfa_loc(dw_cfa_location *,
                                                 HOST_WIDE_INT);
static struct dw_loc_descr_struct *build_cfa_aligned_loc(HOST_WIDE_INT,
                                                         HOST_WIDE_INT);
...
static inline char *stripattributes(const char *s) {
...
static void init_return_column_size(enum machine_mode mode, rtx mem,
                                    unsigned int c) {
...
static inline void add_cfi(dw_cfi_ref *list_head, dw_cfi_ref cfi) {
  dw_cfi_ref *p;
...
static void add_fde_cfi(const char *label, dw_cfi_ref cfi) {
  ...
  if (dwarf2out_do_cfi_asm()) {
    ...
    if (fde->dw_fde_current_label == NULL ||
      ...
      xcfi->dw_cfi_opc =
          fde->dw_fde_current_label ? DW_CFA_advance_loc4 : DW_CFA_set_loc;
...
static void lookup_cfa_1(dw_cfi_ref cfi, dw_cfa_location *loc) {
  switch (cfi->dw_cfi_opc) {
    case DW_CFA_def_cfa_offset:
    case DW_CFA_def_cfa_offset_sf:
...
static void lookup_cfa(dw_cfa_location *loc) {
  dw_cfi_ref cfi;
  dw_fde_ref fde;
...
void dwarf2out_def_cfa(const char *label, unsigned int reg,
                       HOST_WIDE_INT offset) {
  dw_cfa_location loc;
  loc.indirect = 0;
  loc.base_offset = 0;
...
static bool cfa_equal_p(const dw_cfa_location *loc1,
                        const dw_cfa_location *loc2) {
  return (loc1->reg == loc2->reg && loc1->offset == loc2->offset &&
          loc1->indirect == loc2->indirect &&
          (loc1->indirect == 0 || loc1->base_offset == loc2->base_offset));
}

/* This routine does the actual work.  The CFA is now calculated from
   the dw_cfa_location structure.  */

static void def_cfa_1(const char *label, dw_cfa_location *loc_p) {
  dw_cfi_ref cfi;
  ...
  else if (loc.offset == old_cfa.offset && old_cfa.reg != INVALID_REGNUM &&
           !loc.indirect) {
...
static void reg_save(const char *label, unsigned int reg, unsigned int sreg,
                     HOST_WIDE_INT offset) {
  ...
  /* When stack is aligned, store REG using DW_CFA_expression with
     FP.  */
  if (fde && fde->stack_realign && sreg == INVALID_REGNUM) {
...
void dwarf2out_window_save(const char *label) {
  dw_cfi_ref cfi = new_cfi();
...
void dwarf2out_args_size(const char *label, HOST_WIDE_INT size) {
  dw_cfi_ref cfi;
...
void dwarf2out_reg_save(const char *label, unsigned int reg,
                        HOST_WIDE_INT offset) {
...
void dwarf2out_return_save(const char *label, HOST_WIDE_INT offset) {
...
void dwarf2out_return_reg(const char *label, unsigned int sreg) {
...
static HOST_WIDE_INT stack_adjust_offset(const_rtx pattern,
  ...
  if (dest == stack_pointer_rtx) {
    ...
    if (!(code == PLUS || code == MINUS) || XEXP(src, 0) != stack_pointer_rtx ||
...
static HOST_WIDE_INT compute_barrier_args_size_1(rtx insn,
                                                 HOST_WIDE_INT cur_args_size,
                                                 VEC(rtx, heap) * *next) {
  HOST_WIDE_INT offset = 0;
  int i;
...
static void compute_barrier_args_size(void) {
        ...
        if (GET_CODE(body) == SEQUENCE) {
          ...
          for (i = 1; i < XVECLEN(body, 0); i++)
            if (INSN_ANNULLED_BRANCH_P(XVECEXP(body, 0, 0)) &&
              ...
              dest_args_size = compute_barrier_args_size_1(
                  XVECEXP(body, 0, i), dest_args_size, &next);
            else
              cur_args_size = compute_barrier_args_size_1(XVECEXP(body, 0, i),
                                                          cur_args_size, &next);

          if (INSN_ANNULLED_BRANCH_P(XVECEXP(body, 0, 0)))
            compute_barrier_args_size_1(XVECEXP(body, 0, 0), dest_args_size,
                                        &next);
          else
            cur_args_size = compute_barrier_args_size_1(XVECEXP(body, 0, 0),
                                                        cur_args_size, &next);
        } else
          cur_args_size =
...
static void dwarf2out_stack_adjust(rtx insn, bool after_p) {
  HOST_WIDE_INT offset;
  const char *label;
  int i;
  ...
  if (final_sequence && INSN_ANNULLED_BRANCH_P(XVECEXP(final_sequence, 0, 0)) &&
  ...
  if (CALL_P(insn) && !after_p) {
    ...
    for (offset = 0, i = XVECLEN(PATTERN(insn), 0) - 1; i >= 0; i--)
      if (GET_CODE(XVECEXP(PATTERN(insn), 0, i)) == SET)
        offset += stack_adjust_offset(XVECEXP(PATTERN(insn), 0, i), args_size,
                                      offset);
  } else
    return;
...
static void dwarf2out_args_size_adjust(HOST_WIDE_INT offset,
                                       const char *label) {
...
static void queue_reg_save(const char *label, rtx reg, rtx sreg,
                           HOST_WIDE_INT offset) {
...
void dwarf2out_reg_save_reg(const char *label, rtx reg, rtx sreg) {
  size_t i;
...
static void dwarf2out_frame_debug_expr(rtx expr, const char *label) {
  rtx src, dest, span;
  HOST_WIDE_INT offset;
  dw_fde_ref fde;
  ...
  if (GET_CODE(expr) == PARALLEL || GET_CODE(expr) == SEQUENCE) {
    ...
    if (GET_CODE(expr) == PARALLEL)
      for (par_index = 0; par_index < limit; par_index++) {
        ...
        if (GET_CODE(elem) == SET && MEM_P(SET_DEST(elem)) &&
    ...
    for (par_index = 0; par_index < limit; par_index++) {
      ...
      if (GET_CODE(elem) == SET &&
          ...
          (RTX_FRAME_RELATED_P(elem) || par_index == 0))
        dwarf2out_frame_debug_expr(elem, label);
      else if (GET_CODE(elem) == SET && par_index != 0 &&
  ...
  if (GET_CODE(src) == REG && fde && fde->drap_reg == REGNO(src) &&
      (fde->drap_reg_saved || GET_CODE(dest) == REG)) {
    /* Rule 20 */
      ...
      switch (GET_CODE(src)) {
        ...
        case REG:
          if (cfa.reg == (unsigned)REGNO(src)) {
            ...
            if (fde && fde->stack_realign && REGNO(src) == STACK_POINTER_REGNUM)
          ...
          if (dest == stack_pointer_rtx) {
            ...
            switch (GET_CODE(XEXP(src, 1))) {
              case CONST_INT:
                ...
                break;
              case REG:
                gcc_assert((unsigned)REGNO(XEXP(src, 1)) == cfa_temp.reg);
            ...
            /* Rule 4 */
            if (REG_P(XEXP(src, 0)) && REGNO(XEXP(src, 0)) == cfa.reg &&
                GET_CODE(XEXP(src, 1)) == CONST_INT) {
    ...
    case MEM:
      ...
      switch (GET_CODE(XEXP(dest, 0))) {
        ...
        case PRE_MODIFY:
          ...
          gcc_assert(GET_CODE(XEXP(XEXP(XEXP(dest, 0), 1), 1)) == CONST_INT);
        ...
        case PRE_DEC:
          ...
          /* Rule 18: If stack is aligned, we will use FP as a
             ...
             regiser.  */
          if (fde && fde->stack_realign && src == hard_frame_pointer_rtx) {
        ...
        case REG: {
          ...
          /* Rule 14 */
        case POST_INC:
          gcc_assert(cfa_temp.reg == (unsigned)REGNO(XEXP(XEXP(dest, 0), 0)));
          offset = -cfa_temp.offset;
      ...
      if (REGNO(src) != STACK_POINTER_REGNUM &&
        ...
        if (cfa.offset == 0) {
          ...
          /* If stack is aligned, putting CFA reg into stack means
             ...
             value.  */
          if (fde && fde->stack_realign && cfa.indirect == 0 &&
              cfa.reg != HARD_FRAME_POINTER_REGNUM) {
...
void dwarf2out_frame_debug(rtx insn, bool after_p) {
  const char *label;
  rtx src;
...
static void switch_to_eh_frame_section(void) {
  tree label;
  ...
  if (eh_frame_section == 0) {
    ...
    if (EH_TABLES_CAN_BE_READ_ONLY) {
      ...
      lsda_encoding = ASM_PREFERRED_EH_DATA_FORMAT(/*code=*/0,
                                                   /*global=*/0);
      flags = ((!flag_pic || ((fde_encoding & 0x70) != DW_EH_PE_absptr &&
...
static void output_cfi(dw_cfi_ref cfi, dw_fde_ref fde, int for_eh) {
  unsigned long r;
  HOST_WIDE_INT off;

  if (cfi->dw_cfi_opc == DW_CFA_advance_loc)
    dw2_asm_output_data(
        1, (cfi->dw_cfi_opc | (cfi->dw_cfi_oprnd1.dw_cfi_offset & 0x3f)),
    ...
    dw2_asm_output_data(1, cfi->dw_cfi_opc, "%s",
      ...
      case DW_CFA_set_loc:
        if (for_eh)
          dw2_asm_output_encoded_addr_rtx(
              ...
              gen_rtx_SYMBOL_REF(Pmode, cfi->dw_cfi_oprnd1.dw_cfi_addr), false,
              NULL);
        else
          dw2_asm_output_addr(DWARF2_ADDR_SIZE, cfi->dw_cfi_oprnd1.dw_cfi_addr,
                              NULL);
...
static void output_cfi_directive(dw_cfi_ref cfi) {
  unsigned long r, r2;
  ...
  switch (cfi->dw_cfi_opc) {
    ...
    case DW_CFA_def_cfa_offset_sf:
      fprintf(asm_out_file,
              "\t.cfi_def_cfa_offset " HOST_WIDE_INT_PRINT_DEC "\n",
...
static void output_call_frame_info(int for_eh) {
  unsigned int i;
  dw_fde_ref fde;
  dw_cfi_ref cfi;
  ...
  if (TARGET_USES_WEAK_UNWIND_INFO && !flag_asynchronous_unwind_tables &&
      flag_exceptions && for_eh)
    for (i = 0; i < fde_table_in_use; i++)
      if ((fde_table[i].nothrow || fde_table[i].all_throwers_are_sibcalls) &&
          !fde_table[i].uses_eh_lsda && !DECL_WEAK(fde_table[i].decl))
        targetm.asm_out.unwind_label(asm_out_file, fde_table[i].decl, for_eh,
                                     /* empty */ 1);
  ...
  if (for_eh) {
    ...
    for (i = 0; i < fde_table_in_use; i++)
      if (fde_table[i].uses_eh_lsda)
        ...
        any_eh_needed = true;
      else if (!fde_table[i].nothrow && !fde_table[i].all_throwers_are_sibcalls)
        any_eh_needed = true;
  ...
  if (for_eh)
    ...
    if (!debug_frame_section)
      debug_frame_section =
          get_section(DEBUG_FRAME_SECTION, SECTION_DEBUG, NULL);
  ...
  dw2_asm_output_data((for_eh ? 4 : DWARF_OFFSET_SIZE),
                      (for_eh ? 0 : DWARF_CIE_ID), "CIE Identifier Tag");
  ...
  if (augmentation[0]) {
    ...
    if (eh_personality_libfunc) {
      ...
      dw2_asm_output_encoded_addr_rtx(per_encoding, eh_personality_libfunc,
                                      true, NULL);
    }
  ...
  for (i = 0; i < fde_table_in_use; i++) {
    ...
    if (DWARF_INITIAL_LENGTH_SIZE - DWARF_OFFSET_SIZE == 4 && !for_eh)
      dw2_asm_output_data(
          4, 0xffffffff,
          "Initial length escape value indicating 64-bit DWARF extension");
    dw2_asm_output_delta(for_eh ? 4 : DWARF_OFFSET_SIZE, l2, l1, "FDE Length");
    ASM_OUTPUT_LABEL(asm_out_file, l1);
    ...
    if (for_eh) {
      if (fde->dw_fde_switched_sections) {
        rtx sym_ref2 =
            gen_rtx_SYMBOL_REF(Pmode, fde->dw_fde_unlikely_section_label);
        rtx sym_ref3 = gen_rtx_SYMBOL_REF(Pmode, fde->dw_fde_hot_section_label);
      ...
      if (fde->dw_fde_switched_sections) {
        dw2_asm_output_addr(DWARF2_ADDR_SIZE, fde->dw_fde_hot_section_label,
                            "FDE initial location");
        dw2_asm_output_delta(
            DWARF2_ADDR_SIZE, fde->dw_fde_hot_section_end_label,
            fde->dw_fde_hot_section_label, "FDE address range");
        dw2_asm_output_addr(DWARF2_ADDR_SIZE,
                            fde->dw_fde_unlikely_section_label,
                            "FDE initial location");
        dw2_asm_output_delta(
            DWARF2_ADDR_SIZE, fde->dw_fde_unlikely_section_end_label,
            fde->dw_fde_unlikely_section_label, "FDE address range");
      } else {
      ...
      if (any_lsda_needed) {
        ...
        if (fde->uses_eh_lsda) {
          ...
          dw2_asm_output_encoded_addr_rtx(lsda_encoding,
                                          gen_rtx_SYMBOL_REF(Pmode, l1), false,
...
void dwarf2out_begin_prologue(unsigned int line ATTRIBUTE_UNUSED,
  ...
  if (dwarf2out_do_cfi_asm()) {
    ...
    if (crtl->uses_eh_lsda) {
      ...
      enc = ASM_PREFERRED_EH_DATA_FORMAT(/*code=*/0, /*global=*/0);
      ASM_GENERATE_INTERNAL_LABEL(lab, "LLSDA", current_function_funcdef_no);
...
void dwarf2out_end_epilogue(unsigned int line ATTRIBUTE_UNUSED,
                            const char *file ATTRIBUTE_UNUSED) {
  dw_fde_ref fde;
  char label[MAX_ARTIFICIAL_LABEL_BYTES];
...
void dwarf2out_frame_init(void) {
...
static inline dw_loc_descr_ref new_reg_loc_descr(
    unsigned int reg, unsigned HOST_WIDE_INT offset) {
  if (offset) {
    if (reg <= 31)
...
static inline void add_loc_descr(dw_loc_descr_ref *list_head,
...
static unsigned long size_of_locs(dw_loc_descr_ref loc) {
  dw_loc_descr_ref l;
  unsigned long size;
  ...
  switch (loc->dw_loc_opc) {
    ...
    case INTERNAL_DW_OP_tls_addr:
      if (targetm.asm_out.output_dwarf_dtprel) {
        targetm.asm_out.output_dwarf_dtprel(asm_out_file, DWARF2_ADDR_SIZE,
                                            val1->v.val_addr);
        fputc('\n', asm_out_file);
      } else
        gcc_unreachable();
...
static void output_loc_sequence(dw_loc_descr_ref loc) {
  for (; loc != NULL; loc = loc->dw_loc_next) {
    /* Output the opcode.  */
    dw2_asm_output_data(1, loc->dw_loc_opc, "%s",
...
static void output_loc_sequence_raw(dw_loc_descr_ref loc) {
  while (1) {
    /* Output the opcode.  */
...
static struct dw_loc_descr_struct *build_cfa_loc(dw_cfa_location *cfa,
                                                 HOST_WIDE_INT offset) {
...
static void get_cfa_from_loc_descr(dw_cfa_location *cfa,
...
const struct gcc_debug_hooks dwarf2_debug_hooks = {
    dwarf2out_init, dwarf2out_finish, dwarf2out_define, dwarf2out_undef,
    dwarf2out_start_source_file, dwarf2out_end_source_file,
    dwarf2out_begin_block, dwarf2out_end_block, dwarf2out_ignore_block,
    dwarf2out_source_line, dwarf2out_begin_prologue,
    debug_nothing_int_charstar, /* end_prologue */
    dwarf2out_end_epilogue, dwarf2out_begin_function,
    debug_nothing_int,                          /* end_function */
    dwarf2out_decl,                             /* function_decl */
    dwarf2out_global_decl, dwarf2out_type_decl, /* type_decl */
    ...
    dwarf2out_var_location, dwarf2out_switch_text_section,
   ...
   (DECL_ARTIFICIAL(decl) && is_tagged_type(TREE_TYPE(decl)) &&               \
    ((decl == TYPE_STUB_DECL(                                                 \
     ...
     || (DECL_ABSTRACT_ORIGIN(decl) != NULL_TREE &&                           \
         (decl_ultimate_origin(decl) == TYPE_STUB_DECL(TREE_TYPE(decl)))))))
...
static GTY((length("abbrev_die_table_allocated"))) dw_die_ref *abbrev_die_table;
...
static void gen_descr_array_type_die(tree, struct array_descr_info *,
                                     dw_die_ref);
#if 0
...
static inline int is_pseudo_reg(const_rtx rtl) {
...
static inline int is_tagged_type(const_tree type) {
...
static inline enum dw_val_class AT_class(dw_attr_ref a) {
  return a->dw_attr_val.val_class;
}

/* Add a flag value attribute to a DIE.  */

static inline void add_AT_flag(dw_die_ref die, enum dwarf_attribute attr_kind,
                               unsigned int flag) {
  dw_attr_node attr;
...
static inline unsigned AT_flag(dw_attr_ref a) {
...
static inline void add_AT_int(dw_die_ref die, enum dwarf_attribute attr_kind,
...
static inline HOST_WIDE_INT AT_int(dw_attr_ref a) {
...
static inline void add_AT_unsigned(dw_die_ref die,
...
static inline unsigned HOST_WIDE_INT AT_unsigned(dw_attr_ref a) {
...
static inline void add_AT_vec(dw_die_ref die, enum dwarf_attribute attr_kind,
                              unsigned int length, unsigned int elt_size,
                              unsigned char *array) {
  dw_attr_node attr;
...
static hashval_t debug_str_do_hash(const void *x) {
  return htab_hash_string(((const struct indirect_string_node *)x)->str);
}

static int debug_str_eq(const void *x1, const void *x2) {
...
static inline void add_AT_string(dw_die_ref die, enum dwarf_attribute attr_kind,
                                 const char *str) {
  dw_attr_node attr;
  ...
  if (!debug_str_hash)
    debug_str_hash = htab_create_ggc(10, debug_str_do_hash, debug_str_eq, NULL);

  slot = htab_find_slot_with_hash(debug_str_hash, str, htab_hash_string(str),
                                  INSERT);
  if (*slot == NULL) {
    node = (struct indirect_string_node *)ggc_alloc_cleared(
        sizeof(struct indirect_string_node));
...
static inline const char *AT_string(dw_attr_ref a) {
...
static int AT_string_form(dw_attr_ref a) {
  struct indirect_string_node *node;
  unsigned int len;
  char label[32];
...
static inline void add_AT_die_ref(dw_die_ref die,
                                  enum dwarf_attribute attr_kind,
                                  dw_die_ref targ_die) {
  dw_attr_node attr;
...
static inline void add_AT_specification(dw_die_ref die, dw_die_ref targ_die) {
  add_AT_die_ref(die, DW_AT_specification, targ_die);
  gcc_assert(!targ_die->die_definition);
  targ_die->die_definition = die;
}

static inline dw_die_ref AT_ref(dw_attr_ref a) {
  gcc_assert(a && AT_class(a) == dw_val_class_die_ref);
  return a->dw_attr_val.v.val_die_ref.die;
}

static inline int AT_ref_external(dw_attr_ref a) {
...
static inline void set_AT_ref_external(dw_attr_ref a, int i) {
  gcc_assert(a && AT_class(a) == dw_val_class_die_ref);
...
static inline void add_AT_fde_ref(dw_die_ref die,
                                  enum dwarf_attribute attr_kind,
...
static inline void add_AT_loc(dw_die_ref die, enum dwarf_attribute attr_kind,
                              dw_loc_descr_ref loc) {
  dw_attr_node attr;
...
static inline dw_loc_descr_ref AT_loc(dw_attr_ref a) {
  gcc_assert(a && AT_class(a) == dw_val_class_loc);
  return a->dw_attr_val.v.val_loc;
}

static inline void add_AT_loc_list(dw_die_ref die,
                                   enum dwarf_attribute attr_kind,
...
static inline dw_loc_list_ref AT_loc_list(dw_attr_ref a) {
...
static inline void add_AT_addr(dw_die_ref die, enum dwarf_attribute attr_kind,
                               rtx addr) {
  dw_attr_node attr;
...
static inline rtx AT_addr(dw_attr_ref a) {
...
static inline void add_AT_file(dw_die_ref die, enum dwarf_attribute attr_kind,
...
static inline struct dwarf_file_data *AT_file(dw_attr_ref a) {
...
static inline void add_AT_lbl_id(dw_die_ref die, enum dwarf_attribute attr_kind,
                                 const char *lbl_id) {
  dw_attr_node attr;
...
static inline void add_AT_lineptr(dw_die_ref die,
                                  enum dwarf_attribute attr_kind,
                                  const char *label) {
  dw_attr_node attr;
...
static inline void add_AT_macptr(dw_die_ref die, enum dwarf_attribute attr_kind,
                                 const char *label) {
  dw_attr_node attr;
...
static inline void add_AT_offset(dw_die_ref die, enum dwarf_attribute attr_kind,
...
static void add_AT_range_list(dw_die_ref die, enum dwarf_attribute attr_kind,
...
static inline const char *AT_lbl(dw_attr_ref a) {
...
static dw_attr_ref get_AT(dw_die_ref die, enum dwarf_attribute attr_kind) {
  dw_attr_ref a;
  unsigned ix;
...
static inline const char *get_AT_string(dw_die_ref die,
                                        enum dwarf_attribute attr_kind) {
...
static inline unsigned get_AT_unsigned(dw_die_ref die,
                                       enum dwarf_attribute attr_kind) {
  dw_attr_ref a = get_AT(die, attr_kind);
...
static inline dw_die_ref get_AT_ref(dw_die_ref die,
                                    enum dwarf_attribute attr_kind) {
  dw_attr_ref a = get_AT(die, attr_kind);
...
static inline struct dwarf_file_data *get_AT_file(
...
static inline bool is_c_family(void) {
...
static inline bool is_cxx(void) {
  unsigned int lang = get_AT_unsigned(comp_unit_die, DW_AT_language);
...
static inline bool is_fortran(void) {
  unsigned int lang = get_AT_unsigned(comp_unit_die, DW_AT_language);
...
static inline bool is_java(void) {
  unsigned int lang = get_AT_unsigned(comp_unit_die, DW_AT_language);
...
static inline bool is_ada(void) {
  unsigned int lang = get_AT_unsigned(comp_unit_die, DW_AT_language);
...
static void remove_AT(dw_die_ref die, enum dwarf_attribute attr_kind) {
  dw_attr_ref a;
  unsigned ix;
...
static void remove_child_TAG(dw_die_ref die, enum dwarf_tag tag) {
  dw_die_ref c;
...
static void add_child_die(dw_die_ref die, dw_die_ref child_die) {
...
static void splice_child_die(dw_die_ref parent, dw_die_ref child) {
  dw_die_ref p;
...
static inline dw_die_ref new_die(enum dwarf_tag tag_value,
                                 dw_die_ref parent_die, tree t) {
...
static inline void equate_type_number_to_die(tree type, dw_die_ref type_die) {
  TYPE_SYMTAB_DIE(type) = type_die;
}

/* Returns a hash value for X (which really is a die_struct).  */

static hashval_t decl_die_table_hash(const void *x) {
  return (hashval_t)((const_dw_die_ref)x)->decl_id;
}

/* Return nonzero if decl_id of die_struct X is the same as UID of decl *Y.  */

static int decl_die_table_eq(const void *x, const void *y) {
...
static hashval_t decl_loc_table_hash(const void *x) {
...
static int decl_loc_table_eq(const void *x, const void *y) {
  return (((const var_loc_list *)x)->decl_id == DECL_UID((const_tree)y));
}

/* Return the var_loc list associated with a given declaration.  */

static inline var_loc_list *lookup_decl_loc(const_tree decl) {
  return (var_loc_list *)htab_find_with_hash(decl_loc_table, decl,
                                             DECL_UID(decl));
}
...
static void add_var_loc_to_decl(tree decl, struct var_loc_node *loc) {
  unsigned int decl_id = DECL_UID(decl);
  var_loc_list *temp;
  void **slot;
...
static dw_die_ref push_new_compile_unit(dw_die_ref old_unit,
...
static dw_die_ref pop_compile_unit(dw_die_ref old_unit) {
...
static inline void loc_checksum(dw_loc_descr_ref loc, struct md5_ctx *ctx) {
...
static void attr_checksum(dw_attr_ref at, struct md5_ctx *ctx, int *mark) {
  dw_loc_descr_ref loc;
  rtx r;
...
static void die_checksum(dw_die_ref die, struct md5_ctx *ctx, int *mark) {
  dw_die_ref c;
  dw_attr_ref a;
  unsigned ix;
...
static inline int same_loc_p(dw_loc_descr_ref loc1, dw_loc_descr_ref loc2,
                             int *mark) {
...
static int same_dw_val_p(const dw_val_node *v1, const dw_val_node *v2,
                         int *mark) {
  ...
  switch (v1->val_class) {
    ...
    case dw_val_class_loc:
      for (loc1 = v1->v.val_loc, loc2 = v2->v.val_loc; loc1 && loc2;
...
static int same_attr_p(dw_attr_ref at1, dw_attr_ref at2, int *mark) {
...
static int same_die_p(dw_die_ref die1, dw_die_ref die2, int *mark) {
  dw_die_ref c1, c2;
  dw_attr_ref a1;
  unsigned ix;
...
static int same_die_p_wrap(dw_die_ref die1, dw_die_ref die2) {
  int mark = 0;
...
static void compute_section_prefix(dw_die_ref unit_die) {
...
static int is_type_die(dw_die_ref die) {
  switch (die->die_tag) {
    case DW_TAG_array_type:
    case DW_TAG_class_type:
    case DW_TAG_interface_type:
...
static int is_comdat_die(dw_die_ref c) {
  ...
  if (c->die_tag == DW_TAG_pointer_type ||
      c->die_tag == DW_TAG_reference_type || c->die_tag == DW_TAG_const_type ||
...
static int is_symbol_die(dw_die_ref c) {
  return (is_type_die(c) ||
          (get_AT(c, DW_AT_declaration) && !get_AT(c, DW_AT_specification)) ||
          c->die_tag == DW_TAG_namespace || c->die_tag == DW_TAG_module);
}
...
static void assign_symbol_names(dw_die_ref die) {
  dw_die_ref c;
  ...
  if (is_symbol_die(die)) {
    if (comdat_symbol_id) {
      ...
      sprintf(p, "%s.%s.%x", DIE_LABEL_PREFIX, comdat_symbol_id,
...
static hashval_t htab_cu_hash(const void *of) {
...
static int htab_cu_eq(const void *of1, const void *of2) {
...
static void htab_cu_del(void *what) {
...
/* Check whether we have already seen this CU and set up SYM_NUM
   accordingly.  */
static int check_duplicate_cu(dw_die_ref cu, htab_t htable,
  ...
  slot = (struct cu_hash_table_entry **)htab_find_slot_with_hash(
      htable, cu, htab_hash_string(cu->die_symbol), INSERT);
  entry = *slot;
...
static void record_comdat_symbol_number(dw_die_ref cu, htab_t htable,
                                        unsigned int sym_num) {
  struct cu_hash_table_entry **slot, *entry;

  slot = (struct cu_hash_table_entry **)htab_find_slot_with_hash(
      htable, cu, htab_hash_string(cu->die_symbol), NO_INSERT);
  entry = *slot;
...
static void break_out_includes(dw_die_ref die) {
  dw_die_ref c;
  ...
  for (node = limbo_die_list, pnode = &limbo_die_list; node;
       node = node->next) {
    int is_dupl;

    compute_section_prefix(node->die);
    is_dupl =
        check_duplicate_cu(node->die, cu_hash_table, &comdat_symbol_number);
...
static void build_abbrev_table(dw_die_ref die) {
  ...
  for (ix = 0; VEC_iterate(dw_attr_node, die->die_attr, ix, a); ix++)
    if (AT_class(a) == dw_val_class_die_ref && AT_ref(a)->die_mark == 0) {
  ...
  if (abbrev_id >= abbrev_die_table_in_use) {
    if (abbrev_die_table_in_use >= abbrev_die_table_allocated) {
      ...
      abbrev_die_table = GGC_RESIZEVEC(dw_die_ref, abbrev_die_table, n_alloc);
...
static void unmark_all_dies(dw_die_ref die) {
  dw_die_ref c;
  dw_attr_ref a;
  unsigned ix;
...
static unsigned long size_of_pubnames(VEC(pubname_entry, gc) * names) {
  unsigned long size;
  unsigned i;
  pubname_ref p;
  ...
  for (i = 0; VEC_iterate(pubname_entry, names, i, p); i++)
    if (names != pubtype_table || p->die->die_offset != 0 ||
...
static enum dwarf_form value_format(dw_attr_ref a) {
...
static void output_value_format(dw_attr_ref a) {
...
static inline void output_die_symbol(dw_die_ref die) {
...
static inline dw_loc_list_ref new_loc_list(dw_loc_descr_ref expr,
...
static inline void add_loc_descr_to_loc_list(dw_loc_list_ref *list_head,
                                             dw_loc_descr_ref descr,
                                             const char *begin, const char *end,
                                             const char *section) {
  dw_loc_list_ref *d;
...
static void output_loc_list(dw_loc_list_ref list_head) {
  ...
  dw2_asm_output_data(DWARF2_ADDR_SIZE, 0,
                      ...
                      list_head->ll_symbol);
  dw2_asm_output_data(DWARF2_ADDR_SIZE, 0, "Location list terminator end (%s)",
                      list_head->ll_symbol);
}
...
static void output_die(dw_die_ref die) {
  ...
  for (ix = 0; VEC_iterate(dw_attr_node, die->die_attr, ix, a); ix++) {
    ...
    switch (AT_class(a)) {
      ...
      case dw_val_class_unsigned_const:
        dw2_asm_output_data(constant_size(AT_unsigned(a)), AT_unsigned(a), "%s",
                            name);
        break;
      ...
      case dw_val_class_long_long: {
        ...
        dw2_asm_output_data(HOST_BITS_PER_LONG / HOST_BITS_PER_CHAR, first,
                            "long long constant");
        dw2_asm_output_data(HOST_BITS_PER_LONG / HOST_BITS_PER_CHAR, second,
                            NULL);
      } break;
      ...
      case dw_val_class_vec: {
        ...
        dw2_asm_output_data(constant_size(len * elt_size), len * elt_size, "%s",
                            name);
        if (elt_size > sizeof(HOST_WIDE_INT)) {
          elt_size /= 2;
          len *= 2;
        }
        for (i = 0, p = a->dw_attr_val.v.val_vec.array; i < len;
             i++, p += elt_size)
      ...
      case dw_val_class_loc_list: {
        ...
        gcc_assert(sym);
        dw2_asm_output_offset(DWARF_OFFSET_SIZE, sym, debug_loc_section, "%s",
                              name);
      } break;
      ...
      case dw_val_class_die_ref:
        if (AT_ref_external(a)) {
          ...
          gcc_assert(sym);
          dw2_asm_output_offset(DWARF2_ADDR_SIZE, sym, debug_info_section, "%s",
                                name);
        } else {
          gcc_assert(AT_ref(a)->die_offset);
          dw2_asm_output_data(DWARF_OFFSET_SIZE, AT_ref(a)->die_offset, "%s",
                              name);
        }
        break;
      ...
      case dw_val_class_fde_ref: {
        ...
        dw2_asm_output_offset(DWARF_OFFSET_SIZE, l1, debug_frame_section, "%s",
                              name);
      } break;
      ...
      case dw_val_class_lineptr:
        dw2_asm_output_offset(DWARF_OFFSET_SIZE, AT_lbl(a), debug_line_section,
                              "%s", name);
        break;
      ...
      case dw_val_class_str:
        if (AT_string_form(a) == DW_FORM_strp)
          dw2_asm_output_offset(
              DWARF_OFFSET_SIZE, a->dw_attr_val.v.val_str->label,
              debug_str_section, "%s: \"%s\"", name, AT_string(a));
        else
...
static void output_compilation_unit_header(void) {
  ...
  dw2_asm_output_offset(DWARF_OFFSET_SIZE, abbrev_section_label,
                        debug_abbrev_section, "Offset Into Abbrev. Section");
  dw2_asm_output_data(1, DWARF2_ADDR_SIZE, "Pointer Size (in bytes)");
}

/* Output the compilation unit DIE and its children.  */

static void output_comp_unit(dw_die_ref die, int output_if_empty) {
  const char *secname;
  char *oldsym, *tmp;
...
static const char *dwarf2_name(tree decl, int scope) {
  return lang_hooks.dwarf_name(decl, scope ? 1 : 0);
}

/* Add a new entry to .debug_pubnames if appropriate.  */

static void add_pubname_string(const char *str, dw_die_ref die) {
  pubname_entry e;
...
static void add_pubtype(tree decl, dw_die_ref die) {
  pubname_entry e;

  e.name = NULL;
  if ((TREE_PUBLIC(decl) || die->die_parent == comp_unit_die) &&
...
static void output_pubnames(VEC(pubname_entry, gc) * names) {
  unsigned i;
  ...
  dw2_asm_output_offset(DWARF_OFFSET_SIZE, debug_info_section_label,
                        debug_info_section, "Offset of Compilation Unit Info");
  ...
  for (i = 0; VEC_iterate(pubname_entry, names, i, pub); i++) {
    ...
    if (names != pubtype_table || pub->die->die_offset != 0 ||
        !flag_eliminate_unused_debug_types) {
...
static void add_arange(tree decl, dw_die_ref die) {
  ...
  if (arange_table_in_use == arange_table_allocated) {
    ...
    arange_table =
        GGC_RESIZEVEC(dw_die_ref, arange_table, arange_table_allocated);
...
static void output_aranges(void) {
  ...
  if (DWARF_ARANGES_PAD_SIZE) {
    /* Pad using a 2 byte words so that padding is correct for any
       pointer size.  */
    dw2_asm_output_data(2, 0, "Pad to %d byte boundary", 2 * DWARF2_ADDR_SIZE);
  ...
  if (text_section_used) {
    ...
    dw2_asm_output_delta(DWARF2_ADDR_SIZE, text_end_label, text_section_label,
                         "Length");
  }
  if (cold_text_section_used) {
    dw2_asm_output_addr(DWARF2_ADDR_SIZE, cold_text_section_label, "Address");
  ...
  for (i = 0; i < arange_table_in_use; i++) {
    ...
    if (die->die_tag == DW_TAG_subprogram) {
      dw2_asm_output_addr(DWARF2_ADDR_SIZE, get_AT_low_pc(die), "Address");
      ...
      dw2_asm_output_addr_rtx(DWARF2_ADDR_SIZE, loc->dw_loc_oprnd1.v.val_addr,
                              "Address");
      dw2_asm_output_data(DWARF2_ADDR_SIZE,
                          get_AT_unsigned(die, DW_AT_byte_size), "Length");
    }
  }
...
static unsigned int add_ranges_by_labels(const char *begin, const char *end) {
  ...
  if (in_use == ranges_by_label_allocated) {
    ...
    memset(ranges_by_label + ranges_by_label_in_use, 0,
           RANGES_TABLE_INCREMENT * sizeof(struct dw_ranges_by_label_struct));
  }
...
static void output_ranges(void) {
  ...
  for (i = 0; i < ranges_table_in_use; i++) {
    ...
    if (block_num > 0) {
      ...
      if (!have_multiple_function_sections) {
        dw2_asm_output_delta(DWARF2_ADDR_SIZE, blabel, text_section_label, fmt,
                             i * 2 * DWARF2_ADDR_SIZE);
        dw2_asm_output_delta(DWARF2_ADDR_SIZE, elabel, text_section_label,
                             NULL);
      }
      ...
      /* Otherwise, the compilation unit base address is zero,
        ...
        dw2_asm_output_addr(DWARF2_ADDR_SIZE, blabel, fmt,
                            i * 2 * DWARF2_ADDR_SIZE);
    ...
    /* Negative block_num stands for an index into ranges_by_label.  */
      ...
      if (!have_multiple_function_sections) {
        ...
        dw2_asm_output_addr(DWARF2_ADDR_SIZE, ranges_by_label[lab_idx].begin,
                            fmt, i * 2 * DWARF2_ADDR_SIZE);
        dw2_asm_output_addr(DWARF2_ADDR_SIZE, ranges_by_label[lab_idx].end,
                            NULL);
      }
    } else {
      dw2_asm_output_data(DWARF2_ADDR_SIZE, 0, NULL);
...
static int file_info_cmp(const void *p1, const void *p2) {
...
static int file_name_acquire(void **slot, void *data) {
...
static void output_file_names(void) {
  ...
  for (i = 1; i < numfiles; i++)
    if (files[i].fname - files[i].path == dirs[ndirs - 1].length &&
        memcmp(dirs[ndirs - 1].path, files[i].path, dirs[ndirs - 1].length) ==
            0) {
      ...
      dirs[ndirs].prefix = -1;
      for (j = 0; j < ndirs; j++)
        if (dirs[j].length < dirs[ndirs].length && dirs[j].length > 1 &&
            (dirs[ndirs].prefix == -1 ||
...
static void output_line_info(void) {
  ...
  if (DWARF_INITIAL_LENGTH_SIZE - DWARF_OFFSET_SIZE == 4)
    dw2_asm_output_data(
        4, 0xffffffff,
        "Initial length escape value indicating 64-bit DWARF extension");
  dw2_asm_output_delta(DWARF_OFFSET_SIZE, l2, l1, "Length of Source Line Info");
  ASM_OUTPUT_LABEL(asm_out_file, l1);
  ...
  dw2_asm_output_data(1, DWARF_LINE_BASE, "Line Base Value (Special Opcodes)");
  ...
  for (opc = 1; opc < DWARF_LINE_OPCODE_BASE; opc++) {
    ...
    dw2_asm_output_data(1, n_op_args, "opcode: 0x%x has %d args", opc,
                        n_op_args);
  }
    ...
    if (line_info->dw_line_num != current_line) {
      ...
      if (line_delta >= 0 && line_delta < (DWARF_LINE_RANGE - 1))
        /* This can handle deltas from -10 to 234, using the current
           ...
           takes 1 byte.  */
        dw2_asm_output_data(1, DWARF_LINE_OPCODE_BASE + line_delta, "line %lu",
                            current_line);
      else {
        /* This can handle any delta.  This takes at least 4 bytes,
           depending on the value being encoded.  */
        dw2_asm_output_data(1, DW_LNS_advance_line, "advance to line %lu",
                            current_line);
  ...
  if (0) {
    dw2_asm_output_data(1, DW_LNS_fixed_advance_pc, "DW_LNS_fixed_advance_pc");
...
static inline unsigned HOST_WIDE_INT simple_type_size_in_bits(const_tree type) {
...
static inline bool is_subrange_type(const_tree type) {
  ...
  if (TREE_CODE(subtype) != INTEGER_TYPE &&
      TREE_CODE(subtype) != ENUMERAL_TYPE && TREE_CODE(subtype) != BOOLEAN_TYPE)
    return false;

  if (TREE_CODE(type) == TREE_CODE(subtype) &&
      int_size_in_bytes(type) == int_size_in_bytes(subtype) &&
      TYPE_MIN_VALUE(type) != NULL && TYPE_MIN_VALUE(subtype) != NULL &&
      tree_int_cst_equal(TYPE_MIN_VALUE(type), TYPE_MIN_VALUE(subtype)) &&
      TYPE_MAX_VALUE(type) != NULL && TYPE_MAX_VALUE(subtype) != NULL &&
...
static dw_die_ref subrange_type_die(tree type, dw_die_ref context_die) {
...
static dw_die_ref modified_type_die(tree type, int is_const_type,
  ...
  /* See if we already have the appropriately qualified variant of
     this type.  */
  qualified_type =
      get_qualified_type(type, ((is_const_type ? TYPE_QUAL_CONST : 0) |
  ...
  if (name && TREE_CODE(name) == TYPE_DECL && DECL_ORIGINAL_TYPE(name)) {
    ...
    if (qualified_type == dtype) {
      ...
      return modified_type_die(DECL_ORIGINAL_TYPE(name), is_const_type,
  ...
  if (item_type)
    ...
    sub_die = modified_type_die(item_type, TYPE_READONLY(item_type),
...
static inline int type_is_enum(const_tree type) {
  return TREE_CODE(type) == ENUMERAL_TYPE;
}

/* Return the DBX register number described by a given RTL node.  */

static unsigned int dbx_reg_number(const_rtx rtl) {
...
static void add_loc_descr_op_piece(dw_loc_descr_ref *list_head, int size) {
  dw_loc_descr_ref loc;
...
static dw_loc_descr_ref one_reg_loc_descriptor(
    unsigned int regno, enum var_init_status initialized) {
...
static dw_loc_descr_ref multiple_reg_loc_descriptor(
    rtx rtl, rtx regs, enum var_init_status initialized) {
  int nregs, size, i;
  unsigned reg;
...
static dw_loc_descr_ref int_loc_descriptor(HOST_WIDE_INT i) {
...
static dw_loc_descr_ref based_loc_descr(rtx reg, HOST_WIDE_INT offset,
                                        enum var_init_status initialized) {
  unsigned int regno;
  ...
  if (reg == arg_pointer_rtx || reg == frame_pointer_rtx) {
    ...
    if (elim != reg) {
      ...
      gcc_assert((SUPPORTS_STACK_ALIGNMENT && (elim == hard_frame_pointer_rtx ||
      ...
      if (crtl->stack_realign_tried && cfa.reg == HARD_FRAME_POINTER_REGNUM &&
          reg == frame_pointer_rtx) {
        int base_reg = DWARF_FRAME_REGNUM(
  ...
  } else if (fde && fde->drap_reg != INVALID_REGNUM &&
             (fde->drap_reg == REGNO(reg) || fde->vdrap_reg == REGNO(reg))) {
...
static inline int is_based_loc(const_rtx rtl) {
  return (
      GET_CODE(rtl) == PLUS &&
...
static dw_loc_descr_ref concatn_mem_loc_descriptor(
    rtx concatn, enum machine_mode mode, enum var_init_status initialized) {
  unsigned int i;
...
static dw_loc_descr_ref tls_mem_loc_descriptor(rtx mem) {
  ...
  if (base == NULL || TREE_CODE(base) != VAR_DECL || !DECL_THREAD_LOCAL_P(base))
    return NULL;
  ...
  if (INTVAL(MEM_OFFSET(mem))) {
    if (INTVAL(MEM_OFFSET(mem)) >= 0)
      add_loc_descr(&loc_result, new_loc_descr(DW_OP_plus_uconst,
...
static dw_loc_descr_ref mem_loc_descriptor(rtx rtl, enum machine_mode mode,
                                           enum var_init_status initialized) {
  ...
  switch (GET_CODE(rtl)) {
    ...
    case REG:
      ...
      if (REGNO(rtl) < FIRST_PSEUDO_REGISTER)
      ...
      else if (stack_realign_drap && crtl->drap_reg &&
        ...
        mem_loc_result =
            based_loc_descr(crtl->drap_reg, 0, VAR_INIT_STATUS_INITIALIZED);
      }
      break;
    ...
    case PRE_DEC:
      /* Turn these into a PLUS expression and fall into the PLUS code
         below.  */
      rtl = gen_rtx_PLUS(
          word_mode, XEXP(rtl, 0),
          GEN_INT(GET_CODE(rtl) == PRE_INC ? GET_MODE_UNIT_SIZE(mode)
    ...
    plus:
      if (is_based_loc(rtl))
        mem_loc_result = based_loc_descr(XEXP(rtl, 0), INTVAL(XEXP(rtl, 1)),
                                         VAR_INIT_STATUS_INITIALIZED);
      else {
        mem_loc_result =
            mem_loc_descriptor(XEXP(rtl, 0), mode, VAR_INIT_STATUS_INITIALIZED);
        if (mem_loc_result == 0)
          break;

        if (GET_CODE(XEXP(rtl, 1)) == CONST_INT && INTVAL(XEXP(rtl, 1)) >= 0)
          add_loc_descr(
              &mem_loc_result,
              new_loc_descr(DW_OP_plus_uconst, INTVAL(XEXP(rtl, 1)), 0));
        else {
          dw_loc_descr_ref mem_loc_result2 = mem_loc_descriptor(
              XEXP(rtl, 1), mode, VAR_INIT_STATUS_INITIALIZED);
    ...
    do_binop : {
      ...
      dw_loc_descr_ref op1 =
          mem_loc_descriptor(XEXP(rtl, 1), mode, VAR_INIT_STATUS_INITIALIZED);
    ...
    case CONCATN:
      mem_loc_result =
          concatn_mem_loc_descriptor(rtl, mode, VAR_INIT_STATUS_INITIALIZED);
      break;
...
static dw_loc_descr_ref concat_loc_descriptor(
    rtx x0, rtx x1, enum var_init_status initialized) {
  dw_loc_descr_ref cc_loc_result = NULL;
...
static dw_loc_descr_ref concatn_loc_descriptor(
    rtx concatn, enum var_init_status initialized) {
  unsigned int i;
  dw_loc_descr_ref cc_loc_result = NULL;
  unsigned int n = XVECLEN(concatn, 0);
...
static dw_loc_descr_ref loc_descriptor(rtx rtl,
  ...
  switch (GET_CODE(rtl)) {
    ...
    case MEM:
      loc_result = mem_loc_descriptor(XEXP(rtl, 0), GET_MODE(rtl), initialized);
      if (loc_result == NULL)
    ...
    case CONCAT:
      loc_result =
          concat_loc_descriptor(XEXP(rtl, 0), XEXP(rtl, 1), initialized);
      break;
    ...
    case PARALLEL: {
      ...
      loc_result =
          loc_descriptor(XEXP(RTVEC_ELT(par_elems, 0), 0), initialized);
      if (loc_result == NULL)
        return NULL;
      ...
      for (i = 1; i < num_elem; i++) {
        ...
        temp = loc_descriptor(XEXP(RTVEC_ELT(par_elems, i), 0), initialized);
        if (temp == NULL)
          return NULL;
...
static dw_loc_descr_ref loc_descriptor_from_tree_1(tree loc, int want_address) {
  ...
  switch (TREE_CODE(loc)) {
    ...
    case ARRAY_RANGE_REF: {
      ...
      if (ret == 0 || bitpos % BITS_PER_UNIT != 0 ||
      ...
      if (TREE_CODE(TREE_OPERAND(loc, 1)) == INTEGER_CST &&
        ...
        add_loc_descr(&ret,
                      new_loc_descr(DW_OP_plus_uconst,
                                    tree_low_cst(TREE_OPERAND(loc, 1), 0), 0));
        break;
      }
    ...
    case MAX_EXPR: {
      ...
      loc = build3(COND_EXPR, TREE_TYPE(loc),
                   build2(code, integer_type_node, TREE_OPERAND(loc, 0),
                          TREE_OPERAND(loc, 1)),
...
static inline dw_loc_descr_ref loc_descriptor_from_tree(tree loc) {
...
static inline HOST_WIDE_INT ceiling(HOST_WIDE_INT value,
...
static inline tree field_type(const_tree decl) {
  tree type;
...
static inline unsigned simple_type_align_in_bits(const_tree type) {
  return (TREE_CODE(type) != ERROR_MARK) ? TYPE_ALIGN(type) : BITS_PER_WORD;
}

static inline unsigned simple_decl_align_in_bits(const_tree decl) {
  return (TREE_CODE(decl) != ERROR_MARK) ? DECL_ALIGN(decl) : BITS_PER_WORD;
}

/* Return the result of rounding T up to ALIGN.  */

static inline HOST_WIDE_INT round_up_to_align(HOST_WIDE_INT t,
                                              unsigned int align) {
...
static HOST_WIDE_INT field_byte_offset(const_tree decl) {
...
static inline void add_AT_location_description(dw_die_ref die,
                                               enum dwarf_attribute attr_kind,
                                               dw_loc_descr_ref descr) {
  if (descr != 0)
...
static void insert_int(HOST_WIDE_INT val, unsigned int size,
                       unsigned char *dest) {
  while (size != 0) {
    *dest++ = val & 0xff;
    val >>= 8;
...
static void insert_float(const_rtx rtl, unsigned char *array) {
  REAL_VALUE_TYPE rv;
  long val[4];
  int i;
    ...
    case CONST_DOUBLE:
         ...
         represented.  We output CONST_DOUBLEs as blocks.  */
      {
        ...
        if (SCALAR_FLOAT_MODE_P(mode)) {
          ...
          add_AT_long_long(die, DW_AT_const_value, CONST_DOUBLE_HIGH(rtl),
...
/* Determine whether the evaluation of EXPR references any variables
   ...
   output).  */
static tree reference_to_unused(tree *tp, int *walk_subtrees,
...
static rtx rtl_for_decl_init(tree init, tree type) {
  ...
  if (TREE_CODE(init) == STRING_CST && TREE_CODE(type) == ARRAY_TYPE) {
    ...
    if (GET_MODE_CLASS(mode) == MODE_INT && GET_MODE_SIZE(mode) == 1 &&
        domain && integer_zerop(TYPE_MIN_VALUE(domain)) &&
        compare_tree_int(TYPE_MAX_VALUE(domain),
                         TREE_STRING_LENGTH(init) - 1) == 0 &&
        ((size_t)TREE_STRING_LENGTH(init) ==
         strlen(TREE_STRING_POINTER(init)) + 1))
      rtl =
          gen_rtx_CONST_STRING(VOIDmode, ggc_strdup(TREE_STRING_POINTER(init)));
  }
...
static rtx rtl_for_decl_location(tree decl) {
  rtx rtl;
  ...
  if (!reload_completed) {
    if (rtl &&
        (CONSTANT_P(rtl) || (MEM_P(rtl) && CONSTANT_P(XEXP(rtl, 0))) ||
         (REG_P(rtl) && TREE_CODE(decl) == VAR_DECL && TREE_STATIC(decl)))) {
      rtl = targetm.delegitimize_address(rtl);
      return rtl;
    }
    rtl = NULL_RTX;
    ...
    if (rtl == NULL_RTX || is_pseudo_reg(rtl)) {
      ...
      if (dmode == pmode)
        ...
        if (REG_P(inc))
          ...
          if (BYTES_BIG_ENDIAN)
            rtl = adjust_address_nv(
                inc, dmode, GET_MODE_SIZE(pmode) - GET_MODE_SIZE(dmode));
          else
            rtl = inc;
        }
      }
    ...
    /* ??? This is the same series of checks that are made in dbxout.c before
       ...
       thing to do.  */
    else if (MEM_P(rtl) && XEXP(rtl, 0) != const0_rtx &&
             ...
             (GET_MODE_SIZE(TYPE_MODE(TREE_TYPE(decl))) < UNITS_PER_WORD)) {
      int offset = (UNITS_PER_WORD - GET_MODE_SIZE(TYPE_MODE(TREE_TYPE(decl))));

      rtl = gen_rtx_MEM(TYPE_MODE(TREE_TYPE(decl)),
                        plus_constant(XEXP(rtl, 0), offset));
    }
  } else if (TREE_CODE(decl) == VAR_DECL && rtl && MEM_P(rtl) &&
             GET_MODE(rtl) != TYPE_MODE(TREE_TYPE(decl)) && BYTES_BIG_ENDIAN) {
...
static tree fortran_common(tree decl, HOST_WIDE_INT *value) {
  ...
  /* If the decl isn't a VAR_DECL, or if it isn't public or static, or if
     ...
     be handled as such.  */
  if (TREE_CODE(decl) != VAR_DECL || !TREE_PUBLIC(decl) || !TREE_STATIC(decl) ||
      !DECL_HAS_VALUE_EXPR_P(decl) || !is_fortran())
    return NULL_TREE;
  ...
  if (cvar == NULL_TREE || TREE_CODE(cvar) != VAR_DECL ||
      DECL_ARTIFICIAL(cvar) || !TREE_PUBLIC(cvar))
    return NULL_TREE;
...
static dw_loc_descr_ref loc_by_reference(dw_loc_descr_ref loc, tree decl) {
  HOST_WIDE_INT size;
  enum dwarf_location_atom op;
  ...
  if (loc_list && loc_list->first != loc_list->last) {
    ...
    for (; node->next; node = node->next)
      if (NOTE_VAR_LOCATION_LOC(node->var_loc_note) != NULL_RTX) {
        ...
        varloc = NOTE_VAR_LOCATION(node->var_loc_note);
        descr = loc_by_reference(loc_descriptor(varloc, initialized), decl);
        add_loc_descr_to_loc_list(&list, descr, node->label, node->next->label,
                                  secname);
      }
...
static void defer_location(tree variable, dw_die_ref die) {
...
static bool native_encode_initializer(tree init, unsigned char *array,
                                      int size) {
  tree type;
    ...
    case STRING_CST:
      ...
      if (TREE_CODE(type) == ARRAY_TYPE) {
        ...
        if (size > TREE_STRING_LENGTH(init)) {
          memcpy(array, TREE_STRING_POINTER(init), TREE_STRING_LENGTH(init));
          memset(array + TREE_STRING_LENGTH(init), '\0',
    ...
    case CONSTRUCTOR:
      ...
      if (TREE_CODE(type) == ARRAY_TYPE) {
        ...
        for (cnt = 0;
          ...
          if (val && !native_encode_initializer(val, array + pos, fieldsize))
            return false;
        }
        return true;
      }
...
static void tree_add_const_value_attribute(dw_die_ref var_die, tree decl) {
  tree init;
...
static dw_loc_list_ref convert_cfa_to_fb_loc_list(HOST_WIDE_INT offset) {
    ...
    switch (cfi->dw_cfi_opc) {
      ...
      case DW_CFA_advance_loc4:
        if (!cfa_equal_p(&last_cfa, &next_cfa)) {
          *list_tail =
              new_loc_list(build_cfa_loc(&last_cfa, offset), start_label,
  ...
  if (!cfa_equal_p(&last_cfa, &next_cfa)) {
    *list_tail = new_loc_list(build_cfa_loc(&last_cfa, offset), start_label,
                              last_label, section, list == NULL);
    list_tail = &(*list_tail)->dw_loc_next;
    start_label = last_label;
  }
  *list_tail = new_loc_list(build_cfa_loc(&next_cfa, offset), start_label,
...
static void compute_frame_pointer_to_fb_displacement(HOST_WIDE_INT offset) {
  rtx reg, elim;
...
static void add_name_attribute(dw_die_ref die, const char *name_string) {
...
static void add_comp_dir_attribute(dw_die_ref die) {
...
static void add_bound_info(dw_die_ref subrange_die,
                           enum dwarf_attribute bound_attr, tree bound) {
...
static inline void add_bit_size_attribute(dw_die_ref die, tree decl) {
  /* Must be a field and a bit field.  */
  gcc_assert(TREE_CODE(decl) == FIELD_DECL && DECL_BIT_FIELD_TYPE(decl));
...
static inline dw_die_ref add_abstract_origin_attribute(dw_die_ref die,
                                                       tree origin) {
...
static inline void add_pure_or_virtual_attribute(dw_die_ref die,
                                                 tree func_decl) {
    ...
    if (host_integerp(DECL_VINDEX(func_decl), 0))
      add_AT_loc(die, DW_AT_vtable_elem_location,
                 new_loc_descr(DW_OP_constu,
                               tree_low_cst(DECL_VINDEX(func_decl), 0), 0));
...
static void add_name_and_src_coords_attributes(dw_die_ref die, tree decl) {
  ...
  if (decl_name != NULL && IDENTIFIER_POINTER(decl_name) != NULL) {
    ...
    if ((TREE_CODE(decl) == FUNCTION_DECL || TREE_CODE(decl) == VAR_DECL) &&
        TREE_PUBLIC(decl) && DECL_ASSEMBLER_NAME(decl) != DECL_NAME(decl) &&
        !DECL_ABSTRACT(decl) &&
        !(TREE_CODE(decl) == VAR_DECL && DECL_REGISTER(decl)) && !is_fortran())
  ...
  if (TREE_CODE(decl) == FUNCTION_DECL && TREE_ASM_WRITTEN(decl)) {
    add_AT_addr(die, DW_AT_VMS_rtnbeg_pd_address, XEXP(DECL_RTL(decl), 0));
...
static inline int local_scope_p(dw_die_ref context_die) {
...
static inline int class_or_namespace_scope_p(dw_die_ref context_die) {
  return (context_die && (context_die->die_tag == DW_TAG_structure_type ||
...
static void add_type_attribute(dw_die_ref object_die, tree type, int decl_const,
  ...
  type_die =
      modified_type_die(type, decl_const || TYPE_READONLY(type),
                        decl_volatile || TYPE_VOLATILE(type), context_die);
...
/* Given an object die, add the calling convention attribute for the
   function call type.  */
static void add_calling_convention_attribute(dw_die_ref subr_die, tree decl) {
...
static inline tree member_declared_type(const_tree member) {
  return (DECL_BIT_FIELD_TYPE(member) ? DECL_BIT_FIELD_TYPE(member)
                                      : TREE_TYPE(member));
}
...
static void gen_array_type_die(tree type, dw_die_ref context_die) {
  ...
  if (TYPE_STRING_FLAG(type) && TREE_CODE(type) == ARRAY_TYPE && is_fortran() &&
  ...
  if (is_fortran() && TREE_CODE(type) == ARRAY_TYPE &&
...
static dw_loc_descr_ref descr_info_loc(tree val, tree base_decl) {
  HOST_WIDE_INT size;
  ...
  CASE_CONVERT:
    ...
    case PLUS_EXPR:
      if (host_integerp(TREE_OPERAND(val, 1), 1) &&
        ...
        add_loc_descr(&loc,
                      new_loc_descr(DW_OP_plus_uconst,
                                    tree_low_cst(TREE_OPERAND(val, 1), 1), 0));
      } else {
        op = DW_OP_plus;
      do_binop:
        loc = descr_info_loc(TREE_OPERAND(val, 0), base_decl);
...
static void add_descr_info_field(dw_die_ref die, enum dwarf_attribute attr,
...
static void gen_descr_array_type_die(tree type, struct array_descr_info *info,
                                     dw_die_ref context_die) {
  dw_die_ref scope_die = scope_die_for(type, context_die);
  dw_die_ref array_die;
  int dim;
  ...
  /* For Fortran multidimensional arrays use DW_ORD_col_major ordering.  */
  if (is_fortran() && info->ndimensions >= 2)
    add_AT_unsigned(array_die, DW_AT_ordering, DW_ORD_col_major);
  ...
  for (dim = 0; dim < info->ndimensions; dim++) {
    ...
    if (info->dimen[dim].lower_bound) {
      ...
      if ((is_c_family() || is_java()) &&
        ...
        ;
      else if (is_fortran() && integer_onep(info->dimen[dim].lower_bound))
        ;
      else
        add_descr_info_field(subrange_die, DW_AT_lower_bound,
                             info->dimen[dim].lower_bound, info->base_decl);
    }
    if (info->dimen[dim].upper_bound)
      add_descr_info_field(subrange_die, DW_AT_upper_bound,
                           info->dimen[dim].upper_bound, info->base_decl);
    if (info->dimen[dim].stride)
      add_descr_info_field(subrange_die, DW_AT_byte_stride,
                           info->dimen[dim].stride, info->base_decl);
  }
...
static dw_die_ref gen_enumeration_type_die(tree type, dw_die_ref context_die) {
  ...
  if (TYPE_SIZE(type)) {
    ...
    for (link = TYPE_VALUES(type); link != NULL; link = TREE_CHAIN(link)) {
...
static dw_die_ref gen_formal_parameter_die(tree node, tree origin,
                                           dw_die_ref context_die) {
    ...
    case tcc_declaration:
      ...
      if (origin != NULL)
        ...
        if (DECL_BY_REFERENCE(node))
          add_type_attribute(parm_die, TREE_TYPE(type), 0, 0, context_die);
        else
          add_type_attribute(parm_die, type, TREE_READONLY(node),
...
static void gen_formal_types_die(tree function_or_method_type,
  ...
  for (link = TYPE_ARG_TYPES(function_or_method_type); link && TREE_VALUE(link);
       link = TREE_CHAIN(link))
...
static void gen_type_die_for_member(tree type, tree member,
                                    dw_die_ref context_die) {
  gen_type_die(type, context_die);
...
static int premark_used_types_helper(void **slot, void *data ATTRIBUTE_UNUSED) {
  tree type;
  dw_die_ref die;
...
static void gen_subprogram_die(tree decl, dw_die_ref context_die) {
  ...
  if (!declaration && !origin && !old_die && DECL_CONTEXT(decl) &&
      TYPE_P(DECL_CONTEXT(decl)) && !class_or_namespace_scope_p(context_die) &&
  ...
  if (origin != NULL) {
    ...
    if ((old_die->die_parent == comp_unit_die || context_die == NULL) &&
        (DECL_ARTIFICIAL(decl) ||
         (get_AT_file(old_die, DW_AT_decl_file) == file_index &&
          (get_AT_unsigned(old_die, DW_AT_decl_line) == (unsigned)s.line)))) {
      subr_die = old_die;
    ...
    if (debug_info_level > DINFO_LEVEL_TERSE) {
      ...
      add_type_attribute(subr_die, TREE_TYPE(TREE_TYPE(decl)), 0, 0,
                         context_die);
    }
  ...
  if (declaration) {
    ...
    } else { /* Do nothing for now; maybe need to duplicate die, one for
...
static hashval_t common_block_die_table_hash(const void *x) {
...
static int common_block_die_table_eq(const void *x, const void *y) {
  const_dw_die_ref d = (const_dw_die_ref)x;
...
static void gen_variable_die(tree decl, tree origin, dw_die_ref context_die) {
  HOST_WIDE_INT off;
  tree com_decl;
  ...
  int declaration =
      (DECL_EXTERNAL(decl_or_origin)
       ...
       || (TREE_CODE(decl_or_origin) == VAR_DECL &&
           DECL_COMDAT(decl_or_origin) && !TREE_ASM_WRITTEN(decl_or_origin)) ||
       class_or_namespace_scope_p(context_die));
  ...
  if (com_decl) {
      ...
      if (get_AT(var_die, DW_AT_location) == NULL) {
        ...
        if (loc) {
          if (off) {
            ...
            if (loc->dw_loc_opc == DW_OP_addr && loc->dw_loc_next == NULL &&
    ...
    if (common_block_die_table == NULL)
      common_block_die_table = htab_create_ggc(10, common_block_die_table_hash,
  ...
  /* If the compiler emitted a definition for the DECL declaration
     ...
     DIE for it again.  */
  if (old_die && declaration && old_die->die_parent == context_die)
    return;
  ...
  if (!declaration &&
      (!DECL_ABSTRACT(decl_or_origin)
       /* Local static vars are shared between all clones/inlines,
          ...
          already set.  */
       || (TREE_CODE(decl_or_origin) == VAR_DECL &&
           TREE_STATIC(decl_or_origin) && DECL_RTL_SET_P(decl_or_origin)))
...
static void gen_const_die(tree decl, dw_die_ref context_die) {
...
static inline void add_call_src_coords_attributes(tree stmt, dw_die_ref die) {
...
static inline void add_high_low_attributes(tree stmt, dw_die_ref die) {
  char label[MAX_ARTIFICIAL_LABEL_BYTES];
  ...
  if (BLOCK_FRAGMENT_CHAIN(stmt)) {
    ...
    if (inlined_function_outer_scope_p(stmt)) {
      ASM_GENERATE_INTERNAL_LABEL(label, BLOCK_BEGIN_LABEL, BLOCK_NUMBER(stmt));
    ...
    ASM_GENERATE_INTERNAL_LABEL(label, BLOCK_END_LABEL, BLOCK_NUMBER(stmt));
...
static void gen_lexical_block_die(tree stmt, dw_die_ref context_die,
                                  int depth) {
...
static void gen_inlined_subroutine_die(tree stmt, dw_die_ref context_die,
                                       int depth) {
...
static void gen_inheritance_die(tree binfo, tree access,
                                dw_die_ref context_die) {
...
static void gen_member_die(tree type, dw_die_ref context_die) {
  ...
  if (binfo) {
    ...
    for (i = 0; BINFO_BASE_ITERATE(binfo, i, base); i++)
      gen_inheritance_die(
          base, (accesses ? VEC_index(tree, accesses, i) : access_public_node),
          context_die);
  }
...
static void gen_struct_or_union_type_die(tree type, dw_die_ref context_die,
  ...
  int nested = 0;
  int complete =
      (TYPE_SIZE(type) && (!TYPE_STUB_DECL(type) ||
  ...
  /* First occurrence of type or toplevel definition of nested class.  */
  {
    ...
    type_die = new_die(TREE_CODE(type) == RECORD_TYPE ? record_type_tag(type)
                                                      : DW_TAG_union_type,
                       scope_die, type);
    equate_type_number_to_die(type, type_die);
    if (old_die)
  ...
  if (complete && !ns_decl) {
    ...
    if (TYPE_STUB_DECL(type) && !decl_function_context(TYPE_STUB_DECL(type)))
...
static void gen_subroutine_type_die(tree type, dw_die_ref context_die) {
  ...
  dw_die_ref subr_die =
      new_die(DW_TAG_subroutine_type, scope_die_for(type, context_die), type);
...
static void gen_type_die_with_usage(tree type, dw_die_ref context_die,
                                    enum debug_info_usage usage) {
  int need_pop;
  ...
  if (!TREE_ASM_WRITTEN(type) && lang_hooks.types.get_array_descr_info &&
...
static void gen_block_die(tree stmt, dw_die_ref context_die, int depth) {
  ...
  if (inlined_func)
    ...
    if (debug_info_level > DINFO_LEVEL_TERSE)
      ...
      must_output_die =
          ((BLOCK_VARS(stmt) != NULL || BLOCK_NUM_NONLOCALIZED_VARS(stmt)) &&
           (TREE_USED(stmt) || TREE_ASM_WRITTEN(stmt) || BLOCK_ABSTRACT(stmt)));
    else if ((TREE_USED(stmt) || TREE_ASM_WRITTEN(stmt) ||
              BLOCK_ABSTRACT(stmt)) &&
...
static void process_scope_var(tree stmt, tree decl, tree origin,
                              dw_die_ref context_die) {
  dw_die_ref die;
  tree decl_or_origin = decl ? decl : origin;
  ...
  if (die != NULL && die->die_parent == NULL)
    ...
    dwarf2out_imported_module_or_decl_1(
        decl_or_origin, DECL_NAME(decl_or_origin), stmt, context_die);
  else
...
static void decls_for_scope(tree stmt, dw_die_ref context_die, int depth) {
  ...
  for (i = 0; i < BLOCK_NUM_NONLOCALIZED_VARS(stmt); i++)
    process_scope_var(stmt, NULL, BLOCK_NONLOCALIZED_VAR(stmt, i), context_die);
  ...
  for (subblocks = BLOCK_SUBBLOCKS(stmt); subblocks != NULL;
...
static inline int is_redundant_typedef(const_tree decl) {
  if (TYPE_DECL_IS_STUB(decl))
    return 1;

  if (DECL_ARTIFICIAL(decl) && DECL_CONTEXT(decl) &&
...
static inline dw_die_ref get_context_die(tree context) {
  if (context) {
...
static dw_die_ref force_type_die(tree type) {
  ...
  if (!type_die) {
    ...
    type_die = modified_type_die(type, TYPE_READONLY(type), TYPE_VOLATILE(type),
                                 context_die);
    gcc_assert(type_die);
  }
  return type_die;
...
static dw_die_ref setup_namespace_context(tree thing, dw_die_ref context_die) {
...
static dw_die_ref declare_in_namespace(tree thing, dw_die_ref context_die) {
...
static void gen_namespace_die(tree decl, dw_die_ref context_die) {
  ...
  if (DECL_ABSTRACT_ORIGIN(decl) == NULL) {
    ...
    namespace_die = new_die(is_fortran() ? DW_TAG_module : DW_TAG_namespace,
                            context_die, decl);
    ...
    dw_die_ref origin_die = force_decl_die(DECL_ABSTRACT_ORIGIN(decl));
...
static void gen_decl_die(tree decl, tree origin, dw_die_ref context_die) {
  tree decl_or_origin = decl ? decl : origin;
  ...
  switch (TREE_CODE(decl_or_origin)) {
	...
	/* This is only a declaration.  */;
      ...
      if (origin || DECL_ORIGIN(decl) != decl)
        dwarf2out_abstract_function(origin ? origin
...
static void dwarf2out_global_decl(tree decl) {
...
static void dwarf2out_type_decl(tree decl, int local) {
  if (!local)
    dwarf2out_decl(decl);
}
...
static void dwarf2out_imported_module_or_decl_1(tree decl, tree name,
                                                tree lexical_block,
  ...
  if (TREE_CODE(decl) == TYPE_DECL || TREE_CODE(decl) == CONST_DECL) {
    ...
    if (!at_import_die) {
      ...
      if (TREE_CODE(decl) == FIELD_DECL) {
        ...
        if (TYPE_CONTEXT(type) && TYPE_P(TYPE_CONTEXT(type)) &&
            !should_emit_struct_debug(TYPE_CONTEXT(type), DINFO_USAGE_DIR_USE))
          return;
  ...
  if (TREE_CODE(decl) == NAMESPACE_DECL)
    imported_die =
        new_die(DW_TAG_imported_module, lexical_block_die, lexical_block);
  else
    imported_die =
        new_die(DW_TAG_imported_declaration, lexical_block_die, lexical_block);
...
static void dwarf2out_imported_module_or_decl(tree decl, tree name,
    ...
    case TYPE_DECL:
      ...
      if (DECL_IS_BUILTIN(decl)) {
        /* OK, we need to generate one for `bool' so GDB knows what type
           comparisons have.  */
        if (is_cxx() && TREE_CODE(TREE_TYPE(decl)) == BOOLEAN_TYPE &&
            !DECL_IGNORED_P(decl))
...
static void dwarf2out_begin_block(unsigned int line ATTRIBUTE_UNUSED,
...
static void dwarf2out_end_block(unsigned int line ATTRIBUTE_UNUSED,
                                unsigned int blocknum) {
  switch_to_section(current_function_section());
...
static int file_table_eq(const void *p1_p, const void *p2_p) {
  const struct dwarf_file_data *const p1 = (const struct dwarf_file_data *)p1_p;
  const char *const p2 = (const char *)p2_p;
  return strcmp(p1->filename, p2) == 0;
}

static hashval_t file_table_hash(const void *p_p) {
...
static void dwarf2out_var_location(rtx loc_note) {
  ...
  /* If the insn we processed last time is the previous insn
     ...
     last time.  */
  if (last_insn != NULL_RTX && last_insn == prev_insn && NOTE_P(prev_insn) &&
...
static void dwarf2out_source_line(unsigned int line, const char *filename) {
    ...
    if (DWARF2_ASM_LINE_DEBUG_INFO) {
      ...
      if (separate_line_info_table_in_use ==
        ...
        separate_line_info_table =
            GGC_RESIZEVEC(dw_separate_line_info_entry, separate_line_info_table,
                          separate_line_info_table_allocated);
        memset(
            separate_line_info_table + separate_line_info_table_in_use, 0,
            (LINE_INFO_TABLE_INCREMENT * sizeof(dw_separate_line_info_entry)));
      }
...
static void dwarf2out_end_source_file(unsigned int lineno ATTRIBUTE_UNUSED) {
  if (flag_eliminate_dwarf2_dups)
...
static void dwarf2out_define(unsigned int lineno ATTRIBUTE_UNUSED,
...
static void dwarf2out_undef(unsigned int lineno ATTRIBUTE_UNUSED,
                            const char *buffer ATTRIBUTE_UNUSED) {
  if (debug_info_level >= DINFO_LEVEL_VERBOSE) {
    switch_to_section(debug_macinfo_section);
...
static void dwarf2out_init(const char *filename ATTRIBUTE_UNUSED) {
  /* Allocate the file_table.  */
  file_table = htab_create_ggc(50, file_table_hash, file_table_eq, NULL);

  /* Allocate the decl_die_table.  */
  decl_die_table =
      htab_create_ggc(10, decl_die_table_hash, decl_die_table_eq, NULL);

  /* Allocate the decl_loc_table.  */
  decl_loc_table =
      htab_create_ggc(10, decl_loc_table_hash, decl_loc_table_eq, NULL);
  ...
  debug_info_section = get_section(DEBUG_INFO_SECTION, SECTION_DEBUG, NULL);
  debug_abbrev_section = get_section(DEBUG_ABBREV_SECTION, SECTION_DEBUG, NULL);
  debug_aranges_section =
      get_section(DEBUG_ARANGES_SECTION, SECTION_DEBUG, NULL);
  debug_macinfo_section =
      get_section(DEBUG_MACINFO_SECTION, SECTION_DEBUG, NULL);
  debug_line_section = get_section(DEBUG_LINE_SECTION, SECTION_DEBUG, NULL);
  debug_loc_section = get_section(DEBUG_LOC_SECTION, SECTION_DEBUG, NULL);
  debug_pubnames_section =
      get_section(DEBUG_PUBNAMES_SECTION, SECTION_DEBUG, NULL);
#ifdef DEBUG_PUBTYPES_SECTION
  debug_pubtypes_section =
      get_section(DEBUG_PUBTYPES_SECTION, SECTION_DEBUG, NULL);
#endif
  debug_str_section =
      get_section(DEBUG_STR_SECTION, DEBUG_STR_SECTION_FLAGS, NULL);
  debug_ranges_section = get_section(DEBUG_RANGES_SECTION, SECTION_DEBUG, NULL);
  debug_frame_section = get_section(DEBUG_FRAME_SECTION, SECTION_DEBUG, NULL);

  ASM_GENERATE_INTERNAL_LABEL(text_end_label, TEXT_END_LABEL, 0);
  ASM_GENERATE_INTERNAL_LABEL(abbrev_section_label, DEBUG_ABBREV_SECTION_LABEL,
                              0);
  ASM_GENERATE_INTERNAL_LABEL(text_section_label, TEXT_SECTION_LABEL, 0);
  ASM_GENERATE_INTERNAL_LABEL(cold_text_section_label, COLD_TEXT_SECTION_LABEL,
                              0);
  ...
  ASM_GENERATE_INTERNAL_LABEL(ranges_section_label, DEBUG_RANGES_SECTION_LABEL,
                              0);
...
static int output_indirect_string(void **h, void *v ATTRIBUTE_UNUSED) {
...
static void verify_marks_clear(dw_die_ref die) {
  dw_die_ref c;
...
static void prune_unused_types_walk_attribs(dw_die_ref die) {
  dw_attr_ref a;
  unsigned ix;
...
static void prune_unused_types_mark(dw_die_ref die, int dokids) {
  dw_die_ref c;
...
static void prune_unused_types_walk_local_classes(dw_die_ref die) {
  ...
  switch (die->die_tag) {
    ...
    case DW_TAG_subprogram:
      if (!get_AT_flag(die, DW_AT_declaration) || die->die_definition != NULL)
...
static void prune_unused_types_update_strings(dw_die_ref die) {
  ...
  for (ix = 0; VEC_iterate(dw_attr_node, die->die_attr, ix, a); ix++)
    if (AT_class(a) == dw_val_class_str) {
      ...
      if (s->refcount == ((DEBUG_STR_SECTION_FLAGS & SECTION_MERGE) ? 1 : 2)) {
        void **slot;
        slot = htab_find_slot_with_hash(debug_str_hash, s->str,
                                        htab_hash_string(s->str), INSERT);
...
static void prune_unused_types_prune(dw_die_ref die) {
  dw_die_ref c;
...
/* Set the parameter to true if there are any relative pathnames in
   the file table.  */
static int file_table_relative_p(void **slot, void *param) {
  bool *p = (bool *)param;
  struct dwarf_file_data *d = (struct dwarf_file_data *)*slot;
...
static void dwarf2out_finish(const char *filename) {
  ...
  if (!have_multiple_function_sections) {
    ...
    add_AT_range_list(comp_unit_die, DW_AT_ranges,
                      add_ranges_by_labels(text_section_label, text_end_label));
    if (flag_reorder_blocks_and_partition)
      add_ranges_by_labels(cold_text_section_label, cold_end_label);
  ...
  if (debug_info_level >= DINFO_LEVEL_NORMAL)
    add_AT_lineptr(comp_unit_die, DW_AT_stmt_list, debug_line_section_label);
