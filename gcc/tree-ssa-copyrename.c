...
static bool copy_rename_partition_coalesce(var_map map, tree var1, tree var2,
                                           FILE *debug) {
  int p1, p2, p3;
  tree root1, root2;
  tree rep1, rep2;
  ...
  if (ann1->symbol_mem_tag && ann2->symbol_mem_tag &&
  ...
  if (POINTER_TYPE_P(TREE_TYPE(root1)) && POINTER_TYPE_P(TREE_TYPE(root2)) &&
...
static unsigned int rename_ssa_copies(void) {
  var_map map;
  basic_block bb;
