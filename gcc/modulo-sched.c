...
#define PS_STAGE_COUNT(ps) \
  ((PS_MAX_CYCLE(ps) - PS_MIN_CYCLE(ps) + 1 + (ps)->ii - 1) / (ps)->ii)
...
struct ps_insn {
  ...
  ps_insn_ptr next_in_row, prev_in_row;
...
static partial_schedule_ptr create_partial_schedule(int ii, ddg_ptr,
                                                    int history);
...
static void generate_prolog_epilog(partial_schedule_ptr, struct loop *, rtx,
                                   rtx);
static void duplicate_insns_of_cycles(partial_schedule_ptr, int, int, int, rtx);

#define SCHED_ASAP(x) (((node_sched_params_ptr)(x)->aux.info)->asap)
#define SCHED_TIME(x) (((node_sched_params_ptr)(x)->aux.info)->time)
#define SCHED_FIRST_REG_MOVE(x) \
  (((node_sched_params_ptr)(x)->aux.info)->first_reg_move)
#define SCHED_NREG_MOVES(x) (((node_sched_params_ptr)(x)->aux.info)->nreg_moves)
...
static const char *sms_print_insn(const_rtx insn,
...
static void compute_jump_reg_dependencies(rtx insn ATTRIBUTE_UNUSED,
...
static struct haifa_sched_info sms_sched_info = {
    NULL, NULL, NULL, NULL, NULL, sms_print_insn, NULL, NULL,
    NULL, NULL, NULL, 0,    0,
...
static rtx doloop_register_get(rtx head ATTRIBUTE_UNUSED,
  ...
  first_insn_not_to_check =
      (GET_CODE(PATTERN(tail)) == PARALLEL ? tail : PREV_INSN(tail));
...
static rtx const_iteration_count(rtx count_reg, basic_block pre_header,
...
static void set_node_sched_params(ddg_ptr g) {
  int i;

  /* Allocate for each node in the DDG a place to hold the "sched_data".  */
  /* Initialize ASAP/ALAP/HIGHT to zero.  */
  node_sched_params = (node_sched_params_ptr)xcalloc(
      g->num_nodes, sizeof(struct node_sched_params));
...
static void print_node_sched_params(FILE *file, int num_nodes, ddg_ptr g) {
  int i;
...
/*
   ...
   nreg_moves = ----------------------------------- + 1 - {   dependence.
                            ii                          { 1 if not.
*/
static struct undo_replace_buff_elem *generate_reg_moves(
  ...
  for (i = 0; i < g->num_nodes; i++) {
    ...
    for (i_reg_move = 0; i_reg_move < nreg_moves; i_reg_move++) {
      ...
      EXECUTE_IF_SET_IN_SBITMAP(uses_of_defs[i_reg_move], 0, i_use, sbi) {
        ...
        rep = (struct undo_replace_buff_elem *)xcalloc(
            1, sizeof(struct undo_replace_buff_elem));
...
static void free_undo_replace_buff(
...
static void normalize_sched_times(partial_schedule_ptr ps) {
  int row;
    ...
    for (crr_insn = ps->rows[row]; crr_insn; crr_insn = crr_insn->next_in_row) {
      ...
      if (dump_file)
        fprintf(dump_file,
		   ...
		   min_cycle=%d\n",
                crr_insn->node->cuid, SCHED_TIME(u), ps->min_cycle);
...
static void set_columns_for_ps(partial_schedule_ptr ps) {
  int row;
...
static void duplicate_insns_of_cycles(partial_schedule_ptr ps, int from_stage,
  ...
  for (row = 0; row < ps->ii; row++)
    for (ps_ij = ps->rows[row]; ps_ij; ps_ij = ps_ij->next_in_row) {
      ...
      } else /* It's for the epilog.  */
      {
        ...
        i_reg_moves =
            SCHED_NREG_MOVES(u_node) - (from_stage - SCHED_STAGE(u_node) - 1);
        i_reg_moves = MAX(i_reg_moves, 0);
        i_reg_moves = MIN(i_reg_moves, SCHED_NREG_MOVES(u_node));
      ...
      if (SCHED_STAGE(u_node) >= from_stage && SCHED_STAGE(u_node) <= to_stage)
        duplicate_insn_chain(u_node->first_note, u_node->insn);
    }
}

/* Generate the instructions (including reg_moves) for prolog & epilog.  */
static void generate_prolog_epilog(partial_schedule_ptr ps, struct loop *loop,
  ...
  if (!count_init) {
    ...
    sub_reg =
        expand_simple_binop(GET_MODE(count_reg), MINUS, count_reg,
                            GEN_INT(last_stage), count_reg, 1, OPTAB_DIRECT);
...
/* Return true if all the BBs of the loop are empty except the
   loop header.  */
static bool loop_single_full_bb_p(struct loop *loop) {
  unsigned i;
  ...
  for (i = 0; i < loop->num_nodes; i++) {
    ...
    for (; head != NEXT_INSN(tail); head = NEXT_INSN(head)) {
      if (NOTE_P(head) || LABEL_P(head) || (INSN_P(head) && JUMP_P(head)))
        continue;
      empty_bb = false;
      break;
    }
...
#define SIMPLE_SMS_LOOP_P(loop)                                      \
  ((loop->num_nodes < 3) && (EDGE_COUNT(loop->latch->preds) == 1) && \
...
static bool loop_canon_p(struct loop *loop) {
...
static void canon_loop(struct loop *loop) {
  edge e;
  edge_iterator i;
...
/* Setup infos.  */
static void setup_sched_infos(void) {
...
static void sms_schedule(void) {
  ...
  FOR_EACH_LOOP(li, loop, 0) {
    ...
    if (latch_edge->count &&
        (latch_edge->count <
         single_exit(loop)->count * SMS_LOOP_AVERAGE_COUNT_THRESHOLD)) {
      if (dump_file) {
        fprintf(dump_file, " %s %d (file, line)\n", insn_file(tail),
                insn_line(tail));
    ...
    for (insn = head; insn != NEXT_INSN(tail); insn = NEXT_INSN(insn)) {
      ...
      if (CALL_P(insn) || BARRIER_P(insn) ||
          (INSN_P(insn) && !JUMP_P(insn) && !single_set(insn) &&
    ...
    if (insn != NEXT_INSN(tail)) {
      if (dump_file) {
        ...
        else if ((INSN_P(insn) && !JUMP_P(insn) && !single_set(insn) &&
                  GET_CODE(PATTERN(insn)) != USE))
  ...
  FOR_EACH_LOOP(li, loop, 0) {
    ...
    if (stage_count < 1 || (count_init && (loop_count <= stage_count)) ||
      ...
      /* Set the stage boundaries.  If the DDG is built with
         closing_branch_deps, the closing_branch was scheduled and should appear
         in the last (ii-1) row.  Otherwise, we are free to schedule the branch,
         and we let nodes that were scheduled at the first PS_MIN_CYCLE cycle
         appear in the first row; this should reduce stage_count to minimum.
      ...
      if (count_reg && !count_init) {
        rtx comp_rtx =
            gen_rtx_fmt_ee(GT, VOIDmode, count_reg, GEN_INT(stage_count));
        unsigned prob = (PROB_SMS_ENOUGH_ITERATIONS * REG_BR_PROB_BASE) / 100;

        loop_version(loop, comp_rtx, &condition_bb, prob, prob,
                     REG_BR_PROB_BASE - prob, true);
      }

      /* Set new iteration count of loop kernel.  */
      if (count_reg && count_init)
        SET_SRC(single_set(count_init)) = GEN_INT(loop_count - stage_count + 1);
...
/* The SMS scheduling algorithm itself
   ...
   'calculate_register_pressure(PS)'
                             Given a schedule of instructions, calculate the
   register pressure it implies.  One implementation could be the maximum number
   of overlapping live ranges. 'maxRP' The maximum allowed register pressure, it
   is usually derived from the number registers available in the hardware.

   1. II = MII.
   2. PS = empty list
   3. for each node u in O in pre-computed order
   4.   if (PSP(u) != Q && PSS(u) == Q) then
   5.     Early_start(u) = max ( t(v) + l(v) - d(v,u)*II ) over all every v in
   PSP(u).
   6.     start = Early_start; end = Early_start + II - 1; step = 1
   11.  else if (PSP(u) == Q && PSS(u) != Q) then
   12.      Late_start(u) = min ( t(v) - l(v) + d(v,u)*II ) over all every v in
   PSS(u).
   13.     start = Late_start; end = Late_start - II + 1; step = -1
   14.  else if (PSP(u) != Q && PSS(u) != Q) then
   15.     Early_start(u) = max ( t(v) + l(v) - d(v,u)*II ) over all every v in
   PSP(u).
   16.     Late_start(u) = min ( t(v) - l(v) + d(v,u)*II ) over all every v in
   PSS(u).
...
static int get_sched_window(partial_schedule_ptr ps, int *nodes_order, int i,
                            sbitmap sched_nodes, int ii, int *start_p,
                            int *step_p, int *end_p) {
  int start, step, end;
  ddg_edge_ptr e;
  int u = nodes_order[i];
  ...
  if (psp_not_empty && !pss_not_empty) {
    ...
    for (e = u_node->in; e != 0; e = e->next_in) {
      ...
      if (dump_file) {
        ...
        fprintf(dump_file,
                ...
                " checking p %d (%d): ",
                u_node->cuid, INSN_UID(u_node->insn), v_node->cuid,
                INSN_UID(v_node->insn));
      }

      if (TEST_BIT(sched_nodes, v_node->cuid)) {
        int p_st = SCHED_TIME(v_node);

        early_start = MAX(early_start, p_st + e->latency - (e->distance * ii));

        if (dump_file)
          fprintf(dump_file, "pred st = %d; early_start = %d; latency: %d",
    ...
    for (e = u_node->out; e != 0; e = e->next_out) {
      ...
      if (TEST_BIT(sched_nodes, v_node->cuid)) {
        ...
        late_start = MIN(late_start, s_st - e->latency + (e->distance * ii));

        if (dump_file)
          fprintf(dump_file, "succ st = %d; late_start = %d; latency = %d",
...
static void calculate_must_precede_follow(ddg_node_ptr u_node, int start,
...
static bool try_scheduling_node_in_cycle(partial_schedule_ptr ps,
                                         ddg_node_ptr u_node, int u, int cycle,
                                         sbitmap sched_nodes, int *num_splits,
                                         sbitmap must_precede,
                                         sbitmap must_follow) {
  ps_insn_ptr psi;
  bool success = 0;
  ...
  psi =
      ps_add_node_check_conflicts(ps, u_node, cycle, must_precede, must_follow);
  if (psi) {
...
/* This function implements the scheduling algorithm for SMS according to the
   above algorithm.  */
static partial_schedule_ptr sms_schedule_by_order(ddg_ptr g, int mii, int maxii,
                                                  int *nodes_order) {
  int ii = mii;
  ...
  while (flush_and_start_over && (ii < maxii)) {
    ...
    for (i = 0; i < num_nodes; i++) {
      ...
      success = 0;
      if (get_sched_window(ps, nodes_order, i, sched_nodes, ii, &start, &step,
                           &end) == 0) {
        if (dump_file)
          fprintf(dump_file,
                  "\nTrying to schedule node %d \
                        INSN = %d  in (%d .. %d) step %d\n",
                  u, (INSN_UID(g->nodes[u].insn)), start, end, step);

        gcc_assert((step > 0 && start < end) || (step < 0 && start > end));

        calculate_must_precede_follow(u_node, start, end, step, ii, sched_nodes,
        ...
        for (c = start; c != end; c += step) {
          ...
          success = try_scheduling_node_in_cycle(ps, u_node, u, c, sched_nodes,
      ...
      if (!success) {
        ...
        if (step == 1)
          split_row =
              compute_split_row(sched_nodes, start, end, ps->ii, u_node);
        else
          split_row =
              compute_split_row(sched_nodes, end, start, ps->ii, u_node);
...
static void ps_insert_empty_row(partial_schedule_ptr ps, int split_row,
                                sbitmap sched_nodes) {
  ps_insn_ptr crr_insn;
  ...
  for (row = 0; row < split_row; row++) {
    ...
    ps->rows[row] = NULL;
    for (crr_insn = rows_new[row]; crr_insn; crr_insn = crr_insn->next_in_row) {
      ddg_node_ptr u = crr_insn->node;
  ...
  for (row = split_row; row < ii; row++) {
    ...
    ps->rows[row] = NULL;
    for (crr_insn = rows_new[row + 1]; crr_insn;
...
static int compute_split_row(sbitmap sched_nodes, int low, int up, int ii,
                             ddg_node_ptr u_node) {
  ddg_edge_ptr e;
...
static void verify_partial_schedule(partial_schedule_ptr ps,
                                    sbitmap sched_nodes) {
  int row;
  ps_insn_ptr crr_insn;
...
static void check_nodes_order(int *node_order, int num_nodes) {
  int i;
...
static int sms_order_nodes(ddg_ptr g, int mii, int *node_order,
                           int *pmax_asap) {
  int i;
  int rec_mii = 0;
...
static void order_nodes_of_sccs(ddg_all_sccs_ptr all_sccs, int *node_order) {
  int i, pos = 0;
...
static struct node_order_params *calculate_order_params(
    ddg_ptr g, int mii ATTRIBUTE_UNUSED, int *pmax_asap) {
  int u;
  int max_asap;
  int num_nodes = g->num_nodes;
  ddg_edge_ptr e;
  ...
  node_order_params_arr =
      (nopa)xcalloc(num_nodes, sizeof(struct node_order_params));
  ...
  for (u = 0; u < num_nodes; u++) {
    ...
    for (e = u_node->in; e; e = e->next_in)
      if (e->distance == 0)
        ASAP(u_node) = MAX(ASAP(u_node), ASAP(e->src) + e->latency);
  ...
  for (u = num_nodes - 1; u > -1; u--) {
    ...
    for (e = u_node->out; e; e = e->next_out)
      if (e->distance == 0) {
        ALAP(u_node) = MIN(ALAP(u_node), ALAP(e->dest) - e->latency);
        HEIGHT(u_node) = MAX(HEIGHT(u_node), HEIGHT(e->dest) + e->latency);
      }
  }
  if (dump_file) {
    fprintf(dump_file, "\nOrder params\n");
...
static int find_max_asap(ddg_ptr g, sbitmap nodes) {
  unsigned int u = 0;
  int max_asap = -1;
  int result = -1;
  sbitmap_iterator sbi;
...
static int find_max_hv_min_mob(ddg_ptr g, sbitmap nodes) {
  unsigned int u = 0;
  int max_hv = -1;
  int min_mob = INT_MAX;
  int result = -1;
  ...
  EXECUTE_IF_SET_IN_SBITMAP(nodes, 0, u, sbi) {
    ...
    if (max_hv < HEIGHT(u_node)) {
      ...
      min_mob = MOB(u_node);
      result = u;
    } else if ((max_hv == HEIGHT(u_node)) && (min_mob > MOB(u_node))) {
      min_mob = MOB(u_node);
      result = u;
    }
  }
...
static int find_max_dv_min_mob(ddg_ptr g, sbitmap nodes) {
  unsigned int u = 0;
  int max_dv = -1;
  int min_mob = INT_MAX;
  int result = -1;
  ...
  EXECUTE_IF_SET_IN_SBITMAP(nodes, 0, u, sbi) {
    ...
    if (max_dv < DEPTH(u_node)) {
      ...
      min_mob = MOB(u_node);
      result = u;
    } else if ((max_dv == DEPTH(u_node)) && (min_mob > MOB(u_node))) {
      min_mob = MOB(u_node);
      result = u;
    }
  }
...
static int order_nodes_in_scc(ddg_ptr g, sbitmap nodes_ordered, sbitmap scc,
...
static partial_schedule_ptr create_partial_schedule(int ii, ddg_ptr g,
                                                    int history) {
...
static void free_ps_insns(partial_schedule_ptr ps) {
  int i;
...
static void free_partial_schedule(partial_schedule_ptr ps) {
  if (!ps)
    return;
  free_ps_insns(ps);
  free(ps->rows);
...
static void reset_partial_schedule(partial_schedule_ptr ps, int new_ii) {
  if (!ps)
    return;
  free_ps_insns(ps);
  if (new_ii == ps->ii)
    return;
  ps->rows = (ps_insn_ptr *)xrealloc(ps->rows, new_ii * sizeof(ps_insn_ptr));
...
void print_partial_schedule(partial_schedule_ptr ps, FILE *dump) {
  int i;
  ...
  for (i = 0; i < ps->ii; i++) {
    ...
    fprintf(dump, "\n[ROW %d ]: ", i);
    while (ps_i) {
      fprintf(dump, "%d, ", INSN_UID(ps_i->node->insn));
...
static ps_insn_ptr create_ps_insn(ddg_node_ptr node, int rest_count,
                                  int cycle) {
...
static bool remove_node_from_ps(partial_schedule_ptr ps, ps_insn_ptr ps_i) {
  int row;
...
static bool ps_insn_find_column(partial_schedule_ptr ps, ps_insn_ptr ps_i,
  ...
  for (next_ps_i = ps->rows[row]; next_ps_i;
...
static int ps_insn_advance_column(partial_schedule_ptr ps, ps_insn_ptr ps_i,
                                  sbitmap must_follow) {
...
/* Inserts a DDG_NODE to the given partial schedule at the given cycle.
   ...
   in the same cycle.  */
static ps_insn_ptr add_node_to_ps(partial_schedule_ptr ps, ddg_node_ptr node,
                                  int cycle, sbitmap must_precede,
                                  sbitmap must_follow) {
  ps_insn_ptr ps_i;
  int rest_count = 1;
  int row = SMODULO(cycle, ps->ii);

  if (ps->rows[row] && ps->rows[row]->row_rest_count >= issue_rate)
    return NULL;
...
static void advance_one_cycle(void) {
  ...
  if (targetm.sched.dfa_post_cycle_insn)
    state_transition(curr_state, targetm.sched.dfa_post_cycle_insn());
}
...
static int ps_has_conflicts(partial_schedule_ptr ps, int from, int to) {
  int cycle;
  ...
  for (cycle = from; cycle <= to; cycle++) {
    ...
    for (crr_insn = ps->rows[SMODULO(cycle, ps->ii)]; crr_insn;
         crr_insn = crr_insn->next_in_row) {
...
/* Checks if the given node causes resource conflicts when added to PS at
   ...
   PS_I when scheduled in the same cycle.  */
ps_insn_ptr ps_add_node_check_conflicts(partial_schedule_ptr ps, ddg_node_ptr n,
  ...
  has_conflicts = ps_has_conflicts(ps, c, c) ||
                  (ps->history > 0 &&
                   ps_has_conflicts(ps, c - ps->history, c + ps->history));
...
void rotate_partial_schedule(partial_schedule_ptr ps, int start_cycle) {
...
static bool gate_handle_sms(void) {
  return (optimize > 0 && flag_modulo_sched);
}

/* Run instruction scheduler.  */
/* Perform SMS module scheduling.  */
static unsigned int rest_of_handle_sms(void) {
#ifdef INSN_SCHEDULING
  basic_block bb;
