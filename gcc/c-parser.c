...
static void c_lex_one_token(c_parser *parser, c_token *token) {
  timevar_push(TV_LEX);

  token->type = c_lex_with_flags(
      &token->value, &token->location, NULL,
      (parser->lex_untranslated_string ? C_LEX_STRING_NO_TRANSLATE : 0));
    ...
    case CPP_NAME: {
      ...
      if (C_IS_RESERVED_WORD(token->value)) {
        ...
        if (rid_code == RID_CXX_COMPAT_WARN) {
          warning_at(token->location, OPT_Wc___compat,
...
static inline c_token *c_parser_peek_token(c_parser *parser) {
...
static inline bool c_parser_next_token_is(c_parser *parser,
                                          enum cpp_ttype type) {
...
static inline bool c_parser_next_token_is_not(c_parser *parser,
                                              enum cpp_ttype type) {
...
static inline bool c_parser_next_token_is_keyword(c_parser *parser,
                                                  enum rid keyword) {
...
/* Return true if TOKEN can start a type name,
   false otherwise.  */
static bool c_token_starts_typename(c_token *token) {
  switch (token->type) {
    case CPP_NAME:
...
/* Return true if the next token from PARSER can start a type name,
   false otherwise.  */
static inline bool c_parser_next_token_starts_typename(c_parser *parser) {
...
static inline bool c_parser_next_token_starts_declspecs(c_parser *parser) {
  c_token *token = c_parser_peek_token(parser);
...
static void c_parser_consume_token(c_parser *parser) {
  gcc_assert(parser->tokens_avail >= 1);
  gcc_assert(parser->tokens[0].type != CPP_EOF);
...
static void c_parser_error(c_parser *parser, const char *gmsgid) {
  c_token *token = c_parser_peek_token(parser);
  if (parser->error)
    return;
  parser->error = true;
...
static bool c_parser_require(c_parser *parser, enum cpp_ttype type,
                             const char *msgid) {
...
static bool c_parser_require_keyword(c_parser *parser, enum rid keyword,
                                     const char *msgid) {
...
static void c_parser_skip_until_found(c_parser *parser, enum cpp_ttype type,
                                      const char *msgid) {
  ...
  while (true) {
    ...
    if (token->type == CPP_PRAGMA_EOL && parser->in_pragma)
      return;
    if (token->type == CPP_OPEN_BRACE || token->type == CPP_OPEN_PAREN ||
        token->type == CPP_OPEN_SQUARE)
      ++nesting_depth;
    else if (token->type == CPP_CLOSE_BRACE || token->type == CPP_CLOSE_PAREN ||
...
static void c_parser_skip_to_end_of_parameter(c_parser *parser) {
  unsigned nesting_depth = 0;
...
static void c_parser_skip_to_pragma_eol(c_parser *parser) {
  gcc_assert(parser->in_pragma);
...
static inline int disable_extension_diagnostics(void) {
  int ret = (pedantic | (warn_pointer_arith << 1) | (warn_traditional << 2) |
             (flag_iso << 3) | (warn_long_long << 4) |
...
static struct c_declarator *c_parser_direct_declarator(c_parser *, bool,
                                                       c_dtr_syn, bool *);
static struct c_declarator *c_parser_direct_declarator_inner(
    c_parser *, bool, struct c_declarator *);
...
static void c_parser_declaration_or_fndef(c_parser *parser, bool fndef_ok,
  ...
  while (true) {
    ...
    declarator =
        c_parser_declarator(parser, specs->type_seen_p, C_DTR_NORMAL, &dummy);
    if (declarator == NULL) {
      c_parser_skip_to_end_of_block_or_statement(parser);
      return;
    }
...
static void c_parser_asm_definition(c_parser *parser) {
...
static void c_parser_declspecs(c_parser *parser, struct c_declspecs *specs,
                               bool scspec_ok, bool typespec_ok,
                               bool start_attr_ok) {
  ...
  while (c_parser_next_token_is(parser, CPP_NAME) ||
    ...
    if (c_parser_next_token_is(parser, CPP_NAME)) {
      ...
      if (kind == C_ID_TYPENAME &&
          (!c_dialect_objc() || c_parser_next_token_is_not(parser, CPP_LESS))) {
        t.kind = ctsk_typedef;
...
static struct c_typespec c_parser_enum_specifier(c_parser *parser) {
  ...
  if (c_parser_next_token_is(parser, CPP_OPEN_BRACE)) {
    ...
    postfix_attrs = c_parser_attributes(parser);
    ret.spec =
        finish_enum(type, nreverse(values), chainon(attrs, postfix_attrs));
...
static struct c_typespec c_parser_struct_or_union_specifier(c_parser *parser) {
  ...
  end_at_defs:
    ...
    postfix_attrs = c_parser_attributes(parser);
    ret.spec =
        finish_struct(type, nreverse(contents), chainon(attrs, postfix_attrs));
    ret.kind = ctsk_tagdef;
    return ret;
  } else if (!ident) {
    c_parser_error(parser, "expected %<{%>");
...
static tree c_parser_struct_declaration(c_parser *parser) {
  ...
  if (c_parser_next_token_is(parser, CPP_SEMICOLON)) {
    ...
    if (!specs->type_seen_p) {
      ...
      ret = grokfield(c_parser_peek_token(parser)->location,
                      build_id_declarator(NULL_TREE), specs, NULL_TREE, &attrs);
      if (ret)
  ...
  while (true) {
    ...
    if (c_parser_next_token_is(parser, CPP_COLON) ||
      ...
      if (c_parser_next_token_is_keyword(parser, RID_ATTRIBUTE))
        postfix_attrs = c_parser_attributes(parser);
      d = grokfield(c_parser_peek_token(parser)->location, declarator, specs,
                    width, &all_prefix_attrs);
...
static struct c_typespec c_parser_typeof_specifier(c_parser *parser) {
  struct c_typespec ret;
...
static struct c_declarator *c_parser_declarator(c_parser *parser,
                                                bool type_seen_p,
...
static struct c_declarator *c_parser_direct_declarator(c_parser *parser,
  ...
  if (kind != C_DTR_ABSTRACT && c_parser_next_token_is(parser, CPP_NAME) &&
      ((type_seen_p && c_parser_peek_token(parser)->id_kind == C_ID_TYPENAME) ||
  ...
  if (c_parser_next_token_is(parser, CPP_OPEN_PAREN)) {
    ...
    if (kind != C_DTR_NORMAL &&
      ...
      struct c_arg_info *args =
          c_parser_parms_declarator(parser, kind == C_DTR_NORMAL, attrs);
      if (args == NULL)
        return NULL;
      else {
        inner = build_function_declarator(args, build_id_declarator(NULL_TREE));
        return c_parser_direct_declarator_inner(parser, *seen_id, inner);
      }
    }
...
static struct c_declarator *c_parser_direct_declarator_inner(
    c_parser *parser, bool id_present, struct c_declarator *inner) {
  ...
  if (c_parser_next_token_is(parser, CPP_OPEN_SQUARE)) {
    ...
    declarator =
        build_array_declarator(dimen, quals_attrs, static_seen, star_seen);
    if (declarator == NULL)
      return NULL;
...
static struct c_arg_info *c_parser_parms_declarator(c_parser *parser,
                                                    bool id_list_ok,
                                                    tree attrs) {
  push_scope();
  declare_parm_level();
...
static struct c_arg_info *c_parser_parms_list_declarator(c_parser *parser,
                                                         tree attrs) {
...
static struct c_parm *c_parser_parameter_declaration(c_parser *parser,
                                                     tree attrs) {
  struct c_declspecs *specs;
  struct c_declarator *declarator;
  tree prefix_attrs;
  ...
  specs->attrs = NULL_TREE;
  declarator =
      c_parser_declarator(parser, specs->type_seen_p, C_DTR_PARM, &dummy);
  if (declarator == NULL) {
  ...
  if (c_parser_next_token_is_keyword(parser, RID_ATTRIBUTE))
    postfix_attrs = c_parser_attributes(parser);
  return build_c_parm(specs, chainon(postfix_attrs, prefix_attrs), declarator);
}
...
static tree c_parser_asm_string_literal(c_parser *parser) {
  tree str;
...
static tree c_parser_simple_asm_expr(c_parser *parser) {
  tree str;
...
static tree c_parser_attributes(c_parser *parser) {
    ...
    while (c_parser_next_token_is(parser, CPP_COMMA) ||
      ...
      if (c_parser_next_token_is(parser, CPP_NAME) &&
        ...
        if (c_parser_next_token_is(parser, CPP_CLOSE_PAREN))
          ...
          c_parser_consume_token(parser);
          attr_args =
              tree_cons(NULL_TREE, arg1, c_parser_expr_list(parser, false));
        }
      } else {
        if (c_parser_next_token_is(parser, CPP_CLOSE_PAREN))
          attr_args = NULL_TREE;
...
static struct c_type_name *c_parser_type_name(c_parser *parser) {
  ...
  finish_declspecs(specs);
  declarator =
      c_parser_declarator(parser, specs->type_seen_p, C_DTR_ABSTRACT, &dummy);
  if (declarator == NULL)
    return NULL;
...
static struct c_expr c_parser_initializer(c_parser *parser) {
  if (c_parser_next_token_is(parser, CPP_OPEN_BRACE))
...
static struct c_expr c_parser_braced_init(c_parser *parser, tree type,
                                          bool nested_p) {
...
static void c_parser_initelt(c_parser *parser) {
    ...
    while (c_parser_next_token_is(parser, CPP_OPEN_SQUARE) ||
      ...
      if (c_parser_next_token_is(parser, CPP_DOT)) {
        ...
        parse_message_args:
          ...
          c_parser_skip_until_found(parser, CPP_CLOSE_SQUARE, "expected %<]%>");
          mexpr.value = objc_build_message_expr(build_tree_list(rec, args));
...
static void c_parser_initval(c_parser *parser, struct c_expr *after) {
  struct c_expr init;
  ...
  if (c_parser_next_token_is(parser, CPP_OPEN_BRACE) && !after)
    ...
    if (init.value != NULL_TREE && TREE_CODE(init.value) != STRING_CST &&
...
static tree c_parser_compound_statement(c_parser *parser) {
  tree stmt;
...
static void c_parser_compound_statement_nostart(c_parser *parser) {
  ...
  while (c_parser_next_token_is_not(parser, CPP_CLOSE_BRACE)) {
    ...
    } else if (!last_label && c_parser_next_token_starts_declspecs(parser)) {
      last_label = false;
...
static void c_parser_label(c_parser *parser) {
...
static void c_parser_statement(c_parser *parser) {
...
static void c_parser_statement_after_labels(c_parser *parser) {
  location_t loc = c_parser_peek_token(parser)->location;
  tree stmt = NULL_TREE;
      ...
      switch (c_parser_peek_token(parser)->keyword) {
        ...
        case RID_THROW:
          ...
          if (c_parser_next_token_is(parser, CPP_SEMICOLON)) {
            ...
            stmt = objc_build_throw_stmt(c_parser_expression(parser).value);
            goto expect_semicolon;
          }
          break;
        case RID_TRY:
...
static tree c_parser_condition(c_parser *parser) {
  location_t loc;
  tree cond;
...
/* Parse a parenthesized condition from an if, do or while statement.
   ...
   condition:
     ( expression )
*/
static tree c_parser_paren_condition(c_parser *parser) {
  tree cond;
  if (!c_parser_require(parser, CPP_OPEN_PAREN, "expected %<(%>"))
    return error_mark_node;
...
static tree c_parser_c99_block_statement(c_parser *parser) {
...
static tree c_parser_if_body(c_parser *parser, bool *if_p) {
  tree block = c_begin_compound_stmt(flag_isoc99);
  while (c_parser_next_token_is_keyword(parser, RID_CASE) ||
         c_parser_next_token_is_keyword(parser, RID_DEFAULT) ||
         (c_parser_next_token_is(parser, CPP_NAME) &&
...
static tree c_parser_else_body(c_parser *parser) {
  tree block = c_begin_compound_stmt(flag_isoc99);
  while (c_parser_next_token_is_keyword(parser, RID_CASE) ||
         c_parser_next_token_is_keyword(parser, RID_DEFAULT) ||
         (c_parser_next_token_is(parser, CPP_NAME) &&
    ...
    c_parser_label(parser);
  if (c_parser_next_token_is(parser, CPP_SEMICOLON)) {
    warning_at(c_parser_peek_token(parser)->location, OPT_Wempty_body,
...
static void c_parser_if_statement(c_parser *parser) {
  tree block;
  location_t loc;
  tree cond;
  bool first_if = false;
...
static void c_parser_switch_statement(c_parser *parser) {
...
static void c_parser_while_statement(c_parser *parser) {
...
static void c_parser_do_statement(c_parser *parser) {
...
static void c_parser_for_statement(c_parser *parser) {
...
static tree c_parser_asm_statement(c_parser *parser) {
...
done_asm:
  ...
  if (!c_parser_require(parser, CPP_SEMICOLON, "expected %<;%>"))
    c_parser_skip_to_end_of_block_or_statement(parser);
  ret = build_asm_stmt(quals,
                       build_asm_expr(str, outputs, inputs, clobbers, simple));
  return ret;
}
...
static tree c_parser_asm_operands(c_parser *parser, bool convert_p) {
  tree list = NULL_TREE;
  while (true) {
    tree name, str;
    struct c_expr expr;
    if (c_parser_next_token_is(parser, CPP_OPEN_SQUARE)) {
      ...
      if (c_parser_next_token_is(parser, CPP_NAME)) {
        tree id = c_parser_peek_token(parser)->value;
        c_parser_consume_token(parser);
        name = build_string(IDENTIFIER_LENGTH(id), IDENTIFIER_POINTER(id));
      } else {
        c_parser_error(parser, "expected identifier");
    ...
    list =
        chainon(list, build_tree_list(build_tree_list(name, str), expr.value));
    if (c_parser_next_token_is(parser, CPP_COMMA))
      c_parser_consume_token(parser);
    else
      break;
...
static tree c_parser_asm_clobbers(c_parser *parser) {
  tree list = NULL_TREE;
  while (true) {
...
static struct c_expr c_parser_expr_no_commas(c_parser *parser,
                                             struct c_expr *after) {
...
static struct c_expr c_parser_conditional_expression(c_parser *parser,
                                                     struct c_expr *after) {
...
static struct c_expr c_parser_binary_expression(c_parser *parser,
                                                struct c_expr *after) {
...
#define POP                                                                   \
  do {                                                                        \
    ...
    stack[sp].expr = default_function_array_conversion(stack[sp].expr);       \
    stack[sp - 1].expr = parser_build_binary_op(                              \
        binary_loc, stack[sp].op, stack[sp - 1].expr, stack[sp].expr);        \
    sp--;                                                                     \
  } while (0)
  gcc_assert(!after || c_dialect_objc());
...
static struct c_expr c_parser_cast_expression(c_parser *parser,
                                              struct c_expr *after) {
  gcc_assert(!after || c_dialect_objc());
  if (after)
  ...
  switch (c_parser_peek_token(parser)->type) {
    ...
    case CPP_AND:
      ...
      return parser_build_unary_op(ADDR_EXPR,
                                   c_parser_cast_expression(parser, NULL), loc);
    case CPP_MULT:
      c_parser_consume_token(parser);
      op = c_parser_cast_expression(parser, NULL);
      op = default_function_array_conversion(op);
    ...
    case CPP_PLUS:
      if (!c_dialect_objc() && !in_system_header)
        warning_at(c_parser_peek_token(parser)->location, OPT_Wtraditional,
    ...
    case CPP_AND_AND:
      ...
      if (c_parser_next_token_is(parser, CPP_NAME)) {
        ret.value =
            finish_label_address_expr(c_parser_peek_token(parser)->value, loc);
        c_parser_consume_token(parser);
      } else {
        c_parser_error(parser, "expected identifier");
        ret.value = error_mark_node;
...
static struct c_expr c_parser_sizeof_expression(c_parser *parser) {
  ...
  if (c_parser_next_token_is(parser, CPP_OPEN_PAREN) &&
    ...
    if (c_parser_next_token_is(parser, CPP_OPEN_BRACE)) {
      expr = c_parser_postfix_expression_after_paren_type(parser, type_name);
      goto sizeof_expr;
    }
    /* sizeof ( type-name ).  */
    skip_evaluation--;
  ...
  switch (c_parser_peek_token(parser)->type) {
    ...
    case CPP_NAME:
      ...
      }
      {
        ...
        expr.value = build_external_ref(
            id, (c_parser_peek_token(parser)->type == CPP_OPEN_PAREN), loc);
        expr.original_code = ERROR_MARK;
      }
      break;
    case CPP_OPEN_PAREN:
      ...
      switch (c_parser_peek_token(parser)->keyword) {
            ...
            if (c_parser_next_token_is(parser, CPP_NAME)) {
              ...
              while (c_parser_next_token_is(parser, CPP_DOT) ||
                ...
                if (c_parser_next_token_is(parser, CPP_DEREF)) {
                  loc = c_parser_peek_token(parser)->location;
                  offsetof_ref =
                      build_array_ref(offsetof_ref, integer_zero_node, loc);
                  goto do_dot;
        ...
        case RID_TYPES_COMPATIBLE_P:
          ...
          c_parser_skip_until_found(parser, CPP_CLOSE_PAREN, "expected %<)%>");
          {
            ...
            expr.value = comptypes(e1, e2) ? build_int_cst(NULL_TREE, 1)
    ...
    case CPP_OPEN_SQUARE:
      if (c_dialect_objc()) {
        ...
        args = c_parser_objc_message_args(parser);
        c_parser_skip_until_found(parser, CPP_CLOSE_SQUARE, "expected %<]%>");
        expr.value = objc_build_message_expr(build_tree_list(receiver, args));
        expr.original_code = ERROR_MARK;
        break;
      }
...
static struct c_expr c_parser_postfix_expression_after_paren_type(
    c_parser *parser, struct c_type_name *type_name) {
  tree type;
  struct c_expr init;
  struct c_expr expr;
...
static struct c_expr c_parser_postfix_expression_after_primary(
    c_parser *parser, struct c_expr expr) {
  ...
  while (true) {
    switch (c_parser_peek_token(parser)->type) {
      ...
      case CPP_DEREF:
        ...
        c_parser_consume_token(parser);
        expr.value = build_component_ref(
            build_indirect_ref(loc, expr.value, "->"), ident);
        expr.original_code = ERROR_MARK;
        break;
      case CPP_PLUS_PLUS:
        /* Postincrement.  */
        c_parser_consume_token(parser);
        expr = default_function_array_conversion(expr);
        expr.value = build_unary_op(loc, POSTINCREMENT_EXPR, expr.value, 0);
        expr.original_code = ERROR_MARK;
        break;
      case CPP_MINUS_MINUS:
        /* Postdecrement.  */
        c_parser_consume_token(parser);
        expr = default_function_array_conversion(expr);
        expr.value = build_unary_op(loc, POSTDECREMENT_EXPR, expr.value, 0);
        expr.original_code = ERROR_MARK;
        break;
      default:
        return expr;
...
static tree c_parser_expr_list(c_parser *parser, bool convert_p) {
  struct c_expr expr;
  tree ret, cur;
  expr = c_parser_expr_no_commas(parser, NULL);
  if (convert_p)
...
static void c_parser_objc_class_definition(c_parser *parser) {
  bool iface_p;
  tree id1;
  tree superclass;
...
static void c_parser_objc_class_instance_variables(c_parser *parser) {
  gcc_assert(c_parser_next_token_is(parser, CPP_OPEN_BRACE));
  c_parser_consume_token(parser);
  while (c_parser_next_token_is_not(parser, CPP_EOF)) {
    tree decls;
...
static void c_parser_objc_class_declaration(c_parser *parser) {
  tree list = NULL_TREE;
...
static void c_parser_objc_alias_declaration(c_parser *parser) {
  tree id1, id2;
...
static void c_parser_objc_protocol_definition(c_parser *parser) {
...
static enum tree_code c_parser_objc_method_type(c_parser *parser) {
  switch (c_parser_peek_token(parser)->type) {
    case CPP_PLUS:
      c_parser_consume_token(parser);
      return PLUS_EXPR;
...
static void c_parser_objc_method_definition(c_parser *parser) {
...
static void c_parser_objc_methodprotolist(c_parser *parser) {
  while (true) {
...
static void c_parser_objc_methodproto(c_parser *parser) {
  enum tree_code type = c_parser_objc_method_type(parser);
  tree decl;
  objc_set_method_type(type);
...
static tree c_parser_objc_method_decl(c_parser *parser) {
  tree type = NULL_TREE;
  tree sel;
  ...
  if (!sel || c_parser_next_token_is(parser, CPP_COLON)) {
    ...
    while (c_parser_next_token_is(parser, CPP_COMMA)) {
      ...
      if (parm == NULL)
        break;
      parms = chainon(parms, build_tree_list(NULL_TREE, grokparm(parm)));
    }
    sel = list;
  }
...
static tree c_parser_objc_type_name(c_parser *parser) {
  ...
  while (true) {
    ...
    if (token->type == CPP_KEYWORD &&
        (token->keyword == RID_IN || token->keyword == RID_OUT ||
         token->keyword == RID_INOUT || token->keyword == RID_BYCOPY ||
         token->keyword == RID_BYREF || token->keyword == RID_ONEWAY)) {
...
static tree c_parser_objc_protocol_refs(c_parser *parser) {
  tree list = NULL_TREE;
...
static void c_parser_objc_try_catch_statement(c_parser *parser) {
  location_t loc;
  tree stmt;
...
static void c_parser_objc_synchronized_statement(c_parser *parser) {
  location_t loc;
  tree expr, stmt;
...
static tree c_parser_objc_selector(c_parser *parser) {
  c_token *token = c_parser_peek_token(parser);
...
static tree c_parser_objc_selector_arg(c_parser *parser) {
...
static tree c_parser_objc_receiver(c_parser *parser) {
  if (c_parser_peek_token(parser)->type == CPP_NAME &&
      (c_parser_peek_token(parser)->id_kind == C_ID_TYPENAME ||
       c_parser_peek_token(parser)->id_kind == C_ID_CLASSNAME)) {
    tree id = c_parser_peek_token(parser)->value;
...
static tree c_parser_objc_message_args(c_parser *parser) {
  tree sel = c_parser_objc_selector(parser);
  tree list = NULL_TREE;
  if (sel && c_parser_next_token_is_not(parser, CPP_COLON))
    return sel;
...
static tree c_parser_objc_keywordexpr(c_parser *parser) {
...
static void c_parser_pragma_pch_preprocess(c_parser *parser) {
  tree name = NULL;
...
static pragma_omp_clause c_parser_omp_clause_name(c_parser *parser) {
...
static void check_no_duplicate_clause(tree clauses, enum omp_clause_code code,
                                      const char *name) {
  tree c;
...
static tree c_parser_omp_variable_list(c_parser *parser,
...
static tree c_parser_omp_var_list_parens(c_parser *parser,
                                         enum omp_clause_code kind, tree list) {
  if (c_parser_require(parser, CPP_OPEN_PAREN, "expected %<(%>")) {
...
static tree c_parser_omp_clause_collapse(c_parser *parser, tree list) {
  ...
  if (num == error_mark_node)
    return list;
  if (!INTEGRAL_TYPE_P(TREE_TYPE(num)) || !host_integerp(num, 0) ||
      (n = tree_low_cst(num, 0)) <= 0 || (int)n != n) {
    error_at(loc,
...
static tree c_parser_omp_clause_copyin(c_parser *parser, tree list) {
...
static tree c_parser_omp_clause_copyprivate(c_parser *parser, tree list) {
...
static tree c_parser_omp_clause_default(c_parser *parser, tree list) {
...
static tree c_parser_omp_clause_firstprivate(c_parser *parser, tree list) {
...
static tree c_parser_omp_clause_if(c_parser *parser, tree list) {
  if (c_parser_next_token_is(parser, CPP_OPEN_PAREN)) {
...
static tree c_parser_omp_clause_lastprivate(c_parser *parser, tree list) {
...
static tree c_parser_omp_clause_nowait(c_parser *parser ATTRIBUTE_UNUSED,
                                       tree list) {
  tree c;
...
static tree c_parser_omp_clause_num_threads(c_parser *parser, tree list) {
  if (c_parser_require(parser, CPP_OPEN_PAREN, "expected %<(%>")) {
    ...
    if (c == boolean_true_node) {
      warning_at(expr_loc, 0, "%<num_threads%> value must be positive");
      t = integer_one_node;
    }
...
static tree c_parser_omp_clause_ordered(c_parser *parser ATTRIBUTE_UNUSED,
                                        tree list) {
  tree c;
...
static tree c_parser_omp_clause_private(c_parser *parser, tree list) {
...
static tree c_parser_omp_clause_reduction(c_parser *parser, tree list) {
  if (c_parser_require(parser, CPP_OPEN_PAREN, "expected %<(%>")) {
    enum tree_code code;
...
static tree c_parser_omp_clause_schedule(c_parser *parser, tree list) {
  tree c, t;
...
static tree c_parser_omp_clause_shared(c_parser *parser, tree list) {
...
static tree c_parser_omp_clause_untied(c_parser *parser ATTRIBUTE_UNUSED,
                                       tree list) {
  tree c;
...
static tree c_parser_omp_all_clauses(c_parser *parser, unsigned int mask,
                                     const char *where) {
  tree clauses = NULL;
  bool first = true;
...
static tree c_parser_omp_structured_block(c_parser *parser) {
...
static void c_parser_omp_atomic(c_parser *parser) {
  tree lhs, rhs;
  tree stmt;
  enum tree_code code;
    ...
    default:
      switch (c_parser_peek_token(parser)->type) {
        ...
        case CPP_XOR_EQ:
          ...
          break;
        default:
          c_parser_error(parser, "invalid operator for %<#pragma omp atomic%>");
          goto saw_error;
      }
...
static void c_parser_omp_barrier(c_parser *parser) {
  c_parser_consume_pragma(parser);
  c_parser_skip_to_pragma_eol(parser);
...
static tree c_parser_omp_critical(c_parser *parser) {
  tree stmt, name = NULL;
...
static void c_parser_omp_flush(c_parser *parser) {
  c_parser_consume_pragma(parser);
  if (c_parser_next_token_is(parser, CPP_OPEN_PAREN))
...
static tree c_parser_omp_for_loop(c_parser *parser, tree clauses,
                                  tree *par_clauses) {
  ...
  for (i = 0; i < collapse; i++) {
    ...
    if (c_parser_next_token_starts_declspecs(parser)) {
      if (i > 0)
        for_block = tree_cons(NULL, c_begin_compound_stmt(true), for_block);
      c_parser_declaration_or_fndef(parser, true, true, true, true);
      ...
      init = build_modify_expr(init_loc, decl, NOP_EXPR, init_exp.value);
...
#define OMP_FOR_CLAUSE_MASK                                                  \
  ((1u << PRAGMA_OMP_CLAUSE_PRIVATE) |                                       \
   ...
   (1u << PRAGMA_OMP_CLAUSE_REDUCTION) | (1u << PRAGMA_OMP_CLAUSE_ORDERED) | \
   (1u << PRAGMA_OMP_CLAUSE_SCHEDULE) | (1u << PRAGMA_OMP_CLAUSE_COLLAPSE) | \
   (1u << PRAGMA_OMP_CLAUSE_NOWAIT))

static tree c_parser_omp_for(c_parser *parser) {
  tree block, clauses, ret;

  clauses =
      c_parser_omp_all_clauses(parser, OMP_FOR_CLAUSE_MASK, "#pragma omp for");
...
static tree c_parser_omp_master(c_parser *parser) {
  c_parser_skip_to_pragma_eol(parser);
...
static tree c_parser_omp_ordered(c_parser *parser) {
  c_parser_skip_to_pragma_eol(parser);
...
static tree c_parser_omp_sections_scope(c_parser *parser) {
  tree stmt, substmt;
...
#define OMP_SECTIONS_CLAUSE_MASK            \
  ((1u << PRAGMA_OMP_CLAUSE_PRIVATE) |      \
   (1u << PRAGMA_OMP_CLAUSE_FIRSTPRIVATE) | \
   (1u << PRAGMA_OMP_CLAUSE_LASTPRIVATE) |  \
   (1u << PRAGMA_OMP_CLAUSE_REDUCTION) | (1u << PRAGMA_OMP_CLAUSE_NOWAIT))

static tree c_parser_omp_sections(c_parser *parser) {
  tree block, clauses, ret;
...
#define OMP_PARALLEL_CLAUSE_MASK                                            \
  ((1u << PRAGMA_OMP_CLAUSE_IF) | (1u << PRAGMA_OMP_CLAUSE_PRIVATE) |       \
   (1u << PRAGMA_OMP_CLAUSE_FIRSTPRIVATE) |                                 \
   (1u << PRAGMA_OMP_CLAUSE_DEFAULT) | (1u << PRAGMA_OMP_CLAUSE_SHARED) |   \
   (1u << PRAGMA_OMP_CLAUSE_COPYIN) | (1u << PRAGMA_OMP_CLAUSE_REDUCTION) | \
   (1u << PRAGMA_OMP_CLAUSE_NUM_THREADS))

static tree c_parser_omp_parallel(c_parser *parser) {
...
#define OMP_SINGLE_CLAUSE_MASK              \
  ((1u << PRAGMA_OMP_CLAUSE_PRIVATE) |      \
   (1u << PRAGMA_OMP_CLAUSE_FIRSTPRIVATE) | \
   (1u << PRAGMA_OMP_CLAUSE_COPYPRIVATE) | (1u << PRAGMA_OMP_CLAUSE_NOWAIT))

static tree c_parser_omp_single(c_parser *parser) {
  tree stmt = make_node(OMP_SINGLE);
  TREE_TYPE(stmt) = void_type_node;

  OMP_SINGLE_CLAUSES(stmt) = c_parser_omp_all_clauses(
      parser, OMP_SINGLE_CLAUSE_MASK, "#pragma omp single");
...
#define OMP_TASK_CLAUSE_MASK                                               \
  ((1u << PRAGMA_OMP_CLAUSE_IF) | (1u << PRAGMA_OMP_CLAUSE_UNTIED) |       \
   (1u << PRAGMA_OMP_CLAUSE_DEFAULT) | (1u << PRAGMA_OMP_CLAUSE_PRIVATE) | \
   (1u << PRAGMA_OMP_CLAUSE_FIRSTPRIVATE) | (1u << PRAGMA_OMP_CLAUSE_SHARED))

static tree c_parser_omp_task(c_parser *parser) {
  tree clauses, block;
...
static void c_parser_omp_taskwait(c_parser *parser) {
  c_parser_consume_pragma(parser);
  c_parser_skip_to_pragma_eol(parser);
...
static void c_parser_omp_construct(c_parser *parser) {
...
static void c_parser_omp_threadprivate(c_parser *parser) {
  tree vars, t;
