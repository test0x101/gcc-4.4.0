...
void df_add_problem(struct df_problem *problem) {
...
/* Set the MASK flags in the DFLOW problem.  The old flags are
   ...
   checking is enabled.  */
enum df_changeable_flags df_set_flags(
...
/* Clear the MASK flags in the DFLOW problem.  The old flags are
   returned.  If a flag is not allowed to be changed this will fail if
   checking is enabled.  */
enum df_changeable_flags df_clear_flags(
    enum df_changeable_flags changeable_flags) {
  enum df_changeable_flags old_flags = df->changeable_flags;
  df->changeable_flags &= ~changeable_flags;
  return old_flags;
    ...
    if (df->blocks_to_analyze) {
      ...
      for (p = 0; p < df->num_problems_defined; p++) {
        ...
        if (dflow->optional_p && dflow->problem->reset_fun) {
          if (!blocks_to_reset) {
            basic_block bb;
            blocks_to_reset = BITMAP_ALLOC(&df_bitmap_obstack);
            FOR_ALL_BB(bb) {
...
void df_finish_pass(bool verify ATTRIBUTE_UNUSED) {
  int i;
  int removed = 0;
...
static bool gate_opt(void) {
  return optimize > 0;
}

struct rtl_opt_pass pass_df_initialize_opt = {{
    RTL_PASS, "dfinit",           /* name */
    gate_opt,                     /* gate */
...
static void df_worklist_propagate_forward(struct dataflow *dataflow,
                                          unsigned bb_index,
                                          unsigned *bbindex_to_postorder,
                                          bitmap pending, sbitmap considered) {
  edge e;
  edge_iterator ei;
  basic_block bb = BASIC_BLOCK(bb_index);
...
static void df_worklist_propagate_backward(struct dataflow *dataflow,
                                           unsigned bb_index,
                                           unsigned *bbindex_to_postorder,
                                           bitmap pending, sbitmap considered) {
  edge e;
...
static void df_worklist_dataflow_doublequeue(struct dataflow *dataflow,
  ...
  while (!bitmap_empty_p(pending)) {
    ...
    do {
      ...
      if (dir == DF_FORWARD)
        df_worklist_propagate_forward(dataflow, bb_index, bbindex_to_postorder,
                                      pending, considered);
      else
        df_worklist_propagate_backward(dataflow, bb_index, bbindex_to_postorder,
                                       pending, considered);
  ...
  if (dump_file)
    fprintf(dump_file,
            ...
            " count %d (%5.2g)\n",
            n_basic_blocks, n_edges, dcount, dcount / (float)n_basic_blocks);
}
...
void df_worklist_dataflow(struct dataflow *dataflow, bitmap blocks_to_consider,
  ...
  df_worklist_dataflow_doublequeue(dataflow, pending, considered,
                                   blocks_in_postorder, bbindex_to_postorder);
...
void df_analyze_problem(struct dataflow *dflow, bitmap blocks_to_consider,
  ...
  if (dflow->problem->dataflow_fun)
    dflow->problem->dataflow_fun(dflow, blocks_to_consider, postorder,
                                 n_blocks);
...
void df_analyze(void) {
  ...
  if (df->analyze_subset) {
    ...
    df->n_blocks =
        df_prune_to_subcfg(df->postorder, df->n_blocks, df->blocks_to_analyze);
    df->n_blocks_inverted = df_prune_to_subcfg(
        df->postorder_inverted, df->n_blocks_inverted, df->blocks_to_analyze);
  ...
  for (i = 1; i < df->num_problems_defined; i++) {
    ...
    if (dflow->solutions_dirty) {
      if (dflow->problem->dir == DF_FORWARD)
        df_analyze_problem(dflow, df->blocks_to_analyze, df->postorder_inverted,
                           df->n_blocks_inverted);
      else
        df_analyze_problem(dflow, df->blocks_to_analyze, df->postorder,
                           df->n_blocks);
    }
  }
...
void df_simple_dataflow(enum df_flow_dir dir, df_init_function init_fun,
                        df_confluence_function_0 con_fun_0,
                        df_confluence_function_n con_fun_n,
                        df_transfer_function trans_fun, bitmap blocks,
                        int *postorder, int n_blocks) {
...
static void *df_get_bb_info(struct dataflow *dflow, unsigned int index) {
...
static void df_set_bb_info(struct dataflow *dflow, unsigned int index,
                           void *bb_info) {
...
void df_compact_blocks(void) {
  ...
  for (p = 0; p < df->num_problems_defined; p++) {
    ...
    if (dflow->problem->free_bb_fun) {
      ...
      memset(dflow->block_info + i, 0, (last_basic_block - i) * sizeof(void *));
      ...
      for (i = NUM_FIXED_BLOCKS; i < last_basic_block; i++) {
        ...
        if (problem_temps[i] && bb)
          dflow->problem->free_bb_fun(bb, problem_temps[i]);
      }
    }
  }
...
void df_bb_replace(int old_index, basic_block new_block) {
  ...
  for (p = 0; p < df->num_problems_defined; p++) {
    ...
    if (dflow->block_info) {
      ...
      df_set_bb_info(dflow, old_index, df_get_bb_info(dflow, new_block_index));
    }
  }
...
static int *df_compute_cfg_image(void) {
  basic_block bb;
...
df_ref df_find_def(rtx insn, rtx reg) {
  unsigned int uid;
  df_ref *def_rec;
...
bool df_reg_defined(rtx insn, rtx reg) {
...
df_ref df_find_use(rtx insn, rtx reg) {
  unsigned int uid;
  df_ref *use_rec;
...
bool df_reg_used(rtx insn, rtx reg) {
...
void df_print_byte_regset(FILE *file, bitmap r) {
...
void df_refs_chain_dump(df_ref *ref_rec, bool follow_chain, FILE *file) {
  fprintf(file, "{ ");
  while (*ref_rec) {
    df_ref ref = *ref_rec;
    fprintf(file, "%c%d(%d)",
            DF_REF_REG_DEF_P(ref)                  ? 'd'
            : (DF_REF_FLAGS(ref) & DF_REF_IN_NOTE) ? 'e'
                                                   : 'u',
            DF_REF_ID(ref), DF_REF_REGNO(ref));
    if (follow_chain)
...
void df_regs_chain_dump(df_ref ref, FILE *file) {
  fprintf(file, "{ ");
  while (ref) {
    fprintf(file, "%c%d(%d) ", DF_REF_REG_DEF_P(ref) ? 'd' : 'u',
            DF_REF_ID(ref), DF_REF_REGNO(ref));
...
void df_insn_debug(rtx insn, bool follow_chain, FILE *file) {
  df_insn_uid_debug(INSN_UID(insn), follow_chain, file);
}

void df_insn_debug_regno(rtx insn, FILE *file) {
  struct df_insn_info *insn_info = DF_INSN_INFO_GET(insn);

  fprintf(file, "insn %d bb %d luid %d defs ", INSN_UID(insn),
          BLOCK_FOR_INSN(insn)->index, DF_INSN_INFO_LUID(insn_info));
...
void df_regno_debug(unsigned int regno, FILE *file) {
...
void df_ref_debug(df_ref ref, FILE *file) {
  fprintf(file, "%c%d ", DF_REF_REG_DEF_P(ref) ? 'd' : 'u', DF_REF_ID(ref));
  fprintf(file, "reg %d bb %d insn %d flag 0x%x type 0x%x ", DF_REF_REGNO(ref),
          DF_REF_BBNO(ref),
          DF_REF_IS_ARTIFICIAL(ref) ? -1 : DF_REF_INSN_UID(ref),
          DF_REF_FLAGS(ref), DF_REF_TYPE(ref));
  if (DF_REF_LOC(ref))
