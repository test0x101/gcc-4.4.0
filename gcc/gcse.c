...
static int pre_expr_reaches_here_p_work(basic_block, struct expr *, basic_block,
                                        char *);
...
static int gcse_main(rtx f ATTRIBUTE_UNUSED) {
  int changed, pass;
  ...
  if (dump_file) {
    fprintf(dump_file, "GCSE of %s: %d basic blocks, ", current_function_name(),
            n_basic_blocks);
...
bool can_copy_p(enum machine_mode mode) {
...
static void *gcalloc(size_t nelem, size_t elsize) {
...
static void compute_local_properties(sbitmap *transp, sbitmap *comp,
                                     sbitmap *antloc,
...
static void record_set_info(rtx dest, const_rtx setter ATTRIBUTE_UNUSED,
                            void *data) {
...
static bool can_assign_to_reg_p(rtx x) {
  int num_clobbers = 0;
  int icode;
  ...
  if (test_insn == 0) {
    test_insn = make_insn_raw(
        gen_rtx_SET(VOIDmode, gen_rtx_REG(word_mode, FIRST_PSEUDO_REGISTER * 2),
                    const0_rtx));
...
static int oprs_unchanged_p(const_rtx x, const_rtx insn, int avail_p) {
  int i, j;
  enum rtx_code code;
  const char *fmt;
...
static int load_killed_in_block_p(const_basic_block bb, int uid_limit,
                                  const_rtx x, int avail_p) {
...
static int oprs_anticipatable_p(const_rtx x, const_rtx insn) {
...
static int oprs_available_p(const_rtx x, const_rtx insn) {
...
static unsigned int hash_expr(const_rtx x, enum machine_mode mode,
                              int *do_not_record_p, int hash_table_size) {
  unsigned int hash;
...
static int expr_equiv_p(const_rtx x, const_rtx y) {
...
static void insert_expr_in_table(rtx x, enum machine_mode mode, rtx insn,
                                 int antic_p, int avail_p,
                                 struct hash_table *table) {
...
static bool gcse_constant_p(const_rtx x) {
  /* Consider a COMPARE of two integers constant.  */
  if (GET_CODE(x) == COMPARE && GET_CODE(XEXP(x, 0)) == CONST_INT &&
      GET_CODE(XEXP(x, 1)) == CONST_INT)
    return true;

  /* Consider a COMPARE of the same registers is a constant
     if they are not floating point registers.  */
  if (GET_CODE(x) == COMPARE && REG_P(XEXP(x, 0)) && REG_P(XEXP(x, 1)) &&
...
static void hash_scan_set(rtx pat, rtx insn, struct hash_table *table) {
  ...
  if (GET_CODE(src) == CALL)
    ...
    if (note != 0 && REG_NOTE_KIND(note) == REG_EQUAL && !REG_P(src) &&
        (table->set_p ? gcse_constant_p(XEXP(note, 0))
    ...
    if (!table->set_p &&
      ...
      int antic_p = oprs_anticipatable_p(src, insn) && !multiple_sets(insn);
    ...
    else if (table->set_p && regno >= FIRST_PSEUDO_REGISTER &&
...
static void hash_scan_clobber(rtx x ATTRIBUTE_UNUSED, rtx insn ATTRIBUTE_UNUSED,
                              struct hash_table *table ATTRIBUTE_UNUSED) {
  /* Currently nothing to do.  */
}

static void hash_scan_call(rtx x ATTRIBUTE_UNUSED, rtx insn ATTRIBUTE_UNUSED,
                           struct hash_table *table ATTRIBUTE_UNUSED) {
  /* Currently nothing to do.  */
}
...
static void hash_scan_insn(rtx insn, struct hash_table *table) {
...
static void dump_hash_table(FILE *file, const char *name,
                            struct hash_table *table) {
  int i;
...
static void canon_list_insert(rtx dest ATTRIBUTE_UNUSED,
...
static void record_last_set_info(rtx dest, const_rtx setter ATTRIBUTE_UNUSED,
                                 void *data) {
...
static void compute_hash_table_work(struct hash_table *table) {
  ...
  FOR_EACH_BB(current_bb) {
    ...
    if (table->set_p && implicit_sets[current_bb->index] != NULL_RTX)
      hash_scan_set(implicit_sets[current_bb->index], BB_HEAD(current_bb),
                    table);
...
static struct expr *lookup_set(unsigned int regno, struct hash_table *table) {
...
static struct expr *next_set(unsigned int regno, struct expr *expr) {
  do
    expr = expr->next_same_hash;
  while (expr && REGNO(SET_DEST(expr->expr)) != regno);
...
static int oprs_not_set_p(const_rtx x, const_rtx insn) {
  int i, j;
  enum rtx_code code;
  const char *fmt;
  ...
  switch (code) {
    ...
    case MEM:
      if (load_killed_in_block_p(BLOCK_FOR_INSN(insn), INSN_CUID(insn), x, 0))
        return 0;
      else
...
static void compute_transp(const_rtx x, int indx, sbitmap *bmap, int set_p) {
  int i, j;
  basic_block bb;
  enum rtx_code code;
  reg_set *r;
      ...
      if (!MEM_READONLY_P(x)) {
        ...
        EXECUTE_IF_AND_COMPL_IN_BITMAP(modify_mem_list_set, blocks_with_calls,
          ...
          while (list_entry) {
            ...
            if (canon_true_dependence(dest, GET_MODE(dest), dest_addr, x,
                                      rtx_addr_varies_p)) {
              if (set_p)
                SET_BIT(bmap[bb_index], indx);
              else
...
static int try_replace_reg(rtx from, rtx to, rtx insn) {
  ...
  if (note != 0 && REG_NOTE_KIND(note) == REG_EQUAL)
    set_unique_reg_note(
        insn, REG_EQUAL,
        simplify_replace_rtx(XEXP(note, 0), from, copy_rtx(to)));
...
static int cprop_jump(basic_block bb, rtx setcc, rtx jump, rtx from, rtx src) {
  ...
  if (setcc != NULL_RTX && !modified_between_p(from, setcc, jump) &&
    ...
    setcc_src = (setcc_note && GET_CODE(XEXP(setcc_note, 0)) != EXPR_LIST)
                    ? XEXP(setcc_note, 0)
                    : SET_SRC(setcc_set);
    set_src = simplify_replace_rtx(set_src, SET_DEST(setcc_set), setcc_src);
  } else
    setcc = NULL_RTX;
  ...
  if (new_rtx != pc_rtx && simplejump_p(jump)) {
    ...
    for (ei = ei_start(bb->succs); (e = ei_safe_edge(ei)); ei_next(&ei))
      if (e->dest != EXIT_BLOCK_PTR && BB_HEAD(e->dest) == JUMP_LABEL(jump)) {
...
static bool constprop_register(rtx insn, rtx from, rtx to, bool alter_jumps) {
  rtx sset;

  /* Check for reg or cc0 setting instructions followed by
     conditional branch instructions first.  */
  if (alter_jumps && (sset = single_set(insn)) != NULL && NEXT_INSN(insn) &&
  ...
  if (NONJUMP_INSN_P(insn) && try_replace_reg(from, to, insn))
    return 1;
...
static int cprop_insn(rtx insn, int alter_jumps) {
  ...
  for (reg_used = &reg_use_table[0]; reg_use_count > 0;
    ...
    } else if (REG_P(src) && REGNO(src) >= FIRST_PSEUDO_REGISTER &&
               REGNO(src) != regno) {
...
static bool do_local_cprop(rtx x, rtx insn, bool alter_jumps) {
  ...
  if (REG_P(x) &&
    ...
    if (newcnst && constprop_register(insn, x, newcnst, alter_jumps)) {
      ...
      if (dump_file != NULL) {
        fprintf(dump_file, "LOCAL COPY-PROP: Replacing reg %d in insn %d",
...
static void local_cprop_pass(bool alter_jumps) {
  basic_block bb;
  rtx insn;
  struct reg_use *reg_used;
  bool changed = false;
...
rtx fis_get_condition(rtx jump) {
...
static void find_implicit_sets(void) {
  ...
  if (EDGE_COUNT(bb->succs) > 1) {
    ...
    if (cond && (GET_CODE(cond) == EQ || GET_CODE(cond) == NE) &&
        REG_P(XEXP(cond, 0)) && REGNO(XEXP(cond, 0)) >= FIRST_PSEUDO_REGISTER &&
        implicit_set_cond_p(cond)) {
      dest = GET_CODE(cond) == EQ ? BRANCH_EDGE(bb)->dest
                                  : FALLTHRU_EDGE(bb)->dest;

      if (dest && single_pred_p(dest) && dest != EXIT_BLOCK_PTR) {
        new_rtx = gen_rtx_SET(VOIDmode, XEXP(cond, 0), XEXP(cond, 1));
...
static int one_cprop_pass(int pass, bool cprop_jumps, bool bypass_jumps) {
  int changed = 0;
...
static struct expr *find_bypass_set(int regno, int bb) {
...
static int bypass_block(basic_block bb, rtx setcc, rtx jump) {
  ...
  for (ei = ei_start(bb->preds); (e = ei_safe_edge(ei));) {
    ...
    for (i = 0; i < reg_use_count; i++) {
      ...
      if (setcc != NULL)
        src = simplify_replace_rtx(src, SET_DEST(PATTERN(setcc)),
                                   SET_SRC(PATTERN(setcc)));

      new_rtx =
          simplify_replace_rtx(src, reg_used->reg_rtx, SET_SRC(set->expr));
...
static int pre_expr_reaches_here_p_work(basic_block occr_bb, struct expr *expr,
...
static int pre_expr_reaches_here_p(basic_block occr_bb, struct expr *expr,
                                   basic_block bb) {
  int rval;
...
static void insert_insn_end_basic_block(struct expr *expr, basic_block bb,
                                        int pre) {
  rtx insn = BB_END(bb);
  rtx new_insn;
  rtx reg = expr->reaching_reg;
  ...
  if (JUMP_P(insn) ||
    ...
    if (note)
      ...
      if (maybe_cc0_setter && INSN_P(maybe_cc0_setter) &&
  ...
  /* Likewise if the last insn is a call, as will happen in the presence
  ...
  else if (CALL_P(insn) && (!single_succ_p(bb) ||
    ...
    gcc_assert(!pre || TEST_BIT(antloc[bb->index], expr->bitmap_index) ||
               TEST_BIT(transp[bb->index], expr->bitmap_index));
  ...
  if (dump_file) {
    fprintf(dump_file, "PRE/HOIST: end of bb %d, insn %d, ", bb->index,
            INSN_UID(new_insn));
        ...
        if ((insert & 1) != 0 && index_map[j]->reaching_reg != NULL_RTX) {
          ...
          for (occr = expr->antic_occr; occr != NULL; occr = occr->next) {
            ...
            if (!TEST_BIT(inserted[e], j)) {
              ...
              if (dump_file) {
                fprintf(dump_file, "PRE/HOIST: edge (%d,%d), ", bb->index,
    ...
    for (expr = expr_hash_table.table[i]; expr != NULL;
      ...
      for (occr = expr->antic_occr; occr != NULL; occr = occr->next) {
        ...
        for (avail = expr->avail_occr; avail != NULL; avail = avail->next) {
          ...
          if (!pre_expr_reaches_here_p(BLOCK_FOR_INSN(avail->insn), expr,
    ...
    for (expr = expr_hash_table.table[i]; expr != NULL;
      ...
      for (occr = expr->antic_occr; occr != NULL; occr = occr->next) {
        ...
        if (TEST_BIT(pre_delete_map[bb->index], indx) &&
          ...
          if (dump_file) {
            fprintf(dump_file, "PRE: redundant insn %d (expression %d) in ",
                    INSN_UID(insn), indx);
            fprintf(dump_file, "bb %d, reaching reg is %d\n", bb->index,
                    REGNO(expr->reaching_reg));
          }
        }
      }
    }
...
static int hoist_expr_reaches_here_p(basic_block expr_bb, int expr_index,
                                     basic_block bb, char *visited) {
  edge pred;
  edge_iterator ei;
...
static hashval_t pre_ldst_expr_hash(const void *p) {
  int do_not_record_p = 0;
  const struct ls_expr *const x = (const struct ls_expr *)p;
  return hash_rtx(x->pattern, GET_MODE(x->pattern), &do_not_record_p, NULL,
                  false);
}

static int pre_ldst_expr_eq(const void *p1, const void *p2) {
...
static struct ls_expr *ldst_entry(rtx x) {
  ...
  hash =
      hash_rtx(x, GET_MODE(x), &do_not_record_p, NULL, /*have_reg_qty=*/false);
...
static inline struct ls_expr *next_ls_expr(struct ls_expr *ptr) {
  return ptr->next;
}
...
static int simple_mem(const_rtx x) {
  if (!MEM_P(x))
    return 0;
...
static void compute_ld_motion_mems(void) {
  ...
  pre_ldst_mems = NULL;
  pre_ldst_table = htab_create(13, pre_ldst_expr_hash, pre_ldst_expr_eq, NULL);
...
static void trim_ld_motion_mems(void) {
  ...
  while (ptr != NULL) {
    ...
    if (!ptr->invalid) {
      ...
      for (expr = expr_hash_table.table[hash]; expr != NULL;
           expr = expr->next_same_hash)
...
static void reg_set_info(rtx dest, const_rtx setter ATTRIBUTE_UNUSED,
                         void *data) {
...
static void reg_clear_last_set(rtx dest, const_rtx setter ATTRIBUTE_UNUSED,
                               void *data) {
...
static bool store_ops_ok(const_rtx x, int *regs_set) {
  const_rtx reg;
...
static void find_moveable_store(rtx insn, int *regs_set_before,
                                int *regs_set_after) {
  struct ls_expr *ptr;
  rtx dest, set, tmp;
  ...
  if (!MEM_P(dest) || MEM_VOLATILE_P(dest) || GET_MODE(dest) == BLKmode)
    return;
  ...
  if (check_available) {
    ...
    if (LAST_AVAIL_CHECK_FAILURE(ptr)) {
      ...
      check_available =
          store_killed_after(dest, ptr->pattern_regs, insn, bb, regs_set_after,
...
static int compute_store_table(void) {
  ...
  for (ptr = pre_ldst_mems, prev_next_ptr_ptr = &pre_ldst_mems; ptr != NULL;
...
static inline bool store_killed_in_pat(const_rtx x, const_rtx pat, int after) {
  if (GET_CODE(pat) == SET) {
    rtx dest = SET_DEST(pat);
    ...
    if (MEM_P(dest) && !expr_equiv_p(dest, x)) {
      if (after) {
...
static bool store_killed_in_insn(const_rtx x, const_rtx x_regs, const_rtx insn,
                                 int after) {
  ...
  if (CALL_P(insn)) {
    ...
    for (reg = x_regs; reg; reg = XEXP(reg, 1)) {
      ...
      if (!base || (GET_CODE(base) == ADDRESS && GET_MODE(base) == Pmode &&
...
static bool store_killed_after(const_rtx x, const_rtx x_regs, const_rtx insn,
                               const_basic_block bb, int *regs_set_after,
                               rtx *fail_insn) {
...
static bool store_killed_before(const_rtx x, const_rtx x_regs, const_rtx insn,
...
static void build_store_vectors(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    for (ptr = first_ls_expr(); ptr != NULL; ptr = next_ls_expr(ptr)) {
      if (store_killed_after(ptr->pattern, ptr->pattern_regs, BB_HEAD(bb), bb,
...
static void insert_insn_start_basic_block(rtx insn, basic_block bb) {
  /* Insert at start of successor block.  */
  rtx prev = PREV_INSN(BB_HEAD(bb));
  rtx before = BB_HEAD(bb);
  while (before != 0) {
    if (!LABEL_P(before) && !NOTE_INSN_BASIC_BLOCK_P(before))
      break;
    prev = before;
    if (prev == BB_END(bb))
      break;
...
static void remove_reachable_equiv_notes(basic_block bb,
  ...
  while (1) {
    ...
    if (TEST_BIT(st_antloc[bb->index], smexpr->index)) {
      for (last = ANTIC_STORE_LIST(smexpr); BLOCK_FOR_INSN(XEXP(last, 0)) != bb;
           last = XEXP(last, 1))
        continue;
      last = XEXP(last, 0);
    } else
...
static void replace_store_insn(rtx reg, rtx del, basic_block bb,
  ...
  if (dump_file) {
    fprintf(dump_file, "STORE_MOTION  delete insn in BB %d:\n      ",
            bb->index);
...
static void store_motion(void) {
  ...
  edge_list = pre_edge_rev_lcm(num_stores, transp, ae_gen, st_antloc, ae_kill,
...
static bool is_too_expensive(const char *pass) {
  ...
  if (n_edges > 20000 + n_basic_blocks * 4) {
    warning(OPT_Wdisabled_optimization,
            "%s: %d basic blocks and %d edges/basic block", pass,
  ...
  if ((n_basic_blocks * SBITMAP_SET_SIZE(max_reg_num()) *
       sizeof(SBITMAP_ELT_TYPE)) > MAX_GCSE_MEMORY) {
    warning(OPT_Wdisabled_optimization, "%s: %d basic blocks and %d registers",
...
static bool gate_handle_jump_bypass(void) {
  return optimize > 0 && flag_gcse && dbg_cnt(jump_bypass);
}

/* Perform jump bypassing and control flow optimizations.  */
static unsigned int rest_of_handle_jump_bypass(void) {
  delete_unreachable_blocks();
  if (bypass_jumps()) {
...
struct rtl_opt_pass pass_jump_bypass = {{
    ...
    0,                          /* todo_flags_start */
    TODO_dump_func | TODO_ggc_collect | TODO_verify_flow /* todo_flags_finish */
}};

static bool gate_handle_gcse(void) {
  return optimize > 0 && flag_gcse && dbg_cnt(gcse);
}

static unsigned int rest_of_handle_gcse(void) {
