...
static inline bool get_attr_enabled(rtx insn ATTRIBUTE_UNUSED) {
  return true;
}
#endif
...
static bool validate_change_1(rtx object, rtx *loc, rtx new_rtx, bool in_group,
                              bool unshare) {
  rtx old = *loc;
...
bool canonicalize_change_group(rtx insn, rtx x) {
  if (COMMUTATIVE_P(x) && swap_commutative_operands_p(XEXP(x, 0), XEXP(x, 1))) {
...
int insn_invalid_p(rtx insn) {
  ...
  int icode =
      recog(pat, insn,
            (GET_CODE(pat) == SET && !reload_completed && !reload_in_progress)
                ? &num_clobbers
                : 0);
  ...
  if ((is_asm && !check_asm_operands(PATTERN(insn))) || (!is_asm && icode < 0))
    return 1;
  ...
  for (i = num; i < num_changes; i++) {
    ...
    if (MEM_P(object)) {
      if (!memory_address_p(GET_MODE(object), XEXP(object, 0)))
        break;
    } else if (REG_P(changes[i].old) && asm_noperands(PATTERN(object)) > 0 &&
      ...
      if (GET_CODE(pat) == PARALLEL &&
        ...
        if (XVECLEN(pat, 0) == 2)
          ...
          newpat = gen_rtx_PARALLEL(VOIDmode, rtvec_alloc(XVECLEN(pat, 0) - 1));
...
static void simplify_while_replacing(rtx *loc, rtx to, rtx object,
  ...
  if (SWAPPABLE_OPERANDS_P(x) &&
    ...
    validate_unshare_change(
        object, loc,
        gen_rtx_fmt_ee(COMMUTATIVE_ARITH_P(x) ? code : swap_condition(code),
  ...
  switch (code) {
    case PLUS:
      ...
      if (GET_CODE(XEXP(x, 1)) == CONST_INT && XEXP(x, 1) == to)
        validate_change(
            object, loc,
            simplify_gen_binary(PLUS, GET_MODE(x), XEXP(x, 0), XEXP(x, 1)), 1);
      break;
    case MINUS:
      if (GET_CODE(XEXP(x, 1)) == CONST_INT ||
        ...
        validate_change(
            ...
            simplify_gen_binary(
                ...
                simplify_gen_unary(NEG, GET_MODE(x), XEXP(x, 1), GET_MODE(x))),
            1);
      break;
    case ZERO_EXTEND:
    case SIGN_EXTEND:
      if (GET_MODE(XEXP(x, 0)) == VOIDmode) {
        new_rtx = simplify_gen_unary(code, GET_MODE(x), XEXP(x, 0), op0_mode);
    ...
    case SUBREG:
      ...
      new_rtx =
          simplify_subreg(GET_MODE(x), SUBREG_REG(x), op0_mode, SUBREG_BYTE(x));
    ...
    case SIGN_EXTRACT:
      ...
      if (MEM_P(XEXP(x, 0)) && GET_CODE(XEXP(x, 1)) == CONST_INT &&
        ...
        if (wanted_mode != VOIDmode &&
          ...
          if (BYTES_BIG_ENDIAN != BITS_BIG_ENDIAN)
            offset =
                (GET_MODE_SIZE(is_mode) - GET_MODE_SIZE(wanted_mode) - offset);
...
static void validate_replace_rtx_1(rtx *loc, rtx from, rtx to, rtx object,
  ...
  if (x == from ||
      (REG_P(x) && REG_P(from) && GET_MODE(x) == GET_MODE(from) &&
       REGNO(x) == REGNO(from)) ||
  ...
  if (GET_CODE(x) == PARALLEL) {
    for (j = XVECLEN(x, 0) - 1; j >= 0; j--) {
      if (j && GET_CODE(XVECEXP(x, 0, j)) == SET &&
        ...
        validate_replace_rtx_1(&SET_DEST(XVECEXP(x, 0, j)), from, to, object,
                               simplify);
      } else
        validate_replace_rtx_1(&XVECEXP(x, 0, j), from, to, object, simplify);
    }
  } else
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--) {
      if (fmt[i] == 'e')
        ...
        for (j = XVECLEN(x, i) - 1; j >= 0; j--)
          validate_replace_rtx_1(&XVECEXP(x, i, j), from, to, object, simplify);
    }
...
int validate_replace_rtx_part(rtx from, rtx to, rtx *where, rtx insn) {
  validate_replace_rtx_1(where, from, to, insn, true);
  return apply_change_group();
}

/* Same as above, but do not simplify rtx afterwards.  */
int validate_replace_rtx_part_nosimplify(rtx from, rtx to, rtx *where,
                                         rtx insn) {
...
int next_insn_tests_no_inequality(rtx insn) {
  ...
  return (INSN_P(next) && !inequality_comparisons_p(PATTERN(next)));
}
#endif
...
int general_operand(rtx op, enum machine_mode mode) {
  ...
  if (GET_CODE(op) == CONST_INT && mode != VOIDmode &&
...
int register_operand(rtx op, enum machine_mode mode) {
  ...
  if (GET_CODE(op) == SUBREG) {
    ...
    if (REG_P(sub) && REGNO(sub) < FIRST_PSEUDO_REGISTER &&
...
int immediate_operand(rtx op, enum machine_mode mode) {
  ...
  return (
      CONSTANT_P(op) &&
      (GET_MODE(op) == mode || mode == VOIDmode || GET_MODE(op) == VOIDmode) &&
      (!flag_pic || LEGITIMATE_PIC_OPERAND_P(op)) && LEGITIMATE_CONSTANT_P(op));
}
...
int const_int_operand(rtx op, enum machine_mode mode) {
  ...
  if (mode != VOIDmode && trunc_int_for_mode(INTVAL(op), mode) != INTVAL(op))
    return 0;
...
int push_operand(rtx op, enum machine_mode mode) {
  ...
  if (rounded_size == GET_MODE_SIZE(mode)) {
    ...
    if (GET_CODE(op) != PRE_MODIFY || GET_CODE(XEXP(op, 1)) != PLUS ||
...
int indirect_operand(rtx op, enum machine_mode mode) {
  ...
  return (MEM_P(op) && memory_operand(op, mode) &&
...
int comparison_operator(rtx op, enum machine_mode mode) {
  return ((mode == VOIDmode || GET_MODE(op) == mode) && COMPARISON_P(op));
}
...
const char *decode_asm_operands(rtx body, rtx *operands, rtx **operand_locs,
                                const char **constraints,
                                enum machine_mode *modes, location_t *loc) {
  int i;
  int noperands;
  rtx asmop = 0;
...
int asm_operand_ok(rtx op, const char *constraint, const char **constraints) {
  ...
  while (*constraint) {
    ...
    switch (c) {
      ...
      case '<':
        /* ??? Before auto-inc-dec, auto inc/dec insns are not supposed to
           exist, excepting those that expand_call created.  Further, on some
           machines which do not have generalized auto inc/dec, an inc/dec
           is not a memory_operand.

           Match any memory and hope things are resolved after reload.  */

        if (MEM_P(op) && (1 || GET_CODE(XEXP(op, 0)) == PRE_DEC ||
                          GET_CODE(XEXP(op, 0)) == POST_DEC))
          result = 1;
        break;

      case '>':
        if (MEM_P(op) && (1 || GET_CODE(XEXP(op, 0)) == PRE_INC ||
...
rtx *find_constant_term_loc(rtx *p) {
  ...
  if (XEXP(*p, 0) && CONSTANT_P(XEXP(*p, 0)) && XEXP(*p, 1) &&
      CONSTANT_P(XEXP(*p, 1)))
    return p;
...
int offsettable_memref_p(rtx op) {
  return ((MEM_P(op)) && offsettable_address_p(1, GET_MODE(op), XEXP(op, 0)));
}
...
int offsettable_nonstrict_memref_p(rtx op) {
  return ((MEM_P(op)) && offsettable_address_p(0, GET_MODE(op), XEXP(op, 0)));
}
...
int offsettable_address_p(int strictp, enum machine_mode mode, rtx y) {
  ...
  if (GET_CODE(y) == LO_SUM && mode != BLKmode &&
...
int mode_dependent_address_p(rtx addr) {
  ...
  if (GET_CODE(addr) == PRE_INC || GET_CODE(addr) == POST_INC ||
      GET_CODE(addr) == PRE_DEC || GET_CODE(addr) == POST_DEC)
    return 1;
  ...
  switch (GET_CODE(body)) {
    ...
    asm_insn:
      ...
      if (noperands >= 0) {
        ...
        decode_asm_operands(body, recog_data.operand, recog_data.operand_loc,
                            recog_data.constraints, recog_data.operand_mode,
                            NULL);
        if (noperands > 0) {
...
void preprocess_constraints(void) {
  ...
  for (i = 0; i < recog_data.n_operands; i++)
    memset(recog_op_alt[i], 0,
           (recog_data.n_alternatives * sizeof(struct operand_alternative)));
  ...
  do {
        ...
        switch (c = *p, len = CONSTRAINT_LEN(c, p), c) {
          ...
          case 'p':
            /* p is used for address_operands.  When we are called by
               ...
               have gotten them.  */
            if (strict <= 0 ||
                (strict_memory_address_p(recog_data.operand_mode[opno], op)))
              win = 1;
            break;
          ...
          case 'g':
            ...
            if (REG_P(op)) {
              if (strict < 0 || GENERAL_REGS == ALL_REGS ||
          ...
          case TARGET_MEM_CONSTRAINT:
            ...
            if (MEM_P(op)) {
              if (strict > 0 &&
                  !strict_memory_address_p(GET_MODE(op), XEXP(op, 0)))
                break;
              if (strict == 0 && !memory_address_p(GET_MODE(op), XEXP(op, 0)))
                break;
              win = 1;
            }
          ...
          case 'V':
            if (MEM_P(op) &&
                ((strict > 0 && !offsettable_memref_p(op)) ||
                 (strict < 0 && !(CONSTANT_P(op) || MEM_P(op))) ||
                 (reload_in_progress &&
                  !(REG_P(op) && REGNO(op) >= FIRST_PSEUDO_REGISTER))))
              win = 1;
            break;
          ...
          default: {
            ...
            if (cl != NO_REGS) {
              if (strict < 0 ||
                  ...
                  (REG_P(op) && reg_fits_class_p(op, cl, offset, mode)))
                win = 1;
            }
#ifdef EXTRA_CONSTRAINT_STR
    ...
    if (!lose) {
      ...
      if (strict > 0 && seen_earlyclobber_at >= 0)
        for (eopno = seen_earlyclobber_at; eopno < recog_data.n_operands;
             eopno++)
          /* Ignore earlyclobber operands now in memory,
             because we would often report failure when we have
             two memory operands, one of which was formerly a REG.  */
          if (earlyclobber[eopno] && REG_P(recog_data.operand[eopno]))
            for (opno = 0; opno < recog_data.n_operands; opno++)
...
int reg_fits_class_p(rtx operand, enum reg_class cl, int offset,
  ...
  return (regno < FIRST_PSEUDO_REGISTER &&
          in_hard_reg_set_p(reg_class_contents[(int)cl], mode, regno + offset));
}
...
int peep2_regno_dead_p(int ofs, int regno) {
...
int peep2_reg_dead_p(int ofs, rtx reg) {
  int regno, n;
...
rtx peep2_find_free_register(int from, int to, const char *class_str,
    ...
    for (insn = BB_END(bb);; insn = prev) {
      ...
      if (INSN_P(insn)) {
        ...
        if (attempt != NULL) {
          ...
          note =
              find_reg_note(peep2_insn_data[i].insn, REG_EH_REGION, NULL_RTX);

          /* Replace the old sequence with the new.  */
          attempt =
              emit_insn_after_setloc(attempt, peep2_insn_data[i].insn,
          ...
          if (note || (was_call && nonlocal_goto_handler_labels)) {
            ...
            for (x = attempt; x != before_try; x = PREV_INSN(x))
              if (CALL_P(x) ||
                  (flag_non_call_exceptions && may_trap_p(PATTERN(x)) &&
                ...
                if (x != BB_END(bb) && eh_edge) {
                  ...
                  flags = (eh_edge->flags & (EDGE_EH | EDGE_ABNORMAL));
                  if (CALL_P(x))
                    flags |= EDGE_ABNORMAL_CALL;
                  nehe = make_edge(nfte->src, eh_edge->dest, flags);

                  nehe->probability = eh_edge->probability;
                  nfte->probability = REG_BR_PROB_BASE - nehe->probability;
...
static bool gate_handle_peephole2(void) {
...
static unsigned int rest_of_handle_split_all_insns(void) {
  split_all_insns();
  return 0;
}

struct rtl_opt_pass pass_split_all_insns = {{
    RTL_PASS, "split1",             /* name */
    NULL,                           /* gate */
...
static unsigned int rest_of_handle_split_after_reload(void) {
...
static bool gate_handle_split_before_regstack(void) {
...
static unsigned int rest_of_handle_split_before_regstack(void) {
  split_all_insns();
  return 0;
}

struct rtl_opt_pass pass_split_before_regstack = {{
    RTL_PASS, "split3",                   /* name */
...
static bool gate_handle_split_before_sched2(void) {
#ifdef INSN_SCHEDULING
...
static unsigned int rest_of_handle_split_before_sched2(void) {
#ifdef INSN_SCHEDULING
  split_all_insns();
#endif
  return 0;
...
struct rtl_opt_pass pass_split_before_sched2 = {{
    ...
    0,                                  /* properties_destroyed */
    0,                                  /* todo_flags_start */
    TODO_verify_flow | TODO_dump_func   /* todo_flags_finish */
}};

/* The placement of the splitting that we do for shorten_branches
   depends on whether regstack is used by the target or not.  */
static bool gate_do_final_split(void) {
#if defined(HAVE_ATTR_length) && !defined(STACK_REGS)
  return 1;
#else
  return 0;
