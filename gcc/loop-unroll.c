...
void unroll_and_peel_loops(int flags) {
  struct loop *loop;
  bool check;
  loop_iterator li;
...
static bool loop_exit_at_end_p(struct loop *loop) {
...
static void peel_loops_completely(int flags) {
  struct loop *loop;
  loop_iterator li;
...
static void decide_unrolling_and_peeling(int flags) {
  struct loop *loop;
  loop_iterator li;
...
/* Decide whether the LOOP is once rolling and suitable for complete
   peeling.  */
static void decide_peel_once_rolling(struct loop *loop,
...
static void decide_peel_completely(struct loop *loop,
                                   int flags ATTRIBUTE_UNUSED) {
  unsigned npeel;
  struct niter_desc *desc;
...
/* Peel all iterations of LOOP, remove exit edges and cancel the loop
   ...
   body; i++;
   */
static void peel_loop_completely(struct loop *loop) {
  sbitmap wont_exit;
  ...
  if (npeel) {
    ...
    ok = duplicate_loop_to_header_edge(
        loop, loop_preheader_edge(loop), npeel, wont_exit, desc->out_edge,
        &remove_edges,
...
static void decide_unroll_constant_iterations(struct loop *loop, int flags) {
...
/* Unroll LOOP with constant number of iterations LOOP->LPT_DECISION.TIMES + 1
  ...
  */
static void unroll_loop_constant_iterations(struct loop *loop) {
...
/* Decide whether to unroll LOOP iterating runtime computable number of times
   and how much.  */
static void decide_unroll_runtime_iterations(struct loop *loop, int flags) {
...
basic_block split_edge_and_insert(edge e, rtx insns) {
  basic_block bb;
...
/* Unroll LOOP for that we are able to count number of iterations in runtime
   ...
   */
static void unroll_loop_runtime_iterations(struct loop *loop) {
  ...
  niter = expand_simple_binop(desc->mode, AND, niter, GEN_INT(max_unroll),
  ...
  ok = duplicate_loop_to_header_edge(loop, loop_preheader_edge(loop), 1,
...
static void decide_peel_simple(struct loop *loop, int flags) {
  unsigned npeel;
  struct niter_desc *desc;
...
/* Peel a LOOP LOOP->LPT_DECISION.TIMES times.  The transformation:
   ...
   */
static void peel_loop_simple(struct loop *loop) {
  sbitmap wont_exit;
  ...
  ok = duplicate_loop_to_header_edge(
      loop, loop_preheader_edge(loop), npeel, wont_exit, NULL, NULL,
      DLTHE_FLAG_UPDATE_FREQ | (opt_info ? DLTHE_RECORD_COPY_NUMBER : 0));
  gcc_assert(ok);
...
static void decide_unroll_stupid(struct loop *loop, int flags) {
  unsigned nunroll, nunroll_by_av, i;
  struct niter_desc *desc;
...
/* Unroll a LOOP LOOP->LPT_DECISION.TIMES times.  The transformation:
   ...
   */
static void unroll_loop_stupid(struct loop *loop) {
  sbitmap wont_exit;
...
static hashval_t si_info_hash(const void *ivts) {
  return (hashval_t)INSN_UID(((const struct iv_to_split *)ivts)->insn);
}

/* An equality functions for information about insns to split.  */

static int si_info_eq(const void *ivts1, const void *ivts2) {
...
static hashval_t ve_info_hash(const void *ves) {
...
static int ve_info_eq(const void *ivts1, const void *ivts2) {
...
bool referenced_in_one_insn_in_loop_p(struct loop *loop, rtx reg) {
  basic_block *body, bb;
  unsigned i;
  int count_ref = 0;
  rtx insn;
...
static struct var_to_expand *analyze_insn_to_expand_var(struct loop *loop,
                                                        rtx insn) {
  ...
  if (GET_CODE(src) != PLUS && GET_CODE(src) != MINUS && GET_CODE(src) != MULT)
    return NULL;
  ...
  if (accum_pos == 1 && GET_CODE(src) == MINUS)
    return NULL;
  ...
  if ((FLOAT_MODE_P(mode1) || FLOAT_MODE_P(mode2)) && !flag_associative_math)
    return NULL;
...
static struct iv_to_split *analyze_iv_to_split_insn(rtx insn) {
  rtx set, dest;
  struct rtx_iv iv;
...
static struct opt_info *analyze_insns_in_loop(struct loop *loop) {
  basic_block *body, bb;
  unsigned i;
  ...
  if (flag_variable_expansion_in_unroller && can_apply)
    opt_info->insns_with_var_to_expand =
        htab_create(5 * loop->num_nodes, ve_info_hash, ve_info_eq, free);
...
static rtx *get_ivts_expr(rtx expr, struct iv_to_split *ivts) {
  unsigned i;
  rtx *ret = &expr;
...
static int allocate_basic_variable(void **slot, void *data ATTRIBUTE_UNUSED) {
...
static void insert_base_initialization(struct iv_to_split *ivts, rtx insn) {
...
static void split_iv(struct iv_to_split *ivts, rtx insn, unsigned delta) {
  ...
  if (!delta)
    ...
    incr =
        simplify_gen_binary(MULT, mode, ivts->step, gen_int_mode(delta, mode));
...
static void expand_var_during_unrolling(struct var_to_expand *ve, rtx insn) {
  rtx new_reg, set;
...
static int insert_var_expansion_initialization(void **slot, void *place_p) {
...
static int combine_var_copies_in_loop_exit(void **slot, void *place_p) {
  struct var_to_expand *ve = (struct var_to_expand *)*slot;
  basic_block place = (basic_block)place_p;
  rtx sum = ve->reg;
...
static void apply_opt_in_copies(struct opt_info *opt_info, unsigned n_copies,
  ...
  for (i = opt_info->first_new_block; i < (unsigned)last_basic_block; i++) {
    ...
    for (insn = BB_HEAD(bb); insn != NEXT_INSN(BB_END(bb)); insn = next) {
      ...
      if (opt_info->insns_to_split) {
        ivts = (struct iv_to_split *)htab_find(opt_info->insns_to_split,
                                               &ivts_templ);
      ...
      if (unrolling && opt_info->insns_with_var_to_expand) {
        ves = (struct var_to_expand *)htab_find(
            opt_info->insns_with_var_to_expand, &ve_templ);
        if (ves) {
          gcc_assert(GET_CODE(PATTERN(insn)) == GET_CODE(PATTERN(orig_insn)));
  ...
  if (opt_info->insns_with_var_to_expand) {
    ...
    htab_traverse(opt_info->insns_with_var_to_expand,
                  combine_var_copies_in_loop_exit, opt_info->loop_exit);
  }
...
static int release_var_copies(void **slot, void *data ATTRIBUTE_UNUSED) {
  struct var_to_expand *ve = (struct var_to_expand *)*slot;
...
static void free_opt_info(struct opt_info *opt_info) {
  ...
  if (opt_info->insns_with_var_to_expand) {
    htab_traverse(opt_info->insns_with_var_to_expand, release_var_copies, NULL);
