...
static unsigned int lower_function_body(void) {
  ...
  if (gimple_seq_may_fallthru(lowered_body) &&
      (VEC_empty(return_statements_t, data.return_statements) ||
       gimple_return_retval(
           VEC_last(return_statements_t, data.return_statements)->stmt) !=
           NULL)) {
  ...
  while (!VEC_empty(return_statements_t, data.return_statements)) {
    ...
    VEC_truncate(return_statements_t, data.return_statements,
                 VEC_length(return_statements_t, data.return_statements) - 1);
  ...
  BLOCK_SUBBLOCKS(data.block) = blocks_nreverse(BLOCK_SUBBLOCKS(data.block));
...
static void check_call_args(gimple stmt) {
  tree fndecl, parms, p;
  ...
  if (fndecl && DECL_ARGUMENTS(fndecl)) {
    for (i = 0, p = DECL_ARGUMENTS(fndecl); i < nargs; i++, p = TREE_CHAIN(p)) {
      /* We cannot distinguish a varargs function from the case
         of excess parameters, still deferring the inlining decision
         to the callee is possible.  */
      if (!p)
        break;
      if (p == error_mark_node || gimple_call_arg(stmt, i) == error_mark_node ||
          !fold_convertible_p(DECL_ARG_TYPE(p), gimple_call_arg(stmt, i))) {
    ...
    for (i = 0, p = parms; i < nargs; i++, p = TREE_CHAIN(p)) {
      ...
      if (TREE_VALUE(p) == error_mark_node ||
          ...
          !fold_convertible_p(TREE_VALUE(p), gimple_call_arg(stmt, i))) {
        gimple_call_set_cannot_inline(stmt, true);
        break;
      }
    }
...
static void lower_stmt(gimple_stmt_iterator *gsi, struct lower_data *data) {
...
static void lower_gimple_bind(gimple_stmt_iterator *gsi,
  ...
  if (new_block) {
    ...
    BLOCK_SUBBLOCKS(new_block) = blocks_nreverse(BLOCK_SUBBLOCKS(new_block));
...
static void lower_gimple_return(gimple_stmt_iterator *gsi,
                                struct lower_data *data) {
  gimple stmt = gsi_stmt(*gsi);
  gimple t;
  int i;
  ...
  for (i = VEC_length(return_statements_t, data->return_statements) - 1; i >= 0;
       i--) {
...
static void lower_builtin_setjmp(gimple_stmt_iterator *gsi) {
  ...
  if (dest) {
    g = gimple_build_assign(dest,
                            fold_convert(TREE_TYPE(dest), integer_zero_node));
    gimple_set_location(g, gimple_location(stmt));
    gimple_set_block(g, gimple_block(stmt));
    gsi_insert_before(gsi, g, GSI_SAME_STMT);
  }
  ...
  if (dest) {
    g = gimple_build_assign(dest,
                            fold_convert(TREE_TYPE(dest), integer_one_node));
    gimple_set_location(g, gimple_location(stmt));
    gimple_set_block(g, gimple_block(stmt));
    gsi_insert_before(gsi, g, GSI_SAME_STMT);
  }
...
static void mark_blocks_with_used_vars(tree block) {
  tree var;
  tree subblock;
...
static unsigned int mark_used_blocks(void) {
  mark_blocks_with_used_vars(DECL_INITIAL(current_function_decl));
  return 0;
}

struct gimple_opt_pass pass_mark_used_blocks = {{
    GIMPLE_PASS, "blocks", /* name */
    NULL,                  /* gate */
