...
void init_flow(struct function *the_fun) {
  ...
  ENTRY_BLOCK_PTR_FOR_FUNCTION(the_fun) = GGC_CNEW(struct basic_block_def);
  ENTRY_BLOCK_PTR_FOR_FUNCTION(the_fun)->index = ENTRY_BLOCK;
  EXIT_BLOCK_PTR_FOR_FUNCTION(the_fun) = GGC_CNEW(struct basic_block_def);
...
static inline void connect_src(edge e) {
...
static inline void connect_dest(edge e) {
...
static inline void disconnect_src(edge e) {
...
static inline void disconnect_dest(edge e) {
  basic_block dest = e->dest;
...
edge cached_make_edge(sbitmap edge_cache, basic_block src, basic_block dst,
                      int flags) {
  if (edge_cache == NULL || src == ENTRY_BLOCK_PTR || dst == EXIT_BLOCK_PTR)
...
void check_bb_profile(basic_block bb, FILE *file) {
  ...
  if (bb != ENTRY_BLOCK_PTR) {
    ...
    if (abs(sum - bb->frequency) > 100)
      fprintf(file, "Invalid sum of incoming frequencies %i, should be %i\n",
              sum, bb->frequency);
    lsum = 0;
    FOR_EACH_EDGE(e, ei, bb->preds)
    lsum += e->count;
...
void dump_bb_info(basic_block bb, bool header, bool footer, int flags,
  ...
  if (header) {
    ...
    if ((flags & TDF_DETAILS) && (bb->flags & BB_RTL) && df) {
      fprintf(file, "\n");
      df_dump_top(bb, file);
    }
  }
...
void dump_reg_info(FILE *file) {
  ...
  for (i = FIRST_PSEUDO_REGISTER; i < max; i++) {
    ...
    if (regstat_n_sets_and_refs)
      fprintf(file, "\nRegister %d used %d times across %d insns", i,
              REG_N_REFS(i), REG_LIVE_LENGTH(i));
    else if (df)
      fprintf(file, "\nRegister %d used %d times across %d insns", i,
              DF_REG_USE_COUNT(i) + DF_REG_DEF_COUNT(i), REG_LIVE_LENGTH(i));
    ...
    if (REG_FREQ_CALLS_CROSSED(i))
      fprintf(file, "; crosses call with %d frequency",
              REG_FREQ_CALLS_CROSSED(i));
    if (regno_reg_rtx[i] != NULL && PSEUDO_REGNO_BYTES(i) != UNITS_PER_WORD)
...
void dump_flow_info(FILE *file, int flags) {
  basic_block bb;
...
void dump_edge_info(FILE *file, edge e, int do_succ) {
  ...
  if (e->flags) {
    static const char *const bitnames[] = {
        "fallthru", "ab",           "abcall",      "eh",      "fake",
        "dfs_back", "can_fallthru", "irreducible", "sibcall", "loop_exit",
        "true",     "false",        "exec"};
    int comma = 0;
    int i, flags = e->flags;
...
inline void alloc_aux_for_block(basic_block bb, int size) {
...
inline void alloc_aux_for_edge(edge e, int size) {
  /* Verify that aux field is clear.  */
...
/* An edge originally destinating BB of FREQUENCY and COUNT has been proved to
   ...
   respectively.  */
void update_bb_profile_for_threading(basic_block bb, int edge_frequency,
...
void scale_bbs_frequencies_int(basic_block *bbs, int nbbs, int num, int den) {
  int i;
  edge e;
  if (num < 0)
    num = 0;
...
void scale_bbs_frequencies_gcov_type(basic_block *bbs, int nbbs, gcov_type num,
                                     gcov_type den) {
  int i;
  edge e;
...
static hashval_t bb_copy_original_hash(const void *p) {
  const struct htab_bb_copy_original_entry *data =
      ((const struct htab_bb_copy_original_entry *)p);

  return data->index1;
}
static int bb_copy_original_eq(const void *p, const void *q) {
  const struct htab_bb_copy_original_entry *data =
      ((const struct htab_bb_copy_original_entry *)p);
...
void initialize_original_copy_tables(void) {
  ...
  original_copy_bb_pool = create_alloc_pool(
      "original_copy", sizeof(struct htab_bb_copy_original_entry), 10);
  bb_original =
      htab_create(10, bb_copy_original_hash, bb_copy_original_eq, NULL);
...
static void copy_original_table_clear(htab_t tab, unsigned obj) {
  void **slot;
...
static void copy_original_table_set(htab_t tab, unsigned obj, unsigned val) {
  ...
  key.index1 = obj;
  slot =
      (struct htab_bb_copy_original_entry **)htab_find_slot(tab, &key, INSERT);
  if (!*slot) {
    *slot =
        (struct htab_bb_copy_original_entry *)pool_alloc(original_copy_bb_pool);
    (*slot)->index1 = obj;
  }
  (*slot)->index2 = val;
}
...
void set_loop_copy(struct loop *loop, struct loop *copy) {
  if (!copy)
...
struct loop *get_loop_copy(struct loop *loop) {
  struct htab_bb_copy_original_entry *entry;
  struct htab_bb_copy_original_entry key;
