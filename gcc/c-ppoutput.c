...
static void cb_read_pch(cpp_reader *pfile, const char *name, int fd,
...
/* Writes out the preprocessed file, handling spacing and paste
   avoidance issues.  */
static void scan_translation_unit(cpp_reader *pfile) {
  ...
  for (;;) {
    ...
    if (token->type == CPP_PADDING) {
      ...
      if (print.source == NULL ||
          (!(print.source->flags & PREV_WHITE) && token->val.source == NULL))
...
static void print_lines_directives_only(int lines, const void *buf,
                                        size_t size) {
...
/* Writes out the preprocessed file, handling spacing and paste
   avoidance issues.  */
static void scan_translation_unit_directives_only(cpp_reader *pfile) {
...
static void scan_translation_unit_trad(cpp_reader *pfile) {
...
static void print_line(source_location src_loc, const char *special_flags) {
  ...
  if (!flag_no_line_commands) {
    ...
    /* cpp_quote_string does not nul-terminate, so we have to do it
       ourselves.  */
    p = cpp_quote_string(to_file_quoted, (const unsigned char *)map->to_file,
                         to_file_len);
    *p = '\0';
    fprintf(print.outf, "# %u \"%s\"%s",
            print.src_line == 0 ? 1 : print.src_line, to_file_quoted,
            special_flags);
...
static void cb_line_change(cpp_reader *pfile, const cpp_token *token,
                           int parsing_args) {
...
static void cb_ident(cpp_reader *pfile ATTRIBUTE_UNUSED, source_location line,
...
static void cb_define(cpp_reader *pfile, source_location line,
                      cpp_hashnode *node) {
  maybe_print_line(line);
...
static void cb_undef(cpp_reader *pfile ATTRIBUTE_UNUSED, source_location line,
                     cpp_hashnode *node) {
  maybe_print_line(line);
...
static void cb_used_define(cpp_reader *pfile,
...
static void cb_used_undef(cpp_reader *pfile ATTRIBUTE_UNUSED,
                          source_location line ATTRIBUTE_UNUSED,
                          cpp_hashnode *node) {
  macro_queue *q;
  q = XNEW(macro_queue);
...
static void dump_queued_macros(cpp_reader *pfile ATTRIBUTE_UNUSED) {
  macro_queue *q;
...
static void cb_include(cpp_reader *pfile ATTRIBUTE_UNUSED, source_location line,
                       const unsigned char *dir, const char *header,
                       int angle_brackets, const cpp_token **comments) {
  maybe_print_line(line);
  if (angle_brackets)
...
void pp_dir_change(cpp_reader *pfile ATTRIBUTE_UNUSED, const char *dir) {
  ...
  unsigned char *to_file_quoted = (unsigned char *)alloca(to_file_len * 4 + 1);
  unsigned char *p;
...
static void cb_def_pragma(cpp_reader *pfile, source_location line) {
  maybe_print_line(line);
  fputs("#pragma ", print.outf);
...
static int dump_macro(cpp_reader *pfile, cpp_hashnode *node,
