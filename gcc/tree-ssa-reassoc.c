...
static inline long find_operand_rank(tree e) {
...
static inline void insert_operand_rank(tree e, long rank) {
  void **slot;
  gcc_assert(rank > 0);
...
static long get_rank(tree e) {
  ...
  if (TREE_CODE(e) == SSA_NAME) {
    ...
    if (TREE_CODE(SSA_NAME_VAR(e)) == PARM_DECL && SSA_NAME_IS_DEFAULT_DEF(e))
    ...
    if (gimple_assign_single_p(stmt)) {
      ...
      if (n == 0)
        ...
        for (i = 0; i < n && TREE_OPERAND(rhs, i) && rank != maxrank; i++)
...
static inline int constant_type(tree t) {
...
static int sort_by_operand_rank(const void *pa, const void *pb) {
  ...
  if ((oeb->rank - oea->rank == 0) && TREE_CODE(oea->op) == SSA_NAME &&
...
static void add_to_ops_vec(VEC(operand_entry_t, heap) * *ops, tree op) {
...
static bool is_reassociable_op(gimple stmt, enum tree_code code,
                               struct loop *loop) {
  ...
  if (is_gimple_assign(stmt) && gimple_assign_rhs_code(stmt) == code &&
    ...
    switch (opcode) {
      ...
      case BIT_XOR_EXPR:
        ...
        if (VEC_length(operand_entry_t, *ops) == 2) {
          ...
          *ops = NULL;
          add_to_ops_vec(ops,
                         fold_convert(TREE_TYPE(last->op), integer_zero_node));
          *all_done = true;
        } else {
...
static bool eliminate_plus_minus_pair(enum tree_code opcode,
  ...
  for (i = currindex + 1;
    ...
    if (oe->op == negateop) {
      ...
      VEC_ordered_remove(operand_entry_t, *ops, i);
      add_to_ops_vec(ops, fold_convert(TREE_TYPE(oe->op), integer_zero_node));
...
static bool eliminate_not_pairs(enum tree_code opcode,
                                VEC(operand_entry_t, heap) * *ops,
                                unsigned int currindex, operand_entry_t curr) {
  tree notop;
  unsigned int i;
  operand_entry_t oe;
...
static void eliminate_using_constants(enum tree_code opcode,
  ...
  if (oelast->rank == 0 && (INTEGRAL_TYPE_P(type) || FLOAT_TYPE_P(type))) {
    switch (opcode) {
      case BIT_AND_EXPR:
      ...
      case MULT_EXPR:
        if (integer_zerop(oelast->op) ||
            (FLOAT_TYPE_P(type) && !HONOR_NANS(TYPE_MODE(type)) &&
             !HONOR_SIGNED_ZEROS(TYPE_MODE(type)) && real_zerop(oelast->op))) {
          if (VEC_length(operand_entry_t, *ops) != 1) {
            if (dump_file && (dump_flags & TDF_DETAILS))
              fprintf(dump_file, "Found * 0, removing all other ops\n");
            ...
            *ops = NULL;
            ...
            return;
                   ...
                   (FLOAT_TYPE_P(type) && !HONOR_SNANS(TYPE_MODE(type)) &&
                    real_onep(oelast->op))) {
          if (VEC_length(operand_entry_t, *ops) != 1) {
            if (dump_file && (dump_flags & TDF_DETAILS))
              fprintf(dump_file, "Found * 1, removing\n");
...
static void linearize_expr_tree(VEC(operand_entry_t, heap) **, gimple, bool,
                                bool);
...
static hashval_t oecount_hash(const void *p) {
...
static int oecount_eq(const void *p1, const void *p2) {
...
static void zero_one_operation(tree *def, enum tree_code opcode, tree op) {
  ...
  do {
    ...
    /* If this is the operation we look for and one of the operands
       ...
       single use.  */
    if (gimple_assign_rhs_code(stmt) == opcode &&
        (name == op || gimple_assign_rhs2(stmt) == op)) {
      gimple use_stmt;
      use_operand_p use;
    ...
    gcc_assert(name != op && TREE_CODE(name) == SSA_NAME);
...
static gimple build_and_add_sum(tree tmpvar, tree op1, tree op2,
  ...
  if ((!op1def || gimple_nop_p(op1def)) && (!op2def || gimple_nop_p(op2def))) {
...
static bool undistribute_ops_list(enum tree_code opcode,
  ...
  for (i = 0; VEC_iterate(operand_entry_t, *ops, i, oe1); ++i) {
    ...
    if ((dcode != MULT_EXPR && dcode != RDIV_EXPR) ||
  ...
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    ...
    print_generic_expr(
        dump_file,
        VEC_index(operand_entry_t, *ops, sbitmap_first_set_bit(candidates))->op,
        0);
    fprintf(dump_file, " %d\n", nr_candidates);
  }
  ...
  subops =
      XCNEWVEC(VEC(operand_entry_t, heap) *, VEC_length(operand_entry_t, *ops));
  EXECUTE_IF_SET_IN_SBITMAP(candidates, 0, i, sbi0) {
    ...
    linearize_expr_tree(&subops[i], oedef, associative_tree_code(oecode),
                        false);
  ...
  qsort(VEC_address(oecount, cvec), VEC_length(oecount, cvec), sizeof(oecount),
        oecount_cmp);
...
static void optimize_ops_list(enum tree_code opcode,
  ...
  if (oelast->rank == 0 && is_gimple_min_invariant(oelast->op)) {
    ...
    if (oelm1->rank == 0 && is_gimple_min_invariant(oelm1->op) &&
        useless_type_conversion_p(TREE_TYPE(oelm1->op),
                                  TREE_TYPE(oelast->op))) {
      tree folded =
          fold_binary(opcode, TREE_TYPE(oelm1->op), oelm1->op, oelast->op);
...
static bool is_phi_for_stmt(gimple stmt, tree operand) {
  gimple def_stmt;
  tree lhs;
  use_operand_p arg_p;
  ssa_op_iter i;
...
static void rewrite_expr_tree(gimple stmt, unsigned int opindex,
  ...
  if (opindex + 3 == VEC_length(operand_entry_t, ops)) {
    ...
    if ((oe1->rank == oe2->rank && oe2->rank != oe3->rank) ||
        (is_phi_for_stmt(stmt, oe3->op) && !is_phi_for_stmt(stmt, oe1->op) &&
...
static gimple get_single_immediate_use(tree lhs) {
  ...
  if (TREE_CODE(lhs) == SSA_NAME && single_imm_use(lhs, &immuse, &immusestmt) &&
...
static tree negate_value(tree tonegate, gimple_stmt_iterator *gsi) {
  ...
  if (TREE_CODE(tonegate) == SSA_NAME && is_gimple_assign(negatedefstmt) &&
  ...
  resultofnegate = force_gimple_operand_gsi(gsi, tonegate, true, NULL_TREE,
                                            true, GSI_SAME_STMT);
...
static void linearize_expr_tree(VEC(operand_entry_t, heap) * *ops, gimple stmt,
  ...
  gcc_assert(TREE_CODE(binrhs) != SSA_NAME ||
             !is_reassociable_op(SSA_NAME_DEF_STMT(binrhs), rhscode, loop));
  linearize_expr_tree(ops, SSA_NAME_DEF_STMT(binlhs), is_associative,
                      set_visited);
  add_to_ops_vec(ops, binrhs);
}
  ...
  for (i = 0; VEC_iterate(tree, broken_up_subtracts, i, negate); i++) {
    ...
    if (user && is_gimple_assign(user) &&
      ...
      if (gimple_assign_rhs1(user) == negate) {
        swap_tree_operands(user, gimple_assign_rhs1_ptr(user),
...
static void break_up_subtract_bb(basic_block bb) {
  ...
  for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    if (is_gimple_assign(stmt) && gimple_assign_rhs_code(stmt) == MINUS_EXPR) {
      tree lhs = gimple_assign_lhs(stmt);
      tree rhs1 = gimple_assign_rhs1(stmt);
      tree rhs2 = gimple_assign_rhs2(stmt);
      ...
      if ((!INTEGRAL_TYPE_P(TREE_TYPE(lhs)) ||
          ...
          (!SCALAR_FLOAT_TYPE_P(TREE_TYPE(lhs)) ||
           ...
           !SCALAR_FLOAT_TYPE_P(TREE_TYPE(rhs2)) || !flag_associative_math) &&
...
static void reassociate_bb(basic_block bb) {
  gimple_stmt_iterator gsi;
  basic_block son;
    ...
    if (is_gimple_assign(stmt)) {
      ...
      if (associative_tree_code(rhs_code)) {
        ...
        qsort(VEC_address(operand_entry_t, ops),
              VEC_length(operand_entry_t, ops), sizeof(operand_entry_t),
              sort_by_operand_rank);
        optimize_ops_list(rhs_code, &ops);
        if (undistribute_ops_list(rhs_code, &ops, loop_containing_stmt(stmt))) {
          qsort(VEC_address(operand_entry_t, ops),
                VEC_length(operand_entry_t, ops), sizeof(operand_entry_t),
                sort_by_operand_rank);
          optimize_ops_list(rhs_code, &ops);
  ...
  for (son = first_dom_son(CDI_POST_DOMINATORS, bb); son;
...
static void init_reassoc(void) {
  ...
  operand_entry_pool =
      create_alloc_pool("operand entry pool", sizeof(struct operand_entry), 30);
...
static void fini_reassoc(void) {
  statistics_counter_event(cfun, "Linearized", reassociate_stats.linearized);
...
static unsigned int execute_reassoc(void) {
  init_reassoc();
...
static bool gate_tree_ssa_reassoc(void) {
  return flag_tree_reassoc != 0;
}

struct gimple_opt_pass pass_reassoc = {{
    GIMPLE_PASS, "reassoc",           /* name */
