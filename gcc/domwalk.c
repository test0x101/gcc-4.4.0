...
void walk_dominator_tree(struct dom_walk_data *walk_data, basic_block bb) {
  void *bd = NULL;
  basic_block dest;
  ...
  while (true) {
    ...
    if (EDGE_COUNT(bb->preds) > 0 || bb == ENTRY_BLOCK_PTR ||
      ...
      if (is_interesting && walk_data->before_dom_children_walk_stmts) {
        if (walk_data->walk_stmts_backward)
          for (gsi = gsi_last(bb_seq(bb)); !gsi_end_p(gsi); gsi_prev(&gsi))
            (*walk_data->before_dom_children_walk_stmts)(walk_data, bb, gsi);
        else
      ...
      for (dest = first_dom_son(walk_data->dom_direction, bb); dest;
    ...
    while (sp > 0 && !worklist[sp - 1]) {
      ...
      if (is_interesting && walk_data->after_dom_children_walk_stmts) {
        if (walk_data->walk_stmts_backward)
          for (gsi = gsi_last(bb_seq(bb)); !gsi_end_p(gsi); gsi_prev(&gsi))
            (*walk_data->after_dom_children_walk_stmts)(walk_data, bb, gsi);
        else
          for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi))
            (*walk_data->after_dom_children_walk_stmts)(walk_data, bb, gsi);
      }
...
void init_walk_dominator_tree(struct dom_walk_data *walk_data) {
  walk_data->free_block_data = NULL;
  walk_data->block_data_stack = NULL;
}

void fini_walk_dominator_tree(struct dom_walk_data *walk_data) {
  if (walk_data->initialize_block_local_data) {
