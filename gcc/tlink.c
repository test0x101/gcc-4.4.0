...
static int hash_string_eq(const void *s1_p, const void *s2_p) {
  const char *const *s1 = (const char *const *)s1_p;
  const char *s2 = (const char *)s2_p;
  return strcmp(*s1, s2) == 0;
}

static hashval_t hash_string_hash(const void *s_p) {
...
static struct file_hash_entry *file_hash_lookup(const char *string) {
  void **e;
  e = htab_find_slot_with_hash(file_table, string, (*htab_hash_string)(string),
                               INSERT);
  if (*e == NULL) {
...
static void symbol_push(symbol *p) {
...
static int tlink_execute(const char *prog, char **argv, const char *outname,
                         const char *errname) {
  struct pex_obj *pex;
...
static char *frob_extension(const char *s, const char *ext) {
...
static char *obstack_fgets(FILE *stream, struct obstack *ob) {
  int c;
...
static char *tfgets(FILE *stream) {
  return obstack_fgets(stream, &temporary_obstack);
}

static char *pfgets(FILE *stream) {
...
static void freadsym(FILE *stream, file *f, int chosen) {
  symbol *sym;
...
static void read_repo_file(file *f) {
  char c;
...
static void maybe_tweak(char *line, file *f) {
...
static int recompile_files(void) {
  ...
  while ((f = file_pop()) != NULL) {
    ...
    if (!f->args) {
      error(
          "repository file '%s' does not contain command-line "
          "arguments",
          f->key);
      return 0;
    }
...
static int read_repo_files(char **object_lst) {
...
static int scan_linker_output(const char *fname) {
  ...
  while ((line = tfgets(stream)) != NULL) {
    ...
    if (skip_next_in_line && strstr(p, " in "))
      continue;
    skip_next_in_line = 0;
    ...
    *q = 0;
    ...
    /* Try a mangled name in quotes.  */
    {
      ...
      if (q && (ok || strstr(oldq, "ndefined") || strstr(oldq, "nresolved") ||
                strstr(oldq, "nsatisfied") || strstr(oldq, "ultiple"))) {
        *q = 0;
    ...
    if (sym && sym->tweaked) {
      error(
          "'%s' was assigned to '%s', but was not defined "
  ...
  if (exit) {
    ...
    if (read_repo_files(ld_argv))
      while (exit && i++ < MAX_ITERATIONS) {
        ...
        if (!scan_linker_output(ldout) && !scan_linker_output(lderrout))
          break;
        if (!recompile_files())
          break;
        if (tlink_verbose)
