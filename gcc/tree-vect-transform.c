...
static tree vect_create_data_ref_ptr(gimple, struct loop *, tree, tree *,
                                     gimple *, bool, bool *, tree);
static tree vect_create_addr_base_for_vector_ref(gimple, gimple_seq *, tree,
                                                 struct loop *);
...
static void vect_finish_stmt_generation(gimple stmt, gimple vec_stmt,
                                        gimple_stmt_iterator *);
static bool vect_is_simple_cond(tree, loop_vec_info);
static void vect_create_epilog_for_reduction(tree, gimple, int, enum tree_code,
                                             gimple);
static tree get_initial_def_for_reduction(gimple, tree, tree *);

/* Utility function dealing with loop peeling (not peeling itself).  */
static void vect_generate_tmps_on_preheader(loop_vec_info, tree *, tree *,
                                            tree *);
...
static int cost_for_stmt(gimple stmt) {
...
int vect_estimate_min_profitable_iters(loop_vec_info loop_vinfo) {
  int i;
  ...
  if (byte_misalign < 0) {
    ...
    peel_guard_costs +=
        2 * (TARG_COND_TAKEN_BRANCH_COST + TARG_COND_NOT_TAKEN_BRANCH_COST);
  } else {
    if (byte_misalign) {
  ...
  if ((scalar_single_iter_cost * vf) > vec_inside_cost) {
    if (vec_outside_cost <= 0)
      ...
      min_profitable_iters = ((vec_outside_cost - scalar_outside_cost) * vf -
                             ...
                             ((scalar_single_iter_cost * vf) - vec_inside_cost);
...
static bool vect_model_reduction_cost(stmt_vec_info stmt_info,
                                      enum tree_code reduc_code, int ncopies) {
  int outer_cost = 0;
  enum tree_code code;
  optab optab;
  tree vectype;
  ...
  if (!nested_in_vect_loop_p(loop, orig_stmt)) {
    if (reduc_code < NUM_TREE_CODES)
      ...
      tree bitsize = TYPE_SIZE(TREE_TYPE(gimple_assign_lhs(orig_stmt)));
...
static void vect_model_induction_cost(stmt_vec_info stmt_info, int ncopies) {
...
void vect_model_simple_cost(stmt_vec_info stmt_info, int ncopies,
...
static int vect_cost_strided_group_size(stmt_vec_info stmt_info) {
...
void vect_model_store_cost(stmt_vec_info stmt_info, int ncopies,
                           enum vect_def_type dt, slp_tree slp_node) {
  int group_size;
  int inside_cost = 0, outside_cost = 0;
  ...
  if (group_size > 1) {
    ...
    inside_cost =
        ncopies * exact_log2(group_size) * group_size * TARG_VEC_STMT_COST;
...
void vect_model_load_cost(stmt_vec_info stmt_info, int ncopies,
                          slp_tree slp_node)
...
static tree vect_get_new_vect_var(tree type, enum vect_var_kind var_kind,
                                  const char *name) {
  const char *prefix;
  tree new_vect_var;
...
static tree vect_create_addr_base_for_vector_ref(gimple stmt,
  ...
  data_ref_base =
      force_gimple_operand(data_ref_base, &seq, true, data_ref_base_var);
  gimple_seq_add_seq(new_stmt_list, seq);

  /* Create base_offset */
  base_offset = size_binop(PLUS_EXPR, fold_convert(sizetype, base_offset),
  ...
  if (offset) {
    ...
    offset =
        fold_build2(MULT_EXPR, sizetype, fold_convert(sizetype, offset), step);
    base_offset = fold_build2(PLUS_EXPR, sizetype, base_offset, offset);
...
static tree vect_create_data_ref_ptr(gimple stmt, struct loop *at_loop,
                                     tree offset, tree *initial_address,
                                     gimple *ptr_incr, bool only_init,
                                     bool *inv_p, tree type) {
  tree base_name;
  stmt_vec_info stmt_info = vinfo_for_stmt(stmt);
  loop_vec_info loop_vinfo = STMT_VINFO_LOOP_VINFO(stmt_info);
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  ...
  *initial_address = new_temp;
  ...
  vec_stmt =
      gimple_build_assign(vect_ptr, fold_convert(vect_ptr_type, new_temp));
  ...
  if (only_init && at_loop == loop) /* No update in loop is required.  */
  {
    ...
    create_iv(vect_ptr_init, fold_convert(vect_ptr_type, step), vect_ptr, loop,
  ...
  if (!only_init) {
    standard_iv_increment_position(containing_loop, &incr_gsi, &insert_after);
...
static tree bump_vector_ptr(tree dataref_ptr, gimple ptr_incr,
                            gimple_stmt_iterator *gsi, gimple stmt, tree bump) {
  stmt_vec_info stmt_info = vinfo_for_stmt(stmt);
  struct data_reference *dr = STMT_VINFO_DATA_REF(stmt_info);
  tree vectype = STMT_VINFO_VECTYPE(stmt_info);
...
static tree vect_create_destination_var(tree scalar_dest, tree vectype) {
  tree vec_dest;
  const char *new_name;
  tree type;
...
static tree vect_init_vector(gimple stmt, tree vector_var, tree vector_type,
...
static void vect_get_constant_vectors(slp_tree slp_node,
...
static void vect_get_slp_vect_defs(slp_tree slp_node,
                                   VEC(tree, heap) * *vec_oprnds) {
  tree vec_oprnd;
  gimple vec_def_stmt;
  unsigned int i;
...
static void vect_get_slp_defs(slp_tree slp_node, VEC(tree, heap) * *vec_oprnds0,
  ...
  if (SLP_TREE_LEFT(slp_node))
    ...
    vect_get_smallest_scalar_type(first_stmt, &lhs_size_unit, &rhs_size_unit);
...
static tree get_initial_def_for_induction(gimple iv_phi) {
  ...
  ok = vect_is_simple_iv_evolution(iv_loop->num, access_fn, &init_expr,
                                   &step_expr);
  gcc_assert(ok);
  ...
  if (nested_in_vect_loop) {
    ...
    for (i = 1; i < nunits; i++) {
      ...
      enum tree_code code =
          POINTER_TYPE_P(scalar_type) ? POINTER_PLUS_EXPR : PLUS_EXPR;
      init_stmt =
          gimple_build_assign_with_ops(code, new_var, new_name, step_expr);
  ...
  new_stmt =
      gimple_build_assign_with_ops(PLUS_EXPR, vec_dest, induc_def, vec_step);
  ...
  if (ncopies > 1) {
    ...
    for (i = 1; i < ncopies; i++) {
      ...
      new_stmt =
          gimple_build_assign_with_ops(PLUS_EXPR, vec_dest, vec_def, vec_step);
      vec_def = make_ssa_name(vec_dest, new_stmt);
      gimple_assign_set_lhs(new_stmt, vec_def);
...
static tree vect_get_vec_def_for_operand(tree op, gimple stmt,
                                         tree *scalar_def) {
  tree vec_oprnd;
  gimple vec_stmt;
  gimple def_stmt;
...
static tree vect_get_vec_def_for_stmt_copy(enum vect_def_type dt,
                                           tree vec_oprnd) {
...
static void vect_get_vec_defs(tree op0, tree op1, gimple stmt,
                              VEC(tree, heap) * *vec_oprnds0,
                              VEC(tree, heap) * *vec_oprnds1,
                              slp_tree slp_node) {
  if (slp_node)
...
static tree get_initial_def_for_reduction(gimple stmt, tree init_val,
                                          tree *adjustment_def) {
  stmt_vec_info stmt_vinfo = vinfo_for_stmt(stmt);
  loop_vec_info loop_vinfo = STMT_VINFO_LOOP_VINFO(stmt_vinfo);
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
...
static void vect_create_epilog_for_reduction(tree vect_def, gimple stmt,
                                             int ncopies,
  ...
  vec_initial_def =
      vect_get_vec_def_for_operand(reduction_op, stmt, &adjustment_def);
  ...
  if (reduc_code < NUM_TREE_CODES) {
    ...
    if (have_whole_vector_shift) {
      ...
      for (bit_offset = vec_size_in_bits / 2; bit_offset >= element_bitsize;
           bit_offset /= 2) {
        ...
        epilog_stmt =
            gimple_build_assign_with_ops(code, vec_dest, new_name, new_temp);
        new_temp = make_ssa_name(vec_dest, epilog_stmt);
        gimple_assign_set_lhs(epilog_stmt, new_temp);
        gsi_insert_before(&exit_gsi, epilog_stmt, GSI_SAME_STMT);
      }
      ...
      for (bit_offset = element_bitsize; bit_offset < vec_size_in_bits;
        ...
        tree rhs =
            build3(BIT_FIELD_REF, scalar_type, vec_temp, bitsize, bitpos);
  ...
  if (extract_scalar_result) {
    ...
    if (BYTES_BIG_ENDIAN)
      bitpos =
          size_binop(MULT_EXPR, bitsize_int(TYPE_VECTOR_SUBPARTS(vectype) - 1),
...
bool vectorizable_reduction(gimple stmt, gimple_stmt_iterator *gsi,
                            gimple *vec_stmt) {
  tree vec_dest;
  tree scalar_dest;
  ...
  for (i = 0; i < op_type - 1; i++) {
    is_simple_use =
        vect_is_simple_use(ops[i], loop_vinfo, &def_stmt, &def, &dt);
    gcc_assert(is_simple_use);
    if (dt != vect_loop_def && dt != vect_invariant_def &&
...
tree vectorizable_function(gimple call, tree vectype_out, tree vectype_in) {
...
bool vectorizable_call(gimple stmt, gimple_stmt_iterator *gsi,
                       gimple *vec_stmt) {
  tree vec_dest;
  tree scalar_dest;
  tree op, type;
  ...
  for (i = 0; i < nargs; i++) {
    ...
    if (rhs_type && rhs_type != TREE_TYPE(op)) {
      if (vect_print_dump_info(REPORT_DETAILS))
    ...
    case NONE:
      for (j = 0; j < ncopies; ++j) {
        ...
        for (i = 0; i < nargs; i++) {
          op = gimple_call_arg(stmt, i);
          if (j == 0)
            vec_oprnd0 = vect_get_vec_def_for_operand(op, stmt, NULL);
          else
            vec_oprnd0 = vect_get_vec_def_for_stmt_copy(dt[nargs], vec_oprnd0);
    ...
    case NARROW:
      for (j = 0; j < ncopies; ++j) {
        ...
        for (i = 0; i < nargs; i++) {
          ...
          if (j == 0) {
            vec_oprnd0 = vect_get_vec_def_for_operand(op, stmt, NULL);
            vec_oprnd1 = vect_get_vec_def_for_stmt_copy(dt[nargs], vec_oprnd0);
          } else {
            vec_oprnd0 = vect_get_vec_def_for_stmt_copy(dt[nargs], vec_oprnd1);
            vec_oprnd1 = vect_get_vec_def_for_stmt_copy(dt[nargs], vec_oprnd0);
          }
...
static gimple vect_gen_widened_results_half(enum tree_code code, tree decl,
                                            tree vec_oprnd0, tree vec_oprnd1,
                                            int op_type, tree vec_dest,
                                            gimple_stmt_iterator *gsi,
                                            gimple stmt) {
  gimple new_stmt;
  tree new_temp;
  ...
  if (code == CALL_EXPR) {
    ...
    if (op_type != binary_op)
      vec_oprnd1 = NULL;
    new_stmt =
        gimple_build_assign_with_ops(code, vec_dest, vec_oprnd0, vec_oprnd1);
    new_temp = make_ssa_name(vec_dest, new_stmt);
    gimple_assign_set_lhs(new_stmt, new_temp);
  }
  vect_finish_stmt_generation(stmt, new_stmt, gsi);
...
bool vectorizable_conversion(gimple stmt, gimple_stmt_iterator *gsi,
  ...
  if ((modifier == NONE &&
      ...
      (modifier == WIDEN &&
       !supportable_widening_operation(code, stmt, vectype_in, &decl1, &decl2,
                                       &code1, &code2, &dummy_int, &dummy)) ||
      (modifier == NARROW &&
       !supportable_narrowing_operation(code, stmt, vectype_in, &code1,
                                        &dummy_int, &dummy))) {
    if (vect_print_dump_info(REPORT_DETAILS))
  ...
  switch (modifier) {
    ...
    case WIDEN:
      ...
      for (j = 0; j < ncopies; j++) {
        ...
        new_stmt =
            vect_gen_widened_results_half(code1, decl1, vec_oprnd0, vec_oprnd1,
        ...
        new_stmt =
            vect_gen_widened_results_half(code2, decl2, vec_oprnd0, vec_oprnd1,
                                          unary_op, vec_dest, gsi, stmt);
        STMT_VINFO_RELATED_STMT(prev_stmt_info) = new_stmt;
        prev_stmt_info = vinfo_for_stmt(new_stmt);
      }
...
bool vectorizable_assignment(gimple stmt, gimple_stmt_iterator *gsi,
                             gimple *vec_stmt, slp_tree slp_node) {
  tree vec_dest;
  tree scalar_dest;
  tree op;
...
static int vect_min_worthwhile_factor(enum tree_code code) {
  switch (code) {
    case PLUS_EXPR:
    case MINUS_EXPR:
    case NEGATE_EXPR:
...
bool vectorizable_operation(gimple stmt, gimple_stmt_iterator *gsi,
                            gimple *vec_stmt, slp_tree slp_node) {
  tree vec_dest;
  tree scalar_dest;
  tree op0, op1 = NULL;
  ...
  if (code == LSHIFT_EXPR || code == RSHIFT_EXPR || code == LROTATE_EXPR ||
    ...
    /* See if the machine has a vector shifted by scalar insn and if not
      ...
      if (optab && (optab_handler(optab, TYPE_MODE(vectype))->insn_code !=
                    CODE_FOR_nothing)) {
        ...
        optab = optab_for_tree_code(code, vectype, optab_vector);
        if (vect_print_dump_info(REPORT_DETAILS) && optab &&
  ...
  for (j = 0; j < ncopies; j++) {
    ...
    if (j == 0) {
      ...
      if (op_type == binary_op && !vec_oprnd1)
        vect_get_vec_defs(op0, op1, stmt, &vec_oprnds0, &vec_oprnds1, slp_node);
      else
    ...
    /* Arguments are ready. Create the new vector stmt.  */
    for (i = 0; VEC_iterate(tree, vec_oprnds0, i, vop0); i++) {
      vop1 = ((op_type == binary_op) ? VEC_index(tree, vec_oprnds1, i) : NULL);
...
static void vect_get_loop_based_defs(tree *oprnd, gimple stmt,
                                     enum vect_def_type dt,
                                     VEC(tree, heap) * *vec_oprnds,
                                     int multi_step_cvt) {
  tree vec_oprnd;
...
static void vect_create_vectorized_demotion_stmts(
    VEC(tree, heap) * *vec_oprnds, int multi_step_cvt, gimple stmt,
    VEC(tree, heap) * vec_dsts, gimple_stmt_iterator *gsi, slp_tree slp_node,
    enum tree_code code, stmt_vec_info *prev_stmt_info) {
  unsigned int i;
  ...
  if (multi_step_cvt) {
    ...
    vect_create_vectorized_demotion_stmts(vec_oprnds, multi_step_cvt - 1, stmt,
                                          vec_dsts, gsi, slp_node, code,
                                          prev_stmt_info);
  }
}
...
bool vectorizable_type_demotion(gimple stmt, gimple_stmt_iterator *gsi,
                                gimple *vec_stmt, slp_tree slp_node) {
  tree vec_dest;
  tree scalar_dest;
  tree op0;
  ...
  if (!((INTEGRAL_TYPE_P(TREE_TYPE(scalar_dest)) &&
        ...
        (SCALAR_FLOAT_TYPE_P(TREE_TYPE(scalar_dest)) &&
         SCALAR_FLOAT_TYPE_P(TREE_TYPE(op0)) && CONVERT_EXPR_CODE_P(code))))
    return false;
  ...
  if (multi_step_cvt) {
    for (i = VEC_length(tree, interm_types) - 1;
      ...
      vec_dest = vect_create_destination_var(scalar_dest, intermediate_type);
      VEC_quick_push(tree, vec_dsts, vec_dest);
    }
  }
  ...
  for (j = 0; j < ncopies; j++) {
    ...
    if (slp_node)
      ...
      VEC_free(tree, heap, vec_oprnds0);
      vec_oprnds0 = VEC_alloc(
          tree, heap, (multi_step_cvt ? vect_pow2(multi_step_cvt) * 2 : 2));
    ...
    vect_create_vectorized_demotion_stmts(&vec_oprnds0, multi_step_cvt, stmt,
                                          tmp_vec_dsts, gsi, slp_node, code1,
                                          &prev_stmt_info);
  }
...
static void vect_create_vectorized_promotion_stmts(
    VEC(tree, heap) * *vec_oprnds0, VEC(tree, heap) * *vec_oprnds1,
    int multi_step_cvt, gimple stmt, VEC(tree, heap) * vec_dsts,
    gimple_stmt_iterator *gsi, slp_tree slp_node, enum tree_code code1,
    enum tree_code code2, tree decl1, tree decl2, int op_type,
    stmt_vec_info *prev_stmt_info) {
  int i;
  ...
  for (i = 0; VEC_iterate(tree, *vec_oprnds0, i, vop0); i++) {
    ...
    new_stmt1 = vect_gen_widened_results_half(code1, decl1, vop0, vop1, op_type,
                                              vec_dest, gsi, stmt);
    new_stmt2 = vect_gen_widened_results_half(code2, decl2, vop0, vop1, op_type,
                                              vec_dest, gsi, stmt);
  ...
  if (multi_step_cvt) {
    ...
    *vec_oprnds0 = VEC_copy(tree, heap, vec_tmp);
    ...
    vect_create_vectorized_promotion_stmts(
        vec_oprnds0, vec_oprnds1, multi_step_cvt - 1, stmt, vec_dsts, gsi,
        slp_node, code1, code2, decl2, decl2, op_type, prev_stmt_info);
  }
}
...
bool vectorizable_type_promotion(gimple stmt, gimple_stmt_iterator *gsi,
                                 gimple *vec_stmt, slp_tree slp_node) {
  tree vec_dest;
  tree scalar_dest;
  tree op0, op1 = NULL;
  ...
  /* Supportable by target?  */
  if (!supportable_widening_operation(code, stmt, vectype_in, &decl1, &decl2,
  ...
  if (!slp_node) {
    vec_oprnds0 =
        VEC_alloc(tree, heap, (multi_step_cvt ? vect_pow2(multi_step_cvt) : 1));
    if (op_type == binary_op)
      vec_oprnds1 = VEC_alloc(tree, heap, 1);
  }
  ...
  for (j = 0; j < ncopies; j++) {
    ...
    tmp_vec_dsts = VEC_copy(tree, heap, vec_dsts);
    vect_create_vectorized_promotion_stmts(
        &vec_oprnds0, &vec_oprnds1, multi_step_cvt, stmt, tmp_vec_dsts, gsi,
        slp_node, code1, code2, decl1, decl2, op_type, &prev_stmt_info);
  }
...
static bool vect_strided_store_supported(tree vectype) {
  ...
  interleave_high_optab =
      optab_for_tree_code(VEC_INTERLEAVE_HIGH_EXPR, vectype, optab_default);
  interleave_low_optab =
      optab_for_tree_code(VEC_INTERLEAVE_LOW_EXPR, vectype, optab_default);
...
static bool vect_permute_store_chain(VEC(tree, heap) * dr_chain,
                                     unsigned int length, gimple stmt,
                                     gimple_stmt_iterator *gsi,
  ...
  *result_chain = VEC_copy(tree, heap, dr_chain);
  ...
  for (i = 0; i < exact_log2(length); i++) {
    for (j = 0; j < length / 2; j++) {
      ...
      perm_stmt =
          gimple_build_assign_with_ops(high_code, perm_dest, vect1, vect2);
      ...
      add_referenced_var(perm_dest);
      perm_stmt =
          gimple_build_assign_with_ops(low_code, perm_dest, vect1, vect2);
...
bool vectorizable_store(gimple stmt, gimple_stmt_iterator *gsi,
                        gimple *vec_stmt, slp_tree slp_node) {
  tree scalar_dest;
  tree data_ref;
  tree op;
  ...
  if (STMT_VINFO_STRIDED_ACCESS(stmt_info)) {
    strided_store = true;
    first_stmt = DR_GROUP_FIRST_DR(stmt_info);
    if (!vect_strided_store_supported(vectype) && !PURE_SLP_STMT(stmt_info) &&
        !slp)
      return false;
    ...
    if (j == 0) {
      if (slp) {
        ...
        for (i = 0; i < group_size; i++) {
          ...
          gcc_assert(next_stmt && gimple_assign_single_p(next_stmt));
          op = gimple_assign_rhs1(next_stmt);
      ...
      gcc_assert(useless_type_conversion_p(vectype, TREE_TYPE(vec_oprnd)));
      dataref_ptr = vect_create_data_ref_ptr(
          first_stmt, NULL, NULL_TREE, &dummy, &ptr_incr, false, &inv_p, NULL);
      gcc_assert(!inv_p);
    } else {
...
static tree vect_setup_realignment(
    gimple stmt, gimple_stmt_iterator *gsi, tree *realignment_token,
    enum dr_alignment_support alignment_support_scheme, tree init_addr,
    struct loop **at_loop) {
  stmt_vec_info stmt_info = vinfo_for_stmt(stmt);
  tree vectype = STMT_VINFO_VECTYPE(stmt_info);
  loop_vec_info loop_vinfo = STMT_VINFO_LOOP_VINFO(stmt_info);
...
static bool vect_strided_load_supported(tree vectype) {
  ...
  perm_even_optab =
      optab_for_tree_code(VEC_EXTRACT_EVEN_EXPR, vectype, optab_default);
  if (!perm_even_optab) {
    if (vect_print_dump_info(REPORT_DETAILS))
  ...
  perm_odd_optab =
      optab_for_tree_code(VEC_EXTRACT_ODD_EXPR, vectype, optab_default);
  if (!perm_odd_optab) {
    if (vect_print_dump_info(REPORT_DETAILS))
...
static bool vect_permute_load_chain(VEC(tree, heap) * dr_chain,
                                    unsigned int length, gimple stmt,
                                    gimple_stmt_iterator *gsi,
                                    VEC(tree, heap) * *result_chain) {
  ...
  *result_chain = VEC_copy(tree, heap, dr_chain);
  for (i = 0; i < exact_log2(length); i++) {
    for (j = 0; j < length; j += 2) {
      ...
      perm_stmt = gimple_build_assign_with_ops(VEC_EXTRACT_EVEN_EXPR, perm_dest,
      ...
      perm_stmt = gimple_build_assign_with_ops(VEC_EXTRACT_ODD_EXPR, perm_dest,
                                               first_vect, second_vect);
      data_ref = make_ssa_name(perm_dest, perm_stmt);
      gimple_assign_set_lhs(perm_stmt, data_ref);
      vect_finish_stmt_generation(stmt, perm_stmt, gsi);
...
static bool vect_transform_strided_load(gimple stmt, VEC(tree, heap) * dr_chain,
    ...
    while (next_stmt) {
      ...
      if (!STMT_VINFO_VEC_STMT(vinfo_for_stmt(next_stmt)))
        ...
        if (!DR_GROUP_SAME_DR_STMT(vinfo_for_stmt(next_stmt))) {
          gimple prev_stmt = STMT_VINFO_VEC_STMT(vinfo_for_stmt(next_stmt));
          gimple rel_stmt = STMT_VINFO_RELATED_STMT(vinfo_for_stmt(prev_stmt));
          while (rel_stmt) {
            prev_stmt = rel_stmt;
...
static inline void vect_create_mask_and_perm(
    gimple stmt, gimple next_scalar_stmt, int *mask_array, int mask_nunits,
    tree mask_element_type, tree mask_type, int first_vec_indx,
    int second_vec_indx, gimple_stmt_iterator *gsi, slp_tree node,
    tree builtin_decl, tree vectype, VEC(tree, heap) * dr_chain, int ncopies,
...
static bool vect_get_mask_element(gimple stmt, int first_mask_element, int m,
...
bool vect_transform_slp_perm_load(gimple stmt, VEC(tree, heap) * dr_chain,
  ...
  builtin_decl =
      targetm.vectorize.builtin_vec_perm(vectype, &mask_element_type);
  ...
  /* The number of vector stmts to generate based only on SLP_NODE_INSTANCE
     unrolling factor.  */
  orig_vec_stmts_num =
      group_size * SLP_INSTANCE_UNROLLING_FACTOR(slp_node_instance) / nunits;
  ...
  for (i = 0;
       VEC_iterate(slp_tree, SLP_INSTANCE_LOADS(slp_node_instance), i, node);
       i++) {
    scalar_index = 0;
    index = 0;
    ...
    for (j = 0; j < unroll_factor; j++) {
      for (k = 0; k < group_size; k++) {
        ...
        for (m = 0; m < scale; m++) {
          if (!vect_get_mask_element(stmt, first_mask_element, m, mask_nunits,
                                     only_one_vec, index, mask,
        ...
        if (index == mask_nunits) {
          ...
          if (!analyze_only) {
            ...
            next_scalar_stmt =
                VEC_index(gimple, SLP_TREE_SCALAR_STMTS(node), scalar_index++);

            vect_create_mask_and_perm(
                stmt, next_scalar_stmt, mask, mask_nunits, mask_element_type,
                mask_type, first_vec_index, second_vec_index, gsi, node,
                builtin_decl, vectype, dr_chain, ncopies, vect_stmts_counter++);
          }
        }
      }
    }
...
bool vectorizable_load(gimple stmt, gimple_stmt_iterator *gsi, gimple *vec_stmt,
  ...
  code = gimple_assign_rhs_code(stmt);
  if (code != ARRAY_REF && code != INDIRECT_REF &&
      !STMT_VINFO_STRIDED_ACCESS(stmt_info))
    return false;
  ...
  if (STMT_VINFO_STRIDED_ACCESS(stmt_info)) {
    ...
    if (!vect_strided_load_supported(vectype) && !PURE_SLP_STMT(stmt_info) &&
        !slp)
      return false;
  }
  ...
  if (nested_in_vect_loop_p(loop, stmt) &&
      (TREE_INT_CST_LOW(DR_STEP(dr)) % GET_MODE_SIZE(TYPE_MODE(vectype)) !=
       0)) {
    gcc_assert(alignment_support_scheme != dr_explicit_realign_optimized);
    compute_in_loop = true;
  }
  ...
  if ((alignment_support_scheme == dr_explicit_realign_optimized ||
    ...
    msq = vect_setup_realignment(first_stmt, gsi, &realignment_token,
                                 alignment_support_scheme, NULL_TREE, &at_loop);
    if (alignment_support_scheme == dr_explicit_realign_optimized) {
  ...
  for (j = 0; j < ncopies; j++) {
    ...
    if (j == 0)
      dataref_ptr =
          vect_create_data_ref_ptr(first_stmt, at_loop, offset, &dummy,
                                   &ptr_incr, false, &inv_p, NULL_TREE);
    else
      dataref_ptr =
          bump_vector_ptr(dataref_ptr, ptr_incr, gsi, stmt, NULL_TREE);
    ...
    for (i = 0; i < vec_num; i++) {
      ...
      switch (alignment_support_scheme) {
        ...
        case dr_explicit_realign: {
          ...
          if (compute_in_loop)
            msq =
                vect_setup_realignment(first_stmt, gsi, &realignment_token,
                                       dr_explicit_realign, dataref_ptr, NULL);
          ...
          bump = size_binop(MULT_EXPR, vs_minus_1, TYPE_SIZE_UNIT(scalar_type));
      ...
      if (alignment_support_scheme == dr_explicit_realign_optimized ||
        ...
        vec_dest = vect_create_destination_var(scalar_dest, vectype);
        tmp = build3(REALIGN_LOAD_EXPR, vectype, msq, lsq, realignment_token);
      ...
      if (inv_p) {
        ...
        if (j == 0) {
          ...
          bitpos = bitsize_zero_node;
          vec_inv =
              build3(BIT_FIELD_REF, scalar_type, new_temp, bitsize, bitpos);
...
static bool vect_is_simple_cond(tree cond, loop_vec_info loop_vinfo) {
  tree lhs, rhs;
  tree def;
  enum vect_def_type dt;
...
bool vectorizable_condition(gimple stmt, gimple_stmt_iterator *gsi,
                            gimple *vec_stmt) {
  ...
  if (TREE_CODE(then_clause) == SSA_NAME) {
    ...
    if (!vect_is_simple_use(then_clause, loop_vinfo, &then_def_stmt, &def, &dt))
      return false;
  ...
  if (TREE_CODE(else_clause) == SSA_NAME) {
    ...
    if (!vect_is_simple_use(else_clause, loop_vinfo, &else_def_stmt, &def, &dt))
      return false;
  ...
  /* Arguments are ready. Create the new vector stmt.  */
  vec_compare =
      build2(TREE_CODE(cond_expr), vectype, vec_cond_lhs, vec_cond_rhs);
  vec_cond_expr = build3(VEC_COND_EXPR, vectype, vec_compare, vec_then_clause,
                         vec_else_clause);
...
static bool vect_transform_stmt(gimple stmt, gimple_stmt_iterator *gsi,
  ...
  switch (STMT_VINFO_TYPE(stmt_info)) {
    ...
    case load_vec_info_type:
          ...
          vectorizable_load(stmt, gsi, &vec_stmt, slp_node, slp_node_instance);
      gcc_assert(done);
      break;
  ...
  if (vec_stmt && nested_in_vect_loop_p(loop, stmt) &&
      STMT_VINFO_TYPE(stmt_info) != reduc_vec_info_type &&
...
static tree vect_build_loop_niters(loop_vec_info loop_vinfo) {
  tree ni_name, var;
  gimple_seq stmts = NULL;
  edge pe;
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
...
static void vect_generate_tmps_on_preheader(loop_vec_info loop_vinfo,
  ...
  ratio_mult_vf_name =
      fold_build2(LSHIFT_EXPR, TREE_TYPE(ratio_name), ratio_name, log_vf);
  if (!is_gimple_val(ratio_mult_vf_name)) {
    ...
    stmts = NULL;
    ratio_mult_vf_name =
        force_gimple_operand(ratio_mult_vf_name, &stmts, true, var);
    pe = loop_preheader_edge(loop);
    new_bb = gsi_insert_seq_on_edge_immediate(pe, stmts);
    gcc_assert(!new_bb);
  }
...
static void vect_update_ivs_after_vectorizer(loop_vec_info loop_vinfo,
  ...
  for (gsi = gsi_start_phis(loop->header), gsi1 = gsi_start_phis(update_bb);
       !gsi_end_p(gsi) && !gsi_end_p(gsi1); gsi_next(&gsi), gsi_next(&gsi1)) {
    tree access_fn = NULL;
    tree evolution_part;
    tree init_expr;
    tree step_expr;
    ...
    if (POINTER_TYPE_P(TREE_TYPE(init_expr)))
      ni = fold_build2(
          POINTER_PLUS_EXPR, TREE_TYPE(init_expr), init_expr,
          fold_convert(sizetype, fold_build2(MULT_EXPR, TREE_TYPE(niters),
                                             niters, step_expr)));
    else
      ni = fold_build2(
          PLUS_EXPR, TREE_TYPE(init_expr),
          fold_build2(MULT_EXPR, TREE_TYPE(init_expr),
                      fold_convert(TREE_TYPE(init_expr), niters), step_expr),
          init_expr);
    ...
    ni_name = force_gimple_operand_gsi(&last_gsi, ni, false, var, true,
                                       GSI_SAME_STMT);
...
static unsigned int conservative_cost_threshold(loop_vec_info loop_vinfo,
                                                int min_profitable_iters) {
  unsigned int th;
  ...
  if (min_profitable_iters &&
      (!min_scalar_loop_bound || min_profitable_iters > min_scalar_loop_bound))
...
static void vect_do_peeling_for_loop_bound(loop_vec_info loop_vinfo,
                                           tree *ratio) {
  ...
  /* Generate the following variables on the preheader of original loop:
     ...
     ratio = ni_name / vf
     ratio_mult_vf_name = ratio * vf  */
  vect_generate_tmps_on_preheader(loop_vinfo, &ni_name, &ratio_mult_vf_name,
                                  ratio);
  ...
  new_loop =
      slpeel_tree_peel_loop_to_edge(loop, single_exit(loop), ratio_mult_vf_name,
                                    ni_name, false, th, check_profitability);
  gcc_assert(new_loop);
...
static tree vect_gen_niters_for_prolog_loop(loop_vec_info loop_vinfo,
                                            tree loop_niters) {
  struct data_reference *dr = LOOP_VINFO_UNALIGNED_DR(loop_vinfo);
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  tree var;
  ...
  if (LOOP_PEELING_FOR_ALIGNMENT(loop_vinfo) > 0) {
    ...
    iters = build_int_cst(
        niters_type, (((nelements - elem_misalign) & (nelements - 1)) / step));
  } else {
    gimple_seq new_stmts = NULL;
    tree start_addr = vect_create_addr_base_for_vector_ref(dr_stmt, &new_stmts,
                                                           NULL_TREE, loop);
...
static void vect_update_init_of_dr(struct data_reference *dr, tree niters) {
  tree offset = DR_OFFSET(dr);

  niters = fold_build2(MULT_EXPR, sizetype, fold_convert(sizetype, niters),
...
static void vect_update_inits_of_drs(loop_vec_info loop_vinfo, tree niters) {
  unsigned int i;
...
static void vect_do_peeling_for_alignment(loop_vec_info loop_vinfo) {
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  ...
  new_loop = slpeel_tree_peel_loop_to_edge(loop, loop_preheader_edge(loop),
  ...
  LOOP_VINFO_NITERS(loop_vinfo) = fold_build2(MINUS_EXPR, TREE_TYPE(n_iters),
                                              n_iters, niters_of_prolog_loop);
...
static void vect_create_cond_for_align_checks(loop_vec_info loop_vinfo,
  ...
  int_ptrsize_type = lang_hooks.types.type_for_size(tree_low_cst(psize, 1), 0);
  ...
  for (i = 0; VEC_iterate(gimple, may_misalign_stmts, i, ref_stmt); i++) {
    ...
    addr_base = vect_create_addr_base_for_vector_ref(ref_stmt, &new_stmt_list,
                                                     NULL_TREE, loop);
    ...
    addr_stmt = gimple_build_assign_with_ops(NOP_EXPR, addr_tmp_name, addr_base,
                                             NULL_TREE);
    ...
    if (or_tmp_name != NULL_TREE) {
      ...
      or_stmt = gimple_build_assign_with_ops(BIT_IOR_EXPR, new_or_tmp_name,
  ...
  part_cond_expr =
      fold_build2(EQ_EXPR, boolean_type_node, and_tmp_name, ptrsize_zero);
  if (*cond_expr)
    *cond_expr = fold_build2(TRUTH_AND_EXPR, boolean_type_node, *cond_expr,
                             part_cond_expr);
  else
...
static tree vect_vfa_segment_size(struct data_reference *dr, tree vect_factor) {
  tree segment_length =
      fold_build2(MULT_EXPR, integer_type_node, DR_STEP(dr), vect_factor);

  if (vect_supportable_dr_alignment(dr) == dr_explicit_realign_optimized) {
    tree vector_size =
        TYPE_SIZE_UNIT(STMT_VINFO_VECTYPE(vinfo_for_stmt(DR_STMT(dr))));

    segment_length =
        fold_build2(PLUS_EXPR, integer_type_node, segment_length, vector_size);
  }
...
static void vect_create_cond_for_alias_checks(loop_vec_info loop_vinfo,
                                              tree *cond_expr,
                                              gimple_seq *cond_expr_stmt_list) {
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  VEC(ddr_p, heap) *may_alias_ddrs = LOOP_VINFO_MAY_ALIAS_DDRS(loop_vinfo);
  tree vect_factor =
  ...
  for (i = 0; VEC_iterate(ddr_p, may_alias_ddrs, i, ddr); i++) {
    ...
    addr_base_a = vect_create_addr_base_for_vector_ref(
        stmt_a, cond_expr_stmt_list, NULL_TREE, loop);
    addr_base_b = vect_create_addr_base_for_vector_ref(
        stmt_b, cond_expr_stmt_list, NULL_TREE, loop);

    segment_length_a = vect_vfa_segment_size(dr_a, vect_factor);
    segment_length_b = vect_vfa_segment_size(dr_b, vect_factor);

    if (vect_print_dump_info(REPORT_DR_DETAILS)) {
      fprintf(vect_dump, "create runtime check for data references ");
      print_generic_expr(vect_dump, DR_REF(dr_a), TDF_SLIM);
      fprintf(vect_dump, " and ");
      print_generic_expr(vect_dump, DR_REF(dr_b), TDF_SLIM);
    }

    part_cond_expr = fold_build2(
...
static void vect_loop_versioning(loop_vec_info loop_vinfo) {
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  struct loop *nloop;
  ...
  cond_expr =
      force_gimple_operand(cond_expr, &cond_expr_stmt_list, false, NULL_TREE);
  ...
  initialize_original_copy_tables();
  nloop = loop_version(loop, cond_expr, &condition_bb, prob, prob,
                       REG_BR_PROB_BASE - prob, true);
  free_original_copy_tables();
  ...
  for (gsi = gsi_start_phis(merge_bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    new_phi = create_phi_node(SSA_NAME_VAR(PHI_RESULT(orig_phi)), new_exit_bb);
...
static void vect_remove_stores(gimple first_stmt) {
...
static bool vect_schedule_slp_instance(slp_tree node, slp_instance instance,
  ...
  if (SLP_INSTANCE_LOAD_PERMUTATION(instance)) {
    for (i = 0;
      ...
      if (!SLP_TREE_VEC_STMTS(loads_node)) {
        SLP_TREE_VEC_STMTS(loads_node) =
            VEC_alloc(gimple, heap, vec_stmts_size);
...
static bool vect_schedule_slp(loop_vec_info loop_vinfo) {
  ...
  for (i = 0; VEC_iterate(slp_instance, slp_instances, i, instance); i++) {
    ...
    is_store = vect_schedule_slp_instance(SLP_INSTANCE_TREE(instance), instance,
                                          LOOP_VINFO_VECT_FACTOR(loop_vinfo));
...
void vect_transform_loop(loop_vec_info loop_vinfo) {
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  basic_block *bbs = LOOP_VINFO_BBS(loop_vinfo);
  int nbbs = loop->num_nodes;
  gimple_stmt_iterator si;
  ...
  for (i = 0; i < nbbs; i++) {
    ...
    for (si = gsi_start_phis(bb); !gsi_end_p(si); gsi_next(&si)) {
      ...
      if (!STMT_VINFO_RELEVANT_P(stmt_info) && !STMT_VINFO_LIVE_P(stmt_info))
        continue;
