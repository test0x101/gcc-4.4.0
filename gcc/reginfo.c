...
void init_move_cost(enum machine_mode m) {
  ...
  move_cost[m] = (move_table *)xmalloc(sizeof(move_table) * N_REG_CLASSES);
  may_move_in_cost[m] =
...
static void init_reg_sets_1(void) {
  ...
  for (i = 0; i < N_REG_CLASSES; i++) {
    ...
    for (j = i + 1; j < N_REG_CLASSES; j++)
      if (hard_reg_set_subset_p(reg_class_contents[i], reg_class_contents[j])) {
...
int memory_move_secondary_cost(enum machine_mode mode, enum reg_class rclass,
                               int in) {
...
/* Set up preferred and alternate classes for REGNO as PREFCLASS and
   ALTCLASS.  */
void setup_reg_classes(int regno, enum reg_class prefclass,
                       enum reg_class altclass) {
  if (reg_pref == NULL)
    return;
...
void reg_scan(rtx f, unsigned int nregs ATTRIBUTE_UNUSED) {
  rtx insn;
...
static void reg_scan_mark_refs(rtx x, rtx insn) {
  enum rtx_code code;
  rtx dest;
  rtx note;
      ...
      if (REG_P(SET_DEST(x)) &&
          ...
          ((REG_P(SET_SRC(x)) && REG_POINTER(SET_SRC(x))) ||
           ((GET_CODE(SET_SRC(x)) == PLUS || GET_CODE(SET_SRC(x)) == LO_SUM) &&
            ...
            REG_P(XEXP(SET_SRC(x), 0)) && REG_POINTER(XEXP(SET_SRC(x), 0))) ||
...
int reg_classes_intersect_p(enum reg_class c1, enum reg_class c2) {
  return (c1 == c2 || c1 == ALL_REGS || c2 == ALL_REGS ||
...
static hashval_t som_hash(const void *x) {
  const struct subregs_of_mode_node *const a =
      (const struct subregs_of_mode_node *)x;
  return a->block;
}

static int som_eq(const void *x, const void *y) {
  const struct subregs_of_mode_node *const a =
      (const struct subregs_of_mode_node *)x;
...
static void record_subregs_of_mode(rtx subreg) {
  ...
  dummy.block = regno & -8;
  slot = htab_find_slot_with_hash(subregs_of_mode, &dummy, dummy.block, INSERT);
...
static unsigned int init_subregs_of_mode(void) {
  basic_block bb;
  rtx insn;
...
/* Set bits in *USED which correspond to registers which can't change
   ...
   encountered.  */
void cannot_change_mode_set_regs(HARD_REG_SET *used, enum machine_mode from,
                                 unsigned int regno) {
  struct subregs_of_mode_node dummy, *node;
  ...
  dummy.block = regno & -8;
  node = (struct subregs_of_mode_node *)htab_find_with_hash(
      subregs_of_mode, &dummy, dummy.block);
  if (node == NULL)
    return;
...
static unsigned int finish_subregs_of_mode(void) {
...
static bool gate_subregs_of_mode_init(void) {
#ifdef CANNOT_CHANGE_MODE_CLASS
  return true;
#else
  return false;
...
struct rtl_opt_pass pass_subregs_of_mode_init = {{
    RTL_PASS, "subregs_of_mode_init", /* name */
...
struct rtl_opt_pass pass_subregs_of_mode_finish = {{
    RTL_PASS, "subregs_of_mode_finish", /* name */
    gate_subregs_of_mode_init,          /* gate */
