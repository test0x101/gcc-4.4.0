 ...
 */
#define RTX_PREV(X)                                             \
  ((INSN_P(X) || NOTE_P(X) || BARRIER_P(X) || LABEL_P(X)) &&    \
           PREV_INSN(X) != NULL && NEXT_INSN(PREV_INSN(X)) == X \
       ? PREV_INSN(X)                                           \
       : NULL)
...
extern void rtl_check_failed_code_mode(const_rtx, enum rtx_code,
                                       enum machine_mode, bool, const char *,
...
#define RTX_FRAME_RELATED_P(RTX)                                             \
  (RTL_FLAG_CHECK5("RTX_FRAME_RELATED_P", (RTX), INSN, CALL_INSN, JUMP_INSN, \
                   BARRIER, SET)                                             \
       ->frame_related)
...
#define NOTE_INSN_BASIC_BLOCK_P(INSN) \
  (GET_CODE(INSN) == NOTE && NOTE_KIND(INSN) == NOTE_INSN_BASIC_BLOCK)

/* Variable declaration and the location of a variable.  */
#define NOTE_VAR_LOCATION_DECL(INSN) \
  (XCTREE(XCEXP(INSN, 4, NOTE), 0, VAR_LOCATION))
#define NOTE_VAR_LOCATION_LOC(INSN) \
  (XCEXP(XCEXP(INSN, 4, NOTE), 1, VAR_LOCATION))

/* Initialization status of the variable in the location.  Status
   can be unknown, uninitialized or initialized.  See enumeration
   type below.  */
#define NOTE_VAR_LOCATION_STATUS(INSN) \
  (XCINT(XCEXP(INSN, 4, NOTE), 2, VAR_LOCATION))
...
extern int simplify_subreg_regno(unsigned int, enum machine_mode, unsigned int,
                                 enum machine_mode);
...
#define MEM_ALIGN(RTX)                                                 \
  (MEM_ATTRS(RTX) != 0 ? MEM_ATTRS(RTX)->align                         \
                       : (STRICT_ALIGNMENT && GET_MODE(RTX) != BLKmode \
                              ? GET_MODE_ALIGNMENT(GET_MODE(RTX))      \
                              : BITS_PER_UNIT))
...
extern rtx assign_stack_temp_for_type(enum machine_mode, HOST_WIDE_INT, int,
                                      tree);
...
/* In simplify-rtx.c  */
extern rtx simplify_const_unary_operation(enum rtx_code, enum machine_mode, rtx,
                                          enum machine_mode);
...
extern rtx simplify_const_relational_operation(enum rtx_code, enum machine_mode,
                                               rtx, rtx);
...
/* In reginfo.c  */
extern enum machine_mode choose_hard_reg_mode(unsigned int, unsigned int, bool);
...
#define single_set(I)                                                       \
  (INSN_P(I) ? (GET_CODE(PATTERN(I)) == SET ? PATTERN(I) : single_set_1(I)) \
             : NULL_RTX)
...
typedef int (*hash_rtx_callback_function)(const_rtx, enum machine_mode, rtx *,
                                          enum machine_mode *);
extern unsigned hash_rtx_cb(const_rtx, enum machine_mode, int *, int *, bool,
...
#define REGNO_PTR_FRAME_P(REGNUM)                                             \
  ((REGNUM) == STACK_POINTER_REGNUM || (REGNUM) == FRAME_POINTER_REGNUM ||    \
   ...
   ((REGNUM) >= FIRST_VIRTUAL_REGISTER && (REGNUM) <= LAST_VIRTUAL_REGISTER))
...
extern bool validate_subreg(enum machine_mode, enum machine_mode, const_rtx,
                            unsigned int);
...
#ifdef HARD_CONST
extern void cannot_change_mode_set_regs(HARD_REG_SET *, enum machine_mode,
                                        unsigned int);
#endif
...
/* In stor-layout.c.  */
extern void get_mode_bounds(enum machine_mode, int, enum machine_mode, rtx *,
                            rtx *);
...
struct rtl_hooks {
  ...
  rtx (*reg_nonzero_bits)(const_rtx, enum machine_mode, const_rtx,
                          enum machine_mode, unsigned HOST_WIDE_INT,
                          unsigned HOST_WIDE_INT *);
  rtx (*reg_num_sign_bit_copies)(const_rtx, enum machine_mode, const_rtx,
