...
static bool live_on_edge(edge e, tree name) {
...
static tree vrp_evaluate_conditional_warnv_with_ops(enum tree_code, tree, tree,
                                                    bool, bool *, bool *);
...
static inline tree vrp_val_max(const_tree type) {
...
static inline tree vrp_val_min(const_tree type) {
  if (!INTEGRAL_TYPE_P(type))
    return NULL_TREE;
...
static inline bool vrp_val_is_max(const_tree val) {
  tree type_max = vrp_val_max(TREE_TYPE(val));
  return (val == type_max ||
          (type_max != NULL_TREE && operand_equal_p(val, type_max, 0)));
}

/* Return whether VAL is equal to the minimum value of its type.  This
   will be true for a negative overflow infinity.  */

static inline bool vrp_val_is_min(const_tree val) {
  tree type_min = vrp_val_min(TREE_TYPE(val));
  return (val == type_min ||
          (type_min != NULL_TREE && operand_equal_p(val, type_min, 0)));
}
...
static inline bool needs_overflow_infinity(const_tree type) {
...
static inline bool supports_overflow_infinity(const_tree type) {
  tree min = vrp_val_min(type), max = vrp_val_max(type);
#ifdef ENABLE_CHECKING
  gcc_assert(needs_overflow_infinity(type));
#endif
  return (min != NULL_TREE && CONSTANT_CLASS_P(min) && max != NULL_TREE &&
...
static inline tree make_overflow_infinity(tree val) {
#ifdef ENABLE_CHECKING
...
static inline tree negative_overflow_infinity(tree type) {
#ifdef ENABLE_CHECKING
...
static inline tree positive_overflow_infinity(tree type) {
#ifdef ENABLE_CHECKING
  gcc_assert(supports_overflow_infinity(type));
#endif
...
static inline bool is_negative_overflow_infinity(const_tree val) {
  return (needs_overflow_infinity(TREE_TYPE(val)) && CONSTANT_CLASS_P(val) &&
          TREE_OVERFLOW(val) && vrp_val_is_min(val));
}

/* Return whether VAL is a positive overflow infinity.  */

static inline bool is_positive_overflow_infinity(const_tree val) {
  return (needs_overflow_infinity(TREE_TYPE(val)) && CONSTANT_CLASS_P(val) &&
          TREE_OVERFLOW(val) && vrp_val_is_max(val));
}

/* Return whether VAL is a positive or negative overflow infinity.  */

static inline bool is_overflow_infinity(const_tree val) {
  return (needs_overflow_infinity(TREE_TYPE(val)) && CONSTANT_CLASS_P(val) &&
          TREE_OVERFLOW(val) && (vrp_val_is_min(val) || vrp_val_is_max(val)));
}

/* Return whether STMT has a constant rhs that is_overflow_infinity. */

static inline bool stmt_overflow_infinity(gimple stmt) {
  if (is_gimple_assign(stmt) &&
      get_gimple_rhs_class(gimple_assign_rhs_code(stmt)) == GIMPLE_SINGLE_RHS)
...
static inline tree avoid_overflow_infinity(tree val) {
...
static bool nonnull_arg_p(const_tree arg) {
  tree t, attrs, fntype;
  ...
  for (arg_num = 1, t = DECL_ARGUMENTS(current_function_decl); t;
...
static inline void set_value_range_to_varying(value_range_t *vr) {
  vr->type = VR_VARYING;
...
static void set_value_range(value_range_t *vr, enum value_range_type t,
  ...
  if (t == VR_RANGE || t == VR_ANTI_RANGE) {
    ...
    if (needs_overflow_infinity(TREE_TYPE(min)))
      gcc_assert(!is_overflow_infinity(min) || !is_overflow_infinity(max));
  }
...
static void set_and_canonicalize_value_range(value_range_t *vr,
                                             enum value_range_type t, tree min,
                                             tree max, bitmap equiv) {
  /* Nothing to canonicalize for symbolic or unknown or varying ranges.  */
  if ((t != VR_RANGE && t != VR_ANTI_RANGE) || TREE_CODE(min) != INTEGER_CST ||
  ...
  if (t == VR_ANTI_RANGE) {
    ...
    if (is_min && is_max) {
      ...
      return;
               ...
               && !(TYPE_UNSIGNED(TREE_TYPE(min)) && integer_zerop(max))) {
...
static inline void copy_value_range(value_range_t *to, value_range_t *from) {
...
static inline void set_value_range_to_value(value_range_t *vr, tree val,
                                            bitmap equiv) {
...
static inline void set_value_range_to_nonnegative(value_range_t *vr, tree type,
  ...
  set_value_range(vr, VR_RANGE, zero,
                  (overflow_infinity ? positive_overflow_infinity(type)
                                     : TYPE_MAX_VALUE(type)),
                  vr->equiv);
}
...
static inline void set_value_range_to_nonnull(value_range_t *vr, tree type) {
...
static inline void set_value_range_to_null(value_range_t *vr, tree type) {
  set_value_range_to_value(vr, build_int_cst(type, 0), vr->equiv);
}

/* Set value range VR to a range of a truthvalue of type TYPE.  */

static inline void set_value_range_to_truthvalue(value_range_t *vr, tree type) {
  if (TYPE_PRECISION(type) == 1)
    set_value_range_to_varying(vr);
  else
    set_value_range(vr, VR_RANGE, build_int_cst(type, 0),
                    build_int_cst(type, 1), vr->equiv);
}

/* Set value range VR to VR_UNDEFINED.  */

static inline void set_value_range_to_undefined(value_range_t *vr) {
...
static void abs_extent_range(value_range_t *vr, tree min, tree max) {
  int cmp;
...
static value_range_t *get_value_range(const_tree var) {
  value_range_t *vr;
  tree sym;
  ...
  if (SSA_NAME_IS_DEFAULT_DEF(var)) {
    ...
    if (TREE_CODE(sym) == PARM_DECL && POINTER_TYPE_P(TREE_TYPE(sym)) &&
        nonnull_arg_p(sym))
...
static inline bool vrp_operand_equal_p(const_tree val1, const_tree val2) {
  if (val1 == val2)
    return true;
...
static inline bool vrp_bitmap_equal_p(const_bitmap b1, const_bitmap b2) {
  return (b1 == b2 || (b1 && b2 && bitmap_equal_p(b1, b2)));
}
...
static inline bool update_value_range(const_tree var, value_range_t *new_vr) {
...
static void add_equivalence(bitmap *equiv, const_tree var) {
  unsigned ver = SSA_NAME_VERSION(var);
...
static inline bool range_is_nonnull(value_range_t *vr) {
  return vr->type == VR_ANTI_RANGE && integer_zerop(vr->min) &&
         integer_zerop(vr->max);
}

/* Return true if VR is [0, 0].  */

static inline bool range_is_null(value_range_t *vr) {
  return vr->type == VR_RANGE && integer_zerop(vr->min) &&
         integer_zerop(vr->max);
}

/* Return true if value range VR involves at least one symbol.  */

static inline bool symbolic_range_p(value_range_t *vr) {
...
static inline bool overflow_infinity_range_p(value_range_t *vr) {
  return (vr->type == VR_RANGE &&
          (is_overflow_infinity(vr->min) || is_overflow_infinity(vr->max)));
}
...
static bool usable_range_p(value_range_t *vr, bool *strict_overflow_p) {
...
static bool vrp_expr_computes_nonnegative(tree expr, bool *strict_overflow_p) {
  return (tree_expr_nonnegative_warnv_p(expr, strict_overflow_p) ||
          (TREE_CODE(expr) == SSA_NAME && ssa_name_nonnegative_p(expr)));
}
...
static bool gimple_assign_nonnegative_warnv_p(gimple stmt,
                                              bool *strict_overflow_p) {
  enum tree_code code = gimple_assign_rhs_code(stmt);
  switch (get_gimple_rhs_class(code)) {
    case GIMPLE_UNARY_RHS:
      return tree_unary_nonnegative_warnv_p(
          gimple_assign_rhs_code(stmt), gimple_expr_type(stmt),
          gimple_assign_rhs1(stmt), strict_overflow_p);
...
static bool gimple_call_nonnegative_warnv_p(gimple stmt,
                                            bool *strict_overflow_p) {
  tree arg0 =
  ...
  return tree_call_nonnegative_warnv_p(gimple_expr_type(stmt),
                                       gimple_call_fndecl(stmt), arg0, arg1,
                                       strict_overflow_p);
}
...
static bool gimple_stmt_nonnegative_warnv_p(gimple stmt,
                                            bool *strict_overflow_p) {
...
static bool gimple_assign_nonzero_warnv_p(gimple stmt,
                                          bool *strict_overflow_p) {
  enum tree_code code = gimple_assign_rhs_code(stmt);
  switch (get_gimple_rhs_class(code)) {
    case GIMPLE_UNARY_RHS:
      return tree_unary_nonzero_warnv_p(
          gimple_assign_rhs_code(stmt), gimple_expr_type(stmt),
          gimple_assign_rhs1(stmt), strict_overflow_p);
    case GIMPLE_BINARY_RHS:
      return tree_binary_nonzero_warnv_p(
          gimple_assign_rhs_code(stmt), gimple_expr_type(stmt),
          gimple_assign_rhs1(stmt), gimple_assign_rhs2(stmt),
          strict_overflow_p);
    case GIMPLE_SINGLE_RHS:
...
static bool gimple_stmt_nonzero_warnv_p(gimple stmt, bool *strict_overflow_p) {
  switch (gimple_code(stmt)) {
    case GIMPLE_ASSIGN:
...
static bool vrp_stmt_computes_nonzero(gimple stmt, bool *strict_overflow_p) {
  ...
  if (is_gimple_assign(stmt) && gimple_assign_rhs_code(stmt) == ADDR_EXPR) {
    ...
    if (base != NULL_TREE && TREE_CODE(base) == INDIRECT_REF &&
...
static bool valid_value_p(tree expr) {
...
static inline int operand_less_p(tree val, tree val2) {
  ...
  if (TREE_CODE(val) == INTEGER_CST && TREE_CODE(val2) == INTEGER_CST) {
    ...
    if (!tcmp || TREE_CODE(tcmp) != INTEGER_CST)
      return -2;
...
static int compare_values_warnv(tree val1, tree val2, bool *strict_overflow_p) {
  if (val1 == val2)
    return 0;
  ...
  if ((TREE_CODE(val1) == SSA_NAME || TREE_CODE(val1) == PLUS_EXPR ||
       TREE_CODE(val1) == MINUS_EXPR) &&
      (TREE_CODE(val2) == SSA_NAME || TREE_CODE(val2) == PLUS_EXPR ||
...
static int compare_values(tree val1, tree val2) {
  ...
  if (sop && (!is_gimple_min_invariant(val1) || !is_gimple_min_invariant(val2)))
    ret = -2;
  return ret;
}
...
static inline int value_inside_range(tree val, value_range_t *vr) {
  int cmp1, cmp2;
...
static inline bool value_ranges_intersect_p(value_range_t *vr0,
                                            value_range_t *vr1) {
...
static inline bool range_includes_zero_p(value_range_t *vr) {
  tree zero;

  gcc_assert(vr->type != VR_UNDEFINED && vr->type != VR_VARYING &&
             !symbolic_range_p(vr));
...
bool ssa_name_nonnegative_p(const_tree t) {
...
bool ssa_name_nonzero_p(const_tree t) {
  value_range_t *vr = get_value_range(t);
...
static tree op_with_constant_singleton_value_range(tree op) {
  value_range_t *vr;
  ...
  if (vr->type == VR_RANGE && operand_equal_p(vr->min, vr->max, 0) &&
...
static void extract_range_from_assert(value_range_t *vr_p, tree expr) {
  ...
  if (limit_vr && (limit_vr->type == VR_UNDEFINED ||
                   limit_vr->type == VR_VARYING || symbolic_range_p(limit_vr)))
    limit_vr = NULL;
  ...
  if (TREE_CODE(cond) == NOP_EXPR || TREE_CODE(cond) == PLUS_EXPR) {
    ...
    if (cond_code == LE_EXPR)
      set_and_canonicalize_value_range(vr_p, VR_RANGE, min, max, vr_p->equiv);
    else if (cond_code == GT_EXPR)
      set_and_canonicalize_value_range(vr_p, VR_ANTI_RANGE, min, max,
                                       vr_p->equiv);
    else
      gcc_unreachable();
    ...
    if (limit_vr && limit_vr->type == VR_RANGE &&
    ...
    if (INTEGRAL_TYPE_P(type) && vrp_val_is_min(min) && vrp_val_is_max(max))
      min = max = limit;
  ...
  if (vr_p->type == VR_VARYING || vr_p->type == VR_UNDEFINED ||
      var_vr->type == VR_VARYING || var_vr->type == VR_UNDEFINED ||
      symbolic_range_p(vr_p) || symbolic_range_p(var_vr))
    return;
  ...
  if (var_vr->type == VR_RANGE && vr_p->type == VR_RANGE) {
    ...
    if (compare_values(var_vr->min, vr_p->min) == 0 &&
      ...
      if (compare_values(anti_max, real_max) == -1 &&
        ...
        if (!vrp_val_is_min(real_min) || !vrp_val_is_max(real_max))
          set_value_range(vr_p, VR_RANGE, real_min, real_max, vr_p->equiv);
      }
      ...
      /* Case 3a, the anti-range extends into the low
      ...
      else if (((cmp = compare_values(anti_max, real_min)) == 1 || cmp == 0) &&
        ...
        if (needs_overflow_infinity(TREE_TYPE(anti_max)) &&
          ...
          min = fold_build2(PLUS_EXPR, TREE_TYPE(var_vr->min), anti_max,
                            build_int_cst(TREE_TYPE(var_vr->min), 1));
        else
          min = fold_build2(POINTER_PLUS_EXPR, TREE_TYPE(var_vr->min), anti_max,
                            size_int(1));
        max = real_max;
        set_value_range(vr_p, VR_RANGE, min, max, vr_p->equiv);
      }
      /* Case 3b, the anti-range extends into the high
         ...
         higher for the real range.  */
               ...
               ((cmp = compare_values(anti_min, real_max)) == -1 || cmp == 0)) {
        ...
        if (needs_overflow_infinity(TREE_TYPE(anti_min)) &&
          ...
          max = fold_build2(MINUS_EXPR, TREE_TYPE(var_vr->min), anti_min,
                            build_int_cst(TREE_TYPE(var_vr->min), 1));
        else
          max = fold_build2(POINTER_PLUS_EXPR, TREE_TYPE(var_vr->min), anti_min,
                            size_int(-1));
        min = real_min;
        set_value_range(vr_p, VR_RANGE, min, max, vr_p->equiv);
      }
...
static void extract_range_from_ssa_name(value_range_t *vr, tree var) {
...
static tree vrp_int_const_binop(enum tree_code code, tree val1, tree val2) {
  tree res;
  ...
  if (TYPE_OVERFLOW_WRAPS(TREE_TYPE(val1))) {
    ...
    /* Checking for multiplication overflow is done by dividing the
      ...
      tree tmp = int_const_binop(TRUNC_DIV_EXPR, res, val1, 0);
    ...
    if (overflow) {
      ...
      TREE_OVERFLOW(res) = 1;
              ...
              !TREE_OVERFLOW(val2)) ||
             is_overflow_infinity(val1) || is_overflow_infinity(val2)) {
    ...
    if ((code == TRUNC_DIV_EXPR || code == FLOOR_DIV_EXPR ||
         code == CEIL_DIV_EXPR || code == EXACT_DIV_EXPR ||
         code == ROUND_DIV_EXPR || code == RSHIFT_EXPR) &&
        (is_overflow_infinity(val1) || is_overflow_infinity(val2)))
      return NULL_TREE;
    ...
    if ((code == MULT_EXPR && sgn1 == sgn2)
        ...
        ||
        (code == PLUS_EXPR && (sgn1 >= 0 ? !is_negative_overflow_infinity(val2)
        ...
        || (code == MINUS_EXPR &&
            (sgn1 >= 0 ? !is_positive_overflow_infinity(val2)
        ...
        || code == TRUNC_DIV_EXPR || code == FLOOR_DIV_EXPR ||
        code == CEIL_DIV_EXPR || code == EXACT_DIV_EXPR ||
        code == ROUND_DIV_EXPR)
...
static void extract_range_from_binary_expr(value_range_t *vr,
  ...
  if (code != PLUS_EXPR && code != MINUS_EXPR && code != POINTER_PLUS_EXPR &&
      code != MULT_EXPR && code != TRUNC_DIV_EXPR && code != FLOOR_DIV_EXPR &&
      code != CEIL_DIV_EXPR && code != EXACT_DIV_EXPR &&
      code != ROUND_DIV_EXPR && code != RSHIFT_EXPR && code != MIN_EXPR &&
      code != MAX_EXPR && code != BIT_AND_EXPR && code != BIT_IOR_EXPR &&
    ...
    if (const_op0 || const_op1) {
      tree tem = fold_binary(code, expr_type, const_op0 ? const_op0 : op0,
                             const_op1 ? const_op1 : op1);
      if (tem && is_gimple_min_invariant(tem) && !is_overflow_infinity(tem)) {
  ...
  /* Refuse to operate on VARYING ranges, ranges of different kinds
     ...
     some cases.  */
  if (code != BIT_AND_EXPR && code != TRUTH_AND_EXPR && code != TRUTH_OR_EXPR &&
      code != TRUNC_DIV_EXPR && code != FLOOR_DIV_EXPR &&
      code != CEIL_DIV_EXPR && code != EXACT_DIV_EXPR &&
      code != ROUND_DIV_EXPR &&
      (vr0.type == VR_VARYING || vr1.type == VR_VARYING ||
       vr0.type != vr1.type || symbolic_range_p(&vr0) ||
  ...
  if (POINTER_TYPE_P(expr_type) || POINTER_TYPE_P(TREE_TYPE(op0)) ||
  ...
  if (code == TRUTH_AND_EXPR || code == TRUTH_OR_EXPR) {
    /* If one of the operands is zero, we know that the whole
       expression evaluates zero.  */
    if (code == TRUTH_AND_EXPR &&
        ((vr0.type == VR_RANGE && integer_zerop(vr0.min) &&
          integer_zerop(vr0.max)) ||
         (vr1.type == VR_RANGE && integer_zerop(vr1.min) &&
    ...
    /* If one of the operands is one, we know that the whole
       expression evaluates one.  */
             ...
             ((vr0.type == VR_RANGE && integer_onep(vr0.min) &&
               integer_onep(vr0.max)) ||
              (vr1.type == VR_RANGE && integer_onep(vr1.min) &&
               integer_onep(vr1.max)))) {
      type = VR_RANGE;
      min = max = build_int_cst(expr_type, 1);
    } else if (vr0.type != VR_VARYING && vr1.type != VR_VARYING &&
               vr0.type == vr1.type && !symbolic_range_p(&vr0) &&
               !overflow_infinity_range_p(&vr0) && !symbolic_range_p(&vr1) &&
  ...
  } else if (code == PLUS_EXPR || code == MIN_EXPR || code == MAX_EXPR) {
  ...
  } else if (code == MULT_EXPR || code == TRUNC_DIV_EXPR ||
             code == FLOOR_DIV_EXPR || code == CEIL_DIV_EXPR ||
             code == EXACT_DIV_EXPR || code == ROUND_DIV_EXPR ||
             code == RSHIFT_EXPR) {
    tree val[4];
    size_t i;
    bool sop;
    ...
    if (code == RSHIFT_EXPR) {
      if (vr1.type == VR_ANTI_RANGE ||
          ...
          (operand_less_p(
               build_int_cst(TREE_TYPE(vr1.max), TYPE_PRECISION(expr_type) - 1),
               vr1.max) != 0)) {
        set_value_range_to_varying(vr);
        return;
      }
    ...
    else if ((code == TRUNC_DIV_EXPR || code == FLOOR_DIV_EXPR ||
              code == CEIL_DIV_EXPR || code == EXACT_DIV_EXPR ||
              code == ROUND_DIV_EXPR) &&
    ...
    if ((code == TRUNC_DIV_EXPR || code == FLOOR_DIV_EXPR ||
         ...
         code == ROUND_DIV_EXPR) &&
        vr0.type == VR_RANGE &&
        (vr1.type != VR_RANGE || symbolic_range_p(&vr1) ||
         range_includes_zero_p(&vr1))) {
      ...
      for (i = 1; i < 4; i++) {
        ...
        if (val[i]) {
          if (!is_gimple_min_invariant(val[i]) ||
              (TREE_OVERFLOW(val[i]) && !is_overflow_infinity(val[i]))) {
    ...
    if (vr0.type == VR_RANGE && vr0.min == vr0.max &&
        TREE_CODE(vr0.max) == INTEGER_CST && !TREE_OVERFLOW(vr0.max) &&
        tree_int_cst_sgn(vr0.max) >= 0) {
      min = build_int_cst(expr_type, 0);
      max = vr0.max;
    } else if (vr1.type == VR_RANGE && vr1.min == vr1.max &&
               TREE_CODE(vr1.max) == INTEGER_CST && !TREE_OVERFLOW(vr1.max) &&
    ...
    if (vr0.type == VR_RANGE && vr1.type == VR_RANGE &&
        ...
        TREE_CODE(vr1.max) == INTEGER_CST && tree_int_cst_sgn(vr0.min) >= 0 &&
      ...
      if (ior_max.high != 0) {
        ...
        ior_max.high |= ((HOST_WIDE_INT)1 << floor_log2(ior_max.high)) - 1;
      } else if (ior_max.low != 0)
        ior_max.low |=
            ((unsigned HOST_WIDE_INT)1u << floor_log2(ior_max.low)) - 1;
  ...
  /* If either MIN or MAX overflowed, then set the resulting range to
     ...
     representation.  */
  if (min == NULL_TREE || !is_gimple_min_invariant(min) ||
      (TREE_OVERFLOW(min) && !is_overflow_infinity(min)) || max == NULL_TREE ||
      !is_gimple_min_invariant(max) ||
      (TREE_OVERFLOW(max) && !is_overflow_infinity(max))) {
    set_value_range_to_varying(vr);
    return;
...
static void extract_range_from_unary_expr(value_range_t *vr,
  ...
  if (code == FIX_TRUNC_EXPR || code == FLOAT_EXPR || code == BIT_NOT_EXPR ||
      code == CONJ_EXPR) {
    /* We can still do constant propagation here.  */
  ...
  if ((!INTEGRAL_TYPE_P(TREE_TYPE(op0)) && !POINTER_TYPE_P(TREE_TYPE(op0))) ||
      (vr0.type != VR_VARYING && symbolic_range_p(&vr0))) {
    set_value_range_to_varying(vr);
    return;
  }
  ...
  if (POINTER_TYPE_P(type) || POINTER_TYPE_P(TREE_TYPE(op0))) {
    ...
    if (range_is_nonnull(&vr0) ||
        (tree_unary_nonzero_warnv_p(code, type, op0, &sop) && !sop))
  ...
  if (CONVERT_EXPR_CODE_P(code) && INTEGRAL_TYPE_P(type) &&
    ...
    /* If VR0 is a constant range or anti-range and the conversion is
       ...
       not an anti-range.  */
    if ((vr0.type == VR_RANGE || vr0.type == VR_ANTI_RANGE) &&
        TREE_CODE(vr0.min) == INTEGER_CST &&
        TREE_CODE(vr0.max) == INTEGER_CST && !is_overflow_infinity(vr0.min) &&
        !is_overflow_infinity(vr0.max) &&
        (TYPE_PRECISION(outer_type) >= TYPE_PRECISION(inner_type) ||
         (vr0.type == VR_RANGE &&
          integer_zerop(int_const_binop(
              RSHIFT_EXPR, int_const_binop(MINUS_EXPR, vr0.max, vr0.min, 0),
              size_int(TYPE_PRECISION(outer_type)), 0))))) {
      tree new_min, new_max;
      new_min = force_fit_type_double(outer_type, TREE_INT_CST_LOW(vr0.min),
                                      TREE_INT_CST_HIGH(vr0.min), 0, 0);
      new_max = force_fit_type_double(outer_type, TREE_INT_CST_LOW(vr0.max),
                                      TREE_INT_CST_HIGH(vr0.max), 0, 0);
      set_and_canonicalize_value_range(vr, vr0.type, new_min, new_max, NULL);
      return;
    }
  ...
  /* Apply the operation to each end of the range and see what we end
     up with.  */
  if (code == NEGATE_EXPR && !TYPE_UNSIGNED(type)) {
    ...
    if (is_positive_overflow_infinity(vr0.max))
      ...
      if (supports_overflow_infinity(type) && !is_overflow_infinity(vr0.min) &&
  ...
  } else if (code == NEGATE_EXPR && TYPE_UNSIGNED(type)) {
  ...
  } else if (code == ABS_EXPR && !TYPE_UNSIGNED(type)) {
    /* -TYPE_MIN_VALUE = TYPE_MIN_VALUE with flag_wrapv so we can't get a
       useful range.  */
    if (!TYPE_OVERFLOW_UNDEFINED(type) &&
        ((vr0.type == VR_RANGE && vrp_val_is_min(vr0.min)) ||
         (vr0.type == VR_ANTI_RANGE && !vrp_val_is_min(vr0.min) &&
    ...
    if (needs_overflow_infinity(type)) {
      ...
      /* If both sides have overflowed, we don't know
         anything.  */
      if ((is_overflow_infinity(vr0.min) || TREE_OVERFLOW(min)) &&
          (is_overflow_infinity(vr0.max) || TREE_OVERFLOW(max))) {
        set_value_range_to_varying(vr);
        return;
      }
...
static void extract_range_from_cond_expr(value_range_t *vr, tree expr) {
  tree op0, op1;
  value_range_t vr0 = {VR_UNDEFINED, NULL_TREE, NULL_TREE, NULL};
  value_range_t vr1 = {VR_UNDEFINED, NULL_TREE, NULL_TREE, NULL};
...
static void extract_range_basic(value_range_t *vr, gimple stmt) {
  bool sop = false;
  tree type = gimple_expr_type(stmt);

  if (INTEGRAL_TYPE_P(type) && gimple_stmt_nonnegative_warnv_p(stmt, &sop))
    set_value_range_to_nonnegative(vr, type,
                                   sop || stmt_overflow_infinity(stmt));
  else if (vrp_stmt_computes_nonzero(stmt, &sop) && !sop)
    set_value_range_to_nonnull(vr, type);
  else
    set_value_range_to_varying(vr);
}
...
static void extract_range_from_assignment(value_range_t *vr, gimple stmt) {
  enum tree_code code = gimple_assign_rhs_code(stmt);
  ...
  else if (TREE_CODE_CLASS(code) == tcc_binary || code == TRUTH_AND_EXPR ||
           code == TRUTH_OR_EXPR || code == TRUTH_XOR_EXPR)
    extract_range_from_binary_expr(
        vr, gimple_assign_rhs_code(stmt), gimple_expr_type(stmt),
        gimple_assign_rhs1(stmt), gimple_assign_rhs2(stmt));
...
static void adjust_range_with_scev(value_range_t *vr, struct loop *loop,
  ...
  if (step == NULL_TREE || !is_gimple_min_invariant(step) ||
      !valid_value_p(init))
    return;
  ...
  if (/* Do not adjust ranges if we do not know whether the iv increases
      ...
      dir == EV_DIR_UNKNOWN
      /* ... or if it may wrap.  */
      || scev_probably_wraps_p(init, step, stmt, get_chrec_loop(chrec), true))
    return;
...
static bool vrp_var_may_overflow(tree var, gimple stmt) {
  struct loop *l;
  ...
  if (scev_probably_wraps_p(init, step, stmt, get_chrec_loop(chrec), true))
    return true;
...
static tree compare_ranges(enum tree_code comp, value_range_t *vr0,
                           value_range_t *vr1, bool *strict_overflow_p) {
  /* VARYING or UNDEFINED ranges cannot be compared.  */
  if (vr0->type == VR_VARYING || vr0->type == VR_UNDEFINED ||
      vr1->type == VR_VARYING || vr1->type == VR_UNDEFINED)
    return NULL_TREE;
  ...
  if (vr0->type == VR_ANTI_RANGE || vr1->type == VR_ANTI_RANGE) {
    ...
    if (comp == GT_EXPR || comp == GE_EXPR || comp == LT_EXPR ||
        comp == LE_EXPR)
      return NULL_TREE;
  ...
  if (comp == EQ_EXPR) {
    ...
    if (compare_values_warnv(vr0->min, vr0->max, strict_overflow_p) == 0 &&
      ...
      int cmp_min = compare_values_warnv(vr0->min, vr1->min, strict_overflow_p);
      int cmp_max = compare_values_warnv(vr0->max, vr1->max, strict_overflow_p);
    ...
    else if (compare_values_warnv(vr0->min, vr1->max, strict_overflow_p) == 1 ||
             compare_values_warnv(vr1->min, vr0->max, strict_overflow_p) == 1)
      return boolean_false_node;
    ...
    /* If VR0 and VR1 represent a single value and are identical,
       return false.  */
    else if (compare_values_warnv(vr0->min, vr0->max, strict_overflow_p) == 0 &&
             compare_values_warnv(vr1->min, vr1->max, strict_overflow_p) == 0 &&
             compare_values_warnv(vr0->min, vr1->min, strict_overflow_p) == 0 &&
             compare_values_warnv(vr0->max, vr1->max, strict_overflow_p) == 0)
      return boolean_false_node;
    ...
    if ((comp == LT_EXPR && tst == -1) ||
      ...
      if (overflow_infinity_range_p(vr0) || overflow_infinity_range_p(vr1))
        *strict_overflow_p = true;
      return boolean_true_node;
    }
...
static tree compare_range_with_value(enum tree_code comp, value_range_t *vr,
...
void dump_value_range(FILE *file, value_range_t *vr) {
  if (vr == NULL)
    fprintf(file, "[]");
    ...
    else if (INTEGRAL_TYPE_P(type) && !TYPE_UNSIGNED(type) &&
    ...
    if (is_positive_overflow_infinity(vr->max))
      fprintf(file, "+INF(OVF)");
    else if (INTEGRAL_TYPE_P(type) && vrp_val_is_max(vr->max))
      fprintf(file, "+INF");
    else
...
void dump_all_value_ranges(FILE *file) {
  size_t i;
...
static gimple build_assert_expr_for(tree cond, tree v) {
  tree n;
  gimple assertion;
...
static inline bool fp_predicate(gimple stmt) {
...
static bool infer_value_range(gimple stmt, tree op, enum tree_code *comp_code_p,
                              tree *val_p) {
  *val_p = NULL_TREE;
...
void dump_asserts_for(FILE *file, tree name) {
  ...
  while (loc) {
    ...
    fprintf(file, "\n\tBB #%d", loc->bb->index);
    if (loc->e) {
      fprintf(file, "\n\tEDGE %d->%d", loc->e->src->index, loc->e->dest->index);
...
void dump_all_asserts(FILE *file) {
  unsigned i;
  bitmap_iterator bi;
...
static void register_new_assert_for(tree name, tree expr,
                                    enum tree_code comp_code, tree val,
  ...
  if (TREE_CODE(val) == INTEGER_CST && TREE_OVERFLOW(val))
    val = build_int_cst_wide(TREE_TYPE(val), TREE_INT_CST_LOW(val),
  ...
  while (loc) {
    if (loc->comp_code == comp_code &&
        (loc->val == val || operand_equal_p(loc->val, val, 0)) &&
        (loc->expr == expr || operand_equal_p(loc->expr, expr, 0))) {
      ...
      if (e == NULL && dominated_by_p(CDI_DOMINATORS, dest_bb, loc->bb))
        return;
...
static bool extract_code_and_val_from_cond_with_ops(
    tree name, enum tree_code cond_code, tree cond_op0, tree cond_op1,
    bool invert, enum tree_code *code_p, tree *val_p) {
  enum tree_code comp_code;
  tree val;
  ...
  if ((comp_code == GT_EXPR || comp_code == LT_EXPR) &&
    ...
    if (comp_code == GT_EXPR && (!max || compare_values(val, max) == 0))
      return false;

    if (comp_code == LT_EXPR && (!min || compare_values(val, min) == 0))
      return false;
  }
  *code_p = comp_code;
  *val_p = val;
...
static bool register_edge_assert_for_2(tree name, edge e,
                                       gimple_stmt_iterator bsi,
                                       enum tree_code cond_code, tree cond_op0,
                                       tree cond_op1, bool invert) {
  tree val;
  enum tree_code comp_code;
  bool retval = false;

  if (!extract_code_and_val_from_cond_with_ops(
          name, cond_code, cond_op0, cond_op1, invert, &comp_code, &val))
    return false;

  /* Only register an ASSERT_EXPR if NAME was found in the sub-graph
     reachable from E.  */
  if (live_on_edge(e, name) && !has_single_use(name)) {
  ...
  if ((comp_code == LE_EXPR || comp_code == GT_EXPR) &&
      TREE_CODE(val) == INTEGER_CST && TYPE_UNSIGNED(TREE_TYPE(val))) {
    ...
    if (is_gimple_assign(def_stmt) &&
      ...
      if (TREE_CODE(name2) == SSA_NAME && TREE_CODE(cst2) == INTEGER_CST)
    ...
    if (name3 != NULL_TREE && TREE_CODE(name3) == SSA_NAME &&
        (cst2 == NULL_TREE || TREE_CODE(cst2) == INTEGER_CST) &&
        INTEGRAL_TYPE_P(TREE_TYPE(name3)) && live_on_edge(e, name3) &&
    ...
    if (name2 != NULL_TREE && TREE_CODE(name2) == SSA_NAME &&
        TREE_CODE(cst2) == INTEGER_CST && INTEGRAL_TYPE_P(TREE_TYPE(name2)) &&
        live_on_edge(e, name2) && !has_single_use(name2)) {
      tree tmp;
...
static bool register_edge_assert_for_1(tree op, enum tree_code code, edge e,
                                       gimple_stmt_iterator bsi) {
  bool retval = false;
  gimple op_def;
  tree val;
  ...
  if (TREE_CODE_CLASS(rhs_code) == tcc_comparison) {
    ...
    retval |=
        register_edge_assert_for_1(gimple_assign_rhs2(op_def), code, e, bsi);
...
static bool register_edge_assert_for(tree name, edge e, gimple_stmt_iterator si,
                                     enum tree_code cond_code, tree cond_op0,
                                     tree cond_op1) {
  tree val;
  enum tree_code comp_code;
  ...
  if (!extract_code_and_val_from_cond_with_ops(
          name, cond_code, cond_op0, cond_op1, is_else_edge, &comp_code, &val))
    return false;
...
static bool find_conditional_asserts(basic_block bb, gimple last) {
  bool need_assert;
  gimple_stmt_iterator bsi;
  tree op;
  edge_iterator ei;
  ...
  FOR_EACH_EDGE(e, ei, bb->succs) {
    ...
    FOR_EACH_SSA_TREE_OPERAND(op, last, iter, SSA_OP_USE) {
      need_assert |= register_edge_assert_for(
          op, e, bsi, gimple_cond_code(last), gimple_cond_lhs(last),
...
static bool find_switch_asserts(basic_block bb, gimple last) {
  bool need_assert;
  gimple_stmt_iterator bsi;
  tree op;
  edge e;
  ...
  for (idx = 0; idx < n; ++idx) {
    ...
    if (max) {
      need_assert |= register_edge_assert_for(op, e, bsi, LE_EXPR, op,
...
static bool find_assert_locations_1(basic_block bb, sbitmap live) {
  gimple_stmt_iterator si;
  gimple last;
  gimple phi;
  bool need_assert;
  ...
  if (last && gimple_code(last) == GIMPLE_COND && !fp_predicate(last) &&
...
static bool find_assert_locations(void) {
  ...
  for (i = rpo_cnt - 1; i >= 0; --i) {
    ...
    FOR_EACH_EDGE(e, ei, bb->succs)
    if (last_rpo[e->dest->index] == i && live[e->dest->index]) {
...
static bool process_assert_insertions_for(tree name, assert_locus_t loc) {
...
static void process_assert_insertions(void) {
  unsigned i;
  bitmap_iterator bi;
...
static void insert_range_assertions(void) {
...
static void check_array_ref(tree ref, const location_t *location,
  ...
  if (vr && vr->type == VR_ANTI_RANGE) {
    if (TREE_CODE(up_sub) == INTEGER_CST && tree_int_cst_lt(up_bound, up_sub) &&
        TREE_CODE(low_sub) == INTEGER_CST &&
        tree_int_cst_lt(low_sub, low_bound)) {
      warning(OPT_Warray_bounds, "%Harray subscript is outside array bounds",
              location);
      TREE_NO_WARNING(ref) = 1;
             ...
             (!ignore_off_by_one ||
              !tree_int_cst_equal(
                  int_const_binop(PLUS_EXPR, up_bound, integer_one_node, 0),
                  up_sub))) {
...
static void search_for_addr_array(tree t, const location_t *location) {
  while (TREE_CODE(t) == SSA_NAME) {
    ...
    if (get_gimple_rhs_class(gimple_assign_rhs_code(g)) != GIMPLE_SINGLE_RHS)
      return;
...
static void check_all_array_refs(void) {
  basic_block bb;
  gimple_stmt_iterator si;
  ...
  FOR_EACH_BB(bb) {
    ...
    for (si = gsi_start_bb(bb); !gsi_end_p(si); gsi_next(&si)) {
      ...
      if (is_gimple_call(stmt)) {
        ...
        walk_gimple_op(gsi_stmt(si), check_array_bounds, &wi);
      }
    }
  }
}
...
static void remove_range_assertions(void) {
  basic_block bb;
  gimple_stmt_iterator si;
  ...
  for (si = gsi_start_bb(bb); !gsi_end_p(si);) {
    ...
    if (is_gimple_assign(stmt) && gimple_assign_rhs_code(stmt) == ASSERT_EXPR) {
      ...
      FOR_EACH_IMM_USE_STMT(use_stmt, iter, gimple_assign_lhs(stmt))
...
static bool stmt_interesting_for_vrp(gimple stmt) {
  if (gimple_code(stmt) == GIMPLE_PHI &&
    ...
    if (lhs && TREE_CODE(lhs) == SSA_NAME &&
        (INTEGRAL_TYPE_P(TREE_TYPE(lhs)) || POINTER_TYPE_P(TREE_TYPE(lhs))) &&
        ((is_gimple_call(stmt) && gimple_call_fndecl(stmt) != NULL_TREE &&
...
static void vrp_initialize(void) {
  basic_block bb;
...
static enum ssa_prop_result vrp_visit_assignment_or_call(gimple stmt,
                                                         tree *output_p) {
  tree def, lhs;
  ssa_op_iter iter;
  ...
  if (TREE_CODE(lhs) == SSA_NAME &&
      ((INTEGRAL_TYPE_P(TREE_TYPE(lhs))
        /* It is valid to have NULL MIN/MAX values on a type.  See
           build_range_type.  */
        && TYPE_MIN_VALUE(TREE_TYPE(lhs)) && TYPE_MAX_VALUE(TREE_TYPE(lhs))) ||
...
static inline value_range_t get_vr_for_comparison(int i) {
...
static tree compare_name_with_value(enum tree_code comp, tree var, tree val,
                                    bool *strict_overflow_p) {
  bitmap_iterator bi;
  unsigned i;
  bitmap e;
...
static tree compare_names(enum tree_code comp, tree n1, tree n2,
                          bool *strict_overflow_p) {
  tree t, retval;
  bitmap e1, e2;
...
static tree vrp_evaluate_conditional_warnv_with_ops_using_ranges(
  ...
  if (vr0 && vr1)
    ...
    return (compare_range_with_value(swap_tree_comparison(code), vr1, op0,
                                     strict_overflow_p));
  return NULL;
}
...
static tree vrp_evaluate_conditional_warnv_with_ops(enum tree_code code,
  ...
  if (use_equiv_p) {
    ...
    *only_ranges = false;
    ...
    return vrp_evaluate_conditional_warnv_with_ops_using_ranges(
        code, op0, op1, strict_overflow_p);
  return NULL_TREE;
}
...
tree vrp_evaluate_conditional(enum tree_code code, tree op0, tree op1,
                              gimple stmt) {
  bool sop;
  tree ret;
  bool only_ranges;
  ...
  if (warn_type_limits && ret && only_ranges &&
      TREE_CODE_CLASS(code) == tcc_comparison && TREE_CODE(op0) == SSA_NAME) {
    ...
    if (vr0->type != VR_VARYING && INTEGRAL_TYPE_P(type) &&
        vrp_val_is_min(vr0->min) && vrp_val_is_max(vr0->max) &&
...
static enum ssa_prop_result vrp_visit_cond_stmt(gimple stmt,
                                                edge *taken_edge_p) {
  tree val;
  bool sop;
  ...
  *taken_edge_p = NULL;
  ...
  val = vrp_evaluate_conditional_warnv_with_ops(
      gimple_cond_code(stmt), gimple_cond_lhs(stmt), gimple_cond_rhs(stmt),
      false, &sop, NULL);
  if (val) {
    if (!sop)
...
static bool find_case_label_index(gimple stmt, size_t start_idx, tree val,
                                  size_t *idx) {
...
static bool find_case_label_range(gimple stmt, tree min, tree max,
  ...
  if (i == j && min_take_default && max_take_default) {
...
static enum ssa_prop_result vrp_visit_switch_stmt(gimple stmt,
                                                  edge *taken_edge_p) {
  tree op, val;
  value_range_t *vr;
  size_t i = 0, j = 0, n;
  ...
  *taken_edge_p = NULL;
  ...
  if (j < i) {
    ...
    if (take_default &&
        CASE_LABEL(gimple_switch_default_label(stmt)) != CASE_LABEL(val)) {
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf(dump_file,
  ...
  *taken_edge_p = find_edge(gimple_bb(stmt), label_to_block(CASE_LABEL(val)));
...
static enum ssa_prop_result vrp_visit_stmt(gimple stmt, edge *taken_edge_p,
                                           tree *output_p) {
  tree def;
  ssa_op_iter iter;
  ...
  if (is_gimple_assign(stmt) || is_gimple_call(stmt)) {
    ...
    if ((is_gimple_call(stmt) && gimple_call_fndecl(stmt) != NULL_TREE &&
         DECL_IS_BUILTIN(gimple_call_fndecl(stmt))) ||
        ZERO_SSA_OPERANDS(stmt, SSA_OP_ALL_VIRTUALS))
...
static void vrp_meet(value_range_t *vr0, value_range_t *vr1) {
  if (vr0->type == VR_UNDEFINED) {
...
static enum ssa_prop_result vrp_visit_phi_node(gimple phi) {
  size_t i;
  tree lhs = PHI_RESULT(phi);
    ...
    if (!POINTER_TYPE_P(TREE_TYPE(lhs))) {
      ...
      if (cmp_min > 0 || cmp_min < 0) {
        ...
        if (!needs_overflow_infinity(TREE_TYPE(vr_result.min)) ||
          ...
          vr_result.min = negative_overflow_infinity(TREE_TYPE(vr_result.min));
        else
          goto varying;
      }
      ...
      if (cmp_max < 0 || cmp_max > 0) {
        ...
        if (!needs_overflow_infinity(TREE_TYPE(vr_result.max)) ||
          ...
          vr_result.max = positive_overflow_infinity(TREE_TYPE(vr_result.max));
        else
          goto varying;
      }
    }
...
static bool simplify_truth_ops_using_ranges(gimple_stmt_iterator *gsi,
                                            gimple stmt) {
  ...
  need_conversion = !useless_type_conversion_p(
      TREE_TYPE(gimple_assign_lhs(stmt)), TREE_TYPE(op0));

  /* Make sure to not sign-extend -1 as a boolean value.  */
  if (need_conversion && !TYPE_UNSIGNED(TREE_TYPE(op0)) &&
    ...
    case NE_EXPR:
      if (integer_zerop(op1)) {
        gimple_assign_set_rhs_with_ops(
            gsi, need_conversion ? NOP_EXPR : SSA_NAME, op0, NULL);
...
static bool simplify_div_or_mod_using_ranges(gimple stmt) {
  enum tree_code rhs_code = gimple_assign_rhs_code(stmt);
  tree val = NULL;
...
static bool simplify_abs_using_ranges(gimple stmt) {
  tree val = NULL;
...
static tree test_for_singularity(enum tree_code cond_code, tree op0, tree op1,
                                 value_range_t *vr) {
  tree min = NULL;
  tree max = NULL;
...
static bool simplify_cond_using_ranges(gimple stmt) {
  tree op0 = gimple_cond_lhs(stmt);
  tree op1 = gimple_cond_rhs(stmt);
  enum tree_code cond_code = gimple_cond_code(stmt);

  if (cond_code != NE_EXPR && cond_code != EQ_EXPR &&
      TREE_CODE(op0) == SSA_NAME && INTEGRAL_TYPE_P(TREE_TYPE(op0)) &&
      is_gimple_min_invariant(op1)) {
...
static bool simplify_switch_using_ranges(gimple stmt) {
...
static tree simplify_stmt_for_jump_threading(gimple stmt, gimple within_stmt) {
  ...
  return vrp_evaluate_conditional(gimple_cond_code(stmt), gimple_cond_lhs(stmt),
...
static void identify_jump_threads(void) {
  basic_block bb;
  gimple dummy;
  int i;
  edge e;
  ...
  /* To avoid lots of silly node creation, we create a single
     ...
     thread jumps.  */
  dummy = gimple_build_cond(EQ_EXPR, integer_zero_node, integer_zero_node, NULL,
                            NULL);
...
static void finalize_jump_threads(void) {
...
static unsigned int execute_vrp(void) {
  int i;
  edge e;
  switch_update *su;
...
static bool gate_vrp(void) {
  return flag_tree_vrp != 0;
}

struct gimple_opt_pass pass_vrp = {{
    GIMPLE_PASS, "vrp",    /* name */
    gate_vrp,              /* gate */
    execute_vrp,           /* execute */
    NULL,                  /* sub */
    NULL,                  /* next */
    ...
    0,                     /* todo_flags_start */
    TODO_cleanup_cfg | TODO_ggc_collect | TODO_verify_ssa | TODO_dump_func |
