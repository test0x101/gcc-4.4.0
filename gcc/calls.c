...
static int compute_argument_block_size(int, struct args_size *, tree, tree,
                                       int);
static void initialize_argument_information(int, struct arg_data *,
                                            struct args_size *, int, tree, tree,
                                            tree, tree, CUMULATIVE_ARGS *, int,
                                            rtx *, int *, int *, int *, bool *,
                                            bool);
static void compute_argument_addresses(struct arg_data *, rtx, int);
static rtx rtx_for_function_call(tree, tree);
static void load_register_parameters(struct arg_data *, int, rtx *, int, int,
                                     int *);
...
rtx prepare_call_address(rtx funexp, rtx static_chain_value, rtx *call_fusage,
...
static void emit_call_1(rtx funexp, tree fntree, tree fndecl ATTRIBUTE_UNUSED,
                        tree funtype ATTRIBUTE_UNUSED,
                        HOST_WIDE_INT stack_size ATTRIBUTE_UNUSED,
                        HOST_WIDE_INT rounded_stack_size,
                        HOST_WIDE_INT struct_value_size ATTRIBUTE_UNUSED,
                        rtx next_arg_reg ATTRIBUTE_UNUSED, rtx valreg,
                        int old_inhibit_defer_pop, rtx call_fusage,
                        int ecf_flags,
  ...
  if ((ecf_flags & ECF_SIBCALL) && HAVE_sibcall_pop && HAVE_sibcall_value_pop &&
    ...
    if (valreg)
      pat = GEN_SIBCALL_VALUE_POP(valreg, gen_rtx_MEM(FUNCTION_MODE, funexp),
...
#if defined(HAVE_call) && defined(HAVE_call_value)
      if (HAVE_call && HAVE_call_value && HAVE_call_pop &&
          HAVE_call_value_pop && n_popped > 0)
#else
...
#endif
  {
    ...
    if (valreg)
      pat = GEN_CALL_VALUE_POP(valreg, gen_rtx_MEM(FUNCTION_MODE, funexp),
                               rounded_stack_size_rtx, next_arg_reg, n_pop);
    else
    ...
    if (valreg)
      emit_call_insn(
          GEN_SIBCALL_VALUE(valreg, gen_rtx_MEM(FUNCTION_MODE, funexp),
  ...
  if (n_popped > 0) {
    if (!already_popped)
      CALL_INSN_FUNCTION_USAGE(call_insn) = gen_rtx_EXPR_LIST(
          VOIDmode, gen_rtx_CLOBBER(VOIDmode, stack_pointer_rtx),
...
static int special_function_p(const_tree fndecl, int flags) {
  if (fndecl && DECL_NAME(fndecl) &&
    ...
    if (((IDENTIFIER_LENGTH(DECL_NAME(fndecl)) == 6 && name[0] == 'a' &&
          !strcmp(name, "alloca")) ||
    ...
    if (name[0] == '_') {
      if (name[1] == '_' && name[2] == 'b' && !strncmp(name + 3, "uiltin_", 7))
        tname += 10;
...
int setjmp_call_p(const_tree fndecl) {
  ...
  for (i = 0; i < num_actuals; i++)
    if (args[i].reg != 0 && !args[i].pass_on_stack) {
      *reg_parm_seen = 1;
      ...
      if (CONSTANT_P(args[i].value) && !LEGITIMATE_CONSTANT_P(args[i].value))
      ...
      if (GET_CODE(args[i].reg) == PARALLEL) {
        ...
        args[i].parallel_value = emit_group_load_into_temps(
      ...
      /* If the value is expensive, and we are inside an appropriately
         ...
         loading the parameters registers.  */
               ...
               rtx_cost(args[i].value, SET, optimize_insn_for_speed_p()) >
                   COSTS_N_INSNS(1) &&
               ((SMALL_REGISTER_CLASSES && *reg_parm_seen) || optimize))
...
static rtx save_fixed_argument_area(int reg_parm_stack_space, rtx argblock,
    ...
    if (stack_usage_map[low] != 0) {
      ...
      *high_to_save = high;
      ...
      stack_area = gen_rtx_MEM(
          save_mode, memory_address(save_mode, plus_constant(argblock, delta)));
...
static void restore_fixed_argument_area(rtx save_area, rtx argblock,
  ...
  for (i = 0; i < num_actuals; i++)
    if (args[i].reg != 0 && !args[i].pass_on_stack && args[i].mode == BLKmode &&
      ...
      if (bytes < UNITS_PER_WORD
          ...
          && (BLOCK_REG_PADDING(args[i].mode, TREE_TYPE(args[i].tree_value),
                                1) == downward)
#else
          && BYTES_BIG_ENDIAN
#endif
      ...
      for (j = 0; j < args[i].n_aligned_regs; j++) {
        ...
        word = extract_bit_field(word, bitsize, 0, 1, NULL_RTX, word_mode,
                                 word_mode);
        ...
        store_bit_field(reg, bitsize, endian_correction, word_mode, word);
      }
    }
}
...
static void initialize_argument_information(
    int num_actuals ATTRIBUTE_UNUSED, struct arg_data *args,
    struct args_size *args_size, int n_named_args ATTRIBUTE_UNUSED, tree exp,
    tree struct_value_addr_value, tree fndecl, tree fntype,
    CUMULATIVE_ARGS *args_so_far, int reg_parm_stack_space,
    rtx *old_stack_level, int *old_pending_adj, int *must_preallocate,
    int *ecf_flags, bool *may_tailcall, bool call_from_thunk_p) {
     ...
     complex arguments if necessary.  */
  {
    ...
    FOR_EACH_CALL_EXPR_ARG(arg, iter, exp) {
      ...
      if (targetm.calls.split_complex_arg && argtype &&
  ...
  for (argpos = 0; argpos < num_actuals; i += inc, argpos++) {
    ...
    if (pass_by_reference(args_so_far, TYPE_MODE(type), type,
                          argpos < n_named_args)) {
      bool callee_copies;
      tree base;

      callee_copies = reference_callee_copied(args_so_far, TYPE_MODE(type),
                                              type, argpos < n_named_args);

      /* If we're compiling a thunk, pass through invisible references
         instead of making a copy.  */
      if (call_from_thunk_p || (callee_copies && !TREE_ADDRESSABLE(type) &&
        ...
        if (!COMPLETE_TYPE_P(type) ||
            ...
            (flag_stack_check == GENERIC_STACK_CHECK &&
             compare_tree_int(TYPE_SIZE_UNIT(type), STACK_CHECK_MAX_VAR_SIZE) >
                 0)) {
          ...
          copy =
              gen_rtx_MEM(BLKmode, allocate_dynamic_stack_space(
        ...
        args[i].tree_value = build_fold_addr_expr(make_tree(type, copy));
        type = TREE_TYPE(args[i].tree_value);
        *may_tailcall = false;
      }
    }
    ...
    if (targetm.calls.promote_function_args(fndecl ? TREE_TYPE(fndecl)
                                                   : fntype))
    ...
    args[i].tail_call_reg =
        FUNCTION_INCOMING_ARG(*args_so_far, mode, type, argpos < n_named_args);
#else
    args[i].tail_call_reg = args[i].reg;
#endif

    if (args[i].reg)
      args[i].partial = targetm.calls.arg_partial_bytes(args_so_far, mode, type,
                                                        argpos < n_named_args);
    ...
    if (args[i].reg == 0 || args[i].partial != 0 || reg_parm_stack_space > 0 ||
      ...
      locate_and_pad_parm(mode, type,
                          ...
                          1,
                           ...
                           args[i].reg != 0,
#endif
                          args[i].pass_on_stack ? 0 : args[i].partial, fndecl,
                          args_size, &args[i].locate);
#ifdef BLOCK_REG_PADDING
    else
      /* The argument is passed entirely in registers.  See at which
         end it should be padded.  */
      args[i].locate.where_pad = BLOCK_REG_PADDING(
          mode, type, int_size_in_bytes(type) <= UNITS_PER_WORD);
#endif
...
static int compute_argument_block_size(
    int reg_parm_stack_space, struct args_size *args_size,
    tree fndecl ATTRIBUTE_UNUSED, tree fntype ATTRIBUTE_UNUSED,
  ...
  if (args_size->var) {
    ...
    if (reg_parm_stack_space > 0) {
      args_size->var =
          size_binop(MAX_EXPR, args_size->var, ssize_int(reg_parm_stack_space));

      /* The area corresponding to register parameters is not to count in
         the size of the block we need.  So make the adjustment.  */
      if (!OUTGOING_REG_PARM_STACK_SPACE(
              (!fndecl ? fntype : TREE_TYPE(fndecl))))
        args_size->var = size_binop(MINUS_EXPR, args_size->var,
    ...
    args_size->constant =
        (((args_size->constant + stack_pointer_delta +
           preferred_stack_boundary - 1) /
...
static void precompute_arguments(int num_actuals, struct arg_data *args) {
  int i;
  ...
  for (i = 0; i < num_actuals; i++) {
    ...
    args[i].initial_value = args[i].value = expand_normal(args[i].tree_value);

    mode = TYPE_MODE(TREE_TYPE(args[i].tree_value));
    if (mode != args[i].mode) {
      args[i].value =
          convert_modes(args[i].mode, mode, args[i].value, args[i].unsignedp);
#if defined(PROMOTE_FUNCTION_MODE) && !defined(PROMOTE_MODE)
      /* CSE will replace this only if it contains args[i].value
         pseudo, so convert it down to the declared mode using
         a SUBREG.  */
      if (REG_P(args[i].value) && GET_MODE_CLASS(args[i].mode) == MODE_INT) {
        args[i].initial_value = gen_lowpart_SUBREG(mode, args[i].value);
        SUBREG_PROMOTED_VAR_P(args[i].initial_value) = 1;
        SUBREG_PROMOTED_UNSIGNED_SET(args[i].initial_value, args[i].unsignedp);
      }
#endif
    }
  }
...
static int finalize_must_preallocate(int must_preallocate, int num_actuals,
                                     struct arg_data *args,
...
static void compute_argument_addresses(struct arg_data *args, rtx argblock,
                                       int num_actuals) {
  if (argblock) {
    ...
    for (i = 0; i < num_actuals; i++) {
      ...
      if (args[i].partial != 0) {
        ...
        partial_mode =
            mode_for_size(units_on_stack * BITS_PER_UNIT, MODE_INT, 1);
        args[i].stack = gen_rtx_MEM(partial_mode, addr);
        set_mem_size(args[i].stack, GEN_INT(units_on_stack));
      } else {
        args[i].stack = gen_rtx_MEM(args[i].mode, addr);
        set_mem_attributes(args[i].stack, TREE_TYPE(args[i].tree_value), 1);
      }
      align = BITS_PER_UNIT;
      ...
      if (args[i].partial != 0) {
        ...
        set_mem_attributes(args[i].stack_slot, TREE_TYPE(args[i].tree_value),
                           1);
      }
...
static bool mem_overlaps_already_clobbered_arg_p(rtx addr,
  ...
  if (size > 0) {
    ...
    for (k = 0; k < size; k++)
      if (i + k < stored_args_map->n_bits && TEST_BIT(stored_args_map, i + k))
        return true;
  }
...
static void load_register_parameters(struct arg_data *args, int num_actuals,
                                     rtx *call_fusage, int flags,
                                     int is_sibcall, int *sibcall_failure) {
  int i, j;
  ...
  for (i = 0; i < num_actuals; i++) {
    ...
    if (reg) {
      ...
      /* If simple case, just do move.  If normal partial, store_one_arg
        ...
        if (size < UNITS_PER_WORD && (args[i].locate.where_pad ==
      ...
      /* If we have pre-computed the values to put in the registers in
        ...
        if (nregs == 1 && size < UNITS_PER_WORD
          ...
          x = expand_shift(dir, word_mode, x, build_int_cst(NULL_TREE, shift),
                           ri, 1);
          if (x != ri)
            emit_move_insn(ri, x);
        } else
      ...
      /* When a parameter is a block, and perhaps in other cases, it is
         ...
         already clobbered.  */
      if (is_sibcall && check_sibcall_argument_overlap(before_arg, &args[i], 0))
        *sibcall_failure = 1;
...
static int combine_pending_stack_adjustment_and_call(
    int unadjusted_args_size, struct args_size *args_size,
  ...
  unadjusted_alignment = ((stack_pointer_delta + unadjusted_args_size) %
...
/* Scan sequence after INSN if it does not dereference any argument slots
   ...
   MARK_STORED_ARGS_MAP should be 0).  Return nonzero if sequence after INSN
   dereferences such argument slots, zero otherwise.  */
...
bool shift_return_value(enum machine_mode mode, bool left_p, rtx value) {
  HOST_WIDE_INT shift;
...
rtx expand_call(tree exp, rtx target, int ignore) {
  ...
  if (structure_value_addr && struct_value == 0) {
    ...
    rtx temp =
        (!REG_P(structure_value_addr) ||
                 (ACCUMULATE_OUTGOING_ARGS && stack_arg_under_construction &&
  ...
  if (type_arg_types != 0 && targetm.calls.strict_argument_naming(&args_so_far))
    ;
  ...
  initialize_argument_information(
      num_actuals, args, &args_size, n_named_args, exp,
      structure_value_addr_value, fndecl, fntype, &args_so_far,
      reg_parm_stack_space, &old_stack_level, &old_pending_adj,
      &must_preallocate, &flags, &try_tail_call, CALL_FROM_THUNK_P(exp));

  if (args_size.var)
    must_preallocate = 1;

  /* Now make final decision about preallocating stack space.  */
  must_preallocate = finalize_must_preallocate(must_preallocate, num_actuals,
                                               args, &args_size);
  ...
  if (currently_expanding_call++ != 0 || !flag_optimize_sibling_calls ||
      args_size.var || lookup_expr_eh_region(exp) >= 0 ||
  ...
  if (
      ...
      || args_size.constant > (crtl->args.size - crtl->args.pretend_args_size)
      /* If the callee pops its own arguments, then it must pop exactly
         the same number of arguments as the current function.  */
      ||
      (RETURN_POPS_ARGS(fndecl, funtype, args_size.constant) !=
       RETURN_POPS_ARGS(current_function_decl, TREE_TYPE(current_function_decl),
                        crtl->args.size)) ||
  ...
  if (try_tail_call) {
    ...
    if (targetm.calls.promote_function_return(TREE_TYPE(current_function_decl)))
      caller_promoted_mode = promote_mode(TREE_TYPE(caller_res), caller_mode,
                                          &caller_unsignedp, 1);
    if (targetm.calls.promote_function_return(funtype))
      callee_promoted_mode =
          promote_mode(TREE_TYPE(funtype), callee_mode, &callee_unsignedp, 1);
    if (caller_mode != VOIDmode &&
        (caller_promoted_mode != callee_promoted_mode ||
         ((caller_mode != caller_promoted_mode ||
           callee_mode != callee_promoted_mode) &&
          (caller_unsignedp != callee_unsignedp ||
           GET_MODE_BITSIZE(caller_mode) < GET_MODE_BITSIZE(callee_mode)))))
      try_tail_call = 0;
  }
  ...
  for (pass = try_tail_call ? 0 : 1; pass < 2; pass++) {
    ...
    if (pending_stack_adjust >= 32 ||
        ...
        (pending_stack_adjust > 0 && flag_exceptions &&
         !(flags & ECF_NOTHROW)) ||
        pass == 0)
    ...
    /* Compute the actual size of the argument block required.  The variable
       ...
       caller provided.  */
    unadjusted_args_size = compute_argument_block_size(
        reg_parm_stack_space, &adjusted_args_size, fndecl, fntype,
        (pass == 0 ? 0 : preferred_stack_boundary));
        ...
        if (ACCUMULATE_OUTGOING_ARGS) {
          ...
          if (inhibit_defer_pop == 0) {
            /* Try to reuse some or all of the pending_stack_adjust
               to get this space.  */
            needed = (combine_pending_stack_adjustment_and_call(
                unadjusted_args_size, &adjusted_args_size,
                preferred_unit_stack_boundary));
    ...
    if (PUSH_ARGS_REVERSED && argblock == 0 &&
      ...
      if (pending_stack_adjust && !inhibit_defer_pop) {
        pending_stack_adjust = (combine_pending_stack_adjustment_and_call(
            unadjusted_args_size, &adjusted_args_size,
            preferred_unit_stack_boundary));
        do_pending_stack_adjust();
      } else if (argblock == 0)
        anti_adjust_stack(
            GEN_INT(adjusted_args_size.constant - unadjusted_args_size));
    }
    ...
    if (TYPE_MODE(TREE_TYPE(exp)) != VOIDmode && !structure_value_addr) {
      if (pcc_struct_value)
        ...
        valreg =
            hard_function_value(TREE_TYPE(exp), fndecl, fntype, (pass == 0));
    ...
    for (i = 0; i < num_actuals; i++) {
      if (args[i].reg == 0 || args[i].pass_on_stack) {
        ...
        if (store_one_arg(&args[i], argblock, flags,
            ...
            (pass == 0 &&
             check_sibcall_argument_overlap(before_arg, &args[i], 1)))
          sibcall_failure = 1;
      }
      ...
      if (((flags & ECF_CONST) ||
          ...
          args[i].stack)
        call_fusage = gen_rtx_EXPR_LIST(
            VOIDmode, gen_rtx_USE(VOIDmode, args[i].stack), call_fusage);
    }
    ...
    /* If register arguments require space on the stack and stack space
       ...
       passed in registers.  */
    if (OUTGOING_REG_PARM_STACK_SPACE((!fndecl ? fntype : TREE_TYPE(fndecl))) &&
        !ACCUMULATE_OUTGOING_ARGS && must_preallocate == 0 &&
    ...
    funexp = prepare_call_address(funexp, static_chain_value, &call_fusage,
                                  reg_parm_seen, pass == 0);

    load_register_parameters(args, num_actuals, &call_fusage, flags, pass == 0,
                             &sibcall_failure);
    ...
    if (pass == 0)
      next_arg_reg =
          FUNCTION_INCOMING_ARG(args_so_far, VOIDmode, void_type_node, 1);
    else
#endif
    ...
    gcc_assert(!pass || !(stack_pointer_delta % preferred_unit_stack_boundary));

    /* Generate the actual call instruction.  */
    emit_call_1(funexp, exp, fndecl, funtype, unadjusted_args_size,
                adjusted_args_size.constant, struct_value_size, next_arg_reg,
                valreg, old_inhibit_defer_pop, call_fusage, flags,
                &args_so_far);

    /* If the call setup or the call itself overlaps with anything
       of the argument setup we probably clobbered our call address.
       In that case we can't do sibcalls.  */
    if (pass == 0 && check_sibcall_argument_overlap(after_args, 0, 0))
      sibcall_failure = 1;
    ...
    if (!structure_value_addr && !pcc_struct_value &&
    ...
    /* Handle calls that return values in multiple non-contiguous locations.
      ...
      if (target == 0) {
        ...
        tree nt = build_qualified_type(
            TREE_TYPE(exp), (TYPE_QUALS(TREE_TYPE(exp)) | TYPE_QUAL_CONST));
    ...
    if (targetm.calls.promote_function_return(funtype)) {
      ...
      if (REG_P(target) && TYPE_MODE(TREE_TYPE(exp)) != BLKmode &&
        ...
        if ((WORDS_BIG_ENDIAN || BYTES_BIG_ENDIAN) &&
          ...
          offset =
              GET_MODE_SIZE(GET_MODE(target)) - GET_MODE_SIZE(TYPE_MODE(type));
          if (!BYTES_BIG_ENDIAN)
...
#ifdef REG_PARM_STACK_SPACE
      if (save_area)
        restore_fixed_argument_area(save_area, argblock, high_to_save,
                                    low_to_save);
#endif
      ...
      for (i = 0; i < num_actuals; i++)
        if (args[i].save_area) {
          ...
          rtx stack_area = gen_rtx_MEM(
              save_mode,
              memory_address(save_mode, XEXP(args[i].stack_slot, 0)));
...
static rtx emit_library_call_value_1(int retval, rtx orgfun, rtx value,
  ...
  tree fndecl ATTRIBUTE_UNUSED =
      NULL_TREE; /* library calls default to host calling abi ? */
...
#ifdef PCC_STATIC_STRUCT_RETURN
      rtx pointer_reg = hard_function_value(build_pointer_type(tfom), 0, 0, 0);
  ...
  if (mem_value && struct_value == 0 && !pcc_struct_value) {
    ...
    gcc_assert(targetm.calls.arg_partial_bytes(&args_so_far, Pmode, NULL_TREE,
                                               1) == 0);
  ...
  for (; count < nargs; count++) {
    ...
    if (pass_by_reference(&args_so_far, mode, NULL_TREE, 1)) {
      ...
      if (MEM_P(val) && !must_copy)
        ...
        slot = assign_temp(lang_hooks.types.type_for_mode(mode, 0), 0, 1, 1);
        emit_move_insn(slot, val);
      }

      call_fusage =
          gen_rtx_EXPR_LIST(VOIDmode, gen_rtx_USE(VOIDmode, slot), call_fusage);
      if (must_copy)
        call_fusage = gen_rtx_EXPR_LIST(
            VOIDmode, gen_rtx_CLOBBER(VOIDmode, slot), call_fusage);
    ...
    locate_and_pad_parm(mode, NULL_TREE,
                        ...
                        1,
                         ...
                         argvec[count].reg != 0,
#endif
                        argvec[count].partial, NULL_TREE, &args_size,
                        &argvec[count].locate);
  ...
  args_size.constant =
      (((args_size.constant + stack_pointer_delta + STACK_BYTES - 1) /
  ...
  if (argblock == 0 && PUSH_ARGS_REVERSED)
    anti_adjust_stack(
        GEN_INT(args_size.constant - original_args_size.constant));
    ...
    if (!(reg != 0 && partial == 0)) {
      if (ACCUMULATE_OUTGOING_ARGS) {
        ...
        if (i < upper_bound) {
          ...
          rtx adr =
              plus_constant(argblock, argvec[argnum].locate.offset.constant);
          rtx stack_area =
              gen_rtx_MEM(save_mode, memory_address(save_mode, adr));

          if (save_mode == BLKmode) {
            argvec[argnum].save_area = assign_stack_temp(
                BLKmode, argvec[argnum].locate.size.constant, 0);

            emit_block_move(validize_mem(argvec[argnum].save_area), stack_area,
      ...
      emit_push_insn(val, mode, NULL_TREE, NULL_RTX, PARM_BOUNDARY, partial,
                     reg, 0, argblock,
      ...
      if ((flags & ECF_CONST) ||
        ...
        /* Indicate argument access so that alias.c knows that these
           values are live.  */
        if (argblock)
          use = plus_constant(argblock, argvec[argnum].locate.offset.constant);
        else
  ...
  if (mem_value != 0 && struct_value != 0 && !pcc_struct_value) {
    emit_move_insn(
        struct_value,
        force_reg(Pmode, force_operand(XEXP(mem_value, 0), NULL_RTX)));
    if (REG_P(struct_value))
      use_reg(&call_fusage, struct_value);
  }
  ...
  NO_DEFER_POP;
  valreg = (mem_value == 0 && outmode != VOIDmode ? hard_libcall_value(outmode)
                                                  : NULL_RTX);

  /* Stack must be properly aligned now.  */
  gcc_assert(
      !(stack_pointer_delta & (PREFERRED_STACK_BOUNDARY / BITS_PER_UNIT - 1)));
  ...
  emit_call_1(fun, NULL, get_identifier(XSTR(orgfun, 0)),
              build_function_type(tfom, NULL_TREE), original_args_size.constant,
              args_size.constant, struct_value_size,
              FUNCTION_ARG(args_so_far, VOIDmode, void_type_node, 1), valreg,
  ...
  if (ACCUMULATE_OUTGOING_ARGS) {
    ...
    for (count = 0; count < nargs; count++)
      if (argvec[count].save_area) {
        ...
        rtx adr = plus_constant(argblock, argvec[count].locate.offset.constant);
        rtx stack_area = gen_rtx_MEM(save_mode, memory_address(save_mode, adr));

        if (save_mode == BLKmode)
          emit_block_move(stack_area, validize_mem(argvec[count].save_area),
...
void emit_library_call(rtx orgfun, enum libcall_type fn_type,
                       enum machine_mode outmode, int nargs, ...) {
  va_list p;
...
rtx emit_library_call_value(rtx orgfun, rtx value, enum libcall_type fn_type,
                            enum machine_mode outmode, int nargs, ...) {
  rtx result;
  va_list p;
  ...
  va_start(p, nargs);
  result =
      emit_library_call_value_1(1, orgfun, value, fn_type, outmode, nargs, p);
  va_end(p);
...
static int store_one_arg(struct arg_data *arg, rtx argblock, int flags,
    ...
    if (argblock && !variable_size && arg->stack) {
      ...
      if (i < upper_bound) {
        ...
        if (save_mode == BLKmode) {
          ...
          tree nt =
              build_qualified_type(ot, (TYPE_QUALS(ot) | TYPE_QUAL_CONST));
  ...
  if (arg->value == 0) {
    ...
    arg->value = expand_expr(
        pval,
        (partial || TYPE_MODE(TREE_TYPE(pval)) != arg->mode) ? NULL_RTX
                                                             : arg->stack,
  ...
  if (arg->value == arg->stack)
    ...
    emit_push_insn(arg->value, arg->mode, TREE_TYPE(pval), NULL_RTX, parm_align,
    ...
    if (arg->locate.size.var != 0) {
      ...
      excess = (arg->locate.size.constant - int_size_in_bytes(TREE_TYPE(pval)) +
                partial);
      size_rtx = expand_expr(size_in_bytes(TREE_TYPE(pval)), NULL_RTX,
    ...
    if ((flags & ECF_SIBCALL) && MEM_P(arg->value)) {
      ...
      if (XEXP(x, 0) == crtl->args.internal_arg_pointer ||
          (GET_CODE(XEXP(x, 0)) == PLUS &&
           XEXP(XEXP(x, 0), 0) == crtl->args.internal_arg_pointer &&
    ...
    emit_push_insn(arg->value, arg->mode, TREE_TYPE(pval), size_rtx, parm_align,
  ...
  if (arg->reg && GET_CODE(arg->reg) == PARALLEL) {
    ...
    arg->parallel_value = emit_group_load_into_temps(arg->reg, arg->value, type,
                                                     int_size_in_bytes(type));
  }

  /* Mark all slots this store used.  */
  if (ACCUMULATE_OUTGOING_ARGS && !(flags & ECF_SIBCALL) && argblock &&
...
bool must_pass_in_stack_var_size(enum machine_mode mode ATTRIBUTE_UNUSED,
                                 const_tree type) {
  if (!type)
    return false;
...
bool must_pass_in_stack_var_size_or_pad(enum machine_mode mode,
                                        const_tree type) {
  if (!type)
    return false;
