...
static inline void pp_clear_state(pretty_printer *pp) {
...
static void pp_wrap_text(pretty_printer *pp, const char *start,
                         const char *end) {
...
static inline void pp_maybe_wrap_text(pretty_printer *pp, const char *start,
                                      const char *end) {
  if (pp_is_wrapping_line(pp))
...
static inline void pp_append_r(pretty_printer *pp, const char *start,
                               int length) {
...
void pp_base_indent(pretty_printer *pp) {
...
void pp_base_format(pretty_printer *pp, text_info *text) {
  ...
  for (p = text->format_spec; *p;) {
    ...
    switch (*++p) {
      ...
      case '<':
        obstack_grow(&buffer->chunk_obstack, open_quote, strlen(open_quote));
        p++;
        continue;

      case '>':
      case '\'':
        obstack_grow(&buffer->chunk_obstack, close_quote, strlen(close_quote));
        p++;
        continue;
    ...
    switch (*p) {
      ...
      case 'i':
        if (wide)
          ...
          pp_integer_with_precision(pp, *text->args_ptr, precision, int, "d");
        break;
      ...
      case 'o':
        if (wide)
          ...
          pp_integer_with_precision(pp, *text->args_ptr, precision, unsigned,
                                    "o");
        break;
      ...
      default: {
        ...
        ok = pp_format_decoder(pp)(pp, text, p, precision, wide, plus, hash);
        gcc_assert(ok);
      }
    }
...
void pp_base_output_formatted_text(pretty_printer *pp) {
  unsigned int chunk;
...
void pp_base_format_verbatim(pretty_printer *pp, text_info *text) {
  /* Set verbatim mode.  */
...
void pp_base_flush(pretty_printer *pp) {
...
/* Sets the number of maximum characters per line PRETTY-PRINTER can
   ...
   line-wrapping.  */
void pp_base_set_line_maximum_length(pretty_printer *pp, int length) {
  pp_line_cutoff(pp) = length;
  pp_set_real_maximum_length(pp);
}

/* Clear PRETTY-PRINTER output area text info.  */
void pp_base_clear_output_area(pretty_printer *pp) {
  obstack_free(pp->buffer->obstack, obstack_base(pp->buffer->obstack));
  pp->buffer->line_length = 0;
}

/* Set PREFIX for PRETTY-PRINTER.  */
void pp_base_set_prefix(pretty_printer *pp, const char *prefix) {
  pp->prefix = prefix;
  pp_set_real_maximum_length(pp);
  pp->emitted_prefix = false;
  pp_indentation(pp) = 0;
...
void pp_base_destroy_prefix(pretty_printer *pp) {
  if (pp->prefix != NULL) {
...
void pp_base_emit_prefix(pretty_printer *pp) {
  if (pp->prefix != NULL) {
...
/* Construct a PRETTY-PRINTER with PREFIX and of MAXIMUM_LENGTH
   characters per line.  */
void pp_construct(pretty_printer *pp, const char *prefix, int maximum_length) {
...
/* Append a string delimited by START and END to the output area of
   ...
   safe to do so.  */
void pp_base_append_text(pretty_printer *pp, const char *start,
                         const char *end) {
...
const char *pp_base_formatted_text(pretty_printer *pp) {
...
const char *pp_base_last_position_in_text(const pretty_printer *pp) {
  const char *p = NULL;
...
int pp_base_remaining_character_count_for_line(pretty_printer *pp) {
...
void pp_base_newline(pretty_printer *pp) {
  obstack_1grow(pp->buffer->obstack, '\n');
  pp->buffer->line_length = 0;
}

/* Have PRETTY-PRINTER add a CHARACTER.  */
void pp_base_character(pretty_printer *pp, int c) {
  if (pp_is_wrapping_line(pp) &&
...
void pp_base_string(pretty_printer *pp, const char *str) {
  pp_maybe_wrap_text(pp, str, str + (str ? strlen(str) : 0));
}

/* Maybe print out a whitespace if needed.  */

void pp_base_maybe_space(pretty_printer *pp) {
