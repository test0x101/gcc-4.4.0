...
static GTY((if_marked("tree_map_marked_p"),
            param_is(struct tree_map))) htab_t heapvar_for_stmt;
...
static inline varinfo_t get_varinfo(unsigned int n) {
  return VEC_index(varinfo_t, varmap, n);
}

/* Return the varmap element N, following the collapsed_to link.  */

static inline varinfo_t get_varinfo_fc(unsigned int n) {
...
static varinfo_t new_var_info(tree t, unsigned int id, const char *name) {
  ...
  ret->no_tbaa_pruning =
      (DECL_P(var) && POINTER_TYPE_P(TREE_TYPE(var)) && DECL_NO_TBAA_P(var));
...
void dump_constraint_edge(FILE *file, constraint_t c) {
  if (c->rhs.type != ADDRESSOF) {
...
static void constraint_set_union(VEC(constraint_t, heap) * *to,
  ...
  for (i = 0; VEC_iterate(constraint_t, *from, i, c); i++) {
    if (constraint_vec_find(*to, *c) == NULL) {
      unsigned int place =
          VEC_lower_bound(constraint_t, *to, c, constraint_less);
...
static void solution_set_add(bitmap set, unsigned HOST_WIDE_INT offset) {
  ...
  EXECUTE_IF_SET_IN_BITMAP(set, 0, i, bi) {
    ...
    /* If this is a variable with just one field just set its bit
       in the result.  */
    if (vi->is_artificial_var || vi->is_unknown_size_var || vi->is_full_var)
      ...
      /* If the result is not exactly at fieldoffset include the next
         ...
         rationale.  */
      if (v->offset != fieldoffset && v->next != NULL)
...
static bool set_union_with_increment(bitmap to, bitmap from,
...
static void insert_into_complex(constraint_graph_t graph, unsigned int var,
                                constraint_t c) {
  VEC(constraint_t, heap) *complex = graph->complex[var];
  unsigned int place =
      VEC_lower_bound(constraint_t, complex, c, constraint_less);
...
static void merge_node_constraints(constraint_graph_t graph, unsigned int to,
                                   unsigned int from) {
  unsigned int i;
  constraint_t c;
...
static void merge_graph_nodes(constraint_graph_t graph, unsigned int to,
                              unsigned int from) {
  ...
  if (graph->succs[from]) {
    ...
    bitmap_ior_into(graph->succs[to], graph->succs[from]);
  }
...
static void add_implicit_graph_edge(constraint_graph_t graph, unsigned int to,
                                    unsigned int from) {
  if (to == from)
    return;
...
static void add_pred_graph_edge(constraint_graph_t graph, unsigned int to,
                                unsigned int from) {
  if (!graph->preds[to])
...
static bool add_graph_edge(constraint_graph_t graph, unsigned int to,
                           unsigned int from) {
  if (to == from) {
    return false;
  } else {
...
static bool valid_graph_edge(constraint_graph_t graph, unsigned int src,
                             unsigned int dest) {
  return (graph->succs[dest] && bitmap_bit_p(graph->succs[dest], src));
}
...
static void build_pred_graph(void) {
  ...
  for (i = 0; VEC_iterate(constraint_t, constraints, i, c); i++) {
    ...
    } else if (lhsvar > anything_id && lhsvar != rhsvar && lhs.offset == 0 &&
               rhs.offset == 0) {
      /* x = y */
...
static void scc_visit(constraint_graph_t graph, struct scc_info *si,
                      unsigned int n) {
  unsigned int i;
  bitmap_iterator bi;
  unsigned int my_dfs;
...
static void unify_nodes(constraint_graph_t graph, unsigned int to,
                        unsigned int from, bool update_changed) {
...
static struct topo_info *init_topo_info(void) {
...
static void topo_visit(constraint_graph_t graph, struct topo_info *ti,
                       unsigned int n) {
  bitmap_iterator bi;
  unsigned int j;
...
static bool type_safe(unsigned int n, unsigned HOST_WIDE_INT *offset) {
  ...
  /* For things we've globbed to single variables, any offset into the
     ...
     0.  */
  if (ninfo->is_special_var || ninfo->is_artificial_var ||
      ninfo->is_unknown_size_var || ninfo->is_full_var) {
    *offset = 0;
    return true;
  }
...
static void do_sd_constraint(constraint_graph_t graph, constraint_t c,
                             bitmap delta) {
  ...
  if (c->rhs.var == find(escaped_id) || c->rhs.var == find(callused_id)) {
    bitmap vars = NULL;
...
static void do_ds_constraint(constraint_t c, bitmap delta) {
...
static void do_complex_constraint(constraint_graph_t graph, constraint_t c,
                                  bitmap delta) {
  if (c->lhs.type == DEREF) {
    if (c->rhs.type == ADDRESSOF) {
      gcc_unreachable();
...
static struct scc_info *init_scc_info(size_t size) {
...
static hashval_t equiv_class_label_hash(const void *p) {
...
static int equiv_class_label_eq(const void *p1, const void *p2) {
...
static unsigned int equiv_class_lookup(htab_t table, bitmap labels) {
  void **slot;
  ...
  slot = htab_find_slot_with_hash(table, &ecl, ecl.hashcode, NO_INSERT);
  if (!slot)
    return 0;
  else
...
static void equiv_class_add(htab_t table, unsigned int equivalence_class,
                            bitmap labels) {
  void **slot;
  ...
  slot = htab_find_slot_with_hash(table, ecl, ecl->hashcode, INSERT);
  gcc_assert(!*slot);
  *slot = (void *)ecl;
}
...
static void condense_visit(constraint_graph_t graph, struct scc_info *si,
                           unsigned int n) {
  unsigned int i;
  bitmap_iterator bi;
  unsigned int my_dfs;
  ...
  if (si->dfs[n] == my_dfs) {
    while (VEC_length(unsigned, si->scc_stack) != 0 &&
      ...
      if (graph->implicit_preds[w]) {
        ...
        bitmap_ior_into(graph->implicit_preds[n], graph->implicit_preds[w]);
      }
...
static void label_visit(constraint_graph_t graph, struct scc_info *si,
                        unsigned int n) {
  unsigned int i;
  bitmap_iterator bi;
  SET_BIT(si->visited, n);
  ...
  if (!bitmap_empty_p(graph->points_to[n])) {
    unsigned int label =
        equiv_class_lookup(pointer_equiv_class_table, graph->points_to[n]);
    if (!label) {
      label = pointer_equiv_class++;
      equiv_class_add(pointer_equiv_class_table, label, graph->points_to[n]);
    }
...
static struct scc_info *perform_var_substitution(constraint_graph_t graph) {
  unsigned int i;
  unsigned int size = graph->size;
  struct scc_info *si = init_scc_info(size);
  ...
  pointer_equiv_class_table =
      htab_create(511, equiv_class_label_hash, equiv_class_label_eq, free);
  ...
  for (i = 0; i < FIRST_REF_NODE; i++) {
    ...
    EXECUTE_IF_SET_IN_BITMAP(graph->pointed_by[i], 0, j, bi) {
      bitmap_set_bit(pointed_by, graph->pointer_label[si->node_mapping[j]]);
    }
    ...
    /* Look up the location equivalence label if one exists, or make
       one otherwise.  */
    label = equiv_class_lookup(location_equiv_class_table, pointed_by);
    if (label == 0) {
      label = location_equiv_class++;
      equiv_class_add(location_equiv_class_table, label, pointed_by);
    } else {
      if (dump_file && (dump_flags & TDF_DETAILS))
  ...
  if (dump_file && (dump_flags & TDF_DETAILS))
    for (i = 0; i < FIRST_REF_NODE; i++) {
      ...
      fprintf(dump_file,
              ...
              get_varinfo(i)->name, graph->pointer_label[si->node_mapping[i]],
  ...
  for (i = 0; i < FIRST_REF_NODE; i++) {
    ...
    if (graph->pointer_label[node] == 0) {
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf(dump_file, "%s is a non-pointer variable, eliminating edges.\n",
...
static void free_var_substitution_info(struct scc_info *si) {
  free_scc_info(si);
...
static bool eliminate_indirect_cycles(unsigned int node) {
  if (graph->indirect_cycles[node] != -1 &&
    ...
    for (queuepos = 0; VEC_iterate(unsigned, queue, queuepos, i); queuepos++) {
    ...
    while (VEC_length(unsigned, ti->topo_order) != 0) {
      ...
      if (TEST_BIT(changed, i)) {
        ...
        if (!solution_empty
            ...
            && i != find(escaped_id) && i != find(callused_id)) {
          bitmap_iterator bi;
...
static const char *alias_get_name(tree decl) {
  ...
  if (TREE_CODE(decl) == SSA_NAME) {
    num_printed = asprintf(&temp, "%s_%u", alias_get_name(SSA_NAME_VAR(decl)),
...
static struct constraint_expr get_constraint_exp_for_temp(tree t) {
...
static void get_constraint_for_ssa_var(tree t, VEC(ce_s, heap) * *results,
                                       bool address_p) {
  struct constraint_expr cexpr;
  varinfo_t vi;
  ...
  /* For parameters, get at the points-to set for the actual parm
     decl.  */
  if (TREE_CODE(t) == SSA_NAME && TREE_CODE(SSA_NAME_VAR(t)) == PARM_DECL &&
...
static bool type_could_have_pointers(tree type) {
...
static void get_constraint_for_ptr_offset(tree ptr, tree offset,
  ...
  for (j = 0; j < n; j++) {
    ...
    if (c->type == ADDRESSOF && !curr->is_full_var) {
      ...
      /* If the found variable is not exactly at the pointed to
         ...
         solution.  */
      if (temp->offset != curr->offset + rhsoffset && temp->next != NULL) {
...
static void get_constraint_for_component_ref(tree t, VEC(ce_s, heap) * *results,
  ...
  if (result->type == SCALAR && get_varinfo(result->var)->is_full_var)
    ...
    if ((unsigned HOST_WIDE_INT)bitpos < get_varinfo(result->var)->fullsize &&
      ...
      for (curr = get_varinfo(cexpr.var); curr; curr = curr->next) {
        if (ranges_overlap_p(curr->offset, curr->size, bitpos, bitmaxsize)) {
          cexpr.var = curr->id;
          VEC_safe_push(ce_s, heap, *results, &cexpr);
          if (address_p)
            break;
...
static void do_deref(VEC(ce_s, heap) * *constraints) {
  struct constraint_expr *c;
  unsigned int i = 0;
...
static void get_constraint_for_1(tree t, VEC(ce_s, heap) * *results,
                                 bool address_p) {
  struct constraint_expr temp;
  ...
  if (flag_delete_null_pointer_checks && TREE_CODE(t) == INTEGER_CST &&
      integer_zerop(t)) {
    temp.var = nothing_id;
    temp.type = ADDRESSOF;
    temp.offset = 0;
...
static void get_constraint_for(tree t, VEC(ce_s, heap) * *results) {
...
static void handle_rhs_call(gimple stmt) {
  unsigned i;
...
static void handle_lhs_call(tree lhs, int flags) {
  ...
  if (flags & ECF_MALLOC) {
    ...
    rhsc.var = create_variable_info_for(heapvar, alias_get_name(heapvar));
...
static void handle_const_call(gimple stmt) {
...
static void handle_pure_call(gimple stmt) {
  unsigned i;
  ...
  if (gimple_call_lhs(stmt) && could_have_pointers(gimple_call_lhs(stmt)) &&
...
static void find_func_aliases(gimple origt) {
  gimple t = origt;
  VEC(ce_s, heap) *lhsc = NULL;
  VEC(ce_s, heap) *rhsc = NULL;
  struct constraint_expr *c;
  ...
  else if (is_gimple_assign(t) && could_have_pointers(gimple_assign_lhs(t))) {
  ...
  if (stmt_escape_type == ESCAPE_STORED_IN_GLOBAL) {
    ...
    if (gimple_assign_rhs_code(t) == ADDR_EXPR) {
      ...
      if (base && (!DECL_P(base) || !is_global_var(base)))
...
static void sort_fieldstack(VEC(fieldoff_s, heap) * fieldstack) {
  qsort(VEC_address(fieldoff_s, fieldstack), VEC_length(fieldoff_s, fieldstack),
        sizeof(fieldoff_s), fieldoff_compare);
}
...
static inline bool var_can_have_subvars(const_tree v) {
...
static int push_fields_onto_fieldstack(tree type,
    ...
    if (TREE_CODE(field) == FIELD_DECL) {
      ...
      if (!var_can_have_subvars(field) ||
        ...
        push = true;
               ...
               (DECL_SIZE(field) && !integer_zerop(DECL_SIZE(field))))
      ...
      if (push) {
        ...
        if (!DECL_SIZE(field) || !host_integerp(DECL_SIZE(field), 1))
          has_unknown_size = true;

        /* If adjacent fields do not contain pointers merge them.  */
        if (pair && !pair->may_have_pointers && !could_have_pointers(field) &&
...
static unsigned int create_function_info_for(tree decl, const char *name) {
  ...
  if (DECL_RESULT(decl) != NULL || !VOID_TYPE_P(TREE_TYPE(TREE_TYPE(decl)))) {
    varinfo_t resultvi;
    const char *newname;
    char *tempname;
    unsigned int newindex;
...
static unsigned int create_variable_info_for(tree decl, const char *name) {
  ...
  if (var_can_have_subvars(decl) && use_field_sensitive &&
      (!var_ann(decl) || var_ann(decl)->noalias_state == 0) &&
      (!var_ann(decl) || !var_ann(decl)->is_heapvar))
  ...
  if (is_global && (!flag_whole_program || !in_ipa_mode) &&
    ...
    if (var_ann(decl) && var_ann(decl)->noalias_state == NO_ALIAS_ANYTHING)
  ...
  stats.total_vars++;
  if (use_field_sensitive && !vi->is_unknown_size_var &&
      var_can_have_subvars(decl) && VEC_length(fieldoff_s, fieldstack) > 1 &&
    ...
    for (i = VEC_length(fieldoff_s, fieldstack) - 1;
         i >= 1 && VEC_iterate(fieldoff_s, fieldstack, i, fo); i--) {
      varinfo_t newvi;
...
static void intra_create_variable_infos(void) {
  ...
  for (t = DECL_ARGUMENTS(current_function_decl); t; t = TREE_CHAIN(t)) {
    ...
    if (POINTER_TYPE_P(TREE_TYPE(t)) && flag_argument_noalias > 0) {
      ...
      if (heapvar == NULL_TREE) {
        var_ann_t ann;
        heapvar = create_tmp_var_raw(ptr_type_node, "PARM_NOALIAS");
        DECL_EXTERNAL(heapvar) = 1;
        if (gimple_referenced_vars(cfun))
          add_referenced_var(heapvar);
  ...
  if (DECL_RESULT(cfun->decl) && DECL_BY_REFERENCE(DECL_RESULT(cfun->decl))) {
...
static hashval_t shared_bitmap_hash(const void *p) {
...
static int shared_bitmap_eq(const void *p1, const void *p2) {
...
static bitmap shared_bitmap_lookup(bitmap pt_vars) {
  ...
  slot = htab_find_slot_with_hash(shared_bitmap_table, &sbi, sbi.hashcode,
                                  NO_INSERT);
  if (!slot)
    return NULL;
  else
...
static void shared_bitmap_add(bitmap pt_vars) {
  ...
  slot =
      htab_find_slot_with_hash(shared_bitmap_table, sbi, sbi->hashcode, INSERT);
  gcc_assert(!*slot);
  *slot = (void *)sbi;
}
...
static unsigned set_uids_in_ptset(tree ptr, bitmap into, bitmap from,
                                  bool is_derefed, bool no_tbaa_pruning) {
  unsigned int i;
  bitmap_iterator bi;
  unsigned pruned = 0;
  ...
  EXECUTE_IF_SET_IN_BITMAP(from, 0, i, bi) {
    ...
    if (TREE_CODE(vi->decl) == VAR_DECL || TREE_CODE(vi->decl) == PARM_DECL ||
      ...
      if (vi->is_artificial_var || !is_derefed || no_tbaa_pruning ||
          vi->no_tbaa_pruning)
        ...
        if (may_alias_p(SSA_NAME_VAR(ptr), mem_alias_set, vi->decl,
                        var_alias_set, true))
          bitmap_set_bit(into, DECL_UID(vi->decl));
        else
          ++pruned;
...
static void emit_pointer_definition(tree ptr, bitmap visited) {
...
static void emit_alias_warning(tree ptr) {
  gimple use;
  imm_use_iterator ui;
  bool warned = false;
...
bool find_what_p_points_to(tree p) {
  ...
  /* For parameters, get at the points-to set for the actual parm
     decl.  */
  if (TREE_CODE(p) == SSA_NAME && TREE_CODE(SSA_NAME_VAR(p)) == PARM_DECL &&
    ...
    if (vi->size != vi->fullsize) {
      ...
      EXECUTE_IF_SET_IN_BITMAP(vi->solution, 0, i, bi) {
        ...
        if (vi->is_artificial_var) {
          ...
          if (vi->id == nothing_id)
            pi->pt_null = 1;
          else if (vi->id == anything_id || vi->id == nonlocal_id ||
                   vi->id == escaped_id || vi->id == callused_id)
            was_pt_anything = 1;
      ...
      pruned = set_uids_in_ptset(p, finished_solution, vi->solution,
                                 pi->is_dereferenced, vi->no_tbaa_pruning);
      ...
      if (bitmap_empty_p(pi->pt_vars)) {
        pi->pt_vars = NULL;
        if (pruned > 0 && !pi->pt_null && pi->is_dereferenced &&
            warn_strict_aliasing > 0 && !SSA_NAME_IS_DEFAULT_DEF(p)) {
          if (dump_file && dump_flags & TDF_DETAILS) {
...
bool clobber_what_escaped(void) {
  ...
  EXECUTE_IF_SET_IN_BITMAP(vi->solution, 0, i, bi) {
    ...
    if (vi->is_artificial_var) {
      ...
      if (vi->id == anything_id || vi->id == integer_id)
        return false;
    }
...
void compute_call_used_vars(void) {
  ...
  if (has_anything_id)
    bitmap_ior_into(gimple_call_used_vars(cfun), gimple_addressable_vars(cfun));
}
...
static void init_base_vars(void) {
  ...
  var_storedanything =
      new_var_info(storedanything_tree, storedanything_id, "STOREDANYTHING");
...
static void init_alias_vars(void) {
  ...
  constraint_pool =
      create_alloc_pool("Constraint pool", sizeof(struct constraint), 30);
  variable_info_pool =
      create_alloc_pool("Variable info pool", sizeof(struct variable_info), 30);
  ...
  shared_bitmap_table =
      htab_create(511, shared_bitmap_hash, shared_bitmap_eq, free);
  init_base_vars();
}
...
static void remove_preds_and_fake_succs(constraint_graph_t graph) {
  ...
  for (i = 0; i < FIRST_REF_NODE; i++) {
    if (graph->succs[i])
      bitmap_clear_range(graph->succs[i], FIRST_REF_NODE, FIRST_REF_NODE * 2);
  }
...
static void compute_tbaa_pruning(void) {
...
void compute_points_to_sets(void) {
  struct scc_info *si;
  basic_block bb;
...
static unsigned int ipa_pta_execute(void) {
  ...
  for (node = cgraph_nodes; node; node = node->next) {
    if (!node->analyzed || cgraph_is_master_clone(node)) {
      ...
      varid = create_function_info_for(node->decl, cgraph_node_name(node));
