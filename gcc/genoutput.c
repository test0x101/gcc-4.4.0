...
static struct operand_data null_operand = {0, 0, "", "", VOIDmode,
                                           0, 0, 0,  0,  0};
...
static void output_insn_data(void) {
  ...
  for (d = idata; d; d = d->next) {
    ...
    if (d->name) {
      ...
      name_offset++;
      if (next_name && (last_name == 0 || name_offset > next_name_offset / 2))
        printf("    \"%s-%d\",\n", next_name, next_name_offset - name_offset);
      else
...
static void scan_operands(struct data *d, rtx part, int this_address_p,
                          int this_strict_low) {
  int i, j;
  ...
  switch (GET_CODE(part)) {
    case MATCH_OPERAND:
      ...
      if (max_opno >= MAX_MAX_OPERANDS) {
        message_with_line(d->lineno, "maximum number of operands exceeded");
        have_error = 1;
        return;
      }
...
static void place_operands(struct data *d) {
...
static void process_template(struct data *d, const char *template_code) {
  const char *cp;
  int i;
  ...
  if (template_code[0] == '*') {
    ...
    puts("\nstatic const char *");
    printf(
        "output_%d (rtx *operands ATTRIBUTE_UNUSED, rtx insn "
        "ATTRIBUTE_UNUSED)\n",
        d->code_number);
    puts("{");
  ...
  /* If the assembler code template starts with a @ it is a newline-separated
    ...
    for (i = 0, cp = &template_code[1]; *cp;) {
      ...
      if (sp != ep)
        message_with_line(d->lineno, "trailing whitespace in output template");
...
static void validate_insn_operands(struct data *d) {
  int i;
...
static void gen_insn(rtx insn, int lineno) {
...
static void gen_peephole(rtx peep, int lineno) {
  struct data *d = XNEW(struct data);
  int i;
...
static void gen_expand(rtx insn, int lineno) {
  struct data *d = XNEW(struct data);
  int i;
...
static void gen_split(rtx split, int lineno) {
  struct data *d = XNEW(struct data);
  int i;
...
int main(int argc, char **argv) {
  ...
  fflush(stdout);
  return (ferror(stdout) != 0 || have_error ? FATAL_EXIT_CODE
                                            : SUCCESS_EXIT_CODE);
}
...
static void note_constraint(rtx exp, int lineno) {
  ...
  new_cdata =
      XNEWVAR(struct constraint_data, sizeof(struct constraint_data) + namelen);
...
/* Return the length of the constraint name beginning at position S
   ...
   constraints.  */
static int mdep_constraint_len(const char *s, int lineno, int opno) {
...
static int constraint_len(const char *p, int genoutput_default_constraint_len) {
