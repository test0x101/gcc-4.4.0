...
static inline int coalesce_cost_edge(edge e) {
  ...
  return coalesce_cost(EDGE_FREQUENCY(e), optimize_edge_for_size_p(e),
                       EDGE_CRITICAL_P(e));
}
...
static inline int pop_cost_one_pair(coalesce_list_p cl, int *p1, int *p2) {
  cost_one_pair_p ptr;
...
static inline int pop_best_coalesce(coalesce_list_p cl, int *p1, int *p2) {
...
static unsigned int coalesce_pair_map_hash(const void *pair) {
...
static int coalesce_pair_map_eq(const void *pair1, const void *pair2) {
...
static inline coalesce_list_p create_coalesce_list(void) {
  ...
  list->list =
      htab_create(size, coalesce_pair_map_hash, coalesce_pair_map_eq, NULL);
  list->sorted = NULL;
  list->num_sorted = 0;
...
static inline void delete_coalesce_list(coalesce_list_p cl) {
...
static coalesce_pair_p find_coalesce_pair(coalesce_list_p cl, int p1, int p2,
                                          bool create) {
...
static inline void add_cost_one_coalesce(coalesce_list_p cl, int p1, int p2) {
...
static inline void add_coalesce(coalesce_list_p cl, int p1, int p2, int value) {
  coalesce_pair_p node;
...
static inline int num_coalesce_pairs(coalesce_list_p cl) {
...
static inline coalesce_pair_p first_coalesce_pair(
    coalesce_list_p cl, coalesce_pair_iterator *iter) {
...
static inline bool end_coalesce_pair_p(coalesce_pair_iterator *iter) {
  return end_htab_p(&(iter->hti));
}

/* Return the next partition pair to be visited by ITER.  */

static inline coalesce_pair_p next_coalesce_pair(coalesce_pair_iterator *iter) {
  coalesce_pair_p pair;
...
#define FOR_EACH_PARTITION_PAIR(PAIR, ITER, CL)     \
  for ((PAIR) = first_coalesce_pair((CL), &(ITER)); \
       !end_coalesce_pair_p(&(ITER)); (PAIR) = next_coalesce_pair(&(ITER)))
...
static void sort_coalesce_list(coalesce_list_p cl) {
  unsigned x, num;
  coalesce_pair_p p;
...
static void dump_coalesce_list(FILE *f, coalesce_list_p cl) {
  coalesce_pair_p node;
  coalesce_pair_iterator ppi;
  int x;
  tree var;
...
static inline ssa_conflicts_p ssa_conflicts_new(unsigned size) {
  ssa_conflicts_p ptr;
...
static inline void ssa_conflicts_delete(ssa_conflicts_p ptr) {
  unsigned x;
...
static inline bool ssa_conflicts_test_p(ssa_conflicts_p ptr, unsigned x,
                                        unsigned y) {
  bitmap b;
...
static inline void ssa_conflicts_add_one(ssa_conflicts_p ptr, unsigned x,
                                         unsigned y) {
...
static inline void ssa_conflicts_add(ssa_conflicts_p ptr, unsigned x,
                                     unsigned y) {
#ifdef ENABLE_CHECKING
  gcc_assert(x < ptr->size);
  gcc_assert(y < ptr->size);
...
static inline void ssa_conflicts_merge(ssa_conflicts_p ptr, unsigned x,
                                       unsigned y) {
  unsigned z;
  bitmap_iterator bi;
...
static void ssa_conflicts_dump(FILE *file, ssa_conflicts_p ptr) {
  unsigned x;
...
static live_track_p new_live_track(var_map map) {
  live_track_p ptr;
  int lim, x;
...
static void delete_live_track(live_track_p ptr) {
  int x, lim;
...
static inline void live_track_remove_partition(live_track_p ptr,
                                               int partition) {
  int root;
...
static inline void live_track_add_partition(live_track_p ptr, int partition) {
  int root;
...
static inline void live_track_clear_var(live_track_p ptr, tree var) {
  int p;
...
static inline bool live_track_live_p(live_track_p ptr, tree var) {
  int p, root;
...
static inline void live_track_process_use(live_track_p ptr, tree use) {
  int p;
...
static inline void live_track_process_def(live_track_p ptr, tree def,
...
static inline void live_track_init(live_track_p ptr, bitmap init) {
  unsigned p;
  bitmap_iterator bi;
...
static inline void live_track_clear_base_vars(live_track_p ptr) {
...
static ssa_conflicts_p build_ssa_conflict_graph(tree_live_info_p liveinfo) {
  ssa_conflicts_p graph;
  var_map map;
  basic_block bb;
  ssa_op_iter iter;
  ...
  FOR_EACH_BB(bb) {
    ...
    for (gsi = gsi_last_bb(bb); !gsi_end_p(gsi); gsi_prev(&gsi)) {
      ...
      if (is_gimple_assign(stmt)) {
        ...
        if (gimple_assign_copy_p(stmt) && TREE_CODE(lhs) == SSA_NAME &&
...
static inline void print_exprs(FILE *f, const char *str1, tree expr1,
                               const char *str2, tree expr2, const char *str3) {
  fprintf(f, "%s", str1);
...
static inline void abnormal_corrupt(gimple phi, int i) {
...
static inline void fail_abnormal_edge_coalesce(int x, int y) {
...
static var_map create_outofssa_var_map(coalesce_list_p cl,
                                       bitmap used_in_copy) {
  gimple_stmt_iterator gsi;
  basic_block bb;
  tree var;
  ...
  FOR_EACH_BB(bb) {
    ...
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      FOR_EACH_SSA_TREE_OPERAND(var, stmt, iter, SSA_OP_ALL_VIRTUALS) {
        bitmap_set_bit(used_in_virtual_ops, DECL_UID(SSA_NAME_VAR(var)));
      }
...
static inline bool attempt_coalesce(var_map map, ssa_conflicts_p graph, int x,
                                    int y, FILE *debug) {
  int z;
  tree var1, var2;
  int p1, p2;
...
static void coalesce_partitions(var_map map, ssa_conflicts_p graph,
...
static hashval_t hash_ssa_name_by_var(const void *p) {
...
static int eq_ssa_name_by_var(const void *p1, const void *p2) {
...
extern var_map coalesce_ssa_name(void) {
  ...
  if (!optimize) {
    ssa_name_hash =
        htab_create(10, hash_ssa_name_by_var, eq_ssa_name_by_var, NULL);
    for (i = 1; i < num_ssa_names; i++) {
      tree a = ssa_name(i);
