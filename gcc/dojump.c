...
void clear_pending_stack_adjust(void) {
  if (optimize > 0 && (!flag_omit_frame_pointer || cfun->calls_alloca) &&
      EXIT_IGNORE_STACK)
...
static bool prefer_and_bit_test(enum machine_mode mode, int bitnum) {
  if (and_test == 0) {
    ...
    shift_test =
        gen_rtx_AND(mode, gen_rtx_ASHIFTRT(mode, and_reg, NULL), const1_rtx);
  } else {
...
void do_jump(tree exp, rtx if_false_label, rtx if_true_label) {
  ...
  switch (code) {
    ...
    case ARRAY_RANGE_REF: {
      ...
      get_inner_reference(exp, &bitsize, &bitpos, &offset, &mode, &unsignedp,
                          &volatilep, false);
    ...
    case EQ_EXPR: {
      ...
      gcc_assert(GET_MODE_CLASS(TYPE_MODE(inner_type)) != MODE_COMPLEX_FLOAT);
      gcc_assert(GET_MODE_CLASS(TYPE_MODE(inner_type)) != MODE_COMPLEX_INT);
    ...
    case MINUS_EXPR:
      ...
      exp = build2(NE_EXPR, TREE_TYPE(exp), TREE_OPERAND(exp, 0),
                   TREE_OPERAND(exp, 1));
      /* FALLTHRU */
    case NE_EXPR: {
      tree inner_type = TREE_TYPE(TREE_OPERAND(exp, 0));
      ...
      if (integer_onep(TREE_OPERAND(exp, 1))) {
        ...
        if (TREE_CODE(exp0) == RSHIFT_EXPR) {
          ...
          if (TREE_CODE(shift) == INTEGER_CST &&
            ...
            HOST_WIDE_INT mask = (HOST_WIDE_INT)1 << TREE_INT_CST_LOW(shift);
    ...
    normal:
      ...
      do_compare_rtx_and_jump(temp, CONST0_RTX(GET_MODE(temp)), NE,
                              TYPE_UNSIGNED(TREE_TYPE(exp)), GET_MODE(temp),
...
static void do_jump_by_parts_greater_rtx(enum machine_mode mode, int unsignedp,
                                         rtx op0, rtx op1, rtx if_false_label,
                                         rtx if_true_label) {
  ...
  for (i = 0; i < nwords; i++) {
    ...
    do_compare_rtx_and_jump(op0_word, op1_word, GT, (unsignedp || i > 0),
...
static void do_jump_by_parts_greater(tree exp, int swap, rtx if_false_label,
                                     rtx if_true_label) {
...
static void do_jump_by_parts_zero_rtx(enum machine_mode mode, rtx op0,
  ...
  for (i = 1; i < nwords && part != 0; i++)
    part =
        expand_binop(word_mode, ior_optab, part,
                     operand_subword_force(op0, i, mode), part, 1, OPTAB_WIDEN);

  if (part != 0) {
    do_compare_rtx_and_jump(part, const0_rtx, EQ, 1, word_mode, NULL_RTX,
  ...
  for (i = 0; i < nwords; i++)
    do_compare_rtx_and_jump(operand_subword_force(op0, i, mode), const0_rtx, EQ,
                            1, word_mode, NULL_RTX, if_false_label, NULL_RTX);
...
static void do_jump_by_parts_equality_rtx(enum machine_mode mode, rtx op0,
                                          rtx op1, rtx if_false_label,
                                          rtx if_true_label) {
  int nwords = (GET_MODE_SIZE(mode) / UNITS_PER_WORD);
  rtx drop_through_label = 0;
  int i;
  ...
  for (i = 0; i < nwords; i++)
    do_compare_rtx_and_jump(operand_subword_force(op0, i, mode),
                            operand_subword_force(op1, i, mode), EQ, 0,
...
static void do_jump_by_parts_equality(tree exp, rtx if_false_label,
                                      rtx if_true_label) {
  rtx op0 = expand_normal(TREE_OPERAND(exp, 0));
  rtx op1 = expand_normal(TREE_OPERAND(exp, 1));
  enum machine_mode mode = TYPE_MODE(TREE_TYPE(TREE_OPERAND(exp, 0)));
  do_jump_by_parts_equality_rtx(mode, op0, op1, if_false_label, if_true_label);
}
...
void do_compare_rtx_and_jump(rtx op0, rtx op1, enum rtx_code code,
                             int unsignedp, enum machine_mode mode, rtx size,
                             rtx if_false_label, rtx if_true_label) {
  rtx tem;
  ...
  code = unsignedp ? unsigned_condition(code) : code;
  if (0 !=
      (tem = simplify_relational_operation(code, mode, VOIDmode, op0, op1))) {
    if (CONSTANT_P(tem)) {
  ...
  if (GET_MODE_CLASS(mode) == MODE_INT &&
    ...
    switch (code) {
      case LTU:
        do_jump_by_parts_greater_rtx(mode, 1, op1, op0, if_false_label,
                                     if_true_label);
        break;

      case LEU:
        do_jump_by_parts_greater_rtx(mode, 1, op0, op1, if_true_label,
                                     if_false_label);
        break;

      case GTU:
        do_jump_by_parts_greater_rtx(mode, 1, op0, op1, if_false_label,
                                     if_true_label);
        break;

      case GEU:
        do_jump_by_parts_greater_rtx(mode, 1, op1, op0, if_true_label,
                                     if_false_label);
        break;

      case LT:
        do_jump_by_parts_greater_rtx(mode, 0, op1, op0, if_false_label,
                                     if_true_label);
        break;

      case LE:
        do_jump_by_parts_greater_rtx(mode, 0, op0, op1, if_true_label,
                                     if_false_label);
        break;

      case GT:
        do_jump_by_parts_greater_rtx(mode, 0, op0, op1, if_false_label,
                                     if_true_label);
        break;

      case GE:
        do_jump_by_parts_greater_rtx(mode, 0, op1, op0, if_true_label,
                                     if_false_label);
        break;
...
static void do_compare_and_jump(tree exp, enum rtx_code signed_code,
  ...
  if (HAVE_canonicalize_funcptr_for_compare &&
      ...
      TREE_CODE(TREE_TYPE(TREE_TYPE(TREE_OPERAND(exp, 0)))) == FUNCTION_TYPE &&
      TREE_CODE(TREE_TYPE(TREE_OPERAND(exp, 1))) == POINTER_TYPE &&
      TREE_CODE(TREE_TYPE(TREE_TYPE(TREE_OPERAND(exp, 1)))) == FUNCTION_TYPE) {
