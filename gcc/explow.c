  ...
  switch (code) {
    ...
    case MEM:
      ...
      if (GET_CODE(XEXP(x, 0)) == SYMBOL_REF &&
        ...
        tem = force_const_mem(GET_MODE(x),
                              plus_constant(get_pool_constant(XEXP(x, 0)), c));
...
rtx eliminate_constant_term(rtx x, rtx *constptr) {
  ...
  if ((x1 != XEXP(x, 1) || x0 != XEXP(x, 0)) &&
      0 != (tem =
                simplify_binary_operation(PLUS, GET_MODE(x), *constptr, tem)) &&
      GET_CODE(tem) == CONST_INT) {
    *constptr = tem;
...
rtx use_anchored_address(rtx x) {
  ...
  if (GET_CODE(base) != SYMBOL_REF || !SYMBOL_REF_HAS_BLOCK_INFO_P(base) ||
      SYMBOL_REF_ANCHOR_P(base) || SYMBOL_REF_BLOCK(base) == NULL ||
...
enum machine_mode promote_mode(const_tree type, enum machine_mode mode,
                               int *punsignedp, int for_call ATTRIBUTE_UNUSED) {
...
static rtx round_push(rtx size) {
  ...
  if (GET_CODE(size) == CONST_INT) {
    ...
    /* CEIL_DIV_EXPR needs to worry about the addition overflowing,
       ...
       TRUNC_DIV_EXPR.  */
    size = expand_binop(Pmode, add_optab, size, GEN_INT(align - 1), NULL_RTX, 1,
                        OPTAB_LIB_WIDEN);
...
#define MUST_ALIGN (PREFERRED_STACK_BOUNDARY < BIGGEST_ALIGNMENT)
  ...
  if (MUST_ALIGN)
    size = force_operand(
        plus_constant(size, BIGGEST_ALIGNMENT / BITS_PER_UNIT - 1), NULL_RTX);
  ...
  if (!cfun->calls_setjmp) {
    ...
    size = expand_binop(Pmode, add_optab, size, dynamic_offset, NULL_RTX, 1,
                        OPTAB_LIB_WIDEN);
  }
  ...
  /* We ought to be called always on the toplevel and stack ought to be aligned
     properly.  */
  gcc_assert(
      !(stack_pointer_delta % (PREFERRED_STACK_BOUNDARY / BITS_PER_UNIT)));
  ...
  if (target == 0 || !REG_P(target) || REGNO(target) < FIRST_PSEUDO_REGISTER ||
...
#endif
  {
    ...
    if (crtl->limit_stack) {
      ...
      available = expand_binop(Pmode, sub_optab, stack_pointer_rtx,
                               stack_limit_rtx, NULL_RTX, 1, OPTAB_WIDEN);
#else
      available = expand_binop(Pmode, sub_optab, stack_limit_rtx,
                               stack_pointer_rtx, NULL_RTX, 1, OPTAB_WIDEN);
#endif
  ...
  if (MUST_ALIGN) {
    ...
    target =
        expand_divmod(0, TRUNC_DIV_EXPR, Pmode, target,
                      GEN_INT(BIGGEST_ALIGNMENT / BITS_PER_UNIT), NULL_RTX, 1);
    target = expand_mult(
        Pmode, target, GEN_INT(BIGGEST_ALIGNMENT / BITS_PER_UNIT), NULL_RTX, 1);
  }
...
void probe_stack_range(HOST_WIDE_INT first, rtx size) {
  ...
  if (stack_check_libfunc != 0) {
    rtx addr = memory_address(
        QImode, gen_rtx_fmt_ee(STACK_GROW_OP, Pmode, stack_pointer_rtx,
  ...
  /* Next see if we have an insn to check the stack.  Use it if so.  */
    ...
    insn_operand_predicate_fn pred;
    rtx last_addr =
        force_operand(gen_rtx_fmt_ee(STACK_GROW_OP, Pmode, stack_pointer_rtx,
                                     plus_constant(size, first)),
                      NULL_RTX);
  ...
  /* If we have to generate explicit probes, see if we have a constant
    ...
    for (offset = first + STACK_CHECK_PROBE_INTERVAL; offset < INTVAL(size);
      ...
      emit_stack_probe(gen_rtx_fmt_ee(STACK_GROW_OP, Pmode, stack_pointer_rtx,
                                      GEN_INT(offset)));
  ...
  /* In the variable case, do the same as above, but in a loop.  We emit loop
    ...
    rtx test_addr = force_operand(
        gen_rtx_fmt_ee(STACK_GROW_OP, Pmode, stack_pointer_rtx,
    ...
    if (!REG_P(test_addr) || REGNO(test_addr) < FIRST_PSEUDO_REGISTER)
...
#define CMP_OPCODE GTU
    temp = expand_binop(Pmode, sub_optab, test_addr, incr, test_addr, 1,
                        OPTAB_WIDEN);
#else
#define CMP_OPCODE LTU
    temp = expand_binop(Pmode, add_optab, test_addr, incr, test_addr, 1,
                        OPTAB_WIDEN);
#endif
    ...
    emit_label(test_lab);
    emit_cmp_and_jump_insns(test_addr, last_addr, CMP_OPCODE, NULL_RTX, Pmode,
                            1, loop_lab);
    emit_jump(end_lab);
    emit_label(end_lab);
...
rtx hard_function_value(const_tree valtype, const_tree func, const_tree fntype,
                        int outgoing ATTRIBUTE_UNUSED) {
  rtx val;

  val = targetm.calls.function_value(valtype, func ? func : fntype, outgoing);

  if (REG_P(val) && GET_MODE(val) == BLKmode) {
