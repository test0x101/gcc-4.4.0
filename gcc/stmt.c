...
static void expand_asm_loc(tree string, int vol, location_t locus) {
  ...
  body = gen_rtx_ASM_INPUT_loc(VOIDmode,
                               ggc_strdup(TREE_STRING_POINTER(string)), locus);
...
bool parse_output_constraint(const char **constraint_p, int operand_num,
  ...
  *is_inout = (*p == '+');
  ...
  if (p != constraint || *is_inout) {
    ...
    if (p != constraint)
      warning(0,
              "output constraint %qc for operand %d "
...
bool parse_input_constraint(const char **constraint_p, int input_num,
                            int ninputs, int noutputs, int ninout,
                            const char *const *constraints, bool *allows_mem,
                            bool *allows_reg) {
  const char *constraint = *constraint_p;
  ...
  for (j = 0; j < c_len; j += CONSTRAINT_LEN(constraint[j], constraint + j))
    switch (constraint[j]) {
      ...
      default:
        ...
        if (REG_CLASS_FROM_CONSTRAINT(constraint[j], constraint + j) != NO_REGS)
          *allows_reg = true;
#ifdef EXTRA_CONSTRAINT_STR
...
static tree decl_overlaps_hard_reg_set_p(tree *declp,
  ...
  if (TREE_CODE(decl) == VAR_DECL) {
    if (DECL_HARD_REGISTER(decl) && REG_P(DECL_RTL(decl)) &&
...
static void expand_asm_operands(tree string, tree outputs, tree inputs,
  ...
  for (i = 0, tail = outputs; tail; tail = TREE_CHAIN(tail), i++) {
    ...
    if (!allows_reg && (allows_mem || is_inout ||
                        (DECL_P(val) && REG_P(DECL_RTL(val)) &&
  ...
  for (i = 0, tail = outputs; tail; tail = TREE_CHAIN(tail), i++) {
    ...
    ok = parse_output_constraint(&constraints[i], i, ninputs, noutputs,
    ...
    if ((TREE_CODE(val) == INDIRECT_REF && allows_mem) ||
        (DECL_P(val) && (allows_mem || REG_P(DECL_RTL(val))) &&
         !(REG_P(DECL_RTL(val)) &&
           GET_MODE(DECL_RTL(val)) != TYPE_MODE(type))) ||
        !allows_reg || is_inout) {
      ...
      if ((!allows_mem && MEM_P(op)) || GET_CODE(op) == CONCAT) {
  ...
  body =
      gen_rtx_ASM_OPERANDS((noutputs == 0 ? VOIDmode : GET_MODE(output_rtx[0])),
                           ggc_strdup(TREE_STRING_POINTER(string)),
                           empty_string, 0, argvec, constraintvec, locus);
  ...
  for (i = 0, tail = inputs; tail; tail = TREE_CHAIN(tail), ++i) {
    ...
    if (asm_operand_ok(op, constraint, NULL) <= 0) {
      if (allows_reg && TYPE_MODE(type) != BLKmode)
        ...
        if (REG_P(op) || GET_CODE(op) == SUBREG || GET_CODE(op) == CONCAT) {
          tree qual_type =
    ...
    ASM_OPERANDS_INPUT_CONSTRAINT_EXP(body, i) = gen_rtx_ASM_INPUT(
        TYPE_MODE(type), ggc_strdup(constraints[i + noutputs]));
  ...
  if (noutputs == 1 && nclobbers == 0) {
    ...
    for (i = 0, tail = outputs; tail; tail = TREE_CHAIN(tail), i++) {
      XVECEXP(body, 0, i) = gen_rtx_SET(
          VOIDmode, output_rtx[i],
          gen_rtx_ASM_OPERANDS(
              GET_MODE(output_rtx[i]), ggc_strdup(TREE_STRING_POINTER(string)),
              ggc_strdup(constraints[i]), i, argvec, constraintvec, locus));
...
void expand_asm_expr(tree exp) {
  ...
  expand_asm_operands(ASM_STRING(exp), outputs, ASM_INPUTS(exp),
                      ASM_CLOBBERS(exp), ASM_VOLATILE_P(exp), input_location);
  ...
  if (outputs || inputs) {
    ...
    while (tmp) {
      ...
      if (n_occurrences(',', constraint) != nalternatives) {
        error(
            "operand constraints for %<asm%> differ "
...
tree resolve_asm_operand_names(tree string, tree outputs, tree inputs) {
  ...
  for (t = inputs; t; t = TREE_CHAIN(t)) {
    ...
    if (strchr(c, '[') != NULL) {
      ...
      TREE_VALUE(TREE_PURPOSE(t)) = build_string(strlen(buffer), buffer);
      free(buffer);
    }
  }
...
void expand_expr_stmt(tree exp) {
  ...
  if (value && MEM_P(value) && TREE_THIS_VOLATILE(exp)) {
    if (TYPE_MODE(type) == VOIDmode)
      ...
      emit_cmp_and_jump_insns(value, value, EQ, expand_normal(TYPE_SIZE(type)),
                              BLKmode, 0, lab);
      emit_label(lab);
    }
  }
...
int warn_if_unused_value(const_tree exp, location_t locus) {
restart:
  ...
  switch (TREE_CODE(exp)) {
    ...
    default:
      ...
      if ((DECL_P(exp) || REFERENCE_CLASS_P(exp)) && TREE_THIS_VOLATILE(exp))
        return 0;
...
static void expand_value_return(rtx val) {
  ...
  if (return_reg != val) {
    ...
    if (targetm.calls.promote_function_return(
      ...
      enum machine_mode mode = promote_mode(type, old_mode, &unsignedp, 1);
...
void expand_return(tree retval) {
  ...
  /* If the result is an aggregate that is being returned in one (or more)
  ...
  else if (retval_rhs != 0 && TYPE_MODE(TREE_TYPE(retval_rhs)) == BLKmode &&
           REG_P(result_rtl)) {
    int i;
    unsigned HOST_WIDE_INT bitpos, xbitpos;
    unsigned HOST_WIDE_INT padding_correction = 0;
    unsigned HOST_WIDE_INT bytes = int_size_in_bytes(TREE_TYPE(retval_rhs));
    ...
    if (bytes % UNITS_PER_WORD != 0 &&
        (targetm.calls.return_in_msb(TREE_TYPE(retval_rhs)) ? !BYTES_BIG_ENDIAN
                                                            : BYTES_BIG_ENDIAN))
      padding_correction =
    ...
    for (bitpos = 0, xbitpos = padding_correction;
      ...
      if (bitpos % BITS_PER_WORD == 0)
        src =
            operand_subword_force(result_val, bitpos / BITS_PER_WORD, BLKmode);

      /* Use bitpos for the source extraction (left justified) and
         xbitpos for the destination store (right justified).  */
      store_bit_field(dst, bitsize, xbitpos % BITS_PER_WORD, word_mode,
                      extract_bit_field(src, bitsize, bitpos % BITS_PER_WORD, 1,
    ...
    if (tmpmode == BLKmode) {
      ...
      for (tmpmode = GET_CLASS_NARROWEST_MODE(MODE_INT); tmpmode != VOIDmode;
...
void expand_decl(tree decl) {
  ...
  if (type == error_mark_node)
    ...
    if (POINTER_TYPE_P(type))
      mark_reg_pointer(DECL_RTL(decl), TYPE_ALIGN(TREE_TYPE(TREE_TYPE(decl))));
  }
    ...
    DECL_ALIGN(decl) =
        (DECL_MODE(decl) == BLKmode ? BIGGEST_ALIGNMENT
...
static struct case_node *add_case_node(struct case_node *head, tree type,
                                       tree low, tree high, tree label,
...
static int case_bit_test_cmp(const void *p1, const void *p2) {
...
static void emit_case_bit_tests(tree index_type, tree index_expr, tree minval,
                                tree range, case_node_ptr nodes,
                                rtx default_label) {
  ...
  for (n = nodes; n; n = n->right) {
    ...
    lo = tree_low_cst(fold_build2(MINUS_EXPR, index_type, n->low, minval), 1);
    hi = tree_low_cst(fold_build2(MINUS_EXPR, index_type, n->high, minval), 1);
    for (j = lo; j <= hi; j++)
  ...
  if (default_label)
    emit_cmp_and_jump_insns(index, expr, GTU, NULL_RTX, mode, 1, default_label);

  index = convert_to_mode(word_mode, index, 0);
  index = expand_binop(word_mode, ashl_optab, const1_rtx, index, NULL_RTX, 1,
                       OPTAB_WIDEN);

  for (i = 0; i < count; i++) {
    expr = immed_double_const(test[i].lo, test[i].hi, word_mode);
    expr = expand_binop(word_mode, and_optab, index, expr, NULL_RTX, 1,
                        OPTAB_WIDEN);
    emit_cmp_and_jump_insns(expr, const0_rtx, NE, NULL_RTX, word_mode, 1,
                            test[i].label);
  }
...
void expand_case(tree exp) {
  ...
  alloc_pool case_node_pool =
      create_alloc_pool("struct case_node pool", sizeof(struct case_node), 100);
  ...
  if (index_type != error_mark_node) {
    ...
    if (CASE_USE_BIT_TESTS && !TREE_CONSTANT(index_expr) &&
        compare_tree_int(range, GET_MODE_BITSIZE(word_mode)) < 0 &&
        compare_tree_int(range, 0) > 0 && lshift_cheap_p() &&
        ((uniq == 1 && count >= 3) || (uniq == 2 && count >= 5) ||
         (uniq == 3 && count >= 6))) {
      ...
      emit_case_bit_tests(index_type, index_expr, minval, range, case_list,
                          default_label);
    }
    ...
    /* If range of values is much bigger than number of values,
      ...
      use_cost_table = (TREE_CODE(orig_type) != ENUMERAL_TYPE &&
      ...
      if (!try_casesi(index_type, index_expr, minval, range, table_label,
                      default_label, fallback_label)) {
        bool ok;

        /* Index jumptables from zero for suitable values of
           minval to avoid a subtraction.  */
        if (optimize_insn_for_speed_p() && compare_tree_int(minval, 0) > 0 &&
            compare_tree_int(minval, 3) < 0) {
          minval = build_int_cst(index_type, 0);
          range = maxval;
        }

        ok = try_tablejump(index_type, index_expr, minval, range, table_label,
                           default_label);
        gcc_assert(ok);
      }
      ...
      for (n = case_list; n; n = n->right) {
        ...
        HOST_WIDE_INT i_low = tree_low_cst(
            fold_build2(MINUS_EXPR, index_type, n->low, minval), 1);
        HOST_WIDE_INT i_high = tree_low_cst(
            fold_build2(MINUS_EXPR, index_type, n->high, minval), 1);
        HOST_WIDE_INT i;
      ...
      if (CASE_VECTOR_PC_RELATIVE || flag_pic)
        emit_jump_insn(gen_rtx_ADDR_DIFF_VEC(
            CASE_VECTOR_MODE, gen_rtx_LABEL_REF(Pmode, table_label),
            gen_rtvec_v(ncases, labelvec), const0_rtx, const0_rtx));
      else
        emit_jump_insn(
            gen_rtx_ADDR_VEC(CASE_VECTOR_MODE, gen_rtvec_v(ncases, labelvec)));
...
static void do_jump_if_equal(enum machine_mode mode, rtx op0, rtx op1,
                             rtx label, int unsignedp) {
  do_compare_rtx_and_jump(op0, op1, EQ, unsignedp, mode, NULL_RTX, NULL_RTX,
                          label);
}
...
static void emit_case_nodes(rtx index, case_node_ptr node, rtx default_label,
  ...
  if (node_is_bounded(node, index_type))
    ...
    do_jump_if_equal(
        mode, index,
        convert_modes(mode, imode, expand_normal(node->low), unsignedp),
    ...
    if (node->right != 0 && node->left != 0) {
      ...
      if (node_is_bounded(node->right, index_type)) {
        emit_cmp_and_jump_insns(
            index,
            convert_modes(mode, imode, expand_normal(node->high), unsignedp),
            GT, NULL_RTX, mode, unsignedp, label_rtx(node->right->code_label));
        ...
        emit_cmp_and_jump_insns(
            index,
            convert_modes(mode, imode, expand_normal(node->high), unsignedp),
            LT, NULL_RTX, mode, unsignedp, label_rtx(node->left->code_label));
      ...
      /* If both children are single-valued cases with no
        ...
        do_jump_if_equal(
            mode, index,
            convert_modes(mode, imode, expand_normal(node->right->low),
                          unsignedp),
        ...
        do_jump_if_equal(
            mode, index,
            convert_modes(mode, imode, expand_normal(node->left->low),
                          unsignedp),
        ...
        emit_cmp_and_jump_insns(
            index,
            convert_modes(mode, imode, expand_normal(node->high), unsignedp),
            GT, NULL_RTX, mode, unsignedp, label_rtx(test_label));
      ...
      if (node->right->right || node->right->left ||
        ...
        if (!node_has_low_bound(node, index_type)) {
          emit_cmp_and_jump_insns(
              index,
              convert_modes(mode, imode, expand_normal(node->high), unsignedp),
              LT, NULL_RTX, mode, unsignedp, default_label);
        }
      ...
      if (node->left->left || node->left->right ||
        ...
        if (!node_has_high_bound(node, index_type)) {
          emit_cmp_and_jump_insns(
              index,
              convert_modes(mode, imode, expand_normal(node->high), unsignedp),
              GT, NULL_RTX, mode, unsignedp, default_label);
        }
    ...
    if (node->right != 0 && node->left != 0) {
      ...
      emit_cmp_and_jump_insns(
          index,
          convert_modes(mode, imode, expand_normal(node->low), unsignedp), GE,
          NULL_RTX, mode, unsignedp, label_rtx(node->code_label));
      ...
      if (!node_has_low_bound(node, index_type)) {
        emit_cmp_and_jump_insns(
            index,
            convert_modes(mode, imode, expand_normal(node->low), unsignedp), LT,
            NULL_RTX, mode, unsignedp, default_label);
      }
      ...
      emit_cmp_and_jump_insns(
          index,
          convert_modes(mode, imode, expand_normal(node->high), unsignedp), LE,
          NULL_RTX, mode, unsignedp, label_rtx(node->code_label));
      ...
      if (!high_bound && low_bound) {
        ...
        new_index = expand_simple_binop(mode, MINUS, index, low_rtx, NULL_RTX,
                                        unsignedp, OPTAB_WIDEN);
        new_bound = expand_expr(fold_build2(MINUS_EXPR, type, high, low),
                                NULL_RTX, mode, EXPAND_NORMAL);

        emit_cmp_and_jump_insns(new_index, new_bound, GT, NULL_RTX, mode, 1,
                                default_label);
      }
