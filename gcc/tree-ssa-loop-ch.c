...
static bool do_while_loop_p(struct loop *loop) {
...
static unsigned int copy_loop_headers(void) {
  loop_iterator li;
  struct loop *loop;
  basic_block header;
  edge exit, entry;
  ...
  FOR_EACH_LOOP(li, loop, 0) {
    ...
    if (warn_strict_overflow > 0) {
      ...
      for (i = 0; i < n_bbs; ++i) {
        ...
        for (bsi = gsi_start_bb(copied_bbs[i]); !gsi_end_p(bsi);
             gsi_next(&bsi)) {
...
struct gimple_opt_pass pass_ch = {{
    ...
    0,                   /* todo_flags_start */
    TODO_cleanup_cfg | TODO_dump_func | TODO_verify_ssa /* todo_flags_finish */
}};
