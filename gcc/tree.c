...
const char *const tree_code_class_strings[] = {
    "exceptional", "constant",   "type",      "declaration",
    "reference",   "comparison", "unary",     "binary",
    "statement",   "vl_exp",     "expression"};
...
static GTY((if_marked("type_hash_marked_p"),
            param_is(struct type_hash))) htab_t type_hash_table;

/* Hash table and temporary node for larger integer const values.  */
static GTY(()) tree int_cst_node;
static GTY((if_marked("ggc_marked_p"),
            param_is(union tree_node))) htab_t int_cst_hash_table;
...
static GTY((if_marked("ggc_marked_p"),
            param_is(union tree_node))) htab_t cl_option_hash_table;

/* General tree->tree mapping  structure for use in hash tables.  */

static GTY((if_marked("tree_map_marked_p"),
            param_is(struct tree_map))) htab_t debug_expr_for_decl;

static GTY((if_marked("tree_map_marked_p"),
            param_is(struct tree_map))) htab_t value_expr_for_decl;

static GTY((if_marked("tree_priority_map_marked_p"),
            param_is(struct tree_priority_map))) htab_t init_priority_for_decl;

static GTY((if_marked("tree_map_marked_p"),
            param_is(struct tree_map))) htab_t restrict_base_for_decl;
...
const char *const omp_clause_code_name[] = {
    "error_clause", "private",     "shared",   "firstprivate",
    "lastprivate",  "reduction",   "copyin",   "copyprivate",
    "if",           "num_threads", "schedule", "nowait",
    "ordered",      "default",     "collapse", "untied"};
...
void init_ttree(void) {
  ...
  type_hash_table =
      htab_create_ggc(TYPE_HASH_INITIAL_SIZE, type_hash_hash, type_hash_eq, 0);

  debug_expr_for_decl = htab_create_ggc(512, tree_map_hash, tree_map_eq, 0);

  value_expr_for_decl = htab_create_ggc(512, tree_map_hash, tree_map_eq, 0);
  init_priority_for_decl =
      htab_create_ggc(512, tree_priority_map_hash, tree_priority_map_eq, 0);
  restrict_base_for_decl = htab_create_ggc(256, tree_map_hash, tree_map_eq, 0);

  int_cst_hash_table =
      htab_create_ggc(1024, int_cst_hash_hash, int_cst_hash_eq, NULL);

  int_cst_node = make_node(INTEGER_CST);

  cl_option_hash_table =
      htab_create_ggc(64, cl_option_hash_hash, cl_option_hash_eq, NULL);
  ...
  switch (code) {
    ...
    case OMP_CLAUSE:
      return (sizeof(struct tree_omp_clause) +
              (omp_clause_num_ops[OMP_CLAUSE_CODE(node)] - 1) * sizeof(tree));
...
tree build_int_cst_wide_type(tree type, unsigned HOST_WIDE_INT low,
                             HOST_WIDE_INT high) {
...
tree build_int_cst_wide(tree type, unsigned HOST_WIDE_INT low,
                        HOST_WIDE_INT hi) {
  tree t;
  int ix = -1;
  int limit = 0;
...
tree build_low_bits_mask(tree type, unsigned bits) {
  ...
  if (bits == TYPE_PRECISION(type) && !TYPE_UNSIGNED(type)) {
...
bool cst_and_fits_in_hwi(const_tree x) {
  ...
  return (TREE_INT_CST_HIGH(x) == 0 || TREE_INT_CST_HIGH(x) == -1);
}
...
tree build_vector_from_ctor(tree type, VEC(constructor_elt, gc) * v) {
  tree list = NULL_TREE;
...
tree build_constructor(tree type, VEC(constructor_elt, gc) * vals) {
...
real_value_from_int_cst(const_tree type, const_tree i) {
  ...
  real_from_integer(&d, type ? TYPE_MODE(type) : VOIDmode, TREE_INT_CST_LOW(i),
                    TREE_INT_CST_HIGH(i), TYPE_UNSIGNED(TREE_TYPE(i)));
  return d;
}
...
tree build_one_cst(tree type) {
  switch (TREE_CODE(type)) {
    ...
    case COMPLEX_TYPE:
      return build_complex(type, build_one_cst(TREE_TYPE(type)),
...
int integer_zerop(const_tree expr) {
  ...
  return ((TREE_CODE(expr) == INTEGER_CST && TREE_INT_CST_LOW(expr) == 0 &&
...
int integer_onep(const_tree expr) {
  ...
  return ((TREE_CODE(expr) == INTEGER_CST && TREE_INT_CST_LOW(expr) == 1 &&
           TREE_INT_CST_HIGH(expr) == 0) ||
          (TREE_CODE(expr) == COMPLEX_CST &&
...
int integer_pow2p(const_tree expr) {
  ...
  prec = (POINTER_TYPE_P(TREE_TYPE(expr)) ? POINTER_SIZE
...
tree bit_position(const_tree field) {
  return bit_from_pos(DECL_FIELD_OFFSET(field), DECL_FIELD_BIT_OFFSET(field));
}
...
tree byte_position(const_tree field) {
  return byte_from_pos(DECL_FIELD_OFFSET(field), DECL_FIELD_BIT_OFFSET(field));
}
...
unsigned int expr_align(const_tree t) {
  ...
  switch (TREE_CODE(arg)) {
    ...
    case VAR_DECL:
      return ((TREE_STATIC(arg) || DECL_EXTERNAL(arg)) &&
                      !DECL_THREAD_LOCAL_P(arg) && !DECL_DLLIMPORT_P(arg)
                  ? arg
                  : NULL);

    case CONST_DECL:
      return ((TREE_STATIC(arg) || DECL_EXTERNAL(arg)) ? arg : NULL);
...
bool decl_address_invariant_p(const_tree op) {
  ...
  switch (TREE_CODE(op)) {
    ...
    case VAR_DECL:
      if (((TREE_STATIC(op) || DECL_EXTERNAL(op)) && !DECL_DLLIMPORT_P(op)) ||
...
tree substitute_in_expr(tree exp, tree f, tree r) {
  ...
  if (code == TREE_LIST) {
    ...
    for (inner = TREE_OPERAND(exp, 0); REFERENCE_CLASS_P(inner);
         inner = TREE_OPERAND(inner, 0))
      ;
    ...
    new_tree = fold_build3(COMPONENT_REF, TREE_TYPE(exp), op0,
      ...
      case tcc_reference:
        switch (TREE_CODE_LENGTH(code)) {
          ...
          case 4:
            ...
            if (op0 == TREE_OPERAND(exp, 0) && op1 == TREE_OPERAND(exp, 1) &&
                op2 == TREE_OPERAND(exp, 2) && op3 == TREE_OPERAND(exp, 3))
              return exp;
  ...
  if (code == PLACEHOLDER_EXPR) {
    ...
    for (elt = obj; elt != 0;
         elt = ((TREE_CODE(elt) == COMPOUND_EXPR || TREE_CODE(elt) == COND_EXPR)
                    ? TREE_OPERAND(elt, 1)
                : (REFERENCE_CLASS_P(elt) || UNARY_CLASS_P(elt) ||
                   BINARY_CLASS_P(elt) || VL_EXP_CLASS_P(elt) ||
...
tree stabilize_reference(tree ref) {
  ...
  switch (code) {
    ...
    case INDIRECT_REF:
      result =
          build_nt(INDIRECT_REF, stabilize_reference_1(TREE_OPERAND(ref, 0)));
      break;

    case COMPONENT_REF:
      result =
          build_nt(COMPONENT_REF, stabilize_reference(TREE_OPERAND(ref, 0)),
                   TREE_OPERAND(ref, 1), NULL_TREE);
      break;

    case BIT_FIELD_REF:
      result =
          build_nt(BIT_FIELD_REF, stabilize_reference(TREE_OPERAND(ref, 0)),
    ...
    case ARRAY_RANGE_REF:
      result =
          build_nt(ARRAY_RANGE_REF, stabilize_reference(TREE_OPERAND(ref, 0)),
                   stabilize_reference_1(TREE_OPERAND(ref, 1)),
                   TREE_OPERAND(ref, 2), TREE_OPERAND(ref, 3));
      break;
...
#define UPDATE_FLAGS(NODE)                 \
  do {                                     \
    tree _node = (NODE);                   \
    if (_node && !TREE_CONSTANT(_node))    \
      tc = false;                          \
    if (_node && TREE_SIDE_EFFECTS(_node)) \
      se = true;                           \
  } while (0)
    ...
    switch (code) {
      ...
      default:
        if ((TREE_CODE_CLASS(code) == tcc_unary || code == VIEW_CONVERT_EXPR) &&
            node && !TYPE_P(node) && TREE_CONSTANT(node))
          TREE_CONSTANT(t) = 1;
...
tree build2_stat(enum tree_code code, tree tt, tree arg0,
  ...
  if ((code == MINUS_EXPR || code == PLUS_EXPR || code == MULT_EXPR) && arg0 &&
      arg1 && tt &&
      POINTER_TYPE_P(tt)
  ...
  TREE_THIS_VOLATILE(t) = (TREE_CODE_CLASS(code) == tcc_reference && arg0 &&
                           TREE_THIS_VOLATILE(arg0));

  return t;
}

tree build3_stat(enum tree_code code, tree tt, tree arg0, tree arg1,
  ...
  if (code == COND_EXPR && tt == void_type_node && arg1 == NULL_TREE &&
      arg2 == NULL_TREE)
    side_effects = true;
  else
    side_effects = TREE_SIDE_EFFECTS(t);
...
tree build4_stat(enum tree_code code, tree tt, tree arg0, tree arg1, tree arg2,
...
tree build5_stat(enum tree_code code, tree tt, tree arg0, tree arg1, tree arg2,
                 tree arg3, tree arg4 MEM_STAT_DECL) {
  bool constant, read_only, side_effects;
  tree t;
...
tree build7_stat(enum tree_code code, tree tt, tree arg0, tree arg1, tree arg2,
                 tree arg3, tree arg4, tree arg5, tree arg6 MEM_STAT_DECL) {
  bool constant, read_only, side_effects;
  tree t;
...
expanded_location expand_location(source_location loc) {
...
void set_expr_locus(tree node, source_location *loc) {
  if (loc == NULL)
...
static inline hashval_t iterative_hash_pointer(const void *ptr,
                                               hashval_t val2) {
...
/* Produce good hash value combining VAL and VAL2.  */
static inline hashval_t iterative_hash_host_wide_int(HOST_WIDE_INT val,
                                                     hashval_t val2) {
...
static tree build_type_attribute_qual_variant(tree ttype, tree attribute,
                                              int quals) {
  if (!attribute_list_equal(TYPE_ATTRIBUTES(ttype), attribute)) {
    ...
    if (TREE_CODE(ttype) == RECORD_TYPE || TREE_CODE(ttype) == UNION_TYPE ||
    ...
    if (TREE_TYPE(ntype))
      hashcode = iterative_hash_object(TYPE_HASH(TREE_TYPE(ntype)), hashcode);
    ...
    switch (TREE_CODE(ntype)) {
      ...
      case ARRAY_TYPE:
        if (TYPE_DOMAIN(ntype))
          hashcode =
              iterative_hash_object(TYPE_HASH(TYPE_DOMAIN(ntype)), hashcode);
        break;
      case INTEGER_TYPE:
...
tree build_type_attribute_variant(tree ttype, tree attribute) {
  return build_type_attribute_qual_variant(ttype, attribute, TYPE_QUALS(ttype));
}
...
static int is_attribute_with_length_p(const char *attr, int attr_len,
                                      const_tree ident) {
  int ident_len;
  const char *p;
  ...
  if (ident_len == attr_len && strcmp(attr, p) == 0)
    return 1;
...
int is_attribute_p(const char *attr, const_tree ident) {
      ...
      for (; a2 != 0; a2 = TREE_CHAIN(a2)) {
        ...
        for (a = lookup_attribute(IDENTIFIER_POINTER(TREE_PURPOSE(a2)),
             ...
             a = lookup_attribute(IDENTIFIER_POINTER(TREE_PURPOSE(a2)),
                                  TREE_CHAIN(a))) {
          if (TREE_VALUE(a) != NULL && TREE_CODE(TREE_VALUE(a)) == TREE_LIST &&
              TREE_VALUE(a2) != NULL &&
              TREE_CODE(TREE_VALUE(a2)) == TREE_LIST) {
            if (simple_cst_list_equal(TREE_VALUE(a), TREE_VALUE(a2)) == 1)
              break;
          } else if (simple_cst_equal(TREE_VALUE(a), TREE_VALUE(a2)) == 1)
            break;
        }
        if (a == NULL_TREE) {
          a1 = copy_node(a2);
...
tree merge_dllimport_decl_attributes(tree old, tree new_tree) {
  tree a;
  ...
  if (delete_dllimport_p) {
    ...
    for (prev = NULL_TREE, t = a; t; prev = t, t = TREE_CHAIN(t)) {
      if (is_attribute_with_length_p("dllimport", attr_len, TREE_PURPOSE(t))) {
        if (prev == NULL_TREE)
          a = TREE_CHAIN(a);
        else
...
tree handle_dll_attribute(tree *pnode, tree name, tree args, int flags,
                          bool *no_add_attrs) {
  tree node = *pnode;
  ...
  /* Report error on dllimport ambiguities seen now before they cause
    ...
    else if (TREE_CODE(node) == FUNCTION_DECL && DECL_DECLARED_INLINE_P(node)) {
      warning(OPT_Wattributes,
  ...
  if (!TREE_PUBLIC(node) &&
    ...
    error(
        "external linkage required for symbol %q+D because of "
        "%qs attribute",
  ...
  if (!*no_add_attrs) {
    if (DECL_VISIBILITY_SPECIFIED(node) &&
      ...
      error(
          "%qs implies default visibility, but %qD has already "
  ...
  if (!t) {
    ...
    if (TYPE_STRUCTURAL_EQUALITY_P(type))
      ...
      /* Build the underlying canonical type, since it is different
         from TYPE. */
      TYPE_CANONICAL(t) =
          build_qualified_type(TYPE_CANONICAL(type), type_quals);
    else
...
int tree_map_base_marked_p(const void *p) {
...
static struct tree_priority_map *decl_priority_info(tree decl) {
...
tree decl_restrict_base_lookup(tree from) {
  struct tree_map *h;
  struct tree_map in;
...
void decl_restrict_base_insert(tree from, tree to) {
  struct tree_map *h;
  void **loc;
...
static void print_restrict_base_statistics(void) {
...
static unsigned int type_hash_list(const_tree list, hashval_t hashcode) {
  const_tree tail;

  for (tail = list; tail; tail = TREE_CHAIN(tail))
    if (TREE_VALUE(tail) != error_mark_node)
      hashcode = iterative_hash_object(TYPE_HASH(TREE_VALUE(tail)), hashcode);
    ...
    case ENUMERAL_TYPE:
      if (TYPE_VALUES(a->type) != TYPE_VALUES(b->type) &&
          !(TYPE_VALUES(a->type) &&
            ...
            type_list_equal(TYPE_VALUES(a->type), TYPE_VALUES(b->type))))
        return 0;
...
static unsigned int attribute_hash_list(const_tree list, hashval_t hashcode) {
  const_tree tail;

  for (tail = list; tail; tail = TREE_CHAIN(tail))
    /* ??? Do we want to add in TREE_VALUE too? */
    hashcode = iterative_hash_object(IDENTIFIER_HASH_VALUE(TREE_PURPOSE(tail)),
                                     hashcode);
  return hashcode;
}
  ...
  for (; t2 != 0; t2 = TREE_CHAIN(t2)) {
    ...
    for (attr = lookup_attribute(IDENTIFIER_POINTER(TREE_PURPOSE(t2)),
         ...
         attr = lookup_attribute(IDENTIFIER_POINTER(TREE_PURPOSE(t2)),
                                 TREE_CHAIN(attr))) {
      if (TREE_VALUE(t2) != NULL && TREE_CODE(TREE_VALUE(t2)) == TREE_LIST &&
          TREE_VALUE(attr) != NULL &&
          TREE_CODE(TREE_VALUE(attr)) == TREE_LIST) {
        if (simple_cst_list_equal(TREE_VALUE(t2), TREE_VALUE(attr)) == 1)
          break;
...
unsigned int tree_int_cst_min_precision(tree value, bool unsignedp) {
  int log;
...
int simple_cst_equal(const_tree t1, const_tree t2) {
  ...
  if (CONVERT_EXPR_CODE_P(code1) || code1 == NON_LVALUE_EXPR) {
    if (CONVERT_EXPR_CODE_P(code2) || code2 == NON_LVALUE_EXPR)
      return simple_cst_equal(TREE_OPERAND(t1, 0), TREE_OPERAND(t2, 0));
    else
      return simple_cst_equal(TREE_OPERAND(t1, 0), t2);
  }

  else if (CONVERT_EXPR_CODE_P(code2) || code2 == NON_LVALUE_EXPR)
        ...
        return 0;
      {
        ...
        for (arg1 = first_const_call_expr_arg(t1, &iter1),
            arg2 = first_const_call_expr_arg(t2, &iter2);
             arg1 && arg2; arg1 = next_const_call_expr_arg(&iter1),
...
hashval_t iterative_hash_expr(const_tree t, hashval_t val) {
  int i;
  enum tree_code code;
  char tclass;
  ...
  switch (code) {
    ...
    case STRING_CST:
      return iterative_hash(TREE_STRING_POINTER(t), TREE_STRING_LENGTH(t), val);
    case COMPLEX_CST:
    ...
    case FUNCTION_DECL:
      ...
      if (DECL_BUILT_IN(t)) {
        val =
            iterative_hash_pointer(built_in_decls[DECL_FUNCTION_CODE(t)], val);
        return val;
      }
      /* else FALL THROUGH */
    default:
...
hashval_t iterative_hash_exprs_commutative(const_tree t1, const_tree t2,
                                           hashval_t val) {
...
tree build_pointer_type_for_mode(tree to_type, enum machine_mode mode,
                                 bool can_alias_all) {
  tree t;
...
tree build_reference_type_for_mode(tree to_type, enum machine_mode mode,
                                   bool can_alias_all) {
  tree t;
  ...
  if (TYPE_STRUCTURAL_EQUALITY_P(to_type))
    ...
    TYPE_CANONICAL(t) = build_reference_type_for_mode(TYPE_CANONICAL(to_type),
                                                      mode, can_alias_all);
...
tree build_type_no_quals(tree t) {
  switch (TREE_CODE(t)) {
    ...
    case REFERENCE_TYPE:
      return build_reference_type_for_mode(build_type_no_quals(TREE_TYPE(t)),
                                           TYPE_MODE(t),
                                           TYPE_REF_CAN_ALIAS_ALL(t));
    default:
...
tree build_range_type(tree type, tree lowval, tree highval) {
  ...
  if (host_integerp(lowval, 0) && highval != 0 && host_integerp(highval, 0))
    return type_hash_canon(tree_low_cst(highval, 0) - tree_low_cst(lowval, 0),
                           itype);
  else
    return itype;
}
...
static tree maybe_canonicalize_argtypes(tree argtypes, bool *any_structural_p,
...
tree build_function_type(tree value_type, tree arg_types) {
  ...
  canon_argtypes = maybe_canonicalize_argtypes(arg_types, &any_structural_p,
                                               &any_noncanonical_p);
  if (any_structural_p)
    SET_TYPE_STRUCTURAL_EQUALITY(t);
  else if (any_noncanonical_p)
    TYPE_CANONICAL(t) =
        build_function_type(TYPE_CANONICAL(value_type), canon_argtypes);
...
tree build_function_type_skip_args(tree orig_type, bitmap args_to_skip) {
  ...
  /* Use copy_node to preserve as much as possible from original type
     ...
     instead.  */
  if (TREE_CODE(orig_type) != METHOD_TYPE || !bitmap_bit_p(args_to_skip, 0)) {
    new_type = copy_node(orig_type);
    TYPE_ARG_TYPES(new_type) = new_reversed;
  } else {
    new_type = build_distinct_type_copy(
        build_function_type(TREE_TYPE(orig_type), new_reversed));
...
static tree build_function_type_list_1(bool vaargs, tree return_type,
                                       va_list argp) {
  tree t, args, last;
...
tree build_method_type_directly(tree basetype, tree rettype, tree argtypes) {
  ...
  /* Set up the canonical type. */
  any_structural_p = (TYPE_STRUCTURAL_EQUALITY_P(basetype) ||
                      TYPE_STRUCTURAL_EQUALITY_P(rettype));
  any_noncanonical_p = (TYPE_CANONICAL(basetype) != basetype ||
                        TYPE_CANONICAL(rettype) != rettype);
  canon_argtypes = maybe_canonicalize_argtypes(
      TREE_CHAIN(argtypes), &any_structural_p, &any_noncanonical_p);
  if (any_structural_p)
    SET_TYPE_STRUCTURAL_EQUALITY(t);
  else if (any_noncanonical_p)
    TYPE_CANONICAL(t) = build_method_type_directly(
        TYPE_CANONICAL(basetype), TYPE_CANONICAL(rettype), canon_argtypes);
  if (!COMPLETE_TYPE_P(t))
    layout_type(t);
...
tree build_method_type(tree basetype, tree type) {
  ...
  return build_method_type_directly(basetype, TREE_TYPE(type),
                                    TYPE_ARG_TYPES(type));
}
  ...
  if (TYPE_CANONICAL(t) == t) {
    if (TYPE_STRUCTURAL_EQUALITY_P(basetype) ||
      ...
      TYPE_CANONICAL(t) = build_offset_type(
          TYPE_CANONICAL(TYPE_MAIN_VARIANT(basetype)), TYPE_CANONICAL(type));
  }
...
tree build_complex_type(tree component_type) {
  ...
  if (TYPE_CANONICAL(t) == t) {
    if (TYPE_STRUCTURAL_EQUALITY_P(component_type))
      ...
      TYPE_CANONICAL(t) = build_complex_type(TYPE_CANONICAL(component_type));
  }
...
tree get_unwidened(tree op, tree for_type) {
  ...
  int uns = (for_type != 0 && for_type != type &&
             final_prec > TYPE_PRECISION(type) && TYPE_UNSIGNED(type));
  tree win = op;
  ...
  while (CONVERT_EXPR_P(op)) {
    ...
    if (bitschange > 0) {
      ...
      if ((uns || CONVERT_EXPR_P(op)) && TYPE_UNSIGNED(TREE_TYPE(op))) {
        uns = 1;
        win = op;
      }
    }
...
tree get_narrower(tree op, int *unsignedp_ptr) {
  ...
  if (TREE_CODE(op) == COMPONENT_REF
    ...
    if (innerprec < TYPE_PRECISION(TREE_TYPE(op)) &&
        ...
        (first || uns == DECL_UNSIGNED(TREE_OPERAND(op, 1))) && type != 0) {
      if (first)
...
int int_fits_type_p(const_tree c, const_tree type) {
  ...
  if (TREE_CODE(TREE_TYPE(c)) == INTEGER_TYPE &&
      TYPE_IS_SIZETYPE(TREE_TYPE(c)) && unsc)
...
void get_type_static_bounds(const_tree type, mpz_t min, mpz_t max) {
  ...
  if (!POINTER_TYPE_P(type) && TYPE_MAX_VALUE(type) &&
    ...
    if (TYPE_UNSIGNED(type))
      ...
      mpz_set_double_int(max, double_int_mask(TYPE_PRECISION(type) - 1), true);
  }
}
...
bool auto_var_in_fn_p(const_tree var, const_tree fn) {
  return (DECL_P(var) && DECL_CONTEXT(var) == fn &&
          (((TREE_CODE(var) == VAR_DECL || TREE_CODE(var) == PARM_DECL) &&
            !TREE_STATIC(var)) ||
           TREE_CODE(var) == LABEL_DECL || TREE_CODE(var) == RESULT_DECL));
}
...
tree get_callee_fndecl(const_tree call) {
  ...
  if (DECL_P(addr) && TREE_CODE(addr) != FUNCTION_DECL && TREE_READONLY(addr) &&
      !TREE_THIS_VOLATILE(addr) && DECL_INITIAL(addr))
...
void tree_check_failed(const_tree node, const char *file, int line,
                       const char *function, ...) {
  va_list args;
  const char *buffer;
  unsigned length = 0;
  ...
  internal_error("tree check: %s, have %s in %s, at %s:%d", buffer,
                 tree_code_name[TREE_CODE(node)], function, trim_filename(file),
                 line);
}
...
void tree_not_check_failed(const_tree node, const char *file, int line,
                           const char *function, ...) {
  va_list args;
  char *buffer;
  unsigned length = 0;
  ...
  internal_error("tree check: expected none of %s, have %s in %s, at %s:%d",
                 buffer, tree_code_name[TREE_CODE(node)], function,
...
void tree_class_check_failed(const_tree node, const enum tree_code_class cl,
...
void tree_range_check_failed(const_tree node, const char *file, int line,
...
void tree_not_class_check_failed(const_tree node, const enum tree_code_class cl,
...
void omp_clause_check_failed(const_tree node, const char *file, int line,
...
void omp_clause_range_check_failed(const_tree node, const char *file, int line,
                                   const char *function,
  ...
  internal_error("tree check: %s, have %s in %s, at %s:%d", buffer,
                 omp_clause_code_name[TREE_CODE(node)], function,
                 trim_filename(file), line);
}
...
void tree_contains_struct_check_failed(const_tree node,
                                       ...
                                       const char *function) {
  internal_error(
      "tree check: expected tree that contains %qs structure, have %qs in %s, "
      "at %s:%d",
      TS_ENUM_NAME(en), tree_code_name[TREE_CODE(node)], function,
      trim_filename(file), line);
}
...
void tree_vec_elt_check_failed(int idx, int len, const char *file, int line,
                               const char *function) {
  internal_error(
...
void tree_operand_check_failed(int idx, const_tree exp, const char *file,
                               int line, const char *function) {
  int code = TREE_CODE(exp);
  internal_error(
      "tree check: accessed operand %d of %s with %d operands in %s, at %s:%d",
      idx + 1, tree_code_name[code], TREE_OPERAND_LENGTH(exp), function,
      trim_filename(file), line);
}
...
void omp_clause_operand_check_failed(int idx, const_tree t, const char *file,
                                     int line, const char *function) {
  internal_error(
      "tree check: accessed operand %d of omp_clause %s with %d operands "
      "in %s, at %s:%d",
      idx + 1, omp_clause_code_name[OMP_CLAUSE_CODE(t)],
      omp_clause_num_ops[OMP_CLAUSE_CODE(t)], function, trim_filename(file),
      line);
}
...
static tree make_vector_type(tree innertype, int nunits,
  ...
  if (TYPE_STRUCTURAL_EQUALITY_P(innertype))
    SET_TYPE_STRUCTURAL_EQUALITY(t);
  else if (TYPE_CANONICAL(innertype) != innertype || mode != VOIDmode)
    TYPE_CANONICAL(t) =
...
void build_common_tree_nodes(bool signed_char, bool signed_sizetype) {
  ...
  char_type_node = (signed_char ? make_signed_type(CHAR_TYPE_SIZE)
...
#define MAKE_FIXED_TYPE_NODE(KIND, SIZE)                                   \
  sat_##KIND##_type_node = make_sat_signed_##KIND##_type(SIZE);            \
  sat_unsigned_##KIND##_type_node = make_sat_unsigned_##KIND##_type(SIZE); \
  KIND##_type_node = make_signed_##KIND##_type(SIZE);                      \
  unsigned_##KIND##_type_node = make_unsigned_##KIND##_type(SIZE);

#define MAKE_FIXED_TYPE_NODE_WIDTH(KIND, WIDTH, SIZE)                  \
  sat_##WIDTH##KIND##_type_node = make_sat_signed_##KIND##_type(SIZE); \
  sat_unsigned_##WIDTH##KIND##_type_node =                             \
      make_sat_unsigned_##KIND##_type(SIZE);                           \
  WIDTH##KIND##_type_node = make_signed_##KIND##_type(SIZE);           \
  unsigned_##WIDTH##KIND##_type_node = make_unsigned_##KIND##_type(SIZE);
...
#define MAKE_FIXED_MODE_NODE(KIND, NAME, MODE)                               \
  ...
  u##NAME##_type_node =                                                      \
      make_or_reuse_unsigned_##KIND##_type(GET_MODE_BITSIZE(U##MODE##mode)); \
  sat_##NAME##_type_node =                                                   \
      make_or_reuse_sat_signed_##KIND##_type(GET_MODE_BITSIZE(MODE##mode));  \
  sat_u##NAME##_type_node = make_or_reuse_sat_unsigned_##KIND##_type(        \
...
static void local_define_builtin(const char *name, tree type,
...
void build_common_builtin_nodes(void) {
  ...
  if (built_in_decls[BUILT_IN_MEMCPY] == NULL ||
    ...
    if (built_in_decls[BUILT_IN_MEMCPY] == NULL)
      local_define_builtin("__builtin_memcpy", ftype, BUILT_IN_MEMCPY, "memcpy",
                           ECF_NOTHROW);
  ...
  if (built_in_decls[BUILT_IN_MEMCMP] == NULL) {
    ...
    local_define_builtin("__builtin_memcmp", ftype, BUILT_IN_MEMCMP, "memcmp",
  ...
  if (built_in_decls[BUILT_IN_MEMSET] == NULL) {
    ...
    tmp = tree_cons(NULL_TREE, ptr_type_node, tmp);
    ftype = build_function_type(ptr_type_node, tmp);
    local_define_builtin("__builtin_memset", ftype, BUILT_IN_MEMSET, "memset",
                         ECF_NOTHROW);
  }
  ...
  if (built_in_decls[BUILT_IN_ALLOCA] == NULL) {
    tmp = tree_cons(NULL_TREE, size_type_node, void_list_node);
    ftype = build_function_type(ptr_type_node, tmp);
    local_define_builtin("__builtin_alloca", ftype, BUILT_IN_ALLOCA, "alloca",
  ...
  local_define_builtin("__builtin_init_trampoline", ftype,
                       BUILT_IN_INIT_TRAMPOLINE, "__builtin_init_trampoline",
                       ECF_NOTHROW);
  ...
  local_define_builtin("__builtin_adjust_trampoline", ftype,
                       ...
                       "__builtin_adjust_trampoline", ECF_CONST | ECF_NOTHROW);

  tmp = tree_cons(NULL_TREE, ptr_type_node, void_list_node);
  tmp = tree_cons(NULL_TREE, ptr_type_node, tmp);
  ftype = build_function_type(void_type_node, tmp);
  local_define_builtin("__builtin_nonlocal_goto", ftype, BUILT_IN_NONLOCAL_GOTO,
                       "__builtin_nonlocal_goto", ECF_NORETURN | ECF_NOTHROW);

  tmp = tree_cons(NULL_TREE, ptr_type_node, void_list_node);
  tmp = tree_cons(NULL_TREE, ptr_type_node, tmp);
  ftype = build_function_type(void_type_node, tmp);
  local_define_builtin("__builtin_setjmp_setup", ftype, BUILT_IN_SETJMP_SETUP,
  ...
  local_define_builtin("__builtin_setjmp_dispatcher", ftype,
                       ...
                       "__builtin_setjmp_dispatcher", ECF_PURE | ECF_NOTHROW);

  tmp = tree_cons(NULL_TREE, ptr_type_node, void_list_node);
  ftype = build_function_type(void_type_node, tmp);
  local_define_builtin("__builtin_setjmp_receiver", ftype,
                       BUILT_IN_SETJMP_RECEIVER, "__builtin_setjmp_receiver",
                       ECF_NOTHROW);
  ...
  tmp = tree_cons(NULL_TREE, ptr_type_node, void_list_node);
  ftype = build_function_type(void_type_node, tmp);
  local_define_builtin("__builtin_stack_restore", ftype, BUILT_IN_STACK_RESTORE,
     ...
     beasties if the real and complex parts of the arguments are separate.  */
  {
    ...
    for (mode = MIN_MODE_COMPLEX_FLOAT; mode <= MAX_MODE_COMPLEX_FLOAT;
         ++mode) {
...
tree reconstruct_complex_type(tree type, tree bottom) {
  ...
  if (TREE_CODE(type) == POINTER_TYPE) {
    ...
    /* The build_method_type_directly() routine prepends 'this' to argument
       list, so we must compensate by getting rid of it.  */
    outer =
        build_method_type_directly(TREE_TYPE(TREE_VALUE(TYPE_ARG_TYPES(type))),
                                   inner, TREE_CHAIN(TYPE_ARG_TYPES(type)));
...
tree build_vector_type_for_mode(tree innertype, enum machine_mode mode) {
  ...
  switch (GET_MODE_CLASS(mode)) {
    ...
    case MODE_INT:
      ...
      gcc_assert(
          GET_MODE_BITSIZE(mode) % TREE_INT_CST_LOW(TYPE_SIZE(innertype)) == 0);
...
tree build_resx(int region_number) {
  tree t;
  ...
  switch (TREE_CODE(init)) {
    ...
    case REAL_CST:
      /* ??? Note that this is not correct for C4X float formats.  There,
         ...
         negative exponent.  */
      return real_zerop(init) && !REAL_VALUE_MINUS_ZERO(TREE_REAL_CST(init));
...
tree build_omp_clause(enum omp_clause_code code) {
  tree t;
  int size, length;
...
tree build_vl_exp_stat(enum tree_code code, int len MEM_STAT_DECL) {
  tree t;
...
tree build_call_array(tree return_type, tree fn, int nargs, tree *args) {
  tree t;
  int i;
...
bool in_array_bounds_p(tree ref) {
  ...
  if (!min || !max || TREE_CODE(min) != INTEGER_CST ||
      TREE_CODE(max) != INTEGER_CST)
    return false;

  if (tree_int_cst_lt(idx, min) || tree_int_cst_lt(max, idx))
    return false;
...
bool range_in_array_bounds_p(tree ref) {
  tree domain_type = TYPE_DOMAIN(TREE_TYPE(ref));
  tree range_min, range_max, min, max;

  range_min = TYPE_MIN_VALUE(domain_type);
  range_max = TYPE_MAX_VALUE(domain_type);
  if (!range_min || !range_max || TREE_CODE(range_min) != INTEGER_CST ||
  ...
  if (tree_int_cst_lt(range_min, min) || tree_int_cst_lt(max, range_max))
    return false;
...
bool needs_to_live_in_memory(const_tree t) {
  ...
  return (TREE_ADDRESSABLE(t) || is_global_var(t) ||
...
bool fields_compatible_p(const_tree f1, const_tree f2) {
  if (!operand_equal_p(DECL_FIELD_BIT_OFFSET(f1), DECL_FIELD_BIT_OFFSET(f2),
                       OEP_ONLY_CONST))
    return false;

  if (!operand_equal_p(DECL_FIELD_OFFSET(f1), DECL_FIELD_OFFSET(f2),
                       OEP_ONLY_CONST))
    return false;
...
tree find_compatible_field(tree record, tree orig_field) {
  ...
  for (f = TYPE_FIELDS(record); f; f = TREE_CHAIN(f))
    if (TREE_CODE(f) == FIELD_DECL && fields_compatible_p(f, orig_field))
      return f;

  /* ??? Why isn't this on the main fields list?  */
  f = TYPE_VFIELD(record);
  if (f && TREE_CODE(f) == FIELD_DECL && fields_compatible_p(f, orig_field))
    return f;
...
int_cst_value(const_tree x) {
  ...
  gcc_assert(TREE_INT_CST_HIGH(x) == 0 || TREE_INT_CST_HIGH(x) == -1);
...
tree upper_bound_in_type(tree outer, tree inner) {
  ...
  if (prec <= HOST_BITS_PER_WIDE_INT) {
    hi = 0;
    lo = ((~(unsigned HOST_WIDE_INT)0) >> (HOST_BITS_PER_WIDE_INT - prec));
  } else {
    hi = ((~(unsigned HOST_WIDE_INT)0) >> (2 * HOST_BITS_PER_WIDE_INT - prec));
...
tree lower_bound_in_type(tree outer, tree inner) {
  ...
  if (TYPE_UNSIGNED(outer)
    ...
    if (prec <= HOST_BITS_PER_WIDE_INT) {
      ...
      hi =
          ((~(unsigned HOST_WIDE_INT)0) << (prec - HOST_BITS_PER_WIDE_INT - 1));
      lo = 0;
    }
  }
...
static tree walk_type_fields(tree type, walk_tree_fn func, void *data,
  ...
  switch (TREE_CODE(type)) {
    ...
    case REFERENCE_TYPE:
      ...
      if (POINTER_TYPE_P(TREE_TYPE(type)) &&
          ...
          POINTER_TYPE_P(TREE_TYPE(TREE_TYPE(TREE_TYPE(type)))) && !pset) {
        result = walk_tree_without_duplicates(&TREE_TYPE(type), func, data);
        if (result)
          return result;
    ...
    case ARRAY_TYPE:
      /* Don't follow this nodes's type if a pointer for fear that
         ...
         need not fear.  */
      if (pset || (!POINTER_TYPE_P(TREE_TYPE(type)) &&
...
tree walk_tree_1(tree *tp, walk_tree_fn func, void *data,
                 struct pointer_set_t *pset, walk_tree_lh lh) {
  enum tree_code code;
  int walk_subtrees;
  tree result;
...
tail_recurse:
  ...
  switch (code) {
    ...
    case CONSTRUCTOR: {
      ...
      for (idx = 0;
           VEC_iterate(constructor_elt, CONSTRUCTOR_ELTS(*tp), idx, ce); idx++)
    ...
    case DECL_EXPR:
      ...
      if (TREE_CODE(DECL_EXPR_DECL(*tp)) == TYPE_DECL) {
        ...
        if (TREE_CODE(*type_p) == RECORD_TYPE ||
          ...
          for (field = TYPE_FIELDS(*type_p); field; field = TREE_CHAIN(field)) {
...
tree walk_tree_without_duplicates_1(tree *tp, walk_tree_fn func, void *data,
                                    walk_tree_lh lh) {
  tree result;
...
int function_args_count(tree fntype) {
  function_args_iterator args_iter;
  tree t;
  int num = 0;
...
location_t tree_nonartificial_location(tree exp) {
  ...
  while (block && TREE_CODE(block) == BLOCK && BLOCK_ABSTRACT_ORIGIN(block)) {
    ...
    do {
      if (TREE_CODE(ao) == FUNCTION_DECL && DECL_DECLARED_INLINE_P(ao) &&
          lookup_attribute("artificial", DECL_ATTRIBUTES(ao)))
        return BLOCK_SOURCE_LOCATION(block);
      else if (TREE_CODE(ao) == BLOCK && BLOCK_SUPERCONTEXT(ao) != ao)
...
static hashval_t cl_option_hash_hash(const void *x) {
  const_tree const t = (const_tree)x;
  const char *p;
  size_t i;
  size_t len = 0;
...
static int cl_option_hash_eq(const void *x, const void *y) {
  const_tree const xt = (const_tree)x;
  const_tree const yt = (const_tree)y;
  const char *xp;
  const char *yp;
...
tree block_ultimate_origin(const_tree block) {
  ...
  if (immediate_origin == NULL_TREE)
    ...
    do {
      ret_val = lookahead;
      lookahead =
          (TREE_CODE(ret_val) == BLOCK ? BLOCK_ABSTRACT_ORIGIN(ret_val) : NULL);
