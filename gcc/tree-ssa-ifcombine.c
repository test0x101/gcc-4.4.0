...
static bool recognize_if_then_else(basic_block cond_bb, basic_block *then_bb,
                                   basic_block *else_bb) {
  edge t, e;
  ...
  if (!(t->flags & EDGE_TRUE_VALUE) || !(e->flags & EDGE_FALSE_VALUE))
    return false;

  /* Check if the edge destinations point to the required block.  */
  if (*then_bb && t->dest != *then_bb)
    return false;
  if (*else_bb && e->dest != *else_bb)
    return false;
...
static bool same_phi_args_p(basic_block bb1, basic_block bb2,
                            basic_block dest) {
...
static tree get_name_for_bit_test(tree candidate) {
  ...
  if (TREE_CODE(candidate) == SSA_NAME && has_single_use(candidate)) {
    gimple def_stmt = SSA_NAME_DEF_STMT(candidate);
    if (is_gimple_assign(def_stmt) && gimple_assign_cast_p(def_stmt)) {
...
static int operand_precision(tree t) {
  return TYPE_PRECISION(TREE_TYPE(t));
}

static bool integral_operand_p(tree t) {
...
static bool recognize_single_bit_test(gimple cond, tree *name, tree *bit) {
  gimple stmt;
  ...
  if (gimple_assign_rhs_code(stmt) == BIT_AND_EXPR &&
    ...
    if (is_gimple_assign(stmt) && gimple_assign_rhs_code(stmt) == RSHIFT_EXPR) {
      /* op0 & (1 << op1) */
  ...
  if (gimple_assign_rhs_code(stmt) == BIT_AND_EXPR &&
    ...
    *name = gimple_assign_rhs1(stmt);
    *bit =
        build_int_cst(integer_type_node, tree_log2(gimple_assign_rhs2(stmt)));
    return true;
  }
  ...
  if (gimple_assign_rhs_code(stmt) == BIT_AND_EXPR &&
    ...
    if (is_gimple_assign(tmp) && gimple_assign_rhs_code(tmp) == LSHIFT_EXPR &&
...
static bool recognize_bits_test(gimple cond, tree *name, tree *bits) {
  gimple stmt;
  ...
  if (gimple_cond_code(cond) != NE_EXPR ||
    ...
    return false;
  stmt = SSA_NAME_DEF_STMT(gimple_cond_lhs(cond));
  if (!is_gimple_assign(stmt) || gimple_assign_rhs_code(stmt) != BIT_AND_EXPR)
    return false;
...
static bool ifcombine_ifandif(basic_block inner_cond_bb,
  ...
  if (!inner_cond || gimple_code(inner_cond) != GIMPLE_COND)
    return false;

  outer_cond = last_stmt(outer_cond_bb);
  if (!outer_cond || gimple_code(outer_cond) != GIMPLE_COND)
    return false;
  ...
  if (recognize_single_bit_test(inner_cond, &name1, &bit1) &&
      recognize_single_bit_test(outer_cond, &name2, &bit2) && name1 == name2) {
    tree t, t2;
    ...
    t2 = force_gimple_operand_gsi(&gsi, t2, true, NULL_TREE, true,
                                  GSI_SAME_STMT);
...
static bool ifcombine_iforif(basic_block inner_cond_bb,
  ...
  if (recognize_bits_test(inner_cond, &name1, &bits1) &&
    ...
    if (TYPE_PRECISION(TREE_TYPE(bits1)) >= TYPE_PRECISION(TREE_TYPE(bits2))) {
...
static unsigned int tree_ssa_ifcombine(void) {
  basic_block *bbs;
...
static bool gate_ifcombine(void) {
  return 1;
}

struct gimple_opt_pass pass_tree_ifcombine = {{
    GIMPLE_PASS, "ifcombine", /* name */
    gate_ifcombine,           /* gate */
