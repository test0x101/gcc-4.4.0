...
void ipa_push_func_to_list(struct ipa_func_list **wl, struct cgraph_node *mt) {
...
struct cgraph_node *ipa_pop_func_from_list(struct ipa_func_list **wl) {
...
static int ipa_get_param_decl_index(struct ipa_node_params *info, tree ptree) {
  int i, count;
...
static void ipa_count_formal_params(struct cgraph_node *node,
                                    struct ipa_node_params *info) {
  tree fndecl;
  tree fnargs;
  tree parm;
...
void ipa_initialize_node_params(struct cgraph_node *node) {
  ...
  if (!info->params) {
    ...
    info->params =
        XCNEWVEC(struct ipa_param_descriptor, ipa_get_param_count(info));
...
static void ipa_check_stmt_modifications(struct ipa_node_params *info,
                                         gimple stmt) {
  int j;
  int index;
  tree lhs;
...
void ipa_detect_param_modifications(struct cgraph_node *node) {
  tree decl = node->decl;
  basic_block bb;
...
void ipa_count_arguments(struct cgraph_edge *cs) {
  gimple stmt;
  int arg_num;
...
static void compute_scalar_jump_functions(struct ipa_node_params *info,
...
static bool compute_pass_through_member_ptrs(struct ipa_node_params *info,
                                             struct ipa_jump_func *functions,
                                             gimple call) {
...
static void fill_member_ptr_cst_jump_function(struct ipa_jump_func *jfunc,
                                              tree pfn, tree delta) {
...
static void determine_cst_member_ptr(gimple call, tree arg, tree method_field,
                                     tree delta_field,
  ...
  for (; !gsi_end_p(gsi); gsi_prev(&gsi)) {
    ...
    if (TREE_CODE(lhs) != COMPONENT_REF || TREE_OPERAND(lhs, 0) != arg)
      continue;
...
static void compute_cst_member_ptr_arguments(struct ipa_jump_func *functions,
                                             gimple call) {
  unsigned num;
  ...
  for (num = 0; num < gimple_call_num_args(call); num++) {
    ...
    if (functions[num].type == IPA_UNKNOWN &&
        type_like_member_ptr_p(TREE_TYPE(arg), &method_field, &delta_field))
...
void ipa_compute_jump_functions(struct cgraph_edge *cs) {
  ...
  if (ipa_get_cs_argument_count(arguments) == 0 || arguments->jump_functions)
    return;
  arguments->jump_functions =
      XCNEWVEC(struct ipa_jump_func, ipa_get_cs_argument_count(arguments));
...
static tree ipa_get_member_ptr_load_param(tree rhs) {
  tree rec, fld;
  tree ptr_field;
...
static bool ipa_is_ssa_with_stmt_def(tree t) {
  if (TREE_CODE(t) == SSA_NAME && !SSA_NAME_IS_DEFAULT_DEF(t))
    return true;
  else
    return false;
}
...
static void ipa_note_param_call(struct ipa_node_params *info, int formal_id,
                                gimple stmt) {
...
static void ipa_analyze_call_uses(struct ipa_node_params *info, gimple call) {
  ...
  if (!single_pred_p(virt_bb) || !single_succ_p(virt_bb) ||
      single_pred(virt_bb) != bb || single_succ(virt_bb) != join)
    return;
...
static void ipa_analyze_stmt_uses(struct ipa_node_params *info, gimple stmt) {
...
void ipa_analyze_params_uses(struct cgraph_node *node) {
  tree decl = node->decl;
  basic_block bb;
  struct function *func;
  gimple_stmt_iterator gsi;
...
static void print_edge_addition_message(FILE *f, struct ipa_param_call_note *nt,
                                        struct ipa_jump_func *jfunc,
                                        struct cgraph_node *node) {
...
static bool update_call_notes_after_inlining(struct cgraph_edge *cs,
                                             struct cgraph_node *node,
  ...
  for (nt = info->param_calls; nt; nt = nt->next) {
    ...
    if (jfunc->type == IPA_PASS_THROUGH)
      ...
      new_indirect_edge = cgraph_create_edge(node, callee, nt->stmt, nt->count,
...
void ipa_free_edge_args_substructures(struct ipa_edge_args *args) {
...
void ipa_free_all_edge_args(void) {
  ...
  for (i = 0; VEC_iterate(ipa_edge_args_t, ipa_edge_args_vector, i, args); i++)
...
void ipa_free_node_params_substructures(struct ipa_node_params *info) {
  if (info->params)
    free(info->params);
...
void ipa_free_all_node_params(void) {
  ...
  for (i = 0; VEC_iterate(ipa_node_params_t, ipa_node_params_vector, i, info);
       i++)
...
static void ipa_edge_removal_hook(struct cgraph_edge *cs,
                                  void *data ATTRIBUTE_UNUSED) {
  /* During IPA-CP updating we can be called on not-yet analyze clones.  */
  if (VEC_length(ipa_edge_args_t, ipa_edge_args_vector) <= (unsigned)cs->uid)
    return;
...
static void ipa_node_removal_hook(struct cgraph_node *node,
                                  void *data ATTRIBUTE_UNUSED) {
...
static void *duplicate_array(void *src, size_t n) {
  void *p;
...
static void ipa_edge_duplication_hook(struct cgraph_edge *src,
  ...
  new_args->jump_functions = (struct ipa_jump_func *)duplicate_array(
      old_args->jump_functions, sizeof(struct ipa_jump_func) * arg_count);
}

/* Hook that is called by cgraph.c when a node is duplicated.  */

static void ipa_node_duplication_hook(struct cgraph_node *src,
  ...
  new_info->params = (struct ipa_param_descriptor *)duplicate_array(
      old_info->params, sizeof(struct ipa_param_descriptor) * param_count);
  ...
  for (note = old_info->param_calls; note; note = note->next) {
    ...
    nn = (struct ipa_param_call_note *)xcalloc(
        1, sizeof(struct ipa_param_call_note));
