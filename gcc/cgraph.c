...
struct cgraph_edge_hook_list *cgraph_add_edge_removal_hook(
    cgraph_edge_hook hook, void *data) {
...
void cgraph_remove_edge_removal_hook(struct cgraph_edge_hook_list *entry) {
  struct cgraph_edge_hook_list **ptr = &first_cgraph_edge_removal_hook;
...
static void cgraph_call_edge_removal_hooks(struct cgraph_edge *e) {
...
struct cgraph_node_hook_list *cgraph_add_node_removal_hook(
...
void cgraph_remove_node_removal_hook(struct cgraph_node_hook_list *entry) {
  struct cgraph_node_hook_list **ptr = &first_cgraph_node_removal_hook;
...
static void cgraph_call_node_removal_hooks(struct cgraph_node *node) {
...
struct cgraph_2edge_hook_list *cgraph_add_edge_duplication_hook(
    cgraph_2edge_hook hook, void *data) {
...
void cgraph_remove_edge_duplication_hook(struct cgraph_2edge_hook_list *entry) {
  struct cgraph_2edge_hook_list **ptr = &first_cgraph_edge_duplicated_hook;
...
static void cgraph_call_edge_duplication_hooks(struct cgraph_edge *cs1,
...
void cgraph_remove_node_duplication_hook(struct cgraph_2node_hook_list *entry) {
  struct cgraph_2node_hook_list **ptr = &first_cgraph_node_duplicated_hook;
...
static void cgraph_call_node_duplication_hooks(struct cgraph_node *node1,
...
static struct cgraph_node *cgraph_create_node(void) {
  ...
  *slot = node;
  ...
  if (assembler_name_hash) {
    ...
    aslot = htab_find_slot_with_hash(assembler_name_hash, name,
                                     decl_assembler_name_hash(name), INSERT);
...
struct cgraph_node *cgraph_node_for_asm(tree asmname) {
  ...
  if (!assembler_name_hash) {
    assembler_name_hash = htab_create_ggc(10, hash_node_by_assembler_name,
  ...
  slot = htab_find_slot_with_hash(assembler_name_hash, asmname,
                                  decl_assembler_name_hash(asmname), NO_INSERT);
...
static hashval_t edge_hash(const void *x) {
  return htab_hash_pointer(((const struct cgraph_edge *)x)->call_stmt);
}

/* Return nonzero if decl_id of die_struct X is the same as UID of decl *Y.  */

static int edge_eq(const void *x, const void *y) {
...
struct cgraph_edge *cgraph_edge(struct cgraph_node *node, gimple call_stmt) {
  struct cgraph_edge *e, *e2;
  int n = 0;

  if (node->call_site_hash)
    return (struct cgraph_edge *)htab_find_with_hash(
        node->call_site_hash, call_stmt, htab_hash_pointer(call_stmt));
  ...
  if (n > 100) {
    ...
    for (e2 = node->callees; e2; e2 = e2->next_callee) {
      void **slot;
      slot = htab_find_slot_with_hash(node->call_site_hash, e2->call_stmt,
                                      htab_hash_pointer(e2->call_stmt), INSERT);
      gcc_assert(!*slot);
      *slot = e2;
    }
  }
...
void cgraph_set_call_stmt(struct cgraph_edge *e, gimple new_stmt) {
  if (e->caller->call_site_hash) {
    htab_remove_elt_with_hash(e->caller->call_site_hash, e->call_stmt,
                              htab_hash_pointer(e->call_stmt));
  }
  e->call_stmt = new_stmt;
  if (e->caller->call_site_hash) {
    void **slot;
    slot = htab_find_slot_with_hash(e->caller->call_site_hash, e->call_stmt,
...
struct cgraph_edge *cgraph_create_edge(struct cgraph_node *caller,
  ...
  if (caller->call_site_hash) {
    void **slot;
    slot = htab_find_slot_with_hash(caller->call_site_hash, edge->call_stmt,
                                    htab_hash_pointer(edge->call_stmt), INSERT);
    gcc_assert(!*slot);
    *slot = edge;
  }
  return edge;
...
static void cgraph_free_edge(struct cgraph_edge *e) {
  int uid = e->uid;
...
void cgraph_remove_edge(struct cgraph_edge *e) {
  /* Call all edge removal hooks.  */
...
void cgraph_redirect_edge_callee(struct cgraph_edge *e, struct cgraph_node *n) {
...
void cgraph_update_edges_for_call_stmt(gimple old_stmt, gimple new_stmt) {
  ...
  if (old_call != new_call) {
    ...
    if (e) {
      ...
      if (new_call) {
        ...
        if (new_decl) {
          ne = cgraph_create_edge(node, cgraph_node(new_decl), new_stmt, count,
                                  frequency, loop_nest);
...
void cgraph_release_function_body(struct cgraph_node *node) {
...
void cgraph_remove_node(struct cgraph_node *node) {
  void **slot;
  ...
  if (assembler_name_hash) {
    tree name = DECL_ASSEMBLER_NAME(node->decl);
    slot = htab_find_slot_with_hash(assembler_name_hash, name,
                                    decl_assembler_name_hash(name), NO_INSERT);
...
void cgraph_mark_reachable_node(struct cgraph_node *node) {
...
void cgraph_mark_needed_node(struct cgraph_node *node) {
  node->needed = 1;
...
struct cgraph_local_info *cgraph_local_info(tree decl) {
  struct cgraph_node *node;
...
struct cgraph_global_info *cgraph_global_info(tree decl) {
  struct cgraph_node *node;
...
struct cgraph_rtl_info *cgraph_rtl_info(tree decl) {
  struct cgraph_node *node;
...
const char *cgraph_node_name(struct cgraph_node *node) {
...
void dump_cgraph_node(FILE *f, struct cgraph_node *node) {
  struct cgraph_edge *edge;
  ...
  if (node->global.insns &&
      node->global.insns != node->local.inline_summary.self_insns)
  ...
  for (edge = node->callers; edge; edge = edge->next_caller) {
    fprintf(f, "%s/%i ", cgraph_node_name(edge->caller), edge->caller->uid);
    if (edge->count)
  ...
  for (edge = node->callees; edge; edge = edge->next_callee) {
    fprintf(f, "%s/%i ", cgraph_node_name(edge->callee), edge->callee->uid);
    if (!edge->inline_failed)
      fprintf(f, "(inlined) ");
    if (edge->indirect_call)
      fprintf(f, "(indirect) ");
...
void change_decl_assembler_name(tree decl, tree name) {
  ...
  if (TREE_SYMBOL_REFERENCED(DECL_ASSEMBLER_NAME(decl)) && DECL_RTL_SET_P(decl))
...
struct cgraph_asm_node *cgraph_add_asm_node(tree asm_str) {
...
struct cgraph_edge *cgraph_clone_edge(struct cgraph_edge *e,
                                      struct cgraph_node *n, gimple call_stmt,
...
/* Create node representing clone of N executed COUNT times.  Decrease
   ...
   by node.  */
struct cgraph_node *cgraph_clone_node(struct cgraph_node *n, gcov_type count,
                                      int freq, int loop_nest,
...
bool cgraph_is_master_clone(struct cgraph_node *n) {
  return (n == cgraph_master_clone(n));
}

struct cgraph_node *cgraph_master_clone(struct cgraph_node *n) {
...
/* Return function availability.  See cgraph.h for description of individual
   return values.  */
enum availability cgraph_function_body_availability(struct cgraph_node *node) {
  ...
  else if (!(*targetm.binds_local_p)(node->decl) && !DECL_COMDAT(node->decl) &&
...
void cgraph_add_new_function(tree fndecl, bool lowered) {
  struct cgraph_node *node;
  switch (cgraph_state) {
