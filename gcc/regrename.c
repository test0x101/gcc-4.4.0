...
static const char *const scan_actions_name[] = {
    "terminate_all_read", "terminate_overlapping_read",
    "terminate_write",    "terminate_dead",
...
static void scan_rtx_reg(rtx, rtx *, enum reg_class, enum scan_actions,
                         enum op_type, int);
static void scan_rtx_address(rtx, rtx *, enum reg_class, enum scan_actions,
                             enum machine_mode);
...
static void note_sets(rtx x, const_rtx set ATTRIBUTE_UNUSED, void *data) {
...
static void clear_dead_regs(HARD_REG_SET *pset, enum reg_note kind, rtx notes) {
  rtx note;
...
static void regrename_optimize(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    while (all_chains) {
      ...
      for (last = this_du; last->next_use; last = last->next_use) {
        n_uses++;
        IOR_COMPL_HARD_REG_SET(this_unavailable, reg_class_contents[last->cl]);
      }
      if (n_uses < 1)
        continue;
        ...
        for (i = nregs - 1; i >= 0; --i)
          if (TEST_HARD_REG_BIT(this_unavailable, new_reg + i) ||
              ...
              /* We can't use a non-leaf register if we're in a
                 leaf function.  */
              || (current_function_is_leaf && !LEAF_REGISTERS[new_reg + i])
#endif
        ...
        for (tmp = this_du; tmp; tmp = tmp->next_use)
          if (!HARD_REGNO_MODE_OK(new_reg, GET_MODE(*tmp->loc)) ||
              (tmp->need_caller_save_reg &&
               !(HARD_REGNO_CALL_PART_CLOBBERED(reg, GET_MODE(*tmp->loc))) &&
               (HARD_REGNO_CALL_PART_CLOBBERED(new_reg, GET_MODE(*tmp->loc)))))
            break;
        if (!tmp) {
...
static void do_replace(struct du_chain *chain, int reg) {
  while (chain) {
...
static void scan_rtx_reg(rtx insn, rtx *loc, enum reg_class cl,
                         enum scan_actions action, enum op_type type,
                         int earlyclobber) {
  struct du_chain **p;
  rtx x = *loc;
    ...
    if (*this_du->loc == cc0_rtx)
      ...
      if (action != terminate_overlapping_read || !exact_match) {
        ...
        if ((action == terminate_dead || action == terminate_write) &&
          ...
          if (dump_file)
            fprintf(dump_file, "Closing chain %s at insn %d (%s)\n",
                    reg_names[REGNO(*this_du->loc)], INSN_UID(insn),
                    scan_actions_name[(int)action]);
        } else {
          if (dump_file)
            fprintf(dump_file, "Discarding chain %s at insn %d (%s)\n",
                    reg_names[REGNO(*this_du->loc)], INSN_UID(insn),
                    scan_actions_name[(int)action]);
        }
        *p = next;
...
static void scan_rtx_address(rtx insn, rtx *loc, enum reg_class cl,
...
static void scan_rtx(rtx insn, rtx *loc, enum reg_class cl,
                     enum scan_actions action, enum op_type type,
                     int earlyclobber) {
  const char *fmt;
  rtx x = *loc;
  ...
  switch (code) {
    ...
    case ZERO_EXTRACT:
    case SIGN_EXTRACT:
      scan_rtx(insn, &XEXP(x, 0), cl, action, type == OP_IN ? OP_IN : OP_INOUT,
               earlyclobber);
...
static struct du_chain *build_def_use(basic_block bb) {
  rtx insn;
  ...
  for (insn = BB_HEAD(bb);; insn = NEXT_INSN(insn)) {
    if (INSN_P(insn)) {
      ...
      for (i = 0; i < n_ops; i++)
        scan_rtx(insn, recog_data.operand_loc[i], NO_REGS,
                 terminate_overlapping_read, recog_data.operand_type[i], 0);
      ...
      scan_rtx(insn, &PATTERN(insn), NO_REGS, terminate_all_read, OP_IN, 0);
      ...
      if (CALL_P(insn) && CALL_INSN_FUNCTION_USAGE(insn))
        scan_rtx(insn, &CALL_INSN_FUNCTION_USAGE(insn), NO_REGS,
                 terminate_all_read, OP_IN, 0);
      ...
      for (i = 0; i < n_ops + recog_data.n_dups; i++) {
        ...
        rtx *loc = (i < n_ops ? recog_data.operand_loc[opn]
      ...
      for (note = REG_NOTES(insn); note; note = XEXP(note, 1))
        if (REG_NOTE_KIND(note) == REG_INC ||
          ...
          scan_rtx(insn, &XEXP(note, 0), ALL_REGS, mark_read, OP_INOUT, 0);

      /* Step 4: Close chains for registers that die here.  */
      for (note = REG_NOTES(insn); note; note = XEXP(note, 1))
        if (REG_NOTE_KIND(note) == REG_DEAD)
          scan_rtx(insn, &XEXP(note, 0), NO_REGS, terminate_dead, OP_IN, 0);
      ...
      for (note = REG_NOTES(insn); note; note = XEXP(note, 1))
        if (REG_NOTE_KIND(note) == REG_FRAME_RELATED_EXPR)
          scan_rtx(insn, &XEXP(note, 0), ALL_REGS, mark_access, OP_INOUT, 0);
...
static void dump_def_use_chain(struct du_chain *chains) {
  while (chains) {
...
static bool replace_oldest_value_reg(rtx *, enum reg_class, rtx,
                                     struct value_data *);
static bool replace_oldest_value_addr(rtx *, enum reg_class, enum machine_mode,
...
static void kill_value_regno(unsigned int regno, unsigned int nregs,
...
static void kill_value(rtx x, struct value_data *vd) {
  rtx orig_rtx = x;

  if (GET_CODE(x) == SUBREG) {
    x = simplify_subreg(GET_MODE(x), SUBREG_REG(x), GET_MODE(SUBREG_REG(x)),
                        SUBREG_BYTE(x));
    if (x == NULL_RTX)
...
static void set_value_regno(unsigned int regno, enum machine_mode mode,
                            struct value_data *vd) {
  unsigned int nregs;
...
static void init_value_data(struct value_data *vd) {
  int i;
...
static void kill_clobbered_value(rtx x, const_rtx set, void *data) {
...
static void kill_set_value(rtx x, const_rtx set, void *data) {
  struct value_data *const vd = (struct value_data *)data;
  if (GET_CODE(set) != CLOBBER) {
    kill_value(x, vd);
    if (REG_P(x))
...
static int kill_autoinc_value(rtx *px, void *data) {
  rtx x = *px;
  struct value_data *const vd = (struct value_data *)data;
...
static void copy_value(rtx dest, rtx src, struct value_data *vd) {
  ...
  /* If we are narrowing the input to a smaller number of hard regs,
     ...
     record anything then.  */
           ...
           (GET_MODE_SIZE(vd->e[sr].mode) > UNITS_PER_WORD ? WORDS_BIG_ENDIAN
                                                           : BYTES_BIG_ENDIAN))
    return;
...
static rtx find_oldest_value_reg(enum reg_class cl, rtx reg,
                                 struct value_data *vd) {
...
static bool replace_oldest_value_reg(rtx *loc, enum reg_class cl, rtx insn,
                                     struct value_data *vd) {
  rtx new_rtx = find_oldest_value_reg(cl, *loc, vd);
  if (new_rtx) {
    if (dump_file)
      fprintf(dump_file, "insn %u: replaced reg %u with %u\n", INSN_UID(insn),
              REGNO(*loc), REGNO(new_rtx));
...
static bool replace_oldest_value_addr(rtx *loc, enum reg_class cl,
  ...
  switch (code) {
    case PLUS: {
      ...
      if (locI)
        changed |=
            replace_oldest_value_addr(locI, INDEX_REG_CLASS, mode, insn, vd);
      if (locB)
        changed |= replace_oldest_value_addr(
            locB, base_reg_class(mode, PLUS, index_code), mode, insn, vd);
      return changed;
    }
  ...
  for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--) {
    if (fmt[i] == 'e')
      changed |= replace_oldest_value_addr(&XEXP(x, i), cl, mode, insn, vd);
    else if (fmt[i] == 'E')
      for (j = XVECLEN(x, i) - 1; j >= 0; j--)
        changed |=
            replace_oldest_value_addr(&XVECEXP(x, i, j), cl, mode, insn, vd);
  }
...
static bool replace_oldest_value_mem(rtx x, rtx insn, struct value_data *vd) {
  return replace_oldest_value_addr(&XEXP(x, 0),
                                   base_reg_class(GET_MODE(x), MEM, SCRATCH),
                                   GET_MODE(x), insn, vd);
}

/* Perform the forward copy propagation on basic block BB.  */

static bool copyprop_hardreg_forward_1(basic_block bb, struct value_data *vd) {
  bool changed = false;
  rtx insn;
  ...
  for (insn = BB_HEAD(bb);; insn = NEXT_INSN(insn)) {
    ...
    if (set && REG_P(SET_SRC(set))) {
      ...
      for (i = vd->e[regno].oldest_regno; i != regno; i = vd->e[i].next_regno) {
        new_rtx =
            maybe_mode_change(vd->e[i].mode, vd->e[regno].mode, mode, i, regno);
        if (new_rtx != NULL_RTX) {
  ...
  no_move_special_case:
    ...
    for (i = 0; i < n_ops; i++) {
      ...
      if (recog_data.operand_type[i] == OP_IN) {
        if (recog_op_alt[i][alt].is_address)
          replaced[i] = replace_oldest_value_addr(recog_data.operand_loc[i],
                                                  recog_op_alt[i][alt].cl,
                                                  VOIDmode, insn, vd);
        else if (REG_P(recog_data.operand[i]))
          replaced[i] = replace_oldest_value_reg(
              recog_data.operand_loc[i], recog_op_alt[i][alt].cl, insn, vd);
        else if (MEM_P(recog_data.operand[i]))
          replaced[i] =
              replace_oldest_value_mem(recog_data.operand[i], insn, vd);
      } else if (MEM_P(recog_data.operand[i]))
        replaced[i] = replace_oldest_value_mem(recog_data.operand[i], insn, vd);
    ...
    if (any_replacements) {
      if (!apply_change_group()) {
        ...
        if (dump_file)
          fprintf(dump_file, "insn %u: reg replacements not verified\n",
                  INSN_UID(insn));
      } else
        changed = true;
    }
...
static void copyprop_hardreg_forward(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    if (single_pred_p(bb) && TEST_BIT(visited, single_pred(bb)->index) &&
...
void debug_value_data(struct value_data *vd) {
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; ++i)
    if (vd->e[i].oldest_regno == i) {
      ...
      for (j = vd->e[i].next_regno; j != INVALID_REGNUM;
           j = vd->e[j].next_regno) {
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; ++i)
    if (!TEST_HARD_REG_BIT(set, i) &&
        (vd->e[i].mode != VOIDmode || vd->e[i].oldest_regno != i ||
         vd->e[i].next_regno != INVALID_REGNUM))
...
#ifdef ENABLE_CHECKING
static void validate_value_data(struct value_data *vd) {
  HARD_REG_SET set;
  unsigned int i, j;
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; ++i)
    if (vd->e[i].oldest_regno == i) {
      ...
      for (j = vd->e[i].next_regno; j != INVALID_REGNUM;
           j = vd->e[j].next_regno) {
        if (TEST_HARD_REG_BIT(set, j))
          internal_error("validate_value_data: Loop in regno chain (%u)", j);
        if (vd->e[j].oldest_regno != i)
          internal_error("validate_value_data: [%u] Bad oldest_regno (%u)", j,
                         vd->e[j].oldest_regno);
...
static bool gate_handle_regrename(void) {
  return (optimize > 0 && (flag_rename_registers));
}

/* Run the regrename and cprop passes.  */
static unsigned int rest_of_handle_regrename(void) {
  regrename_optimize();
  return 0;
}
...
static bool gate_handle_cprop(void) {
  return (optimize > 0 && (flag_cprop_registers));
}

/* Run the regrename and cprop passes.  */
static unsigned int rest_of_handle_cprop(void) {
  copyprop_hardreg_forward();
  return 0;
}

struct rtl_opt_pass pass_cprop_hardreg = {{
    RTL_PASS, "cprop_hardreg",               /* name */
