...
static rtx see_get_extension_reg(rtx extension, bool return_dest_reg) {
  rtx set, rhs, lhs;
  rtx reg1 = NULL;
  rtx reg2 = NULL;
...
static enum entry_type see_get_extension_data(rtx extension,
                                              enum machine_mode *source_mode) {
  rtx rhs, lhs, set;
  ...
  if (!REG_P(XEXP(rhs, 0)) &&
      !(GET_CODE(XEXP(rhs, 0)) == SUBREG && REG_P(SUBREG_REG(XEXP(rhs, 0)))))
    return NOT_RELEVANT;
...
static rtx see_gen_normalized_extension(rtx reg, enum entry_type extension_code,
...
static int eq_descriptor_pre_extension(const void *p1, const void *p2) {
...
static hashval_t hash_descriptor_pre_extension(const void *p) {
...
static void hash_del_pre_extension(void *p) {
...
static int eq_descriptor_properties(const void *p1, const void *p2) {
...
static hashval_t hash_descriptor_properties(const void *p) {
...
static void hash_del_properties(void *p) {
...
static int eq_descriptor_extension(const void *p1, const void *p2) {
...
static hashval_t hash_descriptor_extension(const void *p) {
...
static void see_free_ref_s(splay_tree_value value) {
...
static struct see_pre_extension_expr *see_seek_pre_extension_expr(
    rtx extension, enum extension_type type) {
  ...
  if (type == DEF_EXTENSION) {
    extension_code = see_get_extension_data(extension, &source_extension_mode);
    gcc_assert(extension_code != NOT_RELEVANT);
    extension = see_gen_normalized_extension(dest_extension_reg, extension_code,
                                             source_extension_mode);
  }
  temp_pre_exp.se_insn = extension;
  slot_pre_exp = (struct see_pre_extension_expr **)htab_find_slot(
      see_pre_extension_hash, &temp_pre_exp, INSERT);
     ...
     it in the hash.  */
  {
    ...
    (*slot_pre_exp)->bitmap_index = (htab_elements(see_pre_extension_hash) - 1);
...
static bool see_update_leader_extra_info(struct web_entry *first,
                                         struct web_entry *second) {
...
static void see_free_data_structures(void) {
  int i;
  unsigned int j;
...
static void see_initialize_data_structures(void) {
  ...
  see_pre_extension_hash =
      htab_create(10, hash_descriptor_pre_extension,
                  eq_descriptor_pre_extension, hash_del_pre_extension);
}
...
static void see_mentioned_reg(rtx *x, void *data) {
  struct see_mentioned_reg_data *d = (struct see_mentioned_reg_data *)data;
...
static bool see_want_to_be_merged_with_extension(rtx ref, rtx extension,
  ...
  if (code == PARALLEL) {
    for (i = 0; i < XVECLEN(pat, 0); i++) {
      ...
      if (GET_CODE(sub) == SET &&
          (REG_P(SET_DEST(sub)) || (GET_CODE(SET_DEST(sub)) == SUBREG &&
                                    REG_P(SUBREG_REG(SET_DEST(sub))))) &&
          (REG_P(SET_SRC(sub)) || (GET_CODE(SET_SRC(sub)) == SUBREG &&
    ...
    if (code == SET &&
        (REG_P(SET_DEST(pat)) || (GET_CODE(SET_DEST(pat)) == SUBREG &&
                                  REG_P(SUBREG_REG(SET_DEST(pat))))) &&
        (REG_P(SET_SRC(pat)) ||
         (GET_CODE(SET_SRC(pat)) == SUBREG && REG_P(SUBREG_REG(SET_SRC(pat))))))
      /* This is a simple move SET.  */
      return false;
  }
...
static int see_print_register_properties(void **slot,
...
static int see_print_pre_extension_expr(void **slot, void *b ATTRIBUTE_UNUSED) {
  const struct see_pre_extension_expr *const pre_extension =
      (const struct see_pre_extension_expr *)*slot;

  gcc_assert(pre_extension && pre_extension->se_insn &&
...
static int see_delete_merged_def_extension(void **slot,
                                           void *b ATTRIBUTE_UNUSED) {
  rtx def_se = (rtx)*slot;
...
static int see_delete_unmerged_def_extension(void **slot,
                                             void *b ATTRIBUTE_UNUSED) {
  rtx def_se = (rtx)*slot;
...
static int see_emit_use_extension(void **slot, void *b) {
  rtx use_se = (rtx)*slot;
...
static int see_commit_ref_changes(splay_tree_node stn,
...
static bool see_pre_insert_extensions(
  ...
  for (e = 0; e < num_edges; e++) {
    ...
    for (i = indx = 0; i < set_size; i++, indx += SBITMAP_ELT_BITS) {
      ...
      for (j = indx; insert && j < (int)pre_extension_num; j++, insert >>= 1)
        if (insert & 1) {
          ...
          if (eg->flags & EDGE_ABNORMAL) {
            ...
            if (dump_file) {
              fprintf(dump_file, "PRE: end of bb %d, insn %d, ", bb->index,
                      INSN_UID(new_insn));
            ...
            if (dump_file) {
              fprintf(dump_file, "PRE: edge (%d,%d), ", bb->index,
...
static int see_pre_delete_extension(void **slot, void *b ATTRIBUTE_UNUSED) {
...
static int see_map_extension(void **slot, void *b) {
  struct see_pre_extension_expr *const expr =
      (struct see_pre_extension_expr *)*slot;
...
static void see_commit_changes(void) {
  struct see_pre_extension_expr **index_map;
  size_t pre_extension_num = htab_elements(see_pre_extension_hash);
  ...
  if (dump_file)
    fprintf(dump_file, "* Phase 4: Commit changes to the insn stream.  *\n");
  ...
  for (i = 0; i < last_bb; i++) {
    if (see_bb_splay_ar[i]) {
      /* Traverse over all the references in the basic block in forward
         order.  */
      splay_tree_foreach(see_bb_splay_ar[i], see_commit_ref_changes, NULL);
    }
  }
...
static int see_analyze_merged_def_local_prop(void **slot, void *b) {
  rtx def_se = (rtx)*slot;
  struct see_ref_s *curr_ref_s = (struct see_ref_s *)b;
  ...
  slot_prop = (struct see_register_properties **)htab_find_slot(
      curr_bb_hash, &temp_prop, INSERT);
  curr_prop = *slot_prop;
...
static int see_analyze_unmerged_def_local_prop(void **slot, void *b) {
  rtx def_se = (rtx)*slot;
  struct see_ref_s *curr_ref_s = (struct see_ref_s *)b;
  rtx ref = curr_ref_s->insn;
  struct see_pre_extension_expr *extension_expr;
...
static int see_analyze_use_local_prop(void **slot, void *b) {
  struct see_ref_s *curr_ref_s = (struct see_ref_s *)b;
  rtx use_se = (rtx)*slot;
  rtx ref = curr_ref_s->insn;
...
static int see_analyze_ref_local_prop(splay_tree_node stn,
                                      void *data ATTRIBUTE_UNUSED) {
  htab_t use_se_hash = ((struct see_ref_s *)(stn->value))->use_se_hash;
  htab_t unmerged_def_se_hash =
      ((struct see_ref_s *)(stn->value))->unmerged_def_se_hash;
...
static void see_execute_LCM(void) {
  size_t pre_extension_num = htab_elements(see_pre_extension_hash);
  int i = 0;
  ...
  for (i = 0; i < last_bb; i++) {
    if (see_bb_splay_ar[i]) {
      /* Traverse over all the references in the basic block in forward
         order.  */
      splay_tree_foreach(see_bb_splay_ar[i], see_analyze_ref_local_prop, NULL);
    }
  }
  ...
  /* Run the LCM.  */
  edge_list = pre_edge_lcm(pre_extension_num, transp, comp, antloc, ae_kill,
...
static int see_set_prop_merged_def(void **slot, void *b) {
  rtx def_se = (rtx)*slot;
  struct see_ref_s *curr_ref_s = (struct see_ref_s *)b;
  ...
  if (!curr_bb_hash) {
    ...
    curr_bb_hash = htab_create(10, hash_descriptor_properties,
                               eq_descriptor_properties, hash_del_properties);
...
static int see_set_prop_unmerged_def(void **slot, void *b) {
  rtx def_se = (rtx)*slot;
  struct see_ref_s *curr_ref_s = (struct see_ref_s *)b;
  rtx insn = curr_ref_s->insn;
  rtx dest_extension_reg = see_get_extension_reg(def_se, 1);
...
static int see_set_prop_unmerged_use(void **slot, void *b) {
  rtx use_se = (rtx)*slot;
  struct see_ref_s *curr_ref_s = (struct see_ref_s *)b;
  rtx insn = curr_ref_s->insn;
  rtx dest_extension_reg = see_get_extension_reg(use_se, 1);
...
static int see_print_one_extension(void **slot, void *b ATTRIBUTE_UNUSED) {
  rtx def_se = (rtx)*slot;
...
static void see_replace_src(rtx *x, void *data) {
  struct see_replace_data *d = (struct see_replace_data *)data;

  *x = replace_rtx(*x, d->from, d->to);
}

static rtx see_copy_insn(rtx insn) {
  ...
  if (NONJUMP_INSN_P(insn))
    ...
    if (CALL_INSN_FUNCTION_USAGE(insn))
      CALL_INSN_FUNCTION_USAGE(ret) = copy_rtx(CALL_INSN_FUNCTION_USAGE(insn));
...
static void see_def_extension_not_merged(struct see_ref_s *curr_ref_s,
                                         rtx def_se) {
  struct see_replace_data d;
  /* If the original insn was already merged with an extension before,
     take the merged one.  */
  rtx ref =
      curr_ref_s->merged_insn ? curr_ref_s->merged_insn : curr_ref_s->insn;
  rtx merged_ref_next =
      curr_ref_s->merged_insn ? NEXT_INSN(curr_ref_s->merged_insn) : NULL_RTX;
  ...
  gcc_assert(set);
  rhs = SET_SRC(set);
  gcc_assert(GET_CODE(rhs) == SIGN_EXTEND || GET_CODE(rhs) == ZERO_EXTEND);
  dest_reg = XEXP(rhs, 0);
  gcc_assert(REG_P(dest_reg) ||
             (GET_CODE(dest_reg) == SUBREG && REG_P(SUBREG_REG(dest_reg))));
...
static int see_merge_one_use_extension(void **slot, void *b) {
  struct see_ref_s *curr_ref_s = (struct see_ref_s *)b;
  rtx use_se = (rtx)*slot;
  rtx ref =
      curr_ref_s->merged_insn ? curr_ref_s->merged_insn : curr_ref_s->insn;
  ...
  if (note) {
    simplified_note =
        simplify_replace_rtx(XEXP(note, 0), dest_extension_reg, extension_rhs);
    if (rtx_equal_p(XEXP(note, 0), simplified_note))
      /* Replacement failed.  Remove the note.  */
      remove_note(ref_copy, note);
    else
      set_unique_reg_note(ref_copy, REG_NOTE_KIND(note), simplified_note);
  }
...
static int see_merge_one_def_extension(void **slot, void *b) {
  struct see_ref_s *curr_ref_s = (struct see_ref_s *)b;
  rtx def_se = (rtx)*slot;
  /* If the original insn was already merged with an extension before,
     take the merged one.  */
  ...
  gcc_assert(def_se && INSN_P(def_se) && curr_ref_s && ref && INSN_P(ref));
  ...
  if (code == PARALLEL) {
    ...
    for (i = 0; i < XVECLEN(*pat, 0); i++) {
      ...
      if (GET_CODE(*sub) == SET && GET_MODE(SET_SRC(*sub)) != VOIDmode &&
    ...
    if (need_to_apply_change)
      if (apply_change_group())
        merge_success = true;
  } else if (code == SET && GET_MODE(SET_SRC(*pat)) != VOIDmode &&
  ...
  if (!curr_ref_s->merged_def_se_hash)
    curr_ref_s->merged_def_se_hash = htab_create(10, hash_descriptor_extension,
...
static int see_handle_extensions_for_one_ref(splay_tree_node stn,
                                             void *data ATTRIBUTE_UNUSED) {
  htab_t use_se_hash = ((struct see_ref_s *)(stn->value))->use_se_hash;
  htab_t unmerged_def_se_hash =
      ((struct see_ref_s *)(stn->value))->unmerged_def_se_hash;
...
static void see_merge_and_eliminate_extensions(void) {
  ...
  for (i = 0; i < last_bb; i++) {
    if (see_bb_splay_ar[i]) {
      ...
      /* Traverse over all the references in the basic block in forward
         order.  */
      splay_tree_foreach(see_bb_splay_ar[i], see_handle_extensions_for_one_ref,
                         NULL);
    }
  }
}
...
static bool see_store_reference_and_extension(rtx ref_insn, rtx se_insn,
                                              enum extension_type type) {
  rtx *rtx_slot;
  int curr_bb_num;
  ...
  if (!see_bb_splay_ar[curr_bb_num])
    ...
    see_bb_splay_ar[curr_bb_num] =
        splay_tree_new(splay_tree_compare_ints, NULL, see_free_ref_s);
  else
  /* Splay tree already exists, check if the current reference is already
     in it.  */
  {
    stn =
        splay_tree_lookup(see_bb_splay_ar[curr_bb_num], DF_INSN_LUID(ref_insn));
    if (stn)
      switch (type) {
        case EXPLICIT_DEF_EXTENSION:
          ...
          if (!se_hash) {
            se_hash = htab_create(10, hash_descriptor_extension,
                                  eq_descriptor_extension, NULL);
            ((struct see_ref_s *)(stn->value))->unmerged_def_se_hash = se_hash;
          }
          break;
        case IMPLICIT_DEF_EXTENSION:
          ...
          if (!se_hash) {
            se_hash = htab_create(10, hash_descriptor_extension,
                                  eq_descriptor_extension, NULL);
            ((struct see_ref_s *)(stn->value))->merged_def_se_hash = se_hash;
          }
          break;
        case USE_EXTENSION:
  ...
  if (!stn) {
    ...
    switch (type) {
      case EXPLICIT_DEF_EXTENSION:
        ref_s->unmerged_def_se_hash = htab_create(
            10, hash_descriptor_extension, eq_descriptor_extension, NULL);
        se_hash = ref_s->unmerged_def_se_hash;
        ref_s->merged_def_se_hash = NULL;
        ref_s->use_se_hash = NULL;
        break;
      case IMPLICIT_DEF_EXTENSION:
        ref_s->merged_def_se_hash = htab_create(10, hash_descriptor_extension,
                                                eq_descriptor_extension, NULL);
        ...
        break;
      case USE_EXTENSION:
        ref_s->use_se_hash = htab_create(10, hash_descriptor_extension,
                                         eq_descriptor_extension, NULL);
  ...
  if (!stn)
    splay_tree_insert(see_bb_splay_ar[curr_bb_num], DF_INSN_LUID(ref_insn),
                      (splay_tree_value)ref_s);
  return true;
}
...
static int see_handle_relevant_defs(df_ref ref, rtx insn) {
  ...
  if (ENTRY_EI(&def_entry[i])->local_relevancy == EXTENDED_DEF ||
    ...
    se_insn = see_gen_normalized_extension(reg, extension_code,
                                           ENTRY_EI(root_entry)->source_mode);
...
static int see_handle_relevant_uses(df_ref ref, rtx insn) {
  struct web_entry *root_entry = NULL;
  rtx se_insn = NULL;
  enum entry_type extension_code;
  rtx reg = DF_REF_REAL_REG(ref);
  ...
  if (!see_store_reference_and_extension(insn, se_insn, USE_EXTENSION))
    /* Something bad happened.  Abort the optimization.  */
    return -1;
  return 1;
}

static int see_handle_relevant_refs(void) {
...
static void see_update_uses_relevancy(rtx insn, df_ref ref, enum entry_type et,
                                      unsigned int index) {
  struct see_entry_extra_info *curr_entry_extra_info;

  if (dump_file) {
    rtx reg = DF_REF_REAL_REG(ref);
    fprintf(dump_file, "u%i insn %i reg %i ", index,
            (insn ? INSN_UID(insn) : -1), REGNO(reg));
    if (et == NOT_RELEVANT)
...
static enum entry_type see_analyze_one_def(
    rtx insn, enum machine_mode *source_mode,
    enum machine_mode *source_mode_unsigned) {
  enum entry_type extension_code;
  rtx rhs = NULL;
  rtx lhs = NULL;
...
static void see_update_defs_relevancy(rtx insn, df_ref ref, enum entry_type et,
                                      enum machine_mode source_mode,
                                      enum machine_mode source_mode_unsigned,
                                      unsigned int index) {
  struct see_entry_extra_info *curr_entry_extra_info =
  ...
  if (dump_file) {
    rtx reg = DF_REF_REAL_REG(ref);
    if (et == NOT_RELEVANT) {
      fprintf(dump_file, "d%i insn %i reg %i ", index,
              (insn ? INSN_UID(insn) : -1), REGNO(reg));
      fprintf(dump_file, "NOT RELEVANT \n");
    } else {
      fprintf(dump_file, "d%i insn %i reg %i ", index, INSN_UID(insn),
              REGNO(reg));
...
static void see_update_relevancy(void) {
  unsigned int d = 0;
  unsigned int u = 0;
  enum entry_type et;
  enum machine_mode source_mode;
  ...
  FOR_ALL_BB(bb) {
    ...
    for (def_rec = df_get_artificial_defs(bb->index); *def_rec; def_rec++) {
      df_ref def = *def_rec;
      see_update_defs_relevancy(NULL, def, NOT_RELEVANT, MAX_MACHINE_MODE,
                                MAX_MACHINE_MODE, d);
      d++;
    }
  }
...
static bool see_propagate_extensions_to_uses(void) {
  int num_relevant_refs;
  basic_block bb;

  if (dump_file)
    fprintf(dump_file, "* Phase 1: Propagate extensions to uses.  *\n");
...
static void see_main(void) {
  ...
  if (cont) {
    ...
    if (dump_file) {
      ...
      htab_traverse(see_pre_extension_hash, see_print_pre_extension_expr, NULL);
      ...
      for (i = 0; i < last_bb; i++) {
        ...
        /* Traverse over all the references in the basic block in
           forward order.  */
        {
          fprintf(dump_file, "Searching register properties in bb %d\n", i);
          htab_traverse(see_bb_hash_ar[i], see_print_register_properties, NULL);
        }
      }
    }
  }
...
static bool gate_handle_see(void) {
  return optimize > 1 && flag_see;
}

static unsigned int rest_of_handle_see(void) {
  see_main();
