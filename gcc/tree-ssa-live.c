...
static void var_map_base_init(var_map map) {
  int x, num_part, num;
  tree var;
  var_ann_t ann;
...
var_map init_var_map(int size) {
  ...
  map->partition_to_var = (tree *)xmalloc(size * sizeof(tree));
...
extern void partition_view_normal(var_map map, bool want_bases) {
  bitmap used;
...
extern void partition_view_bitmap(var_map map, bitmap only, bool want_bases) {
  bitmap used;
...
static tree mark_all_vars_used_1(tree *tp, int *walk_subtrees, void *data) {
  ...
  if (IS_EXPR_CODE_CLASS(c) && (b = TREE_BLOCK(t)) != NULL)
    TREE_USED(b) = true;
...
/* Look if the block is dead (by possibly eliminating its dead subblocks)
   ...
   out of scope blocks to let them to be recycled by GGC and to save copying
   work done by the inliner.  */
...
static bool remove_unused_scope_block_p(tree scope) {
  ...
  for (t = &BLOCK_VARS(scope); *t; t = next) {
    ...
    else if ((ann = var_ann(*t)) != NULL && ann->used)
      unused = false;
...
static inline void mark_all_vars_used(tree *expr_p, void *data) {
  walk_tree(expr_p, mark_all_vars_used_1, data, NULL);
}

/* Dump scope blocks.  */

static void dump_scope_block(FILE *file, int indent, tree scope, int flags) {
  tree var, t;
  unsigned int i;
  ...
  for (i = 0; i < BLOCK_NUM_NONLOCALIZED_VARS(scope); i++) {
    fprintf(file, "%*s", indent, "");
    print_generic_decl(file, BLOCK_NONLOCALIZED_VAR(scope, i), flags);
...
void dump_scope_blocks(FILE *file, int flags) {
...
void remove_unused_locals(void) {
  ...
  /* Remove unused variables from REFERENCED_VARs.  As a special
     ...
     pass is performed.  */
  FOR_EACH_REFERENCED_VAR(t, rvi)
  if (!is_global_var(t) && !MTAG_P(t) && TREE_CODE(t) != PARM_DECL &&
      TREE_CODE(t) != RESULT_DECL && !(ann = var_ann(t))->used &&
      !ann->symbol_mem_tag && !TREE_ADDRESSABLE(t) &&
...
static void loe_visit_block(tree_live_info_p live, basic_block bb,
                            sbitmap visited, bitmap tmp) {
  edge e;
  bool change;
  edge_iterator ei;
  basic_block pred_bb;
  ...
  FOR_EACH_EDGE(e, ei, bb->succs) {
    ...
    for (i = 0; i < (unsigned)num_var_partitions(map); i++) {
      ...
      if (loe && bitmap_bit_p(loe, i)) {
        ...
        bool ok = false;
        gimple_stmt_iterator gsi;
        for (gsi = gsi_start_phis(e->dest); !gsi_end_p(gsi) && !ok;
             gsi_next(&gsi)) {
          gimple phi = gsi_stmt(gsi);
