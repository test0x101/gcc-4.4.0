...
static const char *const files_to_ignore[] = {"X11/", FIXPROTO_IGNORE_LIST 0};
#endif
...
static void add_symbols(symbol_flags flags, namelist names) {
...
tmpnam\0ungetc\0"},
    ...
    /* "div ldiv", - ignored because these depend on div_t, ldiv_t
       ...
       Left out getgroups, because SunOS4 has incompatible BSD and SVR4
       versions. Should perhaps also add NULL */
...
WTERMSIG\0WNOHANG\0WNOTRACED\0"},
    ...
    {"termios.h", POSIX1_SYMBOL,
     "cfgetispeed\0cfgetospeed\0cfsetispeed\0cfsetospeed\0tcdrain\0tcflow\0tcfl"
     "ush\0tcgetattr\0tcsendbreak\0tcsetattr\0"},

    {"time.h", ANSI_SYMBOL,
     "asctime\0clock\0ctime\0difftime\0gmtime\0localtime\0mktime\0strftime\0tim"
     "e\0"},
...
static struct fn_decl *lookup_std_proto(const char *name, int name_length) {
...
static void recognized_macro(const char *fname) {
...
void recognized_function(const cpp_token *fname, unsigned int line, int kind,
                         int have_arg_list) {
  ...
  partial_count++;
  partial = (struct partial_proto *)obstack_alloc(&scan_file_obstack,
...
static void check_macro_names(cpp_reader *pfile, namelist names) {
  size_t len;
  while (*names) {
    len = strlen(names);
...
static void read_scan_file(char *in_fname, int argc, char **argv) {
  cpp_reader *scan_in;
  cpp_callbacks *cb;
  cpp_options *options;
  struct fn_decl *fn;
  ...
  if (special_file_handling == stdio_h &&
    ...
    if (seen_filbuf) {
      ...
      int need_flsbuf = flsbuf_fn && !SEEN(flsbuf_fn) && !REQUIRED(flsbuf_fn);
...
static void write_rbrac(void) {
  ...
  for (cur_symbols = &symbol_table[0]; cur_symbols->names; cur_symbols++) {
   ...
   || (defined(__STRICT_ANSI__) && defined(_XOPEN_EXTENDED_SOURCE))\n");
      ...
      if (!strcmp(fn->fname, "memmove") || !strcmp(fn->fname, "putc") ||
          !strcmp(fn->fname, "getc") || !strcmp(fn->fname, "vprintf") ||
          !strcmp(fn->fname, "vfprintf") || !strcmp(fn->fname, "vsprintf") ||
          !strcmp(fn->fname, "rewinddir") || !strcmp(fn->fname, "abort"))
        macro_protect = 1;
    ...
    if (if_was_emitted)
      fprintf(outf, "#endif /* defined(__USE_FIXED_PROTOTYPES__) || ... */\n");
  }
  if (required_unseen_count) {
#ifdef NO_IMPLICIT_EXTERN_C
    fprintf(outf, "#ifdef __cplusplus\n}\n#endif\n");
  ...
  switch (special_file_handling) {
    ...
    case sys_stat_h:
      if (!seen_S_ISBLK && seen_S_IFBLK)
        fprintf(outf, "#define S_ISBLK(mode) (((mode) & S_IFMT) == S_IFBLK)\n");
      if (!seen_S_ISCHR && seen_S_IFCHR)
        fprintf(outf, "#define S_ISCHR(mode) (((mode) & S_IFMT) == S_IFCHR)\n");
      if (!seen_S_ISDIR && seen_S_IFDIR)
        fprintf(outf, "#define S_ISDIR(mode) (((mode) & S_IFMT) == S_IFDIR)\n");
      if (!seen_S_ISFIFO && seen_S_IFIFO)
        fprintf(outf,
                "#define S_ISFIFO(mode) (((mode) & S_IFMT) == S_IFIFO)\n");
      if (!seen_S_ISLNK && seen_S_IFLNK)
        fprintf(outf, "#define S_ISLNK(mode) (((mode) & S_IFMT) == S_IFLNK)\n");
      if (!seen_S_ISREG && seen_S_IFREG)
        fprintf(outf, "#define S_ISREG(mode) (((mode) & S_IFMT) == S_IFREG)\n");
      break;
...
static int inf_skip_spaces(int c) {
  for (;;) {
    if (c == ' ' || c == '\t')
      c = INF_GET();
    else if (c == '/') {
...
static int inf_read_upto(sstring *str, int delim) {
  int ch;
  for (;;) {
    ch = INF_GET();
    if (ch == EOF || ch == delim)
...
static int inf_scan_ident(sstring *s, int c) {
  s->ptr = s->base;
  if (ISIDST(c)) {
    for (;;) {
      SSTRING_PUT(s, c);
...
static int check_protection(int *ifndef_line, int *endif_line) {
  int c;
  ...
  *ifndef_line = lineno;
  ...
  for (;;) {
    ...
    if (SSTRING_LENGTH(&buf) == 0)
      ;
    else if (!strcmp(buf.base, "ifndef") || !strcmp(buf.base, "ifdef") ||
             !strcmp(buf.base, "if")) {
      if_nesting++;
...
int main(int argc, char **argv) {
  ...
  if (inf_fd < 0) {
    fprintf(stderr, "%s: Cannot open '%s' for reading -", progname, argv[2]);
    perror(NULL);
    exit(FATAL_EXIT_CODE);
  }
  ...
  outf = fopen(argv[3], "w");
  if (outf == NULL) {
    fprintf(stderr, "%s: Cannot open '%s' for writing -", progname, argv[3]);
    perror(NULL);
    exit(FATAL_EXIT_CODE);
  }
