...
static inline void ipa_set_param_count(struct ipa_node_params *info,
                                       int count) {
...
static inline bool ipa_is_param_modified(struct ipa_node_params *info, int i) {
...
static inline bool ipa_is_param_called(struct ipa_node_params *info, int i) {
  return info->params[i].called;
}

/* Flag this node as having callers with variable number of arguments.  */

static inline void ipa_set_called_with_variable_arg(
...
static inline bool ipa_is_called_with_var_arguments(
    struct ipa_node_params *info) {
...
static inline void ipa_set_cs_argument_count(struct ipa_edge_args *args,
                                             int count) {
...
/* Return the associated parameter/argument info corresponding to the given
   node/edge.  */
#define IPA_NODE_REF(NODE) \
  (VEC_index(ipa_node_params_t, ipa_node_params_vector, (NODE)->uid))
#define IPA_EDGE_REF(EDGE) \
  (VEC_index(ipa_edge_args_t, ipa_edge_args_vector, (EDGE)->uid))
...
static inline void ipa_check_create_node_params(void) {
  if (!ipa_node_params_vector)
    ipa_node_params_vector = VEC_alloc(ipa_node_params_t, heap, cgraph_max_uid);

  if (VEC_length(ipa_node_params_t, ipa_node_params_vector) <=
      (unsigned)cgraph_max_uid)
    VEC_safe_grow_cleared(ipa_node_params_t, heap, ipa_node_params_vector,
                          cgraph_max_uid + 1);
}
...
static inline void ipa_check_create_edge_args(void) {
  if (!ipa_edge_args_vector)
    ipa_edge_args_vector =
        VEC_alloc(ipa_edge_args_t, heap, cgraph_edge_max_uid);
...
static inline bool ipa_edge_args_info_available_for_edge_p(
  ...
  return ((unsigned)edge->uid <
          VEC_length(ipa_edge_args_t, ipa_edge_args_vector));
}
