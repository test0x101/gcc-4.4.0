...
#define DEF_MODE_CLASS(M) #M
static const char *const mode_class_names[MAX_MODE_CLASS] = {MODE_CLASSES};
#undef DEF_MODE_CLASS
#undef MODE_CLASSES
...
/* Utility routines.  */
static inline struct mode_data *find_mode(const char *name) {
...
static void ATTRIBUTE_UNUSED new_adjust(
    const char *name, struct mode_adjust **category, const char *catname,
    const char *adjustment, enum mode_class required_class_from,
    enum mode_class required_class_to, const char *file, unsigned int line) {
  ...
  if (required_class_from != MODE_RANDOM &&
    ...
    error("%s:%d: mode \"%s\" is not among class {%s, %s}", file, line, name,
          mode_class_names[required_class_from] + 5,
  ...
  for (a = *category; a; a = a->next)
    if (a->mode == mode) {
      error("%s:%d: mode \"%s\" already has a %s adjustment", file, line, name,
            catname);
...
#define validate_field_(mname, fname, req, val, unset, file, line)             \
  do {                                                                         \
    switch (req) {                                                             \
      case SET:                                                                \
        if (val == unset)                                                      \
          error("%s:%d: (%s) field %s must be set", file, line, mname, fname); \
        break;                                                                 \
      case UNSET:                                                              \
        if (val != unset)                                                      \
          error("%s:%d: (%s) field %s must not be set", file, line, mname,     \
                fname);                                                        \
      case OPTIONAL:                                                           \
        break;                                                                 \
    }                                                                          \
...
static void validate_mode(struct mode_data *m, enum requirement r_precision,
  ...
  switch (m->cl) {
    ...
    case MODE_UACCUM:
      /* A scalar mode must have a byte size, may have a bit size,
         ...
         format.  */
      validate_mode(
          m, OPTIONAL, SET, UNSET, UNSET,
          (m->cl == MODE_FLOAT || m->cl == MODE_DECIMAL_FLOAT) ? SET : UNSET);
...
static void complete_all_modes(void) {
  ...
  for_all_modes(cl, m) complete_mode(m);
}

/* For each mode in class CLASS, construct a corresponding complex mode.  */
#define COMPLEX_MODES(C) make_complex_modes(MODE_##C, __FILE__, __LINE__)
static void make_complex_modes(enum mode_class cl, const char *file,
                               unsigned int line) {
  struct mode_data *m;
  struct mode_data *c;
  char buf[8];
  ...
  for (m = modes[cl]; m; m = m->next) {
    ...
    if (strlen(m->name) >= sizeof buf) {
      error("%s:%d:mode name \"%s\" is too long", m->file, m->line, m->name);
      continue;
    }
    ...
    if (cl == MODE_FLOAT) {
      ...
      if (p == 0)
        q = strchr(buf, 'D');
      if (p == 0 && q == 0) {
        error("%s:%d: float mode \"%s\" has no 'F' or 'D'", m->file, m->line,
              m->name);
        continue;
      }
...
static void make_special_mode(enum mode_class cl, const char *name,
                              const char *file, unsigned int line) {
...
static void make_fixed_point_mode(enum mode_class cl, const char *name,
                                  unsigned int bytesize, unsigned int ibit,
                                  unsigned int fbit, const char *file,
                                  unsigned int line) {
...
static void make_float_mode(const char *name, unsigned int precision,
                            unsigned int bytesize, const char *format,
                            const char *file, unsigned int line) {
...
static void make_decimal_float_mode(const char *name, unsigned int precision,
                                    unsigned int bytesize, const char *format,
                                    const char *file, unsigned int line) {
...
static void ATTRIBUTE_UNUSED make_vector_mode(enum mode_class bclass,
  ...
  if (component->cl != bclass &&
    ...
    error("%s:%d: mode \"%s\" is not class %s", file, line, base,
          mode_class_names[bclass] + 5);
    return;
  }

  if ((size_t)snprintf(namebuf, sizeof namebuf, "V%u%s", ncomponents, base) >=
      sizeof namebuf) {
    error("%s:%d: mode name \"%s\" is too long", file, line, base);
    return;
  }
...
static void calc_wider_mode(void) {
  ...
  sortbuf = (struct mode_data **)alloca((max_n_modes + 1) *
...
enum machine_mode\n{");
  ...
  for (c = 0; c < MAX_MODE_CLASS; c++)
    for (m = modes[c]; m; m = m->next) {
      ...
      printf("%*s/* %s:%d */\n", 27 - count_, "", trim_filename(m->file),
             m->line);
    }
  ...
  for (c = 0; c < MAX_MODE_CLASS; c++) {
    ...
    if (first && last)
      printf("  MIN_%s = %smode,\n  MAX_%s = %smode,\n\n", mode_class_names[c],
             first->name, mode_class_names[c], last->name);
    else
      printf("  MIN_%s = %smode,\n  MAX_%s = %smode,\n\n", mode_class_names[c],
             void_mode->name, mode_class_names[c], void_mode->name);
  }
...
static void emit_mode_class(void) {
  ...
  for_all_modes(c, m) tagged_printf("%s", mode_class_names[m->cl], m->name);
...
static void emit_mode_precision(void) {
  ...
  else tagged_printf("%u*BITS_PER_UNIT", m->bytesize, m->name);
...
static void emit_mode_size(void) {
  ...
  print_maybe_const_decl("%sunsigned char", "mode_size", "NUM_MACHINE_MODES",
                         bytesize);
...
static void emit_mode_wider(void) {
  ...
  for_all_modes(c, m) tagged_printf(
      "%smode", m->wider ? m->wider->name : void_mode->name, m->name);
  ...
  for_all_modes(c, m) {
    ...
    for (m2 = m; m2 && m2 != void_mode; m2 = m2->wider) {
    ...
    if (m2 == void_mode)
      m2 = 0;
    tagged_printf("%smode", m2 ? m2->name : void_mode->name, m->name);
  }
...
static void emit_mode_mask(void) {
  ...
  print_decl("unsigned HOST_WIDE_INT", "mode_mask_array", "NUM_MACHINE_MODES");
  puts(
      "\
#define MODE_MASK(m)                          \\\n\
  ...
  else tagged_printf("MODE_MASK (%u*BITS_PER_UNIT)", m->bytesize, m->name);
...
static void emit_mode_inner(void) {
  ...
  for_all_modes(c, m) tagged_printf(
      "%smode", m->component ? m->component->name : void_mode->name, m->name);
...
static void emit_mode_base_align(void) {
  ...
  print_maybe_const_decl("%sunsigned char", "mode_base_align",
                         "NUM_MACHINE_MODES", alignment);
...
static void emit_class_narrowest_mode(void) {
  ...
  for (c = 0; c < MAX_MODE_CLASS; c++)
    ...
    tagged_printf("MIN_%s", mode_class_names[c],
                  modes[c] ? (modes[c]->precision != 1
                                  ? modes[c]->name
                                  : (modes[c]->next ? modes[c]->next->name
                                                    : void_mode->name))
                           : void_mode->name);
...
\n  size_t s ATTRIBUTE_UNUSED;");
      ...
      switch (m->cl) {
        ...
        case MODE_VECTOR_UACCUM:
          ...
          printf("  mode_base_align[%smode] = (%d*s) & (~(%d*s)+1);\n", m->name,
                 m->ncomponents, m->ncomponents);
          break;

        default:
          internal_error(
              "mode %s is neither vector nor complex but contains %s", m->name,
              a->mode->name);
          /* NOTREACHED */
      }
    }
  ...
  for (a = adj_format; a; a = a->next)
    printf("\n  /* %s:%d */\n  REAL_MODE_FORMAT (%smode) = %s;\n", a->file,
           a->line, a->mode->name, a->adjustment);
...
static void emit_mode_ibit(void) {
  ...
  print_maybe_const_decl("%sunsigned char", "mode_ibit", "NUM_MACHINE_MODES",
                         ibit);

  for_all_modes(c, m) tagged_printf("%u", m->ibit, m->name);
...
static void emit_mode_fbit(void) {
  ...
  print_maybe_const_decl("%sunsigned char", "mode_fbit", "NUM_MACHINE_MODES",
                         fbit);

  for_all_modes(c, m) tagged_printf("%u", m->fbit, m->name);
