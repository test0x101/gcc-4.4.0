...
void rest_of_decl_compilation(tree decl, int top_level, int at_end) {
  ...
  if (TREE_STATIC(decl) || DECL_EXTERNAL(decl) ||
    ...
    if ((at_end || !DECL_DEFER_OUTPUT(decl) || DECL_INITIAL(decl)) &&
        !DECL_EXTERNAL(decl)) {
...
void finish_optimization_passes(void) {
  ...
  if (graph_dump_format != no_graph)
    for (i = TDI_end; (dfi = get_dump_file_info(i)) != NULL; ++i)
      if (dump_initialized_p(i) && (dfi->flags & TDF_GRAPH) != 0 &&
...
static bool gate_rest_of_compilation(void) {
...
static bool gate_postreload(void) {
...
static void set_pass_for_id(int id, struct opt_pass *pass) {
...
struct opt_pass *get_pass_for_id(int id) {
...
static void register_one_dump_file(struct opt_pass *pass) {
  ...
  if (pass->static_pass_number != -1)
    sprintf(num, "%d",
            ((int)pass->static_pass_number < 0 ? 1 : pass->static_pass_number));
...
static int register_dump_files_1(struct opt_pass *pass, int properties) {
  do {
    int new_properties =
...
static void register_dump_files(struct opt_pass *pass, int properties) {
...
#undef NEXT_PASS
  ...
  register_dump_files(all_ipa_passes, PROP_gimple_any | PROP_gimple_lcf |
                                          PROP_gimple_leh | PROP_cfg);
  register_dump_files(all_passes, PROP_gimple_any | PROP_gimple_lcf |
                                      PROP_gimple_leh | PROP_cfg);
}
...
static void do_per_function_toporder(void (*callback)(void *data), void *data) {
  int i;
...
static void execute_function_todo(void *data) {
  ...
  if ((flags & TODO_dump_func) && dump_file && current_function_decl) {
    if (cfun->curr_properties & PROP_trees)
      ...
      else if ((cfun->curr_properties & PROP_cfg) && (dump_flags & TDF_BLOCKS))
        print_rtl_with_bb(dump_file, get_insns());
      else
        print_rtl(dump_file, get_insns());

      if ((cfun->curr_properties & PROP_cfg) && graph_dump_format != no_graph &&
...
static void clear_last_verified(void *data ATTRIBUTE_UNUSED) {
...
#ifdef ENABLE_CHECKING
static void verify_curr_properties(void *data) {
...
static bool pass_init_dump_file(struct opt_pass *pass) {
  ...
  if (pass->static_pass_number != -1) {
    ...
    if (dump_file && current_function_decl) {
      ...
      fprintf(dump_file, "\n;; Function %s (%s)%s\n\n", dname, aname,
              cfun->function_frequency == FUNCTION_FREQUENCY_HOT ? " (hot)"
...
static void pass_fini_dump_file(struct opt_pass *pass) {
...
static void update_properties_after_pass(void *data) {
...
static void add_ipa_transform_pass(void *data) {
...
static void execute_ipa_summary_passes(struct ipa_opt_pass *ipa_pass) {
  while (ipa_pass) {
    struct opt_pass *pass = &ipa_pass->pass;

    /* Execute all of the IPA_PASSes in the list.  */
    if (ipa_pass->pass.type == IPA_PASS && (!pass->gate || pass->gate())) {
...
static bool execute_one_pass(struct opt_pass *pass) {
  ...
  if (cfun && cfun->ipa_transforms_to_apply) {
    ...
    for (i = 0; i < VEC_length(ipa_opt_pass, cfun->ipa_transforms_to_apply);
      ...
      execute_one_ipa_transform_pass(
          node, VEC_index(ipa_opt_pass, cfun->ipa_transforms_to_apply, i));
...
void execute_pass_list(struct opt_pass *pass) {
  do {
