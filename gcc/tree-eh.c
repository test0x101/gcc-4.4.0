...
static int struct_ptr_eq(const void *a, const void *b) {
  const void *const *x = (const void *const *)a;
  const void *const *y = (const void *const *)b;
  return *x == *y;
}

static hashval_t struct_ptr_hash(const void *a) {
  const void *const *x = (const void *const *)a;
  return (size_t)*x >> 4;
}
...
static void record_stmt_eh_region(struct eh_region *region, gimple t) {
  if (!region)
    return;
...
void add_stmt_to_eh_region_fn(struct function *ifun, gimple t, int num) {
  ...
  if (!get_eh_throw_stmt_table(ifun))
    set_eh_throw_stmt_table(
        ifun, htab_create_ggc(31, struct_ptr_hash, struct_ptr_eq, ggc_free));
...
void add_stmt_to_eh_region(gimple t, int num) {
  add_stmt_to_eh_region_fn(cfun, t, num);
}

/* Remove statement T in function IFUN from the EH region holding it.  */

bool remove_stmt_from_eh_region_fn(struct function *ifun, gimple t) {
  ...
  dummy.stmt = t;
  slot = htab_find_slot(get_eh_throw_stmt_table(ifun), &dummy, NO_INSERT);
  if (slot) {
...
bool remove_stmt_from_eh_region(gimple t) {
...
int lookup_stmt_eh_region_fn(struct function *ifun, gimple t) {
...
int lookup_stmt_eh_region(gimple t) {
...
int lookup_expr_eh_region(tree t) {
  /* We can get called from initialized data when -fnon-call-exceptions
     is on; prevent crash.  */
  if (!cfun)
    return -1;
...
static void record_in_finally_tree(treemple child, gimple parent) {
...
static void collect_finally_tree_1(gimple_seq seq, gimple region) {
...
static gimple_seq find_goto_replacement(struct leh_tf_state *tf,
                                        treemple stmt) {
  unsigned int i;
  void **slot;
  ...
  if (!tf->goto_queue_map) {
    ...
    for (i = 0; i < tf->goto_queue_active; i++) {
      slot = pointer_map_insert(tf->goto_queue_map, tf->goto_queue[i].stmt.g);
...
static void replace_goto_queue_cond_clause(tree *tp, struct leh_tf_state *tf,
                                           gimple_stmt_iterator *gsi) {
  tree label;
  gimple_seq new_seq;
  treemple temp;
...
static void replace_goto_queue_1(gimple stmt, struct leh_tf_state *tf,
                                 gimple_stmt_iterator *gsi) {
  gimple_seq seq;
  treemple temp;
  temp.g = NULL;
...
static void record_in_goto_queue(struct leh_tf_state *tf, treemple new_stmt,
                                 int index, bool is_label) {
  size_t active, size;
  ...
  if (active >= size) {
    ...
    tf->goto_queue = XRESIZEVEC(struct goto_queue_node, tf->goto_queue, size);
  }
...
static void record_in_goto_queue_label(struct leh_tf_state *tf, treemple stmt,
                                       tree label) {
  int index;
...
static void verify_norecord_switch_expr(struct leh_state *state,
                                        gimple switch_expr) {
  struct leh_tf_state *tf = state->tf;
  size_t i, n;
...
static void do_return_redirection(struct goto_queue_node *q, tree finlab,
                                  gimple_seq mod, tree *return_value_p) {
  tree ret_expr;
  gimple x;
...
static void do_goto_redirection(struct goto_queue_node *q, tree finlab,
                                gimple_seq mod, struct leh_tf_state *tf) {
  gimple x;
...
static gimple_seq frob_into_branch_around(gimple tp, tree lab, tree over) {
  gimple x;
...
static void honor_protect_cleanup_actions(struct leh_state *outer_state,
  ...
  x = gsi_stmt(gsi);
  if (protect_cleanup_actions && gimple_code(x) == GIMPLE_TRY &&
...
static void lower_try_finally_onedest(struct leh_state *state,
                                      struct leh_tf_state *tf) {
  struct goto_queue_node *q, *qe;
  gimple x;
  gimple_seq finally;
...
static void lower_try_finally_copy(struct leh_state *state,
                                   struct leh_tf_state *tf) {
  gimple_seq finally;
  gimple_seq new_stmt;
  gimple_seq seq;
...
static void lower_try_finally_switch(struct leh_state *state,
                                     struct leh_tf_state *tf) {
  struct goto_queue_node *q, *qe;
  tree return_val = NULL;
  ...
  for (; q < qe; ++q) {
    ...
    if (q->index < 0) {
      x = gimple_build_assign(finally_tmp,
                              build_int_cst(integer_type_node, return_index));
    ...
    if (VEC_length(tree, case_label_vec) <= case_index ||
      ...
      case_lab = build3(CASE_LABEL_EXPR, void_type_node,
                        build_int_cst(NULL_TREE, switch_id), NULL, NULL);
  ...
  /* Build the switch statement, setting last_case to be the default
     label.  */
  switch_stmt = gimple_build_switch_vec(finally_tmp, last_case, case_label_vec);
...
static bool decide_copy_try_finally(int ndests, gimple_seq finally) {
...
static gimple_seq lower_try_finally(struct leh_state *state, gimple tp) {
...
static gimple_seq lower_catch(struct leh_state *state, gimple tp) {
  ...
  for (gsi = gsi_start(gimple_try_cleanup(tp)); !gsi_end_p(gsi);) {
    ...
    gcatch = gsi_stmt(gsi);
    catch_region = gen_eh_region_catch(try_region, gimple_catch_types(gcatch));
    ...
    gsi_insert_seq_before(&gsi, gimple_catch_handler(gcatch), GSI_SAME_STMT);
    gsi_remove(&gsi, false);
  }
...
static gimple_seq lower_eh_filter(struct leh_state *state, gimple tp) {
  struct leh_state this_state;
  ...
  if (gimple_eh_filter_must_not_throw(inner))
    ...
    this_region =
        gen_eh_region_allowed(state->cur_region, gimple_eh_filter_types(inner));
  this_state = *state;
...
static gimple_seq lower_cleanup(struct leh_state *state, gimple tp) {
  struct leh_state this_state;
  struct eh_region *this_region;
...
static void lower_eh_constructs_2(struct leh_state *state,
                                  gimple_stmt_iterator *gsi) {
  gimple_seq replace;
  gimple x;
...
static unsigned int lower_eh_constructs(void) {
...
static void make_eh_edge(struct eh_region *region, void *data) {
  gimple stmt;
  tree lab;
  basic_block src, dst;
...
void make_eh_edges(gimple stmt) {
  int region_nr;
  bool is_resx;
...
static void mark_eh_edge(struct eh_region *region, void *data) {
  gimple stmt;
  tree lab;
  basic_block src, dst;
  edge e;
...
bool operation_could_trap_helper_p(enum tree_code op, bool fp_operation,
                                   bool honor_trapv, bool honor_nans,
                                   bool honor_snans, tree divisor,
                                   bool *handled) {
  *handled = true;
  switch (op) {
    case TRUNC_DIV_EXPR:
...
bool operation_could_trap_p(enum tree_code op, bool fp_operation,
                            bool honor_trapv, tree divisor) {
  bool honor_nans =
      (fp_operation && flag_trapping_math && !flag_finite_math_only);
  bool honor_snans = fp_operation && flag_signaling_nans != 0;
  bool handled;

  if (TREE_CODE_CLASS(op) != tcc_comparison &&
      TREE_CODE_CLASS(op) != tcc_unary && TREE_CODE_CLASS(op) != tcc_binary)
    return false;
...
static bool stmt_could_throw_1_p(gimple stmt) {
  ...
  ret = operation_could_trap_helper_p(code, fp_operation, honor_trapv,
                                      honor_nans, honor_snans, t, &handled);
  if (handled)
    return ret;
...
bool stmt_could_throw_p(gimple stmt) {
  ...
  if (code != GIMPLE_ASSIGN && code != GIMPLE_COND && code != GIMPLE_CALL &&
      code != GIMPLE_ASM)
    return false;
...
bool tree_could_throw_p(tree t) {
  ...
  if (TREE_CODE(t) == MODIFY_EXPR) {
    if (flag_non_call_exceptions && tree_could_trap_p(TREE_OPERAND(t, 0)))
      return true;
    t = TREE_OPERAND(t, 1);
  }
...
bool stmt_can_throw_internal(gimple stmt) {
  int region_nr;
  bool is_resx = false;
...
static bool same_handler_p(gimple_seq oneh, gimple_seq twoh) {
  ...
  if (!is_gimple_call(ones) || !is_gimple_call(twos) || gimple_call_lhs(ones) ||
      gimple_call_lhs(twos) || gimple_call_chain(ones) ||
  ...
  for (ai = 0; ai < gimple_call_num_args(ones); ++ai)
    if (!operand_equal_p(gimple_call_arg(ones, ai), gimple_call_arg(twos, ai),
                         0))
      return false;
...
static void optimize_double_finally(gimple one, gimple two) {
  gimple oneh;
  gimple_stmt_iterator gsi;
...
static void refactor_eh_r(gimple_seq seq) {
  ...
  while (1) {
    ...
    if (gsi_end_p(gsi))
      ...
      two = gsi_stmt(gsi);
    if (one && two && gimple_code(one) == GIMPLE_TRY &&
