...
static void dse_initialize_block_local_data(struct dom_walk_data *, basic_block,
                                            bool);
static void dse_optimize_stmt(struct dom_walk_data *, basic_block,
...
static unsigned get_stmt_uid(gimple stmt) {
...
static void record_voperand_set(bitmap global, bitmap *local,
                                unsigned int uid) {
...
static void dse_initialize_block_local_data(struct dom_walk_data *walk_data,
                                            basic_block bb ATTRIBUTE_UNUSED,
                                            bool recycled) {
  struct dse_block_local_data *bd = (struct dse_block_local_data *)VEC_last(
      void_p, walk_data->block_data_stack);
...
static tree memory_ssa_name_same(tree *expr_p,
  ...
  if (walk_data->store1_bb != def_bb &&
    ...
    if (walk_data->store2_bb == def_bb ||
        !dominated_by_p(CDI_POST_DOMINATORS, walk_data->store2_bb, def_bb))
...
static bool memory_address_same(gimple store1, gimple store2) {
...
static bool get_kill_of_stmt_lhs(gimple stmt, use_operand_p *first_use_p,
...
static bool dse_possible_dead_store_p(gimple stmt, use_operand_p *first_use_p,
                                      use_operand_p *use_p, gimple *use_stmt,
...
static void dse_optimize_stmt(struct dom_walk_data *walk_data,
                              basic_block bb ATTRIBUTE_UNUSED,
  ...
  if (is_gimple_assign(stmt)) {
    ...
    if (use_p != NULL_USE_OPERAND_P &&
        ...
        !operand_equal_p(gimple_assign_lhs(stmt), gimple_assign_lhs(use_stmt),
                         0) &&
...
/* Record that we have seen the PHIs at the start of BB which correspond
   to virtual operands.  */
static void dse_record_phis(struct dom_walk_data *walk_data, basic_block bb) {
  struct dse_block_local_data *bd = (struct dse_block_local_data *)VEC_last(
      void_p, walk_data->block_data_stack);
  struct dse_global_data *dse_gd =
      (struct dse_global_data *)walk_data->global_data;
...
static void dse_finalize_block(struct dom_walk_data *walk_data,
                               basic_block bb ATTRIBUTE_UNUSED) {
  struct dse_block_local_data *bd = (struct dse_block_local_data *)VEC_last(
      void_p, walk_data->block_data_stack);
  struct dse_global_data *dse_gd =
...
static unsigned int execute_simple_dse(void) {
  gimple_stmt_iterator gsi;
  basic_block bb;
  ...
  for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi))
    ...
    if (gimple_loaded_syms(gsi_stmt(gsi)))
      bitmap_ior_into(variables_loaded, gimple_loaded_syms(gsi_stmt(gsi)));
    ...
    if (is_gimple_assign(stmt) &&
      ...
      todo |= TODO_cleanup_cfg;
               ...
               (is_gimple_assign(stmt) ||
                (is_gimple_call(stmt) && gimple_call_lhs(stmt))) &&
      ...
      EXECUTE_IF_SET_IN_BITMAP(gimple_stored_syms(stmt), 0, i, bi) {
        ...
        if (TREE_ADDRESSABLE(var) || is_global_var(var) ||
      ...
      if (dead) {
        ...
        if (is_gimple_call(stmt) && gimple_has_side_effects(stmt)) {
          if (dump_file && (dump_flags & TDF_DETAILS)) {
