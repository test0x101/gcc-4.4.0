...
void pp_c_whitespace(c_pretty_printer *pp) {
  pp_space(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_left_paren(c_pretty_printer *pp) {
  pp_left_paren(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_right_paren(c_pretty_printer *pp) {
  pp_right_paren(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_left_brace(c_pretty_printer *pp) {
  pp_left_brace(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_right_brace(c_pretty_printer *pp) {
  pp_right_brace(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_left_bracket(c_pretty_printer *pp) {
  pp_left_bracket(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_right_bracket(c_pretty_printer *pp) {
  pp_right_bracket(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_dot(c_pretty_printer *pp) {
  pp_dot(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_ampersand(c_pretty_printer *pp) {
  pp_ampersand(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_star(c_pretty_printer *pp) {
  pp_star(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_arrow(c_pretty_printer *pp) {
  pp_arrow(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_semicolon(c_pretty_printer *pp) {
  pp_semicolon(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_complement(c_pretty_printer *pp) {
  pp_complement(pp);
  pp_base(pp)->padding = pp_none;
}

void pp_c_exclamation(c_pretty_printer *pp) {
  pp_exclamation(pp);
  pp_base(pp)->padding = pp_none;
}
...
static void pp_c_cv_qualifier(c_pretty_printer *pp, const char *cv) {
...
static void pp_c_type_cast(c_pretty_printer *pp, tree t) {
  pp_c_left_paren(pp);
  pp_type_id(pp, t);
  pp_c_right_paren(pp);
}
...
void pp_c_space_for_pointer_operator(c_pretty_printer *pp, tree t) {
  if (POINTER_TYPE_P(t)) {
    tree pointee = strip_pointer_operator(TREE_TYPE(t));
    if (TREE_CODE(pointee) != ARRAY_TYPE && TREE_CODE(pointee) != FUNCTION_TYPE)
      pp_c_whitespace(pp);
  }
}
...
void pp_c_type_qualifier_list(c_pretty_printer *pp, tree t) {
  int qualifiers;
...
static void pp_c_pointer(c_pretty_printer *pp, tree t) {
...
void pp_c_type_specifier(c_pretty_printer *pp, tree t) {
    ...
    case FIXED_POINT_TYPE:
      if (TYPE_NAME(t)) {
        ...
        if (TYPE_NAME(t)) {
          ...
          switch (code) {
            case INTEGER_TYPE:
              pp_string(pp, (TYPE_UNSIGNED(t) ? "<unnamed-unsigned:"
                                              : "<unnamed-signed:"));
              break;
            case REAL_TYPE:
...
void pp_c_specifier_qualifier_list(c_pretty_printer *pp, tree t) {
  const enum tree_code code = TREE_CODE(t);
...
void pp_c_parameter_type_list(c_pretty_printer *pp, tree t) {
  ...
  if (parms == void_list_node)
    ...
    for (; parms && parms != void_list_node; parms = TREE_CHAIN(parms)) {
      if (!first)
        pp_separate_with(pp, ',');
      first = false;
      pp_declaration_specifiers(pp, want_parm_decl ? parms : TREE_VALUE(parms));
      if (want_parm_decl)
...
static void pp_c_abstract_declarator(c_pretty_printer *pp, tree t) {
  if (TREE_CODE(t) == POINTER_TYPE) {
...
void pp_c_direct_abstract_declarator(c_pretty_printer *pp, tree t) {
  switch (TREE_CODE(t)) {
    case POINTER_TYPE:
    ...
    case ARRAY_TYPE:
      ...
      if (TYPE_DOMAIN(t) && TYPE_MAX_VALUE(TYPE_DOMAIN(t))) {
        ...
        if (host_integerp(maxval, 0))
          ...
          pp_expression(
              pp, fold_build2(PLUS_EXPR, type, maxval, build_int_cst(type, 1)));
      }
...
void pp_c_type_id(c_pretty_printer *pp, tree t) {
...
void pp_c_storage_class_specifier(c_pretty_printer *pp, tree t) {
...
void pp_c_function_specifier(c_pretty_printer *pp, tree t) {
...
void pp_c_declaration_specifiers(c_pretty_printer *pp, tree t) {
...
/* direct-declarator
   ...
   assignment-expression(opt)] direct-declarator [ type-qualifier-list static
   assignment-expression ] direct-declarator [ type-qualifier-list * ]
      direct-declarator ( parameter-type-list )
      direct-declarator ( identifier-list(opt) )  */

void pp_c_direct_declarator(c_pretty_printer *pp, tree t) {
  switch (TREE_CODE(t)) {
    case VAR_DECL:
    case PARM_DECL:
    case TYPE_DECL:
...
void pp_c_declarator(c_pretty_printer *pp, tree t) {
  switch (TREE_CODE(t)) {
    case INTEGER_TYPE:
    case REAL_TYPE:
    case FIXED_POINT_TYPE:
...
void pp_c_function_definition(c_pretty_printer *pp, tree t) {
  pp_declaration_specifiers(pp, t);
  pp_declarator(pp, t);
  pp_needs_newline(pp) = true;
...
static void pp_c_char(c_pretty_printer *pp, int c) {
  if (ISPRINT(c)) {
    switch (c) {
      case '\\':
        pp_string(pp, "\\\\");
...
void pp_c_string_literal(c_pretty_printer *pp, tree s) {
...
static void pp_c_integer_constant(c_pretty_printer *pp, tree i) {
...
static void pp_c_character_constant(c_pretty_printer *pp, tree c) {
...
static void pp_c_bool_constant(c_pretty_printer *pp, tree b) {
...
static bool pp_c_enumeration_constant(c_pretty_printer *pp, tree e) {
...
static void pp_c_floating_constant(c_pretty_printer *pp, tree r) {
...
static void pp_c_fixed_constant(c_pretty_printer *pp, tree r) {
...
static void pp_c_compound_literal(c_pretty_printer *pp, tree e) {
  tree type = TREE_TYPE(e);
  pp_c_type_cast(pp, type);
...
static void pp_c_complex_expr(c_pretty_printer *pp, tree e) {
  ...
  if (TREE_CODE(realexpr) == NOP_EXPR && TREE_CODE(imagexpr) == NOP_EXPR &&
...
void pp_c_constant(c_pretty_printer *pp, tree e) {
...
void pp_c_identifier(c_pretty_printer *pp, const char *id) {
...
void pp_c_primary_expression(c_pretty_printer *pp, tree e) {
  switch (TREE_CODE(e)) {
    case VAR_DECL:
    case PARM_DECL:
    case FIELD_DECL:
...
static void pp_c_initializer(c_pretty_printer *pp, tree e) {
...
void pp_c_init_declarator(c_pretty_printer *pp, tree t) {
  pp_declarator(pp, t);
...
static void pp_c_initializer_list(c_pretty_printer *pp, tree e) {
  tree type = TREE_TYPE(e);
...
static void pp_c_brace_enclosed_initializer_list(c_pretty_printer *pp, tree l) {
  pp_c_left_brace(pp);
...
void pp_c_id_expression(c_pretty_printer *pp, tree t) {
  switch (TREE_CODE(t)) {
    case VAR_DECL:
    case PARM_DECL:
    case CONST_DECL:
...
void pp_c_postfix_expression(c_pretty_printer *pp, tree e) {
  ...
  switch (code) {
    ...
    case UNORDERED_EXPR:
      pp_c_identifier(pp,
                      flag_isoc99 ? "isunordered" : "__builtin_isunordered");
      goto two_args_fun;
    ...
    case UNLT_EXPR:
      pp_c_identifier(
          pp, flag_isoc99 ? "!isgreaterequal" : "!__builtin_isgreaterequal");
      goto two_args_fun;

    case UNLE_EXPR:
      pp_c_identifier(pp, flag_isoc99 ? "!isgreater" : "!__builtin_isgreater");
      goto two_args_fun;

    case UNGT_EXPR:
      pp_c_identifier(pp,
                      flag_isoc99 ? "!islessequal" : "!__builtin_islessequal");
      goto two_args_fun;

    case UNGE_EXPR:
      pp_c_identifier(pp, flag_isoc99 ? "!isless" : "!__builtin_isless");
      goto two_args_fun;

    case UNEQ_EXPR:
      pp_c_identifier(
          pp, flag_isoc99 ? "!islessgreater" : "!__builtin_islessgreater");
      goto two_args_fun;
    ...
    case BIT_FIELD_REF: {
      ...
      if (type && tree_int_cst_equal(TYPE_SIZE(type), TREE_OPERAND(e, 1))) {
...
void pp_c_expression_list(c_pretty_printer *pp, tree e) {
...
void pp_c_constructor_elts(c_pretty_printer *pp, VEC(constructor_elt, gc) * v) {
...
void pp_c_call_argument_list(c_pretty_printer *pp, tree t) {
  pp_c_left_paren(pp);
...
void pp_c_unary_expression(c_pretty_printer *pp, tree e) {
  enum tree_code code = TREE_CODE(e);
  switch (code) {
...
void pp_c_cast_expression(c_pretty_printer *pp, tree e) {
  switch (TREE_CODE(e)) {
    case FLOAT_EXPR:
    case FIX_TRUNC_EXPR:
    CASE_CONVERT:
...
static void pp_c_multiplicative_expression(c_pretty_printer *pp, tree e) {
  enum tree_code code = TREE_CODE(e);
  switch (code) {
    case MULT_EXPR:
    case TRUNC_DIV_EXPR:
...
static void pp_c_additive_expression(c_pretty_printer *pp, tree e) {
  enum tree_code code = TREE_CODE(e);
  switch (code) {
...
static void pp_c_shift_expression(c_pretty_printer *pp, tree e) {
  enum tree_code code = TREE_CODE(e);
  switch (code) {
    case LSHIFT_EXPR:
    case RSHIFT_EXPR:
...
static void pp_c_equality_expression(c_pretty_printer *pp, tree e) {
  enum tree_code code = TREE_CODE(e);
  switch (code) {
    case EQ_EXPR:
    case NE_EXPR:
...
static void pp_c_and_expression(c_pretty_printer *pp, tree e) {
...
static void pp_c_exclusive_or_expression(c_pretty_printer *pp, tree e) {
  if (TREE_CODE(e) == BIT_XOR_EXPR || TREE_CODE(e) == TRUTH_XOR_EXPR) {
...
static void pp_c_inclusive_or_expression(c_pretty_printer *pp, tree e) {
...
static void pp_c_logical_and_expression(c_pretty_printer *pp, tree e) {
  if (TREE_CODE(e) == TRUTH_ANDIF_EXPR || TREE_CODE(e) == TRUTH_AND_EXPR) {
...
void pp_c_logical_or_expression(c_pretty_printer *pp, tree e) {
  if (TREE_CODE(e) == TRUTH_ORIF_EXPR || TREE_CODE(e) == TRUTH_OR_EXPR) {
...
static void pp_c_conditional_expression(c_pretty_printer *pp, tree e) {
...
static void pp_c_assignment_expression(c_pretty_printer *pp, tree e) {
...
void pp_c_expression(c_pretty_printer *pp, tree e) {
  switch (TREE_CODE(e)) {
    case INTEGER_CST:
      pp_c_integer_constant(pp, e);
      break;
...
void pp_c_statement(c_pretty_printer *pp, tree stmt) {
  if (stmt == NULL)
    return;
...
void pp_c_tree_decl_identifier(c_pretty_printer *pp, tree t) {
  const char *name;
