...
struct web_entry *unionfind_root(struct web_entry *element) {
...
bool unionfind_union(struct web_entry *first, struct web_entry *second) {
  ...
  if (use_link)
    while (*use_link) {
      if (use != *use_link &&
        ...
        (*fun)(use_entry + DF_REF_ID(use), use_entry + DF_REF_ID(*use_link));
      use_link++;
    }
  ...
  if (eq_use_link)
    while (*eq_use_link) {
      if (use != *eq_use_link &&
        ...
        (*fun)(use_entry + DF_REF_ID(use), use_entry + DF_REF_ID(*eq_use_link));
      eq_use_link++;
    }
    ...
    if (def_link)
      while (*def_link) {
        if (DF_REF_REAL_REG(use) == DF_REF_REAL_REG(*def_link))
          (*fun)(use_entry + DF_REF_ID(use), def_entry + DF_REF_ID(*def_link));
        def_link++;
      }
  }
  while (link) {
    ...
    if (link)
      while (*link) {
        if (DF_REF_REAL_REG(*link) == DF_REF_REAL_REG(use))
          (*fun)(use_entry + DF_REF_ID(use), def_entry + DF_REF_ID(*link));
        link++;
      }
  }
}
...
static rtx entry_register(struct web_entry *entry, df_ref ref, char *used) {
  ...
  if (!used[REGNO(reg)])
    ...
    newreg = reg;
    if (dump_file)
      fprintf(dump_file, "New web forced to keep reg=%i (user variable)\n",
              REGNO(reg));
  } else {
...
static bool gate_handle_web(void) {
  return (optimize > 0 && flag_web);
}

/* Main entry point.  */

static unsigned int web_main(void) {
  ...
  FOR_BB_INSNS(bb, insn) {
    ...
    if (INSN_P(insn)) {
      ...
      for (use_rec = DF_INSN_UID_USES(uid); *use_rec; use_rec++) {
        ...
        if (DF_REF_REGNO(use) >= FIRST_PSEUDO_REGISTER)
          replace_ref(use,
                      entry_register(use_entry + DF_REF_ID(use), use, used));
      }
      for (use_rec = DF_INSN_UID_EQ_USES(uid); *use_rec; use_rec++) {
        df_ref use = *use_rec;
        if (DF_REF_REGNO(use) >= FIRST_PSEUDO_REGISTER)
      ...
      for (def_rec = DF_INSN_UID_DEFS(uid); *def_rec; def_rec++) {
        ...
        if (DF_REF_REGNO(def) >= FIRST_PSEUDO_REGISTER)
          replace_ref(def,
                      entry_register(def_entry + DF_REF_ID(def), def, used));
      }
    }
  }
