...
#define ebitmap_free(MAP) \
  (free((MAP)->elts), sbitmap_free((MAP)->wordmask), free((MAP)))
...
static inline void ebitmap_iter_init(ebitmap_iterator *i, ebitmap bmp,
                                     unsigned int min) {
  sbitmap_iter_init(&i->maskiter, bmp->wordmask, min / EBITMAP_ELT_BITS);
  i->size = bmp->numwords;
  if (i->size == 0) {
    i->ptr = NULL;
    i->eltnum = 0;
...
static inline bool ebitmap_iter_cond(ebitmap_iterator *i, unsigned int *n) {
  unsigned int ourn = 0;
...
static inline void ebitmap_iter_next(ebitmap_iterator *i) {
  i->word >>= 1;
  i->bit_num++;
}
...
#define EXECUTE_IF_SET_IN_EBITMAP(EBITMAP, MIN, N, ITER) \
  for (ebitmap_iter_init(&(ITER), (EBITMAP), (MIN));     \
       ebitmap_iter_cond(&(ITER), &(N)); ebitmap_iter_next(&(ITER)))
