...
/* Implementation of selective scheduling approach.
   ...
   loop is pipelined, its prologue can be scheduled together with yet
   unprocessed outer loop. The rest of acyclic regions are found using
   extend_rgns: the blocks that are not yet allocated to any regions are
   traversed in top-down order, and a block is added to a region to which all
   its predecessors belong; otherwise, the block starts its own region.
   ...
   can do several actions such as: determine whether an insn can be moved
   through another (has_dependence_p, moveup_expr); find out whether an insn can
   be scheduled on the current cycle (tick_check_p); find out registers that are
   set/used/clobbered by an insn and find out all the strange stuff that
   ...
   postfixed with the type of data initialized:
   {,sel_,haifa_}sched_{init,finish}, sched_rgn_init/finish,
   sched_deps_init/finish, sched_init_{luids/bbs}, etc. The same splitting is
   done with current_sched_info structure: dependence-related parts are in
   sched_deps_info, common part is in common_sched_info, and haifa/sel/etc part
   is in current_sched_info.
   ...
   As the correct data dependence graph is not supported during scheduling
   (which is to be changed in mid-term), we cache as much of the dependence
   analysis results as possible to avoid reanalyzing.  This includes: bitmap
   caches on each insn in stream of the region saying yes/no for a query with a
   pair of UIDs; hashtables with the previously done transformations on each
   insn in stream; a vector keeping a history of transformations on each expr.
...
static void advance_one_cycle(fence_t fence) {
  unsigned i;
  int cycle;
  rtx insn;
...
static bool in_fallthru_bb_p(rtx insn, rtx succ) {
...
static void extract_new_fences_from(flist_t old_fences, flist_tail_t new_fences,
                                    int orig_max_seqno) {
  ...
  if (!sel_bb_end_p(insn) ||
      (single_succ_p(bb) && single_pred_p(single_succ(bb)))) {
    insn_t succ;

    succ =
        (sel_bb_end_p(insn) ? sel_bb_head(single_succ(bb)) : NEXT_INSN(insn));

    if (INSN_SEQNO(succ) > 0 && INSN_SEQNO(succ) <= orig_max_seqno &&
  ...
  FOR_EACH_SUCC_1(succ, si, insn, SUCCS_NORMAL | SUCCS_SKIP_TO_LOOP_EXITS) {
    ...
    if (0 < seqno && seqno <= orig_max_seqno &&
      ...
      bool b = (in_same_ebb_p(insn, succ) || in_fallthru_bb_p(insn, succ));

      if (sched_verbose >= 1)
        sel_print("Fence %d continues as %d[%d] (state %s)\n", INSN_UID(insn),
                  INSN_UID(succ), BLOCK_NUM(succ), b ? "continue" : "reset");
...
static bool can_substitute_through_p(insn_t insn, ds_t ds) {
  /* We can substitute only true dependencies.  */
  if ((ds & DEP_OUTPUT) || (ds & DEP_ANTI) || !INSN_RHS(insn) ||
      !INSN_LHS(insn))
    return false;

  /* Now we just need to make sure the INSN_RHS consists of only one
     simple REG rtx.  */
  if (REG_P(INSN_LHS(insn)) && REG_P(INSN_RHS(insn)))
    return true;
  return false;
}
...
static bool substitute_reg_in_expr(expr_t expr, insn_t insn, bool undo) {
  rtx *where;
  bool new_insn_valid;
  ...
  /* Do not try to replace in SET_DEST.  Although we'll choose new
     ...
     dest.  */
  where = (has_rhs ? &VINSN_RHS(*vi) : &PATTERN(VINSN_INSN_RTX(*vi)));
  ...
  if (rtx_ok_for_substitution_p(old, *where)) {
    ...
    where_replace =
        (has_rhs ? &SET_SRC(PATTERN(new_insn)) : &PATTERN(new_insn));

    new_insn_valid = validate_replace_rtx_part_nosimplify(
        old, new_rtx, where_replace, new_insn);
    ...
    /* ??? Actually, constrain_operands result depends upon choice of
       ...
       leaving it for now.  */
    if (new_insn_valid) {
      change_vinsn_in_expr(expr, create_vinsn_from_insn_rtx(new_insn, false));

      /* Do not allow clobbering the address register of speculative
         insns.  */
      if ((EXPR_SPEC_DONE_DS(expr) & SPECULATIVE) &&
          bitmap_bit_p(VINSN_REG_USES(EXPR_VINSN(expr)), expr_dest_regno(expr)))
...
static int count_occurrences_1(rtx *cur_rtx, void *arg) {
  ...
  if (exp_equiv_p(*cur_rtx, p->x, 0, true)) {
    ...
    if (REG_P(*cur_rtx) && HARD_REGISTER_P(*cur_rtx) &&
  ...
  if (GET_CODE(*cur_rtx) == SUBREG && REG_P(p->x) &&
...
static int count_occurrences_equiv(rtx what, rtx where) {
...
static bool rtx_ok_for_substitution_p(rtx what, rtx where) {
...
static rtx create_insn_rtx_with_rhs(vinsn_t vi, rtx rhs_rtx) {
  rtx lhs_rtx;
  rtx pattern;
  rtx insn_rtx;
...
static bool replace_src_with_reg_ok_p(insn_t insn, rtx new_src_reg) {
...
/* Returns whether INSN still be valid after replacing it's DEST with
   register NEW_REG.  */
static bool replace_dest_with_reg_ok_p(insn_t insn, rtx new_reg) {
  vinsn_t vi = INSN_VINSN(insn);
  bool res;
...
static rtx create_insn_rtx_with_lhs(vinsn_t vi, rtx lhs_rtx) {
  rtx rhs_rtx;
  rtx pattern;
  rtx insn_rtx;
...
static void replace_dest_with_reg_in_expr(expr_t expr, rtx new_reg) {
  rtx insn_rtx;
  vinsn_t vinsn;
...
static bool vinsn_writes_one_of_regs_p(vinsn_t vi, regset used_regs,
                                       HARD_REG_SET unavailable_hard_regs) {
  unsigned regno;
...
static enum reg_class get_reg_class(rtx insn) {
  int alt, i, n_ops;
  ...
  if (asm_noperands(PATTERN(insn)) > 0) {
    for (i = 0; i < n_ops; i++)
      if (recog_data.operand_type[i] == OP_OUT) {
        ...
        if (REG_P(op) && REGNO(op) == ORIGINAL_REGNO(op))
          continue;
...
static void init_hard_regno_rename(int regno) {
  int cur_reg;
...
/* A wrapper around HARD_REGNO_RENAME_OK that will look into the hard regs
   data first.  */
static inline bool sel_hard_regno_rename_ok(int from ATTRIBUTE_UNUSED,
...
static void init_regs_for_mode(enum machine_mode mode) {
  int cur_reg;
  ...
  for (cur_reg = 0; cur_reg < FIRST_PSEUDO_REGISTER; cur_reg++) {
    ...
    for (i = nregs - 1; i >= 0; --i)
      if (fixed_regs[cur_reg + i] ||
          ...
          /* We can't use a non-leaf register if we're in a
             leaf function.  */
          || (current_function_is_leaf && !LEAF_REGISTERS[cur_reg + i])
#endif
      )
        break;
    ...
    if (HARD_REGNO_CALL_PART_CLOBBERED(cur_reg, mode))
      SET_HARD_REG_BIT(sel_hrd.regs_for_call_clobbered[mode], cur_reg);
...
static void init_hard_regs_data(void) {
  int cur_reg = 0;
...
static void mark_unavailable_hard_regs(def_t def,
  ...
  /* Stop when mode is not supported for renaming.  Also can't proceed
     ...
     frame pointer.  */
  if (fixed_regs[regno] || global_regs[regno]
  ...
  if (IN_RANGE(REGNO(orig_dest), FIRST_STACK_REG, LAST_STACK_REG) &&
    ...
    IOR_HARD_REG_SET(reg_rename_p->unavailable_hard_regs, sel_hrd.stack_regs);
#endif

  /* If there's a call on this path, make regs from call_used_reg_set
     unavailable.  */
  if (def->crosses_call)
    IOR_HARD_REG_SET(reg_rename_p->unavailable_hard_regs, call_used_reg_set);
  ...
  if (def->crosses_call && !HARD_REGNO_CALL_PART_CLOBBERED(regno, mode))
    AND_COMPL_HARD_REG_SET(reg_rename_p->available_for_renaming,
                           sel_hrd.regs_for_call_clobbered[mode]);

  /* Leave only those that are ok to rename.  */
  EXECUTE_IF_SET_IN_HARD_REG_SET(reg_rename_p->available_for_renaming, 0,
                                 cur_reg, hrsi) {
    int nregs;
    int i;
    ...
    if (i >= 0)
      CLEAR_HARD_REG_BIT(reg_rename_p->available_for_renaming, cur_reg);
  }
...
static rtx choose_best_reg_1(HARD_REG_SET hard_regs_used,
                             struct reg_rename *reg_rename_p,
                             def_list_t original_insns,
...
static rtx choose_best_reg(HARD_REG_SET hard_regs_used,
                           struct reg_rename *reg_rename_p,
                           def_list_t original_insns, bool *is_orig_reg_p_ptr) {
  rtx best_reg = choose_best_reg_1(hard_regs_used, reg_rename_p, original_insns,
                                   is_orig_reg_p_ptr);
...
/* Choose the pseudo register for storing rhs value.  As this is supposed
   ...
   not rely on this.  */
static rtx choose_best_pseudo_reg(regset used_regs,
                                  struct reg_rename *reg_rename_p,
                                  def_list_t original_insns,
                                  bool *is_orig_reg_p_ptr) {
  def_list_iterator i;
...
static void verify_target_availability(expr_t expr, regset used_regs,
                                       struct reg_rename *reg_rename_p) {
  unsigned n, i, regno;
  enum machine_mode mode;
...
/* Collect unavailable registers due to liveness for EXPR from BNDS
   ...
   list.  */
static void collect_unavailable_regs_from_bnds(expr_t expr, blist_t bnds,
                                               regset used_regs,
                                               struct reg_rename *reg_rename_p,
                                               def_list_t *original_insns) {
  for (; bnds; bnds = BLIST_NEXT(bnds)) {
    ...
    res = find_used_regs(BND_TO(bnd), orig_ops, used_regs, reg_rename_p,
                         original_insns);
...
static bool try_replace_dest_reg(ilist_t orig_insns, rtx best_reg,
                                 expr_t expr) {
...
static bool find_best_reg_for_expr(expr_t expr, blist_t bnds,
                                   bool *is_orig_reg_p) {
  ...
  *is_orig_reg_p = false;
  ...
  if (EXPR_SEPARABLE_P(expr)) {
    ...
    if (reload_completed) {
      ...
      IOR_HARD_REG_SET(hard_regs_used, reg_rename_data.unavailable_hard_regs);
...
static bool can_speculate_dep_p(ds_t ds) {
  if (spec_info == NULL)
    return false;
...
static insn_t create_speculation_check(expr_t c_expr, ds_t check_ds,
                                       insn_t orig_insn) {
  rtx check_pattern;
  rtx insn_rtx;
  insn_t insn;
  ...
  check_pattern =
      targetm.sched.gen_spec_check(EXPR_INSN_RTX(c_expr), label, check_ds);
  ...
  if (recovery_block != NULL) {
    ...
    twin = sel_gen_recovery_insn_from_rtx_after(twin_rtx, INSN_EXPR(orig_insn),
                                                INSN_SEQNO(insn),
...
static bool identical_copy_p(rtx insn) {
  rtx lhs, rhs, pat;
...
/* Undo all transformations on *AV_PTR that were done when
   moving through INSN.  */
static void undo_transformations(av_set_t *av_ptr, rtx insn) {
  ...
  if (!sched_insns_conditions_mutex_p(insn, EXPR_INSN_RTX(expr)) &&
      bitmap_intersect_p(INSN_REG_SETS(insn), VINSN_REG_USES(EXPR_VINSN(expr)))
  ...
  FOR_EACH_EXPR(expr, av_iter, *av_ptr) {
    int index = find_in_history_vect(EXPR_HISTORY_OF_CHANGES(expr), insn,
                                     EXPR_VINSN(expr), true);

    if (index >= 0) {
      expr_history_def *phist;

      phist = VEC_index(expr_history_def, EXPR_HISTORY_OF_CHANGES(expr), index);
      ...
      switch (phist->type) {
        ...
        case TRANS_SUBSTITUTION: {
          ...
          gcc_assert(VINSN_SEPARABLE_P(new_vi) == EXPR_SEPARABLE_P(expr));
          copy_expr(tmp_expr, expr);

          if (vinsn_equal_p(phist->new_expr_vinsn, EXPR_VINSN(tmp_expr)))
...
/* Propagates EXPR inside an insn group through THROUGH_INSN.
   ...
   performed.  */
static enum MOVEUP_EXPR_CODE moveup_expr_inside_insn_group(
    expr_t expr, insn_t through_insn) {
  vinsn_t vi = EXPR_VINSN(expr);
  ds_t *has_dep_p;
  ds_t full_ds;
  ...
  if (has_dep_p[DEPS_IN_RHS]) {
    ...
    if (can_substitute_through_p(through_insn, has_dep_p[DEPS_IN_RHS]) &&
...
static enum MOVEUP_EXPR_CODE moveup_expr(expr_t expr, insn_t through_insn,
    ...
    if (can_speculate_dep_p(*rhs_dsp)) {
      ...
      if (res >= 0) {
        ...
        *rhs_dsp = 0;
        ...
        return MOVEUP_EXPR_NULL;
    } else if (can_substitute_through_p(through_insn, *rhs_dsp) &&
               substitute_reg_in_expr(expr, through_insn, false)) {
  ...
  return (was_changed ? MOVEUP_EXPR_CHANGED
                      : (as_rhs ? MOVEUP_EXPR_AS_RHS : MOVEUP_EXPR_SAME));
}
...
static bool try_bitmap_cache(expr_t expr, insn_t insn, bool inside_insn_group,
  ...
  if (bitmap_bit_p(INSN_ANALYZED_DEPS(insn), expr_uid)) {
    ...
    if (enable_schedule_as_rhs_p) {
      ...
      *res = MOVEUP_EXPR_AS_RHS;
      ...
      if (sched_verbose >= 6)
        sel_print(
            "removed (cached as RHS, but renaming"
            " is now disabled)\n");
      *res = MOVEUP_EXPR_NULL;
      return true;
    }
...
static bool try_transformation_cache(expr_t expr, insn_t insn,
                                     enum MOVEUP_EXPR_CODE *res) {
  struct transformed_insns *pti =
      (struct transformed_insns *)htab_find_with_hash(
          INSN_TRANSFORMED_INSNS(insn), &EXPR_VINSN(expr),
  ...
  if (pti) {
    /* This EXPR was already moved through this insn and was
       ...
       the hashtable.  */
    insert_in_history_vect(&EXPR_HISTORY_OF_CHANGES(expr), INSN_UID(insn),
                           pti->type, pti->vinsn_old, pti->vinsn_new,
...
static void update_bitmap_cache(expr_t expr, insn_t insn,
                                bool inside_insn_group,
                                enum MOVEUP_EXPR_CODE res) {
  int expr_uid = INSN_UID(EXPR_INSN_RTX(expr));
...
static void update_transformation_cache(expr_t expr, insn_t insn,
                                        bool inside_insn_group,
  ...
  *((struct transformed_insns **)htab_find_slot_with_hash(
      INSN_TRANSFORMED_INSNS(insn), pti, VINSN_HASH_RTX(expr_old_vinsn),
      INSERT)) = pti;
}

/* Same as moveup_expr, but first looks up the result of
   transformation in caches.  */
static enum MOVEUP_EXPR_CODE moveup_expr_cached(expr_t expr, insn_t insn,
                                                bool inside_insn_group) {
  enum MOVEUP_EXPR_CODE res;
  ...
  if (!got_answer) {
    ...
    res = moveup_expr(expr, insn, inside_insn_group, &trans_type);
    switch (res) {
      ...
      case MOVEUP_EXPR_CHANGED:
        ...
        insert_in_history_vect(&EXPR_HISTORY_OF_CHANGES(expr), INSN_UID(insn),
                               trans_type, expr_old_vinsn, EXPR_VINSN(expr),
                               expr_old_spec_ds);
        update_transformation_cache(expr, insn, inside_insn_group, trans_type,
                                    expr_old_vinsn);
        if (sched_verbose >= 6) {
          sel_print("changed: ");
          dump_expr(expr);
...
/* Moves an av set AVP up through INSN, performing necessary
   transformations.  */
static void moveup_set_expr(av_set_t *avp, insn_t insn,
                            bool inside_insn_group) {
  av_set_iterator i;
  expr_t expr;
...
static void moveup_set_inside_insn_group(av_set_t *avp, ilist_t path) {
  int last_cycle;
  ...
  while (path && INSN_SCHED_CYCLE(ILIST_INSN(path)) == last_cycle) {
...
static bool equal_after_moveup_path_p(expr_t expr, ilist_t path,
                                      expr_t expr_vliw) {
  ...
  while (path && res && INSN_SCHED_CYCLE(ILIST_INSN(path)) == last_cycle) {
    res = (moveup_expr_cached(tmp, ILIST_INSN(path), true) != MOVEUP_EXPR_NULL);
    path = ILIST_NEXT(path);
  }
...
/* Returns true if INSN is not a downward continuation of the given path P in
   the current stage.  */
static bool is_ineligible_successor(insn_t insn, ilist_t p) {
  insn_t prev_insn;
  ...
  if (/* a backward edge.  */
      ...
      || (INSN_SEQNO(insn) > INSN_SEQNO(prev_insn) && IN_CURRENT_FENCE_P(insn))
...
static av_set_t compute_av_set_at_bb_end(insn_t insn, ilist_t p, int ws) {
  ...
  for (is = 0; VEC_iterate(rtx, sinfo->succs_ok, is, succ); is++) {
    ...
    av_set_split_usefulness(succ_set, VEC_index(int, sinfo->probs_ok, is),
                            sinfo->all_prob);

    if (sinfo->all_succs_n > 1 && sinfo->all_succs_n == sinfo->succs_ok_n) {
    ...
    if (sinfo->succs_ok_n == 2 && is == 1) {
      ...
      av_set_union_and_live(&av1, &succ_set, BB_LV_SET(bb0), BB_LV_SET(bb1),
                            insn);
    } else
  ...
  if (sinfo->succs_ok_n > 2)
    for (is = 0; VEC_iterate(rtx, sinfo->succs_ok, is, succ); is++) {
      ...
      mark_unavailable_targets(av1, BB_AV_SET(succ_bb), BB_LV_SET(succ_bb));
    }
  ...
  if (sinfo->all_succs_n > sinfo->succs_ok_n)
    for (is = 0; VEC_iterate(rtx, sinfo->succs_other, is, succ); is++)
      mark_unavailable_targets(av1, NULL, BB_LV_SET(BLOCK_FOR_INSN(succ)));
...
static av_set_t compute_av_set_inside_bb(insn_t first_insn, ilist_t p, int ws,
                                         bool need_copy_p) {
  insn_t cur_insn;
  int end_ws = ws;
  ...
  for (last_insn = first_insn; last_insn != after_bb_end;
    ...
    /* The special case: the last insn of the BB may be an
       ...
       it as a bookkeeping.  */
    if (last_insn != first_insn && is_ineligible_successor(last_insn, p)) {
      if (sched_verbose >= 6)
        sel_print("Insn %d is ineligible_successor\n", INSN_UID(last_insn));
      break;
    }
...
static av_set_t compute_av_set(insn_t insn, ilist_t p, int ws, bool unique_p) {
  return compute_av_set_inside_bb(insn, p, ws, unique_p);
}

/* Propagate a liveness set LV through INSN.  */
static void propagate_lv_set(regset lv, insn_t insn) {
...
static regset compute_live_after_bb(basic_block bb) {
  edge e;
  edge_iterator ei;
...
regset compute_live(insn_t insn) {
  basic_block bb = BLOCK_FOR_INSN(insn);
  insn_t final, temp;
  regset lv;
...
static inline void update_liveness_on_insn(rtx insn) {
  ignore_first = true;
  compute_live(insn);
}

/* Compute liveness below INSN and write it into REGS.  */
static inline void compute_live_below_insn(rtx insn, regset regs) {
  rtx succ;
  succ_iterator si;
...
static void update_data_sets(rtx insn) {
...
static ds_t get_spec_check_type_for_insn(insn_t insn, expr_t expr) {
  ds_t to_check_ds;
...
static bool find_used_regs(insn_t insn, av_set_t orig_ops, regset used_regs,
                           struct reg_rename *reg_rename_p,
                           def_list_t *original_insns) {
  def_list_iterator i;
  def_t def;
...
static int sel_target_adjust_priority(expr_t expr) {
...
static int sel_rank_for_schedule(const void *x, const void *y) {
  ...
  if (EXPR_USEFULNESS(tmp) != 0 && EXPR_USEFULNESS(tmp2) != 0) {
    ...
    val = EXPR_PRIORITY(tmp2) - EXPR_PRIORITY(tmp) + EXPR_PRIORITY_ADJ(tmp2) -
...
static void process_pipelined_exprs(av_set_t *av_ptr) {
  expr_t expr;
  av_set_iterator si;
  ...
  FOR_EACH_EXPR_1(expr, si, av_ptr) {
    if (EXPR_SCHED_TIMES(expr) >= PARAM_VALUE(PARAM_SELSCHED_MAX_SCHED_TIMES))
      av_set_iter_remove(&si);
  }
}

/* Filter speculative insns from AV_PTR if we don't want them.  */
static void process_spec_exprs(av_set_t *av_ptr) {
  ...
  FOR_EACH_EXPR_1(expr, si, av_ptr) {
    ...
    if ((ds & SPECULATIVE) &&
        (ds_weak(ds) < spec_info->data_weakness_cutoff ||
         ...
         (pipelining_p && false && (ds & DATA_SPEC) && (ds & CONTROL_SPEC)))) {
      av_set_iter_remove(&si);
      continue;
    }

    if ((spec_info->flags & PREFER_NON_DATA_SPEC) && !(ds & BEGIN_DATA))
      try_data_p = false;

    if ((spec_info->flags & PREFER_NON_CONTROL_SPEC) && !(ds & BEGIN_CONTROL))
      try_control_p = false;
  }
...
static expr_t process_use_exprs(av_set_t *av_ptr) {
  expr_t expr;
  av_set_iterator si;
...
static bool vinsn_vec_has_expr_p(vinsn_vec_t vinsn_vec, expr_t expr) {
  vinsn_t vinsn;
  int n;
...
static bool av_set_could_be_blocked_by_bookkeeping_p(av_set_t orig_ops,
                                                     void *static_params) {
  expr_t expr;
...
static void vinsn_vec_clear(vinsn_vec_t *vinsn_vec) {
...
static void vinsn_vec_add(vinsn_vec_t *vinsn_vec, expr_t expr) {
...
static void vinsn_vec_free(vinsn_vec_t *vinsn_vec) {
  if (*vinsn_vec)
...
/* Turn AV into a vector, filter inappropriate insns and sort it.  Return
   ...
   *PNEED_STALL.  */
static bool fill_vec_av_set(av_set_t av, blist_t bnds, fence_t fence,
                            int *pneed_stall) {
  av_set_iterator si;
  expr_t expr;
  ...
  for (n = VEC_length(expr_t, vec_av_set) - 1, stalled = 0; n >= 0; n--) {
    ...
    if (target_available == true) {
      ...
      is_orig_reg_p = EXPR_SEPARABLE_P(expr);
               ...
               (target_available == false && !EXPR_SEPARABLE_P(expr))
               /* Don't try to find a register for low-priority expression.  */
               || (int)VEC_length(expr_t, vec_av_set) - 1 - n >=
                      max_insns_to_rename
    ...
    if (INSN_UID(insn) < FENCE_READY_TICKS_SIZE(fence)) {
      need_cycles =
          (FENCE_READY_TICKS(fence)[INSN_UID(insn)] - FENCE_CYCLE(fence));
      if (EXPR_ORIG_SCHED_CYCLE(expr) <= 0)
        est_ticks_till_branch =
            MAX(est_ticks_till_branch, EXPR_PRIORITY(expr) + need_cycles);

      if (need_cycles > 0) {
        stalled++;
        min_need_stall =
            (min_need_stall < 0 ? need_cycles
                                : MIN(min_need_stall, need_cycles));
        VEC_unordered_remove(expr_t, vec_av_set, n);

        if (sched_verbose >= 4)
          sel_print("Expr %d is not ready until cycle %d (cached)\n",
                    INSN_UID(insn), FENCE_READY_TICKS(fence)[INSN_UID(insn)]);
        continue;
      }
    }
    ...
    if (EXPR_ORIG_SCHED_CYCLE(expr) <= 0)
      est_ticks_till_branch = MAX(est_ticks_till_branch, new_prio);
    ...
    if (need_cycles > 0) {
      if (INSN_UID(insn) >= FENCE_READY_TICKS_SIZE(fence)) {
        ...
        FENCE_READY_TICKS(fence) =
            (int *)xrecalloc(FENCE_READY_TICKS(fence), new_size,
                             FENCE_READY_TICKS_SIZE(fence), sizeof(int));
      }
      FENCE_READY_TICKS(fence)
      [INSN_UID(insn)] = FENCE_CYCLE(fence) + need_cycles;

      stalled++;
      min_need_stall =
          (min_need_stall < 0 ? need_cycles : MIN(min_need_stall, need_cycles));
...
static void convert_vec_av_set_to_ready(void) {
  int n;
  expr_t expr;
...
static expr_t fill_ready_list(av_set_t *av_ptr, blist_t bnds, fence_t fence,
                              int *pneed_stall) {
  expr_t expr;
...
static bool sel_dfa_new_cycle(insn_t insn, fence_t fence) {
  int last_scheduled_cycle =
  ...
  while (!sort_p && targetm.sched.dfa_new_cycle(sched_dump, sched_verbose, insn,
...
static int invoke_reorder_hooks(fence_t fence) {
  int issue_more;
  bool ran_hook = false;
  ...
  if (FENCE_ISSUED_INSNS(fence) == 0) {
    if (targetm.sched.reorder && !SCHED_GROUP_P(ready_element(&ready, 0)) &&
        ready.n_ready > 1) {
      ...
      issue_more = targetm.sched.reorder(sched_dump, sched_verbose,
                                         ready_lastpos(&ready), &ready.n_ready,
                                         FENCE_CYCLE(fence));
    ...
    if (ready.n_ready == 1)
      issue_more = targetm.sched.reorder2(sched_dump, sched_verbose,
                                          ready_lastpos(&ready), &ready.n_ready,
                                          FENCE_CYCLE(fence));
    else {
      if (pipelining_p)
...
/* Return an EXPR correponding to INDEX element of ready list, if
   ...
   ready.vec otherwise.  */
static inline expr_t find_expr_for_ready(int index, bool follow_ready_element) {
  expr_t expr;
  int real_index;
...
/* Calculate insns worth trying via lookahead_guard hook.  Return a number
   of such insns found.  */
static int invoke_dfa_lookahead_guard(void) {
  int i, n;
  bool have_hook =
...
static int calculate_privileged_insns(void) {
  ...
  if (sched_verbose >= 2)
    sel_print("privileged_n: %d insns with SPEC %d\n", privileged_n,
              privileged_n ? EXPR_SPEC(min_spec_expr) : -1);
  return privileged_n;
}
...
static int invoke_aftermath_hooks(fence_t fence, rtx best_insn,
                                  int issue_more) {
  ...
  if (targetm.sched.variable_issue) {
    memcpy(curr_state, FENCE_STATE(fence), dfa_state_size);
    issue_more = targetm.sched.variable_issue(sched_dump, sched_verbose,
                                              best_insn, issue_more);
    memcpy(FENCE_STATE(fence), curr_state, dfa_state_size);
...
static int estimate_insn_cost(rtx insn, state_t state) {
...
static int get_expr_cost(expr_t expr, fence_t fence) {
  rtx insn = EXPR_INSN_RTX(expr);

  if (recog_memoized(insn) < 0) {
    if (!FENCE_STARTS_CYCLE_P(fence)
        /* FIXME: Is this condition necessary?  */
        && VINSN_UNIQUE_P(EXPR_VINSN(expr)) && INSN_ASM_P(insn))
...
static int choose_best_insn(fence_t fence, int privileged_n, int *index) {
  int can_issue = 0;

  if (dfa_lookahead > 0) {
    cycle_issued_insns = FENCE_ISSUED_INSNS(fence);
    can_issue = max_issue(&ready, privileged_n, FENCE_STATE(fence), index);
    if (sched_verbose >= 2)
...
static expr_t find_best_expr(av_set_t *av_vliw_ptr, blist_t bnds, fence_t fence,
                             int *pneed_stall) {
  expr_t best;
  ...
  if (best != NULL) {
    can_issue_more =
        invoke_aftermath_hooks(fence, EXPR_INSN_RTX(best), can_issue_more);
    if (can_issue_more == 0)
      *pneed_stall = 1;
  }
...
/* Emit an instruction from EXPR with SEQNO and VINSN after
   PLACE_TO_INSERT.  */
static insn_t emit_insn_from_expr_after(expr_t expr, vinsn_t vinsn, int seqno,
  ...
  return sel_gen_insn_from_expr_after(expr, vinsn, seqno, place_to_insert);
}

/* Return TRUE if BB can hold bookkeeping code.  */
static bool block_valid_for_bookkeeping_p(basic_block bb) {
...
/* Attempt to find a block that can hold bookkeeping code for path(s) incoming
   ...
   one must be created.  */
static basic_block find_block_for_bookkeeping(edge e1, edge e2) {
  ...
  for (e = e1;; e = EDGE_SUCC(e->dest, 0)) {
    if (EDGE_COUNT(e->dest->preds) == 2) {
      if (candidate_block == NULL)
        candidate_block =
            (EDGE_PRED(e->dest, 0) == e ? EDGE_PRED(e->dest, 1)->src
    ...
    if (e == e2)
      return (block_valid_for_bookkeeping_p(candidate_block) ? candidate_block
                                                             : NULL);
  }
  gcc_unreachable();
}
...
static basic_block create_block_for_bookkeeping(edge e1, edge e2) {
  ...
  if (current_loop_nest) {
    ...
    gcc_assert(e1->dest != latch || !single_pred_p(latch) ||
...
static insn_t find_place_for_bookkeeping(edge e1, edge e2) {
  insn_t place_to_insert;
...
/* Find a proper seqno for bookkeeing insn inserted at PLACE_TO_INSERT
   for JOIN_POINT.   */
static int find_seqno_for_bookkeeping(insn_t place_to_insert,
                                      insn_t join_point) {
  int seqno;
  rtx next;
...
static insn_t emit_bookkeeping_insn(insn_t place_to_insert, expr_t c_expr,
                                    int new_seqno) {
  rtx new_insn_rtx = create_copy_of_insn_rtx(EXPR_INSN_RTX(c_expr));

  vinsn_t new_vinsn = create_vinsn_from_insn_rtx(
      new_insn_rtx, VINSN_UNIQUE_P(EXPR_VINSN(c_expr)));

  insn_t new_insn =
      emit_insn_from_expr_after(c_expr, new_vinsn, new_seqno, place_to_insert);
...
static basic_block generate_bookkeeping_insn(expr_t c_expr, edge e1, edge e2) {
  ...
  need_to_exchange_data_sets = sel_bb_empty_p(BLOCK_FOR_INSN(place_to_insert));
  ...
  if (need_to_exchange_data_sets)
    exchange_data_sets(BLOCK_FOR_INSN(new_insn), BLOCK_FOR_INSN(join_point));
...
static void remove_insns_that_need_bookkeeping(fence_t fence,
                                               av_set_t *av_ptr) {
  expr_t expr;
  av_set_iterator i;
  ...
  FOR_EACH_EXPR_1(expr, i, av_ptr) {
    if (!control_flow_insn_p(EXPR_INSN_RTX(expr)) &&
        ...
        (EXPR_SPEC(expr) || !EXPR_ORIG_BB_INDEX(expr) ||
         !dominated_by_p(CDI_DOMINATORS, BASIC_BLOCK(EXPR_ORIG_BB_INDEX(expr)),
                         BLOCK_FOR_INSN(FENCE_INSN(fence))))) {
      if (sched_verbose >= 4)
        sel_print(
            "Expr %d removed because it would need bookkeeping, which "
            "cannot be created\n",
            INSN_UID(EXPR_INSN_RTX(expr)));
      av_set_iter_remove(&i);
    }
...
/* Moving conditional jump through some instructions.
   ...
   to this new block.  And the result is this:
      ...
      ...
*/
static void move_cond_jump(rtx insn, bnd_t bnd) {
  edge ft_edge;
  ...
  gcc_assert(!sel_bb_empty_p(block_from) && !sel_bb_empty_p(block_new));
...
/* Remove nops generated during move_op for preventing removal of empty
   basic blocks.  */
static void remove_temp_moveop_nops(void) {
  int i;
  insn_t insn;
...
static void remove_insns_for_debug(blist_t bnds, av_set_t *av_vliw_p) {
...
static void compute_av_set_on_boundaries(fence_t fence, blist_t bnds,
                                         av_set_t *av_vliw_p) {
  if (sched_verbose >= 2) {
...
static av_set_t find_sequential_best_exprs(bnd_t bnd, expr_t expr_vliw,
                                           bool for_moveop) {
...
static void ATTRIBUTE_UNUSED move_nop_to_previous_block(insn_t nop,
                                                        basic_block prev_bb) {
  insn_t prev_insn, next_insn, note;

  gcc_assert(sel_bb_head_p(nop) && prev_bb == BLOCK_FOR_INSN(nop)->prev_bb);
  note = bb_note(BLOCK_FOR_INSN(nop));
  prev_insn = sel_bb_end(prev_bb);
  next_insn = NEXT_INSN(nop);
  gcc_assert(prev_insn != NULL_RTX && PREV_INSN(note) == prev_insn);
...
static insn_t prepare_place_to_insert(bnd_t bnd) {
  insn_t place_to_insert;
...
static bool move_exprs_to_boundary(bnd_t bnd, expr_t expr_vliw,
  ...
  EXECUTE_IF_SET_IN_BITMAP(current_copies, 0, book_uid, bi) {
    ...
    bitmap_copy(INSN_ORIGINATORS_BY_UID(book_uid), current_originators);
  }
...
static void debug_state(state_t state) {
  unsigned char *p;
...
static bool advance_state_on_fence(fence_t fence, insn_t insn) {
  bool asm_p;
...
static void update_fence_and_insn(fence_t fence, insn_t insn, int need_stall) {
  bool asm_p;
  ...
  INSN_READY_CYCLE(insn) =
      ...
      (INSN_CODE(insn) < 0 ? 1 : maximal_insn_latency(insn) + 1);
...
static blist_t *update_boundaries(bnd_t bnd, insn_t insn, blist_t *bndsp,
                                  blist_t *bnds_tailp) {
  succ_iterator si;
  insn_t succ;
...
static insn_t schedule_expr_on_boundary(bnd_t bnd, expr_t expr_vliw,
                                        int seqno) {
  av_set_t expr_seq;
  ...
  if (should_move)
    ...
    insn = emit_insn_from_expr_after(expr_vliw, NULL, seqno, place_to_insert);
  ...
  /* Check that the recent movement didn't destroyed loop
     structure.  */
  gcc_assert(!pipelining_p || current_loop_nest == NULL ||
             loop_latch_edge(current_loop_nest));
  return insn;
}

/* Stall for N cycles on FENCE.  */
static void stall_for_cycles(fence_t fence, int n) {
  int could_more;
...
static void fill_insns(fence_t fence, int seqno,
  ...
  do {
    ...
    if (sel_bb_end_p(insn) ||
        (was_stall && (was_stall >= max_stall || scheduled_insns >= max_insns)))
      break;
  } while (bnds);
...
static rtx get_dest_from_orig_ops(av_set_t orig_ops) {
  rtx dest = NULL_RTX;
  av_set_iterator av_it;
  expr_t expr;
  bool first_p = true;
  ...
  FOR_EACH_EXPR(expr, av_it, orig_ops) {
    ...
    if (first_p) {
      ...
      gcc_assert(dest == x ||
                 (dest != NULL_RTX && x != NULL_RTX && rtx_equal_p(dest, x)));
  }
...
static void update_and_record_unavailable_insns(basic_block book_block) {
  av_set_iterator i;
  ...
  if (AV_SET_VALID_P(sel_bb_head(book_block))) {
    ...
    FOR_EACH_EXPR(cur_expr, i, old_av_set) {
      expr_t new_expr =
          av_set_lookup(BB_AV_SET(book_block), EXPR_VINSN(cur_expr));
...
/* The main effect of this function is that sparams->c_expr is merged
   ...
   we avoid merging anything by copying sparams->c_expr to
   lparams->c_expr_merged. lparams->c_expr_merged is copied back to
   sparams->c_expr after all successors has been traversed.
   lparams->c_expr_local is an expr allocated on stack in the caller function,
   and is used if there is more than one successor.
...
static void move_op_merge_succs(insn_t insn ATTRIBUTE_UNUSED,
...
/*  Add used regs for the successor SUCC into SPARAMS->USED_REGS.
   ...
   SUCC is one of the SUCCS_NORMAL successors of INSN,
   MOVEOP_DRV_CALL_RES is the result of call code_motion_path_driver on succ or
   0, if SUCC is one of SUCCS_BACK or SUCCS_OUT. STATIC_PARAMS contain USED_REGS
   set.  */
static void fur_merge_succs(insn_t insn ATTRIBUTE_UNUSED, insn_t succ,
                            int moveop_drv_call_res,
...
/* This function is called after the last successor.  Copies LP->C_EXPR_MERGED
   into SP->CEXPR.  */
static void move_op_after_merge_succs(cmpd_local_params_p lp, void *sparams) {
...
static void track_scheduled_insns_and_blocks(rtx insn) {
  /* Even if this insn can be a copy that will be removed during current
     move_op, we still need to count it as an originator.  */
...
static bool maybe_emit_renaming_copy(rtx insn, moveop_static_params_p params) {
...
/* Emit a speculative check for INSN speculated as EXPR if needed.
   ...
   parameters.  */
static bool maybe_emit_speculative_check(rtx insn, expr_t expr,
...
static bool handle_emitting_transformations(rtx insn, expr_t expr,
                                            moveop_static_params_p params) {
  bool insn_emitted = false;
...
static void remove_insn_from_stream(rtx insn, bool only_disconnect) {
  ...
  need_nop_to_preserve_bb = ((bb_head == bb_end) ||
                             (NEXT_INSN(bb_head) == bb_end && JUMP_P(bb_end)) ||
...
static void move_op_orig_expr_found(
    insn_t insn, expr_t expr, cmpd_local_params_p lparams ATTRIBUTE_UNUSED,
    void *static_params) {
  ...
  only_disconnect = (params->uid == INSN_UID(insn) && !insn_emitted &&
...
static void fur_orig_expr_found(insn_t insn, expr_t expr ATTRIBUTE_UNUSED,
                                cmpd_local_params_p lparams ATTRIBUTE_UNUSED,
                                void *static_params) {
  ...
  gcc_assert(!VINSN_SEPARABLE_P(INSN_VINSN(insn)) || !MEM_P(INSN_LHS(insn)));
}

/* This function is called on the ascending pass, before returning from
   current basic block.  */
static void move_op_at_first_insn(insn_t insn, cmpd_local_params_p lparams,
                                  void *static_params) {
  moveop_static_params_p sparams = (moveop_static_params_p)static_params;
  ...
  if (!lparams->removed_last_insn && lparams->e1 && sel_bb_head_p(insn)) {
    /* We should generate bookkeeping code only if we are not at the
       top level of the move_op.  */
    if (sel_num_cfg_preds_gt_1(insn))
      book_block =
          generate_bookkeeping_insn(sparams->c_expr, lparams->e1, lparams->e2);
  ...
  /* If bookkeeping code was inserted, we need to update av sets of basic
     ...
     bookkeeping block does not contain valid av set because we are not
     following the original algorithm in every detail with regards to e.g.
     renaming simple reg-reg copies.  Consider example:
...
/* This function is called on the ascending pass, before returning from the
   current basic block.  */
static void fur_at_first_insn(insn_t insn,
                              cmpd_local_params_p lparams ATTRIBUTE_UNUSED,
...
/* Called on the backward stage of recursion to call moveup_expr for insn
   and sparams->c_expr.  */
static void move_op_ascend(insn_t insn, void *static_params) {
  enum MOVEUP_EXPR_CODE res;
  moveop_static_params_p sparams = (moveop_static_params_p)static_params;
...
static int fur_on_enter(insn_t insn ATTRIBUTE_UNUSED,
                        cmpd_local_params_p local_params, void *static_params,
                        bool visited_p) {
  fur_static_params_p sparams = (fur_static_params_p)static_params;
...
static int move_op_on_enter(insn_t insn ATTRIBUTE_UNUSED,
...
static bool move_op_orig_expr_not_found(insn_t insn,
...
static bool fur_orig_expr_not_found(insn_t insn, av_set_t orig_ops,
                                    void *static_params) {
  bool mutexed;
  expr_t r;
  av_set_iterator avi;
...
static int code_motion_process_successors(insn_t insn, av_set_t orig_ops,
...
rescan:
  ...
  FOR_EACH_SUCC_1(succ, succ_i, insn,
                  code_motion_path_driver_info->succ_flags) {
    int b;
  ...
  /* Here, RES==1 if original expr was found at least for one of the
     ...
     move_op.  */
  gcc_assert(res == 1 ||
             (res == 0 && av_set_could_be_blocked_by_bookkeeping_p(
                              orig_ops, static_params)) ||
             res == -1);
#endif
...
static inline void code_motion_path_driver_cleanup(av_set_t *orig_ops_p,
                                                   ilist_t *path_p) {
  ilist_remove(path_p);
...
static int code_motion_path_driver(insn_t insn, av_set_t orig_ops, ilist_t path,
  ...
  if (sel_bb_head_p(insn)) {
    ...
    if (bitmap_bit_p(code_motion_visited_blocks, bb->index)) {
      ...
      if (code_motion_path_driver_info->on_enter)
        return code_motion_path_driver_info->on_enter(insn, local_params_in,
                                                      static_params, true);
    }
  }

  if (code_motion_path_driver_info->on_enter)
    code_motion_path_driver_info->on_enter(insn, local_params_in, static_params,
                                           false);
  ...
  for (;;) {
    ...
    if (expr) {
      ...
      code_motion_path_driver_info->orig_expr_found(insn, expr, local_params_in,
                                                    static_params);
      ...
      /* We haven't found the original expr, continue descending the basic
         block.  */
      if (code_motion_path_driver_info->orig_expr_not_found(insn, orig_ops,
                                                            static_params)) {
  ...
  if (!expr) {
    ...
    res = code_motion_process_successors(insn, orig_ops, path, static_params);
  ...
  code_motion_path_driver_info->at_first_insn(insn, local_params_in,
                                              static_params);
...
static bool move_op(insn_t insn, av_set_t orig_ops, expr_t expr_vliw, rtx dest,
                    expr_t c_expr, bool *should_move) {
...
static void init_seqno_1(basic_block bb, sbitmap visited_bbs,
...
static int init_seqno(int number_of_insns, bitmap blocks_to_reschedule,
                      basic_block from) {
  sbitmap visited_bbs;
  bitmap_iterator bi;
  unsigned bbi;
...
static void sel_setup_region_sched_flags(void) {
...
static bool current_region_empty_p(void) {
  int i;
...
static void setup_current_loop_nest(int rgn) {
...
static void purge_empty_blocks(void) {
  int i;
...
static void sel_compute_priorities(int rgn) {
...
/* Init scheduling data for RGN.  Returns true when this region should not
   be scheduled.  */
static bool sel_region_init(int rgn) {
  int i;
  bb_vec_t bbs;
  ...
  if (current_loop_nest) {
    int header = (sel_is_loop_preheader_p(BASIC_BLOCK(BB_TO_BLOCK(0))) ? 1 : 0);

    if (current_nr_blocks == header + 1)
      update_liveness_on_insn(sel_bb_head(BASIC_BLOCK(BB_TO_BLOCK(header))));
  }
...
static void simplify_changed_insns(void) {
  int i;
...
static void find_ebb_boundaries(basic_block bb, bitmap scheduled_blocks) {
  insn_t head, tail;
  basic_block bb1 = bb;
  if (sched_verbose >= 2)
...
static void reset_sched_cycles_in_current_ebb(void) {
  int last_clock = 0;
  ...
  for (insn = current_sched_info->head; insn != current_sched_info->next_tail;
    ...
    if (haifa_cost > 0) {
      ...
      while (haifa_cost--) {
        ...
        if (!after_stall && real_insn && haifa_cost > 0 &&
...
static void put_TImodes(void) {
  ...
  for (insn = current_sched_info->head; insn != current_sched_info->next_tail;
    ...
    if (issue_rate > 1 && GET_CODE(PATTERN(insn)) != USE &&
...
static void sel_region_target_finish(bool reset_sched_cycles_p) {
  int i;
...
static void sel_region_finish(bool reset_sched_cycles_p) {
...
static void schedule_on_fences(flist_t fences, int max_seqno,
                               ilist_t **scheduled_insns_tailpp) {
...
static void find_min_max_seqno(flist_t fences, int *min_seqno, int *max_seqno) {
...
static flist_t calculate_new_fences(flist_t fences, int orig_max_seqno) {
  flist_t old_fences = fences;
...
static int update_seqnos_and_stage(int min_seqno, int max_seqno,
  ...
  if (*pscheduled_insns) {
    new_hs = (INSN_SEQNO(ILIST_INSN(*pscheduled_insns)) + highest_seqno_in_use +
              max_seqno - min_seqno + 2);
...
/* The main driver for scheduling a region.  This function is responsible
   ...
   of scheduling.  */
static void sel_sched_region_2(int orig_max_seqno) {
  ...
  while (fences) {
    ...
    highest_seqno_in_use = update_seqnos_and_stage(
        min_seqno, max_seqno, highest_seqno_in_use, &scheduled_insns);
  }

  if (sched_verbose >= 1)
    sel_print(
        "Scheduled %d bookkeeping copies, %d insns needed "
        "bookkeeping, %d insns renamed, %d insns substituted\n",
        stat_bookkeeping_copies, stat_insns_needed_bookkeeping,
        stat_renamed_scheduled, stat_substitutions_total);
}
    ...
    while (do_p) {
      ...
      for (i = 0; i < current_nr_blocks; i++) {
        ...
        if (bitmap_bit_p(blocks_to_reschedule, bb->index)) {
          ...
          if (sel_insn_is_speculation_check(BB_END(bb)) && JUMP_P(BB_END(bb)))
            bitmap_set_bit(blocks_to_reschedule, BRANCH_EDGE(bb)->dest->index);
...
static void sel_global_init(void) {
...
static void sel_global_finish(void) {
  free_bb_note_pool();
  free_lv_sets();
