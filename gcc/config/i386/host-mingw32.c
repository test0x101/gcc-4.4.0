...
static inline void w32_error(const char* function, const char* file, int line,
                             const char* my_msg) {
  LPSTR w32_msgbuf;
  FormatMessageA(
  ...
  fprintf(stderr, "internal error in %s, at %s:%d: %s: %s\n", function,
          trim_filename(file), line, my_msg, w32_msgbuf);
...
static void* mingw32_gt_pch_get_address(size_t size, int fd ATTRIBUTE_UNUSED) {
  void* res;
  ...
  res = VirtualAlloc(NULL, pch_VA_max_size, MEM_RESERVE | MEM_TOP_DOWN,
                     PAGE_NOACCESS);
  if (!res)
...
static int mingw32_gt_pch_use_address(void* addr, size_t size, int fd,
                                      size_t offset) {
  void* mmap_addr;
  HANDLE mmap_handle;
...
#define OBJECT_NAME_FMT "Local\\MinGWGCCPCH-"
  ...
  mmap_handle = CreateFileMappingA(
      (HANDLE)_get_osfhandle(fd), NULL, PAGE_WRITECOPY | SEC_COMMIT, 0, 0,
      version_info.dwMajorVersion > 4 ? object_name : NULL);
  if (mmap_handle == NULL) {
    w32_error(__FUNCTION__, __FILE__, __LINE__, "CreateFileMapping");
    return -1;
  }
  mmap_addr =
      MapViewOfFileEx(mmap_handle, FILE_MAP_COPY, 0, offset, size, addr);
  if (mmap_addr != addr) {
