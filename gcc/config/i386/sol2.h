...
#define SUBTARGET_EXTRA_SPECS                                       \
  {"cpp_subtarget", CPP_SUBTARGET_SPEC}, {"asm_cpu", ASM_CPU_SPEC}, \
...
#define ASM_OUTPUT_DEF_FROM_DECLS(FILE, DECL, TARGET)                     \
  do {                                                                    \
    const char *declname = IDENTIFIER_POINTER(DECL_ASSEMBLER_NAME(DECL)); \
    ASM_OUTPUT_DEF((FILE), declname, IDENTIFIER_POINTER(TARGET));         \
    if (!TREE_PUBLIC(DECL)) {                                             \
