...
static tree gen_stdcall_or_fastcall_decoration(tree decl, tree id,
                                               char prefix) {
  ...
  if (prototype_p(type)) {
    ...
    FOREACH_FUNCTION_ARGS(type, arg, args_iter) {
      ...
      total += (parm_size + parm_boundary_bytes - 1) / parm_boundary_bytes *
               parm_boundary_bytes;
    }
  }

  new_str = XALLOCAVEC(char, 1 + strlen(old_str) + 1 + 10 + 1);
  sprintf(new_str, "%c%s@" HOST_WIDE_INT_PRINT_UNSIGNED, prefix, old_str,
          total);
...
static tree gen_regparm_prefix(tree decl, tree id, unsigned int nregs) {
  unsigned HOST_WIDE_INT total = 0;
  const char *old_str =
      IDENTIFIER_POINTER(id != NULL_TREE ? id : DECL_NAME(decl));
  char *new_str;
...
static tree i386_nlm_maybe_mangle_decl_assembler_name(tree decl, tree id) {
  ...
  if (lookup_attribute("stdcall", type_attributes))
    ...
    new_id = gen_regparm_prefix(
        decl, id, TREE_INT_CST_LOW(TREE_VALUE(TREE_VALUE(new_id))));
  else
    new_id = NULL_TREE;
...
tree i386_nlm_mangle_decl_assembler_name(tree decl, tree id) {
...
void i386_nlm_encode_section_info(tree decl, rtx rtl, int first) {
...
const char *i386_nlm_strip_name_encoding(const char *str) {
...
void netware_override_options(void) {
  override_options();
