...
static char *describe_cache(struct cache_desc level1,
...
static void detect_l2_cache(struct cache_desc *level2) {
...
static const char *detect_caches_amd(unsigned max_ext_level) {
  unsigned eax, ebx, ecx, edx;
...
static void decode_caches_intel(unsigned reg, bool xeon_mp,
...
static void detect_caches_cpuid2(bool xeon_mp, struct cache_desc *level1,
                                 struct cache_desc *level2) {
  unsigned regs[4];
  int nreps, i;
...
static void detect_caches_cpuid4(struct cache_desc *level1,
                                 struct cache_desc *level2) {
    ...
    switch (eax & 0x1f) {
      ...
      case CACHE_UNIFIED: {
        ...
        if (cache) {
          ...
          cache->sizekb = (cache->assoc * part * cache->line * sets) / 1024;
        }
      }
      default:
        break;
...
static const char *detect_caches_intel(bool xeon_mp, unsigned max_level,
