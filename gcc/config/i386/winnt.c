...
tree ix86_handle_shared_attribute(tree *node, tree name,
...
/* Handle a "selectany" attribute;
   arguments as in struct attribute_spec.handler.  */
tree ix86_handle_selectany_attribute(tree *node, tree name,
                                     tree args ATTRIBUTE_UNUSED,
                                     int flags ATTRIBUTE_UNUSED,
                                     bool *no_add_attrs) {
...
static tree associated_type(tree decl) {
  return (DECL_CONTEXT(decl) && TYPE_P(DECL_CONTEXT(decl)) ? DECL_CONTEXT(decl)
                                                           : NULL_TREE);
}

/* Return true if DECL should be a dllexport'd object.  */

static bool i386_pe_determine_dllexport_p(tree decl) {
  tree assoc;
...
static bool i386_pe_determine_dllimport_p(tree decl) {
  tree assoc;
...
bool i386_pe_valid_dllimport_attribute_p(const_tree decl) {
...
static tree gen_stdcall_or_fastcall_suffix(tree decl, tree id, bool fastcall) {
  HOST_WIDE_INT total = 0;
  const char *old_str =
      IDENTIFIER_POINTER(id != NULL_TREE ? id : DECL_NAME(decl));
  char *new_str, *p;
  ...
  if (prototype_p(type)) {
    ...
    FOREACH_FUNCTION_ARGS(type, arg, args_iter) {
      ...
      parm_size = ((parm_size + parm_boundary_bytes - 1) / parm_boundary_bytes *
                   parm_boundary_bytes);
      total += parm_size;
    }
  }
...
static tree i386_pe_maybe_mangle_decl_assembler_name(tree decl, tree id) {
...
tree i386_pe_mangle_decl_assembler_name(tree decl, tree id) {
  tree new_id = i386_pe_maybe_mangle_decl_assembler_name(decl, id);

  return (new_id ? new_id : id);
}

void i386_pe_encode_section_info(tree decl, rtx rtl, int first) {
  rtx symbol;
  int flags;
    ...
    case FUNCTION_DECL:
      ...
      if (strcmp(lang_hooks.name, "GNU Ada") == 0) {
        ...
        if (!(*asm_str == '*' || strchr(asm_str, '@')) &&
            (new_id = i386_pe_maybe_mangle_decl_assembler_name(decl, old_id)))
    ...
    case VAR_DECL:
      if (lookup_attribute("selectany", DECL_ATTRIBUTES(decl))) {
        if (DECL_INITIAL(decl)
          ...
          error(
              "%q+D:'selectany' attribute applies only to "
              "initialized objects",
              decl);
      }
      break;
...
bool i386_pe_binds_local_p(const_tree exp) {
...
const char *i386_pe_strip_name_encoding_full(const char *str) {
  const char *p;
...
void i386_pe_unique_section(tree decl, int reloc) {
  int len;
...
unsigned int i386_pe_section_type_flags(tree decl, const char *name,
                                        int reloc) {
  static htab_t htab;
  unsigned int flags;
  unsigned int **slot;
...
void i386_pe_asm_named_section(const char *name, unsigned int flags,
                               tree decl) {
...
void i386_pe_asm_output_aligned_decl_common(
    FILE *stream, tree decl, const char *name, HOST_WIDE_INT size,
  ...
  fprintf(stream,
          ", " HOST_WIDE_INT_PRINT_DEC "\t" ASM_COMMENT_START
...
void i386_pe_declare_function_type(FILE *file, const char *name, int pub) {
  fprintf(file, "\t.def\t");
  assemble_name(file, name);
  fprintf(file, ";\t.scl\t%d;\t.type\t%d;\t.endef\n",
          pub ? (int)C_EXT : (int)C_STAT, (int)DT_FCN << N_BTSHFT);
}
...
void i386_pe_record_external_function(tree decl, const char *name) {
...
void i386_pe_maybe_record_exported_symbol(tree decl, const char *name,
                                          int is_data) {
  rtx symbol;
...
void i386_pe_file_end(void) {
  ...
  for (p = extern_head; p != NULL; p = p->next) {
    ...
    if (!TREE_ASM_WRITTEN(decl) &&
      ...
      i386_pe_declare_function_type(asm_out_file, p->name, TREE_PUBLIC(decl));
    }
  }
