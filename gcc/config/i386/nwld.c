...
void nwld_named_section_asm_out_constructor(rtx symbol, int priority) {
#if !SUPPORTS_INIT_PRIORITY
  const char section[] = ".ctors" TARGET_SUB_SECTION_SEPARATOR;
#else
  char section[20];

  sprintf(section, ".ctors" TARGET_SUB_SECTION_SEPARATOR "%.5u",
...
void nwld_named_section_asm_out_destructor(rtx symbol, int priority) {
#if !SUPPORTS_INIT_PRIORITY
