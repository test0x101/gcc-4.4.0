...
char cygwin_gplusplus_backward_include_dir
    [sizeof(GPLUSPLUS_BACKWARD_INCLUDE_DIR) + 1 + CYGWIN_MINGW_SUBDIR_LEN] =
        GPLUSPLUS_BACKWARD_INCLUDE_DIR;
#undef GPLUSPLUS_BACKWARD_INCLUDE_DIR
#define GPLUSPLUS_BACKWARD_INCLUDE_DIR \
  ((const char *)cygwin_gplusplus_backward_include_dir)
#ifndef GEN_CVT_ARRAY
#define GEN_CVT_ARRAY
#endif
#endif
...
char cygwin_local_include_dir[sizeof(LOCAL_INCLUDE_DIR) + 1 +
                              CYGWIN_MINGW_SUBDIR_LEN] = LOCAL_INCLUDE_DIR;
...
char cygwin_cross_include_dir[sizeof(CROSS_INCLUDE_DIR) + 1 +
                              CYGWIN_MINGW_SUBDIR_LEN] = CROSS_INCLUDE_DIR;
...
char cygwin_tool_include_dir[sizeof(TOOL_INCLUDE_DIR) + 1 +
                             CYGWIN_MINGW_SUBDIR_LEN] = TOOL_INCLUDE_DIR;
...
#define GCC_DRIVER_HOST_INITIALIZATION                                         \
  do {                                                                         \
    ...
    add_prefix(&exec_prefixes, cprefix, "BINUTILS", PREFIX_PRIORITY_LAST, 0,   \
               NULL);                                                          \
    add_prefix(&startfile_prefixes,                                            \
               concat(standard_startfile_prefix, "w32api", NULL), "GCC",       \
