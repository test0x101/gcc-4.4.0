...
bool i386_pe_type_dllimport_p(tree decl) {
  ...
  if (TREE_CODE(decl) == FUNCTION_DECL &&
      (DECL_DECLARED_INLINE_P(decl) || DECL_TEMPLATE_INSTANTIATION(decl) ||
...
bool i386_pe_type_dllexport_p(tree decl) {
  gcc_assert(TREE_CODE(decl) == VAR_DECL || TREE_CODE(decl) == FUNCTION_DECL);
  ...
  if (TREE_CODE(TREE_TYPE(decl)) == METHOD_TYPE && DECL_ARTIFICIAL(decl) &&
      !DECL_THUNK_P(decl))
    return false;
  return true;
}
...
void i386_pe_adjust_class_at_definition(tree t) {
  tree member;
