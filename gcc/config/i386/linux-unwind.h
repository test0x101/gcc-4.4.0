...
static _Unwind_Reason_Code x86_64_fallback_frame_state(
...
static _Unwind_Reason_Code x86_fallback_frame_state(
    struct _Unwind_Context *context, _Unwind_FrameState *fs) {
  unsigned char *pc = context->ra;
  struct sigcontext *sc;
  long new_cfa;
  ...
  if (*(unsigned short *)(pc + 0) == 0xb858 &&
      *(unsigned int *)(pc + 2) == 119 && *(unsigned short *)(pc + 6) == 0x80cd)
    sc = context->cfa + 4;
