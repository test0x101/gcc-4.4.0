...
static bool gate_tree_loop(void) {
  return flag_tree_loop_optimize != 0;
}

struct gimple_opt_pass pass_tree_loop = {{
    GIMPLE_PASS, "loop", /* name */
    gate_tree_loop,      /* gate */
    NULL,                /* execute */
    NULL,                /* sub */
    NULL,                /* next */
...
static unsigned int tree_ssa_loop_init(void) {
...
static unsigned int tree_ssa_loop_im(void) {
  if (number_of_loops() <= 1)
    return 0;
...
static bool gate_tree_ssa_loop_im(void) {
  return flag_tree_loop_im != 0;
}

struct gimple_opt_pass pass_lim = {{
    GIMPLE_PASS, "lim",                /* name */
...
static unsigned int tree_ssa_loop_unswitch(void) {
  if (number_of_loops() <= 1)
    return 0;
...
static unsigned run_tree_predictive_commoning(void) {
  if (!current_loops)
    return 0;
...
static bool gate_tree_predictive_commoning(void) {
  return flag_predictive_commoning != 0;
}

struct gimple_opt_pass pass_predcom = {{
    GIMPLE_PASS, "pcom",            /* name */
...
static unsigned int tree_vectorize(void) {
  if (number_of_loops() <= 1)
    return 0;
...
static unsigned int tree_linear_transform(void) {
  if (number_of_loops() <= 1)
    return 0;
...
struct gimple_opt_pass pass_linear_transform = {{
    ...
    0,                          /* properties_destroyed */
    0,                          /* todo_flags_start */
    TODO_dump_func | TODO_verify_loops | TODO_update_ssa_only_virtuals |
        TODO_ggc_collect /* todo_flags_finish */
}};

/* GRAPHITE optimizations.  */

static unsigned int graphite_transforms(void) {
  if (!current_loops)
    return 0;
...
static unsigned int check_data_deps(void) {
  if (number_of_loops() <= 1)
    return 0;
...
static unsigned int tree_ssa_loop_ivcanon(void) {
  if (number_of_loops() <= 1)
    return 0;
...
static bool gate_tree_ssa_loop_ivcanon(void) {
  return flag_tree_loop_ivcanon != 0;
}

struct gimple_opt_pass pass_iv_canon = {{
    GIMPLE_PASS, "ivcanon",            /* name */
...
static bool gate_scev_const_prop(void) {
  return flag_tree_scev_cprop;
}

struct gimple_opt_pass pass_scev_cprop = {{
    GIMPLE_PASS, "sccp",  /* name */
    ...
    0,                    /* properties_destroyed */
    0,                    /* todo_flags_start */
    TODO_dump_func | TODO_cleanup_cfg | TODO_update_ssa_only_virtuals
    /* todo_flags_finish */
}};

/* Remove empty loops.  */

static unsigned int tree_ssa_empty_loop(void) {
  if (number_of_loops() <= 1)
    return 0;
...
static unsigned int tree_ssa_loop_bounds(void) {
  if (number_of_loops() <= 1)
    return 0;
...
static unsigned int tree_complete_unroll(void) {
  if (number_of_loops() <= 1)
    return 0;

  return tree_unroll_loops_completely(
      flag_unroll_loops || flag_peel_loops || optimize >= 3, true);
}

static bool gate_tree_complete_unroll(void) {
  return true;
}

struct gimple_opt_pass pass_complete_unroll = {{
    GIMPLE_PASS, "cunroll",    /* name */
...
static unsigned int tree_complete_unroll_inner(void) {
  unsigned ret = 0;
...
static bool gate_tree_complete_unroll_inner(void) {
  return optimize >= 2;
}

struct gimple_opt_pass pass_complete_unrolli = {{
    GIMPLE_PASS, "cunrolli",         /* name */
...
static bool gate_tree_parallelize_loops(void) {
  return flag_tree_parallelize_loops > 1;
}

static unsigned tree_parallelize_loops(void) {
  if (number_of_loops() <= 1)
    return 0;
...
static unsigned int tree_ssa_loop_prefetch(void) {
  if (number_of_loops() <= 1)
    return 0;
...
static bool gate_tree_ssa_loop_prefetch(void) {
  return flag_prefetch_loop_arrays != 0;
}

struct gimple_opt_pass pass_loop_prefetch = {{
    GIMPLE_PASS, "aprefetch",          /* name */
...
static unsigned int tree_ssa_loop_ivopts(void) {
  if (number_of_loops() <= 1)
    return 0;
...
static bool gate_tree_ssa_loop_ivopts(void) {
  return flag_ivopts != 0;
}

struct gimple_opt_pass pass_iv_optimize = {{
    GIMPLE_PASS, "ivopts",     /* name */
    ...
    0,                         /* properties_destroyed */
    0,                         /* todo_flags_start */
    TODO_dump_func | TODO_verify_loops | TODO_update_ssa |
        TODO_ggc_collect /* todo_flags_finish */
}};
