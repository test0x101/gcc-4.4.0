...
#define TRISTATE_AND(a, b) \
  ((a) == I ? ((b) == N ? N : I) : (b) == I ? ((a) == N ? N : I) : (a) && (b))

#define TRISTATE_OR(a, b) \
  ((a) == I ? ((b) == Y ? Y : I) : (b) == I ? ((a) == Y ? Y : I) : (a) || (b))
...
static void compute_predicate_codes(rtx exp, char codes[NUM_RTX_CODE]) {
  ...
  switch (GET_CODE(exp)) {
    ...
    case IF_THEN_ELSE:
      ...
      for (i = 0; i < NUM_RTX_CODE; i++)
        codes[i] =
            TRISTATE_OR(TRISTATE_AND(op0_codes[i], op1_codes[i]),
                        TRISTATE_AND(TRISTATE_NOT(op0_codes[i]), op2_codes[i]));
      break;
        ...
        while ((code = scan_comma_elt(&next_code)) != 0) {
          ...
          if (!found_it) {
            message_with_line(pattern_lineno,
                              "match_code \"%.*s\" matches nothing", (int)n,
                              code);
            error_count++;
            for (i = 0; i < NUM_RTX_CODE; i++)
              if (!strncasecmp(code, GET_RTX_NAME(i), n) &&
                  GET_RTX_NAME(i)[n] == '\0' && !did_you_mean_codes[i]) {
                did_you_mean_codes[i] = 1;
                message_with_line(pattern_lineno, "(did you mean \"%s\"?)",
                                  GET_RTX_NAME(i));
              }
          }
        }
...
static struct decision_test *new_decision_test(enum decision_type,
                                               struct decision_test ***);
...
static struct decision *add_to_sequence(rtx, struct decision_head *,
                                        const char *, enum routine_type, int);
...
static void write_action(struct decision *, struct decision_test *, int, int,
...
static void write_tree(struct decision_head *, const char *, enum routine_type,
                       int);
...
static rtx find_operand(rtx pattern, int n, rtx stop) {
  ...
  if ((code == MATCH_SCRATCH || code == MATCH_OPERAND ||
       code == MATCH_OPERATOR || code == MATCH_PARALLEL) &&
      XINT(pattern, 0) == n)
    return pattern;
    ...
    switch (fmt[i]) {
      ...
      case 'E':
        for (j = 0; j < XVECLEN(pattern, i); j++)
          if ((r = find_operand(XVECEXP(pattern, i, j), n, stop)) != NULL_RTX)
            return r;
        break;
...
static rtx find_matching_operand(rtx pattern, int n) {
  ...
  if (code == MATCH_OPERAND &&
      (XSTR(pattern, 2)[0] == '0' + n ||
       (XSTR(pattern, 2)[0] == '%' && XSTR(pattern, 2)[1] == '0' + n)))
    return pattern;
  ...
  switch (code) {
    ...
    case MATCH_OPERATOR: {
      ...
      if (pred_name[0] != 0) {
        ...
        if (!pred)
          message_with_line(pattern_lineno, "warning: unknown predicate '%s'",
                            pred_name);
      } else
        pred = 0;
      ...
      if (code == MATCH_OPERAND) {
        ...
        if (GET_CODE(insn) == DEFINE_EXPAND || GET_CODE(insn) == DEFINE_SPLIT ||
      ...
      if (GET_MODE(pattern) == VOIDmode && code == MATCH_OPERAND &&
          GET_CODE(insn) == DEFINE_INSN && pred && !pred->special &&
          pred->allows_non_const && strstr(c_test, "operands") == NULL &&
          !(set && GET_CODE(set) == SET && GET_CODE(SET_SRC(set)) == CALL))
        message_with_line(pattern_lineno, "warning: operand %d missing mode?",
                          XINT(pattern, 0));
      return;
    }
    ...
    case SET: {
      ...
      if (GET_CODE(dest) == MATCH_DUP || GET_CODE(dest) == MATCH_OP_DUP ||
          GET_CODE(dest) == MATCH_PAR_DUP)
        dest = find_operand(insn, XINT(dest, 0), NULL);

      if (GET_CODE(src) == MATCH_DUP || GET_CODE(src) == MATCH_OP_DUP ||
      ...
      else if (dmode != smode && GET_CODE(dest) != PC &&
               GET_CODE(dest) != CC0 && GET_CODE(src) != PC &&
               GET_CODE(src) != CC0 && GET_CODE(src) != CONST_INT) {
        const char *which;
        which = (dmode == VOIDmode ? "destination" : "source");
        message_with_line(pattern_lineno, "warning: %s missing a mode?", which);
      }
...
static struct decision *add_to_sequence(rtx pattern, struct decision_head *last,
                                        const char *position,
...
restart:
    ...
    case PARALLEL:
      ...
      if (insn_type == PEEPHOLE2 && top) {
        ...
        for (i = 0; i < (size_t)XVECLEN(pattern, 0); i++) {
          ...
          sub = add_to_sequence(XVECEXP(pattern, 0, i), last, subpos, insn_type,
                                0);
          last = &sub->success;
        }
        goto ret;
    ...
    case MATCH_OPERATOR: {
      ...
      if (pred_name[0] != 0) {
        ...
        if (pred) {
          ...
          if (was_code == MATCH_PARALLEL && pred->singleton != PARALLEL)
            message_with_line(pattern_lineno,
          ...
          message_with_line(
              pattern_lineno,
              "warning: unknown predicate '%s' in '%s' expression", pred_name,
      ...
      if (was_code == MATCH_OPERATOR || was_code == MATCH_PARALLEL) {
        ...
        for (i = 0; i < (size_t)XVECLEN(pattern, 2); i++) {
          subpos[depth] = i + base;
          sub = add_to_sequence(XVECEXP(pattern, 2, i), &sub->success, subpos,
                                insn_type, 0);
        }
      }
      goto fini;
  ...
  for (i = 0; i < (size_t)len; i++) {
    if (fmt[i] == 'i') {
      ...
      enum decision_type type = ((int)XWINT(pattern, i) == XWINT(pattern, i))
                                    ? DT_elt_zero_wide_safe
                                    : DT_elt_zero_wide;
  ...
  for (i = 0; i < (size_t)len; i++) {
    switch (fmt[i]) {
      ...
      case 'E': {
        ...
        for (j = 0; j < XVECLEN(pattern, i); j++) {
          subpos[depth] = 'a' + j;
          sub = add_to_sequence(XVECEXP(pattern, i, j), &sub->success, subpos,
                                insn_type, 0);
        }
        break;
      }
...
static int maybe_both_true(struct decision *d1, struct decision *d2,
                           int toplevel) {
...
static int nodes_identical(struct decision *d1, struct decision *d2) {
  ...
  /* Check that their subnodes are at the same position, as any one set
     ...
     invoked.  */
  if (d1->success.first && d2->success.first &&
...
static void factor_tests(struct decision_head *head) {
  ...
  for (first = head->first; first && first->next; first = next) {
    ...
    /* Don't want all node types, just those we can turn into
       switch statements.  */
    if (type != DT_mode && type != DT_code && type != DT_veclen &&
        type != DT_elt_zero_int && type != DT_elt_one_int &&
...
static void change_state(const char *oldpos, const char *newpos,
                         const char *indent) {
...
static void write_afterward(struct decision *start, struct decision *afterward,
                            const char *indent) {
...
static struct decision *write_switch(struct decision *start, int depth) {
  ...
  if (!p->next || p->tests->next || p->next->tests->type != type ||
  ...
  if (type == DT_code) {
    ...
    do {
      ...
      codemap[code] = 1;
      p = p->next;
    } while (p && !p->tests->next && p->tests->type == DT_code &&
  ...
  pred_done:
    ...
    return ret;
  } else if (type == DT_mode || type == DT_veclen || type == DT_elt_zero_int ||
...
static void write_cond(struct decision_test *p, int depth,
...
static void write_action(struct decision *p, struct decision_test *test,
      ...
      case RECOG:
        if (test->u.insn.num_clobbers_to_add != 0)
          printf("%s*pnum_clobbers = %d;\n", indent,
                 test->u.insn.num_clobbers_to_add);
...
static int write_node(struct decision *p, int depth,
                      enum routine_type subroutine_type) {
  ...
  for (test = p->tests; test; test = test->next) {
    if (test->type == DT_code && test->u.code == CONST_INT && test->next &&
...
static void write_tree_1(struct decision_head *head, int depth,
                         enum routine_type subroutine_type) {
...
static void write_tree(struct decision_head *head, const char *prevpos,
  ...
  if (!initial && p->subroutine_number > 0) {
    static const char *const name_prefix[] = {"recog", "split", "peephole2"};

    static const char *const call_suffix[] = {", pnum_clobbers", "",
                                              ", _pmatch_len"};
    ...
    if (p->afterward) {
      printf("  tem = %s_%d (x0, insn%s);\n", name_prefix[type],
             p->subroutine_number, call_suffix[type]);
      if (IS_SPLIT(type))
        printf("  if (tem != 0)\n    return tem;\n");
      else
        printf("  if (tem >= 0)\n    return tem;\n");
      ...
      if (GET_CODE(x) == PARALLEL) {
        ...
        for (i = XVECLEN(x, 0); i > 0; i--) {
          ...
          if (GET_CODE(y) != CLOBBER ||
              (!REG_P(XEXP(y, 0)) && GET_CODE(XEXP(y, 0)) != MATCH_SCRATCH))
            break;
        }
...
static void debug_decision_2(struct decision_test *test) {
  switch (test->type) {
    ...
    case DT_dup:
      ...
      break;
    case DT_pred:
      fprintf(stderr, "pred=(%s,%s)", test->u.pred.name,
              GET_MODE_NAME(test->u.pred.mode));
      break;
    case DT_c_test: {
      char sub[16 + 4];
    ...
    case DT_accept_op:
      ...
      break;
    case DT_accept_insn:
      fprintf(stderr, "A_insn=(%d,%d)", test->u.insn.code_number,
              test->u.insn.num_clobbers_to_add);
      break;
