...
static void draw_edge(FILE *fp, int from, int to, int bb_edge,
                      int color_class) {
  const char *color;
  switch (graph_dump_format) {
    case vcg:
      ...
      if (color_class == 2)
        ...
        color = "color: green ";
      fprintf(fp, "edge: { sourcename: \"%s.%d\" targetname: \"%s.%d\" %s",
              current_function_name(), from, current_function_name(), to,
              color);
      if (color_class)
    ...
    for (tmp_rtx = NEXT_INSN(rtx_first); NULL != tmp_rtx;
      ...
      if (start[INSN_UID(tmp_rtx)] < 0 && end[INSN_UID(tmp_rtx)] < 0) {
        if (BARRIER_P(tmp_rtx))
          continue;
        if (NOTE_P(tmp_rtx) && (1 || in_bb_p[INSN_UID(tmp_rtx)] == NOT_IN_BB))
          continue;
      }
      ...
      if ((i = end[INSN_UID(tmp_rtx)]) >= 0) {
        ...
        FOR_EACH_EDGE(e, ei, bb->succs) {
          if (e->dest != EXIT_BLOCK_PTR) {
            ...
            draw_edge(fp, INSN_UID(tmp_rtx), 999999, next_insn != 0,
                      (e->flags & EDGE_ABNORMAL ? 2 : 0));
      ...
      if (!edge_printed) {
        ...
        if (next_insn == 0 || !BARRIER_P(next_insn))
          ...
          while (next_insn && (NOTE_P(next_insn) || BARRIER_P(next_insn)));
