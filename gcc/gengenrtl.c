...
static void gendecl(const char *format) {
  const char *p;
  int i, pos;
...
static void genmacro(int idx) {
  ...
  printf("#define gen_rtx_%s%s(MODE", special_rtx(idx) ? "raw_" : "",
         defs[idx].enumname);
  ...
  printf(") \\\n  gen_rtx_fmt_%s (%s, (MODE)", defs[idx].format,
         defs[idx].enumname);
...
static void gendef(const char *format) {
  ...
  printf("rtx\ngen_rtx_fmt_%s_stat (RTX_CODE code, enum machine_mode mode",
         format);
  for (p = format, i = 0; *p != 0; p++)
    if (*p != '0')
