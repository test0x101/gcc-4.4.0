...
struct reduction_info {
  ...
  tree initial_value; /* The initial value of the reduction var before entering
                         the loop.  */
  tree field; /*  the name of the field in the parloop data structure intended
                 for reduction.  */
...
static int reduction_info_eq(const void *aa, const void *bb) {
...
static hashval_t reduction_info_hash(const void *aa) {
  const struct reduction_info *a = (const struct reduction_info *)aa;

  return htab_hash_pointer(a->reduc_phi);
}

static struct reduction_info *reduction_phi(htab_t reduction_list, gimple phi) {
...
static int name_to_copy_elt_eq(const void *aa, const void *bb) {
...
static hashval_t name_to_copy_elt_hash(const void *aa) {
  const struct name_to_copy_elt *a = (const struct name_to_copy_elt *)aa;
...
static bool loop_parallel_p(struct loop *loop, htab_t reduction_list,
  ...
  for (gsi = gsi_start_phis(loop->header); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    if (reduc_stmt) {
      ...
      if (dump_file && (dump_flags & TDF_DETAILS)) {
        fprintf(dump_file, "Detected reduction. reduction stmt is: \n");
  ...
  for (gsi = gsi_start_phis(exit->dest); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    if (is_gimple_reg(val)) {
      ...
      if (htab_elements(reduction_list) == 0) {
        if (dump_file && (dump_flags & TDF_DETAILS))
          fprintf(dump_file, "  FAILED: it is not a part of reduction.\n");
        return false;
      }
      reduc_phi = NULL;
...
static inline bool loop_has_blocks_with_irreducible_flag(struct loop *loop) {
  unsigned i;
...
static tree take_address_of(tree obj, tree type, edge entry,
                            htab_t decl_address) {
  int uid;
  void **dslot;
  ...
  for (var_p = &obj; handled_component_p(*var_p);
  ...
  if (var_p != &obj) {
    *var_p = build1(INDIRECT_REF, TREE_TYPE(*var_p), name);
    name = force_gimple_operand(build_addr(obj, current_function_decl), &stmts,
                                true, NULL_TREE);
  ...
  if (TREE_TYPE(name) != type) {
    name =
        force_gimple_operand(fold_convert(type, name), &stmts, true, NULL_TREE);
    if (!gimple_seq_empty_p(stmts))
      gsi_insert_seq_on_edge_immediate(entry, stmts);
  }
...
static int initialize_reductions(void **slot, void *data) {
  tree init, c;
  tree bvar, type, arg;
  edge e;
  ...
  SET_USE(
      PHI_ARG_DEF_PTR_FROM_EDGE(reduc->reduc_phi, loop_preheader_edge(loop)),
      init);
...
static tree eliminate_local_variables_1(tree *tp, int *walk_subtrees,
                                        void *data) {
...
static void eliminate_local_variables_stmt(edge entry, gimple stmt,
                                           htab_t decl_address) {
  struct elv_data dta;
...
static bool expr_invariant_in_region_p(edge entry, edge exit, tree expr) {
  ...
  if (TREE_CODE(expr) == SSA_NAME) {
    ...
    if (def_bb && dominated_by_p(CDI_DOMINATORS, def_bb, entry_bb) &&
...
static tree separate_decls_in_region_name(tree name, htab_t name_copies,
                                          htab_t decl_copies,
                                          bool copy_name_p) {
...
static void separate_decls_in_region_stmt(edge entry, edge exit, gimple stmt,
                                          htab_t name_copies,
                                          htab_t decl_copies) {
  use_operand_p use;
  def_operand_p def;
  ...
  FOR_EACH_PHI_OR_STMT_DEF(def, stmt, oi, SSA_OP_DEF) {
    ...
    copy = separate_decls_in_region_name(name, name_copies, decl_copies, false);
...
static int add_field_for_reduction(void **slot, void *data) {
...
static int add_field_for_name(void **slot, void *data) {
...
static int create_phi_for_local_result(void **slot, void *data) {
  struct reduction_info *const reduc = (struct reduction_info *)*slot;
  ...
  local_res =
      make_ssa_name(SSA_NAME_VAR(gimple_assign_lhs(reduc->reduc_stmt)), NULL);
...
static int create_call_for_reduction_1(void **slot, void *data) {
  struct reduction_info *const reduc = (struct reduction_info *)*slot;
  ...
  gsi = gsi_start_bb(new_bb);
  ref = tmp_load;
  x = fold_build2(reduc->reduction_code, TREE_TYPE(PHI_RESULT(reduc->new_phi)),
                  ref, PHI_RESULT(reduc->new_phi));
...
static void create_call_for_reduction(struct loop *loop, htab_t reduction_list,
...
static int create_loads_for_reductions(void **slot, void *data) {
  struct reduction_info *const red = (struct reduction_info *)*slot;
  struct clsn_data *const clsn_data = (struct clsn_data *)data;
  gimple stmt;
  gimple_stmt_iterator gsi;
  ...
  load_struct = fold_build1(INDIRECT_REF, struct_type, clsn_data->load);
  load_struct = build3(COMPONENT_REF, type, load_struct, red->field, NULL_TREE);
  ...
  for (gsi = gsi_start_phis(gimple_bb(red->keep_res)); !gsi_end_p(gsi);
       gsi_next(&gsi))
...
static void create_final_loads_for_reduction(htab_t reduction_list,
                                             struct clsn_data *ld_st_data) {
  gimple_stmt_iterator gsi;
  tree t;
  gimple stmt;
...
static int create_stores_for_reduction(void **slot, void *data) {
  struct reduction_info *const red = (struct reduction_info *)*slot;
  struct clsn_data *const clsn_data = (struct clsn_data *)data;
  tree t;
  gimple stmt;
...
static int create_loads_and_stores_for_name(void **slot, void *data) {
  struct name_to_copy_elt *const elt = (struct name_to_copy_elt *)*slot;
  struct clsn_data *const clsn_data = (struct clsn_data *)data;
  tree t;
  gimple stmt;
...
static void separate_decls_in_region(edge entry, edge exit,
                                     htab_t reduction_list, tree *arg_struct,
                                     tree *new_arg_struct,
                                     struct clsn_data *ld_st_data)

{
  ...
  htab_t name_copies =
      htab_create(10, name_to_copy_elt_hash, name_to_copy_elt_eq, free);
  htab_t decl_copies =
      htab_create(10, int_tree_map_hash, int_tree_map_eq, free);
  unsigned i;
  ...
  for (i = 0; VEC_iterate(basic_block, body, i, bb); i++) {
    if (bb != entry_bb && bb != exit_bb) {
      for (gsi = gsi_start_phis(bb); !gsi_end_p(gsi); gsi_next(&gsi))
        separate_decls_in_region_stmt(entry, exit, gsi_stmt(gsi), name_copies,
                                      decl_copies);
  ...
  if (htab_elements(name_copies) == 0 && reduction_list == 0) {
    ...
    *new_arg_struct = NULL;
    ...
    type_name = build_decl(TYPE_DECL, create_tmp_var_name(".paral_data"), type);
    TYPE_NAME(type) = type_name;

    htab_traverse(name_copies, add_field_for_name, type);
    if (reduction_list && htab_elements(reduction_list) > 0) {
      /* Create the fields for reductions.  */
      htab_traverse(reduction_list, add_field_for_reduction, type);
    }
    layout_type(type);
    ...
    *new_arg_struct = make_ssa_name(nvar, NULL);
    ...
    if (reduction_list && htab_elements(reduction_list) > 0) {
      htab_traverse(reduction_list, create_stores_for_reduction, ld_st_data);
...
static bool parallelized_function_p(tree fn) {
...
static tree create_loop_fn(void) {
  char buf[100];
  char *tname;
  ...
  t = build_decl(PARM_DECL, get_identifier(".paral_data_param"), ptr_type_node);
  DECL_ARTIFICIAL(t) = 1;
...
tree canonicalize_loop_ivs(struct loop *loop, htab_t reduction_list,
                           tree *nit) {
  ...
  for (psi = gsi_start_phis(loop->header); !gsi_end_p(psi);) {
    ...
    val = fold_build2(POINTER_TYPE_P(atype) ? POINTER_PLUS_EXPR : PLUS_EXPR,
...
static void transform_to_exit_first_loop(struct loop *loop,
                                         htab_t reduction_list, tree nit) {
  ...
  ok = gimple_duplicate_sese_tail(single_succ_edge(loop->header), exit, bbs + 1,
                                  n, nbbs);
  gcc_assert(ok);
  free(bbs);
  ex_bb = nbbs[0];
  ...
  nit =
      force_gimple_operand_gsi(&gsi, fold_convert(TREE_TYPE(control_name), nit),
...
static basic_block create_parallel_loop(struct loop *loop, tree loop_fn,
                                        tree data, tree new_data,
                                        unsigned n_threads) {
  gimple_stmt_iterator gsi;
  ...
  OMP_CLAUSE_NUM_THREADS_EXPR(t) = build_int_cst(integer_type_node, n_threads);
  ...
  if (data) {
    ...
    stmt =
        gimple_build_assign(new_data, fold_convert(TREE_TYPE(new_data), param));
    gsi_insert_before(&gsi, stmt, GSI_SAME_STMT);
  ...
  SET_USE(PHI_ARG_DEF_PTR_FROM_EDGE(phi, loop_preheader_edge(loop)), initvar);
  ...
  for (gsi = gsi_start_phis(ex_bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    add_phi_arg(phi, PHI_ARG_DEF_FROM_EDGE(stmt, loop_preheader_edge(loop)),
                guard);
    add_phi_arg(phi, PHI_ARG_DEF_FROM_EDGE(stmt, loop_latch_edge(loop)), end);
  }
  ...
  gimple_omp_for_set_incr(
      for_stmt, 0, build2(PLUS_EXPR, type, cvar_base, build_int_cst(type, 1)));
...
static void gen_parallel_loop(struct loop *loop, htab_t reduction_list,
                              unsigned n_threads,
                              struct tree_niter_desc *niter) {
  struct loop *nloop;
  loop_iterator li;
  ...
  many_iterations_cond =
      fold_build2(GE_EXPR, boolean_type_node, nit,
  ...
  nloop = loop_version(loop, many_iterations_cond, NULL, prob, prob,
...
static bool loop_has_vector_phi_nodes(struct loop *loop ATTRIBUTE_UNUSED) {
  unsigned i;
  basic_block *bbs = get_loop_body_in_dom_order(loop);
  gimple_stmt_iterator gsi;
  bool res = true;
...
bool parallelize_loops(void) {
  ...
  reduction_list =
      htab_create(10, reduction_info_hash, reduction_info_eq, free);
