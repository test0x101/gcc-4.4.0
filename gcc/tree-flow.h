...
#define FOR_EACH_HTAB_ELEMENT(HTAB, RESULT, TYPE, ITER)    \
  for (RESULT = (TYPE)first_htab_element(&(ITER), (HTAB)); \
       !end_htab_p(&(ITER)); RESULT = (TYPE)next_htab_element(&(ITER)))
...
#define FOR_EACH_IMM_USE_STMT(STMT, ITER, SSAVAR)      \
  for ((STMT) = first_imm_use_stmt(&(ITER), (SSAVAR)); \
       !end_imm_use_stmt_p(&(ITER)); (STMT) = next_imm_use_stmt(&(ITER)))
...
#define FOR_EACH_REFERENCED_VAR(VAR, ITER)                                     \
  for ((VAR) = first_referenced_var(&(ITER)); !end_referenced_vars_p(&(ITER)); \
...
#define FOR_EACH_REFERENCED_VAR_SAFE(VAR, VEC, ITER)  \
  for ((ITER).i = 0, fill_referenced_var_vec(&(VEC)); \
       VEC_iterate(tree, (VEC), (ITER).i, (VAR)); (ITER).i++)
...
extern tree get_ref_base_and_extent(tree, HOST_WIDE_INT *, HOST_WIDE_INT *,
                                    HOST_WIDE_INT *);
...
bool gimple_duplicate_loop_to_header_edge(struct loop *, edge, unsigned int,
                                          sbitmap, edge, VEC(edge, heap) **,
                                          int);
struct loop *slpeel_tree_duplicate_loop_to_edge_cfg(struct loop *, edge);
void rename_variables_in_loop(struct loop *);
void rename_variables_in_bb(basic_block bb);
struct loop *tree_ssa_loop_version(struct loop *, tree, basic_block *);
...
void tree_transform_and_unroll_loop(struct loop *, unsigned, edge,
                                    struct tree_niter_desc *,
...
extern void thread_across_edge(gimple, edge, bool, VEC(tree, heap) **,
                               tree (*)(gimple, gimple));
...
tree force_gimple_operand_gsi(gimple_stmt_iterator *, tree, bool, tree, bool,
