...
static inline funct_state get_function_state(struct cgraph_node *node) {
  if (!funct_state_vec ||
...
static inline void set_function_state(struct cgraph_node *node, funct_state s) {
  if (!funct_state_vec ||
      VEC_length(funct_state, funct_state_vec) <= (unsigned int)node->uid)
...
static inline void check_decl(funct_state local, tree t, bool checking_write) {
...
static void check_operand(funct_state local, tree t, bool checking_write) {
  if (!t)
    return;
...
static void check_tree(funct_state local, tree t, bool checking_write) {
...
static void look_for_address_of(funct_state local, tree t) {
...
static void check_rhs_var(funct_state local, tree t) {
  look_for_address_of(local, t);

  /* Memcmp and strlen can both trap and they are declared pure.  */
  if (tree_could_trap_p(t) && local->pure_const_state == IPA_CONST)
    local->pure_const_state = IPA_PURE;
...
static void check_lhs_var(funct_state local, tree t) {
  /* Memcmp and strlen can both trap and they are declared pure.
...
static void get_asm_expr_operands(funct_state local, gimple stmt) {
...
static void check_call(funct_state local, gimple call) {
...
static tree scan_function_op(tree *tp, int *walk_subtrees, void *data) {
...
static tree scan_function_stmt(gimple_stmt_iterator *gsi_p, bool *handled_ops_p,
...
static void analyze_function(struct cgraph_node *fn) {
  ...
  /* If this function does not return normally or does not bind local,
     ...
     front end.  */
  if (TREE_THIS_VOLATILE(decl) || !targetm.binds_local_p(decl)) {
  ...
  if (!l->state_set_in_source) {
    ...
    FOR_EACH_BB_FN(this_block, this_cfun) {
      ...
      for (gsi = gsi_start_bb(this_block); !gsi_end_p(gsi); gsi_next(&gsi)) {
        ...
        walk_gimple_stmt(&gsi, scan_function_stmt, scan_function_op, &wi);
