...
static bool deletable_insn_p(rtx insn, bool fast, bitmap arg_stores) {
  rtx body, x;
  int i;
...
static inline int marked_insn_p(rtx insn) {
...
static void mark_insn(rtx insn, bool fast) {
...
static void mark_nonreg_stores(rtx body, rtx insn, bool fast) {
  if (fast)
...
static bool find_call_stack_args(rtx call_insn, bool do_mark, bool fast,
                                 bitmap arg_stores) {
  ...
  for (insn = PREV_INSN(call_insn); insn; insn = prev_insn) {
    ...
    for (byte = off; byte < off + GET_MODE_SIZE(GET_MODE(mem)); byte++) {
      if (byte < min_sp_off || byte >= max_sp_off ||
...
static void delete_corresponding_reg_eq_notes(rtx insn) {
  df_ref *def_rec;
...
static void mark_reg_dependencies(rtx insn) {
  struct df_link *defs;
  df_ref *use_rec;
...
static bool byte_dce_process_block(basic_block bb, bool redo_out, bitmap au) {
  ...
  quickexit:
    ...
    if (dump_file) {
      fprintf(dump_file,
              "finished processing insn %d live out = ", INSN_UID(insn));
...
static bool dce_process_block(basic_block bb, bool redo_out, bitmap au) {
  bitmap local_live = BITMAP_ALLOC(&dce_tmp_bitmap_obstack);
  rtx insn;
  bool block_changed;
  df_ref *def_rec;
...
static void fast_dce(bool byte_level) {
  ...
  while (global_changed) {
    ...
    for (i = 0; i < n_blocks; i++) {
      ...
      if (byte_level)
        local_changed = byte_dce_process_block(
            bb, bitmap_bit_p(redo_out, index), bb_has_eh_pred(bb) ? au_eh : au);
      else
...
static unsigned int rest_of_handle_fast_byte_dce(void) {
...
static bool gate_fast_dce(void) {
