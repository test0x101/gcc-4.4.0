...
static bool simple_move_operand(rtx x) {
  ...
  if (MEM_P(x) && (MEM_VOLATILE_P(x) || mode_dependent_address_p(XEXP(x, 0))))
    return false;
...
static rtx simple_move(rtx insn) {
  rtx x;
  rtx set;
  ...
  if (GET_CODE(x) != ASM_OPERANDS && !simple_move_operand(x))
    return NULL_RTX;
...
static int find_decomposable_subregs(rtx *px, void *data) {
...
static rtx simplify_subreg_concatn(enum machine_mode outermode, rtx op,
                                   unsigned int byte) {
...
static rtx simplify_gen_subreg_concatn(enum machine_mode outermode, rtx op,
                                       enum machine_mode innermode,
                                       unsigned int byte) {
  rtx ret;
  ...
  if (GET_CODE(op) == SUBREG && GET_CODE(SUBREG_REG(op)) == CONCATN) {
    ...
    op2 =
        simplify_subreg_concatn(GET_MODE(op), SUBREG_REG(op), SUBREG_BYTE(op));
    if (op2 == NULL_RTX) {
      /* We don't handle paradoxical subregs here.  */
      gcc_assert(GET_MODE_SIZE(outermode) <= GET_MODE_SIZE(GET_MODE(op)));
  ...
  if (ret == NULL_RTX && GET_CODE(op) == SUBREG && SUBREG_BYTE(op) == 0 &&
      (GET_MODE_SIZE(innermode) > GET_MODE_SIZE(GET_MODE(SUBREG_REG(op)))))
...
static int resolve_subreg_use(rtx *px, void *data) {
  rtx insn = (rtx)data;
  rtx x = *px;
  ...
  if (resolve_subreg_p(x)) {
    x = simplify_subreg_concatn(GET_MODE(x), SUBREG_REG(x), SUBREG_BYTE(x));
...
static void move_eh_region_note(rtx insn, rtx insns) {
  rtx note, p;
  ...
  for (p = insns; p != NULL_RTX; p = NEXT_INSN(p)) {
    if (CALL_P(p) ||
        (flag_non_call_exceptions && INSN_P(p) && may_trap_p(PATTERN(p))))
...
static void resolve_reg_notes(rtx insn) {
  rtx *pnote, note;
  ...
  if (note) {
    ...
    else if (old_count != num_validated_changes())
...
static rtx resolve_simple_move(rtx set, rtx insn) {
  ...
  if (GET_CODE(src) == SUBREG && resolve_reg_p(SUBREG_REG(src)) &&
      (SUBREG_BYTE(src) != 0 || (GET_MODE_SIZE(orig_mode) !=
  ...
  if (GET_CODE(dest) == SUBREG && resolve_reg_p(SUBREG_REG(dest)) &&
      (SUBREG_BYTE(dest) != 0 || (GET_MODE_SIZE(orig_mode) !=
  ...
  if (src == SET_SRC(set) && dest == SET_DEST(set) && !resolve_reg_p(src) &&
      !resolve_subreg_p(src) && !resolve_reg_p(dest) &&
  ...
  if (!can_decompose_p(src) || side_effects_p(src) ||
  ...
  if (!can_decompose_p(dest) || (side_effects_p(dest) && !pushing) ||
    ...
    if (!SCALAR_INT_MODE_P(dest_mode)) {
      dest_mode =
          mode_for_size(GET_MODE_SIZE(dest_mode) * BITS_PER_UNIT, MODE_INT, 0);
  ...
  if (pushing) {
    ...
    for (i = 0; i < words; ++i, j += jinc) {
      ...
      temp = adjust_automodify_address_nv(dest, word_mode, temp,
                                          j * UNITS_PER_WORD);
      emit_move_insn(temp, simplify_gen_subreg_concatn(
                               word_mode, src, orig_mode, j * UNITS_PER_WORD));
    }
  } else {
    unsigned int i;
    ...
    for (i = 0; i < words; ++i)
      emit_move_insn(simplify_gen_subreg_concatn(word_mode, dest, dest_mode,
                                                 i * UNITS_PER_WORD),
                     simplify_gen_subreg_concatn(word_mode, src, orig_mode,
                                                 i * UNITS_PER_WORD));
  }
...
static bool resolve_clobber(rtx pat, rtx insn) {
  rtx reg;
  ...
  ret = validate_change(
      NULL_RTX, &XEXP(pat, 0),
      simplify_gen_subreg_concatn(word_mode, reg, orig_mode, 0), 0);
  df_insn_rescan(insn);
  gcc_assert(ret != 0);
...
static bool resolve_use(rtx pat, rtx insn) {
...
static int find_decomposable_shift_zext(rtx insn) {
  rtx set;
  rtx op;
  rtx op_operand;
  ...
  op = SET_SRC(set);
  if (GET_CODE(op) != ASHIFT && GET_CODE(op) != LSHIFTRT &&
...
static rtx resolve_shift_zext(rtx insn) {
  rtx set;
  rtx op;
  rtx op_operand;
  rtx insns;
  ...
  if (WORDS_BIG_ENDIAN && GET_MODE_SIZE(GET_MODE(op_operand)) > UNITS_PER_WORD)
  ...
  dest_reg = simplify_gen_subreg_concatn(word_mode, SET_DEST(set),
                                         GET_MODE(SET_DEST(set)), offset1);
  dest_zero = simplify_gen_subreg_concatn(word_mode, SET_DEST(set),
                                          GET_MODE(SET_DEST(set)), offset2);
  src_reg = simplify_gen_subreg_concatn(word_mode, op_operand,
                                        GET_MODE(op_operand), src_offset);
  if (GET_CODE(op) != ZERO_EXTEND) {
    int shift_count = INTVAL(XEXP(op, 1));
    if (shift_count > BITS_PER_WORD)
      src_reg = expand_shift(
          GET_CODE(op) == ASHIFT ? LSHIFT_EXPR : RSHIFT_EXPR, word_mode,
          src_reg, build_int_cst(NULL_TREE, shift_count - BITS_PER_WORD),
          dest_reg, 1);
  }
...
static void decompose_multiword_subregs(void) {
  unsigned int max;
  basic_block bb;
  ...
  FOR_EACH_BB(bb) {
    ...
    FOR_BB_INSNS(bb, insn) {
      ...
      for (i = 0; i < n; ++i) {
        for_each_rtx(&recog_data.operand[i], find_decomposable_subregs, &cmi);
      ...
      FOR_BB_INSNS(bb, insn) {
        ...
        if (GET_CODE(pat) == CLOBBER)
          ...
          for (i = recog_data.n_operands - 1; i >= 0; --i)
            for_each_rtx(recog_data.operand_loc[i], resolve_subreg_use, insn);
...
static bool gate_handle_lower_subreg(void) {
  return flag_split_wide_types != 0;
}

/* Implement first lower subreg pass.  */

static unsigned int rest_of_handle_lower_subreg(void) {
...
static unsigned int rest_of_handle_lower_subreg2(void) {
  decompose_multiword_subregs();
  return 0;
}

struct rtl_opt_pass pass_lower_subreg = {{
    RTL_PASS, "subreg1",         /* name */
