...
static const char *get_name_of_type(tree type) {
  ...
  while (1) {
    ...
    if (result) {
      ...
      if (types_compatible_p(type, other_type)) {
        ...
        splay_tree_insert(type_to_canon_type, (splay_tree_key)type,
      ...
      splay_tree_insert(all_canon_types, (splay_tree_key)brand,
      ...
      splay_tree_insert(uid_to_canon_type, (splay_tree_key)TYPE_UID(type),
                        (splay_tree_value)type);
...
static tree get_canon_type(tree type, bool see_thru_ptrs,
...
static int get_canon_type_uid(tree type, bool see_thru_ptrs,
                              bool see_thru_arrays) {
...
int ipa_type_escape_star_count_of_interesting_type(tree type) {
  ...
  if (TREE_CODE(type) == RECORD_TYPE || TREE_CODE(type) == QUAL_UNION_TYPE ||
...
static void mark_interesting_type(tree type, enum escape_t escape_status) {
  ...
  if (ipa_type_escape_star_count_of_interesting_type(type) >= 0) {
    if ((escape_status == EXPOSED_PARAMETER) && POINTER_TYPE_P(type))
...
static bool parent_type_p(tree parent, tree child) {
  ...
  if (TREE_CODE(parent) == UNION_TYPE || TREE_CODE(parent) == QUAL_UNION_TYPE) {
    tree field;
    /* Search all of the variants in the union to see if one of them
       is the child.  */
    for (field = TYPE_FIELDS(parent); field; field = TREE_CHAIN(field)) {
      tree field_type;
...
static enum cast_type check_cast_type(tree to_type, tree from_type) {
  ...
  if (parent_type_p(to_type, from_type))
    return CT_UP;
  if (parent_type_p(from_type, to_type))
    return CT_DOWN;
  return CT_SIDEWAYS;
}
...
static bool is_malloc_result(tree var) {
  gimple def_stmt;
...
static enum cast_type check_cast(tree to_type, tree from) {
    ...
    if (from_interesting_type) {
      ...
      */
      ...
      if (POINTER_TYPE_P(to_type) && !POINTER_TYPE_P(from_type))
        cast = CT_FROM_NON_P;
      else if (TREE_CODE(from) == SSA_NAME && is_malloc_result(from))
        cast = CT_FROM_MALLOC;
      else {
        cast = CT_FROM_P_BAD;
        mark_type(to_type, FULL_ESCAPE);
...
static unsigned int look_for_casts_stmt(gimple s) {
  unsigned int cast = 0;
...
static bool is_cast_from_non_pointer(tree var, gimple def_stmt, void *data) {
  if (!def_stmt || !var)
    return false;
  ...
  if (is_gimple_assign(def_stmt)) {
    ...
    if ((cast & CT_FROM_NON_P) == (CT_FROM_NON_P) &&
      ...
      FOR_EACH_SSA_USE_OPERAND(use_p, def_stmt, iter, SSA_OP_ALL_USES) {
        walk_use_def_chains(USE_FROM_PTR(use_p), is_cast_from_non_pointer, data,
                            false);
    ...
    /* Check that there is no cast, or cast is not harmful. */
             ...
             (cast & CT_DOWN) == (CT_DOWN) || (cast & CT_UP) == (CT_UP) ||
...
bool is_array_access_through_pointer_and_index(enum tree_code code, tree op0,
                                               tree op1, tree *base,
                                               tree *offset,
  ...
  *offset_cast_stmt = NULL;
  ...
  if (is_gimple_assign(before_cast_def_stmt)) {
    ...
    if (gimple_assign_rhs_code(before_cast_def_stmt) == MULT_EXPR) {
      ...
      if (!(CONSTANT_CLASS_P(arg0) && simple_cst_equal(arg0, unit_size)) &&
          !(CONSTANT_CLASS_P(arg1) && simple_cst_equal(arg1, unit_size)))
        return false;
    } else
      return false;
  } else
...
static void check_function_parameter_and_return_types(tree fn, bool escapes) {
  ...
  if (TYPE_ARG_TYPES(TREE_TYPE(fn))) {
    for (arg = TYPE_ARG_TYPES(TREE_TYPE(fn));
         arg && TREE_VALUE(arg) != void_type_node; arg = TREE_CHAIN(arg)) {
...
static inline void has_proper_scope_for_analysis(tree t) {
  ...
  if (DECL_EXTERNAL(t) || TREE_PUBLIC(t)) {
    ...
    if (TREE_READONLY(t) && DECL_INITIAL(t) &&
...
static void mark_interesting_addressof(tree to_type, tree from_type) {
  ...
  result =
      splay_tree_lookup(uid_to_addressof_down_map, (splay_tree_key)from_uid);

  if (result)
    type_map = (bitmap)result->value;
  else {
    type_map = BITMAP_ALLOC(&ipa_obstack);
    splay_tree_insert(uid_to_addressof_down_map, from_uid,
                      (splay_tree_value)type_map);
  }
  bitmap_set_bit(type_map, TYPE_UID(to_type));

  /* Process the X into Y reverse map pointer.  */
  result = splay_tree_lookup(uid_to_addressof_up_map, (splay_tree_key)to_uid);

  if (result)
    type_map = (bitmap)result->value;
  else {
    type_map = BITMAP_ALLOC(&ipa_obstack);
    splay_tree_insert(uid_to_addressof_up_map, to_uid,
                      (splay_tree_value)type_map);
  }
...
static unsigned int look_for_casts(tree t) {
  unsigned int cast = 0;
...
static void check_asm(gimple stmt) {
  size_t i;
...
static void check_call(gimple call) {
  ...
  if (callee_t) {
    ...
    if (TYPE_ARG_TYPES(TREE_TYPE(callee_t))) {
      ...
      for (arg_type = DECL_ARGUMENTS(callee_t), i = 0; arg_type;
           arg_type = TREE_CHAIN(arg_type), i++) {
        tree operand = gimple_call_arg(call, i);
        if (operand) {
  ...
  if (avail == AVAIL_NOT_AVAILABLE || avail == AVAIL_OVERWRITABLE) {
    ...
    if (callee_t) {
      tree type = get_canon_type(TREE_TYPE(TREE_TYPE(callee_t)), false, false);
      mark_interesting_type(type, EXPOSED_PARAMETER);
    }
  }
}
...
static bool okay_pointer_operation(enum tree_code code, tree op0, tree op1) {
  ...
  switch (code) {
    ...
    case POINTER_PLUS_EXPR: {
      ...
      if (POINTER_TYPE_P(op0type) && TREE_CODE(op0) == SSA_NAME &&
        ...
        if (CONSTANT_CLASS_P(op1) && size_of_op0_points_to &&
            multiple_of_p(TREE_TYPE(size_of_op0_points_to), op1,
                          size_of_op0_points_to))
          return true;

        if (CONSTANT_CLASS_P(op0) && size_of_op0_points_to &&
            multiple_of_p(TREE_TYPE(size_of_op0_points_to), op0,
                          size_of_op0_points_to))
          return true;
      }
    } break;
...
static void check_assign(gimple t) {
  ...
  switch (TREE_CODE_CLASS(gimple_assign_rhs_code(t))) {
    ...
    case tcc_unary: {
      ...
      if (type0 && POINTER_TYPE_P(type0) &&
          (TREE_CODE(op0) == NEGATE_EXPR || TREE_CODE(op0) == ABS_EXPR))
        mark_interesting_type(type0, FULL_ESCAPE);
...
static void scan_for_refs(gimple t) {
...
static bitmap subtype_map_for_uid(int uid, bool create) {
  ...
  if (result)
    ...
    splay_tree_insert(uid_to_subtype_map, uid, (splay_tree_value)subtype_map);
    return subtype_map;
  } else
    return NULL;
}
...
static void close_type_seen(tree type) {
  ...
  if (TYPE_BINFO(type))
    for (binfo = TYPE_BINFO(type), i = 0;
      ...
      bitmap subtype_map =
          subtype_map_for_uid(TYPE_UID(TYPE_MAIN_VARIANT(binfo_type)), true);
  ...
  for (field = TYPE_FIELDS(type); field; field = TREE_CHAIN(field)) {
    ...
    field_type = TREE_TYPE(field);
    if (ipa_type_escape_star_count_of_interesting_or_array_type(field_type) >=
        0)
...
static void close_type_full_escape(tree type) {
  ...
  address_result =
      splay_tree_lookup(uid_to_addressof_up_map, (splay_tree_key)uid);
  if (address_result) {
...
struct simple_ipa_opt_pass pass_ipa_type_escape = {{
    SIMPLE_IPA_PASS, "type-escape-var", /* name */
