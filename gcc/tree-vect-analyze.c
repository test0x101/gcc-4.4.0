...
tree vect_get_smallest_scalar_type(gimple stmt, HOST_WIDE_INT *lhs_size_unit,
...
static bool vect_determine_vectorization_factor(loop_vec_info loop_vinfo) {
  ...
  for (i = 0; i < nbbs; i++) {
    ...
    for (si = gsi_start_phis(bb); !gsi_end_p(si); gsi_next(&si)) {
      ...
      if (STMT_VINFO_RELEVANT_P(stmt_info)) {
        ...
        if (!vectorization_factor || (nunits > vectorization_factor))
...
static void vect_update_slp_costs_according_to_vf(loop_vec_info loop_vinfo) {
  ...
  for (i = 0; VEC_iterate(slp_instance, slp_instances, i, instance); i++)
    ...
    SLP_INSTANCE_INSIDE_OF_LOOP_COST(instance) *=
        vf / SLP_INSTANCE_UNROLLING_FACTOR(instance);
}
...
static bool vect_analyze_operations(loop_vec_info loop_vinfo) {
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  basic_block *bbs = LOOP_VINFO_BBS(loop_vinfo);
  int nbbs = loop->num_nodes;
  gimple_stmt_iterator si;
  ...
  for (i = 0; i < nbbs; i++) {
    ...
    for (si = gsi_start_phis(bb); !gsi_end_p(si); gsi_next(&si)) {
      ...
      if (!is_loop_header_bb_p(bb)) {
        ...
        if (!STMT_VINFO_RELEVANT_P(stmt_info) || STMT_VINFO_LIVE_P(stmt_info)) {
          if (vect_print_dump_info(REPORT_DETAILS))
            fprintf(vect_dump, "Unsupported loop-closed phi in outer-loop.");
          return false;
        }
        continue;
      }
      ...
      if (!ok) {
        if (vect_print_dump_info(REPORT_UNVECTORIZED_LOOPS)) {
          fprintf(vect_dump, "not vectorized: relevant phi not supported: ");
          print_gimple_stmt(vect_dump, phi, 0, TDF_SLIM);
        }
        return false;
      }
    ...
    for (si = gsi_start_bb(bb); !gsi_end_p(si); gsi_next(&si)) {
      ...
      if (!PURE_SLP_STMT(stmt_info)) {
        ...
        if (STMT_VINFO_STRIDED_ACCESS(stmt_info) &&
            exact_log2(DR_GROUP_SIZE(
                vinfo_for_stmt(DR_GROUP_FIRST_DR(stmt_info)))) == -1) {
          if (vect_print_dump_info(REPORT_DETAILS)) {
            fprintf(vect_dump,
  ...
  if (!need_to_vectorize) {
    if (vect_print_dump_info(REPORT_DETAILS))
      fprintf(vect_dump, "All the computation can be taken out of the loop.");
    if (vect_print_dump_info(REPORT_UNVECTORIZED_LOOPS))
      fprintf(vect_dump,
  ...
  if (only_slp_in_loop)
    ...
    vectorization_factor = least_common_multiple(
        vectorization_factor, LOOP_VINFO_SLP_UNROLLING_FACTOR(loop_vinfo));
  ...
  min_scalar_loop_bound =
      ((PARAM_VALUE(PARAM_MIN_VECT_LOOP_BOUND) * vectorization_factor) - 1);
  ...
  if (!LOOP_VINFO_NITERS_KNOWN_P(loop_vinfo) ||
    ...
    if (!vect_can_advance_ivs_p(loop_vinfo)) {
      if (vect_print_dump_info(REPORT_UNVECTORIZED_LOOPS))
        fprintf(vect_dump, "not vectorized: can't create epilog loop 1.");
      return false;
    }
    if (!slpeel_can_duplicate_loop_p(loop, single_exit(loop))) {
      if (vect_print_dump_info(REPORT_UNVECTORIZED_LOOPS))
        fprintf(vect_dump, "not vectorized: can't create epilog loop 2.");
      return false;
    }
  }
...
static bool exist_non_indexing_operands_for_use_p(tree use, gimple stmt) {
  tree operand;
  stmt_vec_info stmt_info = vinfo_for_stmt(stmt);
...
static void vect_analyze_scalar_cycles_1(loop_vec_info loop_vinfo,
                                         struct loop *loop) {
  ...
  while (VEC_length(gimple, worklist) > 0) {
    ...
    } else if (vect_print_dump_info(REPORT_DETAILS))
...
static int vect_get_place_in_interleaving_chain(gimple stmt,
                                                gimple first_stmt) {
...
static void vect_insert_into_interleaving_chain(struct data_reference *dra,
...
static void vect_update_interleaving_chain(struct data_reference *drb,
  ...
  if (DR_GROUP_FIRST_DR(stmtinfo_a) && !DR_GROUP_FIRST_DR(stmtinfo_b)) {
    ...
    tree init_old =
        DR_INIT(STMT_VINFO_DATA_REF(vinfo_for_stmt(old_first_stmt)));
    gimple tmp;
...
static bool vect_equal_offsets(tree offset1, tree offset2) {
  bool res0, res1;
  ...
  if (TREE_CODE(offset1) != TREE_CODE(offset2) || !BINARY_CLASS_P(offset1) ||
      !BINARY_CLASS_P(offset2))
    return false;

  res0 = vect_equal_offsets(TREE_OPERAND(offset1, 0), TREE_OPERAND(offset2, 0));
  res1 = vect_equal_offsets(TREE_OPERAND(offset1, 1), TREE_OPERAND(offset2, 1));
...
static void vect_check_interleaving(struct data_reference *dra,
                                    struct data_reference *drb) {
  ...
  if (type_size_a != type_size_b ||
      ...
      !types_compatible_p(TREE_TYPE(DR_REF(dra)), TREE_TYPE(DR_REF(drb))))
    return;
...
static bool vect_same_range_drs(data_reference_p dr_i, data_reference_p dr_j) {
...
static bool vect_vfa_range_equal(ddr_p ddr_i, ddr_p ddr_j) {
...
static bool vect_mark_for_runtime_alias_test(ddr_p ddr,
...
static bool vect_analyze_data_ref_dependence(
    struct data_dependence_relation *ddr, loop_vec_info loop_vinfo) {
  unsigned int i;
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
...
static bool vect_analyze_data_ref_dependences(loop_vec_info loop_vinfo) {
  unsigned int i;
...
static bool vect_compute_data_ref_alignment(struct data_reference *dr) {
  ...
  if ((DECL_P(base) && tree_int_cst_compare(ssize_int(DECL_ALIGN_UNIT(base)),
                                            alignment) >= 0) ||
      (TREE_CODE(base_addr) == SSA_NAME &&
       tree_int_cst_compare(
           ssize_int(TYPE_ALIGN_UNIT(TREE_TYPE(TREE_TYPE(base_addr)))),
           alignment) >= 0))
    base_aligned = true;
  else
    base_aligned = false;
  ...
  gcc_assert(base_aligned || (TREE_CODE(base) == VAR_DECL &&
                              DECL_ALIGN(base) >= TYPE_ALIGN(vectype)));
...
static void vect_update_misalignment_for_peel(struct data_reference *dr,
  ...
  same_align_drs = STMT_VINFO_SAME_ALIGN_REFS(vinfo_for_stmt(DR_STMT(dr_peel)));
...
static bool vect_verify_datarefs_alignment(loop_vec_info loop_vinfo) {
  VEC(data_reference_p, heap) *datarefs = LOOP_VINFO_DATAREFS(loop_vinfo);
  struct data_reference *dr;
...
static bool vector_alignment_reachable_p(struct data_reference *dr) {
  gimple stmt = DR_STMT(dr);
  stmt_vec_info stmt_info = vinfo_for_stmt(stmt);
  tree vectype = STMT_VINFO_VECTYPE(stmt_info);
  ...
  if (known_alignment_for_access_p(dr) && !aligned_access_p(dr)) {
    HOST_WIDE_INT elmsize = int_cst_value(TYPE_SIZE_UNIT(TREE_TYPE(vectype)));
    if (vect_print_dump_info(REPORT_DETAILS)) {
...
static bool vect_enhance_data_refs_alignment(loop_vec_info loop_vinfo) {
  VEC(data_reference_p, heap) *datarefs = LOOP_VINFO_DATAREFS(loop_vinfo);
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  enum dr_alignment_support supportable_dr_alignment;
  ...
  if (do_versioning) {
    for (i = 0; VEC_iterate(data_reference_p, datarefs, i, dr); i++) {
      ...
      /* For interleaving, only the alignment of the first access
         matters.  */
      if (aligned_access_p(dr) || (STMT_VINFO_STRIDED_ACCESS(stmt_info) &&
                                   DR_GROUP_FIRST_DR(stmt_info) != stmt))
        continue;
      ...
      if (!supportable_dr_alignment) {
        ...
        LOOP_VINFO_PTR_MASK(loop_vinfo) = mask;
        VEC_safe_push(gimple, heap, LOOP_VINFO_MAY_MISALIGN_STMTS(loop_vinfo),
                      DR_STMT(dr));
      }
    }
...
static bool vect_analyze_data_refs_alignment(loop_vec_info loop_vinfo) {
  if (vect_print_dump_info(REPORT_DETAILS))
...
static bool vect_analyze_group_access(struct data_reference *dr) {
  tree step = DR_STEP(dr);
  ...
  if (!DR_GROUP_FIRST_DR(vinfo_for_stmt(stmt))) {
    ...
    if (DR_IS_READ(dr) && (dr_step % type_size) == 0 && stride > 0 &&
        exact_log2(stride) != -1) {
  ...
  if (DR_GROUP_FIRST_DR(vinfo_for_stmt(stmt)) == stmt) {
    ...
    while (next) {
      ...
      if (!tree_int_cst_compare(
        ...
        if (DR_GROUP_READ_WRITE_DEPENDENCE(vinfo_for_stmt(next)) ||
          ...
          if (vect_print_dump_info(REPORT_DETAILS))
            fprintf(vect_dump, "READ_WRITE dependence in interleaving.");
          return false;
        }
      ...
      diff =
          (TREE_INT_CST_LOW(DR_INIT(data_ref)) - TREE_INT_CST_LOW(prev_init)) /
          type_size;
      if (diff != 1) {
    ...
    if ((dr_step % type_size) != 0) {
      if (vect_print_dump_info(REPORT_DETAILS)) {
        ...
        print_generic_expr(vect_dump, TYPE_SIZE_UNIT(scalar_type), TDF_SLIM);
      }
      return false;
    }
...
static bool vect_analyze_data_ref_access(struct data_reference *dr) {
  tree step = DR_STEP(dr);
  tree scalar_type = TREE_TYPE(DR_REF(dr));
  gimple stmt = DR_STMT(dr);
  stmt_vec_info stmt_info = vinfo_for_stmt(stmt);
...
static bool vect_analyze_data_ref_accesses(loop_vec_info loop_vinfo) {
  unsigned int i;
  VEC(data_reference_p, heap) *datarefs = LOOP_VINFO_DATAREFS(loop_vinfo);
  struct data_reference *dr;
...
static bool vect_prune_runtime_alias_test_list(loop_vec_info loop_vinfo) {
  VEC(ddr_p, heap) *ddrs = LOOP_VINFO_MAY_ALIAS_DDRS(loop_vinfo);
  unsigned i, j;
...
static void vect_free_slp_tree(slp_tree node) {
  if (!node)
    return;
...
static bool vect_get_and_check_slp_defs(
    loop_vec_info loop_vinfo, slp_tree slp_node, gimple stmt,
    VEC(gimple, heap) * *def_stmts0, VEC(gimple, heap) * *def_stmts1,
    enum vect_def_type *first_stmt_dt0, enum vect_def_type *first_stmt_dt1,
    tree *first_stmt_def0_type, tree *first_stmt_def1_type,
    tree *first_stmt_const_oprnd, int ncopies_for_cost, bool *pattern0,
    bool *pattern1) {
  tree oprnd;
      ...
      *first_stmt_dt0 = dt[i];
      ...
      if (!*first_stmt_dt1 && i == 1) {
        ...
        *first_stmt_dt1 = dt[i];
        ...
        if ((i == 0 && (*first_stmt_dt0 != dt[i] ||
                        (*first_stmt_def0_type && def &&
                         *first_stmt_def0_type != TREE_TYPE(def)))) ||
            (i == 1 && (*first_stmt_dt1 != dt[i] ||
                        (*first_stmt_def1_type && def &&
                         *first_stmt_def1_type != TREE_TYPE(def)))) ||
            (!def && TREE_TYPE(*first_stmt_const_oprnd) != TREE_TYPE(oprnd))) {
          if (vect_print_dump_info(REPORT_SLP))
...
static bool vect_build_slp_tree(loop_vec_info loop_vinfo, slp_tree *node,
                                unsigned int group_size, int *inside_cost,
                                int *outside_cost, int ncopies_for_cost,
  ...
  for (i = 0; VEC_iterate(gimple, stmts, i, stmt); i++) {
    ...
    if (i == 0) {
      ...
      if (rhs_code == LSHIFT_EXPR || rhs_code == RSHIFT_EXPR ||
          rhs_code == LROTATE_EXPR || rhs_code == RROTATE_EXPR) {
        vec_mode = TYPE_MODE(vectype);

        /* First see if we have a vector/vector shift.  */
        optab = optab_for_tree_code(rhs_code, vectype, optab_vector);

        if (!optab ||
            (optab->handlers[(int)vec_mode].insn_code == CODE_FOR_nothing)) {
          /* No vector/vector shift, try for a vector/scalar shift.  */
          optab = optab_for_tree_code(rhs_code, vectype, optab_scalar);
      ...
      if (first_stmt_code != rhs_code &&
          (first_stmt_code != IMAGPART_EXPR || rhs_code != REALPART_EXPR) &&
          (first_stmt_code != REALPART_EXPR || rhs_code != IMAGPART_EXPR)) {
        if (vect_print_dump_info(REPORT_SLP)) {
          fprintf(vect_dump, "Build SLP failed: different operation in stmt ");
          print_gimple_stmt(vect_dump, stmt, 0, TDF_SLIM);
        }
      ...
      if (need_same_oprnds &&
        ...
        if (vect_print_dump_info(REPORT_SLP)) {
          fprintf(vect_dump, "Build SLP failed: different shift arguments in ");
          print_gimple_stmt(vect_dump, stmt, 0, TDF_SLIM);
        }
    ...
    if (STMT_VINFO_STRIDED_ACCESS(vinfo_for_stmt(stmt))) {
      if (REFERENCE_CLASS_P(lhs)) {
        /* Store.  */
        if (!vect_get_and_check_slp_defs(
                loop_vinfo, *node, stmt, &def_stmts0, &def_stmts1,
                &first_stmt_dt0, &first_stmt_dt1, &first_stmt_def0_type,
                &first_stmt_def1_type, &first_stmt_const_oprnd,
                ncopies_for_cost, &pattern0, &pattern1))
          return false;
      } else {
        /* Load.  */
        ...
        if (first_load == stmt) {
          ...
          vect_model_load_cost(vinfo_for_stmt(stmt), ncopies_for_cost, *node);
        }

        /* Store the place of this load in the interleaving chain. In
           case that permutation is needed we later decide if a specific
           permutation is supported.  */
        load_place = vect_get_place_in_interleaving_chain(stmt, first_load);
        if (load_place != i)
          permutation = true;
      ...
      if (!vect_get_and_check_slp_defs(
              loop_vinfo, *node, stmt, &def_stmts0, &def_stmts1,
              &first_stmt_dt0, &first_stmt_dt1, &first_stmt_def0_type,
              &first_stmt_def1_type, &first_stmt_const_oprnd, ncopies_for_cost,
              &pattern0, &pattern1))
        return false;
    }
  }
  ...
  *outside_cost += SLP_TREE_OUTSIDE_OF_LOOP_COST(*node);
  ...
  if (first_stmt_dt0 == vect_loop_def) {
    ...
    if (!vect_build_slp_tree(loop_vinfo, &left_node, group_size, inside_cost,
  ...
  if (first_stmt_dt1 == vect_loop_def) {
    ...
    if (!vect_build_slp_tree(loop_vinfo, &right_node, group_size, inside_cost,
                             outside_cost, ncopies_for_cost, max_nunits,
                             load_permutation, loads))
      return false;
...
static void vect_print_slp_tree(slp_tree node) {
  int i;
  gimple stmt;
...
static void vect_mark_slp_stmts(slp_tree node, enum slp_vect_type mark, int j) {
  int i;
  gimple stmt;
...
static bool vect_supported_slp_permutation_p(slp_instance instance) {
...
static bool vect_supported_load_permutation_p(
    slp_instance slp_instn, int group_size, VEC(int, heap) * load_permutation) {
...
static gimple vect_find_first_load_in_slp_instance(slp_instance instance) {
  int i, j;
  slp_tree load_node;
  gimple first_load = NULL, load;

  for (i = 0; VEC_iterate(slp_tree, SLP_INSTANCE_LOADS(instance), i, load_node);
       i++)
    for (j = 0; VEC_iterate(gimple, SLP_TREE_SCALAR_STMTS(load_node), j, load);
         j++)
...
static bool vect_analyze_slp_instance(loop_vec_info loop_vinfo, gimple stmt) {
...
static bool vect_analyze_slp(loop_vec_info loop_vinfo) {
  unsigned int i;
...
static void vect_make_slp_decision(loop_vec_info loop_vinfo) {
...
static void vect_detect_hybrid_slp_stmts(slp_tree node) {
  int i;
  gimple stmt;
  ...
  if (vinfo_for_stmt(use_stmt) && !STMT_SLP_TYPE(vinfo_for_stmt(use_stmt)) &&
...
static void vect_detect_hybrid_slp(loop_vec_info loop_vinfo) {
  unsigned int i;
  VEC(slp_instance, heap) *slp_instances = LOOP_VINFO_SLP_INSTANCES(loop_vinfo);
  slp_instance instance;
...
static bool vect_analyze_data_refs(loop_vec_info loop_vinfo) {
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  unsigned int i;
  ...
  for (i = 0; VEC_iterate(data_reference_p, datarefs, i, dr); i++) {
    ...
    if (nested_in_vect_loop_p(loop, stmt)) {
      ...
      tree inner_base = build_fold_indirect_ref(
          fold_build2(POINTER_PLUS_EXPR, TREE_TYPE(base), base,
      ...
      outer_base =
          get_inner_reference(inner_base, &pbitsize, &pbitpos, &poffset, &pmode,
      ...
      if (!simple_iv(loop, loop_containing_stmt(stmt), outer_base, &base_iv,
                     false)) {
        if (vect_print_dump_info(REPORT_DETAILS))
      ...
      outer_step = size_binop(PLUS_EXPR, fold_convert(ssizetype, base_iv.step),
      ...
      STMT_VINFO_DR_OFFSET(stmt_info) = fold_convert(ssizetype, offset_iv.base);
      STMT_VINFO_DR_ALIGNED_TO(stmt_info) =
          size_int(highest_pow2_factor(offset_iv.base));

      if (vect_print_dump_info(REPORT_DETAILS)) {
        fprintf(vect_dump, "\touter base_address: ");
        print_generic_expr(vect_dump, STMT_VINFO_DR_BASE_ADDRESS(stmt_info),
                           TDF_SLIM);
        ...
        fprintf(vect_dump, "\n\touter aligned to: ");
        print_generic_expr(vect_dump, STMT_VINFO_DR_ALIGNED_TO(stmt_info),
                           TDF_SLIM);
      }
    }
    ...
    if (STMT_VINFO_DATA_REF(stmt_info)) {
      if (vect_print_dump_info(REPORT_UNVECTORIZED_LOOPS)) {
        fprintf(vect_dump, "not vectorized: more than one data ref in stmt: ");
        print_gimple_stmt(vect_dump, stmt, 0, TDF_SLIM);
      }
      return false;
    }
    ...
    if (!STMT_VINFO_VECTYPE(stmt_info)) {
      if (vect_print_dump_info(REPORT_UNVECTORIZED_LOOPS)) {
        fprintf(vect_dump, "not vectorized: no vectype for stmt: ");
        print_gimple_stmt(vect_dump, stmt, 0, TDF_SLIM);
...
static void vect_mark_relevant(VEC(gimple, heap) * *worklist, gimple stmt,
...
static bool vect_stmt_relevant_p(gimple stmt, loop_vec_info loop_vinfo,
                                 enum vect_relevant *relevant, bool *live_p) {
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  ssa_op_iter op_iter;
  imm_use_iterator imm_iter;
...
static bool process_use(gimple stmt, tree use, loop_vec_info loop_vinfo,
                        bool live_p, enum vect_relevant relevant,
...
static bool vect_mark_stmts_to_be_vectorized(loop_vec_info loop_vinfo) {
  VEC(gimple, heap) * worklist;
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  basic_block *bbs = LOOP_VINFO_BBS(loop_vinfo);
...
static bool vect_can_advance_ivs_p(loop_vec_info loop_vinfo) {
  struct loop *loop = LOOP_VINFO_LOOP(loop_vinfo);
  basic_block bb = loop->header;
  gimple phi;
  gimple_stmt_iterator gsi;
...
static gimple vect_get_loop_niters(struct loop *loop,
...
static loop_vec_info vect_analyze_loop_1(struct loop *loop) {
  loop_vec_info loop_vinfo;
...
loop_vec_info vect_analyze_loop_form(struct loop *loop) {
  loop_vec_info loop_vinfo;
  gimple loop_cond;
  ...
  if (!loop->inner) {
    ...
    if (!expr_invariant_in_loop_p(loop, LOOP_VINFO_NITERS(inner_loop_vinfo))) {
      if (vect_print_dump_info(REPORT_BAD_FORM_LOOPS))
        fprintf(vect_dump, "not vectorized: inner-loop count not invariant.");
    ...
    if (entryedge->src != loop->header || !single_exit(innerloop) ||
  ...
  if (!single_exit(loop) || EDGE_COUNT(loop->header->preds) != 2) {
    if (vect_print_dump_info(REPORT_BAD_FORM_LOOPS)) {
      if (!single_exit(loop))
  ...
  if (!empty_block_p(loop->latch) || phi_nodes(loop->latch)) {
    if (vect_print_dump_info(REPORT_BAD_FORM_LOOPS))
...
loop_vec_info vect_analyze_loop(struct loop *loop) {
  bool ok;
  loop_vec_info loop_vinfo;
  ...
  if (loop_outer(loop) && loop_vec_info_for_loop(loop_outer(loop)) &&
