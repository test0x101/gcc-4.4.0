...
static void find_traces_1_round(int, int, gcov_type, struct trace *, int *, int,
                                fibheap_t *, int);
...
static void find_rarely_executed_basic_blocks_and_crossing_edges(edge **, int *,
                                                                 int *);
...
static bool push_to_next_round_p(const_basic_block bb, int round,
                                 int number_of_rounds, int exec_th,
                                 gcov_type count_th) {
  ...
  block_not_hot_enough = (bb->frequency < exec_th || bb->count < count_th ||
...
static void find_traces(int *n_traces, struct trace *traces) {
  ...
  if (dump_file) {
    for (i = 0; i < *n_traces; i++) {
      ...
      for (bb = traces[i].first; bb != traces[i].last;
...
static basic_block rotate_loop(edge back_edge, struct trace *trace,
                               int trace_n) {
  basic_block bb;
  ...
  do {
    ...
    if (e->dest != EXIT_BLOCK_PTR && e->dest->il.rtl->visited != trace_n &&
  ...
  if (best_bb) {
    ...
    if (back_edge->dest == trace->first) {
      ...
      for (prev_bb = trace->first; prev_bb->aux != back_edge->dest;
      ...
      if (single_succ_p(prev_bb)) {
        ...
        if (any_condjump_p(BB_END(header)) && copy_bb_p(header, 0) &&
            !find_reg_note(BB_END(header), REG_CROSSING_JUMP, NULL_RTX))
...
static void find_traces_1_round(int branch_th, int exec_th, gcov_type count_th,
  ...
  while (!fibheap_empty(*heap)) {
    ...
    if (push_to_next_round_p(bb, round, number_of_rounds, exec_th, count_th)) {
    ...
    do {
      ...
      FOR_EACH_EDGE(e, ei, bb->succs) {
        ...
        if (e->dest->il.rtl->visited && e->dest->il.rtl->visited != *n_traces)
          continue;
        ...
        if (better_edge_p(bb, e, prob, freq, best_prob, best_freq, best_edge)) {
          best_edge = e;
          best_prob = prob;
          best_freq = freq;
        }
      ...
      FOR_EACH_EDGE(e, ei, bb->succs) {
        if (e == best_edge || e->dest == EXIT_BLOCK_PTR ||
        ...
        if (bbd[e->dest->index].heap) {
          ...
          if (key != bbd[e->dest->index].node->key) {
            if (dump_file) {
              fprintf(dump_file, "Changing key for bb %d from %ld to %ld.\n",
                      e->dest->index, (long)bbd[e->dest->index].node->key, key);
            }
          ...
          if (!(e->flags & EDGE_CAN_FALLTHRU) || (e->flags & EDGE_COMPLEX) ||
            ...
            if (push_to_next_round_p(e->dest, round, number_of_rounds, exec_th,
                                     count_th))
              which_heap = new_heap;
          }

          bbd[e->dest->index].heap = which_heap;
          bbd[e->dest->index].node = fibheap_insert(which_heap, key, e->dest);

          if (dump_file) {
            fprintf(dump_file, "  Possible start of %s round: %d (key: %ld)\n",
                    (which_heap == new_heap) ? "next" : "this", e->dest->index,
                    (long)key);
          }
        }
      }
        ...
        if (best_edge->dest->il.rtl->visited == *n_traces) {
          ...
          */
          ...
          if (e != best_edge && (e->flags & EDGE_CAN_FALLTHRU) &&
              !(e->flags & EDGE_COMPLEX) && !e->dest->il.rtl->visited &&
              single_pred_p(e->dest) && !(e->flags & EDGE_CROSSING) &&
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      if (e->dest == EXIT_BLOCK_PTR || e->dest->il.rtl->visited)
        continue;
...
static basic_block copy_bb(basic_block old_bb, edge e, basic_block bb,
                           int trace) {
  basic_block new_bb;
...
static fibheapkey_t bb_to_key(basic_block bb) {
  edge e;
  edge_iterator ei;
  int priority = 0;
  ...
  if (BB_PARTITION(bb) == BB_COLD_PARTITION || probably_never_executed_bb_p(bb))
    return BB_FREQ_MAX;
...
static bool better_edge_p(const_basic_block bb, const_edge e, int prob,
                          int freq, int best_prob, int best_freq,
  ...
  if (!is_better_edge && flag_reorder_blocks_and_partition && cur_best_edge &&
      (cur_best_edge->flags & EDGE_CROSSING) && !(e->flags & EDGE_CROSSING))
    is_better_edge = true;
  ...
  for (i = 0; i < n_traces || (two_passes && current_pass == 1); i++) {
    ...
    for (t2 = t; t2 > 0;) {
      ...
      FOR_EACH_EDGE(e, ei, traces[t2].first->preds) {
        ...
        if (e->src != ENTRY_BLOCK_PTR && (e->flags & EDGE_CAN_FALLTHRU) &&
            !(e->flags & EDGE_COMPLEX) && bbd[si].end_of_trace >= 0 &&
    ...
    while (1) {
      ...
      FOR_EACH_EDGE(e, ei, traces[t].last->succs) {
        ...
        if (e->dest != EXIT_BLOCK_PTR && (e->flags & EDGE_CAN_FALLTHRU) &&
            !(e->flags & EDGE_COMPLEX) && bbd[di].start_of_trace >= 0 &&
      ...
      if (best) {
        ...
        if (e->dest != EXIT_BLOCK_PTR && (e->flags & EDGE_CAN_FALLTHRU) &&
          ...
          if (bbd[e->dest->index].start_of_trace >= 0 &&
              traces[bbd[e->dest->index].start_of_trace].length == 1) {
            best = e;
            try_copy = true;
            continue;
          }
          ...
          FOR_EACH_EDGE(e2, ei, e->dest->succs) {
            ...
            if (e2->dest == EXIT_BLOCK_PTR ||
                ((e2->flags & EDGE_CAN_FALLTHRU) &&
                 !(e2->flags & EDGE_COMPLEX) && bbd[di].start_of_trace >= 0 &&
                 !connected[bbd[di].start_of_trace] &&
                 (BB_PARTITION(e2->dest) == current_partition) &&
                 (EDGE_FREQUENCY(e2) >= freq_threshold) &&
                 (e2->count >= count_threshold) &&
                 (!best2 || e2->probability > best2->probability ||
                  (e2->probability == best2->probability &&
                   traces[bbd[di].start_of_trace].length > best2_len)))) {
              best = e;
              best2 = e2;
              if (e2->dest != EXIT_BLOCK_PTR)
        ...
        if (try_copy &&
            copy_bb_p(best->dest, optimize_edge_for_speed_p(best) &&
                                      EDGE_FREQUENCY(best) >= freq_threshold &&
                                      best->count >= count_threshold)) {
          basic_block new_bb;

          if (dump_file) {
            fprintf(dump_file, "Connection: %d %d ", traces[t].last->index,
                    best->dest->index);
            if (!next_bb)
              fputc('\n', dump_file);
...
static bool copy_bb_p(const_basic_block bb, int code_may_grow) {
  int size = 0;
  ...
  if (dump_file) {
    fprintf(dump_file, "Block %d can't be copied because its size = %d.\n",
            bb->index, size);
  }
...
static void find_rarely_executed_basic_blocks_and_crossing_edges(
    edge **crossing_edges, int *n_crossing_edges, int *max_idx) {
  basic_block bb;
  edge e;
  int i;
  edge_iterator ei;
    ...
    if (fall_thru && (fall_thru->dest != EXIT_BLOCK_PTR)) {
      ...
      if (fall_thru->flags & EDGE_CROSSING) {
        ...
        if (cond_jump) {
          ...
          if (!cond_jump_crosses && cur_bb->aux == cond_jump->dest) {
        ...
        if (cond_jump_crosses || !invert_worked) {
          ...
          if (new_bb) {
            ...
            new_bb->il.rtl->footer = unlink_insn_chain(barrier, barrier);
          } else {
            barrier = emit_barrier_after(BB_END(cur_bb));
            cur_bb->il.rtl->footer = unlink_insn_chain(barrier, barrier);
          }
        }
      }
    }
...
static basic_block find_jump_block(basic_block jump_dest) {
  ...
  if (e->flags & EDGE_CROSSING) {
    ...
    if (LABEL_P(BB_HEAD(src)))
      for (insn = BB_HEAD(src); !INSN_P(insn) && insn != NEXT_INSN(BB_END(src));
           insn = NEXT_INSN(insn)) {
        if (INSN_P(insn) && insn == BB_END(src) && JUMP_P(insn) &&
            !any_condjump_p(insn)) {
          source_bb = src;
          break;
        }
      ...
      if (set_src && (GET_CODE(set_src) == IF_THEN_ELSE)) {
        ...
        if (new_bb)
          ...
          if (GET_CODE(old_label) == LABEL_REF) {
            ...
            new_jump =
                emit_jump_insn_after(gen_jump(old_label), BB_END(new_bb));
          } else {
            gcc_assert(HAVE_return && GET_CODE(old_label) == RETURN);
            new_jump = emit_jump_insn_after(gen_return(), BB_END(new_bb));
          }
...
static void add_reg_crossing_jump_notes(void) {
  ...
  FOR_EACH_BB(bb)
  FOR_EACH_EDGE(e, ei, bb->succs)
  if ((e->flags & EDGE_CROSSING) && JUMP_P(BB_END(e->src)))
...
static unsigned int duplicate_computed_gotos(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    if (!single_succ_p(bb) || single_succ(bb) == EXIT_BLOCK_PTR ||
        single_succ(bb) == bb->next_bb || single_pred_p(single_succ(bb)))
      continue;
...
static void partition_hot_cold_basic_blocks(void) {
  ...
  find_rarely_executed_basic_blocks_and_crossing_edges(
      &crossing_edges, &n_crossing_edges, &max_edges);
...
static unsigned int rest_of_handle_reorder_blocks(void) {
  ...
  if ((flag_reorder_blocks || flag_reorder_blocks_and_partition)
      /* Don't reorder blocks when optimizing for size because extra jump insns
         may be created; also barrier may create extra padding.
