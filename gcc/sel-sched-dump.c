...
static int dump_flist_insn_flags =
    (DUMP_INSN_UID | DUMP_INSN_BBN | DUMP_INSN_SEQNO);
...
static void switch_dump(FILE *to) {
...
static int dump_vinsn_flags =
    (DUMP_VINSN_INSN_RTX | DUMP_VINSN_TYPE | DUMP_VINSN_COUNT);
...
void dump_insn_rtx_1(rtx insn, int flags) {
  int all;
...
void dump_insn_rtx(rtx insn) {
  dump_insn_rtx_1(insn, dump_insn_rtx_flags);
}

/* Dump INSN to stderr.  */
void debug_insn_rtx(rtx insn) {
  switch_dump(stderr);
...
void dump_vinsn_1(vinsn_t vi, int flags) {
  int all;
...
void dump_expr_1(expr_t expr, int flags) {
  int all;
...
void dump_insn_1(insn_t i, int flags) {
  int all;
...
void dump_insn(insn_t i) {
  dump_insn_1(i, DUMP_INSN_EXPR | DUMP_INSN_SCHED_CYCLE);
}

/* Dump INSN to stderr.  */
void debug_insn(insn_t insn) {
  switch_dump(stderr);
...
static void dump_ilist(ilist_t p) {
  while (p) {
...
void dump_insn_vector(rtx_vec_t succs) {
  int i;
  rtx succ;
...
static void print_hard_reg_set(FILE *file, const char *prefix,
                               HARD_REG_SET set) {
  int i;
...
void dump_hard_reg_set(const char *prefix, HARD_REG_SET set) {
  print_hard_reg_set(sched_dump, prefix, set);
}

/* Pretty print INSN.  This is used as a hook.  */
const char *sel_print_insn(const_rtx insn, int aligned ATTRIBUTE_UNUSED) {
  static char buf[80];
  ...
  if (s_i_d && INSN_LUID(insn) > 0)
    if (GET_MODE(insn) == TImode)
      sprintf(buf, "%s %4d", INSN_SCHED_TIMES(insn) > 0 ? "> " : "< ",
              INSN_UID(insn));
    else
      sprintf(buf, "%s %4d", INSN_SCHED_TIMES(insn) > 0 ? "! " : "  ",
              INSN_UID(insn));
...
static void replace_str_in_buf(char *buf, const char *str1, const char *str2) {
...
static void sel_dump_cfg_insn(insn_t insn, int flags) {
...
static void sel_dump_cfg_edge(FILE *f, edge e) {
  int w;
  const char *color;
...
static bool has_preds_in_current_region_p(basic_block bb) {
  edge e;
  edge_iterator ei;
...
static void sel_dump_cfg_2(FILE *f, int flags) {
  basic_block bb;
  ...
  FOR_EACH_BB(bb) {
    ...
    edge_iterator ei;
    bool in_region_p =
        ((flags & SEL_DUMP_CFG_CURRENT_REGION) && in_current_region_p(bb));
    bool full_p = (!(flags & SEL_DUMP_CFG_CURRENT_REGION) || in_region_p);
    ...
    if ((flags & SEL_DUMP_CFG_CURRENT_REGION) && in_current_region_p(bb) &&
        BLOCK_TO_BB(bb->index) == 0)
      color = "color = green, ";
    else
      color = "";

    if ((flags & SEL_DUMP_CFG_FENCES) && in_region_p) {
      style = "";
    ...
    fprintf(f, "\tbb%d [%s%slabel = \"{Basic block %d", bb->index, style, color,
            bb->index);

    if ((flags & SEL_DUMP_CFG_BB_LOOP) && bb->loop_father != NULL)
      fprintf(f, ", loop %d", bb->loop_father->num);

    if (full_p && (flags & SEL_DUMP_CFG_BB_NOTES_LIST)) {
    ...
    if (full_p && (flags & SEL_DUMP_CFG_AV_SET) && in_current_region_p(bb) &&
        !sel_bb_empty_p(bb)) {
      fprintf(f, "|");
    ...
    if ((flags & SEL_DUMP_CFG_LV_SET) && !sel_bb_empty_p(bb)) {
      fprintf(f, "|");
    ...
    if (full_p && (flags & SEL_DUMP_CFG_BB_INSNS)) {
      fprintf(f, "|");
...
static void sel_dump_cfg_1(const char *tag, int flags) {
  char *buf;
  int i;
  FILE *f;
...
void sel_debug_cfg_1(int flags) {
...
void debug_insn_vector(rtx_vec_t succs) {
  switch_dump(stderr);
...
void debug_hard_reg_set(HARD_REG_SET set) {
  switch_dump(stderr);
...
rtx debug_mem_addr_value(rtx x) {
  rtx t, addr;
