...
void init_empty_tree_cfg_for_function(struct function *fn) {
  ...
  VEC_safe_grow_cleared(basic_block, gc, basic_block_info_for_function(fn),
                        initial_cfg_capacity);

  /* Build a mapping of labels to their associated blocks.  */
  label_to_block_map_for_function(fn) =
      VEC_alloc(basic_block, gc, initial_cfg_capacity);
  VEC_safe_grow_cleared(basic_block, gc, label_to_block_map_for_function(fn),
                        initial_cfg_capacity);

  SET_BASIC_BLOCK_FOR_FUNCTION(fn, ENTRY_BLOCK,
                               ENTRY_BLOCK_PTR_FOR_FUNCTION(fn));
  SET_BASIC_BLOCK_FOR_FUNCTION(fn, EXIT_BLOCK, EXIT_BLOCK_PTR_FOR_FUNCTION(fn));

  ENTRY_BLOCK_PTR_FOR_FUNCTION(fn)->next_bb = EXIT_BLOCK_PTR_FOR_FUNCTION(fn);
  EXIT_BLOCK_PTR_FOR_FUNCTION(fn)->prev_bb = ENTRY_BLOCK_PTR_FOR_FUNCTION(fn);
}
...
static bool computed_goto_p(gimple t) {
  ...
  FOR_EACH_BB(bb) {
    ...
    if (computed_goto_p(last)) {
      ...
      if (!factored_computed_goto) {
        ...
        factored_computed_goto_label = gimple_build_label(factored_label_decl);
        gsi_insert_after(&new_gsi, factored_computed_goto_label, GSI_NEW_STMT);
        ...
        case GIMPLE_OMP_CONTINUE:
          ...
          switch (cur_region->type) {
            case GIMPLE_OMP_FOR:
              ...
              make_edge(bb, single_succ(cur_region->entry), EDGE_ABNORMAL);
...
static void make_cond_expr_edges(basic_block bb) {
...
static bool edge_to_cases_cleanup(const void *key ATTRIBUTE_UNUSED,
                                  void **value, void *data ATTRIBUTE_UNUSED) {
  tree t, next;
...
static tree get_cases_for_edge(edge e, gimple t) {
  void **slot;
  size_t i, n;
...
static void make_gimple_switch_edges(basic_block bb) {
  gimple entry = last_stmt(bb);
  size_t i, n;
...
void make_abnormal_goto_edges(basic_block bb, bool for_call) {
...
static void make_goto_expr_edges(basic_block bb) {
...
void cleanup_dead_labels(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    for (i = gsi_start_bb(bb); !gsi_end_p(i);) {
      ...
      if (label == label_for_this_bb || !DECL_ARTIFICIAL(label) ||
          DECL_NONLOCAL(label) || FORCED_LABEL(label))
        gsi_next(&i);
      else
        gsi_remove(&i, true);
    }
  ...
  FOR_EACH_BB(bb) {
    ...
    if (stmt && gimple_code(stmt) == GIMPLE_SWITCH) {
      ...
      while (i < old_size) {
        ...
        base_high =
            CASE_HIGH(base_case) ? CASE_HIGH(base_case) : CASE_LOW(base_case);
        i++;
        ...
        while (i < old_size) {
          ...
          tree t = int_const_binop(PLUS_EXPR, base_high, integer_one_node, 1);
      ...
      for (i = 0, j = 0; i < new_size; i++) {
        while (!gimple_switch_label(stmt, j))
          j++;
        gimple_switch_set_label(stmt, i, gimple_switch_label(stmt, j++));
      }
...
static bool gimple_can_merge_blocks_p(basic_block a, basic_block b) {
  gimple stmt;
  gimple_stmt_iterator gsi;
  gimple_seq phis;
...
static void gimple_merge_blocks(basic_block a, basic_block b) {
  ...
  for (psi = gsi_start(phis); !gsi_end_p(psi);) {
    ...
    if (current_loops && loops_state_satisfies_p(LOOP_CLOSED_SSA) &&
        is_gimple_reg(def) && TREE_CODE(use) == SSA_NAME &&
...
static void remove_useless_stmts_1(gimple_stmt_iterator *gsi,
                                   struct rus_data *);
...
static bool remove_useless_stmts_warn_notreached(gimple_seq stmts) {
  gimple_stmt_iterator gsi;
  ...
  for (gsi = gsi_start(stmts); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    switch (gimple_code(stmt)) {
      ...
      case GIMPLE_EH_FILTER:
        return remove_useless_stmts_warn_notreached(
...
static void remove_useless_stmts_cond(gimple_stmt_iterator *gsi,
...
static void remove_useless_stmts_tf(gimple_stmt_iterator *gsi,
                                    struct rus_data *data) {
...
static void remove_useless_stmts_tc(gimple_stmt_iterator *gsi,
                                    struct rus_data *data) {
...
static void remove_useless_stmts_bind(gimple_stmt_iterator *gsi,
                                      struct rus_data *data ATTRIBUTE_UNUSED) {
  tree block;
...
static void remove_useless_stmts_goto(gimple_stmt_iterator *gsi,
                                      struct rus_data *data) {
  gimple stmt = gsi_stmt(*gsi);
  ...
  /* Record iterator for last goto expr, so that we can delete it if
   * unnecessary.  */
...
static void remove_useless_stmts_label(gimple_stmt_iterator *gsi,
                                       struct rus_data *data) {
  gimple stmt = gsi_stmt(*gsi);
...
void notice_special_calls(gimple call) {
...
static unsigned int remove_useless_stmts(void) {
  struct rus_data data;
...
static void remove_bb(basic_block bb) {
  gimple_stmt_iterator i;
  ...
  if (bb_seq(bb) != NULL) {
    for (i = gsi_start_bb(bb); !gsi_end_p(i);) {
      ...
      if (gimple_code(stmt) != GIMPLE_GOTO && gimple_has_location(stmt) && !loc)
...
static edge find_taken_edge_computed_goto(basic_block bb, tree val) {
  basic_block dest;
  edge e = NULL;
...
static edge find_taken_edge_cond_expr(basic_block bb, tree val) {
...
static edge find_taken_edge_switch_expr(basic_block bb, tree val) {
  basic_block dest_bb;
  edge e;
  gimple switch_stmt;
  tree taken_case;
...
static tree find_case_label_for_value(gimple switch_stmt, tree val) {
...
void gimple_debug_cfg(int flags) {
...
void gimple_dump_cfg(FILE *file, int flags) {
...
static void gimple_cfg2vcg(FILE *file) {
  edge e;
  edge_iterator ei;
  basic_block bb;
  const char *funcname =
  ...
  FOR_EACH_BB(bb) {
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      if (e->dest == EXIT_BLOCK_PTR)
        fprintf(file, "edge: { sourcename: \"%d\" targetname: \"EXIT\"",
                bb->index);
      else
...
bool is_ctrl_stmt(gimple t) {
  return gimple_code(t) == GIMPLE_COND || gimple_code(t) == GIMPLE_SWITCH ||
         gimple_code(t) == GIMPLE_GOTO || gimple_code(t) == GIMPLE_RETURN ||
...
static basic_block split_edge_bb_loc(edge edge_in) {
...
static tree verify_expr(tree *tp, int *walk_subtrees,
...
#define CHECK_OP(N, MSG)                      \
  do {                                        \
    if (!is_gimple_val(TREE_OPERAND(t, N))) { \
      error(MSG);                             \
  ...
  switch (TREE_CODE(t)) {
      ...
      while (handled_component_p(t)) {
        if (TREE_CODE(t) == COMPONENT_REF && TREE_OPERAND(t, 2))
          ...
          if (!host_integerp(TREE_OPERAND(t, 1), 1) ||
            ...
            error(
                "integral result type precision does not match "
          ...
          if (!INTEGRAL_TYPE_P(TREE_TYPE(t)) &&
              (GET_MODE_PRECISION(TYPE_MODE(TREE_TYPE(t))) !=
               TREE_INT_CST_LOW(TREE_OPERAND(t, 1)))) {
            error(
                "mode precision of non-integral result does not "
    ...
    case POINTER_PLUS_EXPR:
      ...
      if (!POINTER_TYPE_P(TREE_TYPE(TREE_OPERAND(t, 0)))) {
        error(
            "invalid operand to pointer plus, first operand is not a pointer");
        return t;
      }
      /* Check to make sure the second operand is an integer with type of
         sizetype.  */
      if (!useless_type_conversion_p(sizetype, TREE_TYPE(TREE_OPERAND(t, 1)))) {
        error(
            "invalid operand to pointer plus, second operand is not an "
...
static bool verify_types_in_gimple_min_lval(tree expr) {
  tree op;

  if (is_gimple_id(expr))
    return false;

  if (!INDIRECT_REF_P(expr) && TREE_CODE(expr) != TARGET_MEM_REF) {
  ...
  if (!useless_type_conversion_p(TREE_TYPE(expr), TREE_TYPE(TREE_TYPE(op)))) {
...
static bool verify_types_in_gimple_reference(tree expr) {
  while (handled_component_p(expr)) {
    tree op = TREE_OPERAND(expr, 0);

    if (TREE_CODE(expr) == ARRAY_REF || TREE_CODE(expr) == ARRAY_RANGE_REF) {
      if (!is_gimple_val(TREE_OPERAND(expr, 1)) ||
          (TREE_OPERAND(expr, 2) && !is_gimple_val(TREE_OPERAND(expr, 2))) ||
          (TREE_OPERAND(expr, 3) && !is_gimple_val(TREE_OPERAND(expr, 3)))) {
    ...
    if (TREE_CODE(expr) == ARRAY_REF &&
        !useless_type_conversion_p(TREE_TYPE(expr), TREE_TYPE(TREE_TYPE(op)))) {
    ...
    /* For VIEW_CONVERT_EXPRs which are allowed here, too, there
       ...
       undefined behavior.  */
    if (TREE_CODE(expr) == VIEW_CONVERT_EXPR && !handled_component_p(op))
      return false;
...
static bool valid_fixed_convert_types_p(tree type1, tree type2) {
  return (FIXED_POINT_TYPE_P(type1) &&
          (INTEGRAL_TYPE_P(type2) || SCALAR_FLOAT_TYPE_P(type2) ||
...
static bool verify_gimple_call(gimple stmt) {
  ...
  if (gimple_call_lhs(stmt) && !is_gimple_lvalue(gimple_call_lhs(stmt))) {
...
static bool verify_gimple_comparison(tree type, tree op0, tree op1) {
...
static bool verify_gimple_assign_unary(gimple stmt) {
  ...
  if (!is_gimple_reg(lhs) &&
      !(optimize == 0 && TREE_CODE(lhs_type) == COMPLEX_TYPE)) {
...
static bool verify_gimple_assign_binary(gimple stmt) {
  enum tree_code rhs_code = gimple_assign_rhs_code(stmt);
  tree lhs = gimple_assign_lhs(stmt);
  tree lhs_type = TREE_TYPE(lhs);
  tree rhs1 = gimple_assign_rhs1(stmt);
  ...
  switch (rhs_code) {
    case COMPLEX_EXPR: {
      if (TREE_CODE(lhs_type) != COMPLEX_TYPE ||
          !(INTEGRAL_TYPE_P(rhs1_type) || SCALAR_FLOAT_TYPE_P(rhs1_type)) ||
          !(INTEGRAL_TYPE_P(rhs2_type) || SCALAR_FLOAT_TYPE_P(rhs2_type))) {
    ...
    case LSHIFT_EXPR:
    case RSHIFT_EXPR:
      if (FIXED_POINT_TYPE_P(rhs1_type) && INTEGRAL_TYPE_P(rhs2_type) &&
    ...
    case LROTATE_EXPR:
    case RROTATE_EXPR: {
      if (!INTEGRAL_TYPE_P(rhs1_type) || !INTEGRAL_TYPE_P(rhs2_type) ||
          !useless_type_conversion_p(lhs_type, rhs1_type)) {
    ...
    case PLUS_EXPR:
    case MINUS_EXPR: {
      if (POINTER_TYPE_P(lhs_type) || POINTER_TYPE_P(rhs1_type) ||
...
static bool verify_gimple_assign_single(gimple stmt) {
  enum tree_code rhs_code = gimple_assign_rhs_code(stmt);
  tree lhs = gimple_assign_lhs(stmt);
  tree lhs_type = TREE_TYPE(lhs);
  tree rhs1 = gimple_assign_rhs1(stmt);
  ...
  switch (rhs_code) {
      ...
      if (!one_pointer_to_useless_type_conversion_p(lhs_type, TREE_TYPE(op))
          ...
          && (TREE_CODE(TREE_TYPE(op)) != ARRAY_TYPE ||
              !one_pointer_to_useless_type_conversion_p(
                  lhs_type, TREE_TYPE(TREE_TYPE(op))))) {
    ...
    case IMAGPART_EXPR:
    case TARGET_MEM_REF:
      if (!is_gimple_reg(lhs) && is_gimple_reg_type(TREE_TYPE(lhs))) {
    ...
    case VAR_DECL:
    case PARM_DECL:
      if (!is_gimple_reg(lhs) && !is_gimple_reg(rhs1) &&
...
static bool verify_gimple_assign(gimple stmt) {
...
static bool verify_gimple_return(gimple stmt) {
...
static bool verify_gimple_switch(gimple stmt) {
...
static bool verify_gimple_phi(gimple stmt) {
  ...
  for (i = 0; i < gimple_phi_num_args(stmt); i++) {
    ...
    if ((is_gimple_reg(gimple_phi_result(stmt)) && !is_gimple_val(arg)) ||
...
static bool verify_types_in_gimple_stmt(gimple stmt) {
  ...
  switch (gimple_code(stmt)) {
    ...
    case GIMPLE_COND:
      return verify_gimple_comparison(boolean_type_node, gimple_cond_lhs(stmt),
...
static bool verify_types_in_gimple_seq_2(gimple_seq stmts) {
...
void verify_types_in_gimple_seq(gimple_seq stmts) {
...
static bool verify_stmt(gimple_stmt_iterator *gsi) {
  tree addr;
  ...
  if (is_gimple_call(stmt)) {
    ...
    if (decl && TREE_CODE(decl) == FUNCTION_DECL &&
        DECL_LOOPING_CONST_OR_PURE_P(decl) && (!DECL_PURE_P(decl)) &&
        (!TREE_READONLY(decl))) {
...
static bool tree_node_can_be_shared(tree t) {
  if (IS_TYPE_OR_DECL_P(t) || is_gimple_min_invariant(t) ||
      TREE_CODE(t) == SSA_NAME || t == error_mark_node ||
  ...
  while (((TREE_CODE(t) == ARRAY_REF || TREE_CODE(t) == ARRAY_RANGE_REF) &&
         ...
         TREE_CODE(t) == COMPONENT_REF || TREE_CODE(t) == REALPART_EXPR ||
...
static int verify_eh_throw_stmt_node(void **slot, void *data) {
...
void verify_stmts(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    for (gsi = gsi_start_phis(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      for (i = 0; i < gimple_phi_num_args(phi); i++) {
        ...
        else if (TREE_CODE(t) != SSA_NAME && TREE_CODE(t) != FUNCTION_DECL &&
  ...
  if (get_eh_throw_stmt_table(cfun))
    htab_traverse(get_eh_throw_stmt_table(cfun), verify_eh_throw_stmt_node,
                  visited_stmts);
  ...
  FOR_EACH_BB(bb) {
    ...
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (label_to_block(label) != bb) {
        error("label ");
        print_generic_expr(stderr, label, 0);
        fprintf(stderr, " to block does not match in bb %d", bb->index);
        err = 1;
      }
      ...
      if (decl_function_context(label) != current_function_decl) {
        error("label ");
        print_generic_expr(stderr, label, 0);
        fprintf(stderr, " has incorrect context in bb %d", bb->index);
        err = 1;
      }
    }
    ...
    for (; !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (found_ctrl_stmt) {
        error("control flow in the middle of basic block %d", bb->index);
        err = 1;
      }
    ...
    if (is_ctrl_stmt(stmt)) {
      FOR_EACH_EDGE(e, ei, bb->succs)
      if (e->flags & EDGE_FALLTHRU) {
        error("fallthru edge after a control statement in bb %d", bb->index);
        err = 1;
      }
    }
    ...
    if (gimple_code(stmt) != GIMPLE_COND) {
      ...
      if (e->flags & (EDGE_TRUE_VALUE | EDGE_FALSE_VALUE)) {
        error("true/false edge after a non-GIMPLE_COND in bb %d", bb->index);
        err = 1;
      }
    }
    ...
    switch (gimple_code(stmt)) {
      ...
      case GIMPLE_GOTO:
        if (simple_goto_p(stmt)) {
          ...
          FOR_EACH_EDGE(e, ei, bb->succs)
          if ((e->flags &
               (EDGE_FALLTHRU | EDGE_TRUE_VALUE | EDGE_FALSE_VALUE)) ||
      ...
      case GIMPLE_RETURN:
        if (!single_succ_p(bb) || (single_succ_edge(bb)->flags &
        ...
        if (single_succ(bb) != EXIT_BLOCK_PTR) {
          error("return edge does not point to exit in bb %d", bb->index);
          err = 1;
        }
        break;
      ...
      case GIMPLE_SWITCH: {
        ...
        for (i = 1; i < n; ++i) {
          ...
          if (CASE_LOW(prev) && !tree_int_cst_lt(CASE_LOW(prev), CASE_LOW(c))) {
        ...
        FOR_EACH_EDGE(e, ei, bb->succs) {
          if (!e->dest->aux) {
            error("extra outgoing edge %d->%d", bb->index, e->dest->index);
            err = 1;
          }

          e->dest->aux = (void *)2;
          if ((e->flags & (EDGE_FALLTHRU | EDGE_ABNORMAL | EDGE_TRUE_VALUE |
                           EDGE_FALSE_VALUE))) {
            error("wrong outgoing edge flags at end of bb %d", bb->index);
            err = 1;
          }
...
static void gimple_make_forwarder_block(edge fallthru) {
  edge e;
  edge_iterator ei;
...
static edge gimple_try_redirect_by_replacing_jump(edge e, basic_block target) {
...
static edge gimple_redirect_edge_and_branch(edge e, basic_block dest) {
...
static basic_block gimple_split_block(basic_block bb, void *stmt) {
  ...
  for (gsi_tgt = gsi_start(list); !gsi_end_p(gsi_tgt); gsi_next(&gsi_tgt))
...
static bool gimple_can_duplicate_bb_p(const_basic_block bb ATTRIBUTE_UNUSED) {
  return true;
}
...
static basic_block gimple_duplicate_bb(basic_block bb) {
  basic_block new_bb;
...
static void add_phi_args_after_copy_edge(edge e_copy) {
  ...
  if (!e) {
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      if ((e->dest->flags & BB_DUPLICATED) && get_bb_original(e->dest) == dest)
        break;
    }
  ...
  for (psi = gsi_start_phis(e->dest), psi_copy = gsi_start_phis(e_copy->dest);
       !gsi_end_p(psi); gsi_next(&psi), gsi_next(&psi_copy)) {
    phi = gsi_stmt(psi);
...
bool gimple_duplicate_sese_region(edge entry, edge exit, basic_block *region,
                                  unsigned n_region, basic_block *region_copy) {
  unsigned i;
  ...
  if (total_count) {
    scale_bbs_frequencies_gcov_type(region, n_region, total_count - entry_count,
                                    total_count);
...
bool gimple_duplicate_sese_tail(edge entry ATTRIBUTE_UNUSED,
                                edge exit ATTRIBUTE_UNUSED,
                                basic_block *region ATTRIBUTE_UNUSED,
  ...
  if (total_count) {
    scale_bbs_frequencies_gcov_type(region, n_region, total_count - exit_count,
                                    total_count);
  ...
  if (!last_stmt(entry->src) || !stmt_ends_bb_p(last_stmt(entry->src)))
    switch_bb = entry->src;
  else
...
void gather_blocks_in_sese_region(basic_block entry, basic_block exit,
  ...
  for (son = first_dom_son(CDI_DOMINATORS, entry); son;
...
static void replace_by_duplicate_decl(tree *tp, struct pointer_map_t *vars_map,
                                      tree to_context) {
  tree t = *tp, new_t;
...
static tree replace_ssa_name(tree name, struct pointer_map_t *vars_map,
                             tree to_context) {
  void **loc;
    ...
    if (TREE_CODE(t) == SSA_NAME)
      *tp = replace_ssa_name(t, p->vars_map, p->to_context);
      ...
      if (p->new_label_map) {
        ...
        in.base.from = t;
        out = (struct tree_map *)htab_find_with_hash(p->new_label_map, &in,
                                                     DECL_UID(t));
        if (out)
          *tp = t = out->to;
      }
      ...
      if (SSA_VAR_P(t) && gimple_in_ssa_p(cfun)) {
...
static tree move_stmt_r(gimple_stmt_iterator *gsi_p, bool *handled_ops_p,
                        struct walk_stmt_info *wi) {
  struct move_stmt_d *p = (struct move_stmt_d *)wi->info;
  gimple stmt = gsi_stmt(*gsi_p);
  tree block = gimple_block(stmt);

  if (p->orig_block == NULL_TREE || block == p->orig_block ||
      block == NULL_TREE)
  ...
  if (is_gimple_omp(stmt) && gimple_code(stmt) != GIMPLE_OMP_RETURN &&
...
void mark_virtual_ops_in_bb(basic_block bb) {
  gimple_stmt_iterator gsi;
...
static void mark_virtual_ops_in_region(VEC(basic_block, heap) * bbs) {
  basic_block bb;
  unsigned i;
...
static void move_block_to_fn(struct function *dest_cfun, basic_block bb,
  ...
  if ((unsigned)cfg->x_last_basic_block >= old_len) {
    ...
    VEC_safe_grow_cleared(basic_block, gc, cfg->x_basic_block_info, new_len);
  }

  VEC_replace(basic_block, cfg->x_basic_block_info, bb->index, bb);
  ...
  for (si = gsi_start_phis(bb); !gsi_end_p(si);) {
    ...
    SET_PHI_RESULT(phi, replace_ssa_name(op, d->vars_map, dest_cfun->decl));
  ...
  for (si = gsi_start_bb(bb); !gsi_end_p(si); gsi_next(&si)) {
    ...
    if (gimple_code(stmt) == GIMPLE_LABEL) {
      ...
      if (old_len <= (unsigned)uid) {
        new_len = 3 * uid / 2;
        VEC_safe_grow_cleared(basic_block, gc, cfg->x_label_to_block_map,
                              new_len);
      }
...
static int find_outermost_region_in_block(struct function *src_cfun,
...
basic_block move_sese_region_to_fn(struct function *dest_cfun,
                                   basic_block entry_bb, basic_block exit_bb,
                                   tree orig_block) {
  ...
  gcc_assert(entry_bb != exit_bb &&
             (!exit_bb || dominated_by_p(CDI_DOMINATORS, exit_bb, entry_bb)));
  ...
  dom_bbs =
      get_dominated_by_region(CDI_DOMINATORS, VEC_address(basic_block, bbs),
  ...
  if (orig_block) {
    tree block;
    gcc_assert(BLOCK_SUBBLOCKS(DECL_INITIAL(dest_cfun->decl)) == NULL_TREE);
    BLOCK_SUBBLOCKS(DECL_INITIAL(dest_cfun->decl)) =
        BLOCK_SUBBLOCKS(orig_block);
    for (block = BLOCK_SUBBLOCKS(orig_block); block; block = BLOCK_CHAIN(block))
      BLOCK_SUPERCONTEXT(block) = DECL_INITIAL(dest_cfun->decl);
    BLOCK_SUBBLOCKS(orig_block) = NULL_TREE;
  }

  replace_block_vars_by_duplicates(DECL_INITIAL(dest_cfun->decl), vars_map,
                                   dest_cfun->decl);
...
static void print_loop(FILE *file, struct loop *loop, int indent,
                       int verbosity) {
  char *s_indent;
  basic_block bb;
  ...
  /* Print loop's header.  */
  fprintf(file, "%sloop_%d (header = %d, latch = %d", s_indent, loop->num,
...
static void print_loop_and_siblings(FILE *file, struct loop *loop, int indent,
                                    int verbosity) {
  if (loop == NULL)
    return;
...
static bool need_fake_edge_p(gimple t) {
  ...
  if (is_gimple_call(t) && !(call_flags & ECF_NORETURN))
    return true;
...
static void get_all_dominated_blocks(basic_block bb,
...
void remove_edge_and_dominated_blocks(edge e) {
  ...
  EXECUTE_IF_SET_IN_BITMAP(df_idom, 0, i, bi) {
    bb = BASIC_BLOCK(i);
    for (dbb = first_dom_son(CDI_DOMINATORS, bb); dbb;
...
static void gimple_lv_adjust_loop_header_phi(basic_block first,
  ...
  for (psi2 = gsi_start_phis(second), psi1 = gsi_start_phis(first);
       !gsi_end_p(psi2) && !gsi_end_p(psi1); gsi_next(&psi2), gsi_next(&psi1)) {
    phi1 = gsi_stmt(psi1);
    phi2 = gsi_stmt(psi2);
...
static void gimple_lv_add_condition_to_bb(
    basic_block first_head ATTRIBUTE_UNUSED,
    basic_block second_head ATTRIBUTE_UNUSED, basic_block cond_bb,
    void *cond_e) {
  gimple_stmt_iterator gsi;
...
tree gimplify_build3(gimple_stmt_iterator *gsi, enum tree_code code, tree type,
...
tree gimplify_build2(gimple_stmt_iterator *gsi, enum tree_code code, tree type,
                     tree a, tree b) {
  tree ret;
...
tree gimplify_build1(gimple_stmt_iterator *gsi, enum tree_code code, tree type,
                     tree a) {
  tree ret;
...
static unsigned int execute_warn_function_return(void) {
  ...
  if (TREE_THIS_VOLATILE(cfun->decl) && EDGE_COUNT(EXIT_BLOCK_PTR->preds) > 0) {
  ...
  else if (warn_return_type && !TREE_NO_WARNING(cfun->decl) &&
           EDGE_COUNT(EXIT_BLOCK_PTR->preds) > 0 &&
           !VOID_TYPE_P(TREE_TYPE(TREE_TYPE(cfun->decl)))) {
    FOR_EACH_EDGE(e, ei, EXIT_BLOCK_PTR->preds) {
      gimple last = last_stmt(e->src);
      if (gimple_code(last) == GIMPLE_RETURN &&
          gimple_return_retval(last) == NULL && !gimple_no_warning_p(last)) {
...
void extract_true_false_edges_from_block(basic_block b, edge *true_edge,
                                         edge *false_edge) {
...
static unsigned int execute_warn_function_noreturn(void) {
  if (warn_missing_noreturn && !TREE_THIS_VOLATILE(cfun->decl) &&
      EDGE_COUNT(EXIT_BLOCK_PTR->preds) == 0 &&
