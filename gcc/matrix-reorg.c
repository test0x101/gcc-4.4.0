...
static void collect_data_for_malloc_call(gimple stmt,
                                         struct malloc_call_data *m_data) {
  tree size_var = NULL;
  tree malloc_fn_decl;
  tree arg1;
...
static hashval_t mat_acc_phi_hash(const void *p) {
...
static int mat_acc_phi_eq(const void *p1, const void *p2) {
...
static hashval_t mtt_info_hash(const void *mtt) {
...
static int mtt_info_eq(const void *mtt1, const void *mtt2) {
...
static bool may_flatten_matrices_1(gimple stmt) {
  tree t;
      ...
      while (CONVERT_EXPR_P(t)) {
        if (TREE_TYPE(t) && POINTER_TYPE_P(TREE_TYPE(t))) {
          ...
          if (TREE_CODE(pointee) == VECTOR_TYPE) {
            if (dump_file)
              fprintf(dump_file, "Found vector type, don't flatten matrix\n");
            return false;
          }
        }
        t = TREE_OPERAND(t, 0);
...
static bool may_flatten_matrices(struct cgraph_node *node) {
  tree decl;
...
static struct matrix_info *analyze_matrix_decl(tree var_decl) {
...
static void find_matrices_decl(void) {
...
static void mark_min_matrix_escape_level(struct matrix_info *mi, int l,
                                         gimple s) {
...
static void ssa_accessed_in_tree(tree t, struct ssa_acc_in_tree *a) {
  ...
  switch (a->t_code) {
    case SSA_NAME:
      ...
      break;
    case INDIRECT_REF:
      if (SSA_VAR_P(TREE_OPERAND(t, 0)) && TREE_OPERAND(t, 0) == a->ssa_var)
        a->var_found = true;
      break;
    default:
      break;
...
static void ssa_accessed_in_call_rhs(gimple stmt, struct ssa_acc_in_tree *a) {
  tree decl;
  tree arg;
  size_t i;
...
static void ssa_accessed_in_assign_rhs(gimple stmt, struct ssa_acc_in_tree *a) {
...
static void record_access_alloc_site_info(struct matrix_info *mi, gimple stmt,
                                          tree offset, tree index, int level,
                                          bool is_alloc) {
  ...
  acc_info =
      (struct access_site_info *)xcalloc(1, sizeof(struct access_site_info));
  acc_info->stmt = stmt;
...
static void add_allocation_site(struct matrix_info *mi, gimple stmt,
                                int level) {
  ...
  if (!mi->allocation_function_decl)
    ...
    for (min_malloc_level = 0; min_malloc_level < mi->max_malloced_level &&
                               mi->malloc_for_level[min_malloc_level];
         min_malloc_level++)
      ;
  ...
  if (!mi->malloc_for_level) {
    ...
    mi->malloc_for_level = XRESIZEVEC(gimple, mi->malloc_for_level, level + 1);

    /* Zero the newly allocated items.  */
    memset(&(mi->malloc_for_level[mi->max_malloced_level + 1]), 0,
           (level - mi->max_malloced_level) * sizeof(tree));
...
static void analyze_matrix_allocation_site(struct matrix_info *mi, gimple stmt,
  ...
  /* A result of call to malloc.  */
    ...
    if (!(call_flags & ECF_MALLOC)) {
      ...
      if (DECL_FUNCTION_CODE(malloc_fn_decl) != BUILT_IN_MALLOC) {
        if (dump_file)
          fprintf(dump_file, "Matrix %s is an argument to function %s\n",
    ...
    /* This is a call to malloc of level 'level'.
       ...
       it escaping.  */
    if (mi->malloc_for_level && mi->max_malloced_level - 1 == level &&
        mi->malloc_for_level[level] != stmt) {
      mark_min_matrix_escape_level(mi, level, stmt);
      return;
    } else
...
static int analyze_transpose(void **slot, void *data ATTRIBUTE_UNUSED) {
  ...
  if (!mi->dim_hot_level)
    mi->dim_hot_level = (gcov_type *)xcalloc(min_escape_l, sizeof(gcov_type));

  for (i = 0; VEC_iterate(access_site_info_p, mi->access_l, i, acc_info); i++) {
...
static tree get_index_from_offset(tree offset, gimple def_stmt) {
  tree op1, op2, index;
  ...
  if ((gimple_assign_copy_p(def_stmt) || gimple_assign_cast_p(def_stmt)) &&
    ...
    return get_index_from_offset(
        offset, SSA_NAME_DEF_STMT(gimple_assign_rhs1(def_stmt)));
...
static void update_type_size(struct matrix_info *mi, gimple stmt, tree ssa_var,
  ...
  if (is_gimple_assign(stmt) &&
    ...
    gcc_assert(POINTER_TYPE_P(TREE_TYPE(SSA_NAME_VAR(TREE_OPERAND(lhs, 0)))));
    type_size = int_size_in_bytes(
        TREE_TYPE(TREE_TYPE(SSA_NAME_VAR(TREE_OPERAND(lhs, 0)))));
  } else
  ...
  if (type_size <= 0)
    ...
    if (!mi->dimension_type_size) {
      ...
      mi->dimension_type_size = (HOST_WIDE_INT *)xrealloc(
          mi->dimension_type_size, (l + 1) * sizeof(HOST_WIDE_INT));
      memset(&mi->dimension_type_size[mi->dimension_type_size_len], 0,
             (l + 1 - mi->dimension_type_size_len) * sizeof(HOST_WIDE_INT));
      mi->dimension_type_size_len = l + 1;
    }
...
static int analyze_accesses_for_call_stmt(struct matrix_info *mi, tree ssa_var,
                                          gimple use_stmt,
                                          int current_indirect_level) {
  ...
  if (fndecl) {
    if (DECL_FUNCTION_CODE(fndecl) != BUILT_IN_FREE) {
      if (dump_file)
        fprintf(dump_file, "Matrix %s: Function call %s, level %d escapes.\n",
...
static void analyze_accesses_for_phi_node(struct matrix_info *mi,
                                          gimple use_stmt,
                                          int current_indirect_level,
                                          sbitmap visited,
  ...
  if ((maphi = (struct matrix_access_phi_node *)htab_find(
           htab_mat_acc_phi_nodes, &tmp_maphi))) {
    if (maphi->indirection_level == current_indirect_level)
      return;
    else {
      int level = MIN(maphi->indirection_level, current_indirect_level);
      size_t j;
      gimple stmt = NULL;
  ...
  maphi = (struct matrix_access_phi_node *)xcalloc(
      1, sizeof(struct matrix_access_phi_node));
  maphi->phi = use_stmt;
  maphi->indirection_level = current_indirect_level;

  /* Insert to hash table.  */
  pmaphi = (struct matrix_access_phi_node **)htab_find_slot(
      htab_mat_acc_phi_nodes, maphi, INSERT);
  gcc_assert(pmaphi);
  *pmaphi = maphi;

  if (!TEST_BIT(visited, SSA_NAME_VERSION(PHI_RESULT(use_stmt)))) {
    SET_BIT(visited, SSA_NAME_VERSION(PHI_RESULT(use_stmt)));
    analyze_matrix_accesses(mi, PHI_RESULT(use_stmt), current_indirect_level,
                            false, visited, record_accesses);
...
static int analyze_accesses_for_assign_stmt(struct matrix_info *mi,
                                            tree ssa_var, gimple use_stmt,
                                            int current_indirect_level,
  ...
  if (lhs_acc.var_found) {
    ...
    if (!(gimple_assign_copy_p(use_stmt) || gimple_assign_cast_p(use_stmt)) ||
      ...
      if (record_accesses)
        record_access_alloc_site_info(mi, use_stmt, NULL_TREE, NULL_TREE, l,
                                      true);
  ...
  if (rhs_acc.var_found) {
    if (rhs_acc.t_code != INDIRECT_REF && rhs_acc.t_code != POINTER_PLUS_EXPR &&
        rhs_acc.t_code != SSA_NAME) {
      mark_min_matrix_escape_level(mi, current_indirect_level, use_stmt);
      return current_indirect_level;
    }
    ...
    if (rhs_acc.t_code == INDIRECT_REF) {
      if (record_accesses)
        record_access_alloc_site_info(mi, use_stmt, NULL_TREE, NULL_TREE,
      ...
      if (last_op)
        ...
        if (TREE_CODE(op2) == INTEGER_CST)
          index = build_int_cst(
              TREE_TYPE(op1),
              TREE_INT_CST_LOW(op2) / int_size_in_bytes(TREE_TYPE(op1)));
        else {
          index = get_index_from_offset(op2, SSA_NAME_DEF_STMT(op2));
          if (index == NULL_TREE) {
            mark_min_matrix_escape_level(mi, current_indirect_level, use_stmt);
            return current_indirect_level;
          }
        ...
        if (record_accesses)
          record_access_alloc_site_info(mi, use_stmt, op2, index,
    ...
    if (lhs_acc.t_code == INDIRECT_REF || TREE_CODE(lhs) != SSA_NAME) {
      ...
      analyze_matrix_accesses(mi, lhs, current_indirect_level,
                              rhs_acc.t_code == POINTER_PLUS_EXPR, visited,
                              record_accesses);
    }
  }
  return current_indirect_level;
...
/* Given a SSA_VAR (coming from a use statement of the matrix MI),
   ...
   resulting SSA name.  */
static void analyze_matrix_accesses(struct matrix_info *mi, tree ssa_var,
  ...
  /* Now go over the uses of the SSA_NAME and check how it is used in
     ...
     then a POINTER_PLUS_EXPR, then INDIRECT_REF etc.  while in between there
     could be any number of copies and casts.  */
  ...
  FOR_EACH_IMM_USE_FAST(use_p, imm_iter, ssa_var) {
    ...
    if (gimple_code(use_stmt) == GIMPLE_PHI)
      ...
      current_indirect_level = analyze_accesses_for_assign_stmt(
          mi, ssa_var, use_stmt, current_indirect_level, last_op, visited,
          record_accesses);
  }
}
...
/* A walk_tree function to go over the VAR_DECL, PARM_DECL nodes of
   ...
   over the function.  */
static tree check_var_notmodified_p(tree *tp, int *walk_subtrees, void *data) {
  basic_block bb;
  tree t = *tp;
...
static tree can_calculate_stmt_before_stmt(gimple stmt, sbitmap visited) {
  tree op1, op2, res;
  enum tree_code code;
...
/* Go backwards in the use-def chains and find out the expression
   ...
   we make sure that all the arguments represent the same subexpression,
   otherwise we fail.  */
static tree can_calculate_expr_before_stmt(tree expr, sbitmap visited) {
  gimple def_stmt;
  tree res;
...
static int check_allocation_function(void **slot, void *data ATTRIBUTE_UNUSED) {
  int level;
  gimple_stmt_iterator gsi;
  ...
  for (level = 1; level < mi->min_indirect_level_escape; level++) {
    ...
    if (size == NULL_TREE) {
      ...
      if (dump_file)
        fprintf(dump_file,
                "Matrix %s: Cannot calculate the size of allocation, escaping "
                "at level %d\n",
  ...
  for (level = mi->min_indirect_level_escape; level < mi->max_malloced_level;
       level++)
...
static void find_sites_in_func(bool record) {
  ...
  FOR_EACH_BB(bb) {
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (lhs != NULL_TREE && TREE_CODE(lhs) == VAR_DECL) {
        tmpmi.decl = lhs;
      ...
      if (is_gimple_assign(stmt) && gimple_assign_single_p(stmt) &&
        ...
        if ((mi = (struct matrix_info *)htab_find(matrices_to_reorg, &tmpmi))) {
          sbitmap_zero(visited_stmts_1);
          analyze_matrix_accesses(mi, lhs, 0, false, visited_stmts_1, record);
        }
      }
    }
  }
...
static void record_all_accesses_in_func(void) {
  unsigned i;
  sbitmap visited_stmts_1;
  ...
  for (i = 0; i < num_ssa_names; i++) {
    ...
    if (!ssa_var || !is_gimple_assign(SSA_NAME_DEF_STMT(ssa_var)) ||
    ...
    if ((mi = (struct matrix_info *)htab_find(matrices_to_reorg, &tmpmi))) {
      ...
      sbitmap_zero(visited_stmts_1);
      analyze_matrix_accesses(mi, ssa_var, 0, false, visited_stmts_1, true);
    }
  }
  sbitmap_free(visited_stmts_1);
}
...
static tree compute_offset(HOST_WIDE_INT orig, HOST_WIDE_INT new_val,
                           tree result) {
  int x, y;
  ...
  if (x != -1 && y != -1) {
    if (x == y)
      ...
      result1 = fold_build2(LSHIFT_EXPR, TREE_TYPE(result), result, log);
      return result1;
    }
...
static int transform_access_sites(void **slot, void *data ATTRIBUTE_UNUSED) {
  gimple_stmt_iterator gsi;
  struct matrix_info *mi = (struct matrix_info *)*slot;
  int min_escape_l = mi->min_indirect_level_escape;
  struct access_site_info *acc_info;
  ...
  for (i = 0; VEC_iterate(access_site_info_p, mi->access_l, i, acc_info); i++) {
    ...
    if (acc_info->is_alloc) {
      if (acc_info->level >= 0 && gimple_bb(acc_info->stmt)) {
        ...
        if (TREE_CODE(lhs) == SSA_NAME && acc_info->level < min_escape_l - 1) {
          imm_use_iterator imm_iter;
          use_operand_p use_p;
          gimple use_stmt;
          ...
          FOR_EACH_IMM_USE_ON_STMT(use_p, imm_iter) {
            ...
            gcc_assert(gimple_assign_rhs_code(acc_info->stmt) == INDIRECT_REF);
            ...
            new_stmt = gimple_build_assign(tmp, TREE_OPERAND(rhs, 0));
    ...
    if (code == INDIRECT_REF && acc_info->level < min_escape_l - 1) {
      /* Replace the INDIRECT_REF with NOP (cast) usually we are casting
         from "pointer to type" to "type".  */
      tree t = build1(NOP_EXPR, TREE_TYPE(gimple_assign_rhs1(acc_info->stmt)),
    ...
    } else if (code == POINTER_PLUS_EXPR && acc_info->level < (min_escape_l)) {
      imm_use_iterator imm_iter;
      use_operand_p use_p;
      ...
      if (mi->dim_map[k] == min_escape_l - 1) {
        if (!check_transpose_p || mi->is_transposed_p == false)
          ...
          new_offset = compute_offset(mi->dimension_type_size[min_escape_l],
                                      mi->dimension_type_size[k + 1], offset);

          total_elements = new_offset;
          if (new_offset != offset) {
            gsi = gsi_for_stmt(acc_info->stmt);
            tmp1 = force_gimple_operand_gsi(&gsi, total_elements, true, NULL,
                                            true, GSI_SAME_STMT);
          } else
            tmp1 = offset;
        }
        ...
        num_elements = fold_build2(MULT_EXPR, sizetype,
                                   fold_convert(sizetype, acc_info->index),
                                   fold_convert(sizetype, d_size));
        add_referenced_var(d_size);
        gsi = gsi_for_stmt(acc_info->stmt);
        tmp1 = force_gimple_operand_gsi(&gsi, num_elements, true, NULL, true,
                                        GSI_SAME_STMT);
      }
...
static void sort_dim_hot_level(gcov_type *a, int *dim_map, int n) {
  int i, j, tmp1;
  gcov_type tmp;
...
static int transform_allocation_sites(void **slot,
  ...
  if (check_transpose_p) {
    ...
    if (dump_file) {
      ...
      for (i = 0; i < min_escape_l; i++) {
        ...
        if (mi->dim_hot_level)
          fprintf(dump_file, "count is  " HOST_WIDEST_INT_PRINT_DEC "  \n",
                  mi->dim_hot_level[i]);
      }
    }
    ...
    for (i = 0; i < mi->min_indirect_level_escape; i++) {
      ...
      if (mi->dim_map[i] != i) {
        if (dump_file)
          fprintf(dump_file, "Transposed dimensions: dim %d is now dim %d\n",
                  mi->dim_map[i], i);
  ...
  for (i = mi->min_indirect_level_escape - 1; i >= 0; i--) {
    ...
    dim_var =
        add_new_static_var(TREE_TYPE(mi->dimension_size_orig[mi->dim_map[i]]));
    ...
    for (id = 0; id < mi->min_indirect_level_escape; id++)
      if (mi->dim_map[id] == i)
        break;
    d_type_size = build_int_cst(type, mi->dimension_type_size[id + 1]);
    if (!prev_dim_size)
    ...
    if (!check_transpose_p && i == mi->min_indirect_level_escape - 1) {
      ...
      dim_size = fold_build2(TRUNC_DIV_EXPR, type, mi->dimension_size_orig[id],
                             d_type_size);

      dim_size = fold_build2(MULT_EXPR, type, dim_size, prev_dim_size);
    }
    dim_size = force_gimple_operand_gsi(&gsi, dim_size, true, NULL, true,
                                        GSI_SAME_STMT);
  ...
  tmp = force_gimple_operand_gsi(&gsi, mi->dimension_size[0], true, NULL, true,
                                 GSI_SAME_STMT);
  ...
  for (i = 1; i < mi->min_indirect_level_escape; i++) {
    ...
    FOR_EACH_IMM_USE_STMT(use_stmt, imm_iter, gimple_call_lhs(call_stmt)) {
      use_stmt1 = use_stmt;
      gsi = gsi_for_stmt(use_stmt);
      gsi_remove(&gsi, true);
    }
    /* Remove the assignment of the allocated area.  */
    FOR_EACH_IMM_USE_STMT(use_stmt, imm_iter, gimple_get_lhs(use_stmt1)) {
      gsi = gsi_for_stmt(use_stmt);
      gsi_remove(&gsi, true);
    }
  }
...
static int dump_matrix_reorg_analysis(void **slot,
                                      void *data ATTRIBUTE_UNUSED) {
  struct matrix_info *mi = (struct matrix_info *)*slot;
...
static unsigned int matrix_reorg(void) {
  ...
  for (node = cgraph_nodes; node; node = node->next)
    if (node->analyzed) {
      ...
      htab_mat_acc_phi_nodes =
          htab_create(37, mat_acc_phi_hash, mat_acc_phi_eq, free);
      if (!check_transpose_p)
