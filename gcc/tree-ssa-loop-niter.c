  ...
  switch (TREE_CODE(expr)) {
    ...
    case POINTER_PLUS_EXPR:
      ...
      *var = op0;
      ...
      off = double_int_sext(tree_to_double_int(op1), TYPE_PRECISION(type));
...
static void determine_value_range(tree type, tree var, mpz_t off, mpz_t min,
                                  mpz_t max) {
...
static void bound_difference_of_offsetted_base(tree type, mpz_t x, mpz_t y,
                                               bounds *bnds) {
  int rel = mpz_cmp(x, y);
...
static void refine_bounds_using_guard(tree type, tree varx, mpz_t offx,
                                      tree vary, mpz_t offy, tree c0,
                                      enum tree_code cmp, tree c1,
                                      bounds *bnds) {
  ...
  switch (cmp) {
    ...
    case NE_EXPR:
      /* NE_EXPR comparisons do not contain much of useful information, except
         for special case of comparing with the bounds of the type.  */
      if (TREE_CODE(c1) != INTEGER_CST || !INTEGRAL_TYPE_P(type))
        return;
  ...
  if (!operand_equal_p(varx, varc0, 0) || !operand_equal_p(vary, varc1, 0))
    goto end;
  ...
  if (no_wrap) {
    ...
    x_ok = (integer_zerop(varx) || mpz_cmp(loffx, offc0) >= 0);
    y_ok = (integer_zerop(vary) || mpz_cmp(loffy, offc1) <= 0);
  }
...
static void bound_difference(struct loop *loop, tree x, tree y, bounds *bnds) {
  ...
  if (!integer_zerop(varx) && operand_equal_p(varx, vary, 0)) {
  ...
  for (bb = loop->header; bb != ENTRY_BLOCK_PTR && cnt < MAX_DOMINATORS_TO_WALK;
    ...
    refine_bounds_using_guard(type, varx, offx, vary, offy, c0, cmp, c1, bnds);
    ++cnt;
  }
...
static void bounds_add(bounds *bnds, double_int delta, tree type) {
  mpz_t mdelta, max;
...
static void number_of_iterations_ne_max(mpz_t bnd, bool no_overflow, tree c,
                                        tree s, bounds *bnds) {
  double_int max;
  mpz_t d;
...
static bool number_of_iterations_ne(tree type, affine_iv *iv, tree final,
                                    struct tree_niter_desc *niter,
                                    bool never_infinite, bounds *bnds) {
  ...
  if (tree_int_cst_sign_bit(iv->step)) {
    s = fold_convert(niter_type, fold_build1(NEGATE_EXPR, type, iv->step));
    c = fold_build2(MINUS_EXPR, niter_type, fold_convert(niter_type, iv->base),
                    fold_convert(niter_type, final));
    bounds_negate(bnds);
  } else {
    s = fold_convert(niter_type, iv->step);
    c = fold_build2(MINUS_EXPR, niter_type, fold_convert(niter_type, final),
  ...
  bound = build_low_bits_mask(
      niter_type, (TYPE_PRECISION(niter_type) - tree_low_cst(bits, 1)));
  ...
  if (!never_infinite) {
    ...
    assumption = fold_build2(EQ_EXPR, boolean_type_node, assumption,
...
static bool number_of_iterations_lt_to_ne(tree type, affine_iv *iv0,
                                          affine_iv *iv1,
                                          struct tree_niter_desc *niter,
  ...
  if (integer_nonzerop(iv0->step)) {
    ...
    if (!iv0->no_overflow && !integer_zerop(mod)) {
      bound = fold_build2(MINUS_EXPR, type1, TYPE_MAX_VALUE(type1), tmod);
      if (POINTER_TYPE_P(type))
        bound = fold_convert(type, bound);
      assumption = fold_build2(LE_EXPR, boolean_type_node, iv1->base, bound);
    ...
    if (mpz_cmp(mmod, bnds->below) < 0)
      ...
      noloop =
          fold_build2(GT_EXPR, boolean_type_node, iv0->base,
                      fold_build2(POINTER_PLUS_EXPR, type, iv1->base, tmod));
    else
      noloop = fold_build2(GT_EXPR, boolean_type_node, iv0->base,
                           fold_build2(PLUS_EXPR, type1, iv1->base, tmod));
  } else {
    /* The final value of the iv is iv0->base - MOD, assuming that this
       computation does not overflow, and that
       iv0->base - MOD <= iv1->base. */
    if (!iv1->no_overflow && !integer_zerop(mod)) {
      bound = fold_build2(PLUS_EXPR, type1, TYPE_MIN_VALUE(type1), tmod);
      if (POINTER_TYPE_P(type))
        bound = fold_convert(type, bound);
      assumption = fold_build2(GE_EXPR, boolean_type_node, iv0->base, bound);
      if (integer_zerop(assumption))
        goto end;
    }
    if (mpz_cmp(mmod, bnds->below) < 0)
      ...
      noloop = fold_build2(GT_EXPR, boolean_type_node,
                           fold_build2(POINTER_PLUS_EXPR, type, iv0->base,
                                       fold_build1(NEGATE_EXPR, type1, tmod)),
                           iv1->base);
    else
      noloop = fold_build2(GT_EXPR, boolean_type_node,
                           fold_build2(MINUS_EXPR, type1, iv0->base, tmod),
                           iv1->base);
  }
...
static bool assert_no_overflow_lt(tree type, affine_iv *iv0, affine_iv *iv1,
  ...
  if (integer_nonzerop(iv0->step)) {
    ...
    bound = fold_build2(MINUS_EXPR, type, TYPE_MAX_VALUE(type),
                        fold_convert(type, diff));
    assumption = fold_build2(LE_EXPR, boolean_type_node, iv1->base, bound);
  } else {
    ...
    if (TREE_CODE(iv1->base) == INTEGER_CST) {
      ...
      diff = fold_build2(MINUS_EXPR, niter_type, step,
                         build_int_cst(niter_type, 1));
    bound = fold_build2(PLUS_EXPR, type, TYPE_MIN_VALUE(type),
                        fold_convert(type, diff));
    assumption = fold_build2(GE_EXPR, boolean_type_node, iv0->base, bound);
  }
...
static void assert_loop_rolls_lt(tree type, affine_iv *iv0, affine_iv *iv1,
  ...
  if (integer_nonzerop(iv0->step))
    ...
    dstep =
        double_int_sext(tree_to_double_int(iv1->step), TYPE_PRECISION(type));
  ...
  if (integer_nonzerop(iv0->step)) {
    diff = fold_build2(MINUS_EXPR, type1, iv0->step, build_int_cst(type1, 1));

    /* We need to know that iv0->base >= MIN + iv0->step - 1.  Since
       0 address never belongs to any object, we can assume this for
       pointers.  */
    if (!POINTER_TYPE_P(type)) {
      bound = fold_build2(PLUS_EXPR, type1, TYPE_MIN_VALUE(type), diff);
      assumption = fold_build2(GE_EXPR, boolean_type_node, iv0->base, bound);
    }

    /* And then we can compute iv0->base - diff, and compare it with
       iv1->base.  */
    mbzl = fold_build2(MINUS_EXPR, type1, fold_convert(type1, iv0->base), diff);
    mbzr = fold_convert(type1, iv1->base);
  } else {
    diff = fold_build2(PLUS_EXPR, type1, iv1->step, build_int_cst(type1, 1));

    if (!POINTER_TYPE_P(type)) {
      bound = fold_build2(PLUS_EXPR, type1, TYPE_MAX_VALUE(type), diff);
      assumption = fold_build2(LE_EXPR, boolean_type_node, iv1->base, bound);
    }

    mbzl = fold_convert(type1, iv0->base);
    mbzr = fold_build2(MINUS_EXPR, type1, fold_convert(type1, iv1->base), diff);
  }
  ...
  if (!rolls_p) {
    ...
    niter->may_be_zero =
        fold_build2(TRUTH_OR_EXPR, boolean_type_node, niter->may_be_zero, mbz);
  }
}
...
static bool number_of_iterations_lt(tree type, affine_iv *iv0, affine_iv *iv1,
                                    struct tree_niter_desc *niter,
  ...
  delta =
      fold_build2(MINUS_EXPR, niter_type, fold_convert(niter_type, iv1->base),
                  fold_convert(niter_type, iv0->base));
  ...
  if ((integer_onep(iv0->step) && integer_zerop(iv1->step)) ||
    ...
    if (mpz_sgn(bnds->below) < 0)
      niter->may_be_zero =
          fold_build2(LT_EXPR, boolean_type_node, iv1->base, iv0->base);
    niter->niter = delta;
  ...
  if (integer_nonzerop(iv0->step))
    ...
    step = fold_convert(niter_type, fold_build1(NEGATE_EXPR, type, iv1->step));
  ...
  s = fold_build2(MINUS_EXPR, niter_type, step, build_int_cst(niter_type, 1));
...
static bool number_of_iterations_le(tree type, affine_iv *iv0, affine_iv *iv1,
                                    struct tree_niter_desc *niter,
                                    bool never_infinite, bounds *bnds) {
  tree assumption;
  tree type1 = type;
  ...
  if (!never_infinite) {
    if (integer_nonzerop(iv0->step))
      assumption = fold_build2(NE_EXPR, boolean_type_node, iv1->base,
                               TYPE_MAX_VALUE(type));
    else
      assumption = fold_build2(NE_EXPR, boolean_type_node, iv0->base,
                               TYPE_MIN_VALUE(type));
  ...
  if (integer_nonzerop(iv0->step)) {
    if (POINTER_TYPE_P(type))
      ...
      iv1->base =
          fold_build2(PLUS_EXPR, type1, iv1->base, build_int_cst(type1, 1));
  } else if (POINTER_TYPE_P(type))
    iv0->base =
        fold_build2(POINTER_PLUS_EXPR, type, iv0->base,
                    fold_build1(NEGATE_EXPR, type1, build_int_cst(type1, 1)));
  else
    iv0->base =
        fold_build2(MINUS_EXPR, type1, iv0->base, build_int_cst(type1, 1));
...
static bool number_of_iterations_cond(struct loop *loop, tree type,
                                      affine_iv *iv0, enum tree_code code,
                                      affine_iv *iv1,
                                      struct tree_niter_desc *niter,
                                      bool only_exit) {
  ...
  if (!integer_zerop(iv0->step) && !integer_zerop(iv1->step)) {
    ...
    iv0->step = fold_binary_to_constant(MINUS_EXPR, type, iv0->step, iv1->step);
    iv0->no_overflow = false;
  ...
  if (integer_zerop(
          fold_build2(code, boolean_type_node, iv0->base, iv1->base))) {
  ...
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    fprintf(dump_file, "Analyzing # of iterations of loop %d\n", loop->num);
  ...
  switch (code) {
    case NE_EXPR:
      ...
      ret = number_of_iterations_ne(type, iv0, iv1->base, niter, never_infinite,
                                    &bnds);
      break;

    case LT_EXPR:
      ret =
          number_of_iterations_lt(type, iv0, iv1, niter, never_infinite, &bnds);
      break;

    case LE_EXPR:
      ret =
          number_of_iterations_le(type, iv0, iv1, niter, never_infinite, &bnds);
      break;
...
static tree simplify_replace_tree(tree expr, tree old, tree new_tree) {
  unsigned i, n;
  ...
  if (expr == old || operand_equal_p(expr, old, 0))
...
tree expand_simple_operations(tree expr) {
  ...
  if (gimple_code(stmt) == GIMPLE_PHI) {
    ...
    if (TREE_CODE(e) == SSA_NAME && src->loop_father != dest->loop_father)
      return expr;
...
static tree tree_simplify_using_condition_1(tree cond, tree expr) {
  ...
  if (code == TRUTH_OR_EXPR || code == TRUTH_AND_EXPR || code == COND_EXPR) {
    changed = false;
...
static tree simplify_using_initial_conditions(struct loop *loop, tree expr) {
  edge e;
  basic_block bb;
  gimple stmt;
  tree cond;
...
static tree simplify_using_outer_evolutions(struct loop *loop, tree expr) {
  enum tree_code code = TREE_CODE(expr);
  bool changed;
  tree e, e0, e1, e2;
...
bool loop_only_exit_p(const struct loop *loop, const_edge exit) {
  basic_block *body;
...
bool number_of_iterations_exit(struct loop *loop, edge exit,
                               struct tree_niter_desc *niter, bool warn) {
  gimple stmt;
  tree type;
  tree op0, op1;
  enum tree_code code;
  ...
  if (TREE_CODE(type) != INTEGER_TYPE && !POINTER_TYPE_P(type))
    return false;
  ...
  if (optimize >= 3) {
    niter->assumptions =
        simplify_using_outer_evolutions(loop, niter->assumptions);
    niter->may_be_zero =
        simplify_using_outer_evolutions(loop, niter->may_be_zero);
...
tree find_loop_niter(struct loop *loop, edge *exit) {
  unsigned i;
...
static gimple chain_of_csts_start(struct loop *loop, tree x) {
...
static gimple get_base_for(struct loop *loop, tree x) {
  gimple phi;
  tree init, next;
...
static tree get_val_for(tree x, tree base) {
  ...
  if (gimple_assign_ssa_name_copy_p(stmt))
    ...
    return fold_build1(gimple_assign_rhs_code(stmt), gimple_expr_type(stmt),
    ...
    if (TREE_CODE(rhs1) == SSA_NAME)
      ...
      gcc_unreachable();
    return fold_build2(gimple_assign_rhs_code(stmt), gimple_expr_type(stmt),
                       rhs1, rhs2);
  } else
    gcc_unreachable();
}
...
tree loop_niter_by_eval(struct loop *loop, edge exit) {
  tree acnd;
...
tree find_loop_niter_by_eval(struct loop *loop, edge *exit) {
  unsigned i;
  VEC(edge, heap) *exits = get_loop_exit_edges(loop);
  edge ex;
  tree niter = NULL_TREE, aniter;
  ...
  *exit = NULL;
  for (i = 0; VEC_iterate(edge, exits, i, ex); i++) {
    ...
    if (niter && !tree_int_cst_lt(aniter, niter))
      continue;
...
static double_int derive_constant_upper_bound_ops(tree, tree, enum tree_code,
                                                  tree);

/* Returns a constant upper bound on the value of the right-hand side of
   an assignment statement STMT.  */

static double_int derive_constant_upper_bound_assign(gimple stmt) {
...
static double_int derive_constant_upper_bound_ops(tree type, tree op0,
                                                  enum tree_code code,
                                                  tree op1) {
  tree subtype, maxt;
  ...
  switch (code) {
    ...
    case POINTER_PLUS_EXPR:
    case MINUS_EXPR:
      if (TREE_CODE(op1) != INTEGER_CST || !tree_expr_nonnegative_p(op0))
        return max;
    ...
    case FLOOR_DIV_EXPR:
    case EXACT_DIV_EXPR:
      if (TREE_CODE(op1) != INTEGER_CST || tree_int_cst_sign_bit(op1))
        return max;
    ...
    case SSA_NAME:
      ...
      if (gimple_code(stmt) != GIMPLE_ASSIGN || gimple_assign_lhs(stmt) != op0)
        return max;
...
static void record_niter_bound(struct loop *loop, double_int i_bound,
...
static void record_estimate(struct loop *loop, tree bound, double_int i_bound,
                            gimple at_stmt, bool is_exit, bool realistic,
                            bool upper) {
  double_int delta;
  edge exit;
  ...
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    ...
    fprintf(dump_file, " is %sexecuted at most ", upper ? "" : "probably ");
  ...
  if (is_exit || (exit != NULL && dominated_by_p(CDI_DOMINATORS, exit->src,
                                                 gimple_bb(at_stmt))))
...
static void record_nonwrapping_iv(struct loop *loop, tree base, tree step,
                                  gimple stmt, tree low, tree high,
                                  bool realistic, bool upper) {
...
static bool array_at_struct_end_p(tree ref) {
...
static bool idx_infer_loop_bounds(tree base, tree *idx, void *dta) {
  ...
  if (!init || !step || TREE_CODE(step) != INTEGER_CST || integer_zerop(step) ||
  ...
  /* The case of nonconstant bounds could be handled, but it would be
     complicated.  */
  if (TREE_CODE(low) != INTEGER_CST || !high || TREE_CODE(high) != INTEGER_CST)
    return true;
  ...
  /* The array of length 1 at the end of a structure most likely extends
     beyond its bounds.  */
  if (at_end && operand_equal_p(low, high, 0))
    return true;
...
static void infer_loop_bounds_from_ref(struct loop *loop, gimple stmt, tree ref,
                                       bool reliable) {
...
static void infer_loop_bounds_from_array(struct loop *loop, gimple stmt,
                                         bool reliable) {
...
static void infer_loop_bounds_from_signedness(struct loop *loop, gimple stmt) {
  ...
  if (!base || !step || TREE_CODE(step) != INTEGER_CST ||
...
static void infer_loop_bounds_from_undefined(struct loop *loop) {
  unsigned i;
  basic_block *bbs;
  gimple_stmt_iterator bsi;
  basic_block bb;
...
static double_int gcov_type_to_double_int(gcov_type val) {
  double_int ret;
...
void estimate_numbers_of_iterations_loop(struct loop *loop) {
  VEC(edge, heap) * exits;
  tree niter, type;
  unsigned i;
  ...
  for (i = 0; VEC_iterate(edge, exits, i, ex); i++) {
    ...
    if (TREE_CODE(niter_desc.may_be_zero) != INTEGER_CST)
      niter = build3(COND_EXPR, type, niter_desc.may_be_zero,
                     build_int_cst(type, 0), niter);
    record_estimate(loop, niter, niter_desc.max, last_stmt(ex->src), true, true,
                    true);
  }
  VEC_free(edge, heap, exits);
  ...
  if (loop->any_upper_bound && loop->any_estimate &&
...
static bool n_of_executions_at_most(gimple stmt,
  ...
  if (niter_bound->is_exit) {
    if (stmt && stmt != niter_bound->stmt &&
        stmt_dominates_stmt_p(niter_bound->stmt, stmt))
      cmp = GE_EXPR;
    else
      cmp = GT_EXPR;
  } else {
    if (!stmt || (gimple_bb(stmt) != gimple_bb(niter_bound->stmt) &&
                  !stmt_dominates_stmt_p(niter_bound->stmt, stmt))) {
  ...
  e = fold_binary(cmp, boolean_type_node, niter,
...
bool scev_probably_wraps_p(tree base, tree step, gimple at_stmt,
                           struct loop *loop, bool use_overflow_semantics) {
  ...
  if (chrec_contains_undetermined(base) || chrec_contains_undetermined(step))
    return true;
  ...
  if (tree_int_cst_sign_bit(step)) {
    tree extreme = fold_convert(unsigned_type, lower_bound_in_type(type, type));
    delta = fold_build2(MINUS_EXPR, unsigned_type, base, extreme);
    step_abs = fold_build1(NEGATE_EXPR, unsigned_type,
                           fold_convert(unsigned_type, step));
  } else {
    tree extreme = fold_convert(unsigned_type, upper_bound_in_type(type, type));
    delta = fold_build2(MINUS_EXPR, unsigned_type, extreme, base);
...
void free_numbers_of_iterations_estimates_loop(struct loop *loop) {
...
void substitute_in_loop_info(struct loop *loop, tree name, tree val) {
