...
static unsigned int tree_nrv(void) {
  ...
  FOR_EACH_BB(bb) {
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (gimple_code(stmt) == GIMPLE_RETURN) {
      ...
      } else if (gimple_has_lhs(stmt) && gimple_get_lhs(stmt) == result) {
        tree rhs;
        ...
        if (TREE_CODE(found) != VAR_DECL || TREE_THIS_VOLATILE(found) ||
            DECL_CONTEXT(found) != current_function_decl ||
            TREE_STATIC(found) || TREE_ADDRESSABLE(found) ||
  ...
  FOR_EACH_BB(bb) {
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi);) {
      ...
      if (gimple_assign_copy_p(stmt) && gimple_assign_lhs(stmt) == result &&
...
static bool gate_pass_return_slot(void) {
  return optimize > 0;
}

struct gimple_opt_pass pass_nrv = {{
    GIMPLE_PASS, "nrv",               /* name */
...
static bool dest_safe_for_nrv_p(tree dest) {
...
static unsigned int execute_return_slot_opt(void) {
  basic_block bb;
