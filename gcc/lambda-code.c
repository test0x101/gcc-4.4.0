...
/* This loop nest code generation is based on non-singular matrix
 ...
 A loop iteration space represents the points traversed by the loop.  A point in
 the iteration space can be represented by a vector of size <loop depth>.  You
 can therefore represent the iteration space as an integral combinations of a
...
static bool perfect_nestify(struct loop *, VEC(tree, heap) *, VEC(tree, heap) *,
                            VEC(int, heap) *, VEC(tree, heap) *);
...
static bool lle_equal(lambda_linear_expression, lambda_linear_expression, int,
                      int);
...
lambda_body_vector lambda_body_vector_new(int size,
...
lambda_body_vector lambda_body_vector_compute_new(
    lambda_trans_matrix transform, lambda_body_vector vect,
    struct obstack *lambda_obstack) {
...
void print_lambda_body_vector(FILE *outfile, lambda_body_vector body) {
  print_lambda_vector(outfile, LBV_COEFFICIENTS(body), LBV_SIZE(body));
}

/* Return TRUE if two linear expressions are equal.  */

static bool lle_equal(lambda_linear_expression lle1,
                      lambda_linear_expression lle2, int depth,
                      int invariants) {
  int i;
...
lambda_linear_expression lambda_linear_expression_new(
    int dim, int invariants, struct obstack *lambda_obstack) {
  lambda_linear_expression ret;

  ret = (lambda_linear_expression)obstack_alloc(lambda_obstack, sizeof(*ret));
...
static void print_linear_expression(FILE *outfile, lambda_vector expr, int size,
                                    char start) {
  int i;
  bool first = true;
  for (i = 0; i < size; i++) {
...
void print_lambda_linear_expression(FILE *outfile,
                                    lambda_linear_expression expr, int depth,
  ...
  print_linear_expression(outfile, LLE_INVARIANT_COEFFICIENTS(expr), invariants,
                          'A');
...
void print_lambda_loop(FILE *outfile, lambda_loop loop, int depth,
                       int invariants, char start) {
  int step;
...
void print_lambda_loopnest(FILE *outfile, lambda_loopnest nest, char start) {
  int i;
...
static lambda_lattice lambda_lattice_new(int depth, int invariants,
                                         struct obstack *lambda_obstack) {
  lambda_lattice ret =
...
static lambda_lattice lambda_lattice_compute_base(
    lambda_loopnest nest, struct obstack *lambda_obstack) {
  lambda_lattice ret;
  ...
  for (i = 0; i < depth; i++) {
    ...
    if (step == 1) {
      ...
      for (j = 0; j < i; j++)
        base[i][j] =
            LLE_COEFFICIENTS(expression)[j] * LL_STEP(LN_LOOPS(nest)[j]);
      base[i][i] = step;
      for (j = i + 1; j < depth; j++)
        base[i][j] = 0;
      ...
      for (j = 0; j < invariants; j++)
        LATTICE_ORIGIN_INVARIANTS(ret)
        [i][j] = LLE_INVARIANT_COEFFICIENTS(expression)[j];
    }
  }
  return ret;
}
...
static lambda_loopnest compute_nest_using_fourier_motzkin(
    int size, int depth, int invariants, lambda_matrix A, lambda_matrix B,
    lambda_vector a, struct obstack *lambda_obstack) {
  int multiple, f1, f2;
  int i, j, k;
  lambda_linear_expression expression;
  lambda_loop loop;
  ...
  for (i = depth - 1; i >= 0; i--) {
    ...
    for (j = 0; j < size; j++) {
      if (A[j][i] < 0) {
        ...
        expression =
            lambda_linear_expression_new(depth, invariants, lambda_obstack);
        ...
        if (!lle_equal(LL_LOWER_BOUND(loop), expression, depth, invariants)) {
        ...
        if (!lle_equal(LL_UPPER_BOUND(loop), expression, depth, invariants)) {
      ...
      if (A[j][i] == 0) {
        ...
        for (k = 0; k < size; k++) {
          if (A[k][i] < 0) {
            ...
            lambda_vector_add_mc(A[j], f1, A[k], f2, A1[newsize], depth);
            lambda_vector_add_mc(B[j], f1, B[k], f2, B1[newsize], invariants);
...
static lambda_loopnest lambda_compute_auxillary_space(
    lambda_loopnest nest, lambda_trans_matrix trans,
    struct obstack *lambda_obstack) {
  ...
  lambda_matrix_inverse(LTM_MATRIX(trans), invertedtrans, depth);

  /* A = A1 inv(U).  */
  lambda_matrix_mult(A1, invertedtrans, A, size, depth, depth);

  return compute_nest_using_fourier_motzkin(size, depth, invariants, A, B1, a1,
                                            lambda_obstack);
}
...
static lambda_loopnest lambda_compute_target_space(
    lambda_loopnest auxillary_nest, lambda_trans_matrix H,
    lambda_vector stepsigns, struct obstack *lambda_obstack) {
  ...
  for (i = 0; i < depth; i++) {
    ...
    for (; auxillary_expr != NULL; auxillary_expr = LLE_NEXT(auxillary_expr)) {
      target_expr =
          lambda_linear_expression_new(depth, invariants, lambda_obstack);
      lambda_vector_matrix_mult(LLE_COEFFICIENTS(auxillary_expr), depth,
                                inverse, depth, LLE_COEFFICIENTS(target_expr));
      lambda_vector_mult_const(LLE_COEFFICIENTS(target_expr),
                               LLE_COEFFICIENTS(target_expr), depth, factor);

      LLE_CONSTANT(target_expr) = LLE_CONSTANT(auxillary_expr) * factor;
      lambda_vector_copy(LLE_INVARIANT_COEFFICIENTS(auxillary_expr),
                         LLE_INVARIANT_COEFFICIENTS(target_expr), invariants);
      ...
      if (!lambda_vector_zerop(LLE_COEFFICIENTS(target_expr), depth)) {
        LLE_CONSTANT(target_expr) = LLE_CONSTANT(target_expr) * determinant;
        lambda_vector_mult_const(LLE_INVARIANT_COEFFICIENTS(target_expr),
                                 LLE_INVARIANT_COEFFICIENTS(target_expr),
...
static lambda_vector lambda_compute_step_signs(lambda_trans_matrix trans,
...
lambda_loopnest lambda_loopnest_transform(lambda_loopnest nest,
  ...
  target_nest =
      lambda_compute_target_space(auxillary_nest, H, stepsigns, lambda_obstack);
...
static lambda_linear_expression gcc_tree_to_linear_expression(
    int depth, tree expr, VEC(tree, heap) * outerinductionvars,
    VEC(tree, heap) * invariants, int extra, struct obstack *lambda_obstack) {
      ...
      for (i = 0; VEC_iterate(tree, outerinductionvars, i, iv); i++)
        if (iv != NULL) {
          if (SSA_NAME_VAR(iv) == SSA_NAME_VAR(expr)) {
            lle =
                lambda_linear_expression_new(depth, 2 * depth, lambda_obstack);
...
static int depth_of_nest(struct loop *nest) {
  size_t depth = 0;
  while (nest) {
    depth++;
    nest = nest->inner;
...
static bool invariant_in_loop_and_outer_loops(struct loop *loop, tree op) {
...
static lambda_loop gcc_loop_to_lambda_loop(
    struct loop *loop, int depth, VEC(tree, heap) * *invariants,
    tree *ourinductionvar, VEC(tree, heap) * outerinductionvars,
    VEC(tree, heap) * *lboundvars, VEC(tree, heap) * *uboundvars,
    VEC(int, heap) * *steps, struct obstack *lambda_obstack) {
  gimple phi;
  gimple exit_cond;
  ...
  if (inductionvar == NULL || exit_cond == NULL) {
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file,
              "Unable to convert loop: Cannot determine exit condition or "
              "induction variable for loop.\n");
    return NULL;
  }
  ...
  if (SSA_NAME_DEF_STMT(inductionvar) == NULL) {
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file,
              "Unable to convert loop: Cannot find PHI node for induction "
              "variable\n");
  ...
  *ourinductionvar = PHI_RESULT(phi);
  ...
  if (access_fn == chrec_dont_know) {
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file,
              "Unable to convert loop: Access function for induction variable "
              "phi is unknown\n");
  ...
  if (gimple_phi_num_args(phi) != 2) {
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file,
              "Unable to convert loop: PHI node for induction variable has >2 "
              "arguments\n");
    return NULL;
  }
  ...
  if (flow_bb_inside_loop_p(loop, gimple_phi_arg_edge(phi, 0)->src) &&
    ...
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file,
              "Unable to convert loop: PHI edges both inside loop, or both "
              "outside loop.\n");
  ...
  if (flow_bb_inside_loop_p(loop, gimple_phi_arg_edge(phi, 0)->src)) {
    ...
    lbound = gcc_tree_to_linear_expression(depth, lboundvar, outerinductionvars,
                                           *invariants, 0, lambda_obstack);
  } else {
  ...
  if (!lbound) {
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file,
              "Unable to convert loop: Cannot convert lower bound to linear "
              "expression\n");
  ...
  ubound = gcc_tree_to_linear_expression(depth, uboundvar, outerinductionvars,
  ...
  if (!ubound) {
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file,
              "Unable to convert loop: Cannot convert upper bound to linear "
              "expression\n");
    return NULL;
  }
...
tree find_induction_var_from_exit_cond(struct loop *loop) {
...
lambda_loopnest gcc_loopnest_to_lambda_loopnest(
    struct loop *loop_nest, VEC(tree, heap) * *inductionvars,
    VEC(tree, heap) * *invariants, struct obstack *lambda_obstack) {
  ...
  while (temp) {
    newloop = gcc_loop_to_lambda_loop(temp, depth, invariants, &inductionvar,
                                      *inductionvars, &lboundvars, &uboundvars,
...
static tree lbv_to_gcc_expression(lambda_body_vector lbv, tree type,
...
static tree lle_to_gcc_expression(lambda_linear_expression lle,
                                  lambda_linear_expression offset, tree type,
                                  VEC(tree, heap) * induction_vars,
  ...
  for (; lle != NULL; lle = LLE_NEXT(lle)) {
    ...
    expr = fold_build2(
        PLUS_EXPR, type, expr,
        build_linear_expr(type, LLE_INVARIANT_COEFFICIENTS(lle), invariants));
...
void lambda_loopnest_to_gcc_loopnest(struct loop *old_loopnest,
  ...
  while (temp) {
    ...
    stmts = NULL;
    newlowerbound = lle_to_gcc_expression(
        LL_LOWER_BOUND(newloop), LL_LINEAR_OFFSET(newloop), type, new_ivs,
    ...
    stmts = NULL;
    newupperbound = lle_to_gcc_expression(
        LL_UPPER_BOUND(newloop), LL_LINEAR_OFFSET(newloop), type, new_ivs,
    ...
    create_iv(newlowerbound, build_int_cst(type, LL_STEP(newloop)), ivvar, temp,
              &bsi, insert_after, &ivvar, NULL);
    ...
    inc_stmt =
        gimple_build_assign_with_ops(PLUS_EXPR, SSA_NAME_VAR(ivvar), ivvar,
  ...
  for (i = 0; VEC_iterate(tree, old_ivs, i, oldiv); i++) {
    ...
    FOR_EACH_IMM_USE_STMT(stmt, imm_iter, oldiv_def) {
      ...
      stmts = NULL;
      newiv = lbv_to_gcc_expression(newlbv, TREE_TYPE(oldiv), new_ivs, &stmts);
...
static bool not_interesting_stmt(gimple stmt) {
  ...
  if (gimple_code(stmt) == GIMPLE_LABEL || gimple_code(stmt) == GIMPLE_GOTO ||
...
static bool phi_loop_edge_uses_def(struct loop *loop, gimple phi, tree def) {
  unsigned i;
...
static bool stmt_uses_phi_result(gimple stmt, tree phi_result) {
...
static bool stmt_is_bumper_for_loop(struct loop *loop, gimple stmt) {
  gimple use;
  tree def;
...
bool perfect_nest_p(struct loop *loop) {
  basic_block *bbs;
  size_t i;
  gimple exit_cond;
  ...
  for (i = 0; i < loop->num_nodes; i++) {
    if (bbs[i]->loop_father == loop) {
      ...
      for (bsi = gsi_start_bb(bbs[i]); !gsi_end_p(bsi); gsi_next(&bsi)) {
        ...
        if (gimple_code(stmt) == GIMPLE_COND && exit_cond != stmt)
          goto non_perfectly_nested;

        if (stmt == exit_cond || not_interesting_stmt(stmt) ||
...
static void replace_uses_equiv_to_x_with_y(struct loop *loop, gimple stmt,
                                           tree x, int xstep, tree y,
                                           tree yinit, htab_t replacements,
  ...
  FOR_EACH_SSA_USE_OPERAND(use_p, stmt, iter, SSA_OP_USE) {
    ...
    scev = instantiate_parameters(loop, analyze_scalar_evolution(loop, use));

    if (scev == NULL || scev == chrec_dont_know)
      continue;

    step = evolution_part_in_loop_num(scev, loop->num);
    if (step == NULL || step == chrec_dont_know ||
        TREE_CODE(step) != INTEGER_CST || int_cst_value(step) != xstep)
      continue;
    ...
    val = force_gimple_operand_gsi(firstbsi, val, false, NULL, true,
                                   GSI_SAME_STMT);
...
static bool exit_phi_for_loop_p(struct loop *loop, gimple stmt) {
  if (gimple_code(stmt) != GIMPLE_PHI || gimple_phi_num_args(stmt) != 1 ||
...
static bool can_put_in_inner_loop(struct loop *inner, gimple stmt) {
  imm_use_iterator imm_iter;
  use_operand_p use_p;
...
static bool can_put_after_inner_loop(struct loop *loop, gimple stmt) {
  imm_use_iterator imm_iter;
  use_operand_p use_p;
  ...
  FOR_EACH_IMM_USE_FAST(use_p, imm_iter, gimple_assign_lhs(stmt)) {
    if (!exit_phi_for_loop_p(loop, USE_STMT(use_p))) {
      ...
      if (!dominated_by_p(CDI_DOMINATORS, immbb, loop->inner->header) &&
...
static bool can_duplicate_iv(tree iv, struct loop *loop) {
  tree scev = instantiate_parameters(loop, analyze_scalar_evolution(loop, iv));
...
static bool cannot_convert_modify_to_perfect_nest(gimple stmt,
                                                  struct loop *loop) {
  ...
  /* The statement should not define a variable used in the inner
     loop.  */
  if (TREE_CODE(op0) == SSA_NAME && !can_duplicate_iv(op0, loop))
...
static bool cannot_convert_bb_to_perfect_nest(basic_block bb,
                                              struct loop *loop) {
  gimple_stmt_iterator bsi;
  ...
  for (bsi = gsi_start_bb(bb); !gsi_end_p(bsi); gsi_next(&bsi)) {
    ...
    if (stmt == exit_condition || not_interesting_stmt(stmt) ||
        stmt_is_bumper_for_loop(loop, stmt))
      continue;
    ...
    if (!dominated_by_p(CDI_DOMINATORS, gimple_bb(stmt), loop->inner->header))
      return true;
  }
...
static bool can_convert_to_perfect_nest(struct loop *loop) {
  basic_block *bbs;
  size_t i;
  ...
  for (si = gsi_start_phis(single_exit(loop)->dest); !gsi_end_p(si);
       gsi_next(&si))
...
static bool perfect_nestify(struct loop *loop, VEC(tree, heap) * lbounds,
                            VEC(tree, heap) * ubounds, VEC(int, heap) * steps,
  ...
  set_immediate_dominator(CDI_DOMINATORS, preheaderbb, single_exit(loop)->src);
  ...
  create_iv(VEC_index(tree, lbounds, 0),
            build_int_cst(TREE_TYPE(oldivvar), VEC_index(int, steps, 0)), ivvar,
            newloop, &bsi, insert_after, &ivvar, &ivvarinced);
  ...
  uboundvar =
      create_tmp_var(TREE_TYPE(VEC_index(tree, ubounds, 0)), "uboundvar");
  ...
  replacements = htab_create_ggc(20, tree_map_hash, tree_map_eq, NULL);
  ...
  for (i = loop->num_nodes - 1; i >= 0; i--) {
    ...
    if (bbs[i]->loop_father == loop) {
      ...
      if (dominated_by_p(CDI_DOMINATORS, loop->inner->header, bbs[i])) {
        gimple_stmt_iterator header_bsi = gsi_after_labels(loop->inner->header);
...
bool lambda_transform_legal_p(lambda_trans_matrix trans, int nb_loops,
                              VEC(ddr_p, heap) * dependence_relations) {
  unsigned int i, j;
  lambda_vector distres;
  struct data_dependence_relation *ddr;

  gcc_assert(LTM_COLSIZE(trans) == nb_loops && LTM_ROWSIZE(trans) == nb_loops);
...
static void lambda_collect_parameters_from_af(tree access_function,
...
void lambda_collect_parameters(VEC(data_reference_p, heap) * datarefs,
                               VEC(tree, heap) * *parameters) {
  unsigned i, j;
...
static bool av_for_af_base(tree base_expr, lambda_vector cy,
                           struct access_matrix *am, int cst) {
  bool result = true;
  ...
  switch (TREE_CODE(base_expr)) {
    ...
    case SSA_NAME: {
      int param_index = access_matrix_get_index_for_parameter(base_expr, am);
    ...
    case MULT_EXPR:
      if (TREE_CODE(TREE_OPERAND(base_expr, 0)) == INTEGER_CST)
        result =
            av_for_af_base(TREE_OPERAND(base_expr, 1), cy, am,
                           cst * int_cst_value(TREE_OPERAND(base_expr, 0)));
      else if (TREE_CODE(TREE_OPERAND(base_expr, 1)) == INTEGER_CST)
        result =
            av_for_af_base(TREE_OPERAND(base_expr, 0), cy, am,
                           cst * int_cst_value(TREE_OPERAND(base_expr, 1)));
      else
        result = false;
...
static bool av_for_af(tree access_fun, lambda_vector cy,
                      struct access_matrix *am) {
...
static bool build_access_matrix(data_reference_p data_reference,
                                VEC(tree, heap) * parameters,
...
bool lambda_compute_access_matrices(VEC(data_reference_p, heap) * datarefs,
                                    VEC(tree, heap) * parameters,
                                    VEC(loop_p, heap) * nest) {
