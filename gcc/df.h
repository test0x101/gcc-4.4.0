...
#define DF_REF_LOC(REF)                    \
  ((DF_REF_CLASS(REF) == DF_REF_REGULAR || \
    DF_REF_CLASS(REF) == DF_REF_EXTRACT)   \
       ? (REF)->regular_ref.loc            \
       : NULL)
...
extern df_ref df_ref_create(rtx, rtx *, rtx, basic_block, enum df_ref_type,
...
extern bool df_compute_accessed_bytes(df_ref, enum df_mm, unsigned int *,
                                      unsigned int *);
...
static inline struct df_byte_lr_bb_info *df_byte_lr_get_bb_info(
    unsigned int index) {
...
extern void union_defs(df_ref, struct web_entry *, struct web_entry *,
