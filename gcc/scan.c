...
void make_sstring_space(sstring *str, int count) {
...
void sstring_append(sstring *dst, sstring *src) {
  char *d, *s;
...
int scan_ident(FILE *fp, sstring *s, int c) {
  s->ptr = s->base;
  if (ISIDST(c)) {
    for (;;) {
      SSTRING_PUT(s, c);
...
int scan_string(FILE *fp, sstring *s, int init) {
  int c;
...
int read_upto(FILE *fp, sstring *str, int delim) {
  int ch;
...
int get_token(FILE *fp, sstring *s) {
  int c;
...
unsigned int hashstr(const char *str, unsigned int len) {
  unsigned int n = len;
  unsigned int r = 0;
