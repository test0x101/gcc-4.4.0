...
bool flow_loop_nested_p(const struct loop *outer, const struct loop *loop) {
...
struct loop *superloop_at_depth(struct loop *loop, unsigned depth) {
...
static VEC(edge, heap) * get_loop_latch_edges(const struct loop *loop) {
  edge_iterator ei;
  edge e;
...
void flow_loop_dump(const struct loop *loop, FILE *file,
  ...
  fprintf(file, ";;  depth %d, outer %ld\n", loop_depth(loop),
          (long)(loop_outer(loop) ? loop_outer(loop)->num : -1));
...
void flow_loop_free(struct loop *loop) {
...
int flow_loop_nodes_find(basic_block header, struct loop *loop) {
...
static void establish_preds(struct loop *loop, struct loop *father) {
  loop_p ploop;
...
static void init_loops_structure(struct loops *loops, unsigned num_loops) {
  struct loop *root;
...
static edge find_subloop_latch_edge_by_profile(VEC(edge, heap) * latches) {
  unsigned i;
  edge e, me = NULL;
  ...
  if (dump_file)
    fprintf(dump_file, "Found latch edge %d -> %d using profile information.\n",
...
static edge find_subloop_latch_edge_by_ivs(struct loop *loop ATTRIBUTE_UNUSED,
                                           VEC(edge, heap) * latches) {
  ...
  for (psi = gsi_start_phis(loop->header); !gsi_end_p(psi); gsi_next(&psi)) {
    ...
    if (TREE_CODE(lop) != SSA_NAME || SSA_NAME_DEF_STMT(lop) == phi)
      continue;
    ...
    for (i = 0; VEC_iterate(edge, latches, i, e); i++)
      if (e != latch && PHI_ARG_DEF_FROM_EDGE(phi, e) == lop)
        return NULL;
  }

  if (dump_file)
    fprintf(dump_file, "Found latch edge %d -> %d using iv structure.\n",
...
static edge find_subloop_latch_edge(struct loop *loop) {
...
static void form_subloop(struct loop *loop, edge latch) {
  edge_iterator ei;
  edge e, new_entry;
  ...
  new_entry =
      make_forwarder_block(loop->header, mfb_redirect_edges_in_set, NULL);
...
static void merge_latch_edges(struct loop *loop) {
  VEC(edge, heap) *latches = get_loop_latch_edges(loop);
  edge latch, e;
  unsigned i;
  ...
  if (VEC_length(edge, latches) == 1)
    ...
    for (i = 0; VEC_iterate(edge, latches, i, e); i++)
      pointer_set_insert(mfb_reis_set, e);
    latch = make_forwarder_block(loop->header, mfb_redirect_edges_in_set, NULL);
    pointer_set_destroy(mfb_reis_set);
...
static void disambiguate_multiple_latches(struct loop *loop) {
  edge e;
...
bool flow_bb_inside_loop_p(const struct loop *loop, const_basic_block bb) {
...
unsigned get_loop_body_with_size(const struct loop *loop, basic_block *body,
                                 unsigned max_size) {
  return dfs_enumerate_from(loop->header, 1, glb_enum_p, body, max_size, loop);
}
...
basic_block *get_loop_body(const struct loop *loop) {
  basic_block *body, bb;
  unsigned tv = 0;
...
static void fill_sons_in_loop(const struct loop *loop, basic_block bb,
...
basic_block *get_loop_body_in_dom_order(const struct loop *loop) {
  basic_block *tovisit;
  int tv;
...
basic_block *get_loop_body_in_bfs_order(const struct loop *loop) {
  basic_block *blocks;
  basic_block bb;
  bitmap visited;
  unsigned int i = 0;
...
static hashval_t loop_exit_hash(const void *ex) {
...
static int loop_exit_eq(const void *ex, const void *e) {
  const struct loop_exit *const exit = (const struct loop_exit *)ex;
...
static void loop_exit_free(void *ex) {
...
void rescan_loop_exit(edge e, bool new_edge, bool removed) {
  ...
  slot = htab_find_slot_with_hash(current_loops->exits, e, htab_hash_pointer(e),
...
void record_loop_exits(void) {
  ...
  current_loops->exits =
      htab_create_alloc(2 * number_of_loops(), loop_exit_hash, loop_exit_eq,
                        loop_exit_free, ggc_calloc, ggc_free);
...
static int dump_recorded_exit(void **slot, void *file) {
...
VEC(edge, heap) * get_loop_exit_edges(const struct loop *loop) {
...
unsigned num_loop_branches(const struct loop *loop) {
  unsigned i, n;
  basic_block *body;
...
struct loop *find_common_loop(struct loop *loop_s, struct loop *loop_d) {
...
static void cancel_loop(struct loop *loop) {
  basic_block *bbs;
  unsigned i;
...
void cancel_loop_tree(struct loop *loop) {
  while (loop->inner)
...
void verify_loop_structure(void) {
  ...
  FOR_EACH_LOOP(li, loop, LI_INCLUDE_ROOT) {
    ...
    if (loop->num_nodes != sizes[i]) {
      error("size of loop %d should be %d, not %d", i, sizes[i],
            loop->num_nodes);
      err = 1;
    }
  }
  ...
  FOR_EACH_LOOP(li, loop, 0) {
    ...
    for (j = 0; j < loop->num_nodes; j++)
      if (!flow_bb_inside_loop_p(loop, bbs[j])) {
        error("bb %d do not belong to loop %d", bbs[j]->index, loop->num);
        err = 1;
      }
    free(bbs);
  }
  ...
  if (loops_state_satisfies_p(LOOPS_HAVE_MARKED_IRREDUCIBLE_REGIONS)) {
    ...
    FOR_EACH_BB(bb) {
      ...
      if ((bb->flags & BB_IRREDUCIBLE_LOOP) && !TEST_BIT(irreds, bb->index)) {
  ...
  FOR_EACH_LOOP(li, loop, 0) {
    if (!loop->exits || loop->exits->e != NULL) {
      error("corrupted head of the exits list of loop %d", loop->num);
      err = 1;
    } else {
      ...
      for (mexit = loop->exits, exit = mexit->next, i = 0;
           exit->e && exit != mexit; exit = exit->next) {
        if (i++ & 1)
          mexit = mexit->next;
      }
  ...
  if (loops_state_satisfies_p(LOOPS_HAVE_RECORDED_EXITS)) {
    ...
    FOR_EACH_BB(bb) {
      ...
      FOR_EACH_EDGE(e, ei, bb->succs) {
        ...
        if (!exit) {
          error("Exit %d->%d not recorded", e->src->index, e->dest->index);
          err = 1;
        }
        eloops = 0;
        ...
        for (loop = bb->loop_father; loop != e->dest->loop_father;
        ...
        if (eloops != 0) {
          error("Wrong list of exited loops for edge  %d->%d", e->src->index,
                e->dest->index);
          err = 1;
        }
      }
    ...
    FOR_EACH_LOOP(li, loop, 0) {
      ...
      if (eloops != sizes[loop->num]) {
        error("%d exits recorded for loop %d (having %d exits)", eloops,
              loop->num, sizes[loop->num]);
        err = 1;
      }
    }
  }
...
edge loop_latch_edge(const struct loop *loop) {
  return find_edge(loop->latch, loop->header);
}

/* Returns preheader edge of LOOP.  */
edge loop_preheader_edge(const struct loop *loop) {
  edge e;
  edge_iterator ei;
...
bool loop_exit_edge_p(const struct loop *loop, const_edge e) {
...
edge single_exit(const struct loop *loop) {
...
bool is_loop_exit(struct loop *loop, basic_block bb) {
  edge e;
  edge_iterator ei;
