...
static const cpp_token *get_a_token(cpp_reader *pfile) {
  for (;;) {
...
static void skip_to_closing_brace(cpp_reader *pfile) {
  int nesting = 1;
  for (;;) {
...
int scan_decls(cpp_reader *pfile, int argc ATTRIBUTE_UNUSED,
    ...
    switch (token->type) {
        ...
        if (prev_id.type != CPP_EOF) {
          ...
          for (;;) {
            ...
            if (token->type == CPP_OPEN_PAREN)
              ...
              break;
            else if (token->type == CPP_NAME || token->type == CPP_ELLIPSIS)
              have_arg_list = 1;
          }
          ...
          recognized_function(&prev_id, line,
                              (saw_inline                              ? 'I'
                               : in_extern_C_brace || current_extern_C ? 'F'
                                                                       : 'f'),
                              have_arg_list);
          token = get_a_token(pfile);
          if (token->type == CPP_OPEN_BRACE) {
      ...
      case CPP_NAME:
        ...
        if (cpp_ideq(token, "inline")) {
          ...
          saw_extern = 1;
          token = get_a_token(pfile);
          if (token->type == CPP_STRING && token->val.str.len == 1 &&
            ...
            if (token->type == CPP_OPEN_BRACE) {
              ...
              if (extern_C_braces_length >= MAX_EXTERN_C_BRACES) {
                fprintf(stderr, "Internal error: out-of-bounds index\n");
