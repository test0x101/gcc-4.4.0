...
/* The file implements the tail recursion elimination.  It is also used to
   ...
   3) return a + m * f(...), where a and m do not depend on call to f.
      ...
      a_acc + (a + m * f(...)) * m_acc = (a_acc + a * m_acc) + (m * m_acc) *
   f(...).
...
static bool suitable_for_tail_opt_p(void) {
  ...
  FOR_EACH_REFERENCED_VAR(var, rvi) {
    if (!is_global_var(var) && !MTAG_P(var) &&
...
static tree independent_of_stmt_p(tree expr, gimple at,
...
static bool process_assignment(gimple stmt, gimple_stmt_iterator call, tree *m,
                               tree *a, tree *ass_var) {
  ...
  if (op0 == *ass_var && (non_ass_var = independent_of_stmt_p(op1, stmt, call)))
    ;
  else if (op1 == *ass_var &&
...
static void find_tail_calls(basic_block bb, struct tailcall **ret) {
  ...
  for (gsi = gsi_last_bb(bb); !gsi_end_p(gsi); gsi_prev(&gsi)) {
    ...
    if (!ZERO_SSA_OPERANDS(stmt, (SSA_OP_VUSE | SSA_OP_VIRTUAL_DEFS)) ||
        ...
        (!gimple_aliases_computed_p(cfun) && gimple_references_memory_p(stmt)))
      return;
  }
    ...
    for (param = DECL_ARGUMENTS(func), idx = 0;
      ...
      if (param != arg) {
        ...
        if (!is_gimple_reg_type(TREE_TYPE(param)) ||
            !useless_type_conversion_p(TREE_TYPE(param), TREE_TYPE(arg)))
          break;
...
static tree update_accumulator_with_ops(enum tree_code code, tree acc, tree op1,
                                        gimple_stmt_iterator gsi) {
  gimple stmt = gimple_build_assign_with_ops(code, SSA_NAME_VAR(acc), acc, op1);
...
static void adjust_accumulator_values(gimple_stmt_iterator gsi, tree m, tree a,
                                      edge back) {
  ...
  if (a) {
    if (m_acc) {
      if (integer_onep(a))
        ...
        var = adjust_return_value_with_ops(MULT_EXPR, "acc_tmp", m_acc, a, gsi,
                                           GSI_NEW_STMT);
    } else
      var = a;
...
/* Subtract COUNT and FREQUENCY from the basic block and it's
   outgoing edge.  */
static void decrease_profile(basic_block bb, gcov_type count, int frequency) {
  edge e;
  bb->count -= count;
  if (bb->count < 0)
    bb->count = 0;
...
static bool arg_needs_copy_p(tree param) {
  tree def;
...
static void eliminate_tail_call(struct tailcall *t) {
  tree param, rslt;
  gimple stmt, call;
  tree arg;
  size_t idx;
  ...
  e = redirect_edge_and_branch(single_succ_edge(gsi_bb(t->call_gsi)), first);
  gcc_assert(e);
  PENDING_STMT(e) = NULL;
  ...
  for (param = DECL_ARGUMENTS(current_function_decl), idx = 0,
      gsi = gsi_start_phis(first);
       param; param = TREE_CHAIN(param), idx++) {
...
*/
static void add_virtual_phis(void) {
  referenced_var_iterator rvi;
  tree var;
...
static bool optimize_tail_call(struct tailcall *t, bool opt_tailcalls) {
  if (t->tail_recursion) {
...
static tree create_tailcall_accumulator(const char *label, basic_block bb,
                                        tree init) {
  tree ret_type = TREE_TYPE(DECL_RESULT(current_function_decl));
  tree tmp = create_tmp_var(ret_type, label);
  gimple phi;
...
static unsigned int tree_optimize_tail_calls_1(bool opt_tailcalls) {
  edge e;
  ...
  for (act = tailcalls; act; act = act->next) {
    ...
    if (act->add && !a_acc)
      a_acc = create_tailcall_accumulator("add_acc", first, integer_zero_node);

    if (act->mult && !m_acc)
      m_acc = create_tailcall_accumulator("mult_acc", first, integer_one_node);
  }
...
static unsigned int execute_tail_recursion(void) {
