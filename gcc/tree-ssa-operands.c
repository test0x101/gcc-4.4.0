...
static inline unsigned get_name_decl(const_tree t) {
...
int operand_build_cmp(const void *p, const void *q) {
...
static inline void operand_build_sort_virtual(VEC(tree, heap) * list) {
  ...
  qsort(VEC_address(tree, list), VEC_length(tree, list), sizeof(tree),
        operand_build_cmp);
}
...
static inline int vop_free_bucket_size(int bucket) {
#ifdef ENABLE_CHECKING
...
static inline int vop_free_bucket_index(int num) {
...
static inline void init_vop_buckets(void) {
  int x;
...
static inline void add_vop_to_freelist(voptype_p ptr) {
...
void fini_ssa_operands(void) {
  ...
  for (ix = 0; VEC_iterate(tree, gimple_ssa_operands(cfun)->mpt_table, ix, mpt);
       ix++) {
    if (mpt)
...
static inline void *ssa_operand_alloc(unsigned size) {
  char *ptr;
  ...
  if (gimple_ssa_operands(cfun)->operand_memory_index + size >=
    ...
    else if (gimple_ssa_operands(cfun)->ssa_operand_mem_size ==
             OP_SIZE_1 * sizeof(struct voptype_d))
      gimple_ssa_operands(cfun)->ssa_operand_mem_size =
    ...
    ptr = (struct ssa_operand_memory_d *)ggc_alloc(
        sizeof(struct ssa_operand_memory_d) +
        gimple_ssa_operands(cfun)->ssa_operand_mem_size - 1);
    ptr->next = gimple_ssa_operands(cfun)->operand_memory;
    gimple_ssa_operands(cfun)->operand_memory = ptr;
    gimple_ssa_operands(cfun)->operand_memory_index = 0;
  }
  ptr = &(gimple_ssa_operands(cfun)
              ->operand_memory
...
static inline struct def_optype_d *alloc_def(void) {
  ...
  if (gimple_ssa_operands(cfun)->free_defs) {
    ...
    ret = (struct def_optype_d *)ssa_operand_alloc(sizeof(struct def_optype_d));
  return ret;
}

/* Allocate a USE operand.  */

static inline struct use_optype_d *alloc_use(void) {
...
static inline struct voptype_d *alloc_vop(int num) {
...
static inline void set_virtual_use_link(use_operand_p ptr, gimple stmt) {
...
static inline def_optype_p add_def_op(tree *op, def_optype_p last) {
...
static inline use_optype_p add_use_op(gimple stmt, tree *op,
                                      use_optype_p last) {
...
static inline voptype_p add_vop(gimple stmt, tree op, int num, voptype_p prev) {
  voptype_p new_vop;
  int x;
  ...
  for (x = 0; x < num; x++) {
    ...
    link_imm_use_stmt(VUSE_OP_PTR(new_vop, x), new_vop->usev.uses[x].use_var,
                      stmt);
  }
...
static inline voptype_p add_vuse_op(gimple stmt, tree op, int num,
                                    voptype_p last) {
...
static inline voptype_p add_vdef_op(gimple stmt, tree op, int num,
                                    voptype_p last) {
  voptype_p new_vop = add_vop(stmt, op, num, last);
...
static inline void finalize_ssa_defs(gimple stmt) {
  unsigned new_i;
...
static inline void finalize_ssa_uses(gimple stmt) {
  unsigned new_i;
  ...
  for (new_i = 0; new_i < VEC_length(tree, build_uses); new_i++)
    last = add_use_op(stmt, (tree *)VEC_index(tree, build_uses, new_i), last);
...
static inline void finalize_ssa_vdefs(gimple stmt) {
  unsigned new_i;
...
static inline void finalize_ssa_vuse_ops(gimple stmt) {
  ...
  new_ops = NULL;
  new_i = old_i = 0;
  while (old_ops && old_i < VUSE_NUM(old_ops) &&
...
static void finalize_ssa_vuses(gimple stmt) {
...
static inline void cleanup_build_arrays(void) {
  unsigned i;
  tree t;
...
static inline void finalize_ssa_stmt_operands(gimple stmt) {
...
static inline void start_ssa_stmt_operands(void) {
...
static inline void append_def(tree *def_p) {
  VEC_safe_push(tree, heap, build_defs, (tree)def_p);
}

/* Add USE_P to the list of pointers to operands.  */

static inline void append_use(tree *use_p) {
...
static bool access_can_touch_variable(tree ref, tree alias,
  ...
  */
  if (ref && flag_strict_aliasing && TREE_CODE(ref) != INDIRECT_REF &&
      !MTAG_P(alias) && base &&
      (TREE_CODE(base) != INDIRECT_REF ||
       TREE_CODE(TREE_TYPE(base)) != UNION_TYPE) &&
      (TREE_CODE(base) != INDIRECT_REF || TREE_CODE(ref) != ARRAY_REF ||
       offset != 0 ||
       (DECL_SIZE(alias) && TREE_CODE(DECL_SIZE(alias)) == INTEGER_CST &&
        size != -1 &&
        (unsigned HOST_WIDE_INT)size != TREE_INT_CST_LOW(DECL_SIZE(alias)))) &&
  ...
  else if (ref && flag_strict_aliasing && TREE_CODE(ref) != INDIRECT_REF &&
           !MTAG_P(alias) && !var_ann(alias)->is_heapvar &&
           !POINTER_TYPE_P(TREE_TYPE(alias)) && offsetgtz && DECL_SIZE(alias) &&
...
static void add_virtual_operand(tree var, gimple stmt, int flags, tree full_ref,
  ...
  if (aliases == NULL) {
    ...
    EXECUTE_IF_SET_IN_BITMAP(aliases, 0, i, bi) {
      ...
      if (full_ref && !access_can_touch_variable(full_ref, al, offset, size))
        continue;
    ...
    if (flags & opf_def) {
      ...
      if (none_added || (TREE_CODE(var) == SYMBOL_MEMORY_TAG && is_call_site))
        append_vdef(var);
    } else {
...
static void add_stmt_operand(tree *var_p, gimple stmt, int flags) {
  tree var, sym;
  var_ann_t v_ann;
...
static void get_addr_dereference_operands(gimple stmt, tree *addr, int flags,
  ...
  if (SSA_VAR_P(ptr)) {
    ...
    if (TREE_CODE(ptr) == SSA_NAME && (pi = SSA_NAME_PTR_INFO(ptr)) != NULL &&
        pi->name_mem_tag) {
      /* PTR has its own memory tag.  Use it.  */
      add_virtual_operand(pi->name_mem_tag, stmt, flags, full_ref, offset, size,
                          false);
    } else {
      ...
      if (dump_file && TREE_CODE(ptr) == SSA_NAME &&
          (pi == NULL || (pi->name_mem_tag == NULL_TREE && !pi->pt_anything)) &&
          gimple_aliases_computed_p(cfun)) {
        fprintf(dump_file, "NOTE: no flow-sensitive alias info for ");
      ...
      if (v_ann->symbol_mem_tag) {
        add_virtual_operand(v_ann->symbol_mem_tag, stmt, flags, full_ref, 0, -1,
                            false);
      ...
      else if (!gimple_aliases_computed_p(cfun) && (flags & opf_def))
        gimple_set_has_volatile_ops(stmt, true);
    }
...
static void get_indirect_ref_operands(gimple stmt, tree expr, int flags,
                                      tree full_ref, HOST_WIDE_INT offset,
                                      HOST_WIDE_INT size,
                                      bool recurse_on_base) {
...
static void get_tmr_operands(gimple stmt, tree expr, int flags) {
  tree tag;
...
static void add_call_clobber_ops(gimple stmt, tree callee ATTRIBUTE_UNUSED) {
  unsigned u;
  bitmap_iterator bi;
  ...
  not_read_b =
      callee ? ipa_reference_get_not_read_global(cgraph_node(callee)) : NULL;
  not_written_b =
      callee ? ipa_reference_get_not_written_global(cgraph_node(callee)) : NULL;
  ...
  EXECUTE_IF_SET_IN_BITMAP(gimple_call_clobbered_vars(cfun), 0, u, bi) {
    ...
    not_read =
        not_read_b ? bitmap_bit_p(not_read_b, DECL_UID(real_var)) : false;

    not_written =
        not_written_b ? bitmap_bit_p(not_written_b, DECL_UID(real_var)) : false;
...
static void add_call_read_ops(gimple stmt, tree callee ATTRIBUTE_UNUSED) {
  unsigned u;
  bitmap_iterator bi;
  bitmap not_read_b;
...
static void maybe_add_call_clobbered_vops(gimple stmt) {
...
static void get_asm_expr_operands(gimple stmt) {
  size_t i, noutputs;
...
static void get_expr_operands(gimple stmt, tree *expr_p, int flags) {
  enum tree_code code;
  ...
  switch (code) {
    ...
    case IMAGPART_EXPR: {
      ...
      if (TREE_CODE(ref) == INDIRECT_REF) {
        get_indirect_ref_operands(stmt, ref, flags, expr, offset, maxsize,
                                  false);
        flags |= opf_no_vops;
      }
    ...
    case CONSTRUCTOR: {
      ...
      for (idx = 0;
           VEC_iterate(constructor_elt, CONSTRUCTOR_ELTS(expr), idx, ce); idx++)
...
static void parse_ssa_operands(gimple stmt) {
...
static void build_ssa_operands(gimple stmt) {
...
void free_stmt_operands(gimple stmt) {
...
void update_stmt_operands(gimple stmt) {
...
void copy_virtual_operands(gimple dest, gimple src) {
  unsigned int i, n;
...
void create_ssa_artificial_load_stmt(gimple new_stmt, gimple old_stmt,
...
void swap_tree_operands(gimple stmt, tree *exp0, tree *exp1) {
  tree op0, op1;
  op0 = *exp0;
  op1 = *exp1;
...
void add_to_addressable_set(tree ref, bitmap *set) {
  tree var;
...
void gimple_add_to_addresses_taken(gimple stmt, tree ref) {
...
void dump_immediate_uses_for(FILE *file, tree var) {
  ...
  FOR_EACH_IMM_USE_FAST(use_p, iter, var) {
    ...
    else if (!is_gimple_reg(USE_FROM_PTR(use_p)))
...
void push_stmt_changes(gimple *stmt_p) {
  gimple stmt;
  scb_t buf;
...
static void mark_difference_for_renaming(bitmap s1, bitmap s2) {
  if (s1 == NULL && s2 == NULL)
    return;
...
void pop_stmt_changes(gimple *stmt_p) {
  tree op;
  gimple stmt;
  ssa_op_iter iter;
  bitmap loads, stores;
...
void discard_stmt_changes(gimple *stmt_p) {
  scb_t buf;
  gimple stmt;
