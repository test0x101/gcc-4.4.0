...
static rtx find_use_as_address(rtx x, rtx reg, HOST_WIDE_INT plusconst) {
...
static int try_auto_increment(rtx insn, rtx inc_insn, rtx inc_insn_set, rtx reg,
  ...
  if (pset) {
    ...
    if (use != 0 && use != (rtx)(size_t)1) {
      ...
      if (0 ||
          (HAVE_POST_INCREMENT && pre == 0 &&
           (inc_code = POST_INC, increment == size)) ||
          (HAVE_PRE_INCREMENT && pre == 1 &&
           (inc_code = PRE_INC, increment == size)) ||
          (HAVE_POST_DECREMENT && pre == 0 &&
           (inc_code = POST_DEC, increment == -size)) ||
          (HAVE_PRE_DECREMENT && pre == 1 &&
           (inc_code = PRE_DEC, increment == -size))) {
        if (inc_insn_set)
          validate_change(inc_insn, &SET_SRC(inc_insn_set),
  ...
  for (p = NEXT_INSN(insn); p; p = NEXT_INSN(p)) {
    ...
    if (reg_set_p(src, p) ||
        ...
        /* If SRC is an asm-declared register, it must not be replaced
           ...
           mentioned in an asm.  */
        || (sregno < FIRST_PSEUDO_REGISTER && asm_noperands(PATTERN(p)) >= 0 &&
    ...
    if ((note = find_regno_note(p, REG_DEAD, sregno)) != 0 &&
      ...
      for (q = next_real_insn(insn); q != next_real_insn(p);
           q = next_real_insn(q)) {
...
static void optimize_reg_copy_3(rtx insn, rtx dest, rtx src) {
  ...
  if (src_no < FIRST_PSEUDO_REGISTER || dst_no < FIRST_PSEUDO_REGISTER ||
      !find_reg_note(insn, REG_DEAD, src_reg) || REG_N_DEATHS(src_no) != 1 ||
      REG_N_SETS(src_no) != 1)
    return;
  ...
  while (p = NEXT_INSN(p), p != insn) {
    ...
    validate_replace_rtx_group(src_reg, gen_lowpart_SUBREG(old_mode, src_reg),
                               p);
  }
...
static void copy_src_to_dest(rtx insn, rtx src, rtx dest) {
  rtx seq;
  rtx link;
  rtx next;
  rtx set;
  ...
  if (REG_P(src) && REG_LIVE_LENGTH(REGNO(src)) > 0 && REG_P(dest) &&
...
static bool reg_is_remote_constant_p(rtx reg, rtx insn) {
  basic_block bb;
  rtx p;
  int max;
  ...
  if (!reg_set_in_bb) {
    ...
    FOR_BB_INSNS(bb, p) {
      ...
      if (s != 0 && REG_P(SET_DEST(s)) && REG_N_SETS(REGNO(SET_DEST(s))) == 1 &&
...
static int fixup_match_2(rtx insn, rtx dst, rtx src, rtx offset) {
  rtx p, dst_death = 0;
  ...
  for (length = 0, p = PREV_INSN(insn); p; p = PREV_INSN(p)) {
    ...
    if (pset && SET_DEST(pset) == dst && GET_CODE(SET_SRC(pset)) == PLUS &&
      ...
      HOST_WIDE_INT newconst = INTVAL(offset) - INTVAL(XEXP(SET_SRC(pset), 1));
      ...
      if (add && validate_change(insn, &PATTERN(insn), add, 0)) {
        ...
        if (dump_file)
          fprintf(dump_file, "Fixed operand of insn %d.\n", INSN_UID(insn));
...
static unsigned int regmove_optimize(void) {
  rtx insn;
  struct match match;
  int i;
  ...
  if (flag_expensive_optimizations) {
    ...
    for (insn = get_insns(); insn; insn = NEXT_INSN(insn)) {
      ...
      if ((GET_CODE(SET_SRC(set)) == SIGN_EXTEND ||
          ...
          REG_P(XEXP(SET_SRC(set), 0)) && REG_P(SET_DEST(set)))
    ...
    if (INSN_P(insn)) {
      ...
      for (op_no = 0; op_no < recog_data.n_operands; op_no++) {
        ...
        if (!REG_P(dst) || REGNO(dst) < FIRST_PSEUDO_REGISTER ||
            REG_LIVE_LENGTH(REGNO(dst)) < 0 || GET_MODE(src) != GET_MODE(dst))
          continue;
        ...
        if (REGNO(src) < FIRST_PSEUDO_REGISTER) {
          if (GET_CODE(SET_SRC(set)) == PLUS &&
              ...
              fixup_match_2(insn, dst, src, XEXP(SET_SRC(set), 1)))
            break;
          continue;
        }
          ...
          if (pset && SET_DEST(pset) == src) {
            ...
            if (validate_replace_rtx(src, dst, insn)) {
              if (validate_change(p, &SET_DEST(pset), dst, 0))
                ...
                validate_change(insn, recog_data.operand_loc[match_no], dst, 0);
              }
            }
            break;
          }
