...
#define EDGE_ABNORMAL              \
  2 /* Strange flow, like computed \
       label, or eh */
#define EDGE_ABNORMAL_CALL                          \
  4                      /* Call with abnormal exit \
                            like an exception, or sibcall */
#define EDGE_EH       8  /* Exception throw */
#define EDGE_FAKE     16 /* Not a real edge (profile.c) */
#define EDGE_DFS_BACK 32 /* A backwards edge */
#define EDGE_CAN_FALLTHRU                                        \
  64                              /* Candidate for straight line \
                                     flow.  */
#define EDGE_IRREDUCIBLE_LOOP 128 /* Part of irreducible loop.  */
#define EDGE_SIBCALL          256 /* Edge from sibcall to exit.  */
#define EDGE_LOOP_EXIT        512 /* Exit of a loop.  */
#define EDGE_TRUE_VALUE               \
  1024 /* Edge taken when controlling \
          predicate is nonzero.  */
#define EDGE_FALSE_VALUE              \
  2048 /* Edge taken when controlling \
          predicate is zero.  */
#define EDGE_EXECUTABLE             \
  4096 /* Edge is executable.  Only \
          valid during SSA-CCP.  */
#define EDGE_CROSSING                \
  8192 /* Edge crosses between hot   \
...
struct basic_block_def GTY((chain_next("%h.next_bb"),
                            chain_prev("%h.prev_bb"))) {
...
enum bb_flags {
  ...
  /* Set if the label at the top of this block is the target of a non-local
   * goto.  */
...
#define SET_BASIC_BLOCK(N, BB) \
  (VEC_replace(basic_block, basic_block_info, (N), (BB)))
...
#define EDGE_FREQUENCY(e)                                            \
  (((e)->src->frequency * (e)->probability + REG_BR_PROB_BASE / 2) / \
   REG_BR_PROB_BASE)
...
static inline VEC(edge, gc) * ei_container(edge_iterator i) {
...
static inline edge_iterator ei_start_1(VEC(edge, gc) * *ev) {
  edge_iterator i;
...
/* Return an iterator pointing to the last element of an edge
   vector.  */
static inline edge_iterator ei_last_1(VEC(edge, gc) * *ev) {
  edge_iterator i;
...
extern bool dominated_by_p(enum cdi_direction, const_basic_block,
                           const_basic_block);
extern VEC(basic_block, heap) *
    get_dominated_by(enum cdi_direction, basic_block);
extern VEC(basic_block, heap) *
    get_dominated_by_region(enum cdi_direction, basic_block *, unsigned);
extern void add_to_dominance_info(enum cdi_direction, basic_block);
extern void delete_from_dominance_info(enum cdi_direction, basic_block);
basic_block recompute_dominator(enum cdi_direction, basic_block);
extern void redirect_immediate_dominators(enum cdi_direction, basic_block,
                                          basic_block);
extern void iterate_fix_dominators(enum cdi_direction, VEC(basic_block, heap) *,
                                   bool);
