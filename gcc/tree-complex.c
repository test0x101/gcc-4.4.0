...
static tree cvc_lookup(unsigned int uid) {
  ...
  in.uid = uid;
  h = (struct int_tree_map *)htab_find_with_hash(complex_variable_components,
                                                 &in, uid);
  return h ? h->to : NULL;
}
...
static void cvc_insert(unsigned int uid, tree to) {
  ...
  h->uid = uid;
  h->to = to;
  loc = htab_find_slot_with_hash(complex_variable_components, h, uid, INSERT);
...
static bool init_dont_simulate_again(void) {
  ...
  FOR_EACH_BB(bb) {
    for (gsi = gsi_start_phis(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      phi = gsi_stmt(gsi);
      prop_set_simulate_again(phi, is_complex_reg(gimple_phi_result(phi)));
    }
        ...
        switch (gimple_expr_code(stmt)) {
          ...
          case IMAGPART_EXPR:
            /* The total store transformation performed during
              ...
              to fix things up.  */
            if (TREE_CODE(op0) == SSA_NAME && ssa_undefined_value_p(op0))
              saw_a_complex_op = true;
            break;
...
static enum ssa_prop_result complex_visit_stmt(
    gimple stmt, edge *taken_edge_p ATTRIBUTE_UNUSED, tree *result_p) {
...
static enum ssa_prop_result complex_visit_phi(gimple phi) {
...
static tree create_one_component_var(tree type, tree orig, const char *prefix,
...
static tree get_component_var(tree var, bool imag_p) {
  ...
  if (ret == NULL) {
    ret = create_one_component_var(
        TREE_TYPE(TREE_TYPE(var)), var, imag_p ? "CI" : "CR",
        imag_p ? "$imag" : "$real", imag_p ? IMAGPART_EXPR : REALPART_EXPR);
...
static gimple_seq set_component_ssa_name(tree ssa_name, bool imag_p,
                                         tree value) {
  complex_lattice_t lattice = find_lattice_value(ssa_name);
  size_t ssa_name_index;
  tree comp;
...
static tree extract_component(gimple_stmt_iterator *gsi, tree t,
                              bool imagpart_p, bool gimple_p) {
  switch (TREE_CODE(t)) {
    case COMPLEX_CST:
    ...
    case ARRAY_REF: {
      ...
      t = build1((imagpart_p ? IMAGPART_EXPR : REALPART_EXPR), inner_type,
                 unshare_expr(t));
...
static void update_complex_components(gimple_stmt_iterator *gsi, gimple stmt,
                                      tree r, tree i) {
  tree lhs;
  gimple_seq list;
...
static void update_complex_components_on_edge(edge e, tree lhs, tree r,
                                              tree i) {
  gimple_seq list;
...
static void update_complex_assignment(gimple_stmt_iterator *gsi, tree r,
                                      tree i) {
...
static void update_all_vops(gimple stmt) {
  ssa_op_iter iter;
  tree sym;
...
static void expand_complex_move(gimple_stmt_iterator *gsi, tree type) {
  ...
  if (TREE_CODE(lhs) == SSA_NAME) {
    ...
    } else if (is_gimple_call(stmt) || gimple_has_side_effects(stmt) ||
...
static void expand_complex_addition(gimple_stmt_iterator *gsi, tree inner_type,
                                    tree ar, tree ai, tree br, tree bi,
                                    enum tree_code code, complex_lattice_t al,
...
static void expand_complex_libcall(gimple_stmt_iterator *gsi, tree ar, tree ai,
  ...
  if (gimple_in_ssa_p(cfun)) {
    type = TREE_TYPE(type);
    update_complex_components(gsi, stmt, build1(REALPART_EXPR, type, lhs),
...
static void expand_complex_multiplication(gimple_stmt_iterator *gsi,
                                          tree inner_type, tree ar, tree ai,
                                          tree br, tree bi,
...
static void expand_complex_div_wide(gimple_stmt_iterator *gsi, tree inner_type,
                                    tree ar, tree ai, tree br, tree bi,
                                    enum tree_code code) {
...
static void expand_complex_division(gimple_stmt_iterator *gsi, tree inner_type,
                                    tree ar, tree ai, tree br, tree bi,
                                    enum tree_code code, complex_lattice_t al,
                                    complex_lattice_t bl) {
  tree rr, ri;
...
static void expand_complex_negation(gimple_stmt_iterator *gsi, tree inner_type,
                                    tree ar, tree ai) {
  tree rr, ri;
...
static void expand_complex_conjugate(gimple_stmt_iterator *gsi, tree inner_type,
                                     tree ar, tree ai) {
  tree ri;
...
static void expand_complex_comparison(gimple_stmt_iterator *gsi, tree ar,
  ...
  cc = gimplify_build2(gsi, (code == EQ_EXPR ? TRUTH_AND_EXPR : TRUTH_OR_EXPR),
...
static unsigned int tree_lower_complex(void) {
  ...
  VEC_safe_grow_cleared(complex_lattice_t, heap, complex_lattice_values,
                        num_ssa_names);
...
struct gimple_opt_pass pass_lower_complex = {{
    ...
    0,                        /* todo_flags_start */
    TODO_dump_func | TODO_ggc_collect | TODO_update_ssa |
        TODO_verify_stmts /* todo_flags_finish */
}};

/* Entry point for complex operation lowering without optimization.  */

static unsigned int tree_lower_complex_O0(void) {
...
static bool gate_no_optimization(void) {
