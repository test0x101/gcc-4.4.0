...
int xcoff_assign_fundamental_type_number(tree decl) {
  const char *name;
  size_t i;
...
static void xcoffout_source_file(FILE *file, const char *filename,
                                 int inline_p) {
  if (filename &&
...
void xcoffout_source_line(unsigned int line, const char *filename) {
...
static void xcoffout_block(tree block, int depth, tree args) {
  while (block) {
...
void xcoffout_begin_block(unsigned int line, unsigned int n) {
...
void xcoffout_end_block(unsigned int line, unsigned int n) {
  if (n != 1)
...
void xcoffout_declare_function(FILE *file, tree decl, const char *name) {
  size_t len;
...
void xcoffout_end_epilogue(unsigned int line ATTRIBUTE_UNUSED,
                           const char *file ATTRIBUTE_UNUSED) {
