...
static void subreg_get_info(unsigned int, enum machine_mode, unsigned int,
                            enum machine_mode, struct subreg_info *);
...
static unsigned HOST_WIDE_INT nonzero_bits1(const_rtx, enum machine_mode,
                                            const_rtx, enum machine_mode,
                                            unsigned HOST_WIDE_INT);
static unsigned int cached_num_sign_bit_copies(const_rtx, enum machine_mode,
                                               const_rtx, enum machine_mode,
                                               unsigned int);
static unsigned int num_sign_bit_copies1(const_rtx, enum machine_mode,
                                         const_rtx, enum machine_mode,
                                         unsigned int);
...
static unsigned int num_sign_bit_copies_in_rep[MAX_MODE_INT + 1]
                                              [MAX_MODE_INT + 1];
...
int rtx_unstable_p(const_rtx x) {
...
static int rtx_addr_can_trap_p_1(const_rtx x, HOST_WIDE_INT offset,
                                 HOST_WIDE_INT size, enum machine_mode mode,
                                 bool unaligned_mems) {
  ...
  switch (code) {
    ...
    case CONST:
      return rtx_addr_can_trap_p_1(XEXP(x, 0), offset, size, mode,
                                   unaligned_mems);
    ...
    case PLUS:
      ...
      if (GET_CODE(XEXP(x, 1)) == CONST_INT &&
          !rtx_addr_can_trap_p_1(XEXP(x, 0), offset + INTVAL(XEXP(x, 1)), size,
                                 mode, unaligned_mems))
        return 0;
    ...
    case LO_SUM:
    case PRE_MODIFY:
      return rtx_addr_can_trap_p_1(XEXP(x, 1), offset, size, mode,
                                   unaligned_mems);
...
int rtx_addr_can_trap_p(const_rtx x) {
...
bool nonzero_address_p(const_rtx x) {
  ...
  switch (code) {
    ...
    case PLUS:
      ...
      else if (XEXP(x, 0) == pic_offset_table_rtx && CONSTANT_P(XEXP(x, 1)))
        return true;
      return false;
...
int reg_mentioned_p(const_rtx reg, const_rtx in) {
  const char *fmt;
  int i;
  enum rtx_code code;
...
int no_labels_between_p(const_rtx beg, const_rtx end) {
  rtx p;
  if (beg == end)
    return 0;
...
int reg_used_between_p(const_rtx reg, const_rtx from_insn, const_rtx to_insn) {
  rtx insn;
...
int reg_referenced_p(const_rtx x, const_rtx body) {
  int i;
...
int reg_set_between_p(const_rtx reg, const_rtx from_insn, const_rtx to_insn) {
  const_rtx insn;
...
int reg_set_p(const_rtx reg, const_rtx insn) {
  /* We can be passed an insn or part of one.  If we are passed an insn,
     check if a side-effect of the insn clobbers REG.  */
  if (INSN_P(insn) &&
      (FIND_REG_INC_NOTE(insn, reg) ||
       (CALL_P(insn) && ((REG_P(reg) && REGNO(reg) < FIRST_PSEUDO_REGISTER &&
                          overlaps_hard_reg_set_p(regs_invalidated_by_call,
                                                  GET_MODE(reg), REGNO(reg))) ||
                         MEM_P(reg) || find_reg_fusage(insn, CLOBBER, reg)))))
    return 1;
...
int modified_between_p(const_rtx x, const_rtx start, const_rtx end) {
  const enum rtx_code code = GET_CODE(x);
  const char *fmt;
  int i, j;
  rtx insn;
...
int modified_in_p(const_rtx x, const_rtx insn) {
  const enum rtx_code code = GET_CODE(x);
  const char *fmt;
  int i, j;
...
rtx single_set_2(const_rtx insn, const_rtx pat) {
  rtx set = NULL;
  int set_verified = 1;
  int i;
...
int multiple_sets(const_rtx insn) {
  int found;
  int i;
...
int set_noop_p(const_rtx set) {
  ...
  if (GET_CODE(dst) == ZERO_EXTRACT)
    return rtx_equal_p(XEXP(dst, 0), src) && !BYTES_BIG_ENDIAN &&
           XEXP(dst, 2) == const0_rtx && !side_effects_p(src);
...
int noop_move_p(const_rtx insn) {
...
rtx find_last_value(rtx x, rtx *pinsn, rtx valid_to, int allow_hwreg) {
  ...
  for (p = PREV_INSN(*pinsn); p && !LABEL_P(p); p = PREV_INSN(p))
    if (INSN_P(p)) {
...
int refers_to_regno_p(unsigned int regno, unsigned int endregno, const_rtx x,
                      rtx *loc) {
  int i;
...
int reg_overlap_mentioned_p(const_rtx x, const_rtx in) {
...
int dead_or_set_p(const_rtx insn, const_rtx x) {
...
static bool covers_regno_p(const_rtx dest, unsigned int test_regno) {
  if (GET_CODE(dest) == PARALLEL) {
    ...
    for (i = XVECLEN(dest, 0) - 1; i >= 0; i--) {
      ...
      if (inner != NULL_RTX && covers_regno_no_parallel_p(inner, test_regno))
        return true;
    }
...
int dead_or_set_regno_p(const_rtx insn, unsigned int test_regno) {
  const_rtx pattern;
  ...
  for (link = REG_NOTES(insn); link; link = XEXP(link, 1))
    if (REG_NOTE_KIND(link) == kind
        /* Verify that it is a register, so that scratch and MEM won't cause a
           problem here.  */
        && REG_P(XEXP(link, 0)) && REGNO(XEXP(link, 0)) <= regno &&
...
rtx find_constant_src(const_rtx insn) {
  rtx note, set, x;
...
int find_reg_fusage(const_rtx insn, enum rtx_code code, const_rtx datum) {
...
int find_regno_fusage(const_rtx insn, enum rtx_code code, unsigned int regno) {
  rtx link;

  /* CALL_INSN_FUNCTION_USAGE information cannot contain references
     to pseudo registers, so don't bother checking.  */

  if (regno >= FIRST_PSEUDO_REGISTER || !CALL_P(insn))
    return 0;

  for (link = CALL_INSN_FUNCTION_USAGE(insn); link; link = XEXP(link, 1)) {
    rtx op, reg;

    if (GET_CODE(op = XEXP(link, 0)) == code && REG_P(reg = XEXP(op, 0)) &&
        REGNO(reg) <= regno && END_HARD_REGNO(reg) > regno)
      return 1;
  }
...
void remove_note(rtx insn, const_rtx note) {
  rtx link;
...
void remove_reg_equal_equiv_notes(rtx insn) {
  rtx *loc;
...
int in_expr_list_p(const_rtx listp, const_rtx node) {
  const_rtx x;
...
void remove_node_from_expr_list(const_rtx node, rtx *listp) {
  rtx temp = *listp;
  rtx prev = NULL_RTX;
...
int volatile_insn_p(const_rtx x) {
  const RTX_CODE code = GET_CODE(x);
  switch (code) {
    case LABEL_REF:
    case SYMBOL_REF:
...
int volatile_refs_p(const_rtx x) {
  const RTX_CODE code = GET_CODE(x);
  switch (code) {
    case LABEL_REF:
    case SYMBOL_REF:
...
int side_effects_p(const_rtx x) {
  const RTX_CODE code = GET_CODE(x);
  switch (code) {
    case LABEL_REF:
    case SYMBOL_REF:
...
int may_trap_p_1(const_rtx x, unsigned flags) {
  int i;
  enum rtx_code code;
  const char *fmt;
  ...
  switch (code) {
    ...
    case MEM:
      if (/* MEM_NOTRAP_P only relates to the actual position of the memory
        ...
        return rtx_addr_can_trap_p_1(XEXP(x, 0), 0, size, GET_MODE(x),
                                     code_changed);
      }
    ...
    case COMPARE:
      ...
      /* But often the compare has some CC mode, so check operand
         modes as well.  */
      if (HONOR_NANS(GET_MODE(XEXP(x, 0))) || HONOR_NANS(GET_MODE(XEXP(x, 1))))
        return 1;
      break;
...
int may_trap_or_fault_p(const_rtx x) {
...
int inequality_comparisons_p(const_rtx x) {
  const char *fmt;
  int len, i;
  const enum rtx_code code = GET_CODE(x);
...
int replace_label(rtx *x, void *data) {
  ...
  if ((GET_CODE(l) == LABEL_REF || GET_CODE(l) == INSN_LIST) &&
      XEXP(l, 0) == old_label) {
    XEXP(l, 0) = new_label;
...
static int rtx_referenced_p_1(rtx *body, void *x) {
  rtx y = (rtx)x;
...
int rtx_referenced_p(rtx x, rtx body) {
...
bool tablejump_p(const_rtx insn, rtx *labelp, rtx *tablep) {
  rtx label, table;

  if (JUMP_P(insn) && (label = JUMP_LABEL(insn)) != NULL_RTX &&
      (table = next_active_insn(label)) != NULL_RTX && JUMP_P(table) &&
...
static int computed_jump_p_1(const_rtx x) {
  const enum rtx_code code = GET_CODE(x);
  int i, j;
  const char *fmt;
  ...
  switch (code) {
    ...
    case IF_THEN_ELSE:
      return (computed_jump_p_1(XEXP(x, 1)) || computed_jump_p_1(XEXP(x, 2)));
...
int computed_jump_p(const_rtx insn) {
  int i;
  if (JUMP_P(insn)) {
    rtx pat = PATTERN(insn);
...
int loc_mentioned_in_p(rtx *loc, const_rtx in) {
  enum rtx_code code;
  const char *fmt;
  int i, j;
...
unsigned int subreg_lsb_1(enum machine_mode outer_mode,
  ...
  if (WORDS_BIG_ENDIAN != BYTES_BIG_ENDIAN)
    ...
    gcc_assert(!((subreg_byte % UNITS_PER_WORD + GET_MODE_SIZE(outer_mode)) >
                     UNITS_PER_WORD &&
  ...
  if (WORDS_BIG_ENDIAN)
    word = (GET_MODE_SIZE(inner_mode) -
            (subreg_byte + GET_MODE_SIZE(outer_mode))) /
           UNITS_PER_WORD;
  else
...
unsigned int subreg_lsb(const_rtx x) {
...
static void subreg_get_info(unsigned int xregno, enum machine_mode xmode,
  ...
  if (HARD_REGNO_NREGS_HAS_PADDING(xregno, xmode)) {
    ...
    gcc_assert(hard_regno_nregs[xregno][xmode] ==
               (hard_regno_nregs[xregno][xmode_unit] * GET_MODE_NUNITS(xmode)));
    ...
    /* You can only ask for a SUBREG of a value with holes in the middle
       ...
       of each unit.  */
    if ((offset / GET_MODE_SIZE(xmode_unit) + 1 < GET_MODE_NUNITS(xmode)) &&
        (offset / GET_MODE_SIZE(xmode_unit) !=
         ((offset + GET_MODE_SIZE(ymode) - 1) / GET_MODE_SIZE(xmode_unit)))) {
  ...
  if (!rknown && offset == 0 && GET_MODE_SIZE(ymode) > GET_MODE_SIZE(xmode)) {
    info->representable_p = true;
    /* If this is a big endian paradoxical subreg, which uses more
       actual hard registers than the original register, we must
       return a negative offset so that we find the proper highpart
       of the register.  */
    if (GET_MODE_SIZE(ymode) > UNITS_PER_WORD ? WORDS_BIG_ENDIAN
                                              : BYTES_BIG_ENDIAN)
  ...
  if (!HARD_REGNO_NREGS_HAS_PADDING(xregno, xmode) &&
    ...
    if (!rknown && regsize_xmode > regsize_ymode && nregs_ymode > 1) {
      info->representable_p = false;
      info->nregs = (GET_MODE_SIZE(ymode) + regsize_xmode - 1) / regsize_xmode;
  ...
  offset_adj = offset;
  offset_adj -= subreg_lowpart_offset(
      ymode, mode_for_size(GET_MODE_BITSIZE(xmode) / nregs_xmode, MODE_INT, 0));
  ...
  if (!rknown) {
    info->representable_p =
        (!(y_offset_adj % (mode_multiple / nregs_multiple)));
    rknown = true;
  }
...
unsigned int subreg_regno_offset(unsigned int xregno, enum machine_mode xmode,
                                 unsigned int offset, enum machine_mode ymode) {
  struct subreg_info info;
...
bool subreg_offset_representable_p(unsigned int xregno, enum machine_mode xmode,
                                   unsigned int offset,
...
int simplify_subreg_regno(unsigned int xregno, enum machine_mode xmode,
                          unsigned int offset, enum machine_mode ymode) {
  struct subreg_info info;
  unsigned int yregno;
  ...
  if ((!reload_completed || frame_pointer_needed) &&
      (xregno == FRAME_POINTER_REGNUM || xregno == HARD_FRAME_POINTER_REGNUM))
    return -1;
  ...
  if (!HARD_REGNO_MODE_OK(yregno, ymode) && HARD_REGNO_MODE_OK(xregno, xmode))
    return -1;
...
unsigned int subreg_regno(const_rtx x) {
  ...
  ret = regno + subreg_regno_offset(regno, GET_MODE(subreg), SUBREG_BYTE(x),
                                    GET_MODE(x));
  return ret;
}
...
unsigned int subreg_nregs_with_regno(unsigned int regno, const_rtx x) {
  ...
  subreg_get_info(regno, GET_MODE(subreg), SUBREG_BYTE(x), GET_MODE(x), &info);
  return info.nregs;
}
...
static void parms_set(rtx x, const_rtx pat ATTRIBUTE_UNUSED, void *data) {
...
rtx find_first_parameter_load(rtx call_insn, rtx boundary) {
  ...
  for (p = CALL_INSN_FUNCTION_USAGE(call_insn); p; p = XEXP(p, 1))
    if (GET_CODE(XEXP(p, 0)) == USE && REG_P(XEXP(XEXP(p, 0), 0))) {
...
bool keep_with_call_p(const_rtx insn) {
  rtx set;

  if (INSN_P(insn) && (set = single_set(insn)) != NULL) {
    if (REG_P(SET_DEST(set)) && REGNO(SET_DEST(set)) < FIRST_PSEUDO_REGISTER &&
        fixed_regs[REGNO(SET_DEST(set))] &&
        general_operand(SET_SRC(set), VOIDmode))
      return true;
    if (REG_P(SET_SRC(set)) && FUNCTION_VALUE_REGNO_P(REGNO(SET_SRC(set))) &&
...
bool label_is_jump_target_p(const_rtx label, const_rtx jump_insn) {
  ...
  if (tablejump_p(jump_insn, NULL, &tmp)) {
    rtvec vec = XVEC(PATTERN(tmp), GET_CODE(PATTERN(tmp)) == ADDR_DIFF_VEC);
...
int rtx_cost(rtx x, enum rtx_code outer_code ATTRIBUTE_UNUSED, bool speed) {
  int i, j;
  enum rtx_code code;
  const char *fmt;
  int total;
...
int address_cost(rtx x, enum machine_mode mode, bool speed) {
...
int default_address_cost(rtx x, bool speed) {
  return rtx_cost(x, MEM, speed);
}

unsigned HOST_WIDE_INT nonzero_bits(const_rtx x, enum machine_mode mode) {
  return cached_nonzero_bits(x, mode, NULL_RTX, VOIDmode, 0);
}

unsigned int num_sign_bit_copies(const_rtx x, enum machine_mode mode) {
...
static unsigned HOST_WIDE_INT cached_nonzero_bits(
    const_rtx x, enum machine_mode mode, const_rtx known_x,
...
static unsigned HOST_WIDE_INT nonzero_bits1(const_rtx x, enum machine_mode mode,
                                            const_rtx known_x,
  ...
  if (GET_MODE(x) != VOIDmode && GET_MODE(x) != mode &&
    ...
    nonzero &=
        cached_nonzero_bits(x, GET_MODE(x), known_x, known_mode, known_ret);
  ...
  switch (code) {
    case REG:
      ...
      if (POINTERS_EXTEND_UNSIGNED && GET_MODE(x) == Pmode && REG_POINTER(x))
      ...
      if ((x == stack_pointer_rtx || x == frame_pointer_rtx ||
        ...
        if (x == stack_pointer_rtx && PUSH_ARGS)
          alignment = MIN((unsigned HOST_WIDE_INT)PUSH_ROUNDING(1), alignment);
#endif
...

      {
        ...
        rtx new_rtx = rtl_hooks.reg_nonzero_bits(x, mode, known_x, known_mode,
                                                 known_ret, &nonzero_for_hook);
    ...
    case TRUNCATE:
      nonzero &= (cached_nonzero_bits(XEXP(x, 0), mode, known_x, known_mode,
                                      known_ret) &
                  GET_MODE_MASK(mode));
      break;
    ...
    case ZERO_EXTEND:
      nonzero &=
          cached_nonzero_bits(XEXP(x, 0), mode, known_x, known_mode, known_ret);
    ...
    case SIGN_EXTEND:
      ...
      if (GET_MODE(XEXP(x, 0)) != VOIDmode) {
        ...
        if (inner_nz & (((HOST_WIDE_INT)1
                         << (GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) - 1))))
          inner_nz |=
              (GET_MODE_MASK(mode) & ~GET_MODE_MASK(GET_MODE(XEXP(x, 0))));
      }
    ...
    case AND:
      nonzero &=
          cached_nonzero_bits(XEXP(x, 0), mode, known_x, known_mode,
                              known_ret) &
          cached_nonzero_bits(XEXP(x, 1), mode, known_x, known_mode, known_ret);
      break;
    ...
    case SMAX: {
      ...
      if ((nonzero & nonzero0) != nonzero)
        nonzero &= nonzero0 | cached_nonzero_bits(XEXP(x, 1), mode, known_x,
                                                  known_mode, known_ret);
    } break;
    ...
    case UMOD:
      ...
      {
        unsigned HOST_WIDE_INT nz0 = cached_nonzero_bits(
            XEXP(x, 0), mode, known_x, known_mode, known_ret);
        unsigned HOST_WIDE_INT nz1 = cached_nonzero_bits(
            XEXP(x, 1), mode, known_x, known_mode, known_ret);
        ...
        if (POINTERS_EXTEND_UNSIGNED > 0 && GET_MODE(x) == Pmode &&
            (code == PLUS || code == MINUS) && REG_P(XEXP(x, 0)) &&
    ...
    case SUBREG:
      ...
      if (SUBREG_PROMOTED_VAR_P(x) && SUBREG_PROMOTED_UNSIGNED_P(x) > 0)
        nonzero = GET_MODE_MASK(GET_MODE(x)) &
                  cached_nonzero_bits(SUBREG_REG(x), GET_MODE(x), known_x,
                                      known_mode, known_ret);
      ...
      if (GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(x))) <= BITS_PER_WORD &&
        ...
        nonzero &= cached_nonzero_bits(SUBREG_REG(x), mode, known_x, known_mode,
                                       known_ret);
    ...
    case ROTATE:
      ...
      if (GET_CODE(XEXP(x, 1)) == CONST_INT && INTVAL(XEXP(x, 1)) >= 0 &&
        ...
        unsigned HOST_WIDE_INT op_nonzero = cached_nonzero_bits(
            XEXP(x, 0), mode, known_x, known_mode, known_ret);
    ...
    case IF_THEN_ELSE: {
      ...
      if ((nonzero & nonzero_true) != nonzero)
        nonzero &= nonzero_true | cached_nonzero_bits(XEXP(x, 2), mode, known_x,
                                                      known_mode, known_ret);
    } break;
...
static unsigned int num_sign_bit_copies1(const_rtx x, enum machine_mode mode,
                                         const_rtx known_x,
                                         enum machine_mode known_mode,
                                         unsigned int known_ret) {
  enum rtx_code code = GET_CODE(x);
  ...
  if (bitwidth < GET_MODE_BITSIZE(GET_MODE(x))) {
    num0 = cached_num_sign_bit_copies(x, GET_MODE(x), known_x, known_mode,
                                      known_ret);
    return MAX(1, num0 - (int)(GET_MODE_BITSIZE(GET_MODE(x)) - bitwidth));
  }
  ...
  switch (code) {
    case REG:
...

      {
        ...
        rtx new_rtx = rtl_hooks.reg_num_sign_bit_copies(
            x, mode, known_x, known_mode, known_ret, &copies_for_hook);
    ...
    case MEM:
      ...
      if (LOAD_EXTEND_OP(GET_MODE(x)) == SIGN_EXTEND)
        return MAX(1, ((int)bitwidth - (int)GET_MODE_BITSIZE(GET_MODE(x)) + 1));
#endif
      break;
    ...
    case SUBREG:
      ...
      if (SUBREG_PROMOTED_VAR_P(x) && !SUBREG_PROMOTED_UNSIGNED_P(x)) {
        num0 = cached_num_sign_bit_copies(SUBREG_REG(x), mode, known_x,
                                          known_mode, known_ret);
        return MAX((int)bitwidth - (int)GET_MODE_BITSIZE(GET_MODE(x)) + 1,
                   num0);
      }

      /* For a smaller object, just ignore the high bits.  */
      if (bitwidth <= GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(x)))) {
        num0 = cached_num_sign_bit_copies(SUBREG_REG(x), VOIDmode, known_x,
                                          known_mode, known_ret);
        return MAX(1, (num0 - (int)(GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(x))) -
                                    bitwidth)));
      }
      ...
      if ((GET_MODE_SIZE(GET_MODE(x)) >
          ...
          MEM_P(SUBREG_REG(x)))
        return cached_num_sign_bit_copies(SUBREG_REG(x), mode, known_x,
                                          known_mode, known_ret);
#endif
#endif
      break;
    ...
    case SIGN_EXTEND:
      return (bitwidth - GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) +
              cached_num_sign_bit_copies(XEXP(x, 0), VOIDmode, known_x,
                                         known_mode, known_ret));

    case TRUNCATE:
      /* For a smaller object, just ignore the high bits.  */
      num0 = cached_num_sign_bit_copies(XEXP(x, 0), VOIDmode, known_x,
                                        known_mode, known_ret);
      return MAX(
          1, (num0 - (int)(GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) - bitwidth)));

    case NOT:
      return cached_num_sign_bit_copies(XEXP(x, 0), mode, known_x, known_mode,
                                        known_ret);
    ...
    case ROTATERT:
      ...
      if (GET_CODE(XEXP(x, 1)) == CONST_INT && INTVAL(XEXP(x, 1)) >= 0 &&
        ...
        num0 = cached_num_sign_bit_copies(XEXP(x, 0), mode, known_x, known_mode,
                                          known_ret);
        return MAX(
            1, num0 - (code == ROTATE ? INTVAL(XEXP(x, 1))
    ...
    case UMAX:
      ...
      num0 = cached_num_sign_bit_copies(XEXP(x, 0), mode, known_x, known_mode,
                                        known_ret);
      num1 = cached_num_sign_bit_copies(XEXP(x, 1), mode, known_x, known_mode,
                                        known_ret);
      ...
      if (code == AND && num1 > 1 && bitwidth <= HOST_BITS_PER_WIDE_INT &&
    ...
    case MINUS:
      ...
      if (!POINTERS_EXTEND_UNSIGNED && GET_MODE(x) == Pmode &&
          ...
          REG_POINTER(XEXP(x, 0)))
        result =
            MAX((int)(GET_MODE_BITSIZE(Pmode) - GET_MODE_BITSIZE(ptr_mode) + 1),
                result);
#endif
      return result;
    ...
    case UMOD:
      ...
      return cached_num_sign_bit_copies(XEXP(x, 1), mode, known_x, known_mode,
                                        known_ret);
    ...
    case DIV:
      /* Similar to unsigned division, except that we have to worry about
         ...
         to add 1.  */
      result = cached_num_sign_bit_copies(XEXP(x, 0), mode, known_x, known_mode,
                                          known_ret);
    ...
    case MOD:
      result = cached_num_sign_bit_copies(XEXP(x, 1), mode, known_x, known_mode,
                                          known_ret);
      if (result > 1 && (bitwidth > HOST_BITS_PER_WIDE_INT ||
                         (nonzero_bits(XEXP(x, 1), mode) &
                          ((HOST_WIDE_INT)1 << (bitwidth - 1))) != 0))
    ...
    case IF_THEN_ELSE:
      num0 = cached_num_sign_bit_copies(XEXP(x, 1), mode, known_x, known_mode,
                                        known_ret);
      num1 = cached_num_sign_bit_copies(XEXP(x, 2), mode, known_x, known_mode,
                                        known_ret);
      return MIN(num0, num1);
...
int insn_rtx_cost(rtx pat, bool speed) {
  int i, cost;
  rtx set;
...
rtx canonicalize_condition(rtx insn, rtx cond, int reverse, rtx *earliest,
  ...
  while ((GET_RTX_CLASS(code) == RTX_COMPARE ||
    ...
    /* If comparison with cc0, import actual comparison from compare
       insn.  */
    if (op0 == cc0_rtx) {
      if ((prev = prev_nonnote_insn(prev)) == 0 || !NONJUMP_INSN_P(prev) ||
      ...
      if ((GET_CODE(SET_SRC(set)) == COMPARE ||
           (((code == NE ||
              (code == LT && GET_MODE_CLASS(inner_mode) == MODE_INT &&
               ...
               (STORE_FLAG_VALUE &
                ((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(inner_mode) - 1))))
#ifdef FLOAT_STORE_FLAG_VALUE
              || (code == LT && SCALAR_FLOAT_MODE_P(inner_mode) &&
        ...
        x = SET_SRC(set);
                 ...
                 (code == GE &&
                  ...
                  (STORE_FLAG_VALUE &
                   ((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(inner_mode) - 1))))
#ifdef FLOAT_STORE_FLAG_VALUE
                 || (code == GE && SCALAR_FLOAT_MODE_P(inner_mode) &&
                     (fsfv = FLOAT_STORE_FLAG_VALUE(inner_mode),
                      REAL_VALUE_NEGATIVE(fsfv)))
#endif
                     )) &&
  ...
  if (GET_MODE_CLASS(GET_MODE(op0)) != MODE_CC && GET_CODE(op1) == CONST_INT &&
...
rtx get_condition(rtx jump, rtx *earliest, int allow_cc_mode,
                  int valid_at_insn_p) {
  rtx cond;
  int reverse;
  rtx set;
...
bool truncated_to_mode(enum machine_mode mode, const_rtx x) {
