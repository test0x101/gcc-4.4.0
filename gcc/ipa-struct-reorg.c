...
static inline tree strip_type(tree type) {
...
static inline tree get_type_of_var(tree var) {
  if (!var)
    return NULL;
...
static inline void finalize_stmt(gimple stmt) {
  update_stmt(stmt);
...
static inline void finalize_stmt_and_append(gimple_seq *stmts, gimple stmt) {
...
static tree find_field_in_struct_1(tree str_type, tree field) {
  tree str_field;
...
static struct field_entry *find_field_in_struct(d_str str, tree field_decl) {
  int i;
...
static bool decompose_indirect_ref_acc(tree str_decl,
  ...
  if (!(acc->ref_def_stmt) || (gimple_code(acc->ref_def_stmt) != GIMPLE_ASSIGN))
    return false;
  ...
  if (!is_array_access_through_pointer_and_index(rhs_code, op0, op1, &acc->base,
                                                 &acc->offset, &acc->cast_stmt))
    return false;
...
static bool decompose_access(tree str_decl, struct field_access_site *acc) {
  gcc_assert(acc->ref);
...
static inline struct field_access_site *make_field_acc_node(void) {
...
static struct field_access_site *is_in_field_accs(gimple stmt, htab_t f_accs) {
  return (struct field_access_site *)htab_find_with_hash(
      f_accs, stmt, htab_hash_pointer(stmt));
}

/* This function adds an access ACC to the hashtable
   F_ACCS of field accesses.  */

static void add_field_acc_to_acc_sites(struct field_access_site *acc,
                                       htab_t f_accs) {
  void **slot;

  gcc_assert(!is_in_field_accs(acc->stmt, f_accs));
  slot = htab_find_slot_with_hash(f_accs, acc->stmt,
                                  htab_hash_pointer(acc->stmt), INSERT);
  *slot = acc;
}
...
static void add_access_to_acc_sites(gimple stmt, tree var, htab_t accs) {
  struct access_site *acc;

  acc = (struct access_site *)htab_find_with_hash(accs, stmt,
  ...
  if (!acc) {
    ...
    slot =
        htab_find_slot_with_hash(accs, stmt, htab_hash_pointer(stmt), INSERT);
    *slot = acc;
  }
...
static void finalize_var_creation(tree new_decl) {
...
static void finalize_global_creation(tree var) {
...
static inline void insert_global_to_varpool(tree new_decl) {
...
static int finalize_new_vars_creation(void **slot,
...
static tree find_var_in_new_vars_vec(new_var var, tree new_type) {
  tree n_var;
  unsigned i;
...
static new_var is_in_new_vars_htab(tree decl, htab_t new_vars_htab) {
...
static tree find_new_var_of_type(tree orig_var, tree new_type) {
  new_var var;
...
static gimple gen_size(tree num, tree type, tree *res) {
...
static gimple gen_cast_stmt(tree before_cast, tree new_type,
                            gimple orig_cast_stmt, tree *res_p) {
  tree lhs, new_lhs;
  gimple new_stmt;
...
static edge make_edge_and_fix_phis_of_dest(basic_block bb, edge e) {
  edge new_e;
  tree arg;
...
static void insert_before_stmt(gimple stmt, gimple new_stmt) {
...
static void insert_seq_after_stmt(gimple stmt, gimple_seq new_stmts) {
  gimple_stmt_iterator bsi;
...
static void insert_after_stmt(gimple stmt, gimple new_stmt) {
  gimple_stmt_iterator bsi;
...
static fallocs_t get_fallocs(tree fn_decl) {
...
static gimple get_final_alloc_stmt(gimple alloc_stmt) {
  gimple final_stmt;
  use_operand_p use_p;
  tree alloc_res;
...
static bool is_part_of_malloc(gimple stmt, tree fn_decl) {
  ...
  if (fallocs) {
    ...
    for (i = 0; VEC_iterate(alloc_site_t, fallocs->allocs, i, call); i++)
      if (call->stmt == stmt || get_final_alloc_stmt(call->stmt) == stmt)
        return true;
  }
  return false;
}
...
static bool is_part_of_field_access(gimple stmt, d_str str) {
  int i;
...
static inline tree build_comp_ref(tree base, tree field_id, tree type) {
  tree field;
  bool found = false;
...
static tree find_pos_in_stmt_1(tree *tp, int *walk_subtrees, void *data) {
...
static tree *find_pos_in_stmt(gimple stmt, tree ref) {
...
static void replace_field_acc(struct field_access_site *acc, tree new_type) {
  ...
  while (VEC_length(type_wrapper_t, wrapper) != 0) {
    ...
    if (wr_p->wrap) /* Array.  */
      new_ref =
          build4(ARRAY_REF, type, new_ref, wr_p->domain, NULL_TREE, NULL_TREE);
    else /* Pointer.  */
...
static void replace_field_access_stmt(struct field_access_site *acc,
                                      tree new_type) {
  if (TREE_CODE(acc->ref) == INDIRECT_REF || TREE_CODE(acc->ref) == ARRAY_REF ||
...
static unsigned find_structure(tree type) {
  d_str str;
  unsigned i;
...
static gimple create_base_plus_offset(gimple orig_stmt, tree new_type,
                                      tree offset) {
  tree lhs;
  tree new_lhs;
  gimple new_stmt;
  ...
  gcc_assert(TREE_CODE(lhs) == VAR_DECL || TREE_CODE(lhs) == SSA_NAME);
...
static void create_new_field_access(struct field_access_site *f_acc,
  ...
  if (f_acc->cast_stmt) {
    cast_stmt = gen_cast_stmt(size_res, new_type, f_acc->cast_stmt, &cast_res);
  ...
  if (f_acc->ref_def_stmt) {
    ...
    new_stmt = create_base_plus_offset(f_acc->ref_def_stmt, new_type, offset);
...
static void create_new_stmts_for_cond_expr_1(tree new_var, gimple cond_stmt,
                                             unsigned pos) {
  gimple new_stmt;
  ...
  extract_true_false_edges_from_block(gimple_bb(cond_stmt), &true_e, &false_e);
...
static void create_new_stmts_for_cond_expr(gimple stmt) {
  tree arg0, arg1, arg;
  unsigned str0, str1;
  bool s0, s1;
  d_str str;
...
static gimple create_general_new_stmt(struct access_site *acc, tree new_type) {
  ...
  for (i = 0; VEC_iterate(tree, acc->vars, i, var); i++) {
    ...
    if (is_gimple_assign(new_stmt)) {
      ...
      if (gimple_assign_rhs_code(new_stmt) == CONSTRUCTOR) {
        ...
        gcc_assert(VEC_empty(constructor_elt, CONSTRUCTOR_ELTS(rhs)));
...
static void create_new_stmts_for_general_acc(struct access_site *acc,
                                             d_str str) {
  tree type;
...
static void create_new_general_access(struct access_site *acc, d_str str) {
  gimple stmt = acc->stmt;
...
static int create_new_acc(void **slot, void *data) {
...
static int create_new_field_acc(void **slot, void *data) {
  struct field_access_site *f_acc = *(struct field_access_site **)slot;
  basic_block bb = ((struct create_acc_data *)data)->bb;
  d_str str = ((struct create_acc_data *)data)->str;
...
static void create_new_accs_for_struct(d_str str, basic_block bb) {
  int i;
  ...
  for (i = 0; i < str->num_fields; i++) {
    ...
    if (str->fields[i].acc_sites)
      htab_traverse(str->fields[i].acc_sites, create_new_field_acc, &dt);
  }
  if (str->accs)
...
static int update_varpool_with_new_var(void **slot,
                                       void *data ATTRIBUTE_UNUSED) {
  new_var n_var = *(new_var *)slot;
  tree var;
  unsigned i;
...
static int dump_field_acc(void **slot, void *data ATTRIBUTE_UNUSED) {
  struct field_access_site *f_acc = *(struct field_access_site **)slot;
...
static void dump_field_acc_sites(htab_t f_accs) {
  if (!dump_file)
    return;
...
static hashval_t malloc_hash(const void *x) {
...
static int malloc_eq(const void *x, const void *y) {
...
static int free_accs(void **slot, void *data ATTRIBUTE_UNUSED) {
  struct access_site *acc = *(struct access_site **)slot;
...
static int free_field_accs(void **slot, void *data ATTRIBUTE_UNUSED) {
  struct field_access_site *f_acc = *(struct field_access_site **)slot;
...
static void add_unsuitable_type(VEC(tree, heap) * *unsuitable_types,
                                tree type) {
  unsigned i;
  tree t;
...
static bool is_equal_types(tree type1, tree type2) {
  ...
  switch (TREE_CODE(type1)) {
    ...
    case ENUMERAL_TYPE: {
      ...
      for (field1 = TYPE_FIELDS(type1); field1; field1 = TREE_CHAIN(field1)) {
    ...
    case ARRAY_TYPE: {
      ...
      if (max1 && max2 && min1 && min2 && TREE_CODE(max1) == TREE_CODE(max2) &&
          TREE_CODE(max1) == INTEGER_CST &&
          TREE_CODE(min1) == TREE_CODE(min2) &&
          TREE_CODE(min1) == INTEGER_CST && tree_int_cst_equal(max1, max2) &&
...
static void free_accesses(htab_t accs) {
  if (accs)
...
static void free_field_accesses(htab_t f_accs) {
  if (f_accs)
...
static void update_cgraph_with_malloc_call(gimple malloc_stmt, tree context) {
  ...
  cgraph_create_edge(src, dest, malloc_stmt, 0, 0,
...
static gimple create_new_malloc(gimple malloc_stmt, tree new_type,
                                gimple_seq *new_stmts, tree num) {
...
static tree gen_num_of_structs_in_malloc(gimple stmt, tree str_decl,
  ...
  if (TREE_CODE(arg) != SSA_NAME && !TREE_CONSTANT(arg))
    return NULL_TREE;
  ...
  if (TREE_CODE(arg) == SSA_NAME) {
    ...
    if (exact_log2(struct_size_int) == -1)
      div_stmt =
          gimple_build_assign_with_ops(TRUNC_DIV_EXPR, num, arg, struct_size);
    else {
      tree C = build_int_cst(integer_type_node, exact_log2(struct_size_int));
...
static int dump_new_var(void **slot, void *data ATTRIBUTE_UNUSED) {
  new_var n_var = *(new_var *)slot;
  tree var_type;
  tree var;
  unsigned i;
...
static inline void copy_decl_attributes(tree new_decl, tree orig_decl) {
...
static inline tree gen_struct_type(tree decl, tree new_str_type) {
  ...
  while (POINTER_TYPE_P(type_orig) || TREE_CODE(type_orig) == ARRAY_TYPE) {
...
static tree gen_var_name(tree orig_decl, unsigned HOST_WIDE_INT i) {
  const char *old_name;
  char *prefix;
  char *new_name;

  if (!DECL_NAME(orig_decl) || !IDENTIFIER_POINTER(DECL_NAME(orig_decl)))
    return NULL;
...
static void add_to_new_vars_htab(new_var new_node, htab_t new_vars_htab) {
  void **slot;

  slot =
      htab_find_slot_with_hash(new_vars_htab, new_node->orig_var,
                               htab_hash_pointer(new_node->orig_var), INSERT);
  *slot = new_node;
}
...
static new_var create_new_var_node(tree var, d_str str) {
  new_var node;
...
static bool is_candidate(tree var, tree *type_p,
...
static hashval_t field_acc_hash(const void *x) {
...
static void free_struct_cluster(struct field_cluster *cluster) {
  if (cluster) {
...
static void free_data_struct(d_str d_node) {
  int i;
...
static void create_new_accesses_in_bb(basic_block bb) {
  d_str str;
  unsigned i;
...
static void create_new_alloc_sites(fallocs_t m_data, tree context) {
  alloc_site_t *call;
  unsigned j;
...
static void dump_new_vars(htab_t new_vars_htab) {
  if (!dump_file)
    return;
...
static void create_new_var_1(tree orig_decl, d_str str, new_var node) {
  unsigned i;
  tree type;
...
static void create_new_var(tree var_decl, htab_t new_vars_htab) {
  new_var node;
  d_str str;
  tree type;
  unsigned i;
...
static void check_type_escape(d_str str, VEC(tree, heap) * *unsuitable_types) {
  tree type = str->decl;
...
static struct field_entry *get_fields(tree struct_decl, int num_fields) {
  struct field_entry *list;
  tree t = TYPE_FIELDS(struct_decl);
  int idx = 0;

  list = (struct field_entry *)xmalloc(num_fields * sizeof(struct field_entry));

  for (idx = 0; t; t = TREE_CHAIN(t), idx++)
    if (TREE_CODE(t) == FIELD_DECL) {
      list[idx].index = idx;
      list[idx].decl = t;
      list[idx].acc_sites = htab_create(32, field_acc_hash, field_acc_eq, NULL);
      list[idx].count = 0;
...
static void dump_access_sites(htab_t accs) {
  if (!dump_file)
    return;
...
static int remove_str_allocs_in_func(void **slot, void *data) {
...
static void remove_str_allocs(d_str str) {
  if (!str)
    return;
...
static void remove_structure(unsigned i) {
  d_str str;
...
static bool is_safe_cond_expr(gimple cond_stmt) {
  tree arg0, arg1;
  unsigned str0, str1;
  bool s0, s1;
  unsigned length = VEC_length(structure, structures);
...
static int exclude_from_accs(void **slot, void *data) {
  struct access_site *acc = *(struct access_site **)slot;
...
static tree get_stmt_accesses(tree *tp, int *walk_subtrees, void *data) {
  struct walk_stmt_info *wi = (struct walk_stmt_info *)data;
  ...
  switch (TREE_CODE(t)) {
    ...
    case COMPONENT_REF: {
      ...
      if ((TREE_CODE(ref) == INDIRECT_REF || TREE_CODE(ref) == ARRAY_REF ||
        ...
        if (i != VEC_length(structure, structures)) {
          ...
          struct field_entry *field = find_field_in_struct(str, field_decl);
          ...
          if (field) {
            ...
            if (!decompose_access(str->decl, acc)) {
              if (dump_file) {
                fprintf(dump_file, "\nThe type ");
                print_generic_expr(dump_file, type, 0);
                fprintf(dump_file, " has complicate access in statement ");
...
static void free_structures(void) {
  d_str str;
  unsigned i;
...
static int free_new_var(void **slot, void *data ATTRIBUTE_UNUSED) {
  new_var n_var = *(new_var *)slot;
...
static void free_new_vars_htab(htab_t new_vars_htab) {
  if (new_vars_htab)
...
static void create_new_accesses_for_func(void) {
  basic_block bb;
...
static void create_new_alloc_sites_for_func(struct cgraph_node *node) {
...
static void create_new_local_vars(void) {
  tree var;
  referenced_var_iterator rvi;

  new_local_vars =
      htab_create(num_referenced_vars, new_var_hash, new_var_eq, NULL);
...
static inline void print_shift(unsigned HOST_WIDE_INT shift) {
...
static inline void update_fields_mapping(struct field_cluster *cluster,
                                         tree new_type,
...
static tree build_basic_struct(tree fields, tree name, tree orig_struct) {
...
static tree create_fields(struct field_cluster *cluster,
...
static inline tree gen_cluster_name(tree decl, int clust_num, int str_num) {
...
static void check_bitfields(d_str str, VEC(tree, heap) * *unsuitable_types) {
  tree type = str->decl;
  int i;
...
static void exclude_escaping_types_1(VEC(tree, heap) * *unsuitable_types) {
  d_str str;
  unsigned i;
...
static void exclude_returned_types(VEC(tree, heap) * *unsuitable_types) {
    ...
    if (ret_t) {
      ...
      if (TREE_CODE(ret_t) == RECORD_TYPE) {
        ...
        if (dump_file) {
          ...
          fprintf(dump_file, "\" is return type of function...Excluded.");
        }
      }
    }
  }
...
static void exclude_types_passed_to_local_func(VEC(tree, heap) *
                                               *unsuitable_types) {
  struct cgraph_node *c_node;
  ...
  for (c_node = cgraph_nodes; c_node; c_node = c_node->next)
    if (cgraph_function_body_availability(c_node) == AVAIL_LOCAL) {
      ...
      for (arg = DECL_ARGUMENTS(fn); arg; arg = TREE_CHAIN(arg)) {
        ...
        if (TREE_CODE(type) == RECORD_TYPE) {
          add_unsuitable_type(unsuitable_types, TYPE_MAIN_VARIANT(type));
          if (dump_file) {
            fprintf(dump_file, "\nPointer to type \"");
            print_generic_expr(dump_file, type, 0);
...
static void check_struct_form(d_str str, VEC(tree, heap) * *unsuitable_types) {
  int i;
...
static void add_structure(tree type) {
...
static void add_alloc_site(tree fn_decl, gimple stmt, d_str str) {
  ...
  fallocs = (fallocs_t)htab_find_with_hash(alloc_sites, fn_decl,
                                           htab_hash_pointer(fn_decl));

  if (!fallocs) {
    void **slot;

    fallocs = (fallocs_t)xmalloc(sizeof(struct func_alloc_sites));
  ...
  VEC_safe_push(alloc_site_t, heap, fallocs->allocs, &m_call);
...
static bool is_alloc_of_struct(gimple stmt, unsigned *i_p) {
  tree lhs;
  tree type;
  gimple final_stmt;
  ...
  if (!POINTER_TYPE_P(type) || TREE_CODE(strip_type(type)) != RECORD_TYPE)
    return false;
...
static void dump_accs(d_str str) {
  int i;
...
static int safe_cond_expr_check(void **slot, void *data) {
  struct access_site *acc = *(struct access_site **)slot;

  if (gimple_code(acc->stmt) == GIMPLE_COND && !is_safe_cond_expr(acc->stmt)) {
    if (dump_file) {
...
static void exclude_alloc_and_field_accs_1(d_str str,
...
static void collect_accesses_in_bb(basic_block bb) {
  gimple_stmt_iterator bsi;
  struct walk_stmt_info wi;
...
static void gen_cluster(sbitmap fields, d_str str) {
...
static void peel_field(int i, d_str ds) {
  struct field_cluster *crr_cluster = NULL;
  ...
  crr_cluster->fields_in_cluster = sbitmap_alloc((unsigned int)ds->num_fields);
...
static gcov_type get_max_field_count(d_str str) {
  gcov_type max = 0;
  int i;
...
static void do_reorg_for_func(struct cgraph_node *node) {
...
static void dump_struct_type(tree type, unsigned HOST_WIDE_INT indent,
...
static void create_new_type(d_str str, int *str_num) {
  int cluster_num = 0;

  struct field_cluster *cluster = str->struct_clustering;
  while (cluster) {
    tree name = gen_cluster_name(str->decl, cluster_num, *str_num);
    tree fields;
    tree new_type;
    cluster_num++;
...
static int free_falloc_sites(void **slot, void *data ATTRIBUTE_UNUSED) {
  fallocs_t fallocs = *(fallocs_t *)slot;
...
static void remove_unsuitable_types(VEC(tree, heap) * unsuitable_types) {
  d_str str;
  tree type;
  unsigned i, j;
...
static void exclude_types_with_bit_fields(VEC(tree, heap) * *unsuitable_types) {
  d_str str;
  unsigned i;
...
static void exclude_escaping_types(VEC(tree, heap) * *unsuitable_types) {
...
static void build_data_structure(VEC(tree, heap) * *unsuitable_types) {
  tree var, type;
  tree var_list;
...
static bool program_redefines_malloc_p(void) {
  struct cgraph_node *c_node;
    ...
    for (c_edge = c_node->callees; c_edge; c_edge = c_edge->next_callee) {
      ...
      if (is_gimple_call(c_edge->call_stmt)) {
        ...
        if (DECL_FUNCTION_CODE(fndecl2) == BUILT_IN_OBJECT_SIZE ||
            !strcmp(fname, "__builtin_offsetof") || !strcmp(fname, "realloc"))
          return true;
      }
    }
  }
...
static void collect_alloc_sites(void) {
  struct cgraph_node *node;
...
static void dump_accesses(void) {
  d_str str;
  unsigned i;
...
static void check_cond_exprs(void) {
  d_str str;
  unsigned i;
...
static void exclude_alloc_and_field_accs(struct cgraph_node *node) {
  d_str str;
  unsigned i;
...
static void collect_accesses_in_func(struct function *fn) {
  basic_block bb;
...
static void sum_counts(d_str str, gcov_type *hottest) {
  int i;
...
static void create_new_global_vars(void) {
  struct varpool_node *current_varpool;
  ...
  for (i = 0; i < 2; i++) {
    if (i)
      new_global_vars =
          htab_create(varpool_size, new_var_hash, new_var_eq, NULL);
    FOR_EACH_STATIC_VARIABLE(current_varpool) {
...
static void dump_new_types(void) {
  d_str str;
  tree type;
  unsigned i, j;
...
static void create_new_types(void) {
  d_str str;
  unsigned i;
  int str_num = 0;
...
static void free_alloc_sites(void) {
  if (alloc_sites)
...
static void collect_structures(void) {
...
static void collect_allocation_sites(void) {
...
static void collect_data_accesses(void) {
  struct cgraph_node *c_node;
...
static void exclude_cold_structs(void) {
  gcov_type hottest = 0;
  unsigned i;
  d_str str;
...
static void free_data_structs(void) {
  free_structures();
  free_alloc_sites();
}
...
static void reorg_structs(void) {
  /* Stage1.  */
...
static unsigned int reorg_structs_drive(void) {
  reorg_structs();
  return 0;
}
