...
static void mark_mem_store(rtx loc, const_rtx setter ATTRIBUTE_UNUSED,
...
static void create_ddg_dep_from_intra_loop_link(ddg_ptr g,
  ...
  dep_type t = TRUE_DEP;
  dep_data_type dt =
      (mem_access_insn_p(src_node->insn) && mem_access_insn_p(dest_node->insn)
           ? MEM_DEP
           : REG_DEP);
...
static void create_ddg_dep_no_link(ddg_ptr g, ddg_node_ptr from,
                                   ddg_node_ptr to, dep_type d_t,
                                   dep_data_type d_dt, int distance) {
  ddg_edge_ptr e;
  int l;
...
static void add_cross_iteration_register_deps(ddg_ptr g, df_ref last_def) {
  ...
  for (r_use = DF_REF_CHAIN(last_def); r_use != NULL; r_use = r_use->next) {
    ...
    if (use_node->cuid <= last_def_node->cuid) {
      /* Add true deps from last_def to it's uses in the next
         ...
         the last_def def.  */
      create_ddg_dep_no_link(g, last_def_node, use_node, TRUE_DEP, REG_DEP, 1);
    } else {
      ...
      if (DF_REF_ID(last_def) != DF_REF_ID(first_def) ||
        ...
        create_ddg_dep_no_link(g, use_node, first_def_node, ANTI_DEP, REG_DEP,
                               1);
    }
  }
  ...
  if (!has_use_in_bb_p) {
    ...
    create_ddg_dep_no_link(g, last_def_node, dest_node, OUTPUT_DEP, REG_DEP, 1);
  }
}
...
/* Given two nodes, analyze their RTL insns and add inter-loop mem deps
   to ddg G.  */
static void add_inter_loop_mem_dep(ddg_ptr g, ddg_node_ptr from,
                                   ddg_node_ptr to) {
...
ddg_ptr create_ddg(basic_block bb, int closing_branch_deps) {
  ddg_ptr g;
  rtx insn, first_note;
  int i;
  int num_nodes = 0;
...
void print_ddg_edge(FILE *file, ddg_edge_ptr e) {
  char dep_c;
  ...
  fprintf(file, " [%d -(%c,%d,%d)-> %d] ", INSN_UID(e->src->insn), dep_c,
          e->latency, e->distance, INSN_UID(e->dest->insn));
}

/* Print the DDG nodes with there in/out edges to the dump file.  */
void print_ddg(FILE *file, ddg_ptr g) {
  int i;
...
void vcg_print_ddg(FILE *file, ddg_ptr g) {
  int src_cuid;
...
void print_sccs(FILE *file, ddg_all_sccs_ptr sccs, ddg_ptr g) {
  unsigned int u = 0;
...
static ddg_edge_ptr create_ddg_edge(ddg_node_ptr src, ddg_node_ptr dest,
                                    dep_type t, dep_data_type dt, int l,
                                    int d) {
...
static void add_edge_to_ddg(ddg_ptr g ATTRIBUTE_UNUSED, ddg_edge_ptr e) {
...
static void add_backarc_to_ddg(ddg_ptr g, ddg_edge_ptr e) {
...
static void add_backarc_to_scc(ddg_scc_ptr scc, ddg_edge_ptr e) {
...
static void add_scc_to_ddg(ddg_all_sccs_ptr g, ddg_scc_ptr scc) {
...
static void check_sccs(ddg_all_sccs_ptr sccs, int num_nodes) {
  int i = 0;
...
ddg_all_sccs_ptr create_ddg_all_sccs(ddg_ptr g) {
  ...
  ddg_all_sccs_ptr sccs =
      (ddg_all_sccs_ptr)xmalloc(sizeof(struct ddg_all_sccs));
...
static int update_dist_to_successors(ddg_node_ptr u_node, sbitmap nodes,
                                     sbitmap tmp) {
  ddg_edge_ptr e;
  int result = 0;
  ...
  for (e = u_node->out; e; e = e->next_out) {
    ...
    if (TEST_BIT(nodes, v) && (e->distance == 0) &&
