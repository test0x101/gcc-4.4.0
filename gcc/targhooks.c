...
void default_external_libcall(rtx fun ATTRIBUTE_UNUSED) {
#ifdef ASM_OUTPUT_EXTERNAL_LIBCALL
  ASM_OUTPUT_EXTERNAL_LIBCALL(asm_out_file, fun);
#endif
}

int default_unspec_may_trap_p(const_rtx x, unsigned flags) {
  int i;

  if (GET_CODE(x) == UNSPEC_VOLATILE
      /* Any floating arithmetic may trap.  */
      || (SCALAR_FLOAT_MODE_P(GET_MODE(x)) && flag_trapping_math))
    return 1;
...
enum machine_mode default_cc_modes_compatible(enum machine_mode m1,
...
void default_setup_incoming_varargs(CUMULATIVE_ARGS *ca ATTRIBUTE_UNUSED,
...
bool hook_bool_CUMULATIVE_ARGS_false(CUMULATIVE_ARGS *ca ATTRIBUTE_UNUSED) {
  return false;
}
...
unsigned HOST_WIDE_INT default_shift_truncation_mask(enum machine_mode mode) {
...
int default_mode_rep_extended(enum machine_mode mode ATTRIBUTE_UNUSED,
...
bool hook_bool_CUMULATIVE_ARGS_true(CUMULATIVE_ARGS *a ATTRIBUTE_UNUSED) {
  return true;
}

/* Return machine mode for non-standard suffix
   or VOIDmode if non-standard suffixes are unsupported.  */
enum machine_mode default_mode_for_suffix(char suffix ATTRIBUTE_UNUSED) {
  return VOIDmode;
}
...
bool hook_callee_copies_named(CUMULATIVE_ARGS *ca ATTRIBUTE_UNUSED,
                              enum machine_mode mode ATTRIBUTE_UNUSED,
...
void default_unwind_emit(FILE *stream ATTRIBUTE_UNUSED,
...
bool default_scalar_mode_supported_p(enum machine_mode mode) {
...
const char *default_invalid_within_doloop(const_rtx insn) {
  if (CALL_P(insn))
    return "Function call in loop.";

  if (JUMP_P(insn) && (GET_CODE(PATTERN(insn)) == ADDR_DIFF_VEC ||
...
tree default_builtin_vectorized_function(
    enum built_in_function fn ATTRIBUTE_UNUSED, tree type_out ATTRIBUTE_UNUSED,
...
tree default_builtin_vectorized_conversion(enum tree_code code ATTRIBUTE_UNUSED,
                                           tree type ATTRIBUTE_UNUSED) {
  return NULL_TREE;
}
...
tree default_builtin_reciprocal(enum built_in_function fn ATTRIBUTE_UNUSED,
                                bool md_fn ATTRIBUTE_UNUSED,
                                bool sqrt ATTRIBUTE_UNUSED) {
  return NULL_TREE;
}

bool hook_bool_CUMULATIVE_ARGS_mode_tree_bool_false(
    CUMULATIVE_ARGS *ca ATTRIBUTE_UNUSED,
    enum machine_mode mode ATTRIBUTE_UNUSED, const_tree type ATTRIBUTE_UNUSED,
...
bool hook_bool_CUMULATIVE_ARGS_mode_tree_bool_true(
    CUMULATIVE_ARGS *ca ATTRIBUTE_UNUSED,
    enum machine_mode mode ATTRIBUTE_UNUSED, const_tree type ATTRIBUTE_UNUSED,
    bool named ATTRIBUTE_UNUSED) {
  return true;
...
void hook_void_bitmap(bitmap regs ATTRIBUTE_UNUSED) {}

const char *hook_invalid_arg_for_unprototyped_fn(
    const_tree typelist ATTRIBUTE_UNUSED, const_tree funcdecl ATTRIBUTE_UNUSED,
...
tree default_hidden_stack_protect_fail(void) {
  ...
  if (t == NULL_TREE) {
    t = build_function_type_list(void_type_node, NULL_TREE);
    t = build_decl(FUNCTION_DECL, get_identifier("__stack_chk_fail_local"), t);
    TREE_STATIC(t) = 1;
    TREE_PUBLIC(t) = 1;
    DECL_EXTERNAL(t) = 1;
    TREE_USED(t) = 1;
...
rtx default_function_value(const_tree ret_type ATTRIBUTE_UNUSED,
...
const enum reg_class *default_ira_cover_classes(void) {
  static enum reg_class classes[] = IRA_COVER_CLASSES;
  return classes;
}
#endif

enum reg_class default_secondary_reload(
    bool in_p ATTRIBUTE_UNUSED, rtx x ATTRIBUTE_UNUSED,
  ...
  if (rclass != NO_REGS) {
    ...
    if (icode != CODE_FOR_nothing &&
      ...
      if (!*insn_constraint)
        ...
        insn_class = (insn_letter == 'r'
                          ? GENERAL_REGS
                          : REG_CLASS_FROM_CONSTRAINT(
                                (unsigned char)insn_letter, insn_constraint));
...
bool default_handle_c_option(size_t code ATTRIBUTE_UNUSED,
...
tree default_mangle_decl_assembler_name(tree decl ATTRIBUTE_UNUSED, tree id) {
  return id;
}

bool default_builtin_vector_alignment_reachable(const_tree type,
                                                bool is_packed) {
  if (is_packed)
    return false;
...
bool default_target_option_valid_attribute_p(tree ARG_UNUSED(fndecl),
...
bool default_target_option_pragma_parse(tree ARG_UNUSED(args),
                                        tree ARG_UNUSED(pop_target)) {
  warning(OPT_Wpragmas, "#pragma GCC target is not supported for this machine");

  return false;
}

bool default_target_option_can_inline_p(tree caller, tree callee) {
  bool ret = false;
