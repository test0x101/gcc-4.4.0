...
lambda_linear_expression lambda_linear_expression_new(int, int,
                                                      struct obstack *);
void print_lambda_linear_expression(FILE *, lambda_linear_expression, int, int,
                                    char);
...
#define lambda_loop_new() \
  (lambda_loop) ggc_alloc_cleared(sizeof(struct lambda_loop_s))
...
void lambda_matrix_add(lambda_matrix, lambda_matrix, lambda_matrix, int, int);
void lambda_matrix_add_mc(lambda_matrix, int, lambda_matrix, int, lambda_matrix,
                          int, int);
void lambda_matrix_mult(lambda_matrix, lambda_matrix, lambda_matrix, int, int,
                        int);
...
void lambda_matrix_left_hermite(lambda_matrix, int, int, lambda_matrix,
                                lambda_matrix);
void lambda_matrix_right_hermite(lambda_matrix, int, int, lambda_matrix,
                                 lambda_matrix);
int lambda_matrix_first_nz_vec(lambda_matrix, int, int, int);
void lambda_matrix_project_to_null(lambda_matrix, int, int, int, lambda_vector);
...
lambda_loopnest gcc_loopnest_to_lambda_loopnest(struct loop *,
                                                ...
                                                VEC(tree, heap) **,
                                                struct obstack *);
void lambda_loopnest_to_gcc_loopnest(struct loop *, VEC(tree, heap) *,
                                     VEC(tree, heap) *, VEC(gimple, heap) **,
...
static inline void lambda_vector_mult_const(lambda_vector, lambda_vector, int,
                                            int);
...
static inline void lambda_vector_mult_const(lambda_vector vec1,
                                            lambda_vector vec2, int size,
                                            int const1) {
  int i;
...
static inline void lambda_vector_negate(lambda_vector vec1, lambda_vector vec2,
                                        int size) {
...
static inline void lambda_vector_add(lambda_vector vec1, lambda_vector vec2,
...
static inline void lambda_vector_add_mc(lambda_vector vec1, int const1,
...
static inline void lambda_vector_copy(lambda_vector vec1, lambda_vector vec2,
                                      int size) {
...
static inline bool lambda_vector_equal(lambda_vector vec1, lambda_vector vec2,
                                       int size) {
  int i;
  for (i = 0; i < size; i++)
    if (vec1[i] != vec2[i])
...
static inline int lambda_vector_min_nz(lambda_vector vec1, int n, int start) {
  int j;
  int min = -1;
...
static inline int lambda_vector_first_nz(lambda_vector vec1, int n, int start) {
  int j = start;
  while (j < n && vec1[j] == 0)
    j++;
  return j;
...
static inline void lambda_vector_matrix_mult(lambda_vector vect, int m,
                                             lambda_matrix mat, int n,
                                             lambda_vector dest) {
  int i, j;
...
static inline int lambda_vector_compare(lambda_vector vec1, int length1,
                                        lambda_vector vec2, int length2) {
  int min_length;
  int i;
...
static inline void print_lambda_vector(FILE *outfile, lambda_vector vector,
                                       int n) {
  int i;
...
static inline tree build_linear_expr(tree type, lambda_vector coefs,
                                     VEC(tree, heap) * ivs) {
  unsigned i;
  tree iv;
  ...
  for (i = 0; VEC_iterate(tree, ivs, i, iv); i++) {
    ...
    if (k == 1)
      ...
      expr =
          fold_build2(PLUS_EXPR, type, expr,
                      fold_build2(MULT_EXPR, type, iv, build_int_cst(type, k)));
  }
