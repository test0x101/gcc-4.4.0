...
static struct path_prefix *libpaths[3] = {&cmdline_lib_dirs, &libpath_lib_dirs,
                                          NULL};
#endif
...
void dump_file(const char *name, FILE *to) {
  ...
  while (1) {
    int c;
    while (c = getc(stream), c != EOF && (ISIDNUM(c) || c == '$' || c == '.'))
      obstack_1grow(&temporary_obstack, c);
...
static symkind is_ctor_dtor(const char *s) {
  ...
  for (p = &special[0]; p->len > 0; p++) {
    if (ch == p->name[0] && (!p->two_underscores || ((s - orig_s) >= 2)) &&
...
static void prefix_from_env(const char *env, struct path_prefix *pprefix) {
  const char *p;
...
static void prefix_from_string(const char *p, struct path_prefix *pprefix) {
...
int collect_wait(const char *prog, struct pex_obj *pex) {
  ...
  if (status) {
    if (WIFSIGNALED(status)) {
      ...
      error("%s terminated with signal %d [%s]%s", prog, sig, strsignal(sig),
...
static void do_wait(const char *prog, struct pex_obj *pex) {
...
struct pex_obj *collect_execute(const char *prog, char **argv,
  ...
  errmsg = pex_run(pex, PEX_LAST | PEX_SEARCH, argv[0], argv, outname, errname,
                   &err);
  if (errmsg != NULL) {
    if (err != 0) {
      errno = err;
...
static void fork_execute(const char *prog, char **argv) {
  struct pex_obj *pex;
...
static void maybe_unlink(const char *file) {
  if (!debug)
...
static void add_to_list(struct head *head_ptr, const char *name) {
  ...
  for (p = head_ptr->first; strcmp(name, p->name) != 0; p = p->next)
    ;
  if (p != newid) {
...
static int is_in_args(const char *string, const char **args_begin,
                      const char **args_end) {
...
#ifdef COLLECT_EXPORT_LIST
static void dump_list(FILE *stream, const char *prefix, struct id *list) {
  while (list) {
    fprintf(stream, "%s%s,\n", prefix, list->name);
    list = list->next;
  }
...
static void write_list_with_asm(FILE *stream, const char *prefix,
                                struct id *list) {
  while (list) {
...
#ifdef COLLECT_EXPORT_LIST
static void write_aix_file(FILE *stream, struct id *list) {
  for (; list; list = list->next) {
...
static void scan_prog_file(const char *prog_name, enum pass which_pass) {
...
static void scan_libraries(const char *prog_name) {
...
#define GCC_OK_SYMBOL(X)                                             \
  (((X).n_sclass == C_EXT || (X).n_sclass == C_WEAKEXT) &&           \
   ((X).n_scnum > N_UNDEF) &&                                        \
   (aix64_flag || (((X).n_type & N_TMASK) == (DT_NON << N_BTSHFT) || \
...
static const char *const aix_std_libs[] = {"/unix",
                                           "/lib/libc.a",
                                           "/lib/libm.a",
                                           "/lib/libc_r.a",
                                           "/lib/libm_r.a",
...
static int ignore_library(const char *name) {
      ...
      if (GCC_CHECK_HDR(ldptr)) {
          ...
          if (GCC_OK_SYMBOL(symbol)) {
            ...
            switch (is_ctor_dtor(name)) {
              ...
              default: /* not a constructor or destructor */
                ...
                if (shared_obj && !is_shared && which_pass == PASS_OBJ &&
                    !export_flag)
                  add_to_list(&exports, name);
#endif
                continue;
            ...
            if (debug)
              ...
              fprintf(stderr, "\tsec=%d class=%d type=%s%o %s\n",
                      ...
                      (symbol.n_type ? "0" : ""), symbol.n_type, name);
#else
            fprintf(stderr,
        ...
        if (debug)
          fprintf(stderr, "%s : magic=%o aix64=%d mismatch\n", prog_name,
                  HEADER(ldptr).f_magic, aix64_flag);
      }
#endif
    } else {
...
static char *resolve_lib_name(const char *name) {
  char *lib_buf;
  int i, j, l = 0;
  ...
  for (i = 0; libpaths[i]; i++) {
    ...
    for (; list; list = list->next) {
      ...
      if (list->prefix[strlen(list->prefix) - 1] != '/')
        p = "/";
      for (j = 0; j < 2; j++) {
        sprintf(lib_buf, "%s%slib%s.%s", list->prefix, p, name,
