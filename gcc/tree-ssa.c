...
void redirect_edge_var_map_add(edge e, tree result, tree def) {
  void **slot;
...
edge_var_map_vector redirect_edge_var_map_vector(edge e) {
  void **slot;
...
static bool free_var_map_entry(const void *key ATTRIBUTE_UNUSED, void **value,
...
static bool verify_def(basic_block bb, basic_block *definition_block,
                       tree ssa_name, gimple stmt, bool is_virtual) {
...
static bool verify_use(basic_block bb, basic_block def_bb, use_operand_p use_p,
                       gimple stmt, bool check_abnormal,
  ...
  if (gimple_nop_p(SSA_NAME_DEF_STMT(ssa_name)) &&
    ...
    err = true;
  } else if (bb != def_bb && !dominated_by_p(CDI_DOMINATORS, bb, def_bb)) {
    error("definition in block %i does not dominate use in block %i",
          def_bb->index, bb->index);
    err = true;
  } else if (bb == def_bb && names_defined_in_bb != NULL &&
             !bitmap_bit_p(names_defined_in_bb, SSA_NAME_VERSION(ssa_name))) {
    error("definition in block %i follows the use", def_bb->index);
    err = true;
  }

  if (check_abnormal && !SSA_NAME_OCCURS_IN_ABNORMAL_PHI(ssa_name)) {
...
static bool verify_phi_args(gimple phi, basic_block bb,
  ...
  for (i = 0; i < phi_num_args; i++) {
    ...
    if (op == NULL_TREE) {
      error("PHI argument is missing for edge %d->%d", e->src->index,
            e->dest->index);
      err = true;
      goto error;
    }
    ...
    if (TREE_CODE(op) == SSA_NAME) {
      ...
      err |= verify_use(e->src, definition_block[SSA_NAME_VERSION(op)], op_p,
                        phi, e->flags & EDGE_ABNORMAL, NULL);
    }

    if (e->dest != bb) {
      error("wrong edge %d->%d for PHI argument", e->src->index,
            e->dest->index);
      err = true;
    }
...
static void verify_flow_insensitive_alias_info(void) {
  ...
  FOR_EACH_REFERENCED_VAR(var, rvi) {
    ...
    EXECUTE_IF_SET_IN_BITMAP(aliases, 0, j, bi) {
      ...
      if (TREE_CODE(alias) != MEMORY_PARTITION_TAG && !may_be_aliased(alias)) {
...
static void verify_flow_sensitive_alias_info(void) {
  ...
  for (i = 1; i < num_ssa_names; i++) {
    ...
    if (TREE_CODE(var) == RESULT_DECL && is_gimple_reg(ptr))
      continue;
...
static void verify_call_clobbering(void) {
  unsigned int i;
  bitmap_iterator bi;
  tree var;
  referenced_var_iterator rvi;
  ...
  EXECUTE_IF_SET_IN_BITMAP(gimple_call_clobbered_vars(cfun), 0, i, bi) {
    ...
    if (!MTAG_P(var) && !var_ann(var)->call_clobbered) {
      error(
          "variable in call_clobbered_vars but not marked "
          "call_clobbered");
      debug_variable(var);
      goto err;
    }
  ...
  FOR_EACH_REFERENCED_VAR(var, rvi) {
    ...
    if (!MTAG_P(var) && var_ann(var)->call_clobbered &&
      ...
      error(
          "variable marked call_clobbered but not in "
...
static void verify_memory_partitions(void) {
  unsigned i;
  tree mpt;
  ...
  for (i = 0; VEC_iterate(tree, mpt_table, i, mpt); i++) {
    ...
    EXECUTE_IF_SET_IN_BITMAP(MPT_SYMBOLS(mpt), 0, j, bj) {
      ...
      if (pointer_set_insert(partitioned_syms, var)) {
        error(
            "Partitioned symbols should belong to exactly one "
            "partition");
        debug_variable(var);
        goto err;
      }
...
void verify_ssa(bool check_modified_stmt) {
  size_t i;
  basic_block bb;
  ...
  for (i = 1; i < num_ssa_names; i++) {
    ...
    if (name) {
      ...
      if (!gimple_nop_p(stmt)) {
        ...
        verify_def(bb, definition_block, name, stmt, !is_gimple_reg(name));
      }
    }
  }
  ...
  FOR_EACH_BB(bb) {
    ...
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (is_gimple_assign(stmt) &&
        ...
        if (base_address && gimple_aliases_computed_p(cfun) &&
            SSA_VAR_P(base_address) && !gimple_has_volatile_ops(stmt) &&
      ...
      FOR_EACH_SSA_USE_OPERAND(use_p, stmt, iter, SSA_OP_USE | SSA_OP_VUSE) {
        ...
        if (verify_use(bb, definition_block[SSA_NAME_VERSION(op)], use_p, stmt,
...
int int_tree_map_eq(const void *va, const void *vb) {
...
unsigned int int_tree_map_hash(const void *item) {
  return ((const struct int_tree_map *)item)->uid;
}

/* Return true if the DECL_UID in both trees are equal.  */

int uid_decl_map_eq(const void *va, const void *vb) {
...
unsigned int uid_decl_map_hash(const void *item) {
  return ((const_tree)item)->decl_minimal.uid;
}

/* Return true if the DECL_UID in both trees are equal.  */

static int uid_ssaname_map_eq(const void *va, const void *vb) {
  const_tree a = (const_tree)va;
  const_tree b = (const_tree)vb;
...
static unsigned int uid_ssaname_map_hash(const void *item) {
...
void init_tree_ssa(struct function *fn) {
  ...
  fn->gimple_df->referenced_vars =
      htab_create_ggc(20, uid_decl_map_hash, uid_decl_map_eq, NULL);
  fn->gimple_df->default_defs =
      htab_create_ggc(20, uid_ssaname_map_hash, uid_ssaname_map_eq, NULL);
...
static bool useless_type_conversion_p_1(tree outer_type, tree inner_type) {
  /* Do the following before stripping toplevel qualifiers.  */
  if (POINTER_TYPE_P(inner_type) && POINTER_TYPE_P(outer_type)) {
    /* Do not lose casts to restrict qualified pointers.  */
    if ((TYPE_RESTRICT(outer_type) != TYPE_RESTRICT(inner_type)) &&
  ...
  if (INTEGRAL_TYPE_P(inner_type) && INTEGRAL_TYPE_P(outer_type)) {
    ...
    /* Conversions from a non-base to a base type are not useless.
       ...
       base types only.  */
    if (TREE_TYPE(inner_type) && TREE_TYPE(inner_type) != inner_type &&
  ...
  else if (SCALAR_FLOAT_TYPE_P(inner_type) && SCALAR_FLOAT_TYPE_P(outer_type))
    return true;
  ...
  /* For aggregates we may need to fall back to structural equality
     checks.  */
  else if (AGGREGATE_TYPE_P(inner_type) && AGGREGATE_TYPE_P(outer_type)) {
...
bool tree_ssa_useless_type_conversion(tree expr) {
  ...
  if (CONVERT_EXPR_P(expr) || TREE_CODE(expr) == VIEW_CONVERT_EXPR ||
      TREE_CODE(expr) == NON_LVALUE_EXPR)
    return useless_type_conversion_p(TREE_TYPE(expr),
...
static bool walk_use_def_chains_1(tree var, walk_use_def_chains_fn fn,
                                  void *data, struct pointer_set_t *visited,
                                  bool is_dfs) {
  gimple def_stmt;
...
void walk_use_def_chains(tree var, walk_use_def_chains_fn fn, void *data,
                         bool is_dfs) {
  gimple def_stmt;
...
bool ssa_undefined_value_p(tree t) {
...
static void warn_uninit(tree t, const char *gmsgid, void *data) {
  tree var = SSA_NAME_VAR(t);
  ...
  if (warning_at(location, OPT_Wuninitialized, gmsgid, var)) {
    ...
    if (xloc.file != floc.file || xloc.line < floc.line ||
...
static tree warn_uninitialized_var(tree *tp, int *walk_subtrees, void *data_) {
  ...
  switch (TREE_CODE(t)) {
    ...
    case VAR_DECL: {
      ...
      if (data->stmt == NULL || !gimple_aliases_computed_p(cfun))
        return NULL_TREE;
      ...
      if (t != SSA_NAME_VAR(op) || !SSA_NAME_IS_DEFAULT_DEF(op))
        return NULL_TREE;
...
static void warn_uninitialized_phi(gimple phi) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (TREE_CODE(op) == SSA_NAME)
      warn_uninit(op, "%qD may be used uninitialized in this function", NULL);
  }
}

static unsigned int warn_uninitialized_vars(bool warn_possibly_uninitialized) {
  gimple_stmt_iterator gsi;
  basic_block bb;
  ...
  FOR_EACH_BB(bb) {
    data.always_executed =
        dominated_by_p(CDI_POST_DOMINATORS, single_succ(ENTRY_BLOCK_PTR), bb);
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
...
static unsigned int execute_early_warn_uninitialized(void) {
...
static unsigned int execute_late_warn_uninitialized(void) {
  basic_block bb;
  gimple_stmt_iterator gsi;
...
static unsigned int execute_update_addresses_taken(void) {
  tree var;
  referenced_var_iterator rvi;
  gimple_stmt_iterator gsi;
  basic_block bb;
  ...
  FOR_EACH_REFERENCED_VAR(var, rvi) {
    ...
    if (is_global_var(var) || TREE_CODE(var) == RESULT_DECL ||
