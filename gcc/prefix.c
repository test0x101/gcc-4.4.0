...
static char *lookup_key(char *key) {
  char *dst;
  DWORD size;
  DWORD type;
  LONG res;
  ...
  if (reg_key == (HKEY)INVALID_HANDLE_VALUE) {
    ...
    if (res == ERROR_SUCCESS)
      res = RegOpenKeyExA(reg_key, WIN32_REGISTRY_KEY, 0, KEY_READ, &reg_key);
...
static void tr(char *string, int c1, int c2) {
  do {
    if (*string == c1)
      *string = c2;
  } while (*string++);
