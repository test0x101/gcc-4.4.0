...
#define PARM_PASSED_IN_MEMORY(PARM) (MEM_P(DECL_INCOMING_RTL(PARM)))

/* A C expression for the integer offset value of an automatic variable
   (N_LSYM) having address X (an RTX).  */
#ifndef DEBUGGER_AUTO_OFFSET
#define DEBUGGER_AUTO_OFFSET(X) (GET_CODE(X) == PLUS ? INTVAL(XEXP(X, 1)) : 0)
#endif
...
void dbxout_stab_value_label(const char *label) {
...
/* Write out the difference of two labels, LABEL - BASE, as the value
   of a stab.  */
void dbxout_stab_value_label_diff(const char *label, const char *base) {
  assemble_name(asm_out_file, label);
  putc('-', asm_out_file);
...
void dbxout_stab_value_internal_label(const char *stem, int *counterp) {
  char label[100];
...
void dbxout_stab_value_internal_label_diff(const char *stem, int *counterp,
                                           const char *base) {
  char label[100];
  int counter = counterp ? (*counterp)++ : 0;
...
void dbxout_stabd(int stype, int sdesc) {
...
void dbxout_begin_stabn(int stype) {
...
void dbxout_begin_stabn_sline(int lineno) {
  fputs(ASM_STABN_OP, asm_out_file);
  dbxout_int(N_SLINE);
  fputs(",0,", asm_out_file);
  dbxout_int(lineno);
...
void dbxout_begin_empty_stabs(int stype) {
...
/* Begin a .stabs directive with string STR, type STYPE, and desc 0.
   The value field is the responsibility of the caller.  */
void dbxout_begin_simple_stabs(const char *str, int stype) {
  fputs(ASM_STABS_OP, asm_out_file);
...
void dbxout_begin_simple_stabs_desc(const char *str, int stype, int sdesc) {
  fputs(ASM_STABS_OP, asm_out_file);
  output_quoted_string(asm_out_file, str);
  putc(',', asm_out_file);
  dbxout_int(stype);
...
static void dbxout_begin_complex_stabs(void) {
  emit_pending_bincls_if_required();
  FORCE_TEXT;
  fputs(ASM_STABS_OP, asm_out_file);
  putc('"', asm_out_file);
...
static void dbxout_begin_complex_stabs_noforcetext(void) {
  fputs(ASM_STABS_OP, asm_out_file);
  putc('"', asm_out_file);
  gcc_assert(stabstr_last_contin_point == 0);
}
...
static void stabstr_D(HOST_WIDE_INT num) {
  char buf[64];
  char *p = buf + sizeof buf;
  unsigned int unum;
...
static void stabstr_U(unsigned HOST_WIDE_INT num) {
  char buf[64];
  char *p = buf + sizeof buf;
  if (num == 0) {
    stabstr_C('0');
...
static void stabstr_O(tree cst) {
  ...
  if (high == 0)
    ...
    if (HOST_BITS_PER_WIDE_INT % 3 != 0) {
      ...
      const unsigned HOST_WIDE_INT low_mask =
          (((unsigned HOST_WIDE_INT)1) << n_leftover_bits) - 1;
      const unsigned HOST_WIDE_INT high_mask =
          (((unsigned HOST_WIDE_INT)1) << n_bits_from_high) - 1;
...
static inline void stabstr_continue(void) {
...
static void dbxout_finish_complex_stabs(tree sym, STAB_CODE_TYPE code, rtx addr,
  ...
  if (DBX_CONTIN_LENGTH > 0) {
    ...
    for (;;) {
      ...
      DBX_FINISH_STABS(sym, code, len == 0 ? line : 0, addr, label, number);
      if (len == 0)
        break;
...
static void dbxout_function_end(tree decl) {
  ...
  if (!use_gnu_debug_info_extensions || NO_DBX_FUNCTION_END ||
      !targetm.have_named_sections || DECL_IGNORED_P(decl))
    return;
...
static unsigned int ATTRIBUTE_UNUSED get_lang_number(void) {
...
static void dbxout_init(const char *input_file_name) {
...
static void dbxout_typedefs(tree syms) {
  for (; syms != NULL_TREE; syms = TREE_CHAIN(syms)) {
    if (TREE_CODE(syms) == TYPE_DECL) {
      tree type = TREE_TYPE(syms);
      if (TYPE_NAME(type) && TREE_CODE(TYPE_NAME(type)) == TYPE_DECL &&
          COMPLETE_OR_VOID_TYPE_P(type) && !TREE_ASM_WRITTEN(TYPE_NAME(type)))
...
static void emit_bincl_stab(const char *name) {
...
static void dbxout_start_source_file(unsigned int line ATTRIBUTE_UNUSED,
...
static void dbxout_end_source_file(unsigned int line ATTRIBUTE_UNUSED) {
#ifdef DBX_USE_BINCL
...
static void dbxout_handle_pch(unsigned at_end) {
  if (!at_end) {
...
static void dbxout_source_file(const char *filename) {
...
static void dbxout_begin_prologue(unsigned int lineno, const char *filename) {
  if (use_gnu_debug_info_extensions && !NO_DBX_FUNCTION_END &&
...
static void dbxout_source_line(unsigned int lineno, const char *filename) {
...
static void dbxout_begin_block(unsigned int line ATTRIBUTE_UNUSED,
                               unsigned int n) {
  emit_pending_bincls_if_required();
...
static void dbxout_end_block(unsigned int line ATTRIBUTE_UNUSED,
                             unsigned int n) {
  emit_pending_bincls_if_required();
...
static void dbxout_function_decl(tree decl) {
  emit_pending_bincls_if_required();
...
static void dbxout_global_decl(tree decl) {
...
static void dbxout_type_decl(tree decl, int local) {
...
static void dbxout_finish(const char *filename ATTRIBUTE_UNUSED) {
...
static void dbxout_type_index(tree type) {
...
static void dbxout_type_fields(tree type) {
  tree tem;
  ...
  for (tem = TYPE_FIELDS(type); tem; tem = TREE_CHAIN(tem)) {
    ...
    if (TREE_CODE(tem) == TYPE_DECL
        ...
        || (TREE_CODE(tem) == FIELD_DECL &&
            (!host_integerp(bit_position(tem), 0) || !DECL_SIZE(tem) ||
      ...
      dbxout_type((TREE_CODE(tem) == FIELD_DECL && DECL_BIT_FIELD_TYPE(tem))
...
static void dbxout_type_method_1(tree decl) {
  char c1 = 'A', c2;
...
static void dbxout_type_methods(tree type) {
...
static void dbxout_range_type(tree type) {
  stabstr_C('r');
  if (TREE_TYPE(type))
...
static void dbxout_type(tree type, int full) {
  tree tem;
  tree main_variant;
...
#ifdef DBX_USE_BINCL
    emit_pending_bincls_if_required();
    typevec[TYPE_SYMTAB_ADDRESS(type)].file_number = current_file->file_number;
  ...
  if (flag_debug_only_used_symbols) {
    if ((TREE_CODE(type) == RECORD_TYPE || TREE_CODE(type) == UNION_TYPE ||
        ...
        TYPE_STUB_DECL(type) && DECL_P(TYPE_STUB_DECL(type)) &&
    ...
    case QUAL_UNION_TYPE: {
      ...
      if (binfo) {
        ...
        for (i = 0; BINFO_BASE_ITERATE(binfo, i, child); i++) {
          tree access =
              (accesses ? VEC_index(tree, accesses, i) : access_public_node);

          if (use_gnu_debug_info_extensions) {
            stabstr_C(BINFO_VIRTUAL_P(child) ? '1' : '0');
            stabstr_C(access == access_public_node      ? '2'
                      : access == access_protected_node ? '1'
                                                        : '0');
            if (BINFO_VIRTUAL_P(child) &&
              ...
              stabstr_D(tree_low_cst(BINFO_VPTR_FIELD(child), 0) *
                        BITS_PER_UNIT);
            else
              stabstr_D(tree_low_cst(BINFO_OFFSET(child), 0) * BITS_PER_UNIT);
            stabstr_C(',');
            ...
            stabstr_D(tree_low_cst(BINFO_OFFSET(child), 0) * BITS_PER_UNIT);
            stabstr_C(',');
            stabstr_D(tree_low_cst(TYPE_SIZE(BINFO_TYPE(child)), 0) *
                      BITS_PER_UNIT);
            stabstr_C(';');
          }
        }
...
static bool print_int_cst_bounds_in_octal_p(tree type) {
  ...
  if (use_gnu_debug_info_extensions && TYPE_MIN_VALUE(type) != 0 &&
      ...
      (TYPE_PRECISION(type) > TYPE_PRECISION(integer_type_node) ||
       ...
       (TYPE_PRECISION(type) == HOST_BITS_PER_WIDE_INT && TYPE_UNSIGNED(type))))
    return TRUE;
  else
    return FALSE;
}
...
static void dbxout_type_name(tree type) {
...
static void dbxout_class_name_qualifiers(tree decl) {
  tree context = decl_type_context(decl);

  if (context != NULL_TREE && TREE_CODE(context) == RECORD_TYPE &&
      TYPE_NAME(context) != 0 &&
...
static rtx dbxout_expand_expr(tree expr) {
...
static int output_used_types_helper(void **slot, void *data) {
  tree type = (tree)*slot;
...
static int output_types_sort(const void *pa, const void *pb) {
...
static void output_used_types(void) {
  if (cfun && cfun->used_types_hash) {
    ...
    qsort(VEC_address(tree, types), VEC_length(tree, types), sizeof(tree),
          output_types_sort);
...
int dbxout_symbol(tree decl, int local ATTRIBUTE_UNUSED) {
  ...
  if (flag_debug_only_used_symbols) {
    ...
    if (TREE_CODE(t) == RECORD_TYPE || TREE_CODE(t) == UNION_TYPE ||
        TREE_CODE(t) == QUAL_UNION_TYPE || TREE_CODE(t) == ENUMERAL_TYPE) {
      if (TYPE_STUB_DECL(t) && TYPE_STUB_DECL(t) != decl &&
          DECL_P(TYPE_STUB_DECL(t)) && !DECL_IGNORED_P(TYPE_STUB_DECL(t))) {
        debug_queue_symbol(TYPE_STUB_DECL(t));
        if (TYPE_NAME(t) && TYPE_NAME(t) != TYPE_STUB_DECL(t) &&
            TYPE_NAME(t) != decl && DECL_P(TYPE_NAME(t)))
          debug_queue_symbol(TYPE_NAME(t));
      }
    } else if (TYPE_NAME(t) && TYPE_NAME(t) != decl && DECL_P(TYPE_NAME(t)))
      debug_queue_symbol(TYPE_NAME(t));
  }
  ...
  switch (TREE_CODE(decl)) {
    ...
    case FUNCTION_DECL:
      ...
      if (context && DECL_FROM_INLINE(decl))
        break;
      if (!MEM_P(decl_rtl) || GET_CODE(XEXP(decl_rtl, 0)) != SYMBOL_REF)
        break;
    ...
    case TYPE_DECL:
      ...
      {
        int fundamental_type_number = DBX_ASSIGN_FUNDAMENTAL_TYPE_NUMBER(decl);
      ...
      result = 1;
      {
        ...
        if (DECL_NAME(decl)) {
          ...
          if ((TREE_CODE(type) == RECORD_TYPE ||
               ...
               TREE_CODE(type) == QUAL_UNION_TYPE) &&
              TYPE_NAME(type) == decl && !use_gnu_debug_info_extensions &&
          ...
          stabstr_C('t');
          dbxout_type(type, 1);
          dbxout_finish_complex_stabs(decl, DBX_TYPE_DECL_STABS_CODE, 0, 0, 0);
          did_output = 1;
        }
    ...
    case VAR_DECL:
      ...
      if (TREE_STATIC(decl) && TREE_READONLY(decl) && DECL_INITIAL(decl) != 0 &&
          host_integerp(DECL_INITIAL(decl), 0) && !TREE_ASM_WRITTEN(decl) &&
...
static int dbxout_symbol_location(tree decl, tree type, const char *suffix,
                                  rtx home) {
  int letter = 0;
  STAB_CODE_TYPE code;
  rtx addr = 0;
  ...
  if (MEM_P(home) && GET_CODE(XEXP(home, 0)) == SYMBOL_REF) {
    if (TREE_PUBLIC(decl)) {
      ...
      if (DECL_INITIAL(decl) == 0 || (!strcmp(lang_hooks.name, "GNU C++") &&
  ...
  } else if (MEM_P(home) && REG_P(XEXP(home, 0))) {
    code = N_LSYM;
    number = DEBUGGER_AUTO_OFFSET(XEXP(home, 0));
  } else if (MEM_P(home) && GET_CODE(XEXP(home, 0)) == PLUS &&
             GET_CODE(XEXP(XEXP(home, 0), 1)) == CONST_INT) {
    code = N_LSYM;
    /* RTL looks like (MEM (PLUS (REG...) (CONST_INT...)))
       We want the value of that CONST_INT.  */
    number = DEBUGGER_AUTO_OFFSET(XEXP(home, 0));
  } else if (MEM_P(home) && GET_CODE(XEXP(home, 0)) == CONST) {
...
static void dbxout_symbol_name(tree decl, const char *suffix, int letter) {
  tree name;
...
static void dbxout_common_name(tree decl, const char *name, STAB_CODE_TYPE op) {
  dbxout_begin_complex_stabs();
  stabstr_S(name);
...
static const char *dbxout_common_check(tree decl, int *value) {
  rtx home;
  rtx sym_addr;
  ...
  if (TREE_CODE(decl) != VAR_DECL || !TREE_PUBLIC(decl) || !TREE_STATIC(decl) ||
      !DECL_HAS_VALUE_EXPR_P(decl) || DECL_THREAD_LOCAL_P(decl) ||
      !is_fortran())
    return NULL;
    ...
    switch (GET_CODE(sym_addr)) {
      case PLUS:
        if (GET_CODE(XEXP(sym_addr, 0)) == CONST_INT) {
          name = targetm.strip_name_encoding(XSTR(XEXP(sym_addr, 1), 0));
          *value = INTVAL(XEXP(sym_addr, 0));
          cdecl = SYMBOL_REF_DECL(XEXP(sym_addr, 1));
        } else {
          name = targetm.strip_name_encoding(XSTR(XEXP(sym_addr, 0), 0));
      ...
      default:
        error(
            "common symbol debug info is not structured as "
            "symbol+offset");
    }
...
int dbxout_syms(tree syms) {
  ...
  while (syms) {
    ...
    copen =
        comm_new != NULL && (comm_prev == NULL || strcmp(comm_new, comm_prev));
    cclos =
        comm_prev != NULL && (comm_new == NULL || strcmp(comm_new, comm_prev));
    if (cclos)
...
void dbxout_parms(tree parms) {
  ++debug_nesting;
  emit_pending_bincls_if_required();

  for (; parms; parms = TREE_CHAIN(parms))
    if (DECL_NAME(parms) && TREE_TYPE(parms) != error_mark_node &&
        DECL_RTL_SET_P(parms) && DECL_INCOMING_RTL(parms)) {
      tree eff_type;
      char letter;
      STAB_CODE_TYPE code;
      int number;
      ...
      if (PARM_PASSED_IN_MEMORY(parms)) {
        ...
        /* ??? Here we assume that the parm address is indexed
           ...
           but do not crash.  */
        if (GET_CODE(inrtl) == PLUS && GET_CODE(XEXP(inrtl, 1)) == CONST_INT)
      ...
      } else if (MEM_P(DECL_RTL(parms)) && REG_P(XEXP(DECL_RTL(parms), 0)) &&
        ...
        letter =
            (use_gnu_debug_info_extensions ? 'a' : DBX_REGPARM_STABS_LETTER);
        eff_type = TREE_TYPE(parms);
      ...
      } else if (MEM_P(DECL_RTL(parms)) && MEM_P(XEXP(DECL_RTL(parms), 0))) {
        ...
        number = DEBUGGER_ARG_OFFSET(number, XEXP(XEXP(DECL_RTL(parms), 0), 0));
...
void dbxout_reg_parms(tree parms) {
  ++debug_nesting;
  ...
  for (; parms; parms = TREE_CHAIN(parms))
    if (DECL_NAME(parms) && PARM_PASSED_IN_MEMORY(parms)) {
      ...
      if (REG_P(DECL_RTL(parms)) &&
        ...
        dbxout_symbol_location(parms, TREE_TYPE(parms), 0, DECL_RTL(parms));
...
static void dbxout_args(tree args) {
  while (args) {
    stabstr_C(',');
...
static void dbxout_block(tree block, int depth, tree args) {
  char begin_label[20];
  /* Reference current function start using LFBB.  */
  ASM_GENERATE_INTERNAL_LABEL(begin_label, "LFBB", scope_labelno);
...
static void dbxout_begin_function(tree decl) {
