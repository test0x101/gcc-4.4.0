...
static rtx FP_mode_reg[LAST_STACK_REG + 1 - FIRST_STACK_REG]
...
static rtx next_flags_user(rtx insn) {
...
static void straighten_stack(rtx insn, stack regstack) {
...
static void pop_stack(stack regstack, int regno) {
...
static rtx *get_true_reg(rtx *pat) {
  for (;;)
    switch (GET_CODE(*pat)) {
      case SUBREG:
          ...
          if (FP_REG_P(subreg = SUBREG_REG(*pat))) {
            int regno_off =
                subreg_regno_offset(REGNO(subreg), GET_MODE(subreg),
                                    SUBREG_BYTE(*pat), GET_MODE(*pat));
            *pat = FP_MODE_REG(REGNO(subreg) + regno_off, GET_MODE(subreg));
            return pat;
          }
        }
      case FLOAT:
...
static int check_asm_stack_operands(rtx insn) {
  int i;
  int n_clobbers;
  int malformed_asm = 0;
      ...
      if (reg_class_size[(int)recog_op_alt[i][alt].cl] != 1) {
        ...
        for (j = 0; j < n_clobbers; j++)
          if (REGNO(recog_data.operand[i]) == REGNO(clobber_reg[j])) {
            error_for_asm(insn,
                          "output constraint %d cannot be specified together "
                          "with \"%s\" clobber",
    ...
    if (recog_op_alt[i][alt].matches == -1) {
      ...
      for (j = 0; j < n_outputs; j++)
        if (operands_match_p(recog_data.operand[j], recog_data.operand[i])) {
          error_for_asm(insn, "output operand %d must use %<&%> constraint", j);
          malformed_asm = 1;
        }
    }
...
static int get_asm_operand_n_inputs(rtx body) {
...
static rtx stack_result(tree decl) {
  rtx result;
  ...
  if (result != 0)
    result =
        targetm.calls.function_value(TREE_TYPE(DECL_RESULT(decl)), decl, true);
...
static void replace_reg(rtx *reg, int regno) {
...
static void remove_regno_note(rtx insn, enum reg_note note,
                              unsigned int regno) {
  rtx *note_link, this_rtx;

  note_link = &REG_NOTES(insn);
  for (this_rtx = *note_link; this_rtx; this_rtx = XEXP(this_rtx, 1))
    if (REG_NOTE_KIND(this_rtx) == note && REG_P(XEXP(this_rtx, 0)) &&
        REGNO(XEXP(this_rtx, 0)) == regno) {
...
static int get_hard_regnum(stack regstack, rtx reg) {
  int i;
...
static rtx emit_pop_insn(rtx insn, stack regstack, rtx reg,
...
static void emit_swap_insn(rtx insn, stack regstack, rtx reg) {
  int hard_regno;
  rtx swap_rtx;
  ...
  if (current_block && insn != BB_HEAD(current_block)) {
    ...
    while (tmp != limit) {
      if (LABEL_P(tmp) || CALL_P(tmp) || NOTE_INSN_BASIC_BLOCK_P(tmp) ||
          (NONJUMP_INSN_P(tmp) && stack_regs_mentioned(tmp))) {
        i1 = tmp;
        break;
      }
      tmp = PREV_INSN(tmp);
  ...
  if (i1 != NULL_RTX && (i1set = single_set(i1)) != NULL_RTX) {
    ...
    if (REG_P(i1dest) && REGNO(i1dest) == FIRST_STACK_REG && REG_P(i1src) &&
...
static void swap_to_top(rtx insn, stack regstack, rtx src1, rtx src2) {
  struct stack_def temp_stack;
  int regno, j, k, temp;
...
static bool move_for_stack_reg(rtx insn, stack regstack, rtx pat) {
...
static bool move_nan_for_stack_reg(rtx insn, stack regstack, rtx dest) {
  rtx pat;
...
static int swap_rtx_condition_1(rtx pat) {
  const char *fmt;
  int i, r = 0;
...
static int swap_rtx_condition(rtx insn) {
  rtx pat = PATTERN(insn);
  ...
  if (GET_CODE(pat) == SET && GET_CODE(SET_SRC(pat)) == UNSPEC &&
    ...
    pat = PATTERN(insn);
    if (GET_CODE(pat) != SET || GET_CODE(SET_SRC(pat)) != UNSPEC ||
        XINT(SET_SRC(pat), 1) != UNSPEC_SAHF || !dead_or_set_p(insn, dest))
      return 0;
...
static void compare_for_stack_reg(rtx insn, stack regstack, rtx pat_src) {
  rtx *src1, *src2;
  ...
  if (src2_note && !(STACK_REG_P(*src1) && STACK_REG_P(*src2) &&
...
static bool subst_stack_regs_pat(rtx insn, stack regstack, rtx pat) {
  rtx *dest, *src;
  bool control_flow_insn_deleted = false;
  ...
  switch (GET_CODE(pat)) {
    case USE:
      ...
      if (STACK_REG_P(*src) && find_regno_note(insn, REG_DEAD, REGNO(*src))) {
    ...
    case SET: {
      ...
      if (STACK_REG_P(*src) ||
          (STACK_REG_P(*dest) &&
           (REG_P(*src) || MEM_P(*src) || GET_CODE(*src) == CONST_DOUBLE))) {
      ...
      switch (GET_CODE(pat_src)) {
        ...
        case PLUS:
          ...
          if (COMMUTATIVE_ARITH_P(pat_src) && REG_P(*src1) && REG_P(*src2) &&
          ...
          switch (XINT(pat_src, 1)) {
            ...
            case UNSPEC_FXAM:
              ...
              if (src1_note) {
                ...
                emit_pop_insn(insn, regstack, XEXP(src1_note, 0), EMIT_AFTER);
              }
            ...
            case UNSPEC_TAN:
              ...
              regstack->reg[regstack->top] = regstack->reg[regstack->top - 1];
        ...
        case IF_THEN_ELSE:
          ...
          if (get_hard_regnum(regstack, *dest) >= FIRST_STACK_REG &&
            ...
            if ((REGNO(*src1) == regstack->reg[regstack->top] && src1_note) ||
                (REGNO(*src2) == regstack->reg[regstack->top] && src2_note)) {
              int idx1 = (get_hard_regnum(regstack, *src1) - FIRST_STACK_REG);
              int idx2 = (get_hard_regnum(regstack, *src2) - FIRST_STACK_REG);
...

          {
            ...
            for (i = 1; i <= 2; i++)
              if (src_note[i]) {
                ...
                emit_pop_insn(insn, regstack, XEXP(src_note[i], 0), EMIT_AFTER);
              }
          }
...
static void subst_asm_stack_regs(rtx insn, stack regstack) {
  rtx body = PATTERN(insn);
  int alt;
  ...
  for (note = REG_NOTES(insn); note; note = XEXP(note, 1)) {
    ...
    if (STACK_REG_P(reg) && (REG_NOTE_KIND(note) == REG_DEAD ||
  ...
  for (i = n_outputs; i < n_outputs + n_inputs; i++)
    if (STACK_REG_P(recog_data.operand[i]) &&
        reg_class_subset_p(recog_op_alt[i][alt].cl, FLOAT_REGS) &&
      ...
      if ((unsigned int)regno != REGNO(recog_data.operand[i])) {
        ...
        k = temp_stack.top - (regno - FIRST_STACK_REG);
        j = (temp_stack.top - (REGNO(recog_data.operand[i]) - FIRST_STACK_REG));
  ...
  for (i = n_outputs; i < n_outputs + n_inputs; i++)
    if (STACK_REG_P(recog_data.operand[i])) {
      ...
      if (j < n_clobbers || recog_op_alt[i][alt].matches >= 0) {
        ...
        CLEAR_HARD_REG_BIT(regstack->reg_set, regstack->reg[regstack->top]);
        regstack->top--;
      }
    }
    ...
    if (STACK_REG_P(recog_data.operand[i])) {
      ...
      for (j = 0; j < n_notes; j++)
        if (REGNO(recog_data.operand[i]) == REGNO(note_reg[j]) &&
          ...
          insn =
              emit_pop_insn(insn, regstack, recog_data.operand[i], EMIT_AFTER);
          break;
        }
    }
...
static bool subst_stack_regs(rtx insn, stack regstack) {
  rtx *note_link, note;
  bool control_flow_insn_deleted = false;
  int i;
    ...
    if (GET_CODE(PATTERN(insn)) == PARALLEL)
      for (i = 0; i < XVECLEN(PATTERN(insn), 0); i++) {
        if (stack_regs_mentioned_p(XVECEXP(PATTERN(insn), 0, i))) {
          ...
          control_flow_insn_deleted |= subst_stack_regs_pat(
              insn, regstack, XVECEXP(PATTERN(insn), 0, i));
        }
      }
    else
      control_flow_insn_deleted |=
...
static void change_stack(rtx insn, stack old, stack new_stack,
                         enum emit_where where) {
  int reg;
  int update_end = 0;
  int i;
  ...
  if (current_block && starting_stack_p && where == EMIT_BEFORE) {
  ...
  for (i = FIRST_STACK_REG; i < LAST_STACK_REG + 1; i++)
    if (TEST_HARD_REG_BIT(new_stack->reg_set, i) &&
      ...
      emit_insn_before(gen_rtx_SET(VOIDmode, FP_MODE_REG(i, SFmode), not_a_num),
                       insn);
    }
    ...
    if (new_stack->top != -1)
      do {
        ...
        while (old->reg[old->top] != new_stack->reg[new_stack->top]) {
          ...
          emit_swap_insn(insn, old, FP_MODE_REG(old->reg[reg], DFmode));
        }
...
static void print_stack(FILE *file, stack s) {
  if (!file)
    return;
  ...
  FOR_EACH_EDGE(e, ei, ENTRY_BLOCK_PTR->succs) {
    ...
    for (reg = LAST_STACK_REG; reg >= FIRST_STACK_REG; --reg)
      if (TEST_HARD_REG_BIT(bi->stack_in.reg_set, reg)) {
        ...
        init = gen_rtx_SET(VOIDmode, FP_MODE_REG(FIRST_STACK_REG, SFmode),
                           not_a_num);
...
static void propagate_stack(edge e) {
...
static bool compensate_edge(edge e) {
...
static bool compensate_edges(void) {
  bool inserted = false;
  basic_block bb;
...
static edge better_edge(edge e1, edge e2) {
  if (!e1)
    return e2;
...
static void convert_regs_1(basic_block block) {
  struct stack_def regstack;
  block_info bi = BLOCK_INFO(block);
  int reg;
  rtx insn, next;
  ...
  do {
    ...
    if (stack_regs_mentioned(insn) || CALL_P(insn)) {
      if (dump_file) {
        fprintf(dump_file, "  insn %d input stack: ", INSN_UID(insn));
...
static void convert_regs_2(basic_block block) {
...
static bool reg_to_stack(void) {
  ...
  if ((flag_pic && !TARGET_64BIT) || ix86_cmodel == CM_LARGE ||
  ...
  memset(VEC_address(char, stack_regs_mentioned_data), 0,
         sizeof(char) * (max_uid + 1));
...
static bool gate_handle_stack_regs(void) {
#ifdef STACK_REGS
  return 1;
#else
  return 0;
