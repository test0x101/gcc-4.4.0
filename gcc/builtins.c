...
static rtx expand_builtin_memset_args(tree, tree, tree, rtx, enum machine_mode,
                                      tree);
...
int get_object_alignment(tree exp, unsigned int align, unsigned int max_align) {
  unsigned int inner;
...
int get_pointer_alignment(tree exp, unsigned int max_align) {
...
static rtx c_readstr(const char *str, enum machine_mode mode) {
  HOST_WIDE_INT c[2];
  HOST_WIDE_INT ch;
  unsigned int i, j;
...
static int target_char_cast(tree cst, char *p) {
  ...
  if (!host_integerp(cst, 1) || CHAR_TYPE_SIZE > HOST_BITS_PER_WIDE_INT)
    return 1;
...
void expand_builtin_setjmp_setup(rtx buf_addr, rtx receiver_label) {
  ...
  stack_save =
      gen_rtx_MEM(sa_mode, plus_constant(buf_addr, 2 * GET_MODE_SIZE(Pmode)));
...
static void expand_builtin_update_setjmp_buf(rtx buf_addr) {
  ...
  stack_save = gen_rtx_MEM(
      sa_mode, memory_address(
                   sa_mode, plus_constant(buf_addr, 2 * GET_MODE_SIZE(Pmode))));
...
static void expand_builtin_prefetch(tree exp) {
  ...
  if (INTVAL(op1) != 0 && INTVAL(op1) != 1) {
    warning(0,
            "invalid second argument to %<__builtin_prefetch%>;"
            " using zero");
    op1 = const0_rtx;
  }
  ...
  if (HAVE_prefetch) {
    if ((!(*insn_data[(int)CODE_FOR_prefetch].operand[0].predicate)(
            op0, insn_data[(int)CODE_FOR_prefetch].operand[0].mode)) ||
        (GET_MODE(op0) != Pmode)) {
...
static rtx get_memory_rtx(tree exp, tree len) {
  tree orig_exp = exp;
  rtx addr, mem;
  HOST_WIDE_INT off;
  ...
  if (exp) {
    ...
    if (MEM_EXPR(mem) && TREE_CODE(MEM_EXPR(mem)) == COMPONENT_REF) {
      ...
      while (TREE_CODE(inner) == ARRAY_REF || CONVERT_EXPR_P(inner) ||
      ...
      while (TREE_CODE(inner) == COMPONENT_REF) {
        ...
        gcc_assert(!DECL_BIT_FIELD(field) ||
                   ((tree_low_cst(DECL_FIELD_BIT_OFFSET(field), 1) %
                    ...
                    (TREE_INT_CST_LOW(DECL_SIZE(field)) % BITS_PER_UNIT) == 0));
        ...
        if (length >= 0 && DECL_SIZE_UNIT(field) &&
            host_integerp(DECL_SIZE_UNIT(field), 0)) {
          HOST_WIDE_INT size = TREE_INT_CST_LOW(DECL_SIZE_UNIT(field));
          if (offset <= size && length <= size && offset + length <= size)
            break;
        }

        if (offset >= 0 && host_integerp(DECL_FIELD_OFFSET(field), 0))
          offset +=
              TREE_INT_CST_LOW(DECL_FIELD_OFFSET(field)) +
              tree_low_cst(DECL_FIELD_BIT_OFFSET(field), 1) / BITS_PER_UNIT;
        else {
          offset = -1;
          length = -1;
        }
...
static rtx result_vector(int savep, rtx result) {
  ...
  for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
    if ((mode = apply_result_mode[regno]) != VOIDmode) {
      ...
      savevec[nelts++] = (savep ? gen_rtx_SET(VOIDmode, mem, reg)
...
static rtx expand_builtin_apply_args_1(void) {
  ...
  rtx struct_incoming_value =
      targetm.calls.struct_value_rtx(cfun ? TREE_TYPE(cfun->decl) : 0, 1);
  ...
  tem =
      force_operand(plus_constant(tem, crtl->args.pretend_args_size), NULL_RTX);
#endif
...
static rtx expand_builtin_apply(rtx function, rtx arguments, rtx argsize) {
  ...
  rtx call_fusage = 0;
  rtx struct_value =
      targetm.calls.struct_value_rtx(cfun ? TREE_TYPE(cfun->decl) : 0, 0);
  ...
  if (HAVE_untyped_call)
    ...
    emit_call_insn(GEN_CALL_VALUE(valreg, gen_rtx_MEM(FUNCTION_MODE, function),
...
static enum type_class type_to_class(tree type) {
  switch (TREE_CODE(type)) {
    ...
    case ARRAY_TYPE:
      return (TYPE_STRING_FLAG(type) ? string_type_class : array_type_class);
    case LANG_TYPE:
...
#define CASE_MATHFN(BUILT_IN_MATHFN) \
  case BUILT_IN_MATHFN:              \
  case BUILT_IN_MATHFN##F:           \
  case BUILT_IN_MATHFN##L:           \
    fcode = BUILT_IN_MATHFN;         \
    fcodef = BUILT_IN_MATHFN##F;     \
    fcodel = BUILT_IN_MATHFN##L;     \
    break;
/* Similar to above, but appends _R after any F/L suffix.  */
#define CASE_MATHFN_REENT(BUILT_IN_MATHFN) \
  case BUILT_IN_MATHFN##_R:                \
  case BUILT_IN_MATHFN##F_R:               \
  case BUILT_IN_MATHFN##L_R:               \
    fcode = BUILT_IN_MATHFN##_R;           \
    fcodef = BUILT_IN_MATHFN##F_R;         \
    fcodel = BUILT_IN_MATHFN##L_R;         \
    break;
...
static tree mathfn_built_in_1(tree type, enum built_in_function fn,
                              bool implicit) {
...
static void expand_errno_check(tree exp, rtx target) {
  rtx lab = gen_label_rtx();

  /* Test the result; if it is NaN, set errno=EDOM because
     the argument was not in the domain.  */
  emit_cmp_and_jump_insns(target, target, EQ, 0, GET_MODE(target), 0, lab);
...
static rtx expand_builtin_mathfn(tree exp, rtx target, rtx subtarget) {
  ...
  switch (DECL_FUNCTION_CODE(fndecl)) {
    CASE_FLT_FN(BUILT_IN_SQRT) : errno_set = !tree_expr_nonnegative_p(arg);
    builtin_optab = sqrt_optab;
    break;
    CASE_FLT_FN(BUILT_IN_EXP) : errno_set = true;
    builtin_optab = exp_optab;
    break;
    CASE_FLT_FN(BUILT_IN_EXP10)
        : CASE_FLT_FN(BUILT_IN_POW10) : errno_set = true;
    builtin_optab = exp10_optab;
    break;
    CASE_FLT_FN(BUILT_IN_EXP2) : errno_set = true;
    builtin_optab = exp2_optab;
    break;
    CASE_FLT_FN(BUILT_IN_EXPM1) : errno_set = true;
    builtin_optab = expm1_optab;
    break;
    CASE_FLT_FN(BUILT_IN_LOGB) : errno_set = true;
    builtin_optab = logb_optab;
    break;
    CASE_FLT_FN(BUILT_IN_LOG) : errno_set = true;
    builtin_optab = log_optab;
    break;
    CASE_FLT_FN(BUILT_IN_LOG10) : errno_set = true;
    builtin_optab = log10_optab;
    break;
    CASE_FLT_FN(BUILT_IN_LOG2) : errno_set = true;
    builtin_optab = log2_optab;
    break;
    CASE_FLT_FN(BUILT_IN_LOG1P) : errno_set = true;
    builtin_optab = log1p_optab;
    break;
    CASE_FLT_FN(BUILT_IN_ASIN) : builtin_optab = asin_optab;
    break;
    CASE_FLT_FN(BUILT_IN_ACOS) : builtin_optab = acos_optab;
    break;
    CASE_FLT_FN(BUILT_IN_TAN) : builtin_optab = tan_optab;
    break;
    CASE_FLT_FN(BUILT_IN_ATAN) : builtin_optab = atan_optab;
    break;
    CASE_FLT_FN(BUILT_IN_FLOOR) : builtin_optab = floor_optab;
    break;
    CASE_FLT_FN(BUILT_IN_CEIL) : builtin_optab = ceil_optab;
    break;
    CASE_FLT_FN(BUILT_IN_TRUNC) : builtin_optab = btrunc_optab;
    break;
    CASE_FLT_FN(BUILT_IN_ROUND) : builtin_optab = round_optab;
    break;
    CASE_FLT_FN(BUILT_IN_NEARBYINT) : builtin_optab = nearbyint_optab;
    if (flag_trapping_math)
      break;
    /* Else fallthrough and expand as rint.  */
    CASE_FLT_FN(BUILT_IN_RINT) : builtin_optab = rint_optab;
    break;
    default:
      gcc_unreachable();
  }
...
static rtx expand_builtin_mathfn_2(tree exp, rtx target, rtx subtarget) {
  optab builtin_optab;
  rtx op0, op1, insns;
  ...
  switch (DECL_FUNCTION_CODE(fndecl)) {
    CASE_FLT_FN(BUILT_IN_SCALBN)
        ...
        : CASE_FLT_FN(BUILT_IN_LDEXP) : op1_type = INTEGER_TYPE;
    default:
      break;
  }
  ...
  switch (DECL_FUNCTION_CODE(fndecl)) {
    CASE_FLT_FN(BUILT_IN_POW) : builtin_optab = pow_optab;
    break;
    CASE_FLT_FN(BUILT_IN_ATAN2) : builtin_optab = atan2_optab;
    break;
    CASE_FLT_FN(BUILT_IN_SCALB)
        : if (REAL_MODE_FORMAT(TYPE_MODE(TREE_TYPE(exp)))->b != 2) return 0;
    ...
    CASE_FLT_FN(BUILT_IN_SCALBN)
        ...
        : if (REAL_MODE_FORMAT(TYPE_MODE(TREE_TYPE(exp)))->b != 2) return 0;
    /* Fall through... */
    CASE_FLT_FN(BUILT_IN_LDEXP) : builtin_optab = ldexp_optab;
    break;
    CASE_FLT_FN(BUILT_IN_FMOD) : builtin_optab = fmod_optab;
    break;
    CASE_FLT_FN(BUILT_IN_REMAINDER)
        : CASE_FLT_FN(BUILT_IN_DREM) : builtin_optab = remainder_optab;
    break;
    default:
      gcc_unreachable();
  }
  ...
  /* Compute into TARGET.
     Set TARGET to wherever the result comes back.  */
  target = expand_binop(mode, builtin_optab, op0, op1, target, 0, OPTAB_DIRECT);
...
static rtx expand_builtin_mathfn_3(tree exp, rtx target, rtx subtarget) {
  optab builtin_optab;
  rtx op0, insns;
  tree fndecl = get_callee_fndecl(exp);
  enum machine_mode mode;
  ...
  switch (DECL_FUNCTION_CODE(fndecl)) {
    CASE_FLT_FN(BUILT_IN_SIN)
        : CASE_FLT_FN(BUILT_IN_COS) : builtin_optab = sincos_optab;
    break;
    default:
      gcc_unreachable();
  }
  ...
  if (optab_handler(builtin_optab, mode)->insn_code == CODE_FOR_nothing)
    switch (DECL_FUNCTION_CODE(fndecl)) {
      CASE_FLT_FN(BUILT_IN_SIN) : builtin_optab = sin_optab;
      break;
      CASE_FLT_FN(BUILT_IN_COS) : builtin_optab = cos_optab;
      break;
      default:
        gcc_unreachable();
    }
...
static rtx expand_builtin_interclass_mathfn(tree exp, rtx target,
                                            rtx subtarget) {
  ...
  switch (DECL_FUNCTION_CODE(fndecl)) {
    CASE_FLT_FN(BUILT_IN_ILOGB) : errno_set = true;
    builtin_optab = ilogb_optab;
    break;
    CASE_FLT_FN(BUILT_IN_ISINF) : builtin_optab = isinf_optab;
    break;
  ...
  if (icode != CODE_FOR_nothing) {
    ...
    gcc_assert(insn_data[icode].operand[0].predicate(target, GET_MODE(target)));
  ...
  switch (DECL_FUNCTION_CODE(fndecl)) {
    ...
    CASE_FLT_FN(BUILT_IN_ISINF) : {
      ...
      result = build_call_expr(isgr_fn, 2, fold_build1(ABS_EXPR, type, arg),
                               build_real(type, r));
      return expand_expr(result, target, VOIDmode, EXPAND_NORMAL);
    }
  CASE_FLT_FN(BUILT_IN_FINITE) : case BUILT_IN_ISFINITE : {
    ...
    get_max_float(REAL_MODE_FORMAT(mode), buf, sizeof(buf));
    real_from_string(&r, buf);
    result = build_call_expr(isle_fn, 2, fold_build1(ABS_EXPR, type, arg),
                             build_real(type, r));
    return expand_expr(result, target, VOIDmode, EXPAND_NORMAL);
  }
  case BUILT_IN_ISNORMAL: {
    ...
    result = build_call_expr(isle_fn, 2, arg, build_real(type, rmax));
    result =
        fold_build2(BIT_AND_EXPR, integer_type_node, result,
                    build_call_expr(isge_fn, 2, arg, build_real(type, rmin)));
    return expand_expr(result, target, VOIDmode, EXPAND_NORMAL);
  }
  default:
    break;
...
static rtx expand_builtin_cexpi(tree exp, rtx target, rtx subtarget) {
  tree fndecl = get_callee_fndecl(exp);
  tree arg, type;
  enum machine_mode mode;
  rtx op0, op1, op2;
  ...
  if (optab_handler(sincos_optab, mode)->insn_code != CODE_FOR_nothing) {
    ...
    expand_normal(
        build_call_nary(TREE_TYPE(TREE_TYPE(fn)), call, 3, arg, top1, top2));
  } else {
    ...
    narg = fold_build2(COMPLEX_EXPR, ctype, build_real(type, dconst0), arg);

    /* Make sure not to fold the cexp call again.  */
    call = build1(ADDR_EXPR, build_pointer_type(TREE_TYPE(fn)), fn);
    return expand_expr(build_call_nary(ctype, call, 1, narg), target, VOIDmode,
                       EXPAND_NORMAL);
  }

  /* Now build the proper return type.  */
  return expand_expr(
      build2(COMPLEX_EXPR, build_complex_type(type),
             make_tree(TREE_TYPE(arg), op2), make_tree(TREE_TYPE(arg), op1)),
...
static rtx expand_builtin_int_roundingfn(tree exp, rtx target) {
  ...
  switch (DECL_FUNCTION_CODE(fndecl)) {
    CASE_FLT_FN(BUILT_IN_LCEIL)
        : CASE_FLT_FN(BUILT_IN_LLCEIL) : builtin_optab = lceil_optab;
    fallback_fn = BUILT_IN_CEIL;
    break;

    CASE_FLT_FN(BUILT_IN_LFLOOR)
        : CASE_FLT_FN(BUILT_IN_LLFLOOR) : builtin_optab = lfloor_optab;
  ...
  if (fallback_fndecl == NULL_TREE) {
    ...
    fntype =
        build_function_type_list(TREE_TYPE(arg), TREE_TYPE(arg), NULL_TREE);
...
static rtx expand_builtin_int_roundingfn_2(tree exp, rtx target) {
  convert_optab builtin_optab;
  rtx op0, insns;
  tree fndecl = get_callee_fndecl(exp);
  tree arg;
  ...
  switch (DECL_FUNCTION_CODE(fndecl)) {
    CASE_FLT_FN(BUILT_IN_LRINT)
        : CASE_FLT_FN(BUILT_IN_LLRINT) : builtin_optab = lrint_optab;
    break;
    CASE_FLT_FN(BUILT_IN_LROUND)
        : CASE_FLT_FN(BUILT_IN_LLROUND) : builtin_optab = lround_optab;
    break;
    default:
      gcc_unreachable();
  }
...
static int powi_cost(HOST_WIDE_INT n) {
  ...
  while (val >= POWI_TABLE_SIZE) {
    if (val & 1) {
      ...
      result += powi_lookup_cost(digit, cache) + POWI_WINDOW_SIZE + 1;
...
static rtx expand_powi_1(enum machine_mode mode, unsigned HOST_WIDE_INT n,
                         rtx *cache) {
  unsigned HOST_WIDE_INT digit;
  rtx target, result;
  rtx op0, op1;
...
static rtx expand_powi(rtx x, enum machine_mode mode, HOST_WIDE_INT n) {
  ...
  if (n < 0)
    result = expand_binop(mode, sdiv_optab, CONST1_RTX(mode), result, NULL_RTX,
                          0, OPTAB_LIB_WIDEN);
...
static rtx expand_builtin_pow(tree exp, rtx target, rtx subtarget) {
  ...
  if (TREE_CODE(arg1) != REAL_CST || TREE_OVERFLOW(arg1))
  ...
  if (real_identical(&c, &cint) &&
      ((n >= -1 && n <= 2) ||
       (flag_unsafe_math_optimizations && optimize_insn_for_speed_p() &&
  ...
  if (fn != NULL_TREE) {
    ...
    if (real_identical(&c2, &cint) &&
      ...
      if (n != 1) {
        ...
        op = expand_simple_binop(mode, MULT, op, op2, NULL_RTX, 0,
                                 OPTAB_LIB_WIDEN);
        /* If the original exponent was negative, reciprocate the
           result.  */
        if (n < 0)
          op = expand_binop(mode, sdiv_optab, CONST1_RTX(mode), op, NULL_RTX, 0,
                            OPTAB_LIB_WIDEN);
      }
      return op;
    }
  ...
  if (fn != NULL_TREE && flag_unsafe_math_optimizations &&
    ...
    if (real_identical(&c2, &c) &&
        ((optimize_insn_for_speed_p() && powi_cost(n / 3) <= POWI_MAX_MULTS) ||
         n == 1)) {
      tree call_expr = build_call_expr(fn, 1, narg0);
      op = expand_builtin(call_expr, NULL_RTX, subtarget, mode, 0);
      if (abs(n) % 3 == 2)
        op = expand_simple_binop(mode, MULT, op, op, op, 0, OPTAB_LIB_WIDEN);
      if (n != 1) {
        op2 = expand_expr(narg0, subtarget, VOIDmode, EXPAND_NORMAL);
        op2 = force_reg(mode, op2);
...
static rtx expand_builtin_powi(tree exp, rtx target, rtx subtarget) {
  ...
  if (TREE_CODE(arg1) == INTEGER_CST && !TREE_OVERFLOW(arg1)) {
    HOST_WIDE_INT n = TREE_INT_CST_LOW(arg1);

    /* If the exponent is -1, 0, 1 or 2, then expand_powi is exact.
       Otherwise, check the number of multiplications required.  */
    if ((TREE_INT_CST_HIGH(arg1) == 0 || TREE_INT_CST_HIGH(arg1) == -1) &&
        ((n >= -1 && n <= 2) ||
         (optimize_insn_for_speed_p() && powi_cost(n) <= POWI_MAX_MULTS))) {
      op0 = expand_expr(arg0, subtarget, VOIDmode, EXPAND_NORMAL);
  ...
  target = emit_library_call_value(optab_libfunc(powi_optab, mode), target,
                                   LCT_CONST, mode, 2, op0, mode, op1, mode2);
...
static rtx expand_builtin_strlen(tree exp, rtx target,
  ...
  if (!validate_arglist(exp, POINTER_TYPE, VOID_TYPE))
    ...
    result = target;
    if (!(result != 0 && REG_P(result) && GET_MODE(result) == insn_mode &&
    ...
    pat = GEN_FCN(icode)(result, gen_rtx_MEM(BLKmode, src_reg), char_rtx,
                         GEN_INT(align));
    if (!pat)
      return NULL_RTX;
    emit_insn(pat);
...
static rtx expand_builtin_strstr(tree exp, rtx target, enum machine_mode mode) {
  if (validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, VOID_TYPE)) {
    tree type = TREE_TYPE(exp);
    tree result =
        fold_builtin_strstr(CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1), type);
    if (result)
...
static rtx expand_builtin_strchr(tree exp, rtx target, enum machine_mode mode) {
  if (validate_arglist(exp, POINTER_TYPE, INTEGER_TYPE, VOID_TYPE)) {
    tree type = TREE_TYPE(exp);
    tree result =
        fold_builtin_strchr(CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1), type);
    if (result)
      return expand_expr(result, target, mode, EXPAND_NORMAL);
...
static rtx expand_builtin_strrchr(tree exp, rtx target,
                                  enum machine_mode mode) {
  if (validate_arglist(exp, POINTER_TYPE, INTEGER_TYPE, VOID_TYPE)) {
    tree type = TREE_TYPE(exp);
...
static rtx expand_builtin_strpbrk(tree exp, rtx target,
                                  enum machine_mode mode) {
  if (validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, VOID_TYPE)) {
    tree type = TREE_TYPE(exp);
...
static rtx builtin_memcpy_read_str(void *data, HOST_WIDE_INT offset,
                                   enum machine_mode mode) {
...
static rtx expand_builtin_memcpy(tree exp, rtx target, enum machine_mode mode) {
  tree fndecl = get_callee_fndecl(exp);

  if (!validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, INTEGER_TYPE,
                        VOID_TYPE))
    return NULL_RTX;
  else {
    ...
    unsigned int dest_align = get_pointer_alignment(dest, BIGGEST_ALIGNMENT);
    rtx dest_mem, src_mem, dest_addr, len_rtx;
    tree result = fold_builtin_memory_op(
        dest, src, len, TREE_TYPE(TREE_TYPE(fndecl)), false, /*endp=*/0);
    ...
    if (src_str && GET_CODE(len_rtx) == CONST_INT &&
        (unsigned HOST_WIDE_INT)INTVAL(len_rtx) <= strlen(src_str) + 1 &&
        can_store_by_pieces(INTVAL(len_rtx), builtin_memcpy_read_str,
                            CONST_CAST(char *, src_str), dest_align, false)) {
      dest_mem =
          store_by_pieces(dest_mem, INTVAL(len_rtx), builtin_memcpy_read_str,
                          CONST_CAST(char *, src_str), dest_align, false, 0);
    ...
    dest_addr = emit_block_move_hints(
        dest_mem, src_mem, len_rtx,
        CALL_EXPR_TAILCALL(exp) ? BLOCK_OP_TAILCALL : BLOCK_OP_NORMAL,
...
static rtx expand_builtin_mempcpy(tree exp, rtx target,
                                  enum machine_mode mode) {
  if (!validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, INTEGER_TYPE,
                        VOID_TYPE))
    return NULL_RTX;
    ...
    tree src = CALL_EXPR_ARG(exp, 1);
    tree len = CALL_EXPR_ARG(exp, 2);
    return expand_builtin_mempcpy_args(dest, src, len, TREE_TYPE(exp), target,
                                       mode, /*endp=*/1);
  }
}
...
static rtx expand_builtin_mempcpy_args(tree dest, tree src, tree len, tree type,
  ...
  if (target == const0_rtx && implicit_built_in_decls[BUILT_IN_MEMCPY]) {
    ...
    while (TREE_CODE(result) == COMPOUND_EXPR) {
      expand_expr(TREE_OPERAND(result, 0), const0_rtx, VOIDmode, EXPAND_NORMAL);
      result = TREE_OPERAND(result, 1);
    }
    return expand_expr(result, target, mode, EXPAND_NORMAL);
  } else {
    ...
    if (src_str && GET_CODE(len_rtx) == CONST_INT &&
      ...
      dest_mem =
          store_by_pieces(dest_mem, INTVAL(len_rtx), builtin_memcpy_read_str,
                          CONST_CAST(char *, src_str), dest_align, false, endp);
      dest_mem = force_operand(XEXP(dest_mem, 0), NULL_RTX);
      dest_mem = convert_memory_address(ptr_mode, dest_mem);
      return dest_mem;
    }
    ...
    if (GET_CODE(len_rtx) == CONST_INT &&
        can_move_by_pieces(INTVAL(len_rtx), MIN(dest_align, src_align))) {
      dest_mem = get_memory_rtx(dest, len);
      set_mem_align(dest_mem, dest_align);
      src_mem = get_memory_rtx(src, len);
      set_mem_align(src_mem, src_align);
...
static rtx expand_builtin_memmove(tree exp, rtx target, enum machine_mode mode,
                                  int ignore) {
  if (!validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, INTEGER_TYPE,
                        VOID_TYPE))
    return NULL_RTX;
    ...
    tree src = CALL_EXPR_ARG(exp, 1);
    tree len = CALL_EXPR_ARG(exp, 2);
    return expand_builtin_memmove_args(dest, src, len, TREE_TYPE(exp), target,
                                       mode, ignore);
  }
}
...
static rtx expand_builtin_memmove_args(tree dest, tree src, tree len, tree type,
                                       rtx target, enum machine_mode mode,
                                       int ignore) {
  tree result =
      fold_builtin_memory_op(dest, src, len, type, ignore, /*endp=*/3);
...
static rtx expand_builtin_bcopy(tree exp, int ignore) {
  tree type = TREE_TYPE(exp);
  tree src, dest, size;
  ...
  return expand_builtin_memmove_args(dest, src, fold_convert(sizetype, size),
                                     type, const0_rtx, VOIDmode, ignore);
}
...
static rtx expand_builtin_strcpy(tree fndecl, tree exp, rtx target,
                                 enum machine_mode mode) {
  if (validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, VOID_TYPE)) {
    tree dest = CALL_EXPR_ARG(exp, 0);
    tree src = CALL_EXPR_ARG(exp, 1);
...
static rtx expand_builtin_strcpy_args(tree fndecl, tree dest, tree src,
                                      rtx target, enum machine_mode mode) {
...
static rtx expand_builtin_stpcpy(tree exp, rtx target, enum machine_mode mode) {
  tree dst, src;
  ...
  if (target == const0_rtx && implicit_built_in_decls[BUILT_IN_STRCPY]) {
    ...
    ret = expand_builtin_mempcpy_args(dst, src, lenp1, TREE_TYPE(exp), target,
                                      mode, /*endp=*/2);
    ...
    if (TREE_CODE(len) == INTEGER_CST) {
      ...
      if (GET_CODE(len_rtx) == CONST_INT) {
        ret = expand_builtin_strcpy_args(get_callee_fndecl(exp), dst, src,
                                         target, mode);
...
rtx builtin_strncpy_read_str(void *data, HOST_WIDE_INT offset,
                             enum machine_mode mode) {
  const char *str = (const char *)data;
...
static rtx expand_builtin_strncpy(tree exp, rtx target,
                                  enum machine_mode mode) {
  tree fndecl = get_callee_fndecl(exp);
  ...
  if (validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, INTEGER_TYPE,
    ...
    if (tree_int_cst_lt(slen, len)) {
      ...
      if (!p || dest_align == 0 || !host_integerp(len, 1) ||
          !can_store_by_pieces(tree_low_cst(len, 1), builtin_strncpy_read_str,
                               CONST_CAST(char *, p), dest_align, false))
        return NULL_RTX;

      dest_mem = get_memory_rtx(dest, len);
      store_by_pieces(dest_mem, tree_low_cst(len, 1), builtin_strncpy_read_str,
...
rtx builtin_memset_read_str(void *data, HOST_WIDE_INT offset ATTRIBUTE_UNUSED,
                            enum machine_mode mode) {
...
static rtx expand_builtin_memset(tree exp, rtx target, enum machine_mode mode) {
  if (!validate_arglist(exp, POINTER_TYPE, INTEGER_TYPE, INTEGER_TYPE,
                        VOID_TYPE))
    return NULL_RTX;
  else {
    tree dest = CALL_EXPR_ARG(exp, 0);
...
static rtx expand_builtin_memset_args(tree dest, tree val, tree len, rtx target,
  ...
  if (TREE_CODE(val) != INTEGER_CST) {
    ...
    val_rtx = convert_to_mode(TYPE_MODE(unsigned_char_type_node), val_rtx, 0);
    ...
    c = 1;
    if (host_integerp(len, 1) &&
        can_store_by_pieces(tree_low_cst(len, 1), builtin_memset_read_str, &c,
                            dest_align, true)) {
      val_rtx = force_reg(TYPE_MODE(unsigned_char_type_node), val_rtx);
      store_by_pieces(dest_mem, tree_low_cst(len, 1), builtin_memset_gen_str,
                      val_rtx, dest_align, true, 0);
    } else if (!set_storage_via_setmem(dest_mem, len_rtx, val_rtx, dest_align,
                                       expected_align, expected_size))
      goto do_libcall;
  ...
  if (c) {
    if (host_integerp(len, 1) &&
        can_store_by_pieces(tree_low_cst(len, 1), builtin_memset_read_str, &c,
                            dest_align, true))
      store_by_pieces(dest_mem, tree_low_cst(len, 1), builtin_memset_read_str,
                      &c, dest_align, true, 0);
    else if (!set_storage_via_setmem(dest_mem, len_rtx, GEN_INT(c), dest_align,
                                     expected_align, expected_size))
      goto do_libcall;
...
static rtx expand_builtin_bzero(tree exp) {
  ...
  return expand_builtin_memset_args(dest, integer_zero_node,
                                    fold_convert(sizetype, size), const0_rtx,
                                    VOIDmode, exp);
}
...
static rtx expand_builtin_memchr(tree exp, rtx target, enum machine_mode mode) {
  if (validate_arglist(exp, POINTER_TYPE, INTEGER_TYPE, INTEGER_TYPE,
                       VOID_TYPE)) {
    tree type = TREE_TYPE(exp);
    tree result =
        fold_builtin_memchr(CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1),
...
static rtx expand_builtin_memcmp(tree exp, rtx target, enum machine_mode mode) {
  if (!validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, INTEGER_TYPE,
                        VOID_TYPE))
    return NULL_RTX;
  else {
    tree result = fold_builtin_memcmp(
        CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1), CALL_EXPR_ARG(exp, 2));
    if (result)
      return expand_expr(result, target, mode, EXPAND_NORMAL);
  }
...
#if defined HAVE_cmpmemsi || defined HAVE_cmpstrnsi
  {
    ...
    if (insn)
      ...
      emit_library_call_value(
          memcmp_libfunc, result, LCT_PURE, TYPE_MODE(integer_type_node), 3,
          XEXP(arg1_rtx, 0), Pmode, XEXP(arg2_rtx, 0), Pmode,
...
/* Expand expression EXP, which is a call to the strcmp builtin.  Return
   NULL_RTX if we failed the caller should emit a normal call, otherwise try to
...
static rtx expand_builtin_strcmp(tree exp, rtx target, enum machine_mode mode) {
  if (!validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, VOID_TYPE))
    ...
    tree result =
        fold_builtin_strcmp(CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1));
    if (result)
      return expand_expr(result, target, mode, EXPAND_NORMAL);
  }
...
static rtx expand_builtin_strncmp(tree exp, rtx target,
                                  enum machine_mode mode) {
  if (!validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, INTEGER_TYPE,
                        VOID_TYPE))
    return NULL_RTX;
  else {
    tree result = fold_builtin_strncmp(
        CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1), CALL_EXPR_ARG(exp, 2));
    if (result)
      return expand_expr(result, target, mode, EXPAND_NORMAL);
  }
...
static rtx expand_builtin_strcat(tree fndecl, tree exp, rtx target,
                                 enum machine_mode mode) {
  if (!validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, VOID_TYPE))
    return NULL_RTX;
  else {
    ...
    if (optimize_insn_for_speed_p()) {
      ...
      tree newsrc, newdst, strlen_fn = implicit_built_in_decls[BUILT_IN_STRLEN];
      rtx insns;
...
static rtx expand_builtin_strncat(tree exp, rtx target,
                                  enum machine_mode mode) {
  if (validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, INTEGER_TYPE,
                       VOID_TYPE)) {
    tree result = fold_builtin_strncat(
        CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1), CALL_EXPR_ARG(exp, 2));
    if (result)
      return expand_expr(result, target, mode, EXPAND_NORMAL);
  }
...
static rtx expand_builtin_strspn(tree exp, rtx target, enum machine_mode mode) {
  if (validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, VOID_TYPE)) {
    tree result =
        fold_builtin_strspn(CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1));
    if (result)
      return expand_expr(result, target, mode, EXPAND_NORMAL);
  }
  return NULL_RTX;
...
static rtx expand_builtin_strcspn(tree exp, rtx target,
                                  enum machine_mode mode) {
  if (validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, VOID_TYPE)) {
    tree result =
        fold_builtin_strcspn(CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1));
    if (result)
      return expand_expr(result, target, mode, EXPAND_NORMAL);
  }
  return NULL_RTX;
...
static rtx expand_builtin_next_arg(void) {
  ...
  return expand_binop(ptr_mode, add_optab, crtl->args.internal_arg_pointer,
                      crtl->args.arg_offset_rtx, NULL_RTX, 0, OPTAB_LIB_WIDEN);
}
...
tree std_fn_abi_va_list(tree fndecl ATTRIBUTE_UNUSED) {
...
tree std_gimplify_va_arg_expr(tree valist, tree type, gimple_seq *pre_p,
                              gimple_seq *post_p) {
  ...
  if (boundary > align && !integer_zerop(TYPE_SIZE(type))) {
    ...
    t = build2(
        MODIFY_EXPR, TREE_TYPE(valist), valist_tmp,
        fold_convert(TREE_TYPE(valist), fold_build2(BIT_AND_EXPR, sizetype, t,
                                                    size_int(-boundary))));
    gimplify_and_add(t, pre_p);
  } else
    boundary = align;
...
enum gimplify_status gimplify_va_arg_expr(tree *expr_p, gimple_seq *pre_p,
                                          gimple_seq *post_p) {
...
static rtx expand_builtin_frame_address(tree fndecl, tree exp) {
  ...
  if (call_expr_nargs(exp) == 0)
    ...
    rtx tem = expand_builtin_return_addr(
        DECL_FUNCTION_CODE(fndecl), tree_low_cst(CALL_EXPR_ARG(exp, 0), 1));
...
static rtx expand_builtin_alloca(tree exp, rtx target) {
  rtx op0;
  rtx result;
...
static rtx expand_builtin_bswap(tree exp, rtx target, rtx subtarget) {
  enum machine_mode mode;
  tree arg;
  rtx op0;
...
static rtx expand_builtin_unop(enum machine_mode target_mode, tree exp,
                               rtx target, rtx subtarget, optab op_optab) {
  rtx op0;
  ...
  /* Compute the argument.  */
  op0 = expand_expr(CALL_EXPR_ARG(exp, 0), subtarget, VOIDmode, EXPAND_NORMAL);
  /* Compute op, into TARGET if possible.
     Set TARGET to wherever the result comes back.  */
  target = expand_unop(TYPE_MODE(TREE_TYPE(CALL_EXPR_ARG(exp, 0))), op_optab,
                       op0, target, 1);
  gcc_assert(target);
...
static rtx expand_builtin_fputs(tree exp, rtx target, bool unlocked) {
  /* Verify the arguments in the original call.  */
  if (validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, VOID_TYPE)) {
    tree result =
        fold_builtin_fputs(CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1),
                           (target == const0_rtx), unlocked, NULL_TREE);
    if (result)
      return expand_expr(result, target, VOIDmode, EXPAND_NORMAL);
  }
  return NULL_RTX;
...
static rtx expand_builtin_expect(tree exp, rtx target) {
  tree arg, c;
  ...
  gcc_assert(!flag_guess_branch_prob || optimize == 0 || errorcount ||
             sorrycount);
  return target;
}
...
static rtx expand_builtin_fabs(tree exp, rtx target, rtx subtarget) {
  enum machine_mode mode;
  tree arg;
  rtx op0;
...
tree build_string_literal(int len, const char *str) {
  ...
  t = build1(
      ADDR_EXPR, type,
      build4(ARRAY_REF, elem, t, integer_zero_node, NULL_TREE, NULL_TREE));
  return t;
}
...
/* Expand EXP, a call to printf or printf_unlocked.
   ...
   call.  */
static rtx expand_builtin_printf(tree exp, rtx target, enum machine_mode mode,
                                 bool unlocked) {
  ...
  if (strcmp(fmt_str, target_percent_s_newline) == 0) {
    if ((nargs != 2) || !POINTER_TYPE_P(TREE_TYPE(CALL_EXPR_ARG(exp, 1))))
      return NULL_RTX;
    if (fn_puts)
...
/* Expand EXP, a call to fprintf or fprintf_unlocked.
   ...
   call.  */
static rtx expand_builtin_fprintf(tree exp, rtx target, enum machine_mode mode,
                                  bool unlocked) {
  /* If we're using an unlocked function, assume the other unlocked
     functions exist explicitly.  */
  ...
  if (strcmp(fmt_str, target_percent_s) == 0) {
    if ((nargs != 3) || !POINTER_TYPE_P(TREE_TYPE(CALL_EXPR_ARG(exp, 2))))
      return NULL_RTX;
...
static rtx expand_builtin_sprintf(tree exp, rtx target,
                                  enum machine_mode mode) {
  tree dest, fmt;
  const char *fmt_str;
  int nargs = call_expr_nargs(exp);
  ...
  if (strchr(fmt_str, target_percent) == 0) {
    ...
    if ((nargs > 2) || !fn)
      return NULL_RTX;
    expand_expr(build_call_expr(fn, 2, dest, fmt), const0_rtx, VOIDmode,
                EXPAND_NORMAL);
    if (target == const0_rtx)
      return const0_rtx;
  ...
  /* If the format is "%s", use strcpy if the result isn't used.  */
    ...
    expand_expr(build_call_expr(fn, 2, dest, arg), const0_rtx, VOIDmode,
                EXPAND_NORMAL);
...
static rtx expand_builtin_profile_func(bool exitp) {
  ...
  emit_library_call(which, LCT_NORMAL, VOIDmode, 2, this_rtx, Pmode,
                    expand_builtin_return_addr(BUILT_IN_RETURN_ADDRESS, 0),
                    Pmode);
...
static rtx expand_builtin___clear_cache(tree exp ATTRIBUTE_UNUSED) {
...
static rtx round_trampoline_addr(rtx tramp) {
  ...
  temp =
      expand_simple_binop(Pmode, PLUS, tramp, addend, temp, 0, OPTAB_LIB_WIDEN);
  tramp = expand_simple_binop(Pmode, AND, temp, mask, temp, 0, OPTAB_LIB_WIDEN);

  return tramp;
}

static rtx expand_builtin_init_trampoline(tree exp) {
...
static rtx expand_builtin_signbit(tree exp, rtx target) {
  ...
  if (bitpos < GET_MODE_BITSIZE(rmode)) {
    ...
    temp =
        expand_binop(rmode, and_optab, temp, immed_double_const(lo, hi, rmode),
    ...
    temp = gen_lowpart(rmode, temp);
    temp = expand_binop(rmode, and_optab, temp, const1_rtx, NULL_RTX, 1,
                        OPTAB_LIB_WIDEN);
  }
...
static rtx expand_builtin_fork_or_exec(tree fn, tree exp, rtx target,
                                       int ignore) {
  tree id, decl;
  tree call;
...
static inline enum machine_mode get_builtin_sync_mode(int fcode_diff) {
...
static rtx expand_builtin_sync_operation(enum machine_mode mode, tree exp,
  ...
  if (code == NOT && warn_sync_nand) {
    ...
    switch (fcode) {
      ...
      case BUILT_IN_FETCH_AND_NAND_16:
        ...
        inform(input_location, "%qD changed semantics in GCC 4.4", fndecl);
        warned_f_a_n = true;
        break;
...
static rtx expand_builtin_compare_and_swap(enum machine_mode mode, tree exp,
  ...
  old_val = expand_expr(CALL_EXPR_ARG(exp, 1), NULL_RTX, mode, EXPAND_NORMAL);
  /* If VAL is promoted to a wider mode, convert it back to MODE.  Take care
     of CONST_INTs, where we know the old_mode only from the call argument.  */
  ...
  new_val = expand_expr(CALL_EXPR_ARG(exp, 2), NULL_RTX, mode, EXPAND_NORMAL);
  /* If VAL is promoted to a wider mode, convert it back to MODE.  Take care
     of CONST_INTs, where we know the old_mode only from the call argument.  */
...
static rtx expand_builtin_lock_test_and_set(enum machine_mode mode, tree exp,
                                            rtx target) {
  rtx val, mem;
  enum machine_mode old_mode;
...
static void expand_builtin_lock_release(enum machine_mode mode, tree exp) {
  enum insn_code icode;
  rtx mem, insn;
  rtx val = const0_rtx;
...
rtx expand_builtin(tree exp, rtx target, rtx subtarget, enum machine_mode mode,
                   int ignore) {
  tree fndecl = get_callee_fndecl(exp);
  enum built_in_function fcode = DECL_FUNCTION_CODE(fndecl);
  ...
  if (!optimize && !called_as_built_in(fndecl) &&
      DECL_ASSEMBLER_NAME_SET_P(fndecl) && fcode != BUILT_IN_ALLOCA &&
      fcode != BUILT_IN_FREE)
  ...
  if (target == const0_rtx && (DECL_PURE_P(fndecl) || TREE_READONLY(fndecl))) {
  ...
  switch (fcode) {
    ...
    /* Just do a normal library call if we were unable to fold
       the values.  */
    CASE_FLT_FN(BUILT_IN_CABS) : break;
    ...
    CASE_FLT_FN(BUILT_IN_EXP)
        ...
        : /* Treat these like sqrt only if unsafe math optimizations are
             allowed, because of possible accuracy problems.  */
          if (!flag_unsafe_math_optimizations) break;
    ...
    CASE_FLT_FN(BUILT_IN_ILOGB) : if (!flag_unsafe_math_optimizations) break;
  CASE_FLT_FN(BUILT_IN_ISINF)
      : CASE_FLT_FN(BUILT_IN_FINITE) : case BUILT_IN_ISFINITE:
  case BUILT_IN_ISNORMAL:
    ...
    CASE_FLT_FN(BUILT_IN_SINCOS) : if (!flag_unsafe_math_optimizations) break;
    ...
    case BUILT_IN_APPLY:
      if (!validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, INTEGER_TYPE,
                            VOID_TYPE) &&
          !validate_arglist(exp, REFERENCE_TYPE, POINTER_TYPE, INTEGER_TYPE,
                            VOID_TYPE))
        return const0_rtx;
      else {
        rtx ops[3];
    ...
    CASE_INT_FN(BUILT_IN_FFS) : case BUILT_IN_FFSIMAX:
      target =
          expand_builtin_unop(target_mode, exp, target, subtarget, ffs_optab);
      if (target) return target;
      break;

    CASE_INT_FN(BUILT_IN_CLZ) : case BUILT_IN_CLZIMAX:
      target =
          expand_builtin_unop(target_mode, exp, target, subtarget, clz_optab);
      if (target) return target;
      break;

    CASE_INT_FN(BUILT_IN_CTZ) : case BUILT_IN_CTZIMAX:
      target =
          expand_builtin_unop(target_mode, exp, target, subtarget, ctz_optab);
      if (target) return target;
      break;

    CASE_INT_FN(BUILT_IN_POPCOUNT) : case BUILT_IN_POPCOUNTIMAX:
      target = expand_builtin_unop(target_mode, exp, target, subtarget,
                                   popcount_optab);
      if (target) return target;
      break;

    CASE_INT_FN(BUILT_IN_PARITY) : case BUILT_IN_PARITYIMAX:
      target = expand_builtin_unop(target_mode, exp, target, subtarget,
                                   parity_optab);
      if (target) return target;
      break;
    ...
    case BUILT_IN_SETJMP_SETUP:
      ...
      if (validate_arglist(exp, POINTER_TYPE, POINTER_TYPE, VOID_TYPE)) {
        rtx buf_addr = expand_expr(CALL_EXPR_ARG(exp, 0), subtarget, VOIDmode,
                                   EXPAND_NORMAL);
        ...
        nonlocal_goto_handler_labels =
            gen_rtx_EXPR_LIST(VOIDmode, label_r, nonlocal_goto_handler_labels);
    ...
    case BUILT_IN_UPDATE_SETJMP_BUF:
      if (validate_arglist(exp, POINTER_TYPE, VOID_TYPE)) {
        rtx buf_addr = expand_normal(CALL_EXPR_ARG(exp, 0));
    ...
    CASE_FLT_FN(BUILT_IN_SIGNBIT) : case BUILT_IN_SIGNBITD32:
    ...
    case BUILT_IN_EH_RETURN:
      expand_builtin_eh_return(CALL_EXPR_ARG(exp, 0), CALL_EXPR_ARG(exp, 1));
      return const0_rtx;
    ...
    case BUILT_IN_FETCH_AND_ADD_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, PLUS, false, target, ignore);
      if (target)
        return target;
      break;
    ...
    case BUILT_IN_FETCH_AND_OR_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, IOR, false, target, ignore);
      if (target)
        return target;
      break;
    ...
    case BUILT_IN_FETCH_AND_AND_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, AND, false, target, ignore);
      if (target)
        return target;
      break;
    ...
    case BUILT_IN_FETCH_AND_XOR_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, XOR, false, target, ignore);
      if (target)
        return target;
      break;
    ...
    case BUILT_IN_FETCH_AND_NAND_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, NOT, false, target, ignore);
      if (target)
        return target;
      break;
    ...
    case BUILT_IN_ADD_AND_FETCH_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, PLUS, true, target, ignore);
      if (target)
        return target;
      break;
    ...
    case BUILT_IN_SUB_AND_FETCH_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, MINUS, true, target, ignore);
      if (target)
        return target;
      break;
    ...
    case BUILT_IN_OR_AND_FETCH_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, IOR, true, target, ignore);
      if (target)
        return target;
      break;
    ...
    case BUILT_IN_AND_AND_FETCH_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, AND, true, target, ignore);
      if (target)
        return target;
      break;
    ...
    case BUILT_IN_XOR_AND_FETCH_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, XOR, true, target, ignore);
      if (target)
        return target;
      break;
    ...
    case BUILT_IN_NAND_AND_FETCH_16:
      ...
      target =
          expand_builtin_sync_operation(mode, exp, NOT, true, target, ignore);
      if (target)
        return target;
      break;
...
enum built_in_function builtin_mathfn_code(const_tree t) {
  ...
  if (TREE_CODE(t) != CALL_EXPR || TREE_CODE(CALL_EXPR_FN(t)) != ADDR_EXPR)
    return END_BUILTINS;
...
static tree fold_builtin_constant_p(tree arg) {
  ...
  if (TREE_CODE(arg) == ADDR_EXPR) {
    ...
    if (TREE_CODE(op) == STRING_CST ||
        (TREE_CODE(op) == ARRAY_REF && integer_zerop(TREE_OPERAND(op, 1)) &&
  ...
  if (TREE_SIDE_EFFECTS(arg) || AGGREGATE_TYPE_P(TREE_TYPE(arg)) ||
      POINTER_TYPE_P(TREE_TYPE(arg)) || cfun == 0 || folding_initializer)
...
static tree build_builtin_expect_predicate(tree pred, tree expected) {
...
static tree fold_builtin_expect(tree arg0, tree arg1) {
  tree inner, fndecl;
  enum tree_code code;
  ...
  inner = arg0;
  while (TREE_CODE(inner) == NOP_EXPR && INTEGRAL_TYPE_P(TREE_TYPE(inner)) &&
  ...
  if (TREE_CODE(inner) == ADDR_EXPR) {
    ...
    if ((TREE_CODE(inner) == VAR_DECL || TREE_CODE(inner) == FUNCTION_DECL) &&
        DECL_WEAK(inner))
      return NULL_TREE;
  }
...
static tree fold_builtin_strlen(tree arg) {
...
static tree fold_builtin_nan(tree arg, tree type, int quiet) {
  REAL_VALUE_TYPE real;
  const char *str;
...
static bool integer_valued_real_p(tree t) {
  switch (TREE_CODE(t)) {
    case FLOAT_EXPR:
      return true;
...
static tree fold_trunc_transparent_mathfn(tree fndecl, tree arg) {
  enum built_in_function fcode = DECL_FUNCTION_CODE(fndecl);
  ...
  if (optimize) {
    ...
    if (TYPE_PRECISION(newtype) < TYPE_PRECISION(ftype) &&
      ...
      return fold_convert(
          ftype, build_call_expr(decl, 1, fold_convert(newtype, arg0)));
  }
  return NULL_TREE;
}
...
static tree fold_fixed_mathfn(tree fndecl, tree arg) {
  enum built_in_function fcode = DECL_FUNCTION_CODE(fndecl);
...
static tree fold_builtin_cabs(tree arg, tree type, tree fndecl) {
  tree res;
  ...
  if (TREE_CODE(arg) == COMPLEX_CST &&
      (res = do_mpfr_arg2(TREE_REALPART(arg), TREE_IMAGPART(arg), type,
                          mpfr_hypot)))
    return res;
  ...
  if (TREE_CODE(arg) == COMPLEX_EXPR) {
    ...
    if (flag_unsafe_math_optimizations &&
      ...
      STRIP_NOPS(real);
      return fold_build2(MULT_EXPR, type, fold_build1(ABS_EXPR, type, real),
                         build_real(type, sqrt2_trunc));
    }
  }

  /* Optimize cabs(-z) and cabs(conj(z)) as cabs(z).  */
  if (TREE_CODE(arg) == NEGATE_EXPR || TREE_CODE(arg) == CONJ_EXPR)
    return build_call_expr(fndecl, 1, TREE_OPERAND(arg, 0));

  /* Don't do this when optimizing for size.  */
  if (flag_unsafe_math_optimizations && optimize &&
    ...
    if (sqrtfn != NULL_TREE) {
      ...
      result = fold_build2(PLUS_EXPR, type,
                           fold_build2(MULT_EXPR, type, rpart, rpart),
                           fold_build2(MULT_EXPR, type, ipart, ipart));
...
static tree fold_builtin_sqrt(tree arg, tree type) {
  enum built_in_function fcode;
  tree res;
  ...
  if (flag_unsafe_math_optimizations && BUILTIN_EXPONENT_P(fcode)) {
    ...
    arg = fold_build2(MULT_EXPR, type, CALL_EXPR_ARG(arg, 0),
  ...
  if (flag_unsafe_math_optimizations &&
      (fcode == BUILT_IN_POW || fcode == BUILT_IN_POWF ||
       fcode == BUILT_IN_POWL)) {
    ...
    narg1 = fold_build2(MULT_EXPR, type, arg1, build_real(type, dconsthalf));
...
static tree fold_builtin_cbrt(tree arg, tree type) {
    ...
    if (BUILTIN_CBRT_P(fcode)) {
      ...
      if (tree_expr_nonnegative_p(arg0)) {
        ...
        if (powfn) {
          ...
          real_arithmetic(&dconstroot, MULT_EXPR, dconst_third_ptr(),
                          dconst_third_ptr());
          dconstroot = real_value_truncate(TYPE_MODE(type), dconstroot);
          tree_root = build_real(type, dconstroot);
          return build_call_expr(powfn, 2, arg0, tree_root);
    ...
    if (fcode == BUILT_IN_POW || fcode == BUILT_IN_POWF ||
      ...
      if (tree_expr_nonnegative_p(arg00)) {
        ...
        const REAL_VALUE_TYPE dconstroot =
            real_value_truncate(TYPE_MODE(type), dconst_third());
        tree narg01 =
            fold_build2(MULT_EXPR, type, arg01, build_real(type, dconstroot));
...
static tree fold_builtin_cos(tree arg, tree type, tree fndecl) {
  tree res, narg;
...
static tree fold_builtin_cosh(tree arg, tree type, tree fndecl) {
  if (validate_arg(arg, REAL_TYPE)) {
    tree res, narg;
...
static tree fold_builtin_tan(tree arg, tree type) {
  enum built_in_function fcode;
  tree res;
  ...
  fcode = builtin_mathfn_code(arg);
  if (flag_unsafe_math_optimizations &&
      (fcode == BUILT_IN_ATAN || fcode == BUILT_IN_ATANF ||
...
static tree fold_builtin_sincos(tree arg0, tree arg1, tree arg2) {
  tree type;
  tree res, fn, call;
  ...
  return build2(
      COMPOUND_EXPR, type,
      build2(MODIFY_EXPR, void_type_node, build_fold_indirect_ref(arg1),
             build1(IMAGPART_EXPR, type, call)),
      build2(MODIFY_EXPR, void_type_node, build_fold_indirect_ref(arg2),
...
static tree fold_builtin_cexp(tree arg0, tree type) {
  tree rtype;
  ...
  if ((realp = fold_unary(REALPART_EXPR, rtype, arg0)) && real_zerop(realp)) {
  ...
  /* In case we can easily decompose real and imaginary parts split cexp
     to exp (r) * cexpi (i).  */
  if (flag_unsafe_math_optimizations && realp) {
    ...
    return fold_build2(COMPLEX_EXPR, type,
                       fold_build2(MULT_EXPR, rtype, rcall,
...
static tree fold_builtin_trunc(tree fndecl, tree arg) {
  if (!validate_arg(arg, REAL_TYPE))
    return NULL_TREE;
...
static tree fold_builtin_floor(tree fndecl, tree arg) {
  if (!validate_arg(arg, REAL_TYPE))
    return NULL_TREE;
...
static tree fold_builtin_ceil(tree fndecl, tree arg) {
  if (!validate_arg(arg, REAL_TYPE))
    return NULL_TREE;
...
static tree fold_builtin_round(tree fndecl, tree arg) {
  if (!validate_arg(arg, REAL_TYPE))
    return NULL_TREE;
...
static tree fold_builtin_int_roundingfn(tree fndecl, tree arg) {
  if (!validate_arg(arg, REAL_TYPE))
    return NULL_TREE;
  ...
  switch (DECL_FUNCTION_CODE(fndecl)) {
    CASE_FLT_FN(BUILT_IN_LFLOOR)
        ...
        : /* Fold lfloor (x) where x is nonnegative to FIX_TRUNC (x).  */
          if (tree_expr_nonnegative_p(arg)) return fold_build1(
              FIX_TRUNC_EXPR, TREE_TYPE(TREE_TYPE(fndecl)), arg);
    break;
    default:;
  }
...
static tree fold_builtin_bitop(tree fndecl, tree arg) {
  ...
  if (TREE_CODE(arg) == INTEGER_CST && !TREE_OVERFLOW(arg)) {
    ...
    switch (DECL_FUNCTION_CODE(fndecl)) {
      CASE_INT_FN(BUILT_IN_FFS)
          : if (lo != 0) result = exact_log2(lo & -lo) + 1;
      else if (hi != 0) result =
          HOST_BITS_PER_WIDE_INT + exact_log2(hi & -hi) + 1;
      else result = 0;
      break;

      CASE_INT_FN(BUILT_IN_CLZ)
          : if (hi != 0) result =
                width - floor_log2(hi) - 1 - HOST_BITS_PER_WIDE_INT;
      else if (lo != 0) result = width - floor_log2(lo) - 1;
      else if (!CLZ_DEFINED_VALUE_AT_ZERO(TYPE_MODE(type), result)) result =
          width;
      break;

      CASE_INT_FN(BUILT_IN_CTZ) : if (lo != 0) result = exact_log2(lo & -lo);
      else if (hi != 0) result = HOST_BITS_PER_WIDE_INT + exact_log2(hi & -hi);
      else if (!CTZ_DEFINED_VALUE_AT_ZERO(TYPE_MODE(type), result)) result =
          width;
      break;

      CASE_INT_FN(BUILT_IN_POPCOUNT) : result = 0;
      while (lo)
        result++, lo &= lo - 1;
      while (hi)
        result++, hi &= hi - 1;
      ...
      CASE_INT_FN(BUILT_IN_PARITY) : result = 0;
      while (lo)
        result++, lo &= lo - 1;
      while (hi)
        result++, hi &= hi - 1;
...
static tree fold_builtin_bswap(tree fndecl, tree arg) {
  if (!validate_arg(arg, INTEGER_TYPE))
    return NULL_TREE;
...
static bool real_dconstp(tree expr, const REAL_VALUE_TYPE *value) {
  ...
  return ((TREE_CODE(expr) == REAL_CST && !TREE_OVERFLOW(expr) &&
...
static tree fold_builtin_logarithm(tree fndecl, tree arg,
    ...
    if (flag_unsafe_math_optimizations &&
        ((func == mpfr_log &&
          (fcode == BUILT_IN_EXP || fcode == BUILT_IN_EXPF ||
           fcode == BUILT_IN_EXPL)) ||
         (func == mpfr_log2 &&
          (fcode == BUILT_IN_EXP2 || fcode == BUILT_IN_EXP2F ||
      ...
      switch (fcode) {
        ...
        CASE_FLT_FN(BUILT_IN_CBRT)
            : /* Prepare to do logN(cbrt(x) -> (1/3)*logN(x).  */
              x = CALL_EXPR_ARG(arg, 0);
        exponent = build_real(
            type, real_value_truncate(TYPE_MODE(type), dconst_third()));
        break;
        CASE_FLT_FN(BUILT_IN_POW)
...
static tree fold_builtin_hypot(tree fndecl, tree arg0, tree arg1, tree type) {
...
/* Fold a builtin function call to pow, powf, or powl.  Return
   NULL_TREE if no simplification can be made.  */
static tree fold_builtin_pow(tree fndecl, tree arg0, tree arg1, tree type) {
  tree res;
  ...
  if (TREE_CODE(arg1) == REAL_CST && !TREE_OVERFLOW(arg1)) {
    ...
    if (REAL_VALUES_EQUAL(c, dconstm1))
      return fold_build2(RDIV_EXPR, type, build_real(type, dconst1), arg0);

    /* Optimize pow(x,0.5) = sqrt(x).  */
    if (flag_unsafe_math_optimizations && REAL_VALUES_EQUAL(c, dconsthalf)) {
      tree sqrtfn = mathfn_built_in(type, BUILT_IN_SQRT);
    ...
    if (real_identical(&c, &cint)) {
      ...
      if (TREE_CODE(arg0) == REAL_CST && !TREE_OVERFLOW(arg0) &&
          (n > 0 || (!flag_trapping_math && !flag_errno_math) ||
  ...
  if (flag_unsafe_math_optimizations) {
    ...
    if (BUILTIN_SQRT_P(fcode)) {
      ...
      tree narg1 =
          fold_build2(MULT_EXPR, type, arg1, build_real(type, dconsthalf));
    ...
    if (BUILTIN_CBRT_P(fcode)) {
      ...
      if (tree_expr_nonnegative_p(arg)) {
        const REAL_VALUE_TYPE dconstroot =
            real_value_truncate(TYPE_MODE(type), dconst_third());
        tree narg1 =
            fold_build2(MULT_EXPR, type, arg1, build_real(type, dconstroot));
...
/* Fold a builtin function call to powi, powif, or powil with argument ARG.
   Return NULL_TREE if no simplification can be made.  */
static tree fold_builtin_powi(tree fndecl ATTRIBUTE_UNUSED, tree arg0,
                              tree arg1, tree type) {
...
static tree fold_builtin_exponent(tree fndecl, tree arg,
                                  int (*func)(mpfr_ptr, mpfr_srcptr,
                                              mp_rnd_t)) {
  if (validate_arg(arg, REAL_TYPE)) {
    tree type = TREE_TYPE(TREE_TYPE(fndecl));
    ...
    if (flag_unsafe_math_optimizations) {
      ...
      if ((func == mpfr_exp &&
           (fcode == BUILT_IN_LOG || fcode == BUILT_IN_LOGF ||
            fcode == BUILT_IN_LOGL)) ||
          (func == mpfr_exp2 &&
           (fcode == BUILT_IN_LOG2 || fcode == BUILT_IN_LOG2F ||
            fcode == BUILT_IN_LOG2L)) ||
          (func == mpfr_exp10 &&
           (fcode == BUILT_IN_LOG10 || fcode == BUILT_IN_LOG10F ||
...
static tree fold_builtin_memset(tree dest, tree c, tree len, tree type,
                                bool ignore) {
  tree var, ret;
  unsigned HOST_WIDE_INT length, cval;

  if (!validate_arg(dest, POINTER_TYPE) || !validate_arg(c, INTEGER_TYPE) ||
  ...
  if (!INTEGRAL_TYPE_P(TREE_TYPE(var)) && !POINTER_TYPE_P(TREE_TYPE(var)))
    return NULL_TREE;
...
static tree fold_builtin_bzero(tree dest, tree size, bool ignore) {
  if (!validate_arg(dest, POINTER_TYPE) || !validate_arg(size, INTEGER_TYPE))
    return NULL_TREE;
  ...
  return fold_builtin_memset(dest, integer_zero_node,
                             fold_convert(sizetype, size), void_type_node,
                             ignore);
}
...
static tree fold_builtin_memory_op(tree dest, tree src, tree len, tree type,
                                   bool ignore, int endp) {
  tree destvar, srcvar, expr;

  if (!validate_arg(dest, POINTER_TYPE) || !validate_arg(src, POINTER_TYPE) ||
      !validate_arg(len, INTEGER_TYPE))
    return NULL_TREE;
  ...
  if (operand_equal_p(src, dest, 0))
    ...
    if (!srctype || !desttype || !TYPE_SIZE_UNIT(srctype) ||
        !TYPE_SIZE_UNIT(desttype) ||
        TREE_CODE(TYPE_SIZE_UNIT(srctype)) != INTEGER_CST ||
        TREE_CODE(TYPE_SIZE_UNIT(desttype)) != INTEGER_CST ||
        TYPE_VOLATILE(srctype) || TYPE_VOLATILE(desttype))
      return NULL_TREE;
    ...
    if (srctype == desttype ||
        (gimple_in_ssa_p(cfun) && useless_type_conversion_p(desttype, srctype)))
      expr = srcvar;
  ...
  if (endp == 2)
    len = fold_build2(MINUS_EXPR, TREE_TYPE(len), len, ssize_int(1));
...
tree fold_builtin_strcpy(tree fndecl, tree dest, tree src, tree len) {
  tree fn;
...
tree fold_builtin_strncpy(tree fndecl, tree dest, tree src, tree len,
                          tree slen) {
  tree fn;
...
static tree fold_builtin_memchr(tree arg1, tree arg2, tree len, tree type) {
  if (!validate_arg(arg1, POINTER_TYPE) || !validate_arg(arg2, INTEGER_TYPE) ||
      !validate_arg(len, INTEGER_TYPE))
    return NULL_TREE;
  else {
    const char *p1;

    if (TREE_CODE(arg2) != INTEGER_CST || !host_integerp(len, 1))
      return NULL_TREE;
...
static tree fold_builtin_memcmp(tree arg1, tree arg2, tree len) {
  const char *p1, *p2;

  if (!validate_arg(arg1, POINTER_TYPE) || !validate_arg(arg2, POINTER_TYPE) ||
      !validate_arg(len, INTEGER_TYPE))
    return NULL_TREE;

  /* If the LEN parameter is zero, return zero.  */
  if (integer_zerop(len))
    return omit_two_operands(integer_type_node, integer_zero_node, arg1, arg2);
...
static tree fold_builtin_strcmp(tree arg1, tree arg2) {
  const char *p1, *p2;
...
static tree fold_builtin_strncmp(tree arg1, tree arg2, tree len) {
  const char *p1, *p2;
  ...
  if (p2 && *p2 == '\0' && TREE_CODE(len) == INTEGER_CST &&
  ...
  if (p1 && *p1 == '\0' && TREE_CODE(len) == INTEGER_CST &&
      tree_int_cst_sgn(len) == 1) {
    tree cst_uchar_node = build_type_variant(unsigned_char_type_node, 1, 0);
    tree cst_uchar_ptr_node =
        build_pointer_type_for_mode(cst_uchar_node, ptr_mode, true);
...
static tree fold_builtin_signbit(tree arg, tree type) {
  tree temp;
  ...
  if (!HONOR_SIGNED_ZEROS(TYPE_MODE(TREE_TYPE(arg))))
    return fold_build2(LT_EXPR, type, arg, build_real(TREE_TYPE(arg), dconst0));
...
static tree fold_builtin_copysign(tree fndecl, tree arg1, tree arg2,
                                  tree type) {
  tree tem;
  ...
  if (tree_expr_nonnegative_p(arg2))
    return omit_one_operand(type, fold_build1(ABS_EXPR, type, arg1), arg2);
...
static tree fold_builtin_isascii(tree arg) {
  if (!validate_arg(arg, INTEGER_TYPE))
    return NULL_TREE;
  else {
    /* Transform isascii(c) -> ((c & ~0x7f) == 0).  */
    arg = build2(BIT_AND_EXPR, integer_type_node, arg,
                 build_int_cst(NULL_TREE, ~(unsigned HOST_WIDE_INT)0x7f));
    return fold_build2(EQ_EXPR, integer_type_node, arg, integer_zero_node);
  }
}

/* Fold a call to builtin toascii with argument ARG.  */

static tree fold_builtin_toascii(tree arg) {
  if (!validate_arg(arg, INTEGER_TYPE))
    return NULL_TREE;
...
static tree fold_builtin_isdigit(tree arg) {
  if (!validate_arg(arg, INTEGER_TYPE))
    return NULL_TREE;
  else {
...
static tree fold_builtin_fabs(tree arg, tree type) {
  if (!validate_arg(arg, REAL_TYPE))
    return NULL_TREE;
...
static tree fold_builtin_abs(tree arg, tree type) {
  if (!validate_arg(arg, INTEGER_TYPE))
    return NULL_TREE;
...
static tree fold_builtin_fmin_fmax(tree arg0, tree arg1, tree type, bool max) {
  if (validate_arg(arg0, REAL_TYPE) && validate_arg(arg1, REAL_TYPE)) {
    /* Calculate the result when the argument is a constant.  */
    ...
    if (flag_finite_math_only)
      return fold_build2((max ? MAX_EXPR : MIN_EXPR), type,
                         fold_convert(type, arg0), fold_convert(type, arg1));
  }
  return NULL_TREE;
}
...
static tree fold_builtin_carg(tree arg, tree type) {
...
static tree fold_builtin_logb(tree arg, tree rettype) {
  if (!validate_arg(arg, REAL_TYPE))
    return NULL_TREE;
    ...
    switch (value->cl) {
      ...
      case rvc_normal:
        ...
        if (REAL_MODE_FORMAT(TYPE_MODE(TREE_TYPE(arg)))->b == 2)
          return fold_convert(rettype,
                              build_int_cst(NULL_TREE, REAL_EXP(value) - 1));
        break;
    }
  }
...
static tree fold_builtin_significand(tree arg, tree rettype) {
  if (!validate_arg(arg, REAL_TYPE))
    return NULL_TREE;
...
static tree fold_builtin_frexp(tree arg0, tree arg1, tree rettype) {
  if (!validate_arg(arg0, REAL_TYPE) || !validate_arg(arg1, POINTER_TYPE))
    return NULL_TREE;
...
static tree fold_builtin_load_exponent(tree arg0, tree arg1, tree type,
                                       bool ldexp) {
  if (validate_arg(arg0, REAL_TYPE) && validate_arg(arg1, INTEGER_TYPE)) {
    STRIP_NOPS(arg0);
    STRIP_NOPS(arg1);
    ...
    if (real_zerop(arg0) || integer_zerop(arg1) ||
        (TREE_CODE(arg0) == REAL_CST && !real_isfinite(&TREE_REAL_CST(arg0))))
      return omit_one_operand(type, arg0, arg1);
    ...
    if ((ldexp || REAL_MODE_FORMAT(TYPE_MODE(type))->b == 2) &&
      ...
      const long max_exp_adj =
          2 * labs(REAL_MODE_FORMAT(TYPE_MODE(type))->emax -
...
static tree fold_builtin_modf(tree arg0, tree arg1, tree rettype) {
  if (!validate_arg(arg0, REAL_TYPE) || !validate_arg(arg1, POINTER_TYPE))
    return NULL_TREE;
  ...
  if (TYPE_MAIN_VARIANT(TREE_TYPE(arg1)) == TYPE_MAIN_VARIANT(rettype)) {
    ...
    /* Create the COMPOUND_EXPR (*arg1 = trunc, frac). */
    arg1 = fold_build2(MODIFY_EXPR, rettype, arg1, build_real(rettype, trunc));
    TREE_SIDE_EFFECTS(arg1) = 1;
    return fold_build2(COMPOUND_EXPR, rettype, arg1, build_real(rettype, frac));
  }
...
static tree fold_builtin_classify(tree fndecl, tree arg, int builtin_index) {
  tree type = TREE_TYPE(TREE_TYPE(fndecl));
  REAL_VALUE_TYPE r;
  ...
  switch (builtin_index) {
    case BUILT_IN_ISINF:
      ...
      if (TREE_CODE(arg) == REAL_CST) {
        r = TREE_REAL_CST(arg);
        if (real_isinf(&r))
          return real_compare(GT_EXPR, &r, &dconst0) ? integer_one_node
    ...
    case BUILT_IN_ISINF_SIGN: {
      ...
      if (signbit_fn && isinf_fn) {
        ...
        signbit_call = fold_build2(NE_EXPR, integer_type_node, signbit_call,
                                   integer_zero_node);
        isinf_call = fold_build2(NE_EXPR, integer_type_node, isinf_call,
                                 integer_zero_node);
...
static tree fold_builtin_fpclassify(tree exp) {
  tree fp_nan, fp_infinite, fp_normal, fp_subnormal, fp_zero, arg, type, res,
      tmp;
  enum machine_mode mode;
  REAL_VALUE_TYPE r;
  char buf[128];
  ...
  if (!validate_arglist(exp, INTEGER_TYPE, INTEGER_TYPE, INTEGER_TYPE,
                        INTEGER_TYPE, INTEGER_TYPE, REAL_TYPE, VOID_TYPE))
    return NULL_TREE;
  ...
  if (HONOR_INFINITIES(mode)) {
    real_inf(&r);
    tmp = fold_build2(EQ_EXPR, integer_type_node, arg, build_real(type, r));
...
static tree fold_builtin_unordered_cmp(tree fndecl, tree arg0, tree arg1,
  ...
  code = HONOR_NANS(TYPE_MODE(TREE_TYPE(arg0))) ? unordered_code : ordered_code;
  return fold_build1(TRUTH_NOT_EXPR, type, fold_build2(code, type, arg0, arg1));
}
...
static tree fold_builtin_0(tree fndecl, bool ignore ATTRIBUTE_UNUSED) {
  tree type = TREE_TYPE(TREE_TYPE(fndecl));
  enum built_in_function fcode = DECL_FUNCTION_CODE(fndecl);
  switch (fcode) {
  CASE_FLT_FN(BUILT_IN_INF) : case BUILT_IN_INFD32:
  case BUILT_IN_INFD64:
  case BUILT_IN_INFD128:
    return fold_builtin_inf(type, true);

    CASE_FLT_FN(BUILT_IN_HUGE_VAL) : return fold_builtin_inf(type, false);
...
static tree fold_builtin_1(tree fndecl, tree arg0, bool ignore) {
  tree type = TREE_TYPE(TREE_TYPE(fndecl));
  enum built_in_function fcode = DECL_FUNCTION_CODE(fndecl);
  switch (fcode) {
    case BUILT_IN_CONSTANT_P: {
    ...
    case BUILT_IN_IMAXABS:
      ...
      CASE_FLT_FN(BUILT_IN_CONJ)
          : if (validate_arg(arg0, COMPLEX_TYPE)) return fold_build1(
                CONJ_EXPR, type, arg0);
      break;

      CASE_FLT_FN(BUILT_IN_CREAL)
          : if (validate_arg(arg0, COMPLEX_TYPE)) return non_lvalue(
                fold_build1(REALPART_EXPR, type, arg0));
      ;
      break;

      CASE_FLT_FN(BUILT_IN_CIMAG)
          : if (validate_arg(arg0, COMPLEX_TYPE)) return non_lvalue(
                fold_build1(IMAGPART_EXPR, type, arg0));
      break;
      ...
      CASE_FLT_FN(BUILT_IN_CABS) : return fold_builtin_cabs(arg0, type, fndecl);

      CASE_FLT_FN(BUILT_IN_CARG) : return fold_builtin_carg(arg0, type);

      CASE_FLT_FN(BUILT_IN_SQRT) : return fold_builtin_sqrt(arg0, type);

      CASE_FLT_FN(BUILT_IN_CBRT) : return fold_builtin_cbrt(arg0, type);

      CASE_FLT_FN(BUILT_IN_ASIN)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_asin, &dconstm1, &dconst1, true);
      break;

      CASE_FLT_FN(BUILT_IN_ACOS)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_acos, &dconstm1, &dconst1, true);
      break;

      CASE_FLT_FN(BUILT_IN_ATAN)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_atan, NULL, NULL, 0);
      break;

      CASE_FLT_FN(BUILT_IN_ASINH)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_asinh, NULL, NULL, 0);
      break;

      CASE_FLT_FN(BUILT_IN_ACOSH)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_acosh, &dconst1, NULL, true);
      break;

      CASE_FLT_FN(BUILT_IN_ATANH)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_atanh, &dconstm1, &dconst1, false);
      break;

      CASE_FLT_FN(BUILT_IN_SIN)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_sin, NULL, NULL, 0);
      break;

      CASE_FLT_FN(BUILT_IN_COS) : return fold_builtin_cos(arg0, type, fndecl);
      break;

      CASE_FLT_FN(BUILT_IN_TAN) : return fold_builtin_tan(arg0, type);

      CASE_FLT_FN(BUILT_IN_CEXP) : return fold_builtin_cexp(arg0, type);

      CASE_FLT_FN(BUILT_IN_CEXPI)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_sincos(
                arg0, NULL_TREE, NULL_TREE);
      break;

      CASE_FLT_FN(BUILT_IN_SINH)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_sinh, NULL, NULL, 0);
      break;

      CASE_FLT_FN(BUILT_IN_COSH) : return fold_builtin_cosh(arg0, type, fndecl);

      CASE_FLT_FN(BUILT_IN_TANH)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_tanh, NULL, NULL, 0);
      break;

      CASE_FLT_FN(BUILT_IN_ERF)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_erf, NULL, NULL, 0);
      break;

      CASE_FLT_FN(BUILT_IN_ERFC)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_erfc, NULL, NULL, 0);
      break;

      CASE_FLT_FN(BUILT_IN_TGAMMA)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_gamma, NULL, NULL, 0);
      break;
      ...
      CASE_FLT_FN(BUILT_IN_EXPM1)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_expm1, NULL, NULL, 0);
      break;
      ...
      CASE_FLT_FN(BUILT_IN_LOG1P)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_log1p, &dconstm1, NULL, false);
      break;

      CASE_FLT_FN(BUILT_IN_J0)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_j0, NULL, NULL, 0);
      break;

      CASE_FLT_FN(BUILT_IN_J1)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_j1, NULL, NULL, 0);
      break;

      CASE_FLT_FN(BUILT_IN_Y0)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_y0, &dconst0, NULL, false);
      break;

      CASE_FLT_FN(BUILT_IN_Y1)
          : if (validate_arg(arg0, REAL_TYPE)) return do_mpfr_arg1(
                arg0, type, mpfr_y1, &dconst0, NULL, false);
      break;

    CASE_FLT_FN(BUILT_IN_NAN) : case BUILT_IN_NAND32:
    case BUILT_IN_NAND64:
    case BUILT_IN_NAND128:
      return fold_builtin_nan(arg0, type, true);

      CASE_FLT_FN(BUILT_IN_NANS) : return fold_builtin_nan(arg0, type, false);

      CASE_FLT_FN(BUILT_IN_FLOOR) : return fold_builtin_floor(fndecl, arg0);

      CASE_FLT_FN(BUILT_IN_CEIL) : return fold_builtin_ceil(fndecl, arg0);

      CASE_FLT_FN(BUILT_IN_TRUNC) : return fold_builtin_trunc(fndecl, arg0);

      CASE_FLT_FN(BUILT_IN_ROUND) : return fold_builtin_round(fndecl, arg0);
    ...
    case BUILT_IN_BSWAP64:
      ...
      CASE_FLT_FN(BUILT_IN_SIGNBIT) : return fold_builtin_signbit(arg0, type);

      CASE_FLT_FN(BUILT_IN_SIGNIFICAND)
          : return fold_builtin_significand(arg0, type);

      CASE_FLT_FN(BUILT_IN_ILOGB)
          : CASE_FLT_FN(BUILT_IN_LOGB) : return fold_builtin_logb(arg0, type);
    ...
    CASE_FLT_FN(BUILT_IN_FINITE) : case BUILT_IN_FINITED32:
    case BUILT_IN_FINITED64:
    case BUILT_IN_FINITED128:
    case BUILT_IN_ISFINITE:
      return fold_builtin_classify(fndecl, arg0, BUILT_IN_ISFINITE);

    CASE_FLT_FN(BUILT_IN_ISINF) : case BUILT_IN_ISINFD32:
    ...
    CASE_FLT_FN(BUILT_IN_ISNAN) : case BUILT_IN_ISNAND32:
...
static tree fold_builtin_2(tree fndecl, tree arg0, tree arg1, bool ignore) {
  tree type = TREE_TYPE(TREE_TYPE(fndecl));
  enum built_in_function fcode = DECL_FUNCTION_CODE(fndecl);
  ...
  switch (fcode) {
    CASE_FLT_FN(BUILT_IN_JN)
        : if (validate_arg(arg0, INTEGER_TYPE) &&
              validate_arg(
                  arg1, REAL_TYPE)) return do_mpfr_bessel_n(arg0, arg1, type,
                                                            mpfr_jn, NULL, 0);
    break;
    ...
    CASE_FLT_FN(BUILT_IN_DREM)
        ...
        : if (validate_arg(arg0, REAL_TYPE) &&
              validate_arg(arg1,
                           REAL_TYPE)) return do_mpfr_arg2(arg0, arg1, type,
                                                           mpfr_remainder);
    break;

    CASE_FLT_FN_REENT(BUILT_IN_GAMMA)
        : /* GAMMA_R */
          CASE_FLT_FN_REENT(BUILT_IN_LGAMMA)
        : /* LGAMMA_R */
          if (validate_arg(arg0, REAL_TYPE) &&
              validate_arg(arg1, POINTER_TYPE)) return do_mpfr_lgamma_r(arg0,
                                                                        arg1,
                                                                        type);
    break;
    ...
    CASE_FLT_FN(BUILT_IN_ATAN2)
        : if (validate_arg(arg0, REAL_TYPE) &&
              validate_arg(arg1, REAL_TYPE)) return do_mpfr_arg2(arg0, arg1,
                                                                 type,
                                                                 mpfr_atan2);
    break;
    ...
    CASE_FLT_FN(BUILT_IN_FREXP) : return fold_builtin_frexp(arg0, arg1, type);

    CASE_FLT_FN(BUILT_IN_MODF) : return fold_builtin_modf(arg0, arg1, type);
    ...
    case BUILT_IN_VFPRINTF:
      return fold_builtin_fprintf(fndecl, arg0, arg1, NULL_TREE, ignore, fcode);
...
static tree fold_builtin_3(tree fndecl, tree arg0, tree arg1, tree arg2,
                           bool ignore) {
  tree type = TREE_TYPE(TREE_TYPE(fndecl));
  enum built_in_function fcode = DECL_FUNCTION_CODE(fndecl);
  switch (fcode) {
    CASE_FLT_FN(BUILT_IN_SINCOS) : return fold_builtin_sincos(arg0, arg1, arg2);

    CASE_FLT_FN(BUILT_IN_FMA)
        : if (validate_arg(arg0, REAL_TYPE) && validate_arg(arg1, REAL_TYPE) &&
              validate_arg(arg2, REAL_TYPE)) return do_mpfr_arg3(arg0, arg1,
                                                                 arg2, type,
                                                                 mpfr_fma);
    break;
    ...
    CASE_FLT_FN(BUILT_IN_REMQUO)
        : if (validate_arg(arg0, REAL_TYPE) && validate_arg(arg1, REAL_TYPE) &&
              validate_arg(
                  arg2, POINTER_TYPE)) return do_mpfr_remquo(arg0, arg1, arg2);
    break;
    ...
    case BUILT_IN_BCOPY:
      return fold_builtin_memory_op(arg1, arg0, arg2, void_type_node, true,
                                    /*endp=*/3);
    ...
    case BUILT_IN_VFPRINTF_CHK:
      if (!validate_arg(arg1, INTEGER_TYPE) || TREE_SIDE_EFFECTS(arg1))
        ...
        return fold_builtin_fprintf(fndecl, arg0, arg2, NULL_TREE, ignore,
                                    fcode);
...
static tree fold_builtin_4(tree fndecl, tree arg0, tree arg1, tree arg2,
                           tree arg3, bool ignore) {
  enum built_in_function fcode = DECL_FUNCTION_CODE(fndecl);
  ...
  switch (fcode) {
    ...
    case BUILT_IN_MEMMOVE_CHK:
    case BUILT_IN_MEMSET_CHK:
      return fold_builtin_memory_chk(fndecl, arg0, arg1, arg2, arg3, NULL_TREE,
                                     ignore, DECL_FUNCTION_CODE(fndecl));
    ...
    case BUILT_IN_VFPRINTF_CHK:
      if (!validate_arg(arg1, INTEGER_TYPE) || TREE_SIDE_EFFECTS(arg1))
        ...
        return fold_builtin_fprintf(fndecl, arg0, arg2, arg3, ignore, fcode);
      break;
...
static tree fold_builtin_n(tree fndecl, tree *args, int nargs, bool ignore) {
  tree ret = NULL_TREE;
  ...
  switch (nargs) {
    ...
    case 3:
      ...
      break;
    case 4:
      ret = fold_builtin_4(fndecl, args[0], args[1], args[2], args[3], ignore);
      break;
    default:
      break;
  }
...
static tree fold_builtin_varargs(tree fndecl, tree exp,
...
static bool avoid_folding_inline_builtin(tree fndecl) {
  return (DECL_DECLARED_INLINE_P(fndecl) &&
          DECL_DISREGARD_INLINE_LIMITS(fndecl) && cfun &&
...
tree fold_call_expr(tree exp, bool ignore) {
  tree ret = NULL_TREE;
  tree fndecl = get_callee_fndecl(exp);
  if (fndecl && TREE_CODE(fndecl) == FUNCTION_DECL &&
    ...
    if (nargs && TREE_CODE(CALL_EXPR_ARG(exp, nargs - 1)) == CALL_EXPR) {
      ...
      if (fndecl2 && TREE_CODE(fndecl2) == FUNCTION_DECL &&
    ...
    if (DECL_BUILT_IN_CLASS(fndecl) == BUILT_IN_MD)
      ...
      if (ret) {
        ...
        if (CAN_HAVE_LOCATION_P(exp) && EXPR_HAS_LOCATION(exp)) {
          ...
          if (CAN_HAVE_LOCATION_P(realret) && !EXPR_HAS_LOCATION(realret))
...
tree build_function_call_expr(tree fndecl, tree arglist) {
...
tree fold_builtin_call_array(tree type, tree fn, int n, tree *argarray) {
  tree ret = NULL_TREE;
  int i;
  tree exp;
...
static tree rewrite_call_expr(tree exp, int skip, tree fndecl, int n, ...) {
...
bool validate_gimple_arglist(const_gimple call, ...) {
  enum tree_code code;
  bool res = 0;
  va_list ap;
  const_tree arg;
...
bool validate_arglist(const_tree callexpr, ...) {
  enum tree_code code;
  bool res = 0;
  va_list ap;
  const_call_expr_arg_iterator iter;
...
static tree fold_builtin_strstr(tree s1, tree s2, tree type) {
  if (!validate_arg(s1, POINTER_TYPE) || !validate_arg(s2, POINTER_TYPE))
    ...
    if (p1 != NULL) {
      ...
      tem = fold_build2(POINTER_PLUS_EXPR, TREE_TYPE(s1), s1, size_int(r - p1));
      return fold_convert(type, tem);
    }
...
static tree fold_builtin_strchr(tree s1, tree s2, tree type) {
...
static tree fold_builtin_strrchr(tree s1, tree s2, tree type) {
  if (!validate_arg(s1, POINTER_TYPE) || !validate_arg(s2, INTEGER_TYPE))
    return NULL_TREE;
  else {
    tree fn;
...
static tree fold_builtin_strpbrk(tree s1, tree s2, tree type) {
  if (!validate_arg(s1, POINTER_TYPE) || !validate_arg(s2, POINTER_TYPE))
    return NULL_TREE;
  else {
    tree fn;
...
static tree fold_builtin_strcat(tree dst, tree src) {
...
static tree fold_builtin_strncat(tree dst, tree src, tree len) {
  if (!validate_arg(dst, POINTER_TYPE) || !validate_arg(src, POINTER_TYPE) ||
      !validate_arg(len, INTEGER_TYPE))
    return NULL_TREE;
  else {
...
static tree fold_builtin_strspn(tree s1, tree s2) {
  if (!validate_arg(s1, POINTER_TYPE) || !validate_arg(s2, POINTER_TYPE))
    return NULL_TREE;
  else {
    ...
    if ((p1 && *p1 == '\0') || (p2 && *p2 == '\0'))
      /* Evaluate and ignore both arguments in case either one has
         side-effects.  */
      return omit_two_operands(size_type_node, size_zero_node, s1, s2);
    return NULL_TREE;
  }
}
...
static tree fold_builtin_strcspn(tree s1, tree s2) {
  if (!validate_arg(s1, POINTER_TYPE) || !validate_arg(s2, POINTER_TYPE))
    return NULL_TREE;
  else {
    const char *p1 = c_getstr(s1), *p2 = c_getstr(s2);
    ...
    if (p1 && *p1 == '\0') {
      /* Evaluate and ignore argument s2 in case it has
         side-effects.  */
      return omit_one_operand(size_type_node, size_zero_node, s2);
    }
...
tree fold_builtin_fputs(tree arg0, tree arg1, bool ignore, bool unlocked,
                        tree len) {
  /* If we're using an unlocked function, assume the other unlocked
     functions exist explicitly.  */
  tree const fn_fputc = unlocked ? built_in_decls[BUILT_IN_FPUTC_UNLOCKED]
  ...
  /* Verify the arguments in the original call.  */
  if (!validate_arg(arg0, POINTER_TYPE) || !validate_arg(arg1, POINTER_TYPE))
    return NULL_TREE;
  ...
  switch (compare_tree_int(len, 1)) {
    ...
    case 0: /* length is 1, call fputc.  */
    {
      ...
      if (p != NULL) {
        if (fn_fputc)
          return build_call_expr(fn_fputc, 2, build_int_cst(NULL_TREE, p[0]),
                                 arg1);
        else
          return NULL_TREE;
      }
...
bool fold_builtin_next_arg(tree exp, bool va_start_p) {
  ...
  if (!integer_zerop(arg)) {
    ...
    /* Strip off all nops for the sake of the comparison.  This
       ...
       parameters.  */
    while (CONVERT_EXPR_P(arg) || TREE_CODE(arg) == INDIRECT_REF)
...
static tree fold_builtin_sprintf(tree dest, tree fmt, tree orig, int ignored) {
  tree call, retval;
  ...
  /* Verify the required arguments in the original call.  We deal with two
     ...
     'sprintf (dest, "%s", orig)'.  */
  if (!validate_arg(dest, POINTER_TYPE) || !validate_arg(fmt, POINTER_TYPE))
    return NULL_TREE;
...
rtx expand_builtin_object_size(tree exp) {
  ...
  if (TREE_CODE(ost) != INTEGER_CST || tree_int_cst_sgn(ost) < 0 ||
      compare_tree_int(ost, 3) > 0) {
    error("%Klast argument of %D is not integer constant between 0 and 3", exp,
          fndecl);
    expand_builtin_trap();
    return const0_rtx;
  }
...
static rtx expand_builtin_memory_chk(tree exp, rtx target,
                                     enum machine_mode mode,
                                     enum built_in_function fcode) {
  ...
  if (!validate_arglist(
          exp, POINTER_TYPE,
          fcode == BUILT_IN_MEMSET_CHK ? INTEGER_TYPE : POINTER_TYPE,
  ...
  if (host_integerp(len, 1) || integer_all_onesp(size)) {
    ...
    if (!integer_all_onesp(size) && tree_int_cst_lt(size, len)) {
      warning_at(tree_nonartificial_location(exp), 0,
                 "%Kcall to %D will always overflow destination buffer", exp,
    ...
    while (TREE_CODE(fn) == COMPOUND_EXPR) {
      expand_expr(TREE_OPERAND(fn, 0), const0_rtx, VOIDmode, EXPAND_NORMAL);
...
static void maybe_emit_chk_warning(tree exp, enum built_in_function fcode) {
  int is_strlen = 0;
  tree len, size;
  ...
  if (is_strlen) {
    ...
    if (!src || !host_integerp(src, 1)) {
      warning_at(loc, 0, "%Kcall to %D might overflow destination buffer", exp,
                 get_callee_fndecl(exp));
      return;
...
static void maybe_emit_free_warning(tree exp) {
...
tree fold_builtin_memory_chk(tree fndecl, tree dest, tree src, tree len,
                             tree size, tree maxlen, bool ignore,
                             enum built_in_function fcode) {
  tree fn;

  if (!validate_arg(dest, POINTER_TYPE) ||
      !validate_arg(
          src, (fcode == BUILT_IN_MEMSET_CHK ? INTEGER_TYPE : POINTER_TYPE)) ||
      !validate_arg(len, INTEGER_TYPE) || !validate_arg(size, INTEGER_TYPE))
    return NULL_TREE;
...
tree fold_builtin_stxcpy_chk(tree fndecl, tree dest, tree src, tree size,
  ...
  if (!integer_all_onesp(size)) {
    ...
    if (!len || !host_integerp(len, 1)) {
      ...
      if (maxlen == NULL_TREE || !host_integerp(maxlen, 1)) {
        ...
        len = size_binop(PLUS_EXPR, len, ssize_int(1));
        return fold_convert(TREE_TYPE(TREE_TYPE(fndecl)),
                            build_call_expr(fn, 4, dest, src, len, size));
      }
    } else
      maxlen = len;
  ...
  fn = built_in_decls[fcode == BUILT_IN_STPCPY_CHK ? BUILT_IN_STPCPY
                                                   : BUILT_IN_STRCPY];
  if (!fn)
    return NULL_TREE;
...
tree fold_builtin_strncpy_chk(tree dest, tree src, tree len, tree size,
                              tree maxlen) {
  tree fn;
...
static tree fold_builtin_strcat_chk(tree fndecl, tree dest, tree src,
                                    tree size) {
  tree fn;
  const char *p;
...
static tree fold_builtin_strncat_chk(tree fndecl, tree dest, tree src, tree len,
                                     tree size) {
  tree fn;
  const char *p;
  ...
  if (!integer_all_onesp(size)) {
    ...
    if (src_len && host_integerp(src_len, 1) && host_integerp(len, 1) &&
...
static tree fold_builtin_sprintf_chk(tree exp, enum built_in_function fcode) {
  ...
  fn = built_in_decls[fcode == BUILT_IN_VSPRINTF_CHK ? BUILT_IN_VSPRINTF
                                                     : BUILT_IN_SPRINTF];
  if (!fn)
    return NULL_TREE;
...
tree fold_builtin_snprintf_chk(tree exp, tree maxlen,
                               enum built_in_function fcode) {
  tree dest, size, len, fn, fmt, flag;
  const char *fmt_str;
  ...
  /* If __builtin_{,v}snprintf_chk is used, assume {,v}snprintf is
     available.  */
  fn = built_in_decls[fcode == BUILT_IN_VSNPRINTF_CHK ? BUILT_IN_VSNPRINTF
                                                      : BUILT_IN_SNPRINTF];
  if (!fn)
    return NULL_TREE;
...
static tree fold_builtin_printf(tree fndecl, tree fmt, tree arg, bool ignore,
                                enum built_in_function fcode) {
  ...
  if (strcmp(fmt_str, target_percent_s) == 0 ||
    ...
    if (strcmp(fmt_str, target_percent_s) == 0) {
      ...
      if (fcode != BUILT_IN_VPRINTF && fcode != BUILT_IN_VPRINTF_CHK && arg)
        return NULL_TREE;
      str = fmt_str;
    }
...
static tree fold_builtin_fprintf(tree fndecl, tree fp, tree fmt, tree arg,
                                 bool ignore, enum built_in_function fcode) {
  ...
  /* If the format doesn't contain % args or %%, use strcpy.  */
  if (strchr(fmt_str, target_percent) == NULL) {
    if (fcode != BUILT_IN_VFPRINTF && fcode != BUILT_IN_VFPRINTF_CHK && arg)
      return NULL_TREE;
...
static tree do_mpfr_arg1(tree arg, tree type,
  ...
  if (REAL_MODE_FORMAT(TYPE_MODE(type))->b == 2 && TREE_CODE(arg) == REAL_CST &&
      !TREE_OVERFLOW(arg)) {
...
static tree do_mpfr_arg2(tree arg1, tree arg2, tree type,
...
static tree do_mpfr_arg3(tree arg1, tree arg2, tree arg3, tree type,
                         int (*func)(mpfr_ptr, mpfr_srcptr, mpfr_srcptr,
                                     mpfr_srcptr, mp_rnd_t)) {
  tree result = NULL_TREE;
...
static tree do_mpfr_sincos(tree arg, tree arg_sinp, tree arg_cosp) {
  tree const type = TREE_TYPE(arg);
  tree result = NULL_TREE;
    ...
    if (real_isfinite(ra)) {
      ...
      if (result_s && result_c) {
        ...
        if (!arg_sinp && !arg_cosp)
          return build_complex(build_complex_type(type), result_c, result_s);
        ...
        if (TYPE_MAIN_VARIANT(TREE_TYPE(arg_sinp)) == TYPE_MAIN_VARIANT(type) &&
          ...
          result =
              non_lvalue(fold_build2(COMPOUND_EXPR, type, result_s, result_c));
        }
      }
    }
  }
...
/* If argument ARG1 is an INTEGER_CST and ARG2 is a REAL_CST, call the
   ...
   requested precision.  */
static tree do_mpfr_bessel_n(tree arg1, tree arg2, tree type,
...
static tree do_mpfr_remquo(tree arg0, tree arg1, tree arg_quo) {
    ...
    if (real_isfinite(ra0) && real_isfinite(ra1)) {
      ...
      if (result_rem) {
        ...
        if (TYPE_MAIN_VARIANT(TREE_TYPE(arg_quo)) == integer_type_node) {
          ...
          result = non_lvalue(
              fold_build2(COMPOUND_EXPR, type, result_quo, result_rem));
        }
      }
    }
  }
...
static tree do_mpfr_lgamma_r(tree arg, tree arg_sg, tree type) {
  tree result = NULL_TREE;
  ...
  if (REAL_MODE_FORMAT(TYPE_MODE(type))->b == 2 && TREE_CODE(arg) == REAL_CST &&
      !TREE_OVERFLOW(arg) && TREE_CODE(TREE_TYPE(arg_sg)) == POINTER_TYPE &&
    ...
    /* In addition to NaN and Inf, the argument cannot be zero or a
       negative integer.  */
    if (real_isfinite(ra) && ra->cl != rvc_zero &&
      ...
      if (result_lg) {
        ...
        result_sg = fold_build2(MODIFY_EXPR, TREE_TYPE(arg_sg), arg_sg,
                                build_int_cst(NULL, sg));
        TREE_SIDE_EFFECTS(result_sg) = 1;
        /* Combine the signgam assignment with the lgamma result.  */
        result =
            non_lvalue(fold_build2(COMPOUND_EXPR, type, result_sg, result_lg));
      }
    }
  }
...
static tree gimple_rewrite_call_expr(gimple stmt, int skip, tree fndecl, int n,
                                     ...) {
...
static tree gimple_fold_builtin_sprintf_chk(gimple stmt,
                                            enum built_in_function fcode) {
  tree dest, size, len, fn, fmt, flag;
  const char *fmt_str;
...
tree gimple_fold_builtin_snprintf_chk(gimple stmt, tree maxlen,
                                      enum built_in_function fcode) {
  tree dest, size, len, fn, fmt, flag;
  const char *fmt_str;
...
static tree gimple_fold_builtin_varargs(tree fndecl, gimple stmt,
                                        bool ignore ATTRIBUTE_UNUSED) {
  enum built_in_function fcode = DECL_FUNCTION_CODE(fndecl);
  tree ret = NULL_TREE;
...
tree fold_call_stmt(gimple stmt, bool ignore) {
  tree ret = NULL_TREE;
  tree fndecl = gimple_call_fndecl(stmt);
  if (fndecl && TREE_CODE(fndecl) == FUNCTION_DECL && DECL_BUILT_IN(fndecl) &&
