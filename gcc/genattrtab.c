...
#if 0
#define strcmp_check(S1, S2) \
  ((S1) == (S2) ? 0 : (gcc_assert(strcmp((S1), (S2))), 1))
#else
...
#define SIMPLIFY_TEST_EXP(EXP, INSN_CODE, INSN_INDEX)        \
  (ATTR_IND_SIMPLIFIED_P(EXP) || ATTR_CURR_SIMPLIFIED_P(EXP) \
       ? (EXP)                                               \
...
static void write_attr_set(struct attr_desc *, int, rtx, const char *,
                           const char *, rtx, int, int);
static void write_attr_case(struct attr_desc *, struct attr_value *, int,
                            const char *, const char *, int, rtx);
...
static rtx attr_rtx_1(enum rtx_code code, va_list p) {
  ...
  if (GET_RTX_CLASS(code) == RTX_UNARY) {
    ...
    for (h = attr_hash_table[hashcode % RTL_HASH_SIZE]; h; h = h->next)
      if (h->hashcode == hashcode && GET_CODE(h->u.rtl) == code &&
          XEXP(h->u.rtl, 0) == arg0)
        return h->u.rtl;
...
static char *attr_printf(unsigned int len, const char *fmt, ...) {
  char str[256];
  va_list p;
...
static int attr_equal_p(rtx x, rtx y) {
  return (x == y ||
          (!(ATTR_PERMANENT_P(x) && ATTR_PERMANENT_P(y)) && rtx_equal_p(x, y)));
}
...
static rtx check_attr_test(rtx exp, int is_const, int lineno) {
  struct attr_desc *attr;
  struct attr_value *av;
  ...
  switch (GET_CODE(exp)) {
    case EQ_ATTR:
      ...
      if (XSTR(exp, 1)[0] == '!')
        ...
        if (attr->is_numeric) {
          ...
          if (av == NULL)
            fatal("unknown value `%s' for `%s' attribute", XSTR(exp, 1),
                  XSTR(exp, 0));
        }
      } else {
        if (!strcmp(XSTR(exp, 0), "alternative")) {
    ...
    case EQ:
      if (GET_CODE(XEXP(exp, 0)) == SYMBOL_REF &&
        ...
        exp =
            attr_rtx(GET_CODE(exp), attr_rtx(SYMBOL_REF, XSTR(XEXP(exp, 0), 0)),
  ...
  switch (GET_CODE(exp)) {
    ...
    case COND:
      ...
      for (i = 0; i < XVECLEN(exp, 0); i += 2) {
        XVECEXP(exp, 0, i) =
            check_attr_test(XVECEXP(exp, 0, i), attr ? attr->is_const : 0,
                            attr ? attr->lineno : 0);
    ...
    case ATTR: {
      ...
      if (attr2 == NULL) {
        ...
        have_error = 1;
      } else if (attr && attr->is_numeric != attr2->is_numeric) {
        message_with_line(attr->lineno,
...
static rtx convert_set_attr(rtx exp, struct insn_def *id) {
  ...
  if (n == 1)
    return attr_rtx(SET, attr_rtx(ATTR, XSTR(exp, 0)),
...
static struct attr_value *get_attr_value(rtx value, struct attr_desc *attr,
                                         int insn_code) {
  struct attr_value *av;
  int num_alt = 0;
...
static void expand_delays(void) {
  ...
  for (delay = delays; delay; delay = delay->next) {
    for (i = 0; i < XVECLEN(delay->def, 1); i += 3) {
      ...
      if (condexp == 0)
        condexp = false_rtx;
      newexp = attr_rtx(IF_THEN_ELSE, condexp, make_numeric_value(1),
...
static rtx substitute_address(rtx exp, rtx (*no_address_fn)(rtx),
                              rtx (*address_fn)(rtx)) {
  int i;
  rtx newexp;
...
static void make_length_attrs(void) {
  static const char *new_names[] = {"*insn_default_length", "*insn_min_length",
                                    "*insn_variable_length_p",
                                    "*insn_current_length"};
  static rtx (*const no_address_fn[])(rtx) = {identity_fn, identity_fn, zero_fn,
                                              zero_fn};
  static rtx (*const address_fn[])(rtx) = {max_fn, min_fn, one_fn, identity_fn};
  size_t i;
  ...
  for (i = 0; i < ARRAY_SIZE(new_names); i++) {
    ...
    for (av = length_attr->first_value; av; av = av->next)
      for (ie = av->first_insn; ie; ie = ie->next) {
        new_av = get_attr_value(
            substitute_address(av->value, no_address_fn[i], address_fn[i]),
...
static rtx simplify_cond(rtx exp, int insn_code, int insn_index) {
  ...
  for (i = 0; i < len; i += 2) {
    ...
    if (newtest == true_rtx) {
      ...
      tests[i - 2] =
          insert_right_side(IOR, tests[i - 2], newtest, insn_code, insn_index);
...
static rtx insert_right_side(enum rtx_code code, rtx exp, rtx term,
...
static rtx evaluate_eq_attr(rtx exp, rtx value, int insn_code, int insn_index) {
  ...
  switch (GET_CODE(value)) {
    ...
    case SYMBOL_REF: {
      ...
      gcc_assert(strlen(XSTR(exp, 0)) + strlen(XSTR(exp, 1)) + 2 <= 256);
      ...
      newexp =
          attr_rtx(EQ, value, attr_rtx(SYMBOL_REF, DEF_ATTR_STRING(string)));
      break;
    }
    ...
    case COND:
      ...
      for (i = 0; i < XVECLEN(value, 0); i += 2) {
        ...
        right =
            insert_right_side(AND, andexp, this_cond, insn_code, insn_index);
        right =
            insert_right_side(AND, right,
      ...
      right = insert_right_side(
          AND, andexp,
          evaluate_eq_attr(exp, XEXP(value, 1), insn_code, insn_index),
          insn_code, insn_index);
...
static rtx simplify_and_tree(rtx exp, rtx *pterm, int insn_code,
                             int insn_index) {
  rtx left, right;
  rtx newexp;
  rtx temp;
...
static rtx simplify_or_tree(rtx exp, rtx *pterm, int insn_code,
                            int insn_index) {
  rtx left, right;
  rtx newexp;
  rtx temp;
...
static bool attr_alt_subset_p(rtx s1, rtx s2) {
...
static rtx simplify_test_exp(rtx exp, int insn_code, int insn_index) {
  ...
  switch (GET_CODE(exp)) {
    case AND:
      ...
      if ((GET_CODE(left) == IOR || GET_CODE(right) == IOR) &&
        ...
        newexp = attr_rtx(IOR, attr_rtx(AND, left, XEXP(right, 0)),
      ...
      if (insn_code >= 0 && (GET_CODE(left) == AND || left_alt ||
                             GET_CODE(right) == AND || right_alt)) {
    ...
    case IOR:
      ...
      /* See if all or all but one of the insn's alternatives are specified
         in this tree.  Optimize if so.  */
               ...
               (GET_CODE(left) == IOR ||
                (GET_CODE(left) == EQ_ATTR_ALT && !XINT(left, 1)) ||
                (GET_CODE(left) == EQ_ATTR &&
                 XSTR(left, 0) == alternative_name) ||
                GET_CODE(right) == IOR ||
                (GET_CODE(right) == EQ_ATTR_ALT && !XINT(right, 1)) ||
    ...
    case EQ_ATTR:
      ...
      if (insn_code >= 0 && (attr = find_attr(&XSTR(exp, 0), 0)) != NULL) {
        rtx x;
  ...
  if (insn_code != -2 && !ATTR_IND_SIMPLIFIED_P(newexp))
  ...
  for (i = -2; i < insn_code_number; i++) {
    ...
    for (iv = insn_code_values[i]; iv; iv = iv->next) {
      ...
      while (GET_CODE(newexp) == COND) {
        rtx newexp2 =
            simplify_cond(newexp, ie->def->insn_code, ie->def->insn_index);
        if (newexp2 == newexp)
          break;
        newexp = newexp2;
      }
...
static void gen_delay(rtx def, int lineno) {
  ...
  if (XVECLEN(def, 1) % 3 != 0) {
    message_with_line(
        lineno, "number of elements in DEFINE_DELAY must be multiple of three");
    have_error = 1;
    return;
  }
  ...
  switch (code) {
    ...
    case EQ_ATTR_ALT: {
      ...
      if (!(set & (set - 1))) {
        ...
        printf("which_alternative %s= %d", XINT(exp, 1) ? "!" : "=", bit);
      } else {
        printf("%s((1 << which_alternative) & 0x%x)", XINT(exp, 1) ? "!" : "",
               set);
      }
    } break;
    ...
    case EQ_ATTR:
      ...
      if (attr->is_const) {
        write_test_expr(evaluate_eq_attr(exp, attr->default_val->value, -2, -2),
                        flags);
      } else {
        if (flags & 2)
    ...
    case MATCH_DUP:
      printf(
          ...
          "(operands[%d]) == LABEL_REF ? XEXP (operands[%d], 0) : "
          "operands[%d])) : 0",
...
static int min_attr_value(rtx exp, int *unknownp) {
  int current_min;
  int i, n;
  ...
  if (attr->name[0] == '*')
    ...
    for (av = attr->first_value; av; av = av->next)
      if (av->num_insns == 1)
        ...
        write_attr_set(attr, 2, av->value, "return", ";", true_rtx, -2, 0);
...
static rtx eliminate_known_true(rtx known_true, rtx exp, int insn_code,
                                int insn_index) {
  rtx term;
...
static void write_attr_set(struct attr_desc *attr, int indent, rtx value,
                           const char *prefix, const char *suffix,
                           rtx known_true, int insn_code, int insn_index) {
  if (GET_CODE(value) == COND) {
    ...
    for (i = 0; i < XVECLEN(value, 0); i += 2) {
      ...
      testexp = eliminate_known_true(our_known_true, XVECEXP(value, 0, i),
                                     insn_code, insn_index);
      newexp = attr_rtx(NOT, testexp);
      newexp =
          insert_right_side(AND, our_known_true, newexp, insn_code, insn_index);
      ...
      /* Compute the expression to pass to our recursive call as being
         known true.  */
      inner_true = insert_right_side(AND, our_known_true, testexp, insn_code,
                                     insn_index);
      ...
      write_attr_set(attr, indent + 4, XVECEXP(value, 0, i + 1), prefix, suffix,
    ...
    write_attr_set(attr, first_if ? indent : indent + 4, default_val, prefix,
                   suffix, our_known_true, insn_code, insn_index);
...
static void write_insn_cases(struct insn_ent *ie, int indent) {
  for (; ie != 0; ie = ie->next)
    if (ie->def->insn_code != -1) {
      write_indent(indent);
...
static void write_attr_case(struct attr_desc *attr, struct attr_value *av,
                            int write_case_lines, const char *prefix,
                            const char *suffix, int indent, rtx known_true) {
  if (av->num_insns == 0)
    return;
  ...
  if (av->num_insns == 1)
    ...
    write_attr_set(attr, indent + 2, av->value, prefix, suffix, known_true, -2,
                   0);
...
static void write_attr_value(struct attr_desc *attr, rtx value) {
  ...
  switch (GET_CODE(value)) {
    ...
    case ATTR: {
      ...
      printf("get_attr_%s (%s)", attr2->name, (attr2->is_const ? "" : "insn"));
    } break;
...
static void write_eligible_delay(const char *kind) {
  ...
  printf("int\n");
  printf(
      "eligible_for_%s (rtx delay_insn ATTRIBUTE_UNUSED, int slot, rtx "
...
static struct attr_desc *find_attr(const char **name_p, int create) {
  struct attr_desc *attr;
  int index;
...
static void make_automaton_attrs(void) {
  ...
  for (decl = all_insn_reservs, i = 0; decl; decl = decl->next, i += 2) {
  ...
  if (n_bypasses == 0)
    ...
    for (decl = all_insn_reservs, i = 0; decl; decl = decl->next)
      if (decl->bypassed) {
