...
static const unexpansion unexpansions[] = {{"struct _iobuf", "FILE"}, {0, 0}};
  ...
  while (len > 0) {
    ...
    if (written < 0) {
      ...
      notice("%s: error writing file '%s': %s\n", pname,
             shortpath(NULL, out_fname), xstrerror(errno_val));
      return;
    }
    /* Arithmetic on void pointers is a gcc extension.  */
...
static int is_syscalls_file(const file_info *fi_p) {
...
static int needs_to_be_converted(const file_info *file_p) {
  ...
  for (;;) {
    if (!inp[0])
      ...
      if (!inp[1])
        ...
        inp += 2;
        continue;
      } else if ((inp[1] == '.') && (inp[2] == 0 || IS_DIR_SEPARATOR(inp[2]))) {
...
static file_info *find_file(const char *filename, int do_not_stat) {
    ...
    if (do_not_stat)
      ...
      if (stat(filename, &stat_buf) == -1) {
        ...
        notice("%s: %s: can't get status: %s\n", pname,
               shortpath(NULL, filename), xstrerror(errno_val));
...
static void save_def_or_dec(const char *l, int is_syscalls) {
...

  {
    ...
    def_dec_p->file =
        find_file(abspath(invocation_filename, filename), is_syscalls);
  }
...

  {
    ...
    for (other = def_dec_p->hash_entry->ddip; other;
         other = other->next_for_func) {
...
static void munge_compile_params(const char *params_list) {
  ...
  const char **temp_params = alloca((strlen(params_list) + 8) * sizeof(char *));
  int param_count = 0;
  const char *param;
  struct stat st;
...
static int gen_aux_info_file(const char *base_filename) {
  ...
  if (!quiet_flag)
    notice("%s: compiling '%s'\n", pname,
           compile_params[input_file_name_index]);

  {
    char *errmsg_fmt, *errmsg_arg;
    int wait_status, pid;

    pid = pexecute(compile_params[0], (char *const *)compile_params, pname,
                   NULL, &errmsg_fmt, &errmsg_arg,
...
static void process_aux_info_file(const char *base_source_filename, int keep_it,
                                  int is_syscalls) {
  ...
  if (access(aux_info_filename, R_OK) == -1) {
    if (errno == ENOENT) {
      ...
      notice("%s: can't read aux info file '%s': %s\n", pname,
             shortpath(NULL, aux_info_filename), xstrerror(errno_val));
      errors++;
      return;
    }
  }
...

  {
    ...
    if (!is_syscalls) {
      ...
      if (stat(base_source_filename, &stat_buf) == -1) {
        int errno_val = errno;
        notice("%s: can't get status of aux info file '%s': %s\n", pname,
               shortpath(NULL, base_source_filename), xstrerror(errno_val));
        errors++;
        return;
      }
...
static void rename_c_file(const hash_table_entry *hp) {
  const char *filename = hp->symbol;
  int last_char_index = strlen(filename) - 1;
  char *const new_filename =
      alloca(strlen(filename) + strlen(cplus_suffix) + 1);
  ...
  if (filename[last_char_index] != 'c' ||
      filename[last_char_index - 1] != '.' ||
  ...
  if (rename(filename, new_filename) == -1) {
    ...
    notice("%s: warning: can't rename file '%s' to '%s': %s\n", pname,
           shortpath(NULL, filename), shortpath(NULL, new_filename),
           xstrerror(errno_val));
    errors++;
    return;
  }
...
static void reverse_def_dec_list(const hash_table_entry *hp) {
  ...
  for (dd_p = head; dd_p; dd_p = dd_p->next_for_func)
    if (dd_p->is_func_def && !dd_p->is_static) {
      if (!extern_def_p)     /* Previous definition? */
        ...
        if (!quiet_flag)
          fprintf(stderr, "%s:     %s(%d): %s\n", pname,
                  shortpath(NULL, dd_p->file->hash_entry->symbol), dd_p->line,
                  dd_p->ansi_decl);
      }
    }
      ...
      if (!dd_p->is_func_def && !dd_p->is_static && dd_p->prototyped) {
        ...
        if (!quiet_flag)
          notice(
              ...
              pname, shortpath(NULL, dd_p->file->hash_entry->symbol),
...
static const def_dec_info *find_static_definition(const def_dec_info *user) {
...
static void connect_defs_and_decs(const hash_table_entry *hp) {
  const def_dec_info *dd_p;
  const def_dec_info *extern_def_p = NULL;
  ...
  for (dd_p = hp->ddip; dd_p; dd_p = dd_p->next_for_func)
    if (!dd_p->is_func_def && dd_p->is_static && !dd_p->definition) {
      ...
      for (dd_p2 = dd_p->next_for_func; dd_p2; dd_p2 = dd_p2->next_for_func)
        if (!dd_p2->is_func_def && dd_p2->is_static && !dd_p2->definition &&
            (dd_p2->file == dd_p->file))
...
static void declare_source_confusing(const char *clean_p) {
  if (!quiet_flag) {
    if (clean_p == 0)
      ...
      notice("%s: %d: warning: source too confusing\n",
             shortpath(NULL, convert_filename), identify_lineno(clean_p));
  }
...
static void output_up_to(const char *p) {
  ...
  const char *copy_start =
      orig_text_base + (clean_read_ptr - clean_text_base) + 1;
...
static int other_variable_style_function(const char *ansi_header) {
  ...
  for (p = ansi_header; p;) {
    ...
    if ((candidate = strstr(p, varargs_style_indicator)) == 0)
      return 0;
    else if (!is_id_char(candidate[-1]) && !is_id_char(candidate[len]))
      return 1;
    else
      p = candidate + 1;
  }
...

    {
      ...
      if ((*another_r_paren != ')') ||
        ...
        if (this_f_list_chain_item) {
          if (!quiet_flag)
            notice(
                "\n%s: warning: too few parameter lists in declaration of "
                "'%s'\n",
                pname, def_dec_p->hash_entry->symbol);
...
static int edit_formals_lists(const char *end_formals,
  ...
  if (f_list_count == 0) {
    ...
    for (func_name_start = func_name_limit++; is_id_char(*func_name_start);
         func_name_start--)
    ...
    if (func_name_len != strlen(expected) ||
      ...
      notice("%s: %d: warning: found '%s' but expected '%s'\n",
             ...
             dupnstr(func_name_start, func_name_len), expected);
      return 1;
    }
  }
  ...
  if (other_variable_style_function(def_dec_p->ansi_decl)) {
    if (!quiet_flag)
      notice("%s: %d: warning: definition of %s not converted\n",
             shortpath(NULL, def_dec_p->file->hash_entry->symbol),
             identify_lineno(end_formals), other_var_style);
  ...
  for (scan_p = new_clean_text_base; scan_p < new_clean_text_limit; scan_p++) {
    switch (*scan_p) {
      ...
      case '\'': /* Handle character literals.  */
        non_whitespace_since_newline = 1;
        while (scan_p[1] != '\'' || scan_p[0] == '\\') {
          if (scan_p[0] == '\\' && !ISSPACE((const unsigned char)scan_p[1]))
            scan_p[1] = ' ';
          if (!ISSPACE((const unsigned char)*scan_p))
            *scan_p = ' ';
          ++scan_p;
...
static const char *careful_find_l_paren(const char *p) {
  ...
  for (paren_depth = 1, q = p - 1; paren_depth;
       check_source(--q >= clean_text_base, 0)) {
    switch (*q) {
      case ')':
        paren_depth++;
        break;
...
static void scan_for_missed_items(const file_info *file_p) {
...
static void edit_file(const hash_table_entry *hp) {
  struct stat stat_buf;
  ...
  if (stat(convert_filename, &stat_buf) == -1) {
    ...
    notice("%s: can't get status for file '%s': %s\n", pname,
           shortpath(NULL, convert_filename), xstrerror(errno_val));
    return;
  }
  ...
  if (!nosave_flag) {
      ...
      else if (rename (convert_filename, new_filename) == -1)
	{
      ...
      notice("%s: can't link file '%s' to '%s': %s\n", pname,
             shortpath(NULL, convert_filename), shortpath(NULL, new_filename),
             xstrerror(errno_val));
      return;
	}
  }
...
static void do_processing(void) {
  ...
  if (nondefault_syscalls_dir) {
      syscalls_absolute_filename = xmalloc(strlen(nondefault_syscalls_dir) + 1 +
      ...
      syscalls_absolute_filename =
          xmalloc(strlen(default_syscalls_dir) + 0 + strlen(target_machine) +
                  1 + strlen(target_version) + 1 + sizeof(syscalls_filename));
...
static const struct option longopts[] = {{"version", 0, 0, 'V'},
                                         {"file_name", 0, 0, 'p'},
                                         {"quiet", 0, 0, 'q'},
                                         {"silent", 0, 0, 'q'},
                                         {"force", 0, 0, 'f'},
...
int main(int argc, char **const argv) {
  ...
  base_source_filenames = xmalloc((n_base_source_files + 1) * sizeof(char *));
     ...
     identifier token, delete it from the `varargs_style_indicator'.  */
  {
      ...
      if (*cp != 0)
        varargs_style_indicator =
            savestring(varargs_style_indicator, cp - varargs_style_indicator);
  }
#endif /* !defined (UNPROTOIZE) */
  ...
  if (errors)
      ...
      if (version_flag)
        fprintf(stderr, "%s %s%s\n", pname, pkgversion_string, version_string);
      do_processing();
  }
