...
static int copy_cost(rtx x, enum machine_mode mode, enum reg_class rclass,
                     bool to_p, secondary_reload_info *prev_sri) {
...
/* Record the cost of using memory or hard registers of various
   ...
   the alternatives.  */
static void record_reg_classes(int n_alts, int n_ops, rtx *ops,
                               enum machine_mode *modes,
                               const char **constraints, rtx insn,
  ...
  for (alt = 0; alt < n_alts; alt++) {
    ...
    for (i = 0; i < n_ops; i++) {
      ...
      if (p[0] >= '0' && p[0] <= '0' + i && (p[1] == ',' || p[1] == 0)) {
        ...
        if (!REG_P(op) || REGNO(op) < FIRST_PSEUDO_REGISTER) {
        ...
        } else if (!REG_P(ops[j]) || REGNO(ops[j]) < FIRST_PSEUDO_REGISTER) {
          ...
          for (k = 0; k < cost_classes_num; k++) {
            ...
            pp->cost[k] =
                ((recog_data.operand_type[i] != OP_OUT
                      ? ira_may_move_in_cost[mode][rclass][classes[i]] *
                            frequency
                      : 0) +
          ...
          /* If the alternative actually allows memory, make
             ...
             insn to load it.  */
          pp->mem_cost = ((recog_data.operand_type[i] != OP_IN
          ...
          /* If we have assigned a class to this allocno in our
             ...
             approximation.  */
          if (allocno_pref) {
            enum reg_class pref_class = allocno_pref[ALLOCNO_NUM(
                ira_curr_regno_allocno_map[REGNO(op)])];

            if (pref_class == NO_REGS)
              alt_cost += ((recog_data.operand_type[i] != OP_IN
                                ? ira_memory_move_cost[mode][classes[i]][0]
                                : 0) +
                           (recog_data.operand_type[i] != OP_OUT
                                ? ira_memory_move_cost[mode][classes[i]][1]
                                : 0));
            else if (ira_reg_class_intersect[pref_class][classes[i]] == NO_REGS)
              alt_cost +=
                  (ira_register_move_cost[mode][pref_class][classes[i]]);
          }
      ...
      while ((c = *p)) {
        switch (c) {
          ...
          case 'p':
            ...
            classes[i] = ira_reg_class_union[classes[i]][base_reg_class(
          ...
          case 'g':
            if (MEM_P(op) ||
                (CONSTANT_P(op) && (!flag_pic || LEGITIMATE_PIC_OPERAND_P(op))))
              win = 1;
            insn_allows_mem[i] = allows_mem[i] = 1;
          case 'r':
      ...
      /* Otherwise, if this alternative wins, either because we
         ...
         alternative.  */
               ...
               (REG_P(op) && reg_fits_class_p(op, classes[i], 0, GET_MODE(op))))
        ;
    ...
    for (i = 0; i < n_ops; i++)
      if (REG_P(ops[i]) && REGNO(ops[i]) >= FIRST_PSEUDO_REGISTER) {
        ...
        pp->mem_cost = MIN(pp->mem_cost, (qq->mem_cost + op_cost_add) * scale);

        for (k = 0; k < cost_classes_num; k++)
          pp->cost[k] = MIN(pp->cost[k], (qq->cost[k] + op_cost_add) * scale);
      }
  }
  ...
  /* If this insn is a single set copying operand 1 to operand 0 and
     ...
     instruction.  */
  if ((set = single_set(insn)) != 0 && ops[0] == SET_DEST(set) &&
      ops[1] == SET_SRC(set) && REG_P(ops[0]) && REG_P(ops[1]) &&
...
static inline bool ok_for_index_p_nonstrict(rtx reg) {
...
/* A version of regno_ok_for_base_p for use here, when all
   ...
   regno_ok_for_base_p.  */
static inline bool ok_for_base_p_nonstrict(rtx reg, enum machine_mode mode,
...
static void record_address_regs(enum machine_mode mode, rtx x, int context,
                                enum rtx_code outer_code,
  ...
  switch (code) {
    ...
    case PLUS:
         ...
         sure the register is being used as a pointer.  */
      {
        ...
        /* If both operands are registers but one is already a hard
          ...
          record_address_regs(
              mode, arg1,
              ok_for_base_p_nonstrict(arg0, mode, PLUS, REG) ? 1 : 0, PLUS, REG,
              scale);
        else if (code0 == REG && code1 == REG &&
          ...
          record_address_regs(
              mode, arg0,
              ok_for_base_p_nonstrict(arg1, mode, PLUS, REG) ? 1 : 0, PLUS, REG,
              scale);
    ...
    case PRE_MODIFY:
      ...
      if (REG_P(XEXP(XEXP(x, 1), 1)))
        record_address_regs(mode, XEXP(XEXP(x, 1), 1), 1, code, REG, 2 * scale);
      break;
    ...
    case PRE_DEC:
      ...
      if (REG_P(XEXP(x, 0)) && REGNO(XEXP(x, 0)) >= FIRST_PSEUDO_REGISTER)
        in_inc_dec[ALLOCNO_NUM(ira_curr_regno_allocno_map[REGNO(XEXP(x, 0))])] =
            true;
#endif
    ...
    case REG: {
      ...
      pp = COSTS_OF_ALLOCNO(allocno_costs,
                            ALLOCNO_NUM(ira_curr_regno_allocno_map[REGNO(x)]));
      ...
      for (k = 0; k < cost_classes_num; k++) {
        i = cost_classes[k];
        pp->cost[k] += (ira_may_move_in_cost[Pmode][i][rclass] * scale) / 2;
      }
    } break;
    ...
    default: {
      ...
      for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--)
        if (fmt[i] == 'e')
          record_address_regs(mode, XEXP(x, i), context, code, SCRATCH, scale);
    }
  }
}

/* Calculate the costs of insn operands.  */
static void record_operand_costs(rtx insn, struct costs **op_costs,
                                 enum reg_class *allocno_pref) {
  ...
  for (i = 0; i < recog_data.n_operands; i++) {
    ...
    if (MEM_P(recog_data.operand[i]))
      record_address_regs(GET_MODE(recog_data.operand[i]),
                          XEXP(recog_data.operand[i], 0), 0, MEM, SCRATCH,
                          frequency * 2);
    else if (constraints[i][0] == 'p' ||
             EXTRA_ADDRESS_CONSTRAINT(constraints[i][0], constraints[i]))
      record_address_regs(VOIDmode, recog_data.operand[i], 0, ADDRESS, SCRATCH,
                          frequency * 2);
  }
  ...
  for (i = 0; i < (int)recog_data.n_operands - 1; i++)
    if (constraints[i][0] == '%') {
      ...
      record_reg_classes(recog_data.n_alternatives, recog_data.n_operands,
                         recog_data.operand, modes, xconstraints, insn,
                         op_costs, allocno_pref);
    }
  record_reg_classes(recog_data.n_alternatives, recog_data.n_operands,
                     recog_data.operand, modes, constraints, insn, op_costs,
                     allocno_pref);
}
...
static rtx scan_one_insn(rtx insn) {
  ...
  if (set != 0 && REG_P(SET_DEST(set)) && MEM_P(SET_SRC(set)) &&
    ...
    record_address_regs(GET_MODE(SET_SRC(set)), XEXP(SET_SRC(set), 0), 0, MEM,
                        SCRATCH, frequency * 2);
  }
  ...
  for (i = 0; i < recog_data.n_operands; i++)
    if (REG_P(recog_data.operand[i]) &&
      ...
      struct costs *p = COSTS_OF_ALLOCNO(
          allocno_costs, ALLOCNO_NUM(ira_curr_regno_allocno_map[regno]));
...
static void print_costs(FILE *f) {
  int k;
  ira_allocno_t a;
...
static void find_allocno_class_costs(void) {
  ...
  for (pass = 0; pass <= flag_expensive_optimizations; pass++) {
    ...
    for (i = 0; i < N_REG_CLASSES; i++)
      cost_class_nums[i] = -1;
    for (cost_classes_num = 0; cost_classes_num < ira_important_classes_num;
         cost_classes_num++) {
      cost_classes[cost_classes_num] = ira_important_classes[cost_classes_num];
    ...
    ira_traverse_loop_tree(true, ira_loop_tree_root, process_bb_node_for_costs,
                           NULL);
    ...
    for (i = max_reg_num() - 1; i >= FIRST_PSEUDO_REGISTER; i--) {
      ...
      if (pass == flag_expensive_optimizations) {
        ...
        if (internal_flag_ira_verbose > 2 && ira_dump_file != NULL)
          fprintf(ira_dump_file, "    r%d: preferred %s, alternative %s\n", i,
                  reg_class_names[best], reg_class_names[alt_class]);
      }
      if (best_cost > temp_costs->mem_cost)
          ...
          for (k = 0; k < cost_classes_num; k++) {
            ...
            if (!contains_reg_of_mode[rclass][PSEUDO_REGNO_MODE(i)]
              ...
              ;
                     ...
                     best_cost) {
              best_cost = COSTS_OF_ALLOCNO(total_costs, a_num)->cost[k];
              allocno_cost = COSTS_OF_ALLOCNO(allocno_costs, a_num)->cost[k];
              best = (enum reg_class)rclass;
            } else if (COSTS_OF_ALLOCNO(total_costs, a_num)->cost[k] ==
                       best_cost) {
              best = ira_reg_class_union[best][rclass];
...
static void process_bb_node_for_hard_reg_moves(
  ...
  FOR_BB_INSNS(bb, insn) {
    ...
    ira_allocate_and_set_costs(&ALLOCNO_CONFLICT_HARD_REG_COSTS(a), rclass, 0);
    ALLOCNO_HARD_REG_COSTS(a)[i] -= cost;
    ALLOCNO_CONFLICT_HARD_REG_COSTS(a)[i] -= cost;
    ALLOCNO_COVER_CLASS_COST(a) =
        MIN(ALLOCNO_COVER_CLASS_COST(a), ALLOCNO_HARD_REG_COSTS(a)[i]);
  }
}
...
static void setup_allocno_cover_class_and_costs(void) {
  ...
  FOR_EACH_ALLOCNO(a, ai) {
    ...
    if (optimize && ALLOCNO_COVER_CLASS(a) != allocno_pref[i]) {
      ...
      ALLOCNO_HARD_REG_COSTS(a) = reg_costs =
...
void ira_costs(void) {
  ...
  total_costs =
      (struct costs *)ira_allocate(max_struct_costs_size * ira_allocnos_num);
  allocno_pref_buffer =
      (enum reg_class *)ira_allocate(sizeof(enum reg_class) * ira_allocnos_num);
  common_classes =
      (enum reg_class *)ira_allocate(sizeof(enum reg_class) * max_reg_num());
...
void ira_tune_allocno_costs_and_cover_classes(void) {
  int j, n, regno;
  ...
  FOR_EACH_ALLOCNO(a, ai) {
    ...
    if (ALLOCNO_CALLS_CROSSED_NUM(a) != 0) {
      ira_allocate_and_set_costs(&ALLOCNO_HARD_REG_COSTS(a), cover_class,
                                 ALLOCNO_COVER_CLASS_COST(a));
      ...
      for (j = n - 1; j >= 0; j--) {
        ...
        if (!ira_hard_reg_not_in_set_p(regno, mode, call_used_reg_set) ||
          ...
          cost +=
              (ALLOCNO_CALL_FREQ(a) * (ira_memory_move_cost[mode][rclass][0] +
                                       ira_memory_move_cost[mode][rclass][1]));
#ifdef IRA_HARD_REGNO_ADD_COST_MULTIPLIER
        cost +=
            ((ira_memory_move_cost[mode][rclass][0] +
              ira_memory_move_cost[mode][rclass][1]) *
             ALLOCNO_FREQ(a) * IRA_HARD_REGNO_ADD_COST_MULTIPLIER(regno) / 2);
#endif
        reg_costs[j] += cost;
