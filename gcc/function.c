...
bool frame_offset_overflow(HOST_WIDE_INT offset, tree func) {
...
rtx assign_stack_local_1(enum machine_mode mode, HOST_WIDE_INT size, int align,
    ...
    if (crtl->stack_alignment_estimated < alignment_in_bits) {
      if (!crtl->stack_realign_processed)
        ...
        if (!crtl->stack_realign_needed) {
          ...
          gcc_assert(
              reduce_alignment_ok || size == 0 ||
              (crtl->stack_alignment_estimated >= GET_MODE_ALIGNMENT(mode)));
  ...
  if (STACK_ALIGNMENT_NEEDED || mode != BLKmode || size != 0) {
    ...
    if (FRAME_GROWS_DOWNWARD)
      frame_offset = (FLOOR_ROUND(frame_offset - frame_phase,
                                  (unsigned HOST_WIDE_INT)alignment) +
                      frame_phase);
    else
      frame_offset = (CEIL_ROUND(frame_offset - frame_phase,
                                 (unsigned HOST_WIDE_INT)alignment) +
                      frame_phase);
  }
  ...
  if (virtuals_instantiated)
    addr = plus_constant(
        frame_pointer_rtx,
        trunc_int_for_mode(
            frame_offset + bigend_correction + STARTING_FRAME_OFFSET, Pmode));
  else
    addr = plus_constant(
        virtual_stack_vars_rtx,
        trunc_int_for_mode(frame_offset + bigend_correction, Pmode));
...
static void cut_slot_from_list(struct temp_slot *temp,
                               struct temp_slot **list) {
  if (temp->next)
...
static void insert_slot_to_list(struct temp_slot *temp,
                                struct temp_slot **list) {
  temp->next = *list;
  if (*list)
    (*list)->prev = temp;
...
static struct temp_slot **temp_slots_at_level(int level) {
...
static void move_slot_to_level(struct temp_slot *temp, int level) {
...
static void make_slot_available(struct temp_slot *temp) {
  cut_slot_from_list(temp, temp_slots_at_level(temp->level));
...
static hashval_t temp_slot_address_compute_hash(
  ...
  int do_not_record = 0;
  return hash_rtx(t->address, GET_MODE(t->address), &do_not_record, NULL,
                  false);
}

/* Return the hash value for an address -> temp slot mapping.  */
static hashval_t temp_slot_address_hash(const void *p) {
...
static int temp_slot_address_eq(const void *p1, const void *p2) {
...
static void insert_temp_slot_address(rtx address, struct temp_slot *temp_slot) {
  void **slot;
...
static int remove_unused_temp_slot_addresses_1(void **slot,
...
static void remove_unused_temp_slot_addresses(void) {
  htab_traverse(temp_slot_address_table, remove_unused_temp_slot_addresses_1,
                NULL);
}
...
static struct temp_slot *find_temp_slot_from_address(rtx x) {
  ...
  t = (struct temp_slot_address_entry *)htab_find_with_hash(
      temp_slot_address_table, &tmp, tmp.hash);
  if (t)
    return t->temp_slot;
  ...
  if (GET_CODE(x) == PLUS && XEXP(x, 0) == virtual_stack_vars_rtx &&
...
rtx assign_stack_temp_for_type(enum machine_mode mode, HOST_WIDE_INT size,
                               int keep, tree type) {
  unsigned int align;
  ...
  if (selected == 0) {
    ...
    p->slot = assign_stack_local(
        mode,
        (mode == BLKmode ? CEIL_ROUND(size, (int)align / BITS_PER_UNIT) : size),
        align);
  ...
  if (type != 0) {
    ...
    MEM_SET_IN_STRUCT_P(
        slot, (AGGREGATE_TYPE_P(type) || TREE_CODE(type) == COMPLEX_TYPE));
  }
  MEM_NOTRAP_P(slot) = 1;
...
rtx assign_temp(tree type_or_decl, int keep, int memory_required,
  ...
  if (mode == BLKmode || memory_required) {
    ...
    if (decl && size == -1 && TREE_CODE(TYPE_SIZE_UNIT(type)) == INTEGER_CST) {
...
void init_temp_slots(void) {
  ...
  if (!temp_slot_address_table)
    temp_slot_address_table =
        htab_create_ggc(32, temp_slot_address_hash, temp_slot_address_eq, NULL);
  else
  ...
  ((ACCUMULATE_OUTGOING_ARGS                                 \
        ? (crtl->outgoing_args_size +                        \
           (OUTGOING_REG_PARM_STACK_SPACE(                   \
                (!(FNDECL) ? NULL_TREE : TREE_TYPE(FNDECL))) \
                ? 0                                          \
...
static int instantiate_virtual_regs_in_rtx(rtx *loc, void *data) {
  HOST_WIDE_INT offset;
...
static int safe_insn_predicate(int code, int operand, rtx x) {
...
static void instantiate_virtual_regs_in_insn(rtx insn) {
  HOST_WIDE_INT offset;
  int insn_code, i;
  ...
  if (set) {
    ...
    if (new_rtx && offset != 0 && REG_P(SET_DEST(set)) &&
        REGNO(SET_DEST(set)) > LAST_VIRTUAL_REGISTER) {
      start_sequence();

      x = expand_simple_binop(GET_MODE(SET_DEST(set)), PLUS, new_rtx,
                              GEN_INT(offset), SET_DEST(set), 1,
                              OPTAB_LIB_WIDEN);
      if (x != SET_DEST(set))
    ...
    if (GET_CODE(SET_SRC(set)) == PLUS && recog_data.n_operands >= 3 &&
      ...
      if (offset == 0 && REG_P(SET_DEST(set)) &&
          REGNO(SET_DEST(set)) > LAST_VIRTUAL_REGISTER) {
        start_sequence();
    ...
    switch (GET_CODE(x)) {
      ...
      case REG:
        ...
        if (offset == 0)
          ...
          x = expand_simple_binop(GET_MODE(x), PLUS, new_rtx, GEN_INT(offset),
...
void instantiate_decl_rtl(rtx x) {
  ...
  addr = XEXP(x, 0);
  if (CONSTANT_P(addr) ||
      (REG_P(addr) && (REGNO(addr) < FIRST_VIRTUAL_REGISTER ||
...
static tree instantiate_expr(tree *tp, int *walk_subtrees,
                             void *data ATTRIBUTE_UNUSED) {
  tree t = *tp;
  if (!EXPR_P(t)) {
    *walk_subtrees = 0;
...
int aggregate_value_p(const_tree exp, const_tree fntype) {
  int i, regno, nregs;
  rtx reg;
...
bool pass_by_reference(CUMULATIVE_ARGS *ca, enum machine_mode mode, tree type,
                       bool named_arg) {
  if (type) {
...
bool reference_callee_copied(CUMULATIVE_ARGS *ca, enum machine_mode mode,
...
static tree split_complex_args(tree args) {
  tree p;
...
static tree assign_parms_augmented_arg_list(struct assign_parm_data_all *all) {
...
static void assign_parm_find_data_types(struct assign_parm_data_all *all,
  ...
  if (pass_by_reference(&all->args_so_far, passed_mode, passed_type,
                        data->named_arg)) {
...
static void assign_parm_find_entry_rtl(struct assign_parm_data_all *all,
  ...
  if (targetm.calls.must_pass_in_stack(data->promoted_mode, data->passed_type))
    entry_parm = 0;
  ...
  if (entry_parm) {
    ...
    partial =
        targetm.calls.arg_partial_bytes(&all->args_so_far, data->promoted_mode,
                                        data->passed_type, data->named_arg);
...
static void assign_parm_find_stack_rtl(tree parm,
  ...
  if (data->promoted_mode != BLKmode &&
    ...
    if (MEM_EXPR(stack_parm) && MEM_OFFSET(stack_parm)) {
      int offset = subreg_lowpart_offset(DECL_MODE(parm), data->promoted_mode);
      if (offset)
...
static void assign_parm_adjust_entry_rtl(struct assign_parm_data_one *data) {
  ...
  if (data->partial != 0) {
    ...
    if (GET_CODE(entry_parm) == PARALLEL)
      emit_group_store(validize_mem(stack_parm), entry_parm, data->passed_type,
  ...
  /* When an argument is passed in multiple locations, we can't make use
  ...
  else if (GET_CODE(entry_parm) == PARALLEL && data->nominal_mode != BLKmode &&
    ...
    for (i = 0; i < len; i++)
      if (XEXP(XVECEXP(entry_parm, 0, i), 0) != NULL_RTX &&
          ...
          (GET_MODE(XEXP(XVECEXP(entry_parm, 0, i), 0)) == data->passed_mode) &&
...
static void assign_parm_adjust_stack_rtl(struct assign_parm_data_one *data) {
  ...
  else if (data->entry_parm == stack_parm && data->nominal_mode != BLKmode &&
  ...
  /* If stack protection is in effect for this function, don't leave any
     pointers in their passed stack slots.  */
           ...
           (flag_stack_protect == 2 || data->passed_pointer ||
...
static void assign_parm_setup_block(struct assign_parm_data_all *all, tree parm,
                                    struct assign_parm_data_one *data) {
  rtx entry_parm = data->entry_parm;
  rtx stack_parm = data->stack_parm;
  HOST_WIDE_INT size;
  ...
  if (stack_parm == 0) {
    ...
    stack_parm = assign_stack_local(BLKmode, size_stored, DECL_ALIGN(parm));
    ...
    /* If SIZE is that of a mode no bigger than a word, just use
      ...
      /* Blocks smaller than a word on a BYTES_BIG_ENDIAN
...
#ifdef BLOCK_REG_PADDING
               && (BLOCK_REG_PADDING(mode, data->passed_type, 1) == downward)
#else
               && BYTES_BIG_ENDIAN
#endif
      ) {
        ...
        x = expand_shift(LSHIFT_EXPR, word_mode, reg,
                         build_int_cst(NULL_TREE, by), NULL_RTX, 1);
      ...
      move_block_from_reg(REGNO(entry_parm), mem, size_stored / UNITS_PER_WORD);
...
static void assign_parm_setup_reg(struct assign_parm_data_all *all, tree parm,
                                  struct assign_parm_data_one *data) {
  rtx parmreg;
  ...
  if (data->nominal_mode != data->passed_mode ||
    ...
    if (GET_CODE(tempreg) == SUBREG &&
        GET_MODE(tempreg) == data->nominal_mode && REG_P(SUBREG_REG(tempreg)) &&
        data->nominal_mode == data->passed_mode &&
  ...
  if (data->nominal_mode == data->passed_mode && !did_conversion &&
      data->stack_parm != 0 && MEM_P(data->stack_parm) &&
      data->locate.offset.var == 0 &&
      reg_mentioned_p(virtual_incoming_args_rtx, XEXP(data->stack_parm, 0))) {
    rtx linsn = get_last_insn();
    rtx sinsn, set;

    /* Mark complex types separately.  */
    if (GET_CODE(parmreg) == CONCAT) {
      enum machine_mode submode = GET_MODE_INNER(GET_MODE(parmreg));
      int regnor = REGNO(XEXP(parmreg, 0));
      int regnoi = REGNO(XEXP(parmreg, 1));
      rtx stackr = adjust_address_nv(data->stack_parm, submode, 0);
      rtx stacki =
          adjust_address_nv(data->stack_parm, submode, GET_MODE_SIZE(submode));
  ...
  if (POINTER_TYPE_P(TREE_TYPE(parm)))
    mark_reg_pointer(parmreg, TYPE_ALIGN(TREE_TYPE(TREE_TYPE(parm))));
}

/* A subroutine of assign_parms.  Allocate stack space to hold the current
   parameter.  Get it there.  Perform all ABI specified conversions.  */

static void assign_parm_setup_stack(struct assign_parm_data_all *all, tree parm,
                                    struct assign_parm_data_one *data) {
  ...
  if (data->entry_parm != data->stack_parm) {
    ...
    if (data->stack_parm == 0) {
      int align =
          STACK_SLOT_ALIGNMENT(data->passed_type, GET_MODE(data->entry_parm),
    ...
    if (MEM_P(src)) {
      ...
      emit_block_move(dest, src, GEN_INT(int_size_in_bytes(data->passed_type)),
                      BLOCK_OP_NORMAL);
    } else
  ...
  for (parm = orig_fnargs; parm; parm = TREE_CHAIN(parm)) {
    if (TREE_CODE(TREE_TYPE(parm)) == COMPLEX_TYPE &&
      ...
      if (TREE_ADDRESSABLE(parm)) {
        ...
        int align = STACK_SLOT_ALIGNMENT(TREE_TYPE(parm), DECL_MODE(parm),
...
static void assign_parms(tree fndecl) {
  ...
  for (parm = fnargs; parm; parm = TREE_CHAIN(parm)) {
    ...
    if (SUPPORTS_STACK_ALIGNMENT) {
      unsigned int align =
          FUNCTION_ARG_BOUNDARY(data.promoted_mode, data.passed_type);
  ...
  if (SUPPORTS_STACK_ALIGNMENT) {
    if (DECL_RESULT(fndecl)) {
      ...
      if (mode != BLKmode && mode != VOIDmode && !AGGREGATE_TYPE_P(type)) {
...
#ifdef REG_PARM_STACK_SPACE
  crtl->args.size = MAX(crtl->args.size, REG_PARM_STACK_SPACE(fndecl));
#endif

  crtl->args.size = CEIL_ROUND(crtl->args.size, PARM_BOUNDARY / BITS_PER_UNIT);

#ifdef ARGS_GROW_DOWNWARD
  crtl->args.arg_offset_rtx =
      (all.stack_args_size.var == 0
           ? GEN_INT(-all.stack_args_size.constant)
  ...
  crtl->args.pops_args =
      RETURN_POPS_ARGS(fndecl, TREE_TYPE(fndecl), crtl->args.size);
  ...
  crtl->return_rtx =
      (DECL_RTL_SET_P(DECL_RESULT(fndecl)) ? DECL_RTL(DECL_RESULT(fndecl))
                                           : NULL_RTX);
  ...
  if (DECL_RTL_SET_P(DECL_RESULT(fndecl))) {
    ...
    if (REG_P(decl_rtl) ? REGNO(decl_rtl) >= FIRST_PSEUDO_REGISTER
      ...
      real_decl_rtl =
          targetm.calls.function_value(TREE_TYPE(decl_result), fndecl, true);
...
static tree gimplify_parm_type(tree *tp, int *walk_subtrees, void *data) {
  tree t = *tp;
...
gimple_seq gimplify_parameters(void) {
  ...
  for (parm = fnargs; parm; parm = TREE_CHAIN(parm)) {
    ...
    walk_tree_without_duplicates(&data.passed_type, gimplify_parm_type, &stmts);
      ...
      if (reference_callee_copied(&all.args_so_far, TYPE_MODE(type), type,
        ...
        if (TREE_CODE(DECL_SIZE_UNIT(parm)) == INTEGER_CST &&
            !(flag_stack_check == GENERIC_STACK_CHECK &&
              compare_tree_int(DECL_SIZE_UNIT(parm), STACK_CHECK_MAX_VAR_SIZE) >
                  0)) {
...
void locate_and_pad_parm(enum machine_mode passed_mode, tree type, int in_regs,
  ...
  sizetree = type ? size_in_bytes(type) : size_int(GET_MODE_SIZE(passed_mode));
  ...
  if (initial_offset_ptr->var)
    locate->slot_offset.var =
        size_binop(MINUS_EXPR, ssize_int(0), initial_offset_ptr->var);
  ...
  locate->size.constant =
      (-initial_offset_ptr->constant - locate->slot_offset.constant);
  if (initial_offset_ptr->var)
  ...
  if (!in_regs
    ...
    pad_to_arg_alignment(initial_offset_ptr, boundary, &locate->alignment_pad);
...
static void pad_to_arg_alignment(struct args_size *offset_ptr, int boundary,
  ...
  if (boundary > BITS_PER_UNIT) {
    if (offset_ptr->var) {
      ...
      tree offset =
          size_binop(PLUS_EXPR, ARGS_SIZE_TREE(*offset_ptr), sp_offset_tree);
#ifdef ARGS_GROW_DOWNWARD
...
static void pad_below(struct args_size *offset_ptr,
                      enum machine_mode passed_mode, tree sizetree) {
  if (passed_mode != BLKmode) {
...
static void setjmp_vars_warning(bitmap setjmp_crosses, tree block) {
  ...
  for (decl = BLOCK_VARS(block); decl; decl = TREE_CHAIN(decl)) {
    if (TREE_CODE(decl) == VAR_DECL && DECL_RTL_SET_P(decl) &&
        REG_P(DECL_RTL(decl)) &&
...
static void setjmp_args_warning(bitmap setjmp_crosses) {
  tree decl;
  for (decl = DECL_ARGUMENTS(current_function_decl); decl;
       decl = TREE_CHAIN(decl))
    if (DECL_RTL(decl) != 0 && REG_P(DECL_RTL(decl)) &&
        regno_clobbered_at_setjmp(setjmp_crosses, REGNO(DECL_RTL(decl))))
      warning(OPT_Wclobbered,
...
void generate_setjmp_warnings(void) {
  ...
  if (n_basic_blocks == NUM_FIXED_BLOCKS || bitmap_empty_p(setjmp_crosses))
    return;
...
static void reorder_blocks_1(rtx insns, tree current_block,
                             VEC(tree, heap) * *p_block_stack) {
  rtx insn;
  ...
  for (insn = insns; insn; insn = NEXT_INSN(insn)) {
    if (NOTE_P(insn)) {
      if (NOTE_KIND(insn) == NOTE_INSN_BLOCK_BEG) {
        ...
        origin = (BLOCK_FRAGMENT_ORIGIN(block) ? BLOCK_FRAGMENT_ORIGIN(block)
                                               : block);
        ...
        if (TREE_ASM_WRITTEN(block)) {
          ...
          BLOCK_FRAGMENT_CHAIN(new_block) = BLOCK_FRAGMENT_CHAIN(origin);
...
static int all_blocks(tree block, tree *vector) {
  ...
  while (block) {
    ...
    n_blocks +=
        all_blocks(BLOCK_SUBBLOCKS(block), vector ? vector + n_blocks : 0);
    block = BLOCK_CHAIN(block);
  }
...
static tree *get_block_vector(tree block, int *n_blocks_p) {
  tree *block_vector;
...
static void invoke_set_current_function_hook(tree fndecl) {
  if (!in_dummy_function) {
    tree opts = ((fndecl) ? DECL_FUNCTION_SPECIFIC_OPTIMIZATION(fndecl)
...
void allocate_struct_function(tree fndecl, bool abstract_p) {
  ...
  if (fndecl != NULL_TREE) {
    ...
    cfun->stdarg =
        (fntype && TYPE_ARG_TYPES(fntype) != 0 &&
...
void expand_main_function(void) {
     ...
     (!defined(HAS_INIT_SECTION) && !defined(INIT_SECTION_ASM_OP) && \
...
void expand_function_end(void) {
  ...
  if (DECL_RTL_SET_P(DECL_RESULT(current_function_decl))) {
    ...
    if (REG_P(decl_rtl) ? REGNO(decl_rtl) >= FIRST_PSEUDO_REGISTER
      ...
      if (TYPE_MODE(TREE_TYPE(decl_result)) != BLKmode &&
        ...
        emit_move_insn(gen_rtx_REG(GET_MODE(decl_rtl), REGNO(real_decl_rtl)),
                       decl_rtl);
      ...
      /* If a named return value dumped decl_return to memory, then
        ...
        if (targetm.calls.promote_function_return(
                TREE_TYPE(current_function_decl)))
          promote_mode(TREE_TYPE(decl_result), GET_MODE(decl_rtl), &unsignedp,
                       1);
        ...
        if (GET_CODE(decl_rtl) == PARALLEL)
          ...
          emit_group_load(real_decl_rtl, decl_rtl, TREE_TYPE(decl_result),
  ...
  if (cfun->returns_struct || cfun->returns_pcc_struct) {
    ...
    value_address = convert_memory_address(GET_MODE(outgoing), value_address);
...
static void record_insns(rtx insns, VEC(int, heap) * *vecp) {
  rtx tmp;
...
static void set_insn_locators(rtx insn, int loc) {
  while (insn != NULL_RTX) {
    if (INSN_P(insn))
...
static int contains(const_rtx insn, VEC(int, heap) * *vec) {
  int i, j;
  ...
  if (NONJUMP_INSN_P(insn) && GET_CODE(PATTERN(insn)) == SEQUENCE) {
    ...
    for (i = XVECLEN(PATTERN(insn), 0) - 1; i >= 0; i--)
      for (j = VEC_length(int, *vec) - 1; j >= 0; --j)
        if (INSN_UID(XVECEXP(PATTERN(insn), 0, i)) == VEC_index(int, *vec, j))
          count++;
    return count;
  } else {
    for (j = VEC_length(int, *vec) - 1; j >= 0; --j)
...
int prologue_epilogue_contains(const_rtx insn) {
...
int sibcall_epilogue_contains(const_rtx insn) {
  if (sibcall_epilogue)
...
static void emit_return_into_block(basic_block bb) {
...
static void thread_prologue_and_epilogue_insns(void) {
  int inserted = 0;
  edge e;
    ...
    if (last) {
      ...
      if (note == 0) {
        for (note = last; (note = NEXT_INSN(note));)
          if (NOTE_P(note) && NOTE_KIND(note) == NOTE_INSN_PROLOGUE_END)
            break;
      }
    ...
    if (last) {
      ...
      if (note == 0) {
        for (note = insn; (note = PREV_INSN(note));)
          if (NOTE_P(note) && NOTE_KIND(note) == NOTE_INSN_EPILOGUE_BEG)
            break;
      }
...
const char *current_function_assembler_name(void) {
  return IDENTIFIER_POINTER(DECL_ASSEMBLER_NAME(cfun->decl));
}

static unsigned int rest_of_handle_check_leaf_regs(void) {
...
static void used_types_insert_helper(tree type, struct function *func) {
  if (type != NULL && func != NULL) {
    ...
    if (func->used_types_hash == NULL)
      func->used_types_hash =
          htab_create_ggc(37, htab_hash_pointer, htab_eq_pointer, NULL);
...
static unsigned int rest_of_handle_thread_prologue_and_epilogue(void) {
  if (optimize)
...
struct rtl_opt_pass pass_thread_prologue_and_epilogue = {{
    RTL_PASS, "pro_and_epilogue",                /* name */
    NULL,                                        /* gate */
    ...
    TODO_dump_func | TODO_df_verify | TODO_df_finish | TODO_verify_rtl_sharing |
...
static void match_asm_constraints_1(rtx insn, rtx *p_sets, int noutputs) {
  int i;
  bool changed = false;
  ...
  for (i = 0; i < ninputs; i++) {
    ...
    if (!REG_P(output) || rtx_equal_p(output, input) ||
        (GET_MODE(input) != VOIDmode && GET_MODE(input) != GET_MODE(output)))
      continue;
    ...
    for (j = 0; j < noutputs; j++)
      if (!rtx_equal_p(SET_DEST(p_sets[j]), input) &&
        ...
        SET_DEST(p_sets[j]) = replace_rtx(SET_DEST(p_sets[j]), input, output);
    for (j = 0; j < ninputs; j++)
      if (reg_overlap_mentioned_p(input, RTVEC_ELT(inputs, j)))
        RTVEC_ELT(inputs, j) = replace_rtx(RTVEC_ELT(inputs, j), input, output);
...
static unsigned rest_of_match_asm_constraints(void) {
  basic_block bb;
  rtx insn, pat, *p_sets;
  int noutputs;
