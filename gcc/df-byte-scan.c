...
static bool df_compute_accessed_bytes_extract(df_ref ref, enum df_mm mm,
...
static bool df_compute_accessed_bytes_strict_low_part(df_ref ref,
                                                      unsigned int *start_byte,
                                                      unsigned int *last_byte) {
  int start;
  int last;
...
static bool df_compute_accessed_bytes_subreg(df_ref ref,
                                             unsigned int *start_byte,
                                             unsigned int *last_byte)
...
bool df_compute_accessed_bytes(df_ref ref, enum df_mm mm,
                               unsigned int *start_byte,
                               unsigned int *last_byte) {
  if (!dbg_cnt(df_byte_scan))
    return true;

  if (!DF_REF_REG_DEF_P(ref) && DF_REF_FLAGS_IS_SET(ref, DF_REF_READ_WRITE)) {
  ...
  if (DF_REF_FLAGS_IS_SET(ref, DF_REF_SIGN_EXTRACT | DF_REF_ZERO_EXTRACT))
    ...
    return df_compute_accessed_bytes_strict_low_part(ref, start_byte,
                                                     last_byte);
