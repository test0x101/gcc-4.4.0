...
void regstat_init_n_sets_and_refs(void) {
  ...
  regstat_n_sets_and_refs =
      XNEWVEC(struct regstat_n_sets_and_refs_t, max_regno);
...
static void regstat_bb_compute_ri(unsigned int bb_index, bitmap live,
...
static void regstat_bb_compute_calls_crossed(unsigned int bb_index,
                                             bitmap live) {
  basic_block bb = BASIC_BLOCK(bb_index);
  rtx insn;
  df_ref *def_rec;
