...
static tree record_reference(tree *tp, int *walk_subtrees,
...
static unsigned int build_cgraph_edges(void) {
  basic_block bb;
  ...
  for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    if (is_gimple_call(stmt) && (decl = gimple_call_fndecl(stmt))) {
      ...
      cgraph_create_edge(node, cgraph_node(decl), stmt, bb->count,
                         compute_call_stmt_bb_frequency(bb), bb->loop_depth);
      for (i = 0; i < n; i++)
        walk_tree(gimple_call_arg_ptr(stmt, i), record_reference, node,
                  visited_nodes);
  ...
  for (step = cfun->local_decls; step; step = TREE_CHAIN(step)) {
