...
static bool conditional_replacement(basic_block, basic_block, edge, edge,
                                    gimple, tree, tree);
static bool value_replacement(basic_block, basic_block, edge, edge, gimple,
                              tree, tree);
static bool minmax_replacement(basic_block, basic_block, edge, edge, gimple,
                               tree, tree);
static bool abs_replacement(basic_block, basic_block, edge, edge, gimple, tree,
                            tree);
...
static unsigned int tree_ssa_cs_elim(void) {
...
static unsigned int tree_ssa_phiopt_worker(bool do_store_elim) {
  basic_block bb;
  ...
  for (i = 0; i < n; i++) {
    ...
    if ((e1->flags & EDGE_ABNORMAL) != 0 || (e2->flags & EDGE_ABNORMAL) != 0)
      continue;

    /* If either bb1's succ or bb2 or bb2's succ is non NULL.  */
    if (EDGE_COUNT(bb1->succs) == 0 || bb2 == NULL ||
    ...
    if (!single_succ_p(bb1) || (e1->flags & EDGE_FALLTHRU) == 0)
      continue;
...
#define VISITED_P(BB)    (TEST_BIT(visited, (BB)->index))
  ...
  FOR_EACH_BB(x) {
    ...
    /* Walk the predecessors of x as long as they have precisely one
       ...
       after x.  */
    for (y = x, np = 1; single_pred_p(y) && !VISITED_P(single_pred(y));
         y = single_pred(y))
      np++;
    for (y = x, i = n - np; single_pred_p(y) && !VISITED_P(single_pred(y));
         y = single_pred(y), i++) {
      order[i] = y;
      MARK_VISITED(y);
    }
...
static void replace_phi_edge_with_variable(basic_block cond_block, edge e,
                                           gimple phi, tree new_tree) {
...
static bool conditional_replacement(basic_block cond_bb, basic_block middle_bb,
                                    edge e0, edge e1, gimple phi, tree arg0,
                                    tree arg1) {
  tree result;
  ...
  new_var =
      force_gimple_operand_gsi(&gsi, cond, true, NULL, true, GSI_SAME_STMT);

  if (!useless_type_conversion_p(TREE_TYPE(result), TREE_TYPE(new_var))) {
    new_var2 = create_tmp_var(TREE_TYPE(result), NULL);
    add_referenced_var(new_var2);
    new_stmt =
        gimple_build_assign_with_ops(CONVERT_EXPR, new_var2, new_var, NULL);
...
static bool value_replacement(basic_block cond_bb, basic_block middle_bb,
                              edge e0, edge e1, gimple phi, tree arg0,
                              tree arg1) {
  gimple cond;
  edge true_edge, false_edge;
...
static bool minmax_replacement(basic_block cond_bb, basic_block middle_bb,
                               edge e0, edge e1, gimple phi, tree arg0,
                               tree arg1) {
  tree result, type;
  ...
  if (empty_block_p(middle_bb)) {
    ...
    if (true_edge->src == middle_bb) {
      ...
      if (operand_equal_for_phi_arg_p(arg_false, larger)) {
        ...
        if (!integer_nonzerop(
                fold_build2(LE_EXPR, boolean_type_node, bound, larger)))
          return false;
      } else if (operand_equal_for_phi_arg_p(arg_false, smaller)) {
        /* Case
        ...
        if (!integer_nonzerop(
                fold_build2(GE_EXPR, boolean_type_node, bound, smaller)))
          return false;
      } else
        return false;
    } else {
      ...
      if (operand_equal_for_phi_arg_p(arg_true, larger)) {
        ...
        /* We need BOUND >= LARGER.  */
        if (!integer_nonzerop(
                fold_build2(GE_EXPR, boolean_type_node, bound, larger)))
          return false;
        ...
        /* We need BOUND <= SMALLER.  */
        if (!integer_nonzerop(
                fold_build2(LE_EXPR, boolean_type_node, bound, smaller)))
          return false;
      } else
        return false;
    }
...
static bool abs_replacement(basic_block cond_bb, basic_block middle_bb,
                            edge e0 ATTRIBUTE_UNUSED, edge e1, gimple phi,
                            tree arg0, tree arg1) {
  tree result;
  ...
  if (cond_code != GT_EXPR && cond_code != GE_EXPR && cond_code != LT_EXPR &&
      cond_code != LE_EXPR)
    return false;
...
static hashval_t name_to_bb_hash(const void *p) {
...
static int name_to_bb_eq(const void *p1, const void *p2) {
...
/* We see the expression EXP in basic block BB.  If it's an interesting
   ...
   the RHS.  */
static void add_or_mark_expr(basic_block bb, tree exp,
...
static void nt_init_block(struct dom_walk_data *data ATTRIBUTE_UNUSED,
                          basic_block bb) {
  gimple_stmt_iterator gsi;
  ...
  for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    if (is_gimple_assign(stmt)) {
      ...
      if (get_gimple_rhs_num_ops(gimple_assign_rhs_code(stmt)) > 1)
        add_or_mark_expr(bb, gimple_assign_rhs2(stmt), nontrap_set, false);
    }
  }
}

/* Called by walk_dominator_tree, when basic block BB is exited.  */
static void nt_fini_block(struct dom_walk_data *data ATTRIBUTE_UNUSED,
                          basic_block bb) {
...
static struct pointer_set_t *get_non_trapping(void) {
  ...
  seen_ssa_names = htab_create(128, name_to_bb_hash, name_to_bb_eq, free);
...
static bool cond_store_replacement(basic_block middle_bb, basic_block join_bb,
                                   edge e0, edge e1,
                                   struct pointer_set_t *nontrap) {
  gimple assign = last_and_only_stmt(middle_bb);
  tree lhs, rhs, name;
...
/* Always do these optimizations if we have SSA
   trees to work on.  */
static bool gate_phiopt(void) {
  return 1;
}

struct gimple_opt_pass pass_phiopt = {{
    GIMPLE_PASS, "phiopt",            /* name */
    gate_phiopt,                      /* gate */
    ...
    0,                                /* todo_flags_start */
    TODO_dump_func | TODO_ggc_collect | TODO_verify_ssa | TODO_verify_flow |
        TODO_verify_stmts /* todo_flags_finish */
}};

static bool gate_cselim(void) {
  return flag_tree_cselim;
}

struct gimple_opt_pass pass_cselim = {{
    GIMPLE_PASS, "cselim",            /* name */
    gate_cselim,                      /* gate */
