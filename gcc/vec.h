...
#define VEC_index(T, V, I) \
  (VEC_OP(T, base, index)(VEC_BASE(V), I VEC_CHECK_INFO))
...
#define VEC_embedded_init(T, O, N) \
  (VEC_OP(T, base, embedded_init)(VEC_BASE(O), N))
...
extern void *vec_gc_o_reserve_exact(void *, int, size_t, size_t MEM_STAT_DECL);
extern void ggc_free(void *);
#define vec_gc_free(V) ggc_free(V)
extern void *vec_heap_p_reserve(void *, int MEM_STAT_DECL);
extern void *vec_heap_p_reserve_exact(void *, int MEM_STAT_DECL);
extern void *vec_heap_o_reserve(void *, int, size_t, size_t MEM_STAT_DECL);
extern void *vec_heap_o_reserve_exact(void *, int, size_t,
...
extern void vec_assert_fail(const char *,
                            const char *VEC_CHECK_DECL) ATTRIBUTE_NORETURN;
...
#define DEF_VEC_FUNC_P(T)                                                     \
  static inline unsigned VEC_OP(T, base, length)(const VEC(T, base) * vec_) { \
    return vec_ ? vec_->num : 0;                                              \
                                                                              ...
                                                                              \
  static inline T VEC_OP(T, base,                                             \
                         last)(const VEC(T, base) * vec_ VEC_CHECK_DECL) {    \
    VEC_ASSERT(vec_ && vec_->num, "last", T, base);                           \
                                                                              \
    return vec_->vec[vec_->num - 1];                                          \
  }                                                                           \
                                                                              \
  static inline T VEC_OP(T, base, index)(const VEC(T, base) * vec_,           \
    ...
    return vec_->vec[ix_];                                                    \
                                                                              ...
                                                                              \
  static inline int VEC_OP(T, base, iterate)(const VEC(T, base) * vec_,       \
                                             unsigned ix_, T *ptr) {          \
    if (vec_ && ix_ < vec_->num) {                                            \
      ...
      *ptr = 0;                                                               \
      return 0;                                                               \
                                                                              ...
                                                                              \
  static inline size_t VEC_OP(T, base, embedded_size)(int alloc_) {           \
    return offsetof(VEC(T, base), vec) + alloc_ * sizeof(T);                  \
  }                                                                           \
                                                                              \
  static inline void VEC_OP(T, base, embedded_init)(VEC(T, base) * vec_,      \
                                                    int alloc_) {             \
    vec_->num = 0;                                                            \
    vec_->alloc = alloc_;                                                     \
  }                                                                           \
                                                                              \
  static inline int VEC_OP(T, base, space)(VEC(T, base) * vec_,               \
                                           int alloc_ VEC_CHECK_DECL) {       \
    VEC_ASSERT(alloc_ >= 0, "space", T, base);                                \
    return vec_ ? vec_->alloc - vec_->num >= (unsigned)alloc_ : !alloc_;      \
  }                                                                           \
                                                                              \
  static inline T *VEC_OP(T, base, quick_push)(VEC(T, base) * vec_,           \
  ...
  static inline void VEC_OP(T, base, truncate)(                               \
    ...
    if (vec_)                                                                 \
      vec_->num = size_;                                                      \
                                                                              ...
                                                                              \
  static inline T VEC_OP(T, base, replace)(VEC(T, base) * vec_, unsigned ix_, \
                                           T obj_ VEC_CHECK_DECL) {           \
    T old_obj_;                                                               \
                                                                              \
  ...
  static inline void VEC_OP(T, base, block_remove)(                           \
    ...
    memmove(slot_, slot_ + len_, (vec_->num - ix_) * sizeof(T));              \
                                                                              ...
                                                                              \
  static inline T *VEC_OP(T, base, address)(VEC(T, base) * vec_) {            \
...
#define DEF_VEC_ALLOC_FUNC_P(T, A)                                             \
  static inline VEC(T, A) * VEC_OP(T, A, alloc)(int alloc_ MEM_STAT_DECL) {    \
    return (VEC(T, A) *)vec_##A##_p_reserve_exact(NULL, alloc_ PASS_MEM_STAT); \
  }                                                                            \
                                                                               \
  static inline void VEC_OP(T, A, free)(VEC(T, A) * *vec_) {                   \
    if (*vec_)                                                                 \
      vec_##A##_free(*vec_);                                                   \
    *vec_ = NULL;                                                              \
  }                                                                            \
                                                                               \
  static inline VEC(T, A) *                                                    \
      VEC_OP(T, A, copy)(VEC(T, base) * vec_ MEM_STAT_DECL) {                  \
    size_t len_ = vec_ ? vec_->num : 0;                                        \
    VEC(T, A) *new_vec_ = NULL;                                                \
                                                                               \
    if (len_) {                                                                \
      new_vec_ =                                                               \
          (VEC(T, A) *)(vec_##A##_p_reserve_exact(NULL, len_ PASS_MEM_STAT));  \
                                                                               \
  ...
  static inline int VEC_OP(T, A, reserve)(                                     \
    ...
    int extend =                                                               \
        !VEC_OP(T, base, space)(VEC_BASE(*vec_), alloc_ VEC_CHECK_PASS);       \
                                                                               \
    if (extend)                                                                \
  ...
  static inline int VEC_OP(T, A, reserve_exact)(                               \
    ...
    if (extend)                                                                \
      *vec_ =                                                                  \
          (VEC(T, A) *)vec_##A##_p_reserve_exact(*vec_, alloc_ PASS_MEM_STAT); \
                                                                               \
    return extend;                                                             \
  }                                                                            \
                                                                               \
  static inline void VEC_OP(T, A, safe_grow)(                                  \
    ...
    VEC_ASSERT(size_ >= 0 && VEC_OP(T, base, length)                           \
                                     VEC_BASE(*vec_) <= (unsigned)size_,       \
               "grow", T, A);                                                  \
    VEC_OP(T, A, reserve_exact)                                                \
    (vec_, size_ - (int)(*vec_ ? VEC_BASE(*vec_)->num                          \
                               : 0)VEC_CHECK_PASS PASS_MEM_STAT);              \
  ...
  static inline T *VEC_OP(T, A, safe_insert)(                                  \
    ...
    VEC_OP(T, A, reserve)(vec_, 1 VEC_CHECK_PASS PASS_MEM_STAT);               \
                                                                               \
    return VEC_OP(T, base, quick_insert)(VEC_BASE(*vec_), ix_,                 \
                                         obj_ VEC_CHECK_PASS);                 \
  }
...
#define DEF_VEC_FUNC_O(T)                                                      \
  ...
  static inline T *VEC_OP(T, base, last)(VEC(T, base) * vec_ VEC_CHECK_DECL) { \
    ...
    return &vec_->vec[vec_->num - 1];                                          \
                                                                               ...
                                                                               \
  static inline T *VEC_OP(T, base, index)(VEC(T, base) * vec_,                 \
                                          unsigned ix_ VEC_CHECK_DECL) {       \
    VEC_ASSERT(vec_ &&ix_ < vec_->num, "index", T, base);                      \
                                                                               \
    return &vec_->vec[ix_];                                                    \
                                                                               ...
                                                                               \
  static inline int VEC_OP(T, base, iterate)(VEC(T, base) * vec_,              \
                                             unsigned ix_, T **ptr) {          \
    if (vec_ && ix_ < vec_->num) {                                             \
      *ptr = &vec_->vec[ix_];                                                  \
      return 1;                                                                \
  ...
  static inline T *VEC_OP(T, base, quick_push)(VEC(T, base) * vec_,            \
    ...
    return slot_;                                                              \
                                                                               ...
                                                                               \
  static inline void VEC_OP(T, base,                                           \
                            pop)(VEC(T, base) * vec_ VEC_CHECK_DECL) {         \
    VEC_ASSERT(vec_->num, "pop", T, base);                                     \
    --vec_->num;                                                               \
  }                                                                            \
                                                                               \
...
#define DEF_VEC_ALLOC_FUNC_O(T, A)                                             \
  static inline VEC(T, A) * VEC_OP(T, A, alloc)(int alloc_ MEM_STAT_DECL) {    \
    return (VEC(T, A) *)vec_##A##_o_reserve_exact(                             \
        NULL, alloc_, offsetof(VEC(T, A), base.vec), sizeof(T) PASS_MEM_STAT); \
  }                                                                            \
                                                                               \
  static inline VEC(T, A) *                                                    \
      VEC_OP(T, A, copy)(VEC(T, base) * vec_ MEM_STAT_DECL) {                  \
    ...
    if (len_) {                                                                \
      new_vec_ = (VEC(T, A) *)(vec_##A##_o_reserve_exact(                      \
          NULL, len_, offsetof(VEC(T, A), base.vec),                           \
  ...
  static inline T *VEC_OP(T, A, safe_push)(                                    \
    ...
    return VEC_OP(T, base, quick_push)(VEC_BASE(*vec_), obj_ VEC_CHECK_PASS);  \
                                                                               ...
                                                                               \
  static inline T *VEC_OP(T, A, safe_insert)(                                  \
      VEC(T, A) * *vec_, unsigned ix_,                                         \
      const T *obj_ VEC_CHECK_DECL MEM_STAT_DECL) {                            \
    VEC_OP(T, A, reserve)(vec_, 1 VEC_CHECK_PASS PASS_MEM_STAT);               \
                                                                               \
    return VEC_OP(T, base, quick_insert)(VEC_BASE(*vec_), ix_,                 \
                                         obj_ VEC_CHECK_PASS);                 \
