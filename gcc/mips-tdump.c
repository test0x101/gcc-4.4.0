...
static void *read_seek(void *ptr, size_t size, off_t offset,
                       const char *context) {
  long read_size = 0;
        ...
        case tq_Array: {
          ...
          for (j = i; j >= first_array; j--) {
            ...
            if (qualifiers[j].low_bound != 0)
              sprintf(p2, "%ld:%ld {%ld bits}", (long)qualifiers[j].low_bound,
...
static void print_sym_hdr(HDRR *sym_ptr) {
  ...
  printf("Symbolic header, magic number = 0x%04x, vstamp = %d.%d:\n\n",
         sym_ptr->magic & 0xffff, (sym_ptr->vstamp & 0xffff) >> 8,
         sym_ptr->vstamp & 0xff);
  ...
  printf("    %-*s %11ld %11ld %11ld [%d]\n", width, "Line numbers",
         (long)sym_ptr->cbLineOffset, (long)sym_ptr->cbLine,
         (long)sym_ptr->cbLine, (int)sym_ptr->ilineMax);

  printf("    %-*s %11ld %11ld %11ld\n", width, "Dense numbers",
         (long)sym_ptr->cbDnOffset, (long)sym_ptr->idnMax,
         (long)(sym_ptr->idnMax * sizeof(DNR)));

  printf("    %-*s %11ld %11ld %11ld\n", width, "Procedures Tables",
         (long)sym_ptr->cbPdOffset, (long)sym_ptr->ipdMax,
         (long)(sym_ptr->ipdMax * sizeof(PDR)));

  printf("    %-*s %11ld %11ld %11ld\n", width, "Local Symbols",
         (long)sym_ptr->cbSymOffset, (long)sym_ptr->isymMax,
         (long)(sym_ptr->isymMax * sizeof(SYMR)));

  printf("    %-*s %11ld %11ld %11ld\n", width, "Optimization Symbols",
         (long)sym_ptr->cbOptOffset, (long)sym_ptr->ioptMax,
         (long)(sym_ptr->ioptMax * sizeof(OPTR)));

  printf("    %-*s %11ld %11ld %11ld\n", width, "Auxiliary Symbols",
         (long)sym_ptr->cbAuxOffset, (long)sym_ptr->iauxMax,
         (long)(sym_ptr->iauxMax * sizeof(AUXU)));

  printf("    %-*s %11ld %11ld %11ld\n", width, "Local Strings",
         (long)sym_ptr->cbSsOffset, (long)sym_ptr->issMax,
         (long)sym_ptr->issMax);

  printf("    %-*s %11ld %11ld %11ld\n", width, "External Strings",
         (long)sym_ptr->cbSsExtOffset, (long)sym_ptr->issExtMax,
         (long)sym_ptr->issExtMax);

  printf("    %-*s %11ld %11ld %11ld\n", width, "File Tables",
         (long)sym_ptr->cbFdOffset, (long)sym_ptr->ifdMax,
         (long)(sym_ptr->ifdMax * sizeof(FDR)));

  printf("    %-*s %11ld %11ld %11ld\n", width, "Relative Files",
         (long)sym_ptr->cbRfdOffset, (long)sym_ptr->crfd,
         (long)(sym_ptr->crfd * sizeof(ulong)));

  printf("    %-*s %11ld %11ld %11ld\n", width, "External Symbols",
         (long)sym_ptr->cbExtOffset, (long)sym_ptr->iextMax,
         (long)(sym_ptr->iextMax * sizeof(EXTR)));
}

/* Print out a symbol.  */

static void print_symbol(SYMR *sym_ptr, int number, const char *strbase,
  ...
  if (want_scope) {
    ...
    if (cur_scope == (scope_t *)0)
      ...
      for (scope_ptr = cur_scope; scope_ptr != (scope_t *)0;
...
static void emit_aggregate(char *string, AUXU u, AUXU u2, const char *which,
                           FDR *fdp) {
...
static void print_file_desc(FDR *fdp, int number) {
  ...
  printf("    Debug level = %-10s Language    = %s\n",
         glevel_to_string(fdp->glevel), lang_to_string((lang_t)fdp->lang));
  ...
  for (symi = fdp->isymBase; symi < (fdp->csym + fdp->isymBase); symi++)
    print_symbol(&l_symbols[symi], symi - fdp->isymBase, str_base, aux_base, -1,
                 fdp);
  ...
  for (pdi = fdp->ipdFirst; pdi < (fdp->cpd + fdp->ipdFirst); pdi++) {
    ...
    printf("\t    .mask 0x%08lx,%-9ld .fmask 0x%08lx,%ld\n",
           (long)proc_ptr->regmask, (long)proc_ptr->regoffset,
    ...
    if (want_line && fdp->cline != 0) {
      ...
      uchar *line_ptr =
          (((uchar *)lines) + proc_ptr->cbLineOffset + fdp->cbLineOffset);
      uchar *line_end;
      ...
      while (line_ptr < line_end) { /* sign extend nibble */
...
static void read_tfile(void) {
  ...
  l_strings = (char *)read_seek(NULL, sym_hdr.issMax, sym_hdr.cbSsOffset,
                                "Local string table");

  e_strings = (char *)read_seek(NULL, sym_hdr.issExtMax, sym_hdr.cbSsExtOffset,
...
warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n\n",
  ...
  if (want_aux) {
    ...
    if (sym_hdr.ifdMax == 0)
      ...
      last_aux_in_use = (file_desc[sym_hdr.ifdMax - 1].iauxBase +
