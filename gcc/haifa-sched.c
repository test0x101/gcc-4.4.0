...
void fix_sched_param(const char *param, const char *val) {
...
#define CONST_BASED_ADDRESS_P(x)                                \
  (REG_P(x) || ((GET_CODE(x) == PLUS || GET_CODE(x) == MINUS || \
                 (GET_CODE(x) == LO_SUM)) &&                    \
...
static int may_trap_exp(const_rtx x, int is_store) {
  enum rtx_code code;
...
static int haifa_classify_rtx(const_rtx x) {
  ...
  if (GET_CODE(x) == PARALLEL) {
    ...
    switch (code) {
      ...
      case SET:
        ...
        tmp_class = WORST_CLASS(tmp_class, may_trap_exp(SET_SRC(x), 0));
        break;
      case COND_EXEC:
        tmp_class = haifa_classify_rtx(COND_EXEC_CODE(x));
        if (tmp_class == TRAP_RISKY)
          break;
        tmp_class = WORST_CLASS(tmp_class, may_trap_exp(COND_EXEC_TEST(x), 0));
        break;
      case TRAP_IF:
...
int haifa_classify_insn(const_rtx insn) {
...
/* Compute cost of executing INSN.
   ...
   instruction results.  */
HAIFA_INLINE int insn_cost(rtx insn) {
  int cost;
...
int dep_cost_1(dep_t link, dw_t dw) {
  ...
  if (recog_memoized(used) < 0) {
    ...
    if (INSN_CODE(insn) >= 0) {
      if (dep_type == REG_DEP_ANTI)
        ...
        cost = (insn_default_latency(insn) - insn_default_latency(used));
        if (cost <= 0)
          cost = 1;
    ...
    if (targetm.sched.adjust_cost_2) {
      ...
      cost = targetm.sched.adjust_cost(used, dep_cost_rtx_link, insn, cost);
...
static bool contributes_to_priority_p(dep_t dep) {
  /* Critical path is meaningful in block boundaries only.  */
  if (!current_sched_info->contributes_to_priority(DEP_CON(dep), DEP_PRO(dep)))
    return false;
...
static int rank_for_schedule(const void *x, const void *y) {
  rtx tmp = *(const rtx *)y;
  ...
  val = (sd_lists_size(tmp2, SD_LIST_FORW) - sd_lists_size(tmp, SD_LIST_FORW));
...
HAIFA_INLINE static void swap_sort(rtx *a, int n) {
  rtx insn = a[n - 1];
  int i = n - 2;
...
HAIFA_INLINE static void queue_insn(rtx insn, int n_cycles) {
...
static void queue_remove(rtx insn) {
...
rtx *ready_lastpos(struct ready_list *ready) {
...
HAIFA_INLINE static void ready_add(struct ready_list *ready, rtx insn,
                                   bool first_p) {
  if (!first_p) {
    if (ready->first == ready->n_ready) {
      memmove(ready->vec + ready->veclen - ready->n_ready, ready_lastpos(ready),
...
HAIFA_INLINE static rtx ready_remove_first(struct ready_list *ready) {
  rtx t;
...
rtx ready_element(struct ready_list *ready, int index) {
...
HAIFA_INLINE static rtx ready_remove(struct ready_list *ready, int index) {
  rtx t;
  int i;
...
static void ready_remove_insn(rtx insn) {
  int i;
...
void ready_sort(struct ready_list *ready) {
...
HAIFA_INLINE static void adjust_priority(rtx prev) {
...
HAIFA_INLINE static void advance_one_cycle(void) {
...
static int schedule_insn(rtx insn) {
  sd_iterator_def sd_it;
  dep_t dep;
  int advance = 0;
...
void concat_note_lists(rtx from_end, rtx *to_endp) {
  rtx from_start;
...
static rtx unlink_other_notes(rtx insn, rtx tail) {
...
/* Return the head and tail pointers of ebb starting at BEG and ending
   at END.  */
void get_ebb_head_tail(basic_block beg, basic_block end, rtx *headp,
                       rtx *tailp) {
...
int no_real_insns_p(const_rtx head, const_rtx tail) {
...
/* Delete notes between HEAD and TAIL and put them in the chain
   of notes ended by NOTE_LIST.  */
static void rm_other_notes(rtx head, rtx tail) {
  rtx next_tail;
  rtx insn;
  ...
  for (insn = head; insn != next_tail; insn = NEXT_INSN(insn)) {
    ...
    if (NOTE_NOT_BB_P(insn)) {
      ...
      gcc_assert((sel_sched_p() || prev != tail) && prev != head &&
                 insn != next_tail);
    }
  }
}
...
void remove_notes(rtx head, rtx tail) {
...
rtx restore_other_notes(rtx head, basic_block head_bb) {
  if (note_list != 0) {
...
static int find_set_reg_weight(const_rtx x) {
...
static void find_insn_reg_weight(const_rtx insn) {
  int reg_weight = 0;
  rtx x;
...
static void queue_to_ready(struct ready_list *ready) {
  rtx insn;
  rtx link;
  rtx skip_insn;
  ...
  for (link = insn_queue[q_ptr]; link; link = XEXP(link, 1)) {
    ...
    if (!reload_completed && ready->n_ready > MAX_SCHED_READY_INSNS &&
        !SCHED_GROUP_P(insn) && insn != skip_insn) {
      if (sched_verbose >= 2)
...
static bool ok_for_early_queue_removal(rtx insn) {
  int n_cycles;
      ...
      for (; prev_insn; prev_insn = PREV_INSN(prev_insn)) {
        ...
        if (!NOTE_P(prev_insn)) {
          ...
          if (dep != NULL) {
            ...
            if (targetm.sched.is_costly_dependence(
                    dep, cost, flag_sched_stalled_insns_dep - n_cycles))
              return false;
          }
        }
...
static int early_queue_to_ready(state_t state, struct ready_list *ready) {
  rtx insn;
  rtx link;
  rtx next_link;
  rtx prev_link;
...
static void debug_ready_list(struct ready_list *ready) {
  rtx *p;
  int i;
...
static void move_insn(rtx insn, rtx last, rtx nt) {
  if (PREV_INSN(insn) != last) {
    ...
    /* We move the block note along with jump.  */
    {
      ...
      if (note != nt && (LABEL_P(note) || BARRIER_P(note)))
        note = NEXT_INSN(note);
...
int max_issue(struct ready_list *ready, int privileged_n, state_t state,
              int *index) {
...
static int choose_ready(struct ready_list *ready, rtx *insn_ptr) {
  int lookahead;
  ...
  if (lookahead <= 0 || SCHED_GROUP_P(ready_element(ready, 0))) {
    *insn_ptr = ready_remove_first(ready);
    ...
    if (spec_info &&
        spec_info->flags & (PREFER_NON_DATA_SPEC | PREFER_NON_CONTROL_SPEC)) {
      for (i = 0, n = ready->n_ready; i < n; i++) {
        ...
        if (spec_info->flags & PREFER_NON_DATA_SPEC && !(s & DATA_SPEC)) {
          try_data = 0;
          if (!(spec_info->flags & PREFER_NON_CONTROL_SPEC) || !try_control)
            break;
        }

        if (spec_info->flags & PREFER_NON_CONTROL_SPEC && !(s & CONTROL_SPEC)) {
          try_control = 0;
    ...
    if ((ts & SPECULATIVE) &&
        (((!try_data && (ts & DATA_SPEC)) ||
         ...
         (targetm.sched.first_cycle_multipass_dfa_lookahead_guard_spec &&
          !targetm.sched.first_cycle_multipass_dfa_lookahead_guard_spec(insn))))
    ...
    if (max_issue(ready, 1, curr_state, &index) == 0) {
      *insn_ptr = ready_remove_first(ready);
      ...
      if (sched_verbose >= 4)
        fprintf(
            sched_dump, ";;\t\tChosen insn : %s\n",
            (*current_sched_info->print_insn)(ready_element(ready, index), 0));
...
void schedule_block(basic_block *target_bb) {
  ...
  if (!reload_completed && ready.n_ready > MAX_SCHED_READY_INSNS) {
    ...
    if (sched_verbose >= 2) {
      fprintf(sched_dump, ";;\t\tReady list on entry: %d insns\n",
              ready.n_ready);
      fprintf(sched_dump, ";;\t\t before reload => truncated to %d insns\n", i);
    }
  ...
  while ((*current_sched_info->schedule_more_p)()) {
    ...
    if (sort_p && targetm.sched.reorder &&
        (ready.n_ready == 0 || !SCHED_GROUP_P(ready_element(&ready, 0))))
      can_issue_more = targetm.sched.reorder(sched_dump, sched_verbose,
                                             ready_lastpos(&ready),
    ...
    for (;;) {
      ...
      if (targetm.sched.dfa_new_cycle &&
          targetm.sched.dfa_new_cycle(sched_dump, sched_verbose, insn,
                                      last_clock_var, clock_var, &sort_p))
      ...
      (*current_sched_info->begin_schedule_ready)(insn, last_scheduled_insn);
      ...
      if (targetm.sched.variable_issue)
        can_issue_more = targetm.sched.variable_issue(sched_dump, sched_verbose,
                                                      insn, can_issue_more);
...
int set_priorities(rtx head, rtx tail) {
  rtx insn;
  int n_insn;
...
void setup_sched_dump(void) {
  ...
  if (sched_verbose_param == 0 && dump_file)
    sched_verbose = 1;
  sched_dump = ((sched_verbose_param >= 10 || !dump_file) ? stderr : dump_file);
}
...
void sched_init(void) {
  ...
  if (targetm.sched.set_sched_flags) {
    ...
    if (spec_info->mask != 0) {
      ...
      spec_info->control_weakness_cutoff =
          (PARAM_VALUE(PARAM_SCHED_SPEC_PROB_CUTOFF) * REG_BR_PROB_BASE) / 100;
    } else
  ...
  if (targetm.sched.md_init_global)
    targetm.sched.md_init_global(sched_dump, sched_verbose, get_max_uid() + 1);
...
void haifa_sched_finish(void) {
  ...
  if (spec_info && spec_info->dump) {
    ...
    fprintf(spec_info->dump, ";; %s:\n", current_function_name());

    fprintf(spec_info->dump, ";; Procedure %cr-begin-data-spec motions == %d\n",
            c, nr_begin_data);
    fprintf(spec_info->dump, ";; Procedure %cr-be-in-data-spec motions == %d\n",
            c, nr_be_in_data);
...
static void fix_inter_tick(rtx head, rtx tail) {
...
int try_ready(rtx next) {
  ...
  gcc_assert(!(old_ts & ~(SPECULATIVE | HARD_DEP)) &&
             ((old_ts & HARD_DEP) || (old_ts & SPECULATIVE)));
  ...
  if (*ts & HARD_DEP)
    gcc_assert(*ts == old_ts && QUEUE_INDEX(next) == QUEUE_NOWHERE);
  ...
  gcc_assert(!ORIG_PAT(next) || !IS_SPECULATION_BRANCHY_CHECK_P(next));
  ...
  if (*ts & HARD_DEP) {
    ...
    return -1;
  } else if (!(*ts & BEGIN_SPEC) && ORIG_PAT(next) &&
...
static int fix_tick_ready(rtx next) {
  int tick, delay;
...
static void change_queue_index(rtx next, int delay) {
...
void sched_extend_ready_list(int new_sched_ready_n_insns) {
  int i;
...
static void generate_recovery_code(rtx insn) {
...
static void process_insn_forw_deps_be_in_spec(rtx insn, rtx twin, ds_t fs) {
  sd_iterator_def sd_it;
  dep_t dep;
         ...
         or transform it into 'be in speculative'.  */
      {
        if (/* In try_ready we assert that if insn once became ready
          ...
          if (/* consumer can 'be in speculative'.  */
              sched_insn_is_legitimate_for_speculation_p(consumer, new_ds))
...
static void begin_speculative_block(rtx insn) {
...
static void add_to_speculative_block(rtx insn) {
  ds_t ts;
  sd_iterator_def sd_it;
  dep_t dep;
  rtx twins = NULL;
...
void *xrecalloc(void *p, size_t new_nmemb, size_t old_nmemb, size_t size) {
...
edge find_fallthru_edge(basic_block pred) {
  edge e;
  edge_iterator ei;
  basic_block succ;
...
static void init_before_recovery(basic_block *before_recovery_ptr) {
  basic_block last;
  edge e;
...
basic_block sched_create_recovery_block(basic_block *before_recovery_ptr) {
  rtx label;
  rtx barrier;
  basic_block rec;
...
void sched_create_recovery_edges(basic_block first_bb, basic_block rec,
  ...
  /* Partition type is the same, if it is "unpartitioned".  */
  {
    ...
    if (flag_reorder_blocks_and_partition && targetm.have_named_sections)
    /* We don't need the same note for the check because
       any_condjump_p (check) == true.  */
    {
      REG_NOTES(jump) =
          gen_rtx_EXPR_LIST(REG_CROSSING_JUMP, NULL_RTX, REG_NOTES(jump));
    }
    edge_flags = EDGE_CROSSING;
  } else
    edge_flags = 0;
...
static void create_check_block_twin(rtx insn, bool mutate_p) {
  basic_block rec;
  ...
  for (sd_it = sd_iterator_start(insn, SD_LIST_SPEC_BACK);
       sd_iterator_cond(&sd_it, &dep);) {
    if ((DEP_STATUS(dep) & BEGIN_SPEC) || mutate_p)
  ...
  if (rec != EXIT_BLOCK_PTR) {
    ...
    if (!mutate_p) {
      ...
      {
        sd_it = sd_iterator_start(
            insn, (SD_LIST_FORW | SD_LIST_BACK | SD_LIST_RES_BACK));
...
static void fix_recovery_deps(basic_block rec) {
...
void sched_change_pattern(rtx insn, rtx new_pat) {
  int t;
...
/* Change pattern of INSN to NEW_PAT.  Invalidate cached haifa
   instruction data.  */
static void haifa_change_pattern(rtx insn, rtx new_pat) {
...
int sched_speculate_insn(rtx insn, ds_t request, rtx *new_pat) {
...
static int haifa_speculate_insn(rtx insn, ds_t request, rtx *new_pat) {
  gcc_assert(sched_deps_info->generate_spec_deps &&
             !IS_SPECULATION_CHECK_P(insn));

  if (HAS_INTERNAL_DEP(insn) || SCHED_GROUP_P(insn))
    return -1;
...
static void dump_new_block_header(int i, basic_block bb, rtx head, rtx tail) {
  if (!i)
    fprintf(sched_dump,
            ";;   ======================================================\n");
  else
...
void unlink_bb_notes(basic_block first, basic_block last) {
...
static void restore_bb_notes(basic_block first) {
  if (!bb_header)
    return;
  ...
  while (first != EXIT_BLOCK_PTR && bb_header[first->index]) {
    rtx prev, label, note, next;
...
static void fix_jump_move(rtx jump) {
...
static void move_block_after_check(rtx jump) {
  basic_block bb, jump_bb, jump_bb_next;
  VEC(edge, gc) * t;
  ...
  common_sched_info->fix_recovery_cfg(bb->index, jump_bb->index,
                                      jump_bb_next->index);
}
...
/* Helper function for move_block_after_check.
   ...
   block TO.  */
static void move_succs(VEC(edge, gc) * *succsp, basic_block to) {
  edge e;
  edge_iterator ei;
...
static void sched_remove_insn(rtx insn) {
  sd_finish_insn(insn);
...
static void clear_priorities(rtx insn, rtx_vec_t *roots_ptr) {
  sd_iterator_def sd_it;
  dep_t dep;
...
static void calc_priorities(rtx_vec_t roots) {
  int i;
  rtx insn;
...
static void add_jump_dependencies(rtx insn, rtx jump) {
  do {
    insn = NEXT_INSN(insn);
    if (insn == jump)
      break;
...
/* Helper function for check_cfg.
   ...
   its flags.  */
static int has_edge_p(VEC(edge, gc) * el, int type) {
  edge e;
  edge_iterator ei;
...
/* Check few properties of CFG between HEAD and TAIL.
   ...
   instruction stream.  */
static void check_cfg(rtx head, rtx tail) {
  rtx next_tail;
  basic_block bb = 0;
  ...
  do {
    ...
    if (LABEL_P(head) ||
        (NOTE_INSN_BASIC_BLOCK_P(head) &&
         (!not_first || (not_first && !LABEL_P(PREV_INSN(head)))))) {
      gcc_assert(bb == 0);
    ...
    if (bb == 0) {
      ...
      gcc_assert(inside_basic_block_p(head) || NOTE_P(head));
...
void sched_scan(const struct sched_scan_info_def *ssi, bb_vec_t bbs,
                basic_block bb, insn_vec_t insns, rtx insn) {
  sched_scan_info = ssi;
...
static void luids_extend_insn(void) {
...
static void luids_init_insn(rtx insn) {
...
void sched_init_luids(bb_vec_t bbs, basic_block bb, insn_vec_t insns,
                      rtx insn) {
...
static void init_h_i_d(rtx insn) {
  if (INSN_LUID(insn) > 0) {
    INSN_COST(insn) = -1;
...
void haifa_init_h_i_d(bb_vec_t bbs, basic_block bb, insn_vec_t insns,
                      rtx insn) {
  const struct sched_scan_info_def ssi = {
      NULL,         /* extend_bb */
      NULL,         /* init_bb */
...
static void haifa_init_insn(rtx insn) {
...
static void haifa_init_only_bb(basic_block bb, basic_block after) {
  gcc_assert(bb != NULL);
...
basic_block sched_split_block_1(basic_block first_bb, rtx after) {
  edge e;
...
basic_block sched_create_empty_bb_1(basic_block after) {
