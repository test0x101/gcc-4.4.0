...
static bool can_simplify_addr(rtx addr) {
  ...
  return (!REG_P(reg) || (REGNO(reg) != FRAME_POINTER_REGNUM &&
...
static void canonicalize_address(rtx x) {
  for (;;)
    switch (GET_CODE(x)) {
      case ASHIFT:
        if (GET_CODE(XEXP(x, 1)) == CONST_INT &&
          ...
          PUT_CODE(x, MULT);
          XEXP(x, 1) = gen_int_mode((HOST_WIDE_INT)1 << shift, GET_MODE(x));
        }
      ...
      case PLUS:
        if (GET_CODE(XEXP(x, 0)) == PLUS || GET_CODE(XEXP(x, 0)) == ASHIFT ||
...
static bool should_replace_address(rtx old_rtx, rtx new_rtx,
                                   enum machine_mode mode, bool speed) {
  int gain;
  ...
  gain =
      address_cost(old_rtx, mode, speed) - address_cost(new_rtx, mode, speed);
...
static bool propagate_rtx_1(rtx *px, rtx old_rtx, rtx new_rtx, int flags) {
  ...
  if (!(flags & PR_HANDLE_MEM) && MEM_P(x) && !MEM_READONLY_P(x)) {
    ...
    *px = (side_effects_p(x) ? gen_rtx_CLOBBER(GET_MODE(x), const0_rtx)
  ...
  switch (GET_RTX_CLASS(code)) {
    ...
    case RTX_OBJ:
      if (code == MEM && x != new_rtx) {
        ...
        valid_ops &=
            propagate_rtx_1(&new_op0, old_rtx, new_rtx, flags | PR_CAN_APPEAR);

        /* Dismiss transformation that we do not want to carry on.  */
        if (!valid_ops || new_op0 == op0 ||
...
static int varying_mem_p(rtx *body, void *data ATTRIBUTE_UNUSED) {
  rtx x = *body;
...
static rtx propagate_rtx(rtx x, enum machine_mode mode, rtx old_rtx,
                         rtx new_rtx, bool speed) {
  rtx tem;
  bool collapsed;
  int flags;
...
static bool local_ref_killed_between_p(df_ref ref, rtx from, rtx to) {
  rtx insn;
...
static bool use_killed_between(df_ref use, rtx def_insn, rtx target_insn) {
  ...
  if (def_bb == target_bb ? DF_INSN_LUID(def_insn) >= DF_INSN_LUID(target_insn)
  ...
  if (def && DF_REF_NEXT_REG(def) == NULL && regno >= FIRST_PSEUDO_REGISTER)
    return false;
  ...
  if (single_pred_p(target_bb) && single_pred(target_bb) == def_bb) {
    df_ref x;
...
static bool all_uses_available_at(rtx def_insn, rtx target_insn) {
  df_ref *use_rec;
  ...
  if (NEXT_INSN(def_insn) == target_insn && REG_P(SET_DEST(def_set))) {
...
static int find_occurrence_callback(rtx *px, void *data) {
...
static rtx *find_occurrence(rtx *px, rtx find) {
  struct find_occurrence_data data;

  gcc_assert(REG_P(find) ||
             (GET_CODE(find) == SUBREG && REG_P(SUBREG_REG(find))));
...
/* Inside INSN, the expression rooted at *LOC has been changed, moving some
   ...
   given TYPE.  */
static void update_df(rtx insn, rtx *loc, df_ref *use_rec,
                      enum df_ref_type type, int new_flags) {
  bool changed = false;
  ...
  while (*use_rec) {
    ...
    if (DF_REF_FLAGS_IS_SET(orig_use,
    ...
    new_use = df_ref_create(
        DF_REF_REG(orig_use), new_loc, insn, BLOCK_FOR_INSN(insn), type,
        DF_REF_FLAGS(orig_use) | new_flags, width, offset, mode);
...
static bool try_fwprop_subst(df_ref use, rtx *loc, rtx new_rtx, rtx def_insn,
                             bool set_reg_equal) {
...
static bool forward_propagate_subreg(df_ref use, rtx def_insn, rtx def_set) {
  rtx use_reg = DF_REF_REG(use);
  rtx use_insn, src;

  /* Only consider paradoxical subregs... */
  enum machine_mode use_mode = GET_MODE(use_reg);
  if (GET_CODE(use_reg) != SUBREG || !REG_P(SET_DEST(def_set)) ||
      GET_MODE_SIZE(use_mode) <= GET_MODE_SIZE(GET_MODE(SUBREG_REG(use_reg))))
    return false;
  ...
  src = SET_SRC(def_set);
  if (GET_CODE(src) == SUBREG && REG_P(SUBREG_REG(src)) &&
      GET_MODE(SUBREG_REG(src)) == use_mode && subreg_lowpart_p(src) &&
      all_uses_available_at(def_insn, use_insn))
    return try_fwprop_subst(use, DF_REF_LOC(use), SUBREG_REG(src), def_insn,
                            false);
  else
    return false;
}
...
static bool forward_propagate_and_simplify(df_ref use, rtx def_insn,
                                           rtx def_set) {
...
static void forward_propagate_into(df_ref use) {
  struct df_link *defs;
  df_ref def;
...
static unsigned int fwprop(void) {
  unsigned i;
...
static unsigned int fwprop_addr(void) {
  unsigned i;
  fwprop_init();
