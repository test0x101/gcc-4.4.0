...
tree make_ssa_name_fn(struct function *fn, tree var, gimple stmt) {
  tree t;
  use_operand_p imm;
...
void release_ssa_name(tree var) {
  if (!var)
    return;
  ...
  if (!SSA_NAME_IN_FREE_LIST(var)) {
    ...
    VEC_replace(tree, SSANAMES(cfun), SSA_NAME_VERSION(var), NULL_TREE);
...
tree duplicate_ssa_name(tree name, gimple stmt) {
...
static unsigned int release_dead_ssa_names(void) {
  tree t, next;
  int n = 0;
  ...
  if (dump_file)
    fprintf(dump_file, "Released %i names, %.2f%%\n", n,
            n * 100.0 / num_ssa_names);
  return 0;
}

struct gimple_opt_pass pass_release_ssa_names = {{
    GIMPLE_PASS, "release_ssa", /* name */
    NULL,                       /* gate */
