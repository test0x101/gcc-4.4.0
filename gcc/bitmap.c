...
static hashval_t hash_descriptor(const void *p) {
  const struct bitmap_descriptor *const d = (const struct bitmap_descriptor *)p;
...
static struct bitmap_descriptor *bitmap_descriptor(const char *file,
                                                   const char *function,
                                                   int line) {
  ...
  slot = (struct bitmap_descriptor **)htab_find_slot_with_hash(
      bitmap_desc_hash, &loc, htab_hash_pointer(file) + line, 1);
  if (*slot)
    return *slot;
...
static void register_overhead(bitmap b, int amount) {
...
static inline void bitmap_elem_to_freelist(bitmap head, bitmap_element *elt) {
...
static inline bitmap_element *bitmap_element_allocate(bitmap head) {
...
bitmap bitmap_obstack_alloc_stat(bitmap_obstack *bit_obstack MEM_STAT_DECL) {
  bitmap map;
...
static inline int bitmap_element_zerop(const bitmap_element *element) {
...
static inline void bitmap_element_link(bitmap head, bitmap_element *element) {
  ...
  /* If this index is less than that of the current element, it goes someplace
    ...
    for (ptr = head->current; ptr->prev != 0 && ptr->prev->indx > indx;
         ptr = ptr->prev)
      ;
  ...
  /* Otherwise, it must go someplace after the current element.  */
    ...
    for (ptr = head->current; ptr->next != 0 && ptr->next->indx < indx;
         ptr = ptr->next)
      ;
...
static bitmap_element *bitmap_elt_insert_after(bitmap head, bitmap_element *elt,
                                               unsigned int indx) {
...
static inline bitmap_element *bitmap_find_bit(bitmap head, unsigned int bit) {
  ...
  if (head->indx < indx)
    ...
    for (element = head->first; element->next != 0 && element->indx < indx;
         element = element->next)
      ;
...
void bitmap_and_into(bitmap a, const_bitmap b) {
...
static inline bool bitmap_elt_copy(bitmap dst, bitmap_element *dst_elt,
...
void bitmap_set_range(bitmap head, unsigned int start, unsigned int count) {
  ...
  for (i = first_index; i <= last_index; i++) {
    ...
    if (elt_start_bit <= start) {
      ...
      first_mask = (((BITMAP_WORD)1) << ((start % BITMAP_WORD_BITS))) - 1;
    ...
    if (elt_end_bit_plus1 <= end_bit_plus1) {
      ...
      last_word_to_mod = (end_bit_plus1 - elt_start_bit) / BITMAP_WORD_BITS;
...
static inline bool bitmap_elt_ior(bitmap dst, bitmap_element *dst_elt,
                                  bitmap_element *dst_prev,
                                  const bitmap_element *a_elt,
                                  const bitmap_element *b_elt, bool changed) {
...
bool bitmap_equal_p(const_bitmap a, const_bitmap b) {
  ...
  for (a_elt = a->first, b_elt = b->first; a_elt && b_elt;
...
bool bitmap_ior_and_compl(bitmap dst, const_bitmap a, const_bitmap b,
                          const_bitmap kill) {
  bool changed = false;
  ...
  while (a_elt || b_elt) {
    ...
    if (b_elt && kill_elt && kill_elt->indx == b_elt->indx &&
      ...
      if (ior) {
        changed =
            bitmap_elt_ior(dst, dst_elt, dst_prev, a_elt, &tmp_elt, changed);
        new_element = true;
        if (a_elt && a_elt->indx == b_elt->indx)
          a_elt = a_elt->next;
      }
...
bool bitmap_ior_and_compl_into(bitmap a, const_bitmap from1,
                               const_bitmap from2) {
  bitmap_head tmp;
  bool changed;
...
void debug_bitmap_file(FILE *file, const_bitmap head) {
  ...
  fprintf(file, "\nfirst = %p current = %p indx = %u\n", (void *)head->first,
          (void *)head->current, head->indx);

  for (ptr = head->first; ptr; ptr = ptr->next) {
    unsigned int i, j, col = 26;

    fprintf(file, "\t%p next = %p prev = %p indx = %u\n\t\tbits = {",
            (const void *)ptr, (const void *)ptr->next, (const void *)ptr->prev,
            ptr->indx);
    ...
    for (i = 0; i < BITMAP_ELEMENT_WORDS; i++)
      for (j = 0; j < BITMAP_WORD_BITS; j++)
        if ((ptr->bits[i] >> j) & 1) {
          ...
          fprintf(
              file, " %u",
              (ptr->indx * BITMAP_ELEMENT_ALL_BITS + i * BITMAP_WORD_BITS + j));
          col += 4;
        }
...
void bitmap_print(FILE *file, const_bitmap head, const char *prefix,
                  const char *suffix) {
  const char *comma = "";
  unsigned i;
  bitmap_iterator bi;
...
static int print_statistics(void **slot, void *b) {
  ...
  if (d->allocated) {
    ...
    fprintf(stderr, "%-41s %6d %10d %10d %10d %10d\n", s, d->created,
            d->allocated, d->peak, d->current, d->nsearches);
    i->size += d->allocated;
    i->count += d->created;
  }
  return 1;
