...
#define EXPR_WAS_CHANGED(EXPR) \
  (VEC_length(expr_history_def, EXPR_HISTORY_OF_CHANGES(EXPR)) > 0)
...
static bool _list_iter_cond_expr(av_set_t av, expr_t *exprp) {
  if (av) {
...
#define LOOP_PREHEADER_BLOCKS(LOOP) \
  ((size_t)((LOOP)->aux) == 1 ? NULL : ((VEC(basic_block, heap) *)(LOOP)->aux))
...
#define SEL_REGION_BB_INFO(BB) \
  (VEC_index(sel_region_bb_info_def, sel_region_bb_info, (BB)->index))
...
/* Collect all loop exits recursively, skipping empty BBs between them.
   ...
   as well.  */
static inline VEC(edge, heap) * get_all_loop_exits(basic_block bb) {
  VEC(edge, heap) *exits = NULL;

  /* If bb is empty, and we're skipping to loop exits, then
     consider bb as a possible gate to the inner loop now.  */
  while (sel_bb_empty_p(bb) && in_current_region_p(bb)) {
    bb = single_succ(bb);
...
static inline bool _succ_iter_cond(succ_iterator *ip, rtx *succp, rtx insn,
  ...
  if (!ip->bb_end) {
    ...
    *succp = NEXT_INSN(insn);
    ...
    while (1) {
      ...
      if (ip->loop_exits) {
        do {
          VEC_iterate(edge, ip->loop_exits, ip->current_exit, e_tmp);
          ip->current_exit++;
...
static inline bool _eligible_successor_edge_p(edge e1, succ_iterator *ip) {
  ...
  while (1) {
    ...
    gcc_assert(!in_current_region_p(bb) || (flags & SUCCS_OUT));
  }
...
extern int find_in_history_vect(VEC(expr_history_def, heap) *, rtx, vinsn_t,
                                bool);
extern void insert_in_history_vect(VEC(expr_history_def, heap) **, unsigned,
