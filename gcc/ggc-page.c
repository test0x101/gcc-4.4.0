...
inline static void push_depth(unsigned int i) {
...
inline static void push_by_depth(page_entry *p, unsigned long *s) {
  if (G.by_depth_in_use >= G.by_depth_max) {
    G.by_depth_max *= 2;
    G.by_depth = XRESIZEVEC(page_entry *, G.by_depth, G.by_depth_max);
    G.save_in_use = XRESIZEVEC(unsigned long *, G.save_in_use, G.by_depth_max);
  }
...
void debug_print_page_list(int order) {
  ...
  p = G.pages[order];
  while (p != NULL) {
    printf("%p(%1d|%3d) -> ", (void *)p, p->context_depth, p->num_free_objects);
    p = p->next;
  }
  printf("NULL\n");
  fflush(stdout);
...
static void free_page(page_entry *entry) {
  if (GGC_DEBUG_LEVEL >= 2)
    fprintf(G.debug_file, "Deallocating page at %p, data %p-%p\n",
            (void *)entry, entry->page, entry->page + entry->bytes - 1);
...
void *ggc_alloc_stat(size_t size MEM_STAT_DECL) {
  ...
  if (--entry->num_free_objects == 0 && entry->next != NULL &&
  ...
  /* Make the bytes after the end of the object unaccessible.  Discard the
     handle to avoid handle leak.  */
  VALGRIND_DISCARD(
      VALGRIND_MAKE_MEM_NOACCESS((char *)result + size, object_size - size));
#endif
...
void ggc_free(void *p) {
  ...
  if (GGC_DEBUG_LEVEL >= 3)
    fprintf(G.debug_file, "Freeing object, actual size=%lu, at %p on %p\n",
...
struct alloc_zone *new_ggc_zone(const char *name ATTRIBUTE_UNUSED) {
  return NULL;
}
...
static void ggc_recalculate_in_use_p(page_entry *p) {
  ...
  for (i = 0; i < CEIL(BITMAP_SIZE(num_objects), sizeof(*p->in_use_p)); ++i) {
    unsigned long j;
      ...
      for (i = 0; i < num_objects; i++) {
        ...
        if (((p->in_use_p[word] >> bit) & 1) == 0) {
          ...
          /* Keep poison-by-write when we expect to use Valgrind,
             ...
             below.  */
          VALGRIND_DISCARD(VALGRIND_MAKE_MEM_UNDEFINED(object, size));
...
static void validate_free_objects(void) {
...
void ggc_print_statistics(void) {
  ...
  for (i = 0; i < NUM_ORDERS; ++i) {
    ...
    for (p = G.pages[i]; p; p = p->next) {
      allocated += p->bytes;
      in_use += (OBJECTS_IN_PAGE(p) - p->num_free_objects) * OBJECT_SIZE(i);

      overhead += (sizeof(page_entry) - sizeof(long) +
                   BITMAP_SIZE(OBJECTS_IN_PAGE(p) + 1));
    }
    fprintf(stderr, "%-5lu %10lu%c %10lu%c %10lu%c\n",
            (unsigned long)OBJECT_SIZE(i), SCALE(allocated),
            STAT_LABEL(allocated), SCALE(in_use), STAT_LABEL(in_use),
            SCALE(overhead), STAT_LABEL(overhead));
    total_overhead += overhead;
  }
  fprintf(stderr, "%-5s %10lu%c %10lu%c %10lu%c\n", "Total",
          SCALE(G.bytes_mapped), STAT_LABEL(G.bytes_mapped), SCALE(G.allocated),
          STAT_LABEL(G.allocated), SCALE(total_overhead),
...
char *ggc_pch_alloc_object(struct ggc_pch_data *d, void *x ATTRIBUTE_UNUSED,
                           size_t size, bool is_string ATTRIBUTE_UNUSED,
                           enum gt_types_enum type ATTRIBUTE_UNUSED) {
  unsigned order;
  char *result;
...
void ggc_pch_write_object(struct ggc_pch_data *d ATTRIBUTE_UNUSED, FILE *f,
                          void *x, void *newx ATTRIBUTE_UNUSED, size_t size,
  ...
  if (d->written[order] == d->d.totals[order] &&
      fseek(f,
            ROUND_UP_VALUE(d->d.totals[order] * OBJECT_SIZE(order), G.pagesize),
            SEEK_CUR) != 0)
    fatal_error("can't write PCH file: %m");
}
...
static void move_ptes_to_front(int count_old_page_tables,
  ...
  memcpy(&new_by_depth[0], &G.by_depth[count_old_page_tables],
         count_new_page_tables * sizeof(void *));
  memcpy(&new_by_depth[count_new_page_tables], &G.by_depth[0],
         count_old_page_tables * sizeof(void *));
  memcpy(&new_save_in_use[0], &G.save_in_use[count_old_page_tables],
         count_new_page_tables * sizeof(void *));
  memcpy(&new_save_in_use[count_new_page_tables], &G.save_in_use[0],
         count_old_page_tables * sizeof(void *));
...
void ggc_pch_read(FILE *f, void *addr) {
  ...
  for (i = 0; i < NUM_ORDERS; i++) {
    ...
    entry = XCNEWVAR(
        struct page_entry,
        (sizeof(struct page_entry) - sizeof(long) + BITMAP_SIZE(num_objs + 1)));
    entry->bytes = bytes;
    entry->page = offs;
    ...
    for (j = 0; j + HOST_BITS_PER_LONG <= num_objs + 1; j += HOST_BITS_PER_LONG)
      entry->in_use_p[j / HOST_BITS_PER_LONG] = -1;
    for (; j < num_objs + 1; j++)
      entry->in_use_p[j / HOST_BITS_PER_LONG] |= 1L << (j % HOST_BITS_PER_LONG);

    for (pte = entry->page; pte < entry->page + entry->bytes; pte += G.pagesize)
