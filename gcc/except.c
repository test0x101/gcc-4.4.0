...
static GTY((param_is(union tree_node))) htab_t type_to_runtime_map;
...
void init_eh(void) {
  ...
  if (USING_SJLJ_EXCEPTIONS) {
    ...
    tmp = build_array_type(
        lang_hooks.types.type_for_mode(targetm.unwind_word_mode(), 1), tmp);
    f_data = build_decl(FIELD_DECL, get_identifier("__data"), tmp);
    DECL_FIELD_CONTEXT(f_data) = sjlj_fc_type_node;

    f_per =
        build_decl(FIELD_DECL, get_identifier("__personality"), ptr_type_node);
    DECL_FIELD_CONTEXT(f_per) = sjlj_fc_type_node;

    f_lsda = build_decl(FIELD_DECL, get_identifier("__lsda"), ptr_type_node);
...
struct eh_region *gen_eh_region_try(struct eh_region *outer) {
  return gen_eh_region(ERT_TRY, outer);
}

struct eh_region *gen_eh_region_catch(struct eh_region *t, tree type_or_list) {
...
struct eh_region *gen_eh_region_allowed(struct eh_region *outer, tree allowed) {
...
struct eh_region *gen_eh_region_must_not_throw(struct eh_region *outer) {
...
rtx get_exception_pointer(void) {
  if (!crtl->eh.exc_ptr)
...
static void add_ehl_entry(rtx label, struct eh_region *region) {
  ...
  slot = (struct ehl_map_entry **)htab_find_slot(
      crtl->eh.exception_handler_label_map, entry, INSERT);
...
void find_exception_handler_labels(void) {
  ...
  if (crtl->eh.exception_handler_label_map)
    ...
    crtl->eh.exception_handler_label_map = htab_create_ggc(
        cfun->eh->last_region_number * 3 / 2, ehl_hash, ehl_eq, NULL);
  }
...
bool current_function_has_exception_handlers(void) {
  ...
  for (i = cfun->eh->last_region_number; i > 0; --i) {
    ...
    region = VEC_index(eh_region, cfun->eh->region_array, i);
    if (region && region->region_number == i && region->type != ERT_THROW)
      return true;
  }
...
static eh_region duplicate_eh_regions_1(eh_region old, eh_region outer,
                                        int eh_offset) {
  eh_region ret, n;
...
int duplicate_eh_regions(struct function *ifun, duplicate_eh_regions_map map,
  ...
  memset(VEC_address(eh_region, cfun->eh->region_array) +
             cfun_last_region_number + 1,
         0, num_regions * sizeof(eh_region));
...
static int t2r_eq(const void *pentry, const void *pdata) {
...
static hashval_t t2r_hash(const void *pentry) {
  const_tree const entry = (const_tree)pentry;
...
static int ttypes_filter_eq(const void *pentry, const void *pdata) {
...
static hashval_t ttypes_filter_hash(const void *pentry) {
...
static int ehspec_filter_eq(const void *pentry, const void *pdata) {
  const struct ttypes_filter *entry = (const struct ttypes_filter *)pentry;
...
static hashval_t ehspec_filter_hash(const void *pentry) {
  const struct ttypes_filter *entry = (const struct ttypes_filter *)pentry;
  hashval_t h = 0;
  tree list;
...
static int add_ttypes_entry(htab_t ttypes_hash, tree type) {
  ...
  slot = (struct ttypes_filter **)htab_find_slot_with_hash(
      ttypes_hash, type, TREE_HASH(type), INSERT);
...
static int add_ehspec_entry(htab_t ehspec_hash, htab_t ttypes_hash, tree list) {
  ...
  dummy.t = list;
  slot = (struct ttypes_filter **)htab_find_slot(ehspec_hash, &dummy, INSERT);
            ...
            if (c->u.eh_catch.type_list == NULL)
              ...
              for (; tp_node;) {
                emit_cmp_and_jump_insns(
                    crtl->eh.filter,
                    GEN_INT(tree_low_cst(TREE_VALUE(flt_node), 0)), EQ,
                    NULL_RTX, targetm.eh_return_filter_mode(), 0, c->label);
...
static void connect_post_landing_pads(void) {
  ...
  for (i = cfun->eh->last_region_number; i > 0; --i) {
    ...
    if (outer) {
      ...
      emit_library_call(unwind_resume_libfunc, LCT_THROW, VOIDmode, 1,
                        crtl->eh.exc_ptr, ptr_mode);
...
static void dw2_build_landing_pads(void) {
  ...
  for (i = cfun->eh->last_region_number; i > 0; --i) {
    ...
    if (region->type != ERT_CLEANUP && region->type != ERT_TRY &&
    ...
    emit_move_insn(crtl->eh.filter, gen_rtx_REG(targetm.eh_return_filter_mode(),
...
static bool sjlj_find_directly_reachable_regions(struct sjlj_lp_info *lp_info) {
  ...
  for (insn = get_insns(); insn; insn = NEXT_INSN(insn)) {
    ...
    region =
        VEC_index(eh_region, cfun->eh->region_array, INTVAL(XEXP(note, 0)));
...
static void sjlj_emit_function_enter(rtx dispatch_label) {
...
#ifdef DONT_USE_BUILTIN_SETJMP
  {
    ...
    x = emit_library_call_value(setjmp_libfunc, NULL_RTX, LCT_RETURNS_TWICE,
                                ...
                                plus_constant(XEXP(fc, 0), sjlj_fc_jbuf_ofs),
                                Pmode);

    emit_cmp_and_jump_insns(x, const0_rtx, NE, 0, TYPE_MODE(integer_type_node),
                            0, dispatch_label);
  ...
  emit_library_call(unwind_sjlj_register_libfunc, LCT_NORMAL, VOIDmode, 1,
                    XEXP(fc, 0), Pmode);
...
static void sjlj_emit_function_exit(void) {
  ...
  emit_library_call(unwind_sjlj_unregister_libfunc, LCT_NORMAL, VOIDmode, 1,
...
static void sjlj_emit_dispatch_table(rtx dispatch_label,
  ...
  /* Load up dispatch index, exc_ptr and filter values from the
     function context.  */
  mem = adjust_address(fc, TYPE_MODE(integer_type_node), sjlj_fc_call_site_ofs);
  ...
  before = (((struct eh_region *)VEC_index(eh_region, cfun->eh->region_array,
                                           first_reachable))
                ->post_landing_pad);
...
static void sjlj_build_landing_pads(void) {
  ...
  if (sjlj_find_directly_reachable_regions(lp_info)) {
    ...
    int align =
        STACK_SLOT_ALIGNMENT(sjlj_fc_type_node, TYPE_MODE(sjlj_fc_type_node),
                             TYPE_ALIGN(sjlj_fc_type_node));
    crtl->eh.sjlj_fc =
        assign_stack_local(TYPE_MODE(sjlj_fc_type_node),
                           int_size_in_bytes(sjlj_fc_type_node), align);
...
static hashval_t ehl_hash(const void *pentry) {
...
static int ehl_eq(const void *pentry, const void *pdata) {
  const struct ehl_map_entry *const entry =
      (const struct ehl_map_entry *)pentry;
  const struct ehl_map_entry *const data = (const struct ehl_map_entry *)pdata;
...
static void remove_exception_handler_label(rtx label) {
  ...
  tmp.label = label;
  slot = (struct ehl_map_entry **)htab_find_slot(
      crtl->eh.exception_handler_label_map, &tmp, NO_INSERT);
  gcc_assert(slot);
...
static void remove_eh_handler(struct eh_region *region) {
  ...
  *pp = region->next_peer;
  ...
  if (region->type == ERT_CATCH) {
    ...
    for (eh_try = region->next_peer; eh_try->type == ERT_CATCH;
...
static int for_each_eh_label_1(void **pentry, void *data) {
...
static enum reachable_code reachable_next_level(struct eh_region *region,
  ...
  switch (region->type) {
        ...
        if (!info)
          ...
          for (; tp_node; tp_node = TREE_CHAIN(tp_node)) {
            ...
            if (!check_handled(info->types_caught, type)) {
              info->types_caught = tree_cons(NULL, type, info->types_caught);
    ...
    case ERT_ALLOWED_EXCEPTIONS:
      ...
      if (info)
        info->types_allowed = tree_cons(NULL_TREE, region->u.allowed.type_list,
                                        info->types_allowed);
...
void foreach_reachable_handler(int region_number, bool is_resx,
                               void (*callback)(struct eh_region *, void *),
                               void *callback_data) {
  struct reachable_info info;
  struct eh_region *region;
...
rtx reachable_handlers(rtx insn) {
  ...
  foreach_reachable_handler(
      region_number, is_resx,
      (crtl->eh.built_landing_pads ? arh_to_landing_pad : arh_to_label),
      &handlers);
...
unsigned int set_nothrow_function_flags(void) {
  ...
  for (insn = crtl->epilogue_delay_list; insn; insn = XEXP(insn, 1))
    if (can_throw_external(insn)) {
      TREE_NOTHROW(current_function_decl) = 0;
...
void expand_builtin_eh_return(tree stackadj_tree ATTRIBUTE_UNUSED,
  ...
  tmp = expand_expr(stackadj_tree, crtl->eh.ehr_stackadj, VOIDmode,
                    EXPAND_NORMAL);
  ...
  tmp =
      expand_expr(handler_tree, crtl->eh.ehr_handler, VOIDmode, EXPAND_NORMAL);
  tmp = convert_memory_address(Pmode, tmp);
...
static int action_record_eq(const void *pentry, const void *pdata) {
  const struct action_record *entry = (const struct action_record *)pentry;
  const struct action_record *data = (const struct action_record *)pdata;
  return entry->filter == data->filter && entry->next == data->next;
}

static hashval_t action_record_hash(const void *pentry) {
  const struct action_record *entry = (const struct action_record *)pentry;
    ...
    case ERT_TRY:
      ...
      for (c = region->u.eh_try.last_catch; c; c = c->u.eh_catch.prev_catch) {
        if (c->u.eh_catch.type_list == NULL) {
          ...
          int filter = TREE_INT_CST_LOW(TREE_VALUE(c->u.eh_catch.filter_list));
...
static int add_call_site(rtx landing_pad, int action) {
  ...
  return call_site_base +
         VEC_length(call_site_record, crtl->eh.call_site_record) - 1;
}
      ...
      if (last_action != this_action || last_landing_pad != this_landing_pad) {
        ...
        if (last_action >= -1) {
          ...
          if (first_no_action_insn) {
            ...
            note =
                emit_note_before(NOTE_INSN_EH_REGION_BEG, first_no_action_insn);
...
static void dw2_output_call_site_table(void) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    dw2_asm_output_delta_uleb128(
        reg_start_lab, current_function_func_begin_label, "region %d start", i);
    dw2_asm_output_delta_uleb128(reg_end_lab, reg_start_lab, "length");
    if (cs->landing_pad)
      dw2_asm_output_delta_uleb128(
          landing_pad_lab, current_function_func_begin_label, "landing pad");
    else
      dw2_asm_output_data_uleb128(0, "landing pad");
#else
    dw2_asm_output_delta(4, reg_start_lab, current_function_func_begin_label,
                         "region %d start", i);
    dw2_asm_output_delta(4, reg_end_lab, reg_start_lab, "length");
    if (cs->landing_pad)
      dw2_asm_output_delta(4, landing_pad_lab,
                           current_function_func_begin_label, "landing pad");
    else
...
#endif
        exception_section = s = get_section(".gcc_except_table", flags, NULL);
    } else
      exception_section = s = flag_pic ? data_section : readonly_data_section;
  }
...
static void output_ttype(tree type, int tt_format, int tt_format_size) {
  ...
  if (tt_format == DW_EH_PE_absptr || tt_format == DW_EH_PE_aligned)
    assemble_integer(value, tt_format_size, tt_format_size * BITS_PER_UNIT, 1);
  else
    dw2_asm_output_encoded_addr_rtx(tt_format, value, is_public, NULL);
}

void output_function_exception_table(const char *ARG_UNUSED(fnname)) {
  ...
  if (have_tt_data) {
    ...
    before_disp = 1 + 1;
    after_disp = (1 + size_of_uleb128(call_site_len) + call_site_len +
                  VARRAY_ACTIVE_SIZE(crtl->eh.action_record_data) +
                  (VEC_length(tree, crtl->eh.ttype_data) * tt_format_size));
...
void dump_eh_tree(FILE *out, struct function *fun) {
  ...
  static const char *const type_name[] = {
      "unknown",        "cleanup", "try", "catch", "allowed_exceptions",
      "must_not_throw", "throw"};
  ...
  fprintf(out, "Eh tree:\n");
  while (1) {
    fprintf(out, "  %*s %i %s", depth * 2, "", i->region_number,
            type_name[(int)i->type]);
    if (i->tree_label) {
...
void default_init_unwind_resume_libfunc(void) {
  ...
  unwind_resume_libfunc = init_one_libfunc(
      USING_SJLJ_EXCEPTIONS ? "_Unwind_SjLj_Resume" : "_Unwind_Resume");
}
