...
void init_expmed(void) {
  ...
  for (speed = 0; speed < 2; speed++) {
    ...
    for (mode = GET_CLASS_NARROWEST_MODE(MODE_INT); mode != VOIDmode;
      ...
      sdiv_pow2_cheap[speed][mode] =
          (rtx_cost(&all.sdiv_32, SET, speed) <= 2 * add_cost[speed][mode]);
      smod_pow2_cheap[speed][mode] =
          (rtx_cost(&all.smod_32, SET, speed) <= 4 * add_cost[speed][mode]);
      ...
      if (wider_mode != VOIDmode) {
        ...
        mul_highpart_cost[speed][mode] = rtx_cost(&all.wide_trunc, SET, speed);
      }
...
static bool check_predicate_volatile_ok(enum insn_code icode, int opno, rtx x,
...
static bool store_bit_field_1(rtx str_rtx, unsigned HOST_WIDE_INT bitsize,
  ...
  /* Use vec_set patterns for inserting parts of vectors whenever
     available.  */
  if (VECTOR_MODE_P(GET_MODE(op0)) && !MEM_P(op0) &&
  ...
  bitpos = bitnum % unit;
  byte_offset =
      (bitnum % BITS_PER_WORD) / BITS_PER_UNIT + (offset * UNITS_PER_WORD);
  ...
  if (bitpos == 0 && bitsize == GET_MODE_BITSIZE(fieldmode) &&
    ...
    if (MEM_P(op0))
      ...
      op0 = simplify_gen_subreg(fieldmode, op0, GET_MODE(op0), byte_offset);
  ...
  if (BYTES_BIG_ENDIAN && !MEM_P(op0) && unit > GET_MODE_BITSIZE(GET_MODE(op0)))
  ...
  if (bitsize > BITS_PER_WORD) {
    ...
    for (i = 0; i < nwords; i++) {
      ...
      unsigned int bit_offset =
          (backwards ? MAX((int)bitsize - ((int)i + 1) * BITS_PER_WORD, 0)
                     : (int)i * BITS_PER_WORD);
      rtx value_word = operand_subword_force(value, wordnum, fieldmode);

      if (!store_bit_field_1(
              op0, MIN(BITS_PER_WORD, bitsize - i * BITS_PER_WORD),
              bitnum + bit_offset, word_mode, value_word, fallback_p)) {
  ...
  if (!MEM_P(op0)) {
    if (offset != 0 || GET_MODE_SIZE(GET_MODE(op0)) > UNITS_PER_WORD) {
      ...
      op0 = gen_rtx_SUBREG(mode_for_size(BITS_PER_WORD, MODE_INT, 0), op0,
                           (offset * UNITS_PER_WORD));
    }
    offset = 0;
  }
  ...
  if (HAVE_insv && GET_MODE(value) != BLKmode && bitsize > 0 &&
    ...
    if (GET_MODE(value) != op_mode) {
      if (GET_MODE_BITSIZE(GET_MODE(value)) >= bitsize) {
        ...
        if (GET_MODE_SIZE(GET_MODE(value)) < GET_MODE_SIZE(op_mode)) {
          ...
          if (!tmp)
            tmp =
                simplify_gen_subreg(op_mode, force_reg(GET_MODE(value), value1),
                                    GET_MODE(value), 0);
          value1 = tmp;
        } else
    ...
    if (!((*insn_data[(int)CODE_FOR_insv].operand[3].predicate)(value1,
                                                                op_mode)))
    ...
    if (pat) {
      ...
      if (!TRULY_NOOP_TRUNCATION(GET_MODE_BITSIZE(GET_MODE(op0)),
          ...
          (REG_P(xop0) || GET_CODE(xop0) == SUBREG))
  ...
  if (HAVE_insv && MEM_P(op0)) {
    ...
    if (GET_MODE(op0) == BLKmode ||
      ...
      bestmode =
          get_best_mode(bitsize, bitnum, MEM_ALIGN(op0),
                        (op_mode == MAX_MACHINE_MODE ? VOIDmode : op_mode),
                        MEM_VOLATILE_P(op0));
    else
    ...
    if (bestmode != VOIDmode &&
      ...
      if (store_bit_field_1(tempreg, bitsize, xbitpos, fieldmode, orig_value,
                            false)) {
...
void store_bit_field(rtx str_rtx, unsigned HOST_WIDE_INT bitsize,
...
static void store_fixed_bit_field(rtx op0, unsigned HOST_WIDE_INT offset,
  ...
  if (GET_CODE(value) == CONST_INT) {
    ...
    if (must_and)
      value =
          expand_binop(mode, and_optab, value, mask_rtx(mode, 0, bitsize, 0),
  ...
  if (!all_one) {
    temp =
        expand_binop(mode, and_optab, temp, mask_rtx(mode, bitpos, bitsize, 1),
                     NULL_RTX, 1, OPTAB_LIB_WIDEN);
  ...
  if (!all_zero) {
    temp = expand_binop(mode, ior_optab, temp, value, NULL_RTX, 1,
                        OPTAB_LIB_WIDEN);
    temp = force_reg(mode, temp);
  }
...
static void store_split_bit_field(rtx op0, unsigned HOST_WIDE_INT bitsize,
                                  unsigned HOST_WIDE_INT bitpos, rtx value) {
  unsigned int unit;
  ...
  if (CONSTANT_P(value) && GET_CODE(value) != CONST_INT) {
    ...
    if (word && (value != word))
      ...
      value = gen_lowpart_common(
          word_mode,
          force_reg(GET_MODE(value) != VOIDmode ? GET_MODE(value) : word_mode,
                    value));
  }
  ...
  while (bitsdone < bitsize) {
    ...
    if (BYTES_BIG_ENDIAN) {
      ...
      if (GET_CODE(value) == CONST_INT)
        ...
        part = extract_fixed_bit_field(word_mode, value, 0, thissize, bitsdone,
                                       NULL_RTX, 1);
    }
...
static rtx convert_extracted_bit_field(rtx x, enum machine_mode mode,
...
static rtx extract_bit_field_1(rtx str_rtx, unsigned HOST_WIDE_INT bitsize,
                               unsigned HOST_WIDE_INT bitnum, int unsignedp,
                               rtx target, enum machine_mode mode,
                               enum machine_mode tmode, bool fallback_p) {
  unsigned int unit = (MEM_P(str_rtx)) ? BITS_PER_UNIT : BITS_PER_WORD;
  unsigned HOST_WIDE_INT offset, bitpos;
  rtx op0 = str_rtx;
  ...
  if (REG_P(op0) && mode == GET_MODE(op0) && bitnum == 0 &&
     ...
     if we aren't.  */
  {
    ...
    if (imode != GET_MODE(op0)) {
      if (MEM_P(op0))
        ...
        rtx reg, subreg;
        imode =
            smallest_mode_for_size(GET_MODE_BITSIZE(GET_MODE(op0)), MODE_INT);
        ...
        rtx mem =
            assign_stack_temp(GET_MODE(op0), GET_MODE_SIZE(GET_MODE(op0)), 0);
  ...
  if (((bitsize >= BITS_PER_WORD && bitsize == GET_MODE_BITSIZE(mode) &&
       ...
       (mode1 != BLKmode
        ...
        && (BYTES_BIG_ENDIAN ? bitpos + bitsize == BITS_PER_WORD
                             : bitpos == 0))) &&
      ((!MEM_P(op0) &&
        TRULY_NOOP_TRUNCATION(GET_MODE_BITSIZE(mode1),
                              GET_MODE_BITSIZE(GET_MODE(op0))) &&
        GET_MODE_SIZE(mode1) != 0 && byte_offset % GET_MODE_SIZE(mode1) == 0) ||
       (MEM_P(op0) && (!SLOW_UNALIGNED_ACCESS(mode, MEM_ALIGN(op0)) ||
                       (offset * BITS_PER_UNIT % bitsize == 0 &&
                        MEM_ALIGN(op0) % bitsize == 0))))) {
    if (MEM_P(op0))
      op0 = adjust_address(op0, mode1, offset);
    else if (mode1 != GET_MODE(op0)) {
      rtx sub = simplify_gen_subreg(mode1, op0, GET_MODE(op0), byte_offset);
      if (sub == NULL)
...
no_subreg_mode_swap:
  ...
  if (bitsize > BITS_PER_WORD) {
    ...
    for (i = 0; i < nwords; i++) {
      ...
      unsigned int bit_offset =
          (WORDS_BIG_ENDIAN
               ? MAX(0, ((int)bitsize - ((int)i + 1) * (int)BITS_PER_WORD))
               : (int)i * BITS_PER_WORD);
      rtx target_part = operand_subword(target, wordnum, 1, VOIDmode);
      rtx result_part = extract_bit_field(
          op0, MIN(BITS_PER_WORD, bitsize - i * BITS_PER_WORD),
          bitnum + bit_offset, 1, target_part, mode, word_mode);
    ...
    if (unsignedp) {
      ...
      if (GET_MODE_SIZE(GET_MODE(target)) > nwords * UNITS_PER_WORD) {
        ...
        for (i = nwords; i < total_words; i++)
          emit_move_insn(operand_subword(
                             target, WORDS_BIG_ENDIAN ? total_words - i - 1 : i,
                             1, VOIDmode),
                         const0_rtx);
      }
      return target;
    ...
    target =
        expand_shift(LSHIFT_EXPR, mode, target,
                     build_int_cst(NULL_TREE, GET_MODE_BITSIZE(mode) - bitsize),
                     NULL_RTX, 0);
    return expand_shift(
        RSHIFT_EXPR, mode, target,
        build_int_cst(NULL_TREE, GET_MODE_BITSIZE(mode) - bitsize), NULL_RTX,
        0);
  }
  ...
  if (ext_mode != MAX_MACHINE_MODE && bitsize > 0 &&
    ...
    if (GET_MODE(xtarget) != ext_mode) {
      ...
      if (REG_P(xtarget) &&
        ...
        if (GET_MODE_SIZE(ext_mode) > GET_MODE_SIZE(GET_MODE(xspec_target)))
    ...
    pat = (unsignedp ? gen_extzv(xtarget, xop0, bitsize_rtx, bitpos_rtx)
  ...
  if (ext_mode != MAX_MACHINE_MODE && MEM_P(op0)) {
    ...
    if (GET_MODE(op0) == BLKmode ||
      ...
      bestmode =
          get_best_mode(bitsize, bitnum, MEM_ALIGN(op0),
                        (ext_mode == MAX_MACHINE_MODE ? VOIDmode : ext_mode),
                        MEM_VOLATILE_P(op0));
    else
      bestmode = GET_MODE(op0);
    ...
    if (bestmode != VOIDmode &&
      ...
      if (xoffset * BITS_PER_UNIT + unit >= offset * BITS_PER_UNIT + bitsize) {
        ...
        result = extract_bit_field_1(xop0, bitsize, xbitpos, unsignedp, target,
                                     mode, tmode, false);
        if (result)
          return result;
...
rtx extract_bit_field(rtx str_rtx, unsigned HOST_WIDE_INT bitsize,
                      unsigned HOST_WIDE_INT bitnum, int unsignedp, rtx target,
                      enum machine_mode mode, enum machine_mode tmode) {
  return extract_bit_field_1(str_rtx, bitsize, bitnum, unsignedp, target, mode,
                             tmode, true);
}
...
static rtx extract_fixed_bit_field(enum machine_mode tmode, rtx op0,
                                   unsigned HOST_WIDE_INT offset,
                                   unsigned HOST_WIDE_INT bitsize,
  ...
  if (GET_CODE(op0) == SUBREG || REG_P(op0)) {
    ...
    if (mode == VOIDmode)
      ...
      return extract_split_bit_field(
          op0, bitsize, bitpos + offset * BITS_PER_UNIT, unsignedp);
  ...
  if (unsignedp) {
    ...
    if (GET_MODE_BITSIZE(mode) != bitpos + bitsize)
      return expand_binop(GET_MODE(op0), and_optab, op0,
                          mask_rtx(GET_MODE(op0), 0, bitsize, 0), target, 1,
                          OPTAB_LIB_WIDEN);
    return op0;
  }
  ...
  if (GET_MODE_BITSIZE(mode) != (bitsize + bitpos)) {
    tree amount =
        build_int_cst(NULL_TREE, GET_MODE_BITSIZE(mode) - (bitsize + bitpos));
    /* Maybe propagate the target for the shift.  */
    rtx subtarget = (target != 0 && REG_P(target) ? target : 0);
  ...
  return expand_shift(
      RSHIFT_EXPR, mode, op0,
      build_int_cst(NULL_TREE, GET_MODE_BITSIZE(mode) - bitsize), target, 0);
}
...
static rtx mask_rtx(enum machine_mode mode, int bitpos, int bitsize,
                    int complement) {
...
static rtx lshift_value(enum machine_mode mode, rtx value, int bitpos,
                        int bitsize) {
...
static rtx extract_split_bit_field(rtx op0, unsigned HOST_WIDE_INT bitsize,
  ...
  while (bitsdone < bitsize) {
    ...
    part =
        extract_fixed_bit_field(word_mode, word, offset * unit / BITS_PER_UNIT,
    ...
    if (BYTES_BIG_ENDIAN) {
      if (bitsize != bitsdone)
        part = expand_shift(LSHIFT_EXPR, word_mode, part,
                            build_int_cst(NULL_TREE, bitsize - bitsdone), 0, 1);
    } else {
      if (bitsdone != thissize)
        part =
            expand_shift(LSHIFT_EXPR, word_mode, part,
                         build_int_cst(NULL_TREE, bitsdone - thissize), 0, 1);
    }
...
rtx extract_low_bits(enum machine_mode mode, enum machine_mode src_mode,
                     rtx src) {
...
void expand_inc(rtx target, rtx inc) {
  rtx value = expand_binop(GET_MODE(target), add_optab, target, inc, target, 0,
                           OPTAB_LIB_WIDEN);
  if (value != target)
...
void expand_dec(rtx target, rtx dec) {
  rtx value = expand_binop(GET_MODE(target), sub_optab, target, dec, target, 0,
                           OPTAB_LIB_WIDEN);
  if (value != target)
    emit_move_insn(target, value);
}
...
rtx expand_shift(enum tree_code code, enum machine_mode mode, rtx shifted,
  ...
  if (SHIFT_COUNT_TRUNCATED) {
    if (GET_CODE(op1) == CONST_INT &&
      ...
      op1 =
          GEN_INT((unsigned HOST_WIDE_INT)INTVAL(op1) % GET_MODE_BITSIZE(mode));
  ...
  if (code == LSHIFT_EXPR && GET_CODE(op1) == CONST_INT && INTVAL(op1) > 0 &&
      INTVAL(op1) < GET_MODE_BITSIZE(mode) && INTVAL(op1) < MAX_BITS_PER_WORD &&
    ...
    for (i = 0; i < INTVAL(op1); i++) {
      ...
      shifted = expand_binop(mode, add_optab, temp, temp, NULL_RTX, unsignedp,
                             OPTAB_LIB_WIDEN);
    }
    return shifted;
  }
  ...
  for (attempt = 0; temp == 0 && attempt < 3; attempt++) {
    ...
    if (rotate) {
      ...
      if (methods == OPTAB_WIDEN)
        ...
        if (GET_MODE(op1) != TYPE_MODE(type) && GET_MODE(op1) != VOIDmode)
          op1 = convert_to_mode(TYPE_MODE(type), op1, 1);
        new_amount = make_tree(type, op1);
        other_amount = fold_build2(MINUS_EXPR, type,
                                   build_int_cst(type, GET_MODE_BITSIZE(mode)),
                                   new_amount);

        shifted = force_reg(mode, shifted);

        temp = expand_shift(left ? LSHIFT_EXPR : RSHIFT_EXPR, mode, shifted,
                            new_amount, 0, 1);
        temp1 = expand_shift(left ? RSHIFT_EXPR : LSHIFT_EXPR, mode, shifted,
                             other_amount, subtarget, 1);
        return expand_binop(mode, ior_optab, temp, temp1, target, unsignedp,
                            methods);
      }

      temp = expand_binop(mode, left ? lrotate_optab : rrotate_optab, shifted,
                          op1, target, unsignedp, methods);
    } else if (unsignedp)
      temp = expand_binop(mode, left ? lshift_optab : rshift_uns_optab, shifted,
                          op1, target, unsignedp, methods);
    ...
    if (temp == 0 && !rotate &&
      ...
      temp = expand_binop(mode, left ? lshift_optab : rshift_arith_optab,
...
static rtx expand_mult_highpart_optab(enum machine_mode, rtx, rtx, rtx, int,
                                      int);
...
static void synth_mult(struct algorithm *alg_out, unsigned HOST_WIDE_INT t,
  ...
  if (alg_hash[hash_index].t == t && alg_hash[hash_index].mode == mode &&
...
static bool choose_mult_variant(enum machine_mode mode, HOST_WIDE_INT val,
                                struct algorithm *alg,
                                enum mult_variant *variant, int mult_cost) {
...
static rtx expand_mult_const(enum machine_mode mode, rtx op0, HOST_WIDE_INT val,
  ...
  for (opno = 1; opno < alg->ops; opno++) {
    ...
    rtx add_target = (opno == alg->ops - 1 && target != 0 &&
                      variant != add_variant && !optimize)
                         ? target
                         : 0;
    ...
    switch (alg->op[opno]) {
      case alg_shift:
        accum = expand_shift(LSHIFT_EXPR, mode, accum,
                             build_int_cst(NULL_TREE, log), NULL_RTX, 0);
        val_so_far <<= log;
        break;
      ...
      case alg_add_t2_m:
        accum = expand_shift(LSHIFT_EXPR, mode, accum,
                             build_int_cst(NULL_TREE, log), shift_subtarget, 0);
      ...
      case alg_sub_factor:
        ...
        accum = force_operand(gen_rtx_MINUS(mode, tem, accum),
                              (add_target ? add_target : (optimize ? 0 : tem)));
...
rtx expand_mult(enum machine_mode mode, rtx op0, rtx op1, rtx target,
                int unsignedp) {
  enum mult_variant variant;
  ...
  if (op1 == constm1_rtx)
    return expand_unop(
        mode,
        GET_MODE_CLASS(mode) == MODE_INT && !unsignedp && flag_trapv
            ? negv_optab
            : neg_optab,
        op0, target, 0);
  ...
  if (SCALAR_INT_MODE_P(mode) && (unsignedp || !flag_trapv)) {
    ...
    if (GET_CODE(op1) == CONST_INT) {
      ...
      if (INTVAL(op1) < 0 && GET_MODE_BITSIZE(mode) > HOST_BITS_PER_WIDE_INT) {
        ...
        max_cost = rtx_cost(gen_rtx_MULT(mode, fake_reg, op1), SET, speed) -
                   neg_cost[speed][mode];
        if (max_cost > 0 && choose_mult_variant(mode, -INTVAL(op1), &algorithm,
                                                &variant, max_cost)) {
          rtx temp = expand_mult_const(mode, op0, -INTVAL(op1), NULL_RTX,
                                       &algorithm, variant);
      ...
      if (CONST_DOUBLE_HIGH(op1) == 0 && CONST_DOUBLE_LOW(op1) > 0)
        ...
        int shift = floor_log2(CONST_DOUBLE_HIGH(op1)) + HOST_BITS_PER_WIDE_INT;
        return expand_shift(LSHIFT_EXPR, mode, op0,
                            build_int_cst(NULL_TREE, shift), target, unsignedp);
      }
    }
    ...
    if (coeff != 0) {
      ...
      if (EXACT_POWER_OF_2_OR_ZERO_P(coeff))
        return expand_shift(LSHIFT_EXPR, mode, op0,
                            build_int_cst(NULL_TREE, floor_log2(coeff)), target,
                            unsignedp);
      ...
      if (choose_mult_variant(mode, coeff, &algorithm, &variant, max_cost))
        return expand_mult_const(mode, op0, coeff, target, &algorithm, variant);
    }
  }
  ...
  /* Expand x*2.0 as x+x.  */
  if (GET_CODE(op1) == CONST_DOUBLE && SCALAR_FLOAT_MODE_P(mode)) {
    REAL_VALUE_TYPE d;
    REAL_VALUE_FROM_CONST_DOUBLE(d, op1);

    if (REAL_VALUES_EQUAL(d, dconst2)) {
      op0 = force_reg(GET_MODE(op0), op0);
      return expand_binop(mode, add_optab, op0, op0, target, unsignedp,
                          OPTAB_LIB_WIDEN);
    }
  }
  ...
  op0 = expand_binop(
      mode,
      !unsignedp && flag_trapv && (GET_MODE_CLASS(mode) == MODE_INT)
          ? smulv_optab
          : smul_optab,
...
static unsigned HOST_WIDE_INT choose_multiplier(unsigned HOST_WIDE_INT d, int n,
                                                int precision,
                                                rtx *multiplier_ptr,
                                                int *post_shift_ptr,
                                                int *lgup_ptr) {
  ...
  div_and_round_double(TRUNC_DIV_EXPR, 1, nl, nh, d, (HOST_WIDE_INT)0, &mlow_lo,
                       &mlow_hi, &dummy1, &dummy2);
  ...
  gcc_assert(mlow_hi < mhigh_hi || (mlow_hi == mhigh_hi && mlow_lo < mhigh_lo));
...
static unsigned HOST_WIDE_INT invert_mod2n(unsigned HOST_WIDE_INT x, int n) {
  ...
  mask = (n == HOST_BITS_PER_WIDE_INT ? ~(unsigned HOST_WIDE_INT)0
...
rtx expand_mult_highpart_adjust(enum machine_mode mode, rtx adj_operand,
  ...
  adj_operand = force_operand(gen_rtx_fmt_ee(adj_code, mode, adj_operand, tem),
                              adj_operand);
  ...
  target =
      force_operand(gen_rtx_fmt_ee(adj_code, mode, adj_operand, tem), target);
...
static rtx extract_high_half(enum machine_mode mode, rtx op) {
...
static rtx expand_mult_highpart_optab(enum machine_mode mode, rtx op0, rtx op1,
  ...
  if (mul_highpart_cost[speed][mode] < max_cost) {
    ...
    tem = expand_binop(mode, moptab, op0, narrow_op1, target, unsignedp,
                       OPTAB_DIRECT);
    if (tem)
      return tem;
  }
  ...
  if (size - 1 < BITS_PER_WORD &&
    ...
    if (tem)
      ...
      return expand_mult_highpart_adjust(mode, tem, op0, narrow_op1, tem,
                                         unsignedp);
  }
  ...
  if (optab_handler(moptab, wider_mode)->insn_code != CODE_FOR_nothing &&
    ...
    tem = expand_binop(wider_mode, moptab, op0, narrow_op1, 0, unsignedp,
                       OPTAB_WIDEN);
    if (tem)
  ...
  if (optab_handler(smul_optab, wider_mode)->insn_code != CODE_FOR_nothing &&
    ...
    tem = expand_binop(wider_mode, smul_optab, wop0, wop1, 0, unsignedp,
                       OPTAB_WIDEN);
    insns = get_insns();
    end_sequence();
  ...
  if (optab_handler(moptab, wider_mode)->insn_code != CODE_FOR_nothing &&
      size - 1 < BITS_PER_WORD &&
      (mul_widen_cost[speed][wider_mode] +
           2 * shift_cost[speed][mode][size - 1] + 4 * add_cost[speed][mode] <
       max_cost)) {
    tem = expand_binop(wider_mode, moptab, op0, narrow_op1, NULL_RTX,
                       !unsignedp, OPTAB_WIDEN);
    if (tem != 0) {
      tem = extract_high_half(mode, tem);
      /* We used the wrong signedness.  Adjust the result.  */
      return expand_mult_highpart_adjust(mode, tem, op0, narrow_op1, target,
                                         unsignedp);
    }
  }
...
static rtx expand_mult_highpart(enum machine_mode mode, rtx op0, rtx op1,
                                rtx target, int unsignedp, int max_cost) {
  ...
  if (GET_MODE_BITSIZE(wider_mode) > BITS_PER_WORD)
    return expand_mult_highpart_optab(mode, op0, op1, target, unsignedp,
                                      max_cost);
...
static rtx expand_smod_pow2(enum machine_mode mode, rtx op0, HOST_WIDE_INT d) {
  ...
  if (BRANCH_COST(optimize_insn_for_speed_p(), false) >= 2 &&
    ...
    rtx signmask = emit_store_flag(result, LT, op0, const0_rtx, mode, 0, -1);
    if (signmask) {
      ...
      if (optab_handler(lshr_optab, mode)->insn_code == CODE_FOR_nothing ||
        ...
        temp = expand_binop(mode, xor_optab, op0, signmask, NULL_RTX, 1,
                            OPTAB_LIB_WIDEN);
        temp = expand_binop(mode, sub_optab, temp, signmask, NULL_RTX, 1,
                            OPTAB_LIB_WIDEN);
        temp = expand_binop(mode, and_optab, temp, GEN_INT(masklow), NULL_RTX,
                            1, OPTAB_LIB_WIDEN);
        temp = expand_binop(mode, xor_optab, temp, signmask, NULL_RTX, 1,
                            OPTAB_LIB_WIDEN);
        temp = expand_binop(mode, sub_optab, temp, signmask, NULL_RTX, 1,
                            OPTAB_LIB_WIDEN);
      } else {
        signmask = expand_binop(mode, lshr_optab, signmask, shift, NULL_RTX, 1,
                                OPTAB_LIB_WIDEN);
        signmask = force_reg(mode, signmask);

        temp = expand_binop(mode, add_optab, op0, signmask, NULL_RTX, 1,
                            OPTAB_LIB_WIDEN);
        temp = expand_binop(mode, and_optab, temp, GEN_INT(masklow), NULL_RTX,
                            1, OPTAB_LIB_WIDEN);
        temp = expand_binop(mode, sub_optab, temp, signmask, NULL_RTX, 1,
  ...
  temp = expand_binop(mode, and_optab, op0,
                      immed_double_const(masklow, maskhigh, mode), result, 1,
                      OPTAB_LIB_WIDEN);
  if (temp != result)
  ...
  temp = expand_binop(mode, sub_optab, result, const1_rtx, result, 0,
                      OPTAB_LIB_WIDEN);
  ...
  temp = expand_binop(mode, ior_optab, temp,
                      immed_double_const(masklow, maskhigh, mode), result, 1,
                      OPTAB_LIB_WIDEN);
  temp = expand_binop(mode, add_optab, temp, const1_rtx, result, 0,
                      OPTAB_LIB_WIDEN);
  if (temp != result)
    emit_move_insn(result, temp);
  emit_label(label);
...
static rtx expand_sdiv_pow2(enum machine_mode mode, rtx op0, HOST_WIDE_INT d) {
  rtx temp, label;
  tree shift;
  int logd;
  ...
  if (d == 2 && BRANCH_COST(optimize_insn_for_speed_p(), false) >= 1) {
    ...
    temp =
        expand_binop(mode, add_optab, temp, op0, NULL_RTX, 0, OPTAB_LIB_WIDEN);
  ...
  if (BRANCH_COST(optimize_insn_for_speed_p(), false) >= 2) {
    ...
    temp = expand_binop(mode, add_optab, temp2, GEN_INT(d - 1), NULL_RTX, 0,
                        OPTAB_LIB_WIDEN);
    temp = force_reg(mode, temp);

    /* Construct "temp2 = (temp2 < 0) ? temp : temp2".  */
    temp2 = emit_conditional_move(temp2, LT, temp2, const0_rtx, mode, temp,
                                  temp2, mode, 0);
    if (temp2) {
      rtx seq = get_insns();
      end_sequence();
  ...
  if (BRANCH_COST(optimize_insn_for_speed_p(), false) >= 2) {
    ...
    if (shift_cost[optimize_insn_for_speed_p()][mode][ushift] >
        COSTS_N_INSNS(1))
      temp = expand_binop(mode, and_optab, temp, GEN_INT(d - 1), NULL_RTX, 0,
                          OPTAB_LIB_WIDEN);
    else
      temp = expand_shift(RSHIFT_EXPR, mode, temp,
                          build_int_cst(NULL_TREE, ushift), NULL_RTX, 1);
    temp =
        expand_binop(mode, add_optab, temp, op0, NULL_RTX, 0, OPTAB_LIB_WIDEN);
    return expand_shift(RSHIFT_EXPR, mode, temp, shift, NULL_RTX, 0);
  }
...
rtx expand_divmod(int rem_flag, enum tree_code code, enum machine_mode mode,
                  rtx op0, rtx op1, rtx target, int unsignedp) {
  ...
  if (!unsignedp && op1 == constm1_rtx) {
    ...
    return expand_unop(
        mode,
        flag_trapv && GET_MODE_CLASS(mode) == MODE_INT ? negv_optab : neg_optab,
        op0, target, 0);
  }
  ...
  if (target
      ...
      ((REG_P(target) && REG_FUNCTION_VALUE_P(target))
       ...
       || ((rem_flag || op1_is_constant) &&
           (reg_mentioned_p(target, op0) || (MEM_P(op0) && MEM_P(target)))) ||
       reg_mentioned_p(target, op1) || (MEM_P(op1) && MEM_P(target))))
    target = 0;
  ...
  /* Only deduct something for a REM if the last divide done was
     ...
     divide.  */
  max_cost = unsignedp ? udiv_cost[speed][compute_mode]
  ...
  if (op1 != const0_rtx)
    switch (code) {
      ...
      case TRUNC_DIV_EXPR:
        if (op1_is_constant) {
          if (unsignedp) {
            ...
            rtx ml;
            unsigned HOST_WIDE_INT d =
                (INTVAL(op1) & GET_MODE_MASK(compute_mode));
            ...
            if (EXACT_POWER_OF_2_OR_ZERO_P(d)) {
              ...
              if (d >= ((unsigned HOST_WIDE_INT)1 << (size - 1))) {
                ...
                if (mh != 0 && (d & 1) == 0) {
                  ...
                  mh = choose_multiplier(d >> pre_shift, size, size - pre_shift,
                                         &ml, &post_shift, &dummy);
                  gcc_assert(!mh);
                } else
                  pre_shift = 0;
                ...
                if (mh != 0) {
                  ...
                  t1 = expand_mult_highpart(compute_mode, op0, ml, NULL_RTX, 1,
                                            max_cost - extra_cost);
                  if (t1 == 0)
                    goto fail1;
                  t2 = force_operand(gen_rtx_MINUS(compute_mode, op0, t1),
                                     NULL_RTX);
                  t3 = expand_shift(RSHIFT_EXPR, compute_mode, t2,
                                    build_int_cst(NULL_TREE, 1), NULL_RTX, 1);
                  t4 = force_operand(gen_rtx_PLUS(compute_mode, t1, t3),
                                     NULL_RTX);
                  quotient = expand_shift(
                      RSHIFT_EXPR, compute_mode, t4,
                      build_int_cst(NULL_TREE, post_shift - 1), tquotient, 1);
                } else {
                  rtx t1, t2;
                  ...
                  t1 = expand_shift(RSHIFT_EXPR, compute_mode, op0,
                                    build_int_cst(NULL_TREE, pre_shift),
                                    NULL_RTX, 1);
                  extra_cost = (shift_cost[speed][compute_mode][pre_shift] +
                                shift_cost[speed][compute_mode][post_shift]);
                  t2 = expand_mult_highpart(compute_mode, t1, ml, NULL_RTX, 1,
                                            max_cost - extra_cost);
                  if (t2 == 0)
                    goto fail1;
          ...
          } else /* TRUNC_DIV, signed */
          {
            ...
            abs_d = (d >= 0 ? (unsigned HOST_WIDE_INT)d
                            : -(unsigned HOST_WIDE_INT)d);
            ...
            if (d == 1)
              ...
              quotient =
                  expand_unop(compute_mode, neg_optab, op0, tquotient, 0);
            else if (abs_d == (unsigned HOST_WIDE_INT)1 << (size - 1)) {
              /* This case is not handled correctly below.  */
              quotient =
                  emit_store_flag(tquotient, EQ, op0, op1, compute_mode, 1, 1);
              if (quotient == 0)
                goto fail1;
                       ...
                       && ((optab_handler((rem_flag ? smod_optab : sdiv_optab),
                                          compute_mode)
              ...
              if (sdiv_pow2_cheap[speed][compute_mode] &&
                  ((optab_handler(sdiv_optab, compute_mode)->insn_code !=
                   ...
                   (optab_handler(sdivmod_optab, compute_mode)->insn_code !=
                    CODE_FOR_nothing)))
                quotient = expand_divmod(0, TRUNC_DIV_EXPR, compute_mode, op0,
                                         gen_int_mode(abs_d, compute_mode),
                                         NULL_RTX, 0);
              else
              ...
              if (d < 0) {
                ...
                quotient =
                    expand_unop(compute_mode, neg_optab, quotient, quotient, 0);
              }
            } else if (size <= HOST_BITS_PER_WIDE_INT) {
              choose_multiplier(abs_d, size, size - 1, &mlr, &post_shift,
                                &lgup);
              ...
              if (ml < (unsigned HOST_WIDE_INT)1 << (size - 1)) {
                ...
                t1 = expand_mult_highpart(compute_mode, op0, mlr, NULL_RTX, 0,
                                          max_cost - extra_cost);
                if (t1 == 0)
                  goto fail1;
                ...
                t3 = expand_shift(RSHIFT_EXPR, compute_mode, op0,
                                  build_int_cst(NULL_TREE, size - 1), NULL_RTX,
                                  0);
                if (d < 0)
      ...
      case FLOOR_MOD_EXPR:
        ...
        if (op1_is_constant && HOST_BITS_PER_WIDE_INT >= size) {
          ...
          if (d > 0) {
            ...
            if (EXACT_POWER_OF_2_OR_ZERO_P(d)) {
              ...
              mh =
                  choose_multiplier(d, size, size - 1, &ml, &post_shift, &lgup);
              gcc_assert(!mh);
              ...
              if (post_shift < BITS_PER_WORD && size - 1 < BITS_PER_WORD) {
                t1 = expand_shift(RSHIFT_EXPR, compute_mode, op0,
                                  build_int_cst(NULL_TREE, size - 1), NULL_RTX,
                                  0);
                t2 = expand_binop(compute_mode, xor_optab, op0, t1, NULL_RTX, 0,
                                  OPTAB_WIDEN);
                extra_cost = (shift_cost[speed][compute_mode][post_shift] +
                              shift_cost[speed][compute_mode][size - 1] +
                              2 * add_cost[speed][compute_mode]);
                t3 = expand_mult_highpart(compute_mode, t2, ml, NULL_RTX, 1,
                                          max_cost - extra_cost);
                if (t3 != 0) {
                  t4 = expand_shift(RSHIFT_EXPR, compute_mode, t3,
                                    build_int_cst(NULL_TREE, post_shift),
                                    NULL_RTX, 1);
                  quotient = expand_binop(compute_mode, xor_optab, t4, t1,
                                          tquotient, 0, OPTAB_WIDEN);
                }
              }
            }
          } else {
            rtx nsign, t1, t2, t3, t4;
            t1 = force_operand(gen_rtx_PLUS(compute_mode, op0, constm1_rtx),
                               NULL_RTX);
            t2 = expand_binop(compute_mode, ior_optab, op0, t1, NULL_RTX, 0,
                              OPTAB_WIDEN);
            nsign =
                expand_shift(RSHIFT_EXPR, compute_mode, t2,
                             build_int_cst(NULL_TREE, size - 1), NULL_RTX, 0);
            t3 =
                force_operand(gen_rtx_MINUS(compute_mode, t1, nsign), NULL_RTX);
            t4 = expand_divmod(0, TRUNC_DIV_EXPR, compute_mode, t3, op1,
                               NULL_RTX, 0);
            if (t4) {
              rtx t5;
              t5 =
                  expand_unop(compute_mode, one_cmpl_optab, nsign, NULL_RTX, 0);
              quotient =
                  force_operand(gen_rtx_PLUS(compute_mode, t4, t5), tquotient);
            }
          }
        }
        ...
        if (rem_flag) {
          remainder = REG_P(target) ? target : gen_reg_rtx(compute_mode);
          quotient = gen_reg_rtx(compute_mode);
        } else {
          quotient = REG_P(target) ? target : gen_reg_rtx(compute_mode);
          remainder = gen_reg_rtx(compute_mode);
        }

        if (expand_twoval_binop(sdivmod_optab, op0, op1, quotient, remainder,
                                0)) {
          ...
          tem = expand_binop(compute_mode, xor_optab, op0, op1, NULL_RTX, 0,
                             OPTAB_WIDEN);
        ...
        if (unsignedp) {
          if (op1_is_constant && EXACT_POWER_OF_2_OR_ZERO_P(INTVAL(op1))) {
            ...
            t1 = expand_shift(RSHIFT_EXPR, compute_mode, op0,
                              ...
                              tquotient, 1);
            t2 = expand_binop(compute_mode, and_optab, op0, GEN_INT(d - 1),
                              NULL_RTX, 1, OPTAB_LIB_WIDEN);
            ...
            if (t3 == 0) {
              ...
              quotient =
                  force_operand(gen_rtx_PLUS(compute_mode, t1, t3), tquotient);
            break;
          }
          ...
          if (expand_twoval_binop(udivmod_optab, op0, op1, quotient, remainder,
                                  1)) {
            /* This could be computed with a branch-less sequence.
               Save that for later.  */
            rtx label = gen_label_rtx();
      ...
      case EXACT_DIV_EXPR:
        if (op1_is_constant && HOST_BITS_PER_WIDE_INT >= size) {
          ...
          t1 = expand_shift(RSHIFT_EXPR, compute_mode, op0,
                            build_int_cst(NULL_TREE, pre_shift), NULL_RTX,
                            unsignedp);
      ...
      case ROUND_MOD_EXPR:
        if (unsignedp) {
          ...
          tem = expand_binop(compute_mode, sub_optab, tem, mask, NULL_RTX, 0,
                             OPTAB_WIDEN);
          expand_inc(quotient, tem);
          tem = expand_binop(compute_mode, xor_optab, mask, op1, NULL_RTX, 0,
                             OPTAB_WIDEN);
          tem = expand_binop(compute_mode, sub_optab, tem, mask, NULL_RTX, 0,
                             OPTAB_WIDEN);
  ...
  if (quotient == 0) {
    ...
    if (rem_flag) {
      ...
      if (remainder == 0) {
        ...
        if (!expand_twoval_binop((unsignedp ? udivmod_optab : sdivmod_optab),
                                 op0, op1, NULL_RTX, remainder, unsignedp))
          remainder = 0;
      }
    ...
    quotient = sign_expand_binop(
        compute_mode, udiv_optab, sdiv_optab, op0, op1,
    ...
    if (quotient == 0) {
      ...
      quotient = gen_reg_rtx(compute_mode);
      if (!expand_twoval_binop(unsignedp ? udivmod_optab : sdivmod_optab, op0,
                               op1, quotient, NULL_RTX, unsignedp)) {
        quotient = 0;
        if (!rem_flag)
          /* Still no luck.  If we are not computing the remainder,
             use a library call for the quotient.  */
          quotient =
              sign_expand_binop(compute_mode, udiv_optab, sdiv_optab, op0, op1,
  ...
  if (rem_flag) {
    ...
    if (quotient == 0) {
      ...
      remainder = sign_expand_binop(compute_mode, umod_optab, smod_optab, op0,
                                    op1, target, unsignedp, OPTAB_LIB_WIDEN);
      /* No remainder function.  Try a quotient-and-remainder
         function, keeping the remainder.  */
      if (!remainder) {
        remainder = gen_reg_rtx(compute_mode);
        if (!expand_twoval_binop_libfunc(
                unsignedp ? udivmod_optab : sdivmod_optab, op0, op1, NULL_RTX,
                remainder, unsignedp ? UMOD : MOD))
          remainder = NULL_RTX;
      }
    } else {
      /* We divided.  Now finish doing X - Y * (X / Y).  */
      remainder = expand_mult(compute_mode, quotient, op1, NULL_RTX, unsignedp);
      remainder = expand_binop(compute_mode, sub_optab, op0, remainder, target,
  ...
  switch (GET_CODE(x)) {
    ...
    case CONST_DOUBLE:
      if (GET_MODE(x) == VOIDmode)
        t = build_int_cst_wide(type, CONST_DOUBLE_LOW(x), CONST_DOUBLE_HIGH(x));
      else {
        REAL_VALUE_TYPE d;
    ...
    case LSHIFTRT:
      ...
      return fold_convert(type, build2(RSHIFT_EXPR, t, make_tree(t, XEXP(x, 0)),
                                       make_tree(type, XEXP(x, 1))));
    ...
    case DIV:
      ...
      return fold_convert(type,
                          build2(TRUNC_DIV_EXPR, t, make_tree(t, XEXP(x, 0)),
...
static rtx emit_store_flag_1(rtx target, rtx subtarget, enum machine_mode mode,
                             int normalizep) {
  rtx op0;
...
rtx emit_store_flag(rtx target, enum rtx_code code, rtx op0, rtx op1,
  ...
  if (GET_MODE_BITSIZE(mode) == BITS_PER_WORD * 2 &&
    ...
    if ((code == EQ || code == NE) &&
      ...
      op0both =
          expand_binop(word_mode, op1 == const0_rtx ? ior_optab : and_optab,
                       op00, op01, NULL_RTX, unsignedp, OPTAB_DIRECT);

      if (op0both != 0)
        return emit_store_flag(target, code, op0both, op1, word_mode, unsignedp,
                               normalizep);
      ...
      return emit_store_flag(target, code, op0h, op1, word_mode, unsignedp,
                             normalizep);
    }
  }
  ...
  if (op1 == const0_rtx && (code == LT || code == GE) &&
    ...
    if (code == GE)
      op0 = expand_unop(mode, one_cmpl_optab, op0,
                        ((STORE_FLAG_VALUE == 1 || normalizep) ? 0 : subtarget),
                        0);
  ...
  if (icode != CODE_FOR_nothing) {
    ...
    comparison = compare_from_rtx(op0, op1, code, unsignedp, mode, NULL_RTX);
    ...
    if (pattern) {
      emit_insn(pattern);
      return emit_store_flag_1(target, subtarget, compare_mode, normalizep);
    }
  } else {
    ...
    if (icode != CODE_FOR_nothing) {
      ...
      if (compare_mode != mode) {
        cstore_op0 = convert_modes(compare_mode, mode, cstore_op0, unsignedp);
        cstore_op1 = convert_modes(compare_mode, mode, cstore_op1, unsignedp);
      }

      if (!insn_data[(int)icode].operand[2].predicate(cstore_op0, compare_mode))
        cstore_op0 = copy_to_mode_reg(compare_mode, cstore_op0);

      if (!insn_data[(int)icode].operand[3].predicate(cstore_op1, compare_mode))
        cstore_op1 = copy_to_mode_reg(compare_mode, cstore_op1);

      comparison = gen_rtx_fmt_ee(code, result_mode, cstore_op0, cstore_op1);
      subtarget = target;

      if (optimize ||
          !(insn_data[(int)icode].operand[0].predicate(subtarget, result_mode)))
        subtarget = gen_reg_rtx(result_mode);

      pattern = GEN_FCN(icode)(subtarget, comparison, cstore_op0, cstore_op1);

      if (pattern) {
        emit_insn(pattern);
        return emit_store_flag_1(target, subtarget, result_mode, normalizep);
      }
    }
  }
  ...
  if (BRANCH_COST(optimize_insn_for_speed_p(), false) > 0 &&
    ...
    if (tem != 0)
      tem = emit_store_flag(target, code, tem, const0_rtx, mode, unsignedp,
                            normalizep);
    if (tem == 0)
      delete_insns_since(last);
    return tem;
  ...
  if (BRANCH_COST(optimize_insn_for_speed_p(), false) == 0 ||
      ...
      (code != EQ && code != NE &&
       (BRANCH_COST(optimize_insn_for_speed_p(), false) <= 1 ||
        (code != LE && code != GT))))
    return 0;
  ...
  if (code == GT) {
    ...
    tem = expand_shift(RSHIFT_EXPR, mode, op0,
                       size_int(GET_MODE_BITSIZE(mode) - 1), subtarget, 0);
  ...
  if (code == EQ || code == NE) {
    ...
    if (tem != 0) {
      if (code == EQ)
        tem = expand_binop(mode, sub_optab, tem, const1_rtx, subtarget, 0,
                           OPTAB_WIDEN);
      else
    ...
    if (tem == 0 &&
        (code == NE || BRANCH_COST(optimize_insn_for_speed_p(), false) > 1)) {
      if (rtx_equal_p(subtarget, op0))
        subtarget = 0;
...
rtx emit_store_flag_force(rtx target, enum rtx_code code, rtx op0, rtx op1,
  ...
  label = gen_label_rtx();
  do_compare_rtx_and_jump(op0, op1, code, unsignedp, mode, NULL_RTX, NULL_RTX,
                          label);
...
static void do_cmp_and_jump(rtx arg1, rtx arg2, enum rtx_code op,
                            enum machine_mode mode, rtx label) {
  int unsignedp = (op == LTU || op == LEU || op == GTU || op == GEU);
  do_compare_rtx_and_jump(arg1, arg2, op, unsignedp, mode, NULL_RTX, NULL_RTX,
                          label);
}
