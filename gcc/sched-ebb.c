...
static void debug_ebb_dependencies(rtx head, rtx tail) {
  fprintf(sched_dump,
          ";;   --------------- forward dependences: ------------ \n");
...
static void begin_schedule_ready(rtx insn, rtx last) {
  sched_rgn_n_insns++;
  ...
  if (BLOCK_FOR_INSN(insn) == last_bb
    ...
    gcc_assert(BLOCK_FOR_INSN(insn) == last_bb &&
               !IS_SPECULATION_CHECK_P(insn) && BB_HEAD(last_bb) != insn &&
               BB_END(last_bb) == insn);
...
static const char *ebb_print_insn(const_rtx insn,
...
static int rank(rtx insn1, rtx insn2) {
...
static int ebb_contributes_to_priority(rtx next ATTRIBUTE_UNUSED,
...
void ebb_compute_jump_reg_dependencies(rtx insn, regset cond_set, regset used,
                                       regset set) {
...
static void add_deps_for_risky_insns(rtx head, rtx tail) {
  rtx insn, prev;
  int classification;
          ...
          if (!sched_insns_conditions_mutex_p(insn, prev)) {
            ...
            if (!(current_sched_info->flags & USE_DEPS_LIST)) {
              ...
              if ((current_sched_info->flags & DO_SPECULATION) &&
                ...
                DEP_STATUS(dep) =
                    set_dep_weak(DEP_ANTI, BEGIN_CONTROL, MAX_DEP_WEAK);
...
static basic_block schedule_ebb(rtx head, rtx tail) {
  ...
  {
    gcc_assert(first_bb != last_bb && EDGE_COUNT(last_bb->succs) == 0);
...
static void ebb_add_remove_insn(rtx insn ATTRIBUTE_UNUSED, int remove_p) {
  if (!remove_p)
    rgn_n_insns++;
  else
    rgn_n_insns--;
...
static void ebb_add_block(basic_block bb, basic_block after) {
...
static void ebb_fix_recovery_cfg(int bbi ATTRIBUTE_UNUSED, int jump_bbi,
                                 int jump_bb_nexti) {
