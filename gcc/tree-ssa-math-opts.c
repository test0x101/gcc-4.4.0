...
static struct occurrence *occ_new(basic_block bb, struct occurrence *children) {
...
static void insert_bb(struct occurrence *new_occ, basic_block idom,
...
static inline void register_division_in(basic_block bb) {
  struct occurrence *occ;
...
static inline bool is_division_by(gimple use_stmt, tree def) {
...
static void insert_reciprocals(gimple_stmt_iterator *def_gsi,
                               struct occurrence *occ, tree def, tree recip_def,
                               int threshold) {
  tree type;
  gimple new_stmt;
  ...
  if (!recip_def && (occ->bb_has_division || !flag_trapping_math) &&
...
static inline void replace_reciprocal(use_operand_p use_p) {
  gimple use_stmt = USE_STMT(use_p);
  basic_block bb = gimple_bb(use_stmt);
  struct occurrence *occ = (struct occurrence *)bb->aux;

  if (optimize_bb_for_speed_p(bb) && occ->recip_def &&
      use_stmt != occ->recip_def_stmt) {
...
static bool gate_cse_reciprocals(void) {
  return optimize && flag_reciprocal_math;
}

/* Go through all the floating-point SSA_NAMEs, and call
   execute_cse_reciprocals_1 on each of them.  */
static unsigned int execute_cse_reciprocals(void) {
  basic_block bb;
  tree arg;

  occ_pool =
      create_alloc_pool("dominators for recip", sizeof(struct occurrence),
                        n_basic_blocks / 3 + 1);
  ...
  for (arg = DECL_ARGUMENTS(cfun->decl); arg; arg = TREE_CHAIN(arg))
    if (gimple_default_def(cfun, arg) && FLOAT_TYPE_P(TREE_TYPE(arg)) &&
        is_gimple_reg(arg))
  ...
  FOR_EACH_BB(bb) {
    ...
    for (gsi = gsi_start_phis(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      phi = gsi_stmt(gsi);
      def = PHI_RESULT(phi);
      if (FLOAT_TYPE_P(TREE_TYPE(def)) && is_gimple_reg(def))
    ...
    for (gsi = gsi_after_labels(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (gimple_has_lhs(stmt) &&
          ...
          FLOAT_TYPE_P(TREE_TYPE(def)) && TREE_CODE(def) == SSA_NAME)
    ...
    for (gsi = gsi_after_labels(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (is_gimple_assign(stmt) && gimple_assign_rhs_code(stmt) == RDIV_EXPR) {
        ...
        if (is_gimple_call(stmt1) && gimple_call_lhs(stmt1) &&
...
static bool maybe_record_sincos(VEC(gimple, heap) * *stmts, basic_block *top_bb,
                                gimple use_stmt) {
  basic_block use_bb = gimple_bb(use_stmt);
  if (*top_bb &&
      (*top_bb == use_bb || dominated_by_p(CDI_DOMINATORS, use_bb, *top_bb)))
    VEC_safe_push(gimple, heap, *stmts, use_stmt);
  else if (!*top_bb || dominated_by_p(CDI_DOMINATORS, *top_bb, use_bb)) {
    VEC_safe_push(gimple, heap, *stmts, use_stmt);
    *top_bb = use_bb;
  } else
    return false;
...
static void execute_cse_sincos_1(tree name) {
  gimple_stmt_iterator gsi;
  imm_use_iterator use_iter;
  ...
  FOR_EACH_IMM_USE_STMT(use_stmt, use_iter, name) {
    if (gimple_code(use_stmt) != GIMPLE_CALL || !gimple_call_lhs(use_stmt) ||
    ...
    switch (DECL_FUNCTION_CODE(fndecl)) {
      ...
      CASE_FLT_FN(BUILT_IN_CEXPI)
          : seen_cexpi |=
            maybe_record_sincos(&stmts, &top_bb, use_stmt) ? 1 : 0;
      break;
  ...
  if (!SSA_NAME_IS_DEFAULT_DEF(name) && gimple_code(def_stmt) != GIMPLE_PHI &&
  ...
  for (i = 0; VEC_iterate(gimple, stmts, i, use_stmt); ++i) {
    ...
    switch (DECL_FUNCTION_CODE(fndecl)) {
      CASE_FLT_FN(BUILT_IN_COS) : rhs = fold_build1(REALPART_EXPR, type, res);
      break;

      CASE_FLT_FN(BUILT_IN_SIN) : rhs = fold_build1(IMAGPART_EXPR, type, res);
      break;

      CASE_FLT_FN(BUILT_IN_CEXPI) : rhs = res;
      break;
...
static unsigned int execute_cse_sincos(void) {
  basic_block bb;
    ...
    for (gsi = gsi_after_labels(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (is_gimple_call(stmt) && gimple_call_lhs(stmt) &&
        ...
        switch (DECL_FUNCTION_CODE(fndecl)) {
          CASE_FLT_FN(BUILT_IN_COS)
              : CASE_FLT_FN(BUILT_IN_SIN)
              : CASE_FLT_FN(BUILT_IN_CEXPI) : arg = gimple_call_arg(stmt, 0);
...
static bool gate_cse_sincos(void) {
  /* Make sure we have either sincos or cexp.  */
  return (TARGET_HAS_SINCOS || TARGET_C99_FUNCTIONS) && optimize;
}

struct gimple_opt_pass pass_cse_sincos = {{
    GIMPLE_PASS, "sincos", /* name */
...
static unsigned int execute_convert_to_rsqrt(void) {
  basic_block bb;
