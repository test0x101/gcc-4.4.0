...
static inline void set_control_dependence_map_bit(basic_block bb,
                                                  int edge_index) {
...
static inline void clear_control_dependence_bitmap(basic_block bb) {
...
static inline basic_block find_pdom(basic_block block) {
...
static void find_control_dependence(struct edge_list *el, int edge_index) {
...
static void find_all_control_dependences(struct edge_list *el) {
  int i;
...
static inline void mark_stmt_necessary(gimple stmt, bool add_to_worklist) {
  gcc_assert(stmt);
...
static inline void mark_operand_necessary(tree op) {
  gimple stmt;
  int ver;
...
static void mark_stmt_if_obviously_necessary(gimple stmt, bool aggressive) {
  ...
  if (flag_non_call_exceptions && stmt_could_throw_p(stmt)) {
  ...
  switch (gimple_code(stmt)) {
    ...
    case GIMPLE_ASSIGN:
      ...
      /* These values are mildly magic bits of the EH runtime.  We can't
         ...
         generated.  */
      if (TREE_CODE(lhs) == EXC_PTR_EXPR || TREE_CODE(lhs) == FILTER_EXPR) {
        mark_stmt_necessary(stmt, true);
        return;
      }
      break;
...
static void find_obviously_necessary_stmts(struct edge_list *el) {
  basic_block bb;
...
static void propagate_necessity(struct edge_list *el) {
  gimple stmt;
...
static void remove_dead_stmt(gimple_stmt_iterator *i, basic_block bb) {
  ...
  if (is_ctrl_stmt(stmt)) {
    ...
    if (!post_dom_bb || post_dom_bb == EXIT_BLOCK_PTR || phi_nodes(post_dom_bb))
      ;
    else {
...
/* Pass entry points.  */
static unsigned int tree_ssa_dce(void) {
  return perform_tree_ssa_dce(/*aggressive=*/false);
}

static unsigned int tree_ssa_dce_loop(void) {
  unsigned int todo;
...
static unsigned int tree_ssa_cd_dce(void) {
...
struct gimple_opt_pass pass_cd_dce = {{
    ...
    0,                    /* properties_destroyed */
    0,                    /* todo_flags_start */
    TODO_dump_func | TODO_verify_ssa | TODO_verify_flow /* todo_flags_finish */
}};
