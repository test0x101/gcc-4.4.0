...
unsigned int c_common_init_options(unsigned int argc, const char **argv) {
...
int c_common_handle_option(size_t scode, const char *arg, int value) {
  ...
  switch (code) {
    ...
    case OPT_fhandle_exceptions:
      warning(0,
              "-fhandle-exceptions has been renamed -fexceptions (and is now "
              "on by default)");
...
bool c_common_post_options(const char **pfilename) {
  ...
  register_include_chains(parse_in, sysroot, iprefix, imultilib, std_inc,
                          std_cxx_inc && c_dialect_cxx(), verbose);
  ...
  /* Special format checking options don't work without -Wformat; warn if
     they are used.  */
  if (!warn_format) {
    warning(OPT_Wformat_y2k, "-Wformat-y2k ignored without -Wformat");
    ...
    warning(OPT_Wformat_security, "-Wformat-security ignored without -Wformat");
  }
  ...
  if (flag_preprocess_only) {
    ...
    if (num_in_fnames > 1)
      error("too many filenames given.  Type %s --help for usage", progname);
  ...
  *pfilename = this_input_filename = cpp_read_main_file(parse_in, in_fnames[0]);
  ...
  if (flag_working_directory && flag_preprocess_only && !flag_no_line_commands)
...
void c_common_parse_file(int set_yydebug) {
  ...
  if (set_yydebug)
    switch (c_language) {
      ...
      case clk_objcxx:
        warning(
            0, "The Objective-C++ parser does not support -dy, option ignored");
        break;
      default:
        gcc_unreachable();
    }
...
static void sanitize_cpp_opts(void) {
  ...
  cpp_opts->warn_long_long =
      warn_long_long &&
      ((pedantic && (c_dialect_cxx() ? cxx_dialect == cxx98 : !flag_isoc99)) ||
       warn_traditional);
...
static void add_prefixed_path(const char *suffix, size_t chain) {
  char *path;
  const char *prefix;
...
static void finish_options(void) {
  if (!cpp_opts->preprocessed) {
    ...
    cb_file_change(parse_in, linemap_add(line_table, LC_RENAME, 0,
                                         _("<command-line>"), 0));
    ...
    for (i = 0; i < deferred_count; i++) {
      ...
      if (opt->code == OPT_imacros && cpp_push_include(parse_in, opt->arg)) {
...
static void cb_file_change(cpp_reader *ARG_UNUSED(pfile),
...
void cb_dir_change(cpp_reader *ARG_UNUSED(pfile), const char *dir) {
  if (!set_src_pwd(dir))
...
static void set_std_cxx0x(int iso) {
...
static void handle_OPT_d(const char *arg) {
  char c;
