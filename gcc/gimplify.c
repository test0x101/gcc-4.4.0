...
void push_gimplify_context(struct gimplify_ctx *c) {
  memset(c, '\0', sizeof(*c));
...
static void gimple_push_bind_expr(gimple gimple_bind) {
...
VEC(gimple, heap) * gimple_bind_expr_stack(void) {
...
static gimple gimplify_and_return_first(tree t, gimple_seq *seq_p) {
...
static tree find_single_pointer_decl_1(tree *tp,
...
static tree find_single_pointer_decl(tree t) {
  tree decl = NULL_TREE;
...
tree create_tmp_var_raw(tree type, const char *prefix) {
  ...
  tmp_var =
      build_decl(VAR_DECL, prefix ? create_tmp_var_name(prefix) : NULL, type);
...
static inline tree create_tmp_from_val(tree val) {
...
static bool is_gimple_formal_tmp_or_call_rhs(tree t) {
...
static bool is_gimple_reg_or_call_rhs(tree t) {
...
static bool is_gimple_mem_or_call_rhs(tree t) {
...
static tree internal_get_tmp_var(tree val, gimple_seq *pre_p,
                                 gimple_seq *post_p, bool is_formal) {
  tree t, mod;
...
tree get_initialized_tmp_var(tree val, gimple_seq *pre_p, gimple_seq *post_p) {
...
void declare_vars(tree vars, gimple scope, bool debug_info) {
  tree last = vars;
  if (last) {
    tree temps, block;
    ...
    if (!block || !debug_info) {
      ...
      if (BLOCK_VARS(block))
        ...
        gimple_bind_set_vars(scope, chainon(gimple_bind_vars(scope), temps));
...
static void force_constant_size(tree var) {
  ...
  DECL_SIZE_UNIT(var) = build_int_cst(TREE_TYPE(DECL_SIZE_UNIT(var)), max_size);
  DECL_SIZE(var) =
...
static bool tree_should_carry_location_p(const_tree stmt) {
  /* Don't emit a line note for a label.  We particularly don't want to
     emit one for the break label, since it doesn't actually correspond
     to the beginning of the loop/switch.  */
...
static void annotate_one_with_location(gimple gs, location_t location) {
  if (!gimple_has_location(gs) && !gimple_do_not_emit_location_p(gs) &&
...
static void tree_annotate_one_with_location(tree t, location_t location) {
...
void tree_annotate_all_with_location(tree *stmt_p, location_t location) {
  tree_stmt_iterator i;
  ...
  for (i = tsi_start(*stmt_p); !tsi_end_p(i); tsi_next(&i)) {
    ...
    gcc_assert(TREE_CODE(t) != STATEMENT_LIST && TREE_CODE(t) != COMPOUND_EXPR);
...
static tree mostly_copy_tree_r(tree *tp, int *walk_subtrees, void *data) {
  ...
  if (TREE_CODE_CLASS(code) == tcc_type ||
      ...
      TREE_CODE_CLASS(code) == tcc_constant || code == SAVE_EXPR ||
      code == TARGET_EXPR
...
static tree copy_if_shared_r(tree *tp, int *walk_subtrees ATTRIBUTE_UNUSED,
...
static tree unmark_visited_r(tree *tp, int *walk_subtrees ATTRIBUTE_UNUSED,
                             void *data ATTRIBUTE_UNUSED) {
  if (TREE_VISITED(*tp))
    TREE_VISITED(*tp) = 0;
  else
...
static void build_stack_save_restore(gimple *save, gimple *restore) {
  tree tmp_var;
    ...
    if (TREE_CODE(t) == VAR_DECL) {
      ...
      if (ctx && !is_global_var(t) &&
          (!DECL_SEEN_IN_BIND_EXPR_P(t) ||
           splay_tree_lookup(ctx->variables, (splay_tree_key)t) == NULL))
  ...
  *stmt_p = NULL_TREE;
  ...
  if (TREE_CODE(decl) == VAR_DECL && !DECL_EXTERNAL(decl)) {
    ...
    if (TREE_CODE(DECL_SIZE_UNIT(decl)) != INTEGER_CST ||
        (!TREE_STATIC(decl) && flag_stack_check == GENERIC_STACK_CHECK &&
         compare_tree_int(DECL_SIZE_UNIT(decl), STACK_CHECK_MAX_VAR_SIZE) > 0))
    ...
    if (!DECL_SEEN_IN_BIND_EXPR_P(decl) && DECL_ARTIFICIAL(decl) &&
...
void sort_case_labels(VEC(tree, heap) * label_vec) {
  size_t len = VEC_length(tree, label_vec);
  qsort(VEC_address(tree, label_vec), len, sizeof(tree), compare_case_labels);
}
...
static enum gimplify_status gimplify_switch_expr(tree *expr_p,
  ...
  if (SWITCH_BODY(switch_expr)) {
    ...
    gimple_switch =
        gimple_build_switch_vec(SWITCH_COND(switch_expr), default_case, labels);
...
static void canonicalize_component_ref(tree *expr_p) {
  ...
  if (TREE_TYPE(expr) != type) {
    ...
    /* We need to preserve qualifiers and propagate them from
       operand 0.  */
    type_quals =
        TYPE_QUALS(type) | TYPE_QUALS(TREE_TYPE(TREE_OPERAND(expr, 0)));
...
static void canonicalize_addr_expr(tree *expr_p) {
  ...
  if (!POINTER_TYPE_P(TREE_TYPE(expr)) || TREE_CODE(addr_expr) != ADDR_EXPR)
    return;
  ...
  *expr_p = build4(ARRAY_REF, ddatype, TREE_OPERAND(addr_expr, 0),
                   TYPE_MIN_VALUE(TYPE_DOMAIN(datype)), NULL_TREE, NULL_TREE);
...
static enum gimplify_status gimplify_conversion(tree *expr_p) {
  ...
  if (CONVERT_EXPR_P(*expr_p) && POINTER_TYPE_P(TREE_TYPE(*expr_p)) &&
      POINTER_TYPE_P(TREE_TYPE(TREE_OPERAND(*expr_p, 0))) &&
      (tem = maybe_fold_offset_to_address(
           TREE_OPERAND(*expr_p, 0), integer_zero_node, TREE_TYPE(*expr_p))) !=
          NULL_TREE)
    *expr_p = tem;
...
static enum gimplify_status gimplify_var_or_parm_decl(tree *expr_p) {
  ...
  if (TREE_CODE(decl) == VAR_DECL && !DECL_SEEN_IN_BIND_EXPR_P(decl) &&
...
static enum gimplify_status gimplify_compound_lval(tree *expr_p,
  ...
  tret =
      gimplify_expr(p, pre_p, post_p, is_gimple_min_lval, fallback | fb_lvalue);
  ret = MIN(ret, tret);
  ...
  for (; VEC_length(tree, stack) > 0;) {
    ...
    if (TREE_CODE(t) == ARRAY_REF || TREE_CODE(t) == ARRAY_RANGE_REF) {
      ...
      tret = gimplify_expr(&TREE_OPERAND(t, 1), pre_p, post_p, is_gimple_val,
                           fb_rvalue);
      ret = MIN(ret, tret);
      tret = gimplify_expr(&TREE_OPERAND(t, 2), pre_p, post_p, is_gimple_val,
                           fb_rvalue);
      ret = MIN(ret, tret);
    }
...
static void maybe_with_size_expr(tree *expr_p) {
  ...
  /* If we've already wrapped this or the type is error_mark_node, we can't do
     anything.  */
  if (TREE_CODE(expr) == WITH_SIZE_EXPR || type == error_mark_node)
    return;
...
static enum gimplify_status gimplify_arg(tree *arg_p, gimple_seq *pre_p,
...
static enum gimplify_status gimplify_call_expr(tree *expr_p, gimple_seq *pre_p,
                                               bool want_value) {
  tree fndecl, parms, p;
  enum gimplify_status ret;
  int i, nargs;
  ...
  ret = gimplify_expr(&CALL_EXPR_FN(*expr_p), pre_p, NULL, is_gimple_call_addr,
                      fb_rvalue);
  ...
  if (!p && i < nargs &&
    ...
    if (last_arg_fndecl && TREE_CODE(last_arg_fndecl) == FUNCTION_DECL &&
        DECL_BUILT_IN_CLASS(last_arg_fndecl) == BUILT_IN_NORMAL &&
        DECL_FUNCTION_CODE(last_arg_fndecl) == BUILT_IN_VA_ARG_PACK) {
      tree call = *expr_p;

      --nargs;
      *expr_p = build_call_array(TREE_TYPE(call), CALL_EXPR_FN(call), nargs,
                                 CALL_EXPR_ARGP(call));
      ...
      CALL_EXPR_RETURN_SLOT_OPT(*expr_p) = CALL_EXPR_RETURN_SLOT_OPT(call);
...
static tree shortcut_cond_r(tree pred, tree *true_label_p,
                            tree *false_label_p) {
  ...
  if (TREE_CODE(pred) == TRUTH_ANDIF_EXPR) {
    ...
    expr = build3(
        COND_EXPR, void_type_node, TREE_OPERAND(pred, 0),
        shortcut_cond_r(TREE_OPERAND(pred, 1), true_label_p, false_label_p),
        shortcut_cond_r(TREE_OPERAND(pred, 2), true_label_p, false_label_p));
  } else {
    expr = build3(COND_EXPR, void_type_node, pred, build_and_jump(true_label_p),
...
static tree shortcut_cond_expr(tree expr) {
  ...
  /* If we're done, great.  */
  if (TREE_CODE(pred) != TRUTH_ANDIF_EXPR && TREE_CODE(pred) != TRUTH_ORIF_EXPR)
    return expr;
  ...
  if (then_ && TREE_CODE(then_) == GOTO_EXPR &&
  ...
  if (else_ && TREE_CODE(else_) == GOTO_EXPR &&
...
static enum gimplify_status gimplify_pure_cond_expr(tree *expr_p,
  ...
  tret = gimplify_expr(&COND_EXPR_THEN(expr), pre_p, NULL, is_gimple_val,
                       fb_rvalue);
  ret = MIN(ret, tret);
  tret = gimplify_expr(&COND_EXPR_ELSE(expr), pre_p, NULL, is_gimple_val,
                       fb_rvalue);
...
static bool generic_expr_could_trap_p(tree expr) {
  unsigned i, n;
...
static enum gimplify_status gimplify_cond_expr(tree *expr_p, gimple_seq *pre_p,
                                               fallback_t fallback) {
  tree expr = *expr_p;
  ...
  if (!VOID_TYPE_P(type)) {
    ...
    if (((fallback & fb_rvalue) || !(fallback & fb_lvalue)) &&
      ...
      if (TREE_TYPE(TREE_OPERAND(expr, 1)) != void_type_node)
        TREE_OPERAND(expr, 1) = build_fold_addr_expr(TREE_OPERAND(expr, 1));

      if (TREE_TYPE(TREE_OPERAND(expr, 2)) != void_type_node)
        TREE_OPERAND(expr, 2) = build_fold_addr_expr(TREE_OPERAND(expr, 2));
  ...
  if (TREE_OPERAND(expr, 1) != NULL &&
      ...
      && (optimize || !EXPR_HAS_LOCATION(expr) ||
  ...
  gimple_cond =
      gimple_build_cond(pred_code, arm1, arm2, label_true, label_false);
  ...
  if (!have_then_clause_p) {
    /* For if (...) {} else { code; } put label_true after
       the else block.  */
    if (TREE_OPERAND(expr, 1) == NULL_TREE && !have_else_clause_p &&
...
static tree gimplify_init_ctor_preeval_1(tree *tp, int *walk_subtrees,
                                         void *xdata) {
    ...
    for (type = TYPE_ARG_TYPES(fntype); type; type = TREE_CHAIN(type))
      if (POINTER_TYPE_P(TREE_VALUE(type)) &&
          ...
          alias_sets_conflict_p(data->lhs_alias_set,
                                get_alias_set(TREE_TYPE(TREE_VALUE(type)))))
        return t;
  }
...
static void gimplify_init_ctor_preeval(
    tree *expr_p, gimple_seq *pre_p, gimple_seq *post_p,
    struct gimplify_init_ctor_preeval_data *data) {
...
static void gimplify_init_ctor_eval_range(tree object, tree lower, tree upper,
  ...
  /* Build the reference.  */
  cref = build4(ARRAY_REF, array_elt_type, unshare_expr(object), var, NULL_TREE,
                NULL_TREE);
  ...
  if (TREE_CODE(value) == CONSTRUCTOR)
    ...
    gimplify_init_ctor_eval(cref, CONSTRUCTOR_ELTS(value), pre_p, cleared);
  else
  ...
  gimplify_seq_add_stmt(
      pre_p,
      gimple_build_cond(EQ_EXPR, var, upper, loop_exit_label, fall_thru_label));
...
static bool zero_sized_field_decl(const_tree fdecl) {
...
static bool zero_sized_type(const_tree type) {
...
static void gimplify_init_ctor_eval(tree object,
  ...
  FOR_EACH_CONSTRUCTOR_ELT(elts, ix, purpose, value) {
    ...
    if (array_elt_type) {
      ...
      cref = build4(ARRAY_REF, array_elt_type, unshare_expr(object), purpose,
                    NULL_TREE, NULL_TREE);
    } else {
      gcc_assert(TREE_CODE(purpose) == FIELD_DECL);
      cref = build3(COMPONENT_REF, TREE_TYPE(purpose), unshare_expr(object),
                    purpose, NULL_TREE);
    }
...
static enum gimplify_status gimplify_init_constructor(
    tree *expr_p, gimple_seq *pre_p, gimple_seq *post_p, bool want_value,
  ...
  switch (TREE_CODE(type)) {
    ...
    case ARRAY_TYPE: {
      ...
      valid_const_initializer = categorize_ctor_elements(
          ctor, &num_nonzero_elements, &num_ctor_elements, &cleared);

      /* If a const aggregate variable is being initialized, then it
         should never be a lose to promote the variable to be static.  */
      if (valid_const_initializer && num_nonzero_elements > 1 &&
          TREE_READONLY(object) && TREE_CODE(object) == VAR_DECL &&
      ...
      if (valid_const_initializer && !(cleared || num_nonzero_elements == 0) &&
          !TREE_ADDRESSABLE(type)) {
        ...
        if (size > 0 && num_nonzero_elements > 1 &&
      ...
      if (num_nonzero_elements > 0) {
        ...
        gimplify_init_ctor_preeval(&TREE_OPERAND(*expr_p, 1), pre_p, post_p,
                                   &preeval_data);
      }
    ...
    case VECTOR_TYPE: {
      ...
      for (ix = 0; VEC_iterate(constructor_elt, elts, ix, ce); ix++) {
        ...
        tret =
            gimplify_expr(&ce->value, pre_p, post_p, is_gimple_val, fb_rvalue);
        if (tret == GS_ERROR)
          ret = GS_ERROR;
      }
...
static enum gimplify_status gimplify_modify_expr_rhs(tree *expr_p, tree *from_p,
                                                     tree *to_p,
                                                     gimple_seq *pre_p,
                                                     gimple_seq *post_p,
                                                     bool want_value) {
  ...
  while (ret != GS_UNHANDLED)
    switch (TREE_CODE(*from_p)) {
      case VAR_DECL:
        ...
        if (DECL_INITIAL(*from_p) && TREE_READONLY(*from_p) &&
      ...
      case COND_EXPR:
        ...
        if (!is_gimple_reg_type(TREE_TYPE(*from_p))) {
          ...
          ret = gimplify_expr(&result, pre_p, post_p, is_gimple_lvalue,
                              fb_lvalue);
          if (ret != GS_ERROR)
            ret = GS_OK;
          ...
          if (TREE_TYPE(TREE_OPERAND(cond, 1)) != void_type_node)
            TREE_OPERAND(cond, 1) =
                build2(code, void_type_node, result, TREE_OPERAND(cond, 1));
      ...
      case STATEMENT_LIST: {
        ...
        ret = gimplify_expr(to_p, pre_p, post_p, is_gimple_min_lval, fb_lvalue);
        if (ret != GS_ERROR)
          ret = GS_OK;
...
static enum gimplify_status gimplify_modify_expr(tree *expr_p,
  ...
  ret =
      gimplify_modify_expr_rhs(expr_p, from_p, to_p, pre_p, post_p, want_value);
  if (ret != GS_UNHANDLED)
    return ret;
  ...
  ret =
      gimplify_expr(from_p, pre_p, post_p, rhs_predicate_for(*to_p), fb_rvalue);
  if (ret == GS_ERROR)
    return ret;
  ...
  if (TREE_CODE(*from_p) == WITH_SIZE_EXPR) {
    ...
    if (is_gimple_addressable(from)) {
      *from_p = from;
      return gimplify_modify_expr_to_memcpy(expr_p, size, want_value, pre_p);
    }
  }
  ...
  if (!gimplify_ctxp->into_ssa && DECL_P(*from_p) && DECL_IGNORED_P(*from_p) &&
...
static enum gimplify_status gimplify_scalar_mode_aggregate_compare(
  ...
  *expr_p = fold_build2(TREE_CODE(*expr_p), TREE_TYPE(*expr_p), op0, op1);
...
static enum gimplify_status gimplify_boolean_expr(tree *expr_p) {
  ...
  *expr_p =
      build3(COND_EXPR, type, *expr_p, fold_convert(type, boolean_true_node),
...
static enum gimplify_status gimplify_save_expr(tree *expr_p, gimple_seq *pre_p,
                                               gimple_seq *post_p) {
...
static enum gimplify_status gimplify_addr_expr(tree *expr_p, gimple_seq *pre_p,
                                               gimple_seq *post_p) {
  tree expr = *expr_p;
...
static enum gimplify_status gimplify_asm_expr(tree *expr_p, gimple_seq *pre_p,
                                              gimple_seq *post_p) {
  tree expr;
  int noutputs;
  ...
  for (i = 0, link = ASM_OUTPUTS(expr); link; ++i, link = link_next) {
    ...
    ok = parse_output_constraint(&constraint, i, 0, 0, &allows_mem, &allows_reg,
                                 &is_inout);
    if (!ok) {
      ret = GS_ERROR;
      is_inout = false;
      ...
      if (allows_reg) {
        ...
        if (strchr(p, ',') != NULL) {
          ...
          for (beg = p + 1, dst = str;;) {
            ...
            beg[-1] = '=';
            tem = beg - 1;
            parse_output_constraint(&tem, i, 0, 0, &mem_p, &reg_p, &inout_p);
            if (dst != str)
              *dst++ = ',';
            if (reg_p) {
  ...
  for (link = ASM_INPUTS(expr); link; ++i, link = link_next) {
    ...
    if (!allows_reg && allows_mem) {
      tret = gimplify_expr(&TREE_VALUE(link), pre_p, post_p, is_gimple_lvalue,
                           fb_lvalue | fb_mayfail);
      mark_addressable(TREE_VALUE(link));
      if (tret == GS_ERROR) {
      ...
      tret = gimplify_expr(&TREE_VALUE(link), pre_p, post_p, is_gimple_asm_val,
                           fb_rvalue);
      if (tret == GS_ERROR)
        ret = tret;
    }
  ...
  stmt = gimple_build_asm_vec(TREE_STRING_POINTER(ASM_STRING(expr)), inputs,
                              outputs, clobbers);
  ...
  for (iter = gsi_start(body_sequence); !gsi_end_p(iter);) {
    ...
    if (gimple_code(wce) == GIMPLE_WITH_CLEANUP_EXPR) {
      if (gsi_one_before_end_p(iter)) {
        ...
        gsi_insert_seq_before_without_update(&iter, gimple_wce_cleanup(wce),
                                             GSI_SAME_STMT);
...
static void gimple_push_cleanup(tree var, tree cleanup, bool eh_only,
                                gimple_seq *pre_p) {
  gimple wce;
...
static void omp_firstprivatize_type_sizes(struct gimplify_omp_ctx *ctx,
                                          tree type) {
...
static void omp_add_variable(struct gimplify_omp_ctx *ctx, tree decl,
                             unsigned int flags) {
  splay_tree_node n;
  unsigned int nflags;
  tree t;
...
static bool omp_notice_variable(struct gimplify_omp_ctx *ctx, tree decl,
                                bool in_code) {
  splay_tree_node n;
  ...
  if (n == NULL) {
    ...
    switch (default_kind) {
      ...
      case OMP_CLAUSE_DEFAULT_UNSPECIFIED:
        ...
        if (flags & GOVD_FIRSTPRIVATE)
          break;
        if (octx == NULL && (TREE_CODE(decl) == PARM_DECL ||
                             (!is_global_var(decl) &&
    ...
    if ((flags & GOVD_PRIVATE) && lang_hooks.decls.omp_private_outer_ref(decl))
  ...
  if ((n->value & (GOVD_SEEN | GOVD_LOCAL)) == 0 &&
      (flags & (GOVD_SEEN | GOVD_LOCAL)) == GOVD_SEEN && DECL_SIZE(decl) &&
...
static bool omp_is_private(struct gimplify_omp_ctx *ctx, tree decl) {
  splay_tree_node n;
...
static bool omp_check_private(struct gimplify_omp_ctx *ctx, tree decl) {
  splay_tree_node n;
...
static void gimplify_scan_omp_clauses(tree *list_p, gimple_seq *pre_p,
                                      enum omp_region_type region_type) {
  ...
  while ((c = *list_p) != NULL) {
    ...
    switch (OMP_CLAUSE_CODE(c)) {
      ...
      do_notice:
        ...
        if (check_non_private && region_type == ORT_WORKSHARE &&
      ...
      case OMP_CLAUSE_IF:
        OMP_CLAUSE_OPERAND(c, 0) = gimple_boolify(OMP_CLAUSE_OPERAND(c, 0));
        /* Fall through.  */

      case OMP_CLAUSE_SCHEDULE:
      case OMP_CLAUSE_NUM_THREADS:
        if (gimplify_expr(&OMP_CLAUSE_OPERAND(c, 0), pre_p, NULL, is_gimple_val,
                          fb_rvalue) == GS_ERROR)
          remove = true;
        break;
...
static int gimplify_adjust_omp_clauses_1(splay_tree_node n, void *data) {
  ...
  if (flags & GOVD_DEBUG_PRIVATE) {
    ...
    private_debug = lang_hooks.decls.omp_private_debug_clause(
        decl, !!(flags & GOVD_SHARED));
  if (private_debug)
...
static void gimplify_adjust_omp_clauses(tree *list_p) {
  struct gimplify_omp_ctx *ctx = gimplify_omp_ctxp;
  tree c, decl;
        ...
        if (!remove) {
          ...
          if ((n->value & GOVD_DEBUG_PRIVATE) ||
            ...
            gcc_assert((n->value & GOVD_DEBUG_PRIVATE) == 0 ||
                       ((n->value & GOVD_DATA_SHARE_CLASS) == GOVD_PRIVATE));
...
static void gimplify_omp_parallel(tree *expr_p, gimple_seq *pre_p) {
  ...
  gimplify_scan_omp_clauses(
      &OMP_PARALLEL_CLAUSES(expr), pre_p,
  ...
  g = gimple_build_omp_parallel(body, OMP_PARALLEL_CLAUSES(expr), NULL_TREE,
                                NULL_TREE);
...
static void gimplify_omp_task(tree *expr_p, gimple_seq *pre_p) {
  ...
  g = gimple_build_omp_task(body, OMP_TASK_CLAUSES(expr), NULL_TREE, NULL_TREE,
    ...
    if (var != decl || TREE_VEC_LENGTH(OMP_FOR_INIT(for_stmt)) > 1) {
      ...
      for (c = OMP_FOR_CLAUSES(for_stmt); c; c = OMP_CLAUSE_CHAIN(c))
        if (OMP_CLAUSE_CODE(c) == OMP_CLAUSE_LASTPRIVATE &&
          ...
          gcc_assert(TREE_OPERAND(t, 0) == var);
          t = build2(TREE_CODE(t), TREE_TYPE(decl), decl, TREE_OPERAND(t, 1));
          gimplify_assign(decl, t, &OMP_CLAUSE_LASTPRIVATE_GIMPLE_SEQ(c));
        }
    }
  }
...
static bool goa_lhs_expr_p(tree expr, tree addr) {
  ...
  while ((CONVERT_EXPR_P(expr) || TREE_CODE(expr) == NON_LVALUE_EXPR) &&
  ...
  if (TREE_CODE(expr) == INDIRECT_REF) {
    expr = TREE_OPERAND(expr, 0);
    while (expr != addr &&
           (CONVERT_EXPR_P(expr) || TREE_CODE(expr) == NON_LVALUE_EXPR) &&
...
static int goa_stabilize_expr(tree *expr_p, gimple_seq *pre_p, tree lhs_addr,
                              tree lhs_var) {
  tree expr = *expr_p;
  int saw_lhs;
  ...
  switch (TREE_CODE_CLASS(TREE_CODE(expr))) {
    ...
    case tcc_comparison:
      saw_lhs |=
          goa_stabilize_expr(&TREE_OPERAND(expr, 1), pre_p, lhs_addr, lhs_var);
    case tcc_unary:
      saw_lhs |=
          goa_stabilize_expr(&TREE_OPERAND(expr, 0), pre_p, lhs_addr, lhs_var);
      break;
    case tcc_expression:
      switch (TREE_CODE(expr)) {
        case TRUTH_ANDIF_EXPR:
...
static enum gimplify_status gimplify_omp_atomic(tree *expr_p,
                                                gimple_seq *pre_p) {
  ...
  if (gimplify_expr(&rhs, pre_p, NULL, is_gimple_val, fb_rvalue) != GS_ALL_DONE)
    return GS_ERROR;
...
enum gimplify_status gimplify_expr(tree *expr_p, gimple_seq *pre_p,
                                   gimple_seq *post_p,
  ...
  do {
    ...
    if (save_expr == error_mark_node ||
        (TREE_TYPE(save_expr) && TREE_TYPE(save_expr) == error_mark_node)) {
      ret = GS_ERROR;
      break;
    }
    ...
    switch (TREE_CODE(*expr_p)) {
      ...
      CASE_CONVERT:
        ...
        if (VOID_TYPE_P(TREE_TYPE(*expr_p)) || fallback == fb_none) {
      ...
      case PREDICT_EXPR:
        gimplify_seq_add_stmt(
            pre_p, gimple_build_predict(PREDICT_EXPR_PREDICTOR(*expr_p),
      ...
      case TRY_CATCH_EXPR: {
        ...
        if (TREE_CODE(*expr_p) == TRY_CATCH_EXPR)
          gimple_try_set_catch_is_cleanup(try_, TRY_CATCH_IS_CLEANUP(*expr_p));
      ...
      case EH_FILTER_EXPR: {
        ...
        gimple_eh_filter_set_must_not_throw(ehf,
      ...
      case CHANGE_DYNAMIC_TYPE_EXPR: {
        ...
        ret = gimplify_expr(&CHANGE_DYNAMIC_TYPE_LOCATION(*expr_p), pre_p,
                            post_p, is_gimple_reg, fb_lvalue);
      ...
      case WITH_SIZE_EXPR: {
        gimplify_expr(&TREE_OPERAND(*expr_p, 0), pre_p,
                      post_p == &internal_post ? NULL : post_p, gimple_test_f,
                      fallback);
        gimplify_expr(&TREE_OPERAND(*expr_p, 1), pre_p, post_p, is_gimple_val,
                      fb_rvalue);
      } break;
      ...
      case OMP_CRITICAL: {
        ...
        switch (TREE_CODE(*expr_p)) {
          ...
          case OMP_ORDERED:
            ...
            break;
          case OMP_CRITICAL:
            g = gimple_build_omp_critical(body, OMP_CRITICAL_NAME(*expr_p));
            break;
          default:
            gcc_unreachable();
        }
      ...
      case POINTER_PLUS_EXPR:
        ...
        if (TREE_CODE(TREE_OPERAND(*expr_p, 1)) == INTEGER_CST &&
            (tmp = maybe_fold_offset_to_address(TREE_OPERAND(*expr_p, 0),
        ...
        if (TREE_CODE(TREE_OPERAND(*expr_p, 0)) == NOP_EXPR &&
            ...
            POINTER_TYPE_P(
                TREE_TYPE(TREE_OPERAND(TREE_OPERAND(*expr_p, 0), 0))) &&
    ...
    if (!TREE_SIDE_EFFECTS(*expr_p))
      *expr_p = NULL;
      ...
      switch (code) {
        ...
        case IMAGPART_EXPR:
        case VIEW_CONVERT_EXPR:
          gimplify_expr(&TREE_OPERAND(*expr_p, 0), pre_p, post_p, gimple_test_f,
                        fallback);
          break;
        ...
        case ARRAY_RANGE_REF:
          gimplify_expr(&TREE_OPERAND(*expr_p, 0), pre_p, post_p, gimple_test_f,
                        fallback);
          gimplify_expr(&TREE_OPERAND(*expr_p, 1), pre_p, post_p, gimple_test_f,
                        fallback);
          break;
  ...
  if (*expr_p) {
    ...
    gcc_assert(
        code != MODIFY_EXPR && code != ASM_EXPR && code != BIND_EXPR &&
        code != CATCH_EXPR &&
        (code != COND_EXPR || gimplify_ctxp->allow_rhs_cond_expr) &&
        code != EH_FILTER_EXPR && code != GOTO_EXPR && code != LABEL_EXPR &&
        code != LOOP_EXPR && code != RESX_EXPR && code != SWITCH_EXPR &&
        code != TRY_FINALLY_EXPR && code != OMP_CRITICAL && code != OMP_FOR &&
        code != OMP_MASTER && code != OMP_ORDERED && code != OMP_PARALLEL &&
        code != OMP_SECTIONS && code != OMP_SECTION && code != OMP_SINGLE);
  }
#endif
  ...
  /* We can't return an lvalue if we have an internal postqueue.  The
     ...
     rvalue.  */
  if ((fallback & fb_lvalue) && gimple_seq_empty_p(internal_post) &&
  ...
  } else if ((fallback & fb_rvalue) &&
             is_gimple_formal_tmp_or_call_rhs(*expr_p)) {
...
void gimplify_one_sizepos(tree *expr_p, gimple_seq *stmt_p) {
  ...
  if (expr == NULL_TREE || TREE_CONSTANT(expr) || TREE_CODE(expr) == VAR_DECL ||
  ...
  *expr_p = unshare_expr(expr);
  ...
  if (TREE_TYPE(expr) != type && TREE_CODE(type) == INTEGER_TYPE &&
...
gimple gimplify_body(tree *body_p, tree fndecl, bool do_parms) {
...
void gimplify_function_tree(tree fndecl) {
  ...
  for (parm = DECL_ARGUMENTS(fndecl); parm; parm = TREE_CHAIN(parm)) {
    ...
    if ((TREE_CODE(TREE_TYPE(parm)) == COMPLEX_TYPE ||
        ...
        !TREE_THIS_VOLATILE(parm) && !needs_to_live_in_memory(parm))
...
void gimple_regimplify_operands(gimple stmt, gimple_stmt_iterator *gsi_p) {
  size_t i, num_ops;
  ...
  switch (gimple_code(stmt)) {
    case GIMPLE_COND:
      gimplify_expr(gimple_cond_lhs_ptr(stmt), &pre, NULL, is_gimple_val,
                    fb_rvalue);
      gimplify_expr(gimple_cond_rhs_ptr(stmt), &pre, NULL, is_gimple_val,
                    fb_rvalue);
      break;
    case GIMPLE_SWITCH:
    ...
    case GIMPLE_ASM: {
      ...
      for (i = 0; i < gimple_asm_ninputs(stmt); i++) {
        ...
        if (TREE_ADDRESSABLE(TREE_TYPE(TREE_VALUE(op))) && allows_mem)
          allows_reg = 0;
        if (!allows_reg && allows_mem)
          gimplify_expr(&TREE_VALUE(op), &pre, NULL, is_gimple_lvalue,
                        fb_lvalue | fb_mayfail);
        else
          gimplify_expr(&TREE_VALUE(op), &pre, NULL, is_gimple_asm_val,
                        fb_rvalue);
      }
    } break;
    default:
      ...
      for (i = num_ops; i > 0; i--) {
        ...
        if (i == 1 && (is_gimple_call(stmt) || is_gimple_assign(stmt)))
          ...
          gimplify_expr(&op, &pre, NULL,
                        rhs_predicate_for(gimple_assign_lhs(stmt)), fb_rvalue);
      ...
      if (lhs && !is_gimple_formal_tmp_var(lhs)) {
        ...
        if (is_gimple_assign(stmt) && num_ops == 2 &&
            get_gimple_rhs_class(gimple_expr_code(stmt)) == GIMPLE_SINGLE_RHS)
        ...
        if (need_temp) {
          ...
          if (TREE_CODE(orig_lhs) == VAR_DECL &&
            ...
            SET_DECL_RESTRICT_BASE(temp, DECL_GET_RESTRICT_BASE(orig_lhs));
          }
...
tree force_gimple_operand(tree expr, gimple_seq *stmts, bool simple, tree var) {
  tree t;
  enum gimplify_status ret;
...
tree force_gimple_operand_gsi(gimple_stmt_iterator *gsi, tree expr,
