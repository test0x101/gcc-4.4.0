...
static void print_move_list(FILE *f, move_t list) {
  for (; list != NULL; list = list->next)
    fprintf(f, " a%dr%d->a%dr%d", ALLOCNO_NUM(list->from),
            ALLOCNO_REGNO(list->from), ALLOCNO_NUM(list->to),
            ALLOCNO_REGNO(list->to));
  fprintf(f, "\n");
}
...
/* Create and return new pseudo-register with the same attributes as
   ORIGINAL_REG.  */
static rtx create_new_reg(rtx original_reg) {
  rtx new_reg;
...
static void set_allocno_reg(ira_allocno_t allocno, rtx reg) {
  ...
  for (a = ira_regno_allocno_map[ALLOCNO_REGNO(allocno)]; a != NULL;
  ...
  for (bb_node = loop_node->children; bb_node != NULL; bb_node = bb_node->next)
    if (bb_node->bb != NULL) {
      ...
      if (e->src != ENTRY_BLOCK_PTR &&
        ...
        for (parent = src_loop_node->parent; parent != NULL;
        ...
        if (parent != NULL)
          ...
          continue;
        for (parent = loop_node->parent; parent != NULL;
             parent = parent->parent)
...
static bool store_can_be_removed_p(ira_allocno_t src_allocno,
  ...
  for (node = ALLOCNO_LOOP_TREE_NODE(src_allocno); node != NULL;
       node = node->parent) {
...
static void generate_edge_moves(edge e) {
  ...
  EXECUTE_IF_SET_IN_REG_SET(DF_LR_IN(e->dest), FIRST_PSEUDO_REGISTER, regno, bi)
  if (bitmap_bit_p(DF_LR_OUT(e->src), regno)) {
    src_allocno = src_map[regno];
    dest_allocno = dest_map[regno];
    if (REGNO(ALLOCNO_REG(src_allocno)) == REGNO(ALLOCNO_REG(dest_allocno)))
      continue;
...
static void change_loop(ira_loop_tree_node_t node) {
  ...
  if (node != ira_loop_tree_root) {
    ...
    EXECUTE_IF_SET_IN_REG_SET(ira_curr_loop_tree_node->border_allocnos, 0, i,
                              bi) {
      ...
      if (parent_allocno != NULL &&
          (ALLOCNO_HARD_REGNO(allocno) == ALLOCNO_HARD_REGNO(parent_allocno)) &&
          (ALLOCNO_HARD_REGNO(allocno) < 0 ||
           (parent->reg_pressure[cover_class] + 1 <=
            ira_available_class_regs[cover_class]) ||
           TEST_HARD_REG_BIT(
               ira_prohibited_mode_move_regs[ALLOCNO_MODE(allocno)],
      ...
      if (parent_allocno == NULL ||
        ...
        if (internal_flag_ira_verbose > 3 && ira_dump_file)
          fprintf(ira_dump_file,
                  "  %i vs parent %i:", ALLOCNO_HARD_REGNO(allocno),
  ...
  /* Rename locals: Local allocnos with same regno in different loops
     ...
     ALLOCNO_REG.  */
  bitmap_and_compl(local_allocno_bitmap, ira_curr_loop_tree_node->all_allocnos,
...
/* Return TRUE if move lists on all edges given in vector VEC are
   equal.  */
static bool eq_edge_move_lists_p(VEC(edge, gc) * vec) {
  move_t list;
  int i;
...
static move_t modify_move_list(move_t list) {
  ...
  for (move = first; move != NULL; move = move->next) {
    ...
    if ((hard_regno = ALLOCNO_HARD_REGNO(from)) >= 0) {
      ...
      for (i = 0; i < nregs; i++)
        if (hard_regno_last_set_check[hard_regno + i] == curr_tick &&
            ALLOCNO_HARD_REGNO(hard_regno_last_set[hard_regno + i]->to) >= 0) {
          ...
          ira_set_allocno_cover_class(new_allocno,
                                      ALLOCNO_COVER_CLASS(set_move->to));
          ALLOCNO_ASSIGNED_P(new_allocno) = true;
          ALLOCNO_HARD_REGNO(new_allocno) = -1;
          ALLOCNO_REG(new_allocno) = create_new_reg(ALLOCNO_REG(set_move->to));
          ...
          if (internal_flag_ira_verbose > 2 && ira_dump_file != NULL)
            fprintf(ira_dump_file, "    Creating temporary allocno a%dr%d\n",
                    ALLOCNO_NUM(new_allocno), REGNO(ALLOCNO_REG(new_allocno)));
        }
    }
    if ((hard_regno = ALLOCNO_HARD_REGNO(to)) < 0)
      continue;
...
static rtx emit_move_list(move_t list, int freq) {
  int cost;
  rtx result, insn;
...
static void emit_moves(void) {
  ...
  FOR_EACH_BB(bb) {
    if (at_bb_start[bb->index] != NULL) {
      ...
      insns = emit_move_list(at_bb_start[bb->index], REG_FREQ_FROM_BB(bb));
      tmp = BB_HEAD(bb);
      if (LABEL_P(tmp))
        tmp = NEXT_INSN(tmp);
...
static void add_range_and_copies_from_move_list(move_t list,
  ...
  for (move = list; move != NULL; move = move->next) {
    ...
    IOR_HARD_REG_SET(ALLOCNO_TOTAL_CONFLICT_HARD_REGS(from), hard_regs_live);
    ...
    ira_max_point++;
    ALLOCNO_LIVE_RANGES(to) = ira_create_allocno_live_range(
        to, ira_max_point, -1, ALLOCNO_LIVE_RANGES(to));
    ira_max_point++;
  }
  for (move = list; move != NULL; move = move->next) {
  ...
  EXECUTE_IF_SET_IN_BITMAP(live_through, FIRST_PSEUDO_REGISTER, regno, bi) {
    ...
    ALLOCNO_LIVE_RANGES(a) = ira_create_allocno_live_range(
        a, start, ira_max_point - 1, ALLOCNO_LIVE_RANGES(a));
    if (internal_flag_ira_verbose > 2 && ira_dump_file != NULL)
      fprintf(ira_dump_file,
              "    Adding range [%d..%d] to live through %s allocno a%dr%d\n",
              start, ira_max_point - 1, to != NULL ? "upper level" : "",
...
static void add_ranges_and_copies(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    add_range_and_copies_from_move_list(at_bb_start[bb->index], node,
                                        live_through, REG_FREQ_FROM_BB(bb));
    bitmap_copy(live_through, DF_LR_OUT(bb));
    add_range_and_copies_from_move_list(at_bb_end[bb->index], node,
                                        live_through, REG_FREQ_FROM_BB(bb));
    FOR_EACH_EDGE(e, ei, bb->succs) {
...
void ira_emit(bool loops_p) {
  ...
  allocno_last_set = (move_t *)ira_allocate(sizeof(move_t) * max_reg_num());
  allocno_last_set_check = (int *)ira_allocate(sizeof(int) * max_reg_num());
