...
tree handle_format_arg_attribute(tree *node, tree ARG_UNUSED(name), tree args,
...
static bool check_format_string(tree argument,
  ...
  if (!argument || TREE_CODE(TREE_VALUE(argument)) != POINTER_TYPE ||
      (TYPE_MAIN_VARIANT(TREE_TYPE(TREE_VALUE(argument))) != char_type_node)) {
    if (!(flags & (int)ATTR_FLAG_BUILT_IN))
...
static bool decode_format_attr(tree args, function_format_info *info,
                               int validated_p) {
...
static const format_length_info gcc_gfc_length_specs[] = {
    {"l", FMT_LEN_l, STD_C89, NO_FMT}, {NO_FMT, NO_FMT}};
...
static const format_flag_pair gcc_diag_flag_pairs[] = {{0, 0, 0, 0}};

#define gcc_tdiag_flag_pairs   gcc_diag_flag_pairs
#define gcc_cdiag_flag_pairs   gcc_diag_flag_pairs
#define gcc_cxxdiag_flag_pairs gcc_diag_flag_pairs

static const format_flag_pair gcc_gfc_flag_pairs[] = {{0, 0, 0, 0}};
...
static const format_flag_pair strfmon_flag_pairs[] = {{'+', '(', 0, 0},
                                                      {0, 0, 0, 0}};
...
static const format_char_info gcc_cxxdiag_char_table[] = {
    ...
    /* These accept either an 'int' or an 'enum tree_code' (which is handled as
       an 'int'.)  */
    {"CLOPQ",
     0,
     STD_C89,
...
static const format_char_info gcc_gfc_char_table[] = {
    ...
    {"di",
     0,
     STD_C89,
     {T89_I, BADLEN, BADLEN, T89_L, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN},
     "",
     "",
     NULL},
    {"u",
     0,
     STD_C89,
     {T89_UI, BADLEN, BADLEN, T89_UL, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN},
     "",
     "",
     NULL},
    {"c",
...
static const format_kind_info format_types_orig[] = {
    ...
    {"asm_fprintf", asm_fprintf_length_specs, asm_fprintf_char_table, " +#0-",
     NULL, asm_fprintf_flag_specs, asm_fprintf_flag_pairs,
     FMT_FLAG_ARG_CONVERT | FMT_FLAG_EMPTY_PREC_OK, 'w', 0, 'p', 0, 'L', 0,
     NULL, NULL},
    {"gcc_diag", gcc_diag_length_specs, gcc_diag_char_table, "q+", NULL,
     gcc_diag_flag_specs, gcc_diag_flag_pairs, FMT_FLAG_ARG_CONVERT, 0, 0, 'p',
     0, 'L', 0, NULL, &integer_type_node},
    {"gcc_tdiag", gcc_tdiag_length_specs, gcc_tdiag_char_table, "q+", NULL,
     gcc_tdiag_flag_specs, gcc_tdiag_flag_pairs, FMT_FLAG_ARG_CONVERT, 0, 0,
     'p', 0, 'L', 0, NULL, &integer_type_node},
    {"gcc_cdiag", gcc_cdiag_length_specs, gcc_cdiag_char_table, "q+", NULL,
     gcc_cdiag_flag_specs, gcc_cdiag_flag_pairs, FMT_FLAG_ARG_CONVERT, 0, 0,
     'p', 0, 'L', 0, NULL, &integer_type_node},
    {"gcc_cxxdiag", gcc_cxxdiag_length_specs, gcc_cxxdiag_char_table, "q+#",
     NULL, gcc_cxxdiag_flag_specs, gcc_cxxdiag_flag_pairs, FMT_FLAG_ARG_CONVERT,
     0, 0, 'p', 0, 'L', 0, NULL, &integer_type_node},
    {"gcc_gfc", gcc_gfc_length_specs, gcc_gfc_char_table, "", NULL, NULL,
     gcc_gfc_flag_pairs, FMT_FLAG_ARG_CONVERT, 0, 0, 0, 0, 0, 0, NULL, NULL},
    {"gnu_scanf", scanf_length_specs, scan_char_table, "*'I", NULL,
     ...
     'w', 0, 0, '*', 'L', 'm', NULL, NULL},
    {"gnu_strftime", NULL, time_char_table, "_-0^#", "EO", strftime_flag_specs,
     strftime_flag_pairs, FMT_FLAG_FANCY_PERCENT_OK, 'w', 0, 0, 0, 0, 0, NULL,
     NULL},
    {"gnu_strfmon", strfmon_length_specs, monetary_char_table, "=^+(!-", NULL,
     strfmon_flag_specs, strfmon_flag_pairs, FMT_FLAG_ARG_CONVERT, 'w', '#',
     'p', 0, 'L', 0, NULL, NULL}};
...
static void check_format_info_main(format_check_results *,
                                   function_format_info *, const char *, int,
                                   tree, unsigned HOST_WIDE_INT, alloc_pool);
...
static void format_type_warning(const char *, const char *, int, tree, int,
                                const char *, tree, int);

/* Decode a format type from a string, returning the type, or
   format_type_error if not valid, in which case the caller should print an
   error message.  */
static int decode_format_type(const char *s) {
  int i;
  int slen;
  ...
  for (i = 0; i < n_format_types; i++) {
    ...
    if (slen == alen + 4 && s[0] == '_' && s[1] == '_' && s[slen - 1] == '_' &&
        s[slen - 2] == '_' && !strncmp(s + 2, format_types[i].name, alen))
      return i;
  }
...
void check_function_format(tree attrs, int nargs, tree *argarray) {
  tree a;
  ...
  for (a = attrs; a; a = TREE_CHAIN(a)) {
    if (is_attribute_p("format", TREE_PURPOSE(a))) {
      ...
      if (warn_missing_format_attribute && info.first_arg_num == 0 &&
          (format_types[info.format_type].flags & (int)FMT_FLAG_ARG_CONVERT)) {
        tree c;
        ...
        if (c == NULL_TREE) {
          ...
          tree args;
          for (args = DECL_ARGUMENTS(current_function_decl); args != 0;
...
static int maybe_read_dollar_number(const char **format, int dollar_needed,
                                    tree params, tree *param_ptr,
...
static bool avoid_dollar_number(const char *format) {
  if (!ISDIGIT(*format))
    return false;
  ...
  for (i = 0; i < dollar_max_arg_used; i++) {
    if (!dollar_arguments_used[i]) {
      if (pointer_gap_ok &&
        ...
        warning(OPT_Wformat,
                "format argument %d unused before used argument %d in $-style "
                "format",
...
static const format_flag_spec *get_flag_spec(const format_flag_spec *spec,
                                             int flag, const char *predicates) {
  int i;
  for (i = 0; spec[i].flag_char != 0; i++) {
    if (spec[i].flag_char != flag)
      continue;
    if (predicates != NULL) {
      if (spec[i].predicate != 0 && strchr(predicates, spec[i].predicate) != 0)
        return &spec[i];
...
static void check_format_info(function_format_info *info, tree params) {
...
static void check_format_arg(void *ctx, tree format_tree,
                             unsigned HOST_WIDE_INT arg_num) {
  ...
  if (array_size != 0) {
    ...
    if (host_integerp(array_size, 0)) {
      ...
      if (array_size_value > 0 && array_size_value == (int)array_size_value &&
...
static void check_format_info_main(format_check_results *res,
                                   function_format_info *info,
                                   const char *format_chars, int format_length,
                                   tree params, unsigned HOST_WIDE_INT arg_num,
                                   alloc_pool fwt_pool) {
  ...
  while (1) {
    ...
    if (fli) {
      ...
      if (pedantic) {
        ...
        if (ADJ_STD(length_chars_std) > C_STD_VER)
          warning(OPT_Wformat, "%s does not support the %qs %s length modifier",
                  C_STD_NAME(length_chars_std), length_chars, fki->name);
      }
    }
    ...
    if (format_char == 0 || (!(fki->flags & (int)FMT_FLAG_FANCY_PERCENT_OK) &&
                             format_char == '%')) {
    ...
    /* Validate the individual flags used, removing any that are invalid.  */
    {
      ...
      for (i = 0; flag_chars[i] != 0; i++) {
        const format_flag_spec *s =
            get_flag_spec(flag_specs, flag_chars[i], NULL);
        flag_chars[i - d] = flag_chars[i];
        if (flag_chars[i] == fki->length_code_char)
          continue;
        if (strchr(fci->flag_chars, flag_chars[i]) == 0) {
          warning(OPT_Wformat, "%s used with %<%%%c%> %s format", _(s->name),
        ...
        if (pedantic) {
          ...
          if (ADJ_STD(s->std) > C_STD_VER)
            warning(OPT_Wformat, "%s does not support %s", C_STD_NAME(s->std),
                    _(s->long_name));
          ...
          if (t != NULL && ADJ_STD(t->std) > ADJ_STD(s->std)) {
            ...
            if (ADJ_STD(t->std) > C_STD_VER)
              warning(OPT_Wformat,
                      ...
                      C_STD_NAME(t->std), _(long_name), format_char, fki->name);
          }
        }
      }
      flag_chars[i - d] = 0;
    ...
    if (fki->suppression_char && strchr(flag_chars, fki->suppression_char) != 0)
      suppressed = 1;
    ...
    for (i = 0; bad_flag_pairs[i].flag_char1 != 0; i++) {
      ...
      if (bad_flag_pairs[i].ignored) {
        if (bad_flag_pairs[i].predicate != 0)
          warning(OPT_Wformat, "%s ignored with %s and %<%%%c%> %s format",
                  _(s->name), _(t->name), format_char, fki->name);
        else
          warning(OPT_Wformat, "%s ignored with %s in %s format", _(s->name),
                  _(t->name), fki->name);
      } else {
        if (bad_flag_pairs[i].predicate != 0)
          warning(OPT_Wformat,
    ...
    if (fki->flags & (int)FMT_FLAG_ARG_CONVERT) {
      wanted_type = (fci->types[length_chars_val].type
                         ? *fci->types[length_chars_val].type
                         : 0);
      ...
      if (wanted_type == 0) {
        ...
        if (ADJ_STD(wanted_type_std) > C_STD_VER)
          warning(OPT_Wformat, "%s does not support the %<%%%s%c%> %s format",
                  C_STD_NAME(wanted_type_std), length_chars, format_char,
                  fki->name);
      }
    }
    ...
    if ((fci->pointer_count == 0 && wanted_type == void_type_node) ||
      ...
      while (fci) {
        ...
        fci = fci->chain;
        if (fci) {
          wanted_type_ptr = (format_wanted_type *)pool_alloc(fwt_pool);
          arg_num++;
          wanted_type = *fci->types[length_chars_val].type;
          wanted_type_name = fci->types[length_chars_val].name;
        }
...
static void check_format_types(format_wanted_type *types,
                               const char *format_start, int format_length) {
  for (; types != 0; types = types->next) {
    ...
    */
    ...
    if (wanted_type == char_type_node && (!pedantic || i < 2) && char_type_flag)
      continue;
    /* Now we have a type mismatch.  */
    format_type_warning(types->name, format_start, format_length, wanted_type,
                        types->pointer_count, types->wanted_type_name,
                        orig_cur_type, arg_num);
  }
}
...
/* Give a warning about a format argument of different type from that
   ...
   argument.  */
static void format_type_warning(const char *descr, const char *format_start,
                                int format_length, tree wanted_type,
                                int pointer_count, const char *wanted_type_name,
                                tree arg_type, int arg_num) {
  char *p;
  /* If ARG_TYPE is a typedef with a misleading name (for example,
     size_t but not the standard size_t expected by printf %zu), avoid
     printing the typedef name.  */
  if (wanted_type_name && TYPE_NAME(arg_type) &&
  ...
  if (wanted_type_name) {
    if (descr)
      ...
      warning(OPT_Wformat,
              ...
              "but argument %d has type %qT",
              format_length, format_start, wanted_type_name, p, arg_num,
              arg_type);
  } else {
    if (descr)
      warning(OPT_Wformat,
...
static unsigned int find_length_info_modifier_index(
    const format_length_info *fli, int c) {
  unsigned i;
...
static void init_dynamic_asm_fprintf_info(void) {
  ...
  if (!hwi) {
    ...
    if (hwi != long_integer_type_node && hwi != long_long_integer_type_node) {
      error(
          "%<__gcc_host_wide_int__%> is not defined as %<long%>"
          " or %<long long%>");
      return;
    }

    /* Create a new (writable) copy of asm_fprintf_length_specs.  */
    new_asm_fprintf_length_specs = (format_length_info *)xmemdup(
        asm_fprintf_length_specs, sizeof(asm_fprintf_length_specs),
...
static void init_dynamic_gfc_info(void) {
  ...
  if (!locus) {
    ...
    if (!gfc_fci)
      dynamic_format_types[gcc_gfc_format_type].conversion_specs = gfc_fci =
          (format_char_info *)xmemdup(gcc_gfc_char_table,
...
static void init_dynamic_diag_info(void) {
  ...
  if (!loc || !t || !hwi) {
      ...
      if (hwi) {
        if (TREE_CODE(hwi) != TYPE_DECL) {
          ...
          if (hwi != long_integer_type_node &&
            ...
            error(
                "%<__gcc_host_wide_int__%> is not defined"
      ...
      dynamic_format_types[gcc_diag_format_type].length_char_specs =
          dynamic_format_types[gcc_tdiag_format_type].length_char_specs =
              dynamic_format_types[gcc_cdiag_format_type].length_char_specs =
                  dynamic_format_types[gcc_cxxdiag_format_type]
                      .length_char_specs = diag_ls =
                      (format_length_info *)xmemdup(
                          gcc_diag_length_specs, sizeof(gcc_diag_length_specs),
    ...
    if (!diag_fci)
      dynamic_format_types[gcc_diag_format_type].conversion_specs = diag_fci =
          (format_char_info *)xmemdup(gcc_diag_char_table,
    ...
    if (!tdiag_fci)
      dynamic_format_types[gcc_tdiag_format_type].conversion_specs = tdiag_fci =
          (format_char_info *)xmemdup(gcc_tdiag_char_table,
    ...
    if (!cdiag_fci)
      dynamic_format_types[gcc_cdiag_format_type].conversion_specs = cdiag_fci =
          (format_char_info *)xmemdup(gcc_cdiag_char_table,
    ...
    if (!cxxdiag_fci)
      dynamic_format_types[gcc_cxxdiag_format_type].conversion_specs =
          cxxdiag_fci = (format_char_info *)xmemdup(
              gcc_cxxdiag_char_table, sizeof(gcc_cxxdiag_char_table),
...
/* Handle a "format" attribute; arguments as in
   struct attribute_spec.handler.  */
tree handle_format_attribute(tree *node, tree ARG_UNUSED(name), tree args,
                             int flags, bool *no_add_attrs) {
  tree type = *node;
  function_format_info info;
  tree argument;
  ...
  if (TARGET_FORMAT_TYPES != NULL && !dynamic_format_types) {
    dynamic_format_types =
        XNEWVEC(format_kind_info, n_format_types + TARGET_N_FORMAT_TYPES);
  ...
  argument = TYPE_ARG_TYPES(type);
  if (argument) {
    if (!check_format_string(argument, info.format_num, flags, no_add_attrs))
      return NULL_TREE;
  ...
  if (info.format_type == asm_fprintf_format_type ||
    ...
    if (!dynamic_format_types)
      format_types = dynamic_format_types = (format_kind_info *)xmemdup(
          format_types_orig, sizeof(format_types_orig),
