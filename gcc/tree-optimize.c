...
static bool gate_all_early_local_passes(void) {
  /* Don't bother doing anything if the program has errors.  */
  return (!errorcount && !sorrycount);
}

struct simple_ipa_opt_pass pass_early_local_passes = {{
    SIMPLE_IPA_PASS, "early_local_cleanups", /* name */
...
static unsigned int execute_early_local_optimizations(void) {
...
static bool gate_all_early_optimizations(void) {
  return (optimize >= 1
          /* Don't bother doing anything if the program has errors.  */
...
struct gimple_opt_pass pass_all_early_optimizations = {{
    GIMPLE_PASS, "early_optimizations", /* name */
...
static unsigned int execute_cleanup_cfg_pre_ipa(void) {
  cleanup_tree_cfg();
  return 0;
}

struct gimple_opt_pass pass_cleanup_cfg = {{
    GIMPLE_PASS, "cleanup_cfg",  /* name */
    NULL,                        /* gate */
...
struct gimple_opt_pass pass_cleanup_cfg_post_optimizing = {{
    GIMPLE_PASS, "final_cleanup",        /* name */
    NULL,                                /* gate */
...
static unsigned int execute_free_datastructures(void) {
...
static unsigned int execute_free_cfg_annotations(void) {
  ...
  if (cfun->eh)
    FOR_EACH_BB(bb) {
      for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
        ...
        tree decl = is_gimple_call(stmt) ? gimple_call_fndecl(stmt) : NULL;

        if (decl && gimple_call_flags(stmt) &
                        (ECF_CONST | ECF_PURE | ECF_LOOPING_CONST_OR_PURE)) {
...
static unsigned int execute_init_datastructures(void) {
...
void tree_lowering_passes(tree fn) {
...
void tree_rest_of_compilation(tree fndecl) {
  ...
  if (warn_larger_than && !DECL_EXTERNAL(fndecl) && TREE_TYPE(fndecl)) {
    ...
    if (ret_type && TYPE_SIZE_UNIT(ret_type) &&
        ...
        0 < compare_tree_int(TYPE_SIZE_UNIT(ret_type), larger_than_size)) {
      ...
      if (compare_tree_int(TYPE_SIZE_UNIT(ret_type), size_as_int) == 0)
        ...
        warning(OPT_Wlarger_than_eq,
                "size of return value of %q+D is larger than %wd bytes", fndecl,
                larger_than_size);
    }
  }

  gimple_set_body(fndecl, NULL);
  if (DECL_STRUCT_FUNCTION(fndecl) == 0 && !cgraph_node(fndecl)->origin) {
