...
#ifndef GENERATOR_FILE
static void print_decl_name(FILE *outfile, const_tree node) {
  if (DECL_NAME(node))
...
static void print_rtx(const_rtx in_rtx) {
  int i = 0;
  int j;
  ...
  if (dump_for_graph &&
      (is_insn || NOTE_P(in_rtx) || LABEL_P(in_rtx) || BARRIER_P(in_rtx))) {
    i = 3;
    indent = 0;
  } else {
      ...
      case 'i':
        if (i == 4 && INSN_P(in_rtx)) {
          ...
          if (REG_P(in_rtx) && value < FIRST_PSEUDO_REGISTER)
            fprintf(outfile, " %d %s", REGNO(in_rtx), reg_names[REGNO(in_rtx)]);
          else if (REG_P(in_rtx) && value <= LAST_VIRTUAL_REGISTER) {
...
#endif
              if (flag_dump_unnumbered && (is_insn || NOTE_P(in_rtx)))
            fputc('#', outfile);
          else
          ...
          if (REG_P(in_rtx) && REG_ATTRS(in_rtx)) {
            ...
            if (REG_OFFSET(in_rtx))
              fprintf(outfile, "+" HOST_WIDE_INT_PRINT_DEC, REG_OFFSET(in_rtx));
            fputs(" ]", outfile);
          }
#endif
  ...
  switch (GET_CODE(in_rtx)) {
    ...
    case CONST_DOUBLE:
      if (FLOAT_MODE_P(GET_MODE(in_rtx))) {
        ...
        real_to_hexadecimal(s, CONST_DOUBLE_REAL_VALUE(in_rtx), sizeof(s), 0,
                            1);
        fprintf(outfile, " [%s]", s);
      }
      break;
...
int print_rtl_single(FILE *outf, const_rtx x) {
  outfile = outf;
  sawclose = 0;
  fputs(print_rtx_head, outfile);
  print_rtx(x);
