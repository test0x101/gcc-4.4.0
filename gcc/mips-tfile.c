...
/* Here is a brief description of the MIPS ECOFF symbol table.  The
   ...
   There are 20 external symbols, starting at 1152
        ...
        Symbol# 0: "_iob"
            Type          = array [3 {160}] of struct _iobuf { ifd = 1, index =
   1 } String index  = 0           Ifd    = 1 Storage class = Nil         Index
   = 17 Symbol type   = Global      Value  = 60
...
#define IS_ASM_IDENT(ch) (ISIDNUM(ch) || (ch) == '.' || (ch) == '$')
...
#define CHECK(num, max, str)                    \
  (((unsigned long)num > (unsigned long)max)    \
       ? out_of_bounds(num, max, str, __LINE__) \
       : 0)

#define ORIG_LINENUM(indx) \
  (CHECK((indx), orig_sym_hdr.cbLine, "line#"), (indx) + orig_linenum)
#define ORIG_DENSE(indx) \
  (CHECK((indx), orig_sym_hdr.idnMax, "dense"), (indx) + orig_dense)
#define ORIG_PROCS(indx) \
  (CHECK((indx), orig_sym_hdr.ipdMax, "procs"), (indx) + orig_procs)
#define ORIG_FILES(indx) \
  (CHECK((indx), orig_sym_hdr.ifdMax, "funcs"), (indx) + orig_files)
#define ORIG_LSYMS(indx) \
  (CHECK((indx), orig_sym_hdr.isymMax, "lsyms"), (indx) + orig_local_syms)
#define ORIG_LSTRS(indx) \
  (CHECK((indx), orig_sym_hdr.issMax, "lstrs"), (indx) + orig_local_strs)
#define ORIG_ESYMS(indx) \
  (CHECK((indx), orig_sym_hdr.iextMax, "esyms"), (indx) + orig_ext_syms)
#define ORIG_ESTRS(indx) \
  (CHECK((indx), orig_sym_hdr.issExtMax, "estrs"), (indx) + orig_ext_strs)
#define ORIG_OPT(indx) \
  (CHECK((indx), orig_sym_hdr.ioptMax, "opt"), (indx) + orig_opt_syms)
#define ORIG_AUX(indx) \
  (CHECK((indx), orig_sym_hdr.iauxMax, "aux"), (indx) + orig_aux_syms)
#define ORIG_RFDS(indx) \
  (CHECK((indx), orig_sym_hdr.crfd, "rfds"), (indx) + orig_rfds)
...
static const struct option options[] = {
    {"version", 0, 0, 'V'}, {"verbose", 0, 0, 'v'}, {0, 0, 0, 0}};
...
STATIC void add_varray_page(varray_t *vp) {
  ...
  else { /* 2nd or greater allocation */
...
STATIC shash_t *hash_string(const char *text, Ptrdiff_t hash_len,
                            shash_t **hash_tbl, symint_t *ret_hash_index) {
  unsigned long hi;
  Ptrdiff_t i;
  shash_t *ptr;
  int first_ch = *text;
  ...
  for (ptr = hash_tbl[hi]; ptr != (shash_t *)0; ptr = ptr->next)
    if ((symint_t)hash_len == ptr->len && first_ch == ptr->string[0] &&
...
STATIC symint_t add_string(varray_t *vp, shash_t **hash_tbl, const char *start,
...
STATIC symint_t add_local_symbol(const char *str_start, const char *str_end_p1,
                                 st_t type, sc_t storage, symint_t value,
                                 symint_t indx) {
  symint_t ret;
  SYMR *psym;
  scope_t *pscope;
  ...
  psym->index = indx;
  psym->iss =
      (str_start == (const char *)0)
          ? 0
          : add_string(&cur_file_ptr->strings, &cur_file_ptr->shash_head[0],
    ...
    case st_End:
      ...
      if (pscope == (scope_t *)0)
        ...
        if (begin_type != st_File && storage != sc_Info) {
          ...
          for (ptag = ptag_head->first_tag; ptag != (tag_t *)0;
               ptag = ptag_next) {
        ...
        /* Functions push two or more aux words as follows:
          ...
          type = add_aux_sym_tir(&last_func_type_info, hash_no,
  ...
  if (debug && type != st_File &&
      (debug > 2 || type == st_Block || type == st_End || type == st_Proc ||
       type == st_StaticProc)) {
    ...
    fprintf(stderr, "\tlsym\tv= %10ld, depth= %2d, sc= %-12s", value, depth,
            sc_str);

    if (str_start && str_end_p1 - str_start > 0)
      fprintf(stderr, " st= %-11s name= %.*s\n", st_str,
              (int)(str_end_p1 - str_start), str_start);
    else {
...
STATIC symint_t add_ext_symbol(EXTR *esym, int ifd) {
  ...
  if (debug > 1) {
    ...
    fprintf(stderr, "\tesym\tv= %10ld, ifd= %2d, sc= %-12s", value, ifd,
            sc_str);
  ...
  *psym = *esym;
  ...
  psym->asym.iss = (str_start == (const char *)0)
                       ? 0
                       : add_string(&ext_strings, &ext_str_hash[0], str_start,
                                    str_end_p1, &hash_ptr);
...
STATIC symint_t add_aux_sym_tir(type_info_t *t, hash_state_t state,
                                thash_t **hash_tbl) {
  AUXU *aux_ptr;
  efdr_t *file_ptr = cur_file_ptr;
  varray_t *vp = &file_ptr->aux_syms;
  ...
  if (state != hash_no &&
      (t->type_qualifiers[0] == tq_Array || t->type_qualifiers[1] == tq_Array ||
       t->type_qualifiers[2] == tq_Array || t->type_qualifiers[3] == tq_Array ||
       t->type_qualifiers[4] == tq_Array || t->type_qualifiers[5] == tq_Array ||
       t->basic_type == bt_Struct || t->basic_type == bt_Union ||
  ...
  if (state != hash_no) {
    ...
    for (hash_ptr = hash_tbl[hi]; hash_ptr != (thash_t *)0;
  ...
  *aux_ptr = aux;
  ...
  if (t->basic_type == bt_Struct || t->basic_type == bt_Union ||
    ...
    if (t->unknown_tag) {
      ...
      (void)add_aux_sym_rndx(ST_RFDESCAPE, sym_index);
      forward_ref->index_ptr = &vp->last->datum->aux[vp->objects_last_page - 1];
...
STATIC tag_t *get_tag(const char *tag_start,  /* 1st byte of tag name */
...

{
  shash_t *hash_ptr;
  tag_t *tag_ptr;
  hash_ptr = hash_string(tag_start, tag_end_p1 - tag_start, &tag_hash[0],
                         (symint_t *)0);
  ...
  (void)add_string(&tag_strings, &tag_hash[0], tag_start, tag_end_p1,
                   &hash_ptr);
  ...
  tag_ptr->basic_type = basic_type;
  tag_ptr->indx = indx;
  tag_ptr->ifd = (indx == indexNil ? (symint_t)-1 : cur_file_ptr->file_index);
...
STATIC void add_unknown_tag(tag_t *ptag) {
  ...
  if (debug > 1) {
    ...
    fprintf(stderr, "unknown %s %.*s found\n", agg_type, (int)hash_ptr->len,
            name_start);
  }

  sym_index = add_local_symbol(name_start, name_end_p1, st_Block, sc_Info,
                               (symint_t)0, (symint_t)0);

  (void)add_local_symbol(name_start, name_end_p1, st_End, sc_Info, (symint_t)0,
                         (symint_t)0);
    ...
    const char *func_end_p1) /* 1st byte after func name */
{
  ...
  shash_t *shash_ptr = hash_string(func_start, func_end_p1 - func_start,
  ...
  if (shash_ptr != (shash_t *)0) {
    ...
    if (old_proc_ptr != (PDR *)0 && sym_ptr != (SYMR *)0 &&
  ...
  if (cur_oproc_ptr == (PDR *)0)
    error("did not find a PDR block for %.*s", (int)(func_end_p1 - func_start),
          func_start);
  ...
  (void)add_local_symbol(func_start, func_end_p1, proc_type, sc_Text, value,
                         (symint_t)0);
}
...
STATIC void add_file(const char *file_start,  /* first byte in string */
                     const char *file_end_p1) /* first byte after string */
{
  ...
  for (file_ptr = first_file; file_ptr != (efdr_t *)0;
       file_ptr = file_ptr->next_file) {
    if (first_ch == file_ptr->name[0] && file_ptr->name[len] == '\0' &&
  ...
  if (file_ptr == (efdr_t *)0) {
    ...
    *file_ptr = init_file;
    ...
    add_string(&file_ptr->strings, &file_ptr->shash_head[0], &zero_bytes[0],
               &zero_bytes[0], (shash_t **)0);
    ...
    *last_file_ptr = file_ptr;
    ...
    file_ptr->void_type = add_aux_sym_tir(&void_type_info, hash_yes,
                                          &cur_file_ptr->thash_head[0]);

    file_ptr->int_type =
        add_aux_sym_tir(&int_type_info, hash_yes, &cur_file_ptr->thash_head[0]);
  }
}

/* Add a stream of random bytes to a varray.  */

STATIC void add_bytes(varray_t *vp,    /* virtual array to add too */
...
STATIC char *read_line(void) {
  ...
  if (cur_line_start == (char *)0) { /* allocate initial page */
...
STATIC void parse_begin(const char *start) {
  ...
  hash_ptr =
      hash_string(start, end_p1 - start, &orig_str_hash[0], (symint_t *)0);

  if (hash_ptr == (shash_t *)0) {
    error("label %.*s not found for #.begin", (int)(end_p1 - start), start);
    return;
  }

  if (cur_oproc_begin == (SYMR *)0) {
    error("procedure table %.*s not found for #.begin", (int)(end_p1 - start),
          start);
    return;
  }

  (void)add_local_symbol(
      (const char *)0, (const char *)0, st_Block, sc_Text,
      (symint_t)hash_ptr->sym_ptr->value - cur_oproc_begin->value, (symint_t)0);
}

/* Parse #.bend directives which have a label as the first argument
   which gives the location of the end of the block.  */

STATIC void parse_bend(const char *start) {
  const char *end_p1; /* end of label */
  int ch;
  shash_t *hash_ptr; /* hash pointer to lookup label */
  ...
  if (cur_oproc_begin == (SYMR *)0) {
    error("procedure table %.*s not found for #.bend", (int)(end_p1 - start),
          start);
    return;
  }

  (void)add_local_symbol(
      (const char *)0, (const char *)0, st_End, sc_Text,
      (symint_t)hash_ptr->sym_ptr->value - cur_oproc_begin->value, (symint_t)0);
}
...
STATIC void parse_def(const char *name_start) {
  ...
  for (;;) {
    ...
    for (dir_end_p1 = dir_start + 1; (ch = *dir_end_p1) != ' ' && ch != '\t';
         dir_end_p1++) {
      if (ch == '\0' || ISSPACE(ch)) {
        error_line = __LINE__;
        saber_stop();
    ...
    if (!arg_was_number) {
      ...
      for (arg_end_p1 = arg_start + 1; (ch = *arg_end_p1) != ';' && ch != '\0';
           arg_end_p1++)
        ;
    ...
    switch (dir_start[1]) {
      ...
      case 't':
        if (len == sizeof(".type") - 1 &&
          ...
          for (i = N_TQ - 1; i >= 0; i--) {
            int dt = (arg_number >> ((i * N_TQ_SHIFT) + N_BT_SHIFT) & N_TMASK);
      ...
      case 'v':
        if (len == sizeof(".val") - 1 &&
          ...
          /* If the value is not an integer value, it must be the
            ...
            ext_hash_ptr = hash_string(arg_start, arg_end_p1 - arg_start,
            ...
            orig_hash_ptr = hash_string(arg_start, arg_end_p1 - arg_start,
                                        &orig_str_hash[0], (symint_t *)0);
            ...
            if ((orig_hash_ptr == (shash_t *)0 ||
                ...
                eptr == (EXTR *)0) {
              fprintf(stderr,
                      "warning, %.*s not found in original or external symbol "
                      "tables, value defaults to 0\n",
  ...
  if (symbol_type == st_Block || symbol_type == st_End)
    ...
    if (t.basic_type == bt_Struct || t.basic_type == bt_Union ||
        t.basic_type == bt_Enum) {
      if (tag_start == (char *)0) {
        error("no tag specified for %.*s", (int)(name_end_p1 - name_start),
              name_start);
        return;
      }

      t.tag_ptr =
          get_tag(tag_start, tag_end_p1, (symint_t)indexNil, t.basic_type);
    }
  ...
  if (eptr == (EXTR *)0 || eptr->asym.st == (int)st_Nil ||
      cur_proc_ptr != (PDR *)0) {
    symint_t isym = add_local_symbol(name_start, name_end_p1, symbol_type,
                                     storage_class, value, indx);

    /* Deal with struct, union, and enum tags.  */
    if (symbol_type == st_Block) {
      /* Create or update the tag information.  */
      tag_t *tag_ptr = get_tag(name_start, name_end_p1, isym, t.basic_type);
...
STATIC void parse_end(const char *start) {
  ...
  if (orig_fdr != (FDR *)0 && cur_oproc_end != (SYMR *)0)
    ...
    error("cannot find .end block for %.*s", (int)(end_func_p1 - start_func),
          start_func);

  (void)add_local_symbol(start_func, end_func_p1, st_End, sc_Text, value,
                         (symint_t)0);
...
STATIC void parse_ent(const char *start) {
  const char *start_func, *end_func_p1;
  int ch;
...
STATIC void parse_file(const char *start) {
  char *p;
  char *start_name, *end_name_p1;

  (void)strtol(start, &p, 0);
  if (start == p || (start_name = strchr(p, '"')) == (char *)0 ||
...
static void mark_stabs(const char *start ATTRIBUTE_UNUSED) {
  if (!stabs_seen) {
    /* Add a dummy @stabs symbol.  */
    stabs_seen = 1;
    (void)add_local_symbol(stabs_symbol, stabs_symbol + sizeof(stabs_symbol),
    ...
    const char *rest)         /* rest of the directive.  */
{
  ...
  if (code == (int)N_SLINE) {
    ...
    if (dummy_symr.index != code) {
      error(
          "line number (%lu) for .stabs/.stabn directive cannot fit in index "
          "field (20 bits)",
          code);
    ...
    shash_ptr = hash_string(p, strlen(p) - 1, &orig_str_hash[0], (symint_t *)0);
    ...
    if (ISDIGIT(ch) || ch == '-') {
      ...
      if (shash_ptr == (shash_t *)0 ||
          (sym_ptr = shash_ptr->sym_ptr) == (SYMR *)0) {
        shash_ptr =
            hash_string(start, end_p1 - start, &ext_str_hash[0], (symint_t *)0);

        if (shash_ptr == (shash_t *)0 || shash_ptr->esym_ptr == (EXTR *)0) {
          error("invalid .stabs/.stabn directive, value not found");
          return;
        } else
...
STATIC void parse_stabs(const char *start) {
...
STATIC void parse_stabn(const char *start) {
  ...
  for (file_ptr = first_file; file_ptr != (efdr_t *)0;
    ...
    for (sym = sym_start; sym < sym_end_p1; sym++) {
      if ((st_t)sym->st == st_Static) {
        ...
        if (str[0] == '$' && str[1] == 'L')
          continue;
        hash_ptr = hash_string(str, (Ptrdiff_t)len, &file_ptr->shash_head[0],
                               (symint_t *)0);
        if (hash_ptr == (shash_t *)0) {
          (void)add_local_symbol(str, str + len, (st_t)sym->st, (sc_t)sym->sc,
                                 (symint_t)sym->value, (symint_t)indexNil);
        }
      }
    }
    (void)add_local_symbol((const char *)0, (const char *)0, st_End, sc_Text,
                           (symint_t)0, (symint_t)0);
...
STATIC void write_varray(varray_t *vp,    /* virtual array */
                         ...
                         const char *str) /* string to print out when tracing */
{
  ...
  for (ptr = vp->first; ptr != (vlinks_t *)0; ptr = ptr->next) {
    ...
    if (sys_write <= 0)
      ...
      fatal("wrote %d bytes to %s, system returned %d", num_write, object_name,
            sys_write);
...
STATIC void write_object(void) {
  ...
  if (debug)
    fprintf(stderr, "\n\twrite\tvp = %p, offset = %7u, size = %7lu, %s\n",
            (void *)&symbolic_header, 0, (unsigned long)sizeof(symbolic_header),
            "symbolic header");

  sys_write =
      fwrite(&symbolic_header, 1, sizeof(symbolic_header), object_stream);
  ...
  if (symbolic_header.cbLine > 0) /* line numbers */
  {
    ...
    sys_write = fwrite(orig_linenum, 1, symbolic_header.cbLine, object_stream);
  ...
  if (symbolic_header.ioptMax > 0) /* optimization symbols */
  {
    ...
    if (sys_write <= 0)
      ...
      fatal("wrote %ld bytes to %s, system returned %ld", num_write,
            object_name, sys_write);
  ...
  if (symbolic_header.ifdMax > 0) /* file tables */
  {
    ...
    for (file_ptr = first_file; file_ptr != (efdr_t *)0;
      ...
      if (debug)
        fprintf(stderr, "\twrite\tvp = %p, offset = %7lu, size = %7lu, %s\n",
                (void *)&file_ptr->fdr, file_offset, (unsigned long)sizeof(FDR),
                "File header");
      ...
      if (sys_write < 0)
        ...
        fatal("wrote %d bytes to %s, system returned %d", (int)sizeof(FDR),
              object_name, sys_write);
  ...
  if (symbolic_header.crfd > 0) /* relative file descriptors */
  {
    ...
    if (debug)
      fprintf(stderr, "\twrite\tvp = %p, offset = %7lu, size = %7lu, %s\n",
              (void *)&orig_rfds, (long)symbolic_header.cbRfdOffset, num_write,
              "Relative file descriptors");
    ...
    if (sys_write <= 0)
      ...
      fatal("wrote %lu bytes to %s, system returned %ld", num_write,
            object_name, sys_write);
...
STATIC page_t *read_seek(Size_t size,     /* # bytes to read */
...
STATIC void copy_object(void) {
  ...
  sys_read = fread(&orig_file_header, 1, sizeof(struct filehdr), obj_in_stream);
  ...
  if (orig_sym_hdr.iauxMax > 0) /* aux symbols */
    orig_aux_syms = (AUXU *)read_seek(orig_sym_hdr.iauxMax * sizeof(AUXU),
                                      orig_sym_hdr.cbAuxOffset, "Aux. symbols");

  if (orig_sym_hdr.issMax > 0) /* local strings */
    orig_local_strs = (char *)read_seek(
        orig_sym_hdr.issMax, orig_sym_hdr.cbSsOffset, "Local strings");

  if (orig_sym_hdr.issExtMax > 0) /* external strings */
    orig_ext_strs = (char *)read_seek(
        orig_sym_hdr.issExtMax, orig_sym_hdr.cbSsExtOffset, "External strings");
  ...
  if (orig_sym_hdr.issExtMax > 0) /* external symbols */
    orig_ext_syms =
        (EXTR *)read_seek(orig_sym_hdr.iextMax * sizeof(EXTR),
                          orig_sym_hdr.cbExtOffset, "External symbols");

  if (orig_sym_hdr.idnMax > 0) /* dense numbers */
  {
    orig_dense = (DNR *)read_seek(orig_sym_hdr.idnMax * sizeof(DNR),
                                  orig_sym_hdr.cbDnOffset, "Dense numbers");
  ...
  if (orig_sym_hdr.ioptMax > 0) /* opt symbols */
    orig_opt_syms =
        (OPTR *)read_seek(orig_sym_hdr.ioptMax * sizeof(OPTR),
                          orig_sym_hdr.cbOptOffset, "Optimizer symbols");
  ...
  if (orig_sym_hdr.ifdMax > 1 && orig_files->csym == 2 &&
      orig_files->caux == 0) {
  ...
  for (es = 0; es < orig_sym_hdr.iextMax; es++) {
    ...
    (void)add_ext_symbol(
        eptr, ((long)ifd < orig_sym_hdr.ifdMax) ? remap_file_number[ifd] : ifd);
  }
  ...
  for (fd = delete_ifd; fd < orig_sym_hdr.ifdMax; fd++) {
    ...
    for (sym = sym_start; sym < sym_end_p1; sym++) {
      switch ((st_t)sym->st) {
        ...
        case st_StaticProc: {
          ...
          Size_t len = strlen(str);
          shash_t *shash_ptr =
              hash_string(str, (Ptrdiff_t)len, &orig_str_hash[0], &hash_index);

          if (shash_ptr != (shash_t *)0)
            error("internal error, %s is already in original symbol table",
                  str);
        ...
        case st_End:
          if ((sc_t)sym->sc == sc_Text) {
            ...
            if (*str != '\0') {
              Size_t len = strlen(str);
              shash_t *shash_ptr = hash_string(
                  str, (Ptrdiff_t)len, &orig_str_hash[0], (symint_t *)0);
    ...
    for (proc = proc_start; proc < proc_end_p1; proc++) {
      ...
      Size_t len = strlen(str);
      shash_t *shash_ptr =
          hash_string(str, (Ptrdiff_t)len, &orig_str_hash[0], (symint_t *)0);

      if (shash_ptr == (shash_t *)0)
        error("internal error, function %s is not in original symbol table",
              str);
  ...
  for (remaining = orig_file_header.f_symptr; remaining > 0;
       remaining -= num_write) {
    num_write =
    ...
    if (sys_read <= 0)
      ...
      fatal("wanted to read %d bytes from %s, system returned %d", num_write,
            obj_in_name, sys_read);
...
warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n\n"),
  ...
  if (object_name == (char *)0 || had_errors) {
    ...
    fprintf(stderr, _("\tmips-tfile [-d <num>] [-v] [-i <o-in-file>] -o "
                      "<o-out-file> <s-file> (or)\n"));
    fprintf(stderr, _("\tmips-tfile [-d <num>] [-v] [-I <o-in-file>] -o "
                      "<o-out-file> <s-file> (or)\n"));
    fprintf(
        stderr,
  ...
  if (rename_output && rename(object_name, obj_in_name) != 0) {
    ...
    while ((len = read(in_fd, buffer, 4 * PAGE_SIZE)) > 0) {
      ...
      if (len != len2)
        fatal("wrote %d bytes to %s, expected to write %d", len2, obj_in_name,
              len);
    }
  ...
  if (debug) {
    ...
    for (i = (int)alloc_type_none; i < (int)alloc_type_last; i++)
      if (alloc_counts[i].total_alloc) {
        fprintf(stderr, "\t%s\t%5d allocation(s), %5d free(s), %2d page(s)\n",
                alloc_counts[i].alloc_name, alloc_counts[i].total_alloc,
...
void pfatal_with_name(const char *msg) {
...
static int out_of_bounds(symint_t indx,   /* index that is out of bounds */
                         ...
                         int prog_line)   /* line number within mips-tfile.c */
{
  ...
  fprintf(stderr,
          "%s, %s:%ld index %lu is out of bounds for %s, max is %lu, "
          "mips-tfile.c line# %d\n",
...
STATIC page_t *allocate_cluster(Size_t npages) {
...
STATIC page_t *allocate_multiple_pages(Size_t npages) {
#ifndef MALLOC_CHECK
...
STATIC void free_multiple_pages(page_t *page_ptr, Size_t npages) {
#ifndef MALLOC_CHECK
  if (pages_left == 0) {
...
STATIC void free_forward(forward_t *ptr) {
