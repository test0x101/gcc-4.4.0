...
static enum inc_state set_inc_state(HOST_WIDE_INT val, int size) {
  if (val == 0)
    return INC_ZERO;
  if (val < 0)
...
static void dump_inc_insn(FILE *file) {
  const char *f =
      ((inc_insn.form == FORM_PRE_ADD) || (inc_insn.form == FORM_PRE_INC))
          ? "pre"
          : "post";
    ...
    case FORM_POST_ADD:
      if (inc_insn.reg1_is_const)
        fprintf(file, "found %s add(%d) r[%d]=r[%d]+%d\n", f,
                INSN_UID(inc_insn.insn), REGNO(inc_insn.reg_res),
...
static void move_dead_notes(rtx to_insn, rtx from_insn, rtx pattern) {
  rtx note;
  rtx next_note;
  rtx prev_note = NULL;
  ...
  for (note = REG_NOTES(from_insn); note; note = next_note) {
    ...
    if ((REG_NOTE_KIND(note) == REG_DEAD) && pattern == XEXP(note, 0)) {
...
static rtx insert_move_insn_before(rtx next_insn, rtx dest_reg, rtx src_reg) {
  rtx insns;
...
static bool attempt_change(rtx new_addr, rtx inc_reg) {
  ...
  old_cost =
      rtx_cost(mem, 0, speed) + rtx_cost(PATTERN(inc_insn.insn), 0, speed);
  ...
  switch (inc_insn.form) {
    case FORM_PRE_ADD:
      ...
      mov_insn = insert_move_insn_before(inc_insn.insn, inc_insn.reg_res,
                                         inc_insn.reg0);
    ...
    case FORM_POST_ADD:
      mov_insn = insert_move_insn_before(mem_insn.insn, inc_insn.reg_res,
                                         inc_insn.reg0);
      move_dead_notes(mov_insn, inc_insn.insn, inc_insn.reg0);
...
static bool try_merge(void) {
  ...
  rtx mem = *mem_insn.mem_loc;
  rtx inc_reg =
      inc_insn.form == FORM_POST_ADD ? inc_insn.reg_res : mem_insn.reg0;
  ...
  mem_insn.reg1_state = (mem_insn.reg1_is_const)
                            ? set_inc_state(mem_insn.reg1_val, size)
                            : INC_REG;
  inc_insn.reg1_state = (inc_insn.reg1_is_const)
                            ? set_inc_state(inc_insn.reg1_val, size)
                            : INC_REG;

  /* Now get the form that we are generating.  */
  gen_form =
      decision_table[inc_insn.reg1_state][mem_insn.reg1_state][inc_insn.form];
  ...
  switch (gen_form) {
    ...
    case DISP_PRE: /* ++con   */
      ...
      return attempt_change(
          gen_rtx_PRE_MODIFY(Pmode, inc_reg,
                             gen_rtx_PLUS(Pmode, inc_reg, inc_insn.reg1)),
          inc_reg);
      break;
    ...
    case DISP_POST: /* con++   */
      ...
      return attempt_change(
          gen_rtx_POST_MODIFY(Pmode, inc_reg,
                              gen_rtx_PLUS(Pmode, inc_reg, inc_insn.reg1)),
          inc_reg);
      break;
...
static rtx get_next_ref(int regno, basic_block bb, rtx *next_array) {
...
static bool parse_add_or_inc(rtx insn, bool before_mem) {
  ...
  if ((GET_CODE(SET_SRC(pat)) != PLUS) && (GET_CODE(SET_SRC(pat)) != MINUS))
    return false;
  ...
  if (GET_CODE(XEXP(SET_SRC(pat), 1)) == CONST_INT) {
    ...
    return true;
             ...
             (REG_P(XEXP(SET_SRC(pat), 1))) && GET_CODE(SET_SRC(pat)) == PLUS) {
    /* Process a = b + c where c is a reg.  */
    inc_insn.reg1 = XEXP(SET_SRC(pat), 1);
    inc_insn.reg1_is_const = false;

    if (inc_insn.form == FORM_PRE_INC || inc_insn.form == FORM_POST_INC)
      return true;
...
static int find_address(rtx *address_of_x) {
  ...
  if (code == MEM && GET_CODE(XEXP(x, 0)) == PLUS &&
    ...
    } else if (!inc_insn.reg1_is_const && rtx_equal_p(inc_insn.reg1, b))
...
static bool find_inc(bool first_try) {
  ...
  insn = get_next_ref(REGNO(mem_insn.reg0),
                      BASIC_BLOCK(BLOCK_NUM(mem_insn.insn)), reg_next_inc_use);
  if (!insn)
    return false;
  ...
  for (def_rec = DF_INSN_DEFS(mem_insn.insn); *def_rec; def_rec++) {
    ...
    if ((regno == REGNO(inc_insn.reg0)) || (regno == REGNO(inc_insn.reg_res))) {
      if (dump_file)
  ...
  if (inc_insn.form == FORM_POST_ADD) {
    ...
    rtx other_insn =
        get_next_ref(REGNO(inc_insn.reg_res),
                     BASIC_BLOCK(BLOCK_NUM(mem_insn.insn)), reg_next_def);
    ...
    other_insn =
        get_next_ref(REGNO(inc_insn.reg_res),
                     BASIC_BLOCK(BLOCK_NUM(mem_insn.insn)), reg_next_use);
    if (other_insn && (other_insn != inc_insn.insn) &&
  ...
  if (mem_insn.reg1_is_const) {
    if (mem_insn.reg1_val == 0) {
      if (!inc_insn.reg1_is_const) {
        ...
        if (inc_insn.form == FORM_POST_ADD) {
          ...
          other_insn = get_next_ref(REGNO(inc_insn.reg1), bb, reg_next_use);
        ...
        other_insn = get_next_ref(REGNO(inc_insn.reg1), bb, reg_next_def);
        if (other_insn && luid > DF_INSN_LUID(other_insn))
          return false;
      }
    }
    ...
    if (inc_insn.form == FORM_POST_ADD) {
      ...
      if (rtx_equal_p(mem_insn.reg0, inc_insn.reg0)) {
        ...
        other_insn = get_next_ref(REGNO(inc_insn.reg0), bb, reg_next_def);
        if (other_insn && luid > DF_INSN_LUID(other_insn))
          return false;
      }
      ...
      other_insn = get_next_ref(REGNO(inc_insn.reg_res), bb, reg_next_use);
      if (other_insn && luid > DF_INSN_LUID(other_insn))
        return false;
  ...
  if (inc_insn.form == FORM_POST_INC) {
    other_insn = get_next_ref(REGNO(inc_insn.reg0), bb, reg_next_use);
...
static unsigned int rest_of_handle_auto_inc_dec(void) {
#ifdef AUTO_INC_DEC
  basic_block bb;
...
static bool gate_auto_inc_dec(void) {
#ifdef AUTO_INC_DEC
...
struct rtl_opt_pass pass_inc_dec = {{
    ...
    0,                               /* todo_flags_start */
    TODO_dump_func | TODO_df_finish, /* todo_flags_finish */
}};
