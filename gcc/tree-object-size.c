...
static tree compute_object_offset(const_tree expr, const_tree var) {
...
static unsigned HOST_WIDE_INT addr_object_size(const_tree ptr,
                                               int object_size_type) {
  tree pt_var;
  ...
  if (pt_var && (SSA_VAR_P(pt_var) || TREE_CODE(pt_var) == STRING_CST) &&
      TYPE_SIZE_UNIT(TREE_TYPE(pt_var)) &&
      host_integerp(TYPE_SIZE_UNIT(TREE_TYPE(pt_var)), 1) &&
      (unsigned HOST_WIDE_INT)tree_low_cst(TYPE_SIZE_UNIT(TREE_TYPE(pt_var)),
                                           1) < offset_limit) {
    tree bytes;
    ...
    if (pt_var != TREE_OPERAND(ptr, 0)) {
      ...
      if (object_size_type & 1) {
        ...
        while (var != pt_var && TREE_CODE(var) != BIT_FIELD_REF &&
               TREE_CODE(var) != COMPONENT_REF && TREE_CODE(var) != ARRAY_REF &&
      ...
      if (bytes != error_mark_node) {
        if (TREE_CODE(bytes) == INTEGER_CST &&
          ...
          bytes = size_binop(MINUS_EXPR, TYPE_SIZE_UNIT(TREE_TYPE(var)), bytes);
      }
    } else
...
static unsigned HOST_WIDE_INT alloc_object_size(const_gimple call,
                                                int object_size_type) {
  ...
  if (arg1 < 0 || arg1 >= (int)gimple_call_num_args(call) ||
      ...
      (arg2 >= 0 && (arg2 >= (int)gimple_call_num_args(call) ||
...
static tree pass_through_call(const_gimple call) {
  ...
  if (TREE_CODE(ptr) == SSA_NAME && POINTER_TYPE_P(TREE_TYPE(ptr)) &&
    ...
    if (!bitmap_bit_p(computed[object_size_type], SSA_NAME_VERSION(ptr))) {
      ...
      if (dump_file) {
        ...
        if (object_sizes[object_size_type][i] != unknown[object_size_type]) {
          print_generic_expr(dump_file, ssa_name(i), dump_flags);
          fprintf(dump_file,
                  ": %s %sobject size " HOST_WIDE_INT_PRINT_UNSIGNED "\n",
                  (object_size_type & 2) ? "minimum" : "maximum",
                  (object_size_type & 1) ? "sub" : "",
...
static void expr_object_size(struct object_size_info *osi, tree ptr,
                             tree value) {
  ...
  gcc_assert(TREE_CODE(value) != SSA_NAME || !POINTER_TYPE_P(TREE_TYPE(value)));
...
static void call_object_size(struct object_size_info *osi, tree ptr,
                             gimple call) {
  int object_size_type = osi->object_size_type;
  unsigned int varno = SSA_NAME_VERSION(ptr);
  unsigned HOST_WIDE_INT bytes;
...
static bool merge_object_sizes(struct object_size_info *osi, tree dest,
                               tree orig, unsigned HOST_WIDE_INT offset) {
  int object_size_type = osi->object_size_type;
  ...
  if (orig_bytes != unknown[object_size_type])
    orig_bytes =
        (offset > orig_bytes) ? (unsigned HOST_WIDE_INT)0 : orig_bytes - offset;
...
static bool plus_stmt_object_size(struct object_size_info *osi, tree var,
                                  gimple stmt) {
  int object_size_type = osi->object_size_type;
...
static bool cond_expr_object_size(struct object_size_info *osi, tree var,
                                  tree value) {
  tree then_, else_;
  int object_size_type = osi->object_size_type;
  unsigned int varno = SSA_NAME_VERSION(var);
...
static void collect_object_sizes_for(struct object_size_info *osi, tree var) {
  int object_size_type = osi->object_size_type;
  unsigned int varno = SSA_NAME_VERSION(var);
  gimple stmt;
  bool reexamine;
  ...
  if (osi->pass == 0) {
    if (!bitmap_bit_p(osi->visited, varno)) {
      ...
      object_sizes[object_size_type][varno] = (object_size_type & 2) ? -1 : 0;
    } else {
  ...
  switch (gimple_code(stmt)) {
    case GIMPLE_ASSIGN: {
      if (gimple_assign_rhs_code(stmt) == POINTER_PLUS_EXPR)
        ...
        if (TREE_CODE(rhs) == SSA_NAME && POINTER_TYPE_P(TREE_TYPE(rhs)))
    ...
    case GIMPLE_CALL: {
      ...
      if (arg) {
        if (TREE_CODE(arg) == SSA_NAME && POINTER_TYPE_P(TREE_TYPE(arg)))
...
static void check_for_plus_in_loops_1(struct object_size_info *osi, tree var,
                                      unsigned int depth) {
  ...
  *osi->tos++ = varno;
  ...
  switch (gimple_code(stmt)) {
    case GIMPLE_ASSIGN: {
      if ((gimple_assign_single_p(stmt) || gimple_assign_unary_nop_p(stmt)) &&
...
static void check_for_plus_in_loops(struct object_size_info *osi, tree var) {
  gimple stmt = SSA_NAME_DEF_STMT(var);
...
void init_object_sizes(void) {
  ...
  for (object_size_type = 0; object_size_type <= 3; object_size_type++) {
    object_sizes[object_size_type] =
        XNEWVEC(unsigned HOST_WIDE_INT, num_ssa_names);
      ...
      if (!result) {
        if (gimple_call_num_args(call) == 2 &&
          ...
          if (host_integerp(ost, 1)) {
            unsigned HOST_WIDE_INT object_size_type = tree_low_cst(ost, 1);
