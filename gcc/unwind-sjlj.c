...
static void fc_key_init(void) {
  use_fc_key = __gthread_key_create(&fc_key, 0) == 0;
}

static void fc_key_init_once(void) {
...
static inline struct SjLj_Function_Context *_Unwind_SjLj_GetContext(void) {
#if __GTHREADS
  if (use_fc_key < 0)
    fc_key_init_once();
...
static inline void _Unwind_SjLj_SetContext(struct SjLj_Function_Context *fc) {
#if __GTHREADS
  if (use_fc_key < 0)
    fc_key_init_once();
...
void *_Unwind_FindEnclosingFunction(void *pc __attribute__((unused))) {
  return NULL;
}
...
static inline void uw_update_context(struct _Unwind_Context *context,
...
static inline void uw_init_context(struct _Unwind_Context *context) {
