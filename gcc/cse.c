...
#define CHEAP_REGNO(N)                                                    \
  (REGNO_PTR_FRAME_P(N) || (HARD_REGISTER_NUM_P(N) && FIXED_REGNO_P(N) && \
                            REGNO_REG_CLASS(N) != NO_REGS))
...
static void init_cse_reg_info(unsigned int nregs) {
  ...
  if (nregs > cse_reg_info_table_size) {
    ...
    if (cse_reg_info_table_size < 2048) {
      ...
      new_size = (cse_reg_info_table_size ? cse_reg_info_table_size : 64);
...
static inline struct cse_reg_info *get_cse_reg_info(unsigned int regno) {
...
static void make_regs_eqv(unsigned int new_reg, unsigned int old_reg) {
  ...
  if (!(firstr < FIRST_PSEUDO_REGISTER && FIXED_REGNO_P(firstr))
      ...
      && (new_reg >= FIRST_PSEUDO_REGISTER ||
          REGNO_REG_CLASS(new_reg) != NO_REGS) &&
...
static int insert_regs(rtx x, struct table_elt *classp, int modified) {
  if (REG_P(x)) {
    ...
    if (modified || !qty_valid) {
      ...
      if (!modified && REG_IN_TABLE(regno) >= 0 &&
...
static struct table_elt *lookup(rtx x, unsigned int hash,
                                enum machine_mode mode) {
  struct table_elt *p;

  for (p = table[hash]; p; p = p->next_same_hash)
    if (mode == p->mode &&
        ((x == p->exp && REG_P(x)) || exp_equiv_p(x, p->exp, !REG_P(x), false)))
      return p;
...
static struct table_elt *lookup_for_remove(rtx x, unsigned int hash,
                                           enum machine_mode mode) {
  struct table_elt *p;
  ...
  if (REG_P(x)) {
    ...
    for (p = table[hash]; p; p = p->next_same_hash)
      if (mode == p->mode && (x == p->exp || exp_equiv_p(x, p->exp, 0, false)))
        return p;
  }
...
static rtx lookup_as_function(rtx x, enum rtx_code code) {
  struct table_elt *p = lookup(x, SAFE_HASH(x, VOIDmode), GET_MODE(x));
...
static struct table_elt *insert(rtx x, struct table_elt *classp,
                                unsigned int hash, enum machine_mode mode) {
  struct table_elt *elt;
  ...
  else if (REG_P(x) && classp && !qty_table[REG_QTY(REGNO(x))].const_rtx &&
           !elt->is_const) {
    struct table_elt *p;
  ...
  else if (REG_P(x) && qty_table[REG_QTY(REGNO(x))].const_rtx &&
...
/* Likewise for a subreg with subreg_reg REGNO, subreg_byte OFFSET,
   and mode MODE.  */
static void remove_invalid_subreg_refs(unsigned int regno, unsigned int offset,
                                       enum machine_mode mode) {
  unsigned int i;
  struct table_elt *p, *next;
  ...
  for (i = 0; i < HASH_SIZE; i++)
    for (p = table[i]; p; p = next) {
      ...
      if (!REG_P(exp) &&
          (GET_CODE(exp) != SUBREG || !REG_P(SUBREG_REG(exp)) ||
           REGNO(SUBREG_REG(exp)) != regno ||
           (((SUBREG_BYTE(exp) + (GET_MODE_SIZE(GET_MODE(exp)) - 1)) >=
             offset) &&
            SUBREG_BYTE(exp) <= end)) &&
          refers_to_regno_p(regno, regno + 1, p->exp, (rtx *)0))
        remove_from_table(p, i);
...
static void rehash_using_reg(rtx x) {
  ...
  if (!REG_P(x) || REG_IN_TABLE(REGNO(x)) < 0 ||
  ...
  for (i = 0; i < HASH_SIZE; i++)
    for (p = table[i]; p; p = next) {
      next = p->next_same_hash;
      if (reg_mentioned_p(x, p->exp) && exp_equiv_p(p->exp, p->exp, 1, false) &&
...
static rtx use_related_value(rtx x, struct table_elt *elt) {
  ...
  if (elt != 0 && elt->related_value != 0)
    ...
    if (subexp != 0)
      relt =
          lookup(subexp, SAFE_HASH(subexp, GET_MODE(subexp)), GET_MODE(subexp));
  }
...
unsigned hash_rtx_cb(const_rtx x, enum machine_mode mode, int *do_not_record_p,
...
repeat:
  ...
  if (cb != NULL && ((*cb)(x, mode, &newx, &newmode))) {
    hash += hash_rtx_cb(newx, newmode, do_not_record_p, hash_arg_in_memory_p,
                        have_reg_qty, cb);
    return hash;
  }
  ...
  switch (code) {
    case REG: {
      ...
      if (do_not_record_p && !reload_completed) {
        ...
        if (regno >= FIRST_PSEUDO_REGISTER)
          record = true;
        else if (x == frame_pointer_rtx || x == hard_frame_pointer_rtx ||
    ...
    case CONST_VECTOR: {
      ...
      for (i = 0; i < units; ++i) {
        ...
        hash += hash_rtx_cb(elt, GET_MODE(elt), do_not_record_p,
...
unsigned hash_rtx(const_rtx x, enum machine_mode mode, int *do_not_record_p,
                  int *hash_arg_in_memory_p, bool have_reg_qty) {
  return hash_rtx_cb(x, mode, do_not_record_p, hash_arg_in_memory_p,
                     have_reg_qty, NULL);
}
...
static inline unsigned canon_hash(rtx x, enum machine_mode mode) {
...
static inline unsigned safe_hash(rtx x, enum machine_mode mode) {
...
int exp_equiv_p(const_rtx x, const_rtx y, int validate, bool for_gcse) {
  int i, j;
  enum rtx_code code;
  const char *fmt;
  ...
  switch (code) {
    ...
    case NE:
    case EQ:
      return ((exp_equiv_p(XEXP(x, 0), XEXP(y, 0), validate, for_gcse) &&
               exp_equiv_p(XEXP(x, 1), XEXP(y, 1), validate, for_gcse)) ||
              (exp_equiv_p(XEXP(x, 0), XEXP(y, 1), validate, for_gcse) &&
               exp_equiv_p(XEXP(x, 1), XEXP(y, 0), validate, for_gcse)));
    ...
    case ASM_OPERANDS:
      ...
      if (ASM_OPERANDS_INPUT_LENGTH(x)) {
        for (i = ASM_OPERANDS_INPUT_LENGTH(x) - 1; i >= 0; i--)
          if (!exp_equiv_p(ASM_OPERANDS_INPUT(x, i), ASM_OPERANDS_INPUT(y, i),
                           validate, for_gcse) ||
  ...
  for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--) {
    switch (fmt[i]) {
      case 'e':
        if (!exp_equiv_p(XEXP(x, i), XEXP(y, i), validate, for_gcse))
          return 0;
        break;
...
static bool cse_rtx_varies_p(const_rtx x, bool from_alias) {
  ...
  if (GET_CODE(x) == PLUS && GET_CODE(XEXP(x, 1)) == CONST_INT &&
      REG_P(XEXP(x, 0)) && REGNO_QTY_VALID_P(REGNO(XEXP(x, 0)))) {
    int x0_q = REG_QTY(REGNO(XEXP(x, 0)));
    struct qty_table_elem *x0_ent = &qty_table[x0_q];

    if ((GET_MODE(XEXP(x, 0)) == x0_ent->mode) && x0_ent->const_rtx != NULL_RTX)
      return 0;
  }
  ...
  if (GET_CODE(x) == PLUS && REG_P(XEXP(x, 0)) && REG_P(XEXP(x, 1)) &&
    ...
    if ((GET_MODE(XEXP(x, 0)) == x0_ent->mode) &&
        ...
        (GET_MODE(XEXP(x, 1)) == x1_ent->mode) && x1_ent->const_rtx != NULL_RTX)
      return 0;
  }
...
static void validate_canon_reg(rtx *xloc, rtx insn) {
  if (*xloc) {
...
static enum rtx_code find_comparison_args(enum rtx_code code, rtx *parg1,
                                          rtx *parg2, enum machine_mode *pmode1,
...
static rtx fold_rtx(rtx x, rtx insn) {
    ...
    if (fmt[i] == 'e') {
      ...
      if (const_arg != 0 && const_arg != folded_arg &&
          ...
          (GET_RTX_CLASS(code) != RTX_UNARY ||
           ...
           (code != ZERO_EXTEND && code != SIGN_EXTEND && code != TRUNCATE &&
            code != FLOAT_TRUNCATE && code != FLOAT_EXTEND && code != FLOAT &&
            code != FIX && code != UNSIGNED_FLOAT && code != UNSIGNED_FIX)))
  ...
  switch (GET_RTX_CLASS(code)) {
    case RTX_UNARY: {
      ...
      new_rtx = simplify_unary_operation(
    ...
    case RTX_COMM_COMPARE:
...
#ifdef FLOAT_STORE_FLAG_VALUE
          true_rtx = (CONST_DOUBLE_FROM_REAL_VALUE(FLOAT_STORE_FLAG_VALUE(mode),
                                                   mode));
#else
          true_rtx = NULL_RTX;
#endif
          ...
          if (const_arg1 != NULL) {
            ...
            if (p != NULL) {
              ...
              for (p = p->first_same_value; p != NULL; p = p->next_same_value) {
                ...
                simp_result = simplify_relational_operation(
              ...
              if (cheapest_simplification != x)
                return fold_rtx(copy_rtx(cheapest_simplification), insn);
            }
          }

          /* See if the two operands are the same.  */

          if ((REG_P(folded_arg0) && REG_P(folded_arg1) &&
               (REG_QTY(REGNO(folded_arg0)) == REG_QTY(REGNO(folded_arg1)))) ||
              ((p0 = lookup(folded_arg0, SAFE_HASH(folded_arg0, mode_arg0),
                            mode_arg0)) &&
               (p1 = lookup(folded_arg1, SAFE_HASH(folded_arg1, mode_arg0),
                            mode_arg0)) &&
            ...
            if (REGNO_QTY_VALID_P(REGNO(folded_arg0))) {
              ...
              if ((comparison_dominates_p(ent->comparison_code, code) ||
                  ...
                  (rtx_equal_p(ent->comparison_const, folded_arg1) ||
                   (const_arg1 &&
                    rtx_equal_p(ent->comparison_const, const_arg1)) ||
                   (REG_P(folded_arg1) &&
...

      {
        ...
        new_rtx =
            simplify_relational_operation(code, mode, mode_arg0, op0, op1);
      }
      break;
    ...
    case RTX_COMM_ARITH:
      switch (code) {
        case PLUS:
          ...
          if (const_arg1 && GET_CODE(const_arg1) == LABEL_REF) {
            rtx y = GET_CODE(folded_arg0) == MINUS
                        ? folded_arg0
          ...
          if (const_arg0 && GET_CODE(const_arg0) == LABEL_REF) {
            rtx y = GET_CODE(folded_arg1) == MINUS
                        ? folded_arg1
        ...
        case MINUS:
          ...
          if (const_arg1 != 0 && GET_CODE(const_arg1) == CONST_INT) {
            ...
            if (y && GET_CODE(XEXP(y, 1)) == CONST_INT)
              return fold_rtx(plus_constant(copy_rtx(y), -INTVAL(const_arg1)),
                              NULL_RTX);
          }
        ...
        case ASHIFTRT:
          ...
          if (REG_P(folded_arg0) && const_arg1 &&
            ...
            if (is_shift && (INTVAL(const_arg1) >= GET_MODE_BITSIZE(mode) ||
                             INTVAL(const_arg1) < 0)) {
              if (SHIFT_COUNT_TRUNCATED)
                canon_const_arg1 =
                    GEN_INT(INTVAL(const_arg1) & (GET_MODE_BITSIZE(mode) - 1));
              else
                break;
            }
            ...
            if (is_shift && (INTVAL(inner_const) >= GET_MODE_BITSIZE(mode) ||
                             INTVAL(inner_const) < 0)) {
              if (SHIFT_COUNT_TRUNCATED)
                inner_const =
                    GEN_INT(INTVAL(inner_const) & (GET_MODE_BITSIZE(mode) - 1));
              else
                break;
            }
            ...
            new_const = simplify_binary_operation(
      ...
      new_rtx = simplify_binary_operation(
          code, mode, const_arg0 ? const_arg0 : folded_arg0,
    ...
    case RTX_BITFIELD_OPS:
      new_rtx = simplify_ternary_operation(
          code, mode, mode_arg0, const_arg0 ? const_arg0 : folded_arg0,
          const_arg1 ? const_arg1 : folded_arg1,
...
static rtx equiv_constant(rtx x) {
  ...
  if (GET_CODE(x) == SUBREG) {
    ...
    if (REG_P(SUBREG_REG(x)) && (new_rtx = equiv_constant(SUBREG_REG(x))) != 0)
...
static void record_jump_equiv(rtx insn, bool taken) {
  int cond_known_true;
  rtx op0, op1;
  rtx set;
...
static rtx record_jump_cond_subreg(enum machine_mode mode, rtx op) {
...
static void record_jump_cond(enum rtx_code code, enum machine_mode mode,
                             rtx op0, rtx op1, int reversed_nonequality) {
  ...
  if (code == EQ && GET_CODE(op0) == SUBREG &&
    ...
    if (tem)
      record_jump_cond(code, mode, SUBREG_REG(op0), tem, reversed_nonequality);
  }
  ...
  if (code == EQ && GET_CODE(op1) == SUBREG &&
    ...
    if (tem)
      record_jump_cond(code, mode, SUBREG_REG(op1), tem, reversed_nonequality);
  }
  ...
  if (code != EQ || FLOAT_MODE_P(GET_MODE(op0))) {
    ...
    if ((reversed_nonequality && FLOAT_MODE_P(mode)) || !REG_P(op0) || op1 == 0)
      return;
...
static void cse_insn(rtx insn) {
  ...
  /* Canonicalize a USE of a pseudo register or memory location.  */
           ...
           !(REG_P(XEXP(x, 0)) && REGNO(XEXP(x, 0)) < FIRST_PSEUDO_REGISTER))
    canon_reg(XEXP(x, 0), insn);
  ...
  for (i = 0; i < n_sets; i++) {
    ...
    if (MEM_P(src) && find_reg_note(insn, REG_EQUIV, NULL_RTX) != 0 &&
    ...
    /* See if there is a register linearly related to a constant
       equivalent of SRC.  */
    if (src_const && (GET_CODE(src_const) == CONST ||
    ...
    if (flag_expensive_optimizations && src_related == 0 &&
        (GET_MODE_SIZE(mode) < UNITS_PER_WORD) &&
        GET_MODE_CLASS(mode) == MODE_INT && MEM_P(src) && !do_not_record &&
      ...
      for (tmode = GET_MODE_WIDER_MODE(mode);
        ...
        larger_elt =
            lookup(memory_extend_rtx, HASH(memory_extend_rtx, tmode), tmode);
        if (larger_elt == 0)
          continue;
    ...
    while (1) {
      ...
      if (src_folded &&
          preferable(src_folded_cost, src_folded_regcost, src_cost,
                     src_regcost) <= 0 &&
          preferable(src_folded_cost, src_folded_regcost, src_eqv_cost,
                     src_eqv_regcost) <= 0 &&
          preferable(src_folded_cost, src_folded_regcost, src_related_cost,
                     src_related_regcost) <= 0 &&
          preferable(src_folded_cost, src_folded_regcost, src_elt_cost,
                     src_elt_regcost) <= 0) {
        ...
        if (src_folded_force_flag) {
          ...
          if (forced)
            trial = forced;
                 ...
                 preferable(src_cost, src_regcost, src_eqv_cost,
                            src_eqv_regcost) <= 0 &&
                 preferable(src_cost, src_regcost, src_related_cost,
                            src_related_regcost) <= 0 &&
                 preferable(src_cost, src_regcost, src_elt_cost,
                            src_elt_regcost) <= 0)
        trial = src, src_cost = MAX_COST;
      else if (src_eqv_here &&
               preferable(src_eqv_cost, src_eqv_regcost, src_related_cost,
                          src_related_regcost) <= 0 &&
               preferable(src_eqv_cost, src_eqv_regcost, src_elt_cost,
                          src_elt_regcost) <= 0)
        trial = src_eqv_here, src_eqv_cost = MAX_COST;
      else if (src_related && preferable(src_related_cost, src_related_regcost,
      ...
      if (MEM_P(trial) && MEM_P(SET_DEST(sets[i].rtl))) {
        ...
        if (!MEM_P(src) || !MEM_P(dest) || !nonoverlapping_memrefs_p(src, dest))
          break;
      }
      ...
      else if (validate_unshare_change(insn, &SET_SRC(sets[i].rtl), trial, 0)) {
      ...
      /* If we previously found constant pool entries for
         ...
         this since that is where it likely came from.  */
               ...
               (src_folded == 0 ||
                (!MEM_P(src_folded) && !src_folded_force_flag)) &&
    ...
    if (REG_P(dest) && REGNO_QTY_VALID_P(REGNO(dest))) {
         ...
         SRC is a hard register.  */
      {
        ...
        rtx new_src = (first >= FIRST_PSEUDO_REGISTER
                           ? regno_reg_rtx[first]
    ...
    if (n_sets == 1 && src_const && REG_P(dest) && !REG_P(src_const) &&
    ...
    if (GET_CODE(SET_DEST(sets[i].rtl)) == ZERO_EXTRACT) {
      ...
      if (src_const != 0 && GET_CODE(src_const) == CONST_INT &&
          GET_CODE(width) == CONST_INT &&
          INTVAL(width) < HOST_BITS_PER_WIDE_INT &&
          !(INTVAL(src_const) & ((HOST_WIDE_INT)(-1) << INTVAL(width))))
    ...
    if (dest == cc0_rtx) {
      ...
      if (FLOAT_MODE_P(mode))
        this_insn_cc0 =
            gen_rtx_COMPARE(VOIDmode, this_insn_cc0, CONST0_RTX(mode));
    }
#endif
  }
    ...
    if (sets[i].rtl && !sets[i].src_volatile &&
      ...
      if (GET_CODE(SET_DEST(sets[i].rtl)) == STRICT_LOW_PART) {
        ...
        if (sets[i].src_const && sets[i].src_const_elt == 0 &&
            src != sets[i].src_const && !rtx_equal_p(sets[i].src_const, src))
          sets[i].src_elt =
  ...
  for (i = 0; i < n_sets; i++)
    if (sets[i].rtl) {
      ...
      /* Don't record value if we are not supposed to risk allocating
         ...
         memory.  */
      if ((flag_float_store && MEM_P(dest) && FLOAT_MODE_P(GET_MODE(dest)))
      ...
      elt->in_memory =
          (MEM_P(sets[i].inner_dest) && !MEM_READONLY_P(sets[i].inner_dest));
      ...
      if (GET_CODE(dest) == SUBREG &&
        ...
        for (elt = sets[i].src_elt->first_same_value; elt;
          ...
          if (GET_MODE(elt->exp) == new_mode)
            ...
            if (BYTES_BIG_ENDIAN)
              byte = (GET_MODE_SIZE(GET_MODE(dest)) - GET_MODE_SIZE(new_mode));

            new_src =
                simplify_gen_subreg(new_mode, elt->exp, GET_MODE(dest), byte);
          }
  ...
  if (n_sets == 1 && sets[0].rtl && REG_P(SET_DEST(sets[0].rtl)) &&
      NEXT_INSN(PREV_INSN(insn)) == insn && REG_P(SET_SRC(sets[0].rtl)) &&
    ...
    if (src_ent->first_reg == REGNO(SET_DEST(sets[0].rtl))) {
      ...
      if (NONJUMP_INSN_P(prev) && GET_CODE(PATTERN(prev)) == SET &&
        ...
        if (note != 0 && (reg_mentioned_p(dest, XEXP(note, 0)) ||
...
static rtx cse_process_notes_1(rtx x, rtx object, bool *changed) {
  ...
  switch (code) {
    ...
    case MEM:
      validate_change(x, &XEXP(x, 0), cse_process_notes(XEXP(x, 0), x, changed),
                      0);
      return x;
    ...
    case REG:
      ...
      if (REGNO_QTY_VALID_P(REGNO(x))) {
        ...
        if (ent->const_rtx != NULL_RTX &&
            (CONSTANT_P(ent->const_rtx) || REG_P(ent->const_rtx))) {
...
static rtx cse_process_notes(rtx x, rtx object, bool *changed) {
...
static bool cse_find_path(basic_block first_bb,
                          struct cse_basic_block_data *data, int follow_jumps) {
  basic_block bb;
  edge e;
  int path_size;
...
static void cse_dump_path(struct cse_basic_block_data *data, int nsets,
                          FILE *f) {
  int path_entry;
...
static void cse_extended_basic_block(struct cse_basic_block_data *ebb_data) {
  ...
  for (path_entry = 0; path_entry < path_size; path_entry++) {
    ...
    FOR_BB_INSNS(bb, insn) {
      ...
      /* If we have processed 1,000 insns, flush the hash table to
         ...
         FIXME: This is a real kludge and needs to be done some other
                way.  */
      if (INSN_P(insn) && num_insns++ > PARAM_VALUE(PARAM_MAX_CSE_INSNS)) {
        flush_hash_table();
        num_insns = 0;
      }
      ...
      if (INSN_P(insn)) {
        ...
        if (REG_NOTES(insn)) {
          bool changed = false;
          REG_NOTES(insn) =
              cse_process_notes(REG_NOTES(insn), NULL_RTX, &changed);
          if (changed)
            df_notes_rescan(insn);
        }
        ...
        if (INSN_P(insn) && !recorded_label_ref &&
            for_each_rtx(&PATTERN(insn), check_for_label_ref, (void *)insn))
           ...
           which is true until the final pass.  */
        {
          ...
          if (prev_insn && NONJUMP_INSN_P(prev_insn) &&
              (tem = single_set(prev_insn)) != 0 && SET_DEST(tem) == cc0_rtx &&
    ...
    insn = BB_END(bb);
    if (path_entry < path_size - 1 && JUMP_P(insn) && single_set(insn) &&
        any_condjump_p(insn)) {
      basic_block next_bb = ebb_data->path[path_entry + 1].bb;
...
int cse_main(rtx f ATTRIBUTE_UNUSED, int nregs) {
  ...
  ebb_data.path =
      XNEWVEC(struct branch_path, PARAM_VALUE(PARAM_MAX_CSE_PATH_LENGTH));
  ...
  while (i < n_blocks) {
    ...
    } while (TEST_BIT(cse_visited_basic_blocks, bb->index) && i < n_blocks);
...
static int check_for_label_ref(rtx *rtl, void *data) {
  ...
  return (GET_CODE(*rtl) == LABEL_REF && !LABEL_REF_NONLOCAL_P(*rtl) &&
          (!JUMP_P(insn) || !label_is_jump_target_p(XEXP(*rtl, 0), insn)) &&
          LABEL_P(XEXP(*rtl, 0)) && INSN_UID(XEXP(*rtl, 0)) != 0 &&
...
static void count_reg_usage(rtx x, int *counts, rtx dest, int incr) {
  ...
  switch (code = GET_CODE(x)) {
    ...
    case SET:
      ...
      count_reg_usage(SET_SRC(x), counts, dest ? dest : SET_DEST(x), incr);
      return;
...
#ifdef HAVE_cc0
  else if (GET_CODE(SET_DEST(set)) == CC0 && !side_effects_p(SET_SRC(set)) &&
           ((tem = next_nonnote_insn(insn)) == 0 || !INSN_P(tem) ||
            !reg_referenced_p(cc0_rtx, PATTERN(tem))))
    return false;
#endif
  else if (!REG_P(SET_DEST(set)) ||
           REGNO(SET_DEST(set)) < FIRST_PSEUDO_REGISTER ||
           counts[REGNO(SET_DEST(set))] != 0 || side_effects_p(SET_SRC(set)))
    return true;
  return false;
}
...
static int cse_change_cc_mode(rtx *loc, void *data) {
  struct change_cc_mode_args *args = (struct change_cc_mode_args *)data;

  if (*loc && REG_P(*loc) && REGNO(*loc) == REGNO(args->newreg) &&
...
static void cse_change_cc_mode_insn(rtx insn, rtx newreg) {
...
static void cse_change_cc_mode_insns(rtx start, rtx end, rtx newreg) {
  rtx insn;
...
static enum machine_mode cse_cc_succs(basic_block bb, basic_block orig_bb,
                                      rtx cc_reg, rtx cc_src,
      ...
      if (set && REG_P(SET_DEST(set)) &&
        ...
        if (rtx_equal_p(cc_src, SET_SRC(set)))
          found = true;
                 ...
                 GET_CODE(SET_SRC(set)) == COMPARE && mode != set_mode &&
                 rtx_equal_p(XEXP(cc_src, 0), XEXP(SET_SRC(set), 0)) &&
                 rtx_equal_p(XEXP(cc_src, 1), XEXP(SET_SRC(set), 1)))
  ...
  for (i = 0; i < insn_count; ++i) {
    if (modes[i] != mode) {
      ...
      cse_change_cc_mode_insns(NEXT_INSN(insns[i]), last_insns[i], newreg);
    }
...
static void cse_condition_code_reg(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    cc_src = NULL_RTX;
    for (insn = PREV_INSN(last_insn); insn && insn != PREV_INSN(BB_HEAD(bb));
         insn = PREV_INSN(insn)) {
      rtx set;
    ...
    if (mode != VOIDmode) {
      ...
      if (mode != orig_mode) {
        ...
        cse_change_cc_mode_insns(NEXT_INSN(cc_src_insn), NEXT_INSN(last_insn),
                                 newreg);
      }
    }
  }
...
static bool gate_handle_cse(void) {
  return optimize > 0;
}
...
static bool gate_handle_cse2(void) {
