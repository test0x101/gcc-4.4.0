...
static inline bool is_not_constant_evolution(const_tree cst) {
  return (TREE_CODE(cst) == POLYNOMIAL_CHREC);
}

/* Fold CODE for a polynomial function and a constant.  */

static inline tree chrec_fold_poly_cst(enum tree_code code, tree type,
                                       tree poly, tree cst) {
  gcc_assert(poly);
  gcc_assert(cst);
  ...
  switch (code) {
    case PLUS_EXPR:
      return build_polynomial_chrec(
          CHREC_VARIABLE(poly), chrec_fold_plus(type, CHREC_LEFT(poly), cst),
          CHREC_RIGHT(poly));

    case MINUS_EXPR:
      return build_polynomial_chrec(
          CHREC_VARIABLE(poly), chrec_fold_minus(type, CHREC_LEFT(poly), cst),
          CHREC_RIGHT(poly));
...
static inline tree chrec_fold_plus_poly_poly(enum tree_code code, tree type,
                                             tree poly0, tree poly1) {
  tree left, right;
  ...
  if (flow_loop_nested_p(loop0, loop1)) {
    if (code == PLUS_EXPR || code == POINTER_PLUS_EXPR)
      return build_polynomial_chrec(
          ...
          chrec_fold_plus(type, poly0, CHREC_LEFT(poly1)), CHREC_RIGHT(poly1));
    else
      return build_polynomial_chrec(
          CHREC_VARIABLE(poly1),
  ...
  if (flow_loop_nested_p(loop1, loop0)) {
    if (code == PLUS_EXPR || code == POINTER_PLUS_EXPR)
      return build_polynomial_chrec(
          ...
          chrec_fold_plus(type, CHREC_LEFT(poly0), poly1), CHREC_RIGHT(poly0));
    else
      return build_polynomial_chrec(
          CHREC_VARIABLE(poly0),
          chrec_fold_minus(type, CHREC_LEFT(poly0), poly1), CHREC_RIGHT(poly0));
  }
  ...
  if (code == PLUS_EXPR || code == POINTER_PLUS_EXPR) {
    left = chrec_fold_plus(type, CHREC_LEFT(poly0), CHREC_LEFT(poly1));
    right = chrec_fold_plus(rtype, CHREC_RIGHT(poly0), CHREC_RIGHT(poly1));
  } else {
    left = chrec_fold_minus(type, CHREC_LEFT(poly0), CHREC_LEFT(poly1));
    right = chrec_fold_minus(type, CHREC_RIGHT(poly0), CHREC_RIGHT(poly1));
  }
...
static inline tree chrec_fold_multiply_poly_poly(tree type, tree poly0,
                                                 tree poly1) {
  tree t0, t1, t2;
  int var;
  struct loop *loop0 = get_chrec_loop(poly0);
  ...
  t2 = chrec_fold_multiply(type,
                           SCALAR_FLOAT_TYPE_P(type) ? build_real(type, dconst2)
                                                     : build_int_cst(type, 2),
                           t2);

  var = CHREC_VARIABLE(poly0);
  return build_polynomial_chrec(var, t0, build_polynomial_chrec(var, t1, t2));
}

/* When the operands are automatically_generated_chrec_p, the fold has
   to respect the semantics of the operands.  */

static inline tree chrec_fold_automatically_generated_operands(tree op0,
                                                               tree op1) {
  if (op0 == chrec_dont_know || op1 == chrec_dont_know)
    return chrec_dont_know;

  if (op0 == chrec_known || op1 == chrec_known)
    return chrec_known;

  if (op0 == chrec_not_analyzed_yet || op1 == chrec_not_analyzed_yet)
...
static tree chrec_fold_plus_1(enum tree_code code, tree type, tree op0,
                              tree op1) {
      ...
      switch (TREE_CODE(op1)) {
        ...
        default:
          if (code == PLUS_EXPR || code == POINTER_PLUS_EXPR)
            return build_polynomial_chrec(
                CHREC_VARIABLE(op0),
                chrec_fold_plus(type, CHREC_LEFT(op0), op1), CHREC_RIGHT(op0));
          else
            return build_polynomial_chrec(
                CHREC_VARIABLE(op0),
                chrec_fold_minus(type, CHREC_LEFT(op0), op1), CHREC_RIGHT(op0));
      }
      ...
      switch (TREE_CODE(op1)) {
        case POLYNOMIAL_CHREC:
          if (code == PLUS_EXPR || code == POINTER_PLUS_EXPR)
            return build_polynomial_chrec(
                CHREC_VARIABLE(op1),
                chrec_fold_plus(type, op0, CHREC_LEFT(op1)), CHREC_RIGHT(op1));
          else
            return build_polynomial_chrec(
                CHREC_VARIABLE(op1),
        ...
        default: {
          ...
          if ((tree_contains_chrecs(op0, &size) ||
            ...
            return fold_build2(code, type, fold_convert(type, op0),
...
static tree tree_fold_binomial(tree type, tree n, unsigned int k) {
  ...
  /* Check that k <= n.  */
  if (TREE_INT_CST_HIGH(n) == 0 && TREE_INT_CST_LOW(n) < k)
    return NULL_TREE;
  ...
  div_and_round_double(EXACT_DIV_EXPR, 1, lnum, hnum, ldenom, hdenom, &lres,
                       &hres, &ldum, &hdum);
...
static tree chrec_evaluate(unsigned var, tree chrec, tree n, unsigned int k) {
  ...
  if (TREE_CODE(chrec) == POLYNOMIAL_CHREC && CHREC_VARIABLE(chrec) == var) {
    ...
    if (!binomial_n_k)
      return chrec_dont_know;
    arg0 = fold_build2(MULT_EXPR, type, CHREC_LEFT(chrec), binomial_n_k);
...
tree chrec_apply(unsigned var, tree chrec, tree x) {
  ...
  else if (TREE_CODE(x) == INTEGER_CST && tree_int_cst_sgn(x) == 1)
...
tree hide_evolution_in_other_loops_than_loop(tree chrec, unsigned loop_num) {
  ...
  switch (TREE_CODE(chrec)) {
    case POLYNOMIAL_CHREC:
      ...
      if (chloop == loop)
        return build_polynomial_chrec(loop_num,
                                      hide_evolution_in_other_loops_than_loop(
                                          CHREC_LEFT(chrec), loop_num),
                                      CHREC_RIGHT(chrec));
...
static tree chrec_component_in_loop_num(tree chrec, unsigned loop_num,
                                        bool right) {
  tree component;
  struct loop *loop = get_loop(loop_num), *chloop;
    ...
    case POLYNOMIAL_CHREC:
      ...
      if (chloop == loop) {
        ...
        if (TREE_CODE(CHREC_LEFT(chrec)) != POLYNOMIAL_CHREC ||
          ...
          return build_polynomial_chrec(
              loop_num,
              chrec_component_in_loop_num(CHREC_LEFT(chrec), loop_num, right),
              component);
      }
        ...
        gcc_assert(flow_loop_nested_p(loop, chloop));
        return chrec_component_in_loop_num(CHREC_LEFT(chrec), loop_num, right);
      }
...
tree reset_evolution_in_loop(unsigned loop_num, tree chrec, tree new_evol) {
  ...
  if (TREE_CODE(chrec) == POLYNOMIAL_CHREC &&
    ...
    tree left = reset_evolution_in_loop(loop_num, CHREC_LEFT(chrec), new_evol);
    tree right =
        reset_evolution_in_loop(loop_num, CHREC_RIGHT(chrec), new_evol);
    return build3(POLYNOMIAL_CHREC, TREE_TYPE(left),
                  build_int_cst(NULL_TREE, CHREC_VARIABLE(chrec)), left, right);
  }
...
tree chrec_merge(tree chrec1, tree chrec2) {
  if (chrec1 == chrec_dont_know || chrec2 == chrec_dont_know)
    return chrec_dont_know;

  if (chrec1 == chrec_known || chrec2 == chrec_known)
    return chrec_known;
...
bool is_multivariate_chrec(const_tree chrec) {
  ...
  if (TREE_CODE(chrec) == POLYNOMIAL_CHREC)
    return (
        is_multivariate_chrec_rec(CHREC_LEFT(chrec), CHREC_VARIABLE(chrec)) ||
        is_multivariate_chrec_rec(CHREC_RIGHT(chrec), CHREC_VARIABLE(chrec)));
  else
    return false;
}
...
bool chrec_contains_symbols(const_tree chrec) {
  ...
  if (TREE_CODE(chrec) == SSA_NAME || TREE_CODE(chrec) == VAR_DECL ||
      TREE_CODE(chrec) == PARM_DECL || TREE_CODE(chrec) == FUNCTION_DECL ||
      TREE_CODE(chrec) == LABEL_DECL || TREE_CODE(chrec) == RESULT_DECL ||
...
static bool evolution_function_is_invariant_rec_p(tree chrec, int loopnum) {
  ...
  if (TREE_CODE(chrec) == SSA_NAME &&
      (loopnum == 0 || expr_invariant_in_loop_p(get_loop(loopnum), chrec)))
    return true;

  if (TREE_CODE(chrec) == POLYNOMIAL_CHREC) {
    if (CHREC_VARIABLE(chrec) == (unsigned)loopnum ||
        !evolution_function_is_invariant_rec_p(CHREC_RIGHT(chrec), loopnum) ||
        !evolution_function_is_invariant_rec_p(CHREC_LEFT(chrec), loopnum))
      return false;
    return true;
  }
      ...
      if (evolution_function_is_invariant_rec_p(CHREC_LEFT(chrec), loopnum)) {
        if (evolution_function_is_invariant_rec_p(CHREC_RIGHT(chrec), loopnum))
          ...
          if (TREE_CODE(CHREC_RIGHT(chrec)) == POLYNOMIAL_CHREC &&
              CHREC_VARIABLE(CHREC_RIGHT(chrec)) != CHREC_VARIABLE(chrec) &&
              evolution_function_is_affine_multivariate_p(CHREC_RIGHT(chrec),
                                                          loopnum))
            return true;
          else
            return false;
        ...
        if (evolution_function_is_invariant_rec_p(CHREC_RIGHT(chrec),
            ...
            CHREC_VARIABLE(CHREC_LEFT(chrec)) != CHREC_VARIABLE(chrec) &&
            evolution_function_is_affine_multivariate_p(CHREC_LEFT(chrec),
                                                        loopnum))
          return true;
        else
          return false;
...
unsigned nb_vars_in_chrec(tree chrec) {
  ...
  switch (TREE_CODE(chrec)) {
    case POLYNOMIAL_CHREC:
      return 1 + nb_vars_in_chrec(initial_condition_in_loop_num(
                     chrec, CHREC_VARIABLE(chrec)));
...
static bool avoid_arithmetics_in_type_p(const_tree type) {
...
bool convert_affine_scev(struct loop *loop, tree type, tree *base, tree *step,
                         gimple at_stmt, bool use_overflow_semantics) {
  ...
  enforce_overflow_semantics = (use_overflow_semantics && nowrap_type_p(type));
  ...
  new_base = chrec_convert_1(type, *base, at_stmt, use_overflow_semantics);
  ...
  if (TYPE_PRECISION(step_type) > TYPE_PRECISION(ct) && TYPE_UNSIGNED(ct))
    new_step = chrec_convert_1(signed_type_for(ct), new_step, at_stmt,
                               use_overflow_semantics);
  new_step =
      chrec_convert_1(step_type, new_step, at_stmt, use_overflow_semantics);
...
tree chrec_convert(tree type, tree chrec, gimple at_stmt) {
...
static tree chrec_convert_1(tree type, tree chrec, gimple at_stmt,
...
keep_cast:
  ...
  if (TREE_CODE(res) == INTEGER_CST && TREE_CODE(type) == INTEGER_TYPE &&
...
tree chrec_convert_aggressive(tree type, tree chrec) {
...
static inline bool operator_is_linear(tree scev) {
...
bool scev_is_linear_expression(tree scev) {
  if (scev == NULL || !operator_is_linear(scev))
    return false;
