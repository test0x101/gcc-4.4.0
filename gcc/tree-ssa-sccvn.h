...
static inline hashval_t vn_hash_constant_with_type(tree constant) {
  ...
  return (
      iterative_hash_expr(constant, 0) + INTEGRAL_TYPE_P(type) +
...
tree vn_nary_op_lookup_pieces(unsigned int, enum tree_code, tree, tree, tree,
...
vn_nary_op_t vn_nary_op_insert_pieces(unsigned int, enum tree_code, tree, tree,
                                      tree, tree, tree, tree, unsigned int);
void copy_reference_ops_from_ref(tree, VEC(vn_reference_op_s, heap) **);
void copy_reference_ops_from_call(gimple, VEC(vn_reference_op_s, heap) **);
tree vn_reference_lookup_pieces(VEC(tree, gc) *, VEC(vn_reference_op_s, heap) *,
                                vn_reference_t *, bool);
...
vn_reference_t vn_reference_insert_pieces(VEC(tree, gc) *,
                                          VEC(vn_reference_op_s, heap) *, tree,
                                          unsigned int);
