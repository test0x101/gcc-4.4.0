...
static void do_niy(pretty_printer *buffer, const_tree node) {
  int i, len;
...
static void dump_decl_name(pretty_printer *buffer, tree node, int flags) {
  ...
  if ((flags & TDF_UID) || DECL_NAME(t) == NULL_TREE) {
...
static void dump_function_declaration(pretty_printer *buffer, tree node,
                                      int spc, int flags) {
...
static void dump_array_domain(pretty_printer *buffer, tree domain, int spc,
                              int flags) {
  pp_character(buffer, '[');
  if (domain) {
...
static void dump_omp_clause(pretty_printer *buffer, tree clause, int spc,
                            int flags) {
  const char *name;
  ...
  switch (OMP_CLAUSE_CODE(clause)) {
    ...
    case OMP_CLAUSE_IF:
      pp_string(buffer, "if(");
      dump_generic_node(buffer, OMP_CLAUSE_IF_EXPR(clause), spc, flags, false);
      pp_character(buffer, ')');
      break;

    case OMP_CLAUSE_NUM_THREADS:
      pp_string(buffer, "num_threads(");
      dump_generic_node(buffer, OMP_CLAUSE_NUM_THREADS_EXPR(clause), spc, flags,
                        false);
      pp_character(buffer, ')');
      break;
    ...
    case OMP_CLAUSE_SCHEDULE:
      ...
      if (OMP_CLAUSE_SCHEDULE_CHUNK_EXPR(clause)) {
        pp_character(buffer, ',');
        dump_generic_node(buffer, OMP_CLAUSE_SCHEDULE_CHUNK_EXPR(clause), spc,
                          flags, false);
      }
      pp_character(buffer, ')');
      break;
    ...
    case OMP_CLAUSE_COLLAPSE:
      ...
      dump_generic_node(buffer, OMP_CLAUSE_COLLAPSE_EXPR(clause), spc, flags,
                        false);
      pp_character(buffer, ')');
      break;
...
void dump_omp_clauses(pretty_printer *buffer, tree clause, int spc, int flags) {
  if (clause == NULL)
    return;
...
int dump_generic_node(pretty_printer *buffer, tree node, int spc, int flags,
                      bool is_stmt) {
  tree type;
  tree op0, op1;
  const char *str;
  ...
  switch (TREE_CODE(node)) {
    ...
    case TREE_VEC: {
      ...
      if (TREE_VEC_LENGTH(node) > 0) {
        ...
        for (i = 0; i < len - 1; i++) {
          dump_generic_node(buffer, TREE_VEC_ELT(node, i), spc, flags, false);
          pp_character(buffer, ',');
          pp_space(buffer);
        }
        dump_generic_node(buffer, TREE_VEC_ELT(node, len - 1), spc, flags,
                          false);
      }
    } break;
    ...
    case BOOLEAN_TYPE: {
      ...
      if (tclass == tcc_declaration) {
        ...
        if (TYPE_NAME(node)) {
          ...
          pp_string(buffer, (TYPE_UNSIGNED(node) ? "<unnamed-unsigned:"
                                                 : "<unnamed-signed:"));
    ...
    case TARGET_MEM_REF: {
      ...
      if (flags & TDF_DETAILS) {
        pp_string(buffer, "{");
        dump_generic_node(buffer, TMR_ORIGINAL(node), spc, flags, false);
        pp_string(buffer, "}");
      }
    } break;
    ...
    case TYPE_DECL:
      ...
      if (DECL_NAME(node))
        ...
        if ((TREE_CODE(TREE_TYPE(node)) == RECORD_TYPE ||
          ...
          pp_string(
              buffer,
              (TREE_CODE(TREE_TYPE(node)) == UNION_TYPE ? "union" : "struct "));
          dump_generic_node(buffer, TREE_TYPE(node), spc, flags, false);
        }
      }
      break;
    ...
    case COMPOUND_EXPR: {
      ...
      for (tp = &TREE_OPERAND(node, 1); TREE_CODE(*tp) == COMPOUND_EXPR;
      ...
      if (TREE_TYPE(node) == NULL || TREE_TYPE(node) == void_type_node) {
        ...
        if (COND_EXPR_THEN(node) &&
          ...
          if (COND_EXPR_THEN(node)) {
            ...
            dump_generic_node(buffer, COND_EXPR_THEN(node), spc + 4, flags,
                              true);
            newline_and_indent(buffer, spc + 2);
            pp_character(buffer, '}');
          }
          ...
          if (COND_EXPR_ELSE(node) && !IS_EMPTY_STMT(COND_EXPR_ELSE(node))) {
            ...
            pp_character(buffer, '{');
            newline_and_indent(buffer, spc + 4);
            dump_generic_node(buffer, COND_EXPR_ELSE(node), spc + 4, flags,
                              true);
            newline_and_indent(buffer, spc + 2);
            pp_character(buffer, '}');
          }
    ...
    case OMP_FOR:
      ...
      if (!(flags & TDF_SLIM)) {
        ...
        if (OMP_FOR_PRE_BODY(node)) {
          ...
          spc += 4;
          newline_and_indent(buffer, spc);
          dump_generic_node(buffer, OMP_FOR_PRE_BODY(node), spc, flags, false);
        }
        spc -= 2;
        for (i = 0; i < TREE_VEC_LENGTH(OMP_FOR_INIT(node)); i++) {
          ...
          dump_generic_node(buffer, TREE_VEC_ELT(OMP_FOR_INIT(node), i), spc,
                            flags, false);
          pp_string(buffer, "; ");
          dump_generic_node(buffer, TREE_VEC_ELT(OMP_FOR_COND(node), i), spc,
                            flags, false);
          pp_string(buffer, "; ");
          dump_generic_node(buffer, TREE_VEC_ELT(OMP_FOR_INCR(node), i), spc,
                            flags, false);
          pp_string(buffer, ")");
        }
    ...
    case OMP_CRITICAL:
      ...
      if (OMP_CRITICAL_NAME(node)) {
        pp_space(buffer);
        pp_character(buffer, '(');
        dump_generic_node(buffer, OMP_CRITICAL_NAME(node), spc, flags, false);
        pp_character(buffer, ')');
      }
      goto dump_omp_body;
    ...
    case BLOCK: {
      ...
      if (BLOCK_SUPERCONTEXT(node)) {
        ...
        if (TREE_CODE(BLOCK_SUPERCONTEXT(node)) == BLOCK)
          pp_printf(buffer, "BLOCK %p", (void *)BLOCK_SUPERCONTEXT(node));
        else
          dump_generic_node(buffer, BLOCK_SUPERCONTEXT(node), 0, flags, false);
        newline_and_indent(buffer, spc + 2);
      }
      ...
      if (BLOCK_ABSTRACT_ORIGIN(node)) {
        ...
        if (TREE_CODE(BLOCK_ABSTRACT_ORIGIN(node)) == BLOCK)
          pp_printf(buffer, "BLOCK %p", (void *)BLOCK_ABSTRACT_ORIGIN(node));
        else
...
void print_declaration(pretty_printer *buffer, tree t, int spc, int flags) {
  INDENT(spc);
...
static void print_struct_decl(pretty_printer *buffer, const_tree node, int spc,
                              int flags) {
  /* Print the name of the structure.  */
  if (TYPE_NAME(node)) {
    INDENT(spc);
...
static void print_call_name(pretty_printer *buffer, const_tree node) {
  tree op0;
...
static void pretty_print_string(pretty_printer *buffer, const char *str) {
  if (str == NULL)
    return;
