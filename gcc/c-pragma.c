...
#define GCC_BAD2(gmsgid, arg)           \
  do {                                  \
    warning(OPT_Wpragmas, gmsgid, arg); \
    return;                             \
  } while (0)
...
#define SET_GLOBAL_ALIGNMENT(ALIGN)                   \
  (maximum_field_alignment =                          \
       *(alignment_stack == NULL ? &default_alignment \
...
static void pop_alignment(tree id) {
  align_stack *entry;
...
/* #pragma pack ()
   ...
   #pragma pack (pop)
   #pragma pack (pop, ID) */
static void handle_pragma_pack(cpp_reader *ARG_UNUSED(dummy)) {
  tree x, id = 0;
  int align = -1;
...
static hashval_t dpm_hash(const void *p) {
  return ((const struct def_pragma_macro *)p)->hash;
}

static int dpm_eq(const void *pa, const void *pb) {
  const struct def_pragma_macro *const a = (const struct def_pragma_macro *)pa,
                                       *const b =
                                           (const struct def_pragma_macro *)pb;
...
static void handle_pragma_push_macro(cpp_reader *reader) {
  tree x, id = 0;
  enum cpp_ttype token;
  ...
  slot =
      htab_find_slot_with_hash(pushed_macro_table, &dummy, dummy.hash, INSERT);
...
static void handle_pragma_pop_macro(cpp_reader *reader) {
  tree x, id = 0;
  enum cpp_ttype token;
  struct def_pragma_macro dummy, *c;
  const char *macroname;
  ...
  if (pushed_macro_table)
    slot = htab_find_slot_with_hash(pushed_macro_table, &dummy, dummy.hash,
                                    NO_INSERT);
  if (slot == NULL)
    return;
  c = (struct def_pragma_macro *)*slot;
...
static void apply_pragma_weak(tree decl, tree value) {
  if (value) {
    value = build_string(IDENTIFIER_LENGTH(value), IDENTIFIER_POINTER(value));
    decl_attributes(
        &decl,
        build_tree_list(get_identifier("alias"), build_tree_list(NULL, value)),
        0);
  }
...
#else
void maybe_apply_pragma_weak(tree ARG_UNUSED(decl)) {}
...
static void handle_pragma_redefine_extname(cpp_reader *ARG_UNUSED(dummy)) {
  ...
  if (decl && (TREE_PUBLIC(decl) || DECL_EXTERNAL(decl)) &&
...
static void handle_pragma_extern_prefix(cpp_reader *ARG_UNUSED(dummy)) {
  tree prefix, x;
  enum cpp_ttype t;
...
tree maybe_apply_renaming_pragma(tree decl, tree asmname) {
  ...
  /* The renaming pragmas are only applied to declarations with
     external linkage.  */
  if ((TREE_CODE(decl) != FUNCTION_DECL && TREE_CODE(decl) != VAR_DECL) ||
      (!TREE_PUBLIC(decl) && !DECL_EXTERNAL(decl)) || !has_c_linkage(decl))
    return asmname;
  ...
  for (p = &pending_redefine_extname; (t = *p); p = &TREE_CHAIN(t))
    if (DECL_NAME(decl) == TREE_PURPOSE(t)) {
      ...
      *p = TREE_CHAIN(t);
      ...
      if (asmname) {
        if (strcmp(TREE_STRING_POINTER(asmname), IDENTIFIER_POINTER(newname)) !=
            0)
          warning(OPT_Wpragmas,
                  "#pragma redefine_extname ignored due to "
...
void push_visibility(const char *str) {
  VEC_safe_push(visibility, heap, visstack, default_visibility);
  if (!strcmp(str, "default"))
    ...
    GCC_BAD(
        "#pragma GCC visibility push() must specify default, internal, hidden "
        "or protected");
...
void pop_visibility(void) {
  ...
  visibility_options.inpragma = VEC_length(visibility, visstack) != 0;
}
  ...
  for (option_index = 0; option_index < cl_options_count; option_index++)
    if (strcmp(cl_options[option_index].opt_text, option_string) == 0) {
      ...
      if (cl_options[option_index].var_type == CLVC_BOOLEAN &&
          cl_options[option_index].flag_var && kind != DK_IGNORED)
...
static void handle_pragma_target(cpp_reader *ARG_UNUSED(dummy)) {
  enum cpp_ttype token;
  tree x;
...
static void handle_pragma_optimize(cpp_reader *ARG_UNUSED(dummy)) {
  enum cpp_ttype token;
  tree x;
  bool close_paren_needed_p = false;
  ...
  /* Strings/numbers are user options.  */
    ...
    c_cpp_builtins_optimize_pragma(parse_in, optimization_previous_node,
...
static void handle_pragma_push_options(cpp_reader *ARG_UNUSED(dummy)) {
  enum cpp_ttype token;
  tree x = 0;
  opt_stack *p;
...
static void handle_pragma_pop_options(cpp_reader *ARG_UNUSED(dummy)) {
  enum cpp_ttype token;
  tree x = 0;
  opt_stack *p;
  ...
  if (p->optimize_binary != optimization_current_node) {
    ...
    c_cpp_builtins_optimize_pragma(parse_in, old_optimize, p->optimize_binary);
...
static void handle_pragma_reset_options(cpp_reader *ARG_UNUSED(dummy)) {
  enum cpp_ttype token;
  tree x = 0;
...
static void handle_pragma_message(cpp_reader *ARG_UNUSED(dummy)) {
  enum cpp_ttype token;
  tree x, message = 0;
...
void c_pp_lookup_pragma(unsigned int id, const char **space,
                        const char **name) {
...
static void c_register_pragma_1(const char *space, const char *name,
  ...
  cpp_register_deferred_pragma(parse_in, space, name, id, allow_expansion,
                               false);
}
...
void c_register_pragma_with_expansion(const char *space, const char *name,
                                      pragma_handler handler) {
