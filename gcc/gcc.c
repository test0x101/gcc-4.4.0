...
static char *build_search_list(const struct path_prefix *, const char *, bool,
                               bool);
...
static void add_prefix(struct path_prefix *, const char *, const char *, int,
                       int, int);
...
static const char *process_brace_body(const char *, const char *, const char *,
                                      int, int);
...
static const struct default_spec option_default_specs[] = {
...
} target_option_translations[] = {TARGET_OPTION_TRANSLATE_TABLE, {0, 0}};
#endif
...
static void translate_options(int *argcp, const char *const **argvp) {
  ...
  while (i < argc) {
    ...
    for (tott_idx = 0; target_option_translations[tott_idx].option_found;
         tott_idx++) {
      if (strcmp(target_option_translations[tott_idx].option_found, argv[i]) ==
          0) {
        int spaces = 1;
        const char *sp;
        char *np;

        for (sp = target_option_translations[tott_idx].replacements; *sp;
             sp++) {
          if (*sp == ' ')
            spaces++;
        }
      ...
      for (j = 0; j < ARRAY_SIZE(option_map); j++) {
        ...
        if (!strncmp(argv[i], option_map[j].name, complen)) {
          ...
          if (strchr(arginfo, 'a') != 0) {
            if (arg == 0) {
              if (i + 1 == argc) {
                error("missing argument to '%s' option", option_map[j].name);
                break;
              }
            ...
            if (arg != 0)
              error("extraneous argument to '%s' option", option_map[j].name);
            arg = 0;
          }

          /* Store the translation as one argv elt or as two.  */
          if (arg != 0 && strchr(arginfo, 'j') != 0)
            newv[newindex++] = concat(option_map[j].equivalent, arg, NULL);
          else if (arg != 0) {
...
static void init_gcc_specs(struct obstack *obstack, const char *shared_name,
...
#if USE_LD_AS_NEEDED
               "%{!shared-libgcc:",
               static_name, " --as-needed ", shared_name,
               " --no-as-needed"
               "}"
               "%{shared-libgcc:",
     ...
     do the actual object file scanning.  */
  {
    ...
    while (*p) {
      if (in_sep && *p == '-' && strncmp(p, "-lgcc", 5) == 0) {
        ...
        init_gcc_specs(&obstack, "-lgcc_s", "libgcc.a%s",
                       "libgcc_eh.a%s"
#ifdef USE_LIBUNWIND_EXCEPTIONS
                       " -lunwind"
#endif
...
static void set_spec(const char *name, const char *spec) {
  struct spec_list *sl;
  const char *old_spec;
...
static void store_arg(const char *arg, int delete_always, int delete_failure) {
...
static void read_specs(const char *filename, int main_p) {
  char *buffer;
  char *p;
  ...
  while (1) {
    ...
    if (*p1 != ':')
      fatal("specs file malformed after %ld characters", (long)(p1 - buffer));
    ...
    if (p[1] == 0)
      fatal("specs file malformed after %ld characters", (long)(p - buffer));
    ...
    *out = 0;
    ...
    if (suffix[0] == '*') {
      ...
      compilers = XRESIZEVEC(struct compiler, compilers, n_compilers + 2);
...
void record_temp_file(const char *filename, int always_delete,
                      int fail_delete) {
...
static void *for_each_path(const struct path_prefix *paths, bool do_multi,
                           size_t extra_space,
  ...
  while (1) {
    ...
    for (pl = paths->plist; pl != 0; pl = pl->next) {
      ...
      if (!skip_multi_dir && pl->require_machine_suffix == 2) {
...
static char *build_search_list(const struct path_prefix *paths,
                               const char *prefix, bool check_dir,
                               bool do_multi) {
...
static void putenv_from_prefixes(const struct path_prefix *paths,
                                 const char *env_var, bool do_multi) {
...
static char *find_a_file(const struct path_prefix *pprefix, const char *name,
                         int mode, bool do_multi) {
  ...
  return (char *)for_each_path(
      pprefix, do_multi, info.name_len + info.suffix_len, file_at_path, &info);
}
...
static void add_prefix(struct path_prefix *pprefix, const char *prefix,
  ...
  for (prev = &pprefix->plist; (*prev) != NULL && (*prev)->priority <= priority;
       prev = &(*prev)->next)
    ;
...
static int execute(void) {
  ...
  for (n_commands = 1, i = 0; i < argbuf_index; i++)
    if (strcmp(argbuf[i], "|") == 0) { /* each command.  */
      ...
      string =
          find_a_file(&exec_prefixes, commands[n_commands].prog, X_OK, false);
      if (string)
  ...
  for (i = 0; i < n_commands; i++) {
    ...
    errmsg = pex_run(pex,
                     ...
                     string, CONST_CAST(char **, commands[i].argv), NULL, NULL,
                     &err);
    if (errmsg != NULL) {
      if (err == 0)
        fatal(errmsg);
...
See %s for instructions.",
              strsignal(WTERMSIG(status)), commands[i].prog, bug_report_url);
      ...
      if (report_times) {
        ...
        ut = ((double)pt->user_seconds + (double)pt->user_microseconds / 1.0e6);
...
static void display_help(void) {
  ...
  fputs(_("  --version                Display compiler version information\n"),
        stdout);
  fputs(_("  -dumpspecs               Display all of the built in spec "
          "strings\n"),
        stdout);
  fputs(_("  -dumpversion             Display the version of the compiler\n"),
        stdout);
  fputs(
      _("  -dumpmachine             Display the compiler's target processor\n"),
      stdout);
  fputs(_("  -print-search-dirs       Display the directories in the "
          "compiler's search path\n"),
        stdout);
  fputs(_("  -print-libgcc-file-name  Display the name of the compiler's "
          "companion library\n"),
        stdout);
  fputs(
      _("  -print-file-name=<lib>   Display the full path to library <lib>\n"),
      stdout);
  ...
  -print-multi-lib         Display the mapping between command line options and\n\
        ...
        stdout);
  fputs(_("  -print-multi-os-directory Display the relative path to OS "
          "libraries\n"),
        stdout);
  fputs(
      _("  -print-sysroot           Display the target libraries directory\n"),
      stdout);
  fputs(_("  -print-sysroot-headers-suffix Display the sysroot suffix used to "
          "find headers\n"),
        stdout);
  fputs(_("  -Wa,<options>            Pass comma-separated <options> on to the "
          "assembler\n"),
        stdout);
  fputs(_("  -Wp,<options>            Pass comma-separated <options> on to the "
          "preprocessor\n"),
        stdout);
  fputs(_("  -Wl,<options>            Pass comma-separated <options> on to the "
          "linker\n"),
        stdout);
  ...
  fputs(_("  -combine                 Pass multiple source files to compiler "
          "at once\n"),
        stdout);
  fputs(_("  -save-temps              Do not delete intermediate files\n"),
        stdout);
  fputs(_("  -pipe                    Use pipes rather than intermediate "
          "files\n"),
        stdout);
  ...
  fputs(_("  -specs=<file>            Override built-in specs with the "
          "contents of <file>\n"),
        stdout);
  fputs(_("  -std=<standard>          Assume that the input sources are for "
          "<standard>\n"),
        stdout);
  fputs(_("\
  --sysroot=<directory>    Use <directory> as the root directory for headers\n\
                           and libraries\n"),
        stdout);
  fputs(_("  -B <directory>           Add <directory> to the compiler's search "
          "paths\n"),
        stdout);
  fputs(_("  -b <machine>             Run gcc for target <machine>, if "
          "installed\n"),
        stdout);
  fputs(_("  -V <version>             Run gcc version number <version>, if "
          "installed\n"),
        stdout);
  fputs(_("  -v                       Display the programs invoked by the "
          "compiler\n"),
        stdout);
  fputs(_("  -###                     Like -v but options quoted and commands "
          "not executed\n"),
        stdout);
  fputs(_("  -E                       Preprocess only; do not compile, "
          "assemble or link\n"),
        stdout);
...
static void add_preprocessor_option(const char *option, int len) {
  ...
  if (!preprocessor_options)
    ...
    preprocessor_options =
        XRESIZEVEC(char *, preprocessor_options, n_preprocessor_options);
...
static void add_assembler_option(const char *option, int len) {
  ...
  if (!assembler_options)
    ...
    assembler_options =
        XRESIZEVEC(char *, assembler_options, n_assembler_options);
...
static void process_command(int argc, const char **argv) {
  ...
  if (argc > 1 && argv[1][0] == '-' &&
      (argv[1][1] == 'V' ||
       (argv[1][1] == 'b' &&
        (argv[1][2] == '\0' || NULL != strchr(argv[1] + 2, '-'))))) {
    ...
    for (baselen = strlen(progname); baselen > 0; baselen--)
      if (IS_DIR_SEPARATOR(progname[baselen - 1]))
        break;
    new_argv0 = XDUPVAR(
        char, progname, baselen,
        baselen + concat_length(new_version, new_machine, "-gcc-", NULL) + 1);
    ...
    errmsg = pex_one(PEX_SEARCH, new_argv0, new_argv, progname, NULL, NULL,
                     &status, &err);
    ...
    if (errmsg) {
      if (err == 0)
        ...
        fatal("couldn't run '%s': %s: %s", new_argv0, errmsg, xstrerror(err));
    }
    exit(status);
  }
  ...
  if (!gcc_exec_prefix) {
    ...
    gcc_libexec_prefix = make_relative_prefix(argv[0], standard_bindir_prefix,
    ...
    gcc_libexec_prefix = make_relative_prefix(tmp_prefix, standard_exec_prefix,
                                              standard_libexec_prefix);
  ...
  if (gcc_exec_prefix) {
    ...
    if (len > (int)sizeof("/lib/gcc/") - 1 &&
      ...
      if (IS_DIR_SEPARATOR(*temp) && strncmp(temp + 1, "lib", 3) == 0 &&
          IS_DIR_SEPARATOR(temp[4]) && strncmp(temp + 5, "gcc", 3) == 0)
        len -= sizeof("/lib/gcc/") - 1;
    }

    set_std_prefix(gcc_exec_prefix, len);
    add_prefix(&exec_prefixes, gcc_libexec_prefix, "GCC", PREFIX_PRIORITY_LAST,
               0, 0);
  ...
  if (temp) {
    ...
    while (1) {
      if (*endp == PATH_SEPARATOR || *endp == 0) {
        ...
        add_prefix(&exec_prefixes, nstore, 0, PREFIX_PRIORITY_LAST, 0, 0);
        add_prefix(&include_prefixes, nstore, 0, PREFIX_PRIORITY_LAST, 0, 0);
        if (*endp == 0)
          break;
        endp = startp = endp + 1;
      } else
    ...
    while (1) {
      if (*endp == PATH_SEPARATOR || *endp == 0) {
        ...
        if (endp == startp)
          ...
          nstore[endp - startp] = 0;
        add_prefix(&startfile_prefixes, nstore, NULL, PREFIX_PRIORITY_LAST, 0,
                   1);
        if (*endp == 0)
          break;
        endp = startp = endp + 1;
  ...
  for (i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-dumpspecs")) {
      ...
      printf(_("%s %s%s\n"), programname, pkgversion_string, version_string);
...
warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n\n"),
      ...
      switch (c) {
        ...
        case 'B': {
          ...
          if (!IS_DIR_SEPARATOR(value[len - 1]) && is_directory(value, false)) {
          ...
          add_prefix(&exec_prefixes, value, NULL, PREFIX_PRIORITY_B_OPT, 0, 0);
          add_prefix(&startfile_prefixes, value, NULL, PREFIX_PRIORITY_B_OPT, 0,
                     0);
          add_prefix(&include_prefixes, value, NULL, PREFIX_PRIORITY_B_OPT, 0,
                     0);
          n_switches++;
        } break;
        ...
        normal_switch:
          ...
          for (j = 0; j < ARRAY_SIZE(modify_target); j++)
            if (!strcmp(argv[i], modify_target[j].sw)) {
              char *new_name = XNEWVEC(
                  char, strlen(modify_target[j].str) + strlen(spec_machine));
              const char *p, *r;
              char *q;
              int made_addition = 0;
              ...
              for (p = spec_machine, q = new_name; *p != 0;) {
                ...
                else if (modify_target[j].add_del == ADD && !made_addition &&
                         *p == '-') {
  ...
  tooldir_prefix =
      concat(tooldir_base_prefix, spec_machine, dir_separator_str, NULL);

  /* Look for tools relative to the location from which the driver is
     running, or, if that is not available, the configured prefix.  */
  tooldir_prefix = concat(
      gcc_exec_prefix ? gcc_exec_prefix : standard_exec_prefix, spec_machine,
      dir_separator_str, spec_version, dir_separator_str, tooldir_prefix, NULL);

  add_prefix(&exec_prefixes,
             concat(tooldir_prefix, "bin", dir_separator_str, NULL), "BINUTILS",
             PREFIX_PRIORITY_LAST, 0, 0);
  add_prefix(&startfile_prefixes,
             concat(tooldir_prefix, "lib", dir_separator_str, NULL), "BINUTILS",
  ...
  if (target_system_root && gcc_exec_prefix) {
    char *tmp_prefix = make_relative_prefix(argv[0], standard_bindir_prefix,
                                            target_system_root);
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (is_modify_target_name)
      ...
      for (j = 4; argv[i][j]; j++)
        if (argv[i][j] == ',') {
          ...
          infiles[n_infiles++].name = save_string(argv[i] + prev, j - prev);
          prev = j + 1;
        }
      /* Record the part after the last comma.  */
      infiles[n_infiles].language = "*";
    ...
    /* Xassembler and Xpreprocessor were already handled in the first argv
      ...
      i++;
             ...
             0) { /* POSIX allows separation of -l and the lib arg;
...
static void set_collect_gcc_options(void) {
  ...
  obstack_grow(&collect_obstack,
               "COLLECT_GCC_OPTIONS=", sizeof("COLLECT_GCC_OPTIONS=") - 1);
...
static void do_self_spec(const char *spec) {
  do_spec_2(spec);
  do_spec_1(" ", 0, NULL);
...
static int do_spec_1(const char *spec, int inswitch,
      ...
      case '%':
        switch (c = *p++) {
          ...
          case 'j': {
            ...
            if ((!save_temps_flag) && (stat(HOST_BIT_BUCKET, &st) == 0) &&
                (!S_ISDIR(st.st_mode)) &&
          ...
          create_temp_file : {
            ...
            if (p[0] == '%' && p[1] == 'O') {
              ...
              if (suffix_length == 0)
                ...
                saved_suffix =
                    XNEWVEC(char, suffix_length + strlen(TARGET_OBJECT_SUFFIX));
                strncpy(saved_suffix, suffix, suffix_length);
                strcpy(saved_suffix + suffix_length, TARGET_OBJECT_SUFFIX);
              }
            ...
            if (save_temps_flag) {
              ...
              if (strcmp(temp_filename, input_filename) != 0) {
                ...
                if (input_stat_set != 1 || stat(temp_filename, &st_temp) < 0 ||
          ...
          case 'I': {
            ...
            for_each_path(&include_prefixes, false, info.append_len, spec_path,
                          &info);

            info.append = "include-fixed";
            if (*sysroot_hdrs_suffix_spec)
              info.append =
                  concat(info.append, dir_separator_str, multilib_dir, NULL);
            info.append_len = strlen(info.append);
            for_each_path(&include_prefixes, false, info.append_len, spec_path,
                          &info);
          } break;
          ...
          case 'o': {
            ...
            if (HAVE_GNU_LD && at_file_supplied) {
              ...
              if (f == NULL)
                fatal("could not open temporary response file %s", temp_file);
              ...
              if (EOF == status)
                fatal("could not close temporary response file %s", temp_file);
          ...
          case '*':
            if (soft_matched_part) {
              ...
              /* Catch the case where a spec string contains something like
                 ...
                 hand side of the :.  */
              error(
                  "spec failure: '%%*' has not been initialized by pattern "
                  "match");
            break;
...
static const char *eval_spec_function(const char *func, const char *args) {
  const struct spec_function *sf;
  const char *funcval;
...
static inline bool input_suffix_matches(const char *atom,
                                        const char *end_atom) {
  return (input_suffix && !strncmp(input_suffix, atom, end_atom - atom) &&
...
static bool input_spec_matches(const char *atom, const char *end_atom) {
  return (input_file_compiler && input_file_compiler->suffix &&
          input_file_compiler->suffix[0] != '\0' &&
          !strncmp(input_file_compiler->suffix + 1, atom, end_atom - atom) &&
...
/* Subroutine of handle_braces.  Returns true if a switch
   ...
   command line.  */
static bool switch_matches(const char *atom, const char *end_atom,
                           int starred) {
  int i;
...
static inline void mark_matching_switches(const char *atom,
                                          const char *end_atom, int starred) {
  int i;
  int len = end_atom - atom;
  int plen = starred ? len : -1;
  ...
  do {
    ...
    switch (*p) {
      ...
      case ':':
        ...
        if (atom == end_atom) {
          if (!n_way_choice || disj_matched || *p == '|' || a_is_negated ||
              a_is_suffix || a_is_spectype || a_is_starred)
            goto invalid;
...
static const char *process_brace_body(const char *p, const char *atom,
                                      const char *end_atom, int starred,
                                      int matched) {
  ...
  switch (*name) {
    ...
    case 'm':
      if (!strncmp(name + 1, "no-", 3)) {
        ...
        for (i = switchnum + 1; i < n_switches; i++)
          if (switches[i].part1[0] == name[0] && switches[i].part1[1] == 'n' &&
              switches[i].part1[2] == 'o' && switches[i].part1[3] == '-' &&
...
static int is_directory(const char *path1, bool linker) {
  int len1;
  char *path;
  char *cp;
  struct stat st;
  ...
  *cp = '\0';
  ...
  if (linker && IS_DIR_SEPARATOR(path[0]) &&
      ((cp - path == 6 && strncmp(path + 1, "lib", 3) == 0) ||
       (cp - path == 10 && strncmp(path + 1, "usr", 3) == 0 &&
        IS_DIR_SEPARATOR(path[4]) && strncmp(path + 5, "lib", 3) == 0)))
    return 0;
...
int main(int argc, char **argv) {
     ...
     multilib selection.  */
  {
    ...
    for (i = 0; i < ARRAY_SIZE(multilib_defaults_raw); i++) {
      ...
      obstack_grow(&multilib_obstack, multilib_defaults_raw[i],
  ...
  machine_suffix = concat(spec_machine, dir_separator_str, spec_version,
  ...
  for (i = 0; i < ARRAY_SIZE(option_default_specs); i++)
    do_option_spec(option_default_specs[i].name, option_default_specs[i].spec);
  ...
  if (*cross_compile == '0') {
    if (*md_exec_prefix) {
      add_prefix(&exec_prefixes, md_exec_prefix, "GCC", PREFIX_PRIORITY_LAST, 0,
                 0);
    }
  }
  ...
  if (*startfile_prefix_spec != 0 && do_spec_2(startfile_prefix_spec) == 0 &&
      do_spec_1(" ", 0, NULL) == 0) {
    int ndx;
  ...
  /* We should eventually get rid of all these and stick to
    ...
    if (*md_startfile_prefix_1)
      add_sysrooted_prefix(&startfile_prefixes, md_startfile_prefix_1, "GCC",
                           PREFIX_PRIORITY_LAST, 0, 1);
    ...
    if (IS_ABSOLUTE_PATH(standard_startfile_prefix))
      ...
      add_prefix(
          ...
          concat(gcc_exec_prefix ? gcc_exec_prefix : standard_exec_prefix,
                 machine_suffix, standard_startfile_prefix, NULL),
  ...
  for (uptr = user_specs_head; uptr; uptr = uptr->next) {
    char *filename =
        find_a_file(&startfile_prefixes, uptr->filename, R_OK, true);
  ...
  if (print_sysroot_headers_suffix) {
    if (*sysroot_hdrs_suffix_spec) {
      printf("%s\n",
             (target_sysroot_hdrs_suffix ? target_sysroot_hdrs_suffix : ""));
      return (0);
    } else
  ...
  if (combine_flag && save_temps_flag) {
    ...
    for (i = 0; (int)i < n_infiles; i++) {
      ...
      if (infiles[i].incompiler && (infiles[i].incompiler)->needs_preprocessing)
      ...
      if (input_file_compiler) {
        if (input_file_compiler->spec[0] == '#') {
          error("%s: %s compiler not installed on this system", input_filename,
                &input_file_compiler->spec[1]);
          this_file_error = 1;
        } else {
          ...
          infiles[i].incompiler = lookup_compiler(
              infiles[i].name, strlen(infiles[i].name), infiles[i].language);
  ...
  for (i = 0; (int)i < n_infiles; i++) {
    ...
    if (!combine_inputs)
      input_file_compiler = lookup_compiler(
          infiles[i].name, input_filename_length, infiles[i].language);
    else
      input_file_compiler = infiles[i].incompiler;
  ...
  if (num_linker_inputs > 0 && error_count == 0 && print_subprocess_help < 2) {
    ...
    if (print_subprocess_help == 1) {
      printf(_("\nLinker options\n==============\n\n"));
      printf(
          _("Use \"-Wl,OPTION\" to pass \"OPTION\""
            " to the linker.\n\n"));
      fflush(stdout);
    }
...
static struct compiler *lookup_compiler(const char *name, size_t length,
                                        const char *language) {
  struct compiler *cp;
      ...
      if (/* The suffix `-' matches only the file name `-'.  */
          ...
          (strlen(cp->suffix) < length
           ...
           && ((!strcmp(cp->suffix, name + length - strlen(cp->suffix)) ||
               ...
               !strcasecmp(cp->suffix, name + length - strlen(cp->suffix)))))
        break;
    }
#endif
...
static char *save_string(const char *s, int len) {
...
static inline void validate_switches_from_spec(const char *spec) {
  const char *p = spec;
  char c;
  while ((c = *p++))
    if (c == '%' && (*p == '{' || *p == '<' || (*p == 'W' && *++p == '{')))
...
static int used_arg(const char *p, int len) {
  struct mswitchstr {
    const char *str;
    const char *replace;
    int len;
  ...
  if (!mswitches) {
    ...
    /* Now build a list of the replacement string for switches that we care
       ...
       block of code.  */
    mswitches = XNEWVEC(struct mswitchstr,
...
static int default_arg(const char *p, int len) {
  int i;
...
static void set_multilib_dir(void) {
  ...
  while (*p != '\0') {
    ...
    while (*p != ';') {
      if (*p == '\0') {
      invalid_exclusions:
        fatal("multilib exclusions '%s' is invalid", multilib_exclusions);
      }
...
static void print_multilib_info(void) {
  ...
  while (*p != '\0') {
       ...
       all of its options with the select rule, we skip it.  */
    {
      ...
      while (*e != '\0') {
        ...
        while (*e != ';') {
          ...
          if (*e == '\0') {
          invalid_exclusion:
            fatal("multilib exclusion '%s' is invalid", multilib_exclusions);
          }
    ...
    if (!skip) {
      ...
      skip =
          (last_path != 0 && (unsigned int)(p - this_path) == last_path_len &&
...
static const char *getenv_spec_function(int argc, const char **argv) {
  char *value;
  char *result;
  char *ptr;
  size_t len;
...
static const char *print_asm_header_spec_function(
    int arg ATTRIBUTE_UNUSED, const char **argv ATTRIBUTE_UNUSED) {
