...
static gimple get_prop_dest_stmt(tree name, tree *final_name_p) {
  use_operand_p use;
  gimple use_stmt;
...
static gimple get_prop_source_stmt(tree name, bool single_use_only,
                                   bool *single_use_p) {
...
static bool remove_prop_source_from_use(tree name, gimple up_to_stmt) {
...
static tree combine_cond_expr_cond(enum tree_code code, tree type, tree op0,
                                   tree op1, bool invariant_only) {
  tree t;
...
static int forward_propagate_into_gimple_cond(gimple stmt) {
  int did_something = 0;
  ...
  do {
    ...
    if (TREE_CODE_CLASS(gimple_cond_code(stmt)) == tcc_comparison &&
      ...
      if (def_stmt && can_propagate_from(def_stmt)) {
        ...
        tmp = combine_cond_expr_cond(code, boolean_type_node, rhs0, op1,
                                     !single_use0_p);
      }
      /* If that wasn't successful, try the second operand.  */
      if (tmp == NULL_TREE && TREE_CODE(gimple_cond_rhs(stmt)) == SSA_NAME) {
      ...
      if (tmp == NULL_TREE && rhs0 != NULL_TREE && rhs1 != NULL_TREE)
    ...
    if (tmp) {
      if (dump_file && tmp) {
        tree cond = build2(gimple_cond_code(stmt), boolean_type_node,
...
static int forward_propagate_into_cond(gimple_stmt_iterator *gsi_p) {
  ...
  do {
    ...
    if (COMPARISON_CLASS_P(cond) &&
      ...
      if (def_stmt && can_propagate_from(def_stmt)) {
        ...
        rhs0 = rhs_to_tree(TREE_TYPE(op1), def_stmt);
        tmp = combine_cond_expr_cond(TREE_CODE(cond), boolean_type_node, rhs0,
                                     op1, !single_use0_p);
      }
      /* If that wasn't successful, try the second operand.  */
      if (tmp == NULL_TREE && TREE_CODE(TREE_OPERAND(cond, 1)) == SSA_NAME) {
        ...
        rhs1 = rhs_to_tree(TREE_TYPE(op0), def_stmt);
        tmp = combine_cond_expr_cond(TREE_CODE(cond), boolean_type_node, op0,
                                     rhs1, !single_use1_p);
      }
      /* If that wasn't successful either, try both operands.  */
      if (tmp == NULL_TREE && rhs0 != NULL_TREE && rhs1 != NULL_TREE)
      ...
      tmp = combine_cond_expr_cond(NE_EXPR, boolean_type_node, rhs0,
                                   build_int_cst(TREE_TYPE(rhs0), 0), false);
    }
...
static bool forward_propagate_addr_into_variable_array_index(
    tree offset, tree def_rhs, gimple_stmt_iterator *use_stmt_gsi) {
  tree index;
  ...
  TREE_OPERAND(TREE_OPERAND(gimple_assign_rhs1(use_stmt), 0), 1) = index;
...
static bool forward_propagate_addr_expr_1(tree name, tree def_rhs,
  ...
  if (TREE_CODE(lhs) == SSA_NAME && ((rhs_code == SSA_NAME && rhs == name) ||
  ...
  while (handled_component_p(*rhsp) || TREE_CODE(*rhsp) == ADDR_EXPR)
  ...
  if (TREE_CODE(rhs2) == INTEGER_CST) {
    tree new_rhs =
        maybe_fold_stmt_addition(gimple_expr_type(use_stmt), array_ref, rhs2);
    if (new_rhs) {
...
static bool forward_propagate_addr_expr(tree name, tree rhs) {
  ...
  FOR_EACH_IMM_USE_STMT(use_stmt, iter, name) {
...

    {
      ...
      result = forward_propagate_addr_expr_1(name, rhs, &gsi, single_use_p);
    ...
    if (result && TREE_CODE(gimple_assign_lhs(use_stmt)) == SSA_NAME &&
...
static bool forward_propagate_comparison(gimple stmt) {
  ...
  if (is_gimple_assign(use_stmt) &&
    ...
    /* We can propagate the condition into X op CST where op
      ...
      cond = build2(gimple_assign_rhs_code(stmt), TREE_TYPE(cst),
                    gimple_assign_rhs1(stmt), gimple_assign_rhs2(stmt));
    ...
    if (dump_file && (dump_flags & TDF_DETAILS)) {
      tree old_rhs = rhs_to_tree(TREE_TYPE(gimple_assign_lhs(stmt)), stmt);
      fprintf(dump_file, "  Replaced '");
...
static void simplify_not_neg_expr(gimple_stmt_iterator *gsi_p) {
  gimple stmt = gsi_stmt(*gsi_p);
...
static void simplify_gimple_switch(gimple stmt) {
...
static unsigned int tree_ssa_forward_propagate_single_use_vars(void) {
  basic_block bb;
      ...
      if (is_gimple_assign(stmt)) {
        ...
        if (gimple_assign_rhs_code(stmt) == ADDR_EXPR
            ...
            (CONVERT_EXPR_CODE_P(gimple_assign_rhs_code(stmt)) &&
             TREE_CODE(rhs) == ADDR_EXPR && POINTER_TYPE_P(TREE_TYPE(lhs)))) {
          STRIP_NOPS(rhs);
          ...
          if (did_something == 2)
            cfg_changed = true;
          fold_undefer_overflow_warnings(!TREE_NO_WARNING(rhs) && did_something,
                                         stmt,
