...
static void create_loop_tree_nodes(bool loops_p) {
  unsigned int i, j;
  int max_regno;
  bool skip_p;
  edge_iterator ei;
  ...
  ira_bb_nodes = ((struct ira_loop_tree_node *)ira_allocate(
      sizeof(struct ira_loop_tree_node) * last_basic_block));
  ...
  ira_loop_nodes = ((struct ira_loop_tree_node *)ira_allocate(
      sizeof(struct ira_loop_tree_node) *
  ...
  for (i = 0; VEC_iterate(loop_p, ira_loops.larray, i, loop); i++) {
    if (loop != ira_loops.tree_root) {
      ...
      if (e->src != loop->latch && (e->flags & EDGE_ABNORMAL) &&
          EDGE_CRITICAL_P(e)) {
        skip_p = true;
        break;
      }
...
static void add_loop_to_tree(struct loop *loop) {
  struct loop *parent;
...
static void initiate_allocnos(void) {
  allocno_live_range_pool = create_alloc_pool(
      "allocno live ranges", sizeof(struct ira_allocno_live_range), 100);
  allocno_pool = create_alloc_pool("allocnos", sizeof(struct ira_allocno), 100);
...
ira_allocno_t ira_create_allocno(int regno, bool cap_p,
                                 ira_loop_tree_node_t loop_tree_node) {
  ira_allocno_t a;
...
void ira_set_allocno_cover_class(ira_allocno_t a, enum reg_class cover_class) {
...
bool ira_conflict_vector_profitable_p(ira_allocno_t a, int num) {
  int nw;
...
void ira_allocate_allocno_conflict_vec(ira_allocno_t a, int num) {
  int size;
  ira_allocno_t *vec;
...
static void allocate_allocno_conflict_bit_vec(ira_allocno_t a) {
  unsigned int size;

  ira_assert(ALLOCNO_CONFLICT_ALLOCNO_ARRAY(a) == NULL);
  size = ((ALLOCNO_MAX(a) - ALLOCNO_MIN(a) + IRA_INT_BITS) / IRA_INT_BITS *
          sizeof(IRA_INT_TYPE));
  ALLOCNO_CONFLICT_ALLOCNO_ARRAY(a) = ira_allocate(size);
...
void ira_allocate_allocno_conflicts(ira_allocno_t a, int num) {
...
static void add_to_allocno_conflicts(ira_allocno_t a1, ira_allocno_t a2) {
  int num;
  unsigned int size;
  ...
  if (ALLOCNO_CONFLICT_VEC_P(a1)) {
    ...
    if (ALLOCNO_CONFLICT_ALLOCNO_ARRAY_SIZE(a1) >= num * sizeof(ira_allocno_t))
    ...
    if (ALLOCNO_MIN(a1) > id) {
      ...
      if (ALLOCNO_CONFLICT_ALLOCNO_ARRAY_SIZE(a1) >= size) {
        memmove((char *)vec + added_head_nw * sizeof(IRA_INT_TYPE), vec,
        ...
        memset(vec, 0, added_head_nw * sizeof(IRA_INT_TYPE));
        memset((char *)vec + (nw + added_head_nw) * sizeof(IRA_INT_TYPE), 0,
               size - (nw + added_head_nw) * sizeof(IRA_INT_TYPE));
        ira_free(ALLOCNO_CONFLICT_ALLOCNO_ARRAY(a1));
        ALLOCNO_CONFLICT_ALLOCNO_ARRAY(a1) = vec;
        ALLOCNO_CONFLICT_ALLOCNO_ARRAY_SIZE(a1) = size;
      ...
      if (ALLOCNO_CONFLICT_ALLOCNO_ARRAY_SIZE(a1) < size) {
        ...
        memset((char *)vec + ALLOCNO_CONFLICT_ALLOCNO_ARRAY_SIZE(a1), 0,
               size - ALLOCNO_CONFLICT_ALLOCNO_ARRAY_SIZE(a1));
        ira_free(ALLOCNO_CONFLICT_ALLOCNO_ARRAY(a1));
        ALLOCNO_CONFLICT_ALLOCNO_ARRAY(a1) = vec;
        ALLOCNO_CONFLICT_ALLOCNO_ARRAY_SIZE(a1) = size;
      }
...
void ira_add_allocno_conflict(ira_allocno_t a1, ira_allocno_t a2) {
  add_to_allocno_conflicts(a1, a2);
  add_to_allocno_conflicts(a2, a1);
}

/* Clear all conflicts of allocno A.  */
static void clear_allocno_conflicts(ira_allocno_t a) {
  if (ALLOCNO_CONFLICT_VEC_P(a)) {
    ...
    memset(ALLOCNO_CONFLICT_ALLOCNO_ARRAY(a), 0, nw * sizeof(IRA_INT_TYPE));
  }
}
...
static void compress_allocno_conflict_vec(ira_allocno_t a) {
...
static void compress_conflict_vecs(void) {
  ...
  allocno_conflict_check = (int *)ira_allocate(sizeof(int) * ira_allocnos_num);
...
static ira_allocno_t create_cap_allocno(ira_allocno_t a) {
  ...
  ira_allocate_and_copy_costs(&ALLOCNO_HARD_REG_COSTS(cap), cover_class,
                              ALLOCNO_HARD_REG_COSTS(a));
  ira_allocate_and_copy_costs(&ALLOCNO_CONFLICT_HARD_REG_COSTS(cap),
                              cover_class, ALLOCNO_CONFLICT_HARD_REG_COSTS(a));
...
allocno_live_range_t ira_create_allocno_live_range(ira_allocno_t a, int start,
                                                   int finish,
...
static allocno_live_range_t copy_allocno_live_range(allocno_live_range_t r) {
  allocno_live_range_t p;
...
/* Copy allocno live range list given by its head R and return the
   result.  */
allocno_live_range_t ira_copy_allocno_live_range_list(allocno_live_range_t r) {
...
/* Merge ranges R1 and R2 and returns the result.  The function
   ...
   result ranges.  */
allocno_live_range_t ira_merge_allocno_live_ranges(allocno_live_range_t r1,
...
bool ira_allocno_live_ranges_intersect_p(allocno_live_range_t r1,
                                         allocno_live_range_t r2) {
...
void ira_finish_allocno_live_range(allocno_live_range_t r) {
  pool_free(allocno_live_range_pool, r);
}

/* Free list of allocno live ranges starting with R.  */
void ira_finish_allocno_live_range_list(allocno_live_range_t r) {
...
static void initiate_copies(void) {
  copy_pool = create_alloc_pool("copies", sizeof(struct ira_allocno_copy), 100);
...
static ira_copy_t find_allocno_copy(ira_allocno_t a1, ira_allocno_t a2,
                                    rtx insn,
                                    ira_loop_tree_node_t loop_tree_node) {
...
ira_copy_t ira_create_copy(ira_allocno_t first, ira_allocno_t second, int freq,
...
void ira_add_allocno_copy_to_list(ira_copy_t cp) {
...
void ira_remove_allocno_copy_from_list(ira_copy_t cp) {
  ira_allocno_t first = cp->first, second = cp->second;
...
void ira_swap_allocno_copy_ends_if_necessary(ira_copy_t cp) {
  ira_allocno_t temp;
  ira_copy_t temp_cp;
...
/* Create (or update frequency if the copy already exists) and return
   ...
   LOOP_TREE_NODE.  */
ira_copy_t ira_add_allocno_copy(ira_allocno_t first, ira_allocno_t second,
                                int freq, bool constraint_p, rtx insn,
                                ira_loop_tree_node_t loop_tree_node) {
  ira_copy_t cp;
  ...
  cp = ira_create_copy(first, second, freq, constraint_p, insn, loop_tree_node);
...
static void print_allocno_copies(FILE *f, ira_allocno_t a) {
  ...
  for (cp = ALLOCNO_COPIES(a); cp != NULL; cp = next_cp) {
    if (cp->first == a) {
      ...
      gcc_unreachable();
    fprintf(f, " cp%d:a%d(r%d)@%d", cp->num, ALLOCNO_NUM(another_a),
            ALLOCNO_REGNO(another_a), cp->freq);
  }
  fprintf(f, "\n");
}
...
static void initiate_cost_vectors(void) {
  ...
  for (i = 0; i < ira_reg_class_cover_size; i++) {
    ...
    cost_vector_pool[cover_class] = create_alloc_pool(
        "cost vectors", sizeof(int) * ira_class_hard_regs_num[cover_class],
        100);
  }
}

/* Allocate and return a cost vector VEC for COVER_CLASS.  */
int *ira_allocate_cost_vector(enum reg_class cover_class) {
  return (int *)pool_alloc(cost_vector_pool[cover_class]);
}

/* Free a cost vector VEC for COVER_CLASS.  */
void ira_free_cost_vector(int *vec, enum reg_class cover_class) {
  ira_assert(vec != NULL);
...
/* This recursive function traverses loop tree with root LOOP_NODE
   ...
   subloop nodes).  */
void ira_traverse_loop_tree(bool bb_p, ira_loop_tree_node_t loop_node,
  ...
  if (bb_p)
    for (subloop_node = loop_node->children; subloop_node != NULL;
  ...
  for (subloop_node = loop_node->subloops; subloop_node != NULL;
       subloop_node = subloop_node->subloop_next) {
    ira_assert(subloop_node->bb == NULL);
    ira_traverse_loop_tree(bb_p, subloop_node, preorder_func, postorder_func);
  }
...
/* This recursive function creates allocnos corresponding to
   ...
   a lvalue.  */
static void create_insn_allocnos(rtx x, bool output_p) {
  int i, j;
  const char *fmt;
...
static void create_loop_allocnos(edge e) {
  ...
  EXECUTE_IF_SET_IN_REG_SET(DF_LR_OUT(e->src), FIRST_PSEUDO_REGISTER, i, bi)
  if (bitmap_bit_p(live_in_regs, i)) {
    ...
    bitmap_set_bit(border_allocnos, ALLOCNO_NUM(ira_curr_regno_allocno_map[i]));
  }
}
...
static void propagate_allocno_info(void) {
  ...
  for (i = max_reg_num() - 1; i >= FIRST_PSEUDO_REGISTER; i--)
    for (a = ira_regno_allocno_map[i]; a != NULL;
      ...
      if ((parent = ALLOCNO_LOOP_TREE_NODE(a)->parent) != NULL &&
        ...
        ira_allocate_and_accumulate_costs(&ALLOCNO_HARD_REG_COSTS(parent_a),
                                          cover_class,
                                          ALLOCNO_HARD_REG_COSTS(a));
        ira_allocate_and_accumulate_costs(
            &ALLOCNO_CONFLICT_HARD_REG_COSTS(parent_a), cover_class,
            ALLOCNO_CONFLICT_HARD_REG_COSTS(a));
        ALLOCNO_COVER_CLASS_COST(parent_a) += ALLOCNO_COVER_CLASS_COST(a);
...
static void change_allocno_in_range_list(allocno_live_range_t r,
                                         ira_allocno_t a) {
  for (; r != NULL; r = r->next)
    r->allocno = a;
}
...
static bool low_pressure_loop_node_p(ira_loop_tree_node_t node) {
  ...
  for (i = 0; i < ira_reg_class_cover_size; i++) {
    cover_class = ira_reg_class_cover[i];
    if (node->reg_pressure[cover_class] > ira_available_class_regs[cover_class])
      return false;
  }
  return true;
}
...
static void mark_loops_for_removal(void) {
  ...
  sorted_loops = (ira_loop_tree_node_t *)ira_allocate(
      sizeof(ira_loop_tree_node_t) * VEC_length(loop_p, ira_loops.larray));
...
static void ira_rebuild_regno_allocno_list(int regno) {
  ...
  for (n = 0, a = ira_regno_allocno_map[regno]; a != NULL;
       a = ALLOCNO_NEXT_REGNO_ALLOCNO(a))
    regno_allocnos[n++] = a;
  ira_assert(n > 0);
...
static void propagate_some_info_from_allocno(ira_allocno_t a,
...
static void remove_unnecessary_allocnos(void) {
  ...
  for (regno = max_reg_num() - 1; regno >= FIRST_PSEUDO_REGISTER; regno--) {
    rebuild_p = false;
    for (prev_a = NULL, a = ira_regno_allocno_map[regno]; a != NULL;
         a = next_a) {
    ...
    /* We need to restore the order in regno allocno list.  */
    {
      if (regno_allocnos == NULL)
        regno_allocnos = (ira_allocno_t *)ira_allocate(sizeof(ira_allocno_t) *
                                                       ira_allocnos_num);
...
/* Sort allocnos according to their live ranges.  Allocnos with
   ...
   finish (max).  */
static int allocno_range_compare_func(const void *v1p, const void *v2p) {
  int diff;
  ira_allocno_t a1 = *(const ira_allocno_t *)v1p;
  ira_allocno_t a2 = *(const ira_allocno_t *)v2p;
...
static void setup_min_max_conflict_allocno_ids(void) {
  ...
  for (i = 0; i < ira_allocnos_num; i++) {
    ...
    if (a == NULL)
      continue;
    if (cover_class < 0 || (flag_ira_algorithm != IRA_ALGORITHM_PRIORITY &&
      ...
      while (first_not_finished < i &&
             start >
                 ALLOCNO_MAX(ira_conflict_id_allocno_map[first_not_finished]))
        first_not_finished++;
...
static bool copy_info_to_removed_store_destinations(int regno) {
  ...
  merged_p = false;
  for (a = ira_regno_allocno_map[regno]; a != NULL;
       a = ALLOCNO_NEXT_REGNO_ALLOCNO(a)) {
    ...
    ira_assert(ALLOCNO_CAP_MEMBER(a) == NULL);
    for (parent = ALLOCNO_LOOP_TREE_NODE(a)->parent; parent != NULL;
         parent = parent->parent)
      if ((parent_a = parent->regno_allocno_map[regno]) == NULL ||
          (parent_a ==
               regno_top_level_allocno_map[REGNO(ALLOCNO_REG(parent_a))] &&
    ...
    if (internal_flag_ira_verbose > 4 && ira_dump_file != NULL) {
      fprintf(ira_dump_file,
              "      Coping ranges of a%dr%d to a%dr%d: ", ALLOCNO_NUM(a),
              REGNO(ALLOCNO_REG(a)), ALLOCNO_NUM(parent_a),
              REGNO(ALLOCNO_REG(parent_a)));
      ira_print_live_range_list(ira_dump_file, ALLOCNO_LIVE_RANGES(a));
    }
...
void ira_flattening(int max_regno_before_emit, int ira_max_point_before_emit) {
  ...
  regno_top_level_allocno_map =
      (ira_allocno_t *)ira_allocate(max_reg_num() * sizeof(ira_allocno_t));
  memset(regno_top_level_allocno_map, 0, max_reg_num() * sizeof(ira_allocno_t));
  ...
  for (i = max_regno_before_emit - 1; i >= FIRST_PSEUDO_REGISTER; i--) {
    ...
    for (a = ira_regno_allocno_map[i]; a != NULL;
      ...
      if (ALLOCNO_CAP(a) != NULL ||
          ...
          ((parent_a = parent->regno_allocno_map[ALLOCNO_REGNO(a)]) == NULL)) {
        ALLOCNO_COPIES(a) = NULL;
      ...
      if (REGNO(ALLOCNO_REG(a)) == REGNO(ALLOCNO_REG(parent_a))) {
        ...
        if (internal_flag_ira_verbose > 4 && ira_dump_file != NULL) {
          fprintf(ira_dump_file,
                  "      Moving ranges of a%dr%d to a%dr%d: ", ALLOCNO_NUM(a),
                  REGNO(ALLOCNO_REG(a)), ALLOCNO_NUM(parent_a),
                  REGNO(ALLOCNO_REG(parent_a)));
          ira_print_live_range_list(ira_dump_file, ALLOCNO_LIVE_RANGES(a));
        }
        change_allocno_in_range_list(ALLOCNO_LIVE_RANGES(a), parent_a);
        ALLOCNO_LIVE_RANGES(parent_a) = ira_merge_allocno_live_ranges(
      ...
      for (;;) {
        ...
        ira_assert(ALLOCNO_CALLS_CROSSED_NUM(parent_a) >= 0 &&
                   ALLOCNO_NREFS(parent_a) >= 0 && ALLOCNO_FREQ(parent_a) >= 0);
        cover_class = ALLOCNO_COVER_CLASS(parent_a);
        hard_regs_num = ira_class_hard_regs_num[cover_class];
        if (ALLOCNO_HARD_REG_COSTS(a) != NULL &&
            ALLOCNO_HARD_REG_COSTS(parent_a) != NULL)
          for (j = 0; j < hard_regs_num; j++)
            ALLOCNO_HARD_REG_COSTS(parent_a)[j] -= ALLOCNO_HARD_REG_COSTS(a)[j];
        if (ALLOCNO_CONFLICT_HARD_REG_COSTS(a) != NULL &&
            ALLOCNO_CONFLICT_HARD_REG_COSTS(parent_a) != NULL)
          for (j = 0; j < hard_regs_num; j++)
            ALLOCNO_CONFLICT_HARD_REG_COSTS(parent_a)
            [j] -= ALLOCNO_CONFLICT_HARD_REG_COSTS(a)[j];
        ALLOCNO_COVER_CLASS_COST(parent_a) -= ALLOCNO_COVER_CLASS_COST(a);
        ALLOCNO_MEMORY_COST(parent_a) -= ALLOCNO_MEMORY_COST(a);
        if (ALLOCNO_CAP(parent_a) != NULL ||
            (parent = ALLOCNO_LOOP_TREE_NODE(parent_a)->parent) == NULL ||
            (parent_a = (parent->regno_allocno_map[ALLOCNO_REGNO(parent_a)])) ==
                NULL)
          break;
      }
      ALLOCNO_COPIES(a) = NULL;
  ...
  FOR_EACH_COPY(cp, ci) {
    if (ALLOCNO_CAP_MEMBER(cp->first) != NULL ||
      ...
      if (internal_flag_ira_verbose > 4 && ira_dump_file != NULL)
        fprintf(ira_dump_file, "      Remove cp%d:%c%dr%d-%c%dr%d\n", cp->num,
                ALLOCNO_CAP_MEMBER(cp->first) != NULL ? 'c' : 'a',
    ...
    if (node == NULL)
      ...
      keep_p =
          ((REGNO(ALLOCNO_REG(first)) == REGNO(ALLOCNO_REG(node_first))) &&
           (REGNO(ALLOCNO_REG(second)) == REGNO(ALLOCNO_REG(node_second))));
    }
    if (keep_p) {
  ...
  FOR_EACH_COPY(cp, ci) {
    ...
    ira_assert(ALLOCNO_LOOP_TREE_NODE(cp->first) == ira_loop_tree_root &&
...
static void check_allocno_creation(void) {
  ira_allocno_t a;
  ira_allocno_iterator ai;
  ira_loop_tree_node_t loop_tree_node;
  ...
  FOR_EACH_ALLOCNO(a, ai) {
    loop_tree_node = ALLOCNO_LOOP_TREE_NODE(a);
    ira_assert(bitmap_bit_p(loop_tree_node->all_allocnos, ALLOCNO_NUM(a)));
    if (loop_tree_node == ira_loop_tree_root)
      continue;
    if (ALLOCNO_CAP_MEMBER(a) != NULL)
      ira_assert(ALLOCNO_CAP(a) != NULL);
    else if (ALLOCNO_CAP(a) == NULL)
      ira_assert(loop_tree_node->parent->regno_allocno_map[ALLOCNO_REGNO(a)] !=
                     NULL &&
                 bitmap_bit_p(loop_tree_node->border_allocnos, ALLOCNO_NUM(a)));
  }
}
#endif
