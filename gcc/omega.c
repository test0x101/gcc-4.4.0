...
static inline int int_div(int a, int b) {
  if (a > 0)
    return a / b;
  else
    return -((-a + b - 1) / b);
...
static inline int int_mod(int a, int b) {
...
static inline int check_pos_mul(int x, int y) {
  if (x != 0)
...
static inline int check_mul(int x, int y) {
  if (x >= 0) {
    if (y >= 0)
...
static inline bool omega_eqn_is_red(eqn e, int desired_res) {
  return (desired_res == omega_simplify && e->color == omega_red);
}

/* Return a string for VARIABLE.  */

static inline char *omega_var_to_str(int variable) {
...
static void omega_print_term(FILE *file, omega_pb pb, eqn e, int c) {
  int i;
  bool first = true;
  int n = pb->num_vars;
  int went_first = -1;
  ...
  for (i = 1; i <= n; i++)
    if (c * e->coef[i] > 0) {
      ...
      if (c * e->coef[i] == 1)
        ...
        fprintf(file, "%d * %s", c * e->coef[i], omega_variable_to_str(pb, i));
      break;
    }
...
void omega_print_eqn(FILE *file, omega_pb pb, eqn e, bool test, int extra) {
  int i;
  ...
  for (i = is_lt ? 1 : 0; i <= n; i++)
    if (e->coef[i] < 0) {
      ...
      if (i == 0)
        ...
        fprintf(file, "%d * %s", -e->coef[i], omega_variable_to_str(pb, i));
    }
...
static void omega_print_vars(FILE *file, omega_pb pb) {
  int i;
...
void omega_print_problem(FILE *file, omega_pb pb) {
  int e;
...
int omega_count_red_equations(omega_pb pb) {
  int e, i;
  int result = 0;
...
void omega_print_red_equations(FILE *file, omega_pb pb) {
  int e;
...
void omega_pretty_print_problem(FILE *file, omega_pb pb) {
  int e, v, v1, v2, v3, t;
  ...
  partial_order_type **po =
      XNEWVEC(partial_order_type *, OMEGA_MAX_VARS * OMEGA_MAX_VARS);
    ...
    while (1) {
      ...
      for (e = 0; e < pb->num_geqs; e++)
        if (live[e]) {
          ...
          if (pb->geqs[e].coef[0] > 0 || pb->geqs[e].coef[0] < -1 || v2 <= 0 ||
              v3 > 0 || pb->geqs[e].coef[v1] * pb->geqs[e].coef[v2] != -1) {
      ...
      for (t = 0; t < pb->num_vars; t++) {
        ...
        for (v1 = 1; v1 <= pb->num_vars; v1++)
          for (v2 = 1; v2 <= pb->num_vars; v2++)
            if (po[v1][v2] != none && chain_length[v1] <= chain_length[v2]) {
...
static void omega_name_wild_card(omega_pb pb, int i) {
  --next_wild_card;
...
static int omega_add_new_wild_card(omega_pb pb) {
  int e;
  int i = ++pb->safe_vars;
  pb->num_vars++;
...
static void omega_delete_geq(omega_pb pb, int e, int n_vars) {
...
static void omega_delete_geq_extra(omega_pb pb, int e, int n_vars) {
  if (dump_file && (dump_flags & TDF_DETAILS)) {
...
static void omega_delete_variable(omega_pb pb, int i) {
...
static inline int setup_packing(eqn eqn, int num_vars) {
  int k;
  int n = 0;
...
static inline void omega_substitute_red_1(eqn eq, eqn sub, int var, int c,
                                          bool *found_black, int top_var) {
  if (eq->coef[var] != 0) {
...
static void omega_substitute_red(omega_pb pb, eqn sub, int var, int c,
                                 bool *found_black) {
  ...
  *found_black = false;
  ...
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    ...
    fprintf(dump_file,
            "substituting using %s := ", omega_variable_to_str(pb, var));
...
static void omega_substitute(omega_pb pb, eqn sub, int var, int c) {
  int e, j, j0;
...
static void omega_do_mod(omega_pb pb, int factor, int e, int j) {
  int k, i;
...
void omega_negate_geq(omega_pb pb, int e) {
  int i;
...
static enum omega_result verify_omega_pb(omega_pb pb) {
...
static void adding_equality_constraint(omega_pb pb, int e) {
  if (original_problem != no_problem && original_problem != pb &&
    ...
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file, "adding equality constraint %d to outer problem\n",
              e2);
    omega_init_eqn_zero(&original_problem->eqs[e2], original_problem->num_vars);
...
static normalize_return_type normalize_omega_problem(omega_pb pb) {
  int e, i, j, k, n_vars;
      ...
      if (top_var == -1) {
        ...
        if (g > 1) {
          ...
          for (; i0 >= 0; i0--) {
            ...
            pb->geqs[e].coef[i] = pb->geqs[e].coef[i] / g;
            hashCode =
                hashCode * hash_key_multiplier * (i + 3) + pb->geqs[e].coef[i];
          }
        }
...

    {
      ...
      if (e > 0) {
        ...
        if (e2 < e && pb->geqs[e2].key == -eKey &&
          ...
          if (pb->geqs[e2].coef[0] == -cTerm &&
              (create_color || pb->geqs[e].color == omega_black)) {
          ...
          if (pb->geqs[e2].coef[0] > cTerm) {
            if (pb->geqs[e].color == omega_black) {
              if (dump_file && (dump_flags & TDF_DETAILS)) {
                ...
                omega_print_geq(dump_file, pb, &(pb->geqs[e2]));
                fprintf(dump_file, "\n");
                fprintf(dump_file, "[a]      Made Redundant by: ");
                omega_print_geq(dump_file, pb, &(pb->geqs[e]));
                fprintf(dump_file, "\n");
              }
...
static inline void divide_eqn_by_gcd(eqn eqn, int n_vars) {
  int var, g = 0;
...
static void cleanout_wildcards(omega_pb pb) {
  int e, i, j;
  int n_vars = pb->num_vars;
    ...
    for (i = n_vars; !omega_safe_var_p(pb, i); i--)
      if (pb->eqs[e].coef[i] != 0) {
        ...
        if (omega_safe_var_p(pb, j)) {
          ...
          if (dump_file && (dump_flags & TDF_DETAILS)) {
            fprintf(dump_file, "Found a single wild card equality: ");
          ...
          for (e2 = pb->num_geqs - 1; e2 >= 0; e2--)
            if (pb->geqs[e2].coef[i] && (pb->geqs[e2].color == omega_red ||
                                         (pb->eqs[e].color == omega_black &&
                                          pb->geqs[e2].color == omega_black))) {
              eqn eqn = &(pb->geqs[e2]);
              int var, k;
          ...
          for (e2 = pb->num_subs - 1; e2 >= 0; e2--)
            if (pb->subs[e2].coef[i] && (pb->subs[e2].color == omega_red ||
...
static inline void bswap(bool *i, bool *j) {
  bool tmp;
  tmp = *i;
  *i = *j;
  *j = tmp;
...
static inline void omega_unprotect_1(omega_pb pb, int *idx, bool *unprotect) {
  /* If IDX is protected...  */
  if (*idx < pb->safe_vars) {
...
static void resurrect_subs(omega_pb pb) {
  if (pb->num_subs > 0 && please_no_equalities_in_simplified_problems == 0) {
    int i, e, n, m;

    if (dump_file && (dump_flags & TDF_DETAILS)) {
      fprintf(dump_file, "problem reduced, bringing variables back to life\n");
      omega_print_problem(dump_file, pb);
    }
...
static inline bool implies(unsigned int a, unsigned int b) {
  return (a == (a & b));
}
...
enum omega_result omega_eliminate_redundant(omega_pb pb, bool expensive) {
          ...
          for (e3 = pb->num_geqs - 1; e3 >= 0; e3--)
            if (e3 != e1 && e3 != e2) {
              ...
              if (alpha3 > 0) {
                ...
                if (!implies(peqs[e3], pp) || !implies(neqs[e3], pn))
                  goto nextE3;
                ...
                if (c < alpha3 * (pb->geqs[e3].coef[0] + 1)) {
                  if (dump_file && (dump_flags & TDF_DETAILS)) {
                    fprintf(dump_file, "found redundant inequality\n");
                ...
                if (!implies(peqs[e3], pn) || !implies(neqs[e3], pp))
                  goto nextE3;
                ...
                if (c < alpha3 * (pb->geqs[e3].coef[0])) {
                  ...
                  if (dump_file && (dump_flags & TDF_DETAILS)) {
                    fprintf(dump_file, "found implied over tight inequality\n");
                    fprintf(dump_file, "alpha1, alpha2, alpha3 = %d,%d,%d\n",
                            alpha1, alpha2, -alpha3);
                    omega_print_geq(dump_file, pb, &(pb->geqs[e1]));
                    fprintf(dump_file, "\n");
                  ...
                  if (dump_file && (dump_flags & TDF_DETAILS)) {
                    fprintf(dump_file, "found implied tight inequality\n");
                    fprintf(dump_file, "alpha1, alpha2, alpha3 = %d,%d,%d\n",
                            alpha1, alpha2, -alpha3);
                    omega_print_geq(dump_file, pb, &(pb->geqs[e1]));
                    fprintf(dump_file, "\n");
                  ...
                  omega_copy_eqn(&pb->eqs[pb->num_eqs++], &pb->geqs[e3],
                                 pb->num_vars);
                  gcc_assert(pb->num_eqs <= OMEGA_MAX_EQS);
  ...
  for (e = pb->num_geqs - 1; e >= 0; e--) {
    if (dump_file && (dump_flags & TDF_DETAILS)) {
      fprintf(dump_file, "checking equation %d to see if it is redundant: ", e);
      omega_print_geq(dump_file, pb, &(pb->geqs[e]));
      fprintf(dump_file, "\n");
    }
...
static int smooth_weird_equations(omega_pb pb) {
      ...
      if (g > 20) {
        ...
        for (e2 = pb->num_geqs - 1; e2 >= 0; e2--)
          if (e1 != e2 && pb->geqs[e2].color == omega_black) {
            for (p = pb->num_vars; p > 1; p--) {
              for (q = p - 1; q > 0; q--) {
                alpha = (pb->geqs[e1].coef[p] * pb->geqs[e2].coef[q] -
          ...
          foundPQ:
            ...
            if (alpha3 > 0) {
              ...
              c = alpha1 * pb->geqs[e1].coef[0] + alpha2 * pb->geqs[e2].coef[0];
        ...
        if (pb->geqs[e3].coef[0] < 9997) {
          ...
          if (dump_file && (dump_flags & TDF_DETAILS)) {
            fprintf(dump_file, "Smoothing weird equations; adding:\n");
            omega_print_geq(dump_file, pb, &pb->geqs[e3]);
            fprintf(dump_file, "\nto:\n");
            omega_print_problem(dump_file, pb);
            fprintf(dump_file, "\n\n");
...
static void coalesce(omega_pb pb) {
  int e, e2;
  int colors = 0;
  bool *is_dead;
  ...
  for (e = 0; e < pb->num_geqs; e++)
    if (pb->geqs[e].color == omega_red && !pb->geqs[e].touched)
...
void omega_eliminate_red(omega_pb pb, bool eliminate_all) {
  ...
  for (e = pb->num_geqs - 1; e >= 0; e--)
    if (pb->geqs[e].color == omega_black && !is_dead[e])
      for (e2 = e - 1; e2 >= 0; e2--)
        if (pb->geqs[e2].color == omega_black && !is_dead[e2]) {
          a = 0;
        ...
        found_pair:
          if (dump_file && (dump_flags & TDF_DETAILS)) {
            fprintf(dump_file, "found two equations to combine, i = %s, ",
                ...
                if (k < 0 ||
                  ...
                  if (dump_file && (dump_flags & TDF_DETAILS)) {
                    ...
                    fprintf(dump_file,
                            ...
                            e3, dead_count, pb->num_geqs - dead_count);
                    omega_print_geq(dump_file, pb, &(pb->geqs[e]));
                    fprintf(dump_file, "\n");
                    omega_print_geq(dump_file, pb, &(pb->geqs[e2]));
                    fprintf(dump_file, "\n");
  ...
  if (!omega_verify_simplification && verify_omega_pb(pb) == omega_false)
    return;
...
static void chain_unprotect(omega_pb pb) {
  int i, e;
...
static void omega_problem_reduced(omega_pb pb) {
  if (omega_verify_simplification && !in_approximate_mode &&
...
static void omega_free_eliminations(omega_pb pb, int fv) {
  bool try_again = true;
  int i, e, e2;
  int n_vars = pb->num_vars;
...
static void free_red_eliminations(omega_pb pb) {
  bool try_again = true;
  int i, e, e2;
  int n_vars = pb->num_vars;
...
void omega_convert_eq_to_geqs(omega_pb pb, int eq) {
  int i;
...
static void omega_do_elimination(omega_pb pb, int e, int i) {
...
static inline enum omega_result omega_problem_has_no_solution(void) {
  if (dump_file && (dump_flags & TDF_DETAILS))
    fprintf(dump_file, "\nequations have no solution \n");
...
static enum omega_result omega_solve_eq(omega_pb pb,
  ...
  for (e = pb->num_eqs - 1; e >= 0; e--) {
    ...
    if (g == 1) {
      ...
      bool promotion_possible =
          (omega_safe_var_p(pb, j) && pb->safe_vars + 1 == i &&
           !omega_eqn_is_red(eqn, desired_res) && !in_approximate_mode);
    ...
    normalizeEQ:
      ...
      if (g2 > 1 && !in_approximate_mode) {
        ...
        if (dump_file && (dump_flags & TDF_DETAILS)) {
          fprintf(dump_file, "adding equation to handle safe variable \n");
          omega_print_eq(dump_file, pb, eqn);
          fprintf(dump_file, "\n----\n");
          omega_print_problem(dump_file, pb);
          fprintf(dump_file, "\n----\n");
...
static enum omega_result parallel_splinter(omega_pb pb, int e, int diff,
                                           enum omega_result desired_res) {
  omega_pb tmp_problem;
  int i;
...
static enum omega_result omega_solve_geq(omega_pb pb,
                                         enum omega_result desired_res) {
  int i, e;
  int n_vars, fv;
  enum omega_result result;
  ...
  do {
    ...
    if (n_vars == 1) {
      ...
      if (desired_res == omega_simplify) {
        pb->num_geqs = 0;
        if (pb->safe_vars == 1) {
          if (lower_bound == upper_bound && u_color == omega_black &&
              l_color == omega_black) {
      ...
      if (original_problem != no_problem && l_color == omega_black &&
          u_color == omega_black && !conservative &&
    ...
    do {
      ...
      for (i = n_vars; i != fv; i--) {
        ...
        if (ub >= 0 && lb >= 0 && pb->geqs[lb].key == -pb->geqs[ub].key) {
          ...
          int diff = Lc * pb->geqs[ub].coef[0] + Uc * pb->geqs[lb].coef[0];
          lucky = (diff >= (Uc - 1) * (Lc - 1));
        }

        if (maxC == 1 || minC == -1 || lucky || in_approximate_mode) {
          ...
          if (dump_file && (dump_flags & TDF_DETAILS))
            fprintf(dump_file,
                    ...
                    omega_variable_to_str(pb, i), upper_bound_count,
                    lower_bound_count, minC, maxC, lucky, in_approximate_mode);
      ...
      for (e = pb->num_geqs - 1; e >= 0; e--)
        if (pb->geqs[e].coef[i] > 0) {
          if (pb->geqs[e].coef[i] == -minC)
            ...
            max_splinters +=
                check_pos_mul((pb->geqs[e].coef[i] - 1), (-minC - 1)) /
                    (-minC) +
                1;
        }
      ...
      if (!exact) {
            ...
            for (e2 = e1 - 1; e2 >= 0; e2--)
              if (pb->geqs[e2].color == omega_black &&
                  ...
                  ((pb->geqs[e1].coef[0] + pb->geqs[e2].coef[0]) *
                       (3 - single_var_geq(&pb->geqs[e1], pb->num_vars)) / 2 <
                   parallel_difference)) {
                parallel_difference =
                    (pb->geqs[e1].coef[0] + pb->geqs[e2].coef[0]) *
                    (3 - single_var_geq(&pb->geqs[e1], pb->num_vars)) / 2;
        ...
        if (dump_file && (dump_flags & TDF_DETAILS) && best_parallel_eqn >= 0) {
          fprintf(dump_file, "Possible parallel projection, diff = %d, in ",
                  parallel_difference);
      ...
      if (dump_file && (dump_flags & TDF_DETAILS)) {
        fprintf(dump_file, "going to eliminate %s, (%d,%d,%d)\n",
                omega_variable_to_str(pb, i), i, minC, lower_bound_count);
        omega_print_problem(dump_file, pb);
      ...
      if (exact) {
        if (n_vars == 1) {
            ...
            if ((Lc = pb->geqs[Le].coef[i]) == 0) {
                ...
                if (dump_file && (dump_flags & TDF_DETAILS)) {
                  if (pb->geqs[Le].color == omega_black)
                    fprintf(dump_file, " :::=> %s >= %d\n",
                            omega_variable_to_str(pb, 1), constantTerm);
                  else
                    fprintf(dump_file, " :::=> [%s >= %d]\n",
                            omega_variable_to_str(pb, 1), constantTerm);
                }
              ...
              for (Ue = topeqn; Ue >= 0; Ue--)
                if (pb->geqs[Ue].coef[i] < 0 &&
                  ...
                  int coefficient =
                      pb->geqs[Ue].coef[1] * Lc + pb->geqs[Le].coef[1] * Uc;
                  int constantTerm =
                      pb->geqs[Ue].coef[0] * Lc + pb->geqs[Le].coef[0] * Uc;
          ...
          if (dump_file && (dump_flags & TDF_DETAILS))
            fprintf(
                dump_file, " therefore, %c%d <= %c%s%c <= %d%c\n",
          ...
          if (pb->safe_vars == 1) {
            if (upper_bound == lower_bound &&
              ...
              if (ub_color == omega_red || lb_color == omega_red)
          ...
          if (desired_res == omega_simplify) {
            ...
            if (!conservative &&
                ...
                original_problem != no_problem && lower_bound == upper_bound) {
              ...
              if (dump_file && (dump_flags & TDF_DETAILS)) {
                fprintf(dump_file, "adding equality %d to outer problem\n", e);
                omega_print_problem(dump_file, original_problem);
              }
            }
            return omega_true;
          ...
          for (Ue = pb->num_geqs - 1; Ue >= 0; Ue--)
            if (pb->geqs[Ue].coef[i] < 0) {
              if (lbeqn->key == -pb->geqs[Ue].key)
                ...
                for (k = 0; k <= n_vars; k++)
                  pb->geqs[Ue].coef[k] = check_mul(pb->geqs[Ue].coef[k], Lc) +
              ...
              for (Ue = top_eqn; Ue >= 0; Ue--)
                if (pb->geqs[Ue].coef[i] < 0) {
                  if (pb->geqs[Le].key != -pb->geqs[Ue].key) {
                    ...
                    if (dump_file && (dump_flags & TDF_DETAILS)) {
                      fprintf(dump_file, "Le = %d, Ue = %d, gen = %d\n", Le, Ue,
                              e2);
                      omega_print_geq_extra(dump_file, pb, &(pb->geqs[Le]));
                      fprintf(dump_file, "\n");
                      omega_print_geq_extra(dump_file, pb, &(pb->geqs[Ue]));
        ...
        for (e = 0; e < pb->num_geqs; e++)
          if (pb->geqs[e].coef[i] == 0) {
            ...
            if (dump_file && (dump_flags & TDF_DETAILS)) {
              int t;
              fprintf(dump_file, "Copying (%d, %d): ", i, pb->geqs[e].coef[i]);
              omega_print_geq_extra(dump_file, pb, &pb->geqs[e]);
              fprintf(dump_file, "\n");
              ...
              if (pb->geqs[Ue].coef[i] < 0) {
                ...
                if (pb->geqs[Le].key != -pb->geqs[Ue].key) {
                  ...
                  gcc_assert(e2 < OMEGA_MAX_GEQS);
                               ...
                               pb->geqs[Le].coef[0] * Uc - (Uc - 1) * (Lc - 1) <
                           0)
        ...
        if (desired_res != omega_false) {
          ...
          if (dump_file && (dump_flags & TDF_DETAILS)) {
            ...
            for (j = 0; j < lower_bounds; j++)
              fprintf(dump_file, " %d", pb->geqs[lower_bound[j]].coef[i]);
          ...
          for (j = 0; j < lower_bounds; j++) {
            ...
            e = lower_bound[j];
            max_incr =
                (((pb->geqs[e].coef[i] - 1) * (worst_lower_bound_constant - 1) -
                  1) /
            ...
            if (max_incr > 50 && !smoothed && smooth_weird_equations(pb)) {
              conservative--;
              free(rS);
              free(iS);
              smoothed = true;
            ...
            for (c = max_incr; c >= 0; c--) {
              if (dump_file && (dump_flags & TDF_DETAILS)) {
                fprintf(dump_file, "trying next decrement of %d\n",
                        max_incr - c);
                omega_print_problem(dump_file, pb);
              }
...
enum omega_result omega_solve_problem(omega_pb pb,
                                      enum omega_result desired_res) {
  enum omega_result result;
  ...
  if (!omega_reduce_with_subs) {
    resurrect_subs(pb);
    gcc_assert(please_no_equalities_in_simplified_problems || !result ||
               pb->num_subs == 0);
  }
...
bool omega_problem_has_red_equations(omega_pb pb) {
  bool result;
  int e;
  int i;
...
enum omega_result omega_simplify_approximate(omega_pb pb) {
  enum omega_result result;
...
enum omega_result omega_simplify_problem(omega_pb pb) {
  int i;
  ...
  if (!may_be_red && pb->num_subs == 0 && pb->safe_vars == 0) {
    ...
    if (omega_found_reduction != omega_false && !return_single_result) {
      pb->num_geqs = 0;
      pb->num_eqs = 0;
      (*omega_when_reduced)(pb);
    }
  ...
  if (!omega_reduce_with_subs)
    gcc_assert(please_no_equalities_in_simplified_problems ||
               omega_found_reduction == omega_false || pb->num_subs == 0);
...
void omega_unprotect_variable(omega_pb pb, int var) {
  int e, idx;
  ...
  if (idx < 0) {
    ...
    if (idx < pb->num_subs) {
      omega_copy_eqn(&pb->subs[idx], &pb->subs[pb->num_subs], pb->num_vars);
    ...
    for (e2 = pb->num_subs - 1; e2 >= 0; e2--)
      if (bring_to_life[e2]) {
        ...
        if (pb->safe_vars < pb->num_vars) {
          ...
          pb->forwarding_address[pb->var[pb->num_vars]] = pb->num_vars;
        } else {
          for (e = pb->num_geqs - 1; e >= 0; e--)
            pb->geqs[e].coef[pb->safe_vars] = 0;
...
void omega_constrain_variable_value(omega_pb pb, enum omega_eqn_color color,
                                    int var, int value) {
  int e;
  int k = pb->forwarding_address[var];
...
bool omega_query_variable(omega_pb pb, int i, int *lower_bound,
                          int *upper_bound) {
  int n_vars = pb->num_vars;
  int e, j;
  bool is_simple;
...
static void query_coupled_variable(omega_pb pb, int i, int *l, int *u,
                                   bool *could_be_zero, int lower_bound,
                                   int upper_bound) {
  int e, b1, b2;
  eqn eqn;
  int sign;
  ...
  *could_be_zero =
      (*l <= 0 && 0 <= *u && int_mod(eqn->coef[0], abs(eqn->coef[v])) == 0);
}

/* Return false when a lower bound L and an upper bound U for variable
   I in problem PB have been initialized.  */

bool omega_query_variable_bounds(omega_pb pb, int i, int *l, int *u) {
  *l = neg_infinity;
  *u = pos_infinity;
...
int omega_query_variable_signs(omega_pb pb, int i, int dd_lt, int dd_eq,
                               int dd_gt, int lower_bound, int upper_bound,
                               bool *dist_known, int *dist) {
  int result;
  int l, u;
  bool could_be_zero;
...
omega_pb omega_alloc_problem(int nvars, int nprot) {
  omega_pb pb;
