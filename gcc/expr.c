...
static void store_constructor_field(rtx, unsigned HOST_WIDE_INT, HOST_WIDE_INT,
                                    enum machine_mode, tree, tree, int,
                                    alias_set_type);
...
void convert_move(rtx to, rtx from, int unsignedp) {
  ...
  enum rtx_code equiv_code =
      (unsignedp < 0 ? UNKNOWN : (unsignedp ? ZERO_EXTEND : SIGN_EXTEND));
  ...
  if (to_real) {
    ...
    gcc_assert(
        (GET_MODE_PRECISION(from_mode) != GET_MODE_PRECISION(to_mode)) ||
        (DECIMAL_FLOAT_MODE_P(from_mode) != DECIMAL_FLOAT_MODE_P(to_mode)));
    ...
    start_sequence();
    value = emit_library_call_value(libcall, NULL_RTX, LCT_CONST, to_mode, 1,
                                    from, from_mode);
    insns = get_insns();
    end_sequence();
    emit_libcall_block(insns, to, value,
                       tab == trunc_optab
                           ? gen_rtx_FLOAT_TRUNCATE(to_mode, from)
  ...
  if (GET_MODE_CLASS(to_mode) == MODE_PARTIAL_INT) {
    ...
    gcc_assert(
        convert_optab_handler(trunc_optab, to_mode, full_mode)->insn_code !=
        CODE_FOR_nothing);

    if (full_mode != from_mode)
      from = convert_to_mode(full_mode, from, unsignedp);
    emit_unop_insn(
        convert_optab_handler(trunc_optab, to_mode, full_mode)->insn_code, to,
        from, UNKNOWN);
    return;
  }
  if (GET_MODE_CLASS(from_mode) == MODE_PARTIAL_INT) {
    ...
    gcc_assert(
        convert_optab_handler(sext_optab, full_mode, from_mode)->insn_code !=
        CODE_FOR_nothing);
  ...
  if (GET_MODE_BITSIZE(from_mode) < GET_MODE_BITSIZE(to_mode) &&
    ...
    if (unsignedp)
      ...
      if (HAVE_slt &&
          ...
          STORE_FLAG_VALUE == -1) {
        emit_cmp_insn(lowfrom, const0_rtx, NE, NULL_RTX, lowpart_mode, 0);
  ...
  if (GET_MODE_BITSIZE(from_mode) > BITS_PER_WORD &&
      GET_MODE_BITSIZE(to_mode) <= BITS_PER_WORD) {
    if (!((MEM_P(from) && !MEM_VOLATILE_P(from) && direct_load[(int)to_mode] &&
  ...
  if (GET_MODE_BITSIZE(to_mode) > GET_MODE_BITSIZE(from_mode)) {
    ...
    if ((code = can_extend_p(to_mode, from_mode, unsignedp)) !=
      ...
      for (intermediate = from_mode; intermediate != VOIDmode;
        ...
        if (((can_extend_p(to_mode, intermediate, unsignedp) !=
            ...
            (can_extend_p(intermediate, from_mode, unsignedp) !=
             CODE_FOR_nothing)) {
          convert_move(to, convert_to_mode(intermediate, from, unsignedp),
                       unsignedp);
          return;
        }
      ...
      shift_amount = build_int_cst(
          NULL_TREE, GET_MODE_BITSIZE(to_mode) - GET_MODE_BITSIZE(from_mode));
      from = gen_lowpart(to_mode, force_reg(from_mode, from));
      tmp =
          expand_shift(LSHIFT_EXPR, to_mode, from, shift_amount, to, unsignedp);
      tmp =
          expand_shift(RSHIFT_EXPR, to_mode, tmp, shift_amount, to, unsignedp);
      if (tmp != to)
  ...
  if (convert_optab_handler(trunc_optab, to_mode, from_mode)->insn_code !=
      CODE_FOR_nothing) {
    emit_unop_insn(
        convert_optab_handler(trunc_optab, to_mode, from_mode)->insn_code, to,
        from, UNKNOWN);
    return;
  }
...
rtx convert_modes(enum machine_mode mode, enum machine_mode oldmode, rtx x,
                  int unsignedp) {
  rtx temp;
       ...
       (GET_CODE(x) == CONST_DOUBLE ||
        (GET_MODE_SIZE(mode) <= GET_MODE_SIZE(oldmode) &&
         ((MEM_P(x) && !MEM_VOLATILE_P(x) && direct_load[(int)mode]) ||
          (REG_P(x) &&
           (!HARD_REGISTER_P(x) || HARD_REGNO_MODE_OK(REGNO(x), mode)) &&
...
rtx move_by_pieces(rtx to, rtx from, unsigned HOST_WIDE_INT len,
  ...
  if (align >= GET_MODE_ALIGNMENT(tmode))
    ...
    for (tmode = GET_CLASS_NARROWEST_MODE(MODE_INT), xmode = tmode;
         tmode != VOIDmode; xmode = tmode, tmode = GET_MODE_WIDER_MODE(tmode))
    ...
    if (data.autinc_to) {
      if (endp == 2) {
        if (HAVE_POST_INCREMENT && data.explicit_inc_to > 0)
          ...
          data.to_addr = copy_addr_to_reg(plus_constant(data.to_addr, -1));
      }
      to1 =
          adjust_automodify_address(data.to, QImode, data.to_addr, data.offset);
    } else {
      if (endp == 2)
        --data.offset;
...
static void move_by_pieces_1(rtx (*genfun)(rtx, ...), enum machine_mode mode,
                             struct move_by_pieces *data) {
  ...
  while (data->len >= size) {
    ...
    if (HAVE_PRE_DECREMENT && data->explicit_inc_to < 0)
      emit_insn(gen_add2_insn(data->to_addr, GEN_INT(-(HOST_WIDE_INT)size)));
    if (HAVE_PRE_DECREMENT && data->explicit_inc_from < 0)
      emit_insn(gen_add2_insn(data->from_addr, GEN_INT(-(HOST_WIDE_INT)size)));
...
rtx emit_block_move_hints(rtx x, rtx y, rtx size, enum block_op_methods method,
  ...
  else if (emit_block_move_via_movmem(x, y, size, align, expected_align,
                                      expected_size))
    ;
  else if (may_use_call)
    retval =
        emit_block_move_via_libcall(x, y, size, method == BLOCK_OP_TAILCALL);
  else
...
static bool emit_block_move_via_movmem(rtx x, rtx y, rtx size,
                                       unsigned int align,
                                       unsigned int expected_align,
                                       HOST_WIDE_INT expected_size) {
  ...
  for (mode = GET_CLASS_NARROWEST_MODE(MODE_INT); mode != VOIDmode;
    ...
    if (code != CODE_FOR_nothing
      ...
      if (insn_data[(int)code].n_operands == 4)
        ...
        pat = GEN_FCN((int)code)(x, y, op2, opalign,
                                 GEN_INT(expected_align / BITS_PER_UNIT),
...
rtx emit_block_move_via_libcall(rtx dst, rtx src, rtx size, bool tailcall) {
...
void init_block_move_fn(const char *asmspec) {
  if (!block_move_fn) {
    ...
    args = build_function_type_list(ptr_type_node, ptr_type_node,
                                    const_ptr_type_node, sizetype, NULL_TREE);
...
static void emit_block_move_via_loop(rtx x, rtx y, rtx size,
                                     unsigned int align ATTRIBUTE_UNUSED) {
  ...
  tmp = expand_simple_binop(iter_mode, PLUS, iter, const1_rtx, iter, true,
                            OPTAB_LIB_WIDEN);
  if (tmp != iter)
...
void move_block_to_reg(int regno, rtx x, int nregs, enum machine_mode mode) {
  ...
  if (HAVE_load_multiple) {
    last = get_last_insn();
    pat = gen_load_multiple(gen_rtx_REG(word_mode, regno), x, GEN_INT(nregs));
    if (pat) {
      emit_insn(pat);
      return;
    } else
...
void move_block_from_reg(int regno, rtx x, int nregs) {
  ...
  if (HAVE_store_multiple) {
    rtx last = get_last_insn();
    rtx pat =
        gen_store_multiple(x, gen_rtx_REG(word_mode, regno), GEN_INT(nregs));
    if (pat) {
      emit_insn(pat);
      return;
    } else
...
static void emit_group_load_1(rtx *tmps, rtx dst, rtx orig_src, tree type,
                              int ssize) {
  rtx src;
  int start, i;
  ...
  if (m != VOIDmode && !SCALAR_INT_MODE_P(m) && !MEM_P(orig_src) &&
  ...
  for (i = start; i < XVECLEN(dst, 0); i++) {
    ...
    if (!MEM_P(orig_src) &&
        (!CONSTANT_P(orig_src) ||
         (GET_MODE(orig_src) != mode && GET_MODE(orig_src) != VOIDmode))) {
    ...
    if (MEM_P(src) &&
    ...
    } else if (COMPLEX_MODE_P(mode) && GET_MODE(src) == mode &&
               bytelen == GET_MODE_SIZE(mode))
      ...
      if ((bytepos == 0 && bytelen == slen0) ||
        ...
        tmps[i] = extract_bit_field(mem, bytelen * BITS_PER_UNIT, 0, 1,
                                    NULL_RTX, mode, mode);
      }
    }
    /* FIXME: A SIMD parallel will eventually lead to a subreg of a
    ...
    else if (VECTOR_MODE_P(GET_MODE(dst)) && REG_P(src)) {
      ...
      tmps[i] =
          extract_bit_field(src, bytelen * BITS_PER_UNIT,
                            bytepos * BITS_PER_UNIT, 1, NULL_RTX, mode, mode);
...
void emit_group_move(rtx dst, rtx src) {
  ...
  gcc_assert(GET_CODE(src) == PARALLEL && GET_CODE(dst) == PARALLEL &&
             XVECLEN(src, 0) == XVECLEN(dst, 0));

  /* Skip first entry if NULL.  */
  for (i = XEXP(XVECEXP(src, 0, 0), 0) ? 0 : 1; i < XVECLEN(src, 0); i++)
    emit_move_insn(XEXP(XVECEXP(dst, 0, i), 0), XEXP(XVECEXP(src, 0, i), 0));
}
...
void emit_group_store(rtx orig_dst, rtx src, tree type ATTRIBUTE_UNUSED,
                      int ssize) {
  rtx *tmps, dst;
  int start, finish, i;
  ...
  for (i = start; i < finish; i++) {
    ...
    if (GET_CODE(dst) == CONCAT) {
      if (bytepos + adj_bytelen <= GET_MODE_SIZE(GET_MODE(XEXP(dst, 0))))
        dest = XEXP(dst, 0);
        ...
        if (GET_MODE_ALIGNMENT(dest_mode) >= GET_MODE_ALIGNMENT(tmp_mode)) {
          dest = assign_stack_temp(dest_mode, GET_MODE_SIZE(dest_mode), 0);
          emit_move_insn(adjust_address(dest, tmp_mode, bytepos), tmps[i]);
          dst = dest;
        } else {
          dest = assign_stack_temp(tmp_mode, GET_MODE_SIZE(tmp_mode), 0);
...
rtx copy_blkmode_from_reg(rtx tgtblk, rtx srcreg, tree type) {
  ...
  if (tgtblk == 0) {
    tgtblk = assign_temp(
        build_qualified_type(type, (TYPE_QUALS(type) | TYPE_QUAL_CONST)), 0, 1,
        1);
  ...
  if (bytes % UNITS_PER_WORD != 0 &&
      (targetm.calls.return_in_msb(type) ? !BYTES_BIG_ENDIAN
                                         : BYTES_BIG_ENDIAN))
    padding_correction =
  ...
  for (bitpos = 0, xbitpos = padding_correction; bitpos < bytes * BITS_PER_UNIT;
    ...
    store_bit_field(dst, bitsize, bitpos % BITS_PER_WORD, copy_mode,
                    extract_bit_field(src, bitsize, xbitpos % BITS_PER_WORD, 1,
...
int can_store_by_pieces(unsigned HOST_WIDE_INT len,
  ...
  if (!(memsetp ? SET_BY_PIECES_P(len, align) : STORE_BY_PIECES_P(len, align)))
    return 0;
  ...
  for (reverse = 0; reverse <= (HAVE_PRE_DECREMENT || HAVE_POST_DECREMENT);
       reverse++) {
    l = len;
    mode = VOIDmode;
...
rtx store_by_pieces(rtx to, unsigned HOST_WIDE_INT len,
                    rtx (*constfun)(void *, HOST_WIDE_INT, enum machine_mode),
                    void *constfundata, unsigned int align, bool memsetp,
                    int endp) {
  ...
  gcc_assert(memsetp ? SET_BY_PIECES_P(len, align)
...
static void clear_by_pieces(rtx to, unsigned HOST_WIDE_INT len,
                            unsigned int align) {
  struct store_by_pieces data;
...
static void store_by_pieces_2(rtx (*genfun)(rtx, ...), enum machine_mode mode,
                              struct store_by_pieces *data) {
  unsigned int size = GET_MODE_SIZE(mode);
  rtx to1, cst;
...
rtx clear_storage_hints(rtx object, rtx size, enum block_op_methods method,
                        unsigned int expected_align,
                        HOST_WIDE_INT expected_size) {
  ...
  if (mode != BLKmode && GET_CODE(size) == CONST_INT &&
  ...
  if (GET_CODE(size) == CONST_INT && CLEAR_BY_PIECES_P(INTVAL(size), align))
...
rtx set_storage_via_libcall(rtx object, rtx size, rtx val, bool tailcall) {
  ...
  call_expr = build_call_expr(fn, 3, object_tree, integer_zero_node, size_tree);
  CALL_EXPR_TAILCALL(call_expr) = tailcall;
...
void init_block_clear_fn(const char *asmspec) {
  if (!block_clear_fn) {
    ...
    args = build_function_type_list(ptr_type_node, ptr_type_node,
                                    integer_type_node, sizetype, NULL_TREE);
...
static void write_complex_part(rtx cplx, rtx val, bool imag_p) {
  ...
  if (MEM_P(cplx)) {
    emit_move_insn(
        adjust_address_nv(cplx, imode, imag_p ? GET_MODE_SIZE(imode) : 0), val);
    return;
  }
  ...
  if (ibitsize >= BITS_PER_WORD
      ...
      || (REG_P(cplx) && REGNO(cplx) < FIRST_PSEUDO_REGISTER &&
...
static rtx read_complex_part(rtx cplx, bool imag_p) {
  ...
  /* For MEMs simplify_gen_subreg may generate an invalid new address
     ...
     invalid address, we call adjust_address_nv directly.  */
  if (MEM_P(cplx))
    return adjust_address_nv(cplx, imode, imag_p ? GET_MODE_SIZE(imode) : 0);
  ...
  return extract_bit_field(cplx, ibitsize, imag_p ? ibitsize : 0, true,
                           NULL_RTX, imode, imode);
}
...
static rtx emit_move_resolve_push(enum machine_mode mode, rtx x) {
  ...
  /* Do not use anti_adjust_stack, since we don't want to update
     stack_pointer_delta.  */
  temp = expand_simple_binop(Pmode, PLUS, stack_pointer_rtx, GEN_INT(adjust),
...
rtx emit_move_complex_push(enum machine_mode mode, rtx x, rtx y) {
...
rtx emit_move_complex_parts(rtx x, rtx y) {
  /* Show the output dies here.  This is necessary for SUBREGs
     of pseudos since we cannot track their lifetimes correctly;
     hard regs shouldn't appear here except as return values.  */
  if (!reload_completed && !reload_in_progress && REG_P(x) &&
      !reg_overlap_mentioned_p(x, y))
    emit_clobber(x);
...
static rtx emit_move_complex(enum machine_mode mode, rtx x, rtx y) {
  ...
  if (GET_MODE_CLASS(mode) == MODE_COMPLEX_FLOAT &&
      optab_handler(mov_optab, GET_MODE_INNER(mode))->insn_code !=
          CODE_FOR_nothing)
    try_int = false;
  ...
  /* If one of the operands is a memory, and alignment constraints
     ...
     usually better with the by-parts thing below.  */
           ...
           (!STRICT_ALIGNMENT || get_mode_alignment(mode) == BIGGEST_ALIGNMENT))
    try_int = true;
  else
    try_int = false;
  ...
  if (try_int) {
    ...
    if (MEM_P(x) && MEM_P(y)) {
      emit_block_move(x, y, GEN_INT(GET_MODE_SIZE(mode)), BLOCK_OP_NO_LIBCALL);
      return get_last_insn();
    }
...
static rtx emit_move_multi_word(enum machine_mode mode, rtx x, rtx y) {
  ...
  need_clobber = false;
  for (i = 0; i < (GET_MODE_SIZE(mode) + (UNITS_PER_WORD - 1)) / UNITS_PER_WORD;
       i++) {
...
rtx emit_move_insn(rtx x, rtx y) {
  ...
  if (CONSTANT_P(y)) {
    if (optimize && SCALAR_FLOAT_MODE_P(GET_MODE(x)) &&
  ...
  /* If X or Y are memory references, verify that their addresses are valid
     for the machine.  */
  if (MEM_P(x) && (!memory_address_p(GET_MODE(x), XEXP(x, 0)) &&
                   !push_operand(x, GET_MODE(x))))
    x = validize_mem(x);

  if (MEM_P(y) && !memory_address_p(GET_MODE(y), XEXP(y, 0)))
    y = validize_mem(y);
  ...
  if (y_cst && REG_P(x) && (set = single_set(last_insn)) != NULL_RTX &&
...
rtx push_block(rtx size, int extra, int below) {
  ...
  if (CONSTANT_P(size))
    ...
    if (extra != 0)
      temp = expand_binop(Pmode, add_optab, temp, GEN_INT(extra), temp, 0,
                          OPTAB_LIB_WIDEN);
...
static void emit_single_push_insn(enum machine_mode mode, rtx x, tree type) {
  rtx dest_addr;
  ...
  /* If we are to pad downward, adjust the stack pointer first and
    ...
    emit_move_insn(stack_pointer_rtx,
                   expand_binop(Pmode,
                                ...
                                sub_optab,
                                  ...
                                  add_optab,
#endif
                                stack_pointer_rtx, GEN_INT(rounded_size),
    ...
    dest_addr = gen_rtx_PLUS(Pmode, stack_pointer_rtx, GEN_INT(rounded_size));
#endif
...
void emit_push_insn(rtx x, enum machine_mode mode, tree type, rtx size,
  ...
  if (mode == BLKmode ||
    ...
    if (args_addr == 0 && PUSH_ARGS && GET_CODE(size) == CONST_INT &&
        skip == 0 && MEM_ALIGN(xinner) >= align &&
        ...
        ((!SLOW_UNALIGNED_ACCESS(word_mode, align)) ||
         ...
         (PUSH_ROUNDING(align / BITS_PER_UNIT) == (align / BITS_PER_UNIT))) &&
        PUSH_ROUNDING(INTVAL(size)) == INTVAL(size)) {
      /* Push padding now if padding above and stack grows down,
         or if padding below and stack grows up.
         But if space already allocated, this has already been done.  */
      if (extra && args_addr == 0 && where_pad != none &&
...
#endif /* PUSH_ROUNDING  */
    {
      ...
      if (partial != 0) {
        if (GET_CODE(size) == CONST_INT)
          ...
          size = expand_binop(GET_MODE(size), sub_optab, size, GEN_INT(used),
                              NULL_RTX, 0, OPTAB_LIB_WIDEN);
      }
      ...
      if (!args_addr) {
        ...
        temp = memory_address(
            BLKmode, plus_constant(args_addr, skip + INTVAL(args_so_far)));
      else
        temp = memory_address(
            BLKmode,
            plus_constant(gen_rtx_PLUS(Pmode, args_addr, args_so_far), skip));
    ...
    for (i = size - 1; i >= not_stack; i--)
      ...
      if (i >= not_stack + offset)
        emit_push_insn(
            operand_subword_force(x, i, mode), word_mode, NULL_TREE, NULL_RTX,
...
#endif
    {
      if (GET_CODE(args_so_far) == CONST_INT)
        addr =
            memory_address(mode, plus_constant(args_addr, INTVAL(args_so_far)));
      else
        addr =
            memory_address(mode, gen_rtx_PLUS(Pmode, args_addr, args_so_far));
...
static bool optimize_bitfield_assignment_op(unsigned HOST_WIDE_INT bitsize,
  ...
  if (mode1 != VOIDmode || bitsize >= BITS_PER_WORD ||
      str_bitsize > BITS_PER_WORD || TREE_SIDE_EFFECTS(to) ||
      TREE_THIS_VOLATILE(to))
    return false;

  STRIP_NOPS(src);
  if (!BINARY_CLASS_P(src) || TREE_CODE(TREE_TYPE(src)) != INTEGER_TYPE)
    return false;
  ...
  if (MEM_P(str_rtx)) {
    ...
    if (str_bitsize == 0 || str_bitsize > BITS_PER_WORD)
      str_mode = word_mode;
    str_mode = get_best_mode(bitsize, bitpos, MEM_ALIGN(str_rtx), str_mode, 0);
  ...
  switch (TREE_CODE(src)) {
    ...
    case MINUS_EXPR:
      ...
      value = convert_modes(str_mode, TYPE_MODE(TREE_TYPE(op1)), value,
      ...
      result = expand_binop(str_mode, binop, str_rtx, value, str_rtx, 1,
                            OPTAB_WIDEN);
      if (result != str_rtx)
    ...
    case BIT_XOR_EXPR:
      ...
      value = convert_modes(GET_MODE(str_rtx), TYPE_MODE(TREE_TYPE(op1)), value,
                            TYPE_UNSIGNED(TREE_TYPE(op1)));
      ...
      if (bitpos + bitsize != GET_MODE_BITSIZE(GET_MODE(str_rtx))) {
        ...
        value = expand_and(GET_MODE(str_rtx), value, mask, NULL_RTX);
      }
      value = expand_shift(LSHIFT_EXPR, GET_MODE(str_rtx), value,
                           build_int_cst(NULL_TREE, bitpos), NULL_RTX, 1);
      result = expand_binop(GET_MODE(str_rtx), binop, str_rtx, value, str_rtx,
                            1, OPTAB_WIDEN);
      if (result != str_rtx)
        emit_move_insn(str_rtx, result);
      return true;
...
void expand_assignment(tree to, tree from, bool nontemporal) {
  ...
  /* Assignment of a structure component needs special treatment
     ...
     problem.  */
  if (handled_component_p(to) || TREE_CODE(TREE_TYPE(to)) == ARRAY_TYPE) {
    enum machine_mode mode1;
    ...
    if (offset != 0) {
      ...
      if (MEM_P(to_rtx) && GET_MODE(to_rtx) == BLKmode &&
          GET_MODE(XEXP(to_rtx, 0)) != VOIDmode && bitsize > 0 &&
          (bitpos % bitsize) == 0 &&
    ...
    if (GET_CODE(to_rtx) == CONCAT) {
      if (TREE_CODE(TREE_TYPE(from)) == COMPLEX_TYPE) {
        ...
        result =
            store_expr(from, XEXP(to_rtx, bitpos != 0), false, nontemporal);
      }
    } else {
      if (MEM_P(to_rtx)) {
      ...
      if (optimize_bitfield_assignment_op(bitsize, bitpos, mode1, to_rtx, to,
                                          from))
        result = NULL;
      else
        result = store_field(to_rtx, bitsize, bitpos, mode1, from,
                             TREE_TYPE(tem), get_alias_set(to), nontemporal);
    }
  ...
  if (TREE_CODE(to) == RESULT_DECL && TREE_CODE(from) == INDIRECT_REF &&
      cfun->returns_struct && !cfun->returns_pcc_struct) {
    rtx from_rtx, size;
    ...
    emit_library_call(
        memmove_libfunc, LCT_NORMAL, VOIDmode, 3, XEXP(to_rtx, 0), Pmode,
        XEXP(from_rtx, 0), Pmode,
        convert_to_mode(TYPE_MODE(sizetype), size, TYPE_UNSIGNED(sizetype)),
        TYPE_MODE(sizetype));
...
static bool emit_storent_insn(rtx to, rtx from) {
...
rtx store_expr(tree exp, rtx target, int call_param_p, bool nontemporal) {
  rtx temp;
  ...
  if (TREE_CODE(exp) == COMPOUND_EXPR) {
    ...
    store_expr(TREE_OPERAND(exp, 1), target, call_param_p, nontemporal);
    emit_jump_insn(gen_jump(lab2));
    emit_barrier();
    emit_label(lab1);
    store_expr(TREE_OPERAND(exp, 2), target, call_param_p, nontemporal);
    emit_label(lab2);
    OK_DEFER_POP;
     ...
     expression.  */
  {
    ...
    if (INTEGRAL_TYPE_P(TREE_TYPE(exp)) && TREE_TYPE(TREE_TYPE(exp)) == 0 &&
      ...
      if (TYPE_UNSIGNED(TREE_TYPE(exp)) != SUBREG_PROMOTED_UNSIGNED_P(target)) {
        /* Some types, e.g. Fortran's logical*4, won't have a signed
           version, so use the mode instead.  */
        tree ntype = (signed_or_unsigned_type_for(
            SUBREG_PROMOTED_UNSIGNED_P(target), TREE_TYPE(exp)));
        if (ntype == NULL)
          ntype = lang_hooks.types.type_for_mode(
              TYPE_MODE(TREE_TYPE(exp)), SUBREG_PROMOTED_UNSIGNED_P(target));

        exp = fold_convert(ntype, exp);
      }

      exp = fold_convert(
          lang_hooks.types.type_for_mode(GET_MODE(SUBREG_REG(target)),
    ...
    if (CONSTANT_P(temp) && GET_MODE(temp) == VOIDmode) {
      temp = convert_modes(GET_MODE(target), TYPE_MODE(TREE_TYPE(exp)), temp,
                           SUBREG_PROMOTED_UNSIGNED_P(target));
      temp = convert_modes(GET_MODE(SUBREG_REG(target)), GET_MODE(target), temp,
                           SUBREG_PROMOTED_UNSIGNED_P(target));
    }
    ...
    return NULL_RTX;
  } else if (TREE_CODE(exp) == STRING_CST && !nontemporal && !call_param_p &&
    ...
    dest_mem = store_by_pieces(dest_mem, str_copy_len, builtin_strncpy_read_str,
                               CONST_CAST(char *, TREE_STRING_POINTER(exp)),
                               MEM_ALIGN(target), false,
  ...
  normal_expr:
    ...
    if (!(target && REG_P(target) && REGNO(target) < FIRST_PSEUDO_REGISTER) &&
        !(MEM_P(target) && MEM_VOLATILE_P(target)) &&
        !rtx_equal_p(temp, target) && CONSTANT_P(temp))
  ...
  if ((!rtx_equal_p(temp, target) ||
    ...
    if (GET_MODE(temp) != GET_MODE(target) && GET_MODE(temp) != VOIDmode) {
      ...
      } else if (GET_MODE(target) == BLKmode || GET_MODE(temp) == BLKmode)
      ...
      if (GET_CODE(size) == CONST_INT && INTVAL(size) < TREE_STRING_LENGTH(exp))
        ...
        tree copy_size = size_binop(MIN_EXPR, make_tree(sizetype, size),
        ...
        /* Copy that much.  */
        copy_size_rtx =
            convert_to_mode(ptr_mode, copy_size_rtx, TYPE_UNSIGNED(sizetype));
        ...
        if (GET_CODE(copy_size_rtx) == CONST_INT) {
          ...
          target = adjust_address(target, BLKmode, INTVAL(copy_size_rtx));
        } else {
          ...
          if (GET_MODE(copy_size_rtx) != Pmode)
            copy_size_rtx =
                convert_to_mode(Pmode, copy_size_rtx, TYPE_UNSIGNED(sizetype));
#endif
...
static bool categorize_ctor_elements_1(const_tree ctor,
  ...
  FOR_EACH_CONSTRUCTOR_ELT(CONSTRUCTOR_ELTS(ctor), idx, purpose, value) {
    ...
    if (TREE_CODE(purpose) == RANGE_EXPR) {
      ...
      if (host_integerp(lo_index, 1) && host_integerp(hi_index, 1))
        mult = (tree_low_cst(hi_index, 1) - tree_low_cst(lo_index, 1) + 1);
    }
  ...
  if (!*p_must_clear && (TREE_CODE(TREE_TYPE(ctor)) == UNION_TYPE ||
    ...
    if (!VEC_empty(constructor_elt, CONSTRUCTOR_ELTS(ctor))) {
      ...
      gcc_assert(VEC_length(constructor_elt, CONSTRUCTOR_ELTS(ctor)) == 1);

      init_sub_type = TREE_TYPE(
          VEC_index(constructor_elt, CONSTRUCTOR_ELTS(ctor), 0)->value);
...
bool categorize_ctor_elements(const_tree ctor, HOST_WIDE_INT *p_nz_elts,
                              HOST_WIDE_INT *p_elt_count, bool *p_must_clear) {
  *p_nz_elts = 0;
  *p_elt_count = 0;
  *p_must_clear = false;

  return categorize_ctor_elements_1(ctor, p_nz_elts, p_elt_count, p_must_clear);
}
      ...
      for (f = TYPE_FIELDS(type); f; f = TREE_CHAIN(f))
        if (TREE_CODE(f) == FIELD_DECL) {
          ...
          if (t < 0) {
            ...
            tree tf = TREE_TYPE(f);
            if (allow_flexarr && TREE_CHAIN(f) == NULL &&
                TREE_CODE(tf) == ARRAY_TYPE && TYPE_DOMAIN(tf) &&
...
static int mostly_zeros_p(const_tree exp) {
...
static int all_zeros_p(const_tree exp) {
  if (TREE_CODE(exp) == CONSTRUCTOR)
...
static void store_constructor_field(rtx target, unsigned HOST_WIDE_INT bitsize,
                                    HOST_WIDE_INT bitpos,
                                    enum machine_mode mode, tree exp, tree type,
                                    int cleared, alias_set_type alias_set) {
  if (TREE_CODE(exp) == CONSTRUCTOR
    ...
    if (MEM_P(target))
      target = adjust_address(
          target,
          GET_MODE(target) == BLKmode ||
                  0 != (bitpos % GET_MODE_ALIGNMENT(GET_MODE(target)))
              ? BLKmode
              : VOIDmode,
  ...
  switch (TREE_CODE(type)) {
    ...
    case QUAL_UNION_TYPE: {
      ...
      FOR_EACH_CONSTRUCTOR_ELT(CONSTRUCTOR_ELTS(exp), idx, field, value) {
        ...
        if (host_integerp(offset, 0) && host_integerp(bit_position(field), 0)) {
        ...
        if (offset) {
          ...
          offset = SUBSTITUTE_PLACEHOLDER_IN_EXPR(
          ...
          to_rtx =
              offset_address(to_rtx, offset_rtx, highest_pow2_factor(offset));
        }
        ...
        /* If this initializes a field that is smaller than a
           ...
           can understand.  */
        if (REG_P(target) && bitsize < BITS_PER_WORD &&
            bitpos % BITS_PER_WORD == 0 && GET_MODE_CLASS(mode) == MODE_INT &&
            TREE_CODE(value) == INTEGER_CST && exp_size >= 0 &&
            bitpos + BITS_PER_WORD <= exp_size * BITS_PER_UNIT) {
          tree type = TREE_TYPE(value);

          if (TYPE_PRECISION(type) < BITS_PER_WORD) {
            type = lang_hooks.types.type_for_size(BITS_PER_WORD,
                                                  TYPE_UNSIGNED(type));
            value = fold_convert(type, value);
          }

          if (BYTES_BIG_ENDIAN)
            value = fold_build2(LSHIFT_EXPR, type, value,
                                build_int_cst(type, BITS_PER_WORD - bitsize));
        ...
        store_constructor_field(to_rtx, bitsize, bitpos, mode, value, type,
                                cleared, get_alias_set(TREE_TYPE(field)));
      }
      break;
    }
    case ARRAY_TYPE: {
      ...
      const_bounds_p = (TYPE_MIN_VALUE(domain) && TYPE_MAX_VALUE(domain) &&
      ...
      if (cleared)
        ...
        FOR_EACH_CONSTRUCTOR_ELT(CONSTRUCTOR_ELTS(exp), idx, index, value) {
          ...
          if (index != NULL_TREE && TREE_CODE(index) == RANGE_EXPR) {
            ...
            this_node_count =
                (tree_low_cst(hi_index, 1) - tree_low_cst(lo_index, 1) + 1);
          } else
            this_node_count = 1;
      ...
      FOR_EACH_CONSTRUCTOR_ELT(CONSTRUCTOR_ELTS(exp), i, index, value) {
        ...
        if (index != NULL_TREE && TREE_CODE(index) == RANGE_EXPR) {
          ...
          if (const_bounds_p && host_integerp(lo_index, 0) &&
              host_integerp(hi_index, 0) &&
              (lo = tree_low_cst(lo_index, 0), hi = tree_low_cst(hi_index, 0),
               count = hi - lo + 1,
               (!MEM_P(target) || count <= 2 ||
                (host_integerp(TYPE_SIZE(elttype), 1) &&
                 (tree_low_cst(TYPE_SIZE(elttype), 1) * count <= 40 * 8))))) {
            lo -= minelt;
            hi -= minelt;
            for (; lo <= hi; lo++) {
            ...
            index_r = gen_reg_rtx(
                promote_mode(domain, DECL_MODE(index), &unsignedp, 0));
            ...
            position = fold_convert(
                ssizetype, fold_build2(MINUS_EXPR, TREE_TYPE(index), index,
                                       TYPE_MIN_VALUE(domain)));

            position =
                size_binop(MULT_EXPR, position,
                           fold_convert(ssizetype, TYPE_SIZE_UNIT(elttype)));

            pos_rtx = expand_normal(position);
            xtarget =
                offset_address(target, pos_rtx, highest_pow2_factor(position));
            ...
            expand_assignment(
                index,
                build2(PLUS_EXPR, TREE_TYPE(index), index, integer_one_node),
                false);
          ...
          position =
              size_binop(MULT_EXPR, index,
                         fold_convert(ssizetype, TYPE_SIZE_UNIT(elttype)));
          xtarget = offset_address(target, expand_normal(position),
          ...
          store_constructor_field(target, bitsize, bitpos, mode, value, type,
    ...
    case VECTOR_TYPE: {
      ...
      if (cleared)
        ...
        FOR_EACH_CONSTRUCTOR_VALUE(CONSTRUCTOR_ELTS(exp), idx, value) {
          int n_elts_here = tree_low_cst(
              int_const_binop(TRUNC_DIV_EXPR, TYPE_SIZE(TREE_TYPE(value)),
                              TYPE_SIZE(elttype), 0),
              1);
      ...
      if (vector)
        emit_insn(
            GEN_FCN(icode)(target, gen_rtx_PARALLEL(GET_MODE(target), vector)));
      break;
    }
...
static rtx store_field(rtx target, HOST_WIDE_INT bitsize, HOST_WIDE_INT bitpos,
                       enum machine_mode mode, tree exp, tree type,
  ...
  if (mode == BLKmode && (REG_P(target) || GET_CODE(target) == SUBREG)) {
  ...
  if (mode == VOIDmode ||
      ...
      ||
      (mode != BLKmode && ((((MEM_ALIGN(target) < GET_MODE_ALIGNMENT(mode)) ||
      ...
      /* If the RHS and field are a constant size and the size of the
         ...
         operations.  */
      || (bitsize >= 0 && TREE_CODE(TYPE_SIZE(TREE_TYPE(exp))) == INTEGER_CST &&
    ...
    if (BYTES_BIG_ENDIAN && GET_MODE_CLASS(GET_MODE(temp)) == MODE_INT &&
      ...
      temp = expand_shift(RSHIFT_EXPR, GET_MODE(temp), temp,
                          size_int(GET_MODE_BITSIZE(GET_MODE(temp)) - bitsize),
                          NULL_RTX, 1);
    ...
    if (GET_MODE(temp) == BLKmode &&
        (GET_MODE(target) == BLKmode ||
         (MEM_P(target) && GET_MODE_CLASS(GET_MODE(target)) == MODE_INT &&
          (bitpos % BITS_PER_UNIT) == 0 && (bitsize % BITS_PER_UNIT) == 0))) {
      gcc_assert(MEM_P(target) && MEM_P(temp) && (bitpos % BITS_PER_UNIT) == 0);

      target = adjust_address(target, VOIDmode, bitpos / BITS_PER_UNIT);
      emit_block_move(target, temp,
                      GEN_INT((bitsize + BITS_PER_UNIT - 1) / BITS_PER_UNIT),
                      BLOCK_OP_NORMAL);
...
tree get_inner_reference(tree exp, HOST_WIDE_INT *pbitsize,
  ...
  if (TREE_CODE(exp) == COMPONENT_REF) {
    ...
    *punsignedp =
        (!INTEGRAL_TYPE_P(TREE_TYPE(exp)) || TYPE_UNSIGNED(TREE_TYPE(exp)));
  ...
  while (1) {
    switch (TREE_CODE(exp)) {
      ...
      case COMPONENT_REF: {
        ...
        bit_offset =
            size_binop(PLUS_EXPR, bit_offset, DECL_FIELD_BIT_OFFSET(field));
      ...
      case IMAGPART_EXPR:
        bit_offset = size_binop(PLUS_EXPR, bit_offset, bitsize_int(*pbitsize));
        break;
      ...
      case VIEW_CONVERT_EXPR:
        if (keep_aligning && STRICT_ALIGNMENT &&
            ...
            (TYPE_ALIGN(TREE_TYPE(TREE_OPERAND(exp, 0))) < BIGGEST_ALIGNMENT) &&
  ...
  if (mode == VOIDmode && blkmode_bitfield && (*pbitpos % BITS_PER_UNIT) == 0 &&
...
bool contains_packed_reference(const_tree exp) {
  bool packed_p = false;
  ...
  while (1) {
    switch (TREE_CODE(exp)) {
      case COMPONENT_REF: {
        tree field = TREE_OPERAND(exp, 1);
        packed_p = DECL_PACKED(field) || TYPE_PACKED(TREE_TYPE(field)) ||
...
int handled_component_p(const_tree t) {
  switch (TREE_CODE(t)) {
    case BIT_FIELD_REF:
    case COMPONENT_REF:
    case ARRAY_REF:
...
rtx force_operand(rtx value, rtx target) {
  ...
  if (code == SUBREG && !REG_P(SUBREG_REG(value)) &&
      !MEM_P(SUBREG_REG(value))) {
    value = simplify_gen_subreg(
        GET_MODE(value),
  ...
  if (ARITHMETIC_P(value)) {
    ...
    if (code == PLUS && GET_CODE(op2) == CONST_INT &&
        GET_CODE(XEXP(value, 0)) == PLUS && REG_P(XEXP(XEXP(value, 0), 0)) &&
        REGNO(XEXP(XEXP(value, 0), 0)) >= FIRST_VIRTUAL_REGISTER &&
        REGNO(XEXP(XEXP(value, 0), 0)) <= LAST_VIRTUAL_REGISTER) {
      rtx temp =
          expand_simple_binop(GET_MODE(value), code, XEXP(XEXP(value, 0), 0),
                              op2, subtarget, 0, OPTAB_LIB_WIDEN);
    ...
    switch (code) {
      ...
      case DIV:
        if (!INTEGRAL_MODE_P(GET_MODE(value)))
          return expand_simple_binop(GET_MODE(value), code, op1, op2, target, 1,
                                     OPTAB_LIB_WIDEN);
        else
          return expand_divmod(
              0, FLOAT_MODE_P(GET_MODE(value)) ? RDIV_EXPR : TRUNC_DIV_EXPR,
      ...
      case UMOD:
        return expand_divmod(1, TRUNC_MOD_EXPR, GET_MODE(value), op1, op2,
                             target, 1);
      case ASHIFTRT:
        return expand_simple_binop(GET_MODE(value), code, op1, op2, target, 0,
                                   OPTAB_LIB_WIDEN);
      default:
        return expand_simple_binop(GET_MODE(value), code, op1, op2, target, 1,
                                   OPTAB_LIB_WIDEN);
...
int safe_from_p(const_rtx x, tree exp, int top_p) {
  rtx exp_rtl = 0;
  int i, nops;
  ...
  if (x == 0
      ...
      || (MEM_P(x) && (XEXP(x, 0) == virtual_outgoing_args_rtx ||
    ...
    case tcc_vl_exp:
      ...
      switch (TREE_CODE(exp)) {
        case ADDR_EXPR:
          ...
          if (staticp(TREE_OPERAND(exp, 0)) || TREE_STATIC(exp) ||
          ...
          exp = TREE_OPERAND(exp, 0);
          if (DECL_P(exp)) {
            if (!DECL_RTL_SET_P(exp) || !MEM_P(DECL_RTL(exp)))
              return 0;
            else
        ...
        case INDIRECT_REF:
          if (MEM_P(x) &&
              alias_sets_conflict_p(MEM_ALIAS_SET(x), get_alias_set(exp)))
            return 0;
          break;
        ...
        case CALL_EXPR:
          /* Assume that the call will clobber all hard registers and
             all of memory.  */
          if ((REG_P(x) && REGNO(x) < FIRST_PSEUDO_REGISTER) || MEM_P(x))
            return 0;
          break;
  ...
  if (exp_rtl) {
    if (GET_CODE(exp_rtl) == SUBREG) {
      ...
      if (REG_P(exp_rtl) && REGNO(exp_rtl) < FIRST_PSEUDO_REGISTER)
        return 0;
    }
    ...
    return !(rtx_equal_p(x, exp_rtl) ||
             (MEM_P(x) && MEM_P(exp_rtl) &&
              true_dependence(exp_rtl, VOIDmode, x, rtx_addr_varies_p)));
  }
...
static tree emutls_var_address(tree var) {
...
static void expand_operands(tree exp0, tree exp1, rtx target, rtx *op0,
                            rtx *op1, enum expand_modifier modifier) {
...
static rtx expand_expr_constant(tree exp, int defer,
...
static rtx expand_expr_addr_expr_1(tree exp, rtx target,
                                   enum machine_mode tmode,
                                   enum expand_modifier modifier) {
  ...
  switch (TREE_CODE(exp)) {
    ...
    case CONST_DECL:
      ...
      return expand_expr_addr_expr_1(DECL_INITIAL(exp), target, tmode,
                                     modifier);
    ...
    case VAR_DECL:
      ...
      if (!targetm.have_tls && TREE_CODE(exp) == VAR_DECL &&
    ...
    default:
      ...
      if (DECL_P(exp) || TREE_CODE(exp) == CONSTRUCTOR ||
          TREE_CODE(exp) >= LAST_AND_UNUSED_TREE_CODE) {
        result =
            expand_expr(exp, target, tmode,
                        modifier == EXPAND_INITIALIZER ? EXPAND_INITIALIZER
        ...
        if (modifier != EXPAND_INITIALIZER && modifier != EXPAND_CONST_ADDRESS)
  ...
  if (offset) {
    ...
    if (modifier == EXPAND_SUM || modifier == EXPAND_INITIALIZER)
      ...
      result = expand_simple_binop(tmode, PLUS, result, tmp, subtarget, 1,
                                   OPTAB_LIB_WIDEN);
    }
  }
...
static rtx expand_expr_addr_expr(tree exp, rtx target, enum machine_mode tmode,
                                 enum expand_modifier modifier) {
  ...
  result =
      expand_expr_addr_expr_1(TREE_OPERAND(exp, 0), target, tmode, modifier);
...
static rtx expand_constructor(tree exp, rtx target,
                              enum expand_modifier modifier,
                              bool avoid_temp_mem) {
  tree type = TREE_TYPE(exp);
  ...
  /* Try to avoid creating a temporary at all.  This is possible
     ...
     with memset.  */
  if (TREE_STATIC(exp) && !TREE_ADDRESSABLE(exp) && target != 0 &&
      mode == BLKmode && all_zeros_p(exp)) {
    clear_storage(target, expr_size(exp), BLOCK_OP_NORMAL);
    return target;
  }
  ...
  if ((TREE_STATIC(exp) &&
       ((mode == BLKmode && !(target != 0 && safe_from_p(target, exp, 1))) ||
        ...
        (host_integerp(TYPE_SIZE_UNIT(type), 1) &&
         (!MOVE_BY_PIECES_P(tree_low_cst(TYPE_SIZE_UNIT(type), 1),
                            TYPE_ALIGN(type))) &&
         !mostly_zeros_p(exp)))) ||
  ...
  if (target == 0 || !safe_from_p(target, exp, 1) ||
    ...
    target = assign_temp(
        build_qualified_type(
            type, (TYPE_QUALS(type) | (TREE_READONLY(exp) * TYPE_QUAL_CONST))),
...
rtx expand_expr_real(tree exp, rtx target, enum machine_mode tmode,
...
static rtx expand_expr_real_1(tree exp, rtx target, enum machine_mode tmode,
                              enum expand_modifier modifier, rtx *alt_rtl) {
...
#define REDUCE_BIT_FIELD(expr)                                            \
  (reduce_bit_field ? reduce_to_bit_field_precision((expr), target, type) \
                    : (expr))
  ...
  ignore = (target == const0_rtx ||
            ((CONVERT_EXPR_CODE_P(code) || code == COND_EXPR ||
  ...
  reduce_bit_field = (!ignore && TREE_CODE(type) == INTEGER_TYPE &&
  ...
  if (ignore) {
    ...
    if (TREE_THIS_VOLATILE(exp) && TREE_CODE(exp) != FUNCTION_DECL &&
    ...
    if (TREE_CODE_CLASS(code) == tcc_unary || code == COMPONENT_REF ||
        code == INDIRECT_REF)
      return expand_expr(TREE_OPERAND(exp, 0), const0_rtx, VOIDmode, modifier);

    else if (TREE_CODE_CLASS(code) == tcc_binary ||
             TREE_CODE_CLASS(code) == tcc_comparison || code == ARRAY_REF ||
  ...
  switch (code) {
    ...
    case RESULT_DECL:
      ...
      /* If DECL_RTL is memory, we are in the normal case and the
        ...
        if (modifier != EXPAND_CONST_ADDRESS && modifier != EXPAND_SUM &&
          ...
          temp = replace_equiv_address(decl_rtl, copy_rtx(XEXP(decl_rtl, 0)));
      }
      ...
      if (REG_P(decl_rtl) && GET_MODE(decl_rtl) != DECL_MODE(exp)) {
        ...
        pmode = promote_mode(
            type, DECL_MODE(exp), &unsignedp,
            (TREE_CODE(exp) == RESULT_DECL || TREE_CODE(exp) == PARM_DECL) ? 1
                                                                           : 0);
    ...
    case INTEGER_CST:
      temp = immed_double_const(TREE_INT_CST_LOW(exp), TREE_INT_CST_HIGH(exp),
                                mode);
    ...
    case VECTOR_CST: {
      ...
      if (!tmp)
        tmp = build_constructor_from_list(type, TREE_VECTOR_CST_ELTS(exp));
      return expand_expr(tmp, ignore ? const0_rtx : target, tmode, modifier);
    }
    ...
    case STRING_CST:
      ...
      if (modifier != EXPAND_CONST_ADDRESS && modifier != EXPAND_INITIALIZER &&
          modifier != EXPAND_SUM && !memory_address_p(mode, XEXP(temp, 0)))
    ...
    case INDIRECT_REF: {
      ...
      if (code == MISALIGNED_INDIRECT_REF) {
        ...
        gcc_assert(modifier == EXPAND_NORMAL || modifier == EXPAND_STACK_PARM);
...

    {
      ...
      if (modifier != EXPAND_CONST_ADDRESS && modifier != EXPAND_INITIALIZER &&
          modifier != EXPAND_MEMORY && TREE_CODE(array) == CONSTRUCTOR &&
          !TREE_SIDE_EFFECTS(array) && TREE_CODE(index) == INTEGER_CST) {
      ...
      else if (optimize >= 1 && modifier != EXPAND_CONST_ADDRESS &&
               modifier != EXPAND_INITIALIZER && modifier != EXPAND_MEMORY &&
        ...
        if (TREE_CODE(index) == INTEGER_CST) {
          ...
          if (TREE_CODE(init) == CONSTRUCTOR) {
            ...
            if (tree_int_cst_equal(field, index)) {
              ...
              if (TREE_CODE(value) == CONSTRUCTOR) {
                /* If VALUE is a CONSTRUCTOR, this
                   ...
                   the array directly.  */
                rtx ret = expand_constructor(value, target, modifier, true);
                if (ret == NULL_RTX)
                  break;
              }
            ...
            if (!integer_zerop(low_bound))
              index1 = size_diffop(index1, fold_convert(sizetype, low_bound));

            if (0 > compare_tree_int(index1, TREE_STRING_LENGTH(init))) {
              tree type = TREE_TYPE(TREE_TYPE(init));
              enum machine_mode mode = TYPE_MODE(type);

              if (GET_MODE_CLASS(mode) == MODE_INT && GET_MODE_SIZE(mode) == 1)
                return gen_int_mode(
                    TREE_STRING_POINTER(init)[TREE_INT_CST_LOW(index1)], mode);
            }
          }
        }
      }
    ...
    case COMPONENT_REF:
      ...
      if (TREE_CODE(TREE_OPERAND(exp, 0)) == CONSTRUCTOR) {
        ...
        FOR_EACH_CONSTRUCTOR_ELT(CONSTRUCTOR_ELTS(TREE_OPERAND(exp, 0)), idx,
                                 field, value)
        if (field == TREE_OPERAND(exp, 1)
          ...
          if (DECL_BIT_FIELD(field) && modifier == EXPAND_STACK_PARM)
            target = 0;
          ...
          if (DECL_BIT_FIELD(field)) {
            ...
            if (TYPE_UNSIGNED(TREE_TYPE(field))) {
              ...
              tree count =
                  build_int_cst(NULL_TREE, GET_MODE_BITSIZE(imode) - bitsize);
    ...
    normal_inner_ref : {
      ...
      tree tem = get_inner_reference(exp, &bitsize, &bitpos, &offset, &mode1,
                                     &unsignedp, &volatilep, true);
      rtx orig_op0, memloc;
      ...
      /* If we have either an offset, a BLKmode result, or a reference
         ...
         to a larger size.  */
      must_force_mem = (offset || mode1 == BLKmode ||
                        bitpos + bitsize > GET_MODE_BITSIZE(mode2));

      /* If this is a constant, put it in a register if it is a legitimate
         constant and we don't need a memory reference.  */
      if (CONSTANT_P(op0) && mode2 != BLKmode && LEGITIMATE_CONSTANT_P(op0) &&
          !must_force_mem)
      ...
      /* Otherwise, if this is a constant or the object is not in memory
        ...
        tree nt = build_qualified_type(
            TREE_TYPE(tem), (TYPE_QUALS(TREE_TYPE(tem)) | TYPE_QUAL_CONST));
      ...
      if (offset) {
        ...
        if (GET_MODE(op0) == BLKmode
            ...
            && GET_MODE(XEXP(op0, 0)) != VOIDmode && bitsize != 0 &&
            (bitpos % bitsize) == 0 &&
            (bitsize % GET_MODE_ALIGNMENT(mode1)) == 0 &&
        ...
        op0 = offset_address(op0, offset_rtx, highest_pow2_factor(offset));
      }
      ...
      if (GET_CODE(op0) == CONCAT) {
        gcc_assert(bitpos == 0 && bitsize == GET_MODE_BITSIZE(GET_MODE(op0)));
        return op0;
      }
      ...
      if (mode1 == VOIDmode || REG_P(op0) || GET_CODE(op0) == SUBREG ||
          ...
          || (mode1 != BLKmode &&
              (((TYPE_ALIGN(TREE_TYPE(tem)) < GET_MODE_ALIGNMENT(mode) ||
                 ...
                 (MEM_P(op0) && (MEM_ALIGN(op0) < GET_MODE_ALIGNMENT(mode1) ||
          ...
          || (bitsize >= 0 && TYPE_SIZE(TREE_TYPE(exp)) &&
              TREE_CODE(TYPE_SIZE(TREE_TYPE(exp))) == INTEGER_CST &&
              0 != compare_tree_int(TYPE_SIZE(TREE_TYPE(exp)), bitsize))) {
        enum machine_mode ext_mode = mode;

        if (ext_mode == BLKmode &&
            !(target != 0 && MEM_P(op0) && MEM_P(target) &&
              bitpos % BITS_PER_UNIT == 0))
        ...
        if (ext_mode == BLKmode) {
          ...
          emit_block_move(
              target, adjust_address(op0, VOIDmode, bitpos / BITS_PER_UNIT),
              GEN_INT((bitsize + BITS_PER_UNIT - 1) / BITS_PER_UNIT),
              (modifier == EXPAND_STACK_PARM ? BLOCK_OP_CALL_PARM
                                             : BLOCK_OP_NORMAL));
        ...
        op0 = extract_bit_field(
            op0, bitsize, bitpos, unsignedp,
            (modifier == EXPAND_STACK_PARM ? NULL_RTX : target), ext_mode,
            ext_mode);
        ...
        if (TREE_CODE(type) == RECORD_TYPE && BYTES_BIG_ENDIAN &&
          ...
          op0 = expand_shift(
              LSHIFT_EXPR, GET_MODE(op0), op0,
              size_int(GET_MODE_BITSIZE(GET_MODE(op0)) - bitsize), op0, 1);
    ...
    case CALL_EXPR:
        ...
        error("%Kinvalid use of %<__builtin_va_arg_pack ()%>", exp);
      {
        ...
        if (fndecl &&
            (attr = lookup_attribute("error", DECL_ATTRIBUTES(fndecl))) != NULL)
          error("%Kcall to %qs declared with attribute error: %s", exp,
                lang_hooks.decl_printable_name(fndecl, 1),
                TREE_STRING_POINTER(TREE_VALUE(TREE_VALUE(attr))));
        if (fndecl && (attr = lookup_attribute(
                           "warning", DECL_ATTRIBUTES(fndecl))) != NULL)
          warning_at(tree_nonartificial_location(exp), 0,
                     "%Kcall to %qs declared with attribute warning: %s", exp,
                     lang_hooks.decl_printable_name(fndecl, 1),
                     TREE_STRING_POINTER(TREE_VALUE(TREE_VALUE(attr))));
    ...
    CASE_CONVERT:
      ...
      if (TREE_CODE(type) == UNION_TYPE) {
        ...
        if (MEM_P(target))
          ...
          store_field(target,
                      MIN((int_size_in_bytes(TREE_TYPE(TREE_OPERAND(exp, 0))) *
                           BITS_PER_UNIT),
                          (HOST_WIDE_INT)GET_MODE_BITSIZE(mode)),
                      0, TYPE_MODE(valtype), TREE_OPERAND(exp, 0), type, 0,
                      false);
        }
    ...
    case VIEW_CONVERT_EXPR:
      ...
      if (mode == BLKmode &&
        ...
        tree tem =
            get_inner_reference(TREE_OPERAND(exp, 0), &bitsize, &bitpos,
                                &offset, &mode1, &unsignedp, &volatilep, true);
        rtx orig_op0;

        /* ??? We should work harder and deal with non-zero offsets.  */
        if (!offset && (bitpos % BITS_PER_UNIT) == 0 && bitsize >= 0 &&
            compare_tree_int(TYPE_SIZE(TREE_TYPE(exp)), bitsize) == 0) {
          /* See the normal_inner_ref case for the rationale.  */
          orig_op0 = expand_expr(
              tem,
              (TREE_CODE(TREE_TYPE(tem)) == UNION_TYPE &&
                       (TREE_CODE(TYPE_SIZE(TREE_TYPE(tem))) != INTEGER_CST) &&
                       modifier != EXPAND_STACK_PARM
      ...
      /* As a last resort, spill op0 to memory, and reload it in a
        ...
        if (target == 0 || GET_MODE(target) != TYPE_MODE(inner_type))
          target = assign_stack_temp_for_type(
              TYPE_MODE(inner_type), GET_MODE_SIZE(TYPE_MODE(inner_type)), 0,
              inner_type);
      ...
      if (MEM_P(op0)) {
        ...
        else if (STRICT_ALIGNMENT && mode != BLKmode &&
                 MEM_ALIGN(op0) < GET_MODE_ALIGNMENT(mode)) {
          tree inner_type = TREE_TYPE(TREE_OPERAND(exp, 0));
          HOST_WIDE_INT temp_size = MAX(int_size_in_bytes(inner_type),
                                        (HOST_WIDE_INT)GET_MODE_SIZE(mode));
          rtx new_rtx = assign_stack_temp_for_type(mode, temp_size, 0, type);
    ...
    case POINTER_PLUS_EXPR:
      ...
      if (TYPE_PRECISION(sizetype) < TYPE_PRECISION(type))
        exp = build2(
            PLUS_EXPR, type, TREE_OPERAND(exp, 0),
            fold_convert(type, fold_convert(ssizetype, TREE_OPERAND(exp, 1))));
    case PLUS_EXPR:
      ...
      if ((TREE_CODE(type) == INTEGER_TYPE ||
        ...
        this_code =
            TREE_CODE(type) == INTEGER_TYPE ? NOP_EXPR : FIXED_CONVERT_EXPR;
        if (code0 == this_code && code1 == this_code &&
          ...
          if (sat_p == 0)
            ...
            this_optab = zextend_p ? usmadd_widen_optab : ssmadd_widen_optab;
          if (mode == GET_MODE_2XWIDER_MODE(innermode) &&
              (optab_handler(this_optab, mode)->insn_code !=
               CODE_FOR_nothing)) {
            expand_operands(TREE_OPERAND(subsubexp0, 0),
                            TREE_OPERAND(subsubexp1, 0), NULL_RTX, &op0, &op1,
                            EXPAND_NORMAL);
            op2 = expand_expr(TREE_OPERAND(exp, 1), subtarget, VOIDmode,
                              EXPAND_NORMAL);
            temp = expand_ternary_op(mode, this_optab, op0, op1, op2, target,
                                     unsignedp);
            gcc_assert(temp);
      ...
      if (modifier == EXPAND_SUM || modifier == EXPAND_INITIALIZER ||
        ...
        if (TREE_CODE(TREE_OPERAND(exp, 0)) == INTEGER_CST &&
          ...
          constant_part = immed_double_const(
              TREE_INT_CST_LOW(TREE_OPERAND(exp, 0)), (HOST_WIDE_INT)0,
          ...
          if (!CONSTANT_P(op0)) {
            op1 =
                expand_expr(TREE_OPERAND(exp, 1), NULL_RTX, VOIDmode, modifier);
          ...
          /* Use immed_double_const to ensure that the constant is
             ...
             in non-canonical RTL in a 64x32 cross compile.  */
          constant_part = immed_double_const(
              TREE_INT_CST_LOW(TREE_OPERAND(exp, 1)), (HOST_WIDE_INT)0,
    ...
    case MINUS_EXPR:
      ...
      if ((TREE_CODE(type) == INTEGER_TYPE ||
        ...
        if (code0 == this_code && code1 == this_code &&
          ...
          if (sat_p == 0)
            ...
            this_optab = zextend_p ? usmsub_widen_optab : ssmsub_widen_optab;
          if (mode == GET_MODE_2XWIDER_MODE(innermode) &&
              (optab_handler(this_optab, mode)->insn_code !=
               CODE_FOR_nothing)) {
            expand_operands(TREE_OPERAND(subsubexp0, 0),
                            TREE_OPERAND(subsubexp1, 0), NULL_RTX, &op0, &op1,
                            EXPAND_NORMAL);
            op2 = expand_expr(TREE_OPERAND(exp, 0), subtarget, VOIDmode,
                              EXPAND_NORMAL);
            temp = expand_ternary_op(mode, this_optab, op0, op1, op2, target,
                                     unsignedp);
            gcc_assert(temp);
      ...
      if ((modifier == EXPAND_SUM || modifier == EXPAND_INITIALIZER) &&
        ...
        expand_operands(TREE_OPERAND(exp, 0), TREE_OPERAND(exp, 1), NULL_RTX,
                        &op0, &op1, modifier);
    ...
    case MULT_EXPR:
      ...
      if (modifier == EXPAND_SUM && mode == ptr_mode &&
        ...
        op0 =
            expand_expr(TREE_OPERAND(exp, 0), subtarget, VOIDmode, EXPAND_SUM);
        ...
        return REDUCE_BIT_FIELD(gen_rtx_MULT(
            mode, op0,
            gen_int_mode(tree_low_cst(exp1, 0), TYPE_MODE(TREE_TYPE(exp1)))));
      }
      ...
      /* First, check if we have a multiplication of one signed and one
         unsigned operand.  */
      if (TREE_CODE(subexp0) == NOP_EXPR && TREE_CODE(subexp1) == NOP_EXPR &&
          ...
          if (optab_handler(this_optab, mode)->insn_code != CODE_FOR_nothing) {
            if (TYPE_UNSIGNED(TREE_TYPE(TREE_OPERAND(subexp0, 0))))
              expand_operands(TREE_OPERAND(subexp0, 0),
                              TREE_OPERAND(subexp1, 0), NULL_RTX, &op0, &op1,
                              0);
            else
              expand_operands(TREE_OPERAND(subexp0, 0),
                              TREE_OPERAND(subexp1, 0), NULL_RTX, &op1, &op0,
                              0);
      ...
      /* Check for a multiplication with matching signedness.  */
               ...
               ((TREE_CODE(TREE_OPERAND(exp, 1)) == INTEGER_CST &&
                 ...
                 && ((GET_MODE_BITSIZE(TYPE_MODE(TREE_TYPE(
                          TREE_OPERAND(exp, 1)))) > HOST_BITS_PER_WIDE_INT) ||
          ...
          if (optab_handler(this_optab, mode)->insn_code != CODE_FOR_nothing) {
            if (TREE_CODE(TREE_OPERAND(exp, 1)) == INTEGER_CST)
              expand_operands(TREE_OPERAND(TREE_OPERAND(exp, 0), 0),
                              TREE_OPERAND(exp, 1), NULL_RTX, &op0, &op1,
                              EXPAND_NORMAL);
            else
              expand_operands(TREE_OPERAND(TREE_OPERAND(exp, 0), 0),
                              TREE_OPERAND(TREE_OPERAND(exp, 1), 0), NULL_RTX,
                              &op0, &op1, EXPAND_NORMAL);
            goto binop3;
          } else if (optab_handler(other_optab, mode)->insn_code !=
                         CODE_FOR_nothing &&
                     innermode == word_mode) {
            rtx htem, hipart;
            ...
            if (TREE_CODE(TREE_OPERAND(exp, 1)) == INTEGER_CST)
              op1 =
                  convert_modes(innermode, mode,
                                expand_normal(TREE_OPERAND(exp, 1)), unsignedp);
            else
              op1 = expand_normal(TREE_OPERAND(TREE_OPERAND(exp, 1), 0));
            temp = expand_binop(mode, other_optab, op0, op1, target, unsignedp,
                                OPTAB_LIB_WIDEN);
            hipart = gen_highpart(innermode, temp);
            htem = expand_mult_highpart_adjust(innermode, hipart, op0, op1,
                                               hipart, zextend_p);
            if (htem != hipart)
    ...
    case FLOAT_EXPR:
      ...
      expand_float(target, op0, TYPE_UNSIGNED(TREE_TYPE(TREE_OPERAND(exp, 0))));
      return target;

    case NEGATE_EXPR:
      op0 =
          expand_expr(TREE_OPERAND(exp, 0), subtarget, VOIDmode, EXPAND_NORMAL);
      if (modifier == EXPAND_STACK_PARM)
        target = 0;
      temp = expand_unop(mode,
                         optab_for_tree_code(NEGATE_EXPR, type, optab_default),
                         op0, target, 0);
      gcc_assert(temp);
      return REDUCE_BIT_FIELD(temp);
    ...
    case MIN_EXPR:
      ...
      if (target == 0 || modifier == EXPAND_STACK_PARM ||
        ...
        target = gen_reg_rtx(mode);
      expand_operands(TREE_OPERAND(exp, 0), TREE_OPERAND(exp, 1), target, &op0,
                      &op1, 0);
...

      {
        ...
        if (can_conditionally_move_p(mode)) {
          ...
          insn = emit_conditional_move(target, comparison_code, op0, cmpop1,
                                       mode, op0, op1, mode, unsignedp);
    ...
    case RROTATE_EXPR:
      gcc_assert(VECTOR_MODE_P(TYPE_MODE(type)) ||
                 (GET_MODE_PRECISION(TYPE_MODE(type)) == TYPE_PRECISION(type)));
      /* fall through */
    ...
    case LTGT_EXPR:
      temp =
          do_store_flag(exp, modifier != EXPAND_STACK_PARM ? target : NULL_RTX,
      ...
      if (code == NE_EXPR && integer_zerop(TREE_OPERAND(exp, 1)) &&
          original_target && REG_P(original_target) &&
          (GET_MODE(original_target) ==
           TYPE_MODE(TREE_TYPE(TREE_OPERAND(exp, 0))))) {
        temp = expand_expr(TREE_OPERAND(exp, 0), original_target, VOIDmode,
                           EXPAND_NORMAL);
        ...
        op1 = gen_label_rtx();
        emit_cmp_and_jump_insns(temp, const0_rtx, EQ, NULL_RTX, GET_MODE(temp),
                                unsignedp, op1);
    ...
    case TRUTH_ORIF_EXPR:
      if (!ignore &&
          (target == 0 || modifier == EXPAND_STACK_PARM ||
    ...
    case TRUTH_NOT_EXPR:
      ...
      temp = expand_binop(mode, xor_optab, op0, const1_rtx, target, 1,
                          OPTAB_LIB_WIDEN);
      gcc_assert(temp);
      return temp;
    ...
    case COND_EXPR:
      ...
      if (modifier != EXPAND_STACK_PARM && original_target &&
          safe_from_p(original_target, TREE_OPERAND(exp, 0), 1) &&
          GET_MODE(original_target) == mode
#ifdef HAVE_conditional_move
          && (!can_conditionally_move_p(mode) || REG_P(original_target))
#endif
      ...
      store_expr(TREE_OPERAND(exp, 1), temp, modifier == EXPAND_STACK_PARM,
                 false);

      emit_jump_insn(gen_jump(op1));
      emit_barrier();
      emit_label(op0);
      store_expr(TREE_OPERAND(exp, 2), temp, modifier == EXPAND_STACK_PARM,
                 false);
    ...
    case REALIGN_LOAD_EXPR: {
      ...
      temp =
          expand_ternary_op(mode, this_optab, op0, op1, op2, target, unsignedp);
      gcc_assert(temp);
      return temp;
    }
    ...
    case DOT_PROD_EXPR: {
      ...
      expand_operands(oprnd0, oprnd1, NULL_RTX, &op0, &op1, EXPAND_NORMAL);
      op2 = expand_normal(oprnd2);
      target = expand_widen_pattern_expr(exp, op0, op1, op2, target, unsignedp);
      return target;
    }
    ...
    case WIDEN_SUM_EXPR: {
      ...
      target =
          expand_widen_pattern_expr(exp, op0, NULL_RTX, op1, target, unsignedp);
      return target;
    }
    ...
    case VEC_UNPACK_LO_EXPR: {
      op0 = expand_normal(TREE_OPERAND(exp, 0));
      this_optab = optab_for_tree_code(code, type, optab_default);
      temp = expand_widen_pattern_expr(exp, op0, NULL_RTX, NULL_RTX, target,
                                       unsignedp);
      gcc_assert(temp);
      return temp;
    }
    ...
    case VEC_UNPACK_FLOAT_LO_EXPR: {
      ...
      this_optab = optab_for_tree_code(code, TREE_TYPE(TREE_OPERAND(exp, 0)),
                                       optab_default);
      temp = expand_widen_pattern_expr(
          exp, op0, NULL_RTX, NULL_RTX, target,
          TYPE_UNSIGNED(TREE_TYPE(TREE_OPERAND(exp, 0))));
    ...
    case VEC_WIDEN_MULT_LO_EXPR: {
      ...
      expand_operands(oprnd0, oprnd1, NULL_RTX, &op0, &op1, 0);
      target =
          expand_widen_pattern_expr(exp, op0, op1, NULL_RTX, target, unsignedp);
      gcc_assert(target);
      return target;
    }
...
binop:
  expand_operands(TREE_OPERAND(exp, 0), TREE_OPERAND(exp, 1), subtarget, &op0,
                  &op1, 0);
binop2:
  this_optab = optab_for_tree_code(code, type, optab_default);
binop3:
...
static rtx reduce_to_bit_field_precision(rtx exp, rtx target, tree type) {
  ...
  if (GET_CODE(exp) == CONST_INT) {
    ...
    if (prec < HOST_BITS_PER_WIDE_INT)
      ...
      mask = immed_double_const(
          (unsigned HOST_WIDE_INT) - 1,
          ((unsigned HOST_WIDE_INT)1 << (prec - HOST_BITS_PER_WIDE_INT)) - 1,
          GET_MODE(exp));
    return expand_and(GET_MODE(exp), exp, mask, target);
  } else {
    tree count =
        build_int_cst(NULL_TREE, GET_MODE_BITSIZE(GET_MODE(exp)) - prec);
...
static int is_aligning_offset(const_tree offset, const_tree exp) {
...
tree string_constant(tree arg, tree *ptr_offset) {
  ...
  if (TREE_CODE(arg) == ADDR_EXPR) {
    if (TREE_CODE(TREE_OPERAND(arg, 0)) == STRING_CST) {
      *ptr_offset = size_zero_node;
      ...
      if (TREE_CODE(array) != STRING_CST && TREE_CODE(array) != VAR_DECL)
        return 0;
      ...
      return 0;
             ...
             TREE_CODE(arg) == POINTER_PLUS_EXPR) {
  ...
  if (TREE_CODE(array) == STRING_CST) {
    *ptr_offset = fold_convert(sizetype, offset);
    ...
    if (!TREE_READONLY(array) || TREE_SIDE_EFFECTS(array) ||
        !targetm.binds_local_p(array))
      return 0;
    ...
    if (compare_tree_int(DECL_SIZE_UNIT(array), length) > 0 &&
        (!host_integerp(offset, 1) || compare_tree_int(offset, length) >= 0))
      return 0;
...
static rtx do_store_flag(tree exp, rtx target, enum machine_mode mode,
                         int only_cheap) {
  enum rtx_code code;
  tree arg0, arg1, type;
  tree tem;
  ...
  if ((code == NE || code == EQ) && TREE_CODE(arg0) == BIT_AND_EXPR &&
      integer_zerop(arg1) && integer_pow2p(TREE_OPERAND(arg0, 1))) {
    tree type = lang_hooks.types.type_for_mode(mode, unsignedp);
    return expand_expr(
        fold_single_bit_test(code == NE ? NE_EXPR : EQ_EXPR, arg0, arg1, type),
  ...
  if (icode == CODE_FOR_nothing) {
    ...
    for (wmode = operand_mode; icode == CODE_FOR_nothing && wmode != VOIDmode;
  ...
  if (result) {
    if (invert)
      result = expand_binop(mode, xor_optab, result, const1_rtx, result, 0,
                            OPTAB_LIB_WIDEN);
    return result;
  }

  /* If this failed, we have to do this with set/compare/jump/set code.  */
  if (!REG_P(target) || reg_mentioned_p(target, op0) ||
...
int try_casesi(tree index_type, tree index_expr, tree minval, tree range,
  ...
  if (GET_MODE_BITSIZE(TYPE_MODE(index_type)) > GET_MODE_BITSIZE(index_mode)) {
    ...
    if (default_label)
      emit_cmp_and_jump_insns(rangertx, index, LTU, NULL_RTX, omode, 1,
                              default_label);
  ...
  if (!(*insn_data[(int)CODE_FOR_casesi].operand[0].predicate)(index, op_mode))
    index = copy_to_mode_reg(op_mode, index);

  op1 = expand_normal(minval);

  op_mode = insn_data[(int)CODE_FOR_casesi].operand[1].mode;
  op1 = convert_modes(op_mode, TYPE_MODE(TREE_TYPE(minval)), op1,
                      TYPE_UNSIGNED(TREE_TYPE(minval)));
  if (!(*insn_data[(int)CODE_FOR_casesi].operand[1].predicate)(op1, op_mode))
    op1 = copy_to_mode_reg(op_mode, op1);

  op2 = expand_normal(range);

  op_mode = insn_data[(int)CODE_FOR_casesi].operand[2].mode;
  op2 = convert_modes(op_mode, TYPE_MODE(TREE_TYPE(range)), op2,
                      TYPE_UNSIGNED(TREE_TYPE(range)));
  if (!(*insn_data[(int)CODE_FOR_casesi].operand[2].predicate)(op2, op_mode))
    op2 = copy_to_mode_reg(op_mode, op2);

  emit_jump_insn(gen_casesi(index, op1, op2, table_label,
...
static void do_tablejump(rtx index, enum machine_mode mode, rtx range,
...
int try_tablejump(tree index_type, tree index_expr, tree minval, tree range,
                  rtx table_label, rtx default_label) {
  rtx index;
  ...
  index_expr =
      fold_build2(MINUS_EXPR, index_type, fold_convert(index_type, index_expr),
                  fold_convert(index_type, minval));
  index = expand_normal(index_expr);
  do_pending_stack_adjust();

  do_tablejump(
      index, TYPE_MODE(index_type),
      convert_modes(TYPE_MODE(index_type), TYPE_MODE(TREE_TYPE(range)),
                    expand_normal(range), TYPE_UNSIGNED(TREE_TYPE(range))),
...
int vector_mode_valid_p(enum machine_mode mode) {
  ...
  if (mclass != MODE_VECTOR_INT && mclass != MODE_VECTOR_FLOAT &&
      ...
      mclass != MODE_VECTOR_ACCUM && mclass != MODE_VECTOR_UACCUM)
    return 0;
...
static rtx const_vector_from_tree(tree exp) {
  ...
  for (i = 0; link; link = TREE_CHAIN(link), ++i) {
    ...
    if (TREE_CODE(elt) == REAL_CST)
      RTVEC_ELT(v, i) = CONST_DOUBLE_FROM_REAL_VALUE(TREE_REAL_CST(elt), inner);
    else if (TREE_CODE(elt) == FIXED_CST)
      RTVEC_ELT(v, i) =
          CONST_FIXED_FROM_FIXED_VALUE(TREE_FIXED_CST(elt), inner);
    else
      RTVEC_ELT(v, i) = immed_double_const(TREE_INT_CST_LOW(elt),
                                           TREE_INT_CST_HIGH(elt), inner);
  }
