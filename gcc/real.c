...
static bool sticky_rshift_significand(REAL_VALUE_TYPE *r,
  ...
  if (n != 0) {
    ...
    for (i = 0; i < SIGSZ; ++i) {
      r->sig[i] = (((ofs + i >= SIGSZ ? 0 : a->sig[ofs + i]) >> n) |
...
static void rshift_significand(REAL_VALUE_TYPE *r, const REAL_VALUE_TYPE *a,
                               unsigned int n) {
...
static void lshift_significand(REAL_VALUE_TYPE *r, const REAL_VALUE_TYPE *a,
                               unsigned int n) {
  unsigned int i, ofs = n / HOST_BITS_PER_LONG;
...
static inline void lshift_significand_1(REAL_VALUE_TYPE *r,
                                        const REAL_VALUE_TYPE *a) {
  unsigned int i;
...
static inline bool add_significands(REAL_VALUE_TYPE *r,
                                    const REAL_VALUE_TYPE *a,
...
static inline bool sub_significands(REAL_VALUE_TYPE *r,
                                    const REAL_VALUE_TYPE *a,
...
static inline void neg_significand(REAL_VALUE_TYPE *r,
                                   const REAL_VALUE_TYPE *a) {
  bool carry = true;
  int i;
...
static inline int cmp_significands(const REAL_VALUE_TYPE *a,
                                   const REAL_VALUE_TYPE *b) {
  int i;
...
static inline int cmp_significand_0(const REAL_VALUE_TYPE *a) {
  int i;
...
static inline void set_significand_bit(REAL_VALUE_TYPE *r, unsigned int n) {
  r->sig[n / HOST_BITS_PER_LONG] |= (unsigned long)1
                                    << (n % HOST_BITS_PER_LONG);
}
...
static inline bool div_significands(REAL_VALUE_TYPE *r,
                                    const REAL_VALUE_TYPE *a,
                                    const REAL_VALUE_TYPE *b) {
  REAL_VALUE_TYPE u;
...
static bool do_add(REAL_VALUE_TYPE *r, const REAL_VALUE_TYPE *a,
...
static bool do_multiply(REAL_VALUE_TYPE *r, const REAL_VALUE_TYPE *a,
                        const REAL_VALUE_TYPE *b) {
...
static bool do_divide(REAL_VALUE_TYPE *r, const REAL_VALUE_TYPE *a,
                      const REAL_VALUE_TYPE *b) {
...
static int do_compare(const REAL_VALUE_TYPE *a, const REAL_VALUE_TYPE *b,
                      int nan_result) {
  int ret;
...
bool real_can_shorten_arithmetic(enum machine_mode imode,
  ...
  /* These conditions are conservative rather than trying to catch the
     ...
     and double.  */
  return (ifmt->b == tfmt->b && ifmt->p > 2 * tfmt->p &&
          ...
          ifmt->has_nans >= tfmt->has_nans && ifmt->has_inf >= tfmt->has_inf &&
          ifmt->has_signed_zero >= tfmt->has_signed_zero &&
          !MODE_COMPOSITE_P(tmode) && !MODE_COMPOSITE_P(imode));
}
...
void real_to_integer2(HOST_WIDE_INT *plow, HOST_WIDE_INT *phigh,
...
void real_to_decimal(char *str, const REAL_VALUE_TYPE *r_orig, size_t buf_size,
  ...
  real_to_decimal_for_mode(str, r_orig, buf_size, digits, crop_trailing_zeros,
                           VOIDmode);
}
  ...
  if (str[0] == '0' && (str[1] == 'x' || str[1] == 'X')) {
    ...
    while (1) {
      ...
      if (d == _hex_bad)
        break;
      if (pos >= 0) {
        r->sig[pos / HOST_BITS_PER_LONG] |= (unsigned long)d
                                            << (pos % HOST_BITS_PER_LONG);
        pos -= 4;
      } else if (d)
...
void real_from_string3(REAL_VALUE_TYPE *r, const char *s,
...
void real_from_integer(REAL_VALUE_TYPE *r, enum machine_mode mode,
...
void real_2expN(REAL_VALUE_TYPE *r, int n, enum machine_mode fmode) {
  memset(r, 0, sizeof(*r));
...
void real_convert(REAL_VALUE_TYPE *r, enum machine_mode mode,
                  const REAL_VALUE_TYPE *a) {
  const struct real_format *fmt;
...
long real_to_target_fmt(long *buf, const REAL_VALUE_TYPE *r_orig,
                        const struct real_format *fmt) {
  REAL_VALUE_TYPE r;
  long buf1;
...
long real_to_target(long *buf, const REAL_VALUE_TYPE *r,
                    enum machine_mode mode) {
  const struct real_format *fmt;
...
void real_from_target_fmt(REAL_VALUE_TYPE *r, const long *buf,
                          const struct real_format *fmt) {
...
void real_from_target(REAL_VALUE_TYPE *r, const long *buf,
                      enum machine_mode mode) {
  const struct real_format *fmt;
...
static void encode_ieee_single(const struct real_format *fmt, long *,
                               const REAL_VALUE_TYPE *);
static void decode_ieee_single(const struct real_format *, REAL_VALUE_TYPE *,
                               const long *);

static void encode_ieee_single(const struct real_format *fmt, long *buf,
                               const REAL_VALUE_TYPE *r) {
...
static void decode_ieee_single(const struct real_format *fmt,
  ...
  if (exp == 0) {
    ...
    if (image) {
      ...
      r->sign = sign;
      r->signalling =
          (((image >> (HOST_BITS_PER_LONG - 2)) & 1) ^ fmt->qnan_msb_set);
      r->sig[SIGSZ - 1] = image;
    } else {
      r->cl = rvc_inf;
      r->sign = sign;
...
const struct real_format ieee_single_format = {encode_ieee_single,
                                               decode_ieee_single,
                                               2,
                                               24,
                                               24,
...
const struct real_format mips_single_format = {encode_ieee_single,
                                               decode_ieee_single,
                                               2,
                                               24,
                                               24,
...
const struct real_format motorola_single_format = {encode_ieee_single,
                                                   decode_ieee_single,
                                                   2,
                                                   24,
                                                   24,
...
const struct real_format spu_single_format = {encode_ieee_single,
                                              decode_ieee_single,
                                              2,
                                              24,
                                              24,
...
static void encode_ieee_double(const struct real_format *fmt, long *,
                               const REAL_VALUE_TYPE *);
static void decode_ieee_double(const struct real_format *, REAL_VALUE_TYPE *,
                               const long *);

static void encode_ieee_double(const struct real_format *fmt, long *buf,
                               const REAL_VALUE_TYPE *r) {
...
const struct real_format ieee_double_format = {encode_ieee_double,
                                               decode_ieee_double,
                                               2,
                                               53,
                                               53,
...
const struct real_format mips_double_format = {encode_ieee_double,
                                               decode_ieee_double,
                                               2,
                                               53,
                                               53,
...
const struct real_format motorola_double_format = {encode_ieee_double,
                                                   decode_ieee_double,
                                                   2,
                                                   53,
                                                   53,
...
static void encode_ieee_extended(const struct real_format *fmt, long *buf,
                                 const REAL_VALUE_TYPE *r) {
...
static void encode_ibm_extended(const struct real_format *fmt, long *,
                                const REAL_VALUE_TYPE *);
static void decode_ibm_extended(const struct real_format *, REAL_VALUE_TYPE *,
                                const long *);

static void encode_ibm_extended(const struct real_format *fmt, long *buf,
                                const REAL_VALUE_TYPE *r) {
...
const struct real_format ibm_extended_format = {encode_ibm_extended,
                                                decode_ibm_extended,
                                                2,
                                                53 + 53,
                                                53,
...
const struct real_format mips_extended_format = {encode_ibm_extended,
                                                 decode_ibm_extended,
                                                 2,
                                                 53 + 53,
                                                 53,
...
static void encode_ieee_quad(const struct real_format *fmt, long *,
                             const REAL_VALUE_TYPE *);
static void decode_ieee_quad(const struct real_format *, REAL_VALUE_TYPE *,
                             const long *);

static void encode_ieee_quad(const struct real_format *fmt, long *buf,
                             const REAL_VALUE_TYPE *r) {
...
static void decode_ieee_quad(const struct real_format *fmt, REAL_VALUE_TYPE *r,
                             const long *buf) {
...
const struct real_format ieee_quad_format = {encode_ieee_quad,
                                             decode_ieee_quad,
                                             2,
                                             113,
                                             113,
...
const struct real_format mips_quad_format = {encode_ieee_quad,
                                             decode_ieee_quad,
                                             2,
                                             113,
                                             113,
...
static void encode_vax_f(const struct real_format *fmt, long *,
                         const REAL_VALUE_TYPE *);
static void decode_vax_f(const struct real_format *, REAL_VALUE_TYPE *,
                         const long *);
static void encode_vax_d(const struct real_format *fmt, long *,
                         const REAL_VALUE_TYPE *);
static void decode_vax_d(const struct real_format *, REAL_VALUE_TYPE *,
                         const long *);
static void encode_vax_g(const struct real_format *fmt, long *,
                         const REAL_VALUE_TYPE *);
static void decode_vax_g(const struct real_format *, REAL_VALUE_TYPE *,
                         const long *);
...
const struct real_format vax_f_format = {
    encode_vax_f, decode_vax_f, 2,     24,    24,    -127,  127,   15,   15,
    false,        false,        false, false, false, false, false, false};

const struct real_format vax_d_format = {
    encode_vax_d, decode_vax_d, 2,     56,    56,    -127,  127,   15,   15,
    false,        false,        false, false, false, false, false, false};

const struct real_format vax_g_format = {
    encode_vax_g, decode_vax_g, 2,     53,    53,    -1023, 1023,  15,   15,
    false,        false,        false, false, false, false, false, false};

/* Encode real R into a single precision DFP value in BUF.  */
static void encode_decimal_single(
    const struct real_format *fmt ATTRIBUTE_UNUSED, long *buf ATTRIBUTE_UNUSED,
...
static void decode_decimal_single(
...
static void encode_decimal_double(
    const struct real_format *fmt ATTRIBUTE_UNUSED, long *buf ATTRIBUTE_UNUSED,
    const REAL_VALUE_TYPE *r ATTRIBUTE_UNUSED) {
...
static void decode_decimal_double(
    const struct real_format *fmt ATTRIBUTE_UNUSED,
    REAL_VALUE_TYPE *r ATTRIBUTE_UNUSED, const long *buf ATTRIBUTE_UNUSED) {
...
static void encode_decimal_quad(const struct real_format *fmt ATTRIBUTE_UNUSED,
...
static void decode_decimal_quad(const struct real_format *fmt ATTRIBUTE_UNUSED,
...
const struct real_format decimal_single_format = {encode_decimal_single,
...
const struct real_format decimal_double_format = {encode_decimal_double,
...
const struct real_format decimal_quad_format = {encode_decimal_quad,
                                                decode_decimal_quad,
                                                10,
                                                34,
                                                34,
...
static void encode_internal(const struct real_format *fmt, long *,
                            const REAL_VALUE_TYPE *);
static void decode_internal(const struct real_format *, REAL_VALUE_TYPE *,
                            const long *);
...
const struct real_format real_internal_format = {encode_internal,
                                                 decode_internal,
                                                 2,
                                                 SIGNIFICAND_BITS - 2,
                                                 SIGNIFICAND_BITS - 2,
...
bool real_sqrt(REAL_VALUE_TYPE *r, enum machine_mode mode,
...
bool real_powi(REAL_VALUE_TYPE *r, enum machine_mode mode,
...
void real_trunc(REAL_VALUE_TYPE *r, enum machine_mode mode,
                const REAL_VALUE_TYPE *x) {
  do_fix_trunc(r, x);
  if (mode != VOIDmode)
    real_convert(r, mode, r);
...
void real_floor(REAL_VALUE_TYPE *r, enum machine_mode mode,
                const REAL_VALUE_TYPE *x) {
  REAL_VALUE_TYPE t;
...
void real_ceil(REAL_VALUE_TYPE *r, enum machine_mode mode,
               const REAL_VALUE_TYPE *x) {
  REAL_VALUE_TYPE t;
...
void real_round(REAL_VALUE_TYPE *r, enum machine_mode mode,
                const REAL_VALUE_TYPE *x) {
...
void mpfr_from_real(mpfr_ptr m, const REAL_VALUE_TYPE *r, mp_rnd_t rndmode) {
...
void real_from_mpfr(REAL_VALUE_TYPE *r, mpfr_srcptr m, tree type,
                    mp_rnd_t rndmode) {
  /* We use a string as an intermediate type.  */
  char buf[128], *rstr;
  mp_exp_t exp;
