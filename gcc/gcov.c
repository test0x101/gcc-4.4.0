...
static void fnotice(FILE *file, const char *cmsgid, ...) {
  va_list ap;
...
static void print_usage(int error_p) {
  ...
  fnotice(file,
          "  -a, --all-blocks                Show information for every basic "
          "block\n");
  fnotice(file,
          "  -b, --branch-probabilities      Include branch probabilities in "
          "output\n");
  fnotice(file,
...
static void generate_results(const char *file_name) {
  source_t *src;
  function_t *fn;
...
static source_t *find_source(const char *file_name) {
  source_t *src;
  struct stat status;
  ...
  if (src->file_time > bbg_file_time) {
    ...
    fnotice(stderr, "%s:source file is newer than graph file '%s'\n", src->name,
            bbg_file_name);
    if (!info_emitted) {
      fnotice(stderr, "(the message is only displayed one per source file)\n");
      info_emitted = 1;
    }
    src->file_time = 0;
  }
...
static int read_graph_file(void) {
  ...
  while ((tag = gcov_read_unsigned())) {
    ...
    if (tag == GCOV_TAG_FUNCTION) {
      ...
      for (probe = src->functions, prev = NULL; probe && probe->line > lineno;
  ...
  /* Reverse functions.  */
  {
    ...
    for (fn_p = old_functions_head, fn = functions; fn != old_functions_head;
         fn_p = fn, fn = fn_n) {
      unsigned ix;
      ...
      for (arc = blk->succ; arc; arc = arc->succ_next)
        if (!arc->fake) {
          ...
          /* If this block is instrumenting a call, it might be
             ...
             hold.  */
          if (blk->is_call_site && arc->fall_through && arc->dst->pred == arc &&
              !arc->pred_next)
...
static void add_line_counts(coverage_t *coverage, function_t *fn) {
  ...
  for (ix = 0; ix != fn->num_blocks; ix++) {
    ...
    if (block->count && ix && ix + 1 != fn->num_blocks)
      fn->blocks_executed++;
    for (jx = 0, encoding = block->u.line.encoding; jx != block->u.line.num;
         jx++, encoding++)
      if (!*encoding) {
  ...
  if (arc->is_call_non_return) {
    if (arc->src->count) {
      fnotice(gcov_file, "call   %2d returned %s\n", ix,
              format_gcov(arc->src->count - arc->count, arc->src->count,
                          -flag_counts));
    } else
...
static void output_lines(FILE *gcov_file, const source_t *src) {
  ...
  for (line_num = 1, line = &src->lines[line_num]; line_num < src->num_lines;
       line_num++, line++) {
    for (; fn && fn->line == line_num; fn = fn->line_next) {
      ...
      fprintf(gcov_file, " called %s", format_gcov(fn->blocks[0].count, 0, -1));
