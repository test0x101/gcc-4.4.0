...
static struct mem_ref_group *find_or_create_group(struct mem_ref_group **groups,
                                                  tree base,
                                                  HOST_WIDE_INT step) {
  struct mem_ref_group *group;

  for (; *groups; groups = &(*groups)->next) {
    if ((*groups)->step == step && operand_equal_p((*groups)->base, base, 0))
      return *groups;
...
static void record_ref(struct mem_ref_group *group, gimple stmt, tree mem,
  ...
  for (aref = &group->refs; *aref; aref = &(*aref)->next) {
    ...
    if (!WRITE_CAN_USE_READ_PREFETCH && write_p && !(*aref)->write_p)
      continue;
    if (!READ_CAN_USE_WRITE_PREFETCH && !write_p && (*aref)->write_p)
      continue;
...
static bool idx_analyze_ref(tree base, tree *index, void *data) {
  ...
  if (!simple_iv(ar_data->loop, loop_containing_stmt(ar_data->stmt), *index,
                 &iv, false))
    return false;
  ibase = iv.base;
  step = iv.step;
...
static bool analyze_ref(struct loop *loop, tree *ref_p, tree *base,
...
static bool gather_memory_references_ref(struct loop *loop,
                                         struct mem_ref_group **refs, tree ref,
                                         bool write_p, gimple stmt) {
  tree base;
...
static struct mem_ref_group *gather_memory_references(struct loop *loop,
                                                      bool *no_other_refs) {
    ...
    for (bsi = gsi_start_bb(bb); !gsi_end_p(bsi); gsi_next(&bsi)) {
      ...
      if (gimple_code(stmt) != GIMPLE_ASSIGN) {
        if (!ZERO_SSA_OPERANDS(stmt, SSA_OP_ALL_VIRTUALS) ||
            (is_gimple_call(stmt) && !(gimple_call_flags(stmt) & ECF_CONST)))
          *no_other_refs = false;
        continue;
      }
...
static void prune_ref_by_group_reuse(struct mem_ref *ref, struct mem_ref *by,
                                     bool by_is_before) {
  HOST_WIDE_INT step = ref->group->step;
  bool backward = step < 0;
  ...
  if (!step) {
    ...
    if (ddown(ref->delta, PREFETCH_BLOCK) != ddown(by->delta, PREFETCH_BLOCK))
      return;
  ...
  while ((step & 1) == 0 && prefetch_block > 1) {
    step >>= 1;
    prefetch_block >>= 1;
    delta >>= 1;
  }
...
static void prune_ref_by_reuse(struct mem_ref *ref, struct mem_ref *refs) {
  ...
  for (prune_by = refs; prune_by; prune_by = prune_by->next) {
    ...
    if (!WRITE_CAN_USE_READ_PREFETCH && ref->write_p && !prune_by->write_p)
      continue;
    if (!READ_CAN_USE_WRITE_PREFETCH && !ref->write_p && prune_by->write_p)
      continue;
    ...
    if (dump_file && (dump_flags & TDF_DETAILS)) {
      ...
      if (ref_pruned->prefetch_before == PREFETCH_ALL &&
        ...
        if (ref_pruned->prefetch_mod != 1) {
          ...
          fprintf(dump_file, HOST_WIDE_INT_PRINT_DEC, ref_pruned->prefetch_mod);
        }
      }
      fprintf(dump_file, "\n");
    }
  ...
  for (; groups; groups = groups->next)
    for (ref = groups->refs; ref; ref = ref->next) {
      ...
      /* If we need to prefetch the reference each PREFETCH_MOD iterations,
         ...
         iteration.  */
      n_prefetches =
          ((unroll_factor + ref->prefetch_mod - 1) / ref->prefetch_mod);
...
static bool anything_to_prefetch_p(struct mem_ref_group *groups) {
  struct mem_ref *ref;
...
static void issue_prefetch_ref(struct mem_ref *ref, unsigned unroll_factor,
  ...
  for (ap = 0; ap < n_prefetches; ap++) {
    ...
    addr = fold_build2(POINTER_PLUS_EXPR, ptr_type_node, addr_base,
                       size_int(delta));
    addr = force_gimple_operand_gsi(&bsi, unshare_expr(addr), true, NULL, true,
                                    GSI_SAME_STMT);

    /* Create the prefetch instruction.  */
    prefetch = gimple_build_call(built_in_decls[BUILT_IN_PREFETCH], 3, addr,
                                 write_p, local);
...
static bool nontemporal_store_p(struct mem_ref *ref) {
  ...
  if (!ref->write_p || !ref->independent_p ||
...
static void emit_mfence_after_loop(struct loop *loop) {
...
static bool may_use_storent_in_loop_p(struct loop *loop) {
  bool ret = true;
  ...
  if (FENCE_FOLLOWING_MOVNT != NULL_TREE) {
    ...
    for (i = 0; VEC_iterate(edge, exits, i, exit); i++)
      if ((exit->flags & EDGE_ABNORMAL) && exit->dest == EXIT_BLOCK_PTR)
        ret = false;
...
static void mark_nontemporal_stores(struct loop *loop,
...
static unsigned volume_of_dist_vector(lambda_vector vec, unsigned *loop_sizes,
                                      unsigned n) {
  unsigned i;
...
static void add_subscript_strides(tree access_fn, unsigned stride,
                                  HOST_WIDE_INT *strides, unsigned n,
                                  struct loop *loop) {
  struct loop *aloop;
  tree step;
  HOST_WIDE_INT astep;
...
static unsigned self_reuse_distance(data_reference_p dr, unsigned *loop_sizes,
                                    unsigned n, struct loop *loop) {
...
static void determine_loop_nest_reuse(struct loop *loop,
                                      struct mem_ref_group *refs,
                                      bool no_other_refs) {
  ...
  while (1) {
    ...
    if (aloop == current_loops->tree_root || aloop->inner->next)
      break;
  ...
  for (i = 0; VEC_iterate(ddr_p, dependences, i, dep); i++) {
    ...
    if (DDR_ARE_DEPENDENT(dep) == chrec_dont_know ||
      ...
      for (j = 0; j < DDR_NUM_DIST_VECTS(dep); j++) {
        adist = volume_of_dist_vector(DDR_DIST_VECT(dep, j), loop_data_size, n);
        ...
        if (lambda_vector_zerop(DDR_DIST_VECT(dep, j), n - 1) &&
            (ref != refb || DDR_DIST_VECT(dep, j)[n - 1] != 0)) {
          ref->independent_p = false;
          refb->independent_p = false;
        }
...
static bool loop_prefetch_arrays(struct loop *loop) {
  struct mem_ref_group *refs;
...
unsigned int tree_ssa_prefetch_arrays(void) {
  ...
  if (!built_in_decls[BUILT_IN_PREFETCH]) {
    ...
    tree decl =
        add_builtin_function("__builtin_prefetch", type, BUILT_IN_PREFETCH,
