...
static hashval_t hash_statistics_hash(const void *p) {
...
static int hash_statistics_eq(const void *p, const void *q) {
...
static void hash_statistics_free(void *p) {
...
static htab_t curr_statistics_hash(void) {
  unsigned idx = current_pass->static_pass_number;

  if (idx < nr_statistics_hashes && statistics_hashes[idx] != NULL)
  ...
  statistics_hashes[idx] = htab_create(
      15, hash_statistics_hash, hash_statistics_eq, hash_statistics_free);
...
static int statistics_fini_pass_1(void **slot, void *data ATTRIBUTE_UNUSED) {
...
static int statistics_fini_pass_2(void **slot, void *data ATTRIBUTE_UNUSED) {
  statistics_counter_t *counter = (statistics_counter_t *)*slot;
  unsigned HOST_WIDE_INT count = counter->count - counter->prev_dumped_count;
  if (count == 0)
    return 1;
  ...
  if (counter->histogram_p)
    fprintf(statistics_dump_file,
            ...
            current_pass->static_pass_number, current_pass->name, counter->id,
            counter->val,
            cfun ? IDENTIFIER_POINTER(DECL_NAME(cfun->decl)) : "(nofn)", count);
  else
    fprintf(statistics_dump_file,
...
static int statistics_fini_pass_3(void **slot, void *data ATTRIBUTE_UNUSED) {
  statistics_counter_t *counter = (statistics_counter_t *)*slot;
  counter->prev_dumped_count = counter->count;
  return 1;
}
...
void statistics_fini_pass(void) {
  ...
  if (dump_file && dump_flags & TDF_STATS) {
    ...
    fprintf(dump_file, "----------------\n");
    htab_traverse_noresize(curr_statistics_hash(), statistics_fini_pass_1,
                           NULL);
    fprintf(dump_file, "\n");
  }
  if (statistics_dump_file && !(statistics_dump_flags & TDF_STATS ||
                                statistics_dump_flags & TDF_DETAILS))
    htab_traverse_noresize(curr_statistics_hash(), statistics_fini_pass_2,
                           NULL);
  htab_traverse_noresize(curr_statistics_hash(), statistics_fini_pass_3, NULL);
}

/* Helper for printing summary information.  */

static int statistics_fini_1(void **slot, void *data) {
  ...
  if (counter->histogram_p)
    fprintf(statistics_dump_file,
            ...
            pass->static_pass_number, pass->name, counter->id, counter->val,
            counter->count);
  else
    fprintf(statistics_dump_file, "%d %s \"%s\" " HOST_WIDE_INT_PRINT_DEC "\n",
            pass->static_pass_number, pass->name, counter->id, counter->count);
  return 1;
}
...
void statistics_fini(void) {
  ...
  if (statistics_dump_flags & TDF_STATS) {
    ...
    for (i = 0; i < nr_statistics_hashes; ++i)
      if (statistics_hashes[i] != NULL && get_pass_for_id(i) != NULL)
        htab_traverse_noresize(statistics_hashes[i], statistics_fini_1,
                               get_pass_for_id(i));
  }
...
void statistics_early_init(void) {
  statistics_dump_nr =
      dump_register(".statistics", "statistics", "statistics", TDF_TREE);
}
...
static statistics_counter_t *lookup_or_add_counter(htab_t hash, const char *id,
                                                   int val, bool histogram_p) {
...
void statistics_counter_event(struct function *fn, const char *id, int incr) {
  ...
  if ((!(dump_flags & TDF_STATS) && !statistics_dump_file) || incr == 0)
    return;
  ...
  if (!statistics_dump_file || !(statistics_dump_flags & TDF_DETAILS))
    return;

  fprintf(statistics_dump_file, "%d %s \"%s\" \"%s\" %d\n",
          current_pass->static_pass_number, current_pass->name, id,
          fn ? IDENTIFIER_POINTER(DECL_NAME(fn->decl)) : "(nofn)", incr);
}
...
void statistics_histogram_event(struct function *fn, const char *id, int val) {
  ...
  if (!(dump_flags & TDF_STATS) && !statistics_dump_file)
    return;
  ...
  fprintf(statistics_dump_file, "%d %s \"%s == %d\" \"%s\" 1\n",
          current_pass->static_pass_number, current_pass->name, id, val,
