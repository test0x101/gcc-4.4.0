...
static void gen_addr_rtx(rtx symbol, rtx base, rtx index, rtx step, rtx offset,
  ...
  *addr = NULL_RTX;
  ...
  if (symbol) {
    ...
    if (offset) {
      ...
      if (GET_CODE(symbol) == SYMBOL_REF || GET_CODE(symbol) == LABEL_REF ||
...
rtx addr_for_mem_ref(struct mem_address *addr, bool really_expand) {
  ...
  if (!really_expand) {
    ...
    if (!templates_initialized) {
      ...
      for (i = 0; i < 32; i++)
        gen_addr_rtx((i & 16 ? sym : NULL_RTX), (i & 8 ? bse : NULL_RTX),
                     (i & 4 ? idx : NULL_RTX), (i & 2 ? const0_rtx : NULL_RTX),
                     (i & 1 ? const0_rtx : NULL_RTX), &templates[i].ref,
                     &templates[i].step_p, &templates[i].off_p);
    }

    templ =
        templates + TEMPL_IDX(addr->symbol, addr->base, addr->index, st, off);
    if (st)
      *templ->step_p = st;
    if (off)
      *templ->off_p = off;
  ...
  sym = (addr->symbol
             ...
             : NULL_RTX);
  bse = (addr->base ? expand_expr(addr->base, NULL_RTX, Pmode, EXPAND_NORMAL)
                    : NULL_RTX);
  idx = (addr->index ? expand_expr(addr->index, NULL_RTX, Pmode, EXPAND_NORMAL)
                     : NULL_RTX);
...
static bool valid_mem_ref_p(enum machine_mode mode, struct mem_address *addr) {
  rtx address;
...
static tree create_mem_ref_raw(tree type, struct mem_address *addr) {
  ...
  return build7(TARGET_MEM_REF, type, addr->symbol, addr->base, addr->index,
...
static bool fixed_address_object_p(tree obj) {
  return (TREE_CODE(obj) == VAR_DECL &&
          (TREE_STATIC(obj) || DECL_EXTERNAL(obj)) && !DECL_DLLIMPORT_P(obj));
}

/* If ADDR contains an address of object that is a link time constant,
   move it to PARTS->symbol.  */

static void move_fixed_address_to_symbol(struct mem_address *parts,
                                         aff_tree *addr) {
  unsigned i;
  tree val = NULL_TREE;
...
static void add_to_parts(struct mem_address *parts, tree elt) {
  ...
  if (POINTER_TYPE_P(type))
    parts->base = fold_build2(POINTER_PLUS_EXPR, type, parts->base,
...
static void most_expensive_mult_to_index(struct mem_address *parts,
  ...
  for (i = 0; i < addr->n; i++) {
    ...
    if (coef == 1 || !multiplier_allowed_in_address_p(coef, Pmode))
      continue;
...
static void addr_to_parts(aff_tree *addr, struct mem_address *parts,
                          bool speed) {
  tree part;
  unsigned i;
...
static void gimplify_mem_ref_parts(gimple_stmt_iterator *gsi,
  ...
  if (parts->base)
    parts->base = force_gimple_operand_gsi(gsi, parts->base, true, NULL_TREE,
                                           true, GSI_SAME_STMT);
  if (parts->index)
    parts->index = force_gimple_operand_gsi(gsi, parts->index, true, NULL_TREE,
                                            true, GSI_SAME_STMT);
}
...
tree create_mem_ref(gimple_stmt_iterator *gsi, tree type, aff_tree *addr,
                    bool speed) {
  tree mem_ref, tmp;
  tree atype;
  struct mem_address parts;
  ...
  if (parts.step && !integer_onep(parts.step)) {
    ...
    parts.index = force_gimple_operand_gsi(
        gsi, fold_build2(MULT_EXPR, sizetype, parts.index, parts.step), true,
  ...
  if (parts.symbol) {
    ...
    if (parts.base) {
      ...
      if (parts.index) {
        atype = TREE_TYPE(tmp);
        parts.base = force_gimple_operand_gsi(
            gsi,
            fold_build2(POINTER_PLUS_EXPR, atype, tmp,
  ...
  if (parts.index) {
    ...
    if (parts.base) {
      ...
      parts.base = force_gimple_operand_gsi(
          gsi, fold_build2(POINTER_PLUS_EXPR, atype, parts.base, parts.index),
          true, NULL_TREE, true, GSI_SAME_STMT);
    } else
  ...
  if (parts.offset && !integer_zerop(parts.offset)) {
    ...
    if (parts.base) {
      ...
      parts.base = force_gimple_operand_gsi(
          gsi,
          fold_build2(POINTER_PLUS_EXPR, atype, parts.base,
...
void copy_mem_ref_info(tree to, tree from) {
...
tree maybe_fold_tmr(tree ref) {
  ...
  if (addr.base && TREE_CODE(addr.base) == INTEGER_CST) {
    if (addr.offset)
      addr.offset = fold_binary_to_constant(PLUS_EXPR, sizetype, addr.offset,
  ...
  if (addr.index && TREE_CODE(addr.index) == INTEGER_CST) {
    off = addr.index;
    if (addr.step) {
      off = fold_binary_to_constant(MULT_EXPR, sizetype, off, addr.step);
      addr.step = NULL_TREE;
    }

    if (addr.offset) {
      addr.offset =
          fold_binary_to_constant(PLUS_EXPR, sizetype, addr.offset, off);
    } else
      addr.offset = off;
...
void dump_mem_address(FILE *file, struct mem_address *parts) {
  if (parts->symbol) {
    fprintf(file, "symbol: ");
