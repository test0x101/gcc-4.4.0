...
static void builtin_define_with_hex_fp_value(const char *, tree, int,
...
static void builtin_define_type_sizeof(const char *name, tree type) {
  builtin_define_with_int_value(name, tree_low_cst(TYPE_SIZE_UNIT(type), 1));
}
...
static void builtin_define_float_constants(const char *name_prefix,
  ...
  */
  ...
  builtin_define_with_int_value(name, MODE_HAS_INFINITIES(TYPE_MODE(type)));
...
static void builtin_define_fixed_point_constants(const char *name_prefix,
                                                 const char *suffix,
                                                 tree type) {
...
void c_cpp_builtins_optimize_pragma(cpp_reader *pfile, tree prev_tree,
                                    tree cur_tree) {
...
void c_cpp_builtins(cpp_reader *pfile) {
  ...
  if (flag_abi_version == 0)
    ...
    builtin_define_with_int_value("__GXX_ABI_VERSION", 1000 + flag_abi_version);
  ...
  builtin_define_with_int_value("__FLT_EVAL_METHOD__", TARGET_FLT_EVAL_METHOD);
  ...
  if (targetm.fixed_point_supported_p()) {
    builtin_define_fixed_point_constants("SFRACT", "HR", short_fract_type_node);
    builtin_define_fixed_point_constants("USFRACT", "UHR",
                                         unsigned_short_fract_type_node);
    builtin_define_fixed_point_constants("FRACT", "R", fract_type_node);
    builtin_define_fixed_point_constants("UFRACT", "UR",
                                         unsigned_fract_type_node);
    builtin_define_fixed_point_constants("LFRACT", "LR", long_fract_type_node);
    ...
    builtin_define_fixed_point_constants("SACCUM", "HK", short_accum_type_node);
    builtin_define_fixed_point_constants("USACCUM", "UHK",
                                         unsigned_short_accum_type_node);
    builtin_define_fixed_point_constants("ACCUM", "K", accum_type_node);
    builtin_define_fixed_point_constants("UACCUM", "UK",
                                         unsigned_accum_type_node);
    builtin_define_fixed_point_constants("LACCUM", "LK", long_accum_type_node);
  ...
  if (TYPE_PRECISION(long_integer_type_node) == 64 && POINTER_SIZE == 64 &&
...
/* Pass an object-like macro and a value to define it to.  The third
   ...
   constant.  */
void builtin_define_with_value(const char *macro, const char *expansion,
                               int is_str) {
  char *buf;
...
static void builtin_define_with_hex_fp_value(const char *macro, tree type,
                                             int digits, const char *hex_str,
                                             const char *fp_suffix,
                                             const char *fp_cast) {
...
static void builtin_define_type_max(const char *macro, tree type, int is_long) {
  ...
  buf = (char *)alloca(strlen(macro) + 1 + strlen(value) + strlen(suffix) + 1);
