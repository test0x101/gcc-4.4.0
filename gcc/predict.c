...
static inline bool maybe_hot_frequency_p(int freq) {
...
static inline bool maybe_hot_count_p(gcov_type count) {
  if (profile_status != PROFILE_READ)
    return true;
  /* Code executed at most once is not hot.  */
  if (profile_info->runs >= count)
    return false;
  return (count > profile_info->sum_max / PARAM_VALUE(HOT_BB_COUNT_FRACTION));
}

/* Return true in case BB can be CPU intensive and should be optimized
   for maximal performance.  */

bool maybe_hot_bb_p(const_basic_block bb) {
  if (profile_status == PROFILE_READ)
...
bool cgraph_maybe_hot_edge_p(struct cgraph_edge *edge) {
  if (profile_info && flag_branch_probabilities &&
      (edge->count <=
  ...
  if (flag_guess_branch_prob &&
      edge->frequency <
          (CGRAPH_FREQ_MAX / PARAM_VALUE(HOT_BB_FREQUENCY_FRACTION)))
    return false;
  return true;
}
...
bool probably_never_executed_bb_p(const_basic_block bb) {
  if (profile_info && flag_branch_probabilities)
...
bool optimize_function_for_size_p(struct function *fun) {
  return (optimize_size || (fun && (fun->function_frequency ==
...
bool optimize_bb_for_size_p(const_basic_block bb) {
...
bool optimize_edge_for_size_p(edge e) {
...
bool optimize_loop_for_size_p(struct loop *loop) {
  return optimize_bb_for_size_p(loop->header);
}

/* Return TRUE when LOOP should be optimized for speed.  */

bool optimize_loop_for_speed_p(struct loop *loop) {
  return optimize_bb_for_speed_p(loop->header);
}

/* Return TRUE when LOOP nest should be optimized for speed.  */

bool optimize_loop_nest_for_speed_p(struct loop *loop) {
  struct loop *l = loop;
...
bool optimize_loop_nest_for_size_p(struct loop *loop) {
...
bool predictable_edge_p(edge e) {
  if (profile_status == PROFILE_ABSENT)
    return false;
  if ((e->probability <= PARAM_VALUE(PARAM_PREDICTABLE_BRANCH_OUTCOME) *
...
static void predict_insn(rtx insn, enum br_predictor predictor,
                         int probability) {
  ...
  add_reg_note(insn, REG_BR_PRED,
               gen_rtx_CONCAT(VOIDmode, GEN_INT((int)predictor),
                              GEN_INT((int)probability)));
}

/* Predict insn by given predictor.  */

void predict_insn_def(rtx insn, enum br_predictor predictor,
...
static bool can_predict_insn_p(const_rtx insn) {
  return (JUMP_P(insn) && any_condjump_p(insn) &&
...
static void dump_prediction(FILE *file, enum br_predictor predictor,
                            int probability, basic_block bb, int used) {
  edge e;
  edge_iterator ei;
  ...
  fprintf(file, "  %s heuristics%s: %.1f%%", predictor_info[predictor].name,
  ...
  if (preds) {
    ...
    for (pred = (struct edge_prediction *)*preds; pred; pred = pred->ep_next) {
      ...
      if (best_predictor > predictor) {
        ...
        for (pred2 = (struct edge_prediction *)*preds; pred2;
             pred2 = pred2->ep_next)
          if (pred2 != pred && pred2->ep_predictor == pred->ep_predictor) {
            ...
            if ((probability >= REG_BR_PROB_BASE / 2 &&
                ...
                (probability <= REG_BR_PROB_BASE / 2 &&
                 (probability2 < probability)))
              prob = probability2;
          }
        if (!pred2)
  ...
  FOR_EACH_LOOP(li, loop, 0) {
    ...
    for (j = 0; VEC_iterate(edge, exits, j, ex); j++) {
      ...
      if (TREE_CODE(niter) == INTEGER_CST) {
        if (host_integerp(niter, 1) && compare_tree_int(niter, max - 1) == -1)
    ...
    for (j = 0; j < loop->num_nodes; j++) {
      ...
      if (!header_found
        ...
        int probability =
            ((REG_BR_PROB_BASE - predictor_info[(int)PRED_LOOP_EXIT].hitrate) /
             n_exits);
...
static tree expr_expected_value_1(tree type, tree op0, enum tree_code code,
  ...
  if (get_gimple_rhs_class(code) == GIMPLE_SINGLE_RHS) {
    ...
    if (is_gimple_assign(def)) {
      ...
      return expr_expected_value_1(
          TREE_TYPE(gimple_assign_lhs(def)), gimple_assign_rhs1(def),
          gimple_assign_rhs_code(def), gimple_assign_rhs2(def), visited);
    }
...
static tree expr_expected_value(tree expr, bitmap visited) {
  ...
  return expr_expected_value_1(TREE_TYPE(expr), op0, code, op1, visited);
}

/* Get rid of all builtin_expect calls and GIMPLE_PREDICT statements
   we no longer need.  */
static unsigned int strip_predict_hints(void) {
  basic_block bb;
  gimple ass_stmt;
  tree var;
...
static void tree_predict_by_opcode(basic_block bb) {
  ...
  if (POINTER_TYPE_P(type)) {
    ...
    switch (cmp) {
      ...
      case LE_EXPR:
      case LT_EXPR:
        if (integer_zerop(op1) || integer_onep(op1) || integer_all_onesp(op1) ||
            real_zerop(op1) || real_onep(op1) || real_minus_onep(op1))
...
static void apply_return_prediction(void) {
  ...
  FOR_EACH_EDGE(e, ei, EXIT_BLOCK_PTR->preds) {
    ...
    if (return_stmt && gimple_code(return_stmt) == GIMPLE_RETURN)
      break;
  }
  if (!e)
    return;
  ...
  if (!return_val)
    return;
  if (TREE_CODE(return_val) != SSA_NAME || !SSA_NAME_DEF_STMT(return_val) ||
  ...
  FOR_EACH_BB(bb) {
    ...
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      if (is_gimple_call(stmt)) {
        if ((gimple_call_flags(stmt) & ECF_NORETURN) && has_return_edges)
          predict_paths_leading_to(bb, PRED_NORETURN, NOT_TAKEN);
        decl = gimple_call_fndecl(stmt);
        if (decl && lookup_attribute("cold", DECL_ATTRIBUTES(decl)))
          predict_paths_leading_to(bb, PRED_COLD_FUNCTION, NOT_TAKEN);
...
static bool assert_is_empty(const void *key ATTRIBUTE_UNUSED, void **value,
  ...
  FOR_EACH_BB(bb) {
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      /* Predict early returns to be probable, as we've already taken
         ...
         return_block:
           return_stmt.  */
      if (e->dest != bb->next_bb && e->dest != EXIT_BLOCK_PTR &&
        ...
        } else if (!predicted_by_p(e->src, PRED_NULL_RETURN) &&
      ...
      if (e->dest != EXIT_BLOCK_PTR && e->dest != bb &&
        ...
        /* The call heuristic claims that a guarded function call
           ...
           messages.  */
        for (bi = gsi_start_bb(e->dest); !gsi_end_p(bi); gsi_next(&bi)) {
          gimple stmt = gsi_stmt(bi);
          if (is_gimple_call(stmt)
...
static void predict_paths_for_bb(basic_block cur, basic_block bb,
                                 enum br_predictor pred,
                                 enum prediction taken) {
  edge e;
  edge_iterator ei;
  ...
  for (son = first_dom_son(CDI_POST_DOMINATORS, cur); son;
...
static void predict_paths_leading_to(basic_block bb, enum br_predictor pred,
                                     enum prediction taken) {
...
static void propagate_freq(basic_block head, bitmap tovisit) {
  basic_block bb;
  basic_block last;
  unsigned i;
  edge e;
  ...
  EXECUTE_IF_SET_IN_BITMAP(tovisit, 0, i, bi) {
    ...
    FOR_EACH_EDGE(e, ei, bb->preds) {
      ...
      if (visit && !(e->flags & EDGE_DFS_BACK))
        ...
        fprintf(dump_file, "Irreducible region hit, ignoring edge to %i->%i\n",
  ...
  for (bb = head; bb; bb = nextbb) {
    ...
    if (bb != head) {
      ...
      if (sreal_compare(&cyclic_probability, &real_zero) == 0) {
        ...
        sreal_div(&BLOCK_INFO(bb)->frequency, &frequency, &cyclic_probability);
      }
    }
    ...
    if (e) {
      ...
      sreal_mul(&EDGE_INFO(e)->back_edge_prob, &tmp, &real_inv_br_prob_base);
    }

    /* Propagate to successor blocks.  */
    FOR_EACH_EDGE(e, ei, bb->succs)
    if (!(e->flags & EDGE_DFS_BACK) && BLOCK_INFO(e->dest)->npredecessors) {
  ...
  if (profile_status != PROFILE_READ || !counts_to_freqs()) {
    ...
    FOR_BB_BETWEEN(bb, ENTRY_BLOCK_PTR, NULL, next_bb) {
      ...
      FOR_EACH_EDGE(e, ei, bb->succs) {
        ...
        sreal_mul(&EDGE_INFO(e)->back_edge_prob, &EDGE_INFO(e)->back_edge_prob,
...
tree build_predict_expr(enum br_predictor predictor, enum prediction taken) {
  tree t = build1(PREDICT_EXPR, void_type_node, build_int_cst(NULL, predictor));
