...
static _Unwind_Ptr base_from_cb_data(unsigned char encoding,
...
static int _Unwind_IteratePhdrCallback(struct dl_phdr_info *info, size_t size,
                                       void *ptr) {
  ...
  if (data->check_cache && size >= sizeof(struct ext_dl_phdr_info)) {
    ...
    if (einfo->dlpi_adds == adds && einfo->dlpi_subs == subs) {
      ...
      for (cache_entry = frame_hdr_cache_head; cache_entry;
  ...
  for (n = info->dlpi_phnum; --n >= 0; phdr++) {
    if (phdr->p_type == PT_LOAD) {
      _Unwind_Ptr vaddr =
          (_Unwind_Ptr)__RELOC_POINTER(phdr->p_vaddr, load_base);
...
found:
  ...
  hdr = (const struct unw_eh_frame_hdr *)__RELOC_POINTER(
  ...
  if (p_dynamic) {
    ...
    ElfW(Dyn) *dyn =
        (ElfW(Dyn) *)__RELOC_POINTER(p_dynamic->p_vaddr, load_base);
...
#error What is DW_EH_PE_datarel base on this platform?
  ...
  if (hdr->fde_count_enc != DW_EH_PE_omit &&
    ...
    if ((((_Unwind_Ptr)p) & 3) == 0) {
      ...
      read_encoded_value_with_base(f_enc & 0x0f, 0, &f->pc_begin[f_enc_size],
                                   &range);
  ...
  if (data->ret != NULL) {
    ...
    read_encoded_value_with_base(encoding, base_from_cb_data(encoding, data),
