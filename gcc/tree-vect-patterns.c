...
/* Function prototypes */
static void vect_pattern_recog_1(gimple (*)(gimple, tree *, tree *),
...
static bool widened_name_p(tree name, gimple use_stmt, tree *half_type,
                           gimple *def_stmt) {
  tree dummy;
  gimple dummy_gimple;
...
static tree vect_recog_temp_ssa_var(tree type, gimple stmt) {
...
static gimple vect_recog_dot_prod_pattern(gimple last_stmt, tree *type_in,
                                          tree *type_out) {
  gimple stmt;
  tree oprnd0, oprnd1;
  ...
  if (STMT_VINFO_IN_PATTERN_P(stmt_vinfo)) {
    ...
    oprnd0 = gimple_assign_rhs1(stmt);
    oprnd1 = gimple_assign_rhs2(stmt);
    if (TYPE_MAIN_VARIANT(TREE_TYPE(oprnd0)) != TYPE_MAIN_VARIANT(prod_type) ||
        TYPE_MAIN_VARIANT(TREE_TYPE(oprnd1)) != TYPE_MAIN_VARIANT(prod_type))
      return NULL;
...
static gimple vect_recog_widen_mult_pattern(gimple last_stmt, tree *type_in,
                                            tree *type_out) {
  ...
  if (!vectype || !supportable_widening_operation(
  ...
  *type_out = NULL_TREE;
  ...
  var = vect_recog_temp_ssa_var(type, NULL);
  pattern_stmt =
      gimple_build_assign_with_ops(WIDEN_MULT_EXPR, var, oprnd0, oprnd1);
...
static gimple vect_recog_pow_pattern(gimple last_stmt, tree *type_in,
                                     tree *type_out) {
  tree type;
  ...
  switch (DECL_FUNCTION_CODE(fn)) {
    ...
    case BUILT_IN_POW:
      ...
      if (TREE_CODE(exp) != REAL_CST && TREE_CODE(exp) != INTEGER_CST)
        return NULL;
      break;
  ...
  *type_out = NULL_TREE;
  ...
  /* Catch squaring.  */
  if ((host_integerp(exp, 0) && tree_low_cst(exp, 0) == 2) ||
  ...
  if (TREE_CODE(exp) == REAL_CST &&
    ...
    *type_in = get_vectype_for_scalar_type(TREE_TYPE(base));
    if (*type_in) {
      ...
      if (vectorizable_function(stmt, *type_in, *type_in) != NULL_TREE) {
...
static gimple vect_recog_widen_sum_pattern(gimple last_stmt, tree *type_in,
                                           tree *type_out) {
  gimple stmt;
  tree oprnd0, oprnd1;
  stmt_vec_info stmt_vinfo = vinfo_for_stmt(last_stmt);
  ...
  *type_out = type;
  ...
  var = vect_recog_temp_ssa_var(type, NULL);
  pattern_stmt =
      gimple_build_assign_with_ops(WIDEN_SUM_EXPR, var, oprnd0, oprnd1);
  SSA_NAME_DEF_STMT(var) = pattern_stmt;
...
static void vect_pattern_recog_1(gimple (*vect_recog_func)(gimple, tree *,
                                                           tree *),
  ...
  set_vinfo_for_stmt(pattern_stmt, new_stmt_vec_info(pattern_stmt, loop_vinfo));
...
void vect_pattern_recog(loop_vec_info loop_vinfo) {
