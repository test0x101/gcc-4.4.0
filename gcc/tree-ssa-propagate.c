...
static void cfg_blocks_add(basic_block bb) {
  ...
  if (cfg_blocks_empty_p()) {
    ...
    /* Minor optimization: we prefer to see blocks with more
       ...
       the incoming edges will be executable.  */
             ...
             EDGE_COUNT(
                 VEC_index(basic_block, cfg_blocks, cfg_blocks_head)->preds))
      cfg_blocks_tail =
          ((cfg_blocks_tail + 1) % VEC_length(basic_block, cfg_blocks));
    else {
  ...
  VEC_replace(basic_block, cfg_blocks, head ? cfg_blocks_head : cfg_blocks_tail,
              bb);
...
static basic_block cfg_blocks_get(void) {
  ...
  cfg_blocks_head =
      ((cfg_blocks_head + 1) % VEC_length(basic_block, cfg_blocks));
  --cfg_blocks_num;
...
static void simulate_stmt(gimple stmt) {
...
static void process_ssa_edge_worklist(VEC(gimple, gc) * *worklist) {
...
bool valid_gimple_rhs_p(tree expr) {
...
bool valid_gimple_call_p(tree expr) {
  unsigned i, nargs;
...
void move_ssa_defining_stmt_for_defs(gimple new_stmt, gimple old_stmt) {
  tree var;
  ssa_op_iter iter;
...
bool update_call_from_tree(gimple_stmt_iterator *si_p, tree expr) {
  tree lhs;
...
bool stmt_makes_single_load(gimple stmt) {
  tree rhs;
  ...
  return (!TREE_THIS_VOLATILE(rhs) && (DECL_P(rhs) || REFERENCE_CLASS_P(rhs)));
}
...
static bool replace_uses_in(gimple stmt, prop_value_t *prop_value) {
  bool replaced = false;
  use_operand_p use;
  ssa_op_iter iter;
...
static void replace_phi_args_in(gimple phi, prop_value_t *prop_value) {
  size_t i;
  bool replaced = false;
...
static bool fold_predicate_in(gimple_stmt_iterator *si) {
  ...
  if (is_gimple_assign(stmt) &&
    ...
    val =
        vrp_evaluate_conditional(gimple_cond_code(stmt), gimple_cond_lhs(stmt),
...
bool substitute_and_fold(prop_value_t *prop_value, bool use_ranges_p) {
  basic_block bb;
  ...
  FOR_EACH_BB(bb) {
    ...
    for (i = gsi_last_bb(bb); !gsi_end_p(i);) {
      ...
      if (gimple_get_lhs(stmt) && TREE_CODE(gimple_get_lhs(stmt)) == SSA_NAME &&
          has_zero_uses(gimple_get_lhs(stmt)) && !stmt_could_throw_p(stmt) &&
  ...
  statistics_counter_event(cfun, "Predicates folded",
                           prop_stats.num_pred_folded);
  statistics_counter_event(cfun, "Statements deleted", prop_stats.num_dce);
