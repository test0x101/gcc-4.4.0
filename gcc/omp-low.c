...
static inline tree scan_omp_op(tree *tp, omp_context *ctx) {
...
tree find_omp_clause(tree clauses, enum omp_clause_code kind) {
...
static inline bool is_combined_parallel(struct omp_region *region) {
...
static void extract_omp_for_data(gimple for_stmt, struct omp_for_data *fd,
                                 struct omp_for_data_loop *loops) {
  ...
  for (i = 0; i < fd->collapse; i++) {
    ...
    switch (TREE_CODE(t)) {
      ...
      case MINUS_EXPR:
        loop->step = TREE_OPERAND(t, 1);
        loop->step =
            fold_build1(NEGATE_EXPR, TREE_TYPE(loop->step), loop->step);
        break;
      default:
        gcc_unreachable();
    }
    ...
    if (iter_type != long_long_unsigned_type_node) {
      if (POINTER_TYPE_P(TREE_TYPE(loop->v)))
        ...
        if (loop->cond_code == LT_EXPR)
          n = fold_build2(PLUS_EXPR, TREE_TYPE(loop->v), loop->n2, loop->step);
        else
          n = loop->n1;
        ...
        if (loop->cond_code == LT_EXPR) {
          n1 = loop->n1;
          n2 = fold_build2(PLUS_EXPR, TREE_TYPE(loop->v), loop->n2, loop->step);
        } else {
          n1 =
              fold_build2(MINUS_EXPR, TREE_TYPE(loop->v), loop->n2, loop->step);
          n2 = loop->n1;
        }
        if (TREE_CODE(n1) != INTEGER_CST || TREE_CODE(n2) != INTEGER_CST ||
    ...
    if (collapse_count && *collapse_count == NULL) {
      if ((i == 0 || count != NULL_TREE) &&
          ...
          TREE_CONSTANT(loop->n1) && TREE_CONSTANT(loop->n2) &&
        ...
        t = fold_build2(PLUS_EXPR, itype, fold_convert(itype, loop->step), t);
        t = fold_build2(PLUS_EXPR, itype, t, fold_convert(itype, loop->n2));
        t = fold_build2(MINUS_EXPR, itype, t, fold_convert(itype, loop->n1));
        if (TYPE_UNSIGNED(itype) && loop->cond_code == GT_EXPR)
          t = fold_build2(
              TRUNC_DIV_EXPR, itype, fold_build1(NEGATE_EXPR, itype, t),
              fold_build1(NEGATE_EXPR, itype, fold_convert(itype, loop->step)));
        else
        ...
        if (count != NULL_TREE)
          count =
              fold_build2(MULT_EXPR, long_long_unsigned_type_node, count, t);
        else
          count = t;
...
static tree get_ws_args_for(gimple ws_stmt) {
  tree t;
...
static void determine_parallel_type(struct omp_region *region) {
  ...
  if (region == NULL || region->inner == NULL || region->exit == NULL ||
      region->inner->exit == NULL || region->inner->cont == NULL)
    return;
  ...
  if (single_succ(par_entry_bb) == ws_entry_bb &&
      ...
      (gimple_omp_parallel_combined_p(last_stmt(par_entry_bb)) ||
       (last_and_only_stmt(ws_entry_bb) && last_and_only_stmt(par_exit_bb)))) {
...
static inline bool is_variable_sized(const_tree expr) {
  return !TREE_CONSTANT(TYPE_SIZE_UNIT(TREE_TYPE(expr)));
}

/* Return true if DECL is a reference type.  */

static inline bool is_reference(tree decl) {
...
static inline tree lookup_decl(tree var, omp_context *ctx) {
  tree *n;
  n = (tree *)pointer_map_contains(ctx->cb.decl_map, var);
  return *n;
}

static inline tree maybe_lookup_decl(const_tree var, omp_context *ctx) {
  tree *n;
  n = (tree *)pointer_map_contains(ctx->cb.decl_map, var);
  return n ? *n : NULL_TREE;
}

static inline tree lookup_field(tree var, omp_context *ctx) {
  splay_tree_node n;
  n = splay_tree_lookup(ctx->field_map, (splay_tree_key)var);
  return (tree)n->value;
}

static inline tree lookup_sfield(tree var, omp_context *ctx) {
  splay_tree_node n;
  n = splay_tree_lookup(ctx->sfield_map ? ctx->sfield_map : ctx->field_map,
                        (splay_tree_key)var);
  return (tree)n->value;
}

static inline tree maybe_lookup_field(tree var, omp_context *ctx) {
  splay_tree_node n;
  n = splay_tree_lookup(ctx->field_map, (splay_tree_key)var);
  ...
  if (shared_ctx) {
    ...
    if (!TREE_READONLY(decl) && shared_ctx->is_nested) {
      ...
      if (up) {
        ...
        for (c = gimple_omp_taskreg_clauses(up->stmt); c;
...
static tree build_sender_ref(tree var, omp_context *ctx) {
  ...
  return build3(COMPONENT_REF, TREE_TYPE(field), ctx->sender_decl, field, NULL);
}

/* Add a new field for VAR inside the structure CTX->SENDER_DECL.  */

static void install_var_field(tree var, bool by_ref, int mask,
                              omp_context *ctx) {
  ...
  if ((mask & 3) == 3) {
    ...
    sfield = field;
    insert_field_into_struct((mask & 1) ? ctx->record_type : ctx->srecord_type,
                             field);
  }
...
static void fixup_remapped_decl(tree decl, omp_context *ctx,
                                bool private_debug) {
  tree new_decl, size;
...
void dump_omp_region(FILE *file, struct omp_region *region, int indent) {
...
struct omp_region *new_omp_region(basic_block bb, enum gimple_code type,
...
static omp_context *new_omp_context(gimple stmt, omp_context *outer_ctx) {
...
static void finalize_task_copyfn(gimple task_stmt) {
...
static void fixup_child_record_type(omp_context *ctx) {
  ...
  if (f) {
    ...
    for (f = TYPE_FIELDS(ctx->record_type); f; f = TREE_CHAIN(f)) {
      ...
      walk_tree(&DECL_SIZE_UNIT(new_f), copy_tree_body_r, &ctx->cb, NULL);
      walk_tree(&DECL_FIELD_OFFSET(new_f), copy_tree_body_r, &ctx->cb, NULL);
      new_fields = new_f;
  ...
  for (c = clauses; c; c = OMP_CLAUSE_CHAIN(c)) {
    ...
    switch (OMP_CLAUSE_CODE(c)) {
      ...
      case OMP_CLAUSE_SHARED:
        ...
        if (!TREE_READONLY(decl) || TREE_ADDRESSABLE(decl) || by_ref ||
            is_reference(decl)) {
      ...
      do_private:
        if (is_variable_sized(decl)) {
          ...
          if (is_task_ctx(ctx) && (global || by_ref || is_reference(decl))) {
            install_var_field(decl, false, 1, ctx);
            if (!global)
...
static void create_omp_child_function(omp_context *ctx, bool task_copy) {
  ...
  if (!task_copy)
    ...
    t = build_decl(PARM_DECL, get_identifier(".omp_data_o"), ptr_type_node);
    DECL_ARTIFICIAL(t) = 1;
    DECL_ARG_TYPE(t) = ptr_type_node;
    DECL_CONTEXT(t) = current_function_decl;
    TREE_USED(t) = 1;
...
static void scan_omp_parallel(gimple_stmt_iterator *gsi,
  ...
  /* Ignore parallel directives with empty bodies, unless there
     are copyin clauses.  */
  if (optimize > 0 && empty_body_p(gimple_omp_body(stmt)) &&
      find_omp_clause(gimple_omp_parallel_clauses(stmt), OMP_CLAUSE_COPYIN) ==
          NULL) {
...
static void scan_omp_task(gimple_stmt_iterator *gsi, omp_context *outer_ctx) {
  ...
  if (TYPE_FIELDS(ctx->record_type) == NULL) {
    ...
    *p = vla_fields;
    ...
    t = fold_convert(long_integer_type_node, TYPE_SIZE_UNIT(ctx->record_type));
    gimple_omp_task_set_arg_size(stmt, t);
...
static void scan_omp_for(gimple stmt, omp_context *outer_ctx) {
  omp_context *ctx;
  size_t i;
...
static void scan_omp_sections(gimple stmt, omp_context *outer_ctx) {
  omp_context *ctx;
...
static void scan_omp_single(gimple stmt, omp_context *outer_ctx) {
  omp_context *ctx;
  tree name;
...
static void check_omp_nesting_restrictions(gimple stmt, omp_context *ctx) {
...
static tree scan_omp_1_op(tree *tp, int *walk_subtrees, void *data) {
...
static tree scan_omp_1_stmt(gimple_stmt_iterator *gsi, bool *handled_ops_p,
                            struct walk_stmt_info *wi) {
  gimple stmt = gsi_stmt(*gsi);
  omp_context *ctx = (omp_context *)wi->info;
...
static void scan_omp(gimple_seq body, omp_context *ctx) {
...
static tree build_omp_barrier(void) {
...
static tree lookup_decl_in_outer_ctx(tree decl, omp_context *ctx) {
  tree t;
  omp_context *up;
...
static tree maybe_lookup_decl_in_outer_ctx(tree decl, omp_context *ctx) {
  tree t = NULL;
  omp_context *up;
...
static void lower_rec_input_clauses(tree clauses, gimple_seq *ilist,
                                    gimple_seq *dlist, omp_context *ctx) {
  ...
  *dlist = gimple_seq_alloc();
    ...
    for (c = clauses; c; c = OMP_CLAUSE_CHAIN(c)) {
      ...
      if (c_kind == OMP_CLAUSE_SHARED || c_kind == OMP_CLAUSE_COPYIN) {
        ...
        if (c_kind != OMP_CLAUSE_FIRSTPRIVATE || !is_task_ctx(ctx)) {
          ...
          stmt = gimple_build_call(built_in_decls[BUILT_IN_ALLOCA], 1, x);
        ...
        if (c_kind == OMP_CLAUSE_FIRSTPRIVATE && is_task_ctx(ctx)) {
          ...
          x = create_tmp_var_raw(TREE_TYPE(TREE_TYPE(new_var)), name);
      ...
      switch (OMP_CLAUSE_CODE(c)) {
        ...
        case OMP_CLAUSE_FIRSTPRIVATE:
          if (is_task_ctx(ctx)) {
            if (is_reference(var) || is_variable_sized(var))
              goto do_dtor;
            else if (is_global_var(maybe_lookup_decl_in_outer_ctx(var, ctx)) ||
        ...
        case OMP_CLAUSE_REDUCTION:
          if (OMP_CLAUSE_REDUCTION_PLACEHOLDER(c)) {
            ...
            gimple_seq_add_seq(ilist, OMP_CLAUSE_REDUCTION_GIMPLE_INIT(c));
  ...
  if (copyin_seq) {
    ...
    x = build2(NE_EXPR, boolean_type_node, x, build_int_cst(TREE_TYPE(x), 0));
...
static void lower_lastprivate_clauses(tree clauses, tree predicate,
                                      gimple_seq *stmt_list, omp_context *ctx) {
  tree x, c, label = NULL;
  ...
  if (predicate) {
    ...
    stmt =
        gimple_build_cond(TREE_CODE(predicate), arm1, arm2, label_true, label);
  ...
  for (c = clauses; c;) {
    ...
    if (OMP_CLAUSE_CODE(c) == OMP_CLAUSE_LASTPRIVATE) {
      ...
      if (OMP_CLAUSE_LASTPRIVATE_GIMPLE_SEQ(c)) {
        ...
        gimple_seq_add_seq(stmt_list, OMP_CLAUSE_LASTPRIVATE_GIMPLE_SEQ(c));
      }
...
static void lower_reduction_clauses(tree clauses, gimple_seq *stmt_seqp,
                                    omp_context *ctx) {
...
static void lower_copyprivate_clauses(tree clauses, gimple_seq *slist,
                                      gimple_seq *rlist, omp_context *ctx) {
  tree c;
...
static void lower_send_clauses(tree clauses, gimple_seq *ilist,
                               gimple_seq *olist, omp_context *ctx) {
  tree c;
  ...
  for (c = clauses; c; c = OMP_CLAUSE_CHAIN(c)) {
    ...
    if (OMP_CLAUSE_CODE(c) != OMP_CLAUSE_COPYIN && is_global_var(var))
      continue;
...
static void lower_send_shared_vars(gimple_seq *ilist, gimple_seq *olist,
                                   omp_context *ctx) {
    ...
    if (use_pointer_for_field(ovar, ctx)) {
      ...
      if (!TREE_READONLY(var)
          /* We don't need to receive a new reference to a result
             ...
             during inlining.  */
          && !((TREE_CODE(var) == RESULT_DECL || TREE_CODE(var) == PARM_DECL) &&
...
static gimple gimple_build_cond_empty(tree cond) {
...
static void expand_parallel_call(struct omp_region *region, basic_block bb,
  ...
  if (cond) {
    ...
    if (integer_zerop(val))
      ...
      stmt =
          gimple_build_assign(tmp_else, build_int_cst(unsigned_type_node, 1));
      gsi_insert_after(&gsi, stmt, GSI_CONTINUE_LINKING);
    ...
    gsi = gsi_start_bb(bb);
    val = force_gimple_operand_gsi(&gsi, val, true, NULL_TREE, false,
  ...
  if (ws_args) {
    tree args =
        tree_cons(NULL, t2, tree_cons(NULL, t1, tree_cons(NULL, val, ws_args)));
...
static void expand_task_call(basic_block bb, gimple entry_stmt) {
...
static gimple_seq maybe_catch_exception(gimple_seq body) {
  ...
  t = gimple_build_try(body, gimple_seq_alloc_with_stmt(f), GIMPLE_TRY_CATCH);
    ...
    if (gimple_code(stmt) == GIMPLE_OMP_RETURN &&
      ...
      if (any_addressable_vars < 0) {
        ...
        for (block = gimple_block(stmt);
             !any_addressable_vars && block && TREE_CODE(block) == BLOCK;
             block = BLOCK_SUPERCONTEXT(block)) {
          for (local_decls = BLOCK_VARS(block); local_decls;
      ...
      for (gsi = gsi_start_bb(entry_succ_bb);; gsi_next(&gsi)) {
        ...
        if (gimple_num_ops(stmt) == 2) {
          ...
          if (TREE_CODE(arg) == ADDR_EXPR &&
              TREE_OPERAND(arg, 0) == gimple_omp_taskreg_data_arg(entry_stmt)) {
            parcopy_stmt = stmt;
            break;
          }
        }
...
static void expand_omp_for_generic(struct omp_region *region,
  ...
  gcc_assert(fd->iter_type == long_integer_type_node || !in_combined_parallel);
  ...
  if (fd->collapse > 1) {
    ...
    for (i = 0; i < fd->collapse; i++) {
      ...
      if (POINTER_TYPE_P(itype))
        itype = lang_hooks.types.type_for_size(TYPE_PRECISION(itype), 0);
      t = build_int_cst(itype, (fd->loops[i].cond_code == LT_EXPR ? -1 : 1));
      t = fold_build2(PLUS_EXPR, itype, fold_convert(itype, fd->loops[i].step),
                      t);
  ...
  if (in_combined_parallel) {
    ...
    if (POINTER_TYPE_P(type) &&
      ...
      tree itype = lang_hooks.types.type_for_size(TYPE_PRECISION(type), 0);
    ...
    if (fd->iter_type == long_integer_type_node) {
      if (fd->chunk_size) {
        ...
        t = build_call_expr(built_in_decls[start_fn], 6, t0, t1, t2, t, t3, t4);
      } else
        t = build_call_expr(built_in_decls[start_fn], 5, t0, t1, t2, t3, t4);
    } else {
      tree t5;
      tree c_bool_type;
      ...
      t5 = build_int_cst(c_bool_type, fd->loop.cond_code == LT_EXPR ? 1 : 0);
      if (fd->chunk_size) {
        t = fold_convert(fd->iter_type, fd->chunk_size);
        t = build_call_expr(built_in_decls[start_fn], 7, t5, t0, t1, t2, t, t3,
                            t4);
      } else
        t = build_call_expr(built_in_decls[start_fn], 6, t5, t0, t1, t2, t3,
                            t4);
    }
  }
  ...
  if (bias)
    t = fold_convert(type,
                     fold_build2(MINUS_EXPR, fd->iter_type, istart0, bias));
  else
  ...
  if (bias)
    t = fold_convert(type, fold_build2(MINUS_EXPR, fd->iter_type, iend0, bias));
  else
    t = fold_convert(type, iend0);
  iend = force_gimple_operand_gsi(&gsi, t, true, NULL_TREE, false,
                                  GSI_CONTINUE_LINKING);
  if (fd->collapse > 1) {
    ...
    for (i = fd->collapse - 1; i >= 0; i--) {
      ...
      if (POINTER_TYPE_P(vtype))
        t = fold_build2(POINTER_PLUS_EXPR, vtype, fd->loops[i].n1,
  ...
  if (!broken_loop) {
    ...
    if (fd->collapse > 1) {
      ...
      for (i = fd->collapse - 1; i >= 0; i--) {
        ...
        if (POINTER_TYPE_P(vtype))
          t = fold_build2(POINTER_PLUS_EXPR, vtype, fd->loops[i].v,
...
static void expand_omp_for_static_nochunk(struct omp_region *region,
  ...
  fd->loop.n1 = force_gimple_operand_gsi(&gsi, fold_convert(type, fd->loop.n1),
                                         true, NULL_TREE, true, GSI_SAME_STMT);
  fd->loop.n2 = force_gimple_operand_gsi(&gsi, fold_convert(itype, fd->loop.n2),
                                         true, NULL_TREE, true, GSI_SAME_STMT);
  fd->loop.step =
      force_gimple_operand_gsi(&gsi, fold_convert(itype, fd->loop.step), true,
  ...
  if (TYPE_UNSIGNED(itype) && fd->loop.cond_code == GT_EXPR)
    t = fold_build2(TRUNC_DIV_EXPR, itype, fold_build1(NEGATE_EXPR, itype, t),
  ...
  if (POINTER_TYPE_P(type))
    ...
    t = fold_build2(PLUS_EXPR, type, t, fd->loop.n1);
  e = force_gimple_operand_gsi(&gsi, t, true, NULL_TREE, false,
                               GSI_CONTINUE_LINKING);
  ...
  gsi = gsi_last_bb(exit_bb);
  if (!gimple_omp_return_nowait_p(gsi_stmt(gsi)))
    force_gimple_operand_gsi(&gsi, build_omp_barrier(), false, NULL_TREE, false,
                             GSI_SAME_STMT);
  gsi_remove(&gsi, true);
...
static void expand_omp_for_static_chunk(struct omp_region *region,
  ...
  gcc_assert(BRANCH_EDGE(iter_part_bb)->dest == FALLTHRU_EDGE(cont_bb)->dest);
  ...
  fd->loop.n1 = force_gimple_operand_gsi(&si, fold_convert(type, fd->loop.n1),
                                         true, NULL_TREE, true, GSI_SAME_STMT);
  fd->loop.n2 = force_gimple_operand_gsi(&si, fold_convert(itype, fd->loop.n2),
                                         true, NULL_TREE, true, GSI_SAME_STMT);
  fd->loop.step =
      force_gimple_operand_gsi(&si, fold_convert(itype, fd->loop.step), true,
                               NULL_TREE, true, GSI_SAME_STMT);
  fd->chunk_size =
      force_gimple_operand_gsi(&si, fold_convert(itype, fd->chunk_size), true,
                               NULL_TREE, true, GSI_SAME_STMT);
  ...
  if (TYPE_UNSIGNED(itype) && fd->loop.cond_code == GT_EXPR)
    ...
    t = fold_build2(TRUNC_DIV_EXPR, itype, t, fd->loop.step);
  t = fold_convert(itype, t);
  n = force_gimple_operand_gsi(&si, t, true, NULL_TREE, true, GSI_SAME_STMT);
  ...
  s0 = force_gimple_operand_gsi(&si, t, true, NULL_TREE, false,
                                GSI_CONTINUE_LINKING);

  t = fold_build2(PLUS_EXPR, itype, s0, fd->chunk_size);
  t = fold_build2(MIN_EXPR, itype, t, n);
  e0 = force_gimple_operand_gsi(&si, t, true, NULL_TREE, false,
                                GSI_CONTINUE_LINKING);
  ...
  if (POINTER_TYPE_P(type))
    ...
    t = fold_build2(PLUS_EXPR, type, t, fd->loop.n1);
  t = force_gimple_operand_gsi(&si, t, false, NULL_TREE, false,
                               GSI_CONTINUE_LINKING);
  stmt = gimple_build_assign(fd->loop.v, t);
  ...
  if (POINTER_TYPE_P(type))
    ...
    t = fold_build2(PLUS_EXPR, type, t, fd->loop.n1);
  e = force_gimple_operand_gsi(&si, t, true, NULL_TREE, false,
                               GSI_CONTINUE_LINKING);
  ...
  if (!gimple_omp_return_nowait_p(gsi_stmt(si)))
    force_gimple_operand_gsi(&si, build_omp_barrier(), false, NULL_TREE, false,
                             GSI_SAME_STMT);
  gsi_remove(&si, true);
...
static void expand_omp_for(struct omp_region *region) {
  ...
  loops = (struct omp_for_data_loop *)alloca(
  ...
  if (fd.sched_kind == OMP_CLAUSE_SCHEDULE_STATIC && !fd.have_ordered &&
      fd.collapse == 1 && region->cont != NULL) {
    ...
    fn_index =
        (fd.sched_kind == OMP_CLAUSE_SCHEDULE_RUNTIME) ? 3 : fd.sched_kind;
    ...
    if (fd.iter_type == long_long_unsigned_type_node) {
      start_ix +=
          BUILT_IN_GOMP_LOOP_ULL_STATIC_START - BUILT_IN_GOMP_LOOP_STATIC_START;
      next_ix +=
          BUILT_IN_GOMP_LOOP_ULL_STATIC_NEXT - BUILT_IN_GOMP_LOOP_STATIC_NEXT;
    }
...
static void expand_omp_sections(struct omp_region *region) {
  ...
  if (exit_reachable) {
    if (single_pred(l2_bb) == l0_bb)
      ...
      l2 = NULL_TREE;
      if (gsi_end_p(si) || gimple_code(gsi_stmt(si)) != GIMPLE_OMP_SECTION)
  ...
  if (!is_combined_parallel(region)) {
    ...
    t = build_int_cst(unsigned_type_node, exit_reachable ? len - 1 : len);
  ...
  for (inner = region->inner, casei = 1; inner;
...
static bool expand_omp_atomic_fetch_op(basic_block load_bb, tree addr,
                                       tree loaded_val, tree stored_val,
                                       int index) {
...
static bool expand_omp_atomic_pipeline(basic_block load_bb,
                                       basic_block store_bb, tree addr,
                                       tree loaded_val, tree stored_val,
                                       int index) {
  ...
  if (!INTEGRAL_TYPE_P(type) && !POINTER_TYPE_P(type)) {
    ...
    iaddr_val =
        force_gimple_operand_gsi(&si, fold_convert(TREE_TYPE(iaddr), addr),
  ...
  initial = force_gimple_operand_gsi(&si, build_fold_indirect_ref(iaddr), true,
                                     NULL_TREE, true, GSI_SAME_STMT);
  ...
  if (gimple_in_ssa_p(cfun)) {
    ...
    SET_USE(PHI_ARG_DEF_PTR_FROM_EDGE(phi, single_succ_edge(load_bb)), initial);
  } else
    gsi_insert_before(&si, gimple_build_assign(loadedi, initial),
                      GSI_SAME_STMT);
  if (loadedi != loaded_val) {
    ...
    if (gimple_in_ssa_p(cfun)) {
      gimple stmt;
      x = force_gimple_operand_gsi(&gsi2, x, true, NULL_TREE, true,
                                   GSI_SAME_STMT);
      ...
      force_gimple_operand_gsi(&gsi2, x, true, NULL_TREE, true, GSI_SAME_STMT);
    }
  }
  gsi_remove(&si, true);
  ...
  if (iaddr == addr)
    ...
    storedi = force_gimple_operand_gsi(
        &si, build1(VIEW_CONVERT_EXPR, itype, stored_val), true, NULL_TREE,
        true, GSI_SAME_STMT);

  /* Build the compare&swap statement.  */
  new_storedi = build_call_expr(cmpxchg, 3, iaddr, loadedi, storedi);
  new_storedi = force_gimple_operand_gsi(&si, fold_convert(itype, new_storedi),
                                         true, NULL_TREE, true, GSI_SAME_STMT);
  ...
  stmt = gimple_build_cond_empty(
      build2(NE_EXPR, boolean_type_node, new_storedi, old_vali));
  gsi_insert_before(&si, stmt, GSI_SAME_STMT);
...
static bool expand_omp_atomic_mutex(basic_block load_bb, basic_block store_bb,
  ...
  if (index >= 0 && index <= 4) {
    ...
    if (exact_log2(align) >= index) {
      ...
      if ((INTEGRAL_TYPE_P(type) || POINTER_TYPE_P(type)) &&
        ...
        if (expand_omp_atomic_fetch_op(load_bb, addr, loaded_val, stored_val,
                                       index))
          return;
      }
      ...
      if (expand_omp_atomic_pipeline(load_bb, store_bb, addr, loaded_val,
                                     stored_val, index))
        return;
    }
  }
...
static void lower_omp_sections(gimple_stmt_iterator *gsi_p, omp_context *ctx) {
  ...
  dlist = NULL;
  ilist = NULL;
  lower_rec_input_clauses(gimple_omp_sections_clauses(stmt), &ilist, &dlist,
                          ctx);
  ...
  for (i = 0; i < len; i++, gsi_next(&tgsi)) {
    ...
    if (i == len - 1) {
      gimple_seq l = NULL;
      lower_lastprivate_clauses(gimple_omp_sections_clauses(stmt), NULL, &l,
                                ctx);
  ...
  t = gimple_build_omp_return(
      !!find_omp_clause(gimple_omp_sections_clauses(stmt), OMP_CLAUSE_NOWAIT));
  gimple_seq_add_stmt(&new_body, t);
...
static void lower_omp_single_simple(gimple single_stmt, gimple_seq *pre_p) {
...
static void lower_omp_single_copy(gimple single_stmt, gimple_seq *pre_p,
                                  omp_context *ctx) {
...
static void lower_omp_single(gimple_stmt_iterator *gsi_p, omp_context *ctx) {
  ...
  bind_body = NULL;
  lower_rec_input_clauses(gimple_omp_single_clauses(single_stmt), &bind_body,
                          &dlist, ctx);
  ...
  t = gimple_build_omp_return(!!find_omp_clause(
      gimple_omp_single_clauses(single_stmt), OMP_CLAUSE_NOWAIT));
...
static GTY((param1_is(tree), param2_is(tree))) splay_tree critical_name_mutexes;
...
static void lower_omp_critical(gimple_stmt_iterator *gsi_p, omp_context *ctx) {
  ...
  if (name) {
    ...
    if (!critical_name_mutexes)
      critical_name_mutexes = splay_tree_new_ggc(splay_tree_compare_pointers);
    ...
    if (n == NULL) {
      ...
      new_str =
          ACONCAT((".gomp_critical_user_", IDENTIFIER_POINTER(name), NULL));
...
static void lower_omp_for_lastprivate(struct omp_for_data *fd,
                                      gimple_seq *body_p, gimple_seq *dlist,
  ...
  if (!gimple_seq_empty_p(stmts)) {
    ...
    *dlist = stmts;
    ...
    vinit = fd->loop.n1;
    if (cond_code == EQ_EXPR && host_integerp(fd->loop.n2, 0) &&
...
static void lower_omp_for(gimple_stmt_iterator *gsi_p, omp_context *ctx) {
  ...
  gimple_seq_add_stmt(&body, gimple_build_omp_continue(fd.loop.v, fd.loop.v));
...
static tree task_copyfn_remap_type(struct omp_taskcopy_context *tcctx,
                                   tree orig_type) {
  ...
  for (f = TYPE_FIELDS(orig_type); f; f = TREE_CHAIN(f)) {
    ...
    walk_tree(&DECL_FIELD_OFFSET(new_f), copy_tree_body_r, &tcctx->cb, NULL);
    new_fields = new_f;
...
static void create_task_copyfn(gimple task_stmt, omp_context *ctx) {
  struct function *child_cfun;
  ...
  if (tcctx.cb.decl_map)
    for (c = gimple_omp_task_clauses(task_stmt); c; c = OMP_CLAUSE_CHAIN(c))
      if (OMP_CLAUSE_CODE(c) == OMP_CLAUSE_FIRSTPRIVATE) {
        ...
        t = build2(MODIFY_EXPR, TREE_TYPE(ptr), ptr, build_fold_addr_expr(dst));
        append_to_statement_list(t, &list);
      }
...
static void lower_omp_taskreg(gimple_stmt_iterator *gsi_p, omp_context *ctx) {
  ...
  if (ctx->record_type) {
    ctx->sender_decl =
        create_tmp_var(ctx->srecord_type ? ctx->srecord_type : ctx->record_type,
                       ".omp_data_o");
  ...
  if (ctx->record_type) {
    ...
    gimple_seq_add_stmt(&new_body, gimple_build_assign(ctx->receiver_decl, t));
  }
...
static void lower_omp_1(gimple_stmt_iterator *gsi_p, omp_context *ctx) {
  ...
  switch (gimple_code(stmt)) {
    ...
    default:
      if ((ctx || task_shared_vars) &&
          walk_gimple_op(stmt, lower_omp_regimplify_p, ctx ? NULL : &wi))
        gimple_regimplify_operands(stmt, gsi_p);
      break;
  }
}

static void lower_omp(gimple_seq body, omp_context *ctx) {
...
static unsigned int execute_lower_omp(void) {
  ...
  all_contexts =
      splay_tree_new(splay_tree_compare_pointers, 0, delete_omp_context);
...
static bool diagnose_sb_0(gimple_stmt_iterator *gsi_p, gimple branch_ctx,
                          gimple label_ctx) {
  if (label_ctx == branch_ctx)
    return false;

    /*
       Previously we kept track of the label's entire context in
       diagnose_sb_[12] so we could traverse it and issue a correct "exit" or
       "enter" error message upon a structured block violation.
...
static tree diagnose_sb_1(gimple_stmt_iterator *gsi_p, bool *handled_ops_p,
                          struct walk_stmt_info *wi) {
  ...
  switch (gimple_code(stmt)) {
    ...
    case GIMPLE_OMP_FOR:
      ...
      /* gimple_omp_for_{index,initial,final} are all DECLs; no need to
         walk them.  */
      walk_gimple_seq(gimple_omp_for_pre_body(stmt), diagnose_sb_1, NULL, wi);
      walk_gimple_seq(gimple_omp_body(stmt), diagnose_sb_1, NULL, wi);
      wi->info = context;
      break;
...
static tree diagnose_sb_2(gimple_stmt_iterator *gsi_p, bool *handled_ops_p,
                          struct walk_stmt_info *wi) {
  gimple context = (gimple)wi->info;
  splay_tree_node n;
  gimple stmt = gsi_stmt(*gsi_p);
  ...
  *handled_ops_p = true;
  ...
  switch (gimple_code(stmt)) {
    ...
    case GIMPLE_OMP_FOR:
      ...
      /* gimple_omp_for_{index,initial,final} are all DECLs; no need to
         walk them.  */
      walk_gimple_seq(gimple_omp_for_pre_body(stmt), diagnose_sb_2, NULL, wi);
      walk_gimple_seq(gimple_omp_body(stmt), diagnose_sb_2, NULL, wi);
      wi->info = context;
      break;
