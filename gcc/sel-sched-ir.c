...
ilist_t ilist_invert(ilist_t l) {
  ilist_t res = NULL;
...
void blist_add(blist_t *lp, insn_t to, ilist_t ptr, deps_t dc) {
  bnd_t bnd;
...
static void init_fence_for_scheduling(fence_t f) {
  FENCE_BNDS(f) = NULL;
...
static void flist_add(flist_t *lp, insn_t insn, state_t state, deps_t dc,
                      void *tc, insn_t last_scheduled_insn,
                      VEC(rtx, gc) * executing_insns, int *ready_ticks,
                      int ready_ticks_size, insn_t sched_next, int cycle,
                      int cycle_issued_insns, bool starts_cycle_p,
                      bool after_stall_p) {
  fence_t f;
...
static void flist_remove(flist_t *lp) {
...
void flist_clear(flist_t *lp) {
  while (*lp)
    flist_remove(lp);
}

/* Add ORIGINAL_INSN the def list DL honoring CROSSES_CALL.  */
void def_list_add(def_list_t *dl, insn_t original_insn, bool crosses_call) {
  def_t d;
...
static tc_t alloc_target_context(void) {
  return (targetm.sched.alloc_sched_context
              ? targetm.sched.alloc_sched_context()
              : bulk_tc);
}
...
static void init_target_context(tc_t tc, bool clean_p) {
...
tc_t create_target_context(bool clean_p) {
...
static void copy_target_context(tc_t to, tc_t from) {
...
/* Create a copy of TC.  */
static tc_t create_copy_of_target_context(tc_t tc) {
...
void reset_target_context(tc_t tc, bool clean_p) {
  clear_target_context(tc);
  init_target_context(tc, clean_p);
}
...
/* Make a copy of FROM in TO.  */
static void copy_deps_context(deps_t to, deps_t from) {
  init_deps(to);
  deps_join(to, from);
}

/* Allocate store for dep context.  */
static deps_t alloc_deps_context(void) {
  return XNEW(struct deps);
}

/* Allocate and initialize dep context.  */
static deps_t create_deps_context(void) {
...
static deps_t create_copy_of_deps_context(deps_t from) {
...
static void clear_deps_context(deps_t dc) {
  free_deps(dc);
}

/* Clear and free DC.  */
static void delete_deps_context(deps_t dc) {
  clear_deps_context(dc);
  free(dc);
}

/* Clear and init DC.  */
static void reset_deps_context(deps_t dc) {
  clear_deps_context(dc);
  init_deps(dc);
}
...
void advance_deps_context(deps_t dc, insn_t insn) {
...
static state_t state_alloc(void) {
...
/* Make a copy of FROM in TO.  */
static void state_copy(state_t to, state_t from) {
  memcpy(to, from, dfa_state_size);
}

/* Create a copy of FROM.  */
static state_t state_create_copy(state_t from) {
...
/* Clear the fence.  */
static void fence_clear(fence_t f) {
...
void init_fences(insn_t old_fence) {
  insn_t succ;
  succ_iterator si;
  bool first = true;
  ...
  FOR_EACH_SUCC_1(succ, si, old_fence,
    ...
    flist_add(&fences, succ, state_create(), create_deps_context() /* dc */,
              create_target_context(true) /* tc */,
              NULL_RTX /* last_scheduled_insn */, NULL, /* executing_insns */
              XCNEWVEC(int, ready_ticks_size),          /* ready_ticks */
              ready_ticks_size, NULL_RTX /* sched_next */, 1 /* cycle */,
              0 /* cycle_issued_insns */, 1 /* starts_cycle_p */,
              0 /* after_stall_p */);
  }
}
...
static void merge_fences(fence_t f, insn_t insn, state_t state, deps_t dc,
                         void *tc, rtx last_scheduled_insn,
                         VEC(rtx, gc) * executing_insns, int *ready_ticks,
                         int ready_ticks_size, rtx sched_next, int cycle,
                         bool after_stall_p) {
  insn_t last_scheduled_insn_old = FENCE_LAST_SCHEDULED_INSN(f);

  gcc_assert(sel_bb_head_p(FENCE_INSN(f)) && !sched_next &&
             !FENCE_SCHED_NEXT(f));
  ...
  if (last_scheduled_insn == NULL ||
    ...
    if (edge_old == NULL || edge_new == NULL) {
      ...
      if (FENCE_READY_TICKS(f))
        memset(FENCE_READY_TICKS(f), 0, FENCE_READY_TICKS_SIZE(f));
    } else if (edge_new->probability > edge_old->probability) {
...
/* Add a new fence to NEW_FENCES list, initializing it from all
   other parameters.  */
static void add_to_fences(flist_tail_t new_fences, insn_t insn, state_t state,
                          deps_t dc, void *tc, rtx last_scheduled_insn,
                          VEC(rtx, gc) * executing_insns, int *ready_ticks,
                          int ready_ticks_size, rtx sched_next, int cycle,
                          int cycle_issued_insns, bool starts_cycle_p,
                          bool after_stall_p) {
  ...
  if (!f) {
    ...
    FLIST_TAIL_TAILP(new_fences) = &FLIST_NEXT(*FLIST_TAIL_TAILP(new_fences));
  } else {
    merge_fences(f, insn, state, dc, tc, last_scheduled_insn, executing_insns,
                 ready_ticks, ready_ticks_size, sched_next, cycle,
                 after_stall_p);
  }
}

/* Move the first fence in the OLD_FENCES list to NEW_FENCES.  */
void move_fence_to_fences(flist_t old_fences, flist_tail_t new_fences) {
  fence_t f, old;
  ...
  if (f) {
    merge_fences(f, old->insn, old->state, old->dc, old->tc,
                 ...
                 old->ready_ticks, old->ready_ticks_size, old->sched_next,
                 old->cycle, old->after_stall_p);
  } else {
    _list_add(tailp);
...
/* Add a new fence to NEW_FENCES list and initialize most of its data
   as a clean one.  */
void add_clean_fence_to_fences(flist_tail_t new_fences, insn_t succ,
                               fence_t fence) {
  int ready_ticks_size = get_max_uid() + 1;

  add_to_fences(new_fences, succ, state_create(), create_deps_context(),
                create_target_context(true), NULL_RTX, NULL,
                XCNEWVEC(int, ready_ticks_size), ready_ticks_size, NULL_RTX,
                FENCE_CYCLE(fence) + 1, 0, 1, FENCE_AFTER_STALL_P(fence));
}

/* Add a new fence to NEW_FENCES list and initialize all of its data
   from FENCE and SUCC.  */
void add_dirty_fence_to_fences(flist_tail_t new_fences, insn_t succ,
                               fence_t fence) {
  int *new_ready_ticks = XNEWVEC(int, FENCE_READY_TICKS_SIZE(fence));

  memcpy(new_ready_ticks, FENCE_READY_TICKS(fence),
         FENCE_READY_TICKS_SIZE(fence) * sizeof(int));
  add_to_fences(new_fences, succ, state_create_copy(FENCE_STATE(fence)),
                ...
                new_ready_ticks, FENCE_READY_TICKS_SIZE(fence),
                FENCE_SCHED_NEXT(fence), FENCE_CYCLE(fence),
                FENCE_ISSUED_INSNS(fence), FENCE_STARTS_CYCLE_P(fence),
...
void return_regset_to_pool(regset rs) {
  regset_pool.diff--;
...
static int cmp_v_in_regset_pool(const void *x, const void *xx) {
...
insn_t get_nop_from_pool(insn_t insn) {
  insn_t nop;
...
void return_nop_to_pool(insn_t nop) {
  gcc_assert(INSN_IN_STREAM_P(nop));
  sel_remove_insn(nop, false, true);

  if (nop_pool.n == nop_pool.s)
    nop_pool.v = XRESIZEVEC(rtx, nop_pool.v, (nop_pool.s = 2 * nop_pool.s + 1));
...
static int skip_unspecs_callback(const_rtx *xx, const_rtx *yy, rtx *nx,
                                 rtx *ny) {
  const_rtx x = *xx;
  const_rtx y = *yy;

  if (GET_CODE(x) == UNSPEC &&
      (targetm.sched.skip_rtx_p == NULL || targetm.sched.skip_rtx_p(x))) {
    *nx = XVECEXP(x, 0, 0);
    *ny = CONST_CAST_RTX(y);
    return 1;
  }
  ...
  if (GET_CODE(y) == UNSPEC &&
      (targetm.sched.skip_rtx_p == NULL || targetm.sched.skip_rtx_p(y))) {
    *nx = CONST_CAST_RTX(x);
    *ny = XVECEXP(y, 0, 0);
    return 1;
  }
...
/* Callback, called from hash_rtx_cb.  Helps to hash UNSPEC rtx X in a correct
   way to support ia64 speculation.  When changes are needed, new rtx X and new
   mode NMODE are written, and the callback returns true.  */
static int hash_with_unspec_callback(const_rtx x,
                                     enum machine_mode mode ATTRIBUTE_UNUSED,
                                     rtx *nx, enum machine_mode *nmode) {
  if (GET_CODE(x) == UNSPEC && targetm.sched.skip_rtx_p &&
...
static bool lhs_and_rhs_separable_p(rtx lhs, rtx rhs) {
...
static void vinsn_init(vinsn_t vi, insn_t insn, bool force_unique_p) {
  ...
  if (VINSN_SEPARABLE_P(vi)) {
    ...
    VINSN_HASH(vi) = hash_rtx_cb(rhs, GET_MODE(rhs), NULL, NULL, false, hrcf);
    VINSN_HASH_RTX(vi) =
        hash_rtx_cb(VINSN_PATTERN(vi), VOIDmode, NULL, NULL, false, hrcf);
  } else {
    VINSN_HASH(vi) =
        hash_rtx_cb(VINSN_PATTERN(vi), VOIDmode, NULL, NULL, false, hrcf);
  ...
  if (insn_class >= 2 &&
      (!targetm.sched.get_insn_spec_ds ||
       ((targetm.sched.get_insn_spec_ds(insn) & BEGIN_CONTROL) == 0)))
...
/* Create and init VI from the INSN.  Use UNIQUE_P for determining the correct
   VINSN_TYPE (VI).  */
static vinsn_t vinsn_create(insn_t insn, bool force_unique_p) {
...
/* Return a copy of VI.  When REATTACH_P is true, detach VI and attach
   the copy.  */
vinsn_t vinsn_copy(vinsn_t vi, bool reattach_p) {
  rtx copy;
...
static void vinsn_delete(vinsn_t vi) {
...
bool vinsn_cond_branch_p(vinsn_t vi) {
  insn_t insn;
...
static int sel_insn_rtx_cost(rtx insn) {
  int cost;
...
/* Emit new insn after AFTER based on PATTERN and initialize its data from
   EXPR and SEQNO.  */
insn_t sel_gen_insn_from_rtx_after(rtx pattern, expr_t expr, int seqno,
                                   insn_t after) {
  insn_t new_insn;
...
insn_t sel_gen_recovery_insn_from_rtx_after(rtx pattern, expr_t expr, int seqno,
                                            insn_t after) {
  insn_t insn;
...
insn_t sel_gen_insn_from_expr_after(expr_t expr, vinsn_t vinsn, int seqno,
                                    insn_t after) {
  expr_t emit_expr;
  insn_t insn;
  int flags;

  emit_expr = set_insn_init(expr, vinsn ? vinsn : EXPR_VINSN(expr), seqno);
...
insn_t sel_move_insn(expr_t expr, int seqno, insn_t after) {
...
static bool find_in_history_vect_1(VEC(expr_history_def, heap) * vect,
  ...
  while (i <= j) {
    ...
    if (auid == uid
        /* When undoing transformation on a bookkeeping copy, the new vinsn
           ...
           substituted itself.  */
        && (!compare_vinsns || vinsn_equal_p(avinsn, new_vinsn))) {
      *indp = i;
      return true;
    } else if (auid > uid)
      break;
...
int find_in_history_vect(VEC(expr_history_def, heap) * vect, rtx insn,
                         vinsn_t new_vinsn, bool originators_p) {
  int ind;

  if (find_in_history_vect_1(vect, INSN_UID(insn), new_vinsn, false, &ind))
    return ind;
...
void insert_in_history_vect(VEC(expr_history_def, heap) * *pvect, unsigned uid,
                            enum local_trans_type type, vinsn_t old_expr_vinsn,
                            vinsn_t new_expr_vinsn, ds_t spec_ds) {
  ...
  if (res) {
    ...
    gcc_assert(phist->old_expr_vinsn == old_expr_vinsn ||
               (phist->new_expr_vinsn != new_expr_vinsn &&
                (vinsn_equal_p(phist->old_expr_vinsn, old_expr_vinsn))));
...
static void free_history_vect(VEC(expr_history_def, heap) * *pvect) {
  unsigned i;
  ...
  for (i = 0; VEC_iterate(expr_history_def, *pvect, i, phist); i++) {
...
bool vinsn_equal_p(vinsn_t x, vinsn_t y) {
...
/* Initialize EXPR.  */
static void init_expr(expr_t expr, vinsn_t vi, int spec, int use, int priority,
                      int sched_times, int orig_bb_index, ds_t spec_done_ds,
                      ds_t spec_to_check_ds, int orig_sched_cycle,
                      VEC(expr_history_def, heap) * history,
                      bool target_available, bool was_substituted,
                      bool was_renamed, bool needs_spec_check_p,
                      bool cant_move) {
  vinsn_attach(vi);
...
void copy_expr(expr_t to, expr_t from) {
  ...
  if (EXPR_HISTORY_OF_CHANGES(from)) {
    ...
    for (i = 0; VEC_iterate(expr_history_def, temp, i, phist); i++) {
  ...
  init_expr(to, EXPR_VINSN(from), EXPR_SPEC(from), EXPR_USEFULNESS(from),
            ...
            EXPR_SPEC_TO_CHECK_DS(from), EXPR_ORIG_SCHED_CYCLE(from), temp,
...
void copy_expr_onside(expr_t to, expr_t from) {
  init_expr(to, EXPR_VINSN(from), EXPR_SPEC(from), EXPR_USEFULNESS(from),
...
static void prepare_insn_expr(insn_t insn, int seqno) {
...
/* Update target_available bits when merging exprs TO and FROM.  SPLIT_POINT
   ...
   a split point.  */
static void update_target_availability(expr_t to, expr_t from,
                                       insn_t split_point) {
  if (EXPR_TARGET_AVAILABLE(to) < 0 || EXPR_TARGET_AVAILABLE(from) < 0)
...
/* Update speculation bits when merging exprs TO and FROM.  SPLIT_POINT
   is non-null when expressions are merged from different successors at
   a split point.  */
static void update_speculative_bits(expr_t to, expr_t from,
                                    insn_t split_point) {
    ...
    if (old_to_ds != old_from_ds) {
      ...
      if (split_point != NULL) {
        ...
        insert_in_history_vect(&EXPR_HISTORY_OF_CHANGES(to),
                               ...
                               EXPR_VINSN(from), EXPR_VINSN(to), record_ds);
      }
    }
  }
}
...
void merge_expr_data(expr_t to, expr_t from, insn_t split_point) {
  int i;
  expr_history_def *phist;
  ...
  if (split_point)
    ...
    EXPR_USEFULNESS(to) = MAX(EXPR_USEFULNESS(to), EXPR_USEFULNESS(from));
  ...
  EXPR_ORIG_SCHED_CYCLE(to) =
      MIN(EXPR_ORIG_SCHED_CYCLE(to), EXPR_ORIG_SCHED_CYCLE(from));

  /* We keep this vector sorted.  */
  for (i = 0;
       VEC_iterate(expr_history_def, EXPR_HISTORY_OF_CHANGES(from), i, phist);
       i++)
    insert_in_history_vect(&EXPR_HISTORY_OF_CHANGES(to), phist->uid,
                           phist->type, phist->old_expr_vinsn,
                           phist->new_expr_vinsn, phist->spec_ds);
...
void merge_expr(expr_t to, expr_t from, insn_t split_point) {
  ...
  if (EXPR_SPEC_DONE_DS(to) == 0 && EXPR_SPEC_DONE_DS(from) != 0)
...
static void set_unavailable_target_for_expr(expr_t expr, regset lv_set) {
  if (EXPR_SEPARABLE_P(expr)) {
    if (REG_P(EXPR_LHS(expr)) && bitmap_bit_p(lv_set, REGNO(EXPR_LHS(expr)))) {
      ...
      if (bitmap_bit_p(VINSN_REG_USES(EXPR_VINSN(expr)), REGNO(EXPR_LHS(expr))))
        EXPR_TARGET_AVAILABLE(expr) = -1;
      else
    ...
    EXECUTE_IF_SET_IN_REG_SET(VINSN_REG_SETS(EXPR_VINSN(expr)), 0, regno, rsi)
    if (bitmap_bit_p(lv_set, regno)) {
      EXPR_TARGET_AVAILABLE(expr) = false;
      break;
    }

    EXECUTE_IF_SET_IN_REG_SET(VINSN_REG_CLOBBERS(EXPR_VINSN(expr)), 0, regno,
                              rsi)
    if (bitmap_bit_p(lv_set, regno)) {
      EXPR_TARGET_AVAILABLE(expr) = false;
      break;
...
int speculate_expr(expr_t expr, ds_t ds) {
  int res;
  rtx orig_insn_rtx;
  rtx spec_pat;
...
unsigned expr_dest_regno(expr_t expr) {
...
void mark_unavailable_targets(av_set_t join_set, av_set_t av_set,
                              regset lv_set) {
  expr_t expr;
  av_set_iterator avi;
...
static av_set_t av_set_add_element(av_set_t *setp) {
...
/* Add EXPR to SETP.  */
void av_set_add(av_set_t *setp, expr_t expr) {
  av_set_t elem;
...
static void av_set_add_nocopy(av_set_t *setp, expr_t expr) {
  av_set_t elem;
...
void av_set_iter_remove(av_set_iterator *ip) {
...
/* Search for an expr in SET, such that it's equivalent to SOUGHT_VINSN in the
   ...
   in SET was found.  */
expr_t av_set_lookup(av_set_t set, vinsn_t sought_vinsn) {
  expr_t expr;
  av_set_iterator i;
...
static expr_t av_set_lookup_and_remove(av_set_t *setp, vinsn_t sought_vinsn) {
  expr_t expr;
  av_set_iterator i;
...
static expr_t av_set_lookup_other_equiv_expr(av_set_t set, expr_t expr) {
  expr_t cur_expr;
  av_set_iterator i;
...
expr_t merge_with_other_exprs(av_set_t *avp, av_set_iterator *ip, expr_t expr) {
  expr_t expr2;
...
bool av_set_is_in_p(av_set_t set, vinsn_t vi) {
  return av_set_lookup(set, vi) != NULL;
}

/* Return a copy of SET.  */
av_set_t av_set_copy(av_set_t set) {
  expr_t expr;
  av_set_iterator i;
  av_set_t res = NULL;
...
static void join_distinct_sets(av_set_t *to_tailp, av_set_t *fromp) {
...
void av_set_union_and_clear(av_set_t *top, av_set_t *fromp, insn_t insn) {
  expr_t expr1;
  av_set_iterator i;
...
void av_set_union_and_live(av_set_t *top, av_set_t *fromp, regset to_lv_set,
  ...
  FOR_EACH_EXPR_1(expr1, i, top) {
    ...
    if (expr2) {
      ...
      if (EXPR_SEPARABLE_P(expr1)) {
        int regno1 =
            (REG_P(EXPR_LHS(expr1)) ? (int)expr_dest_regno(expr1) : -1);
        int regno2 =
            (REG_P(EXPR_LHS(expr2)) ? (int)expr_dest_regno(expr2) : -1);
...
void av_set_clear(av_set_t *setp) {
  expr_t expr;
  av_set_iterator i;
...
void av_set_leave_one_nonspec(av_set_t *setp) {
  expr_t expr;
  av_set_iterator i;
...
expr_t av_set_element(av_set_t set, int n) {
  expr_t expr;
  av_set_iterator i;
...
void av_set_substract_cond_branches(av_set_t *avp) {
  av_set_iterator i;
  expr_t expr;
...
/* Multiplies usefulness attribute of each member of av-set *AVP by
   value PROB / ALL_PROB.  */
void av_set_split_usefulness(av_set_t av, int prob, int all_prob) {
  av_set_iterator i;
  expr_t expr;

  FOR_EACH_EXPR(expr, i, av)
  EXPR_USEFULNESS(expr) =
      (all_prob ? (EXPR_USEFULNESS(expr) * prob) / all_prob : 0);
}

/* Leave in AVP only those expressions, which are present in AV,
   and return it.  */
void av_set_intersect(av_set_t *avp, av_set_t av) {
  av_set_iterator i;
  expr_t expr;
...
/* Setup ID for INSN.  FORCE_UNIQUE_P is true when INSN should not be
   clonable.  */
static void setup_id_for_insn(idata_t id, insn_t insn, bool force_unique_p) {
  int type;
...
static void deps_init_id_start_insn(insn_t insn) {
...
static void deps_init_id_start_lhs(rtx lhs) {
...
static void deps_init_id_finish_lhs(void) {
  deps_init_id_data.where = DEPS_IN_INSN;
}

/* Note a set of REGNO.  */
static void deps_init_id_note_reg_set(int regno) {
...
static void deps_init_id_note_reg_clobber(int regno) {
...
/* Note a use of REGNO.  */
static void deps_init_id_note_reg_use(int regno) {
...
static void deps_init_id_start_rhs(rtx rhs) {
  gcc_assert(deps_init_id_data.where == DEPS_IN_INSN);
...
static void deps_init_id_finish_rhs(void) {
...
static void deps_init_id_finish_insn(void) {
  gcc_assert(deps_init_id_data.where == DEPS_IN_INSN);
...
static void setup_id_lhs_rhs(idata_t id, insn_t insn, bool force_unique_p) {
  rtx pat = PATTERN(insn);

  if (GET_CODE(insn) == INSN && GET_CODE(pat) == SET && !force_unique_p) {
...
static void maybe_downgrade_id_to_use(idata_t id, insn_t insn) {
  ...
  for (rec = DF_INSN_UID_DEFS(uid); *rec; rec++) {
    ...
    if (DF_REF_INSN(def) && DF_REF_FLAGS_IS_SET(def, DF_REF_PRE_POST_MODIFY) &&
...
static void setup_id_reg_sets(idata_t id, insn_t insn) {
  unsigned uid = INSN_UID(insn);
  df_ref *rec;
  ...
  for (rec = DF_INSN_UID_DEFS(uid); *rec; rec++) {
    ...
    if (DF_REF_FLAGS_IS_SET(def,
                            (DF_REF_MUST_CLOBBER | DF_REF_PRE_POST_MODIFY)))
...
static void init_id_from_df(idata_t id, insn_t insn, bool force_unique_p) {
...
static void deps_init_id(idata_t id, insn_t insn, bool force_unique_p) {
  ...
  memcpy(&deps_init_id_sched_deps_info, &const_deps_init_id_sched_deps_info,
...
static void free_transformed_insns(void *p) {
...
static void init_first_time_insn_data(insn_t insn) {
  ...
  if (!INSN_NOP_P(insn)) {
    ...
    INSN_TRANSFORMED_INSNS(insn) =
        htab_create(16, hash_transformed_insns, eq_transformed_insns,
...
static void free_first_time_insn_data(insn_t insn) {
  gcc_assert(!first_time_insn_init(insn));
...
static void init_global_and_expr_for_bb(basic_block bb) {
...
static void init_global_and_expr_for_insn(insn_t insn) {
  if (LABEL_P(insn))
    return;
  ...
  if (GET_CODE(PATTERN(insn)) == ASM_INPUT || asm_noperands(PATTERN(insn)) >= 0)
    /* Mark INSN as an asm.  */
...

  {
    ...
    if (CANT_MOVE(insn) || INSN_ASM_P(insn) || SCHED_GROUP_P(insn) ||
...
void sel_init_global_and_expr(bb_vec_t bbs) {
...
static void finish_global_and_expr_for_bb(basic_block bb) {
  av_set_clear(&BB_AV_SET(bb));
  BB_AV_LEVEL(bb) = 0;
}

/* Finalize INSN's data.  */
static void finish_global_and_expr_insn(insn_t insn) {
...
static void has_dependence_start_insn(insn_t insn ATTRIBUTE_UNUSED) {
...
static void has_dependence_finish_insn(void) {
...
static void has_dependence_start_lhs(rtx lhs ATTRIBUTE_UNUSED) {
  gcc_assert(has_dependence_data.where == DEPS_IN_INSN);
...
static void has_dependence_finish_lhs(void) {
  has_dependence_data.where = DEPS_IN_INSN;
}

/* Start analyzing dependencies of RHS.  */
static void has_dependence_start_rhs(rtx rhs ATTRIBUTE_UNUSED) {
  gcc_assert(has_dependence_data.where == DEPS_IN_INSN);
...
static void has_dependence_finish_rhs(void) {
...
/* Note a set of REGNO.  */
static void has_dependence_note_reg_set(int regno) {
  struct deps_reg *reg_last = &has_dependence_data.dc->reg_last[regno];

  if (!sched_insns_conditions_mutex_p(
          has_dependence_data.pro, VINSN_INSN_RTX(has_dependence_data.con))) {
    ds_t *dsp = &has_dependence_data.has_dep_p[has_dependence_data.where];

    if (reg_last->sets != NULL || reg_last->clobbers != NULL)
...
/* Note a clobber of REGNO.  */
static void has_dependence_note_reg_clobber(int regno) {
  struct deps_reg *reg_last = &has_dependence_data.dc->reg_last[regno];
...
/* Note a use of REGNO.  */
static void has_dependence_note_reg_use(int regno) {
  struct deps_reg *reg_last = &has_dependence_data.dc->reg_last[regno];
  ...
  if (!sched_insns_conditions_mutex_p(
    ...
    if (reg_last->uses) {
      ...
      if (pro_spec_checked_ds != 0)
        ...
        *dsp = ds_full_merge(*dsp, pro_spec_checked_ds, NULL_RTX, NULL_RTX);
    }
  }
}

/* Note a memory dependence.  */
static void has_dependence_note_mem_dep(rtx mem ATTRIBUTE_UNUSED,
...
/* Note a dependence.  */
static void has_dependence_note_dep(insn_t pro ATTRIBUTE_UNUSED,
                                    ds_t ds ATTRIBUTE_UNUSED) {
  if (!sched_insns_conditions_mutex_p(
          has_dependence_data.pro, VINSN_INSN_RTX(has_dependence_data.con))) {
    ds_t *dsp = &has_dependence_data.has_dep_p[has_dependence_data.where];
...
void sel_mark_hard_insn(rtx insn) {
  int i;
...
static void setup_has_dependence_sched_deps_info(void) {
  memcpy(&has_dependence_sched_deps_info, &const_has_dependence_sched_deps_info,
...
ds_t has_dependence_p(expr_t expr, insn_t pred, ds_t **has_dep_pp) {
  int i;
  ds_t ds;
  struct deps *dc;
  ...
  if (EXPR_LHS(expr) && MEM_P(EXPR_LHS(expr)) &&
  ...
  *has_dep_pp = has_dependence_data.has_dep_p;
  ...
  for (i = 0; i < DEPS_IN_NOWHERE; i++)
    ds =
        ds_full_merge(ds, has_dependence_data.has_dep_p[i], NULL_RTX, NULL_RTX);
...
static void tick_check_dep_with_dw(insn_t pro_insn, ds_t ds, dw_t dw) {
  ...
  if (con_insn != pro_insn) {
    ...
    if (/* PROducer was removed from above due to pipelining.  */
        ...
        /* Or PROducer was originally on the next iteration regarding the
           CONsumer.  */
        || (INSN_SCHED_TIMES(pro_insn) - EXPR_SCHED_TIMES(con_expr)) > 1)
...
static void tick_check_note_dep(insn_t pro, ds_t ds) {
  tick_check_dep_with_dw(pro, ds, 0);
}

/* An implementation of note_mem_dep hook.  */
static void tick_check_note_mem_dep(rtx mem1, rtx mem2, insn_t pro, ds_t ds) {
  dw_t dw;

  dw = (ds_to_dt(ds) == REG_DEP_TRUE ? estimate_dep_weak(mem1, mem2) : 0);
...
int tick_check_p(expr_t expr, deps_t dc, fence_t fence) {
  int cycles_left;
...
/* Returns true if LHS of INSN is the same as DEST of an insn
   being moved.  */
bool lhs_of_insn_equals_to_dest_p(insn_t insn, rtx dest) {
...
sel_insn_data_def insn_sid(insn_t insn) {
  return *SID(insn);
}
...
/* True when INSN is a speculative check.  We can tell this by looking
   ...
   the pattern.  */
bool sel_insn_is_speculation_check(rtx insn) {
  return s_i_d && !!INSN_SPEC_CHECKED_DS(insn);
}

/* Extracts machine mode MODE and destination location DST_LOC
   for given INSN.  */
void get_dest_and_mode(rtx insn, rtx *dst_loc, enum machine_mode *mode) {
  rtx pat = PATTERN(insn);
...
/* Returns true when moving through JUMP will result in bookkeeping
   creation.  */
bool bookkeeping_can_be_created_if_moved_through_p(insn_t jump) {
  insn_t succ;
  succ_iterator si;
...
static bool insn_is_the_only_one_in_bb_p(insn_t insn) {
...
/* Check that the region we're scheduling still has at most one
   backedge.  */
static void verify_backedges(void) {
  if (pipelining_p) {
    int i, n = 0;
    edge e;
    edge_iterator ei;
    ...
    if (in_current_region_p(e->dest) && BLOCK_TO_BB(e->dest->index) < i)
      n++;
...
bool maybe_tidy_empty_bb(basic_block bb) {
  ...
  if (!sel_bb_empty_p(bb) ||
      (single_succ_p(bb) && single_succ(bb) == EXIT_BLOCK_PTR &&
       (!single_pred_p(bb) || !(single_pred_edge(bb)->flags & EDGE_FALLTHRU))))
    return false;
...
bool tidy_control_flow(basic_block xbb, bool full_tidying) {
  bool changed = true;
  ...
  if (jump_leads_only_to_bb_p(BB_END(xbb), xbb->next_bb) &&
      INSN_SCHED_TIMES(BB_END(xbb)) == 0 && !IN_CURRENT_FENCE_P(BB_END(xbb))) {
  ...
  /* Check if there is an unnecessary jump in previous basic block leading
     ...
     can be harmful.  */
  if (sel_bb_head(xbb) == sel_bb_end(xbb) && !sel_bb_empty_p(xbb) &&
...
static int sel_estimate_number_of_insns(basic_block bb) {
  int res = 0;
...
static int sel_luid_for_non_insn(rtx x) {
...
static int get_seqno_of_a_pred(insn_t insn) {
  int seqno;
  ...
  if (!sel_bb_head_p(insn))
    ...
    if (single_pred_p(bb) && !in_current_region_p(single_pred(bb))) {
...
int get_seqno_by_preds(rtx insn) {
  basic_block bb = BLOCK_FOR_INSN(insn);
...
static void extend_region_bb_info(void) {
...
static void extend_bb_info(void) {
...
/* Finalize region-scope data structures for basic blocks.  */
static void finish_region_bb_info(void) {
...
static void extend_insn_data(void) {
  ...
  /* Extend data structures for insns from current region.  */
  reserve = (sched_max_luid + 1 - VEC_length(sel_insn_data_def, s_i_d));
  if (reserve > 0 && !VEC_space(sel_insn_data_def, s_i_d, reserve))
...
static expr_t set_insn_init(expr_t expr, vinsn_t vi, int seqno) {
...
/* Init data for INSN.  */
static void init_insn_data(insn_t insn) {
  expr_t expr;
  ...
  gcc_assert(!ssid->asm_p && ssid->sched_next == NULL && !ssid->after_stall_p &&
             ssid->sched_cycle == 0);
...
/* This is used to initialize spurious jumps generated by
   sel_redirect_edge ().  */
static void init_simplejump_data(insn_t insn) {
  init_expr(INSN_EXPR(insn), vinsn_create(insn, false), 0, REG_BR_PROB_BASE, 0,
            0, 0, 0, 0, 0, NULL, true, false, false, false, true);
...
void sel_init_new_insn(insn_t insn, int flags) {
  /* We create data structures for bb when the first insn is emitted in it.  */
  if (INSN_P(insn) && INSN_IN_STREAM_P(insn) &&
  ...
  gcc_assert(CONTAINING_RGN(BLOCK_NUM(insn)) == CONTAINING_RGN(BB_TO_BLOCK(0)));
}

/* Functions to init/finish work with lv sets.  */

/* Init BB_LV_SET of BB from DF_LR_IN set of BB.  */
static void init_lv_set(basic_block bb) {
...
static void copy_lv_set_from(basic_block bb, basic_block from_bb) {
  gcc_assert(!BB_LV_SET_VALID_P(bb));
...
static void free_lv_set(basic_block bb) {
...
static void invalidate_av_set(basic_block bb) {
  gcc_assert(BB_AV_LEVEL(bb) <= 0 && BB_AV_SET(bb) == NULL);

  BB_AV_LEVEL(bb) = -1;
}

/* Create initial data sets for BB (they will be invalid).  */
static void create_initial_data_sets(basic_block bb) {
  if (BB_LV_SET(bb))
    BB_LV_SET_VALID_P(bb) = false;
  else
    BB_LV_SET(bb) = get_regset_from_pool();
...
/* Free av set of BB.  */
static void free_av_set(basic_block bb) {
  av_set_clear(&BB_AV_SET(bb));
  BB_AV_LEVEL(bb) = 0;
}

/* Free data sets of BB.  */
void free_data_sets(basic_block bb) {
  free_lv_set(bb);
  free_av_set(bb);
}

/* Exchange lv sets of TO and FROM.  */
static void exchange_lv_sets(basic_block to, basic_block from) {
  {
...
static void exchange_av_sets(basic_block to, basic_block from) {
  {
...
void exchange_data_sets(basic_block to, basic_block from) {
  exchange_lv_sets(to, from);
  exchange_av_sets(to, from);
}

/* Copy data sets of FROM to TO.  */
void copy_data_sets(basic_block to, basic_block from) {
...
av_set_t get_av_set(insn_t insn) {
  av_set_t av_set;
...
insn_t sel_bb_head(basic_block bb) {
  insn_t head;
...
basic_block fallthru_bb_of_jump(rtx jump) {
  if (!JUMP_P(jump))
    return NULL;
...
static void init_bb(basic_block bb) {
  remove_notes(bb_note(bb), BB_END(bb));
  BB_NOTE_LIST(bb) = note_list;
}

void sel_init_bbs(bb_vec_t bbs, basic_block bb) {
  const struct sched_scan_info_def ssi = {
...
static void sel_restore_other_notes(void) {
  int bb;
...
bool sel_insn_has_single_succ_p(insn_t insn, int flags) {
  insn_t succ;
  succ_iterator si;
  bool first_p = true;
...
static struct succs_info *alloc_succs_info(void) {
...
void free_succs_info(struct succs_info *sinfo) {
  gcc_assert(succs_info_pool.top >= 0 &&
             &succs_info_pool.stack[succs_info_pool.top] == sinfo);
  succs_info_pool.top--;

  /* Clear stale info.  */
  VEC_block_remove(rtx, sinfo->succs_ok, 0, VEC_length(rtx, sinfo->succs_ok));
  VEC_block_remove(rtx, sinfo->succs_other, 0,
                   VEC_length(rtx, sinfo->succs_other));
  VEC_block_remove(int, sinfo->probs_ok, 0, VEC_length(int, sinfo->probs_ok));
  sinfo->all_prob = 0;
  sinfo->succs_ok_n = 0;
  sinfo->all_succs_n = 0;
}
...
struct succs_info *compute_succs_info(insn_t insn, short flags) {
  succ_iterator si;
  insn_t succ;
...
static void cfg_preds_1(basic_block bb, insn_t **preds, int *n, int *size) {
  edge e;
  edge_iterator ei;
  ...
  FOR_EACH_EDGE(e, ei, bb->preds) {
    ...
    if (sel_bb_empty_p(pred_bb))
      ...
      if (*n == *size)
        *preds = XRESIZEVEC(insn_t, *preds, (*size = 2 * *size + 1));
      (*preds)[(*n)++] = bb_end;
    }
  }
...
/* Find all predecessors of BB and record them in PREDS and their number
   ...
   edges are processed.  */
static void cfg_preds(basic_block bb, insn_t **preds, int *n) {
  int size = 0;
...
bool sel_num_cfg_preds_gt_1(insn_t insn) {
  basic_block bb;
...
/* Returns true when BB should be the end of an ebb.  Adapted from the
   code in sched-ebb.c.  */
bool bb_ends_ebb_p(basic_block bb) {
...
/* Returns true when INSN and SUCC are in the same EBB, given that SUCC is a
   successor of INSN.  */
bool in_same_ebb_p(insn_t insn, insn_t succ) {
...
static void recompute_rev_top_order(void) {
  int *postorder;
  int n_blocks, i;
  ...
  if (!rev_top_order_index || rev_top_order_index_len < last_basic_block) {
    ...
    rev_top_order_index =
        XRESIZEVEC(int, rev_top_order_index, rev_top_order_index_len);
  }
...
void clear_outdated_rtx_info(basic_block bb) {
  rtx insn;
...
static void return_bb_to_pool(basic_block bb) {
  rtx note = bb_note(bb);

  gcc_assert(NOTE_BASIC_BLOCK(note) == bb && bb->aux == NULL);
...
static rtx get_bb_note_from_pool(void) {
...
void alloc_sched_pools(void) {
  ...
  sched_lists_pool =
      create_alloc_pool("sel-sched-lists", sizeof(struct _list_node), 500);
}
...
/* Returns a position in RGN where BB can be inserted retaining
   topological order.  */
static int find_place_to_insert_bb(basic_block bb, int rgn) {
  ...
  if (flag_sel_sched_pipelining_outer_loops &&
    ...
    for (i = RGN_NR_BLOCKS(rgn) - 1; i >= 0; i--) {
      ...
      if (rev_top_order_index[bbi] < rev_top_order_index[cur_bbi])
        break;
    }
...
static void delete_and_free_basic_block(basic_block bb) {
  ...
  gcc_assert(BB_LV_SET(bb) == NULL && !BB_LV_SET_VALID_P(bb) &&
             BB_AV_LEVEL(bb) == 0 && BB_AV_SET(bb) == NULL);

  delete_basic_block(bb);
}

/* Add BB to the current region and update the region data.  */
static void add_block_to_current_region(basic_block bb) {
  int i, pos, bbi = -2, rgn;
  ...
  gcc_assert(RGN_HAS_REAL_EBB(rgn) == 0 && ebb_head[bbi] == pos);
  ...
  memmove(rgn_bb_table + pos + 1, rgn_bb_table + pos,
...
static void remove_bb_from_region(basic_block bb) {
  int i, pos, bbi = -2, rgn;
  ...
  memmove(rgn_bb_table + pos, rgn_bb_table + pos + 1,
          (RGN_BLOCKS(nr_regions) - pos) * sizeof(*rgn_bb_table));
...
static void sel_add_bb(basic_block bb) {
  ...
  if (bb != NULL) {
    gcc_assert(VEC_length(basic_block, last_added_blocks) == 1 &&
               VEC_index(basic_block, last_added_blocks, 0) == bb);
  ...
  /* BB is NULL - process LAST_ADDED_BLOCKS instead.  */
  {
    ...
    for (i = 0; VEC_iterate(basic_block, last_added_blocks, i, bb); i++) {
      add_block_to_current_region(bb);
      temp_bb = bb;
    }
...
static void sel_remove_bb(basic_block bb, bool remove_from_cfg_p) {
...
static void move_bb_info(basic_block merge_bb, basic_block empty_bb) {
  gcc_assert(in_current_region_p(merge_bb));

  concat_note_lists(BB_NOTE_LIST(empty_bb), &BB_NOTE_LIST(merge_bb));
...
/* Remove an empty basic block EMPTY_BB.  When MERGE_UP_P is true, we put
   ...
   the empty block.  */
void sel_remove_empty_bb(basic_block empty_bb, bool merge_up_p,
  ...
  if (merge_up_p) {
    ...
    /* Redirect incoming edges (except fallthrough one) of EMPTY_BB to its
       successor block.  */
    for (ei = ei_start(empty_bb->preds); (e = ei_safe_edge(ei));) {
      if (!(e->flags & EDGE_FALLTHRU))
...
static void remove_empty_bb(basic_block empty_bb, bool remove_from_cfg_p) {
  ...
  if (remove_from_cfg_p &&
      (EDGE_COUNT(empty_bb->preds) > 0 || EDGE_COUNT(empty_bb->succs) > 0)) {
    basic_block pred;
    basic_block succ;
    ...
    if (EDGE_COUNT(empty_bb->preds) > 0) {
      ...
      gcc_assert(e->src == empty_bb->prev_bb && (e->flags & EDGE_FALLTHRU));
...
static basic_block sel_create_basic_block(void *headp, void *endp,
                                          basic_block after) {
  basic_block new_bb;
  insn_t new_bb_note;
  ...
  if (new_bb_note == NULL_RTX)
    ...
    new_bb =
        create_basic_block_structure((rtx)headp, (rtx)endp, new_bb_note, after);
    new_bb->aux = NULL;
  }
...
static void sel_init_only_bb(basic_block bb, basic_block after) {
...
static void change_loops_latches(basic_block from, basic_block to) {
  gcc_assert(from != to);
...
static basic_block sel_split_block(basic_block bb, rtx after) {
  basic_block new_bb;
  insn_t insn;
  ...
  if (!sel_bb_empty_p(new_bb) && bitmap_bit_p(blocks_to_reschedule, bb->index))
...
static rtx check_for_new_jump(basic_block bb, int prev_max_uid) {
  rtx end;
...
/* Look for a new jump either in FROM_BB block or in newly created JUMP_BB
   block. New means having UID at least equal to PREV_MAX_UID.  */
static rtx find_new_jump(basic_block from, basic_block jump_bb,
                         int prev_max_uid) {
  rtx jump;
  ...
  if (jump_bb != NULL && (jump = check_for_new_jump(jump_bb, prev_max_uid)))
    return jump;
  return NULL;
}
...
basic_block sel_split_edge(edge e) {
...
static basic_block sel_create_empty_bb(basic_block after) {
  basic_block new_bb;
...
basic_block sel_create_recovery_block(insn_t orig_insn) {
...
void sel_merge_blocks(basic_block a, basic_block b) {
...
void sel_redirect_edge_and_branch_force(edge e, basic_block to) {
  ...
  if (current_loop_nest && pipelining_p)
...
void sel_redirect_edge_and_branch(edge e, basic_block to) {
  bool latch_edge_p;
  basic_block src;
  int prev_max_uid;
  rtx jump;

  latch_edge_p = (pipelining_p && current_loop_nest &&
...
rtx create_insn_rtx_from_pattern(rtx pattern, rtx label) {
  rtx insn_rtx;
...
vinsn_t create_vinsn_from_insn_rtx(rtx insn_rtx, bool force_unique_p) {
...
rtx create_copy_of_insn_rtx(rtx insn_rtx) {
  rtx res;

  gcc_assert(NONJUMP_INSN_P(insn_rtx));

  res = create_insn_rtx_from_pattern(copy_rtx(PATTERN(insn_rtx)), NULL_RTX);
  return res;
}

/* Change vinsn field of EXPR to hold NEW_VINSN.  */
void change_vinsn_in_expr(expr_t expr, vinsn_t new_vinsn) {
  vinsn_detach(EXPR_VINSN(expr));
...
void setup_nop_and_exit_insns(void) {
  gcc_assert(nop_pattern == NULL_RTX && exit_insn == NULL_RTX);
...
void sel_setup_sched_infos(void) {
  ...
  sel_common_sched_info.estimate_number_of_insns = sel_estimate_number_of_insns;
...
static void sel_add_block_to_region(basic_block bb, int *bb_ord_index,
                                    int rgn) {
  RGN_NR_BLOCKS(rgn) += 1;
...
static int sel_create_new_region(void) {
  ...
  if (new_rgn_number != 0)
    RGN_BLOCKS(new_rgn_number) =
        RGN_BLOCKS(new_rgn_number - 1) + RGN_NR_BLOCKS(new_rgn_number - 1);
  else
  ...
  RGN_BLOCKS(new_rgn_number + 1) =
      RGN_BLOCKS(new_rgn_number) + RGN_NR_BLOCKS(new_rgn_number);
...
static int bb_top_order_comparator(const void *x, const void *y) {
  basic_block bb1 = *(const basic_block *)x;
  basic_block bb2 = *(const basic_block *)y;

  gcc_assert(bb1 == bb2 || rev_top_order_index[bb1->index] !=
...
static int make_region_from_loop(struct loop *loop) {
  unsigned int i;
  ...
  if (loop->num_nodes > (unsigned)PARAM_VALUE(PARAM_MAX_PIPELINE_REGION_BLOCKS))
    return -1;
  ...
  for (i = 0; i < loop->num_nodes; i++) {
    ...
    if (!TEST_BIT(bbs_in_loop_rgns, loop_blocks[i]->index)) {
      sel_add_block_to_region(loop_blocks[i], &bb_ord_index, new_rgn_number);
...
void make_region_from_loop_preheader(VEC(basic_block, heap) * *loop_blocks) {
  unsigned int i;
  int new_rgn_number = -1;
  basic_block bb;
...
/* Create region(s) from loop nest LOOP, such that inner loops will be
   ...
   is created.  */
static bool make_regions_from_loop_nest(struct loop *loop) {
...
void sel_init_pipelining(void) {
  ...
  loop_optimizer_init(LOOPS_HAVE_PREHEADERS | LOOPS_HAVE_FALLTHRU_PREHEADERS |
...
loop_p get_loop_nest_for_rgn(unsigned int rgn) {
...
bool considered_for_pipelining_p(struct loop *loop) {
...
/* Makes regions from the rest of the blocks, after loops are chosen
   for pipelining.  */
static void make_regions_from_the_rest(void) {
  int cur_rgn_blocks;
  int *loop_hdr;
  int i;
...
void sel_find_rgns(void) {
  ...
  if (current_loops) {
    ...
    FOR_EACH_LOOP(li, loop,
                  (flag_sel_sched_pipelining_outer_loops ? LI_FROM_INNERMOST
                                                         : LI_ONLY_INNERMOST))
...
void sel_add_loop_preheaders(void) {
  int i;
  basic_block bb;
...
bool sel_is_loop_preheader_p(basic_block bb) {
  if (current_loop_nest) {
    struct loop *outer;
    ...
    /* Support the situation when the latch block of outer loop
       could be from here.  */
    for (outer = loop_outer(current_loop_nest); outer;
...
static void sel_remove_loop_preheader(void) {
  ...
  for (i = VEC_length(basic_block, preheader_blocks) - 1; i >= old_len; i--) {
  ...
  if (!considered_for_pipelining_p(loop_outer(current_loop_nest))) {
    if (!all_empty_p)
      ...
      for (i = 0; VEC_iterate(basic_block, preheader_blocks, i, bb); i++) {
        ...
        if (next_bb->prev_bb == prev_bb && prev_bb != ENTRY_BLOCK_PTR &&
    ...
    SET_LOOP_PREHEADER_BLOCKS(loop_outer(current_loop_nest), preheader_blocks);
}
#endif
