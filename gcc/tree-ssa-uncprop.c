    ...
    /* For a SWITCH_EXPR, a case label which represents a single
      ...
      if (TREE_CODE(cond) == SSA_NAME &&
        ...
        for (i = 0; i < n_labels; i++) {
          ...
          if (CASE_HIGH(label) || !CASE_LOW(label) || info[bb->index])
...
static hashval_t equiv_hash(const void *p) {
  tree const value = ((const struct equiv_hash_elt *)p)->value;
  return iterative_hash_expr(value, 0);
}

static int equiv_eq(const void *p1, const void *p2) {
...
static unsigned int tree_ssa_uncprop(void) {
...
static void uncprop_finalize_block(struct dom_walk_data *walk_data
                                       ATTRIBUTE_UNUSED,
...
static void uncprop_initialize_block(
    struct dom_walk_data *walk_data ATTRIBUTE_UNUSED, basic_block bb) {
  basic_block parent;
  edge e;
  bool recorded = false;
...
static bool gate_uncprop(void) {
  return flag_tree_dom != 0;
}

struct gimple_opt_pass pass_uncprop = {{
    GIMPLE_PASS, "uncprop",          /* name */
    gate_uncprop,                    /* gate */
