...
static int reload_reg_free_for_value_p(int, int, int, enum reload_type, rtx,
                                       rtx, int, int);
static int free_for_value_p(int, enum machine_mode, int, enum reload_type, rtx,
                            rtx, int, int);
...
static void emit_input_reload_insns(struct insn_chain *, struct reload *, rtx,
                                    int);
...
void init_reload(void) {
  ...
  rtx tem = gen_rtx_MEM(
      Pmode, gen_rtx_PLUS(Pmode, gen_rtx_REG(Pmode, LAST_VIRTUAL_REGISTER + 1),
                          GEN_INT(4)));
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++) {
    tem = gen_rtx_PLUS(Pmode, gen_rtx_REG(Pmode, HARD_FRAME_POINTER_REGNUM),
                       gen_rtx_REG(Pmode, i));
...
static void replace_pseudos_in(rtx *loc, enum machine_mode mem_mode,
                               rtx usage) {
  rtx x = *loc;
  enum rtx_code code;
  const char *fmt;
...
int reload(rtx first, int global) {
  ...
  offsets_at = (HOST_WIDE_INT(*)[NUM_ELIMINABLE_REGS])xmalloc(
      num_labels * NUM_ELIMINABLE_REGS * sizeof(HOST_WIDE_INT));
  ...
  for (i = FIRST_PSEUDO_REGISTER; i < max_regno; i++) {
    if (reg_renumber[i] < 0 && reg_equiv_init[i] != 0) {
      ...
      for (list = reg_equiv_init[i]; list; list = XEXP(list, 1)) {
        ...
        if (NOTE_P(equiv_insn) || can_throw_internal(equiv_insn))
          ;
...
failed:
  ...
  for (insn = first; insn; insn = NEXT_INSN(insn))
    if (INSN_P(insn)) {
      ...
      if (CALL_P(insn))
        replace_pseudos_in(&CALL_INSN_FUNCTION_USAGE(insn), VOIDmode,
      ...
      if (GET_CODE(PATTERN(insn)) == CLOBBER)
        replace_pseudos_in(&XEXP(PATTERN(insn), 0), VOIDmode, PATTERN(insn));
      ...
      if (NONJUMP_INSN_P(insn) && GET_CODE(PATTERN(insn)) == SET &&
          REG_P(SET_SRC(PATTERN(insn))) && REG_P(SET_DEST(PATTERN(insn))) &&
          (REGNO(SET_SRC(PATTERN(insn))) == REGNO(SET_DEST(PATTERN(insn))))) {
        delete_insn(insn);
        continue;
      }
        ...
        switch (c) {
          ...
          default:
            if (EXTRA_ADDRESS_CONSTRAINT(c, p))
              ...
              cls = (int)
                  reg_class_subunion[cls][(int)REG_CLASS_FROM_CONSTRAINT(c, p)];
        }
...
static void calculate_needs_all_insns(int global) {
  ...
  for (chain = reload_insn_chain; chain != 0; chain = next) {
    ...
    if (LABEL_P(insn) || JUMP_P(insn) || (INSN_P(insn) && REG_NOTES(insn) != 0))
    ...
    if (INSN_P(insn)) {
      ...
      if (flag_expensive_optimizations && n_reloads > 1) {
        rtx set = single_set(insn);
        if (set &&
            ((SET_SRC(set) == SET_DEST(set) && REG_P(SET_SRC(set)) &&
...
static void order_regs_for_reload(struct insn_chain *chain) {
  ...
  EXECUTE_IF_SET_IN_REG_SET(&chain->live_throughout, FIRST_PSEUDO_REGISTER, i,
                            rsi) {
    count_pseudo(i);
  }
  EXECUTE_IF_SET_IN_REG_SET(&chain->dead_or_set, FIRST_PSEUDO_REGISTER, i,
                            rsi) {
    count_pseudo(i);
  }
  CLEAR_REG_SET(&pseudos_counted);
...
static void count_spilled_pseudo(int spilled, int spilled_nregs, int reg) {
  ...
  /* Ignore spilled pseudo-registers which can be here only if IRA is
     used.  */
  if ((ira_conflicts_p && r < 0) || REGNO_REG_SET_P(&spilled_pseudos, reg) ||
...
static int find_reg(struct insn_chain *chain, int order) {
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++) {
    ...
    if (!TEST_HARD_REG_BIT(not_usable, regno) &&
      ...
      if (ira_conflicts_p) {
        ...
        if (best_reg < 0 || ira_better_spill_reload_regno_p(
                                regno_pseudo_regs, best_regno_pseudo_regs,
...
#else
              && call_used_regs[regno] && !call_used_regs[best_reg]
#endif
                  )) {
        best_reg = regno;
        best_cost = this_cost;
  ...
  EXECUTE_IF_SET_IN_REG_SET(&chain->live_throughout, FIRST_PSEUDO_REGISTER, j,
                            rsi) {
    count_spilled_pseudo(best_reg, rl->nregs, j);
  }

  EXECUTE_IF_SET_IN_REG_SET(&chain->dead_or_set, FIRST_PSEUDO_REGISTER, j,
                            rsi) {
    count_spilled_pseudo(best_reg, rl->nregs, j);
  }
...
static void spill_failure(rtx insn, enum reg_class rclass) {
  if (asm_noperands(PATTERN(insn)) >= 0)
    error_for_asm(insn,
...
static void alter_reg(int i, int from_reg, bool dont_share_p) {
  ...
  SET_REGNO(regno_reg_rtx[i], reg_renumber[i] >= 0 ? reg_renumber[i] : i);
  ...
  if (reg_renumber[i] < 0 && REG_N_REFS(i) > 0 && reg_equiv_constant[i] == 0 &&
    ...
    /* Each pseudo reg has an inherent size which comes from its own mode,
      ...
      x = assign_stack_local(
          mode, total_size,
          min_align > inherent_align || total_size > inherent_size ? -1 : 0);
      ...
      if (BYTES_BIG_ENDIAN) {
        ...
        if (adjust)
          stack_slot = adjust_address_nv(
              x, mode_for_size(total_size * BITS_PER_UNIT, MODE_INT, 1),
              adjust);
      }
...
static void mark_home_live_1(int regno, enum machine_mode mode) {
  int i, lim;
...
static void set_label_offsets(rtx x, rtx insn, int initial_p) {
  ...
  switch (code) {
    ...
    case CODE_LABEL:
      ...
      else if (x == insn && (tem = prev_nonnote_insn(insn)) != 0 &&
               BARRIER_P(tem))
    ...
    case ADDR_DIFF_VEC:
      ...
      for (i = 0; i < (unsigned)XVECLEN(x, code == ADDR_DIFF_VEC); i++)
        set_label_offsets(XVECEXP(x, code == ADDR_DIFF_VEC, i), insn,
                          initial_p);
      return;
...
static rtx eliminate_regs_1(rtx x, enum machine_mode mem_mode, rtx insn,
  ...
  switch (code) {
        ...
        for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS]; ep++)
          if (ep->from_rtx == XEXP(x, 0) && ep->can_eliminate) {
            ...
            if (mem_mode != 0 && GET_CODE(XEXP(x, 1)) == CONST_INT &&
              ...
              return gen_rtx_PLUS(
                  Pmode, ep->to_rtx,
                  plus_constant(XEXP(x, 1), ep->previous_offset));
          }
...

      {
        ...
        if (reg_renumber && (new0 != XEXP(x, 0) || new1 != XEXP(x, 1))) {
          ...
          if (GET_CODE(new0) == PLUS && REG_P(new1) &&
              ...
              reg_renumber[REGNO(new1)] < 0 && reg_equiv_constant != 0 &&
    ...
    case LTU: {
      ...
      rtx new1 =
          XEXP(x, 1) ? eliminate_regs_1(XEXP(x, 1), mem_mode, insn, false) : 0;
    ...
    case INSN_LIST:
      ...
      if (XEXP(x, 1)) {
        ...
        if (new_rtx != XEXP(x, 1))
          return gen_rtx_fmt_ee(GET_CODE(x), GET_MODE(x), XEXP(x, 0), new_rtx);
      }
      return x;
    ...
    case POST_MODIFY:
      ...
      if (GET_CODE(XEXP(x, 1)) == PLUS && XEXP(XEXP(x, 1), 0) == XEXP(x, 0)) {
        rtx new_rtx =
            eliminate_regs_1(XEXP(XEXP(x, 1), 1), mem_mode, insn, true);

        if (new_rtx != XEXP(XEXP(x, 1), 1))
          return gen_rtx_fmt_ee(code, GET_MODE(x), XEXP(x, 0),
                                gen_rtx_PLUS(GET_MODE(x), XEXP(x, 0), new_rtx));
      }
      return x;
    ...
    case MEM:
      ...
      return replace_equiv_address_nv(
          x, eliminate_regs_1(XEXP(x, 0), GET_MODE(x), insn, true));
    ...
    if (*fmt == 'e') {
      ...
      for (j = 0; j < XVECLEN(x, i); j++) {
        ...
        if (new_rtx != XVECEXP(x, i, j) && !copied_vec) {
          rtvec new_v = gen_rtvec_v(XVECLEN(x, i), XVEC(x, i)->elem);
          if (!copied) {
            x = shallow_copy_rtx(x);
            copied = 1;
          }
...
static int eliminate_regs_in_insn(rtx insn, int replace) {
    ...
    for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS]; ep++)
      if (ep->from_rtx == SET_DEST(old_set) && ep->can_eliminate) {
        ...
        if (ep->from == FRAME_POINTER_REGNUM &&
          ...
          if (base == ep->to_rtx) {
            rtx src = plus_constant(ep->to_rtx, offset - ep->offset);
            ...
            validate_change(insn, &SET_DEST(old_set), ep->to_rtx, 1);
  ...
  if (old_set && REG_P(SET_DEST(old_set))) {
    ...
    if (plus_src && GET_CODE(XEXP(plus_src, 1)) == CONST_INT)
      plus_cst_src = plus_src;
    else if (REG_P(SET_SRC(old_set)) || plus_src) {
      ...
      if (ep->from_rtx == reg && ep->can_eliminate) {
        ...
        if (offset == 0 || plus_src) {
          ...
          if (!validate_change(insn, &SET_SRC(old_set), new_src, 0)) {
            rtx new_pat = gen_rtx_SET(VOIDmode, SET_DEST(old_set), new_src);
  ...
  for (i = 0; i < recog_data.n_operands; i++) {
    ...
    if (insn_is_asm || insn_data[icode].operand[i].eliminable) {
      ...
      /* Check for setting a register that we know about.  */
      if (recog_data.operand_type[i] != OP_IN && REG_P(orig_operand[i])) {
      ...
      if (old_set && recog_data.operand_loc[i] == &SET_SRC(old_set))
        is_set_src = true;
      in_plus = false;
      if (plus_src && (recog_data.operand_loc[i] == &XEXP(plus_src, 0) ||
                       recog_data.operand_loc[i] == &XEXP(plus_src, 1)))
        in_plus = true;

      substed_operand[i] =
          eliminate_regs_1(recog_data.operand[i], 0, replace ? insn : NULL_RTX,
                           is_set_src || in_plus);
      ...
      *recog_data.operand_loc[i] = 0;
      ...
      if (recog_data.operand_type[i] != OP_IN && REG_P(orig_operand[i]) &&
          MEM_P(substed_operand[i]) && replace)
  ...
  if (val) {
    ...
    if (!insn_is_asm && old_set != 0 &&
        ((REG_P(SET_SRC(old_set)) &&
         ...
         || (old_set != 0 && ((MEM_P(SET_SRC(old_set)) &&
...
static void mark_not_eliminable(rtx dest, const_rtx x,
  ...
  for (i = 0; i < NUM_ELIMINABLE_REGS; i++)
    if (reg_eliminate[i].can_eliminate && dest == reg_eliminate[i].to_rtx &&
        (GET_CODE(x) != SET || GET_CODE(SET_SRC(x)) != PLUS ||
         XEXP(SET_SRC(x), 0) != dest ||
         GET_CODE(XEXP(SET_SRC(x), 1)) != CONST_INT)) {
      reg_eliminate[i].can_eliminate_previous = reg_eliminate[i].can_eliminate =
          0;
      num_eliminable--;
    }
}
...
static void spill_hard_reg(unsigned int regno, int cant_eliminate) {
  ...
  for (i = FIRST_PSEUDO_REGISTER; i < max_regno; i++)
    if (reg_renumber[i] >= 0 && (unsigned int)reg_renumber[i] <= regno &&
...
static int finish_spills(int global) {
  ...
  if (global && ira_conflicts_p) {
    ...
    for (chain = insns_need_reload; chain; chain = chain->next_need_reload) {
      EXECUTE_IF_SET_IN_REG_SET(&chain->live_throughout, FIRST_PSEUDO_REGISTER,
                                i, rsi) {
    ...
    if (ira_reassign_pseudos(temp_pseudo_reg_arr, n, bad_spill_regs_global,
...
static void scan_paradoxical_subregs(rtx x) {
  ...
  switch (code) {
    ...
    case SUBREG:
      if (REG_P(SUBREG_REG(x)) && (GET_MODE_SIZE(GET_MODE(x)) >
...
static void fixup_eh_region_note(rtx insn, rtx prev, rtx next) {
      ...
      if (n_reloads > 0) {
        ...
        if (asm_noperands(PATTERN(insn)) >= 0)
          for (p = NEXT_INSN(prev); p != next; p = NEXT_INSN(p))
            if (p != insn && INSN_P(p) && GET_CODE(PATTERN(p)) != USE &&
                (recog_memoized(p) < 0 ||
      ...
      for (i = n_reloads - 1; i >= 0; i--) {
        ...
        if (in_reg) {
          ...
          if ((code == POST_INC || code == POST_DEC) &&
              TEST_HARD_REG_BIT(reg_reloaded_valid, REGNO(rld[i].reg_rtx))
...

          {
            ...
            for (p = PREV_INSN(old_next); p != prev; p = PREV_INSN(p)) {
              ...
              if (n == 1) {
                n = validate_replace_rtx(
                    reload_reg, gen_rtx_fmt_e(code, mode, reload_reg), p);
                ...
                if (!n) {
                  validate_replace_rtx(gen_rtx_fmt_e(code, mode, reload_reg),
                                       reload_reg, p);
                  break;
                }
              }
            ...
            if (n == 1) {
              ...
              SET_HARD_REG_BIT(reg_is_output_reload, REGNO(reload_reg));
              SET_REGNO_REG_SET(&reg_has_output_reload, REGNO(XEXP(in_reg, 0)));
            } else
              forget_old_reloads_1(XEXP(in_reg, 0), NULL_RTX, NULL);
                     ...
                     ((unsigned)reg_reloaded_contents[REGNO(rld[i].reg_rtx)] ==
                      REGNO(XEXP(in_reg, 0)))) {
            SET_HARD_REG_BIT(reg_is_output_reload, REGNO(rld[i].reg_rtx));
            SET_REGNO_REG_SET(&reg_has_output_reload, REGNO(XEXP(in_reg, 0)));
          } else if (code == PRE_INC || code == PRE_DEC || code == POST_INC ||
                     code == POST_DEC) {
            ...
            if (reg_last_reload_reg[in_regno] != NULL_RTX) {
              ...
              if (TEST_HARD_REG_BIT(reg_reloaded_valid, in_hard_regno)) {
                for (x = old_prev ? NEXT_INSN(old_prev) : insn; x != old_next;
                     x = NEXT_INSN(x))
...
static void forget_old_reloads_1(rtx x, const_rtx ignored ATTRIBUTE_UNUSED,
                                 void *data) {
  unsigned int regno;
  unsigned int nr;
...
static void forget_marked_reloads(regset regs) {
  ...
  EXECUTE_IF_SET_IN_REG_SET(regs, 0, reg, rsi) {
    if (reg < FIRST_PSEUDO_REGISTER
        /* But don't do this if the reg actually serves as an output
           reload reg in the current instruction.  */
        && (n_reloads == 0 || !TEST_HARD_REG_BIT(reg_is_output_reload, reg))) {
      CLEAR_HARD_REG_BIT(reg_reloaded_valid, reg);
      spill_reg_store[reg] = 0;
    }
    if (n_reloads == 0 || !REGNO_REG_SET_P(&reg_has_output_reload, reg))
...
static void mark_reload_reg_in_use(unsigned int regno, int opnum,
...
static void clear_reload_reg_in_use(unsigned int regno, int opnum,
                                    enum reload_type type,
                                    enum machine_mode mode) {
  unsigned int nregs = hard_regno_nregs[regno][mode];
  ...
  if (check_opnum || check_any) {
    for (i = n_reloads - 1; i >= 0; i--) {
      if (rld[i].when_needed == type && (check_any || rld[i].opnum == opnum) &&
          rld[i].reg_rtx) {
        unsigned int conflict_start = true_regnum(rld[i].reg_rtx);
        unsigned int conflict_end = end_hard_regno(rld[i].mode, conflict_start);
...
static int reload_reg_free_p(unsigned int regno, int opnum,
                             enum reload_type type) {
  int i;
...
static int reload_reg_reaches_end_p(unsigned int regno, int opnum,
                                    enum reload_type type) {
  int i;
...
static bool reload_regs_reach_end_p(unsigned int regno, int nregs, int opnum,
                                    enum reload_type type) {
  int i;
...
static bool reloads_unique_chain_p(int r1, int r2) {
  ...
  if (rld[r1].opnum != rld[r2].opnum || rtx_equal_p(rld[r1].in, rld[r2].in) ||
...
/* The recursive function change all occurrences of WHAT in *WHERE
   onto REPL.  */
static void substitute(rtx *where, const_rtx what, rtx repl) {
  const char *fmt;
  int i;
  enum rtx_code code;
...
*/
static bool gen_reload_chain_without_interm_reg_p(int r1, int r2) {
  bool result;
  int regno, n, code;
  rtx out, in, tem, insn;
...
static int reloads_conflict(int r1, int r2) {
...
static int reload_reg_free_for_value_p(int start_regno, int regno, int opnum,
                                       enum reload_type type, rtx value,
                                       rtx out, int reloadnum,
  ...
  for (i = 0; i < n_reloads; i++) {
    ...
    if (reg && REG_P(reg) &&
      ...
      if (!other_input || !rtx_equal_p(other_input, value) || rld[i].out ||
          out) {
        int time2;
        switch (rld[i].when_needed) {
          ...
          case RELOAD_FOR_INPUT_ADDRESS:
            if (type == RELOAD_FOR_INPUT && opnum == rld[i].opnum &&
                ignore_address_reloads && !rld[reloadnum].out)
              continue;
        ...
        if ((time1 >= time2 &&
             (!rld[i].in || rld[i].out || !rtx_equal_p(other_input, value))) ||
...
static int free_for_value_p(int regno, enum machine_mode mode, int opnum,
                            enum reload_type type, rtx value, rtx out,
                            int reloadnum, int ignore_address_reloads) {
  int nregs = hard_regno_nregs[regno][mode];
  while (nregs-- > 0)
    if (!reload_reg_free_for_value_p(regno, regno + nregs, opnum, type, value,
                                     out, reloadnum, ignore_address_reloads))
      return 0;
  return 1;
}
...
int function_invariant_p(const_rtx x) {
  if (CONSTANT_P(x))
    return 1;
    ...
    if (!(rld[r].in != 0 && test_mode != VOIDmode &&
      ...
      if (!(rld[r].out != 0 &&
        ...
        /* Mark as in use for this insn the reload regs we use
           for this.  */
        mark_reload_reg_in_use(spill_regs[i], rld[r].opnum, rld[r].when_needed,
                               rld[r].mode);
...
static int allocate_reload_reg(struct insn_chain *chain ATTRIBUTE_UNUSED, int r,
                               int last_reload) {
  int i, pass, count;
  ...
  for (pass = 0; pass < 2; pass++) {
    ...
    for (count = 0; count < n_spills; count++) {
      ...
      if ((reload_reg_free_p(regnum, rld[r].opnum, rld[r].when_needed) ||
           (rld[r].in
            /* We check reload_reg_used to make sure we
               don't clobber the return register.  */
            && !TEST_HARD_REG_BIT(reload_reg_used, regnum) &&
            free_for_value_p(regnum, rld[r].mode, rld[r].opnum,
                             rld[r].when_needed, rld[r].in, rld[r].out, r,
                             1))) &&
          ...
          &&
          (pass || (TEST_HARD_REG_BIT(reload_reg_used_at_all, regnum) &&
        ...
        while (nr > 1) {
          ...
          if (!(TEST_HARD_REG_BIT(reg_class_contents[rclass], regno) &&
                ...
                reload_reg_free_p(regno, rld[r].opnum, rld[r].when_needed)))
            break;
          nr--;
        }
        if (nr == 1)
...
static void choose_reload_regs(struct insn_chain *chain) {
  ...
  for (j = 0; j < n_reloads; j++) {
    ...
    if (rld[j].reg_rtx != NULL_RTX) {
      gcc_assert(REG_P(rld[j].reg_rtx) && HARD_REGISTER_P(rld[j].reg_rtx));
    ...
    if (rld[j].nregs > 1) {
      ...
      group_class = reg_class_superunion[(int)rld[j].rclass][(int)group_class];
    }
    ...
    for (j = 0; j < n_reloads; j++) {
      ...
      if (rld[r].in != 0 && rld[r].reg_rtx != 0 &&
          (rtx_equal_p(rld[r].in, rld[r].reg_rtx) ||
           (rtx_equal_p(rld[r].out, rld[r].reg_rtx) && !MEM_P(rld[r].in) &&
      ...
      if (inheritance) {
        ...
        if (regno >= 0 &&
          ...
          if (byte == 0)
            ...
            need_mode = smallest_mode_for_size(
                GET_MODE_BITSIZE(mode) + byte * BITS_PER_UNIT,
                GET_MODE_CLASS(mode) == MODE_PARTIAL_INT
                    ? MODE_INT
                    : GET_MODE_CLASS(mode));

          if ((GET_MODE_SIZE(GET_MODE(last_reg)) >= GET_MODE_SIZE(need_mode)) &&
              ...
              (TEST_HARD_REG_BIT(reg_class_contents[(int)rclass], i)
               ...
               ((REGISTER_MOVE_COST(mode, last_class, rclass) <
                ...
                (secondary_reload_class(1, rclass, mode, last_reg) == NO_REGS)
#ifdef SECONDARY_MEMORY_NEEDED
                && !SECONDARY_MEMORY_NEEDED(last_class, rclass, mode)
#endif
                    ))
              ...
              free_for_value_p(i, rld[r].mode, rld[r].opnum, rld[r].when_needed,
            ...
            if (k == nr) {
              ...
              last_reg = (GET_MODE(last_reg) == mode ? last_reg
                                                     : gen_rtx_REG(mode, i));

              bad_for_class = 0;
              for (k = 0; k < nr; k++)
                bad_for_class |= !TEST_HARD_REG_BIT(
                    reg_class_contents[(int)rld[r].rclass], i + k);
              ...
              if (i1 != n_earlyclobbers ||
                  !(free_for_value_p(i, rld[r].mode, rld[r].opnum,
                                     rld[r].when_needed, rld[r].in, rld[r].out,
                                     r, 1))
                  /* Don't use it if we'd clobber a pseudo reg.  */
                  || (TEST_HARD_REG_BIT(reg_used_in_insn, i) && rld[r].out &&
                      !TEST_HARD_REG_BIT(reg_reloaded_dead, i))
                  /* Don't clobber the frame pointer.  */
                  || (i == HARD_FRAME_POINTER_REGNUM && frame_pointer_needed &&
                      rld[r].out)
                  /* Don't really use the inherited spill reg
                     if we need it wider than we've got it.  */
                  || (GET_MODE_SIZE(rld[r].mode) > GET_MODE_SIZE(mode)) ||
                  bad_for_class
                ...
                /* Mark the register as in use for this part of
                   the insn.  */
                mark_reload_reg_in_use(i, rld[r].opnum, rld[r].when_needed,
                                       rld[r].mode);
      ...
      if (inheritance && rld[r].in != 0 && !reload_inherited[r] &&
          rld[r].out == 0 &&
          (CONSTANT_P(rld[r].in) || GET_CODE(rld[r].in) == PLUS ||
      ...
      /* If this is an output reload from a simple move insn, look
        ...
        if (set && rtx_equal_p(rld[r].out, SET_DEST(set)) &&
            CONSTANT_P(SET_SRC(set)))
          search_equiv = SET_SRC(set);
      }

      if (search_equiv) {
        rtx equiv = find_equiv_reg(search_equiv, insn, rld[r].rclass, -1, NULL,
                                   0, rld[r].mode);
        int regno = 0;
        ...
        if (equiv != 0) {
          ...
          for (i = regno; i < max_regno; i++) {
            regs_used |= TEST_HARD_REG_BIT(reload_reg_used_at_all, i);
            bad_for_class |=
                !TEST_HARD_REG_BIT(reg_class_contents[(int)rld[r].rclass], i);
          }

          if ((regs_used && !free_for_value_p(regno, rld[r].mode, rld[r].opnum,
        ...
        if (equiv != 0 &&
          ...
          for (k = 0; k < nr; k++) {
            ...
            if (i >= 0) {
              mark_reload_reg_in_use(regno, rld[r].opnum, rld[r].when_needed,
                                     rld[r].mode);
  ...
  for (pass = flag_expensive_optimizations; pass >= 0; pass--) {
    for (j = 0; j < n_reloads; j++) {
      ...
      if (reload_inherited[r] && rld[r].reg_rtx)
        ...
        continue;
      if (!free_for_value_p(true_regnum(check_reg), rld[r].mode, rld[r].opnum,
                            rld[r].when_needed, rld[r].in,
                            (reload_inherited[r] ? rld[r].out : const0_rtx), r,
                            1)) {
        if (pass)
          continue;
      ...
      else if (rld[r].in && rld[r].out != rld[r].in &&
  ...
  for (j = 0; j < n_reloads; j++)
    if (rld[j].reg_rtx != 0 &&
      ...
      if (spill_reg_order[regno] >= 0)
        clear_reload_reg_in_use(regno, rld[j].opnum, rld[j].when_needed,
                                rld[j].mode);
      rld[j].reg_rtx = 0;
      reload_spill_index[j] = -1;
    }
  ...
  for (j = 0; j < n_reloads; j++) {
    ...
    if (rld[r].out_reg != 0 && REG_P(rld[r].out_reg) && rld[r].reg_rtx != 0) {
      ...
      while (--nr >= 0)
        SET_REGNO_REG_SET(&reg_has_output_reload, nregno + nr);
  ...
  for (i = 0; i < n_reloads; i++) {
    ...
    for (j = 0; j < n_reloads; j++) {
      if (i == j || rld[j].reg_rtx == 0 ||
          !reg_overlap_mentioned_p(rld[j].reg_rtx, rld[i].reg_rtx))
        continue;
      ...
      if (!rtx_equal_p(rld[i].reg_rtx, rld[j].reg_rtx) || rld[j].out != 0 ||
          rld[j].in == 0 || !rtx_equal_p(rld[i].in, rld[j].in)) {
    ...
    if (j == n_reloads &&
      ...
      if (rld[i].when_needed == RELOAD_OTHER)
        for (j = 0; j < n_reloads; j++)
          if (rld[j].in != 0 && rld[j].when_needed != RELOAD_OTHER &&
              rld[j].when_needed != RELOAD_FOR_OTHER_ADDRESS &&
              ...
              reg_overlap_mentioned_for_reload_p(rld[j].in, rld[i].in)) {
            int k;
...
static bool reload_adjust_reg_for_temp(rtx *reload_reg, rtx alt_reload_reg,
...
/* Check if *RELOAD_REG is suitable as a scratch register for the reload
   ...
   adjusted register, and return true.  Otherwise, return false.  */
static bool reload_adjust_reg_for_icode(rtx *reload_reg, rtx alt_reload_reg,
...

{
  ...
  return reload_adjust_reg_for_temp(reload_reg, alt_reload_reg, new_class,
                                    new_mode);
}
...
static void emit_input_reload_insns(struct insn_chain *chain, struct reload *rl,
                                    rtx old, int j) {
  rtx insn = chain->insn;
  rtx reloadreg;
  rtx oldequiv_reg = 0;
  ...
  if (reload_override_in[j] && REG_P(rl->in_reg)) {
    oldequiv = old;
    old = rl->in_reg;
  }
  if (oldequiv == 0)
  ...
  if (optimize && REG_P(oldequiv) && REGNO(oldequiv) < FIRST_PSEUDO_REGISTER &&
      spill_reg_store[REGNO(oldequiv)] && REG_P(old) &&
      (dead_or_set_p(insn, spill_reg_stored_to[REGNO(oldequiv)]) ||
       rtx_equal_p(spill_reg_stored_to[REGNO(oldequiv)], rl->out_reg)))
  ...
  /* If we are reloading a pseudo-register that was set by the previous
  ...
  else if (optimize && REG_P(old) && REGNO(old) >= FIRST_PSEUDO_REGISTER &&
    ...
    while (temp && NOTE_P(temp))
      temp = PREV_INSN(temp);
    if (temp && NONJUMP_INSN_P(temp) && GET_CODE(PATTERN(temp)) == SET &&
  ...
  if (!special && rl->secondary_in_reload >= 0) {
    ...
    if (REG_P(tmp) && REGNO(tmp) >= FIRST_PSEUDO_REGISTER &&
      ...
      if (!reg_equiv_mem[REGNO(tmp)] || num_not_at_initial_offset ||
    ...
    if ((old != oldequiv && !rtx_equal_p(old, oldequiv)) ||
      ...
      sri.prev_sri = NULL;
      new_class =
          targetm.secondary_reload(1, real_oldequiv, rl->rclass, mode, &sri);

      if (new_class == NO_REGS && sri.icode == CODE_FOR_nothing)
        second_reload_reg = 0;
      else if (new_class == NO_REGS) {
        if (reload_adjust_reg_for_icode(&second_reload_reg, third_reload_reg,
                                        sri.icode))
        ...
        sri2.prev_sri = &sri;
        new_t_class =
            targetm.secondary_reload(1, real_oldequiv, new_class, mode, &sri);
        if (new_t_class == NO_REGS && sri2.icode == CODE_FOR_nothing) {
          if (reload_adjust_reg_for_temp(&second_reload_reg, third_reload_reg,
                                         new_class, mode))
          ...
          if (reload_adjust_reg_for_temp(&intermediate, NULL, new_class,
                                         mode) &&
          ...
          if (reload_adjust_reg_for_temp(&intermediate, NULL, new_class,
                                         mode) &&
              reload_adjust_reg_for_temp(&third_reload_reg, NULL, new_t_class,
                                         mode)) {
            second_reload_reg = intermediate;
            tertiary_icode = sri2.icode;
          } else
    ...
    if (second_reload_reg) {
      if (icode != CODE_FOR_nothing) {
        ...
        if (tertiary_icode != CODE_FOR_nothing) {
          emit_insn((GEN_FCN(tertiary_icode)(second_reload_reg, real_oldequiv,
                                             third_reload_reg)));
        } else if (third_reload_reg) {
          gen_reload(third_reload_reg, real_oldequiv, rl->opnum,
                     rl->when_needed);
          gen_reload(second_reload_reg, third_reload_reg, rl->opnum,
                     rl->when_needed);
        } else
          gen_reload(second_reload_reg, real_oldequiv, rl->opnum,
                     rl->when_needed);
  ...
  if (!special && !rtx_equal_p(reloadreg, oldequiv)) {
    ...
    if ((REG_P(oldequiv) && REGNO(oldequiv) >= FIRST_PSEUDO_REGISTER &&
         (reg_equiv_memory_loc[REGNO(oldequiv)] != 0 ||
          reg_equiv_constant[REGNO(oldequiv)] != 0)) ||
        (GET_CODE(oldequiv) == SUBREG && REG_P(SUBREG_REG(oldequiv)) &&
        ...
        (CONSTANT_P(oldequiv) &&
         (PREFERRED_RELOAD_CLASS(oldequiv, REGNO_REG_CLASS(REGNO(reloadreg))) ==
          NO_REGS)))
      real_oldequiv = rl->in;
    gen_reload(reloadreg, real_oldequiv, rl->opnum, rl->when_needed);
  }
...
static void emit_output_reload_insns(struct insn_chain *chain,
  ...
  if (rl->secondary_out_reload >= 0) {
    ...
    if (secondary_reload_class(0, rl->rclass, mode, real_old) != NO_REGS) {
      ...
      if (rl->secondary_out_icode != CODE_FOR_nothing) {
        ...
        emit_insn((GEN_FCN(rl->secondary_out_icode)(real_old, second_reloadreg,
                                                    reloadreg)));
        special = 1;
      } else {
        ...
        if (tertiary_icode != CODE_FOR_nothing) {
          ...
          if (GET_CODE(real_old) == SUBREG &&
              ...
              0 != (tem = gen_lowpart_common(GET_MODE(SUBREG_REG(real_old)),
                                             reloadreg)))
            real_old = SUBREG_REG(real_old), reloadreg = tem;

          gen_reload(reloadreg, second_reloadreg, rl->opnum, rl->when_needed);
          emit_insn(
          ...
          if (tertiary_reload >= 0) {
            ...
            gen_reload(third_reloadreg, reloadreg, rl->opnum, rl->when_needed);
  ...
  if (!special) {
    ...
    if (!flag_expensive_optimizations || !REG_P(old) ||
        !(set = single_set(insn)) || rtx_equal_p(old, SET_DEST(set)) ||
        !reg_mentioned_p(old, SET_SRC(set)) ||
        !((REGNO(old) < FIRST_PSEUDO_REGISTER) &&
          regno_clobbered_p(REGNO(old), insn, rl->mode, 0)))
      gen_reload(old, reloadreg, rl->opnum, rl->when_needed);
  }
  ...
  for (p = get_insns(); p; p = NEXT_INSN(p))
    if (INSN_P(p)) {
      ...
      if (reg_mentioned_p(rl_reg_rtx, pat)) {
        rtx set = single_set(insn);
        if (reload_spill_index[j] < 0 && set && SET_SRC(set) == rl_reg_rtx) {
        ...
        if (HARD_REGISTER_P(rl_reg_rtx)) {
          ...
          if (s >= 0 && set == NULL_RTX)
            ...
            ;
          else if (s >= 0 && SET_SRC(set) == rl_reg_rtx &&
...
static void do_input_reload(struct insn_chain *chain, struct reload *rl,
                            int j) {
  rtx insn = chain->insn;
  rtx old = (rl->in && MEM_P(rl->in) ? rl->in_reg : rl->in);
  ...
  if (old != 0
      ...
      !rtx_equal_p(reg_rtx, old) && reg_rtx != 0)
    emit_input_reload_insns(chain, rld + j, old, j);

  /* When inheriting a wider reload, we have a MEM in rl->in,
     e.g. inheriting a SImode output reload for
     (mem:HI (plus:SI (reg:SI 14 fp) (const_int 10)))  */
  if (optimize && reload_inherited[j] && rl->in && MEM_P(rl->in) &&
      MEM_P(rl->in_reg) && reload_spill_index[j] >= 0 &&
  ...
  if (optimize && (reload_inherited[j] || reload_override_in[j]) && reg_rtx &&
      REG_P(reg_rtx) &&
...
static void do_output_reload(struct insn_chain *chain, struct reload *rl,
                             int j) {
  rtx note, old;
  rtx insn = chain->insn;
  ...
  if (pseudo && optimize && REG_P(pseudo) && !rtx_equal_p(rl->in_reg, pseudo) &&
  ...
  old = rl->out_reg;
  if (old == 0 || reg_rtx == 0 || rtx_equal_p(old, reg_rtx))
    return;
  ...
  else if (GET_CODE(old) == SUBREG && REG_P(SUBREG_REG(old)) &&
...
static bool inherit_piecemeal_p(int dest ATTRIBUTE_UNUSED,
...
static void emit_reload_insns(struct insn_chain *chain) {
  ...
  for (j = 0; j < reload_n_operands; j++)
    input_reload_insns[j] = input_address_reload_insns[j] =
        inpaddr_address_reload_insns[j] = output_reload_insns[j] =
            output_address_reload_insns[j] = outaddr_address_reload_insns[j] =
  ...
  for (j = 0; j < n_reloads; j++) {
    ...
    /* If this is a non-inherited input reload from a pseudo, we must
       ...
       being reloaded.  */
    if (rld[r].in_reg != 0 && !(reload_inherited[r] || reload_override_in[r])) {
    ...
    if (i >= 0 && rld[r].reg_rtx != 0) {
      ...
      for (k = 0; k < nr; k++)
        if (reload_reg_reaches_end_p(i + k, rld[r].opnum, rld[r].when_needed))
          CLEAR_HARD_REG_BIT(reg_reloaded_valid, i + k);

      /* Maybe the spill reg contains a copy of reload_out.  */
      if (rld[r].out != 0 && (REG_P(rld[r].out)
#ifdef AUTO_INC_DEC
                              || !rld[r].out_reg
#endif
                              || REG_P(rld[r].out_reg))) {
        ...
        if (reload_regs_reach_end_p(regno, nregs, rld[r].opnum,
                                    rld[r].when_needed)) {
          rtx out = (REG_P(rld[r].out) ? rld[r].out
                     : rld[r].out_reg  ? rld[r].out_reg
                                      /* AUTO_INC */
                                      : XEXP(rld[r].in_reg, 0));
          int out_regno = REGNO(out);
          int out_nregs = (!HARD_REGISTER_NUM_P(out_regno)
                               ? 1
          ...
          piecemeal = (HARD_REGISTER_NUM_P(out_regno) && nregs == out_nregs &&
          ...
          for (k = 0; k < nregs; k++) {
            ...
            reg_reloaded_contents[regno + k] =
                (!HARD_REGISTER_NUM_P(out_regno) || !piecemeal ? out_regno
                                                               : out_regno + k);
            reg_reloaded_insn[regno + k] = insn;
            SET_HARD_REG_BIT(reg_reloaded_valid, regno + k);
            if (HARD_REGNO_CALL_PART_CLOBBERED(regno + k, mode))
              SET_HARD_REG_BIT(reg_reloaded_call_part_clobbered, regno + k);
            else
              CLEAR_HARD_REG_BIT(reg_reloaded_call_part_clobbered, regno + k);
          }
        }
      }
      /* Maybe the spill reg contains a copy of reload_in.  Only do
      ...
      else if (rld[r].out_reg == 0 && rld[r].in != 0 &&
               ((REG_P(rld[r].in) && !HARD_REGISTER_P(rld[r].in) &&
                 !REGNO_REG_SET_P(&reg_has_output_reload, REGNO(rld[r].in))) ||
                (REG_P(rld[r].in_reg) &&
        ...
        if (reload_regs_reach_end_p(regno, nregs, rld[r].opnum,
          ...
          if (REG_P(rld[r].in) && REGNO(rld[r].in) >= FIRST_PSEUDO_REGISTER)
            in = rld[r].in;
          else if (REG_P(rld[r].in_reg))
            in = rld[r].in_reg;
          else
          ...
          in_nregs = (!HARD_REGISTER_NUM_P(in_regno)
                          ? 1
                          : hard_regno_nregs[in_regno][mode]);

          reg_last_reload_reg[in_regno] = reg;

          piecemeal = (HARD_REGISTER_NUM_P(in_regno) && nregs == in_nregs &&
          ...
          for (k = 0; k < nregs; k++) {
            CLEAR_HARD_REG_BIT(reg_reloaded_dead, regno + k);
            reg_reloaded_contents[regno + k] =
                (!HARD_REGISTER_NUM_P(in_regno) || !piecemeal ? in_regno
                                                              : in_regno + k);
            reg_reloaded_insn[regno + k] = insn;
            SET_HARD_REG_BIT(reg_reloaded_valid, regno + k);
            if (HARD_REGNO_CALL_PART_CLOBBERED(regno + k, mode))
    ...
    /* If a register gets output-reloaded from a non-spill register,
       ...
       output is discarded.  */
    if (i < 0 &&
        ((rld[r].out != 0 && (REG_P(rld[r].out) ||
                              (MEM_P(rld[r].out) && REG_P(rld[r].out_reg)))) ||
         (rld[r].out == 0 && rld[r].out_reg && REG_P(rld[r].out_reg)))) {
      rtx out =
          ((rld[r].out && REG_P(rld[r].out)) ? rld[r].out : rld[r].out_reg);
      int out_regno = REGNO(out);
      ...
      if (!HARD_REGISTER_NUM_P(out_regno)) {
        ...
        if (src_reg && REG_P(src_reg) &&
          ...
          for (k = 0; k < src_nregs; k++) {
            ...
            if (HARD_REGNO_CALL_PART_CLOBBERED(src_regno + k, mode))
              SET_HARD_REG_BIT(reg_reloaded_call_part_clobbered, src_regno + k);
            else
          ...
          /* We have to set reg_has_output_reload here, or else
             ...
             right away.  */
          SET_REGNO_REG_SET(&reg_has_output_reload, out_regno);
        }
      } else {
...
static rtx emit_insn_if_valid_for_reload(rtx insn) {
  rtx last = get_last_insn();
  int code;
...
static rtx gen_reload(rtx out, rtx in, int opnum, enum reload_type type) {
  rtx last = get_last_insn();
  rtx tem;
  ...
  if (GET_CODE(in) == PLUS &&
    ...
    if (REG_P(XEXP(in, 1)) && REGNO(out) == REGNO(XEXP(in, 1)))
    ...
    if (CONSTANT_P(op1) || MEM_P(op1) || GET_CODE(op1) == SUBREG ||
        (REG_P(op1) && REGNO(op1) >= FIRST_PSEUDO_REGISTER) ||
  ...
  /* If we need a memory location to do the move, do it that way.  */
  else if ((REG_P(in) || (GET_CODE(in) == SUBREG && REG_P(SUBREG_REG(in)))) &&
    ...
    insn = gen_rtx_SET(VOIDmode, out,
                       gen_rtx_fmt_e(GET_CODE(in), GET_MODE(in), out_moded));
...
static void delete_output_reload(rtx insn, int j, int last_reload_reg,
  ...
  if (CALL_P(insn) && CALL_INSN_FUNCTION_USAGE(insn))
    n_occurrences += count_occurrences(CALL_INSN_FUNCTION_USAGE(insn), reg, 0);
  if (substed)
    n_occurrences += count_occurrences(PATTERN(insn),
                                       eliminate_regs(substed, 0, NULL_RTX), 0);
  ...
  if (rld[j].out != rld[j].in && REG_N_DEATHS(REGNO(reg)) == 1 &&
...
static void delete_address_reloads(rtx dead_insn, rtx current_insn) {
  ...
  if (!set || !set2 || GET_CODE(SET_SRC(set)) != PLUS ||
...
static void delete_address_reloads_1(rtx dead_insn, rtx x, rtx current_insn) {
  ...
  if (!set)
    return;
  dst = SET_DEST(set);
  if (!REG_P(dst) || !rtx_equal_p(dst, x))
    return;
...
static rtx inc_for_reload(rtx reloadreg, rtx in, rtx value, int inc_amount) {
  ...
  if (in == value) {
    ...
    last = get_last_insn();
    add_insn = emit_insn(gen_rtx_SET(
        VOIDmode, incloc, gen_rtx_PLUS(GET_MODE(incloc), incloc, inc)));
...
#ifdef AUTO_INC_DEC
static void add_auto_inc_notes(rtx insn, rtx x) {
  enum rtx_code code = GET_CODE(x);
  const char *fmt;
  int i, j;
  ...
  FOR_EACH_BB(bb) {
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      if (e->flags & EDGE_ABNORMAL_CALL)
        break;
      if ((e->flags & (EDGE_ABNORMAL | EDGE_EH)) == (EDGE_ABNORMAL | EDGE_EH))
        break;
    }
