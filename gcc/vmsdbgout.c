...
#define ASM_OUTPUT_DEBUG_DATA2(FILE, VALUE) \
  fprintf((FILE), "\t%s\t0x%x", UNALIGNED_SHORT_ASM_OP, (unsigned short)VALUE)
#endif
...
#define ASM_OUTPUT_DEBUG_ADDR_DATA(FILE, VALUE) \
  fprintf((FILE), "\t%s\t0x%lx", UNALIGNED_PTR_ASM_OP, (unsigned long)VALUE)
#endif
...
static void addr_const_to_string(char *str, rtx x) {
  char buf1[256];
  char buf2[256];
    ...
    case CONST_DOUBLE:
      if (GET_MODE(x) == VOIDmode) {
        ...
        if (CONST_DOUBLE_HIGH(x))
          sprintf(buf1, HOST_WIDE_INT_PRINT_DOUBLE_HEX, CONST_DOUBLE_HIGH(x),
                  CONST_DOUBLE_LOW(x));
...
static int write_debug_header(DST_HEADER *header, const char *comment,
                              int dosizeonly) {
  if (!dosizeonly) {
    ...
    ASM_OUTPUT_DEBUG_DATA2(asm_out_file, header->dst__header_type.dst_w_type);
...
static int write_debug_addr(const char *symbol, const char *comment,
                            int dosizeonly) {
  if (!dosizeonly) {
...
static int write_debug_data1(unsigned int data1, const char *comment,
                             int dosizeonly) {
  if (!dosizeonly) {
...
static int write_debug_data2(unsigned int data2, const char *comment,
                             int dosizeonly) {
  if (!dosizeonly) {
...
static int write_debug_data4(unsigned long data4, const char *comment,
                             int dosizeonly) {
  if (!dosizeonly) {
...
static int write_debug_data8(unsigned long long data8, const char *comment,
                             int dosizeonly) {
  if (!dosizeonly) {
...
static int write_debug_delta4(const char *label1, const char *label2,
...
static int write_debug_string(const char *string, const char *comment,
                              int dosizeonly) {
  if (!dosizeonly) {
...
static int write_modbeg(int dosizeonly) {
  ...
  totsize +=
      write_debug_header(&modbeg.dst_a_modbeg_header, "modbeg", dosizeonly);
  totsize += write_debug_data1(*((char *)&modbeg.dst_b_modbeg_flags), "flags",
                               dosizeonly);
  totsize +=
      write_debug_data1(modbeg.dst_b_modbeg_unused, "unused", dosizeonly);
  totsize +=
      write_debug_data4(modbeg.dst_l_modbeg_language, "language", dosizeonly);
  totsize += write_debug_data2(modbeg.dst_w_version_major, "DST major version",
                               dosizeonly);
  totsize += write_debug_data2(modbeg.dst_w_version_minor, "DST minor version",
                               dosizeonly);
...
static int write_modend(int dosizeonly) {
  ...
  totsize +=
      write_debug_header(&modend.dst_a_modend_header, "modend", dosizeonly);
...
static int write_rtnbeg(int rtnnum, int dosizeonly) {
  const char *rtnname;
  int rtnnamelen;
  char *rtnentryname;
  int totsize = 0;
  ...
  if (!strcmp(rtnname, "main")) {
    ...
    totsize +=
        write_debug_addr(rtnname, "main procedure descriptor", dosizeonly);
    totsize += write_debug_data1(strlen(go), "length of main_name", dosizeonly);
  ...
  totsize +=
      write_debug_header(&rtnbeg.dst_a_rtnbeg_header, "rtnbeg", dosizeonly);
  totsize += write_debug_data1(*((char *)&rtnbeg.dst_b_rtnbeg_flags), "flags",
                               dosizeonly);
  ...
  /* Routine Begin PD Address */
  totsize +=
      write_debug_addr(rtnname, "routine procedure descriptor", dosizeonly);
  ...
  if (debug_info_level > DINFO_LEVEL_TERSE) {
    ...
    totsize +=
        write_debug_header(&prolog.dst_a_prolog_header, "prolog", dosizeonly);

    ASM_GENERATE_INTERNAL_LABEL(label, FUNC_PROLOG_LABEL, fde->funcdef_number);
    totsize += write_debug_addr(label, "prolog breakpoint addr", dosizeonly);
  }
...
static int write_rtnend(int rtnnum, int dosizeonly) {
  ...
  totsize +=
      write_debug_header(&rtnend.dst_a_rtnend_header, "rtnend", dosizeonly);
  totsize +=
      write_debug_data1(rtnend.dst_b_rtnend_unused, "unused", dosizeonly);
...
static int write_pclines(int dosizeonly) {
  ...
  totsize += write_debug_header(&line_num.dst_a_line_num_header, "line_num",
                                dosizeonly);
  ...
  for (i = 1; i < line_info_table_in_use; i++) {
    ...
    if (ln - lastln > 1) {
      ...
      totsize += write_debug_data1(pcline.dst_b_pcline_command,
                                   "line_num (INCR LINUM LONG)", dosizeonly);
    ...
    totsize +=
        write_debug_delta4(label, lastlabel, "increment line_num", dosizeonly);
...
static int write_srccorr(int fileid, dst_file_info_entry file_info_entry,
                         int dosizeonly) {
  ...
  totsize += write_debug_string(remap_debug_filename(file_info_entry.file_name),
  ...
  if (src_command_dl.dst_a_src_cmd_fields.dst_w_src_unsword) {
    ...
    totsize +=
        write_debug_data2(src_command_sf.dst_a_src_cmd_fields.dst_w_src_unsword,
                          "source_corr (fileid)", dosizeonly);
    ...
    totsize +=
        write_debug_data2(src_command_sr.dst_a_src_cmd_fields.dst_w_src_unsword,
    ...
    totsize +=
        write_debug_data4(src_command_sl.dst_a_src_cmd_fields.dst_l_src_unslong,
    ...
    totsize +=
        write_debug_data2(src_command_dl.dst_a_src_cmd_fields.dst_w_src_unsword,
                          buff, dosizeonly);
    ...
    while (linesleft > 0) {
      ...
      sprintf(buff, "source_corr (%d)",
              src_command_dl.dst_a_src_cmd_fields.dst_w_src_unsword);
      totsize += write_debug_data2(
          src_command_dl.dst_a_src_cmd_fields.dst_w_src_unsword, buff,
          dosizeonly);
    }
  }
...
static int write_srccorrs(int dosizeonly) {
  unsigned int i;
  int totsize = 0;
...
static void vmsdbgout_begin_prologue(unsigned int line, const char *file) {
  char label[MAX_ARTIFICIAL_LABEL_BYTES];
...
static void vmsdbgout_end_prologue(unsigned int line, const char *file) {
  char label[MAX_ARTIFICIAL_LABEL_BYTES];
...
static void vmsdbgout_end_function(unsigned int line) {
  if (write_symbols == VMS_AND_DWARF2_DEBUG)
...
static void vmsdbgout_end_epilogue(unsigned int line, const char *file) {
  char label[MAX_ARTIFICIAL_LABEL_BYTES];
...
static void vmsdbgout_begin_block(register unsigned line,
...
static void vmsdbgout_end_block(register unsigned line,
                                register unsigned blocknum) {
  if (write_symbols == VMS_AND_DWARF2_DEBUG)
...
static bool vmsdbgout_ignore_block(const_tree block) {
  bool retval = 0;
...
static void vmsdbgout_begin_function(tree decl) {
  ...
  if (func_table_in_use == func_table_allocated) {
    ...
    func_table = (vms_func_ref)xrealloc(
        func_table, func_table_allocated * sizeof(vms_func_node));
  }
...
static char *full_name(const char *filename) {
#ifdef VMS
...
static unsigned int lookup_filename(const char *file_name) {
...
static void vmsdbgout_start_source_file(unsigned int lineno,
                                        const char *filename) {
  if (write_symbols == VMS_AND_DWARF2_DEBUG)
...
static void vmsdbgout_end_source_file(unsigned int lineno ATTRIBUTE_UNUSED) {
  if (write_symbols == VMS_AND_DWARF2_DEBUG)
...
static void vmsdbgout_init(const char *main_input_filename) {
  ...
  func_table =
      (vms_func_ref)xcalloc(FUNC_TABLE_INCREMENT, sizeof(vms_func_node));
  func_table_allocated = FUNC_TABLE_INCREMENT;
  func_table_in_use = 1;
...
static void vmsdbgout_define(unsigned int lineno, const char *buffer) {
  if (write_symbols == VMS_AND_DWARF2_DEBUG)
...
static void vmsdbgout_undef(unsigned int lineno, const char *buffer) {
  if (write_symbols == VMS_AND_DWARF2_DEBUG)
...
static void vmsdbgout_decl(tree decl) {
  if (write_symbols == VMS_AND_DWARF2_DEBUG)
...
static void vmsdbgout_global_decl(tree decl) {
  if (write_symbols == VMS_AND_DWARF2_DEBUG)
...
static void vmsdbgout_abstract_function(tree decl) {
  if (write_symbols == VMS_AND_DWARF2_DEBUG)
...
static void vmsdbgout_finish(const char *main_input_filename ATTRIBUTE_UNUSED) {
  unsigned int i;
  int totsize;
