...
static void do_niy(pretty_printer *buffer, gimple gs) {
...
static void maybe_init_pretty_print(FILE *file) {
  if (!initialized) {
...
void print_gimple_stmt(FILE *file, gimple g, int spc, int flags) {
...
void print_gimple_expr(FILE *file, gimple g, int spc, int flags) {
  flags |= TDF_RHS_ONLY;
  maybe_init_pretty_print(file);
  dump_gimple_stmt(&buffer, g, spc, flags);
}
...
static void dump_gimple_seq(pretty_printer *buffer, gimple_seq seq, int spc,
                            int flags) {
...
void print_gimple_seq(FILE *file, gimple_seq seq, int spc, int flags) {
  maybe_init_pretty_print(file);
...
static void dump_gimple_fmt(pretty_printer *buffer, int spc, int flags,
                            const char *fmt, ...) {
  va_list args;
  const char *c;
  const char *tmp;
...
static void dump_unary_rhs(pretty_printer *buffer, gimple gs, int spc,
                           int flags) {
  ...
  switch (rhs_code) {
    ...
    default:
      if (TREE_CODE_CLASS(rhs_code) == tcc_declaration ||
          ...
          TREE_CODE_CLASS(rhs_code) == tcc_reference || rhs_code == SSA_NAME ||
          rhs_code == ADDR_EXPR || rhs_code == CONSTRUCTOR) {
        dump_generic_node(buffer, rhs, spc, flags, false);
        break;
...
static void dump_binary_rhs(pretty_printer *buffer, gimple gs, int spc,
                            int flags) {
  const char *p;
...
static void dump_gimple_assign(pretty_printer *buffer, gimple gs, int spc,
                               int flags) {
  if (flags & TDF_RAW) {
    tree last;
...
static void dump_gimple_return(pretty_printer *buffer, gimple gs, int spc,
                               int flags) {
  tree t;
...
static void dump_gimple_call_args(pretty_printer *buffer, gimple gs,
                                  int flags) {
  size_t i;
...
static void dump_gimple_call(pretty_printer *buffer, gimple gs, int spc,
                             int flags) {
  tree lhs = gimple_call_lhs(gs);

  if (flags & TDF_RAW) {
    dump_gimple_fmt(buffer, spc, flags, "%G <%T, %T", gs, gimple_call_fn(gs),
                    lhs);
    if (gimple_call_num_args(gs) > 0) {
      pp_string(buffer, ", ");
...
static void dump_gimple_switch(pretty_printer *buffer, gimple gs, int spc,
                               int flags) {
  unsigned int i;
...
static void dump_gimple_cond(pretty_printer *buffer, gimple gs, int spc,
                             int flags) {
  if (flags & TDF_RAW)
    dump_gimple_fmt(buffer, spc, flags, "%G <%s, %T, %T, %T, %T>", gs,
                    tree_code_name[gimple_cond_code(gs)], gimple_cond_lhs(gs),
                    gimple_cond_rhs(gs), gimple_cond_true_label(gs),
...
static void dump_gimple_label(pretty_printer *buffer, gimple gs, int spc,
                              int flags) {
...
static void dump_gimple_goto(pretty_printer *buffer, gimple gs, int spc,
                             int flags) {
...
static void dump_gimple_bind(pretty_printer *buffer, gimple gs, int spc,
                             int flags) {
  if (flags & TDF_RAW)
...
static void dump_gimple_try(pretty_printer *buffer, gimple gs, int spc,
                            int flags) {
  if (flags & TDF_RAW) {
    const char *type;
    ...
    dump_gimple_fmt(buffer, spc, flags, "%G <%s,%+EVAL <%S>%nCLEANUP <%S>%->",
                    gs, type, gimple_try_eval(gs), gimple_try_cleanup(gs));
  } else {
...
static void dump_gimple_catch(pretty_printer *buffer, gimple gs, int spc,
                              int flags) {
  if (flags & TDF_RAW)
...
static void dump_gimple_eh_filter(pretty_printer *buffer, gimple gs, int spc,
                                  int flags) {
  if (flags & TDF_RAW)
    dump_gimple_fmt(buffer, spc, flags, "%G <%T, %+FAILURE <%S>%->", gs,
                    gimple_eh_filter_types(gs), gimple_eh_filter_failure(gs));
  else
...
static void dump_gimple_resx(pretty_printer *buffer, gimple gs, int spc,
                             int flags) {
  if (flags & TDF_RAW)
    dump_gimple_fmt(buffer, spc, flags, "%G <%d>", gs, gimple_resx_region(gs));
  else
    dump_gimple_fmt(buffer, spc, flags, "resx %d", gimple_resx_region(gs));
}

/* Dump a GIMPLE_OMP_FOR tuple on the pretty_printer BUFFER.  */
static void dump_gimple_omp_for(pretty_printer *buffer, gimple gs, int spc,
                                int flags) {
  size_t i;
  ...
  if (flags & TDF_RAW) {
    ...
    for (i = 0; i < gimple_omp_for_collapse(gs); i++)
      dump_gimple_fmt(buffer, spc, flags, "%+%T, %T, %T, %s, %T,%n",
    ...
    for (i = 0; i < gimple_omp_for_collapse(gs); i++) {
      ...
      dump_generic_node(buffer, gimple_omp_for_index(gs, i), spc, flags, false);
      pp_string(buffer, " = ");
      dump_generic_node(buffer, gimple_omp_for_initial(gs, i), spc, flags,
                        false);
      pp_string(buffer, "; ");
      ...
      pp_space(buffer);
      dump_generic_node(buffer, gimple_omp_for_final(gs, i), spc, flags, false);
      pp_string(buffer, "; ");

      dump_generic_node(buffer, gimple_omp_for_index(gs, i), spc, flags, false);
      pp_string(buffer, " = ");
      dump_generic_node(buffer, gimple_omp_for_incr(gs, i), spc, flags, false);
      pp_character(buffer, ')');
    }
...
static void dump_gimple_omp_continue(pretty_printer *buffer, gimple gs, int spc,
                                     int flags) {
  if (flags & TDF_RAW) {
    ...
    dump_generic_node(buffer, gimple_omp_continue_control_def(gs), spc, flags,
                      false);
    pp_character(buffer, ',');
    pp_space(buffer);
    dump_generic_node(buffer, gimple_omp_continue_control_use(gs), spc, flags,
                      false);
    pp_character(buffer, ')');
  }
}
...
static void dump_gimple_omp_single(pretty_printer *buffer, gimple gs, int spc,
                                   int flags) {
  if (flags & TDF_RAW) {
    dump_gimple_fmt(buffer, spc, flags, "%G <%+BODY <%S>%nCLAUSES <", gs,
                    gimple_omp_body(gs));
...
static void dump_gimple_omp_sections(pretty_printer *buffer, gimple gs, int spc,
                                     int flags) {
  if (flags & TDF_RAW) {
    dump_gimple_fmt(buffer, spc, flags, "%G <%+BODY <%S>%nCLAUSES <", gs,
                    gimple_omp_body(gs));
    ...
    if (gimple_omp_sections_control(gs)) {
      pp_string(buffer, " <");
      dump_generic_node(buffer, gimple_omp_sections_control(gs), spc, flags,
                        false);
      pp_character(buffer, '>');
    }
    dump_omp_clauses(buffer, gimple_omp_sections_clauses(gs), spc, flags);
...
static void dump_gimple_omp_block(pretty_printer *buffer, gimple gs, int spc,
                                  int flags) {
  if (flags & TDF_RAW)
...
static void dump_gimple_omp_critical(pretty_printer *buffer, gimple gs, int spc,
                                     int flags) {
  if (flags & TDF_RAW)
    dump_gimple_fmt(buffer, spc, flags, "%G <%+BODY <%S> >", gs,
                    gimple_omp_body(gs));
    ...
    if (gimple_omp_critical_name(gs)) {
      pp_string(buffer, " (");
      dump_generic_node(buffer, gimple_omp_critical_name(gs), spc, flags,
                        false);
      pp_character(buffer, ')');
    }
    if (!gimple_seq_empty_p(gimple_omp_body(gs))) {
...
static void dump_gimple_omp_return(pretty_printer *buffer, gimple gs, int spc,
                                   int flags) {
  if (flags & TDF_RAW) {
...
static void dump_gimple_asm(pretty_printer *buffer, gimple gs, int spc,
                            int flags) {
  unsigned int i;
  ...
  if (gimple_asm_ninputs(gs) || gimple_asm_noutputs(gs) ||
...
static void dump_symbols(pretty_printer *buffer, bitmap syms, int flags) {
  unsigned i;
  bitmap_iterator bi;
...
static void dump_gimple_phi(pretty_printer *buffer, gimple phi, int spc,
                            int flags) {
  size_t i;
  ...
  for (i = 0; i < gimple_phi_num_args(phi); i++) {
    dump_generic_node(buffer, gimple_phi_arg_def(phi, i), spc, flags, false);
    pp_character(buffer, '(');
...
static void dump_gimple_omp_parallel(pretty_printer *buffer, gimple gs, int spc,
                                     int flags) {
  if (flags & TDF_RAW) {
    dump_gimple_fmt(buffer, spc, flags, "%G <%+BODY <%S>%nCLAUSES <", gs,
                    gimple_omp_body(gs));
    ...
    if (gimple_omp_parallel_child_fn(gs)) {
      ...
      dump_generic_node(buffer, gimple_omp_parallel_child_fn(gs), spc, flags,
                        false);
      pp_string(buffer, " (");
      if (gimple_omp_parallel_data_arg(gs))
        dump_generic_node(buffer, gimple_omp_parallel_data_arg(gs), spc, flags,
                          false);
      else
        pp_string(buffer, "???");
      pp_string(buffer, ")]");
...
static void dump_gimple_omp_task(pretty_printer *buffer, gimple gs, int spc,
                                 int flags) {
  if (flags & TDF_RAW) {
    dump_gimple_fmt(buffer, spc, flags, "%G <%+BODY <%S>%nCLAUSES <", gs,
                    gimple_omp_body(gs));
    ...
    dump_gimple_fmt(buffer, spc, flags, " >, %T, %T, %T, %T, %T%n>",
                    gimple_omp_task_child_fn(gs), gimple_omp_task_data_arg(gs),
                    gimple_omp_task_copy_fn(gs), gimple_omp_task_arg_size(gs),
    ...
    if (gimple_omp_task_child_fn(gs)) {
      pp_string(buffer, " [child fn: ");
      dump_generic_node(buffer, gimple_omp_task_child_fn(gs), spc, flags,
                        false);
      pp_string(buffer, " (");
      if (gimple_omp_task_data_arg(gs))
        dump_generic_node(buffer, gimple_omp_task_data_arg(gs), spc, flags,
                          false);
      else
        pp_string(buffer, "???");
      pp_string(buffer, ")]");
...
static void dump_gimple_omp_atomic_load(pretty_printer *buffer, gimple gs,
                                        int spc, int flags) {
  if (flags & TDF_RAW) {
    dump_gimple_fmt(buffer, spc, flags, "%G <%T, %T>", gs,
    ...
    newline_and_indent(buffer, spc + 2);
    dump_generic_node(buffer, gimple_omp_atomic_load_lhs(gs), spc, flags,
                      false);
    pp_space(buffer);
    pp_character(buffer, '=');
    pp_space(buffer);
    pp_character(buffer, '*');
    dump_generic_node(buffer, gimple_omp_atomic_load_rhs(gs), spc, flags,
                      false);
  }
}
...
static void dump_gimple_omp_atomic_store(pretty_printer *buffer, gimple gs,
                                         int spc, int flags) {
  if (flags & TDF_RAW) {
    dump_gimple_fmt(buffer, spc, flags, "%G <%T>", gs,
    ...
    dump_generic_node(buffer, gimple_omp_atomic_store_val(gs), spc, flags,
                      false);
    pp_character(buffer, ')');
  }
}
...
static void dump_gimple_cdt(pretty_printer *buffer, gimple gs, int spc,
                            int flags) {
  if (flags & TDF_RAW)
    dump_gimple_fmt(buffer, spc, flags, "%G <%T, %T>", gs,
...
static void dump_gimple_mem_ops(pretty_printer *buffer, gimple gs, int spc,
                                int flags) {
  ...
  /* Even if the statement doesn't have virtual operators yet, it may
     ...
     computed).  */
  if ((flags & TDF_MEMSYMS) && gimple_vuse_ops(gs) == NULL &&
...
void dump_gimple_stmt(pretty_printer *buffer, gimple gs, int spc, int flags) {
  if (!gs)
    return;
  ...
  if ((flags & (TDF_VOPS | TDF_MEMSYMS)) && gimple_has_mem_ops(gs))
...
static void dump_bb_header(pretty_printer *buffer, basic_block bb, int indent,
                           int flags) {
  edge e;
  gimple stmt;
  edge_iterator ei;
...
static void dump_bb_end(pretty_printer *buffer, basic_block bb, int indent,
                        int flags) {
  edge e;
  edge_iterator ei;
...
static void dump_phi_nodes(pretty_printer *buffer, basic_block bb, int indent,
                           int flags) {
  gimple_stmt_iterator i;
...
static void dump_implicit_edges(pretty_printer *buffer, basic_block bb,
...
static void gimple_dump_bb_buff(pretty_printer *buffer, basic_block bb,
                                int indent, int flags) {
  gimple_stmt_iterator gsi;
  gimple stmt;
...
void gimple_dump_bb(basic_block bb, FILE *file, int indent, int flags) {
  maybe_init_pretty_print(file);
