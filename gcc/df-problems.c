...
void df_chain_dump(struct df_link *link, FILE *file) {
  ...
  for (; link; link = link->next) {
    fprintf(file, "%c%d(bb %d insn %d) ",
            DF_REF_REG_DEF_P(link->ref) ? 'd' : 'u', DF_REF_ID(link->ref),
...
static void df_rd_set_bb_info(unsigned int index,
...
static void df_rd_free_bb_info(basic_block bb ATTRIBUTE_UNUSED,
                               void *vbb_info) {
...
static void df_rd_alloc(bitmap all_blocks) {
  ...
  if (!df_rd->block_pool)
    df_rd->block_pool =
        create_alloc_pool("df_rd_block pool", sizeof(struct df_rd_bb_info), 50);
...
static void df_rd_bb_local_compute(unsigned int bb_index) {
  ...
  FOR_BB_INSNS_REVERSE(bb, insn) {
    ...
    df_rd_bb_local_compute_process_def(bb_info, DF_INSN_UID_DEFS(uid), 0);
  ...
  if (!(df->changeable_flags & DF_NO_HARD_REGS))
    df_rd_bb_local_compute_process_def(
        bb_info, df_get_artificial_defs(bb_index), DF_REF_AT_TOP);
}
...
static void df_rd_local_compute(bitmap all_blocks) {
  ...
  EXECUTE_IF_SET_IN_BITMAP(regs_invalidated_by_call_regset, 0, regno, bi) {
    if (DF_DEFS_COUNT(regno) > DF_SPARSE_THRESHOLD)
      ...
      bitmap_set_range(dense_invalidated, DF_DEFS_BEGIN(regno),
                       DF_DEFS_COUNT(regno));
  }
  df_unset_seen();
}
...
static void df_rd_confluence_n(edge e) {
...
static void df_rd_start_dump(FILE *file) {
  ...
  for (regno = 0; regno < m; regno++)
    if (DF_DEFS_COUNT(regno))
      fprintf(file, "%d[%d,%d] ", regno, DF_DEFS_BEGIN(regno),
              DF_DEFS_COUNT(regno));
  fprintf(file, "\n");
}
...
static void df_lr_set_bb_info(unsigned int index,
...
static void df_lr_free_bb_info(basic_block bb ATTRIBUTE_UNUSED,
                               void *vbb_info) {
...
static void df_lr_alloc(bitmap all_blocks ATTRIBUTE_UNUSED) {
  ...
  if (!df_lr->block_pool)
    df_lr->block_pool =
        create_alloc_pool("df_lr_block pool", sizeof(struct df_lr_bb_info), 50);

  df_grow_bb_info(df_lr);

  EXECUTE_IF_SET_IN_BITMAP(df_lr->out_of_date_transfer_functions, 0, bb_index,
                           bi) {
...
static void df_lr_confluence_n(edge e) {
...
void df_lr_verify_transfer_functions(void) {
  ...
  FOR_ALL_BB(bb) {
    ...
    if (bb_info) {
      ...
      gcc_assert(
          bitmap_bit_p(df_lr->out_of_date_transfer_functions, bb->index));
    }
...
static void df_live_set_bb_info(unsigned int index,
...
static void df_live_free_bb_info(basic_block bb ATTRIBUTE_UNUSED,
                                 void *vbb_info) {
...
static void df_live_alloc(bitmap all_blocks ATTRIBUTE_UNUSED) {
  ...
  if (!df_live->block_pool)
    df_live->block_pool = create_alloc_pool(
        "df_live_block pool", sizeof(struct df_live_bb_info), 100);
  if (!df_live_scratch)
    df_live_scratch = BITMAP_ALLOC(NULL);

  df_grow_bb_info(df_live);

  EXECUTE_IF_SET_IN_BITMAP(df_live->out_of_date_transfer_functions, 0, bb_index,
                           bi) {
...
static void df_live_confluence_n(edge e) {
  ...
  FOR_ALL_BB(bb) {
    ...
    if (bb_info) {
      ...
      /* If we do not have basic block info, the block must be in
         ...
         block behind our backs. */
      gcc_assert(
          bitmap_bit_p(df_live->out_of_date_transfer_functions, bb->index));
    }
    /* Make sure no one created a block without following
       procedures.  */
    gcc_assert(df_scan_get_bb_info(bb->index));
...
#define df_chain_problem_p(FLAG) \
  (((enum df_chain_flags)df_chain->local_flags) & (FLAG))

/* Create a du or ud chain from SRC to DST and link it into SRC.   */

struct df_link *df_chain_create(df_ref src, df_ref dst) {
...
static void df_chain_alloc(bitmap all_blocks ATTRIBUTE_UNUSED) {
  df_chain_remove_problem();
  df_chain->block_pool =
      create_alloc_pool("df_chain_block pool", sizeof(struct df_link), 50);
...
static void df_chain_create_bb_process_use(bitmap local_rd, df_ref *use_rec,
                                           enum df_ref_flags top_flag) {
  bitmap_iterator bi;
...
static void df_chain_create_bb(unsigned int bb_index) {
  ...
  /* Artificials are only hard regs.  */
  if (!(df->changeable_flags & DF_NO_HARD_REGS))
    df_chain_create_bb_process_use(cpy, df_get_artificial_uses(bb->index),
                                   DF_REF_AT_TOP);
#endif
  ...
  for (def_rec = df_get_artificial_defs(bb_index); *def_rec; def_rec++) {
    ...
    if (DF_REF_FLAGS(def) & DF_REF_AT_TOP) {
      unsigned int dregno = DF_REF_REGNO(def);
      if (!(DF_REF_FLAGS(def) & (DF_REF_PARTIAL | DF_REF_CONDITIONAL)))
        bitmap_clear_range(cpy, DF_DEFS_BEGIN(dregno), DF_DEFS_COUNT(dregno));
  ...
  if (!(df->changeable_flags & DF_NO_HARD_REGS))
    df_chain_create_bb_process_use(cpy, df_get_artificial_uses(bb->index), 0);
...
static struct df_problem problem_CHAIN = {
    ...
    df_chain_fully_remove_problem, /* Remove this problem from the stack of
                                      dataflow problems.  */
    NULL,                          /* Debugging.  */
...
void df_chain_add_problem(enum df_chain_flags chain_flags) {
...
int df_byte_lr_get_regno_start(unsigned int regno) {
...
int df_byte_lr_get_regno_len(unsigned int regno) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
  ;
...
static void df_byte_lr_set_bb_info(unsigned int index,
...
static void df_byte_lr_free_bb_info(basic_block bb ATTRIBUTE_UNUSED,
                                    void *vbb_info) {
...
static void df_byte_lr_check_regs(df_ref *ref_rec) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
...
static void df_byte_lr_expand_bitmap(bitmap dest, bitmap src) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
  bitmap_iterator bi;
  unsigned int i;
...
static void df_byte_lr_alloc(bitmap all_blocks ATTRIBUTE_UNUSED) {
  unsigned int bb_index;
  bitmap_iterator bi;
  basic_block bb;
  unsigned int regno;
  ...
  struct df_byte_lr_problem_data *problem_data = problem_data =
      XNEW(struct df_byte_lr_problem_data);

  df_byte_lr->problem_data = problem_data;

  if (!df_byte_lr->block_pool)
    df_byte_lr->block_pool = create_alloc_pool(
        "df_byte_lr_block pool", sizeof(struct df_byte_lr_bb_info), 50);
  ...
  problem_data->hardware_regs_used =
      BITMAP_ALLOC(&problem_data->byte_lr_bitmaps);
  ...
  EXECUTE_IF_SET_IN_BITMAP(df_byte_lr->out_of_date_transfer_functions, 0,
                           bb_index, bi) {
...
static void df_byte_lr_reset(bitmap all_blocks) {
  unsigned int bb_index;
  bitmap_iterator bi;
...
static void df_byte_lr_bb_local_compute(unsigned int bb_index) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
  basic_block bb = BASIC_BLOCK(bb_index);
  struct df_byte_lr_bb_info *bb_info = df_byte_lr_get_bb_info(bb_index);
...
static void df_byte_lr_local_compute(bitmap all_blocks ATTRIBUTE_UNUSED) {
  unsigned int bb_index;
  bitmap_iterator bi;
...
static void df_byte_lr_init(bitmap all_blocks) {
  unsigned int bb_index;
  bitmap_iterator bi;
...
/* Confluence function that processes infinite loops.  This might be a
   noreturn function that throws.  And even if it isn't, getting the
   unwind info right helps debugging.  */
static void df_byte_lr_confluence_0(basic_block bb) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
...
static void df_byte_lr_confluence_n(edge e) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
...
static bool df_byte_lr_transfer_function(int bb_index) {
  struct df_byte_lr_bb_info *bb_info = df_byte_lr_get_bb_info(bb_index);
  bitmap in = bb_info->in;
  bitmap out = bb_info->out;
  bitmap use = bb_info->use;
...
static void df_byte_lr_free(void) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
...
static void df_byte_lr_top_dump(basic_block bb, FILE *file) {
...
static void df_byte_lr_bottom_dump(basic_block bb, FILE *file) {
  struct df_byte_lr_bb_info *bb_info = df_byte_lr_get_bb_info(bb->index);
  if (!bb_info || !bb_info->out)
    return;
...
void df_byte_lr_add_problem(void) {
...
void df_byte_lr_simulate_defs(rtx insn, bitmap live) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
  df_ref *def_rec;
  unsigned int uid = INSN_UID(insn);
...
void df_byte_lr_simulate_uses(rtx insn, bitmap live) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
  df_ref *use_rec;
  unsigned int uid = INSN_UID(insn);
...
void df_byte_lr_simulate_artificial_refs_at_top(basic_block bb, bitmap live) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
  df_ref *def_rec;
#ifdef EH_USES
...
void df_byte_lr_simulate_artificial_refs_at_end(basic_block bb, bitmap live) {
  struct df_byte_lr_problem_data *problem_data =
      (struct df_byte_lr_problem_data *)df_byte_lr->problem_data;
  df_ref *def_rec;
  df_ref *use_rec;
...
static void df_print_note(const char *prefix, rtx insn, rtx note) {
  if (dump_file) {
...
#ifdef STACK_REGS
static inline bool df_ignore_stack_reg(int regno) {
  return regstack_completed && IN_RANGE(regno, FIRST_STACK_REG, LAST_STACK_REG);
}
#else
...
static void df_kill_notes(rtx insn, rtx *old_dead_notes,
...
static inline rtx df_set_note(enum reg_note note_type, rtx insn, rtx old,
                              rtx reg) {
  rtx curr = old;
  rtx prev = NULL;
...
static bool df_whole_mw_reg_unused_p(struct df_mw_hardreg *mws, bitmap live,
...
static rtx df_set_unused_notes_for_mw(rtx insn, rtx old,
                                      struct df_mw_hardreg *mws, bitmap live,
                                      bitmap do_not_gen,
                                      bitmap artificial_uses) {
  unsigned int r;
...
static bool df_whole_mw_reg_dead_p(struct df_mw_hardreg *mws, bitmap live,
                                   bitmap artificial_uses, bitmap do_not_gen) {
  unsigned int r;
...
static rtx df_set_dead_notes_for_mw(rtx insn, rtx old,
                                    struct df_mw_hardreg *mws, bitmap live,
                                    bitmap do_not_gen, bitmap artificial_uses) {
  unsigned int r;
...
static rtx df_create_unused_note(rtx insn, rtx old, df_ref def, bitmap live,
                                 bitmap artificial_uses) {
  unsigned int dregno = DF_REF_REGNO(def);
  ...
  if (!(bitmap_bit_p(live, dregno) || (DF_REF_FLAGS(def) & DF_REF_MW_HARDREG) ||
        bitmap_bit_p(artificial_uses, dregno) || df_ignore_stack_reg(dregno))) {
...
static void df_note_bb_compute(unsigned int bb_index, bitmap live,
                               bitmap do_not_gen, bitmap artificial_uses) {
  basic_block bb = BASIC_BLOCK(bb_index);
  rtx insn;
  df_ref *def_rec;
  ...
  FOR_BB_INSNS_REVERSE(bb, insn) {
    ...
    if (CALL_P(insn)) {
      ...
      while (*mws_rec) {
        ...
        if ((DF_MWS_REG_DEF_P(mws)) && !df_ignore_stack_reg(mws->start_regno))
          old_unused_notes = df_set_unused_notes_for_mw(
              insn, old_unused_notes, mws, live, do_not_gen, artificial_uses);
        mws_rec++;
      }
      ...
      for (def_rec = DF_INSN_UID_DEFS(uid); *def_rec; def_rec++) {
        ...
        if (!DF_REF_FLAGS_IS_SET(def,
          ...
          old_unused_notes = df_create_unused_note(insn, old_unused_notes, def,
                                                   live, artificial_uses);
    ...
    while (*mws_rec) {
      struct df_mw_hardreg *mws = *mws_rec;
      if ((DF_MWS_REG_DEF_P(mws)) && !df_ignore_stack_reg(mws->start_regno))
        old_dead_notes = df_set_dead_notes_for_mw(
            insn, old_dead_notes, mws, live, do_not_gen, artificial_uses);
      mws_rec++;
    }
...
void df_simulate_find_defs(rtx insn, bitmap defs) {
  df_ref *def_rec;
  unsigned int uid = INSN_UID(insn);
...
void df_simulate_defs(rtx insn, bitmap live) {
  df_ref *def_rec;
  unsigned int uid = INSN_UID(insn);
...
void df_simulate_uses(rtx insn, bitmap live) {
  df_ref *use_rec;
  unsigned int uid = INSN_UID(insn);
...
void df_simulate_one_insn_backwards(basic_block bb, rtx insn, bitmap live) {
  if (!INSN_P(insn))
    return;
...
void df_simulate_one_insn_forwards(basic_block bb, rtx insn, bitmap live) {
  rtx link;
  if (!INSN_P(insn))
    return;
...
void df_simulate_finalize_forwards(basic_block bb, bitmap live) {
  df_ref *def_rec;
  int bb_index = bb->index;
