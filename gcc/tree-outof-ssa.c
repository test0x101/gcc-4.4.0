...
static void insert_copy_on_edge(edge e, tree dest, tree src) {
  ...
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    fprintf(dump_file, "Inserting a copy on edge BB%d->BB%d :", e->src->index,
            e->dest->index);
...
static inline void clear_elim_graph(elim_graph g) {
...
static inline void delete_elim_graph(elim_graph g) {
...
static inline int elim_graph_size(elim_graph g) {
  return VEC_length(tree, g->nodes);
}

/* Add NODE to graph G, if it doesn't exist already.  */

static inline void elim_graph_add_node(elim_graph g, tree node) {
  int x;
  tree t;
...
static inline void elim_graph_add_edge(elim_graph g, int pred, int succ) {
...
static inline int elim_graph_remove_succ_edge(elim_graph g, int node) {
  int y;
  unsigned x;
...
static inline void eliminate_name(elim_graph g, tree T) {
...
static void eliminate_build(elim_graph g, basic_block B) {
  ...
  for (gsi = gsi_start_phis(B); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    /* If this argument is a constant, or a SSA_NAME which is being
       ...
       edge.  */
    if (!phi_ssa_name_p(Ti) || (TREE_CODE(Ti) == SSA_NAME &&
...
static void elim_backward(elim_graph g, int T) {
  ...
  FOR_EACH_ELIM_GRAPH_PRED(g, T, P, {
    if (!TEST_BIT(g->visited, P)) {
      elim_backward(g, P);
      insert_copy_on_edge(g->e, partition_to_var(g->map, P),
...
static void elim_create(elim_graph g, int T) {
  ...
  if (elim_unvisited_predecessor(g, T)) {
    ...
    if (S != -1) {
      SET_BIT(g->visited, T);
      insert_copy_on_edge(g->e, partition_to_var(g->map, T),
...
static inline bool replace_use_variable(var_map map, use_operand_p p,
                                        gimple *expr) {
  tree new_var;
...
static inline bool replace_def_variable(var_map map, def_operand_p def_p,
                                        tree *expr) {
  tree new_var;
...
static void remove_gimple_phi_args(gimple phi) {
  use_operand_p arg_p;
  ssa_op_iter iter;
    ...
    for (gsi = gsi_start_phis(bb); !gsi_end_p(gsi);) {
      ...
      if (!is_gimple_reg(SSA_NAME_VAR(result))) {
        ...
        for (i = 0; i < gimple_phi_num_args(phi); i++) {
          ...
          if (TREE_CODE(arg) == SSA_NAME && is_gimple_reg(SSA_NAME_VAR(arg))) {
...
static void rewrite_trees(var_map map, gimple *values) {
  elim_graph g;
  basic_block bb;
  gimple_stmt_iterator gsi;
  edge e;
...
static inline bool same_stmt_list_p(edge e) {
  return (e->aux == (PTR)leader_match) ? true : false;
}

/* Return TRUE if S1 and S2 are equivalent copies.  */

static inline bool identical_copies_p(const_gimple s1, const_gimple s2) {
#ifdef ENABLE_CHECKING
...
static inline bool identical_stmt_lists_p(const_edge e1, const_edge e2) {
  gimple_seq t1 = PENDING_STMT(e1);
  gimple_seq t2 = PENDING_STMT(e2);
  gimple_stmt_iterator gsi1, gsi2;

  for (gsi1 = gsi_start(t1), gsi2 = gsi_start(t2);
       !gsi_end_p(gsi1) && !gsi_end_p(gsi2); gsi_next(&gsi1), gsi_next(&gsi2)) {
...
static tree contains_tree_r(tree *tp, int *walk_subtrees, void *data) {
  if (*tp == data) {
    *walk_subtrees = 0;
    return (tree)data;
  } else
...
static bool process_single_block_loop_latch(edge single_edge) {
  gimple_seq stmts;
  ...
  if (single_edge == NULL || (single_edge->dest != single_edge->src) ||
      (EDGE_COUNT(b_loop->succs) != 2) || (EDGE_COUNT(b_loop->preds) != 2))
    return false;
  ...
  for (tsi = gsi_start(stmts); !gsi_end_p(tsi); gsi_next(&tsi)) {
    ...
    if (TREE_THIS_VOLATILE(var) || TYPE_VOLATILE(TREE_TYPE(var)) ||
...
static void analyze_edges_for_bb(basic_block bb) {
  edge e;
  edge_iterator ei;
  int count;
  unsigned int x;
  ...
  for (x = 0; VEC_iterate(edge, edge_leader, x, leader); x++)
    if (bitmap_bit_p(leader_has_match, x)) {
      ...
      new_edge = make_forwarder_block(leader->dest, same_stmt_list_p, NULL);
      bb = new_edge->dest;
      if (dump_file) {
...
static void perform_edge_inserts(void) {
  basic_block bb;
    ...
    for (gsi = gsi_start_phis(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      for (i = 0; i < gimple_phi_num_args(phi); i++) {
        ...
        if ((e->flags & EDGE_DFS_BACK) &&
            (TREE_CODE(arg) != SSA_NAME || SSA_NAME_VAR(arg) != result_var)) {
          tree name;
          ...
          /* Create a new instance of the underlying variable of the
             PHI result.  */
          stmt = gimple_build_assign(result_var, gimple_phi_arg_def(phi, i));
...
static unsigned int rewrite_out_of_ssa(void) {
