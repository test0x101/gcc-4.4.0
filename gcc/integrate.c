...
void set_decl_origin_self(tree decl) {
...
rtx get_hard_reg_initial_val(enum machine_mode mode, unsigned int regno) {
  ...
  if (ivs->num_entries >= ivs->max_entries) {
    ivs->max_entries += 5;
    ivs->entries =
        GGC_RESIZEVEC(initial_value_pair, ivs->entries, ivs->max_entries);
  }
...
rtx has_hard_reg_initial_val(enum machine_mode mode, unsigned int regno) {
  struct initial_value_struct *ivs;
  int i;
...
void allocate_initial_values(rtx *reg_equiv_memory_loc) {
