...
static const char *varpool_node_name(struct varpool_node *node) {
  return lang_hooks.decl_printable_name(node->decl, 2);
}

/* Returns a hash code for P.  */
static hashval_t hash_varpool_node(const void *p) {
  const struct varpool_node *n = (const struct varpool_node *)p;
  return (hashval_t)DECL_UID(n->decl);
}

/* Returns nonzero if P1 and P2 are equal.  */
static int eq_varpool_node(const void *p1, const void *p2) {
  const struct varpool_node *n1 = (const struct varpool_node *)p1;
  const struct varpool_node *n2 = (const struct varpool_node *)p2;
...
struct varpool_node *varpool_node(tree decl) {
  ...
  if (!varpool_hash)
    varpool_hash =
        htab_create_ggc(10, hash_varpool_node, eq_varpool_node, NULL);
  key.decl = decl;
...
void dump_varpool_node(FILE *f, struct varpool_node *node) {
...
static void varpool_enqueue_needed_node(struct varpool_node *node) {
...
/* Notify finalize_compilation_unit that given node is reachable
   or needed.  */
void varpool_mark_needed_node(struct varpool_node *node) {
  if (!node->needed && node->finalized && !TREE_ASM_WRITTEN(node->decl))
    varpool_enqueue_needed_node(node);
  node->needed = 1;
}

/* Reset the queue of needed nodes.  */
static void varpool_reset_queue(void) {
...
/* Determine if variable DECL is needed.  That is, visible to something
   ...
   configury */
bool decide_is_variable_needed(struct varpool_node *node, tree decl) {
...
/* Mark DECL as finalized.  By finalizing the declaration, frontend instruct the
   ...
   visible.  */
void varpool_finalize_decl(tree decl) {
