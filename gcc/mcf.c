...
static void dump_fixup_edge(FILE *file, fixup_graph_type *fixup_graph,
  ...
  if (fedge->type) {
    fprintf(file, "flow/capacity=" HOST_WIDEST_INT_PRINT_DEC "/", fedge->flow);
...
static void dump_fixup_graph(FILE *file, fixup_graph_type *fixup_graph,
  ...
  fprintf(file, "There are %d vertices and %d edges. new_exit_index is %d.\n\n",
  ...
  for (i = 0; i < fnum_vertices; i++) {
    ...
    fprintf(file, "vertex_list[%d]: %d succ fixup edges.\n", i,
            VEC_length(fixup_edge_p, pfvertex->succ_edges));
...
static fixup_edge_p add_edge(fixup_graph_type *fixup_graph, int src, int dest,
                             gcov_type cost) {
...
static void add_fixup_edge(fixup_graph_type *fixup_graph, int src, int dest,
                           int type, gcov_type weight, gcov_type cost,
                           gcov_type max_capacity) {
...
static void add_rfixup_edge(fixup_graph_type *fixup_graph, int src, int dest,
...
static fixup_edge_p find_fixup_edge(fixup_graph_type *fixup_graph, int src,
                                    int dest) {
  int j;
  fixup_edge_p pfedge;
  ...
  for (j = 0; VEC_iterate(fixup_edge_p, pfvertex->succ_edges, j, pfedge); j++)
    if (pfedge->dest == dest)
      return pfedge;
...
static void create_fixup_graph(fixup_graph_type *fixup_graph) {
  ...
  diff_out_in = (gcov_type *)xcalloc(1 + fnum_vertices_after_transform,
                                     sizeof(gcov_type));
  ...
  FOR_BB_BETWEEN(bb, ENTRY_BLOCK_PTR, NULL, next_bb) {
    ...
    add_fixup_edge(fixup_graph, i, i + 1, VERTEX_SPLIT_EDGE, bb->count, fcost,
                   CAP_INFINITY);
  ...
  for (i = 0; i < fnum_edges_after_transform; i++) {
    ...
    if (!r_pfedge && pfedge->weight) {
      ...
      add_fixup_edge(fixup_graph, pfedge->dest, pfedge->src, REVERSE_EDGE, 0,
                     fcost, pfedge->weight);
    }
  }
  ...
  add_fixup_edge(fixup_graph, 2 * EXIT_BLOCK + 1, new_exit_index,
                 SINK_CONNECT_EDGE, 0 /* demand_value */, 0,
                 0 /* demand_value */);
...
static void init_augmenting_path(augmenting_path_type *augmenting_path,
                                 int graph_size) {
  augmenting_path->queue_list.queue =
      (int *)xcalloc(graph_size + 2, sizeof(int));
...
static bool cancel_negative_cycle(fixup_graph_type *fixup_graph, int *pi,
                                  gcov_type *d, int *cycle) {
  int i, j, k;
  int fnum_vertices, fnum_edges;
  ...
  if (dump_file) {
    ...
    fprintf(dump_file,
            ...
            ")\n",
            sum_cost, cycle_flow);
    fprintf(dump_file, "Augment cycle with " HOST_WIDEST_INT_PRINT_DEC "\n",
            cycle_flow);
  }
...
/* Routine to find the maximal flow:
   ...
   3. Send flow equal to the path's residual capacity along the edges of this
path.
...
FIXUP_GRAPH: adjacency matrix representing the graph. The flow of the edges will
be set to have a valid maximal flow by this routine. (input) Return: Maximum
flow possible.  */

static gcov_type find_max_flow(fixup_graph_type *fixup_graph, int source,
                               int sink) {
  int fnum_edges;
  augmenting_path_type augmenting_path;
  int *bb_pred;
  ...
  while (find_augmenting_path(fixup_graph, &augmenting_path, source, sink)) {
    ...
    if (dump_file) {
      ...
      fprintf(dump_file,
              ...
              increment);
      fprintf(dump_file, "Network flow is " HOST_WIDEST_INT_PRINT_DEC ".\n",
              max_flow);
    }
  }
...
static void adjust_cfg_counts(fixup_graph_type *fixup_graph) {
  ...
  FOR_BB_BETWEEN(bb, ENTRY_BLOCK_PTR, EXIT_BLOCK_PTR, next_bb) {
    ...
    /* Fixup BB.  */
    if (dump_file)
      fprintf(dump_file, "BB%d: " HOST_WIDEST_INT_PRINT_DEC "", bb->index,
              bb->count);
    ...
    if (pfedge->flow) {
      ...
      if (dump_file) {
        fprintf(dump_file, " + " HOST_WIDEST_INT_PRINT_DEC "(", pfedge->flow);
        print_edge(dump_file, fixup_graph, i, i + 1);
        fprintf(dump_file, ")");
      }
    }

    pfedge_n = find_fixup_edge(fixup_graph, i + 1, pfedge->norm_vertex_index);
    /* Deduct flow from normalized reverse edge.  */
    if (pfedge->norm_vertex_index && pfedge_n->flow) {
      bb->count -= pfedge_n->flow;
      if (dump_file) {
        fprintf(dump_file, " - " HOST_WIDEST_INT_PRINT_DEC "(", pfedge_n->flow);
        print_edge(dump_file, fixup_graph, i + 1, pfedge->norm_vertex_index);
        fprintf(dump_file, ")");
      }
    }
    if (dump_file)
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      ...
      j = 2 * e->dest->index;
      if (dump_file)
        fprintf(dump_file, "%d->%d: " HOST_WIDEST_INT_PRINT_DEC "", bb->index,
                e->dest->index, e->count);
      ...
      if (bb->index != e->dest->index) {
        ...
        pfedge_n = find_fixup_edge(fixup_graph, j, pfedge->norm_vertex_index);
        /* Deduct flow from normalized reverse edge.  */
        if (pfedge->norm_vertex_index && pfedge_n->flow) {
          ...
          if (dump_file) {
            fprintf(dump_file, " - " HOST_WIDEST_INT_PRINT_DEC "(",
                    pfedge_n->flow);
            print_edge(dump_file, fixup_graph, j, pfedge->norm_vertex_index);
            fprintf(dump_file, ")");
          }
        }
      } else {
  ...
  if (dump_file) {
    ...
    FOR_BB_BETWEEN(bb, ENTRY_BLOCK_PTR->next_bb, EXIT_BLOCK_PTR, next_bb) {
      if ((bb->count != sum_edge_counts(bb->preds)) ||
        ...
        fprintf(dump_file,
                "BB%d(" HOST_WIDEST_INT_PRINT_DEC ")  **INVALID**: ", bb->index,
                bb->count);
        fprintf(stderr,
...
static void find_minimum_cost_flow(fixup_graph_type *fixup_graph) {
  ...
  while (cancel_negative_cycle(fixup_graph, pred, d, cycle)) {
    iteration++;
    if (iteration > MAX_ITER(fixup_graph->num_vertices, fixup_graph->num_edges))
      break;
  }

  if (dump_file)
    dump_fixup_graph(dump_file, fixup_graph, "After find_minimum_cost_flow()");
...
gcov_type sum_edge_counts(VEC(edge, gc) * to_edges) {
  gcov_type sum = 0;
  edge e;
  edge_iterator ei;
