...
extern char C_SIZEOF_STRUCT_LANG_IDENTIFIER_isnt_accurate
    [(sizeof(struct lang_identifier) == C_SIZEOF_STRUCT_LANG_IDENTIFIER) ? 1
                                                                         : -1];
...
static tree grokdeclarator(const struct c_declarator *, struct c_declspecs *,
...
void c_print_identifier(FILE *file, tree node, int indent) {
  ...
  if (C_IS_RESERVED_WORD(node) && C_RID_CODE(node) != RID_CXX_COMPAT_WARN) {
    ...
    fprintf(file, "rid %p \"%s\"", (void *)rid, IDENTIFIER_POINTER(rid));
  }
}
...
/* Establish a binding between NAME, an IDENTIFIER_NODE, and DECL,
   ...
   in the scope SCOPE.  */
static void bind(tree name, tree decl, struct c_scope *scope, bool invisible,
                 bool nested) {
...
void c_finish_incomplete_decl(tree decl) {
  if (TREE_CODE(decl) == VAR_DECL) {
    ...
    if (type != error_mark_node && TREE_CODE(type) == ARRAY_TYPE &&
...
void objc_mark_locals_volatile(void *enclosing_blk) {
  ...
  for (scope = current_scope; scope && scope != enclosing_blk;
       scope = scope->outer) {
...
static void set_type_context(tree type, tree context) {
  for (type = TYPE_MAIN_VARIANT(type); type; type = TYPE_NEXT_VARIANT(type))
  ...
  for (b = scope->bindings; b; b = free_binding_and_advance(b)) {
    ...
    switch (TREE_CODE(p)) {
      ...
      case FUNCTION_DECL:
        ...
        if (!TREE_ASM_WRITTEN(p) && DECL_INITIAL(p) != 0 &&
            TREE_ADDRESSABLE(p) && DECL_ABSTRACT_ORIGIN(p) != 0 &&
            DECL_ABSTRACT_ORIGIN(p) != p)
          TREE_ADDRESSABLE(DECL_ABSTRACT_ORIGIN(p)) = 1;
        if (!DECL_EXTERNAL(p) && !DECL_INITIAL(p) && scope != file_scope &&
            scope != external_scope) {
          error("nested function %q+D declared but never defined", p);
          undef_nested_function = true;
        } else if (DECL_DECLARED_INLINE_P(p) && TREE_PUBLIC(p) &&
                   !DECL_INITIAL(p)) {
      ...
      case VAR_DECL:
        ...
        if (!TREE_USED(p) && !TREE_NO_WARNING(p) && !DECL_IN_SYSTEM_HEADER(p) &&
            DECL_NAME(p) && !DECL_ARTIFICIAL(p) && scope != file_scope &&
            scope != external_scope)
          warning(OPT_Wunused_variable, "unused variable %q+D", p);

        if (b->inner_comp) {
          error(
              "type of array %q+D completed incompatibly with"
...
static tree match_builtin_function_types(tree newtype, tree oldtype) {
  ...
  while (oldargs || newargs) {
    if (!oldargs || !newargs || !TREE_VALUE(oldargs) || !TREE_VALUE(newargs) ||
        TYPE_MODE(TREE_VALUE(oldargs)) != TYPE_MODE(TREE_VALUE(newargs)))
      return 0;
...
/* Subroutine of diagnose_mismatched_decls.  Check for function type
   ...
   diagnostics.  */
static void diagnose_arglist_conflict(tree newdecl, tree olddecl, tree newtype,
                                      tree oldtype) {
  tree t;
...
static bool validate_proto_after_old_defn(tree newdecl, tree newtype,
                                          tree oldtype) {
  tree newargs, oldargs;
  int i;
...
#define END_OF_ARGLIST(t) ((t) == void_type_node)
  ...
  for (;;) {
    ...
    if (END_OF_ARGLIST(oldargtype)) {
      error(
          "prototype for %q+D declares more arguments "
          "than previous old-style definition",
          newdecl);
      return false;
    } else if (END_OF_ARGLIST(newargtype)) {
      error(
          "prototype for %q+D declares fewer arguments "
          "than previous old-style definition",
          newdecl);
      return false;
    }
    ...
    /* Type for passing arg must be consistent with that declared
      ...
      error(
          "prototype for %q+D declares argument %d"
...
static bool diagnose_mismatched_decls(tree newdecl, tree olddecl,
  ...
  *newtypep = newtype = TREE_TYPE(newdecl);
  ...
  if (!comptypes(oldtype, newtype)) {
    ...
    /* Permit void foo (...) to match int foo (...) if the latter is
      ...
      pedwarned =
          pedwarn(input_location, 0, "conflicting types for %q+D", newdecl);
  ...
  if (TREE_CODE(newdecl) == TYPE_DECL) {
    if (DECL_IN_SYSTEM_HEADER(newdecl) || DECL_IN_SYSTEM_HEADER(olddecl) ||
        TREE_NO_WARNING(newdecl) || TREE_NO_WARNING(olddecl))
  ...
  /* Function declarations can either be 'static' or 'extern' (no
    ...
    if (DECL_BUILT_IN(olddecl) && !C_DECL_DECLARED_BUILTIN(olddecl) &&
        (!TREE_PUBLIC(newdecl) ||
         (DECL_INITIAL(newdecl) && !TYPE_ARG_TYPES(TREE_TYPE(newdecl))))) {
      warning(OPT_Wshadow,
              "declaration of %q+D shadows "
              "a built-in function",
              newdecl);
    ...
    if (DECL_INITIAL(newdecl)) {
      if (DECL_INITIAL(olddecl)) {
        /* If both decls are in the same TU and the new declaration
           ...
           unit.  */
        if ((!DECL_EXTERN_INLINE(olddecl) || DECL_EXTERN_INLINE(newdecl) ||
             (!flag_gnu89_inline &&
              (!DECL_DECLARED_INLINE_P(olddecl) ||
               !lookup_attribute("gnu_inline", DECL_ATTRIBUTES(olddecl))) &&
    ...
    else if (DECL_INITIAL(olddecl) && !TYPE_ARG_TYPES(oldtype) &&
             TYPE_ARG_TYPES(newtype) && TYPE_ACTUAL_ARG_TYPES(oldtype) &&
    ...
    if (TREE_PUBLIC(olddecl) && !TREE_PUBLIC(newdecl)) {
      ...
      if (!DECL_IS_BUILTIN(olddecl) && !DECL_EXTERN_INLINE(olddecl)) {
        error(
      ...
      if (DECL_CONTEXT(olddecl)) {
        error(
            "non-static declaration of %q+D follows "
            "static declaration",
            newdecl);
        locate_old_decl(olddecl);
        return false;
    ...
    if (DECL_DECLARED_INLINE_P(olddecl) && DECL_DECLARED_INLINE_P(newdecl)) {
      bool newa =
          lookup_attribute("gnu_inline", DECL_ATTRIBUTES(newdecl)) != NULL;
      bool olda =
          lookup_attribute("gnu_inline", DECL_ATTRIBUTES(olddecl)) != NULL;
      if (newa != olda) {
    ...
    if (C_DECL_THREADPRIVATE_P(olddecl) && !DECL_THREAD_LOCAL_P(newdecl)) {
      ...
      if (DECL_THREAD_LOCAL_P(newdecl))
        error(
            "thread-local declaration of %q+D follows "
            "non-thread-local declaration",
            newdecl);
      else
        error(
            "non-thread-local declaration of %q+D follows "
    ...
    if (DECL_FILE_SCOPE_P(newdecl) &&
      ...
      if (DECL_EXTERNAL(newdecl)) {
        if (!DECL_FILE_SCOPE_P(olddecl)) {
          error(
              "extern declaration of %q+D follows "
    ...
    /* Two objects with the same name declared at the same block
      ...
      if (DECL_EXTERNAL(newdecl)) {
        ...
        error(
            "declaration of %q+D with no linkage follows "
            "extern declaration",
            newdecl);
        locate_old_decl(olddecl);
      } else {
  ...
  if (CODE_CONTAINS_STRUCT(TREE_CODE(newdecl), TS_DECL_WITH_VIS) &&
    ...
    warned |= warning(0,
                      "redeclaration of %q+D with different visibility "
                      "(old visibility preserved)",
                      newdecl);
  }
  ...
  if (!warned && !pedwarned &&
      ...
      && !(TREE_CODE(newdecl) == FUNCTION_DECL && !DECL_BUILT_IN(newdecl) &&
           DECL_BUILT_IN(olddecl) && !C_DECL_DECLARED_BUILTIN(olddecl))
      /* Don't warn about an extern followed by a definition.  */
      && !(DECL_EXTERNAL(olddecl) && !DECL_EXTERNAL(newdecl))
      /* Don't warn about forward parameter decls.  */
      && !(TREE_CODE(newdecl) == PARM_DECL && TREE_ASM_WRITTEN(olddecl) &&
           !TREE_ASM_WRITTEN(newdecl))
      /* Don't warn about a variable definition following a declaration.  */
      && !(TREE_CODE(newdecl) == VAR_DECL && DECL_INITIAL(newdecl) &&
           !DECL_INITIAL(olddecl))) {
...
static void merge_decls(tree newdecl, tree olddecl, tree newtype,
                        tree oldtype) {
  ...
  if (TREE_CODE(newdecl) == PARM_DECL && TREE_ASM_WRITTEN(olddecl) &&
      !TREE_ASM_WRITTEN(newdecl)) {
    struct c_binding *b, **here;
  ...
  DECL_ATTRIBUTES(newdecl) = targetm.merge_decl_attributes(olddecl, newdecl);

  /* Merge the data types specified in the two decls.  */
  TREE_TYPE(newdecl) = TREE_TYPE(olddecl) = composite_type(newtype, oldtype);
  ...
  if (!comptypes(oldtype, TREE_TYPE(newdecl))) {
    ...
    if (TREE_CODE(newdecl) != FUNCTION_DECL &&
        TREE_CODE(newdecl) != TYPE_DECL && TREE_CODE(newdecl) != CONST_DECL)
  ...
  /* In c99, 'extern' declaration before (or after) 'inline' means this
     ...
     is present.  */
  if (TREE_CODE(newdecl) == FUNCTION_DECL && !flag_gnu89_inline &&
  ...
  if (TREE_CODE(newdecl) == FUNCTION_DECL) {
    ...
    if (DECL_BUILT_IN(olddecl)) {
      ...
      if (new_is_prototype)
        ...
        C_DECL_BUILTIN_PROTOTYPE(newdecl) = C_DECL_BUILTIN_PROTOTYPE(olddecl);
    }
...
tree pushdecl(tree x) {
  ...
  if (DECL_EXTERNAL(x) || scope == file_scope) {
    ...
    while (b && !B_IN_EXTERNAL_SCOPE(b)) {
      ...
      if (B_IN_FILE_SCOPE(b) && TREE_CODE(b->decl) == VAR_DECL &&
          TREE_STATIC(b->decl) && TREE_CODE(TREE_TYPE(b->decl)) == ARRAY_TYPE &&
          !TYPE_DOMAIN(TREE_TYPE(b->decl)) && TREE_CODE(type) == ARRAY_TYPE &&
    ...
    if (b && (TREE_PUBLIC(x) || same_translation_unit_p(x, b->decl)) && b->type)
    ...
    if (b && (TREE_PUBLIC(x) || same_translation_unit_p(x, b->decl)) &&
      ...
      if (TREE_CODE(b->decl) == FUNCTION_DECL && DECL_BUILT_IN(b->decl))
        thistype =
            build_type_attribute_variant(thistype, TYPE_ATTRIBUTES(b->type));
...
skip_external_and_shadow_checks:
  ...
  if (TREE_TYPE(x) != error_mark_node && !COMPLETE_TYPE_P(TREE_TYPE(x))) {
    ...
    if ((TREE_CODE(element) == RECORD_TYPE ||
        ...
        (TREE_CODE(x) != TYPE_DECL || TREE_CODE(TREE_TYPE(x)) == ARRAY_TYPE) &&
  ...
  if (decl) {
    ...
    if (!DECL_BUILT_IN(decl) && DECL_IS_BUILTIN(decl)) {
      ...
      if (DECL_BUILT_IN(decl)) {
        newtype = build_type_attribute_variant(
            newtype, TYPE_ATTRIBUTES(TREE_TYPE(decl)));
        ...
        if (!comptypes(newtype, TREE_TYPE(decl))) {
          error("incompatible implicit declaration of function %qD", decl);
...
tree define_label(location_t location, tree name) {
  ...
  if (!in_system_header && lookup_name(name))
    warning(OPT_Wtraditional,
            ...
            "for labels, identifier %qE conflicts",
            &location, name);
...
void pending_xref_error(void) {
  if (pending_invalid_xref != 0)
    error("%H%qE defined as wrong kind of tag", &pending_invalid_xref_location,
...
void c_init_decl_processing(void) {
  ...
  pushdecl(build_decl(TYPE_DECL, get_identifier("_Bool"), boolean_type_node));
...
void shadow_tag_warned(const struct c_declspecs *declspecs, int warned) {
       ...
       That caused warning for `struct foo;' at top level in the file.  */
    {
      ...
      if (name == 0) {
        ...
        pending_xref_error();
                 ...
                 (declspecs->const_p || declspecs->volatile_p ||
  ...
  if (!warned && !in_system_header &&
      (declspecs->const_p || declspecs->volatile_p || declspecs->restrict_p)) {
...
int quals_from_declspecs(const struct c_declspecs *specs) {
  ...
  gcc_assert(!specs->type && !specs->decl_attr &&
             specs->typespec_word == cts_none &&
             specs->storage_class == csc_none && !specs->typedef_p &&
             !specs->explicit_signed_p && !specs->deprecated_p &&
             !specs->long_p && !specs->long_long_p && !specs->short_p &&
             !specs->signed_p && !specs->unsigned_p && !specs->complex_p &&
             !specs->inline_p && !specs->thread_p);
  return quals;
}
...
struct c_declarator *build_array_declarator(tree expr,
                                            ...
                                            bool static_p, bool vla_unspec_p) {
...
static void add_flexible_array_elts_to_size(tree decl, tree init) {
  ...
  if (TREE_CODE(type) == ARRAY_TYPE && TYPE_SIZE(type) == NULL_TREE &&
    ...
    DECL_SIZE(decl) = size_binop(PLUS_EXPR, DECL_SIZE(decl), TYPE_SIZE(type));
    DECL_SIZE_UNIT(decl) =
...
tree start_decl(struct c_declarator *declarator, struct c_declspecs *declspecs,
  ...
  if (TREE_CODE(decl) == VAR_DECL && !initialized && TREE_PUBLIC(decl) &&
      !DECL_THREAD_LOCAL_P(decl) && !flag_no_common)
    DECL_COMMON(decl) = 1;
  ...
  if (declspecs->inline_p && !flag_gnu89_inline &&
  ...
  if (TREE_CODE(decl) == VAR_DECL && current_scope != file_scope &&
...
void c_maybe_initialize_eh(void) {
  ...
  eh_personality_libfunc = init_one_libfunc(
      USING_SJLJ_EXCEPTIONS ? "__gcc_personality_sj0" : "__gcc_personality_v0");
...
void finish_decl(tree decl, tree init, tree asmspec_tree) {
  ...
  if (TREE_CODE(type) == ARRAY_TYPE && TYPE_DOMAIN(type) == 0 &&
    ...
    bool do_default = (TREE_STATIC(decl)
                           /* Even if pedantic, an external linkage array
                              may have incomplete type at first.  */
                           ? pedantic && !TREE_PUBLIC(decl)
                           : !DECL_EXTERNAL(decl));
    int failure =
        complete_array_type(&TREE_TYPE(decl), DECL_INITIAL(decl), do_default);
  ...
  if (TREE_CODE(decl) == VAR_DECL) {
    ...
    if (DECL_SIZE(decl) == 0
        ...
        (TREE_STATIC(decl)
             ...
             ? (DECL_INITIAL(decl) != 0 || !DECL_FILE_SCOPE_P(decl))
  ...
  if (TREE_CODE(decl) == VAR_DECL || TREE_CODE(decl) == FUNCTION_DECL) {
    ...
    if (asmspec) {
      /* If this is not a static variable, issue a warning.
         ...
         this case.  */
      if (!DECL_FILE_SCOPE_P(decl) && TREE_CODE(decl) == VAR_DECL &&
          !C_DECL_REGISTER(decl) && !TREE_STATIC(decl))
        warning(0,
                "ignoring asm-specifier for non-static local "
                "variable %q+D",
                decl);
      else
        set_user_assembler_name(decl, asmspec);
...
tree build_compound_literal(tree type, tree init) {
  ...
  if (TREE_CODE(type) == ARRAY_TYPE && !COMPLETE_TYPE_P(type)) {
    int failure =
        complete_array_type(&TREE_TYPE(decl), DECL_INITIAL(decl), true);
    gcc_assert(!failure);
...
static void check_bitfield_type_and_width(tree *type, tree *width,
  ...
  if (TREE_CODE(*type) != INTEGER_TYPE && TREE_CODE(*type) != BOOLEAN_TYPE &&
  ...
  if (!in_system_header && type_mv != integer_type_node &&
...
static void warn_variable_length_array(const char *name, tree size) {
  ...
  if (!flag_isoc99 && pedantic && warn_vla != 0) {
    if (const_size) {
      ...
      if (name)
        pedwarn(input_location, OPT_Wvla,
                "ISO C90 forbids variable length array %qs", name);
      else
        pedwarn(input_location, OPT_Wvla,
    ...
    if (const_size) {
      if (name)
        ...
        warning(OPT_Wvla, "the size of array can %'t be evaluated");
    } else {
      if (name)
        warning(OPT_Wvla, "variable length array %qs is used", name);
      else
        warning(OPT_Wvla, "variable length array is used");
    }
  }
}
...
static tree grokdeclarator(const struct c_declarator *declarator,
                           ...
                           enum decl_context decl_context, bool initialized,
  ...
  if (declspecs->default_int_p && !in_system_header) {
    ...
    if ((warn_implicit_int || warn_return_type || flag_isoc99) && funcdef_flag)
  ...
  if (funcdef_flag &&
      (threadp || storage_class == csc_auto || storage_class == csc_register ||
       storage_class == csc_typedef)) {
    if (storage_class == csc_auto)
      pedwarn(input_location, (current_scope == file_scope) ? 0 : OPT_pedantic,
    ...
    if (decl_context == PARM && storage_class == csc_register)
      ...
      switch (decl_context) {
        case FIELD:
          error("storage class specified for structure field %qs", name);
          break;
        case PARM:
    ...
    if (storage_class == csc_extern && funcdef_flag)
      ...
      error(
          "function-scope %qs implicitly auto and declared "
          "%<__thread%>",
          name);
      threadp = false;
    }
  ...
  while (declarator && declarator->kind != cdk_id) {
    ...
    if (array_ptr_quals != TYPE_UNQUALIFIED || array_ptr_attrs != NULL_TREE ||
        array_parm_static) {
    ...
    switch (declarator->kind) {
      case cdk_attrs: {
        ...
        returned_attrs =
            decl_attributes(&type, chainon(returned_attrs, attrs), attr_flags);
        break;
      }
      case cdk_array: {
        ...
        if (size) {
          ...
          if (integer_zerop(size)) {
            ...
            itype =
                fold_build2(MINUS_EXPR, index_type, convert(index_type, size),
            ...
            /* If that overflowed, the array is too big.  ???
               ...
               complication.  */
            if (TREE_CODE(itype) == INTEGER_CST && TREE_OVERFLOW(itype)) {
          ...
          if (flexible_array_member && pedantic && !flag_isoc99 &&
              !in_system_header)
            pedwarn(input_location, OPT_pedantic,
      ...
      case cdk_function: {
           ...
           the FUNCTION_TYPE node itself.  */
        {
          ...
          for (link = arg_info->tags; link; link = TREE_CHAIN(link))
  ...
  *decl_attrs = chainon(returned_attrs, *decl_attrs);
  ...
  if (decl_context == TYPENAME) {
    /* Note that the grammar rejects storage classes in typenames
       and fields.  */
    gcc_assert(storage_class == csc_none && !threadp && !declspecs->inline_p);
    if (pedantic && TREE_CODE(type) == FUNCTION_TYPE && type_quals)
      pedwarn(input_location, OPT_pedantic,
  ...
  if (pedantic && decl_context == FIELD &&
    ...
    /* C99 6.7.2.1p8 */
    pedwarn(input_location, OPT_pedantic,
            "a member of a structure or union cannot have a variably modified "
            "type");
  }
  ...
  if (VOID_TYPE_P(type) && decl_context != PARM &&
      !((decl_context != FIELD && TREE_CODE(type) != FUNCTION_TYPE) &&
        (storage_class == csc_extern ||
         (current_scope == file_scope &&
          !(storage_class == csc_static || storage_class == csc_register))))) {
...

  {
    ...
    if (decl_context == PARM) {
      ...
      /* In C99, a function which is declared 'inline' with 'extern'
        ...
        DECL_EXTERNAL(decl) =
            ((storage_class == csc_extern) == flag_gnu89_inline);
      else
      ...
      if (extern_ref && current_scope != file_scope) {
        ...
        if (global_decl && global_decl != visible_decl &&
            TREE_CODE(global_decl) == VAR_DECL && !TREE_PUBLIC(global_decl))
          error(
              "variable previously declared %<static%> redeclared "
              "%<extern%>");
      }
...
static tree grokparms(struct c_arg_info *arg_info, bool funcdef_flag) {
  ...
  if (arg_types == error_mark_node)
    ...
    for (parm = arg_info->parms, typelt = arg_types, parmno = 1; parm;
      ...
      if (!COMPLETE_TYPE_P(type)) {
        if (funcdef_flag) {
          if (DECL_NAME(parm))
            error("parameter %u (%q+D) has incomplete type", parmno, parm);
          else
            error("%Jparameter %u has incomplete type", parm, parmno);

          TREE_VALUE(typelt) = error_mark_node;
          TREE_TYPE(parm) = error_mark_node;
        } else if (VOID_TYPE_P(type)) {
          if (DECL_NAME(parm))
            warning(0, "parameter %u (%q+D) has void type", parmno, parm);
          else
            warning(0, "%Jparameter %u has void type", parm, parmno);
        }
      }
...
struct c_arg_info *get_parm_info(bool ellipsis) {
  struct c_binding *b = current_scope->bindings;
  struct c_arg_info *arg_info = XOBNEW(&parser_obstack, struct c_arg_info);
  tree parms = 0;
  tree tags = 0;
  tree types = 0;
  tree others = 0;
  ...
  {
    if (TREE_THIS_VOLATILE(b->decl) || TREE_READONLY(b->decl) ||
      ...
      tag:
        ...
        if (TREE_CODE(decl) != UNION_TYPE || b->id != 0) {
          if (b->id)
            ...
            warning(0, "%<%s %E%> declared inside parameter list", keyword,
                    b->id);
          else
            /* The %s will be one of 'struct', 'union', or 'enum'.  */
...
tree grokfield(location_t loc, struct c_declarator *declarator,
               struct c_declspecs *declspecs, tree width, tree *decl_attrs) {
  tree value;
  ...
  if (declarator->kind == cdk_id && declarator->u.id == NULL_TREE &&
    ...
    if (type_ok && (flag_ms_extensions || !declspecs->typedef_p)) {
  ...
  value = grokdeclarator(declarator, declspecs, FIELD, false,
                         width ? &width : NULL, decl_attrs, DEPRECATED_NORMAL);
...
tree finish_struct(tree t, tree fieldlist, tree attributes) {
  ...
  for (x = fieldlist; x; x = TREE_CHAIN(x)) {
    ...
    if (TYPE_PACKED(t) &&
        (DECL_BIT_FIELD(x) || TYPE_ALIGN(TREE_TYPE(x)) > BITS_PER_UNIT))
      DECL_PACKED(x) = 1;
  ...
  for (x = C_TYPE_INCOMPLETE_VARS(TYPE_MAIN_VARIANT(t)); x; x = TREE_CHAIN(x)) {
...
tree start_enum(struct c_enum_contents *the_enum, tree name) {
  tree enumtype = 0;
...
tree build_enumerator(struct c_enum_contents *the_enum, tree name, tree value,
  ...
  the_enum->enum_next_value = build_binary_op(
...
int start_function(struct c_declspecs *declspecs,
                   struct c_declarator *declarator, tree attributes) {
  tree decl1, old_decl;
  ...
  if (!COMPLETE_OR_VOID_TYPE_P(TREE_TYPE(TREE_TYPE(decl1)))) {
    ...
    TREE_TYPE(decl1) =
        build_function_type(void_type_node, TYPE_ARG_TYPES(TREE_TYPE(decl1)));
  }

  if (warn_about_return_type)
    pedwarn_c99(input_location,
                flag_isoc99
                    ? 0
  ...
  if (warn_strict_prototypes && old_decl != error_mark_node &&
      TYPE_ARG_TYPES(TREE_TYPE(decl1)) == 0 && C_DECL_ISNT_PROTOTYPE(old_decl))
    warning(OPT_Wstrict_prototypes, "function declaration isn%'t a prototype");
  /* Optionally warn of any global def with no previous prototype.  */
  else if (warn_missing_prototypes && old_decl != error_mark_node &&
           TREE_PUBLIC(decl1) && !MAIN_NAME_P(DECL_NAME(decl1)) &&
           C_DECL_ISNT_PROTOTYPE(old_decl))
    warning(OPT_Wmissing_prototypes, "no previous prototype for %q+D", decl1);
  /* Optionally warn of any def with no previous prototype
     if the function has already been used.  */
  else if (warn_missing_prototypes && old_decl != 0 &&
           old_decl != error_mark_node && TREE_USED(old_decl) &&
  ...
  /* Optionally warn of any def with no previous declaration
     if the function has already been used.  */
  else if (warn_missing_declarations && old_decl != 0 &&
           old_decl != error_mark_node && TREE_USED(old_decl) &&
  ...
  if (warn_main && MAIN_NAME_P(DECL_NAME(decl1))) {
    if (TYPE_MAIN_VARIANT(TREE_TYPE(TREE_TYPE(decl1))) != integer_type_node)
      pedwarn(input_location, OPT_Wmain, "return type of %q+D is not %<int%>",
              decl1);
...
static void store_parm_decls_newstyle(tree fndecl,
  ...
  if (current_scope->bindings) {
    error(
        "%Jold-style parameter declarations in prototyped "
        "function definition",
        fndecl);
  ...
  /* Don't issue this warning for nested functions, and don't issue this
    ...
    warning(OPT_Wtraditional,
            "%Jtraditional C rejects ISO C style function definitions", fndecl);
  ...
  for (parm = parmids; parm; parm = TREE_CHAIN(parm)) {
    ...
    /* If no declaration found, default to int.  */
      ...
      if (flag_isoc99)
        ...
        warning(OPT_Wmissing_parameter_type, "type of %q+D defaults to %<int%>",
                decl);
    }
    ...
    for (parm = DECL_ARGUMENTS(fndecl),
      ...
      if (parm == 0 || type == 0 ||
        ...
        if (current_function_prototype_built_in)
          ...
          error("%Hprototype declaration", &current_function_prototype_locus);
        }
        break;
      }
      ...
      if (TREE_TYPE(parm) != error_mark_node &&
        ...
        if (TYPE_MAIN_VARIANT(TREE_TYPE(parm)) ==
          ...
          if (targetm.calls.promote_prototypes(
  ...
  for (b = current_scope->bindings; b; b = b->prev) {
    ...
    switch (TREE_CODE(decl)) {
      case VAR_DECL:
        if (TREE_STATIC(decl))
          error(
              "declaration of static variable %q+D in %<for%> loop "
              "initial declaration",
              decl);
        else if (DECL_EXTERNAL(decl))
          error(
              "declaration of %<extern%> variable %q+D in %<for%> loop "
              "initial declaration",
              decl);
        break;
      ...
      default:
        error(
            "declaration of non-variable %q+D in %<for%> loop "
            "initial declaration",
            decl);
    }
...
struct c_parm *build_c_parm(struct c_declspecs *specs, tree attrs,
                            struct c_declarator *declarator) {
...
struct c_declarator *build_id_declarator(tree ident) {
  struct c_declarator *ret = XOBNEW(&parser_obstack, struct c_declarator);
  ret->kind = cdk_id;
  ret->declarator = 0;
  ret->u.id = ident;
...
struct c_declarator *make_pointer_declarator(
    struct c_declspecs *type_quals_attrs, struct c_declarator *target) {
  tree attrs;
  int quals = 0;
...
struct c_declspecs *build_null_declspecs(void) {
...
struct c_declspecs *declspecs_add_qual(struct c_declspecs *specs, tree qual) {
  enum rid i;
  bool dupe = false;
...
struct c_declspecs *declspecs_add_type(struct c_declspecs *specs,
  ...
  if (specs->type || specs->typespec_word != cts_none || specs->long_p ||
      specs->short_p || specs->signed_p || specs->unsigned_p ||
      specs->complex_p)
    error("two or more data types in declaration specifiers");
...
struct c_declspecs *declspecs_add_attrs(struct c_declspecs *specs, tree attrs) {
...
struct c_declspecs *finish_declspecs(struct c_declspecs *specs) {
  ...
  if (specs->typespec_word == cts_none) {
    if (specs->saturating_p) {
      ...
      if (!targetm.fixed_point_supported_p())
        error("fixed-point types not supported for this target");
      specs->typespec_word = cts_fract;
    } else if (specs->long_p || specs->short_p || specs->signed_p ||
               specs->unsigned_p) {
      specs->typespec_word = cts_int;
    } else if (specs->complex_p) {
      specs->typespec_word = cts_double;
  ...
  switch (specs->typespec_word) {
    ...
    case cts_int:
      ...
      gcc_assert(!(specs->signed_p && specs->unsigned_p));
      if (specs->long_long_p)
        specs->type = (specs->unsigned_p ? long_long_unsigned_type_node
                                         : long_long_integer_type_node);
      else if (specs->long_p)
        specs->type = (specs->unsigned_p ? long_unsigned_type_node
                                         : long_integer_type_node);
      else if (specs->short_p)
        specs->type = (specs->unsigned_p ? short_unsigned_type_node
                                         : short_integer_type_node);
      else
        specs->type =
            (specs->unsigned_p ? unsigned_type_node : integer_type_node);
      if (specs->complex_p) {
        pedwarn(input_location, OPT_pedantic,
                "ISO C does not support complex integer types");
        specs->type = build_complex_type(specs->type);
    ...
    case cts_float:
      gcc_assert(!specs->long_p && !specs->short_p && !specs->signed_p &&
                 !specs->unsigned_p);
      specs->type =
          (specs->complex_p ? complex_float_type_node : float_type_node);
      break;
    case cts_double:
      gcc_assert(!specs->long_long_p && !specs->short_p && !specs->signed_p &&
                 !specs->unsigned_p);
      if (specs->long_p) {
        specs->type = (specs->complex_p ? complex_long_double_type_node
                                        : long_double_type_node);
      } else {
        specs->type =
            (specs->complex_p ? complex_double_type_node : double_type_node);
      }
      break;
    case cts_dfloat32:
    case cts_dfloat64:
    ...
    case cts_fract:
      ...
      if (!targetm.fixed_point_supported_p())
        ...
        if (specs->long_long_p)
          ...
          specs->type = specs->unsigned_p ? sat_unsigned_long_fract_type_node
                                          : sat_long_fract_type_node;
        else if (specs->short_p)
          specs->type = specs->unsigned_p ? sat_unsigned_short_fract_type_node
                                          : sat_short_fract_type_node;
        else
          specs->type = specs->unsigned_p ? sat_unsigned_fract_type_node
                                          : sat_fract_type_node;
      } else {
        if (specs->long_long_p)
          specs->type = specs->unsigned_p ? unsigned_long_long_fract_type_node
                                          : long_long_fract_type_node;
        else if (specs->long_p)
          specs->type = specs->unsigned_p ? unsigned_long_fract_type_node
                                          : long_fract_type_node;
        else if (specs->short_p)
          specs->type = specs->unsigned_p ? unsigned_short_fract_type_node
                                          : short_fract_type_node;
        else
          specs->type =
              specs->unsigned_p ? unsigned_fract_type_node : fract_type_node;
      }
      break;
    case cts_accum:
      gcc_assert(!specs->complex_p);
      if (!targetm.fixed_point_supported_p())
        ...
        if (specs->long_long_p)
          ...
          specs->type = specs->unsigned_p ? sat_unsigned_long_accum_type_node
                                          : sat_long_accum_type_node;
        else if (specs->short_p)
          specs->type = specs->unsigned_p ? sat_unsigned_short_accum_type_node
                                          : sat_short_accum_type_node;
        else
          specs->type = specs->unsigned_p ? sat_unsigned_accum_type_node
                                          : sat_accum_type_node;
      } else {
        if (specs->long_long_p)
          specs->type = specs->unsigned_p ? unsigned_long_long_accum_type_node
                                          : long_long_accum_type_node;
        else if (specs->long_p)
          specs->type = specs->unsigned_p ? unsigned_long_accum_type_node
                                          : long_accum_type_node;
        else if (specs->short_p)
          specs->type = specs->unsigned_p ? unsigned_short_accum_type_node
                                          : short_accum_type_node;
        else
          specs->type =
              specs->unsigned_p ? unsigned_accum_type_node : accum_type_node;
      }
      break;
    default:
      gcc_unreachable();
...
static void c_write_global_declarations_1(tree globals) {
  ...
  for (decl = globals; decl; decl = TREE_CHAIN(decl)) {
    ...
    if (TREE_CODE(decl) == FUNCTION_DECL && DECL_INITIAL(decl) == 0 &&
        DECL_EXTERNAL(decl) && !TREE_PUBLIC(decl) && C_DECL_USED(decl)) {
