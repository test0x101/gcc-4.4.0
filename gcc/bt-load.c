...
static btr_def add_btr_def(fibheap_t, basic_block, int, rtx, unsigned int, int,
                           btr_def_group *);
...
static void augment_live_range(bitmap, HARD_REG_SET *, basic_block, basic_block,
                               int);
...
static int basic_block_freq(const_basic_block bb) {
  return bb->frequency;
}
...
static int find_btr_reference(rtx *px, void *preg) {
  rtx x;
...
static int btr_referenced_p(rtx x, rtx *excludep) {
...
static int insn_sets_btr_p(const_rtx insn, int check_const, int *regno) {
  rtx set;

  if (NONJUMP_INSN_P(insn) && (set = single_set(insn))) {
    ...
    if (REG_P(dest) && TEST_HARD_REG_BIT(all_btrs, REGNO(dest))) {
...
static rtx *find_btr_use(rtx insn) {
...
static void find_btr_def_group(btr_def_group *all_btr_def_groups, btr_def def) {
  if (insn_sets_btr_p(def->insn, 1, NULL)) {
    ...
    for (this_group = *all_btr_def_groups; this_group != NULL;
...
/* Create a new target register definition structure, for a definition in
   ...
   the new definition.  */
static btr_def add_btr_def(fibheap_t all_btr_defs, basic_block bb,
                           int insn_luid, rtx insn, unsigned int dest_reg,
  ...
  if (dump_file)
    fprintf(dump_file,
            "Found target reg definition: sets %u { bb %d, insn %d }%s "
            "priority %d\n",
...
static btr_user new_btr_user(basic_block bb, int insn_luid, rtx insn) {
  ...
  if (dump_file) {
    fprintf(dump_file, "Uses target reg: { bb %d, insn %d }", bb->index,
            INSN_UID(insn));
...
static void dump_hard_reg_set(HARD_REG_SET s) {
  int reg;
...
static void dump_btrs_live(int bb) {
...
static void note_btr_set(rtx dest, const_rtx set ATTRIBUTE_UNUSED, void *data) {
...
static void compute_defs_uses_and_gen(fibheap_t all_btr_defs,
                                      btr_def *def_array, btr_user *use_array,
                                      sbitmap *btr_defset, sbitmap *bb_gen,
    ...
    for (insn = BB_HEAD(bb), last = NEXT_INSN(BB_END(bb)); insn != last;
      ...
      if (INSN_P(insn)) {
        ...
        if (insn_sets_btr_p(insn, 0, &regno)) {
          btr_def def =
              add_btr_def(all_btr_defs, bb, insn_luid, insn, regno,
        ...
        /* Check for the blockage emitted by expand_nl_goto_receiver.  */
          ...
          for (user = info.users_this_bb; user != NULL; user = user->next)
            if (user->use)
              user->other_use_this_block = 1;
          ...
          if (btr_referenced_p(PATTERN(insn), NULL)) {
            ...
            if (user->use)
              SET_HARD_REG_BIT(info.btrs_live_in_block, REGNO(user->use));
            else {
              int reg;
              for (reg = first_btr; reg <= last_btr; reg++)
                if (TEST_HARD_REG_BIT(all_btrs, reg) &&
                    refers_to_regno_p(reg, reg + 1, user->insn, NULL)) {
                  note_other_use_this_block(reg, info.users_this_bb);
                  SET_HARD_REG_BIT(info.btrs_live_in_block, reg);
                }
            ...
            if (GET_CODE(pat) == PARALLEL)
              for (i = XVECLEN(pat, 0) - 1; i >= 0; i--)
                if (GET_CODE(XVECEXP(pat, 0, i)) == RETURN) {
                  COMPL_HARD_REG_SET(call_saved, call_used_reg_set);
...
static void compute_kill(sbitmap *bb_kill, sbitmap *btr_defset,
                         HARD_REG_SET *btrs_written) {
  int i;
  int regno;
  ...
  for (i = NUM_FIXED_BLOCKS; i < n_basic_blocks; i++) {
    for (regno = first_btr; regno <= last_btr; regno++)
      if (TEST_HARD_REG_BIT(all_btrs, regno) &&
        ...
        sbitmap_a_or_b(bb_kill[i], bb_kill[i], btr_defset[regno - first_btr]);
  }
}

static void compute_out(sbitmap *bb_out, sbitmap *bb_gen, sbitmap *bb_kill,
                        int max_uid) {
  ...
  while (changed) {
    ...
    for (i = NUM_FIXED_BLOCKS; i < n_basic_blocks; i++) {
      ...
      changed |=
          sbitmap_union_of_diff_cg(bb_out[i], bb_gen[i], bb_in, bb_kill[i]);
    }
  }
  sbitmap_free(bb_in);
}

static void link_btr_uses(btr_def *def_array, btr_user *use_array,
                          sbitmap *bb_out, sbitmap *btr_defset, int max_uid) {
  int i;
    ...
    for (insn = BB_HEAD(bb), last = NEXT_INSN(BB_END(bb)); insn != last;
      ...
      if (INSN_P(insn)) {
        ...
        if (user != NULL) {
          ...
          if (user->use)
            sbitmap_a_and_b(reaching_defs_of_reg, reaching_defs,
            ...
            for (reg = first_btr; reg <= last_btr; reg++)
              if (TEST_HARD_REG_BIT(all_btrs, reg) &&
                  refers_to_regno_p(reg, reg + 1, user->insn, NULL))
                sbitmap_a_or_b_and_c(reaching_defs_of_reg, reaching_defs_of_reg,
                                     reaching_defs,
          ...
          EXECUTE_IF_SET_IN_SBITMAP(reaching_defs_of_reg, 0, uid, sbi) {
            ...
            if (dump_file)
              fprintf(dump_file, "Def in insn %d reaches use in insn %d\n", uid,
                      insn_uid);

            user->n_reaching_defs++;
            if (!user->use)
              def->has_ambiguous_use = 1;
            if (user->first_reaching_def !=
                -1) { /* There is more than one reaching def.  This is
                         a rare case, so just give up on this def/use
                         web when it occurs.  */
              def->has_ambiguous_use = 1;
              def_array[user->first_reaching_def]->has_ambiguous_use = 1;
              if (dump_file)
                fprintf(dump_file, "(use %d has multiple reaching defs)\n",
                        insn_uid);
            } else
...
static void build_btr_def_use_webs(fibheap_t all_btr_defs) {
  const int max_uid = get_max_uid();
  btr_def *def_array = XCNEWVEC(btr_def, max_uid);
  btr_user *use_array = XCNEWVEC(btr_user, max_uid);
  sbitmap *btr_defset =
      sbitmap_vector_alloc((last_btr - first_btr) + 1, max_uid);
...
static int block_at_edge_of_live_range_p(int bb, btr_def def) {
...
static void clear_btr_from_live_range(btr_def def) {
  ...
  EXECUTE_IF_SET_IN_BITMAP(def->live_range, 0, bb, bi) {
    if ((!def->other_btr_uses_before_def && !def->other_btr_uses_after_use) ||
...
static void add_btr_to_live_range(btr_def def, int own_end) {
  unsigned bb;
  bitmap_iterator bi;
...
static void augment_live_range(bitmap live_range,
  ...
  while (tos != worklist) {
    ...
    if (!bitmap_bit_p(live_range, bb->index)) {
      ...
      IOR_HARD_REG_SET(*btrs_live_in_range, btrs_live[bb->index]);
      /* A previous btr migration could have caused a register to be
         live just at the end of a block which we need in full.  */
      IOR_HARD_REG_SET(*btrs_live_in_range, btrs_live_at_end[bb->index]);
      if (dump_file) {
        fprintf(dump_file, "Adding block %d to live range\n", bb->index);
        fprintf(dump_file, "Now live btrs are ");
        dump_hard_reg_set(*btrs_live_in_range);
        fprintf(dump_file, "\n");
      }
...
/*  Return the most desirable target register that is not in
    the set USED_BTRS.  */
static int choose_btr(HARD_REG_SET used_btrs) {
  int i;
...
static void btr_def_live_range(btr_def def, HARD_REG_SET *btrs_live_in_range) {
  if (!def->live_range) {
    btr_user user;
    ...
    COPY_HARD_REG_SET(
        *btrs_live_in_range,
        (flag_btr_bb_exclusive ? btrs_live : btrs_live_at_end)[def->bb->index]);

    for (user = def->uses; user != NULL; user = user->next)
      augment_live_range(
          def->live_range, btrs_live_in_range, def->bb, user->bb,
          (flag_btr_bb_exclusive || user->insn != BB_END(def->bb) ||
           !JUMP_P(user->insn)));
  } else {
    ...
    */
    ...
    EXECUTE_IF_SET_IN_BITMAP(def->live_range, 0, bb, bi) {
      IOR_HARD_REG_SET(*btrs_live_in_range,
                       (def_bb == bb ? btrs_live_at_end : btrs_live)[bb]);
    }
  }
  if (!def->other_btr_uses_before_def && !def->other_btr_uses_after_use)
...
static void combine_btr_defs(btr_def def, HARD_REG_SET *btrs_live_in_range) {
  btr_def other_def;

  for (other_def = def->group->members; other_def != NULL;
       other_def = other_def->next_this_group) {
    if (other_def != def && other_def->uses != NULL &&
      ...
      for (user = other_def->uses; user != NULL; user = user->next)
        augment_live_range(
            combined_live_range, &combined_btrs_live, def->bb, user->bb,
            (flag_btr_bb_exclusive || user->insn != BB_END(def->bb) ||
             !JUMP_P(user->insn)));
      ...
      if (btr != -1) {
        /* We can combine them.  */
        if (dump_file)
          fprintf(dump_file, "Combining def in insn %d with def in insn %d\n",
        ...
        for (user = def->uses; user != NULL; user = user->next)
          remove_note(user->insn,
                      find_regno_note(user->insn, REG_DEAD, REGNO(user->use)));
...
static void move_btr_def(basic_block new_def_bb, int btr, btr_def def,
                         bitmap live_range, HARD_REG_SET *btrs_live_in_range) {
...
static int can_move_up(const_basic_block bb, const_rtx insn, int n_insns) {
...
static int migrate_btr_def(btr_def def, int min_cost) {
  bitmap live_range;
  HARD_REG_SET btrs_live_in_range;
  ...
  */
  ...
  for (user = def->uses; user != NULL; user = user->next) {
    if (user->bb == def->bb && user->luid > def->luid &&
  ...
  for (attempt = get_immediate_dominator(CDI_DOMINATORS, def->bb);
       !give_up && attempt && attempt != ENTRY_BLOCK_PTR &&
       def->cost >= min_cost;
...
static void migrate_btr_defs(enum reg_class btr_class, int allow_callee_save) {
  ...
  while (!fibheap_empty(all_btr_defs)) {
    ...
    if (migrate_btr_def(def, min_cost)) {
      ...
      if (dump_file) {
        fprintf(dump_file, "Putting insn %d back on queue with priority %d\n",
...
static void branch_target_load_optimize(bool after_prologue_epilogue_gen) {
  ...
  if (klass != NO_REGS) {
    ...
    migrate_btr_defs(klass, (targetm.branch_target_register_callee_saved(
...
static bool gate_handle_branch_target_load_optimize1(void) {
  return flag_branch_target_load_optimize;
}

static unsigned int rest_of_handle_branch_target_load_optimize1(void) {
...
static bool gate_handle_branch_target_load_optimize2(void) {
  return (optimize > 0 && flag_branch_target_load_optimize2);
}

static unsigned int rest_of_handle_branch_target_load_optimize2(void) {
  static int warned = 0;

  /* Leave this a warning for now so that it is possible to experiment
     with running this pass twice.  In 3.6, we should either make this
     an error, or use separate dump files.  */
  if (flag_branch_target_load_optimize && flag_branch_target_load_optimize2 &&
      !warned) {
    warning(0,
