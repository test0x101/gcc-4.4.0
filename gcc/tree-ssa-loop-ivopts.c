...
static inline unsigned n_iv_uses(struct ivopts_data *data) {
  return VEC_length(iv_use_p, data->iv_uses);
}

/* Ith use recorded in DATA.  */

static inline struct iv_use *iv_use(struct ivopts_data *data, unsigned i) {
  return VEC_index(iv_use_p, data->iv_uses, i);
}

/* Number of candidates recorded in DATA.  */

static inline unsigned n_iv_cands(struct ivopts_data *data) {
  return VEC_length(iv_cand_p, data->iv_candidates);
}

/* Ith candidate recorded in DATA.  */

static inline struct iv_cand *iv_cand(struct ivopts_data *data, unsigned i) {
  return VEC_index(iv_cand_p, data->iv_candidates, i);
}

/* The single loop exit if it dominates the latch, NULL otherwise.  */

edge single_dom_exit(struct loop *loop) {
...
void dump_iv(FILE *file, struct iv *iv) {
  if (iv->ssa_name) {
...
void dump_uses(FILE *file, struct ivopts_data *data) {
  unsigned i;
  struct iv_use *use;
...
static inline struct version_info *ver_info(struct ivopts_data *data,
                                            unsigned ver) {
...
static inline struct version_info *name_info(struct ivopts_data *data,
                                             tree name) {
...
static bool stmt_after_ip_normal_pos(struct loop *loop, gimple stmt) {
...
static bool stmt_after_ip_original_pos(struct iv_cand *cand, gimple stmt) {
...
static bool stmt_after_increment(struct loop *loop, struct iv_cand *cand,
                                 gimple stmt) {
  switch (cand->pos) {
    case IP_END:
      return false;
...
static bool abnormal_ssa_name_p(tree exp) {
  if (!exp)
    return false;
...
static bool idx_contains_abnormal_ssa_name_p(tree base, tree *index,
...
bool contains_abnormal_ssa_name_p(tree expr) {
  ...
  if (code == INTEGER_CST || is_gimple_min_invariant(expr))
    return false;

  if (code == ADDR_EXPR)
    return !for_each_index(&TREE_OPERAND(expr, 0),
                           idx_contains_abnormal_ssa_name_p, NULL);
...
static tree niter_for_exit(struct ivopts_data *data, edge exit) {
  ...
  if (!slot) {
    ...
    if (number_of_iterations_exit(data->current_loop, exit, &desc, true) &&
...
static tree niter_for_single_dom_exit(struct ivopts_data *data) {
...
static tree determine_base_object(tree expr) {
  ...
  switch (code) {
    ...
    case ADDR_EXPR:
      ...
      return fold_convert(ptr_type_node, build_fold_addr_expr(base));
...
static struct iv *alloc_iv(tree base, tree step) {
...
static void set_iv(struct ivopts_data *data, tree iv, tree base, tree step) {
...
static struct iv *get_iv(struct ivopts_data *data, tree var) {
  basic_block bb;
  ...
  if (!name_info(data, var)->iv) {
    ...
    if (!bb || !flow_bb_inside_loop_p(data->current_loop, bb))
...
static tree determine_biv_step(gimple phi) {
...
static bool find_givs_in_stmt_scev(struct ivopts_data *data, gimple stmt,
                                   affine_iv *iv) {
  tree lhs;
  struct loop *loop = data->current_loop;
...
static struct iv_use *record_use(struct ivopts_data *data, tree *use_p,
                                 struct iv *iv, gimple stmt,
...
static void record_invariant(struct ivopts_data *data, tree op,
                             bool nonlinear_use) {
  basic_block bb;
...
static struct iv_use *find_interesting_uses_op(struct ivopts_data *data,
                                               tree op) {
  struct iv *iv;
  struct iv *civ;
  gimple stmt;
...
static bool extract_cond_operands(struct ivopts_data *data, gimple stmt,
...
bool expr_invariant_in_loop_p(struct loop *loop, tree expr) {
  basic_block def_bb;
  unsigned i, len;
...
bool stmt_invariant_in_loop_p(struct loop *loop, gimple stmt) {
  unsigned i;
  tree lhs;
...
/* Cumulates the steps of indices into DATA and replaces their values with the
   ...
   determined. Callback for for_each_index.  */
...
static bool idx_find_step(tree base, tree *idx, void *data) {
  ...
  *idx = iv->base;
  ...
  iv_base = iv->base;
  iv_step = iv->step;
  if (!convert_affine_scev(dta->ivopts_data->current_loop, sizetype, &iv_base,
                           &iv_step, dta->stmt, false)) {
...
static bool constant_multiple_of(tree top, tree bot, double_int *mul) {
  tree mby;
  enum tree_code code;
...
static bool may_be_nonaddressable_p(tree expr) {
...
static void find_interesting_uses_address(struct ivopts_data *data, gimple stmt,
                                          tree *op_p) {
  ...
  if (TREE_CODE(base) == TARGET_MEM_REF) {
    ...
    if (TMR_BASE(base) && TREE_CODE(TMR_BASE(base)) == SSA_NAME) {
    ...
    if (TMR_INDEX(base) && TREE_CODE(TMR_INDEX(base)) == SSA_NAME) {
...
static void find_interesting_uses_stmt(struct ivopts_data *data, gimple stmt) {
  ...
  if (is_gimple_assign(stmt)) {
    ...
    if (get_gimple_rhs_class(code) == GIMPLE_SINGLE_RHS &&
        (REFERENCE_CLASS_P(*rhs) || is_gimple_val(*rhs))) {
...
static void find_interesting_uses(struct ivopts_data *data) {
  basic_block bb;
  gimple_stmt_iterator bsi;
...
static tree strip_offset_1(tree expr, bool inside_addr, bool top_compref,
  ...
  *offset = 0;
  ...
  switch (code) {
    case INTEGER_CST:
      if (!cst_and_fits_in_hwi(expr) || integer_zerop(expr))
        return orig_expr;
    ...
    case MINUS_EXPR:
      ...
      *offset = (code == MINUS_EXPR ? off0 - off1 : off0 + off1);
      if (op0 == TREE_OPERAND(expr, 0) && op1 == TREE_OPERAND(expr, 1))
        return orig_expr;
    ...
    case ARRAY_RANGE_REF:
      ...
      *offset = off1 * st;
      ...
      if (top_compref && integer_zerop(op1)) {
    ...
    case COMPONENT_REF:
      ...
      if (top_compref && cst_and_fits_in_hwi(tmp)) {
        /* Strip the component reference completely.  */
        op0 = TREE_OPERAND(expr, 0);
        op0 = strip_offset_1(op0, inside_addr, top_compref, &off0);
  ...
  *offset += off0;
  ...
  if (op0 == TREE_OPERAND(expr, 0) && (!op1 || op1 == TREE_OPERAND(expr, 1)))
    return orig_expr;
...
static tree strip_offset(tree expr, unsigned HOST_WIDE_INT *offset) {
...
static tree find_depends(tree *expr_p, int *ws ATTRIBUTE_UNUSED, void *data) {
...
static struct iv_cand *add_candidate_1(struct ivopts_data *data, tree base,
...
static bool allow_ip_end_pos_p(struct loop *loop) {
...
static void add_candidate(struct ivopts_data *data, tree base, tree step,
                          bool important, struct iv_use *use) {
  if (ip_normal_pos(data->current_loop))
    add_candidate_1(data, base, step, important, IP_NORMAL, use, NULL);
  if (ip_end_pos(data->current_loop) && allow_ip_end_pos_p(data->current_loop))
...
static void add_standard_iv_candidates_for_size(struct ivopts_data *data,
                                                unsigned int size) {
  tree type = lang_hooks.types.type_for_size(size, true);
  add_candidate(data, build_int_cst(type, 0), build_int_cst(type, 1), true,
                NULL);
}
...
static void add_old_iv_candidates(struct ivopts_data *data, struct iv *iv) {
  gimple phi;
  tree def;
  struct iv_cand *cand;
  ...
  if (POINTER_TYPE_P(TREE_TYPE(iv->base)))
    ...
    add_candidate(data, build_int_cst(TREE_TYPE(iv->base), 0), iv->step, true,
                  NULL);
  ...
  if (gimple_code(phi) == GIMPLE_PHI) {
    ...
    cand = add_candidate_1(data, iv->base, iv->step, true, IP_ORIGINAL, NULL,
...
static void add_iv_value_candidates(struct ivopts_data *data, struct iv *iv,
                                    struct iv_use *use) {
  unsigned HOST_WIDE_INT offset;
  tree base;
  tree basetype;
...
static void add_derived_ivs_candidates(struct ivopts_data *data) {
  unsigned i;
...
static void record_important_candidates(struct ivopts_data *data) {
  ...
  data->consider_all_candidates =
      (n_iv_cands(data) <= CONSIDER_ALL_CANDIDATES_BOUND);
...
static comp_cost new_cost(unsigned runtime, unsigned complexity) {
  comp_cost cost;
...
static comp_cost add_costs(comp_cost cost1, comp_cost cost2) {
...
static comp_cost sub_costs(comp_cost cost1, comp_cost cost2) {
  cost1.cost -= cost2.cost;
  cost1.complexity -= cost2.complexity;
...
static int compare_costs(comp_cost cost1, comp_cost cost2) {
...
static bool infinite_cost_p(comp_cost cost) {
...
static void set_use_iv_cost(struct ivopts_data *data, struct iv_use *use,
                            struct iv_cand *cand, comp_cost cost,
...
static unsigned seq_cost(rtx seq, bool speed) {
  unsigned cost = 0;
  rtx set;
...
static tree var_at_stmt(struct loop *loop, struct iv_cand *cand, gimple stmt) {
...
static bool get_computation_aff(struct loop *loop, struct iv_use *use,
                                struct iv_cand *cand, gimple at,
...
static tree get_computation_at(struct loop *loop, struct iv_use *use,
                               struct iv_cand *cand, gimple at) {
  aff_tree aff;
...
static tree get_computation(struct loop *loop, struct iv_use *use,
                            struct iv_cand *cand) {
...
static unsigned add_cost(enum machine_mode mode, bool speed) {
  ...
  start_sequence();
  force_operand(
      gen_rtx_fmt_ee(PLUS, mode, gen_raw_REG(mode, LAST_VIRTUAL_REGISTER + 1),
...
static hashval_t mbc_entry_hash(const void *entry) {
...
static int mbc_entry_eq(const void *entry1, const void *entry2) {
  const struct mbc_entry *e1 = (const struct mbc_entry *)entry1;
  const struct mbc_entry *e2 = (const struct mbc_entry *)entry2;

  return (e1->mode == e2->mode && e1->cst == e2->cst);
}

/* Returns cost of multiplication by constant CST in MODE.  */

unsigned multiply_by_cost(HOST_WIDE_INT cst, enum machine_mode mode,
                          bool speed) {
  static htab_t costs;
...
static comp_cost get_address_cost(bool symbol_present, bool var_present,
                                  unsigned HOST_WIDE_INT offset,
                                  HOST_WIDE_INT ratio,
                                  enum machine_mode mem_mode, bool speed) {
  static bool initialized[MAX_MACHINE_MODE];
  static HOST_WIDE_INT rat[MAX_MACHINE_MODE], off[MAX_MACHINE_MODE];
  static HOST_WIDE_INT min_offset[MAX_MACHINE_MODE],
      max_offset[MAX_MACHINE_MODE];
  ...
  if (!initialized[mem_mode]) {
    ...
    for (i = 0; i < 16; i++) {
      ...
      if (sym_p) {
        ...
        if (off_p)
          base =
              gen_rtx_fmt_e(CONST, Pmode,
                            gen_rtx_fmt_ee(PLUS, Pmode, base,
                                           gen_int_mode(off[mem_mode], Pmode)));
      } else if (off_p)
  ...
  cost = 0;
  offset_p = (s_offset != 0 && min_offset[mem_mode] <= s_offset &&
              s_offset <= max_offset[mem_mode]);
  ratio_p = (ratio != 1 && multiplier_allowed_in_address_p(ratio, mem_mode));
...
static comp_cost force_expr_to_var_cost(tree expr, bool speed) {
  ...
  if (!costs_initialized) {
    ...
    for (i = 0; i < 2; i++) {
      integer_cost[i] =
          computation_cost(build_int_cst(integer_type_node, 2000), i);

      symbol_cost[i] = computation_cost(addr, i) + 1;

      address_cost[i] = computation_cost(build2(POINTER_PLUS_EXPR, type, addr,
                                                build_int_cst(sizetype, 2000)),
                                         i) +
                        1;
      if (dump_file && (dump_flags & TDF_DETAILS)) {
  ...
  if (is_gimple_min_invariant(expr)) {
    ...
    if (TREE_CODE(expr) == ADDR_EXPR) {
      ...
      if (TREE_CODE(obj) == VAR_DECL || TREE_CODE(obj) == PARM_DECL ||
...
static comp_cost force_var_cost(struct ivopts_data *data, tree expr,
                                bitmap *depends_on) {
  if (depends_on) {
    fd_ivopts_data = data;
...
static comp_cost split_address_cost(struct ivopts_data *data, tree addr,
  ...
  if (toffset != 0 || bitpos % BITS_PER_UNIT != 0 ||
  ...
  *offset += bitpos / BITS_PER_UNIT;
  if (TREE_STATIC(core) || DECL_EXTERNAL(core)) {
    *symbol_present = true;
    *var_present = false;
    return zero_cost;
  }
...
static comp_cost ptr_difference_cost(struct ivopts_data *data, tree e1, tree e2,
                                     bool *symbol_present, bool *var_present,
                                     unsigned HOST_WIDE_INT *offset,
                                     bitmap *depends_on) {
  ...
  if (integer_zerop(e2))
    return split_address_cost(data, TREE_OPERAND(e1, 0), symbol_present,
                              var_present, offset, depends_on);
...
static comp_cost difference_cost(struct ivopts_data *data, tree e1, tree e2,
                                 bool *symbol_present, bool *var_present,
                                 unsigned HOST_WIDE_INT *offset,
                                 bitmap *depends_on) {
  comp_cost cost;
...
static comp_cost get_computation_cost_at(struct ivopts_data *data,
                                         struct iv_use *use,
                                         struct iv_cand *cand, bool address_p,
                                         bitmap *depends_on, gimple at) {
  ...
  *depends_on = NULL;
  ...
  if (address_p) {
    /* Do not try to express address of an object with computation based
       ...
       anyway.  */
    if (use->iv->base_object && cand->iv->base_object &&
  ...
  if (cst_and_fits_in_hwi(cbase)) {
    ...
    cost = difference_cost(data, ubase, cbase, &symbol_present, &var_present,
                           &offset, depends_on);
  } else {
    cost = force_var_cost(data, cbase, depends_on);
    cost.cost += add_cost(TYPE_MODE(ctype), data->speed);
    cost = add_costs(cost, difference_cost(data, ubase, build_int_cst(utype, 0),
  ...
  if (address_p)
    return add_costs(
        cost, get_address_cost(symbol_present, var_present, offset, ratio,
...
static void cand_value_at(struct loop *loop, struct iv_cand *cand, gimple at,
                          tree niter, aff_tree *val) {
...
static tree iv_period(struct iv *iv) {
  ...
  period = build_low_bits_mask(
      type, (TYPE_PRECISION(type) - tree_low_cst(pow2div, 1)));
...
static bool may_eliminate_iv(struct ivopts_data *data, struct iv_use *use,
                             struct iv_cand *cand, tree *bound) {
  basic_block ex_bb;
  edge exit;
  tree nit, period;
  struct loop *loop = data->current_loop;
  ...
  ex_bb = gimple_bb(use->stmt);
  if (use->stmt != last_stmt(ex_bb) || gimple_code(use->stmt) != GIMPLE_COND ||
...
static bool determine_use_iv_cost_condition(struct ivopts_data *data,
  ...
  express_cost =
      get_computation_cost(data, use, cand, false, &depends_on_express);
  fd_ivopts_data = data;
...
static bool determine_use_iv_cost(struct ivopts_data *data, struct iv_use *use,
                                  struct iv_cand *cand) {
  switch (use->type) {
    case USE_NONLINEAR_EXPR:
...
static void determine_use_iv_costs(struct ivopts_data *data) {
  unsigned i, j;
  struct iv_use *use;
  struct iv_cand *cand;
  ...
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    ...
    for (i = 0; i < n_iv_uses(data); i++) {
      ...
      for (j = 0; j < use->n_map_members; j++) {
        if (!use->cost_map[j].cand || infinite_cost_p(use->cost_map[j].cost))
          continue;

        fprintf(dump_file, "  %d\t%d\t%d\t", use->cost_map[j].cand->id,
                use->cost_map[j].cost.cost, use->cost_map[j].cost.complexity);
        if (use->cost_map[j].depends_on)
          bitmap_print(dump_file, use->cost_map[j].depends_on, "", "");
        fprintf(dump_file, "\n");
      }
...
static void determine_iv_cost(struct ivopts_data *data, struct iv_cand *cand) {
  ...
  if (cand->pos == IP_END && empty_block_p(ip_end_pos(data->current_loop)))
    cost++;
...
static unsigned ivopts_global_cost_for_size(struct ivopts_data *data,
                                            unsigned size) {
...
static void determine_set_costs(struct ivopts_data *data) {
  ...
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    ...
    for (j = 0; j <= 2 * target_avail_regs; j++)
      fprintf(dump_file, "  %d\t%d\n", j, ivopts_global_cost_for_size(data, j));
    fprintf(dump_file, "\n");
  }
}
...
static bool cheaper_cost_pair(struct cost_pair *a, struct cost_pair *b) {
  int cmp;
...
static void iv_ca_recount_cost(struct ivopts_data *data, struct iv_ca *ivs) {
...
static void iv_ca_set_remove_invariants(struct iv_ca *ivs, bitmap invs) {
  bitmap_iterator bi;
  unsigned iid;
...
static void iv_ca_set_no_cp(struct ivopts_data *data, struct iv_ca *ivs,
                            struct iv_use *use) {
...
static void iv_ca_set_add_invariants(struct iv_ca *ivs, bitmap invs) {
  bitmap_iterator bi;
  unsigned iid;
...
static void iv_ca_set_cp(struct ivopts_data *data, struct iv_ca *ivs,
...
static void iv_ca_add_use(struct ivopts_data *data, struct iv_ca *ivs,
                          struct iv_use *use) {
...
static comp_cost iv_ca_cost(struct iv_ca *ivs) {
...
static bool iv_ca_has_deps(struct iv_ca *ivs, struct cost_pair *cp) {
  unsigned i;
  bitmap_iterator bi;
...
static struct cost_pair *iv_ca_cand_for_use(struct iv_ca *ivs,
                                            struct iv_use *use) {
...
static void iv_ca_delta_commit(struct ivopts_data *data, struct iv_ca *ivs,
...
static bool iv_ca_cand_used_p(struct iv_ca *ivs, struct iv_cand *cand) {
  return ivs->n_cand_uses[cand->id] > 0;
}

/* Returns number of induction variable candidates in the set IVS.  */

static unsigned iv_ca_n_cands(struct iv_ca *ivs) {
  return ivs->n_cands;
}
...
static struct iv_ca *iv_ca_new(struct ivopts_data *data) {
...
static void iv_ca_free(struct iv_ca **ivs) {
...
static void iv_ca_dump(struct ivopts_data *data, FILE *file,
                       struct iv_ca *ivs) {
...
static comp_cost iv_ca_extend(struct ivopts_data *data, struct iv_ca *ivs,
...
static comp_cost iv_ca_narrow(struct ivopts_data *data, struct iv_ca *ivs,
                              struct iv_cand *cand,
...
static comp_cost iv_ca_prune(struct ivopts_data *data, struct iv_ca *ivs,
...
static bool try_add_cand_for(struct ivopts_data *data, struct iv_ca *ivs,
                             struct iv_use *use) {
  ...
  if (infinite_cost_p(best_cost)) {
    for (i = 0; i < use->n_map_members; i++) {
      ...
      iv_ca_set_no_cp(data, ivs, use);
      act_delta =
          iv_ca_delta_add(use, iv_ca_cand_for_use(ivs, use), cp, act_delta);
...
static struct iv_ca *get_initial_solution(struct ivopts_data *data) {
...
static bool try_improve_iv_set(struct ivopts_data *data, struct iv_ca *ivs) {
  unsigned i, n_ivs;
...
static void create_new_iv(struct ivopts_data *data, struct iv_cand *cand) {
  ...
  create_iv(base, unshare_expr(cand->iv->step), cand->var_before,
            data->current_loop, &incr_pos, after, &cand->var_before,
            &cand->var_after);
}

/* Creates new induction variables described in SET.  */

static void create_new_ivs(struct ivopts_data *data, struct iv_ca *set) {
  unsigned i;
  struct iv_cand *cand;
  bitmap_iterator bi;
...
static gimple get_phi_with_result(basic_block bb, tree result) {
...
static void remove_statement(gimple stmt, bool including_defined_name) {
  if (gimple_code(stmt) == GIMPLE_PHI) {
    gimple bb_phi =
        get_phi_with_result(gimple_bb(stmt), gimple_phi_result(stmt));
...
static void rewrite_use_nonlinear_expr(struct ivopts_data *data,
  ...
  if (cand->pos == IP_ORIGINAL && cand->incremented_at == use->stmt) {
    ...
    if (old_code == PLUS_EXPR || old_code == MINUS_EXPR ||
    ...
    if (op && (TREE_CODE(op) == INTEGER_CST || operand_equal_p(op, step, 0)))
      return;
    ...
    comp =
        fold_convert(utype, build2(incr_code, ctype, op, unshare_expr(step)));
  } else {
  ...
  op = force_gimple_operand_gsi(&bsi, comp, false, SSA_NAME_VAR(tgt), true,
                                GSI_SAME_STMT);
...
static bool idx_remove_ssa_names(tree base, tree *idx,
                                 void *data ATTRIBUTE_UNUSED) {
  tree *op;
...
static tree unshare_and_remove_ssa_names(tree ref) {
...
static tree get_ref_tag(tree ref, tree orig) {
...
static void copy_ref_info(tree new_ref, tree old_ref) {
...
static void rewrite_use_address(struct ivopts_data *data, struct iv_use *use,
                                struct iv_cand *cand) {
  aff_tree aff;
...
static void rewrite_use_compare(struct ivopts_data *data, struct iv_use *use,
                                struct iv_cand *cand) {
  ...
  if (bound) {
    ...
    if (stmts)
      gsi_insert_seq_on_edge_immediate(loop_preheader_edge(data->current_loop),
                                       stmts);
...
static void rewrite_use(struct ivopts_data *data, struct iv_use *use,
                        struct iv_cand *cand) {
...
static void remove_unused_ivs(struct ivopts_data *data) {
  unsigned j;
  bitmap_iterator bi;
  ...
  EXECUTE_IF_SET_IN_BITMAP(data->relevant, 0, j, bi) {
    ...
    if (info->iv && !integer_zerop(info->iv->step) && !info->inv_id &&
        !info->iv->have_use_for && !info->preserve_biv)
...
static bool tree_ssa_iv_optimize_loop(struct ivopts_data *data,
                                      struct loop *loop) {
  bool changed = false;
  struct iv_ca *iv_ca;
  edge exit;
