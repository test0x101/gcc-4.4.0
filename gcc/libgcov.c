...
static int create_file_directory(char *filename) {
  char *s;
...
static void gcov_exit(void) {
  ...
  for (gi_ptr = gcov_list; gi_ptr; gi_ptr = gi_ptr->next) {
    ...
    if (tag) {
      ...
      for (f_ix = 0; f_ix < gi_ptr->n_functions; f_ix++) {
        fi_ptr = (const struct gcov_fn_info *)((const char *)gi_ptr->functions +
                                               f_ix * fi_stride);
        tag = gcov_read_unsigned();
        length = gcov_read_unsigned();

        /* Check function.  */
        if (tag != GCOV_TAG_FUNCTION || length != GCOV_TAG_FUNCTION_LENGTH ||
  ...
  read_error:;
    fprintf(stderr,
            error < 0 ? "profiling:%s:Overflow merging\n"
  ...
  rewrite:;
    ...
    for (t_ix = 0; t_ix < GCOV_COUNTERS_SUMMABLE; t_ix++) {
      ...
      if (!cs_all->runs && cs_prg->runs)
        ...
        fprintf(
            stderr,
            "profiling:%s:Invocation mismatch - some data files may have been "
            "removed%s",
            gi_filename,
            GCOV_LOCKED ? "" : " or concurrent update without locking support");
        all.checksum = ~0u;
      }
    }
    ...
    if ((error = gcov_close()))
      fprintf(stderr,
              error < 0 ? "profiling:%s:Overflow writing\n"
...
/* The profile merging function that just adds the counters.  It is given
   an array COUNTERS of N_COUNTERS old counters and it reads the same number
   of counters from the gcov file.  */
void __gcov_merge_ior(gcov_type *counters, unsigned n_counters) {
  for (; n_counters; counters++, n_counters--)
    *counters |= gcov_read_counter();
}
#endif
...
void __gcov_interval_profiler(gcov_type *counters, gcov_type value, int start,
                              unsigned steps) {
