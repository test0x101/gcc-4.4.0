...
static basic_block find_bb_for_arg(gimple phi, tree def) {
  size_t i;
  bool foundone = false;
...
static bool all_immediate_uses_same_place(gimple stmt) {
  ...
  FOR_EACH_SSA_TREE_OPERAND(var, stmt, op_iter, SSA_OP_ALL_DEFS) {
    FOR_EACH_IMM_USE_FAST(use_p, imm_iter, var) {
      ...
      else if (firstuse != USE_STMT(use_p))
        return false;
    }
  }
...
bool is_hidden_global_store(gimple stmt) {
...
static basic_block nearest_common_dominator_of_uses(gimple stmt) {
  ...
  commondom =
      nearest_common_dominator(CDI_DOMINATORS, commondom, BASIC_BLOCK(j));
  BITMAP_FREE(blocks);
  return commondom;
}
...
static bool statement_sink_location(gimple stmt, basic_block frombb,
  ...
  */
  ...
  if (stmt_ends_bb_p(stmt) || gimple_has_side_effects(stmt) ||
      code == EXC_PTR_EXPR || code == FILTER_EXPR ||
      is_hidden_global_store(stmt) || gimple_has_volatile_ops(stmt) ||
...
static void sink_code_in_bb(basic_block bb) {
  basic_block son;
...
*/
static void execute_sink_code(void) {
...
static unsigned int do_sink(void) {
  execute_sink_code();
  return 0;
}

static bool gate_sink(void) {
  return flag_tree_sink != 0;
}

struct gimple_opt_pass pass_sink_code = {{
    GIMPLE_PASS, "sink", /* name */
    gate_sink,           /* gate */
    do_sink,             /* execute */
    NULL,                /* sub */
    NULL,                /* next */
