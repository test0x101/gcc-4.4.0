...
static rtx simplify_immed_subreg(enum machine_mode, rtx, enum machine_mode,
                                 unsigned int);
static rtx simplify_associative_operation(enum rtx_code, enum machine_mode, rtx,
                                          rtx);
...
static rtx simplify_binary_operation_1(enum rtx_code, enum machine_mode, rtx,
                                       rtx, rtx, rtx);

/* Negate a CONST_INT rtx, truncating (because a conversion from a
   maximally negative number can overflow).  */
static rtx neg_const_int(enum machine_mode mode, const_rtx i) {
...
bool mode_signbit_p(enum machine_mode mode, const_rtx x) {
  ...
  if (width <= HOST_BITS_PER_WIDE_INT && GET_CODE(x) == CONST_INT)
    val = INTVAL(x);
  else if (width <= 2 * HOST_BITS_PER_WIDE_INT && GET_CODE(x) == CONST_DOUBLE &&
...
rtx simplify_gen_binary(enum rtx_code code, enum machine_mode mode, rtx op0,
                        rtx op1) {
  rtx tem;
...
rtx simplify_gen_unary(enum rtx_code code, enum machine_mode mode, rtx op,
...
rtx simplify_gen_ternary(enum rtx_code code, enum machine_mode mode,
  ...
  /* If this simplifies, use it.  */
  if (0 !=
      (tem = simplify_ternary_operation(code, mode, op0_mode, op0, op1, op2)))
    return tem;
...
rtx simplify_gen_relational(enum rtx_code code, enum machine_mode mode,
  ...
  switch (GET_RTX_CLASS(code)) {
    ...
    case RTX_EXTRA:
      ...
      if (code == SUBREG) {
        ...
        if (op0 == SUBREG_REG(x))
          return x;
        op0 = simplify_gen_subreg(GET_MODE(x), op0, GET_MODE(SUBREG_REG(x)),
                                  SUBREG_BYTE(x));
        return op0 ? op0 : x;
      }
      break;
...
rtx simplify_unary_operation(enum rtx_code code, enum machine_mode mode, rtx op,
                             enum machine_mode op_mode) {
  rtx trueop, tem;
...
/* Perform some simplifications we can do even if the operands
   aren't constant.  */
static rtx simplify_unary_operation_1(enum rtx_code code,
  ...
  switch (code) {
    case NOT:
      ...
      if (COMPARISON_P(op) && (mode == BImode || STORE_FLAG_VALUE == -1) &&
          ((reversed = reversed_comparison_code(op, NULL_RTX)) != UNKNOWN))
        return simplify_gen_relational(reversed, mode, VOIDmode, XEXP(op, 0),
                                       XEXP(op, 1));
      ...
      if (GET_CODE(op) == XOR && GET_CODE(XEXP(op, 1)) == CONST_INT &&
          (temp = simplify_unary_operation(NOT, mode, XEXP(op, 1), mode)) != 0)
      ...
      if (STORE_FLAG_VALUE == -1 && GET_CODE(op) == ASHIFTRT &&
          GET_CODE(XEXP(op, 1)) == CONST_INT &&
          INTVAL(XEXP(op, 1)) == GET_MODE_BITSIZE(mode) - 1)
        return simplify_gen_relational(GE, mode, VOIDmode, XEXP(op, 0),
                                       const0_rtx);

      if (GET_CODE(op) == SUBREG && subreg_lowpart_p(op) &&
        ...
        x = gen_rtx_ROTATE(
            inner_mode,
            simplify_gen_unary(NOT, inner_mode, const1_rtx, inner_mode),
    ...
    case NEG:
      ...
      if (GET_CODE(op) == MINUS && !HONOR_SIGNED_ZEROS(mode) &&
          !HONOR_SIGN_DEPENDENT_ROUNDING(mode))
        return simplify_gen_binary(MINUS, mode, XEXP(op, 1), XEXP(op, 0));

      if (GET_CODE(op) == PLUS && !HONOR_SIGNED_ZEROS(mode) &&
          !HONOR_SIGN_DEPENDENT_ROUNDING(mode)) {
      ...
      if (GET_CODE(op) == ASHIFTRT && GET_CODE(XEXP(op, 1)) == CONST_INT &&
          INTVAL(XEXP(op, 1)) == GET_MODE_BITSIZE(mode) - 1)
        return simplify_gen_binary(LSHIFTRT, mode, XEXP(op, 0), XEXP(op, 1));

      /* (neg (lshiftrt X C)) can be replaced by (ashiftrt X C) when
         C is equal to the width of MODE minus 1.  */
      if (GET_CODE(op) == LSHIFTRT && GET_CODE(XEXP(op, 1)) == CONST_INT &&
          INTVAL(XEXP(op, 1)) == GET_MODE_BITSIZE(mode) - 1)
        return simplify_gen_binary(ASHIFTRT, mode, XEXP(op, 0), XEXP(op, 1));
    ...
    case TRUNCATE:
      ...
      if ((GET_CODE(op) == ABS || GET_CODE(op) == NEG) &&
        ...
        return simplify_gen_unary(GET_CODE(op), mode, XEXP(XEXP(op, 0), 0),
                                  mode);

      /* (truncate:A (subreg:B (truncate:C X) 0)) is
         (truncate:A X).  */
      if (GET_CODE(op) == SUBREG && GET_CODE(SUBREG_REG(op)) == TRUNCATE &&
          subreg_lowpart_p(op))
      ...
      if ((TRULY_NOOP_TRUNCATION(GET_MODE_BITSIZE(mode),
          ...
          !(GET_CODE(op) == LSHIFTRT && GET_CODE(XEXP(op, 0)) == MULT))
    ...
    case FLOAT_TRUNCATE:
      ...
      if ((GET_CODE(op) == FLOAT_TRUNCATE && flag_unsafe_math_optimizations) ||
          GET_CODE(op) == FLOAT_EXTEND)
        return simplify_gen_unary(
            GET_MODE_SIZE(GET_MODE(XEXP(op, 0))) > GET_MODE_SIZE(mode)
                ? FLOAT_TRUNCATE
                : FLOAT_EXTEND,
            mode, XEXP(op, 0), mode);
           ...
           (SCALAR_FLOAT_MODE_P(GET_MODE(op)) &&
            ((unsigned)significand_size(GET_MODE(op)) >=
             (GET_MODE_BITSIZE(GET_MODE(XEXP(op, 0))) -
              num_sign_bit_copies(XEXP(op, 0), GET_MODE(XEXP(op, 0))))))))
    ...
    case FLOAT_EXTEND:
      ...
      if (GET_CODE(op) == FLOAT_EXTEND ||
          (GET_CODE(op) == FLOAT && SCALAR_FLOAT_MODE_P(GET_MODE(op)) &&
           ((unsigned)significand_size(GET_MODE(op)) >=
            (GET_MODE_BITSIZE(GET_MODE(XEXP(op, 0))) -
             num_sign_bit_copies(XEXP(op, 0), GET_MODE(XEXP(op, 0)))))))
    ...
    case ABS:
      ...
      if (GET_CODE(op) == FFS || GET_CODE(op) == ABS ||
          ((GET_MODE_BITSIZE(GET_MODE(op)) <= HOST_BITS_PER_WIDE_INT) &&
           ((nonzero_bits(op, GET_MODE(op)) &
             ((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(GET_MODE(op)) - 1))) == 0)))
        return op;
    ...
    case SIGN_EXTEND:
      ...
      if (GET_CODE(op) == SUBREG && SUBREG_PROMOTED_VAR_P(op) &&
          !SUBREG_PROMOTED_UNSIGNED_P(op) &&
          GET_MODE_SIZE(mode) <= GET_MODE_SIZE(GET_MODE(XEXP(op, 0))))
        return rtl_hooks.gen_lowpart_no_emit(mode, op);

#if defined(POINTERS_EXTEND_UNSIGNED) && !defined(HAVE_ptr_extend)
      if (!POINTERS_EXTEND_UNSIGNED && mode == Pmode &&
          GET_MODE(op) == ptr_mode &&
          (CONSTANT_P(op) ||
           (GET_CODE(op) == SUBREG && REG_P(SUBREG_REG(op)) &&
            REG_POINTER(SUBREG_REG(op)) && GET_MODE(SUBREG_REG(op)) == Pmode)))
...
#if defined(POINTERS_EXTEND_UNSIGNED) && !defined(HAVE_ptr_extend)
      if (POINTERS_EXTEND_UNSIGNED > 0 && mode == Pmode &&
          GET_MODE(op) == ptr_mode &&
          (CONSTANT_P(op) ||
           (GET_CODE(op) == SUBREG && REG_P(SUBREG_REG(op)) &&
            REG_POINTER(SUBREG_REG(op)) && GET_MODE(SUBREG_REG(op)) == Pmode)))
...
rtx simplify_const_unary_operation(enum rtx_code code, enum machine_mode mode,
  ...
  if (code == VEC_DUPLICATE) {
    ...
    if (GET_MODE(op) != VOIDmode) {
      if (!VECTOR_MODE_P(GET_MODE(op)))
        ...
        gcc_assert(GET_MODE_INNER(mode) == GET_MODE_INNER(GET_MODE(op)));
    }
  ...
  if (GET_CODE(op) == CONST_INT && width <= HOST_BITS_PER_WIDE_INT &&
      width > 0) {
    ...
    switch (code) {
      ...
      case SIGN_EXTEND:
        ...
        if (GET_MODE_BITSIZE(op_mode) == HOST_BITS_PER_WIDE_INT) {
          ...
          val = arg0 & ~((HOST_WIDE_INT)(-1) << GET_MODE_BITSIZE(op_mode));
          if (val & ((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(op_mode) - 1)))
  ...
  else if (GET_MODE(op) == VOIDmode && width <= HOST_BITS_PER_WIDE_INT * 2 &&
           (GET_CODE(op) == CONST_DOUBLE || GET_CODE(op) == CONST_INT)) {
    ...
    switch (code) {
      ...
      case SIGN_EXTEND:
        if (op_mode == VOIDmode ||
          ...
          lv = l1 & GET_MODE_MASK(op_mode);
          if (GET_MODE_BITSIZE(op_mode) < HOST_BITS_PER_WIDE_INT &&
              (lv & ((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(op_mode) - 1))) != 0)
  ...
  else if (GET_CODE(op) == CONST_DOUBLE && SCALAR_FLOAT_MODE_P(mode)) {
    REAL_VALUE_TYPE d, t;
  ...
  else if (GET_CODE(op) == CONST_DOUBLE && SCALAR_FLOAT_MODE_P(GET_MODE(op)) &&
    ...
    switch (code) {
      ...
      case UNSIGNED_FIX:
        ...
        if (width == 2 * HOST_BITS_PER_WIDE_INT) {
          ...
          th = ((unsigned HOST_WIDE_INT)1 << (width - HOST_BITS_PER_WIDE_INT)) -
               1;
          tl = -1;
        } else {
          th = 0;
...
static rtx simplify_associative_operation(enum rtx_code code,
                                          enum machine_mode mode, rtx op0,
                                          rtx op1) {
  rtx tem;
...
static rtx simplify_binary_operation_1(enum rtx_code code,
                                       enum machine_mode mode, rtx op0, rtx op1,
  ...
  switch (code) {
    case PLUS:
      ...
      /* (~a) + 1 -> -a */
      if (INTEGRAL_MODE_P(mode) && GET_CODE(op0) == NOT &&
          trueop1 == const1_rtx)
      ...
      if ((GET_CODE(op0) == CONST || GET_CODE(op0) == SYMBOL_REF ||
           GET_CODE(op0) == LABEL_REF) &&
          GET_CODE(op1) == CONST_INT)
        return plus_constant(op0, INTVAL(op1));
      else if ((GET_CODE(op1) == CONST || GET_CODE(op1) == SYMBOL_REF ||
      ...
      if (SCALAR_INT_MODE_P(mode)) {
        ...
        if (rtx_equal_p(lhs, rhs)) {
          ...
          return rtx_cost(tem, SET, speed) <= rtx_cost(orig, SET, speed) ? tem
                                                                         : 0;
        }
      }

      /* (plus (xor X C1) C2) is (xor X (C1^C2)) if C2 is signbit.  */
      if ((GET_CODE(op1) == CONST_INT || GET_CODE(op1) == CONST_DOUBLE) &&
          GET_CODE(op0) == XOR &&
        ...
        return simplify_gen_binary(
            XOR, mode, XEXP(op0, 0),
            simplify_gen_binary(XOR, mode, op1, XEXP(op0, 1)));

      /* Canonicalize (plus (mult (neg B) C) A) to (minus A (mult B C)).  */
      if (!HONOR_SIGN_DEPENDENT_ROUNDING(mode) && GET_CODE(op0) == MULT &&
        ...
        return simplify_gen_binary(MINUS, mode, op1,
                                   simplify_gen_binary(MULT, mode, in1, in2));
      }
      ...
      if (INTEGRAL_MODE_P(mode) &&
          (plus_minus_operand_p(op0) || plus_minus_operand_p(op1)) &&
    ...
    case COMPARE:
      ...
      if (!(HONOR_SIGNED_ZEROS(mode) && HONOR_SIGN_DEPENDENT_ROUNDING(mode)) &&
    ...
    case MINUS:
      ...
      if (GET_CODE(op1) == CONST_INT && GET_MODE(op0) != VOIDmode)
        return simplify_gen_binary(PLUS, mode, op0, neg_const_int(mode, op1));
      ...
      if (STORE_FLAG_VALUE == 1 && trueop0 == const1_rtx && COMPARISON_P(op1) &&
          (reversed = reversed_comparison(op1, mode)))
        return reversed;

      /* Canonicalize (minus A (mult (neg B) C)) to (plus (mult B C) A).  */
      if (!HONOR_SIGN_DEPENDENT_ROUNDING(mode) && GET_CODE(op1) == MULT &&
    ...
    case MULT:
      ...
      if (!HONOR_NANS(mode) && !HONOR_SIGNED_ZEROS(mode) &&
      ...
      if (GET_CODE(trueop1) == CONST_DOUBLE &&
          ...
          GET_MODE(op0) == mode && CONST_DOUBLE_LOW(trueop1) == 0 &&
      ...
      if (GET_CODE(trueop1) == CONST_DOUBLE &&
          SCALAR_FLOAT_MODE_P(GET_MODE(trueop1)) && GET_MODE(op0) == mode) {
        REAL_VALUE_TYPE d;
        ...
        if (!HONOR_SNANS(mode) && REAL_VALUES_EQUAL(d, dconstm1))
          return simplify_gen_unary(NEG, mode, op0, mode);
      }

      /* Optimize -x * -x as x * x.  */
      if (FLOAT_MODE_P(mode) && GET_CODE(op0) == NEG && GET_CODE(op1) == NEG &&
          rtx_equal_p(XEXP(op0, 0), XEXP(op1, 0)) &&
          !side_effects_p(XEXP(op0, 0)))
        return simplify_gen_binary(MULT, mode, XEXP(op0, 0), XEXP(op1, 0));

      /* Likewise, optimize abs(x) * abs(x) as x * x.  */
      if (SCALAR_FLOAT_MODE_P(mode) && GET_CODE(op0) == ABS &&
          GET_CODE(op1) == ABS && rtx_equal_p(XEXP(op0, 0), XEXP(op1, 0)) &&
          !side_effects_p(XEXP(op0, 0)))
        return simplify_gen_binary(MULT, mode, XEXP(op0, 0), XEXP(op1, 0));
    ...
    case IOR:
      if (trueop1 == const0_rtx)
        return op0;
      if (GET_CODE(trueop1) == CONST_INT &&
          ((INTVAL(trueop1) & GET_MODE_MASK(mode)) == GET_MODE_MASK(mode)))
        return op1;
      if (rtx_equal_p(trueop0, trueop1) && !side_effects_p(op0))
        return op0;
      /* A | (~A) -> -1 */
      if (((GET_CODE(op0) == NOT && rtx_equal_p(XEXP(op0, 0), op1)) ||
          ...
          !side_effects_p(op0) && SCALAR_INT_MODE_P(mode))
        return constm1_rtx;
      ...
      if (GET_CODE(op0) == AND && GET_CODE(trueop1) == CONST_INT &&
          GET_CODE(XEXP(op0, 1)) == CONST_INT) {
      ...
      if (GET_CODE(op1) == CONST_INT &&
          (GET_MODE_BITSIZE(mode) <= HOST_BITS_PER_WIDE_INT ||
           INTVAL(op1) > 0) &&
          GET_CODE(op0) == AND && GET_CODE(XEXP(op0, 1)) == CONST_INT &&
          GET_CODE(op1) == CONST_INT &&
          (INTVAL(XEXP(op0, 1)) & INTVAL(op1)) != 0)
        return simplify_gen_binary(
            IOR, mode,
            simplify_gen_binary(AND, mode, XEXP(op0, 0),
                                GEN_INT(INTVAL(XEXP(op0, 1)) & ~INTVAL(op1))),
            op1);
      ...
      if (GET_CODE(trueop1) == CONST_INT && GET_CODE(op0) == ASHIFTRT &&
        ...
        if (mask >> count == INTVAL(trueop1) &&
          ...
          return simplify_gen_binary(
              ASHIFTRT, mode, plus_constant(XEXP(op0, 0), mask), XEXP(op0, 1));
      }
    ...
    case XOR:
         ...
         (NOT y).  */
      {
        ...
        if (num_negated == 2)
          ...
          return simplify_gen_unary(
              NOT, mode, simplify_gen_binary(XOR, mode, op0, op1), mode);
      }
      ...
      if (GET_CODE(op0) == AND && rtx_equal_p(XEXP(op0, 1), op1) &&
          !side_effects_p(op1))
        return simplify_gen_binary(
            AND, mode, simplify_gen_unary(NOT, mode, XEXP(op0, 0), mode), op1);

      else if (GET_CODE(op0) == AND && rtx_equal_p(XEXP(op0, 0), op1) &&
               !side_effects_p(op1))
        return simplify_gen_binary(
            AND, mode, simplify_gen_unary(NOT, mode, XEXP(op0, 1), mode), op1);

      /* (xor (comparison foo bar) (const_int 1)) can become the reversed
         comparison if STORE_FLAG_VALUE is 1.  */
      if (STORE_FLAG_VALUE == 1 && trueop1 == const1_rtx && COMPARISON_P(op0) &&
          (reversed = reversed_comparison(op0, mode)))
        return reversed;
      ...
      if (STORE_FLAG_VALUE == 1 && trueop1 == const1_rtx &&
          GET_CODE(op0) == LSHIFTRT && GET_CODE(XEXP(op0, 1)) == CONST_INT &&
      ...
      if (GET_MODE_BITSIZE(mode) <= HOST_BITS_PER_WIDE_INT &&
          ...
          trueop1 == const_true_rtx && COMPARISON_P(op0) &&
          (reversed = reversed_comparison(op0, mode)))
        return reversed;
    ...
    case AND:
      ...
      if ((GET_CODE(op0) == SIGN_EXTEND || GET_CODE(op0) == ZERO_EXTEND) &&
          ...
          GET_MODE_BITSIZE(mode) <= HOST_BITS_PER_WIDE_INT &&
          (~GET_MODE_MASK(GET_MODE(XEXP(op0, 0))) & INTVAL(trueop1)) == 0) {
        enum machine_mode imode = GET_MODE(XEXP(op0, 0));
        tem = simplify_gen_binary(AND, imode, XEXP(op0, 0),
                                  gen_int_mode(INTVAL(trueop1), imode));
        return simplify_gen_unary(ZERO_EXTEND, mode, tem, imode);
      }

      /* Canonicalize (A | C1) & C2 as (A & C2) | (C1 & C2).  */
      if (GET_CODE(op0) == IOR && GET_CODE(trueop1) == CONST_INT &&
          GET_CODE(XEXP(op0, 1)) == CONST_INT) {
        HOST_WIDE_INT tmp = INTVAL(trueop1) & INTVAL(XEXP(op0, 1));
        return simplify_gen_binary(
            IOR, mode, simplify_gen_binary(AND, mode, XEXP(op0, 0), op1),
            gen_int_mode(tmp, mode));
      }

      /* Convert (A ^ B) & A to A & (~B) since the latter is often a single
         insn (and may simplify more).  */
      if (GET_CODE(op0) == XOR && rtx_equal_p(XEXP(op0, 0), op1) &&
          !side_effects_p(op1))
        return simplify_gen_binary(
            AND, mode, simplify_gen_unary(NOT, mode, XEXP(op0, 1), mode), op1);

      if (GET_CODE(op0) == XOR && rtx_equal_p(XEXP(op0, 1), op1) &&
          !side_effects_p(op1))
        return simplify_gen_binary(
            AND, mode, simplify_gen_unary(NOT, mode, XEXP(op0, 0), mode), op1);
      ...
      if (GET_CODE(trueop1) == CONST_INT &&
          GET_MODE_BITSIZE(mode) <= HOST_BITS_PER_WIDE_INT &&
          ~INTVAL(trueop1) && (INTVAL(trueop1) & (INTVAL(trueop1) + 1)) == 0 &&
          ...
          switch (GET_CODE(tem)) {
            case AND:
              if (GET_CODE(XEXP(tem, 1)) == CONST_INT &&
                  (INTVAL(XEXP(tem, 1)) & INTVAL(trueop1)) == INTVAL(trueop1))
        ...
        if (pmop[0] != XEXP(op0, 0) || pmop[1] != XEXP(op0, 1)) {
          tem = simplify_gen_binary(GET_CODE(op0), mode, pmop[0], pmop[1]);
          return simplify_gen_binary(code, mode, tem, op1);
        }
      }

      /* (and X (ior (not X) Y) -> (and X Y) */
      if (GET_CODE(op1) == IOR && GET_CODE(XEXP(op1, 0)) == NOT &&
          op0 == XEXP(XEXP(op1, 0), 0))
        return simplify_gen_binary(AND, mode, op0, XEXP(op1, 1));

      /* (and (ior (not X) Y) X) -> (and X Y) */
      if (GET_CODE(op0) == IOR && GET_CODE(XEXP(op0, 0)) == NOT &&
    ...
    case DIV:
      ...
      if (SCALAR_FLOAT_MODE_P(mode)) {
        ...
        if (trueop0 == CONST0_RTX(mode) && !HONOR_NANS(mode) &&
            !HONOR_SIGNED_ZEROS(mode) && !side_effects_p(op1))
          return op0;
        /* x/1.0 is x.  */
        if (trueop1 == CONST1_RTX(mode) && !HONOR_SNANS(mode))
          return op0;

        if (GET_CODE(trueop1) == CONST_DOUBLE && trueop1 != CONST0_RTX(mode)) {
          REAL_VALUE_TYPE d;
          REAL_VALUE_FROM_CONST_DOUBLE(d, trueop1);

          /* x/-1.0 is -x.  */
          if (REAL_VALUES_EQUAL(d, dconstm1) && !HONOR_SNANS(mode))
            return simplify_gen_unary(NEG, mode, op0, mode);

          /* Change FP division by a constant into multiplication.
             Only do this with -freciprocal-math.  */
          if (flag_reciprocal_math && !REAL_VALUES_EQUAL(d, dconst0)) {
    ...
    case UMOD:
      ...
      if (GET_CODE(trueop1) == CONST_INT && exact_log2(INTVAL(trueop1)) > 0)
        return simplify_gen_binary(AND, mode, op0, GEN_INT(INTVAL(op1) - 1));
      break;
    ...
    case LSHIFTRT:
      ...
      if (GET_CODE(op0) == CLZ && GET_CODE(trueop1) == CONST_INT &&
          STORE_FLAG_VALUE == 1 && INTVAL(trueop1) < (HOST_WIDE_INT)width) {
        enum machine_mode imode = GET_MODE(XEXP(op0, 0));
        ...
        if (CLZ_DEFINED_VALUE_AT_ZERO(imode, zero_val) &&
          ...
          return simplify_gen_relational(EQ, mode, imode, XEXP(op0, 0),
                                         const0_rtx);
      }
      goto canonicalize_shift;
    ...
    case VEC_SELECT:
      if (!VECTOR_MODE_P(mode)) {
        ...
        if (GET_CODE(trueop0) == VEC_SELECT) {
          ...
          tmp = gen_rtx_fmt_ee(code, mode, tmp_op,
                               gen_rtx_PARALLEL(VOIDmode, vec));
          return tmp;
        }
      } else {
        gcc_assert(VECTOR_MODE_P(GET_MODE(trueop0)));
        gcc_assert(GET_MODE_INNER(mode) == GET_MODE_INNER(GET_MODE(trueop0)));
        gcc_assert(GET_CODE(trueop1) == PARALLEL);
    ...
    case VEC_CONCAT: {
      enum machine_mode op0_mode =
          (GET_MODE(trueop0) != VOIDmode ? GET_MODE(trueop0)
                                         : GET_MODE_INNER(mode));
      enum machine_mode op1_mode =
          (GET_MODE(trueop1) != VOIDmode ? GET_MODE(trueop1)
                                         : GET_MODE_INNER(mode));
      ...
      if (VECTOR_MODE_P(op0_mode))
        gcc_assert(GET_MODE_INNER(mode) == GET_MODE_INNER(op0_mode));
      else
        gcc_assert(GET_MODE_INNER(mode) == op0_mode);

      if (VECTOR_MODE_P(op1_mode))
        gcc_assert(GET_MODE_INNER(mode) == GET_MODE_INNER(op1_mode));
      else
        ...
        for (i = 0; i < n_elts; i++) {
          if (i < in_n_elts) {
            ...
            if (!VECTOR_MODE_P(op1_mode))
              ...
              RTVEC_ELT(v, i) = CONST_VECTOR_ELT(trueop1, i - in_n_elts);
          }
        }
...
rtx simplify_const_binary_operation(enum rtx_code code, enum machine_mode mode,
                                    rtx op0, rtx op1) {
  HOST_WIDE_INT arg0, arg1, arg0s, arg1s;
  HOST_WIDE_INT val;
  unsigned int width = GET_MODE_BITSIZE(mode);

  if (VECTOR_MODE_P(mode) && code != VEC_CONCAT &&
  ...
  if (VECTOR_MODE_P(mode) && code == VEC_CONCAT &&
      (CONST_INT_P(op0) || GET_CODE(op0) == CONST_DOUBLE ||
       GET_CODE(op0) == CONST_FIXED) &&
      (CONST_INT_P(op1) || GET_CODE(op1) == CONST_DOUBLE ||
  ...
  if (SCALAR_FLOAT_MODE_P(mode) && GET_CODE(op0) == CONST_DOUBLE &&
      GET_CODE(op1) == CONST_DOUBLE && mode == GET_MODE(op0) &&
      mode == GET_MODE(op1)) {
    if (code == AND || code == IOR || code == XOR) {
      ...
      real_to_target(tmp0, CONST_DOUBLE_REAL_VALUE(op0), GET_MODE(op0));
      real_to_target(tmp1, CONST_DOUBLE_REAL_VALUE(op1), GET_MODE(op1));
      for (i = 0; i < 4; i++) {
        switch (code) {
          case AND:
            tmp0[i] &= tmp1[i];
      ...
      if (HONOR_SNANS(mode) && (REAL_VALUE_ISNAN(f0) || REAL_VALUE_ISNAN(f1)))
        return 0;
      ...
      if (MODE_HAS_INFINITIES(mode) && HONOR_NANS(mode) && flag_trapping_math &&
      ...
      if (flag_trapping_math && MODE_HAS_INFINITIES(mode) &&
          REAL_VALUE_ISINF(result) && !REAL_VALUE_ISINF(f0) &&
          !REAL_VALUE_ISINF(f1))
  ...
  if (GET_MODE_CLASS(mode) == MODE_INT && width == HOST_BITS_PER_WIDE_INT * 2 &&
    ...
    switch (code) {
      ...
      case DIV:
        if (div_and_round_double(TRUNC_DIV_EXPR, 0, l1, h1, l2, h2, &lv, &hv,
                                 &lt, &ht))
          return 0;
        break;

      case MOD:
        if (div_and_round_double(TRUNC_DIV_EXPR, 0, l1, h1, l2, h2, &lt, &ht,
                                 &lv, &hv))
          return 0;
        break;

      case UDIV:
        if (div_and_round_double(TRUNC_DIV_EXPR, 1, l1, h1, l2, h2, &lv, &hv,
                                 &lt, &ht))
          return 0;
        break;

      case UMOD:
        if (div_and_round_double(TRUNC_DIV_EXPR, 1, l1, h1, l2, h2, &lt, &ht,
                                 &lv, &hv))
          return 0;
        break;
      ...
      case SMIN:
        if (h1 < h2 || (h1 == h2 && ((unsigned HOST_WIDE_INT)l1 <
      ...
      case UMIN:
        if ((unsigned HOST_WIDE_INT)h1 < (unsigned HOST_WIDE_INT)h2 ||
            (h1 == h2 &&
             ((unsigned HOST_WIDE_INT)l1 < (unsigned HOST_WIDE_INT)l2)))
          lv = l1, hv = h1;
        else
          lv = l2, hv = h2;
        break;
  ...
  if (GET_CODE(op0) == CONST_INT && GET_CODE(op1) == CONST_INT &&
    ...
    switch (code) {
      ...
      case ASHIFTRT:
        ...
        val = (code == ASHIFT ? ((unsigned HOST_WIDE_INT)arg0) << arg1
      ...
      case UMIN:
        val = ((unsigned HOST_WIDE_INT)arg0 <= (unsigned HOST_WIDE_INT)arg1
                   ? arg0
                   : arg1);
        break;
...
static bool simplify_plus_minus_op_data_cmp(rtx x, rtx y) {
  int result;

  result =
      (commutative_operand_precedence(y) - commutative_operand_precedence(x));
  if (result)
    return result > 0;
...
static rtx simplify_plus_minus(enum rtx_code code, enum machine_mode mode,
                               rtx op0, rtx op1) {
  ...
  do {
    ...
    for (i = 0; i < n_ops; i++) {
      ...
      switch (this_code) {
        ...
        case CONST:
          if (n_ops < 7 && GET_CODE(XEXP(this_op, 0)) == PLUS &&
    ...
    for (i = n_ops - 1; i > 0; i--)
      for (j = i - 1; j >= 0; j--) {
        ...
        if (lhs != 0 && rhs != 0) {
          ...
          if (tem &&
              !(GET_CODE(tem) == CONST && GET_CODE(XEXP(tem, 0)) == ncode &&
                XEXP(XEXP(tem, 0), 0) == lhs && XEXP(XEXP(tem, 0), 1) == rhs)) {
            lneg &= rneg;
            if (GET_CODE(tem) == NEG)
  ...
  if (n_ops == 2 && GET_CODE(ops[1].op) == CONST_INT && CONSTANT_P(ops[0].op) &&
      ops[0].neg)
  ...
  if (n_ops > 1 && GET_CODE(ops[n_ops - 1].op) == CONST_INT &&
...
static bool plus_minus_operand_p(const_rtx x) {
  return GET_CODE(x) == PLUS || GET_CODE(x) == MINUS ||
         (GET_CODE(x) == CONST && GET_CODE(XEXP(x, 0)) == PLUS &&
          CONSTANT_P(XEXP(XEXP(x, 0), 0)) && CONSTANT_P(XEXP(XEXP(x, 0), 1)));
}
...
/* Like simplify_binary_operation except used for relational operators.
   ...
   "infinite precision".  */
rtx simplify_relational_operation(enum rtx_code code, enum machine_mode mode,
                                  enum machine_mode cmp_mode, rtx op0,
                                  rtx op1) {
  ...
  if (GET_CODE(op0) == COMPARE && op1 == const0_rtx)
    return simplify_relational_operation(code, mode, VOIDmode, XEXP(op0, 0),
                                         XEXP(op0, 1));
  ...
  trueop0 = avoid_constant_pool_reference(op0);
  trueop1 = avoid_constant_pool_reference(op1);
  return simplify_relational_operation_1(code, mode, cmp_mode, trueop0,
                                         trueop1);
}
...
static rtx simplify_relational_operation_1(enum rtx_code code,
  ...
  if (op1 == const0_rtx && COMPARISON_P(op0)) {
    ...
    if (code == NE) {
      ...
      if (new_code != UNKNOWN)
        return simplify_gen_relational(new_code, mode, VOIDmode, XEXP(op0, 0),
                                       XEXP(op0, 1));
    }
  }

  /* Canonicalize (LTU/GEU (PLUS a b) b) as (LTU/GEU (PLUS a b) a).  */
  if ((code == LTU || code == GEU) && GET_CODE(op0) == PLUS &&
  ...
  if (op1 == const0_rtx) {
    ...
    switch (code) {
      case GE:
        ...
        return simplify_gen_relational(GT, mode, cmp_mode, op0, const0_rtx);
      case GEU:
        /* Canonicalize (GEU x 1) as (NE x 0).  */
        return simplify_gen_relational(NE, mode, cmp_mode, op0, const0_rtx);
      case LT:
        /* Canonicalize (LT x 1) as (LE x 0).  */
        return simplify_gen_relational(LE, mode, cmp_mode, op0, const0_rtx);
      case LTU:
        /* Canonicalize (LTU x 1) as (EQ x 0).  */
        return simplify_gen_relational(EQ, mode, cmp_mode, op0, const0_rtx);
      default:
        break;
    }
  ...
  if ((code == EQ || code == NE) && (op0code == PLUS || op0code == MINUS) &&
      CONSTANT_P(op1) && CONSTANT_P(XEXP(op0, 1)) &&
      (INTEGRAL_MODE_P(cmp_mode) || flag_unsafe_math_optimizations)) {
    rtx x = XEXP(op0, 0);
    rtx c = XEXP(op0, 1);

    c = simplify_gen_binary(op0code == PLUS ? MINUS : PLUS, cmp_mode, op1, c);
    return simplify_gen_relational(code, mode, cmp_mode, x, c);
  }

  /* (ne:SI (zero_extract:SI FOO (const_int 1) BAR) (const_int 0))) is
     the same as (zero_extract:SI FOO (const_int 1) BAR).  */
  if (code == NE && op1 == const0_rtx && GET_MODE_CLASS(mode) == MODE_INT &&
      cmp_mode != VOIDmode
      /* ??? Work-around BImode bugs in the ia64 backend.  */
      && mode != BImode && cmp_mode != BImode &&
      nonzero_bits(op0, cmp_mode) == 1 && STORE_FLAG_VALUE == 1)
  ...
  if ((code == EQ || code == NE) && op0code == XOR &&
      ...
      (GET_CODE(XEXP(op0, 1)) == CONST_INT ||
       GET_CODE(XEXP(op0, 1)) == CONST_DOUBLE))
    return simplify_gen_relational(
        code, mode, cmp_mode, XEXP(op0, 0),
        simplify_gen_binary(XOR, cmp_mode, XEXP(op0, 1), op1));
...
rtx simplify_const_relational_operation(enum rtx_code code,
  ...
  if (INTEGRAL_MODE_P(mode) && trueop1 != const0_rtx &&
    ...
    return simplify_const_relational_operation(signed_condition(code), mode,
                                               tem, const0_rtx);
  ...
  if ((!HONOR_NANS(GET_MODE(trueop0)) || code == UNEQ || code == UNLE ||
       code == UNGE ||
  ...
  /* If the operands are floating-point constants, see if we can fold
     the result.  */
  if (GET_CODE(trueop0) == CONST_DOUBLE && GET_CODE(trueop1) == CONST_DOUBLE &&
    ...
    return comparison_result(code, (REAL_VALUES_EQUAL(d0, d1)  ? CMP_EQ
  ...
  if ((GET_MODE_CLASS(mode) == MODE_INT || mode == VOIDmode) &&
      (GET_CODE(trueop0) == CONST_DOUBLE || GET_CODE(trueop0) == CONST_INT) &&
      (GET_CODE(trueop1) == CONST_DOUBLE || GET_CODE(trueop1) == CONST_INT)) {
  ...
  if (trueop1 == const0_rtx) {
    ...
    if (GET_CODE(op0) == IOR) {
      ...
      if (GET_CODE(inner_const) == CONST_INT && inner_const != const0_rtx) {
        ...
        int has_sign =
            (HOST_BITS_PER_WIDE_INT >= sign_bitnum &&
             (INTVAL(inner_const) & ((HOST_WIDE_INT)1 << sign_bitnum)));
  ...
  if (trueop1 == CONST0_RTX(mode) &&
      (GET_CODE(trueop0) == ABS || (GET_CODE(trueop0) == FLOAT_EXTEND &&
...
rtx simplify_ternary_operation(enum rtx_code code, enum machine_mode mode,
  ...
  switch (code) {
    ...
    case IF_THEN_ELSE:
      ...
      if (GET_CODE(op0) == NE && !side_effects_p(op0) && !HONOR_NANS(mode) &&
      ...
      if (GET_CODE(op0) == EQ && !side_effects_p(op0) && !HONOR_NANS(mode) &&
          !HONOR_SIGNED_ZEROS(mode) &&
          ((rtx_equal_p(XEXP(op0, 0), op1) && rtx_equal_p(XEXP(op0, 1), op2)) ||
           (rtx_equal_p(XEXP(op0, 0), op2) && rtx_equal_p(XEXP(op0, 1), op1))))
        return op2;
      ...
      if (COMPARISON_P(op0) && !side_effects_p(op0)) {
        enum machine_mode cmp_mode =
            (GET_MODE(XEXP(op0, 0)) == VOIDmode ? GET_MODE(XEXP(op0, 1))
                                                : GET_MODE(XEXP(op0, 0)));
        rtx temp;
    ...
    case VEC_MERGE:
      ...
      if (GET_CODE(op2) == CONST_INT) {
        ...
        if (GET_CODE(op0) == CONST_VECTOR && GET_CODE(op1) == CONST_VECTOR) {
          rtvec v = rtvec_alloc(n_elts);
          unsigned int i;

          for (i = 0; i < n_elts; i++)
            RTVEC_ELT(v, i) =
                (INTVAL(op2) & (1 << i) ? CONST_VECTOR_ELT(op0, i)
                                        : CONST_VECTOR_ELT(op1, i));
          return gen_rtx_CONST_VECTOR(mode, v);
        }
      }
...
static rtx simplify_immed_subreg(enum machine_mode outermode, rtx op,
  ...
  for (elem = 0; elem < num_elem; elem++) {
       ...
       a mistake.)  */
    {
      ...
      unsigned ibyte = (((num_elem - 1 - elem) * elem_bitsize) / BITS_PER_UNIT);
    ...
    switch (GET_CODE(el)) {
      case CONST_INT:
        for (i = 0; i < HOST_BITS_PER_WIDE_INT && i < elem_bitsize;
             i += value_bit)
          *vp++ = INTVAL(el) >> i;
      ...
      case CONST_DOUBLE:
        if (GET_MODE(el) == VOIDmode) {
          ...
          while (i < HOST_BITS_PER_WIDE_INT * 2 && i < elem_bitsize) {
            *vp++ = CONST_DOUBLE_HIGH(el) >> (i - HOST_BITS_PER_WIDE_INT);
            i += value_bit;
          }
          ...
          real_to_target(tmp, CONST_DOUBLE_REAL_VALUE(el), GET_MODE(el));
  ...
  if (GET_MODE_SIZE(innermode) >= GET_MODE_SIZE(outermode)) {
    unsigned ibyte =
        (GET_MODE_SIZE(innermode) - GET_MODE_SIZE(outermode) - byte);
    unsigned word_byte = WORDS_BIG_ENDIAN ? ibyte : byte;
    unsigned subword_byte = BYTES_BIG_ENDIAN ? ibyte : byte;
...
rtx simplify_subreg(enum machine_mode outermode, rtx op,
  ...
  if (GET_CODE(op) == CONST_INT || GET_CODE(op) == CONST_DOUBLE ||
      GET_CODE(op) == CONST_FIXED || GET_CODE(op) == CONST_VECTOR)
  ...
  if (GET_CODE(op) == SUBREG) {
    ...
    newx =
        simplify_subreg(outermode, SUBREG_REG(op), innermostmode, final_offset);
    if (newx)
      return newx;
    if (validate_subreg(outermode, innermostmode, SUBREG_REG(op),
                        final_offset)) {
      newx = gen_rtx_SUBREG(outermode, SUBREG_REG(op), final_offset);
      if (SUBREG_PROMOTED_VAR_P(op) && SUBREG_PROMOTED_UNSIGNED_P(op) >= 0 &&
          GET_MODE_CLASS(outermode) == MODE_INT &&
          IN_RANGE(GET_MODE_SIZE(outermode), GET_MODE_SIZE(innermode),
                   GET_MODE_SIZE(innermostmode)) &&
          subreg_lowpart_p(newx)) {
        SUBREG_PROMOTED_VAR_P(newx) = 1;
        SUBREG_PROMOTED_UNSIGNED_SET(newx, SUBREG_PROMOTED_UNSIGNED_P(op));
      }
      return newx;
    }
    return NULL_RTX;
  ...
  if (MEM_P(op) &&
      ...
      && (!MEM_VOLATILE_P(op) || !have_insn_for(SET, innermode)) &&
    ...
    if (bitpos == 0) {
      ...
      if (GET_MODE_BITSIZE(outermode) <= GET_MODE_BITSIZE(origmode))
        return simplify_gen_subreg(outermode, XEXP(op, 0), origmode,
                                   subreg_lowpart_offset(outermode, origmode));
      if (SCALAR_INT_MODE_P(outermode))
        return simplify_gen_unary(GET_CODE(op), outermode, XEXP(op, 0),
                                  origmode);
    }
  ...
  if ((GET_CODE(op) == LSHIFTRT || GET_CODE(op) == ASHIFTRT) &&
    ...
    return simplify_gen_binary(ASHIFTRT, outermode, XEXP(XEXP(op, 0), 0),
                               XEXP(op, 1));
  ...
  if (GET_CODE(op) == ASHIFT && SCALAR_INT_MODE_P(outermode) &&
      ...
      INTVAL(XEXP(op, 1)) < GET_MODE_BITSIZE(outermode) &&
      subreg_lsb_1(outermode, innermode, byte) == 0)
    return simplify_gen_binary(ASHIFT, outermode, XEXP(XEXP(op, 0), 0),
                               XEXP(op, 1));
...
rtx simplify_gen_subreg(enum machine_mode outermode, rtx op,
                        enum machine_mode innermode, unsigned int byte) {
  rtx newx;
...
rtx simplify_rtx(const_rtx x) {
  ...
  switch (GET_RTX_CLASS(code)) {
    case RTX_UNARY:
      return simplify_unary_operation(code, mode, XEXP(x, 0),
                                      GET_MODE(XEXP(x, 0)));
    case RTX_COMM_ARITH:
    ...
    case RTX_COMPARE:
    case RTX_COMM_COMPARE:
      return simplify_relational_operation(
          code, mode,
    ...
    case RTX_EXTRA:
      if (code == SUBREG)
        return simplify_subreg(mode, SUBREG_REG(x), GET_MODE(SUBREG_REG(x)),
                               SUBREG_BYTE(x));
      break;
