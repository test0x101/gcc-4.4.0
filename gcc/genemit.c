...
static void gen_rtx_scratch(rtx x, enum rtx_code subroutine_type) {
...
static void gen_exp(rtx x, enum rtx_code subroutine_type, char *used) {
  RTX_CODE code;
  int i;
  int len;
  const char *fmt;
  ...
  switch (code) {
    ...
    case CLOBBER:
      if (REG_P(XEXP(x, 0))) {
        printf("gen_hard_reg_clobber (%smode, %i)",
               GET_MODE_NAME(GET_MODE(XEXP(x, 0))), REGNO(XEXP(x, 0)));
        return;
      }
      break;
    ...
    case CONST_INT:
      if (INTVAL(x) == 0)
        ...
        printf("const_int_rtx[MAX_SAVED_CONST_INT + (%d)]", (int)INTVAL(x));
...
static void gen_expand(rtx expand) {
  ...
  used = XCNEWVEC(char, MAX(operands, MAX(max_scratch_opno, max_dup_opno) + 1));
  ...
  for (i = 0; i < XVECLEN(expand, 1); i++) {
    ...
    if ((GET_CODE(next) == SET && GET_CODE(SET_DEST(next)) == PC) ||
      ...
      printf("  emit_label (");
    else if (GET_CODE(next) == MATCH_OPERAND || GET_CODE(next) == MATCH_DUP ||
             GET_CODE(next) == MATCH_OPERATOR ||
             GET_CODE(next) == MATCH_OP_DUP ||
             GET_CODE(next) == MATCH_PARALLEL ||
             GET_CODE(next) == MATCH_PAR_DUP || GET_CODE(next) == PARALLEL)
      printf("  emit (");
    else
      printf("  emit_insn (");
...
static void gen_split(rtx split) {
  ...
  if (XVEC(split, 0) == 0)
    fatal("define_%s (definition %d) lacks a pattern", name, insn_index_number);
  ...
  for (i = 0; i < XVECLEN(split, 2); i++) {
    ...
    if ((GET_CODE(next) == SET && GET_CODE(SET_DEST(next)) == PC) ||
      ...
      printf("  emit_label (");
             ...
             GET_CODE(next) == MATCH_OPERATOR ||
             GET_CODE(next) == MATCH_PARALLEL ||
             GET_CODE(next) == MATCH_OP_DUP || GET_CODE(next) == MATCH_DUP ||
             GET_CODE(next) == PARALLEL)
      printf("  emit (");
    else
      printf("  emit_insn (");
...
static void output_add_clobbers(void) {
  ...
  for (clobber = clobber_list; clobber; clobber = clobber->next) {
    ...
    for (i = clobber->first_clobber; i < XVECLEN(clobber->pattern, 1); i++) {
      ...
      gen_exp(XVECEXP(clobber->pattern, 1, i), GET_CODE(clobber->pattern),
              NULL);
      printf(";\n");
    }
...
static void output_peephole2_scratches(rtx split) {
  ...
  for (i = 0; i < XVECLEN(split, 0); i++) {
    ...
    if (GET_CODE(elt) == MATCH_SCRATCH) {
      ...
      printf(
          "  if ((operands[%d] = peep2_find_free_register (%d, %d, \"%s\", %smode, &_regs_allocated)) == NULL_RTX)\n\
    return NULL;\n",
          XINT(elt, 0), insn_nr, last_insn_nr, XSTR(elt, 1),
