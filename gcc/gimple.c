...
static const char *const gimple_alloc_kind_names[] = {
    "assignments", "phi nodes", "conditionals", "sequences", "everything else"};
...
static inline void gimple_set_code(gimple g, enum gimple_code code) {
  g->gsbase.code = code;
}
...
static size_t gimple_size(enum gimple_code code) {
...
static gimple gimple_alloc_stat(enum gimple_code code,
...
static inline void gimple_set_subcode(gimple g, unsigned subcode) {
...
gimple gimple_build_return(tree retval) {
...
static inline gimple gimple_build_call_1(tree fn, unsigned nargs) {
...
gimple gimple_build_call_vec(tree fn, VEC(tree, heap) * args) {
  unsigned i;
...
gimple gimple_build_call(tree fn, unsigned nargs, ...) {
  va_list ap;
  gimple call;
  unsigned i;
...
gimple gimple_build_call_from_tree(tree t) {
  unsigned i, nargs;
  gimple call;
...
void extract_ops_from_tree(tree expr, enum tree_code *subcode_p, tree *op1_p,
                           tree *op2_p) {
...
gimple gimple_build_assign_stat(tree lhs, tree rhs MEM_STAT_DECL) {
  enum tree_code subcode;
  tree op1, op2;

  extract_ops_from_tree(rhs, &subcode, &op1, &op2);
  return gimple_build_assign_with_ops_stat(subcode, lhs, op1,
                                           op2 PASS_MEM_STAT);
}
...
gimple gimple_build_assign_with_ops_stat(enum tree_code subcode, tree lhs,
                                         tree op1, tree op2 MEM_STAT_DECL) {
  unsigned num_ops;
  gimple p;
  ...
  p = gimple_build_with_ops_stat(GIMPLE_ASSIGN, subcode, num_ops PASS_MEM_STAT);
...
inline gimple gimplify_assign(tree dst, tree src, gimple_seq *seq_p) {
...
gimple gimple_build_cond(enum tree_code pred_code, tree lhs, tree rhs,
...
void gimple_cond_get_ops_from_tree(tree cond, enum tree_code *code_p,
...
gimple gimple_build_cond_from_tree(tree cond, tree t_label, tree f_label) {
  enum tree_code code;
  tree lhs, rhs;
...
void gimple_cond_set_condition_from_tree(gimple stmt, tree cond) {
  enum tree_code code;
  tree lhs, rhs;
...
gimple gimple_build_bind(tree vars, gimple_seq body, tree block) {
...
static inline gimple gimple_build_asm_1(const char *string, unsigned ninputs,
                                        unsigned noutputs, unsigned nclobbers) {
  gimple p;
...
gimple gimple_build_asm_vec(const char *string, VEC(tree, gc) * inputs,
                            VEC(tree, gc) * outputs, VEC(tree, gc) * clobbers) {
  gimple p;
  unsigned i;

  p = gimple_build_asm_1(string, VEC_length(tree, inputs),
...
gimple gimple_build_asm(const char *string, unsigned ninputs, unsigned noutputs,
                        unsigned nclobbers, ...) {
  gimple p;
  unsigned i;
  va_list ap;
...
gimple gimple_build_catch(tree types, gimple_seq handler) {
...
gimple gimple_build_eh_filter(tree types, gimple_seq failure) {
...
gimple gimple_build_try(gimple_seq eval, gimple_seq cleanup,
...
gimple gimple_build_resx(int region) {
...
static inline gimple gimple_build_switch_1(unsigned nlabels, tree index,
                                           tree default_label) {
...
gimple gimple_build_switch(unsigned nlabels, tree index, tree default_label,
                           ...) {
  va_list al;
  unsigned i;
  gimple p;
...
gimple gimple_build_switch_vec(tree index, tree default_label,
                               VEC(tree, heap) * args) {
  unsigned i;
...
gimple gimple_build_omp_critical(gimple_seq body, tree name) {
...
gimple gimple_build_omp_for(gimple_seq body, tree clauses, size_t collapse,
                            gimple_seq pre_body) {
...
gimple gimple_build_omp_parallel(gimple_seq body, tree clauses, tree child_fn,
                                 tree data_arg) {
...
gimple gimple_build_omp_task(gimple_seq body, tree clauses, tree child_fn,
...
gimple gimple_build_omp_continue(tree control_def, tree control_use) {
...
gimple gimple_build_omp_sections(gimple_seq body, tree clauses) {
...
gimple gimple_build_omp_single(gimple_seq body, tree clauses) {
...
gimple gimple_build_cdt(tree type, tree ptr) {
...
gimple gimple_build_omp_atomic_load(tree lhs, tree rhs) {
...
gimple gimple_build_omp_atomic_store(tree val) {
...
void gimple_check_failed(const_gimple gs, const char *file, int line,
                         const char *function, enum gimple_code code,
                         enum tree_code subcode) {
  internal_error(
      "gimple check: expected %s(%s), have %s(%s) in %s, at %s:%d",
      gimple_code_name[code], tree_code_name[subcode],
      gimple_code_name[gimple_code(gs)],
      gs->gsbase.subcode > 0 ? tree_code_name[gs->gsbase.subcode] : "",
...
void gimple_seq_free(gimple_seq seq) {
  if (seq == NULL)
    return;
...
gimple walk_gimple_seq(gimple_seq seq, walk_stmt_fn callback_stmt,
...
static tree walk_gimple_asm(gimple stmt, walk_tree_fn callback_op,
...
inline tree walk_gimple_op(gimple stmt, walk_tree_fn callback_op,
                           struct walk_stmt_info *wi) {
  ...
  switch (gimple_code(stmt)) {
    ...
    case GIMPLE_CALL:
      ...
      for (i = 0; i < gimple_call_num_args(stmt); i++) {
        ret = walk_tree(gimple_call_arg_ptr(stmt, i), callback_op, wi, pset);
        if (ret)
          return ret;
      }
    ...
    case GIMPLE_CATCH:
      ret = walk_tree(gimple_catch_types_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      break;

    case GIMPLE_EH_FILTER:
      ret = walk_tree(gimple_eh_filter_types_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      break;
    ...
    case GIMPLE_OMP_CONTINUE:
      ret = walk_tree(gimple_omp_continue_control_def_ptr(stmt), callback_op,
                      wi, pset);
      if (ret)
        return ret;

      ret = walk_tree(gimple_omp_continue_control_use_ptr(stmt), callback_op,
                      wi, pset);
      if (ret)
        return ret;
      break;
    ...
    case GIMPLE_OMP_CRITICAL:
      ret =
          walk_tree(gimple_omp_critical_name_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      break;

    case GIMPLE_OMP_FOR:
      ret = walk_tree(gimple_omp_for_clauses_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      for (i = 0; i < gimple_omp_for_collapse(stmt); i++) {
        ret =
            walk_tree(gimple_omp_for_index_ptr(stmt, i), callback_op, wi, pset);
        if (ret)
          return ret;
        ret = walk_tree(gimple_omp_for_initial_ptr(stmt, i), callback_op, wi,
                        pset);
        if (ret)
          return ret;
        ret =
            walk_tree(gimple_omp_for_final_ptr(stmt, i), callback_op, wi, pset);
        if (ret)
          return ret;
        ret =
            walk_tree(gimple_omp_for_incr_ptr(stmt, i), callback_op, wi, pset);
      }
      if (ret)
        return ret;
      break;
    ...
    case GIMPLE_OMP_PARALLEL:
      ret = walk_tree(gimple_omp_parallel_clauses_ptr(stmt), callback_op, wi,
                      pset);
      if (ret)
        return ret;
      ret = walk_tree(gimple_omp_parallel_child_fn_ptr(stmt), callback_op, wi,
                      pset);
      if (ret)
        return ret;
      ret = walk_tree(gimple_omp_parallel_data_arg_ptr(stmt), callback_op, wi,
                      pset);
      if (ret)
        return ret;
      break;
    ...
    case GIMPLE_OMP_TASK:
      ret = walk_tree(gimple_omp_task_clauses_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      ret =
          walk_tree(gimple_omp_task_child_fn_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      ret =
          walk_tree(gimple_omp_task_data_arg_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      ret = walk_tree(gimple_omp_task_copy_fn_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      ret =
          walk_tree(gimple_omp_task_arg_size_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      ret =
          walk_tree(gimple_omp_task_arg_align_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      break;

    case GIMPLE_OMP_SECTIONS:
      ret = walk_tree(gimple_omp_sections_clauses_ptr(stmt), callback_op, wi,
                      pset);
      if (ret)
        return ret;

      ret = walk_tree(gimple_omp_sections_control_ptr(stmt), callback_op, wi,
                      pset);
      if (ret)
        return ret;
    ...
    case GIMPLE_OMP_SINGLE:
      ret =
          walk_tree(gimple_omp_single_clauses_ptr(stmt), callback_op, wi, pset);
      if (ret)
        return ret;
      break;
    ...
    case GIMPLE_OMP_ATOMIC_STORE:
      ret = walk_tree(gimple_omp_atomic_store_val_ptr(stmt), callback_op, wi,
                      pset);
      if (ret)
        return ret;
      break;
...
tree walk_gimple_stmt(gimple_stmt_iterator *gsi, walk_stmt_fn callback_stmt,
                      walk_tree_fn callback_op, struct walk_stmt_info *wi) {
  gimple ret;
  tree tree_ret;
  ...
  switch (gimple_code(stmt)) {
    case GIMPLE_BIND:
      ret = walk_gimple_seq(gimple_bind_body(stmt), callback_stmt, callback_op,
                            wi);
      if (ret)
...
int gimple_call_flags(const_gimple stmt) {
  int flags;
...
bool gimple_assign_ssa_name_copy_p(gimple gs) {
  return (
      gimple_code(gs) == GIMPLE_ASSIGN &&
      (get_gimple_rhs_class(gimple_assign_rhs_code(gs)) == GIMPLE_SINGLE_RHS) &&
...
bool gimple_assign_single_p(gimple gs) {
  return (gimple_code(gs) == GIMPLE_ASSIGN &&
          get_gimple_rhs_class(gimple_assign_rhs_code(gs)) ==
              GIMPLE_SINGLE_RHS);
}
  ...
  if (cfun->cfg && gimple_code(stmt) == GIMPLE_LABEL) {
    ...
    if (uid == -1) {
      ...
      if (old_len <= (unsigned)uid) {
        ...
        VEC_safe_grow_cleared(basic_block, gc, label_to_block_map, new_len);
      }
    }
...
tree gimple_fold(const_gimple stmt) {
  switch (gimple_code(stmt)) {
    case GIMPLE_COND:
      return fold_binary(gimple_cond_code(stmt), boolean_type_node,
    ...
    case GIMPLE_ASSIGN:
      switch (get_gimple_rhs_class(gimple_assign_rhs_code(stmt))) {
        ...
        case GIMPLE_BINARY_RHS:
          return fold_binary(
              gimple_assign_rhs_code(stmt), TREE_TYPE(gimple_assign_lhs(stmt)),
...
tree gimple_get_lhs(const_gimple stmt) {
  ...
  if (gimple_has_substatements(stmt)) {
    ...
    switch (gimple_code(stmt)) {
      ...
      case GIMPLE_OMP_FOR:
        ...
        copy->gimple_omp_for.iter = GGC_NEWVEC(struct gimple_omp_for_iter,
                                               gimple_omp_for_collapse(stmt));
        for (i = 0; i < gimple_omp_for_collapse(stmt); i++) {
          gimple_omp_for_set_cond(copy, i, gimple_omp_for_cond(stmt, i));
          gimple_omp_for_set_index(copy, i, gimple_omp_for_index(stmt, i));
...
void gimple_set_modified(gimple s, bool modifiedp) {
  if (gimple_has_ops(s)) {
    ...
    if (modifiedp && cfun->gimple_df && is_gimple_call(s) &&
...
bool gimple_has_side_effects(const_gimple s) {
  unsigned i;
  ...
  if (is_gimple_call(s)) {
    ...
    if (gimple_call_lhs(s) && TREE_SIDE_EFFECTS(gimple_call_lhs(s))) {
...
bool gimple_rhs_has_side_effects(const_gimple s) {
  unsigned i;
...
static bool gimple_could_trap_p_1(gimple s, bool include_lhs) {
  unsigned i, start;
  ...
  switch (gimple_code(s)) {
    ...
    case GIMPLE_ASSIGN:
      ...
      return (operation_could_trap_p(
          op, FLOAT_TYPE_P(t), (INTEGRAL_TYPE_P(t) && TYPE_OVERFLOW_TRAPS(t)),
          div));
...
bool gimple_could_trap_p(gimple s) {
...
void gimple_set_stored_syms(gimple stmt, bitmap syms, bitmap_obstack *obs) {
...
void gimple_set_loaded_syms(gimple stmt, bitmap syms, bitmap_obstack *obs) {
  gcc_assert(gimple_has_mem_ops(stmt));
...
#define DEFTREECODE(SYM, STRING, TYPE, NARGS)                               \
  (unsigned char)((TYPE) == tcc_unary ? GIMPLE_UNARY_RHS                    \
                  : ((TYPE) == tcc_binary || (TYPE) == tcc_comparison)      \
                      ? GIMPLE_BINARY_RHS                                   \
                  : ((TYPE) == tcc_constant || (TYPE) == tcc_declaration || \
                     (TYPE) == tcc_reference)                               \
                      ? GIMPLE_SINGLE_RHS                                   \
                  : ((SYM) == TRUTH_AND_EXPR || (SYM) == TRUTH_OR_EXPR ||   \
                     (SYM) == TRUTH_XOR_EXPR)                               \
                      ? GIMPLE_BINARY_RHS                                   \
                  : (SYM) == TRUTH_NOT_EXPR ? GIMPLE_UNARY_RHS              \
                  : ((SYM) == COND_EXPR || (SYM) == CONSTRUCTOR ||          \
                     (SYM) == OBJ_TYPE_REF || (SYM) == ASSERT_EXPR ||       \
                     (SYM) == ADDR_EXPR || (SYM) == WITH_SIZE_EXPR ||       \
                     (SYM) == EXC_PTR_EXPR || (SYM) == SSA_NAME ||          \
                     (SYM) == FILTER_EXPR || (SYM) == POLYNOMIAL_CHREC ||   \
                     (SYM) == DOT_PROD_EXPR || (SYM) == VEC_COND_EXPR ||    \
...
bool is_gimple_operand(const_tree op) {
...
bool is_gimple_condexpr(tree t) {
  return (is_gimple_val(t) || (COMPARISON_CLASS_P(t) && !tree_could_trap_p(t) &&
...
static const_tree strip_invariant_refs(const_tree op) {
  while (handled_component_p(op)) {
    switch (TREE_CODE(op)) {
      case ARRAY_REF:
...
bool is_gimple_val(tree t) {
  ...
  if (is_gimple_variable(t) && is_gimple_reg_type(TREE_TYPE(t)) &&
      !is_gimple_reg(t))
    return false;
...
bool is_gimple_cast(tree t) {
  return (CONVERT_EXPR_P(t) || TREE_CODE(t) == FIX_TRUNC_EXPR);
}
...
tree get_base_address(tree t) {
  ...
  if (SSA_VAR_P(t) || TREE_CODE(t) == STRING_CST ||
      TREE_CODE(t) == CONSTRUCTOR || INDIRECT_REF_P(t))
    return t;
  else
    return NULL_TREE;
}
...
tree canonicalize_cond_expr_cond(tree t) {
  /* For (bool)x use x != 0.  */
  if (TREE_CODE(t) == NOP_EXPR && TREE_TYPE(t) == boolean_type_node) {
    tree top0 = TREE_OPERAND(t, 0);
    t = build2(NE_EXPR, TREE_TYPE(t), top0, build_int_cst(TREE_TYPE(top0), 0));
  }
  /* For !x use x == 0.  */
  else if (TREE_CODE(t) == TRUTH_NOT_EXPR) {
    tree top0 = TREE_OPERAND(t, 0);
    t = build2(EQ_EXPR, TREE_TYPE(t), top0, build_int_cst(TREE_TYPE(top0), 0));
  }
  /* For cmp ? 1 : 0 use cmp.  */
    ...
    tree top0 = TREE_OPERAND(t, 0);
    t = build2(TREE_CODE(top0), TREE_TYPE(t), TREE_OPERAND(top0, 0),
               TREE_OPERAND(top0, 1));
  }
...
gimple gimple_call_copy_skip_args(gimple stmt, bitmap args_to_skip) {
  int i;
