...
tree lhd_do_nothing_iii_return_null_tree(int ARG_UNUSED(i), int ARG_UNUSED(j),
                                         int ARG_UNUSED(k)) {
  return NULL_TREE;
}
...
void lhd_print_tree_nothing(FILE *ARG_UNUSED(file), tree ARG_UNUSED(node),
                            int ARG_UNUSED(indent)) {}

/* Called from staticp.  */

tree lhd_staticp(tree ARG_UNUSED(exp)) {
  return NULL;
}
...
void lhd_set_decl_assembler_name(tree decl) {
  ...
  gcc_assert(TREE_CODE(decl) == FUNCTION_DECL ||
             (TREE_CODE(decl) == VAR_DECL &&
              (TREE_STATIC(decl) || DECL_EXTERNAL(decl) || TREE_PUBLIC(decl))));
...
void lhd_register_builtin_type(tree ARG_UNUSED(type),
...
rtx lhd_expand_expr(tree ARG_UNUSED(t), rtx ARG_UNUSED(r),
                    enum machine_mode ARG_UNUSED(mm), int ARG_UNUSED(em),
                    rtx *ARG_UNUSED(a)) {
  gcc_unreachable();
}
...
const char *lhd_dwarf_name(tree t, int verbosity) {
  gcc_assert(DECL_P(t));
...
bool lhd_tree_dump_dump_tree(void *di ATTRIBUTE_UNUSED,
...
tree lhd_expr_size(const_tree exp) {
...
int lhd_gimplify_expr(tree *expr_p ATTRIBUTE_UNUSED,
...
size_t lhd_tree_size(enum tree_code c ATTRIBUTE_UNUSED) {
  gcc_unreachable();
}
...
bool lhd_decl_ok_for_sibcall(const_tree decl ATTRIBUTE_UNUSED) {
  return true;
}

/* Return the COMDAT group into which DECL should be placed.  */

const char *lhd_comdat_group(tree decl) {
...
void lhd_initialize_diagnostics(
    struct diagnostic_context *ctx ATTRIBUTE_UNUSED) {}
...
void lhd_print_error_function(diagnostic_context *context, const char *file,
  ...
  if (diagnostic_last_function_changed(context, diagnostic)) {
    ...
    char *new_prefix =
        (file && abstract_origin == NULL) ? file_name_as_prefix(file) : NULL;
    ...
    if (current_function_decl == NULL)
      ...
      if (TREE_CODE(TREE_TYPE(fndecl)) == METHOD_TYPE)
        pp_printf(context->printer, _("In member function %qs"),
                  lang_hooks.decl_printable_name(fndecl, 2));
      else
        pp_printf(context->printer, _("In function %qs"),
                  lang_hooks.decl_printable_name(fndecl, 2));
        ...
        if (fndecl) {
          ...
          if (s.file != NULL) {
            if (flag_show_column && s.column != 0)
              pp_printf(context->printer, _("    inlined from %qs at %s:%d:%d"),
                        lang_hooks.decl_printable_name(fndecl, 2), s.file,
                        s.line, s.column);
            else
              pp_printf(context->printer, _("    inlined from %qs at %s:%d"),
                        lang_hooks.decl_printable_name(fndecl, 2), s.file,
                        s.line);
...
tree lhd_callgraph_analyze_expr(tree *tp ATTRIBUTE_UNUSED,
...
tree lhd_expr_to_decl(tree expr, bool *tc ATTRIBUTE_UNUSED,
...
tree lhd_omp_assignment(tree clause ATTRIBUTE_UNUSED, tree dst, tree src) {
...
void lhd_omp_firstprivatize_type_sizes(
    struct gimplify_omp_ctx *c ATTRIBUTE_UNUSED, tree t ATTRIBUTE_UNUSED) {}

/* Common function for add_builtin_function and
   add_builtin_function_ext_scope.  */
static tree add_builtin_function_common(const char *name, tree type,
                                        int function_code,
...
tree add_builtin_function(const char *name, tree type, int function_code,
...
tree add_builtin_function_ext_scope(const char *name, tree type,
