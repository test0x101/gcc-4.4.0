...
static tree phiprop_insert_phi(basic_block bb, gimple phi, gimple use_stmt,
...
static bool propagate_with_phi(basic_block bb, gimple phi,
                               struct phiprop_d *phivn, size_t n) {
  ...
  if (MTAG_P(SSA_NAME_VAR(ptr)) || !POINTER_TYPE_P(TREE_TYPE(ptr)) ||
  ...
  FOR_EACH_PHI_ARG(arg_p, phi, i, SSA_OP_USE) {
    ...
    /* Walk the ssa chain until we reach a ssa name we already
       ...
       ssa_name_n = &var;  */
    while (TREE_CODE(arg) == SSA_NAME && !SSA_NAME_IS_DEFAULT_DEF(arg) &&
...
static bool tree_ssa_phiprop_1(basic_block bb, struct phiprop_d *phivn,
                               size_t n) {
...
static unsigned int tree_ssa_phiprop(void) {
...
static bool gate_phiprop(void) {
  return 1;
}

struct gimple_opt_pass pass_phiprop = {{
    GIMPLE_PASS, "phiprop", /* name */
    gate_phiprop,           /* gate */
