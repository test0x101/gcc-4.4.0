...
static bool flow_active_insn_p(const_rtx insn) {
  ...
  if (GET_CODE(PATTERN(insn)) == CLOBBER && REG_P(XEXP(PATTERN(insn), 0)) &&
...
bool forwarder_block_p(const_basic_block bb) {
  ...
  if (bb == EXIT_BLOCK_PTR || bb == ENTRY_BLOCK_PTR || !single_succ_p(bb))
    return false;
  ...
  return (!INSN_P(insn) || (JUMP_P(insn) && simplejump_p(insn)) ||
  ...
  while (sp) {
    ...
    if (dest != EXIT_BLOCK_PTR && !TEST_BIT(visited, dest->index)) {
      ...
      if (dest != EXIT_BLOCK_PTR && src != ENTRY_BLOCK_PTR &&
          pre[src->index] >= pre[dest->index] && post[dest->index] == 0)
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      ...
      if (INDEX_EDGE_PRED_BB(elist, index)->index != pred)
        fprintf(f, "*p* Pred for index %d should be %d not %d\n", index, pred,
                INDEX_EDGE_PRED_BB(elist, index)->index);
      if (INDEX_EDGE_SUCC_BB(elist, index)->index != succ)
        fprintf(f, "*p* Succ for index %d should be %d not %d\n", index, succ,
                INDEX_EDGE_SUCC_BB(elist, index)->index);
    }
  }
...
int find_edge_index(struct edge_list *edge_list, basic_block pred,
                    basic_block succ) {
  int x;
...
void flow_nodes_print(const char *str, const_sbitmap nodes, FILE *file) {
  unsigned int node = 0;
...
void flow_edge_list_print(const char *str, const edge *edge_list, int num_edges,
                          FILE *file) {
  int i;
...
void connect_infinite_loops_to_exit(void) {
  ...
  while (1) {
    unvisited_block =
        flow_dfs_compute_reverse_execute(&dfs_ds, unvisited_block);
    if (!unvisited_block)
      break;
...
static basic_block dfs_find_deadend(basic_block bb) {
...
int inverted_post_order_compute(int *post_order) {
  basic_block bb;
  edge_iterator *stack;
  int sp;
  int post_order_num = 0;
...
int pre_and_rev_post_order_compute(int *pre_order, int *rev_post_order,
  ...
  while (sp) {
    ...
    if (dest != EXIT_BLOCK_PTR && !TEST_BIT(visited, dest->index)) {
      ...
      if (ei_one_before_end_p(ei) && src != ENTRY_BLOCK_PTR && rev_post_order)
...
static basic_block flow_dfs_compute_reverse_execute(
    depth_first_search_ds data, basic_block last_unvisited) {
  basic_block bb;
  edge e;
  edge_iterator ei;
...
int dfs_enumerate_from(basic_block bb, int reverse,
...
bitmap compute_idf(bitmap def_blocks, bitmap *dfs) {
  bitmap_iterator bi;
  unsigned bb_index, i;
  ...
  while (VEC_length(int, work_stack) > 0) {
    ...
    EXECUTE_IF_AND_COMPL_IN_BITMAP(dfs[bb_index], phi_insertion_points, 0, i,
                                   bi) {
