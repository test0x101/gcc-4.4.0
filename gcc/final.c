...
static int align_fuzz(rtx start, rtx end, int known_align_log,
                      unsigned int growth) {
...
unsigned int compute_alignments(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    if (!has_fallthru &&
        (branch_frequency > freq_threshold ||
         (bb->frequency > bb->prev_bb->frequency * 10 &&
          (bb->prev_bb->frequency <= ENTRY_BLOCK_PTR->frequency / 2)))) {
    ...
    if (has_fallthru && optimize_bb_for_speed_p(bb) &&
...
#define MAX_CODE_ALIGN 16
  ...
  for (insn_current_address = 0, insn = first; insn != 0;
  ...
  while (something_changed) {
    ...
    for (insn_current_address = 0, insn = first; insn != 0;
      ...
      if (optimize && JUMP_P(insn) &&
        ...
        for (prev = rel_lab; prev && !insn_lengths[INSN_UID(prev)] &&
...
void final_start_function(rtx first ATTRIBUTE_UNUSED, FILE *file,
  ...
  if (warn_frame_larger_than && get_frame_size() > frame_larger_than_size) {
    /* Issue a warning */
...
static rtx call_from_call_insn(rtx insn) {
  rtx x;
...
rtx final_scan_insn(rtx insn, FILE *file, int optimize ATTRIBUTE_UNUSED,
  ...
  switch (GET_CODE(insn)) {
    case NOTE:
      switch (NOTE_KIND(insn)) {
        ...
        case NOTE_INSN_BLOCK_BEG:
          ...
          if (write_symbols == DBX_DEBUG || write_symbols == SDB_DEBUG) {
        ...
        case NOTE_INSN_BLOCK_END:
          ...
          if (write_symbols == DBX_DEBUG || write_symbols == SDB_DEBUG) {
            ...
            location_t *locus_ptr = block_nonartificial_location(outer_block);
    ...
    case CODE_LABEL:
      ...
      if (next != 0 && JUMP_P(next)) {
        ...
        if (GET_CODE(nextbody) == ADDR_VEC ||
          ...
          ASM_OUTPUT_CASE_LABEL(file, "L", CODE_LABEL_NUMBER(insn), next);
#else
    ...
    default: {
        ...
        for (idx = 0; idx < vlen; idx++) {
          if (GET_CODE(body) == ADDR_VEC) {
            ...
            ASM_OUTPUT_ADDR_DIFF_ELT(
                file, body, CODE_LABEL_NUMBER(XEXP(XVECEXP(body, 1, idx), 0)),
      ...
      if (optimize) {
        if (set && GET_CODE(SET_DEST(set)) == CC0 &&
      ...
      if (cc_status.flags != 0 && JUMP_P(insn) && GET_CODE(body) == SET &&
      ...
      /* If this is a conditional trap, maybe modify it if the cc's
         ...
         set up normally.  */
      if (cc_status.flags != 0 && NONJUMP_INSN_P(insn) &&
          GET_CODE(body) == TRAP_IF && COMPARISON_P(TRAP_CONDITION(body)) &&
      ...
      if (cc_status.flags != 0 && set != 0) {
        ...
        if (!JUMP_P(insn) && GET_CODE(SET_SRC(set)) == IF_THEN_ELSE) {
      ...
      if (templ == 0) {
        ...
        for (prev = insn; prev != last_ignored_compare;
             prev = PREV_INSN(prev)) {
          if (NOTE_P(prev))
...
static bool notice_source_line(rtx insn) {
  const char *filename;
  int linenum;
  ...
  if (filename && (force_source_line || filename != last_filename ||
...
rtx alter_subreg(rtx *xp) {
  ...
  if (MEM_P(y)) {
    ...
    if (offset == 0 &&
      ...
      int difference = GET_MODE_SIZE(GET_MODE(y)) - GET_MODE_SIZE(GET_MODE(x));
      if (WORDS_BIG_ENDIAN)
    ...
    *xp = adjust_address(y, GET_MODE(x), offset);
    ...
    rtx new_rtx = simplify_subreg(GET_MODE(x), y, GET_MODE(y), SUBREG_BYTE(x));
...
static rtx walk_alter_subreg(rtx *xp, bool *changed) {
  rtx x = *xp;
  switch (GET_CODE(x)) {
    case PLUS:
    case MULT:
...
static void output_asm_name(void) {
  if (debug_insn) {
    ...
    fprintf(asm_out_file, "\t%s %d\t%s", ASM_COMMENT_START,
            INSN_UID(debug_insn), insn_data[num].name);
...
static void output_asm_operand_names(rtx *operands, int *oporder, int nops) {
  ...
  for (i = 0; i < nops; i++) {
    ...
    fprintf(asm_out_file, "%c%s", wrote ? ',' : '\t',
...
static int mark_symbol_ref_as_used(rtx *xp, void *dummy ATTRIBUTE_UNUSED) {
  rtx x = *xp;
...
int only_leaf_regs_used(void) {
  ...
  if (crtl->uses_pic_offset_table && pic_offset_table_rtx != 0 &&
...
static void leaf_renumber_regs(rtx first) {
  rtx insn;
...
void leaf_renumber_regs_insn(rtx in_rtx) {
  int i, j;
...
/* When -gused is used, emit debug info for only used symbols. But in
   ...
   calls into this file, i.e., dbxout_symbol, dbxout_parms, and
   dbxout_reg_params. Those routines may also be called from a higher level
   intercepted routine. So to prevent recording data for an inner call to one of
   these for an intercept, we maintain an intercept nesting counter
   (debug_nesting). We only save the intercepted arguments if the nesting is 1.
 */
int debug_nesting = 0;
...
static unsigned int rest_of_handle_final(void) {
  ...
  if (DECL_STATIC_DESTRUCTOR(current_function_decl) && targetm.have_ctors_dtors)
