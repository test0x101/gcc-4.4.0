...
/* Indexed by pseudo register number, gives the rtx for that pseudo.
   ...
   deal with length attribute nested in top level structures.  */
...
static GTY((if_marked("ggc_marked_p"),
            param_is(struct rtx_def))) htab_t const_int_htab;

/* A hash table storing memory attribute structures.  */
static GTY((if_marked("ggc_marked_p"),
            param_is(struct mem_attrs))) htab_t mem_attrs_htab;

/* A hash table storing register attribute structures.  */
static GTY((if_marked("ggc_marked_p"),
            param_is(struct reg_attrs))) htab_t reg_attrs_htab;

/* A hash table storing all CONST_DOUBLEs.  */
static GTY((if_marked("ggc_marked_p"),
            param_is(struct rtx_def))) htab_t const_double_htab;

/* A hash table storing all CONST_FIXEDs.  */
static GTY((if_marked("ggc_marked_p"),
            param_is(struct rtx_def))) htab_t const_fixed_htab;
...
static hashval_t const_fixed_htab_hash(const void *x) {
  const_rtx const value = (const_rtx)x;
  hashval_t h;
...
static int const_fixed_htab_eq(const void *x, const void *y) {
  const_rtx const a = (const_rtx)x, b = (const_rtx)y;
...
static int mem_attrs_htab_eq(const void *x, const void *y) {
  ...
  return (
      p->alias == q->alias && p->offset == q->offset && p->size == q->size &&
      p->align == q->align &&
      (p->expr == q->expr || (p->expr != NULL_TREE && q->expr != NULL_TREE &&
...
static mem_attrs *get_mem_attrs(alias_set_type alias, tree expr, rtx offset,
                                rtx size, unsigned int align,
  ...
  if (alias == 0 && expr == 0 && offset == 0 &&
      (size == 0 || (mode != BLKmode && GET_MODE_SIZE(mode) == INTVAL(size))) &&
      (STRICT_ALIGNMENT && mode != BLKmode ? align == GET_MODE_ALIGNMENT(mode)
                                           : align == BITS_PER_UNIT))
    return 0;
...
static reg_attrs *get_reg_attrs(tree decl, int offset) {
  reg_attrs attrs;
  void **slot;
...
rtx gen_rtx_CONST_INT(enum machine_mode mode ATTRIBUTE_UNUSED,
  ...
  slot = htab_find_slot_with_hash(const_int_htab, &arg, (hashval_t)arg, INSERT);
  if (*slot == 0)
...
rtx immed_double_const(HOST_WIDE_INT i0, HOST_WIDE_INT i1,
                       enum machine_mode mode) {
  rtx value;
  unsigned int i;
...
bool validate_subreg(enum machine_mode omode, enum machine_mode imode,
...
rtx gen_reg_rtx(enum machine_mode mode) {
  ...
  /* If a virtual register with bigger mode alignment is generated,
     ...
     to stack later.  */
  if (SUPPORTS_STACK_ALIGNMENT && crtl->stack_alignment_estimated < align &&
      !crtl->stack_realign_processed)
    crtl->stack_alignment_estimated = align;

  if (generating_concat_p && (GET_MODE_CLASS(mode) == MODE_COMPLEX_FLOAT ||
...
static void update_reg_offset(rtx new_rtx, rtx reg, int offset) {
  REG_ATTRS(new_rtx) = get_reg_attrs(REG_EXPR(reg), REG_OFFSET(reg) + offset);
}

/* Generate a register with same attributes as REG, but with OFFSET
   added to the REG_OFFSET.  */

rtx gen_rtx_REG_offset(rtx reg, enum machine_mode mode, unsigned int regno,
                       int offset) {
...
rtx gen_reg_rtx_offset(rtx reg, enum machine_mode mode, int offset) {
...
void adjust_reg_mode(rtx reg, enum machine_mode mode) {
...
static void set_reg_attrs_for_decl_rtl(tree t, rtx x) {
  ...
  if (REG_P(x))
    REG_ATTRS(x) =
        get_reg_attrs(t, byte_lowpart_offset(GET_MODE(x), DECL_MODE(t)));
...
rtx gen_lowpart_common(enum machine_mode mode, rtx x) {
  ...
  } else if (GET_CODE(x) == SUBREG || REG_P(x) || GET_CODE(x) == CONCAT ||
...
rtx gen_highpart_mode(enum machine_mode outermode, enum machine_mode innermode,
                      rtx exp) {
...
int subreg_lowpart_p(const_rtx x) {
  if (GET_CODE(x) != SUBREG)
    return 1;
...
rtx operand_subword(rtx op, unsigned int offset, int validate_address,
                    enum machine_mode mode) {
  if (mode == VOIDmode)
    mode = GET_MODE(op);
...
static tree component_ref_for_mem_expr(tree ref) {
  ...
  if (TREE_CODE(inner) == COMPONENT_REF)
    ...
    while (CONVERT_EXPR_P(inner) || TREE_CODE(inner) == VIEW_CONVERT_EXPR ||
  ...
  if (inner == TREE_OPERAND(ref, 0))
    ...
    return build3(COMPONENT_REF, TREE_TYPE(ref), inner, TREE_OPERAND(ref, 1),
                  NULL_TREE);
}
...
int mem_expr_equal_p(const_tree expr1, const_tree expr2) {
  if (expr1 == expr2)
    return 1;
  ...
  if (TREE_CODE(expr1) == COMPONENT_REF)
    return mem_expr_equal_p(TREE_OPERAND(expr1, 0), TREE_OPERAND(expr2, 0)) &&
...
int get_mem_align_offset(rtx mem, int align) {
  tree expr;
  ...
  expr = MEM_EXPR(mem);
  if (expr == NULL_TREE || MEM_OFFSET(mem) == NULL_RTX ||
      !CONST_INT_P(MEM_OFFSET(mem)))
    return -1;
  ...
  if (DECL_P(expr)) {
    ...
    while (1) {
      ...
      if (!byte_offset || !host_integerp(byte_offset, 1) ||
      ...
      if (inner == NULL_TREE) {
        if (TYPE_ALIGN(DECL_FIELD_CONTEXT(field)) < (unsigned int)align)
          return -1;
        break;
...
void set_mem_attributes_minus_bitpos(rtx ref, tree t, int objectp,
  ...
  if (objectp || TREE_CODE(t) == INDIRECT_REF ||
      TREE_CODE(t) == ALIGN_INDIRECT_REF || TYPE_ALIGN_OK(type))
  ...
  if (!TYPE_P(t)) {
    ...
    while (TREE_CODE(base) == COMPONENT_REF ||
           ...
           TREE_CODE(base) == IMAGPART_EXPR || TREE_CODE(base) == BIT_FIELD_REF)
    ...
    if (base && DECL_P(base) && TREE_READONLY(base) &&
    ...
    if (DECL_P(t)) {
      ...
      offset = const0_rtx;
      apply_bitpos = bitpos;
      size = (DECL_SIZE_UNIT(t) && host_integerp(DECL_SIZE_UNIT(t), 1)
                  ? GEN_INT(tree_low_cst(DECL_SIZE_UNIT(t), 1))
                  : 0);
      align = DECL_ALIGN(t);
      align_computed = true;
    }
    ...
    /* If this is an array reference, look for an outer field reference.  */
      ...
      if (DECL_P(t2)) {
        ...
        /* ??? Any reason the field size would be different than
           the size we got from the type?  */
      } else if (flag_argument_noalias > 1 && (INDIRECT_REF_P(t2)) &&
    ...
    /* If this is a Fortran indirect argument reference, record the
       parameter decl.  */
    else if (flag_argument_noalias > 1 && (INDIRECT_REF_P(t)) &&
  ...
  /* If it is a reference into an aggregate, this is part of an aggregate.
     Otherwise we don't know.  */
           ...
           TREE_CODE(t) == ARRAY_RANGE_REF || TREE_CODE(t) == BIT_FIELD_REF)
...
void set_mem_attrs_from_reg(rtx mem, rtx reg) {
  MEM_ATTRS(mem) =
      get_mem_attrs(MEM_ALIAS_SET(mem), REG_EXPR(reg), GEN_INT(REG_OFFSET(reg)),
...
void set_mem_align(rtx mem, unsigned int align) {
  MEM_ATTRS(mem) =
      get_mem_attrs(MEM_ALIAS_SET(mem), MEM_EXPR(mem), MEM_OFFSET(mem),
...
void set_mem_expr(rtx mem, tree expr) {
  MEM_ATTRS(mem) = get_mem_attrs(MEM_ALIAS_SET(mem), expr, MEM_OFFSET(mem),
                                 MEM_SIZE(mem), MEM_ALIGN(mem), GET_MODE(mem));
}
...
void set_mem_offset(rtx mem, rtx offset) {
  MEM_ATTRS(mem) = get_mem_attrs(MEM_ALIAS_SET(mem), MEM_EXPR(mem), offset,
                                 MEM_SIZE(mem), MEM_ALIGN(mem), GET_MODE(mem));
}
...
void set_mem_size(rtx mem, rtx size) {
  MEM_ATTRS(mem) =
      get_mem_attrs(MEM_ALIAS_SET(mem), MEM_EXPR(mem), MEM_OFFSET(mem), size,
...
static rtx change_address_1(rtx memref, enum machine_mode mode, rtx addr,
                            int validate) {
  rtx new_rtx;
...
rtx change_address(rtx memref, enum machine_mode mode, rtx addr) {
  ...
  if (new_rtx == memref) {
    if (MEM_ATTRS(memref) == 0 ||
        (MEM_EXPR(memref) == NULL && MEM_OFFSET(memref) == NULL &&
...
rtx adjust_address_1(rtx memref, enum machine_mode mode, HOST_WIDE_INT offset,
  ...
  if (HOST_BITS_PER_WIDE_INT > pbits) {
    ...
    offset =
        (((HOST_WIDE_INT)((unsigned HOST_WIDE_INT)offset << shift)) >> shift);
  }
  ...
  MEM_ATTRS(new_rtx) =
      get_mem_attrs(MEM_ALIAS_SET(memref), MEM_EXPR(memref), memoffset, size,
                    memalign, GET_MODE(new_rtx));
...
rtx adjust_automodify_address_1(rtx memref, enum machine_mode mode, rtx addr,
...
rtx offset_address(rtx memref, rtx offset, unsigned HOST_WIDE_INT pow2) {
  ...
  if (!memory_address_p(GET_MODE(memref), new_rtx) && GET_CODE(addr) == PLUS &&
  ...
  /* Update the alignment to reflect the offset.  Reset the offset, which
     we don't know.  */
  MEM_ATTRS(new_rtx) = get_mem_attrs(
      MEM_ALIAS_SET(memref), MEM_EXPR(memref), 0, 0,
      MIN(MEM_ALIGN(memref), pow2 * BITS_PER_UNIT), GET_MODE(new_rtx));
  return new_rtx;
}
...
rtx widen_memory_access(rtx memref, enum machine_mode mode,
  ...
  while (expr) {
    if (TREE_CODE(expr) == COMPONENT_REF) {
      ...
      memoffset = (GEN_INT(
          INTVAL(memoffset) + tree_low_cst(offset, 1) +
          (tree_low_cst(DECL_FIELD_BIT_OFFSET(field), 1) / BITS_PER_UNIT)));
    }
    /* Similarly for the decl.  */
    else if (DECL_P(expr) && DECL_SIZE_UNIT(expr) &&
...
tree get_spill_slot_decl(bool force_build_p) {
  ...
  MEM_NOTRAP_P(rd) = 1;
  MEM_ATTRS(rd) =
      get_mem_attrs(new_alias_set(), d, const0_rtx, NULL_RTX, 0, BLKmode);
  SET_DECL_RTL(d, rd);
...
void set_mem_attrs_for_spill(rtx mem) {
  ...
  MEM_ATTRS(mem) = get_mem_attrs(alias, expr, offset, MEM_SIZE(mem),
                                 MEM_ALIGN(mem), GET_MODE(mem));
  MEM_NOTRAP_P(mem) = 1;
}
...
rtx gen_label_rtx(void) {
  return gen_rtx_CODE_LABEL(VOIDmode, 0, NULL_RTX, NULL_RTX, NULL, label_num++,
                            NULL);
}
...
static void unshare_all_rtl_1(rtx insn) {
...
static void verify_rtx_sharing(rtx orig, rtx insn) {
  ...
  switch (code) {
    ...
    case MEM:
      ...
      if (CONSTANT_ADDRESS_P(XEXP(x, 0)) || reload_completed ||
          reload_in_progress)
        return;
...
rtx make_safe_from(rtx x, rtx other) {
  ...
  if ((MEM_P(other) && !CONSTANT_P(x) && !REG_P(x) && GET_CODE(x) != SUBREG) ||
      (REG_P(other) &&
...
rtx get_first_nonnote_insn(void) {
  ...
  if (insn) {
    if (NOTE_P(insn))
      for (insn = next_insn(insn); insn && NOTE_P(insn); insn = next_insn(insn))
        continue;
    else {
...
rtx get_last_nonnote_insn(void) {
  ...
  if (insn) {
    if (NOTE_P(insn))
      for (insn = previous_insn(insn); insn && NOTE_P(insn);
...
rtx last_call_insn(void) {
  ...
  for (insn = get_last_insn(); insn && !CALL_P(insn); insn = PREV_INSN(insn))
    ;
...
int active_insn_p(const_rtx insn) {
  return (CALL_P(insn) || JUMP_P(insn) ||
          (NONJUMP_INSN_P(insn) &&
           (!reload_completed || (GET_CODE(PATTERN(insn)) != USE &&
...
static int find_auto_inc(rtx *xp, void *data) {
  rtx x = *xp;
  rtx reg = (rtx)data;
...
rtx try_split(rtx pat, rtx trial, int last) {
  ...
  for (insn = insn_last; insn; insn = PREV_INSN(insn)) {
    if (JUMP_P(insn)) {
      ...
      njumps++;
      if (probability != -1 && any_condjump_p(insn) &&
    ...
    switch (REG_NOTE_KIND(note)) {
      case REG_EH_REGION:
        for (insn = insn_last; insn != NULL_RTX; insn = PREV_INSN(insn)) {
          if (CALL_P(insn) || (flag_non_call_exceptions && INSN_P(insn) &&
...
rtx make_jump_insn_raw(rtx pattern) {
  rtx insn;
...
void add_insn_after(rtx insn, rtx after, basic_block bb) {
...
void add_insn_before(rtx insn, rtx before, basic_block bb) {
...
rtx emit_insn_before_noloc(rtx x, rtx before, basic_block bb) {
  rtx last = before;
  rtx insn;
...
rtx emit_note_before(enum insn_note subtype, rtx before) {
...
static rtx emit_insn_after_1(rtx first, rtx after, basic_block bb) {
  rtx last;
  rtx after_after;
...
rtx emit_insn_after_noloc(rtx x, rtx after, basic_block bb) {
  rtx last = after;
...
rtx emit_note_after(enum insn_note subtype, rtx after) {
  rtx note = rtx_alloc(NOTE);
  INSN_UID(note) = cur_insn_uid++;
  NOTE_KIND(note) = subtype;
  BLOCK_FOR_INSN(note) = NULL;
...
rtx emit_note(enum insn_note kind) {
  rtx note;
...
void push_to_sequence2(rtx first, rtx last) {
  start_sequence();
...
void init_emit(void) {
  ...
  regno_reg_rtx = GGC_NEWVEC(rtx, crtl->emit.regno_pointer_align_length);

  /* Put copies of all the hard registers into regno_reg_rtx.  */
  memcpy(regno_reg_rtx, static_regno_reg_rtx,
...
void init_emit_regs(void) {
  ...
  virtual_incoming_args_rtx = gen_raw_REG(Pmode, VIRTUAL_INCOMING_ARGS_REGNUM);
  virtual_stack_vars_rtx = gen_raw_REG(Pmode, VIRTUAL_STACK_VARS_REGNUM);
  virtual_stack_dynamic_rtx = gen_raw_REG(Pmode, VIRTUAL_STACK_DYNAMIC_REGNUM);
  virtual_outgoing_args_rtx = gen_raw_REG(Pmode, VIRTUAL_OUTGOING_ARGS_REGNUM);
...
void init_emit_once(int line_numbers) {
  ...
  /* Initialize the CONST_INT, CONST_DOUBLE, CONST_FIXED, and memory attribute
     hash tables.  */
  const_int_htab =
      htab_create_ggc(37, const_int_htab_hash, const_int_htab_eq, NULL);

  const_double_htab =
      htab_create_ggc(37, const_double_htab_hash, const_double_htab_eq, NULL);

  const_fixed_htab =
      htab_create_ggc(37, const_fixed_htab_hash, const_fixed_htab_eq, NULL);

  mem_attrs_htab =
      htab_create_ggc(37, mem_attrs_htab_hash, mem_attrs_htab_eq, NULL);
  reg_attrs_htab =
      htab_create_ggc(37, reg_attrs_htab_hash, reg_attrs_htab_eq, NULL);
  ...
  for (i = 0; i < (int)ARRAY_SIZE(const_tiny_rtx); i++) {
    const REAL_VALUE_TYPE *const r = (i == 0   ? &dconst0
                                      : i == 1 ? &dconst1
                                               : &dconst2);
    ...
    for (mode = GET_CLASS_NARROWEST_MODE(MODE_DECIMAL_FLOAT); mode != VOIDmode;
         mode = GET_MODE_WIDER_MODE(mode))
      const_tiny_rtx[i][(int)mode] = CONST_DOUBLE_FROM_REAL_VALUE(*r, mode);
    ...
    for (mode = GET_CLASS_NARROWEST_MODE(MODE_PARTIAL_INT); mode != VOIDmode;
         mode = GET_MODE_WIDER_MODE(mode))
      const_tiny_rtx[i][(int)mode] = GEN_INT(i);
  }
  ...
  for (mode = GET_CLASS_NARROWEST_MODE(MODE_VECTOR_INT); mode != VOIDmode;
       mode = GET_MODE_WIDER_MODE(mode)) {
    const_tiny_rtx[0][(int)mode] = gen_const_vector(mode, 0);
    const_tiny_rtx[1][(int)mode] = gen_const_vector(mode, 1);
  }

  for (mode = GET_CLASS_NARROWEST_MODE(MODE_VECTOR_FLOAT); mode != VOIDmode;
       mode = GET_MODE_WIDER_MODE(mode)) {
    const_tiny_rtx[0][(int)mode] = gen_const_vector(mode, 0);
    const_tiny_rtx[1][(int)mode] = gen_const_vector(mode, 1);
  }

  for (mode = GET_CLASS_NARROWEST_MODE(MODE_FRACT); mode != VOIDmode;
       mode = GET_MODE_WIDER_MODE(mode)) {
    FCONST0(mode).data.high = 0;
    FCONST0(mode).data.low = 0;
    FCONST0(mode).mode = mode;
    const_tiny_rtx[0][(int)mode] =
        CONST_FIXED_FROM_FIXED_VALUE(FCONST0(mode), mode);
  }

  for (mode = GET_CLASS_NARROWEST_MODE(MODE_UFRACT); mode != VOIDmode;
       mode = GET_MODE_WIDER_MODE(mode)) {
    FCONST0(mode).data.high = 0;
    FCONST0(mode).data.low = 0;
    FCONST0(mode).mode = mode;
  ...
  for (mode = GET_CLASS_NARROWEST_MODE(MODE_ACCUM); mode != VOIDmode;
       mode = GET_MODE_WIDER_MODE(mode)) {
    FCONST0(mode).data.high = 0;
    FCONST0(mode).data.low = 0;
    FCONST0(mode).mode = mode;
    ...
    lshift_double(1, 0, GET_MODE_FBIT(mode), 2 * HOST_BITS_PER_WIDE_INT,
                  &FCONST1(mode).data.low, &FCONST1(mode).data.high,
                  SIGNED_FIXED_POINT_MODE_P(mode));
    const_tiny_rtx[1][(int)mode] =
        CONST_FIXED_FROM_FIXED_VALUE(FCONST1(mode), mode);
  }

  for (mode = GET_CLASS_NARROWEST_MODE(MODE_UACCUM); mode != VOIDmode;
       mode = GET_MODE_WIDER_MODE(mode)) {
    FCONST0(mode).data.high = 0;
    FCONST0(mode).data.low = 0;
    FCONST0(mode).mode = mode;
  ...
  for (mode = GET_CLASS_NARROWEST_MODE(MODE_VECTOR_FRACT); mode != VOIDmode;
       mode = GET_MODE_WIDER_MODE(mode)) {
    const_tiny_rtx[0][(int)mode] = gen_const_vector(mode, 0);
  }

  for (mode = GET_CLASS_NARROWEST_MODE(MODE_VECTOR_UFRACT); mode != VOIDmode;
       mode = GET_MODE_WIDER_MODE(mode)) {
    const_tiny_rtx[0][(int)mode] = gen_const_vector(mode, 0);
  }

  for (mode = GET_CLASS_NARROWEST_MODE(MODE_VECTOR_ACCUM); mode != VOIDmode;
       mode = GET_MODE_WIDER_MODE(mode)) {
    const_tiny_rtx[0][(int)mode] = gen_const_vector(mode, 0);
    const_tiny_rtx[1][(int)mode] = gen_const_vector(mode, 1);
  }

  for (mode = GET_CLASS_NARROWEST_MODE(MODE_VECTOR_UACCUM); mode != VOIDmode;
       mode = GET_MODE_WIDER_MODE(mode)) {
    const_tiny_rtx[0][(int)mode] = gen_const_vector(mode, 0);
    const_tiny_rtx[1][(int)mode] = gen_const_vector(mode, 1);
  }
  ...
  for (link = REG_NOTES(insn); link; link = XEXP(link, 1))
    if (REG_NOTE_KIND(link) != REG_LABEL_OPERAND) {
      if (GET_CODE(link) == EXPR_LIST)
        add_reg_note(new_rtx, REG_NOTE_KIND(link), copy_insn_1(XEXP(link, 0)));
      else
