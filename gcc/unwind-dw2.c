...
static inline _Unwind_Word _Unwind_IsSignalFrame(
    struct _Unwind_Context *context) {
  return (context->flags & SIGNAL_FRAME_BIT) ? 1 : 0;
}

static inline void _Unwind_SetSignalFrame(struct _Unwind_Context *context,
                                          int val) {
  if (val)
...
static inline _Unwind_Word _Unwind_IsExtendedContext(
    struct _Unwind_Context *context) {
...
inline void _Unwind_SetGR(struct _Unwind_Context *context, int index,
                          _Unwind_Word val) {
  int size;
  void *ptr;
...
static inline void *_Unwind_GetGRPtr(struct _Unwind_Context *context,
                                     int index) {
  index = DWARF_REG_TO_UNWIND_COLUMN(index);
  if (_Unwind_IsExtendedContext(context) && context->by_value[index])
...
static inline void _Unwind_SetGRPtr(struct _Unwind_Context *context, int index,
                                    void *p) {
  index = DWARF_REG_TO_UNWIND_COLUMN(index);
...
static inline void _Unwind_SetGRValue(struct _Unwind_Context *context,
                                      int index, _Unwind_Word val) {
  index = DWARF_REG_TO_UNWIND_COLUMN(index);
  gcc_assert(index < (int)sizeof(dwarf_reg_size_table));
...
static inline int _Unwind_GRByValue(struct _Unwind_Context *context,
                                    int index) {
  index = DWARF_REG_TO_UNWIND_COLUMN(index);
...
static void execute_cfa_program(const unsigned char *insn_ptr,
  ...
  while (insn_ptr < insn_end &&
    ...
    if ((insn & 0xc0) == DW_CFA_advance_loc)
      ...
      fs->regs.reg[DWARF_REG_TO_UNWIND_COLUMN(reg)].how = REG_SAVED_OFFSET;
      ...
      switch (insn) {
        case DW_CFA_set_loc: {
          ...
          insn_ptr =
              read_encoded_value(context, fs->fde_encoding, insn_ptr, &pc);
          fs->pc = (void *)pc;
        } break;
        ...
        case DW_CFA_val_expression:
          insn_ptr = read_uleb128(insn_ptr, &reg);
          fs->regs.reg[DWARF_REG_TO_UNWIND_COLUMN(reg)].how = REG_SAVED_VAL_EXP;
          fs->regs.reg[DWARF_REG_TO_UNWIND_COLUMN(reg)].loc.exp = insn_ptr;
          insn_ptr = read_uleb128(insn_ptr, &utmp);
          insn_ptr += utmp;
          break;
...
static inline void _Unwind_SetSpColumn(struct _Unwind_Context *context,
                                       void *cfa, _Unwind_SpTmp *tmp_sp) {
...
static void uw_update_context_1(struct _Unwind_Context *context,
  ...
  switch (fs->regs.cfa_how) {
    ...
    case CFA_EXP: {
      ...
      cfa = (void *)(_Unwind_Ptr)execute_stack_op(exp, exp + len, &orig_context,
                                                  0);
      break;
    }
    ...
    switch (fs->regs.reg[i].how) {
      ...
      case REG_SAVED_REG:
        if (_Unwind_GRByValue(&orig_context, fs->regs.reg[i].loc.reg))
          _Unwind_SetGRValue(
              context, i,
              _Unwind_GetGR(&orig_context, fs->regs.reg[i].loc.reg));
        else
          _Unwind_SetGRPtr(
              context, i,
      ...
      case REG_SAVED_VAL_OFFSET:
        _Unwind_SetGRValue(
            context, i,
            (_Unwind_Internal_Ptr)(cfa + fs->regs.reg[i].loc.offset));
        break;
...
static void uw_init_context_1(struct _Unwind_Context *context, void *outer_cfa,
                              void *outer_ra) {
...
#if defined(USE_GAS_SYMVER) && defined(SHARED) && \
    defined(USE_LIBUNWIND_EXCEPTIONS)
