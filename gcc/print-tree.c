...
void print_node_brief(FILE *file, const char *prefix, const_tree node,
                      int indent) {
  ...
  if (tclass == tcc_declaration) {
    ...
    else if (TREE_CODE(node) == LABEL_DECL && LABEL_DECL_UID(node) != -1)
...
void print_node(FILE *file, const char *prefix, tree node, int indent) {
  int hash;
  struct bucket *b;
  ...
  if (tclass == tcc_declaration) {
    if (DECL_NAME(node))
      fprintf(file, " %s", IDENTIFIER_POINTER(DECL_NAME(node)));
    else if (code == LABEL_DECL && LABEL_DECL_UID(node) != -1)
      fprintf(file, " L.%d", (int)LABEL_DECL_UID(node));
    else
      fprintf(file, " %c.%u", code == CONST_DECL ? 'C' : 'D', DECL_UID(node));
  } else if (tclass == tcc_type) {
    if (TYPE_NAME(node)) {
      if (TREE_CODE(TYPE_NAME(node)) == IDENTIFIER_NODE)
        fprintf(file, " %s", IDENTIFIER_POINTER(TYPE_NAME(node)));
  ...
  switch (TREE_CODE_CLASS(code)) {
    case tcc_declaration:
      ...
      if (CODE_CONTAINS_STRUCT(code, TS_DECL_WRTL) && code != LABEL_DECL &&
          code != FUNCTION_DECL && DECL_REGISTER(node))
        fputs(" regdecl", file);

      if (code == TYPE_DECL && TYPE_DECL_SUPPRESS_DEBUG(node))
        fputs(" suppress-debug", file);

      if (code == FUNCTION_DECL && DECL_FUNCTION_SPECIFIC_TARGET(node))
        fputs(" function-specific-target", file);
      if (code == FUNCTION_DECL && DECL_FUNCTION_SPECIFIC_OPTIMIZATION(node))
      ...
      if (code == FIELD_DECL) {
        ...
        print_node(file, "bit offset", DECL_FIELD_BIT_OFFSET(node), indent + 4);
      ...
      if (CODE_CONTAINS_STRUCT(code, TS_DECL_COMMON)) {
        print_node_brief(file, "attributes", DECL_ATTRIBUTES(node), indent + 4);
        if (code != PARM_DECL)
          print_node_brief(file, "initial", DECL_INITIAL(node), indent + 4);
      }
      if (CODE_CONTAINS_STRUCT(code, TS_DECL_WRTL)) {
        print_node_brief(file, "abstract_origin", DECL_ABSTRACT_ORIGIN(node),
                         indent + 4);
      }
      ...
      } else if (code == FUNCTION_DECL && DECL_STRUCT_FUNCTION(node) != 0) {
        indent_to(file, indent + 4);
    ...
    case tcc_type:
      ...
      else if (code == INTEGER_TYPE && TYPE_IS_SIZETYPE(node))
        fputs(" sizetype", file);
      ...
      else if (code == ARRAY_TYPE && TYPE_NONALIASED_COMPONENT(node))
      ...
      if (code == ENUMERAL_TYPE)
        ...
        if (TYPE_METHOD_BASETYPE(node))
          print_node_brief(file, "method basetype", TYPE_METHOD_BASETYPE(node),
                           indent + 4);
    ...
    case tcc_exceptional:
      switch (code) {
        ...
        case BLOCK:
          ...
          print_node(file, "abstract_origin", BLOCK_ABSTRACT_ORIGIN(node),
                     indent + 4);
          break;
        ...
        case SSA_NAME:
          ...
          if (SSA_NAME_PTR_INFO(node) || SSA_NAME_VALUE(node)) {
            indent_to(file, indent + 3);
        ...
        case OMP_CLAUSE: {
          int i;
          fprintf(file, " %s", omp_clause_code_name[OMP_CLAUSE_CODE(node)]);
        ...
        case IMPORTED_DECL:
          ...
          print_node_brief(file, "associated declaration",
                           IMPORTED_DECL_ASSOCIATED_DECL(node), indent + 4);
          break;
