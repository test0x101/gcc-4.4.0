...
#define FOR_EACH_ACTUAL_CHILD(CHILD, ELT)                               \
  for ((CHILD) = (ELT)->is_group ? next_child_for_group(NULL, (ELT))    \
                                 : (ELT)->children;                     \
       (CHILD);                                                         \
       (CHILD) = (ELT)->is_group ? next_child_for_group((CHILD), (ELT)) \
                                 : (CHILD)->sibling)

/* Helper function for above macro.  Return next child in group.  */
static struct sra_elt *next_child_for_group(struct sra_elt *child,
                                            struct sra_elt *group) {
...
static bool is_sra_candidate_decl(tree decl) {
  return DECL_P(decl) && bitmap_bit_p(sra_candidates, DECL_UID(decl));
}

/* Return true if TYPE is a scalar type.  */

static bool is_sra_scalar_type(tree type) {
  enum tree_code code = TREE_CODE(type);
  return (code == INTEGER_TYPE || code == REAL_TYPE || code == VECTOR_TYPE ||
          code == FIXED_POINT_TYPE || code == ENUMERAL_TYPE ||
          code == BOOLEAN_TYPE || code == POINTER_TYPE || code == OFFSET_TYPE ||
          code == REFERENCE_TYPE);
}
...
bool sra_type_can_be_decomposed_p(tree type) {
    ...
    case RECORD_TYPE: {
      ...
      for (t = TYPE_FIELDS(type); t; t = TREE_CHAIN(t))
        if (TREE_CODE(t) == FIELD_DECL) {
          ...
          if (DECL_BIT_FIELD(t) && INTEGRAL_TYPE_P(TREE_TYPE(t)) &&
              (tree_low_cst(DECL_SIZE(t), 1) != TYPE_PRECISION(TREE_TYPE(t))))
            goto fail;
...
static bool is_va_list_type(tree type) {
  tree h;
...
static bool decl_can_be_decomposed_p(tree var) {
...
static bool type_can_instantiate_all_elements(tree type) {
...
static bool can_completely_scalarize_p(struct sra_elt *elt) {
  struct sra_elt *c;
...
static hashval_t sra_hash_tree(tree t) {
  hashval_t h;
...
static hashval_t sra_elt_hash(const void *x) {
...
static int sra_elt_eq(const void *x, const void *y) {
  ...
  switch (TREE_CODE(ae)) {
    ...
    case RANGE_EXPR:
      return tree_int_cst_equal(TREE_OPERAND(ae, 0), TREE_OPERAND(be, 0)) &&
    ...
    case BIT_FIELD_REF:
      return tree_int_cst_equal(TREE_OPERAND(ae, 1), TREE_OPERAND(be, 1)) &&
...
static struct sra_elt *lookup_element(struct sra_elt *parent, tree child,
                                      tree type, enum insert_option insert) {
...
static struct sra_elt *maybe_lookup_element_for_expr(tree expr) {
  struct sra_elt *elt;
  tree child;
  ...
  switch (TREE_CODE(expr)) {
    ...
    case ARRAY_RANGE_REF:
      ...
      if (range_in_array_bounds_p(expr)) {
        ...
        child = build2(RANGE_EXPR, integer_type_node, TYPE_MIN_VALUE(domain),
...
struct sra_walk_fns {
  ...
  void (*use)(struct sra_elt *elt, tree *expr_p, gimple_stmt_iterator *gsi,
  ...
  void (*ldst)(struct sra_elt *elt, tree other, gimple_stmt_iterator *gsi,
               bool is_output);
...
static tree sra_find_candidate_decl(tree *tp, int *walk_subtrees,
...
static void sra_walk_expr(tree *expr_p, gimple_stmt_iterator *gsi,
                          bool is_output, const struct sra_walk_fns *fns) {
  tree expr = *expr_p;
  tree inner = expr;
...
static void sra_walk_gimple_call(gimple stmt, gimple_stmt_iterator *gsi,
...
static void sra_walk_gimple_asm(gimple stmt, gimple_stmt_iterator *gsi,
                                const struct sra_walk_fns *fns) {
  size_t i;
...
static void sra_walk_gimple_assign(gimple stmt, gimple_stmt_iterator *gsi,
                                   const struct sra_walk_fns *fns) {
  ...
  if (lhs_elt) {
    ...
    if (TREE_CODE(rhs) == COMPLEX_EXPR || TREE_CODE(rhs) == COMPLEX_CST ||
    ...
    else if (TREE_CODE(rhs) == VAR_DECL && TREE_STATIC(rhs) &&
             !DECL_EXTERNAL(rhs) && TREE_READONLY(rhs) &&
    ...
    else if (!lhs_elt->is_scalar && !TREE_SIDE_EFFECTS(rhs) &&
...
static void sra_walk_function(const struct sra_walk_fns *fns) {
  basic_block bb;
    ...
    switch (gimple_code(stmt)) {
      case GIMPLE_RETURN:
        ...
        if (gimple_return_retval(stmt) == NULL_TREE)
          ...
          sra_walk_expr(gimple_return_retval_ptr(stmt), &si, false, fns);
        break;
...
static bool find_candidates_for_sra(void) {
  bool any_set = false;
  tree var;
...
static void scan_use(struct sra_elt *elt, tree *expr_p ATTRIBUTE_UNUSED,
...
static void scan_copy(struct sra_elt *lhs_elt, struct sra_elt *rhs_elt,
                      gimple_stmt_iterator *gsi ATTRIBUTE_UNUSED) {
  lhs_elt->n_copies += 1;
  rhs_elt->n_copies += 1;
}

static void scan_init(struct sra_elt *lhs_elt, tree rhs ATTRIBUTE_UNUSED,
                      gimple_stmt_iterator *gsi ATTRIBUTE_UNUSED) {
  lhs_elt->n_copies += 1;
}

static void scan_ldst(struct sra_elt *elt, tree other ATTRIBUTE_UNUSED,
                      gimple_stmt_iterator *gsi ATTRIBUTE_UNUSED,
                      bool is_output ATTRIBUTE_UNUSED) {
  elt->n_copies += 1;
}
...
static void scan_dump(struct sra_elt *elt) {
  struct sra_elt *c;
...
static void scan_function(void) {
  static const struct sra_walk_fns fns = {scan_use, scan_copy, scan_init,
                                          scan_ldst, true};
  bitmap_iterator bi;
...
static void build_element_name_1(struct sra_elt *elt) {
  tree t;
  char buffer[32];
...
static char *build_element_name(struct sra_elt *elt) {
...
static void instantiate_element(struct sra_elt *elt) {
  ...
  if (DECL_P(elt->element) &&
    ...
    elt->replacement = fold_build3(
        BIT_FIELD_REF, elt->type, var, DECL_SIZE(var),
        BYTES_BIG_ENDIAN
            ? size_binop(MINUS_EXPR, TYPE_SIZE(elt->type), DECL_SIZE(var))
            : bitsize_int(0));
  }
  ...
  if (TREE_CODE(elt->element) == BIT_FIELD_REF ||
    ...
    gimple_seq init = sra_build_assignment(
        var, fold_convert(TREE_TYPE(var), integer_zero_node));
...
static void decide_instantiation_1(struct sra_elt *elt,
...
static unsigned int sum_instantiated_sizes(struct sra_elt *elt,
                                           unsigned HOST_WIDE_INT *sizep) {
  if (elt->replacement) {
...
static struct sra_elt *instantiate_missing_elements_1(struct sra_elt *elt,
                                                      tree child, tree type) {
...
static tree canon_type_for_field(tree f, tree element) {
  ...
  if (INTEGRAL_TYPE_P(field_type) && DECL_MODE(f) != TYPE_MODE(field_type))
    field_type = TREE_TYPE(get_unwidened(
        build3(COMPONENT_REF, field_type, element, f, NULL_TREE), NULL_TREE));
...
static tree try_instantiate_multiple_fields(struct sra_elt *elt, tree f) {
  int count;
  ...
  for (prev = f, f = TREE_CHAIN(f);
       f && TREE_CODE(f) == FIELD_DECL && is_sra_scalar_type(TREE_TYPE(f)) &&
       host_integerp(DECL_FIELD_OFFSET(f), 1) &&
       host_integerp(DECL_FIELD_BIT_OFFSET(f), 1) &&
       host_integerp(DECL_SIZE(f), 1) &&
       !lookup_element(elt, f, NULL, NO_INSERT);
    ...
    if (bit + size == nbit) {
      if ((bit & alchk) != ((nbit + nsize - 1) & alchk)) {
        /* If we're at an alignment boundary, don't bother
           ...
           field.  */
        if ((nbit & alchk) || GET_MODE_BITSIZE(DECL_MODE(f)) <= align)
          break;
      ...
      if ((nbit & alchk) != ((bit + size - 1) & alchk)) {
        if ((bit & alchk) || GET_MODE_BITSIZE(DECL_MODE(f)) <= align)
          break;
  ...
  if ((bit & ~alchk) || size != align) {
    ...
    for (f = TYPE_FIELDS(elt->type); f; f = TREE_CHAIN(f)) {
      ...
      if ((fbit & alchk) < (bit & alchk)) {
        ...
        if (fbit + fsize <= mbit || mbit + msize <= fbit)
          continue;
  ...
  gcc_assert(type);
  var = build3(BIT_FIELD_REF, type, NULL_TREE, bitsize_int(size),
               bitsize_int(bit));
  ...
  for (f = first; f != TREE_CHAIN(prev); f = TREE_CHAIN(f)) {
    ...
    fld->replacement = fold_build3(
        BIT_FIELD_REF, field_type, var, bitsize_int(TYPE_PRECISION(field_type)),
        bitsize_int((TREE_INT_CST_LOW(DECL_FIELD_OFFSET(f)) * BITS_PER_UNIT +
                     (TREE_INT_CST_LOW(DECL_FIELD_BIT_OFFSET(f))) -
                     (TREE_INT_CST_LOW(TREE_OPERAND(block->element, 2)))) &
                    ~alchk));
      ...
      for (f = TYPE_FIELDS(type); f; f = TREE_CHAIN(f))
        if (TREE_CODE(f) == FIELD_DECL) {
          ...
          instantiate_missing_elements_1(elt, f,
                                         canon_type_for_field(f, elt->element));
        }
      break;
    }
...
static bool single_scalar_field_in_record_p(tree type) {
  int num_fields = 0;
  tree field;
  if (TREE_CODE(type) != RECORD_TYPE)
    return false;
...
static bool decide_block_copy(struct sra_elt *elt) {
  struct sra_elt *c;
  bool any_inst;
  ...
  if (elt->n_uses == 0 && elt->n_copies == 0 && elt->groups == NULL)
    ...
    /* Don't bother trying to figure out the rest if the structure is
      ...
      if (full_size <= max_size && (full_count - inst_count) <= max_count &&
      ...
      if (!use_block_copy && (!can_completely_scalarize_p(elt) ||
...
static void decide_instantiations(void) {
  unsigned int i;
  bool cleared_any;
...
static void mark_all_v_defs_stmt(gimple stmt) {
  tree sym;
  ssa_op_iter iter;
...
static void mark_all_v_defs_seq(gimple_seq seq) {
  gimple_stmt_iterator gsi;
...
static void mark_no_warning(struct sra_elt *elt) {
...
static tree generate_one_element_ref(struct sra_elt *elt, tree base) {
...
static tree generate_element_ref(struct sra_elt *elt) {
  if (elt->parent)
...
static bool scalar_bitfield_p(tree bf) {
          ...
          (is_gimple_reg(TREE_OPERAND(bf, 0)) ||
           (TYPE_MODE(TREE_TYPE(TREE_OPERAND(bf, 0))) != BLKmode &&
            (!TREE_SIDE_EFFECTS(TREE_OPERAND(bf, 0)) ||
             (GET_MODE_BITSIZE(TYPE_MODE(TREE_TYPE(TREE_OPERAND(bf, 0)))) <=
              BITS_PER_WORD)))));
}
...
static gimple_seq sra_build_assignment(tree dst, tree src) {
  ...
  if (scalar_bitfield_p(src)) {
    ...
    tree utype, stype;
    bool unsignedp =
        (INTEGRAL_TYPE_P(TREE_TYPE(src)) ? TYPE_UNSIGNED(TREE_TYPE(src))
                                         : true);
    ...
    if (!INTEGRAL_TYPE_P(stype))
      stype =
          lang_hooks.types.type_for_size(TREE_INT_CST_LOW(TYPE_SIZE(stype)), 1);
    ...
    if (!INTEGRAL_TYPE_P(utype))
      utype =
          lang_hooks.types.type_for_size(TREE_INT_CST_LOW(TYPE_SIZE(utype)), 1);
    ...
    if (!unsignedp) {
      tree signbit =
          int_const_binop(LSHIFT_EXPR, build_int_cst_wide(utype, 1, 0),
                          size_binop(MINUS_EXPR, width, bitsize_int(1)), 0);
  ...
  if (!handled_component_p(src) && !SSA_VAR_P(src)) {
...
static gimple_seq sra_build_bf_assignment(tree dst, tree src) {
  ...
  cst2 = size_binop(PLUS_EXPR, fold_convert(bitsizetype, TREE_OPERAND(dst, 1)),
                    cst);
  ...
  if (!integer_zerop(mask)) {
    ...
    stmt =
        gimple_build_assign(tmp, fold_build2(BIT_AND_EXPR, utype, tmp2, mask));
    gimple_seq_add_stmt(&seq, stmt);
  } else
    tmp = mask;
  ...
  if (is_gimple_reg(src) && INTEGRAL_TYPE_P(TREE_TYPE(src)))
    ...
    gimple_seq tmp_seq;
    tmp2 = make_rename_temp(lang_hooks.types.type_for_size(
                                TREE_INT_CST_LOW(TYPE_SIZE(TREE_TYPE(src))), 1),
                            "SR");
    tmp_seq = sra_build_assignment(
        tmp2, fold_build1(VIEW_CONVERT_EXPR, TREE_TYPE(tmp2), src));
    gimple_seq_add_seq(&seq, tmp_seq);
  }
  ...
  if (!integer_zerop(minshift)) {
    tmp3 = make_rename_temp(utype, "SR");
    stmt = gimple_build_assign(tmp3,
                               fold_build2(LSHIFT_EXPR, utype, tmp2, minshift));
    gimple_seq_add_stmt(&seq, stmt);
    tmp2 = tmp3;
  }
  ...
  if (utype != TREE_TYPE(var))
    ...
    tmp3 = var;
  stmt = gimple_build_assign(tmp3, fold_build2(BIT_IOR_EXPR, utype, tmp, tmp2));
  gimple_seq_add_stmt(&seq, stmt);
  ...
  if (tmp3 != var) {
    if (TREE_TYPE(var) == type)
      ...
      stmt = gimple_build_assign(
          var, fold_build1(VIEW_CONVERT_EXPR, TREE_TYPE(var), tmp3));
    gimple_seq_add_stmt(&seq, stmt);
  }
...
static gimple_seq sra_build_elt_assignment(struct sra_elt *elt, tree src) {
  tree dst = elt->replacement;
  tree var, tmp, cst, cst2;
  gimple stmt;
  gimple_seq seq;

  if (TREE_CODE(dst) != BIT_FIELD_REF || !elt->in_bitfld_block)
    return sra_build_assignment(REPLDUP(dst), src);
  ...
  if (elt->in_bitfld_block == 2 && TREE_CODE(src) == BIT_FIELD_REF) {
    tmp = src;
    cst = TYPE_SIZE(TREE_TYPE(var));
    cst2 = size_binop(MINUS_EXPR, TREE_OPERAND(src, 2), TREE_OPERAND(dst, 2));
    ...
    if (integer_zerop(cst2) &&
      ...
      if (INTEGRAL_TYPE_P(TREE_TYPE(src)) && !TYPE_UNSIGNED(TREE_TYPE(src)))
      ...
      if (TYPE_MAIN_VARIANT(TREE_TYPE(var)) !=
        ...
        tmp_seq = sra_build_assignment(var, fold_convert(TREE_TYPE(var), tmp));
        gimple_seq_add_seq(&seq, tmp_seq);
...
static void generate_copy_inout(struct sra_elt *elt, bool copy_out, tree expr,
                                gimple_seq *seq_p) {
  struct sra_elt *c;
  gimple_seq tmp_seq;
  tree t;
...
static void generate_element_copy(struct sra_elt *dst, struct sra_elt *src,
                                  gimple_seq *seq_p) {
    ...
    if (!sc) {
      ...
      for (f = TYPE_FIELDS(src->type); f; f = TREE_CHAIN(f))
        if (simple_cst_equal(DECL_FIELD_OFFSET(f),
            ...
            simple_cst_equal(DECL_SIZE(f), DECL_SIZE(dc->element)) > 0 &&
            (useless_type_conversion_p(TREE_TYPE(dc->element), TREE_TYPE(f)) ||
...
static void generate_element_zero(struct sra_elt *elt, gimple_seq *seq_p) {
  struct sra_elt *c;
...
static void generate_one_element_init(struct sra_elt *elt, tree init,
                                      gimple_seq *seq_p) {
...
static bool generate_element_init_1(struct sra_elt *elt, tree init,
                                    gimple_seq *seq_p) {
  bool result = true;
    ...
    case COMPLEX_EXPR:
      FOR_EACH_ACTUAL_CHILD(sub, elt) {
        if (sub->element == integer_zero_node)
          t = (init_code == COMPLEX_EXPR ? TREE_OPERAND(init, 0)
                                         : TREE_REALPART(init));
        else
          t = (init_code == COMPLEX_EXPR ? TREE_OPERAND(init, 1)
                                         : TREE_IMAGPART(init));
        ...
        if (TREE_CODE(purpose) == RANGE_EXPR) {
          ...
          while (1) {
            ...
            if (tree_int_cst_equal(lower, upper))
              break;
            lower = int_const_binop(PLUS_EXPR, lower, integer_one_node, true);
          }
        } else {
...
static bool generate_element_init(struct sra_elt *elt, tree init,
                                  gimple_seq *seq_p) {
  bool ret;
  struct gimplify_ctx gctx;
...
void insert_edge_copies_seq(gimple_seq seq, basic_block bb) {
  edge e;
  edge_iterator ei;
  unsigned n_copies = -1;
...
void sra_insert_before(gimple_stmt_iterator *gsi, gimple_seq seq) {
...
void sra_insert_after(gimple_stmt_iterator *gsi, gimple_seq seq) {
  gimple stmt = gsi_stmt(*gsi);
...
static void sra_replace(gimple_stmt_iterator *gsi, gimple_seq seq) {
...
static bool bitfield_overlaps_p(tree blen, tree bpos, struct sra_elt *fld,
  ...
  if (TREE_CODE(fld->element) == FIELD_DECL) {
    ...
    if (domain_type && TYPE_MIN_VALUE(domain_type))
      fpos = size_binop(MINUS_EXPR, fpos,
                        fold_convert(bitsizetype, TYPE_MIN_VALUE(domain_type)));
    fpos = size_binop(MULT_EXPR, flen, fpos);
  } else
    gcc_unreachable();

  gcc_assert(host_integerp(blen, 1) && host_integerp(bpos, 1) &&
             host_integerp(flen, 1) && host_integerp(fpos, 1));

  ret = ((!tree_int_cst_lt(fpos, bpos) &&
          tree_int_cst_lt(size_binop(MINUS_EXPR, fpos, bpos), blen)) ||
         (!tree_int_cst_lt(bpos, fpos) &&
          tree_int_cst_lt(size_binop(MINUS_EXPR, bpos, fpos), flen)));
  ...
  if (data) {
    ...
    if (tree_int_cst_lt(fpos, bpos)) {
      ...
      data->overlap_len = size_binop(
          MINUS_EXPR, data->overlap_len ? data->overlap_len : data->field_len,
          data->overlap_pos);
    } else
...
static void sra_explode_bitfield_assignment(tree var, tree vpos, bool to_var,
                                            gimple_seq *seq_p, tree blen,
                                            tree bpos, struct sra_elt *elt) {
  struct sra_elt *fld;
  ...
  FOR_EACH_ACTUAL_CHILD(fld, elt) {
    ...
    if (fld->replacement) {
      ...
      if (TREE_CODE(infld) == BIT_FIELD_REF) {
        ...
        fpos = size_binop(PLUS_EXPR, fpos, TYPE_SIZE(TREE_TYPE(infld)));
        fpos = size_binop(MINUS_EXPR, fpos, DECL_SIZE(fld->element));
      }
      ...
      sra_explode_bitfield_assignment(var, sub, to_var, seq_p, flen, fpos, fld);
    }
  }
}
...
static void sra_sync_for_bitfield_assignment(gimple_seq *seq_before_p,
                                             gimple_seq *seq_after_p, tree blen,
                                             tree bpos, struct sra_elt *elt) {
  struct sra_elt *fld;
  struct bitfield_overlap_info flp;
  ...
  if (bitfield_overlaps_p(blen, bpos, fld, &flp)) {
    if (fld->replacement || (!flp.overlap_len && !flp.overlap_pos)) {
      generate_copy_inout(fld, false, generate_element_ref(fld), seq_before_p);
      mark_no_warning(fld);
      if (seq_after_p)
        generate_copy_inout(fld, true, generate_element_ref(fld), seq_after_p);
    } else {
      tree flen = flp.overlap_len ? flp.overlap_len : flp.field_len;
      tree fpos = flp.overlap_pos ? flp.overlap_pos : bitsize_int(0);

      sra_sync_for_bitfield_assignment(seq_before_p, seq_after_p, flen, fpos,
                                       fld);
    }
  }
}
...
static void scalarize_use(struct sra_elt *elt, tree *expr_p,
  ...
  if (elt->replacement) {
    ...
    if (is_output && TREE_CODE(elt->replacement) == BIT_FIELD_REF &&
        is_gimple_reg(TREE_OPERAND(elt->replacement, 0)) &&
        is_gimple_assign(stmt) && gimple_assign_lhs_ptr(stmt) == expr_p) {
      gimple_seq newseq;
      ...
      return;
    } else if (!is_output && TREE_CODE(elt->replacement) == BIT_FIELD_REF &&
               is_gimple_assign(stmt) &&
               gimple_assign_rhs1_ptr(stmt) == expr_p) {
      tree tmp = make_rename_temp(TREE_TYPE(gimple_assign_lhs(stmt)), "SR");
    ...
    *expr_p = REPLDUP(replacement);
    update_stmt(stmt);
  } else if (use_all && is_output && is_gimple_assign(stmt) &&
             TREE_CODE(bfexpr = gimple_assign_lhs(stmt)) == BIT_FIELD_REF &&
    ...
    if (!elt->use_block_copy) {
      ...
      if (BYTES_BIG_ENDIAN)
        ...
        vpos = bitsize_int(0);
      sra_explode_bitfield_assignment(var, vpos, false, &seq_after, blen, bpos,
                                      elt);
    } else
      sra_sync_for_bitfield_assignment(&seq_before, &seq_after, blen, bpos,
                                       elt);
    ...
    if (update)
      update_stmt(stmt);
             ...
             TREE_CODE(bfexpr = gimple_assign_rhs1(stmt)) == BIT_FIELD_REF &&
    ...
    if (!elt->use_block_copy) {
      ...
      if (!TYPE_UNSIGNED(type)) {
        type = unsigned_type_for(type);
        tmp = make_rename_temp(type, "SR");
        st = gimple_build_assign(var, fold_convert(TREE_TYPE(var), tmp));
        var = tmp;
      }

      gimple_seq_add_stmt(
          &seq, gimple_build_assign(var, build_int_cst_wide(type, 0, 0)));
      ...
      if (BYTES_BIG_ENDIAN)
        ...
        vpos = bitsize_int(0);
      sra_explode_bitfield_assignment(var, vpos, true, &seq, blen, bpos, elt);

      if (st)
        gimple_seq_add_stmt(&seq, st);
    } else
      sra_sync_for_bitfield_assignment(&seq, NULL, blen, bpos, elt);
...
static void scalarize_copy(struct sra_elt *lhs_elt, struct sra_elt *rhs_elt,
                           gimple_stmt_iterator *gsi) {
  gimple_seq seq;
  gimple stmt;
  ...
  if (lhs_elt->replacement && rhs_elt->replacement) {
    ...
    seq = NULL;
    generate_copy_inout(rhs_elt, false, generate_element_ref(rhs_elt), &seq);
    if (seq) {
      mark_all_v_defs_seq(seq);
      sra_insert_before(gsi, seq);
    }
    ...
    seq = NULL;
    generate_copy_inout(lhs_elt, true, generate_element_ref(lhs_elt), &seq);
    if (seq) {
      mark_all_v_defs_seq(seq);
      sra_insert_after(gsi, seq);
    }
...
static void scalarize_init(struct sra_elt *lhs_elt, tree rhs,
                           gimple_stmt_iterator *gsi) {
  bool result = true;
  ...
  if (!result) {
    ...
    generate_copy_inout(lhs_elt, true, generate_element_ref(lhs_elt), &seq0);
...
static tree mark_notrap(tree *tp, int *walk_subtrees,
                        void *data ATTRIBUTE_UNUSED) {
  tree t = *tp;
...
static void scalarize_ldst(struct sra_elt *elt, tree other,
                           gimple_stmt_iterator *gsi, bool is_output) {
...
static void scalarize_parms(void) {
  gimple_seq seq = NULL;
  unsigned i;
  bitmap_iterator bi;
...
static void scalarize_function(void) {
...
static void dump_sra_elt_name(FILE *f, struct sra_elt *elt) {
...
static unsigned int tree_sra(void) {
...
static unsigned int tree_sra_early(void) {
  unsigned int ret;
...
static bool gate_sra(void) {
  return flag_tree_sra != 0;
}

struct gimple_opt_pass pass_sra_early = {{
    GIMPLE_PASS, "esra", /* name */
    gate_sra,            /* gate */
