...
static struct varray_descriptor *varray_descriptor(const char *name) {
  struct varray_descriptor **slot;

  if (!varray_hash)
    varray_hash = htab_create(10, hash_descriptor, eq_descriptor, NULL);

  slot = (struct varray_descriptor **)htab_find_slot_with_hash(
      varray_hash, name, htab_hash_pointer(name), 1);
  if (*slot)
    return *slot;
...
varray_type varray_init(size_t num_elements, enum varray_data_enum element_kind,
                        const char *name) {
...
varray_type varray_grow(varray_type va, size_t n) {
  ...
  if (n != old_elements) {
    ...
    if (element[va->type].uses_ggc)
      ...
      va = XRESIZEVAR(struct varray_head_tag, va, VARRAY_HDR_SIZE + data_size);
    va->num_elements = n;
    if (n > old_elements)
...
void varray_check_failed(varray_type va, size_t n, const char *file, int line,
                         const char *function) {
  internal_error(
      "virtual array %s[%lu]: element %lu out of bounds "
      "in %s, at %s:%d",
      va->name, (unsigned long)va->num_elements, (unsigned long)n, function,
      trim_filename(file), line);
}

void varray_underflow(varray_type va, const char *file, int line,
                      const char *function) {
  internal_error("underflowed virtual array %s in %s, at %s:%d", va->name,
...
static int print_statistics(void **slot, void *b) {
  ...
  if (d->allocated) {
    fprintf(stderr, "%-21s %6d %10d %7d %7d\n", d->name, d->created,
            d->allocated, d->resized, d->copied);
    i->size += d->allocated;
    i->count += d->created;
  }
  return 1;
