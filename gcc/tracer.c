...
static inline void mark_bb_seen(basic_block bb) {
...
static inline bool bb_seen_p(basic_block bb) {
  return TEST_BIT(bb_seen, bb->index);
}

/* Return true if we should ignore the basic block for purposes of tracing.  */
static bool ignore_bb_p(const_basic_block bb) {
...
static bool better_p(const_edge e1, const_edge e2) {
...
static void tail_duplicate(void) {
  ...
  while (traced_insns < cover_insns && nduplicated < max_dup_insns &&
    ...
    for (pos = 1; pos < n; pos++) {
      ...
      if (EDGE_COUNT(bb2->preds) > 1 && can_duplicate_block_p(bb2)) {
        edge e;
        basic_block copy;
        ...
        /* Reconsider the original copy of block we've duplicated.
           ...
           head.  */
        blocks[bb2->index] = fibheap_insert(heap, -bb2->frequency, bb2);

        if (dump_file)
          fprintf(dump_file, "Duplicated %i as %i [%i]\n", bb2->index,
                  copy->index, copy->frequency);
...
static unsigned int tracer(void) {
...
static bool gate_tracer(void) {
  return (optimize > 0 && flag_tracer && flag_reorder_blocks);
}

struct gimple_opt_pass pass_tracer = {{
    GIMPLE_PASS, "tracer", /* name */
    gate_tracer,           /* gate */
    tracer,                /* execute */
    NULL,                  /* sub */
    NULL,                  /* next */
    ...
    0,                     /* todo_flags_start */
    TODO_dump_func | TODO_update_ssa | TODO_verify_ssa /* todo_flags_finish */
}};
