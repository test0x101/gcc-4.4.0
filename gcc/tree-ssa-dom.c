...
/* Local functions.  */
static void optimize_stmt(struct dom_walk_data *, basic_block,
...
static void initialize_hash_element(gimple stmt, tree lhs,
...
static void initialize_expr_from_cond(tree cond, struct hashable_expr *expr) {
  expr->type = boolean_type_node;
...
static bool hashable_expr_equal_p(const struct hashable_expr *expr0,
  ...
  if (type0 != type1 &&
      (TREE_CODE(type0) == ERROR_MARK || TREE_CODE(type1) == ERROR_MARK ||
  ...
  switch (expr0->kind) {
    case EXPR_SINGLE:
      return operand_equal_p(expr0->ops.single.rhs, expr1->ops.single.rhs, 0);
    ...
    case EXPR_UNARY:
      ...
      return operand_equal_p(expr0->ops.unary.opnd, expr1->ops.unary.opnd, 0);

    case EXPR_BINARY: {
      if (expr0->ops.binary.op != expr1->ops.binary.op)
        return false;

      if (operand_equal_p(expr0->ops.binary.opnd0, expr1->ops.binary.opnd0,
                          0) &&
          operand_equal_p(expr0->ops.binary.opnd1, expr1->ops.binary.opnd1, 0))
        return true;

      /* For commutative ops, allow the other order.  */
      return (
          commutative_tree_code(expr0->ops.binary.op) &&
          operand_equal_p(expr0->ops.binary.opnd0, expr1->ops.binary.opnd1,
                          0) &&
          operand_equal_p(expr0->ops.binary.opnd1, expr1->ops.binary.opnd0, 0));
    }
    ...
    case EXPR_CALL: {
      ...
      if (!operand_equal_p(expr0->ops.call.fn, expr1->ops.call.fn, 0))
        return false;
      ...
      for (i = 0; i < expr0->ops.call.nargs; i++)
        if (!operand_equal_p(expr0->ops.call.args[i], expr1->ops.call.args[i],
                             0))
          return false;
...
static hashval_t iterative_hash_hashable_expr(const struct hashable_expr *expr,
                                              hashval_t val) {
  switch (expr->kind) {
    case EXPR_SINGLE:
...
static void print_expr_hash_elt(FILE *stream,
                                const struct expr_hash_elt *element) {
  if (element->stmt)
    fprintf(stream, "STMT ");
  else
    fprintf(stream, "COND ");
...
static void free_expr_hash_elt(void *elt) {
...
static struct edge_info *allocate_edge_info(edge e) {
...
static unsigned int tree_ssa_dominator_optimize(void) {
  ...
  avail_exprs = htab_create(1024, real_avail_expr_hash, avail_expr_eq,
                            free_expr_hash_elt);
  ...
  statistics_counter_event(cfun, "Copies propagated", opt_stats.num_copy_prop);
...
static bool gate_dominator(void) {
  return flag_tree_dom != 0;
}

struct gimple_opt_pass pass_dominator = {{
    GIMPLE_PASS, "dom",               /* name */
    gate_dominator,                   /* gate */
    ...
    0,                                /* todo_flags_start */
    TODO_dump_func | TODO_update_ssa | TODO_cleanup_cfg |
...
static void canonicalize_comparison(gimple condstmt) {
  tree op0;
  tree op1;
  enum tree_code code;
...
static void dom_opt_initialize_block(
    struct dom_walk_data *walk_data ATTRIBUTE_UNUSED, basic_block bb) {
...
static tree simplify_stmt_for_jump_threading(
    gimple stmt, gimple within_stmt ATTRIBUTE_UNUSED) {
...
static void dom_thread_across_edge(struct dom_walk_data *walk_data, edge e) {
  if (!walk_data->global_data) {
    gimple dummy_cond = gimple_build_cond(NE_EXPR, integer_zero_node,
                                          integer_zero_node, NULL, NULL);
    walk_data->global_data = dummy_cond;
  }

  thread_across_edge((gimple)walk_data->global_data, e, false,
                     &const_and_copies_stack, simplify_stmt_for_jump_threading);
}
...
static void dom_opt_finalize_block(struct dom_walk_data *walk_data,
                                   basic_block bb) {
  gimple last;
  ...
  if (single_succ_p(bb) && (single_succ_edge(bb)->flags & EDGE_ABNORMAL) == 0 &&
      potentially_threadable_block(single_succ(bb))) {
    dom_thread_across_edge(walk_data, single_succ_edge(bb));
  } else if ((last = last_stmt(bb)) && gimple_code(last) == GIMPLE_COND &&
...
static edge single_incoming_edge_ignoring_loop_edges(basic_block bb) {
  edge retval = NULL;
  edge e;
  edge_iterator ei;
...
void dump_dominator_optimization_stats(FILE *file) {
...
static void record_cond(struct cond_equivalence *p) {
  struct expr_hash_elt *element = XCNEW(struct expr_hash_elt);
  void **slot;

  initialize_hash_element_from_expr(&p->cond, p->value, element);

  slot = htab_find_slot_with_hash(avail_exprs, (void *)element, element->hash,
                                  INSERT);
  if (*slot == NULL) {
    *slot = (void *)element;
...
static void build_and_record_new_cond(enum tree_code code, tree op0, tree op1,
...
static void record_conditions(struct edge_info *edge_info, tree cond,
                              tree inverted) {
  tree op0, op1;
  ...
  switch (TREE_CODE(cond)) {
    ...
    case GT_EXPR:
      ...
      build_and_record_new_cond(
          (TREE_CODE(cond) == LT_EXPR ? LE_EXPR : GE_EXPR), op0, op1,
    ...
    case UNGT_EXPR:
      ...
      edge_info->cond_equivalences = XNEWVEC(struct cond_equivalence, 4);
      build_and_record_new_cond(
          (TREE_CODE(cond) == UNLT_EXPR ? UNLE_EXPR : UNGE_EXPR), op0, op1,
          &edge_info->cond_equivalences[2]);
      build_and_record_new_cond(NE_EXPR, op0, op1,
                                &edge_info->cond_equivalences[3]);
      break;
...
static void record_const_or_copy_1(tree x, tree y, tree prev_x) {
  SSA_NAME_VALUE(x) = y;
...
static void record_const_or_copy(tree x, tree y) {
...
static void record_equality(tree x, tree y) {
...
static bool simple_iv_increment_p(gimple stmt) {
  tree lhs, preinc;
  gimple phi;
  size_t i;
...
static void cprop_into_successor_phis(basic_block bb) {
  edge e;
  edge_iterator ei;
...
static void record_edge_info(basic_block bb) {
  ...
  if (!gsi_end_p(gsi)) {
    ...
    if (gimple_code(stmt) == GIMPLE_SWITCH) {
      ...
      if (TREE_CODE(index) == SSA_NAME) {
        ...
        for (i = 0; i < n_labels; i++) {
          ...
          if (CASE_HIGH(label) || !CASE_LOW(label) || info[target_bb->index])
    ...
    if (gimple_code(stmt) == GIMPLE_COND) {
      ...
      if ((code == EQ_EXPR || code == NE_EXPR) && TREE_CODE(op0) == SSA_NAME &&
        ...
        if (code == EQ_EXPR) {
          ...
          edge_info->lhs = op0;
          edge_info->rhs =
              (integer_zerop(op1) ? boolean_false_node : boolean_true_node);

          edge_info = allocate_edge_info(false_edge);
          edge_info->lhs = op0;
          edge_info->rhs =
              (integer_zerop(op1) ? boolean_true_node : boolean_false_node);
        } else {
          edge_info = allocate_edge_info(true_edge);
          edge_info->lhs = op0;
          edge_info->rhs =
...
static void propagate_to_outgoing_edges(
    struct dom_walk_data *walk_data ATTRIBUTE_UNUSED, basic_block bb) {
  record_edge_info(bb);
...
static bool eliminate_redundant_computations(gimple_stmt_iterator *gsi) {
  tree expr_type;
  tree cached_lhs;
  bool insert = true;
  bool retval = false;
  ...
  if ((TREE_CODE(cached_lhs) != SSA_NAME &&
    ...
    if (TREE_CODE(cached_lhs) == ADDR_EXPR ||
        (POINTER_TYPE_P(expr_type) && is_gimple_min_invariant(cached_lhs)))
      retval = true;
...
static bool gimple_assign_unary_useless_conversion_p(gimple gs) {
...
static void record_equivalences_from_stmt(gimple stmt, int may_optimize_p) {
  tree lhs;
  ...
  if (!gimple_has_volatile_ops(stmt) && gimple_references_memory_p(stmt) &&
      gimple_assign_single_p(stmt) &&
...
static bool cprop_operand(gimple stmt, use_operand_p op_p) {
  ...
  if (val && val != op) {
    ...
    if (!is_gimple_reg(op) &&
        (TREE_CODE(val) != SSA_NAME || is_gimple_reg(val) ||
         get_virtual_var(val) != get_virtual_var(op)))
      return false;

    /* Do not replace hard register operands in asm statements.  */
    if (gimple_code(stmt) == GIMPLE_ASM && !may_propagate_copy_into_asm(op))
      return false;
    ...
    if (POINTER_TYPE_P(TREE_TYPE(val)) &&
        TYPE_VOLATILE(TREE_TYPE(TREE_TYPE(val))) && gimple_has_mem_ops(stmt) &&
    ...
    if (TREE_CODE(val) == ADDR_EXPR ||
        (POINTER_TYPE_P(TREE_TYPE(op)) && is_gimple_min_invariant(val)))
...
static bool cprop_into_stmt(gimple stmt) {
  bool may_have_exposed_new_symbols = false;
  use_operand_p op_p;
  ssa_op_iter iter;
...
static void optimize_stmt(struct dom_walk_data *walk_data ATTRIBUTE_UNUSED,
  ...
  may_optimize_p =
      (!gimple_has_volatile_ops(stmt) &&
       ((is_gimple_assign(stmt) && !gimple_rhs_has_side_effects(stmt)) ||
...
static tree lookup_avail_expr(gimple stmt, bool insert) {
  void **slot;
  tree lhs;
  tree temp;
...
static hashval_t avail_expr_hash(const void *p) {
...
static hashval_t real_avail_expr_hash(const void *p) {
...
static tree degenerate_phi_result(gimple phi) {
  tree lhs = gimple_phi_result(phi);
  tree val = NULL;
  size_t i;
...
static void remove_stmt_or_phi(gimple stmt) {
...
static tree get_rhs_or_phi_arg(gimple stmt) {
  if (gimple_code(stmt) == GIMPLE_PHI)
...
static tree get_lhs_or_phi_result(gimple stmt) {
  if (gimple_code(stmt) == GIMPLE_PHI)
...
static void propagate_rhs_into_lhs(gimple stmt, tree lhs, tree rhs,
                                   bitmap interesting_names) {
  /* First verify that propagation is valid and isn't going to move a
     loop variant variable outside its loop.  */
  if (!SSA_NAME_OCCURS_IN_ABNORMAL_PHI(lhs) &&
      (TREE_CODE(rhs) != SSA_NAME || !SSA_NAME_OCCURS_IN_ABNORMAL_PHI(rhs)) &&
    ...
    FOR_EACH_IMM_USE_STMT(use_stmt, iter, lhs) {
      ...
      if (gimple_code(use_stmt) == GIMPLE_PHI ||
          (!is_gimple_reg(lhs) && TREE_CODE(rhs) == SSA_NAME &&
      ...
      /* Propagation into these nodes may make certain edges in
        ...
        if (gimple_code(use_stmt) == GIMPLE_COND)
          val =
              fold_binary(gimple_cond_code(use_stmt), boolean_type_node,
                          gimple_cond_lhs(use_stmt), gimple_cond_rhs(use_stmt));
        ...
        if (val && is_gimple_min_invariant(val)) {
          ...
          for (ei = ei_start(bb->succs); (e = ei_safe_edge(ei));) {
            if (e != te) {
              ...
              for (psi = gsi_start_phis(e->dest); !gsi_end_p(psi);
                   gsi_next(&psi)) {
...
static void eliminate_const_or_copy(gimple stmt, bitmap interesting_names) {
...
static void eliminate_degenerate_phis_1(basic_block bb,
                                        bitmap interesting_names) {
  gimple_stmt_iterator gsi;
  basic_block son;
...
static unsigned int eliminate_degenerate_phis(void) {
  bitmap interesting_names;
...
struct gimple_opt_pass pass_phi_only_cprop = {{
    ...
    0,                                /* properties_destroyed */
    0,                                /* todo_flags_start */
    TODO_cleanup_cfg | TODO_dump_func | TODO_ggc_collect | TODO_verify_ssa |
