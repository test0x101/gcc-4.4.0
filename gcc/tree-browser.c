...
#define TB_NIY \
  fprintf(TB_OUT_FILE, "Sorry this command is not yet implemented.\n")
...
#define TB_SET_HEAD(N)                                              \
  ...
  while (24) {
    ...
    switch (tbc) {
      ...
      case TB_MAX:
        if (head && (INTEGRAL_TYPE_P(head) || TREE_CODE(head) == REAL_TYPE ||
      ...
      case TB_FIELDS:
        if (head &&
            (TREE_CODE(head) == RECORD_TYPE || TREE_CODE(head) == UNION_TYPE ||
      ...
      case TB_CONTEXT:
        ...
        else if (head && TYPE_P(head) && TYPE_CONTEXT(head))
      ...
      case TB_DECL_SAVED_TREE:
        if (head && TREE_CODE(head) == FUNCTION_DECL && DECL_SAVED_TREE(head))
      ...
      case TB_SEARCH_CODE: {
        ...
        if (arg_text == NULL) {
          fprintf(TB_OUT_FILE,
                  "First argument is missing.  This isn't a valid search "
                  "command.  \n");
          break;
        }
        ...
        /* Search in the subtree a node with the given code.  */
        {
          ...
          if (res == NULL_TREE) {
            fprintf(TB_OUT_FILE,
                    "There's no node with this code (reachable via the "
                    "walk_tree function from this node).\n");
          } else {
...
static tree TB_first_in_bind(tree node) {
  tree t;
...
static tree TB_last_in_bind(tree node) {
  tree t;
...
static tree TB_up_expr(tree node) {
  tree res;
  if (node == NULL_TREE)
    return NULL_TREE;
...
static tree TB_prev_expr(tree node) {
...
static tree TB_next_expr(tree node) {
  node = TB_current_chain_node(node);
...
static tree TB_current_chain_node(tree node) {
  if (node == NULL_TREE)
    return NULL_TREE;
...
static tree store_child_info(tree *tp, int *walk_subtrees ATTRIBUTE_UNUSED,
...
static int TB_parent_eq(const void *p1, const void *p2) {
...
static void TB_update_up(tree node) {
  while (node) {
...
static TB_CODE TB_get_command(char *input) {
...
static enum tree_code TB_get_tree_code(char *input) {
  unsigned int mn, size_tok;
  int comp;
  char *space;
...
static tree find_node_with_code(tree *tp, int *walk_subtrees ATTRIBUTE_UNUSED,
                                void *data) {
  enum tree_code *code;
...
static long TB_getline(char **lineptr, long *n, FILE *stream) {
  char *line, *p;
  long size, copy;
