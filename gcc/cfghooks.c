...
void verify_flow_info(void) {
  ...
  FOR_BB_BETWEEN(bb, ENTRY_BLOCK_PTR->next_bb, NULL, next_bb) {
    ...
    if (bb->prev_bb != last_bb_seen) {
      error("prev_bb of %d should be %d, not %d", bb->index,
            last_bb_seen->index, bb->prev_bb->index);
      err = 1;
    }
  ...
  FOR_EACH_BB_REVERSE(bb) {
    ...
    if (bb->count < 0) {
      error("verify_flow_info: Wrong count of block %i %i", bb->index,
            (int)bb->count);
      err = 1;
    }
    if (bb->frequency < 0) {
      error("verify_flow_info: Wrong frequency of block %i %i", bb->index,
            bb->frequency);
      err = 1;
    }
    FOR_EACH_EDGE(e, ei, bb->succs) {
      if (last_visited[e->dest->index] == bb) {
        error("verify_flow_info: Duplicate edge %i->%i", e->src->index,
              e->dest->index);
        err = 1;
      }
      ...
      if (e->count < 0) {
        error("verify_flow_info: Wrong count of edge %i->%i %i", e->src->index,
              e->dest->index, (int)e->count);
        err = 1;
      }
    ...
    FOR_EACH_EDGE(e, ei, bb->preds) {
      ...
      if (ei.index != e->dest_idx) {
        error("basic block %d pred edge is corrupted", bb->index);
        error("its dest_idx should be %d, not %d", ei.index, e->dest_idx);
        fputs("Predecessor: ", stderr);
        dump_edge_info(stderr, e, 0);
        fputs("\nSuccessor: ", stderr);
        dump_edge_info(stderr, e, 1);
...
void dump_bb(basic_block bb, FILE *outf, int indent) {
  ...
  fprintf(outf, ";;%s basic block %d, loop depth %d, count ", s_indent,
...
bool can_remove_branch_p(const_edge e) {
  if (!cfg_hooks->can_remove_branch_p)
    internal_error("%s does not support can_remove_branch_p", cfg_hooks->name);
...
edge make_forwarder_block(basic_block bb, bool (*redirect_edge_p)(edge),
  ...
  if (!cfg_hooks->make_forwarder_block)
    internal_error("%s does not support make_forwarder_block", cfg_hooks->name);
  ...
  for (ei = ei_start(dummy->preds); (e = ei_safe_edge(ei));) {
    ...
    if (jump != NULL) {
      ...
      if (current_loops != NULL && dummy->loop_father != NULL &&
  ...
  if (current_loops != NULL) {
    ...
    if (loop->header == dummy && loop->latch != NULL &&
...
basic_block duplicate_block(basic_block bb, edge e, basic_block after) {
  edge s, n;
  basic_block new_bb;
  ...
  if (!cfg_hooks->duplicate_block)
    internal_error("%s does not support duplicate_block", cfg_hooks->name);
...
int flow_call_edges_add(sbitmap blocks) {
  if (!cfg_hooks->flow_call_edges_add)
    internal_error("%s does not support flow_call_edges_add", cfg_hooks->name);
...
bool cfg_hook_duplicate_loop_to_header_edge(struct loop *loop, edge e,
  ...
  return cfg_hooks->cfg_hook_duplicate_loop_to_header_edge(
...
void lv_adjust_loop_header_phi(basic_block first, basic_block second,
...
/* Conditions in trees and RTL are different so we need
   ...
   versioning code.  */
void lv_add_condition_to_bb(basic_block first, basic_block second,
