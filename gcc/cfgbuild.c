...
bool inside_basic_block_p(const_rtx insn) {
  switch (GET_CODE(insn)) {
    case CODE_LABEL:
      /* Avoid creating of basic block for jumptables.  */
      return (NEXT_INSN(insn) == 0 || !JUMP_P(NEXT_INSN(insn)) ||
...
bool control_flow_insn_p(const_rtx insn) {
  rtx note;
  ...
  switch (GET_CODE(insn)) {
    ...
    case CALL_INSN:
      ...
      return ((nonlocal_goto_handler_labels &&
               (0 == (note = find_reg_note(insn, REG_EH_REGION, NULL_RTX)) ||
...
static int count_basic_blocks(const_rtx f) {
  ...
  for (insn = f; insn; insn = NEXT_INSN(insn)) {
    ...
    if ((LABEL_P(insn) || BARRIER_P(insn)) && saw_insn)
...
static void make_label_edge(sbitmap edge_cache, basic_block src, rtx label,
                            int flags) {
...
static void make_edges(basic_block min, basic_block max, int update_p) {
  basic_block bb;
    ...
    if (code == JUMP_INSN) {
      ...
      /* Recognize a tablejump and do the right thing.  */
        ...
        for (j = GET_NUM_ELEM(vec) - 1; j >= 0; --j)
          make_label_edge(edge_cache, bb, XEXP(RTVEC_ELT(vec, j), 0), 0);
        ...
        if ((tmp = single_set(insn)) != NULL && SET_DEST(tmp) == pc_rtx &&
          ...
          make_label_edge(edge_cache, bb, XEXP(XEXP(SET_SRC(tmp), 2), 0), 0);
      }
...
static void find_basic_blocks_1(rtx f) {
  rtx insn, next;
  ...
  for (insn = f; insn; insn = next) {
    ...
    if ((LABEL_P(insn) || BARRIER_P(insn)) && head) {
...
static void purge_dead_tablejump_edges(basic_block bb, rtx table) {
  ...
  for (ei = ei_start(bb->succs); (e = ei_safe_edge(ei));) {
    if (FULL_STATE(e->dest) & BLOCK_USED_BY_TABLEJUMP)
      SET_STATE(e->dest,
                FULL_STATE(e->dest) & ~(size_t)BLOCK_USED_BY_TABLEJUMP);
...
static void find_bb_boundaries(basic_block bb) {
  ...
  while (1) {
    ...
    if (code == CODE_LABEL) {
      ...
      if (flow_transfer_insn) {
        ...
        for (x = NEXT_INSN(flow_transfer_insn); x != BB_HEAD(fallthru->dest);
             x = NEXT_INSN(x))
          if (!BARRIER_P(x))
...
static void compute_outgoing_frequencies(basic_block b) {
  ...
  if (EDGE_COUNT(b->succs) == 2) {
    ...
    if (note) {
      ...
      e->count =
          ((b->count * probability + REG_BR_PROB_BASE / 2) / REG_BR_PROB_BASE);
      f = FALLTHRU_EDGE(b);
  ...
  e->count =
      ((b->count * e->probability + REG_BR_PROB_BASE / 2) / REG_BR_PROB_BASE);
}
...
void find_many_sub_basic_blocks(sbitmap blocks) {
  ...
  FOR_EACH_BB(bb)
  SET_STATE(bb, TEST_BIT(blocks, bb->index) ? BLOCK_TO_SPLIT : BLOCK_ORIGINAL);
