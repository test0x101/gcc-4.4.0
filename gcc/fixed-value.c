...
void fixed_from_string(FIXED_VALUE_TYPE *f, const char *str,
  ...
  if (temp == FIXED_UNDERFLOW || temp == FIXED_GT_MAX_EPS ||
  ...
  if (temp == FIXED_MAX_EPS && ALL_FRACT_MODE_P(f->mode)) {
    ...
    f->data.low = -1;
    f->data.high = -1;
    f->data = double_int_ext(
        f->data, GET_MODE_FBIT(f->mode) + GET_MODE_IBIT(f->mode), 1);
  } else
    f->data =
        double_int_ext(f->data,
                       SIGNED_FIXED_POINT_MODE_P(f->mode) +
                           GET_MODE_FBIT(f->mode) + GET_MODE_IBIT(f->mode),
...
static bool fixed_saturate1(enum machine_mode mode, double_int a, double_int *f,
                            bool sat_p) {
  ...
  } else /* Signed type.  */
  {
    ...
    min.high = 0;
    min.low = 1;
    lshift_double(min.low, min.high, i_f_bits, 2 * HOST_BITS_PER_WIDE_INT,
                  &min.low, &min.high, 1);
...
static bool fixed_saturate2(enum machine_mode mode, double_int a_high,
                            double_int a_low, double_int *f, bool sat_p) {
  bool overflow_p = false;
  bool unsigned_p = UNSIGNED_FIXED_POINT_MODE_P(mode);
  int i_f_bits = GET_MODE_IBIT(mode) + GET_MODE_FBIT(mode);
  ...
  } else /* Signed type.  */
  {
    ...
    min_s.high = 0;
    min_s.low = 1;
    lshift_double(min_s.low, min_s.high, i_f_bits, 2 * HOST_BITS_PER_WIDE_INT,
...
static inline int get_fixed_sign_bit(double_int a, int i_f_bits) {
...
static bool do_fixed_add(FIXED_VALUE_TYPE *f, const FIXED_VALUE_TYPE *a,
                         const FIXED_VALUE_TYPE *b, bool subtract_p,
                         bool sat_p) {
  bool overflow_p = false;
  bool unsigned_p;
  double_int temp;
    ...
    if ((!subtract_p &&
      ...
      if (sat_p) {
        ...
        lshift_double(f->data.low, f->data.high, i_f_bits,
                      2 * HOST_BITS_PER_WIDE_INT, &f->data.low, &f->data.high,
                      1);
...
static bool do_fixed_multiply(FIXED_VALUE_TYPE *f, const FIXED_VALUE_TYPE *a,
  ...
  if (GET_MODE_PRECISION(f->mode) <= HOST_BITS_PER_WIDE_INT) {
    ...
    lshift_double(f->data.low, f->data.high, (-GET_MODE_FBIT(f->mode)),
                  2 * HOST_BITS_PER_WIDE_INT, &f->data.low, &f->data.high,
                  !unsigned_p);
    ...
    if (double_int_cmp(s, temp1, 1) == -1 || double_int_cmp(s, temp2, 1) == -1)
      carry++; /* Carry */
    ...
    if (GET_MODE_FBIT(f->mode) == 2 * HOST_BITS_PER_WIDE_INT) {
      ...
      lshift_double(s.low, s.high, (-GET_MODE_FBIT(f->mode)),
                    2 * HOST_BITS_PER_WIDE_INT, &s.low, &s.high, 0);
      lshift_double(r.low, r.high,
                    (2 * HOST_BITS_PER_WIDE_INT - GET_MODE_FBIT(f->mode)),
                    2 * HOST_BITS_PER_WIDE_INT, &f->data.low, &f->data.high, 0);
      f->data.low = f->data.low | s.low;
      f->data.high = f->data.high | s.high;
      s.low = f->data.low;
      s.high = f->data.high;
      lshift_double(r.low, r.high, (-GET_MODE_FBIT(f->mode)),
                    2 * HOST_BITS_PER_WIDE_INT, &r.low, &r.high, !unsigned_p);
    }
...
static bool do_fixed_divide(FIXED_VALUE_TYPE *f, const FIXED_VALUE_TYPE *a,
                            const FIXED_VALUE_TYPE *b, bool sat_p) {
  bool overflow_p = false;
  bool unsigned_p = UNSIGNED_FIXED_POINT_MODE_P(a->mode);
  int i_f_bits = GET_MODE_IBIT(a->mode) + GET_MODE_FBIT(a->mode);
  f->mode = a->mode;
  if (GET_MODE_PRECISION(f->mode) <= HOST_BITS_PER_WIDE_INT) {
    lshift_double(a->data.low, a->data.high, GET_MODE_FBIT(f->mode),
                  2 * HOST_BITS_PER_WIDE_INT, &f->data.low, &f->data.high,
                  !unsigned_p);
    ...
    if (GET_MODE_FBIT(f->mode) == 2 * HOST_BITS_PER_WIDE_INT) {
      ...
      lshift_double(pos_a.low, pos_a.high, GET_MODE_FBIT(f->mode),
                    2 * HOST_BITS_PER_WIDE_INT, &s.low, &s.high, 0);
      lshift_double(pos_a.low, pos_a.high,
                    -(2 * HOST_BITS_PER_WIDE_INT - GET_MODE_FBIT(f->mode)),
                    2 * HOST_BITS_PER_WIDE_INT, &r.low, &r.high, 0);
    }
    ...
    for (i = 0; i < 2 * HOST_BITS_PER_WIDE_INT; i++) {
      ...
      lshift_double(mod.low, mod.high, 1, 2 * HOST_BITS_PER_WIDE_INT, &mod.low,
                    &mod.high, 0);
      ...
      lshift_double(s.low, s.high, 1, 2 * HOST_BITS_PER_WIDE_INT, &s.low,
                    &s.high, 0);
    }
...
static bool do_fixed_shift(FIXED_VALUE_TYPE *f, const FIXED_VALUE_TYPE *a,
  ...
  if (GET_MODE_PRECISION(f->mode) <= HOST_BITS_PER_WIDE_INT || (!left_p)) {
    lshift_double(
        a->data.low, a->data.high, left_p ? b->data.low : (-b->data.low),
        2 * HOST_BITS_PER_WIDE_INT, &f->data.low, &f->data.high, !unsigned_p);
  ...
  } else /* We need two double_int to store the left-shift result.  */
  {
    ...
    if (b->data.low == 2 * HOST_BITS_PER_WIDE_INT) {
      ...
      lshift_double(a->data.low, a->data.high, b->data.low,
                    2 * HOST_BITS_PER_WIDE_INT, &temp_low.low, &temp_low.high,
                    !unsigned_p);
      /* Logical shift right to temp_high.  */
      lshift_double(
          a->data.low, a->data.high, b->data.low - 2 * HOST_BITS_PER_WIDE_INT,
          2 * HOST_BITS_PER_WIDE_INT, &temp_high.low, &temp_high.high, 0);
    }
    if (!unsigned_p && a->data.high < 0) /* Signed-extend temp_high.  */
      temp_high = double_int_ext(temp_high, b->data.low, unsigned_p);
    f->data = temp_low;
    overflow_p = fixed_saturate2(f->mode, temp_high, temp_low, &f->data, sat_p);
  }
  f->data = double_int_ext(f->data, (!unsigned_p) + i_f_bits, unsigned_p);
  return overflow_p;
}
...
static bool do_fixed_neg(FIXED_VALUE_TYPE *f, const FIXED_VALUE_TYPE *a,
                         bool sat_p) {
  bool overflow_p = false;
  bool unsigned_p = UNSIGNED_FIXED_POINT_MODE_P(a->mode);
  int i_f_bits = GET_MODE_IBIT(a->mode) + GET_MODE_FBIT(a->mode);
...
bool fixed_arithmetic(FIXED_VALUE_TYPE *f, int icode,
                      const FIXED_VALUE_TYPE *op0, const FIXED_VALUE_TYPE *op1,
                      bool sat_p) {
  switch (icode) {
    case NEGATE_EXPR:
...
bool fixed_compare(int icode, const FIXED_VALUE_TYPE *op0,
...
bool fixed_convert(FIXED_VALUE_TYPE *f, enum machine_mode mode,
  ...
  if (GET_MODE_FBIT(mode) > GET_MODE_FBIT(a->mode)) {
    ...
    /* Logical shift right to temp_high.  */
    lshift_double(
        a->data.low, a->data.high, amount - 2 * HOST_BITS_PER_WIDE_INT,
        2 * HOST_BITS_PER_WIDE_INT, &temp_high.low, &temp_high.high, 0);
    ...
    if (SIGNED_FIXED_POINT_MODE_P(a->mode) ==
        SIGNED_FIXED_POINT_MODE_P(f->mode))
      overflow_p =
          fixed_saturate2(f->mode, temp_high, temp_low, &f->data, sat_p);
    else {
      ...
      if (SIGNED_FIXED_POINT_MODE_P(a->mode)) {
        ...
        if (temp_high.high < 0) {
          if (sat_p) {
            ...
            f->data = double_int_ext(
                f->data, GET_MODE_FBIT(f->mode) + GET_MODE_IBIT(f->mode),
                1); /* Clear the sign.  */
          } else
            overflow_p = true;
        } else
...
bool fixed_convert_from_int(FIXED_VALUE_TYPE *f, enum machine_mode mode,
  ...
  if (amount == 2 * HOST_BITS_PER_WIDE_INT) {
    ...
    lshift_double(a.low, a.high, amount, 2 * HOST_BITS_PER_WIDE_INT,
                  &temp_low.low, &temp_low.high, 0);

    /* Logical shift right to temp_high.  */
    lshift_double(a.low, a.high, amount - 2 * HOST_BITS_PER_WIDE_INT,
                  2 * HOST_BITS_PER_WIDE_INT, &temp_high.low, &temp_high.high,
                  0);
  }
...
bool fixed_convert_from_real(FIXED_VALUE_TYPE *f, enum machine_mode mode,
...
void real_convert_from_fixed(REAL_VALUE_TYPE *r, enum machine_mode mode,
