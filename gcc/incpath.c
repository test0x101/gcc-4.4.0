...
#if defined INO_T_EQ
#define DIRS_EQ(A, B) ((A)->dev == (B)->dev && INO_T_EQ((A)->ino, (B)->ino))
#else
...
static void free_path(struct cpp_dir *path, int reason) {
  switch (reason) {
    case REASON_DUP:
    case REASON_DUP_SYS:
      fprintf(stderr, _("ignoring duplicate directory \"%s\"\n"), path->name);
      if (reason == REASON_DUP_SYS)
        fprintf(stderr, _("  as it is a non-system directory that duplicates a "
                          "system directory\n"));
      break;
...
static void add_env_var_paths(const char *env_var, int chain) {
  char *p, *q, *path;
...
static void add_standard_paths(const char *sysroot, const char *iprefix,
  ...
  for (p = cpp_include_defaults; p->fname; p++) {
    if (!p->cplusplus || cxx_stdinc) {
      ...
      if (sysroot && p->add_sysroot)
        ...
        str = concat(relocated_prefix, p->fname + cpp_PREFIX_len, NULL);
...
static struct cpp_dir *remove_duplicates(cpp_reader *pfile,
                                         ...
                                         struct cpp_dir *join, int verbose) {
  ...
  for (pcur = &head; *pcur;) {
    ...
    if (stat(cur->name, &st)) {
      ...
      cpp_error_with_line(pfile, CPP_DL_ERROR, 0, 0, "%s: not a directory",
                          cur->name);
    else {
      ...
      if (!tmp) {
        ...
        if (tmp == cur
            ...
            && !(cur->next == NULL && join && DIRS_EQ(cur, join) &&
...
static void add_sysroot_to_chain(const char *sysroot, int chain) {
  struct cpp_dir *p;
...
static void merge_include_chains(const char *sysroot, cpp_reader *pfile,
                                 int verbose) {
...
void register_include_chains(cpp_reader *pfile, const char *sysroot,
