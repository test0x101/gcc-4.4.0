...
static bool cgraph_decide_inlining_incrementally(struct cgraph_node *,
                                                 enum inlining_mode, int);
...
static inline struct inline_summary *inline_summary(struct cgraph_node *node) {
  return &node->local.inline_summary;
}

/* Estimate size of the function after inlining WHAT into TO.  */

static int cgraph_estimate_size_after_inlining(int times,
...
static bool cgraph_mark_inline_edge(struct cgraph_edge *e, bool update_original,
  ...
  for (; e && !e->inline_failed; e = e->caller->callers) {
    ...
    new_insns = cgraph_estimate_size_after_inlining(1, e->caller, what);
...
static struct cgraph_edge *cgraph_mark_inline(struct cgraph_edge *edge) {
...
static int cgraph_estimate_growth(struct cgraph_node *node) {
  int growth = 0;
...
static bool cgraph_check_inline_limits(struct cgraph_node *to,
                                       struct cgraph_node *what,
  ...
  if (newsize >= to->global.insns &&
      newsize > PARAM_VALUE(PARAM_LARGE_FUNCTION_INSNS) && newsize > limit) {
    if (reason)
  ...
  stack_size_limit +=
      stack_size_limit * PARAM_VALUE(PARAM_STACK_FRAME_GROWTH) / 100;
...
bool cgraph_default_inline_p(struct cgraph_node *n, const char **reason) {
  tree decl = n->decl;
...
static bool cgraph_recursive_inlining_p(struct cgraph_node *to,
                                        struct cgraph_node *what,
                                        const char **reason) {
  bool recursive;
  if (to->global.inlined_to)
  ...
  if (recursive && reason)
    *reason =
        (what->local.disregard_inline_limits ? N_("recursive inlining") : "");
  return recursive;
}
...
static int cgraph_edge_badness(struct cgraph_edge *edge) {
  int badness;
  int growth =
...
static void update_caller_keys(fibheap_t heap, struct cgraph_node *node,
...
static void update_callee_keys(fibheap_t heap, struct cgraph_node *node,
                               bitmap updated_nodes) {
  struct cgraph_edge *e;
  node->global.estimated_growth = INT_MIN;
...
static void lookup_recursive_calls(struct cgraph_node *node,
                                   struct cgraph_node *where, fibheap_t heap) {
  static int priority;
  struct cgraph_edge *e;
...
static bool cgraph_decide_recursive_inlining(struct cgraph_node *node,
  ...
  if (!max_depth || cgraph_estimate_size_after_inlining(1, node, node) >= limit)
    return false;
  heap = fibheap_new();
  ...
  master_clone =
      cgraph_clone_node(node, node->count, CGRAPH_FREQ_BASE, 1, false);
  ...
  while (!fibheap_empty(heap) && (cgraph_estimate_size_after_inlining(
                                      1, node, master_clone) <= limit)) {
    struct cgraph_edge *curr = (struct cgraph_edge *)fibheap_extract_min(heap);
    struct cgraph_node *cnode;

    depth = 1;
    for (cnode = curr->caller; cnode->global.inlined_to;
         cnode = cnode->callers->caller)
      if (node->decl == curr->callee->decl)
        depth++;
    if (depth > max_depth) {
      if (dump_file)
        fprintf(dump_file, "   maximal depth reached\n");
      continue;
    }
    ...
    if (max_count) {
      ...
      if (curr->count * 100 / node->count < probability) {
        if (dump_file)
          fprintf(dump_file, "   Probability of edge is too small\n");
        continue;
      }
    }
  ...
  /* Remove master clone we used for inlining.  We rely that clones inlined
     ...
     need recursion.  */
  for (node = cgraph_nodes; node != master_clone; node = next) {
    next = node->next;
...
static void cgraph_set_inline_failed(struct cgraph_node *node,
                                     const char *reason) {
  struct cgraph_edge *e;
...
static int compute_max_insns(int insns) {
  int max_insns = insns;
...
static void add_new_edges_to_heap(fibheap_t heap,
                                  VEC(cgraph_edge_p, heap) * new_edges) {
...
static void cgraph_decide_inlining_of_small_functions(void) {
  ...
  while (overall_insns <= max_insns &&
    ...
    if (dump_file) {
      fprintf(dump_file, "\nConsidering %s with %i insns\n",
              cgraph_node_name(edge->callee), edge->callee->global.insns);
      fprintf(dump_file,
              " to be inlined into %s\n"
              " Estimated growth after inlined into all callees is %+i insns.\n"
              " Estimated badness is %i, frequency %.2f.\n",
              cgraph_node_name(edge->caller),
              cgraph_estimate_growth(edge->callee), cgraph_edge_badness(edge),
    ...
    if (!edge->count) {
      ...
      if (where->global.inlined_to) {
        edge->inline_failed = (edge->callee->local.disregard_inline_limits
                                   ? N_("recursive inlining")
                                   : "");
        if (dump_file)
          fprintf(dump_file,
                  " inline_failed:Recursive inlining performed only for "
                  "function itself.\n");
        continue;
      }
    }
    ...
    if (!flag_inline_functions && !DECL_DECLARED_INLINE_P(edge->callee->decl))
    ...
    if (cgraph_recursive_inlining_p(edge->caller, edge->callee,
      ...
      if (!cgraph_decide_recursive_inlining(
              where, flag_indirect_inlining ? &new_indirect_edges : NULL))
        continue;
      if (flag_indirect_inlining)
    ...
    if (dump_file) {
      fprintf(dump_file,
              ...
              cgraph_node_name(edge->caller), edge->caller->global.insns,
...
static unsigned int cgraph_decide_inlining(void) {
  struct cgraph_node *node;
  int nnodes;
  ...
  while (redo_always_inline) {
    ...
    for (i = nnodes - 1; i >= 0; i--) {
      ...
      for (e = node->callers; e; e = next) {
        ...
        if (dump_file)
          fprintf(dump_file, " Inlined into %s which now has %i insns.\n",
      ...
      if (dump_file)
        fprintf(dump_file, " Inlined for a net change of %+i insns.\n",
                overall_insns - old_insns);
    }
  }
  ...
  if (flag_inline_functions_called_once) {
    ...
    for (i = nnodes - 1; i >= 0; i--) {
      ...
      if (node->callers && !node->callers->next_caller && !node->needed &&
        ...
        if (cgraph_check_inline_limits(node->callers->caller, node, NULL,
                                       false)) {
...
static bool try_inline(struct cgraph_edge *e, enum inlining_mode mode,
                       int depth) {
  ...
  if (callee_mode) {
    ...
    /* Otherwise it is time to give up.  */
      ...
      if (dump_file) {
        indent_to(dump_file, depth);
        fprintf(dump_file, "Not inlining %s into %s to avoid cycle.\n",
                cgraph_node_name(callee), cgraph_node_name(e->caller));
      }
      e->inline_failed =
          (e->callee->local.disregard_inline_limits ? N_("recursive inlining")
                                                    : "");
      return false;
    }
  }
...
static bool cgraph_decide_inlining_incrementally(struct cgraph_node *node,
  ...
  for (e = node->callees; e; e = e->next_callee) {
    ...
    if (dump_file) {
      indent_to(dump_file, depth);
      fprintf(dump_file, "Considering to always inline inline candidate %s.\n",
              cgraph_node_name(e->callee));
    }
    ...
    if (!tree_can_inline_p(node->decl, e->callee->decl)) {
      ...
      if (dump_file) {
        indent_to(dump_file, depth);
        fprintf(dump_file, "Not inlining: Target specific option mismatch.\n");
      }
      continue;
    }
  ...
  if (mode != INLINE_ALL && mode != INLINE_ALWAYS_INLINE)
    for (e = node->callees; e; e = e->next_callee) {
       ...
       */
      if ((mode == INLINE_SIZE || (!flag_inline_functions &&
...
static unsigned int cgraph_early_inlining(void) {
...
static bool cgraph_gate_early_inlining(void) {
  return flag_early_inlining;
}

struct gimple_opt_pass pass_early_inline = {{
    GIMPLE_PASS, "einline",     /* name */
...
/* When inlining shall be performed.  */
static bool cgraph_gate_ipa_early_inlining(void) {
  return (flag_early_inlining && (flag_branch_probabilities ||
...
struct simple_ipa_opt_pass pass_ipa_early_inline = {{
    SIMPLE_IPA_PASS, "einline_ipa", /* name */
...
unsigned int compute_inline_parameters(struct cgraph_node *node) {
...
static unsigned int compute_inline_parameters_for_current(void) {
...
static void analyze_function(struct cgraph_node *node) {
...
/* Note function body size.  */
static void inline_generate_summary(void) {
  struct cgraph_node *node;
...
static unsigned int inline_transform(struct cgraph_node *node) {
  unsigned int todo = 0;
  struct cgraph_edge *e;
