...
static void gen_insn(rtx insn, int insn_code_number) {
  int i;
  ...
  p = XNEWVAR(struct extraction,
              sizeof(struct extraction) + op_count * sizeof(char *) +
                  dup_count * sizeof(char *) + dup_count * sizeof(int));
  p->op_count = op_count;
  ...
  memcpy(p->duplocs, VEC_address(locstr, acc.duplocs),
         dup_count * sizeof(locstr));
...
static void VEC_safe_set_locstr(VEC(locstr, heap) * *vp, unsigned int ix,
                                char *str) {
...
static char *VEC_char_to_string(VEC(char, heap) * v) {
...
static void walk_rtx(rtx x, struct accum_extract *acc) {
  RTX_CODE code;
  int i, len, base;
  const char *fmt;
