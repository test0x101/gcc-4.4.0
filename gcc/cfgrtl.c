...
static int can_delete_note_p(const_rtx note) {
...
static int can_delete_label_p(const_rtx label) {
  return (!LABEL_PRESERVE_P(label)
          /* User declared labels must be preserved.  */
          && LABEL_NAME(label) == 0 && !in_expr_list_p(forced_labels, label));
}
...
rtx delete_insn(rtx insn) {
  ...
  if (JUMP_P(insn)) {
    ...
    while ((note = find_reg_note(insn, REG_LABEL_TARGET, NULL_RTX)) !=
               NULL_RTX &&
           LABEL_P(XEXP(note, 0))) {
  ...
  while ((note = find_reg_note(insn, REG_LABEL_OPERAND, NULL_RTX)) !=
             NULL_RTX &&
         LABEL_P(XEXP(note, 0))) {
    LABEL_NUSES(XEXP(note, 0))--;
    remove_note(insn, note);
...
void delete_insn_chain(rtx start, rtx finish, bool clear_bb) {
  rtx next;
...
void delete_insn_chain_and_edges(rtx first, rtx last) {
  ...
  if (INSN_P(last) && BLOCK_FOR_INSN(last) &&
...
basic_block create_basic_block_structure(rtx head, rtx end, rtx bb_note,
                                         basic_block after) {
  basic_block bb;
...
static basic_block rtl_create_basic_block(void *headp, void *endp,
                                          basic_block after) {
...
static basic_block cfg_layout_create_basic_block(void *head, void *end,
                                                 basic_block after) {
...
rtx entry_of_function(void) {
  return (n_basic_blocks > NUM_FIXED_BLOCKS ? BB_HEAD(ENTRY_BLOCK_PTR->next_bb)
                                            : get_insns());
}
...
static void update_bb_for_insn_chain(rtx begin, rtx end, basic_block bb) {
  rtx insn;
...
static rtx first_insn_after_basic_block_note(basic_block block) {
  rtx insn;
...
static basic_block rtl_split_block(basic_block bb, void *insnp) {
  basic_block new_bb;
  rtx insn = (rtx)insnp;
  edge e;
  edge_iterator ei;
...
static bool rtl_can_merge_blocks(basic_block a, basic_block b) {
  ...
  return (single_succ_p(a) && single_succ(a) == b && single_pred_p(b) &&
          a != b
          /* Must be simple edge.  */
          && !(single_succ_edge(a)->flags & EDGE_COMPLEX) && a->next_bb == b &&
          a != ENTRY_BLOCK_PTR &&
          b != EXIT_BLOCK_PTR
          /* If the jump insn has side effects,
             we can't kill the edge.  */
          &&
          (!JUMP_P(BB_END(a)) || (reload_completed ? simplejump_p(BB_END(a))
                                                   : onlyjump_p(BB_END(a)))));
}
...
edge try_redirect_by_replacing_jump(edge e, basic_block target,
                                    bool in_cfglayout) {
  ...
  if (in_cfglayout || can_fallthru(src, target)) {
    ...
    if (in_cfglayout) {
      ...
      delete_insn_chain(kill_from, PREV_INSN(BB_HEAD(target)), false);
  }
  ...
  /* Or replace possibly complicated jump insn by simple jump insn.  */
    ...
    if (!barrier || !BARRIER_P(barrier))
      ...
      if (barrier != NEXT_INSN(BB_END(src))) {
        ...
        update_bb_for_insn_chain(NEXT_INSN(BB_END(src)), PREV_INSN(barrier),
                                 src);
...
static edge redirect_branch_edge(edge e, basic_block target) {
  ...
  if (tablejump_p(insn, NULL, &tmp)) {
    ...
    if ((tmp = single_set(insn)) != NULL && SET_DEST(tmp) == pc_rtx &&
        GET_CODE(SET_SRC(tmp)) == IF_THEN_ELSE &&
        GET_CODE(XEXP(SET_SRC(tmp), 2)) == LABEL_REF &&
        XEXP(XEXP(SET_SRC(tmp), 2), 0) == old_label) {
      XEXP(SET_SRC(tmp), 2) = gen_rtx_LABEL_REF(Pmode, new_label);
      --LABEL_NUSES(old_label);
      ++LABEL_NUSES(new_label);
    }
  } else {
...
static edge rtl_redirect_edge_and_branch(edge e, basic_block target) {
  edge ret;
  basic_block src = e->src;
...
static basic_block force_nonfallthru_and_redirect(edge e, basic_block target) {
  ...
  if (EDGE_COUNT(e->src->succs) >= 2 || abnormal_edge_flags) {
    ...
    if (flag_reorder_blocks_and_partition && targetm.have_named_sections &&
        JUMP_P(BB_END(jump_block)) && !any_condjump_p(BB_END(jump_block)) &&
...
static bool rtl_move_block_after(basic_block bb ATTRIBUTE_UNUSED,
...
static void commit_one_edge_insertion(edge e) {
  ...
  if (!before && !after) {
    ...
    else if ((e->flags & EDGE_ABNORMAL) == 0 && single_succ_p(e->src) &&
  ...
  if (returnjump_p(last)) {
    ...
    gcc_assert(e->dest == EXIT_BLOCK_PTR && single_succ_p(bb) &&
               (e->flags & EDGE_FALLTHRU));
...
static void rtl_dump_bb(basic_block bb, FILE *outf, int indent,
...
void print_rtl_with_bb(FILE *outf, const_rtx rtx_first) {
  const_rtx tmp_rtx;
  if (rtx_first == 0)
    fprintf(outf, "(nil)\n");
  else {
    ...
    for (tmp_rtx = rtx_first; NULL != tmp_rtx; tmp_rtx = NEXT_INSN(tmp_rtx)) {
      ...
      if (in_bb_p[INSN_UID(tmp_rtx)] == NOT_IN_BB && !NOTE_P(tmp_rtx) &&
          !BARRIER_P(tmp_rtx))
...
/* Get the last insn associated with block BB (that includes barriers and
   tablejumps after BB).  */
rtx get_last_bb_insn(basic_block bb) {
  rtx tmp;
  rtx end = BB_END(bb);
...
static int rtl_verify_flow_info_1(void) {
  ...
  FOR_EACH_BB_REVERSE(bb) {
    ...
    if (JUMP_P(BB_END(bb)) &&
        (note = find_reg_note(BB_END(bb), REG_BR_PROB, NULL_RTX)) &&
        EDGE_COUNT(bb->succs) >= 2 && any_condjump_p(BB_END(bb))) {
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      ...
      if ((e->flags &
           ~(EDGE_DFS_BACK | EDGE_CAN_FALLTHRU | EDGE_IRREDUCIBLE_LOOP |
             EDGE_LOOP_EXIT | EDGE_CROSSING)) == 0)
        n_branch++;
    ...
    if (n_abnormal && (!CALL_P(BB_END(bb)) && n_call != n_abnormal) &&
        (!JUMP_P(BB_END(bb)) || any_condjump_p(BB_END(bb)) ||
    ...
    for (x = BB_HEAD(bb); x != NEXT_INSN(BB_END(bb)); x = NEXT_INSN(x))
      ...
      if (!BARRIER_P(x) && BLOCK_FOR_INSN(x) != bb) {
        debug_rtx(x);
        if (!BLOCK_FOR_INSN(x))
          error("insn %d inside basic block %d but block_for_insn is NULL",
                INSN_UID(x), bb->index);
        else
          error("insn %d inside basic block %d but block_for_insn is %i",
  ...
  FOR_EACH_BB_REVERSE(bb) {
    ...
    for (; x != NULL_RTX; x = PREV_INSN(x)) {
      ...
      if (bb_info[INSN_UID(x)] != NULL) {
        error("insn %d is in multiple basic blocks (%d and %d)", INSN_UID(x),
              bb->index, bb_info[INSN_UID(x)]->index);
        err = 1;
      }
    ...
    if (!e) {
      ...
      for (insn = BB_END(bb); !insn || !BARRIER_P(insn); insn = NEXT_INSN(insn))
      ...
      if (e->src->next_bb != e->dest) {
        ...
        for (insn = NEXT_INSN(BB_END(e->src)); insn != BB_HEAD(e->dest);
          ...
          if (BARRIER_P(insn) || INSN_P(insn)) {
            error("verify_flow_info: Incorrect fallthru %i->%i", e->src->index,
                  e->dest->index);
    ...
    if (!curr_bb) {
      switch (GET_CODE(x)) {
        ...
        case CODE_LABEL:
          ...
          if (NEXT_INSN(x) && JUMP_P(NEXT_INSN(x)) &&
  ...
  for (ei = ei_start(bb->succs); (e = ei_safe_edge(ei));) {
    ...
    if (e->flags & EDGE_EH) {
      if (can_throw_internal(BB_END(bb))
          ...
          && (!(e->flags & EDGE_ABNORMAL_CALL) || CALL_P(BB_END(bb)))) {
        ei_next(&ei);
        continue;
      }
...
static basic_block cfg_layout_split_block(basic_block bb, void *insnp) {
  rtx insn = (rtx)insnp;
...
static edge cfg_layout_redirect_edge_and_branch(edge e, basic_block dest) {
  basic_block src = e->src;
  edge ret;
  ...
  if (e->src == ENTRY_BLOCK_PTR && (e->flags & EDGE_FALLTHRU) &&
  ...
  if (e->flags & EDGE_FALLTHRU) {
    ...
    if (EDGE_COUNT(src->succs) == 2) {
      ...
      if (s->dest == dest && any_condjump_p(BB_END(src)) &&
...
static void cfg_layout_delete_block(basic_block bb) {
...
static bool cfg_layout_can_merge_blocks_p(basic_block a, basic_block b) {
  ...
  /* There must be exactly one edge in between the blocks.  */
  return (single_succ_p(a) && single_succ(a) == b && single_pred_p(b) == 1 &&
          a != b
          /* Must be simple edge.  */
          ...
          && (!JUMP_P(BB_END(a)) ||
              ((!optimize || reload_completed) ? simplejump_p(BB_END(a))
                                               : onlyjump_p(BB_END(a)))));
}
...
static void cfg_layout_merge_blocks(basic_block a, basic_block b) {
  ...
  if (NEXT_INSN(BB_END(a)) != BB_HEAD(b)) {
    ...
    update_bb_for_insn_chain(NEXT_INSN(first), BB_END(b), a);
...
static basic_block cfg_layout_split_edge(edge e) {
  basic_block new_bb = create_basic_block(
      e->src != ENTRY_BLOCK_PTR ? NEXT_INSN(BB_END(e->src)) : get_insns(),
      NULL_RTX, e->src);
...
static void rtl_make_forwarder_block(edge fallthru ATTRIBUTE_UNUSED) {}
...
static bool rtl_block_ends_with_call_p(basic_block bb) {
  ...
  while (!CALL_P(insn) && insn != BB_HEAD(bb) &&
         (keep_with_call_p(insn) || NOTE_P(insn)))
    insn = PREV_INSN(insn);
  return (CALL_P(insn));
}
...
static bool need_fake_edge_p(const_rtx insn) {
  if (!INSN_P(insn))
    return false;
  ...
  return ((GET_CODE(PATTERN(insn)) == ASM_OPERANDS &&
          ...
          (GET_CODE(PATTERN(insn)) == PARALLEL && asm_noperands(insn) != -1 &&
...
static void rtl_lv_add_condition_to_bb(basic_block first_head,
  ...
  do_compare_rtx_and_jump(op0, op1, comp, 0, mode, NULL_RTX, NULL_RTX, label);
  jump = get_last_insn();
...
static void rtl_extract_cond_bb_edges(basic_block b, edge *branch_edge,
                                      edge *fallthru_edge) {
...
rtx insert_insn_end_bb_new(rtx pat, basic_block bb) {
  rtx insn = BB_END(bb);
  rtx new_insn;
  rtx pat_end = pat;
