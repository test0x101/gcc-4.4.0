...
#define IS_DOMINATED(bb_src, bb_trg) (TEST_BIT(dom[bb_src], bb_trg))
...
static int is_cfg_nonregular(void) {
  ...
  FOR_BB_INSNS(b, insn) {
    ...
    if (next == NULL_RTX || !JUMP_P(next) ||
        (JUMP_LABEL(next) != XEXP(note, 0) &&
         find_reg_note(next, REG_LABEL_TARGET, XEXP(note, 0)) == NULL_RTX) ||
  ...
  FOR_EACH_BB(b) {
    if (EDGE_COUNT(b->preds) == 0 || (single_pred_p(b) && single_pred(b) == b))
      return 1;
  }
...
static void extract_edgelst(sbitmap set, edgelst *el) {
  unsigned int i = 0;
...
static bool bb_in_region_p(int bb_index, int rgn) {
  int i;
...
void dump_region_dot(FILE *f, int rgn) {
  int i;
...
void dump_region_dot_file(const char *fname, int rgn) {
...
static void find_single_block_region(bool ebbs_p) {
...
static int rgn_estimate_number_of_insns(basic_block bb) {
...
static bool too_large(int block, int *num_bbs, int *num_insns) {
  (*num_bbs)++;
  (*num_insns) +=
      (common_sched_info->estimate_number_of_insns(BASIC_BLOCK(block)));
...
static void haifa_find_rgns(void) {
      ...
      if (TEST_BIT(header, bb->index) && TEST_BIT(inner, bb->index)) {
        ...
        if (no_loops) {
          ...
          /* Leaf nodes have only a single successor which must
             be EXIT_BLOCK.  */
          if (single_succ_p(jbb) && single_succ(jbb) == EXIT_BLOCK_PTR) {
       ...
       scheduling code will properly handle this case. ?!? */
        ...
        while (head < tail && !too_large_failure) {
          ...
          FOR_EACH_EDGE(e, ei, BASIC_BLOCK(child)->preds) {
            ...
            if (e->src == ENTRY_BLOCK_PTR || max_hdr[node] != loop_head) {
              tail = -1;
              break;
...
static int gather_region_statistics(int **rsp) {
  int i, *a = 0, a_sz = 0;
...
static void print_region_statistics(int *s1, int s1_sz, int *s2, int s2_sz) {
  int i;
...
void extend_rgns(int *degree, int *idxp, sbitmap header, int *loop_hdr) {
...
static void compute_dom_prob_ps(int bb) {
  edge_iterator in_ei;
  edge in_edge;
...
static void split_edges(int bb_src, int bb_trg, edgelst *bl) {
...
static void compute_trg_info(int trg) {
  candidate *sp;
  edgelst el = {NULL, 0};
...
static void free_trg_info(void) {
...
static int check_live_1(int src, rtx x) {
  int i;
  int regno;
  rtx reg = SET_DEST(x);
...
static void update_live_1(int src, rtx x) {
  int i;
  int regno;
  rtx reg = SET_DEST(x);
...
static int check_live(rtx insn, int src) {
...
static void update_live(rtx insn, int src) {
  /* Find the registers set by instruction.  */
  if (GET_CODE(PATTERN(insn)) == SET || GET_CODE(PATTERN(insn)) == CLOBBER)
...
#define IS_REACHABLE(bb_from, bb_to)                              \
  (bb_from == bb_to || IS_RGN_ENTRY(bb_from) ||                   \
   (TEST_BIT(ancestor_edges[bb_to], EDGE_TO_BIT(single_pred_edge( \
                                        BASIC_BLOCK(BB_TO_BLOCK(bb_from)))))))

/* Turns on the fed_by_spec_load flag for insns fed by load_insn.  */

static void set_spec_fed(rtx load_insn) {
...
static int find_conditional_protection(rtx insn, int load_insn_bb) {
  sd_iterator_def sd_it;
  dep_t dep;
  ...
  FOR_EACH_DEP(insn, SD_LIST_FORW, sd_it, dep) {
    ...
    if ((CONTAINING_RGN(BLOCK_NUM(next)) ==
        ...
        load_insn_bb != INSN_BB(next) && DEP_TYPE(dep) == REG_DEP_TRUE &&
        (JUMP_P(next) || find_conditional_protection(next, load_insn_bb)))
      return 1;
  }
  return 0;
...
static int is_conditionally_protected(rtx load_insn, int bb_src, int bb_trg) {
  sd_iterator_def sd_it;
  dep_t dep;
  ...
  FOR_EACH_DEP(load_insn, SD_LIST_BACK, sd_it, dep) {
    ...
    if (DEP_TYPE(dep) != REG_DEP_TRUE || JUMP_P(insn1))
      continue;
...
static int is_pfree(rtx load_insn, int bb_src, int bb_trg) {
...
static int is_prisky(rtx load_insn, int bb_src, int bb_trg) {
...
static int is_exception_free(rtx insn, int bb_src, int bb_trg) {
...
static void init_ready_list(void) {
  ...
  for (bb_src = target_bb + 1; bb_src < current_nr_blocks; bb_src++)
    if (IS_VALID(bb_src)) {
      ...
      get_ebb_head_tail(EBB_FIRST_BB(bb_src), EBB_LAST_BB(bb_src), &head,
                        &tail);
...
static int can_schedule_ready_p(rtx insn) {
...
static void begin_schedule_ready(rtx insn, rtx last ATTRIBUTE_UNUSED) {
  /* An interblock motion?  */
...
static ds_t new_ready(rtx next, ds_t ts) {
  if (INSN_BB(next) != target_bb) {
    int not_ex_free = 0;

    /* For speculative insns, before inserting to ready/queue,
       check live, exception-free, and issue-delay.  */
    if (!IS_VALID(INSN_BB(next)) || CANT_MOVE(next) ||
        (IS_SPECULATIVE_INSN(next) &&
         ((recog_memoized(next) >= 0 &&
           min_insn_conflict_delay(curr_state, next, next) >
               PARAM_VALUE(PARAM_MAX_SCHED_INSN_CONFLICT_DELAY)) ||
          IS_SPECULATION_CHECK_P(next) || !check_live(next, INSN_BB(next)) ||
          (not_ex_free =
...
static const char *rgn_print_insn(const_rtx insn, int aligned) {
  static char tmp[80];
...
static int rgn_rank(rtx insn1, rtx insn2) {
...
int contributes_to_priority(rtx next, rtx insn) {
...
static bool sets_likely_spilled(rtx pat) {
  bool ret = false;
  note_stores(pat, sets_likely_spilled_1, &ret);
  return ret;
}

static void sets_likely_spilled_1(rtx x, const_rtx pat, void *data) {
  bool *ret = (bool *)data;

  if (GET_CODE(pat) == SET && REG_P(x) && REGNO(x) < FIRST_PSEUDO_REGISTER &&
...
/* Add dependences so that branches are scheduled to run last in their
   block.  */
static void add_branch_dependences(rtx head, rtx tail) {
  rtx insn, last;
  ...
  insn = tail;
  last = 0;
  while (CALL_P(insn) || JUMP_P(insn) ||
         (NONJUMP_INSN_P(insn) &&
          (GET_CODE(PATTERN(insn)) == USE ||
           GET_CODE(PATTERN(insn)) == CLOBBER || can_throw_internal(insn)
#ifdef HAVE_cc0
           || sets_cc0_p(PATTERN(insn))
#endif
           || (!reload_completed && sets_likely_spilled(PATTERN(insn))))) ||
         NOTE_P(insn)) {
    if (!NOTE_P(insn)) {
      if (last != 0 && sd_find_dep_between(insn, last, false) == NULL) {
...
static rtx concat_INSN_LIST(rtx copy, rtx old) {
  rtx new_rtx = old;
...
static void concat_insn_mem_list(rtx copy_insns, rtx copy_mems,
                                 rtx *old_insns_p, rtx *old_mems_p) {
...
void deps_join(struct deps *succ_deps, struct deps *pred_deps) {
  unsigned reg;
  ...
  EXECUTE_IF_SET_IN_REG_SET(&pred_deps->reg_last_in_use, 0, reg, rsi) {
    ...
    succ_rl->clobbers = concat_INSN_LIST(pred_rl->clobbers, succ_rl->clobbers);
  ...
  concat_insn_mem_list(
      pred_deps->pending_read_insns, pred_deps->pending_read_mems,
      &succ_deps->pending_read_insns, &succ_deps->pending_read_mems);
  concat_insn_mem_list(
      pred_deps->pending_write_insns, pred_deps->pending_write_mems,
      &succ_deps->pending_write_insns, &succ_deps->pending_write_mems);
  ...
  succ_deps->last_function_call = concat_INSN_LIST(
      pred_deps->last_function_call, succ_deps->last_function_call);

  /* sched_before_next_call is inherited by successor.  */
  succ_deps->sched_before_next_call = concat_INSN_LIST(
      pred_deps->sched_before_next_call, succ_deps->sched_before_next_call);
}

/* After computing the dependencies for block BB, propagate the dependencies
   found in TMP_DEPS to the successors of the block.  */
static void propagate_deps(int bb, struct deps *pred_deps) {
...
static void compute_block_dependences(int bb) {
  rtx head, tail;
...
static void free_block_dependencies(int bb) {
  rtx head;
  rtx tail;
...
void debug_dependencies(rtx head, rtx tail) {
  ...
  for (insn = head; insn != next_tail; insn = NEXT_INSN(insn)) {
    ...
    fprintf(sched_dump, ";;   %s%5d%6d%6d%6d%6d%6d   ",
            (SCHED_GROUP_P(insn) ? "+" : " "), INSN_UID(insn), INSN_CODE(insn),
            BLOCK_NUM(insn),
            sched_emulate_haifa_p ? -1 : sd_lists_size(insn, SD_LIST_BACK),
            (sel_sched_p() ? (sched_emulate_haifa_p ? -1 : INSN_PRIORITY(insn))
                           : INSN_PRIORITY(insn)),
            (sel_sched_p() ? (sched_emulate_haifa_p ? -1 : insn_cost(insn))
                           : insn_cost(insn)));
...
static void schedule_region(int rgn) {
  int bb;
  ...
  gcc_assert(haifa_recovery_bb_ever_added_p || deps_pools_are_empty_p());
}

/* Initialize data structures for region scheduling.  */

void sched_rgn_init(bool single_blocks_p) {
  min_spec_prob = ((PARAM_VALUE(PARAM_MIN_SPEC_PROB) * REG_BR_PROB_BASE) / 100);
  ...
  if (single_blocks_p || n_basic_blocks == NUM_FIXED_BLOCKS + 1 ||
      !flag_schedule_interblock || is_cfg_nonregular()) {
  ...
  RGN_BLOCKS(nr_regions) =
      (RGN_BLOCKS(nr_regions - 1) + RGN_NR_BLOCKS(nr_regions - 1));
}
...
void sched_rgn_compute_dependencies(int rgn) {
...
/* Init region data structures.  Returns true if this region should
   not be scheduled.  */
void sched_rgn_local_init(int rgn) {
  int bb;
...
void rgn_setup_common_sched_info(void) {
  ...
  rgn_common_sched_info.estimate_number_of_insns = rgn_estimate_number_of_insns;
...
static void rgn_add_remove_insn(rtx insn, int remove_p) {
  if (!remove_p)
    rgn_n_insns++;
  else
    rgn_n_insns--;
...
void rgn_make_new_region_out_of_new_block(basic_block bb) {
  int i;
...
static void rgn_add_block(basic_block bb, basic_block after) {
  extend_regions();
  ...
  if (after == 0 || after == EXIT_BLOCK_PTR) {
    ...
    memmove(rgn_bb_table + pos + 1, rgn_bb_table + pos,
            ((RGN_BLOCKS(nr_regions) - 1) - (pos) + 1) * sizeof(*rgn_bb_table));
...
static void rgn_fix_recovery_cfg(int bbi, int check_bbi, int check_bb_nexti) {
  ...
  memmove(rgn_bb_table + new_pos + 1, rgn_bb_table + new_pos,
...
static basic_block advance_target_bb(basic_block bb, rtx insn) {
  if (insn)
    return 0;
...
static bool gate_handle_sched(void) {
#ifdef INSN_SCHEDULING
...
static bool gate_handle_sched2(void) {
#ifdef INSN_SCHEDULING
