...
int ebitmap_last_set_bit(ebitmap map) {
  unsigned int i = 0;
...
static inline void ebitmap_array_grow(ebitmap map, unsigned int newsize) {
...
static inline void ebitmap_array_maybe_grow(ebitmap map, unsigned int size) {
  if (size >= map->n_elts)
...
static inline EBITMAP_ELT_TYPE ebitmap_array_get(ebitmap map,
                                                 unsigned int idx) {
...
static inline EBITMAP_ELT_TYPE *ebitmap_array_grow_get(ebitmap map,
                                                       unsigned int idx) {
  if (idx >= map->n_elts)
...
static inline void ebitmap_array_init(ebitmap map, unsigned int size) {
  if (size > 0) {
...
static inline void ebitmap_array_clear(ebitmap map) {
  if (map->elts) {
    free(map->elts);
    map->elts = NULL;
  }
...
void ebitmap_clear(ebitmap map) {
...
ebitmap ebitmap_alloc(unsigned int size) {
...
void ebitmap_clear_bit(ebitmap map, unsigned int bit) {
  ...
  if (wordindex >= map->wordmask->n_bits || !TEST_BIT(map->wordmask, wordindex))
    return;
...
void ebitmap_set_bit(ebitmap map, unsigned int bit) {
  unsigned int wordindex = bit / EBITMAP_ELT_BITS;
...
bool ebitmap_bit_p(ebitmap map, unsigned int bit) {
  unsigned int wordindex = bit / EBITMAP_ELT_BITS;
  unsigned int bitindex = bit % EBITMAP_ELT_BITS;
...
void ebitmap_copy(ebitmap dst, ebitmap src) {
  ...
  memcpy(dst->elts, src->elts, src->numwords * sizeof(EBITMAP_ELT_TYPE));
...
void dump_ebitmap(FILE *file, ebitmap bmap) {
  unsigned int pos;
  unsigned int i;
  int res;
  unsigned int size;
...
void ebitmap_and_into(ebitmap dst, ebitmap src) {
...
#ifdef EBITMAP_DEBUGGING
  {
    ...
    gcc_assert(sbitmap_popcount(dst->wordmask, dst->wordmask->n_bits) ==
               dst->numwords);
  }
#endif
...
void ebitmap_and(ebitmap dst, ebitmap src1, ebitmap src2) {
  sbitmap_iterator sbi;
  unsigned int i;
  unsigned int neweltindex = 0;
  ...
  dst->wordmask = sbitmap_resize(
      dst->wordmask, MIN(src1->wordmask->n_bits, src2->wordmask->n_bits), 0);
...
bool ebitmap_ior_into(ebitmap dst, ebitmap src) {
  ...
  if (srcsize == dstsize) {
    ...
    dst->wordmask = sbitmap_resize(dst->wordmask, MAX(srcsize, dstsize), 0);
    if (srcsize >= dstsize) {
  ...
  EXECUTE_IF_SET_IN_SBITMAP(tempmask, 0, i, sbi) {
    ...
    dsthasword = (i < dst->wordmask->n_bits && TEST_BIT(dst->wordmask, i));
    srchasword = (i < src->wordmask->n_bits && TEST_BIT(src->wordmask, i));
...
bool ebitmap_ior(ebitmap dst, ebitmap src1, ebitmap src2) {
  ...
  EXECUTE_IF_SET_IN_SBITMAP(tempmask, 0, i, sbi) {
    ...
    src1hasword = (i < src1->wordmask->n_bits && TEST_BIT(src1->wordmask, i));
    src2hasword = (i < src2->wordmask->n_bits && TEST_BIT(src2->wordmask, i));
...
bool ebitmap_and_compl_into(ebitmap dst, ebitmap src) {
  bool changed = false;
  unsigned int i;
  unsigned int neweltindex = 0;
  unsigned int dsteltindex = 0;
...
bool ebitmap_and_compl(ebitmap dst, ebitmap src1, ebitmap src2) {
  unsigned int src1size = src1->wordmask->n_bits;
  sbitmap_iterator sbi;
  unsigned int i;
  sbitmap tempmask;
...
bool ebitmap_ior_and_compl(ebitmap dst, ebitmap a, ebitmap b, ebitmap c) {
  bool changed;
...
bool ebitmap_equal_p(ebitmap dst, ebitmap src) {
