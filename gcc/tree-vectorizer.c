...
void rename_variables_in_loop(struct loop *loop) {
  unsigned i;
  basic_block *bbs;
...
static void slpeel_update_phis_for_duplicate_loop(struct loop *orig_loop,
  ...
  for (gsi_new = gsi_start_phis(new_loop->header),
    ...
    if (!after) {
      ...
      SET_PHI_ARG_DEF(phi_orig, new_loop_exit_e->dest_idx, new_ssa_name);
    }
  }
}
...
static void slpeel_update_phi_nodes_for_guard1(edge guard_edge,
  ...
  *new_exit_bb = split_edge(single_exit(loop));
  ...
  for (gsi_orig = gsi_start_phis(orig_bb),
    ...
    new_phi = create_phi_node(SSA_NAME_VAR(PHI_RESULT(orig_phi)), new_merge_bb);
...
static void slpeel_update_phi_nodes_for_guard2(edge guard_edge,
  ...
  *new_exit_bb = split_edge(single_exit(loop));
  ...
  for (gsi = gsi_start_phis(update_bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    new_phi =
        create_phi_node(SSA_NAME_VAR(PHI_RESULT(orig_phi)), guard_edge->src);
    ...
    gcc_assert(PHI_ARG_DEF_FROM_EDGE(update_phi2, guard_edge) == guard_arg);
...
void slpeel_make_loop_iterate_ntimes(struct loop *loop, tree niters) {
  ...
  indx_after_incr = force_gimple_operand_gsi(
      &loop_cond_gsi, indx_after_incr, true, NULL_TREE, true, GSI_SAME_STMT);
  niters = force_gimple_operand_gsi(&loop_cond_gsi, niters, true, NULL_TREE,
                                    true, GSI_SAME_STMT);

  code = (exit_edge->flags & EDGE_TRUE_VALUE) ? GE_EXPR : LT_EXPR;
  cond_stmt =
      gimple_build_cond(code, indx_after_incr, niters, NULL_TREE, NULL_TREE);
...
struct loop *slpeel_tree_duplicate_loop_to_edge_cfg(struct loop *loop, edge e) {
  struct loop *new_loop;
  ...
  was_imm_dom =
      (get_immediate_dominator(CDI_DOMINATORS, exit_dest) == loop->header
           ? true
           : false);
  ...
  copy_bbs(bbs, loop->num_nodes, new_bbs, &exit, 1, &new_exit, NULL, e->src);
  ...
  } else /* Add the copy at entry.  */
  {
    ...
    if (!flow_bb_inside_loop_p(new_loop, EDGE_SUCC(new_loop->header, 0)->dest))
    ...
    set_immediate_dominator(CDI_DOMINATORS, loop->header, new_exit_e->src);
...
static edge slpeel_add_loop_guard(basic_block guard_bb, tree cond,
  ...
  cond_stmt = gimple_build_cond(
      NE_EXPR, cond, build_int_cst(TREE_TYPE(cond), 0), NULL_TREE, NULL_TREE);
...
bool slpeel_can_duplicate_loop_p(const struct loop *loop, const_edge e) {
  ...
  if (loop->inner
      /* All loops have an outer scope; the only case loop->outer is NULL is for
         the function itself.  */
      || !loop_outer(loop) || loop->num_nodes != 2 ||
...
void slpeel_verify_cfg_after_peeling(struct loop *first_loop,
...
void set_prologue_iterations(basic_block bb_before_first_loop,
                             tree first_niters, struct loop *loop,
                             unsigned int th) {
  edge e;
  ...
  e_false =
      make_single_succ_edge(cond_bb, bb_before_first_loop, EDGE_FALSE_VALUE);
  ...
  cost_pre_condition = force_gimple_operand(
      cost_pre_condition, &gimplify_stmt_list, true, NULL_TREE);
  cond_stmt = gimple_build_cond(NE_EXPR, cost_pre_condition,
                                build_int_cst(TREE_TYPE(cost_pre_condition), 0),
                                NULL_TREE, NULL_TREE);
...
struct loop *slpeel_tree_peel_loop_to_edge(struct loop *loop, edge e,
                                           tree first_niters, tree niters,
  ...
  */
  ...
  if (!update_first_loop_count) {
    pre_condition = fold_build2(LE_EXPR, boolean_type_node, first_niters,
                                build_int_cst(TREE_TYPE(first_niters), 0));
    if (check_profitability) {
      tree scalar_loop_iters = unshare_expr(
  ...
  /* Prologue peeling.  */
    ...
    if (check_profitability)
      set_prologue_iterations(bb_before_first_loop, first_niters, loop, th);
  ...
  pre_condition = fold_build2(EQ_EXPR, boolean_type_node, first_niters, niters);
...
LOC find_loop_location(struct loop *loop) {
  gimple stmt = NULL;
  basic_block bb;
...
void vect_set_verbosity_level(const char *val) {
  unsigned int vl;
...
bool vect_print_dump_info(enum verbosity_levels vl) {
  ...
  if (vect_loop_location == UNKNOWN_LOC)
    fprintf(vect_dump,
            "\n%s:%d: note: ", DECL_SOURCE_FILE(current_function_decl),
...
stmt_vec_info new_stmt_vec_info(gimple stmt, loop_vec_info loop_vinfo) {
  stmt_vec_info res;
  ...
  if (gimple_code(stmt) == GIMPLE_PHI && is_loop_header_bb_p(gimple_bb(stmt)))
...
loop_vec_info new_loop_vec_info(struct loop *loop) {
  ...
  nbbs = dfs_enumerate_from(loop->header, 0, bb_in_loop_p, bbs, loop->num_nodes,
                            loop);
  ...
  LOOP_VINFO_MAY_MISALIGN_STMTS(res) = VEC_alloc(
      gimple, heap, PARAM_VALUE(PARAM_VECT_MAX_VERSION_FOR_ALIGNMENT_CHECKS));
  LOOP_VINFO_MAY_ALIAS_DDRS(res) = VEC_alloc(
      ddr_p, heap, PARAM_VALUE(PARAM_VECT_MAX_VERSION_FOR_ALIAS_CHECKS));
...
void destroy_loop_vec_info(loop_vec_info loop_vinfo, bool clean_stmts) {
  struct loop *loop;
  basic_block *bbs;
  int nbbs;
  gimple_stmt_iterator si;
    ...
    for (si = gsi_start_bb(bb); !gsi_end_p(si);) {
      ...
      if (stmt_info) {
        ...
        if (orig_stmt) {
          ...
          if (orig_stmt_info && STMT_VINFO_IN_PATTERN_P(orig_stmt_info))
            remove_stmt_p = true;
        }
...
bool vect_can_force_dr_alignment_p(const_tree decl, unsigned int alignment) {
...
tree get_vectype_for_scalar_type(tree scalar_type) {
...
enum dr_alignment_support vect_supportable_dr_alignment(
...
bool vect_is_simple_use(tree operand, loop_vec_info loop_vinfo,
...
bool supportable_widening_operation(enum tree_code code, gimple stmt,
                                    tree vectype, tree *decl1, tree *decl2,
                                    enum tree_code *code1,
                                    enum tree_code *code2, int *multi_step_cvt,
  ...
  if (!ordered_p && code == WIDEN_MULT_EXPR &&
  ...
  if ((icode1 = optab_handler(optab1, vec_mode)->insn_code) ==
          CODE_FOR_nothing ||
      (icode2 = optab_handler(optab2, vec_mode)->insn_code) == CODE_FOR_nothing)
    return false;
    ...
    *interm_types = VEC_alloc(tree, heap, MAX_INTERM_CVT_STEPS);
    for (i = 0; i < 3; i++) {
      ...
      intermediate_type = lang_hooks.types.type_for_mode(
          intermediate_mode, TYPE_UNSIGNED(prev_type));
...
bool supportable_narrowing_operation(enum tree_code code, const_gimple stmt,
                                     tree vectype, enum tree_code *code1,
                                     int *multi_step_cvt,
                                     VEC(tree, heap) * *interm_types) {
  enum machine_mode vec_mode;
  ...
  if (insn_data[icode1].operand[0].mode != TYPE_MODE(narrow_vectype)) {
    ...
    *interm_types = VEC_alloc(tree, heap, MAX_INTERM_CVT_STEPS);
    for (i = 0; i < 3; i++) {
      ...
      intermediate_type = lang_hooks.types.type_for_mode(
          intermediate_mode, TYPE_UNSIGNED(prev_type));
      interm_optab = optab_for_tree_code(c1, intermediate_type, optab_default);
      if (!interm_optab ||
          (icode1 = optab1->handlers[(int)prev_mode].insn_code) ==
              CODE_FOR_nothing ||
          insn_data[icode1].operand[0].mode != intermediate_mode ||
          (icode1 = interm_optab->handlers[(int)intermediate_mode].insn_code) ==
              CODE_FOR_nothing)
        return false;
...
bool reduction_code_for_scalar_code(enum tree_code code,
...
static void report_vect_op(gimple stmt, const char *msg) {
...
gimple vect_is_simple_reduction(loop_vec_info loop_info, gimple phi) {
  ...
  if (def2 == phi && flow_bb_inside_loop_p(loop, gimple_bb(def1)) &&
    ...
    return def_stmt;
  } else if (def1 == phi && flow_bb_inside_loop_p(loop, gimple_bb(def2)) &&
    ...
    /* Swap operands (just for simplicity - so that the rest of the code
       ...
       argument).  */
    if (vect_print_dump_info(REPORT_DETAILS))
      report_vect_op(def_stmt, "detected reduction: need to swap operands:");
...
bool vect_is_simple_iv_evolution(unsigned loop_nb, tree access_fn, tree *init,
                                 tree *step) {
  tree init_expr;
  tree step_expr;
...
static bool gate_increase_alignment(void) {
  return flag_section_anchors && flag_tree_vectorize;
}

struct simple_ipa_opt_pass pass_ipa_increase_alignment = {{
    SIMPLE_IPA_PASS, "increase_alignment", /* name */
