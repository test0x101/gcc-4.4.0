...
static void update_phis_for_loop_copy(struct loop *orig_loop,
...
static struct loop *copy_loop_before(struct loop *loop) {
  struct loop *res;
...
static void create_bb_after_loop(struct loop *loop) {
...
static bool generate_loops_for_partition(struct loop *loop, bitmap partition,
                                         bool copy_p) {
  unsigned i, x;
...
static inline tree build_size_arg(tree nb_iter, tree op,
                                  gimple_seq *stmt_list) {
  tree nb_bytes;
  gimple_seq stmts = NULL;

  nb_bytes = fold_build2(MULT_EXPR, TREE_TYPE(nb_iter), nb_iter,
...
static bool generate_memset_zero(gimple stmt, tree op0, tree nb_iter,
                                 gimple_stmt_iterator bsi) {
  tree t, addr_base;
  tree nb_bytes = NULL;
  bool res = false;
  ...
  if (integer_zerop(fold_build2(MINUS_EXPR, integer_type_node, DR_STEP(dr),
    ...
    tree offset = fold_convert(
        sizetype, size_binop(PLUS_EXPR, DR_OFFSET(dr), DR_INIT(dr)));
    addr_base = fold_build2(POINTER_PLUS_EXPR, TREE_TYPE(DR_BASE_ADDRESS(dr)),
...
static void prop_phis(basic_block b) {
...
static bool generate_builtin(struct loop *loop, bitmap partition, bool copy_p) {
  bool res = false;
  unsigned i, x = 0;
  basic_block *bbs;
  gimple write = NULL;
  ...
  for (i = 0; i < loop->num_nodes; i++) {
    ...
    for (bsi = gsi_start_bb(bb); !gsi_end_p(bsi); gsi_next(&bsi)) {
      ...
      if (bitmap_bit_p(partition, x++) && is_gimple_assign(stmt) &&
...
static bool generate_code_for_partition(struct loop *loop, bitmap partition,
                                        bool copy_p) {
...
static bool rdg_cannot_recompute_vertex_p(struct graph *rdg, int v) {
...
static inline bool already_processed_vertex_p(bitmap processed, int v) {
  return (bitmap_bit_p(processed, v) || !bitmap_bit_p(remaining_stmts, v));
}
...
static struct graph_edge *has_anti_dependence(struct vertex *v) {
  struct graph_edge *e;
...
static bool predecessor_has_mem_write(struct graph *rdg, struct vertex *v) {
  struct graph_edge *e;
...
static void mark_nodes_having_upstream_mem_writes(struct graph *rdg) {
  int v, x;
...
static bool has_upstream_mem_writes(int u) {
...
static void rdg_flag_all_uses(struct graph *rdg, int u, bitmap partition,
  ...
  for (e = rdg->vertices[u].succ; e; e = e->succ_next)
    if (!bitmap_bit_p(processed, e->dest)) {
      rdg_flag_vertex_and_dependent(rdg, e->dest, partition, loops, processed,
                                    part_has_writes);
...
static void rdg_flag_uses(struct graph *rdg, int u, bitmap partition,
                          bitmap loops, bitmap processed,
                          bool *part_has_writes) {
  ssa_op_iter iter;
  use_operand_p use_p;
  ...
  if (gimple_code(stmt) != GIMPLE_PHI) {
    FOR_EACH_SSA_USE_OPERAND(use_p, stmt, iter, SSA_OP_VIRTUAL_USES) {
      ...
      if (TREE_CODE(use) == SSA_NAME) {
        ...
        if (v >= 0 && !already_processed_vertex_p(processed, v))
          rdg_flag_vertex_and_dependent(rdg, v, partition, loops, processed,
                                        part_has_writes);
      }
    }
...
static void rdg_flag_vertex(struct graph *rdg, int v, bitmap partition,
                            bitmap loops, bool *part_has_writes) {
  struct loop *loop;
...
static void rdg_flag_vertex_and_dependent(struct graph *rdg, int v,
                                          bitmap partition, bitmap loops,
                                          bitmap processed,
                                          bool *part_has_writes) {
  unsigned i;
  VEC(int, heap) *nodes = VEC_alloc(int, heap, 3);
...
static void collect_condition_stmts(struct loop *loop,
...
static void rdg_flag_loop_exits(struct graph *rdg, bitmap loops,
                                bitmap partition, bitmap processed,
                                bool *part_has_writes) {
  unsigned i;
  bitmap_iterator bi;
...
static void rdg_flag_similar_memory_accesses(struct graph *rdg,
                                             bitmap partition, bitmap loops,
                                             bitmap processed,
...
static bitmap build_rdg_partition_for_component(struct graph *rdg, rdgc c,
                                                bool *part_has_writes,
                                                VEC(int, heap) *
                                                    *other_stores) {
  int i, v;
...
static void free_rdg_components(VEC(rdgc, heap) * components) {
  int i;
  rdgc x;
...
static void rdg_build_partitions(struct graph *rdg,
  ...
  for (i = 0; i < rdg->n_vertices; i++)
    if (!bitmap_bit_p(processed, i) && rdg_defs_used_in_other_loops_p(rdg, i))
...
static void dump_rdg_partitions(FILE *file, VEC(bitmap, heap) * partitions) {
  int i;
  bitmap partition;
...
void debug_rdg_partitions(VEC(bitmap, heap) * partitions) {
...
static int number_of_rw_in_partition(struct graph *rdg, bitmap partition) {
  int res = 0;
  unsigned i;
  bitmap_iterator ii;
...
static int ldist_gen(struct loop *loop, struct graph *rdg,
                     VEC(int, heap) * starting_vertices) {
  int i, nbp;
  ...
  rdg_build_partitions(rdg, components, &other_stores, &partitions, processed);
  BITMAP_FREE(processed);
  nbp = VEC_length(bitmap, partitions);

  if (nbp <= 1 || partition_contains_all_rw(rdg, partitions))
    goto ldist_done;
...
static int distribute_loop(struct loop *loop, VEC(gimple, heap) * stmts) {
  bool res = false;
  struct graph *rdg;
  gimple s;
  unsigned i;
  ...
  for (i = 0; VEC_iterate(gimple, stmts, i, s); i++) {
    ...
    if (v >= 0) {
      ...
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf(dump_file, "ldist asked to generate code for vertex %d\n", v);
    }
  }
...
static unsigned int tree_loop_distribution(void) {
  struct loop *loop;
  loop_iterator li;
