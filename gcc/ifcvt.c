...
#define MAX_CONDITIONAL_EXECUTE \
  (BRANCH_COST(optimize_function_for_speed_p(cfun), false) + 1)
#endif
...
static bool cheap_bb_rtx_cost_p(const_basic_block bb, int max_cost) {
  int count = 0;
  rtx insn = BB_HEAD(bb);
...
static rtx first_active_insn(basic_block bb) {
  rtx insn = BB_HEAD(bb);
...
static rtx last_active_insn(basic_block bb, int skip_use_p) {
  rtx insn = BB_END(bb);
  rtx head = BB_HEAD(bb);

  while (
      NOTE_P(insn) || JUMP_P(insn) ||
...
static int cond_exec_process_insns(
  ...
  for (insn = start;; insn = NEXT_INSN(insn)) {
    ...
    if (CALL_P(insn) && prob_val)
      validate_change(insn, &REG_NOTES(insn),
                      alloc_EXPR_LIST(REG_BR_PROB, prob_val, REG_NOTES(insn)),
                      1);
...
static rtx cond_exec_get_condition(rtx jump) {
  rtx test_if, cond;
...
static int cond_exec_process_if_block(
  ...
  if (ce_info->num_multiple_test_blocks > 0) {
    ...
    do {
      ...
      if (start && !cond_exec_process_insns(ce_info, start, end, false_expr,
  ...
  if (then_end && (!false_expr || !cond_exec_process_insns(
                                      ce_info, then_start, then_end, false_expr,
                                      false_prob_val, then_mod_ok)))
    goto fail;

  if (else_bb && else_end &&
      !cond_exec_process_insns(ce_info, else_start, else_end, true_expr,
                               true_prob_val, TRUE))
    goto fail;
...
static rtx noce_emit_cmove(struct noce_if_info *, rtx, enum rtx_code, rtx, rtx,
                           rtx, rtx);
...
static rtx noce_emit_store_flag(struct noce_if_info *if_info, rtx x,
                                int reversep, int normalize) {
  ...
  if ((if_info->cond_earliest == if_info->jump || cond_complex) &&
    ...
    tmp = gen_rtx_fmt_ee(code, GET_MODE(x), XEXP(cond, 0), XEXP(cond, 1));
  ...
  return emit_store_flag(
      x, code, XEXP(cond, 0), XEXP(cond, 1), VOIDmode,
      (code == LTU || code == LEU || code == GEU || code == GTU), normalize);
}

/* Emit instruction to move an rtx, possibly into STRICT_LOW_PART.
   X is the destination/target and Y is the value to copy.  */

static void noce_emit_move_insn(rtx x, rtx y) {
    ...
    if (recog_memoized(insn) <= 0) {
      ...
      switch (GET_RTX_CLASS(GET_CODE(y))) {
        ...
        case RTX_COMM_ARITH:
          ...
          if (ot) {
            start_sequence();
            target = expand_binop(GET_MODE(y), ot, XEXP(y, 0), XEXP(y, 1), x, 0,
                                  OPTAB_DIRECT);
            if (target != NULL_RTX) {
              if (target != x)
                emit_move_insn(x, target);
...
static rtx end_ifcvt_sequence(struct noce_if_info *if_info) {
  rtx insn;
  rtx seq = get_insns();
  ...
  for (insn = seq; insn; insn = NEXT_INSN(insn))
    if (JUMP_P(insn) || recog_memoized(insn) == -1)
      return NULL_RTX;
...
static int noce_try_move(struct noce_if_info *if_info) {
  rtx cond = if_info->cond;
...
static int noce_try_store_flag(struct noce_if_info *if_info) {
  int reversep;
  rtx target, seq;
  ...
  if (GET_CODE(if_info->b) == CONST_INT &&
    ...
    reversep = 0;
  else if (if_info->b == const0_rtx && GET_CODE(if_info->a) == CONST_INT &&
...
static int noce_try_store_flag_constants(struct noce_if_info *if_info) {
  rtx target, seq;
  int reversep;
  ...
  if (GET_CODE(if_info->a) == CONST_INT && GET_CODE(if_info->b) == CONST_INT) {
    ...
    /* if (test) x = 3; else x = 4;
       =>   x = 3 + (test == 0);  */
    if (diff == STORE_FLAG_VALUE || diff == -STORE_FLAG_VALUE) {
      target = expand_simple_binop(
          mode, (diff == STORE_FLAG_VALUE ? PLUS : MINUS), GEN_INT(ifalse),
    ...
    /* if (test) x = -1; else x = b;
      ...
      target = expand_simple_binop(mode, IOR, target, GEN_INT(ifalse),
                                   if_info->x, 0, OPTAB_WIDEN);
    }
    ...
    /* if (test) x = a; else x = b;
      ...
      target = expand_simple_binop(mode, AND, target, GEN_INT(diff), if_info->x,
                                   0, OPTAB_WIDEN);
      if (target)
        target = expand_simple_binop(mode, PLUS, target, GEN_INT(ifalse),
                                     if_info->x, 0, OPTAB_WIDEN);
    }
...
static int noce_try_addcc(struct noce_if_info *if_info) {
  rtx target, seq;
  ...
  if (GET_CODE(if_info->a) == PLUS &&
    ...
    if (general_operand(XEXP(cond, 0), VOIDmode) &&
        general_operand(XEXP(cond, 1), VOIDmode)) {
      start_sequence();
      target = emit_conditional_add(
          if_info->x, code, XEXP(cond, 0), XEXP(cond, 1), VOIDmode, if_info->b,
          XEXP(if_info->a, 1), GET_MODE(if_info->x),
    ...
    /* If that fails, construct conditional increment or decrement using
       setcc.  */
    if (if_info->branch_cost >= 2 && (XEXP(if_info->a, 1) == const1_rtx ||
      ...
      target = noce_emit_store_flag(if_info, gen_reg_rtx(GET_MODE(if_info->x)),
                                    1, normalize);

      if (target)
        target =
            expand_simple_binop(GET_MODE(if_info->x), subtract ? MINUS : PLUS,
                                if_info->b, target, if_info->x, 0, OPTAB_WIDEN);
      if (target) {
        if (target != if_info->x)
          noce_emit_move_insn(if_info->x, target);
...
static int noce_try_store_flag_mask(struct noce_if_info *if_info) {
  rtx target, seq;
  int reversep;

  reversep = 0;
  if ((if_info->branch_cost >= 2 || STORE_FLAG_VALUE == -1) &&
      ((if_info->a == const0_rtx && rtx_equal_p(if_info->b, if_info->x)) ||
       ((reversep = (reversed_comparison_code(if_info->cond, if_info->jump) !=
                     UNKNOWN)) &&
        if_info->b == const0_rtx && rtx_equal_p(if_info->a, if_info->x)))) {
    start_sequence();
    target = noce_emit_store_flag(if_info, gen_reg_rtx(GET_MODE(if_info->x)),
                                  reversep, -1);
    if (target)
      target = expand_simple_binop(GET_MODE(if_info->x), AND, if_info->x,
                                   target, if_info->x, 0, OPTAB_WIDEN);
...
static rtx noce_emit_cmove(struct noce_if_info *if_info, rtx x,
                           enum rtx_code code, rtx cmp_a, rtx cmp_b, rtx vfalse,
                           rtx vtrue) {
  ...
  return emit_conditional_move(
      x, code, cmp_a, cmp_b, VOIDmode, vtrue, vfalse, GET_MODE(x),
      (code == LTU || code == GEU || code == LEU || code == GTU));
#else
...
static int noce_try_cmove(struct noce_if_info *if_info) {
  enum rtx_code code;
  rtx target, seq;
  ...
  if ((CONSTANT_P(if_info->a) || register_operand(if_info->a, VOIDmode)) &&
    ...
    target = noce_emit_cmove(if_info, if_info->x, code, XEXP(if_info->cond, 0),
                             XEXP(if_info->cond, 1), if_info->a, if_info->b);
...
static int noce_try_cmove_arith(struct noce_if_info *if_info) {
  rtx a = if_info->a;
  rtx b = if_info->b;
  rtx x = if_info->x;
  rtx orig_a, orig_b;
  ...
  if (cse_not_expected && MEM_P(a) && MEM_P(b) && if_info->branch_cost >= 5) {
    a = XEXP(a, 0);
    b = XEXP(b, 0);
    x = gen_reg_rtx(Pmode);
    is_mem = 1;
  ...
  */
  ...
  if (is_mem) {
    ...
    set_mem_align(tmp, MIN(MEM_ALIGN(if_info->a), MEM_ALIGN(if_info->b)));
...
static rtx noce_get_alt_condition(struct noce_if_info *if_info, rtx target,
                                  rtx *earliest) {
  rtx cond, set, insn;
  int reverse;
  ...
  if (GET_CODE(target) == CONST_INT) {
    ...
    /* If we made any changes, generate a new conditional that is
       ...
       constants in it.  */
    if (code != GET_CODE(if_info->cond) || op_a != XEXP(if_info->cond, 0) ||
  ...
  cond = canonicalize_condition(if_info->jump, cond, reverse, earliest, target,
                                false, true);
  ...
  for (insn = *earliest; insn != if_info->jump; insn = NEXT_INSN(insn))
    if (INSN_P(insn) &&
        (modified_in_p(if_info->a, insn) || modified_in_p(if_info->b, insn)))
      return NULL;
...
static int noce_try_minmax(struct noce_if_info *if_info) {
  ...
  target = expand_simple_binop(GET_MODE(if_info->x), op, if_info->a, if_info->b,
...
static int noce_try_abs(struct noce_if_info *if_info) {
  ...
  if (REG_P(c)) {
    ...
    if (insn && BLOCK_NUM(insn) == BLOCK_NUM(earliest) &&
  ...
  if (MEM_P(c) && GET_CODE(XEXP(c, 0)) == SYMBOL_REF &&
...
static int noce_try_sign_mask(struct noce_if_info *if_info) {
  rtx cond, t, m, c, seq;
  enum machine_mode mode;
  enum rtx_code code;
...
static int noce_try_bitop(struct noce_if_info *if_info) {
  ...
  if (GET_CODE(cond) == ZERO_EXTRACT) {
    if (XEXP(cond, 1) != const1_rtx || GET_CODE(XEXP(cond, 2)) != CONST_INT ||
  ...
  if (GET_CODE(a) == IOR || GET_CODE(a) == XOR) {
    ...
    if (!rtx_equal_p(x, XEXP(a, 0)) || GET_CODE(XEXP(a, 1)) != CONST_INT ||
        (INTVAL(XEXP(a, 1)) & GET_MODE_MASK(mode)) != (unsigned HOST_WIDE_INT)1
                                                          << bitnum)
      return FALSE;
...
static rtx noce_get_condition(rtx jump, rtx *earliest,
                              bool then_else_reversed) {
  rtx cond, set, tmp;
  bool reverse;
  ...
  if (REG_P(tmp) && GET_MODE_CLASS(GET_MODE(tmp)) == MODE_INT) {
    *earliest = jump;
    ...
    if (reverse)
      cond = gen_rtx_fmt_ee(reverse_condition(GET_CODE(cond)), GET_MODE(cond),
                            tmp, XEXP(cond, 1));
    return cond;
  }
  ...
  return canonicalize_condition(jump, cond, reverse, earliest, NULL_RTX, false,
                                true);
}

/* Return true if OP is ok for if-then-else processing.  */

static int noce_operand_ok(const_rtx op) {
...
static bool noce_mem_write_may_trap_or_fault_p(const_rtx mem) {
  rtx addr;
...
static bool noce_can_store_speculate_p(basic_block top_bb, const_rtx mem) {
  ...
  for (dominator = get_immediate_dominator(CDI_POST_DOMINATORS, top_bb);
    ...
    FOR_BB_INSNS(dominator, insn) {
      ...
      if (INSN_P(insn) && (volatile_insn_p(PATTERN(insn)) ||
...
static int noce_process_if_block(struct noce_if_info *if_info) {
  ...
  if (!insn_a || insn_a != last_active_insn(then_bb, FALSE) ||
  ...
  if (else_bb) {
    ...
    if (!insn_b || insn_b != last_active_insn(else_bb, FALSE) ||
    ...
    /* We're going to be moving the evaluation of B down from above
       ...
       intact.  */
    if (!insn_b || BLOCK_NUM(insn_b) != BLOCK_NUM(if_info->cond_earliest) ||
        !NONJUMP_INSN_P(insn_b) || (set_b = single_set(insn_b)) == NULL_RTX ||
        !rtx_equal_p(x, SET_DEST(set_b)) || !noce_operand_ok(SET_SRC(set_b)) ||
        reg_overlap_mentioned_p(x, SET_SRC(set_b)) ||
        modified_between_p(SET_SRC(set_b), PREV_INSN(if_info->cond_earliest),
                           jump)
        /* Likewise with X.  In particular this can happen when
           noce_get_condition looks farther back in the instruction
           stream than one might expect.  */
        || reg_overlap_mentioned_p(x, cond) || reg_overlap_mentioned_p(x, a) ||
  ...
  if (!REG_P(x) ||
    ...
    if (GET_CODE(x) == ZERO_EXTRACT && (GET_CODE(XEXP(x, 1)) != CONST_INT ||
                                        GET_CODE(XEXP(x, 2)) != CONST_INT))
      return FALSE;

    x = gen_reg_rtx(GET_MODE(GET_CODE(x) == STRICT_LOW_PART ? XEXP(x, 0) : x));
  }
...
static int check_cond_move_block(basic_block bb, rtx *vals,
                                 VEC(int, heap) * *regs, rtx cond) {
  rtx insn;
  ...
  FOR_BB_INSNS(bb, insn) {
    ...
    dest = SET_DEST(set);
    src = SET_SRC(set);
    if (!REG_P(dest) || (SMALL_REGISTER_CLASSES && HARD_REGISTER_P(dest)))
      return FALSE;
    ...
    if ((REG_P(src) && vals[REGNO(src)] != NULL) ||
...
static bool cond_move_convert_if_block(struct noce_if_info *if_infop,
                                       basic_block bb, rtx cond, rtx *then_vals,
                                       rtx *else_vals, bool else_block_p) {
  enum rtx_code code;
  ...
  FOR_BB_INSNS(bb, insn) {
    ...
    target = noce_emit_cmove(if_infop, dest, code, cond_arg0, cond_arg1, t, e);
    if (!target)
      return false;
...
static int cond_move_process_if_block(struct noce_if_info *if_info) {
  ...
  for (i = 0; VEC_iterate(int, then_regs, i, reg); i++) {
    ...
    if (!else_vals[reg])
      ...
      if (!CONSTANT_P(then_vals[reg]) && !CONSTANT_P(else_vals[reg]) &&
  ...
  start_sequence();
  if (!cond_move_convert_if_block(if_info, then_bb, cond, then_vals, else_vals,
                                  false) ||
      (else_bb && !cond_move_convert_if_block(if_info, else_bb, cond, then_vals,
                                              else_vals, true))) {
    end_sequence();
    VEC_free(int, heap, then_regs);
    VEC_free(int, heap, else_regs);
...
static int noce_find_if_block(basic_block test_bb, edge then_edge,
                              edge else_edge, int pass) {
  ...
  if (single_pred_p(then_edge->dest) && single_succ_p(then_edge->dest) &&
      single_pred_p(else_edge->dest) && single_succ_p(else_edge->dest) &&
  ...
  else if (single_pred_p(then_edge->dest) && single_succ_p(then_edge->dest) &&
  ...
  /* Recognize an IF-ELSE-JOIN block.  We can have those because the order
     ...
     cfgrtl mode).  */
  else if (single_pred_p(else_edge->dest) && single_succ_p(else_edge->dest) &&
  ...
  if (HAVE_conditional_move && cond_move_process_if_block(&if_info))
    return TRUE;
...
static void merge_if_block(struct ce_if_block *ce_info) {
  basic_block test_bb = ce_info->test_bb; /* last test block */
  basic_block then_bb = ce_info->then_bb; /* THEN */
  basic_block else_bb = ce_info->else_bb; /* ELSE or NULL */
  ...
  if (!join_bb) {
    ...
    if (EDGE_COUNT(combo_bb->succs) == 0)
      gcc_assert(find_reg_note(last, REG_NORETURN, NULL) ||
                 (NONJUMP_INSN_P(last) && GET_CODE(PATTERN(last)) == TRAP_IF &&
      ...
      gcc_assert(JUMP_P(last) ||
                 (EDGE_SUCC(combo_bb, 0)->dest == EXIT_BLOCK_PTR &&
                  CALL_P(last) && SIBLING_CALL_P(last)) ||
  ...
  else if (EDGE_COUNT(join_bb->preds) < 2 && join_bb != EXIT_BLOCK_PTR) {
...
static basic_block find_if_header(basic_block test_bb, int pass) {
  ce_if_block_t ce_info;
  edge then_edge;
  edge else_edge;
...
static int block_jumps_and_fallthru_p(basic_block cur_bb,
  ...
  while (insn != NULL_RTX) {
    ...
    if (INSN_P(insn) && !JUMP_P(insn) && GET_CODE(PATTERN(insn)) != USE &&
...
static int cond_exec_find_if_block(struct ce_if_block *ce_info) {
  ...
  if (dump_file) {
    ...
    if (else_bb)
      fprintf(dump_file, ", else %d [%d]", else_bb->index,
              BB_HEAD(else_bb) ? (int)INSN_UID(BB_HEAD(else_bb)) : -1);

    fprintf(dump_file, ", join %d [%d]", join_bb->index,
...
static int find_cond_trap(basic_block test_bb, edge then_edge, edge else_edge) {
  ...
  seq = gen_cond_trap(code, copy_rtx(XEXP(cond, 0)), copy_rtx(XEXP(cond, 1)),
...
static rtx block_has_only_trap(basic_block bb) {
  rtx trap;
  ...
  if (!(trap == BB_END(bb) && GET_CODE(PATTERN(trap)) == TRAP_IF &&
...
static int find_if_case_1(basic_block test_bb, edge then_edge, edge else_edge) {
  basic_block then_bb = then_edge->dest;
  basic_block else_bb = else_edge->dest;
  basic_block new_bb;
  int then_bb_index;
  ...
  if ((BB_END(then_bb) &&
      ...
      (BB_END(else_bb) &&
       find_reg_note(BB_END(else_bb), REG_CROSSING_JUMP, NULL_RTX)))
    return FALSE;
  ...
  if (!cheap_bb_rtx_cost_p(then_bb, COSTS_N_INSNS(BRANCH_COST(
                                        optimize_bb_for_speed_p(then_edge->src),
                                        predictable_edge_p(then_edge)))))
    return FALSE;

  /* Registers set are dead, or are predicable.  */
  if (!dead_or_predicable(test_bb, then_bb, else_bb, single_succ(then_bb), 1))
    return FALSE;
  ...
  if (then_bb->next_bb == else_bb && then_bb->prev_bb == test_bb &&
...
static int find_if_case_2(basic_block test_bb, edge then_edge, edge else_edge) {
  basic_block then_bb = then_edge->dest;
  basic_block else_bb = else_edge->dest;
  edge else_succ;
  rtx note;
  ...
  if (!cheap_bb_rtx_cost_p(else_bb, COSTS_N_INSNS(BRANCH_COST(
                                        optimize_bb_for_speed_p(else_edge->src),
...
static int find_memory(rtx *px, void *data ATTRIBUTE_UNUSED) {
  return MEM_P(*px);
}
...
static int dead_or_predicable(basic_block test_bb, basic_block merge_bb,
                              basic_block other_bb, basic_block new_dest,
                              int reversep) {
  ...
  if (HAVE_conditional_execution) {
    ...
    if (!cond_exec_process_insns((ce_if_block_t *)0, head, end, cond, prob_val,
                                 0))
      goto cancel;
...
#endif
  {
    ...
    if (SMALL_REGISTER_CLASSES && !reload_completed) {
      EXECUTE_IF_SET_IN_BITMAP(merge_set, 0, i, bi) {
        if (i < FIRST_PSEUDO_REGISTER && !fixed_regs[i] && !global_regs[i])
          fail = 1;
      }
    }
...
no_body:
  ...
  if (other_bb != new_dest) {
    ...
    if (reversep ? !invert_jump_1(jump, new_label)
...
static bool gate_handle_if_conversion(void) {
  return (optimize > 0) && dbg_cnt(if_conversion);
}

/* If-conversion and CFG cleanup.  */
static unsigned int rest_of_handle_if_conversion(void) {
...
static bool gate_handle_if_after_combine(void) {
  return optimize > 0 && flag_if_conversion && dbg_cnt(if_after_combine);
}

/* Rerun if-conversion, as combine may have simplified things enough
   to now meet sequence length restrictions.  */
static unsigned int rest_of_handle_if_after_combine(void) {
  if_convert();
  return 0;
}
...
struct rtl_opt_pass pass_if_after_combine = {{
    ...
    0,                               /* properties_destroyed */
    0,                               /* todo_flags_start */
    TODO_df_finish | TODO_verify_rtl_sharing | TODO_dump_func |
        TODO_ggc_collect /* todo_flags_finish */
}};

static bool gate_handle_if_after_reload(void) {
  return optimize > 0 && flag_if_conversion2 && dbg_cnt(if_after_reload);
}

static unsigned int rest_of_handle_if_after_reload(void) {
  if_convert();
  return 0;
}
