...
static inline struct scev_info_str *new_scev_info_str(
    basic_block instantiated_below, tree var) {
...
bool chrec_contains_symbols_defined_in_loop(const_tree chrec,
  ...
  if (TREE_CODE(chrec) == VAR_DECL || TREE_CODE(chrec) == PARM_DECL ||
      TREE_CODE(chrec) == FUNCTION_DECL || TREE_CODE(chrec) == LABEL_DECL ||
      TREE_CODE(chrec) == RESULT_DECL || TREE_CODE(chrec) == FIELD_DECL)
    return true;
...
static tree compute_overall_effect_of_inner_loop(struct loop *loop,
  ...
  if (evolution_fn == chrec_dont_know)
    ...
    if (inner_loop == loop || flow_loop_nested_p(loop, inner_loop)) {
...
static void set_scalar_evolution(basic_block instantiated_below, tree scalar,
                                 tree chrec) {
  tree *scalar_info;
...
static tree add_to_evolution_1(unsigned loop_nb, tree chrec_before, tree to_add,
                               gimple at_stmt) {
  ...
  switch (TREE_CODE(chrec_before)) {
    case POLYNOMIAL_CHREC:
      ...
      if (chloop == loop || flow_loop_nested_p(chloop, loop)) {
        unsigned var;
        ...
        if (chloop != loop) {
          var = loop_nb;
          left = chrec_before;
          right = SCALAR_FLOAT_TYPE_P(type) ? build_real(type, dconst0)
        ...
        left = add_to_evolution_1(loop_nb, CHREC_LEFT(chrec_before), to_add,
                                  at_stmt);
        right = CHREC_RIGHT(chrec_before);
        right = chrec_convert_rhs(chrec_type(left), right, at_stmt);
        return build_polynomial_chrec(CHREC_VARIABLE(chrec_before), left,
                                      right);
      }
...
static tree add_to_evolution(unsigned loop_nb, tree chrec_before,
                             enum tree_code code, tree to_add, gimple at_stmt) {
...
static inline tree set_nb_iterations_in_loop(struct loop *loop, tree res) {
  if (dump_file && (dump_flags & TDF_DETAILS)) {
...
gimple get_loop_exit_condition(const struct loop *loop) {
  gimple res = NULL;
...
static void select_loops_exit_conditions(VEC(gimple, heap) * *exit_conditions) {
...
static t_bool follow_ssa_edge_binary(struct loop *loop, gimple at_stmt,
                                     tree type, tree rhs0, enum tree_code code,
                                     tree rhs1, gimple halting_phi,
    ...
    case PLUS_EXPR:
      if (TREE_CODE(rhs0) == SSA_NAME) {
        if (TREE_CODE(rhs1) == SSA_NAME) {
          ...
          if (res == t_true)
            *evolution_of_loop =
                add_to_evolution(loop->num, chrec_convert(type, evol, at_stmt),
                                 code, rhs1, at_stmt);

          else if (res == t_false) {
            res = follow_ssa_edge(loop, SSA_NAME_DEF_STMT(rhs1), halting_phi,
        ...
        if (res == t_true)
          *evolution_of_loop = add_to_evolution(
              loop->num, chrec_convert(type, *evolution_of_loop, at_stmt), code,
              rhs0, at_stmt);
...
static t_bool follow_ssa_edge_expr(struct loop *loop, gimple at_stmt, tree expr,
                                   gimple halting_phi, tree *evolution_of_loop,
                                   int limit) {
  t_bool res = t_false;
  tree rhs0, rhs1;
  ...
  switch (code) {
    ...
    case SSA_NAME:
      ...
      res = follow_ssa_edge(loop, SSA_NAME_DEF_STMT(expr), halting_phi,
                            evolution_of_loop, limit);
      break;
    ...
    case ASSERT_EXPR: {
      ...
      if (TREE_CODE(op0) == SSA_NAME)
        res = follow_ssa_edge(loop, SSA_NAME_DEF_STMT(op0), halting_phi,
                              evolution_of_loop, limit);
      else
        res = t_false;
      break;
...
static t_bool follow_ssa_edge_in_rhs(struct loop *loop, gimple stmt,
                                     gimple halting_phi,
                                     tree *evolution_of_loop, int limit) {
  ...
  switch (get_gimple_rhs_class(code)) {
    case GIMPLE_BINARY_RHS:
      return follow_ssa_edge_binary(loop, stmt, type, gimple_assign_rhs1(stmt),
                                    code, gimple_assign_rhs2(stmt), halting_phi,
                                    evolution_of_loop, limit);
...
static bool backedge_phi_arg_p(gimple phi, int i) {
...
static inline t_bool follow_ssa_edge_in_condition_phi_branch(
    int i, struct loop *loop, gimple condition_phi, gimple halting_phi,
...
static t_bool follow_ssa_edge_in_condition_phi(struct loop *loop,
  ...
  t_bool res = follow_ssa_edge_in_condition_phi_branch(
      0, loop, condition_phi, halting_phi, &evolution_of_branch, init, limit);
  ...
  *evolution_of_loop = evolution_of_branch;
  ...
  for (i = 1; i < n; i++) {
    ...
    res = follow_ssa_edge_in_condition_phi_branch(
        i, loop, condition_phi, halting_phi, &evolution_of_branch, init, limit);
    if (res == t_false || res == t_dont_know)
      return res;

    *evolution_of_loop = chrec_merge(*evolution_of_loop, evolution_of_branch);
  }
  ...
  if (ev == PHI_RESULT(loop_phi_node)) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (!flow_bb_inside_loop_p(loop, bb))
        res = follow_ssa_edge_expr(outer_loop, loop_phi_node, arg, halting_phi,
                                   evolution_of_loop, limit);
      if (res == t_true)
        break;
    }
...
static t_bool follow_ssa_edge(struct loop *loop, gimple def, gimple halting_phi,
                              tree *evolution_of_loop, int limit) {
    ...
    case GIMPLE_PHI:
      if (!loop_phi_node_p(def))
        /* DEF is a condition-phi-node.  Follow the branches, and
           ...
           variable.  */
        return follow_ssa_edge_in_condition_phi(loop, def, halting_phi,
                                                evolution_of_loop, limit);
      ...
      if (flow_loop_nested_p(loop, def_loop))
        return follow_ssa_edge_inner_loop_phi(loop, def, halting_phi,
                                              evolution_of_loop, limit + 1);

      /* Outer loop.  */
      return t_false;

    case GIMPLE_ASSIGN:
      return follow_ssa_edge_in_rhs(loop, def, halting_phi, evolution_of_loop,
                                    limit);
...
static tree analyze_initial_condition(gimple loop_phi_node) {
  int i, n;
...
static tree interpret_loop_phi(struct loop *loop, gimple loop_phi_node) {
  tree res;
  ...
  if (phi_loop != loop) {
    struct loop *subloop;
    tree evolution_fn =
        analyze_scalar_evolution(phi_loop, PHI_RESULT(loop_phi_node));
...
static tree interpret_condition_phi(struct loop *loop, gimple condition_phi) {
  ...
  for (i = 0; i < n; i++) {
    ...
    branch_chrec =
        analyze_scalar_evolution(loop, PHI_ARG_DEF(condition_phi, i));
...
static tree interpret_rhs_expr(struct loop *loop, gimple at_stmt, tree type,
                               tree rhs1, enum tree_code code, tree rhs2) {
  ...
  if (get_gimple_rhs_class(code) == GIMPLE_SINGLE_RHS) {
    ...
    if (code == SSA_NAME)
      return chrec_convert(type, analyze_scalar_evolution(loop, rhs1), at_stmt);
  ...
  switch (code) {
    ...
    case BIT_NOT_EXPR:
      ...
      chrec1 = analyze_scalar_evolution(loop, rhs1);
      chrec1 = chrec_convert(type, chrec1, at_stmt);
      res = chrec_fold_minus(type, fold_convert(type, integer_minus_one_node),
                             chrec1);
      break;
...
static tree interpret_expr(struct loop *loop, gimple at_stmt, tree expr) {
  enum tree_code code;
  ...
  return interpret_rhs_expr(loop, at_stmt, type, op0, code, op1);
}

/* Interpret the rhs of the assignment STMT.  */

static tree interpret_gimple_assign(struct loop *loop, gimple stmt) {
  tree type = TREE_TYPE(gimple_assign_lhs(stmt));
  enum tree_code code = gimple_assign_rhs_code(stmt);

  return interpret_rhs_expr(loop, stmt, type, gimple_assign_rhs1(stmt), code,
...
static tree analyze_scalar_evolution_1(struct loop *loop, tree var, tree res) {
  ...
  if (bb == NULL || !flow_bb_inside_loop_p(loop, bb)) {
...
static tree analyze_scalar_evolution_in_loop(struct loop *wrto_loop,
  ...
  while (1) {
    ...
    if (!no_evolution_in_loop_p(ev, use_loop->num, &val) || !val)
      return chrec_dont_know;
...
static tree get_instantiated_value(htab_t cache, basic_block instantiated_below,
                                   tree version) {
...
static void set_instantiated_value(htab_t cache, basic_block instantiated_below,
...
static tree instantiate_scev_1(basic_block instantiate_below,
                               ...
                               bool fold_conversions, htab_t cache,
                               int size_expr) {
  ...
  if (automatically_generated_chrec_p(chrec) || is_gimple_min_invariant(chrec))
    return chrec;
  ...
  switch (TREE_CODE(chrec)) {
    case SSA_NAME:
      ...
      if (!def_bb || loop_depth(def_bb->loop_father) == 0 ||
    ...
    case POLYNOMIAL_CHREC:
      ...
      if (CHREC_LEFT(chrec) != op0 || CHREC_RIGHT(chrec) != op1) {
    ...
    case PLUS_EXPR:
      ...
      if (TREE_OPERAND(chrec, 0) != op0 || TREE_OPERAND(chrec, 1) != op1) {
  ...
  switch (TREE_CODE_LENGTH(TREE_CODE(chrec))) {
    case 3:
      ...
      op2 = instantiate_scev_1(instantiate_below, evolution_loop,
                               TREE_OPERAND(chrec, 2), fold_conversions, cache,
                               size_expr);
      if (op2 == chrec_dont_know)
        return chrec_dont_know;

      if (op0 == TREE_OPERAND(chrec, 0) && op1 == TREE_OPERAND(chrec, 1) &&
          op2 == TREE_OPERAND(chrec, 2))
        return chrec;

      return fold_build3(TREE_CODE(chrec), TREE_TYPE(chrec), op0, op1, op2);
...
tree resolve_mixers(struct loop *loop, tree chrec) {
  htab_t cache = htab_create(10, hash_scev_info, eq_scev_info, del_scev_info);
  tree ret =
      instantiate_scev_1(block_before_loop(loop), loop, chrec, true, cache, 0);
  htab_delete(cache);
  return ret;
}
...
tree number_of_latch_executions(struct loop *loop) {
  tree res, type;
  edge exit;
...
tree number_of_exit_cond_executions(struct loop *loop) {
...
static void number_of_iterations_for_all_loops(VEC(gimple, heap) *
                                               *exit_conditions) {
  unsigned int i;
...
static inline void reset_chrecs_counters(struct chrec_stats *stats) {
  stats->nb_chrecs = 0;
  stats->nb_affine = 0;
...
static void analyze_scalar_evolution_for_all_loop_phi_nodes(VEC(gimple, heap) *
                                                            *exit_conditions) {
  unsigned int i;
...
void gather_stats_on_scev_database(void) {
  ...
  htab_traverse(scalar_evolution_info, gather_stats_on_scev_database_1, &stats);
...
void scev_initialize(void) {
  ...
  scalar_evolution_info = htab_create_alloc(
      100, hash_scev_info, eq_scev_info, del_scev_info, ggc_calloc, ggc_free);
...
bool simple_iv(struct loop *wrto_loop, struct loop *use_loop, tree op,
  ...
  type = TREE_TYPE(op);
  if (TREE_CODE(type) != INTEGER_TYPE && TREE_CODE(type) != POINTER_TYPE)
    return false;

  ev = analyze_scalar_evolution_in_loop(wrto_loop, use_loop, op, &folded_casts);
...
bool expression_expensive_p(tree expr) {
  ...
  code = TREE_CODE(expr);
  if (code == TRUNC_DIV_EXPR || code == CEIL_DIV_EXPR ||
      code == FLOOR_DIV_EXPR || code == ROUND_DIV_EXPR ||
...
unsigned int scev_const_prop(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    for (psi = gsi_start_phis(bb); !gsi_end_p(psi); gsi_next(&psi)) {
      ...
      if (!is_gimple_min_invariant(ev) || !may_propagate_copy(name, ev))
        continue;
  ...
  FOR_EACH_LOOP(li, loop, LI_FROM_INNERMOST) {
    ...
    ex_loop = superloop_at_depth(loop, loop_depth(exit->dest->loop_father) + 1);
    ...
    for (psi = gsi_start_phis(exit->dest); !gsi_end_p(psi);) {
      ...
      if (!POINTER_TYPE_P(TREE_TYPE(def)) && !INTEGRAL_TYPE_P(TREE_TYPE(def))) {
        gsi_next(&psi);
        continue;
      }
      ...
      def = force_gimple_operand_gsi(&bsi, def, false, NULL_TREE, true,
                                     GSI_SAME_STMT);
