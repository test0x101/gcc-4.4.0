...
static bool gate_handle_loop2(void) {
  return (optimize > 0 && (flag_move_loop_invariants || flag_unswitch_loops ||
...
static unsigned int rtl_loop_done(void) {
...
static bool gate_rtl_move_loop_invariants(void) {
  return flag_move_loop_invariants;
}

static unsigned int rtl_move_loop_invariants(void) {
...
struct rtl_opt_pass pass_rtl_move_loop_invariants = {{
    ...
    0,                             /* properties_destroyed */
    0,                             /* todo_flags_start */
    TODO_df_verify | TODO_df_finish | TODO_verify_rtl_sharing |
        TODO_dump_func /* todo_flags_finish */
}};

/* Loop unswitching for RTL.  */
static bool gate_rtl_unswitch(void) {
  return flag_unswitch_loops;
}

static unsigned int rtl_unswitch(void) {
  if (number_of_loops() > 1)
    unswitch_loops();
  return 0;
}
...
/* Loop unswitching for RTL.  */
static bool gate_rtl_unroll_and_peel_loops(void) {
  return (flag_peel_loops || flag_unroll_loops || flag_unroll_all_loops);
}

static unsigned int rtl_unroll_and_peel_loops(void) {
  if (number_of_loops() > 1) {
    int flags = 0;
    if (dump_file)
      df_dump(dump_file);
...
static bool gate_rtl_doloop(void) {
#ifdef HAVE_doloop_end
...
static unsigned int rtl_doloop(void) {
#ifdef HAVE_doloop_end
  if (number_of_loops() > 1)
