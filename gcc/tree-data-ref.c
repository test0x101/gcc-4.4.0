...
static inline bool tree_fold_divides_p(const_tree a, const_tree b) {
...
void dump_data_references(FILE *file, VEC(data_reference_p, heap) * datarefs) {
  unsigned int i;
...
void debug_data_dependence_relations(VEC(ddr_p, heap) * ddrs) {
  dump_data_dependence_relations(stderr, ddrs);
}

/* Dump into FILE all the dependence relations from DDRS.  */

void dump_data_dependence_relations(FILE *file, VEC(ddr_p, heap) * ddrs) {
  unsigned int i;
...
static void dump_affine_function(FILE *outf, affine_fn fn) {
  unsigned i;
  tree coef;
...
static void dump_conflict_function(FILE *outf, conflict_function *cf) {
  unsigned i;
...
void dump_subscript(FILE *outf, struct subscript *subscript) {
...
void print_direction_vector(FILE *outf, lambda_vector dirv, int length) {
  int eq;
...
void print_dir_vectors(FILE *outf, VEC(lambda_vector, heap) * dir_vects,
                       int length) {
  unsigned j;
  lambda_vector v;
...
void print_dist_vectors(FILE *outf, VEC(lambda_vector, heap) * dist_vects,
                        int length) {
  unsigned j;
  lambda_vector v;
...
void dump_data_dependence_relation(FILE *outf,
  ...
  if (DDR_ARE_DEPENDENT(ddr) == chrec_known)
    ...
    for (i = 0; i < DDR_NUM_DIR_VECTS(ddr); i++) {
      ...
      print_direction_vector(outf, DDR_DIR_VECT(ddr, i), DDR_NB_LOOPS(ddr));
    }
  }
...
void dump_data_dependence_direction(FILE *file,
...
void dump_dist_dir_vectors(FILE *file, VEC(ddr_p, heap) * ddrs) {
  unsigned int i, j;
  struct data_dependence_relation *ddr;
  lambda_vector v;
...
void dump_ddrs(FILE *file, VEC(ddr_p, heap) * ddrs) {
  unsigned int i;
  struct data_dependence_relation *ddr;
...
static bool split_constant_offset_1(tree type, tree op0, enum tree_code code,
                                    tree op1, tree *var, tree *off) {
  tree var0, var1;
  tree off0, off1;
  ...
  *off = NULL_TREE;
  ...
  switch (code) {
    ...
    case ADDR_EXPR: {
      ...
      base = get_inner_reference(op0, &pbitsize, &pbitpos, &poffset, &pmode,
      ...
      if (poffset) {
        ...
        if (POINTER_TYPE_P(TREE_TYPE(base)))
          base = fold_build2(POINTER_PLUS_EXPR, TREE_TYPE(base), base,
                             fold_convert(sizetype, poffset));
        else
...
bool dr_analyze_innermost(struct data_reference *dr) {
  ...
  } else if (!simple_iv(loop, loop_containing_stmt(stmt), poffset, &offset_iv,
                        false)) {
    if (dump_file && (dump_flags & TDF_DETAILS))
  ...
  step = size_binop(PLUS_EXPR, fold_convert(ssizetype, base_iv.step),
...
static void dr_analyze_indices(struct data_reference *dr, struct loop *nest) {
  gimple stmt = DR_STMT(dr);
  struct loop *loop = loop_containing_stmt(stmt);
...
static bool dr_address_invariant_p(struct data_reference *dr) {
  unsigned i;
  tree idx;
...
struct data_reference *create_data_ref(struct loop *nest, tree memref,
                                       gimple stmt, bool is_read) {
  struct data_reference *dr;
...
static bool affine_function_equal_p(affine_fn fna, affine_fn fnb) {
  ...
  for (i = 0; i < n; i++)
    if (!operand_equal_p(VEC_index(tree, fna, i), VEC_index(tree, fnb, i), 0))
      return false;
...
static affine_fn affine_fn_op(enum tree_code op, affine_fn fna, affine_fn fnb) {
  ...
  for (i = 0; i < n; i++) {
    ...
    VEC_quick_push(tree, ret,
                   fold_build2(op, type, VEC_index(tree, fna, i),
                               VEC_index(tree, fnb, i)));
  }

  for (; VEC_iterate(tree, fna, i, coef); i++)
    VEC_quick_push(tree, ret,
                   fold_build2(op, signed_type_for(TREE_TYPE(coef)), coef,
                               integer_zero_node));
...
static conflict_function *conflict_fn_not_known(void) {
...
static bool object_address_invariant_in_loop_p(const struct loop *loop,
                                               const_tree obj) {
...
static bool disjoint_objects_p(tree a, tree b) {
  tree base_a, base_b;
  ...
  if (DECL_P(base_a) && DECL_P(base_b) && base_a != base_b)
    return true;
  ...
  ret = false;
  while (1) {
    if (VEC_length(tree, comp_a) == 0 || VEC_length(tree, comp_b) == 0)
      break;
    ...
    if ((TREE_CODE(a) == REALPART_EXPR && TREE_CODE(b) == IMAGPART_EXPR) ||
        (TREE_CODE(a) == IMAGPART_EXPR && TREE_CODE(b) == REALPART_EXPR)) {
      ret = true;
      break;
    }
...
bool dr_may_alias_p(const struct data_reference *a,
  ...
  if (TYPE_RESTRICT(type_a) && TYPE_RESTRICT(type_b) &&
      (!DR_IS_READ(a) || !DR_IS_READ(b)) && decl_a && DECL_P(decl_a) &&
      decl_b && DECL_P(decl_b) && decl_a != decl_b &&
...
static struct data_dependence_relation *initialize_data_dependence_relation(
...
static void free_subscripts(VEC(subscript_p, heap) * subscripts) {
  unsigned i;
  subscript_p s;
...
static inline void finalize_ddr_dependent(struct data_dependence_relation *ddr,
                                          tree chrec) {
  if (dump_file && (dump_flags & TDF_DETAILS)) {
...
static inline void non_affine_dependence_relation(
    struct data_dependence_relation *ddr) {
  if (dump_file && (dump_flags & TDF_DETAILS))
    fprintf(
        dump_file,
...
static inline bool ziv_subscript_p(const_tree chrec_a, const_tree chrec_b) {
...
static void analyze_ziv_subscript(tree chrec_a, tree chrec_b,
...
bool estimated_loop_iterations(struct loop *loop, bool conservative,
                               double_int *nit) {
...
static tree estimated_loop_iterations_tree(struct loop *loop,
                                           bool conservative) {
  double_int nit;
  tree type;
...
static void analyze_siv_subscript_cst_affine(tree chrec_a, tree chrec_b,
                                             conflict_function **overlaps_a,
                                             conflict_function **overlaps_b,
                                             tree *last_conflicts) {
          ...
          */
          ...
          if (tree_fold_divides_p(CHREC_RIGHT(chrec_b), difference)) {
            ...
            *last_conflicts = integer_one_node;
            ...
            if (numiter >= 0 && compare_tree_int(tmp, numiter) > 0) {
...
static tree initialize_matrix_A(lambda_matrix A, tree chrec, unsigned index,
                                int mult) {
  gcc_assert(chrec);
...
static void compute_overlap_steps_for_affine_univar(
    int niter, int step_a, int step_b, affine_fn *overlaps_a,
    affine_fn *overlaps_b, tree *last_conflicts, int dim) {
...
static void compute_overlap_steps_for_affine_1_2(tree chrec_a, tree chrec_b,
                                                 conflict_function **overlaps_a,
                                                 conflict_function **overlaps_b,
                                                 tree *last_conflicts) {
  ...
  niter_x =
      estimated_loop_iterations_int(get_chrec_loop(CHREC_LEFT(chrec_a)), false);
  ...
  compute_overlap_steps_for_affine_univar(niter, step_x, step_z, &overlaps_a_xz,
                                          &overlaps_b_xz, &last_conflicts_xz,
                                          1);
  niter = MIN(niter_y, niter_z);
  compute_overlap_steps_for_affine_univar(niter, step_y, step_z, &overlaps_a_yz,
                                          &overlaps_b_yz, &last_conflicts_yz,
                                          2);
  niter = MIN(niter_x, niter_z);
  niter = MIN(niter_y, niter);
  compute_overlap_steps_for_affine_univar(niter, step_x + step_y, step_z,
                                          &overlaps_a_xyz, &overlaps_b_xyz,
...
static void analyze_subscript_affine_affine(tree chrec_a, tree chrec_b,
                                            conflict_function **overlaps_a,
                                            conflict_function **overlaps_b,
                                            tree *last_conflicts) {
  ...
  */
  if (gamma == 0) {
    if (nb_vars_a == 1 && nb_vars_b == 1) {
      ...
      niter_a = estimated_loop_iterations_int(get_chrec_loop(chrec_a), false);
      niter_b = estimated_loop_iterations_int(get_chrec_loop(chrec_b), false);
      niter = MIN(niter_a, niter_b);
      step_a = int_cst_value(CHREC_RIGHT(chrec_a));
      step_b = int_cst_value(CHREC_RIGHT(chrec_b));

      compute_overlap_steps_for_affine_univar(niter, step_a, step_b, &ova, &ovb,
                                              last_conflicts, 1);
      ...
      *overlaps_b = conflict_fn(1, ovb);
      ...
      compute_overlap_steps_for_affine_1_2(chrec_a, chrec_b, overlaps_a,
                                           overlaps_b, last_conflicts);

    else if (nb_vars_a == 1 && nb_vars_b == 2)
      compute_overlap_steps_for_affine_1_2(chrec_b, chrec_a, overlaps_b,
                                           overlaps_a, last_conflicts);
    ...
    if (((A[0][0] > 0 && -A[1][0] > 0) || (A[0][0] < 0 && -A[1][0] < 0))) {
      ...
      if (i1 > 0 && j1 > 0) {
        HOST_WIDE_INT niter_a =
            estimated_loop_iterations_int(get_chrec_loop(chrec_a), false);
        HOST_WIDE_INT niter_b =
            estimated_loop_iterations_int(get_chrec_loop(chrec_b), false);
        HOST_WIDE_INT niter = MIN(niter_a, niter_b);

        /* (X0, Y0) is a solution of the Diophantine equation:
           "chrec_a (X0) = chrec_b (Y0)".  */
        HOST_WIDE_INT tau1 = MAX(CEIL(-i0, i1), CEIL(-j0, j1));
        ...
        if (niter > 0) {
          HOST_WIDE_INT tau2 =
              MIN(FLOOR_DIV(niter - i0, i1), FLOOR_DIV(niter - j0, j1));
        ...
        *overlaps_a =
            conflict_fn(1, affine_fn_univar(build_int_cst(NULL_TREE, x1), 1,
                                            build_int_cst(NULL_TREE, i1)));
        *overlaps_b =
            conflict_fn(1, affine_fn_univar(build_int_cst(NULL_TREE, y1), 1,
...
static bool can_use_analyze_subscript_affine_affine(tree *chrec_a,
  ...
  *chrec_a = build_polynomial_chrec(CHREC_VARIABLE(*chrec_a), diff,
                                    CHREC_RIGHT(*chrec_a));
  right_b = chrec_convert(type, CHREC_RIGHT(*chrec_b), NULL);
  *chrec_b = build_polynomial_chrec(CHREC_VARIABLE(*chrec_b),
                                    build_int_cst(type, 0), right_b);
  return true;
}
...
static void analyze_siv_subscript(tree chrec_a, tree chrec_b,
                                  conflict_function **overlaps_a,
                                  conflict_function **overlaps_b,
                                  tree *last_conflicts, int loop_nest_num) {
  ...
  if (evolution_function_is_constant_p(chrec_a) &&
    ...
    analyze_siv_subscript_cst_affine(chrec_a, chrec_b, overlaps_a, overlaps_b,
                                     last_conflicts);
    ...
    analyze_siv_subscript_cst_affine(chrec_b, chrec_a, overlaps_b, overlaps_a,
           ...
           evolution_function_is_affine_in_loop(chrec_b, loop_nest_num)) {
    if (!chrec_contains_symbols(chrec_a) && !chrec_contains_symbols(chrec_b)) {
      analyze_subscript_affine_affine(chrec_a, chrec_b, overlaps_a, overlaps_b,
                                      last_conflicts);

      if (CF_NOT_KNOWN_P(*overlaps_a) || CF_NOT_KNOWN_P(*overlaps_b))
        dependence_stats.num_siv_unimplemented++;
        ...
        dependence_stats.num_siv_dependent++;
    } else if (can_use_analyze_subscript_affine_affine(&chrec_a, &chrec_b)) {
      analyze_subscript_affine_affine(chrec_a, chrec_b, overlaps_a, overlaps_b,
                                      last_conflicts);
...
static void analyze_miv_subscript(tree chrec_a, tree chrec_b,
                                  conflict_function **overlaps_a,
                                  conflict_function **overlaps_b,
                                  tree *last_conflicts,
  ...
  */
  ...
  if (eq_evolutions_p(chrec_a, chrec_b)) {
    ...
    *overlaps_b = conflict_fn(1, affine_fn_cst(integer_zero_node));
    *last_conflicts =
        estimated_loop_iterations_tree(get_chrec_loop(chrec_a), true);
    ...
    */
    ...
    else if (CF_NO_DEPENDENCE_P(*overlaps_a) || CF_NO_DEPENDENCE_P(*overlaps_b))
      dependence_stats.num_miv_independent++;
    else
      dependence_stats.num_miv_dependent++;
  }
...
static void analyze_overlapping_iterations(
    tree chrec_a, tree chrec_b, conflict_function **overlap_iterations_a,
    conflict_function **overlap_iterations_b, tree *last_conflicts,
    struct loop *loop_nest) {
  ...
  if (chrec_a == NULL_TREE || chrec_b == NULL_TREE ||
  ...
  /* If they aren't the same, and aren't affine, we can't do anything
    ...
    *overlap_iterations_b = conflict_fn_not_known();
    ...
    analyze_ziv_subscript(chrec_a, chrec_b, overlap_iterations_a,
                          overlap_iterations_b, last_conflicts);

  else if (siv_subscript_p(chrec_a, chrec_b))
    analyze_siv_subscript(chrec_a, chrec_b, overlap_iterations_a,
                          overlap_iterations_b, last_conflicts, lnn);

  else
    analyze_miv_subscript(chrec_a, chrec_b, overlap_iterations_a,
                          overlap_iterations_b, last_conflicts, loop_nest);
...
static bool build_classic_dist_vector_1(struct data_dependence_relation *ddr,
  ...
  for (i = 0; i < DDR_NUM_SUBSCRIPTS(ddr); i++) {
    ...
    if (TREE_CODE(access_fn_a) == POLYNOMIAL_CHREC &&
      ...
      int dist, index;
      int index_a =
          index_in_loop_nest(CHREC_VARIABLE(access_fn_a), DDR_LOOP_NEST(ddr));
      int index_b =
          index_in_loop_nest(CHREC_VARIABLE(access_fn_b), DDR_LOOP_NEST(ddr));
...
static bool constant_access_functions(
    const struct data_dependence_relation *ddr) {
  unsigned i;
...
static void add_other_self_distances(struct data_dependence_relation *ddr) {
  ...
  for (i = 0; i < DDR_NUM_SUBSCRIPTS(ddr); i++) {
    ...
    if (TREE_CODE(access_fun) == POLYNOMIAL_CHREC) {
      ...
      index_carry = MIN(
          index_carry,
          index_in_loop_nest(CHREC_VARIABLE(access_fun), DDR_LOOP_NEST(ddr)));
    }
  }
...
static bool build_classic_dist_vector(struct data_dependence_relation *ddr,
  ...
  dist_v = lambda_vector_new(DDR_NB_LOOPS(ddr));
  if (!build_classic_dist_vector_1(ddr, DDR_A(ddr), DDR_B(ddr), dist_v, &init_b,
                                   &index_carry))
    return false;
  ...
  if (init_b) {
    ...
    if (!lambda_vector_lexico_pos(dist_v, DDR_NB_LOOPS(ddr))) {
      ...
      if (!build_classic_dist_vector_1(ddr, DDR_B(ddr), DDR_A(ddr), save_v,
                                       &init_b, &index_carry))
        return false;
      save_dist_v(ddr, save_v);
    ...
    */
    add_outer_distances(ddr, dist_v,
                        lambda_vector_first_nz(dist_v, DDR_NB_LOOPS(ddr), 0));
  }
  ...
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    ...
    for (i = 0; i < DDR_NUM_DIST_VECTS(ddr); i++) {
      ...
      print_lambda_vector(dump_file, DDR_DIST_VECT(ddr, i), DDR_NB_LOOPS(ddr));
      fprintf(dump_file, "  )\n");
    }
    fprintf(dump_file, ")\n");
  }
...
static inline enum data_dependence_direction dir_from_dist(int dist) {
  if (dist > 0)
    return dir_positive;
  else if (dist < 0)
    return dir_negative;
...
static bool subscript_dependence_tester_1(struct data_dependence_relation *ddr,
  ...
  for (i = 0; VEC_iterate(subscript_p, DDR_SUBSCRIPTS(ddr), i, subscript);
    ...
    analyze_overlapping_iterations(DR_ACCESS_FN(dra, i), DR_ACCESS_FN(drb, i),
...
static bool access_functions_are_affine_or_constant_p(
    const struct data_reference *a, const struct loop *loop_nest) {
  unsigned int i;
...
bool stmt_simple_memref_p(struct loop *loop, gimple stmt, tree op) {
  data_reference_p dr;
  bool res = true;
...
static bool init_omega_eq_with_af(omega_pb pb, unsigned eq, unsigned int offset,
                                  tree access_fun,
                                  struct data_dependence_relation *ddr) {
...
static void omega_extract_distance_vectors(
    omega_pb pb, struct data_dependence_relation *ddr) {
  int eq, geq;
  unsigned i, j;
  ...
  for (i = 0; i <= DDR_INNER_LOOP(ddr) &&
              VEC_iterate(loop_p, DDR_LOOP_NEST(ddr), i, loopi);
       i++) {
    int dist = 0;
    omega_pb copy =
        omega_alloc_problem(2 * DDR_NB_LOOPS(ddr), DDR_NB_LOOPS(ddr));

    omega_copy_problem(copy, pb);

    /* For all the outer loops "loop_j", add "dj = 0".  */
    for (j = 0; j < i && VEC_iterate(loop_p, DDR_LOOP_NEST(ddr), j, loopj);
         j++) {
    ...
    if (res == omega_false || res == omega_unknown ||
...
static bool omega_setup_subscript(tree access_fun_a, tree access_fun_b,
                                  struct data_dependence_relation *ddr,
                                  omega_pb pb, bool *maybe_dependent) {
  int eq;
  tree type =
      signed_type_for_types(TREE_TYPE(access_fun_a), TREE_TYPE(access_fun_b));
  ...
  if (DDR_NB_LOOPS(ddr) != 0 && pb->eqs[eq].coef[0] &&
      !int_divides_p(lambda_vector_gcd((lambda_vector) & (pb->eqs[eq].coef[1]),
                                       2 * DDR_NB_LOOPS(ddr)),
                     pb->eqs[eq].coef[0])) {
    /* There is no dependence.  */
    *maybe_dependent = false;
...
static bool init_omega_for_ddr_1(struct data_reference *dra,
  ...
  for (i = 0; i < DDR_NUM_SUBSCRIPTS(ddr); i++) {
    if (!omega_setup_subscript(DR_ACCESS_FN(dra, i), DR_ACCESS_FN(drb, i), ddr,
                               pb, maybe_dependent))
      return false;
...
static bool init_omega_for_ddr(struct data_dependence_relation *ddr,
  ...
  *maybe_dependent = true;
  ...
  if (same_access_functions(ddr)) {
    ...
    res =
        init_omega_for_ddr_1(DDR_A(ddr), DDR_B(ddr), ddr, pb, maybe_dependent);
  ...
  pb = omega_alloc_problem(2 * DDR_NB_LOOPS(ddr), DDR_NB_LOOPS(ddr));
  res = init_omega_for_ddr_1(DDR_A(ddr), DDR_B(ddr), ddr, pb, maybe_dependent);
  omega_free_problem(pb);
  ...
  pb = omega_alloc_problem(2 * DDR_NB_LOOPS(ddr), DDR_NB_LOOPS(ddr));
  res = init_omega_for_ddr_1(DDR_B(ddr), DDR_A(ddr), ddr, pb, maybe_dependent);
  omega_free_problem(pb);
...
static bool ddr_consistent_p(FILE *file, struct data_dependence_relation *ddr,
  ...
  if (VEC_length(lambda_vector, dist_vects) != DDR_NUM_DIST_VECTS(ddr)) {
    ...
    fprintf(file,
            "\n(Number of distance vectors differ: Banerjee has %d, Omega has "
            "%d.\n",
            VEC_length(lambda_vector, dist_vects), DDR_NUM_DIST_VECTS(ddr));
  ...
  if (VEC_length(lambda_vector, dir_vects) != DDR_NUM_DIR_VECTS(ddr)) {
    fprintf(file,
            "\n(Number of direction vectors differ: Banerjee has %d, Omega has "
            "%d.)\n",
            VEC_length(lambda_vector, dir_vects), DDR_NUM_DIR_VECTS(ddr));
    return false;
  }
...
static void compute_affine_dependence(struct data_dependence_relation *ddr,
                                      struct loop *loop_nest) {
  ...
  if (DDR_ARE_DEPENDENT(ddr) == NULL_TREE && !DDR_SELF_REFERENCE(ddr)) {
    ...
    if (access_functions_are_affine_or_constant_p(dra, loop_nest) &&
      ...
      if (flag_check_data_deps) {
        ...
        if (DDR_ARE_DEPENDENT(ddr) == NULL_TREE) {
          ...
          if (maybe_dependent)
            gcc_assert(ddr_consistent_p(stderr, ddr, dist_vects, dir_vects));
        }
      } else
        subscript_dependence_tester(ddr, loop_nest);
    }
    ...
    csys_dont_know:;
      ...
      if (dump_file && (dump_flags & TDF_DETAILS)) {
        ...
        fprintf(dump_file,
                "affine dependence test not usable: access function not affine "
                "or constant.\n");
      }
      finalize_ddr_dependent(ddr, chrec_dont_know);
    }
...
static void compute_self_dependence(struct data_dependence_relation *ddr) {
  unsigned int i;
  struct subscript *subscript;
...
void compute_all_dependences(VEC(data_reference_p, heap) * datarefs,
...
bool get_references_in_stmt(gimple stmt,
  ...
  *references = NULL;
  ...
  if ((stmt_code == GIMPLE_CALL &&
      ...
      (stmt_code == GIMPLE_ASM && gimple_asm_volatile_p(stmt)))
  ...
  if (stmt_code == GIMPLE_ASSIGN) {
    ...
    if (DECL_P(*op1) ||
        (REFERENCE_CLASS_P(*op1) && (base = get_base_address(*op1)) &&
    ...
    if (DECL_P(*op0) || (REFERENCE_CLASS_P(*op0) && get_base_address(*op0))) {
      ref = VEC_safe_push(data_ref_loc, heap, *references, NULL);
      ref->pos = op0;
      ref->is_read = false;
    }
...
bool find_data_references_in_stmt(struct loop *nest, gimple stmt,
  ...
  for (i = 0; VEC_iterate(data_ref_loc, references, i, ref); i++) {
    ...
    /* FIXME -- data dependence analysis does not work correctly for objects
       with invariant addresses.  Let us fail here until the problem is fixed.
     */
...
static bool find_loop_nest_1(struct loop *loop,
                             VEC(loop_p, heap) * *loop_nest) {
...
bool find_loop_nest(struct loop *loop, VEC(loop_p, heap) * *loop_nest) {
  VEC_safe_push(loop_p, heap, *loop_nest, loop);
  if (loop->inner)
    return find_loop_nest_1(loop->inner, loop_nest);
  return true;
...
bool compute_data_dependences_for_loop(
    struct loop *loop, bool compute_self_and_read_read_dependences,
    VEC(data_reference_p, heap) * *datarefs,
    VEC(ddr_p, heap) * *dependence_relations) {
  bool res = true;
  ...
  /* If the loop nest is not well formed, or one of the data references
     ...
     dependences.  */
  if (!loop || !find_loop_nest(loop, &vloops) ||
...
static void analyze_all_data_dependences(struct loop *loop) {
  unsigned int i;
  int nb_data_refs = 10;
  VEC(data_reference_p, heap) *datarefs =
...
void free_dependence_relations(VEC(ddr_p, heap) * dependence_relations) {
  unsigned int i;
  struct data_dependence_relation *ddr;
  ...
  for (i = 0; VEC_iterate(ddr_p, dependence_relations, i, ddr); i++) {
    ...
    if (loop_nest == NULL)
      ...
      gcc_assert(DDR_LOOP_NEST(ddr) == NULL || DDR_LOOP_NEST(ddr) == loop_nest);
...
void free_data_refs(VEC(data_reference_p, heap) * datarefs) {
  unsigned int i;
  struct data_reference *dr;
...
void dump_rdg_vertex(FILE *file, struct graph *rdg, int i) {
...
void debug_rdg_vertex(struct graph *rdg, int i) {
...
void debug_rdg_component(struct graph *rdg, int c) {
  dump_rdg_component(stderr, rdg, c, NULL);
}

/* Dump the reduced dependence graph RDG to FILE.  */

void dump_rdg(FILE *file, struct graph *rdg) {
  int i;
...
void debug_rdg(struct graph *rdg) {
  dump_rdg(stderr, rdg);
}
...
int rdg_vertex_for_stmt(struct graph *rdg, gimple stmt) {
...
static void create_rdg_edge_for_ddr(struct graph *rdg, ddr_p ddr) {
  struct graph_edge *e;
  int va, vb;
...
static void create_rdg_edges(struct graph *rdg, VEC(ddr_p, heap) * ddrs) {
  int i;
  struct data_dependence_relation *ddr;
  def_operand_p def_p;
  ssa_op_iter iter;
  ...
  for (i = 0; i < rdg->n_vertices; i++)
    FOR_EACH_PHI_OR_STMT_DEF(def_p, RDG_STMT(rdg, i), iter, SSA_OP_DEF)
  create_rdg_edges_for_scalar(rdg, DEF_FROM_PTR(def_p), i);
}

/* Build the vertices of the reduced dependence graph RDG.  */

void create_rdg_vertices(struct graph *rdg, VEC(gimple, heap) * stmts) {
  int i, j;
  gimple stmt;
...
static void stmts_from_loop(struct loop *loop, VEC(gimple, heap) * *stmts) {
  unsigned int i;
...
static bool known_dependences_p(VEC(ddr_p, heap) * dependence_relations) {
  ddr_p ddr;
  unsigned int i;
...
static hashval_t hash_stmt_vertex_info(const void *elt) {
  const struct rdg_vertex_info *const rvi = (const struct rdg_vertex_info *)elt;
...
struct graph *build_empty_rdg(int n_stmts) {
  int nb_data_refs = 10;
...
void free_rdg(struct graph *rdg) {
  int i;
...
void stores_from_loop(struct loop *loop, VEC(gimple, heap) * *stmts) {
  unsigned int i;
  basic_block *bbs = get_loop_body_in_dom_order(loop);
...
static inline tree ref_base_address(gimple stmt, data_ref_loc *ref) {
  tree base = NULL_TREE;
  tree base_address;
...
bool rdg_defs_used_in_other_loops_p(struct graph *rdg, int v) {
...
bool have_similar_memory_accesses(gimple s1, gimple s2) {
  bool res = false;
  unsigned i, j;
...
static int have_similar_memory_accesses_1(const void *s1, const void *s2) {
...
static hashval_t ref_base_address_1(const void *s) {
...
void remove_similar_memory_refs(VEC(gimple, heap) * *stmts) {
  unsigned i;
  gimple stmt;
...
int access_matrix_get_index_for_parameter(tree parameter,
