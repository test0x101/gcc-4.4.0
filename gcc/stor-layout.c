...
unsigned int maximum_field_alignment =
    TARGET_DEFAULT_PACK_STRUCT * BITS_PER_UNIT;
...
tree variable_size(tree size) {
  ...
  /* If the language-processor is to take responsibility for variable-sized
     ...
     constant sizes.  */
  if (TREE_CONSTANT(size) || lang_hooks.decls.global_bindings_p() < 0 ||
...
enum machine_mode mode_for_size(unsigned int size, enum mode_class mclass,
                                int limit) {
...
enum machine_mode mode_for_size_tree(const_tree size, enum mode_class mclass,
                                     int limit) {
...
void layout_decl(tree decl, unsigned int known_align) {
  ...
  if (DECL_SIZE(decl) == 0) {
    ...
    DECL_SIZE_UNIT(decl) = fold_convert(
        sizetype,
        size_binop(CEIL_DIV_EXPR, DECL_SIZE(decl), bitsize_unit_node));
  ...
  if (warn_larger_than && (code == VAR_DECL || code == PARM_DECL) &&
      !DECL_EXTERNAL(decl)) {
    ...
    if (size != 0 && TREE_CODE(size) == INTEGER_CST &&
      ...
      if (compare_tree_int(size, size_as_int) == 0)
        warning(OPT_Wlarger_than_eq, "size of %q+D is %d bytes", decl,
                size_as_int);
      else
...
tree bit_from_pos(tree offset, tree bitpos) {
  return size_binop(PLUS_EXPR, bitpos,
                    size_binop(MULT_EXPR, fold_convert(bitsizetype, offset),
                               bitsize_unit_node));
}

tree byte_from_pos(tree offset, tree bitpos) {
  return size_binop(PLUS_EXPR, offset,
                    fold_convert(sizetype, size_binop(TRUNC_DIV_EXPR, bitpos,
                                                      bitsize_unit_node)));
}

void pos_from_bit(tree *poffset, tree *pbitpos, unsigned int off_align,
                  tree pos) {
  *poffset = size_binop(
      MULT_EXPR,
      fold_convert(sizetype,
                   size_binop(FLOOR_DIV_EXPR, pos, bitsize_int(off_align))),
...
void normalize_offset(tree *poffset, tree *pbitpos, unsigned int off_align) {
  ...
  if (compare_tree_int(*pbitpos, off_align) >= 0) {
    tree extra_aligns =
        size_binop(FLOOR_DIV_EXPR, *pbitpos, bitsize_int(off_align));

    *poffset =
        size_binop(PLUS_EXPR, *poffset,
                   size_binop(MULT_EXPR, fold_convert(sizetype, extra_aligns),
                              size_int(off_align / BITS_PER_UNIT)));

    *pbitpos = size_binop(FLOOR_MOD_EXPR, *pbitpos, bitsize_int(off_align));
  }
}
...
unsigned int update_alignment_for_field(record_layout_info rli, tree field,
  ...
  is_bitfield = (type != error_mark_node && DECL_BIT_FIELD_TYPE(field) &&
  ...
  if (targetm.ms_bitfield_layout_p(rli->t)) {
    ...
    if ((!is_bitfield && !DECL_PACKED(field)) ||
        (!integer_zerop(DECL_SIZE(field))
             ? !DECL_PACKED(field)
             : (rli->prev_field && DECL_BIT_FIELD_TYPE(rli->prev_field) &&
    ...
    if (DECL_NAME(field) != 0 || targetm.align_anon_bitfield()) {
      unsigned int type_align = TYPE_ALIGN(type);
      ...
      if (integer_zerop(DECL_SIZE(field))) {
        if (initial_max_fld_align)
          type_align = MIN(type_align, initial_max_fld_align * BITS_PER_UNIT);
...
static int excess_unit_span(HOST_WIDE_INT byte_offset, HOST_WIDE_INT bit_offset,
  ...
  return ((offset + size + align - 1) / align >
          ((unsigned HOST_WIDE_INT)tree_low_cst(TYPE_SIZE(type), 1) / align));
}
#endif
...
void place_field(record_layout_info rli, tree field) {
  ...
  if (TREE_CODE(field) == VAR_DECL) {
    rli->pending_statics = tree_cons(NULL_TREE, field, rli->pending_statics);
    return;
  }
  ...
  if (!integer_zerop(rli->bitpos))
    ...
    known_align = (BITS_PER_UNIT * (tree_low_cst(rli->offset, 1) &
  ...
  if (known_align < desired_align && !targetm.ms_bitfield_layout_p(rli->t)) {
    ...
    if (desired_align < rli->offset_align)
      ...
      rli->offset = size_binop(
          ...
          fold_convert(sizetype, size_binop(CEIL_DIV_EXPR, rli->bitpos,
                                            bitsize_unit_node)));
      rli->bitpos = bitsize_zero_node;
...
#ifdef PCC_BITFIELD_TYPE_MATTERS
  if (PCC_BITFIELD_TYPE_MATTERS && !targetm.ms_bitfield_layout_p(rli->t) &&
      TREE_CODE(field) == FIELD_DECL && type != error_mark_node &&
      DECL_BIT_FIELD(field) &&
      (!DECL_PACKED(field)
       /* Enter for these packed fields only to issue a warning.  */
       || TYPE_ALIGN(type) <= BITS_PER_UNIT) &&
      maximum_field_alignment == 0 && !integer_zerop(DECL_SIZE(field)) &&
      host_integerp(DECL_SIZE(field), 1) && host_integerp(rli->offset, 1) &&
  ...
  if (BITFIELD_NBYTES_LIMITED && !targetm.ms_bitfield_layout_p(rli->t) &&
      TREE_CODE(field) == FIELD_DECL && type != error_mark_node &&
      DECL_BIT_FIELD_TYPE(field) && !DECL_PACKED(field) &&
      !integer_zerop(DECL_SIZE(field)) && host_integerp(DECL_SIZE(field), 1) &&
      host_integerp(rli->offset, 1) && host_integerp(TYPE_SIZE(type), 1)) {
    unsigned int type_align = TYPE_ALIGN(type);
    tree dsize = DECL_SIZE(field);
    HOST_WIDE_INT field_size = tree_low_cst(dsize, 1);
    HOST_WIDE_INT offset = tree_low_cst(rli->offset, 0);
  ...
  if (targetm.ms_bitfield_layout_p(rli->t)) {
    ...
    if (rli->prev_field) {
      ...
      if (DECL_BIT_FIELD_TYPE(field) && !integer_zerop(DECL_SIZE(field)) &&
          !integer_zerop(DECL_SIZE(rli->prev_field)) &&
    ...
    if (!DECL_BIT_FIELD_TYPE(field) ||
      ...
      if (DECL_SIZE(field) != NULL &&
        ...
        HOST_WIDE_INT bitsize = tree_low_cst(DECL_SIZE(field), 1);
        HOST_WIDE_INT typesize = tree_low_cst(TYPE_SIZE(TREE_TYPE(field)), 1);
  ...
  if (!integer_zerop(DECL_FIELD_BIT_OFFSET(field)))
    ...
    actual_align =
        (BITS_PER_UNIT * (tree_low_cst(DECL_FIELD_OFFSET(field), 1) &
  ...
  if (DECL_SIZE(field) == 0)
    ...
    rli->offset =
        size_binop(PLUS_EXPR, rli->offset,
                   fold_convert(sizetype, size_binop(CEIL_DIV_EXPR, rli->bitpos,
                                                     bitsize_unit_node)));
    rli->offset = size_binop(PLUS_EXPR, rli->offset, DECL_SIZE_UNIT(field));
    rli->bitpos = bitsize_zero_node;
    ...
    if ((TREE_CHAIN(field) == NULL ||
        ...
        DECL_BIT_FIELD_TYPE(field) && !integer_zerop(DECL_SIZE(field)))
      rli->bitpos = size_binop(PLUS_EXPR, rli->bitpos,
                               bitsize_int(rli->remaining_in_alignment));
...
static void finalize_record_size(record_layout_info rli) {
  ...
  TYPE_ALIGN(rli->t) =
      ROUND_TYPE_ALIGN(rli->t, TYPE_ALIGN(rli->t), rli->record_align);
#else
  ...
  if (warn_packed && TREE_CODE(rli->t) == RECORD_TYPE && TYPE_PACKED(rli->t) &&
      !rli->packed_maybe_necessary && TREE_CONSTANT(unpadded_size)) {
    tree unpacked_size;
    ...
    if (simple_cst_equal(unpacked_size, TYPE_SIZE(rli->t))) {
      ...
      if (TYPE_NAME(rli->t)) {
        ...
        if (STRICT_ALIGNMENT)
          ...
          warning(OPT_Wpacked, "packed attribute is unnecessary for %qs", name);
      } else {
        if (STRICT_ALIGNMENT)
...
void compute_record_mode(tree type) {
  ...
  for (field = TYPE_FIELDS(type); field; field = TREE_CHAIN(field)) {
    ...
    if (TREE_CODE(TREE_TYPE(field)) == ERROR_MARK ||
        ...
        !host_integerp(bit_position(field), 1) || DECL_SIZE(field) == 0 ||
  ...
  if (TYPE_MODE(type) != BLKmode && STRICT_ALIGNMENT &&
...
static void finalize_type_size(tree type) {
  ...
  if (TYPE_SIZE_UNIT(type) == 0 && TYPE_SIZE(type) != 0)
    ...
    TYPE_SIZE_UNIT(type) = fold_convert(
        sizetype,
        size_binop(FLOOR_DIV_EXPR, TYPE_SIZE(type), bitsize_unit_node));
  ...
  if (TYPE_NEXT_VARIANT(type) || type != TYPE_MAIN_VARIANT(type)) {
    ...
    for (variant = TYPE_MAIN_VARIANT(type); variant != 0;
...
void finish_builtin_struct(tree type, const char *name, tree fields,
                           tree align_type) {
  tree tail, next;
...
void layout_type(tree type) {
  ...
  switch (TREE_CODE(type)) {
    ...
    case REAL_TYPE:
      SET_TYPE_MODE(type, mode_for_size(TYPE_PRECISION(type), MODE_FLOAT, 0));
      TYPE_SIZE(type) = bitsize_int(GET_MODE_BITSIZE(TYPE_MODE(type)));
      TYPE_SIZE_UNIT(type) = size_int(GET_MODE_SIZE(TYPE_MODE(type)));
      break;
    ...
    case COMPLEX_TYPE:
      ...
      SET_TYPE_MODE(type, mode_for_size(2 * TYPE_PRECISION(TREE_TYPE(type)),
    ...
    case VECTOR_TYPE: {
      ...
      if (TYPE_MODE(type) == VOIDmode) {
        ...
        if (mode == VOIDmode && GET_MODE_CLASS(innermode) == MODE_INT)
          mode =
              mode_for_size(nunits * GET_MODE_BITSIZE(innermode), MODE_INT, 0);

        if (mode == VOIDmode ||
            (GET_MODE_CLASS(mode) == MODE_INT && !have_regs_of_mode[mode]))
          SET_TYPE_MODE(type, BLKmode);
        else
          SET_TYPE_MODE(type, mode);
      }
      ...
      TYPE_UNSIGNED(type) = TYPE_UNSIGNED(TREE_TYPE(type));
      TYPE_SIZE_UNIT(type) = int_const_binop(
          MULT_EXPR, TYPE_SIZE_UNIT(innertype), size_int(nunits), 0);
    ...
    case REFERENCE_TYPE: {
      enum machine_mode mode =
          ((TREE_CODE(type) == REFERENCE_TYPE && reference_types_internal)
               ? Pmode
               : TYPE_MODE(type));
    ...
    case ARRAY_TYPE: {
      ...
      if (index && TYPE_MAX_VALUE(index) && TYPE_MIN_VALUE(index) &&
        ...
        length = size_binop(
            PLUS_EXPR, size_one_node,
            fold_convert(sizetype,
                         fold_build2(MINUS_EXPR, TREE_TYPE(lb), ub, lb)));
        ...
        if (TYPE_PACKED(type) && INTEGRAL_TYPE_P(element) &&
          ...
          HOST_WIDE_INT maxvalue = tree_low_cst(TYPE_MAX_VALUE(element), 1);
          HOST_WIDE_INT minvalue = tree_low_cst(TYPE_MIN_VALUE(element), 1);
        ...
        TYPE_SIZE(type) = size_binop(MULT_EXPR, element_size,
                                     fold_convert(bitsizetype, length));
      ...
      if (TYPE_SIZE(type) != 0
        ...
        if (simple_cst_equal(TYPE_SIZE(type), TYPE_SIZE(TREE_TYPE(type))))
      ...
      if (TYPE_SIZE_UNIT(element) &&
          ...
          compare_tree_int(TYPE_SIZE_UNIT(element), TYPE_ALIGN_UNIT(element)) <
              0)
...
enum machine_mode vector_type_mode(const_tree t) {
  enum machine_mode mode;
  ...
  if (VECTOR_MODE_P(mode) &&
      (!targetm.vector_mode_supported_p(mode) || !have_regs_of_mode[mode])) {
    enum machine_mode innermode = TREE_TYPE(t)->type.mode;

    /* For integers, try mapping it to a same-sized scalar mode.  */
    if (GET_MODE_CLASS(innermode) == MODE_INT) {
      mode = mode_for_size(
          TYPE_VECTOR_SUBPARTS(t) * GET_MODE_BITSIZE(innermode), MODE_INT, 0);
...
void set_sizetype(tree type) {
  ...
  /* The *bitsizetype types use a precision that avoids overflows when
     ...
     precision.  */
  int precision =
      MIN(MIN(oprecision + BITS_PER_UNIT_LOG + 1, MAX_FIXED_MODE_SIZE),
  ...
  if (TYPE_UNSIGNED(type)) {
    ...
    new_max = build_int_cst_wide_type(sizetype, TREE_INT_CST_LOW(orig_max),
...
void set_min_and_max_values_for_integral_type(tree type, int precision,
                                              bool is_unsigned) {
  tree min_value;
  tree max_value;
    ...
    max_value = build_int_cst_wide(
        ...
        precision - HOST_BITS_PER_WIDE_INT > 0
            ? ((unsigned HOST_WIDE_INT) ~0 >>
               (HOST_BITS_PER_WIDE_INT - (precision - HOST_BITS_PER_WIDE_INT)))
            : 0);
  } else {
    min_value = build_int_cst_wide(
        type,
        (precision - HOST_BITS_PER_WIDE_INT > 0
             ? 0
             : (HOST_WIDE_INT)(-1) << (precision - 1)),
        (((HOST_WIDE_INT)(-1) << (precision - HOST_BITS_PER_WIDE_INT - 1 > 0
...
enum machine_mode get_best_mode(int bitsize, int bitpos, unsigned int align,
  ...
  if ((SLOW_BYTE_ACCESS && !volatilep) ||
    ...
    for (tmode = GET_CLASS_NARROWEST_MODE(MODE_INT); tmode != VOIDmode;
      ...
      if (bitpos / unit == (bitpos + bitsize - 1) / unit &&
          unit <= BITS_PER_WORD && unit <= MIN(align, BIGGEST_ALIGNMENT) &&
          (largest_mode == VOIDmode || unit <= GET_MODE_BITSIZE(largest_mode)))
        wide_mode = tmode;
    }
...
void get_mode_bounds(enum machine_mode mode, int sign,
                     enum machine_mode target_mode, rtx *mmin, rtx *mmax) {
