...
static void stack_adjust_offset_pre_post(rtx pattern, HOST_WIDE_INT *pre,
                                         HOST_WIDE_INT *post) {
...
static void insn_stack_adjust_offset_pre_post(rtx insn, HOST_WIDE_INT *pre,
                                              HOST_WIDE_INT *post) {
  rtx pattern;
  ...
  *post = 0;
  ...
  else if (GET_CODE(pattern) == PARALLEL || GET_CODE(pattern) == SEQUENCE) {
    int i;
...
static void bb_stack_adjust_offset(basic_block bb) {
  HOST_WIDE_INT offset;
  int i;
...
static rtx adjust_stack_reference(rtx mem, HOST_WIDE_INT adjustment) {
  rtx addr, cfa, tmp;
...
static hashval_t variable_htab_hash(const void *x) {
...
static int variable_htab_eq(const void *x, const void *y) {
  const_variable const v = (const_variable)x;
...
static void variable_htab_free(void *elem) {
  int i;
...
static attrs attrs_list_member(attrs list, tree decl, HOST_WIDE_INT offset) {
...
static void attrs_list_insert(attrs *listp, tree decl, HOST_WIDE_INT offset,
                              rtx loc) {
  attrs list;
...
static void attrs_list_copy(attrs *dstp, attrs src) {
  attrs n;
...
static void attrs_list_union(attrs *dstp, attrs src) {
  for (; src; src = src->next) {
...
static void vars_clear(htab_t vars) {
  htab_empty(vars);
}
...
static variable unshare_variable(dataflow_set *set, variable var,
  ...
  slot = htab_find_slot_with_hash(set->vars, new_var->decl,
                                  VARIABLE_HASH_VAL(new_var->decl), INSERT);
  *slot = new_var;
  return new_var;
}
...
static int vars_copy_1(void **slot, void *data) {
  ...
  dstp = (variable *)htab_find_slot_with_hash(
      dst, src->decl, VARIABLE_HASH_VAL(src->decl), INSERT);
  *dstp = src;
...
static void vars_copy(htab_t dst, htab_t src) {
  vars_clear(dst);
...
static inline tree var_debug_decl(tree decl) {
  if (decl && DECL_P(decl) && DECL_DEBUG_EXPR_IS_FROM(decl) &&
      DECL_DEBUG_EXPR(decl) && DECL_P(DECL_DEBUG_EXPR(decl)))
...
static void var_reg_set(dataflow_set *set, rtx loc,
...
static int get_init_value(dataflow_set *set, rtx loc, tree decl) {
  void **slot;
  variable var;
  int i;
  ...
  if (slot) {
    var = *(variable *)slot;
    for (i = 0; i < var->n_var_parts && ret_val == VAR_INIT_STATUS_UNKNOWN;
         i++) {
      location_chain nextp;
...
static void var_reg_delete_and_set(dataflow_set *set, rtx loc, bool modify,
                                   enum var_init_status initialized,
                                   rtx set_src) {
  tree decl = REG_EXPR(loc);
  HOST_WIDE_INT offset = REG_OFFSET(loc);
...
static void var_reg_delete(dataflow_set *set, rtx loc, bool clobber) {
...
static void var_regno_delete(dataflow_set *set, int regno) {
...
static void var_mem_set(dataflow_set *set, rtx loc,
                        enum var_init_status initialized, rtx set_src) {
...
static void var_mem_delete_and_set(dataflow_set *set, rtx loc, bool modify,
                                   enum var_init_status initialized,
                                   rtx set_src) {
  tree decl = MEM_EXPR(loc);
  HOST_WIDE_INT offset = INT_MEM_OFFSET(loc);
...
static void var_mem_delete(dataflow_set *set, rtx loc, bool clobber) {
  tree decl = MEM_EXPR(loc);
  HOST_WIDE_INT offset = INT_MEM_OFFSET(loc);
...
static void dataflow_set_init(dataflow_set *set, int vars_size) {
...
static void dataflow_set_copy(dataflow_set *dst, dataflow_set *src) {
  int i;
...
static int variable_union_info_cmp_pos(const void *n1, const void *n2) {
...
static int variable_union(void **slot, void *data) {
  ...
  src = *(variable *)slot;
  dstp = (variable *)htab_find_slot_with_hash(
      set->vars, src->decl, VARIABLE_HASH_VAL(src->decl), INSERT);
  if (!*dstp) {
    src->refcount++;
  ...
  for (i = 0, j = 0, k = 0; i < src->n_var_parts && j < dst->n_var_parts; k++) {
    ...
    if (i >= 0 && j >= 0 &&
      ...
      if (dst->refcount > 1) {
        for (node = src->var_part[i].loc_chain,
            node2 = dst->var_part[j].loc_chain;
             node && node2; node = node->next, node2 = node2->next) {
          if (!((REG_P(node2->loc) && REG_P(node->loc) &&
      ...
      for (node = src->var_part[i].loc_chain, ii = 0; node;
        ...
        for (jj = 0; jj < dst_l; jj++) {
          if ((REG_P(vui[jj].lc->loc) && REG_P(node->loc) &&
...
static void dataflow_set_union(dataflow_set *dst, dataflow_set *src) {
  int i;
...
static bool variable_part_different_p(variable_part *vp1, variable_part *vp2) {
...
static bool variable_different_p(variable var1, variable var2,
  ...
  for (i = 0; i < var1->n_var_parts; i++) {
    ...
    if (compare_current_location) {
      if (!((REG_P(var1->var_part[i].cur_loc) &&
            ...
            rtx_equal_p(var1->var_part[i].cur_loc, var2->var_part[i].cur_loc)))
        return true;
    }
...
static int dataflow_set_different_1(void **slot, void *data) {
...
static int dataflow_set_different_2(void **slot, void *data) {
  htab_t htab = (htab_t)data;
  variable var1, var2;
...
static bool dataflow_set_different(dataflow_set *old_set,
...
static bool contains_symbol_ref(rtx x) {
  const char *fmt;
  RTX_CODE code;
  int i;
...
static bool track_expr_p(tree expr) {
  rtx decl_rtl;
  tree realdecl;
  ...
  /* When the EXPR is a DECL for alias of some variable (see example)
     ...
     char **_dl_argv;
  */
  if (MEM_P(decl_rtl) && contains_symbol_ref(XEXP(decl_rtl, 0)))
    return 0;
  ...
  if (MEM_P(decl_rtl)) {
    ...
    if (GET_MODE(decl_rtl) == BLKmode || AGGREGATE_TYPE_P(TREE_TYPE(realdecl)))
      return 0;
...
static bool same_variable_part_p(rtx loc, tree expr, HOST_WIDE_INT offset) {
  tree expr2;
...
static bool track_loc_p(rtx loc, tree expr, HOST_WIDE_INT offset,
                        bool store_reg_p, enum machine_mode *mode_out,
  ...
  if ((GET_MODE_SIZE(mode) > GET_MODE_SIZE(DECL_MODE(expr)) ||
       (store_reg_p && !COMPLEX_MODE_P(DECL_MODE(expr)) &&
...
static rtx var_lowpart(enum machine_mode mode, rtx loc) {
  ...
  regno = REGNO(loc) +
          subreg_regno_offset(REGNO(loc), GET_MODE(loc), reg_offset, mode);
...
static int count_uses(rtx *loc, void *insn) {
  ...
  if (REG_P(*loc)) {
    ...
    VTI(bb)->n_mos++;
             ...
             track_loc_p(*loc, MEM_EXPR(*loc), INT_MEM_OFFSET(*loc), false,
                         NULL, NULL)) {
    VTI(bb)->n_mos++;
  }
...
static void count_uses_1(rtx *x, void *insn) {
...
static void count_stores(rtx loc, const_rtx expr ATTRIBUTE_UNUSED, void *insn) {
  count_uses(&loc, insn);
}
...
static int add_uses(rtx *loc, void *insn) {
  enum machine_mode mode;
  ...
  if (REG_P(*loc)) {
    ...
    if (track_loc_p(*loc, REG_EXPR(*loc), REG_OFFSET(*loc), false, &mode,
                    NULL)) {
      mo->type = MO_USE;
...
static void add_uses_1(rtx *x, void *insn) {
...
static void add_stores(rtx loc, const_rtx expr, void *insn) {
  enum machine_mode mode;
  ...
  if (REG_P(loc)) {
    ...
    if (GET_CODE(expr) == CLOBBER ||
        !track_loc_p(loc, REG_EXPR(loc), REG_OFFSET(loc), true, &mode, NULL)) {
      mo->type = MO_CLOBBER;
      mo->u.loc = loc;
    } else {
      rtx src = NULL;
    ...
    mo->insn = (rtx)insn;
  } else if (MEM_P(loc) && track_loc_p(loc, MEM_EXPR(loc), INT_MEM_OFFSET(loc),
                                       false, &mode, NULL)) {
    basic_block bb = BLOCK_FOR_INSN((rtx)insn);
    micro_operation *mo = VTI(bb)->mos + VTI(bb)->n_mos++;
    ...
    if (GET_CODE(expr) == CLOBBER) {
      ...
      if (src == NULL) {
        ...
        if (SET_SRC(expr) != src)
          expr = gen_rtx_SET(VOIDmode, loc, src);
        if (same_variable_part_p(SET_SRC(expr), MEM_EXPR(loc),
                                 INT_MEM_OFFSET(loc)))
          mo->type = MO_COPY;
        else
          mo->type = MO_SET;
          ...
          FOR_EACH_EDGE(e, ei, bb->succs) {
            ...
            if (TEST_BIT(visited, e->dest->index)) {
              if (!TEST_BIT(in_pending, e->dest->index)) {
                ...
                fibheap_insert(pending, bb_order[e->dest->index], e->dest);
              }
            } else if (!TEST_BIT(in_worklist, e->dest->index)) {
              /* Add E->DEST to current round.  */
              SET_BIT(in_worklist, e->dest->index);
              fibheap_insert(worklist, bb_order[e->dest->index], e->dest);
            }
          }
        }
      }
...
static int dump_variable(void **slot, void *data ATTRIBUTE_UNUSED) {
  variable var = *(variable *)slot;
  int i;
  location_chain node;

  fprintf(dump_file, "  name: %s", IDENTIFIER_POINTER(DECL_NAME(var->decl)));
...
static void dump_vars(htab_t vars) {
...
static void variable_was_changed(variable var, htab_t htab) {
  hashval_t hash = VARIABLE_HASH_VAL(var->decl);

  if (emit_notes) {
    variable *slot;

    slot = (variable *)htab_find_slot_with_hash(changed_variables, var->decl,
                                                hash, INSERT);
    ...
    if (htab && var->n_var_parts == 0) {
      ...
      *slot = empty_var;
      ...
      old = htab_find_slot_with_hash(htab, var->decl, hash, NO_INSERT);
      if (old)
    ...
    if (var->n_var_parts == 0) {
      void **slot = htab_find_slot_with_hash(htab, var->decl, hash, NO_INSERT);
      if (slot)
...
static inline int find_variable_location_part(variable var,
                                              HOST_WIDE_INT offset,
                                              int *insertion_point) {
  int pos, low, high;
...
static void set_variable_part(dataflow_set *set, rtx loc, tree decl,
                              HOST_WIDE_INT offset,
                              enum var_init_status initialized, rtx set_src) {
  int pos;
  location_chain node, next;
  ...
  if (!*slot) {
    ...
    *slot = var;
    ...
    if (pos >= 0) {
      ...
      if (node &&
          ((REG_P(node->loc) && REG_P(loc) && REGNO(node->loc) == REGNO(loc)) ||
  ...
  for (node = var->var_part[pos].loc_chain; node; node = next) {
    next = node->next;
    if ((REG_P(node->loc) && REG_P(loc) && REGNO(node->loc) == REGNO(loc)) ||
        rtx_equal_p(node->loc, loc)) {
...
static void clobber_variable_part(dataflow_set *set, rtx loc, tree decl,
  ...
  if (slot) {
    ...
    if (pos >= 0) {
      ...
      for (node = next; node; node = next) {
        next = node->next;
        if (node->loc != loc &&
            (!flag_var_tracking_uninit || !set_src || MEM_P(set_src) ||
          ...
          if (REG_P(node->loc)) {
            ...
            for (anode = *anextp; anode; anode = anext) {
              anext = anode->next;
              if (anode->decl == decl && anode->offset == offset) {
...
static void delete_variable_part(dataflow_set *set, rtx loc, tree decl,
                                 HOST_WIDE_INT offset) {
  void **slot;
  ...
  if (slot) {
    ...
    if (pos >= 0) {
      ...
      if (var->refcount > 1) {
        ...
        for (node = var->var_part[pos].loc_chain; node; node = node->next) {
...
static int emit_note_insn_var_location(void **varp, void *data) {
  ...
  for (i = 0; i < var->n_var_parts; i++) {
    ...
    for (j = i + 1; j < var->n_var_parts; j++)
      if (last_limit <= var->var_part[j].offset)
        break;
    if (j < var->n_var_parts && wider_mode != VOIDmode &&
      ...
      if (REG_P(loc[n_var_parts]) &&
          ...
          end_hard_regno(mode, REGNO(loc[n_var_parts])) == REGNO(loc2)) {
        if (!WORDS_BIG_ENDIAN && !BYTES_BIG_ENDIAN)
          new_loc = simplify_subreg(wider_mode, loc[n_var_parts], mode, 0);
        else if (WORDS_BIG_ENDIAN && BYTES_BIG_ENDIAN)
          new_loc = simplify_subreg(wider_mode, loc2, mode, 0);
        if (new_loc) {
          if (!REG_P(new_loc) || REGNO(new_loc) != REGNO(loc[n_var_parts]))
            new_loc = NULL;
          else
            REG_ATTRS(new_loc) = REG_ATTRS(loc[n_var_parts]);
        }
      } else if (MEM_P(loc[n_var_parts]) && GET_CODE(XEXP(loc2, 0)) == PLUS &&
                 GET_CODE(XEXP(XEXP(loc2, 0), 0)) == REG &&
                 GET_CODE(XEXP(XEXP(loc2, 0), 1)) == CONST_INT) {
        if ((GET_CODE(XEXP(loc[n_var_parts], 0)) == REG &&
             rtx_equal_p(XEXP(loc[n_var_parts], 0), XEXP(XEXP(loc2, 0), 0)) &&
             INTVAL(XEXP(XEXP(loc2, 0), 1)) == GET_MODE_SIZE(mode)) ||
            (GET_CODE(XEXP(loc[n_var_parts], 0)) == PLUS &&
             GET_CODE(XEXP(XEXP(loc[n_var_parts], 0), 1)) == CONST_INT &&
             rtx_equal_p(XEXP(XEXP(loc[n_var_parts], 0), 0),
                         XEXP(XEXP(loc2, 0), 0)) &&
             INTVAL(XEXP(XEXP(loc[n_var_parts], 0), 1)) + GET_MODE_SIZE(mode) ==
                 INTVAL(XEXP(XEXP(loc2, 0), 1))))
          new_loc = adjust_address_nv(loc[n_var_parts], wider_mode, 0);
      }
  ...
  if (!complete) {
    NOTE_VAR_LOCATION(note) =
        gen_rtx_VAR_LOCATION(VOIDmode, var->decl, NULL_RTX, (int)initialized);
  } else if (n_var_parts == 1) {
    rtx expr_list = gen_rtx_EXPR_LIST(VOIDmode, loc[0], GEN_INT(offsets[0]));

    NOTE_VAR_LOCATION(note) =
        gen_rtx_VAR_LOCATION(VOIDmode, var->decl, expr_list, (int)initialized);
  } else if (n_var_parts) {
    rtx parallel;

    for (i = 0; i < n_var_parts; i++)
      loc[i] = gen_rtx_EXPR_LIST(VOIDmode, loc[i], GEN_INT(offsets[i]));

    parallel = gen_rtx_PARALLEL(VOIDmode, gen_rtvec_v(n_var_parts, loc));
    NOTE_VAR_LOCATION(note) =
        gen_rtx_VAR_LOCATION(VOIDmode, var->decl, parallel, (int)initialized);
  }
...
static void emit_notes_for_changes(rtx insn, enum emit_note_where where) {
  emit_note_data data;
...
static int emit_notes_for_differences_1(void **slot, void *data) {
...
static int emit_notes_for_differences_2(void **slot, void *data) {
...
static void emit_notes_for_differences(rtx insn, dataflow_set *old_set,
                                       dataflow_set *new_set) {
...
static void emit_notes_in_bb(basic_block bb) {
  int i;
  dataflow_set set;
...
static bool vt_get_decl_and_offset(rtx rtl, tree *declp,
  ...
  for (parm = DECL_ARGUMENTS(current_function_decl); parm;
    ...
    if (!vt_get_decl_and_offset(incoming, &decl, &offset)) {
      if (!vt_get_decl_and_offset(decl_rtl, &decl, &offset))
        continue;
      offset += byte_lowpart_offset(GET_MODE(incoming), GET_MODE(decl_rtl));
    }
    ...
    if (REG_P(incoming)) {
      ...
      attrs_list_insert(&out->regs[REGNO(incoming)], parm, offset, incoming);
...
static void vt_initialize(void) {
  ...
  attrs_pool =
      create_alloc_pool("attrs_def pool", sizeof(struct attrs_def), 1024);
  var_pool =
      create_alloc_pool("variable_def pool", sizeof(struct variable_def), 64);
  loc_chain_pool = create_alloc_pool("location_chain_def pool",
                                     sizeof(struct location_chain_def), 1024);
  changed_variables =
      htab_create(10, variable_htab_hash, variable_htab_eq, NULL);
...
static bool gate_handle_var_tracking(void) {
  return (flag_var_tracking);
}

struct rtl_opt_pass pass_variable_tracking = {{
    RTL_PASS, "vartrack",                    /* name */
