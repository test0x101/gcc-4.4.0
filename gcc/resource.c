...
static rtx find_dead_or_set_registers(rtx, struct resources *, rtx *, int,
                                      struct resources, struct resources);

/* Utility function called from mark_target_live_regs via note_stores.
   It deadens any CLOBBERed registers and livens any SET registers.  */

static void update_live_status(rtx dest, const_rtx x,
...
static int find_basic_block(rtx insn, int search_limit) {
  basic_block bb;
  ...
  for (insn = next_nonnote_insn(insn); insn && LABEL_P(insn);
...
static rtx next_insn_no_annul(rtx insn) {
  if (insn) {
    /* If INSN is an annulled branch, skip any insns from the target
       of the branch.  */
    if (INSN_P(insn) && INSN_ANNULLED_BRANCH_P(insn) &&
...
void mark_referenced_resources(rtx x, struct resources *res,
            ...
            if (GET_CODE(XEXP(link, 0)) == USE) {
              for (i = 1; i < seq_size; i++) {
                ...
                if (GET_CODE(slot_pat) == SET &&
                    rtx_equal_p(SET_DEST(slot_pat), XEXP(XEXP(link, 0), 0)))
                  break;
              }
              if (i >= seq_size)
                mark_referenced_resources(XEXP(XEXP(link, 0), 0), res, 0);
            }
        }
      }
...
static rtx find_dead_or_set_registers(rtx target, struct resources *res,
      ...
      if (jump_count++ < 10) {
        if (any_uncondjump_p(this_jump_insn) ||
          ...
          if (GET_CODE(PATTERN(insn)) == SEQUENCE &&
            ...
            target_set = set;
            mark_set_resources(insn, &target_set, 0, MARK_SRC_DEST_CALL);
          ...
          find_dead_or_set_registers(JUMP_LABEL(this_jump_insn), &target_res, 0,
                                     jump_count, target_set, needed);
          find_dead_or_set_registers(next, &fallthrough_res, 0, jump_count, set,
                                     needed);
...
void mark_set_resources(rtx x, struct resources *res, int in_dest,
...
restart:
  ...
  switch (code) {
      ...
      if (mark_type == MARK_SRC_DEST_CALL) {
        ...
        for (link = CALL_INSN_FUNCTION_USAGE(x); link; link = XEXP(link, 1))
          if (GET_CODE(XEXP(link, 0)) == CLOBBER)
            mark_set_resources(SET_DEST(XEXP(link, 0)), res, 1, MARK_SRC_DEST);
    ...
    case SET:
      ...
      mark_set_resources(
          SET_DEST(x), res,
          (mark_type == MARK_SRC_DEST_CALL || GET_CODE(SET_SRC(x)) != CALL),
          mark_type);
...
void mark_target_live_regs(rtx insns, rtx target, struct resources *res) {
  ...
  if (b != -1) {
    ...
    /* Get starting and ending insn, handling the case where each might
       be a SEQUENCE.  */
    start_insn =
        (b == ENTRY_BLOCK_PTR->next_bb->index ? insns
    ...
    for (insn = start_insn; insn != stop_insn;
      ...
      if (CALL_P(real_insn)) {
        /* CALL clobbers all call-used regs that aren't fixed except
           ...
           call live.  */
        AND_COMPL_HARD_REG_SET(current_live_regs, regs_invalidated_by_call);
      ...
      if ((NONJUMP_INSN_P(real_insn) && GET_CODE(PATTERN(real_insn)) != USE &&
          ...
          JUMP_P(real_insn) || CALL_P(real_insn)) {
        for (link = REG_NOTES(real_insn); link; link = XEXP(link, 1))
          if (REG_NOTE_KIND(link) == REG_DEAD && REG_P(XEXP(link, 0)) &&
              REGNO(XEXP(link, 0)) < FIRST_PSEUDO_REGISTER)
            add_to_hard_reg_set(&pending_dead_regs, GET_MODE(XEXP(link, 0)),
                                REGNO(XEXP(link, 0)));
  ...
  jump_insn =
      find_dead_or_set_registers(target, res, &jump_target, 0, set, needed);
...
void init_resource_info(rtx epilogue_insn) {
  ...
  if (crtl->return_rtx != 0)
    mark_referenced_resources(crtl->return_rtx, &end_of_function_needs, 1);
