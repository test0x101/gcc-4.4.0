...
static inline struct elt_loc_list *new_elt_loc_list(struct elt_loc_list *next,
                                                    rtx loc) {
...
static inline void unchain_one_elt_list(struct elt_list **pl) {
...
static int entry_and_rtx_equal_p(const void *entry, const void *x_arg) {
  ...
  if (GET_CODE(x) == CONST && (GET_CODE(XEXP(x, 0)) == CONST_INT ||
...
static hashval_t get_value_hash(const void *entry) {
  const cselib_val *const v = (const cselib_val *)entry;
  return v->value;
}
...
int references_value_p(const_rtx x, int only_useless) {
  const enum rtx_code code = GET_CODE(x);
  const char *fmt = GET_RTX_FORMAT(code);
  int i, j;

  if (GET_CODE(x) == VALUE && (!only_useless || CSELIB_VAL_PTR(x)->locs == 0))
    return 1;
...
static int discard_useless_locs(void **x, void *info ATTRIBUTE_UNUSED) {
...
static int discard_useless_values(void **x, void *info ATTRIBUTE_UNUSED) {
  cselib_val *v = (cselib_val *)*x;
...
enum machine_mode cselib_reg_set_mode(const_rtx x) {
  if (!REG_P(x))
    return GET_MODE(x);
...
int rtx_equal_for_cselib_p(rtx x, rtx y) {
  enum rtx_code code;
  const char *fmt;
  int i;
  ...
  for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--) {
    ...
    switch (fmt[i]) {
      ...
      case 'E':
        ...
        for (j = 0; j < XVECLEN(x, i); j++)
          if (!rtx_equal_for_cselib_p(XVECEXP(x, i, j), XVECEXP(y, i, j)))
            return 0;
        break;

      case 'e':
        if (i == 1 && targetm.commutative_p(x, UNKNOWN) &&
...
/* We need to pass down the mode of constants through the hash table
   ...
   mode.  */
static rtx wrap_constant(enum machine_mode mode, rtx x) {
...
static unsigned int cselib_hash_rtx(rtx x, int create) {
  cselib_val *e;
  int i, j;
  enum rtx_code code;
  const char *fmt;
  ...
  switch (code) {
    ...
    case CONST_DOUBLE:
      ...
      if (GET_MODE(x) != VOIDmode)
        ...
        hash +=
            ((unsigned)CONST_DOUBLE_LOW(x) + (unsigned)CONST_DOUBLE_HIGH(x));
    ...
    case LABEL_REF:
      ...
      hash += (((unsigned int)LABEL_REF << 7) + CODE_LABEL_NUMBER(XEXP(x, 0)));
  ...
  for (; i >= 0; i--) {
    switch (fmt[i]) {
      ...
      case 'E':
        for (j = 0; j < XVECLEN(x, i); j++) {
          unsigned int tem_hash = cselib_hash_rtx(XVECEXP(x, i, j), create);
...
static inline cselib_val *new_cselib_val(unsigned int value,
...
static void add_mem_for_addr(cselib_val *addr_elt, cselib_val *mem_elt, rtx x) {
  struct elt_loc_list *l;
  ...
  mem_elt->locs = new_elt_loc_list(
      mem_elt->locs, replace_equiv_address_nv(x, addr_elt->val_rtx));
...
static cselib_val *cselib_lookup_mem(rtx x, int create) {
  enum machine_mode mode = GET_MODE(x);
  void **slot;
  cselib_val *addr;
  cselib_val *mem_elt;
  ...
  if (MEM_VOLATILE_P(x) || mode == BLKmode || !cselib_record_memory ||
...
static rtx expand_loc(struct elt_loc_list *p, bitmap regs_active,
                      int max_depth) {
  rtx reg_result = NULL;
  ...
  for (; p; p = p->next) {
    /* Avoid infinite recursion trying to expand a reg into a
       the same reg.  */
    if ((REG_P(p->loc)) && (REGNO(p->loc) < regno) &&
    ...
    /* Avoid infinite recursion and do not try to expand the
      ...
      if (GET_CODE(p->loc) == LO_SUM &&
          GET_CODE(XEXP(p->loc, 1)) == SYMBOL_REF && p->setting_insn &&
...
rtx cselib_expand_value_rtx(rtx orig, bitmap regs_active, int max_depth) {
  ...
  switch (code) {
    ...
    case SUBREG: {
      rtx subreg =
          cselib_expand_value_rtx(SUBREG_REG(orig), regs_active, max_depth - 1);
      if (!subreg)
        return NULL;
      scopy =
          simplify_gen_subreg(GET_MODE(orig), subreg,
                              GET_MODE(SUBREG_REG(orig)), SUBREG_BYTE(orig));
      if (scopy == NULL ||
          (GET_CODE(scopy) == SUBREG && !REG_P(SUBREG_REG(scopy)) &&
      ...
      case 'V':
        if (XVEC(orig, i) != NULL) {
          ...
          for (j = 0; j < XVECLEN(copy, i); j++) {
            rtx result = cselib_expand_value_rtx(XVECEXP(orig, i, j),
                                                 regs_active, max_depth - 1);
            if (!result)
              return NULL;
  ...
  switch (GET_RTX_CLASS(code)) {
    ...
    case RTX_BITFIELD_OPS:
      if (CONST_INT_P(XEXP(copy, 0)) && GET_MODE(XEXP(orig, 0)) != VOIDmode) {
        scopy = simplify_ternary_operation(code, mode, GET_MODE(XEXP(orig, 0)),
    ...
    case RTX_COMM_COMPARE:
      if (CONST_INT_P(XEXP(copy, 0)) && GET_MODE(XEXP(copy, 1)) == VOIDmode &&
          (GET_MODE(XEXP(orig, 0)) != VOIDmode ||
           GET_MODE(XEXP(orig, 1)) != VOIDmode)) {
        scopy = simplify_relational_operation(
            code, mode,
            (GET_MODE(XEXP(orig, 0)) != VOIDmode) ? GET_MODE(XEXP(orig, 0))
  ...
  if (scopy == NULL_RTX) {
    XEXP(copy, 0) = gen_rtx_CONST(GET_MODE(XEXP(orig, 0)), XEXP(copy, 0));
    if (dump_file)
      fprintf(dump_file,
...
cselib_val *cselib_lookup(rtx x, enum machine_mode mode, int create) {
  void **slot;
  cselib_val *e;
...
static bool cselib_rtx_varies_p(const_rtx x ATTRIBUTE_UNUSED,
  ...
  for (v = *vp; v != &dummy_val; v = next) {
    ...
    while (*p) {
      ...
      if (num_mems < PARAM_VALUE(PARAM_MAX_CSELIB_MEMORY_LOCATIONS) &&
          !canon_true_dependence(mem_rtx, GET_MODE(mem_rtx), mem_addr, x,
                                 cselib_rtx_varies_p)) {
        has_mem = true;
        num_mems++;
        p = &(*p)->next;
...
static void cselib_record_set(rtx dest, cselib_val *src_elt,
  ...
  } else if (MEM_P(dest) && dest_addr_elt != 0 && cselib_record_memory) {
    if (src_elt->locs == 0)
      n_useless_values--;
...
static void cselib_record_sets(rtx insn) {
  int n_sets = 0;
  int i;
  ...
  if (n_sets == 1 && MEM_P(sets[0].src) && !cselib_record_memory &&
  ...
  for (i = 0; i < n_sets; i++) {
    ...
    if (REG_P(dest) || (MEM_P(dest) && cselib_record_memory)) {
      rtx src = sets[i].src;
      if (cond)
...
void cselib_process_insn(rtx insn) {
  int i;
  rtx x;
  ...
  if (LABEL_P(insn) ||
      ...
      (NONJUMP_INSN_P(insn) && GET_CODE(PATTERN(insn)) == ASM_OPERANDS &&
  ...
  if (CALL_P(insn)) {
    for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
      if (call_used_regs[i] || (REG_VALUES(i) && REG_VALUES(i)->elt &&
...
void cselib_init(bool record_memory) {
  elt_list_pool = create_alloc_pool("elt_list", sizeof(struct elt_list), 10);
  elt_loc_list_pool =
      create_alloc_pool("elt_loc_list", sizeof(struct elt_loc_list), 10);
  cselib_val_pool =
      create_alloc_pool("cselib_val_list", sizeof(cselib_val), 10);
  ...
  n_used_regs = 0;
  cselib_hash_table =
      htab_create(31, get_value_hash, entry_and_rtx_equal_p, NULL);
}
