...
static void process_include(rtx desc, int lineno) {
...
static void process_rtx(rtx desc, int lineno) {
  switch (GET_CODE(desc)) {
    ...
    case DEFINE_INSN_AND_SPLIT: {
      ...
      insn_elem =
          queue_pattern(desc, &define_insn_tail, read_rtx_filename, lineno);
      split_elem = queue_pattern(split, &other_tail, read_rtx_filename, lineno);
...
static int is_predicable(struct queue_elem *elem) {
  ...
  for (i = GET_NUM_ELEM(vec) - 1; i >= 0; --i) {
    ...
    switch (GET_CODE(sub)) {
      ...
      case SET:
        ...
        message_with_line(elem->lineno, "non-constant value for `predicable'");
        errors = 1;
        return 0;
...
found:
  ...
  if (strchr(value, ',') != NULL) {
    message_with_line(elem->lineno, "multiple alternatives for `predicable'");
    errors = 1;
    return 0;
  }
...
found:
  ...
  if (p_true == NULL || strchr(++p_true, ',') != NULL) {
    message_with_line(elem->lineno, "attribute `predicable' is not a boolean");
    errors = 1;
    if (p_false)
      free(p_false);
    return;
  ...
  switch (GET_CODE(XEXP(elem->data, 2))) {
    ...
    case CONST:
      message_with_line(elem->lineno, "attribute `predicable' cannot be const");
      errors = 1;
      if (p_false)
        free(p_false);
      return;
...
static int n_alternatives(const char *s) {
  int n = 1;
...
static void collect_insn_data(rtx pattern, int *palt, int *pmax) {
  const char *fmt;
  enum rtx_code code;
  int i, j, len;
...
static rtx alter_predicate_for_insn(rtx pattern, int alt, int max_op,
                                    int lineno) {
  const char *fmt;
  enum rtx_code code;
  int i, j, len;
  ...
  switch (code) {
    case MATCH_OPERAND: {
      ...
      if (n_alternatives(c) != 1) {
        message_with_line(lineno, "too many alternatives for operand %d",
                          XINT(pattern, 0));
        errors = 1;
        return NULL;
      }
  ...
  for (i = 0; i < len; i++) {
    ...
    switch (fmt[i]) {
      case 'e':
      case 'u':
        r = alter_predicate_for_insn(XEXP(pattern, i), alt, max_op, lineno);
        if (r == NULL)
          return r;
        break;
      ...
      case 'E':
        for (j = XVECLEN(pattern, i) - 1; j >= 0; --j) {
          r = alter_predicate_for_insn(XVECEXP(pattern, i, j), alt, max_op,
                                       lineno);
          if (r == NULL)
            return r;
        }
...
static const char *alter_test_for_insn(struct queue_elem *ce_elem,
                                       struct queue_elem *insn_elem) {
  return join_c_conditions(XSTR(ce_elem->data, 1), XSTR(insn_elem->data, 2));
}
...
static char *shift_output_template(char *dest, const char *src, int disp) {
  while (*src) {
    char c = *src++;
    *dest++ = c;
    if (c == '%') {
...
static const char *alter_output_for_insn(struct queue_elem *ce_elem,
                                         struct queue_elem *insn_elem, int alt,
                                         int max_op) {
...
static void process_one_cond_exec(struct queue_elem *ce_elem) {
  struct queue_elem *insn_elem;
  for (insn_elem = define_insn_queue; insn_elem; insn_elem = insn_elem->next) {
    ...
    if (XVECLEN(ce_elem->data, 0) != 1) {
      message_with_line(ce_elem->lineno, "too many patterns in predicate");
      errors = 1;
      return;
    }
    ...
    XTMPL(insn, 3) =
        alter_output_for_insn(ce_elem, insn_elem, alternatives, max_operand);
    ...
    queue_pattern(insn, &other_tail, insn_elem->filename, insn_elem->lineno);
...
int init_md_reader_args_cb(int argc, char **argv,
...
void add_predicate_code(struct pred_data *pred, enum rtx_code code) {
  if (!pred->codes[code]) {
    ...
    if (code != REG && code != SUBREG && code != MEM && code != CONCAT &&
        code != PARALLEL && code != STRICT_LOW_PART)
...
static const struct std_pred_table std_preds[] = {
    ...
    {"comparison_operator",
     false,
     false,
     {EQ, NE, LE, LT, GE, GT, LEU, LTU, GEU, GTU, UNORDERED, ORDERED, UNEQ,
      UNGE, UNGT, UNLE, UNLT, LTGT}}};
...
static void init_predicate_table(void) {
  ...
  predicate_table = htab_create_alloc(37, hash_struct_pred_data,
                                      eq_struct_pred_data, 0, xcalloc, free);
