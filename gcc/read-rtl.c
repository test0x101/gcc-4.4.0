...
static const char *apply_iterator_to_string(const char *, struct mapping *,
                                            int);
static rtx apply_iterator_to_rtx(rtx, struct mapping *, int, struct map_value *,
                                 FILE *, const char **);
...
static struct mapping *add_mapping(struct iterator_group *, htab_t t,
                                   const char *, FILE *);
static struct map_value **add_map_value(struct map_value **, int, const char *);
...
static void fatal_with_file_and_line(FILE *infile, const char *msg, ...) {
  char context[64];
  size_t i;
  int c;
  va_list ap;
...
static int find_mode(const char *name, FILE *infile) {
  int i;
...
static bool uses_mode_iterator_p(rtx x, int mode) {
  return (int)GET_MODE(x) == mode;
}

static void apply_mode_iterator(rtx x, int mode) {
  PUT_MODE(x, (enum machine_mode)mode);
}

/* Implementations of the iterator_group callbacks for codes.  */

static int find_code(const char *name, FILE *infile) {
  int i;
...
static bool uses_code_iterator_p(rtx x, int code) {
  return (int)GET_CODE(x) == code;
}

static void apply_code_iterator(rtx x, int code) {
...
static struct map_value *map_attr_string(const char *p,
                                         struct mapping *iterator, int value) {
  const char *attr;
  struct mapping *m;
  struct map_value *v;
...
static unsigned int mode_attr_index(struct map_value **mode_maps,
                                    const char *string) {
  char *p;
  struct map_value *mv;
...
static void apply_mode_maps(rtx x, struct map_value *mode_maps,
                            struct mapping *iterator, int value, FILE *infile,
                            const char **unknown) {
  unsigned int offset;
  int indx;
  struct map_value *pm;
...
static const char *apply_iterator_to_string(const char *string,
...
static rtx apply_iterator_to_rtx(rtx original, struct mapping *iterator,
                                 int value, struct map_value *mode_maps,
                                 FILE *infile, const char **unknown_mode_attr) {
  struct iterator_group *group;
  ...
  for (i = 0; format_ptr[i] != 0; i++)
    switch (format_ptr[i]) {
      ...
      case 'e':
        XEXP(x, i) = apply_iterator_to_rtx(
            XEXP(x, i), iterator, value, mode_maps, infile, unknown_mode_attr);
        break;
      ...
      case 'E':
        if (XVEC(original, i)) {
          ...
          for (j = 0; j < XVECLEN(x, i); j++)
            XVECEXP(x, i, j) =
                apply_iterator_to_rtx(XVECEXP(original, i, j), iterator, value,
                                      mode_maps, infile, unknown_mode_attr);
        }
        break;
...
static bool uses_iterator_p(rtx x, struct mapping *iterator) {
  struct iterator_group *group;
  const char *format_ptr;
  int i, j;
...
static const char *add_condition_to_string(const char *original,
                                           const char *extra) {
...
static void add_condition_to_rtx(rtx x, const char *extra) {
  switch (GET_CODE(x)) {
    case DEFINE_INSN:
    case DEFINE_EXPAND:
...
static int apply_iterator_traverse(void **slot, void *data) {
  ...
  for (elem = mtd->queue; elem != 0; elem = XEXP(elem, 1))
    if (uses_iterator_p(XEXP(elem, 0), iterator)) {
      ...
      for (v = iterator->values; v != 0; v = v->next) {
        x = apply_iterator_to_rtx(original, iterator, v->number, mtd->mode_maps,
                                  mtd->infile, &mtd->unknown_mode_attr);
...
static struct mapping *add_mapping(struct iterator_group *group, htab_t table,
...
static struct map_value **add_map_value(struct map_value **end_ptr, int number,
                                        const char *string) {
...
static void set_rtx_ptr_loc(const void *ptr, const char *filename, int lineno) {
  struct ptr_loc *loc;

  loc =
      (struct ptr_loc *)obstack_alloc(&ptr_loc_obstack, sizeof(struct ptr_loc));
  loc->ptr = ptr;
...
static const struct ptr_loc *get_rtx_ptr_loc(const void *ptr) {
  return (const struct ptr_loc *)htab_find(ptr_locs, &ptr);
}

/* Associate NEW_PTR with the same file position as OLD_PTR.  */

void copy_rtx_ptr_loc(const void *new_ptr, const void *old_ptr) {
...
void print_rtx_ptr_loc(const void *ptr) {
...
static void read_escape(FILE *infile) {
  int c = getc(infile);
...
/* Read a double-quoted string onto the obstack.  Caller has scanned
   the leading quote.  */
static char *read_quoted_string(FILE *infile) {
  int c;
...
static char *read_braced_string(FILE *infile) {
  int c;
...
static char *read_string(FILE *infile, int star_if_braced) {
  char *stringbuf;
  int saw_paren = 0;
  int c, old_lineno;
...
/* Given an object that starts with a char * name field, return a hash
   code for its name.  */
static hashval_t def_hash(const void *def) {
  unsigned result, i;
...
static int def_name_eq_p(const void *def1, const void *def2) {
  return !strcmp(*(const char *const *)def1, *(const char *const *)def2);
}
    ...
    if (!*entry_ptr) {
      ...
      *entry_ptr = def;
      ...
      if (strcmp(def->value, tmp_char))
        fatal_with_file_and_line(infile, "redefinition of %s, was %s, now %s",
...
static int find_iterator(struct iterator_group *group, const char *name,
                         FILE *infile) {
  struct mapping *m;
...
static struct mapping *read_mapping(struct iterator_group *group, htab_t table,
                                    FILE *infile) {
  char tmp_char[256];
  struct mapping *m;
...
static void check_code_iterator(struct mapping *iterator, FILE *infile) {
  struct map_value *v;
  ...
  for (v = iterator->values->next; v != 0; v = v->next)
    if (strcmp(GET_RTX_FORMAT(bellwether), GET_RTX_FORMAT(v->number)) != 0)
      fatal_with_file_and_line(infile,
                               ...
                               "different rtx formats",
                               iterator->name);

  bellwether_codes =
      XRESIZEVEC(enum rtx_code, bellwether_codes, iterator->index + 1);
...
bool read_rtx(FILE *infile, rtx *x, int *lineno) {
  ...
  if (queue_next == 0) {
    ...
    if (mtd.unknown_mode_attr)
      fatal_with_file_and_line(infile, "undefined attribute '%s' used for mode",
...
static rtx read_rtx_1(FILE *infile, struct map_value **mode_maps) {
  int i;
...
again:
  ...
  if (strcmp(tmp_char, "define_code_iterator") == 0) {
    check_code_iterator(read_mapping(&codes, codes.iterators, infile), infile);
    goto again;
  }
  ...
  if (c != ')') {
    ...
    if (c == '(' &&
        (GET_CODE(return_rtx) == AND || GET_CODE(return_rtx) == IOR))
...
static rtx read_rtx_variadic(FILE *infile, struct map_value **mode_maps,
                             rtx form) {
  char c = '(';
  rtx p = form, q;
