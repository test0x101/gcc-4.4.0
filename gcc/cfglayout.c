...
rtx unlink_insn_chain(rtx first, rtx last) {
...
static rtx skip_insns_after_block(basic_block bb) {
  ...
  for (last_insn = insn = BB_END(bb); (insn = NEXT_INSN(insn)) != 0;) {
    ...
    switch (GET_CODE(insn)) {
      ...
      case CODE_LABEL:
        if (NEXT_INSN(insn) && JUMP_P(NEXT_INSN(insn)) &&
...
static void record_effective_endpoints(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    if (PREV_INSN(BB_HEAD(bb)) && next_insn != BB_HEAD(bb))
      bb->il.rtl->header = unlink_insn_chain(next_insn, PREV_INSN(BB_HEAD(bb)));
...
void insn_locators_finalize(void) {
  if (curr_rtl_loc >= 0)
...
void set_curr_insn_source_location(location_t location) {
...
void set_curr_insn_block(tree b) {
  /* IV opts calls into RTL expansion to compute costs of operations.  At this
     time locators are not initialized.  */
  if (curr_rtl_loc == -1)
    return;
...
static unsigned int into_cfg_layout_mode(void) {
  cfg_layout_initialize(0);
  return 0;
}

static unsigned int outof_cfg_layout_mode(void) {
  basic_block bb;
...
static tree locator_scope(int loc) {
...
static tree insn_scope(const_rtx insn) {
  return locator_scope(INSN_LOCATOR(insn));
}

/* Return line number of the statement specified by the locator.  */
location_t locator_location(int loc) {
...
int insn_line(const_rtx insn) {
  return locator_line(INSN_LOCATOR(insn));
}

/* Return source file of the statement specified by LOC.  */
const char *locator_file(int loc) {
  expanded_location xloc;
  if (!loc)
    return 0;
  else
...
const char *insn_file(const_rtx insn) {
  return locator_file(INSN_LOCATOR(insn));
}

/* Return true if LOC1 and LOC2 locators have the same location and scope.  */
bool locator_eq(int loc1, int loc2) {
  if (loc1 == loc2)
    return true;
  ...
  for (; insn; insn = next_active_insn(insn)) {
    ...
    if (GET_CODE(PATTERN(insn)) == SEQUENCE) {
      ...
      for (i = 0; i < XVECLEN(body, 0); i++)
        this_block =
            choose_inner_scope(this_block, insn_scope(XVECEXP(body, 0, i)));
    }
    if (!this_block)
      continue;
...
void relink_block_chain(bool stay_in_cfglayout_mode) {
  ...
  if (dump_file) {
    ...
    for (bb = ENTRY_BLOCK_PTR->next_bb, index = NUM_FIXED_BLOCKS; bb;
         bb = (basic_block)bb->aux, index++) {
      fprintf(dump_file, " %i ", index);
      if (get_bb_original(bb))
        fprintf(dump_file, "duplicate of %i ", get_bb_original(bb)->index);
  ...
  for (bb = ENTRY_BLOCK_PTR->next_bb; bb; bb = (basic_block)bb->aux) {
    ...
    if (JUMP_P(bb_end_insn)) {
      if (any_condjump_p(bb_end_insn)) {
        ...
        if (bb->aux == e_fall->dest || e_fall->dest == EXIT_BLOCK_PTR)
          continue;
        ...
        /* There is another special case: if *neither* block is next,
          ...
          if (note && INTVAL(XEXP(note, 0)) < REG_BR_PROB_BASE / 2 &&
              invert_jump(
                  bb_end_insn,
                  (e_fall->dest == EXIT_BLOCK_PTR ? NULL_RTX
                                                  : label_for_bb(e_fall->dest)),
                  0)) {
            e_fall->flags &= ~EDGE_FALLTHRU;
#ifdef ENABLE_CHECKING
            gcc_assert(could_fall_through(e_taken->src, e_taken->dest));
#endif
    ...
    FOR_EACH_BB(bb) {
      ...
      if (e->goto_locus && !(e->flags & EDGE_ABNORMAL)) {
        ...
        while (insn != end && (!INSN_P(insn) || INSN_LOCATOR(insn) == 0))
          insn = PREV_INSN(insn);
        if (insn != end && locator_eq(INSN_LOCATOR(insn), (int)e->goto_locus))
          continue;
        if (simplejump_p(BB_END(e->src)) && INSN_LOCATOR(BB_END(e->src)) == 0) {
        ...
        nb = split_edge(e);
        if (!INSN_P(BB_END(nb)))
          BB_END(nb) = emit_insn_after_noloc(gen_nop(), BB_END(nb), nb);
...
void verify_insn_chain(void) {
  ...
  for (prevx = NULL, insn_cnt1 = 1, x = get_insns(); x != 0;
       prevx = x, insn_cnt1++, x = NEXT_INSN(x))
    gcc_assert(PREV_INSN(x) == prevx);

  gcc_assert(prevx == get_last_insn());

  for (nextx = NULL, insn_cnt2 = 1, x = get_last_insn(); x != 0;
...
bool cfg_layout_can_duplicate_bb_p(const_basic_block bb) {
...
rtx duplicate_insn_chain(rtx from, rtx to) {
  rtx insn, last;
...
basic_block cfg_layout_duplicate_bb(basic_block bb) {
  ...
  new_bb = create_basic_block(insn, insn ? get_last_insn() : NULL,
...
void cfg_layout_initialize(unsigned int flags) {
  rtx x;
  basic_block bb;
...
void copy_bbs(basic_block *bbs, unsigned n, basic_block *new_bbs, edge *edges,
              unsigned num_edges, edge *new_edges, struct loop *base,
              basic_block after) {
  unsigned i, j;
