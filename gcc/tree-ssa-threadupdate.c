...
static hashval_t redirection_data_hash(const void *p) {
  edge e = ((const struct redirection_data *)p)->outgoing_edge;
  return e->dest->index;
}

static int redirection_data_eq(const void *p1, const void *p2) {
...
static void create_edge_and_update_destination_phis(
    struct redirection_data *rd) {
...
static int create_duplicates(void **slot, void *data) {
...
static int fixup_template_block(void **slot, void *data) {
  struct redirection_data *rd = (struct redirection_data *)*slot;
  struct local_info *local_info = (struct local_info *)data;
...
static int redirect_edges(void **slot, void *data) {
  struct redirection_data *rd = (struct redirection_data *)*slot;
  struct local_info *local_info = (struct local_info *)data;
  struct el *next, *el;
    ...
    if (rd->dup_block) {
      ...
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf(dump_file, "  Threaded jump %d --> %d to %d\n", e->src->index,
                e->dest->index, rd->dup_block->index);
      ...
      if (dump_file && (dump_flags & TDF_DETAILS))
        fprintf(dump_file, "  Threaded jump %d --> %d to %d\n", e->src->index,
                e->dest->index, local_info->bb->index);
...
static bool redirection_block_p(basic_block bb) {
  ...
  return gsi_stmt(gsi) && (gimple_code(gsi_stmt(gsi)) == GIMPLE_COND ||
                           gimple_code(gsi_stmt(gsi)) == GIMPLE_GOTO ||
                           gimple_code(gsi_stmt(gsi)) == GIMPLE_SWITCH);
}
...
static bool thread_block(basic_block bb, bool noloop_only) {
  ...
  redirection_data = htab_create(EDGE_COUNT(bb->succs), redirection_data_hash,
                                 redirection_data_eq, free);
  ...
  FOR_EACH_EDGE(e, ei, bb->preds) {
    ...
    if (!e2
        ...
        || (noloop_only && bb == bb->loop_father->header &&
            !loop_exit_edge_p(bb->loop_father, e2))) {
      all = false;
      continue;
    }

    update_bb_profile_for_threading(e->dest, EDGE_FREQUENCY(e), e->count,
                                    (edge)e->aux);
...
static basic_block thread_single_edge(edge e) {
...
static bool dbds_continue_enumeration_p(const_basic_block bb,
                                        const void *stop) {
  return (bb != (const_basic_block)stop && bb != dbds_ce_stop);
}
...
static enum bb_dom_status determine_bb_domination_status(struct loop *loop,
                                                         basic_block bb) {
  basic_block *bblocks;
  unsigned nblocks, i;
...
static bool thread_through_loop_header(struct loop *loop,
  ...
  if (latch->aux) {
  ...
  } else if (!may_peel_loop_headers && !redirection_block_p(loop->header))
    goto fail;
  else {
    tgt_bb = NULL;
    tgt_edge = NULL;
    ...
    if (tgt_bb == loop->latch && empty_block_p(loop->latch))
      goto fail;
  }
...
static void mark_threaded_blocks(bitmap threaded_blocks) {
  unsigned int i;
  bitmap_iterator bi;
  ...
  if (optimize_function_for_size_p(cfun)) {
    EXECUTE_IF_SET_IN_BITMAP(tmp, 0, i, bi) {
      bb = BASIC_BLOCK(i);
      if (EDGE_COUNT(bb->preds) > 1 && !redirection_block_p(bb)) {
        FOR_EACH_EDGE(e, ei, bb->preds)
        e->aux = NULL;
      } else
...
bool thread_through_all_blocks(bool may_peel_loop_headers) {
  ...
  FOR_EACH_LOOP(li, loop, LI_FROM_INNERMOST) {
    if (!loop->header || !bitmap_bit_p(threaded_blocks, loop->header->index))
      continue;
...
void register_jump_thread(edge e, edge e2) {
  if (threaded_edges == NULL)
