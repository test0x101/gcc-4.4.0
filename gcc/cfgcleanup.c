...
static bool try_simplify_condjump(basic_block cbranch_block) {
  ...
  if (!single_pred_p(jump_block) || jump_block->next_bb == EXIT_BLOCK_PTR ||
...
static edge thread_jump(edge e, basic_block b) {
...
static bool try_forward_edges(int mode, basic_block b) {
  ...
  for (ei = ei_start(b->succs); (e = ei_safe_edge(ei));) {
    ...
    if (counter >= n_basic_blocks) {
      ...
      edge_frequency =
          ((edge_probability * b->frequency + REG_BR_PROB_BASE / 2) /
           REG_BR_PROB_BASE);
      ...
      do {
        ...
        if (!single_succ_p(first)) {
          ...
          update_bb_profile_for_threading(first, edge_frequency, edge_count, t);
...
static basic_block merge_blocks_move(edge e, basic_block b, basic_block c,
                                     int mode) {
  basic_block next;
...
void merge_memattrs(rtx x, rtx y) {
  int i;
  int j;
  enum rtx_code code;
  const char *fmt;
  ...
  if (code == MEM && MEM_ATTRS(x) != MEM_ATTRS(y)) {
    if (!MEM_ATTRS(x))
      ...
      if (!MEM_SIZE(x))
        ...
        mem_size = GEN_INT(MAX(INTVAL(MEM_SIZE(x)), INTVAL(MEM_SIZE(y))));
...
static bool old_insns_match_p(int mode ATTRIBUTE_UNUSED, rtx i1, rtx i2) {
  rtx p1, p2;
  ...
  if (CALL_P(i1) && (!rtx_equal_p(CALL_INSN_FUNCTION_USAGE(i1),
...
static int flow_find_cross_jump(int mode ATTRIBUTE_UNUSED, basic_block bb1,
...
static bool outgoing_edges_match(int mode, basic_block bb1, basic_block bb2) {
  ...
  if (single_succ_p(bb1) &&
    ...
    return (single_succ_p(bb2) &&
            (single_succ_edge(bb2)->flags & (EDGE_COMPLEX | EDGE_FAKE)) == 0 &&
            (!JUMP_P(BB_END(bb2)) || simplejump_p(BB_END(bb2))));

  /* Match conditional jumps - this may get tricky when fallthru and branch
     edges are crossed.  */
  if (EDGE_COUNT(bb1->succs) == 2 && any_condjump_p(BB_END(bb1)) &&
    ...
    if (EDGE_COUNT(bb2->succs) != 2 || !any_condjump_p(BB_END(bb2)) ||
    ...
    /* To simplify use of this function, return false if there are
       ...
       during cleanup_cfg.  */
    if (FORWARDER_BLOCK_P(f1->dest) || FORWARDER_BLOCK_P(f2->dest) ||
    ...
    match = ((code1 == code2 &&
             ...
             (code1 == swap_condition(code2) &&
              rtx_renumbered_equal_p(XEXP(cond1, 1), XEXP(cond2, 0)) &&
              rtx_renumbered_equal_p(XEXP(cond1, 0), XEXP(cond2, 1))));

    /* If we return true, we will join the blocks.  Which means that
       we will only have one branch prediction bit to work with.  Thus
       we require the existing branches to have probabilities that are
       roughly similar.  */
    if (match && optimize_bb_for_speed_p(bb1) && optimize_bb_for_speed_p(bb2)) {
      int prob2;
  ...
  if (fallthru1) {
    basic_block d1 =
        (forwarder_block_p(fallthru1->dest) ? single_succ(fallthru1->dest)
                                            : fallthru1->dest);
    basic_block d2 =
        (forwarder_block_p(fallthru2->dest) ? single_succ(fallthru2->dest)
                                            : fallthru2->dest);
...
static bool block_has_preserve_label(basic_block bb) {
  return (bb && block_label(bb) && LABEL_PRESERVE_P(block_label(bb)));
}
...
static bool try_crossjump_to_edge(int mode, edge e1, edge e2) {
  ...
  if (block_has_preserve_label(e1->dest) && (e1->flags & EDGE_ABNORMAL))
    return false;
  ...
  if (newpos2 == BB_HEAD(src2) && !(EDGE_PRED(src2, 0)->flags & EDGE_EH))
    redirect_to = src2;
  else {
  ...
  FOR_EACH_EDGE(s, ei, redirect_to->succs) {
    ...
    if (!redirect_to->frequency && !src1->frequency)
      ...
      s->probability = ((s->probability * redirect_to->frequency +
...
static bool try_crossjump_bb(int mode, basic_block bb) {
  ...
  if (optimize_bb_for_size_p(bb) && bb != EXIT_BLOCK_PTR &&
  ...
  for (ix = 0, ev = bb; ix < EDGE_COUNT(ev->preds);) {
    ...
    for (ix2 = 0, ev2 = bb; ix2 < EDGE_COUNT(ev2->preds);) {
      ...
      /* If nothing changed since the last attempt, there is nothing
         we can do.  */
      if (!first_pass &&
          (!(df_get_bb_dirty(e->src)) && !(df_get_bb_dirty(e2->src))))
        continue;
  ...
  if (!targetm.cannot_modify_jumps_p()) {
    ...
    do {
      ...
      for (b = ENTRY_BLOCK_PTR->next_bb; b != EXIT_BLOCK_PTR;) {
        ...
        if (single_pred_p(b) && (single_pred_edge(b)->flags & EDGE_FALLTHRU) &&
            ...
            && (single_pred(b) == ENTRY_BLOCK_PTR ||
                ...
                !label_is_jump_target_p(BB_HEAD(b), BB_END(single_pred(b))))) {
          rtx label = BB_HEAD(b);
        ...
        if (!(mode & CLEANUP_CFGLAYOUT) && single_pred_p(b) &&
          ...
          redirect_edge_succ_nodup(single_pred_edge(b), single_succ(b));
          delete_basic_block(b);
          changed = true;
          b = c;
        }

        if (single_succ_p(b) && (s = single_succ_edge(b)) &&
            !(s->flags & EDGE_COMPLEX) && (c = s->dest) != EXIT_BLOCK_PTR &&
            single_pred_p(c) && b != c) {
            ...
            changed_here = true;
                     ...
                     && (!JUMP_P(BB_END(b)) ||
                         (reload_completed
                              ...
                              : (onlyjump_p(BB_END(b)) &&
                                 !tablejump_p(BB_END(b), NULL, NULL)))) &&
        ...
        if ((mode & CLEANUP_EXPENSIVE) && !(mode & CLEANUP_CFGLAYOUT) &&
        ...
        if (single_succ_p(b) && single_succ(b) != EXIT_BLOCK_PTR &&
            onlyjump_p(BB_END(b)) &&
            !find_reg_note(BB_END(b), REG_CROSSING_JUMP, NULL_RTX) &&
            try_redirect_by_replacing_jump(single_succ_edge(b), single_succ(b),
...
void delete_dead_jumptables(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    for (insn = NEXT_INSN(BB_END(bb)); insn && !NOTE_INSN_BASIC_BLOCK_P(insn);
         insn = next) {
      next = NEXT_INSN(insn);
      if (LABEL_P(insn) && LABEL_NUSES(insn) == LABEL_PRESERVE_P(insn) &&
          JUMP_P(next) &&
  ...
  while (try_optimize_cfg(mode)) {
    ...
    if (!(mode & CLEANUP_NO_INSN_DEL)) {
      ...
      if ((mode & CLEANUP_EXPENSIVE) && !reload_completed &&
          !delete_trivially_dead_insns(get_insns(), max_reg_num()))
        break;
      else if ((mode & CLEANUP_CROSSJUMP) && crossjumps_occured)
        run_fast_dce();
    } else
      break;
  }
...
static unsigned int rest_of_handle_jump(void) {
...
static unsigned int rest_of_handle_jump2(void) {
  delete_trivially_dead_insns(get_insns(), max_reg_num());
  if (dump_file)
