/* Precompiled header implementation for the C languages.
   Copyright (C) 2000, 2002, 2003, 2004, 2005, 2007, 2008 Free Software
Foundation, Inc.
...
int c_common_valid_pch(cpp_reader *pfile, const char *name, int fd) {
  ...
  if (sizeread == -1)
    ...
    if (cpp_get_options(pfile)->warn_invalid_pch)
      cpp_error(pfile, CPP_DL_WARNING, "%s: too short to be a PCH file", name);
    return 2;
  }
  ...
  /* Check flags that must match exactly.  */
  {
    ...
    for (i = 0; i < MATCH_SIZE; i++)
      if (*pch_matching[i].flag_var != v.match[i]) {
        if (cpp_get_options(pfile)->warn_invalid_pch)
          cpp_error(pfile, CPP_DL_WARNING, "%s: settings for %s do not match",
                    name, pch_matching[i].flag_name);
        return 2;
      }
  }
...
void c_common_read_pch(cpp_reader *pfile, const char *name, int fd,
