...
static int cached_reg_save_code[FIRST_PSEUDO_REGISTER][MAX_MACHINE_MODE];
static int cached_reg_restore_code[FIRST_PSEUDO_REGISTER][MAX_MACHINE_MODE];
...
static int reg_save_code(int reg, enum machine_mode mode) {
  bool ok;
...
static int reg_restore_code(int reg, enum machine_mode mode) {
...
void init_caller_save(void) {
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
    if (TEST_HARD_REG_BIT(reg_class_contents[(int)base_reg_class(
                              regno_save_mode[i][1], PLUS, CONST_INT)],
                          i))
      break;
...
/* Allocate and return new saved hard register with given REGNO and
   CALL_FREQ.  */
static struct saved_hard_reg *new_saved_hard_reg(int regno, int call_freq) {
  struct saved_hard_reg *saved_reg;

  saved_reg = (struct saved_hard_reg *)xmalloc(sizeof(struct saved_hard_reg));
...
static int saved_hard_reg_compare_func(const void *v1p, const void *v2p) {
...
void setup_save_areas(void) {
  ...
  for (i = FIRST_PSEUDO_REGISTER; i < max_regno; i++)
    if (reg_renumber[i] >= 0 && REG_N_CALLS_CROSSED(i) > 0) {
      ...
      unsigned int endregno = end_hard_regno(GET_MODE(regno_reg_rtx[i]), regno);
    ...
    for (i = 0; i < saved_regs_num; i++) {
      ...
      for (j = 0; j < i; j++) {
        ...
        if (k < 0 &&
            (GET_MODE_SIZE(regno_save_mode[regno][1]) <=
             GET_MODE_SIZE(regno_save_mode[saved_reg2->hard_regno][1]))) {
          saved_reg->slot = adjust_address_nv(
      ...
      if (j == i) {
        ...
        if (best_slot_num >= 0) {
          ...
          saved_reg->slot = adjust_address_nv(
              saved_reg->slot, regno_save_mode[saved_reg->hard_regno][1], 0);
          if (dump_file != NULL)
            fprintf(dump_file, "%d uses a slot from prev iteration\n", regno);
          prev_save_slots[best_slot_num] = NULL_RTX;
          if (best_slot_num + 1 == prev_save_slots_num)
            prev_save_slots_num--;
        } else {
          saved_reg->slot = assign_stack_local_1(
      ...
      for (j = MOVE_MAX_WORDS; j > 0; j--) {
        ...
        regno_save_mem[i][j] =
            assign_stack_local_1(regno_save_mode[i][j],
                                 GET_MODE_SIZE(regno_save_mode[i][j]), 0, true);
        ...
        for (k = 0; k < j; k++)
          ...
          regno_save_mem[i + k][1] =
              adjust_address_nv(regno_save_mem[i][j], regno_save_mode[i + k][1],
                                k * UNITS_PER_WORD);
      }
  }
  ...
  for (chain = reload_insn_chain; chain != 0; chain = next) {
    ...
    if (INSN_P(insn)) {
      ...
      if (code == CALL_INSN && !SIBLING_CALL_P(insn) &&
        ...
        EXECUTE_IF_SET_IN_REG_SET(&chain->live_throughout,
          ...
          mode =
              HARD_REGNO_CALLER_SAVE_MODE(r, nregs, PSEUDO_REGNO_MODE(regno));
          if (GET_MODE_BITSIZE(mode) > GET_MODE_BITSIZE(save_mode[r]))
            save_mode[r] = mode;
          while (nregs-- > 0)
        ...
        for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
          if (TEST_HARD_REG_BIT(hard_regs_to_save, regno))
            regno +=
                insert_save(chain, 1, regno, &hard_regs_to_save, save_mode);
    ...
    if (chain->next == 0 || chain->next->block != chain->block) {
      ...
      if (n_regs_saved)
        for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
          if (TEST_HARD_REG_BIT(hard_regs_saved, regno))
            regno += insert_restore(chain, JUMP_P(insn), regno, MOVE_MAX_WORDS,
                                    save_mode);
    }
  }
}
...
static void mark_set_regs(rtx reg, const_rtx setter ATTRIBUTE_UNUSED,
                          void *data) {
...
/* Here from note_stores when an insn stores a value in a register.
   ...
   been assigned hard regs have had their register number changed already,
   so we can ignore pseudos.  */
static void add_stored_regs(rtx reg, const_rtx setter, void *data) {
  int regno, endregno, i;
  ...
  if (GET_CODE(reg) == SUBREG && REG_P(SUBREG_REG(reg)) &&
    ...
    offset =
        subreg_regno_offset(REGNO(SUBREG_REG(reg)), GET_MODE(SUBREG_REG(reg)),
...
static void mark_referenced_regs(rtx x) {
  ...
  if (code == SET || code == CLOBBER) {
    x = SET_DEST(x);
    code = GET_CODE(x);
    if ((code == REG && REGNO(x) < FIRST_PSEUDO_REGISTER) || code == PC ||
        code == CC0 ||
...
static int insert_restore(struct insn_chain *chain, int before_p, int regno,
  ...
  if (save_mode[regno] != VOIDmode && save_mode[regno] != GET_MODE(mem) &&
  ...
  pat = gen_rtx_SET(VOIDmode, gen_rtx_REG(GET_MODE(mem), regno), mem);
...
static int insert_save(struct insn_chain *chain, int before_p, int regno,
  ...
  pat = gen_rtx_SET(VOIDmode, mem, gen_rtx_REG(GET_MODE(mem), regno));
...
static int add_used_regs_1(rtx *loc, void *data) {
  int regno, i;
  regset live;
  rtx x;
...
static struct insn_chain *insert_one_insn(struct insn_chain *chain,
                                          int before_p, int code, rtx pat) {
  rtx insn = chain->insn;
  struct insn_chain *new_chain;
  ...
  if ((NONJUMP_INSN_P(insn) || JUMP_P(insn)) && before_p &&
  ...
  if (before_p) {
    ...
    if (CALL_P(chain->insn))
      for (link = CALL_INSN_FUNCTION_USAGE(chain->insn); link != NULL_RTX;
           link = XEXP(link, 1))
        note_uses(&XEXP(link, 0), add_used_regs, &new_chain->live_throughout);
