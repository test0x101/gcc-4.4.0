...
static gimple *free_temp_expr_table(temp_expr_table_p t) {
  gimple *ret = NULL;
...
static inline bool version_to_be_replaced_p(temp_expr_table_p tab,
                                            int version) {
...
static inline void make_dependent_on_partition(temp_expr_table_p tab,
                                               int version, int p) {
...
static inline void add_to_partition_kill_list(temp_expr_table_p tab, int p,
                                              int ver) {
  if (!tab->kill_list[p]) {
...
static inline void remove_from_partition_kill_list(temp_expr_table_p tab, int p,
                                                   int version) {
#ifdef ENABLE_CHECKING
...
static void add_dependence(temp_expr_table_p tab, int version, tree var) {
  int i;
  bitmap_iterator bi;
  unsigned x;
  ...
  if (version_to_be_replaced_p(tab, i)) {
    if (!bitmap_empty_p(tab->new_replaceable_dependencies)) {
      ...
      bitmap_ior_into(tab->partition_in_use, tab->new_replaceable_dependencies);
...
static inline bool is_replaceable_p(gimple stmt) {
  use_operand_p use_p;
  tree def;
  gimple use_stmt;
  ...
  if (flag_float_store && FLOAT_TYPE_P(gimple_expr_type(stmt)))
    return false;
...
static void finished_with_expr(temp_expr_table_p tab, int version,
                               bool free_expr) {
  unsigned i;
  bitmap_iterator bi;
...
static void process_replaceable(temp_expr_table_p tab, gimple stmt) {
...
static inline void kill_expr(temp_expr_table_p tab, int partition) {
  unsigned version;
...
static inline void kill_virtual_exprs(temp_expr_table_p tab) {
...
extern gimple *find_replaceable_exprs(var_map map) {
  basic_block bb;
...
void dump_replaceable_exprs(FILE *f, gimple *expr) {
  tree var;
  unsigned x;
