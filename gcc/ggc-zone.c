...
struct alloc_zone {
  ...
  /* Linked lists of free storage.  Slots 1 ... NUM_FREE_BINS have chunks of
     size FREE_BIN_DELTA.  All other chunks are in slot 0.  */
...
static inline page_entry *lookup_page_table_if_allocated(const void *p) {
  page_entry ***base;
  size_t L1, L2;
...
static inline struct page_entry *zone_get_object_page(const void *object) {
  return lookup_page_table_entry(object);
}

/* Find which element of the alloc_bits array OBJECT should be
   recorded in.  */
static inline unsigned int zone_get_object_alloc_word(const void *object) {
...
static inline unsigned int zone_get_object_alloc_bit(const void *object) {
  return (((size_t)object / BYTES_PER_ALLOC_BIT) % (8 * sizeof(alloc_type)));
}

/* Find which element of the mark_bits array OBJECT should be recorded
   in.  */
static inline unsigned int zone_get_object_mark_word(const void *object) {
  return (((size_t)object & (GGC_PAGE_SIZE - 1)) /
...
/* Find which bit of the appropriate word in the mark_bits array
   OBJECT should be recorded in.  */
static inline unsigned int zone_get_object_mark_bit(const void *object) {
  return (((size_t)object / BYTES_PER_MARK_BIT) % (8 * sizeof(mark_type)));
}

/* Set the allocation bit corresponding to OBJECT in its page's
   bitmap.  Used to split this object from the preceding one.  */
static inline void zone_set_object_alloc_bit(const void *object) {
...
/* Clear the allocation bit corresponding to OBJECT in PAGE's
   ...
   one.  */
static inline void zone_clear_object_alloc_bit(struct small_page_entry *page,
                                               const void *object) {
  unsigned int start_word = zone_get_object_alloc_word(object);
  unsigned int start_bit = zone_get_object_alloc_bit(object);
...
static inline size_t zone_object_size_1(alloc_type *alloc_bits,
...
static inline size_t zone_find_object_size(struct small_page_entry *page,
                                           const void *object) {
  ...
  size_t max_size =
      (page->common.page + SMALL_PAGE_SIZE - (const char *)object);

  return zone_object_size_1(page->alloc_bits, start_word, start_bit, max_size);
}
...
static inline alloc_type highest_bit(alloc_type value) {
...
static unsigned long zone_find_object_offset(alloc_type *alloc_bits,
                                             size_t start_word,
                                             size_t start_bit) {
...
/* Allocate the mark bits for every zone, and set the pointers on each
   page.  */
static void zone_allocate_marks(void) {
  struct alloc_zone *zone;
  ...
  for (zone = G.zones; zone; zone = zone->next_zone) {
    ...
    zone->mark_bits = (mark_type *)xcalloc(sizeof(mark_type), mark_words);
  ...
  /* We don't collect the PCH zone, but we do have to mark it
     (for now).  */
  if (pch_zone.bytes)
    pch_zone.mark_bits = (mark_type *)xcalloc(
        sizeof(mark_type), CEIL(pch_zone.bytes, BYTES_PER_MARK_WORD));
}

/* After marking and sweeping, release the memory used for mark bits.  */
static void zone_free_marks(void) {
  struct alloc_zone *zone;
...
#endif
#ifdef HAVE_MMAP_DEV_ZERO
  char *page = (char *)mmap(pref, size, PROT_READ | PROT_WRITE, MAP_PRIVATE,
                            G.dev_zero_fd, 0);
#endif
...
static struct small_page_entry *alloc_small_page(struct alloc_zone *zone) {
  ...
  if (GGC_DEBUG_LEVEL >= 2)
    fprintf(G.debug_file, "Allocating %s page at %p, data %p-%p\n",
...
static struct large_page_entry *alloc_large_page(size_t size,
                                                 struct alloc_zone *zone) {
  ...
  if (GGC_DEBUG_LEVEL >= 2)
    fprintf(G.debug_file, "Allocating %s large page at %p, data %p-%p\n",
            entry->common.zone->name, (PTR)entry, entry->common.page,
            entry->common.page + SMALL_PAGE_SIZE - 1);
...
static inline void free_small_page(struct small_page_entry *entry) {
  if (GGC_DEBUG_LEVEL >= 2)
    fprintf(G.debug_file, "Deallocating %s page at %p, data %p-%p\n",
            entry->common.zone->name, (PTR)entry, entry->common.page,
            entry->common.page + SMALL_PAGE_SIZE - 1);
  ...
  /* Mark the page as inaccessible.  Discard the handle to
     avoid handle leak.  */
  VALGRIND_DISCARD(
      VALGRIND_MAKE_MEM_NOACCESS(entry->common.page, SMALL_PAGE_SIZE));
...
static inline void free_large_page(struct large_page_entry *entry) {
  if (GGC_DEBUG_LEVEL >= 2)
    fprintf(G.debug_file, "Deallocating %s page at %p, data %p-%p\n",
            entry->common.zone->name, (PTR)entry, entry->common.page,
            entry->common.page + SMALL_PAGE_SIZE - 1);
...
static void release_pages(struct alloc_zone *zone) {
#ifdef USING_MMAP
...
static inline void free_chunk(char *ptr, size_t size, struct alloc_zone *zone) {
  ...
  if (bin > NUM_FREE_BINS) {
    bin = 0;
    VALGRIND_DISCARD(
        VALGRIND_MAKE_MEM_UNDEFINED(chunk, sizeof(struct alloc_chunk)));
    chunk->size = size;
    chunk->next_free = zone->free_chunks[bin];
    VALGRIND_DISCARD(VALGRIND_MAKE_MEM_NOACCESS(
        ptr + sizeof(struct alloc_chunk), size - sizeof(struct alloc_chunk)));
  } else {
    VALGRIND_DISCARD(
        VALGRIND_MAKE_MEM_UNDEFINED(chunk, sizeof(struct alloc_chunk *)));
    chunk->next_free = zone->free_chunks[bin];
    VALGRIND_DISCARD(
        VALGRIND_MAKE_MEM_NOACCESS(ptr + sizeof(struct alloc_chunk *),
...
void *ggc_alloc_zone_stat(size_t orig_size,
  ...
  if (bin <= NUM_FREE_BINS && (chunk = zone->free_chunks[bin]) != NULL) {
...
found:
  ...
  VALGRIND_DISCARD(
      VALGRIND_MAKE_MEM_NOACCESS(result + orig_size, size - orig_size));
#endif
...
void *ggc_alloc_typed_stat(enum gt_types_enum gte, size_t size MEM_STAT_DECL) {
  switch (gte) {
...
/* Poison the chunk.  */
#ifdef ENABLE_GC_CHECKING
#define poison_region(PTR, SIZE) memset((PTR), 0xa5, (SIZE))
#else
...
void ggc_free(void *p) {
  ...
  if (page->large_p) {
    struct large_page_entry *large_page = (struct large_page_entry *)page;
...
void gt_ggc_m_S(const void *p) {
  ...
  if (entry->pch_p) {
    ...
    offset =
        zone_find_object_offset(pch_zone.alloc_bits, alloc_word, alloc_bit);
...
int ggc_set_mark(const void *p) {
  ...
  if (page->pch_p) {
    ...
    struct small_page_entry *small_page = (struct small_page_entry *)page;
...
static void new_ggc_zone_1(struct alloc_zone *new_zone, const char *name) {
  new_zone->name = name;
  new_zone->next_zone = G.zones->next_zone;
  G.zones->next_zone = new_zone;
}

struct alloc_zone *new_ggc_zone(const char *name) {
...
/* Destroy a GGC zone.  */
void destroy_ggc_zone(struct alloc_zone *dead_zone) {
  struct alloc_zone *z;
...
static void sweep_pages(struct alloc_zone *zone) {
    ...
    do {
      ...
      while ((n = alloc_ffs(alloc_word)) != 0) {
        ...
        if (mark_word & 1) {
          if (last_free) {
            VALGRIND_DISCARD(
                VALGRIND_MAKE_MEM_UNDEFINED(last_free, object - last_free));
...
#ifdef ENABLE_GC_CHECKING
      VALGRIND_DISCARD(
          VALGRIND_MAKE_MEM_UNDEFINED(sp->common.page, SMALL_PAGE_SIZE));
      /* Poison the page.  */
...
static bool ggc_collect_1(struct alloc_zone *zone, bool need_marking) {
#if 0
  /* */
  {
    int i;
  ...
  if (!quiet_flag)
    fprintf(stderr, " {%s GC %luk -> ", zone->name,
            (unsigned long)zone->allocated / 1024);
...
static float calculate_average_page_survival(struct alloc_zone *zone) {
  float count = 0.0;
  float survival = 0.0;
...
void ggc_collect(void) {
  ...
  if (!ggc_force_collect) {
    ...
    allocated_last_gc =
        MAX(allocated_last_gc, (size_t)PARAM_VALUE(GGC_MIN_HEAPSIZE) * 1024);
...
void ggc_print_statistics(void) {
  ...
  for (zone = G.zones; zone; zone = zone->next_zone) {
    ...
    fprintf(stderr, "%20s %10lu%c %10lu%c %10lu%c\n", zone->name,
            SCALE(allocated), LABEL(allocated), SCALE(in_use), LABEL(in_use),
  ...
  fprintf(stderr, "%20s %10lu%c %10lu%c %10lu%c\n", "Total",
          ...
          SCALE(total_allocated), LABEL(total_allocated), SCALE(total_overhead),
...
static int pch_bucket(void *x, enum gt_types_enum type, bool is_string) {
...
void ggc_pch_count_object(struct ggc_pch_data *d, void *x ATTRIBUTE_UNUSED,
...
void ggc_pch_this_base(struct ggc_pch_data *d, void *base_) {
  int i;
...
char *ggc_pch_alloc_object(struct ggc_pch_data *d, void *x, size_t size,
                           bool is_string, enum gt_types_enum type) {
  ...
  alloc_word = ((d->type_bases[bucket] - d->orig_base) /
                (8 * sizeof(alloc_type) * BYTES_PER_ALLOC_BIT));
  alloc_bit = ((d->type_bases[bucket] - d->orig_base) / BYTES_PER_ALLOC_BIT) %
              (8 * sizeof(alloc_type));
...
void ggc_pch_write_object(struct ggc_pch_data *d, FILE *f, void *x, void *newx,
...
void ggc_pch_read(FILE *f, void *addr) {
  ...
  for (zone = G.zones; zone; zone = zone->next_zone) {
    ...
    for (page = zone->pages; page != NULL; page = next_page) {
      next_page = page->next;
      memset(page->alloc_bits, 0, G.small_page_overhead - PAGE_OVERHEAD);
