...
static tree combine_comparisons(enum tree_code, enum tree_code, enum tree_code,
                                tree, tree, tree);
...
static tree decode_field_reference(tree, HOST_WIDE_INT *, HOST_WIDE_INT *,
                                   enum machine_mode *, int *, int *, tree *,
                                   tree *);
...
static tree fold_binary_op_with_conditional_arg(enum tree_code, tree, tree,
                                                tree, tree, tree, int);
static tree fold_mathfn_compare(enum built_in_function, enum tree_code, tree,
                                tree, tree);
...
static void encode(HOST_WIDE_INT *words, unsigned HOST_WIDE_INT low,
                   HOST_WIDE_INT hi) {
  words[0] = LOWPART(low);
...
static void decode(HOST_WIDE_INT *words, unsigned HOST_WIDE_INT *low,
                   HOST_WIDE_INT *hi) {
...
int fit_double_type(unsigned HOST_WIDE_INT l1, HOST_WIDE_INT h1,
  ...
  if (!sign_extended_type)
    ...
    if (h1 & ((unsigned HOST_WIDE_INT)1 << (prec - HOST_BITS_PER_WIDE_INT - 1)))
      h1 |= (HOST_WIDE_INT)(-1) << (prec - HOST_BITS_PER_WIDE_INT);
  } else if (prec == HOST_BITS_PER_WIDE_INT) {
...
tree force_fit_type_double(tree type, unsigned HOST_WIDE_INT low,
...
int add_double_with_sign(unsigned HOST_WIDE_INT l1, HOST_WIDE_INT h1,
...
int neg_double(unsigned HOST_WIDE_INT l1, HOST_WIDE_INT h1,
               unsigned HOST_WIDE_INT *lv, HOST_WIDE_INT *hv) {
  if (l1 == 0) {
    *lv = 0;
    *hv = -h1;
...
int mul_double_with_sign(unsigned HOST_WIDE_INT l1, HOST_WIDE_INT h1,
                         unsigned HOST_WIDE_INT l2, HOST_WIDE_INT h2,
                         unsigned HOST_WIDE_INT *lv, HOST_WIDE_INT *hv,
                         bool unsigned_p) {
...
void lshift_double(unsigned HOST_WIDE_INT l1, HOST_WIDE_INT h1,
  ...
  signmask =
      -((prec > HOST_BITS_PER_WIDE_INT ? ((unsigned HOST_WIDE_INT) * hv >>
...
void rshift_double(unsigned HOST_WIDE_INT l1, HOST_WIDE_INT h1,
                   HOST_WIDE_INT count, unsigned int prec,
                   unsigned HOST_WIDE_INT *lv, HOST_WIDE_INT *hv, int arith) {
  unsigned HOST_WIDE_INT signmask;

  signmask =
      (arith ? -((unsigned HOST_WIDE_INT)h1 >> (HOST_BITS_PER_WIDE_INT - 1))
             : 0);
...
void lrotate_double(unsigned HOST_WIDE_INT l1, HOST_WIDE_INT h1,
                    HOST_WIDE_INT count, unsigned int prec,
                    unsigned HOST_WIDE_INT *lv, HOST_WIDE_INT *hv) {
...
void rrotate_double(unsigned HOST_WIDE_INT l1, HOST_WIDE_INT h1,
                    HOST_WIDE_INT count, unsigned int prec,
                    unsigned HOST_WIDE_INT *lv, HOST_WIDE_INT *hv) {
  unsigned HOST_WIDE_INT s1l, s2l;
  HOST_WIDE_INT s1h, s2h;
...
int div_and_round_double(
    enum tree_code code, int uns,
    unsigned HOST_WIDE_INT lnum_orig, /* num == numerator == dividend */
    HOST_WIDE_INT hnum_orig,
    unsigned HOST_WIDE_INT lden_orig, /* den == denominator == divisor */
    HOST_WIDE_INT hden_orig, unsigned HOST_WIDE_INT *lquo, HOST_WIDE_INT *hquo,
    unsigned HOST_WIDE_INT *lrem, HOST_WIDE_INT *hrem) {
  int quo_neg = 0;
...
finish_up:
  ...
  switch (code) {
    ...
    case FLOOR_MOD_EXPR: /* round toward negative infinity */
      ...
      {
        /* quo = quo - 1;  */
        add_double(*lquo, *hquo, (HOST_WIDE_INT)-1, (HOST_WIDE_INT)-1, lquo,
                   hquo);
      } else
        return overflow;
      break;
    ...
    case CEIL_MOD_EXPR: /* round toward positive infinity */
      if (!quo_neg && (*lrem != 0 || *hrem != 0)) /* ratio > 0 && rem != 0 */
      {
        add_double(*lquo, *hquo, (HOST_WIDE_INT)1, (HOST_WIDE_INT)0, lquo,
                   hquo);
      } else
        return overflow;
      break;
    ...
    case ROUND_MOD_EXPR: /* round to closest integer */
    {
      ...
      mul_double((HOST_WIDE_INT)2, (HOST_WIDE_INT)0, labs_rem, habs_rem,
                 &ltwice, &htwice);

      if (((unsigned HOST_WIDE_INT)habs_den < (unsigned HOST_WIDE_INT)htwice) ||
...
static tree div_if_zero_remainder(enum tree_code code, const_tree arg1,
                                  const_tree arg2) {
  ...
  div_and_round_double(code, uns, int1l, int1h, int2l, int2h, &quol, &quoh,
                       &reml, &remh);
  if (remh != 0 || reml != 0)
    return NULL_TREE;
...
void fold_undefer_overflow_warnings(bool issue, const_gimple stmt, int code) {
  const char *warnmsg;
  location_t locus;
  ...
  if (fold_deferring_overflow_warnings > 0) {
    if (fold_deferred_overflow_warning != NULL && code != 0 &&
...
static bool negate_mathfn_p(enum built_in_function code) {
  switch (code) {
    ...
    CASE_FLT_FN(BUILT_IN_LLRINT)
        ...
        : CASE_FLT_FN(BUILT_IN_RINT) : return !flag_rounding_math;
...
static bool negate_expr_p(tree t) {
  tree type;
  ...
  switch (TREE_CODE(t)) {
    ...
    case COMPLEX_CST:
      return negate_expr_p(TREE_REALPART(t)) && negate_expr_p(TREE_IMAGPART(t));
...
static tree fold_negate_expr(tree t) {
  ...
  switch (TREE_CODE(t)) {
    ...
    case INTEGER_CST:
      ...
      if (TREE_OVERFLOW(tem) == TREE_OVERFLOW(t) || !TYPE_OVERFLOW_TRAPS(type))
        return tem;
      break;
    ...
    case COMPLEX_CST: {
      ...
      if ((TREE_CODE(rpart) == REAL_CST && TREE_CODE(ipart) == REAL_CST) ||
          (TREE_CODE(rpart) == INTEGER_CST && TREE_CODE(ipart) == INTEGER_CST))
    ...
    case PLUS_EXPR:
      if (!HONOR_SIGN_DEPENDENT_ROUNDING(TYPE_MODE(type)) &&
        ...
        if (negate_expr_p(TREE_OPERAND(t, 1)) &&
          ...
          return fold_build2(MINUS_EXPR, type, tem, TREE_OPERAND(t, 0));
        }

        /* -(A + B) -> (-A) - B.  */
        if (negate_expr_p(TREE_OPERAND(t, 0))) {
          tem = negate_expr(TREE_OPERAND(t, 0));
          return fold_build2(MINUS_EXPR, type, tem, TREE_OPERAND(t, 1));
        }
      }
      break;
    ...
    case MINUS_EXPR:
      ...
      if (!HONOR_SIGN_DEPENDENT_ROUNDING(TYPE_MODE(type)) &&
          !HONOR_SIGNED_ZEROS(TYPE_MODE(type)) &&
          reorder_operands_p(TREE_OPERAND(t, 0), TREE_OPERAND(t, 1)))
        return fold_build2(MINUS_EXPR, type, TREE_OPERAND(t, 1),
                           TREE_OPERAND(t, 0));
      break;
    ...
    case RDIV_EXPR:
      if (!HONOR_SIGN_DEPENDENT_ROUNDING(TYPE_MODE(type))) {
        ...
        if (negate_expr_p(tem))
          return fold_build2(TREE_CODE(t), type, TREE_OPERAND(t, 0),
                             negate_expr(tem));
        tem = TREE_OPERAND(t, 0);
        if (negate_expr_p(tem))
          return fold_build2(TREE_CODE(t), type, negate_expr(tem),
                             TREE_OPERAND(t, 1));
      }
      break;
    ...
    case EXACT_DIV_EXPR:
      ...
      if (!INTEGRAL_TYPE_P(type) || TYPE_OVERFLOW_UNDEFINED(type)) {
        ...
        if (negate_expr_p(tem)) {
          if (INTEGRAL_TYPE_P(type) &&
              (TREE_CODE(tem) != INTEGER_CST || integer_onep(tem)))
    ...
    case RSHIFT_EXPR:
      ...
      if (TREE_CODE(TREE_OPERAND(t, 1)) == INTEGER_CST) {
        ...
        if (TREE_INT_CST_HIGH(op1) == 0 &&
            (unsigned HOST_WIDE_INT)(TYPE_PRECISION(type) - 1) ==
                TREE_INT_CST_LOW(op1)) {
          tree ntype = TYPE_UNSIGNED(type) ? signed_type_for(type)
...
static tree split_tree(tree in, enum tree_code code, tree *conp, tree *litp,
...
static tree associate_trees(tree t1, tree t2, enum tree_code code, tree type) {
  if (t1 == 0)
    return t2;
  else if (t2 == 0)
    return t1;
  ...
  if (TREE_CODE(t1) == code || TREE_CODE(t2) == code ||
    ...
    return build2(code, type, fold_convert(type, t1), fold_convert(type, t2));
  }
...
static bool int_binop_types_match_p(enum tree_code code, const_tree type1,
                                    const_tree type2) {
...
tree int_const_binop(enum tree_code code, const_tree arg1, const_tree arg2,
                     int notrunc) {
  unsigned HOST_WIDE_INT int1l, int2l;
  HOST_WIDE_INT int1h, int2h;
  ...
  switch (code) {
    ...
    case LSHIFT_EXPR:
      ...
      lshift_double(int1l, int1h, int2l, TYPE_PRECISION(type), &low, &hi, !uns);
      break;
    ...
    case EXACT_DIV_EXPR:
      ...
      if (int2h == 0 && (HOST_WIDE_INT)int2l > 0 && !TREE_OVERFLOW(arg1) &&
          !TREE_OVERFLOW(arg2) && int1h == 0 && (HOST_WIDE_INT)int1l >= 0) {
    ...
    case MAX_EXPR:
      if (uns)
        low =
            (((unsigned HOST_WIDE_INT)int1h < (unsigned HOST_WIDE_INT)int2h) ||
  ...
  if (notrunc) {
    ...
    if (((!uns || is_sizetype) && overflow) | TREE_OVERFLOW(arg1) |
        TREE_OVERFLOW(arg2)) {
      t = copy_node(t);
      TREE_OVERFLOW(t) = 1;
    }
...
static tree const_binop(enum tree_code code, tree arg1, tree arg2,
  ...
  if (TREE_CODE(arg1) == REAL_CST) {
    ...
    if (HONOR_SNANS(mode) && (REAL_VALUE_ISNAN(d1) || REAL_VALUE_ISNAN(d2)))
      return NULL_TREE;

    /* Don't perform operation if it would raise a division
       by zero exception.  */
    if (code == RDIV_EXPR && REAL_VALUES_EQUAL(d2, dconst0) &&
    ...
    if (flag_trapping_math && MODE_HAS_INFINITIES(mode) &&
        REAL_VALUE_ISINF(result) && !REAL_VALUE_ISINF(d1) &&
        !REAL_VALUE_ISINF(d2))
      return NULL_TREE;
  ...
  if (TREE_CODE(arg1) == COMPLEX_CST) {
    ...
    switch (code) {
      ...
      case MULT_EXPR:
        real = const_binop(MINUS_EXPR, const_binop(MULT_EXPR, r1, r2, notrunc),
                           const_binop(MULT_EXPR, i1, i2, notrunc), notrunc);
        imag = const_binop(PLUS_EXPR, const_binop(MULT_EXPR, r1, i2, notrunc),
                           const_binop(MULT_EXPR, i1, r2, notrunc), notrunc);
        break;

      case RDIV_EXPR: {
        tree magsquared =
            const_binop(PLUS_EXPR, const_binop(MULT_EXPR, r2, r2, notrunc),
                        const_binop(MULT_EXPR, i2, i2, notrunc), notrunc);
        tree t1 =
            const_binop(PLUS_EXPR, const_binop(MULT_EXPR, r1, r2, notrunc),
                        const_binop(MULT_EXPR, i1, i2, notrunc), notrunc);
        tree t2 =
            const_binop(MINUS_EXPR, const_binop(MULT_EXPR, i1, r2, notrunc),
                        const_binop(MULT_EXPR, r1, i2, notrunc), notrunc);
...
tree size_diffop(tree arg0, tree arg1) {
  ...
  gcc_assert(
      int_binop_types_match_p(MINUS_EXPR, TREE_TYPE(arg0), TREE_TYPE(arg1)));
  ...
  if (tree_int_cst_equal(arg0, arg1))
    ...
    return size_binop(MINUS_EXPR, build_int_cst(ctype, 0),
                      fold_convert(ctype, size_binop(MINUS_EXPR, arg1, arg0)));
}
...
static tree fold_convert_const_int_from_int(tree type, const_tree arg1) {
  ...
  t = force_fit_type_double(
      type, TREE_INT_CST_LOW(arg1), TREE_INT_CST_HIGH(arg1),
      ...
      !POINTER_TYPE_P(TREE_TYPE(arg1))
          ...
          && !((TYPE_PRECISION(TREE_TYPE(arg1)) == TYPE_PRECISION(type)) &&
               (TYPE_UNSIGNED(TREE_TYPE(arg1)) == TYPE_UNSIGNED(type)) &&
...
static tree fold_convert_const_int_from_real(enum tree_code code, tree type,
                                             const_tree arg1) {
  int overflow = 0;
  tree t;
...
static tree fold_convert_const_int_from_fixed(tree type, const_tree arg1) {
  ...
  if (GET_MODE_FBIT(mode) < 2 * HOST_BITS_PER_WIDE_INT) {
    lshift_double(temp.low, temp.high, -GET_MODE_FBIT(mode),
                  2 * HOST_BITS_PER_WIDE_INT, &temp.low, &temp.high,
                  SIGNED_FIXED_POINT_MODE_P(mode));

    /* Left shift temp to temp_trunc by fbit.  */
    lshift_double(temp.low, temp.high, GET_MODE_FBIT(mode),
                  2 * HOST_BITS_PER_WIDE_INT, &temp_trunc.low, &temp_trunc.high,
                  SIGNED_FIXED_POINT_MODE_P(mode));
  } else {
    temp.low = 0;
    temp.high = 0;
...
static tree fold_convert_const_fixed_from_int(tree type, const_tree arg1) {
  ...
  overflow_p = fixed_convert_from_int(
      &value, TYPE_MODE(type), TREE_INT_CST(arg1),
      TYPE_UNSIGNED(TREE_TYPE(arg1)), TYPE_SATURATING(type));
  t = build_fixed(type, value);
...
static tree fold_convert_const_fixed_from_real(tree type, const_tree arg1) {
  ...
  overflow_p = fixed_convert_from_real(
      &value, TYPE_MODE(type), &TREE_REAL_CST(arg1), TYPE_SATURATING(type));
  t = build_fixed(type, value);
...
bool fold_convertible_p(const_tree type, const_tree arg) {
  ...
  if (TREE_CODE(arg) == ERROR_MARK || TREE_CODE(type) == ERROR_MARK ||
  ...
  switch (TREE_CODE(type)) {
    ...
    case COMPLEX_TYPE:
      switch (TREE_CODE(orig)) {
        ...
        case REAL_TYPE:
        case FIXED_POINT_TYPE:
          return build2(COMPLEX_EXPR, type, fold_convert(TREE_TYPE(type), arg),
...
tree combine_comparisons(enum tree_code code, enum tree_code lcode,
                         enum tree_code rcode, tree truth_type, tree ll_arg,
                         tree lr_arg) {
  ...
  if (!honor_nans) {
    ...
    bool ltrap = (lcompcode & COMPCODE_UNORD) == 0 &&
                 (lcompcode != COMPCODE_EQ) && (lcompcode != COMPCODE_ORD);
    bool rtrap = (rcompcode & COMPCODE_UNORD) == 0 &&
                 (rcompcode != COMPCODE_EQ) && (rcompcode != COMPCODE_ORD);
    bool trap = (compcode & COMPCODE_UNORD) == 0 && (compcode != COMPCODE_EQ) &&
  ...
  if (compcode == COMPCODE_TRUE)
    ...
    return fold_build2(compcode_to_comparison(compcode), truth_type, ll_arg,
                       lr_arg);
}
...
int operand_equal_p(const_tree arg0, const_tree arg1, unsigned int flags) {
  ...
  if (TREE_CODE(arg0) != TREE_CODE(arg1) && COMPARISON_CLASS_P(arg0) &&
      COMPARISON_CLASS_P(arg1)) {
    enum tree_code swap_code = swap_tree_comparison(TREE_CODE(arg1));

    if (TREE_CODE(arg0) == swap_code)
      return operand_equal_p(TREE_OPERAND(arg0, 0), TREE_OPERAND(arg1, 1),
                             flags) &&
             operand_equal_p(TREE_OPERAND(arg0, 1), TREE_OPERAND(arg1, 0),
                             flags);
  }
  ...
  if (TREE_CONSTANT(arg0) && TREE_CONSTANT(arg1))
    switch (TREE_CODE(arg0)) {
      ...
      case REAL_CST:
        if (REAL_VALUES_IDENTICAL(TREE_REAL_CST(arg0), TREE_REAL_CST(arg1)))
          return 1;
      ...
      case VECTOR_CST: {
        ...
        while (v1 && v2) {
          if (!operand_equal_p(TREE_VALUE(v1), TREE_VALUE(v2), flags))
            return 0;
          v1 = TREE_CHAIN(v1);
          v2 = TREE_CHAIN(v2);
        }
      ...
      case COMPLEX_CST:
        return (
            operand_equal_p(TREE_REALPART(arg0), TREE_REALPART(arg1), flags) &&
            operand_equal_p(TREE_IMAGPART(arg0), TREE_IMAGPART(arg1), flags));

      case STRING_CST:
        return (TREE_STRING_LENGTH(arg0) == TREE_STRING_LENGTH(arg1) &&
                !memcmp(TREE_STRING_POINTER(arg0), TREE_STRING_POINTER(arg1),
...
#define OP_SAME_WITH_NULL(N)                            \
  ...
  switch (TREE_CODE_CLASS(TREE_CODE(arg0))) {
    ...
    case tcc_binary:
      ...
      return (
          commutative_tree_code(TREE_CODE(arg0)) &&
          operand_equal_p(TREE_OPERAND(arg0, 0), TREE_OPERAND(arg1, 1),
                          flags) &&
          operand_equal_p(TREE_OPERAND(arg0, 1), TREE_OPERAND(arg1, 0), flags));
        ...
        case ARRAY_RANGE_REF:
          ...
          return (OP_SAME(0) &&
                  (tree_int_cst_equal(TREE_OPERAND(arg0, 1),
                                      TREE_OPERAND(arg1, 1)) ||
                   OP_SAME(1)) &&
                  OP_SAME_WITH_NULL(2) && OP_SAME_WITH_NULL(3));

        case COMPONENT_REF:
          /* Handle operand 2 the same as for ARRAY_REF.  Operand 0
             may be NULL when we're called to compare MEM_EXPRs.  */
          return OP_SAME_WITH_NULL(0) && OP_SAME(1) && OP_SAME_WITH_NULL(2);
    ...
    case tcc_vl_exp:
      switch (TREE_CODE(arg0)) {
        case CALL_EXPR:
          ...
          if (!operand_equal_p(CALL_EXPR_FN(arg0), CALL_EXPR_FN(arg1), flags))
            return 0;
          ...
          /* Now see if all the arguments are the same.  */
          {
            ...
            for (a0 = first_const_call_expr_arg(arg0, &iter0),
                a1 = first_const_call_expr_arg(arg1, &iter1);
                 a0 && a1; a0 = next_const_call_expr_arg(&iter0),
    ...
    case tcc_declaration:
      ...
      return (TREE_CODE(arg0) == FUNCTION_DECL && DECL_BUILT_IN(arg0) &&
              DECL_BUILT_IN(arg1) &&
...
static int operand_equal_for_comparison_p(tree arg0, tree arg1, tree other) {
  ...
  if (unsignedp1 == unsignedpo &&
    ...
    primarg1 = fold_convert(
        signed_or_unsigned_type_for(unsignedp1, TREE_TYPE(primarg1)), primarg1);
...
static int twoval_comparison_p(tree arg, tree *cval1, tree *cval2,
                               int *save_p) {
  ...
  switch (tclass) {
    ...
    case tcc_expression:
      if (code == COND_EXPR)
        return (
            twoval_comparison_p(TREE_OPERAND(arg, 0), cval1, cval2, save_p) &&
            twoval_comparison_p(TREE_OPERAND(arg, 1), cval1, cval2, save_p) &&
...
static tree eval_subst(tree arg, tree old0, tree new0, tree old1, tree new1) {
  ...
  switch (tclass) {
    case tcc_unary:
      return fold_build1(
          code, type, eval_subst(TREE_OPERAND(arg, 0), old0, new0, old1, new1));
    ...
    case tcc_expression:
      switch (code) {
        ...
        case COND_EXPR:
          return fold_build3(
              code, type,
              eval_subst(TREE_OPERAND(arg, 0), old0, new0, old1, new1),
              eval_subst(TREE_OPERAND(arg, 1), old0, new0, old1, new1),
...
tree fold_truth_not_expr(tree arg) {
  ...
  if (TREE_CODE_CLASS(code) == tcc_comparison) {
    ...
    if (FLOAT_TYPE_P(op_type) && flag_trapping_math && code != ORDERED_EXPR &&
        code != UNORDERED_EXPR && code != NE_EXPR && code != EQ_EXPR)
      return NULL_TREE;
    else {
      ...
      if (code == ERROR_MARK)
        ...
        return build2(code, type, TREE_OPERAND(arg, 0), TREE_OPERAND(arg, 1));
    }
  }
  ...
  switch (code) {
    ...
    case COND_EXPR: {
      ...
      /* A COND_EXPR may have a throw as one operand, which
         ...
         as they are.  */
      return build3(
          COND_EXPR, type, TREE_OPERAND(arg, 0),
          VOID_TYPE_P(TREE_TYPE(arg1)) ? arg1 : invert_truthvalue(arg1),
          VOID_TYPE_P(TREE_TYPE(arg2)) ? arg2 : invert_truthvalue(arg2));
    }
    ...
    case BIT_AND_EXPR:
      if (!integer_onep(TREE_OPERAND(arg, 1)))
        break;
      return build2(EQ_EXPR, type, arg, build_int_cst(type, 0));
...
static tree distribute_bit_expr(enum tree_code code, tree type, tree arg0,
                                tree arg1) {
  tree common;
  tree left, right;
...
static tree distribute_real_division(enum tree_code code, tree type, tree arg0,
                                     tree arg1) {
  ...
  if (mul0 == mul1 &&
    ...
    return fold_build2(
        mul0 ? MULT_EXPR : RDIV_EXPR, type,
        fold_build2(code, type, TREE_OPERAND(arg0, 0), TREE_OPERAND(arg1, 0)),
        TREE_OPERAND(arg0, 1));
  ...
  if (operand_equal_p(TREE_OPERAND(arg0, 0), TREE_OPERAND(arg1, 0), 0) &&
    ...
    return fold_build2(MULT_EXPR, type, TREE_OPERAND(arg0, 0),
                       build_real(type, r0));
  }
...
static tree make_bit_field_ref(tree inner, tree type, HOST_WIDE_INT bitsize,
  ...
  bftype = type;
  if (TYPE_PRECISION(bftype) != bitsize || TYPE_UNSIGNED(bftype) == !unsignedp)
    bftype = build_nonstandard_integer_type(bitsize, 0);

  result = build3(BIT_FIELD_REF, bftype, inner, size_int(bitsize),
                  bitsize_int(bitpos));
...
static tree optimize_bit_field_compare(enum tree_code code, tree compare_type,
                                       tree lhs, tree rhs) {
  ...
  mask = const_binop(RSHIFT_EXPR, mask, size_int(nbitsize - lbitsize - lbitpos),
                     0);
  ...
  rhs = const_binop(BIT_AND_EXPR,
                    const_binop(LSHIFT_EXPR, fold_convert(unsigned_type, rhs),
                                size_int(lbitpos), 0),
                    mask, 0);
...
static tree decode_field_reference(tree exp, HOST_WIDE_INT *pbitsize,
                                   HOST_WIDE_INT *pbitpos,
                                   enum machine_mode *pmode, int *punsignedp,
                                   int *pvolatilep, tree *pmask,
                                   tree *pand_mask) {
  tree outer_type = 0;
  tree and_mask = 0;
...
static int all_ones_mask_p(const_tree mask, int size) {
  ...
  return tree_int_cst_equal(
      mask, const_binop(
                RSHIFT_EXPR,
                const_binop(LSHIFT_EXPR, tmask, size_int(precision - size), 0),
...
static tree sign_bit_p(tree exp, const_tree val) {
  ...
  if (TREE_CODE(val) != INTEGER_CST || TREE_OVERFLOW(val))
    return NULL_TREE;
...
static int simple_operand_p(const_tree exp) {
  /* Strip any conversions that don't change the machine mode.  */
  STRIP_NOPS(exp);

  return (CONSTANT_CLASS_P(exp) || TREE_CODE(exp) == SSA_NAME ||
          (DECL_P(exp) && !TREE_ADDRESSABLE(exp) && !TREE_THIS_VOLATILE(exp) &&
           !DECL_NONLOCAL(exp)
...
static tree range_binop(enum tree_code code, tree type, tree arg0, int upper0_p,
  ...
  if (arg0 != 0 && arg1 != 0) {
    tem = fold_build2(code, type != 0 ? type : TREE_TYPE(arg0), arg0,
...
static tree make_range(tree exp, int *pin_p, tree *plow, tree *phigh,
  ...
  while (1) {
    ...
    switch (code) {
      ...
      case GT_EXPR:
        ...
        if (low == 0 || high == 0 || !integer_zerop(low) ||
            !integer_zerop(high) || TREE_CODE(arg1) != INTEGER_CST)
          break;
        ...
        if (TYPE_UNSIGNED(arg0_type) && (low == 0 || high == 0)) {
          if (!merge_ranges(&n_in_p, &n_low, &n_high, in_p, low, high, 1,
                            build_int_cst(arg0_type, 0), NULL_TREE))
            break;
          ...
          if (high == 0 && low && !integer_zerop(low)) {
            in_p = !in_p;
            high =
                range_binop(MINUS_EXPR, NULL_TREE, low, 0, integer_one_node, 0);
      ...
      case NEGATE_EXPR:
        /* (-x) IN [a,b] -> x in [-b, -a]  */
        n_low = range_binop(MINUS_EXPR, exp_type, build_int_cst(exp_type, 0), 0,
                            high, 1);
        n_high = range_binop(MINUS_EXPR, exp_type, build_int_cst(exp_type, 0),
                             0, low, 0);
      ...
      case MINUS_EXPR:
        ...
        if (!TYPE_UNSIGNED(arg0_type) && !TYPE_OVERFLOW_UNDEFINED(arg0_type))
          break;
        ...
        if (n_low && n_high && tree_int_cst_lt(n_high, n_low)) {
          low =
              range_binop(PLUS_EXPR, arg0_type, n_high, 0, integer_one_node, 0);
          high =
              range_binop(MINUS_EXPR, arg0_type, n_low, 0, integer_one_node, 0);
        ...
        if (!TYPE_UNSIGNED(exp_type) && TYPE_UNSIGNED(arg0_type)) {
          ...
          if (ALL_FIXED_POINT_MODE_P(TYPE_MODE(arg0_type)))
            equiv_type = lang_hooks.types.type_for_mode(
                TYPE_MODE(arg0_type), TYPE_SATURATING(arg0_type));
          else
            equiv_type =
          ...
          if (TYPE_PRECISION(exp_type) == TYPE_PRECISION(arg0_type))
            high_positive = fold_build2(RSHIFT_EXPR, arg0_type,
                                        fold_convert(arg0_type, high_positive),
  ...
  if (TREE_CODE(exp) == INTEGER_CST) {
    in_p = in_p == (integer_onep(range_binop(GE_EXPR, integer_type_node, exp, 0,
                                             low, 0)) &&
                    integer_onep(range_binop(LE_EXPR, integer_type_node, exp, 1,
                                             high, 1)));
    low = high = 0;
    exp = 0;
  }
...
static tree build_range_check(tree type, tree exp, int in_p, tree low,
                              tree high) {
  ...
  if (low == 0)
    return fold_build2(LE_EXPR, type, exp, fold_convert(etype, high));

  if (high == 0)
    return fold_build2(GE_EXPR, type, exp, fold_convert(etype, low));

  if (operand_equal_p(low, high, 0))
    return fold_build2(EQ_EXPR, type, exp, fold_convert(etype, low));
    ...
    if (TREE_INT_CST_HIGH(high) == hi && TREE_INT_CST_LOW(high) == lo) {
      if (TYPE_UNSIGNED(etype)) {
        ...
        if (TYPE_PRECISION(signed_etype) != TYPE_PRECISION(etype))
          etype = build_nonstandard_integer_type(TYPE_PRECISION(etype), 0);
        else
          etype = signed_etype;
        exp = fold_convert(etype, exp);
      }
      return fold_build2(GT_EXPR, type, exp, build_int_cst(etype, 0));
    }
  }
  ...
  if (TREE_CODE(etype) == INTEGER_TYPE && !TYPE_OVERFLOW_WRAPS(etype)) {
    ...
    maxv = range_binop(PLUS_EXPR, NULL_TREE, maxv, 1, integer_one_node, 1);
    minv = fold_convert(utype, TYPE_MIN_VALUE(etype));

    if (integer_zerop(
            range_binop(NE_EXPR, integer_type_node, minv, 1, maxv, 1)))
      etype = utype;
    else
      return 0;
  }
  ...
  if (value != 0 && !TREE_OVERFLOW(value))
    return build_range_check(type, fold_build2(MINUS_EXPR, etype, exp, low), 1,
                             build_int_cst(etype, 0), value);
...
static tree range_predecessor(tree val) {
  ...
  if (INTEGRAL_TYPE_P(type) && operand_equal_p(val, TYPE_MIN_VALUE(type), 0))
    return 0;
  else
...
static tree range_successor(tree val) {
  ...
  if (INTEGRAL_TYPE_P(type) && operand_equal_p(val, TYPE_MAX_VALUE(type), 0))
    return 0;
  else
...
static int merge_ranges(int *pin_p, tree *plow, tree *phigh, int in0_p,
                        tree low0, tree high0, int in1_p, tree low1,
                        tree high1) {
  int no_overlap;
  int subset;
  int temp;
  ...
  int highequal = ((high0 == 0 && high1 == 0) ||
                   integer_onep(range_binop(EQ_EXPR, integer_type_node, high0,
                                            1, high1, 1)));
  ...
  if (integer_onep(range_binop(GT_EXPR, integer_type_node, low0, 0, low1, 0)) ||
      (lowequal && integer_onep(range_binop(GT_EXPR, integer_type_node, high1,
                                            1, high0, 1)))) {
  ...
  no_overlap =
      integer_onep(range_binop(LT_EXPR, integer_type_node, high0, 1, low1, 0));
  subset =
      integer_onep(range_binop(LE_EXPR, integer_type_node, high1, 1, high0, 1));
  ...
  if (in0_p && in1_p) {
    ...
    if (no_overlap) {
      if (integer_onep(range_binop(EQ_EXPR, integer_type_node,
                                   range_successor(high0), 1, low1, 0)))
        ...
        if (low0 && TREE_CODE(low0) == INTEGER_CST)
          switch (TREE_CODE(TREE_TYPE(low0))) {
            case ENUMERAL_TYPE:
              ...
              /* FALLTHROUGH */
            case INTEGER_TYPE:
              if (tree_int_cst_equal(low0, TYPE_MIN_VALUE(TREE_TYPE(low0))))
                low0 = 0;
              break;
            case POINTER_TYPE:
              if (TYPE_UNSIGNED(TREE_TYPE(low0)) && integer_zerop(low0))
                low0 = 0;
              break;
            default:
              break;
        ...
        if (high1 && TREE_CODE(high1) == INTEGER_CST)
          switch (TREE_CODE(TREE_TYPE(high1))) {
            case ENUMERAL_TYPE:
              ...
              /* FALLTHROUGH */
            case INTEGER_TYPE:
              if (tree_int_cst_equal(high1, TYPE_MAX_VALUE(TREE_TYPE(high1))))
                high1 = 0;
              break;
            case POINTER_TYPE:
              if (TYPE_UNSIGNED(TREE_TYPE(high1)) &&
                  integer_zerop(range_binop(PLUS_EXPR, NULL_TREE, high1, 1,
                                            integer_one_node, 1)))
                high1 = 0;
              break;
            default:
...
static tree fold_cond_expr_with_comparison(tree type, tree arg0, tree arg1,
                                           tree arg2) {
  ...
  if (!HONOR_SIGNED_ZEROS(TYPE_MODE(type)) &&
      (FLOAT_TYPE_P(TREE_TYPE(arg01)) ? real_zerop(arg01)
                                      : integer_zerop(arg01)) &&
      ((TREE_CODE(arg2) == NEGATE_EXPR &&
        operand_equal_p(TREE_OPERAND(arg2, 0), arg1, 0))
       /* In the case that A is of the form X-Y, '-A' (arg2) may
          have already been folded to Y-X, check for that. */
       || (TREE_CODE(arg1) == MINUS_EXPR && TREE_CODE(arg2) == MINUS_EXPR &&
           operand_equal_p(TREE_OPERAND(arg1, 0), TREE_OPERAND(arg2, 1), 0) &&
           operand_equal_p(TREE_OPERAND(arg1, 1), TREE_OPERAND(arg2, 0), 0))))
    switch (comp_code) {
      case EQ_EXPR:
      case UNEQ_EXPR:
      ...
      case GT_EXPR:
        if (TYPE_UNSIGNED(TREE_TYPE(arg1)))
          arg1 = fold_convert(signed_type_for(TREE_TYPE(arg1)), arg1);
  ...
  if (!HONOR_SIGNED_ZEROS(TYPE_MODE(type)) && integer_zerop(arg01) &&
      integer_zerop(arg2)) {
  ...
  if (INTEGRAL_TYPE_P(type) && TREE_CODE(arg01) == INTEGER_CST &&
    ...
    switch (comp_code) {
      ...
      case LT_EXPR:
        ...
        if (!operand_equal_p(arg2, TYPE_MAX_VALUE(type), OEP_ONLY_CONST) &&
            operand_equal_p(
                arg01, const_binop(PLUS_EXPR, arg2, build_int_cst(type, 1), 0),
                OEP_ONLY_CONST))
          return pedantic_non_lvalue(
              fold_build2(MIN_EXPR, type, fold_convert(type, arg1), arg2));
        break;

      case LE_EXPR:
        /* If C1 is C2 - 1, this is min(A, C2).  */
        if (!operand_equal_p(arg2, TYPE_MIN_VALUE(type), OEP_ONLY_CONST) &&
            operand_equal_p(
                arg01, const_binop(MINUS_EXPR, arg2, build_int_cst(type, 1), 0),
                OEP_ONLY_CONST))
          return pedantic_non_lvalue(
              fold_build2(MIN_EXPR, type, fold_convert(type, arg1), arg2));
        break;
      ...
      case GT_EXPR:
        ...
        if (!operand_equal_p(arg2, TYPE_MIN_VALUE(type), OEP_ONLY_CONST) &&
            operand_equal_p(
                ...
                OEP_ONLY_CONST))
          return pedantic_non_lvalue(
              fold_build2(MAX_EXPR, type, fold_convert(type, arg1), arg2));
        break;
...
static tree fold_range_test(enum tree_code code, tree type, tree op0,
                            tree op1) {
  ...
  if ((lhs == 0 || rhs == 0 || operand_equal_p(lhs, rhs, 0)) &&
      merge_ranges(&in_p, &low, &high, in0_p, low0, high0, in1_p, low1,
                   high1) &&
  ...
  /* On machines where the branch cost is expensive, if this is a
  ...
  else if (LOGICAL_OP_NON_SHORT_CIRCUIT && lhs != 0 && rhs != 0 &&
           (code == TRUTH_ANDIF_EXPR || code == TRUTH_ORIF_EXPR) &&
    ...
    if (simple_operand_p(lhs))
      return build2(code == TRUTH_ANDIF_EXPR ? TRUTH_AND_EXPR : TRUTH_OR_EXPR,
                    type, op0, op1);
      ...
      if (0 != (lhs = build_range_check(type, common, or_op ? !in0_p : in0_p,
                                        low0, high0)) &&
          (0 != (rhs = build_range_check(type, common, or_op ? !in1_p : in1_p,
                                         low1, high1)))) {
        if (strict_overflow_p)
          fold_overflow_warning(warnmsg, WARN_STRICT_OVERFLOW_COMPARISON);
        return build2(code == TRUTH_ANDIF_EXPR ? TRUTH_AND_EXPR : TRUTH_OR_EXPR,
...
static tree unextend(tree c, int p, int unsignedp, tree mask) {
  ...
  if (mask != 0)
    temp = const_binop(BIT_AND_EXPR, temp, fold_convert(TREE_TYPE(c), mask), 0);
...
static tree fold_truthop(enum tree_code code, tree truth_type, tree lhs,
                         tree rhs) {
  ...
  if (lcode == BIT_AND_EXPR && integer_onep(TREE_OPERAND(lhs, 1))) {
    lhs = build2(NE_EXPR, truth_type, lhs, build_int_cst(TREE_TYPE(lhs), 0));
    lcode = NE_EXPR;
  }

  if (rcode == BIT_AND_EXPR && integer_onep(TREE_OPERAND(rhs, 1))) {
    rhs = build2(NE_EXPR, truth_type, rhs, build_int_cst(TREE_TYPE(rhs), 0));
    rcode = NE_EXPR;
  }
  ...
  if (simple_operand_p(ll_arg) && simple_operand_p(lr_arg)) {
    ...
    if (operand_equal_p(ll_arg, rl_arg, 0) &&
      ...
      result =
          combine_comparisons(code, lcode, rcode, truth_type, ll_arg, lr_arg);
      if (result)
        return result;
    } else if (operand_equal_p(ll_arg, rr_arg, 0) &&
               operand_equal_p(lr_arg, rl_arg, 0)) {
      result = combine_comparisons(code, lcode, swap_tree_comparison(rcode),
  ...
  code = ((code == TRUTH_AND_EXPR || code == TRUTH_ANDIF_EXPR) ? TRUTH_AND_EXPR
                                                               : TRUTH_OR_EXPR);
  ...
  if (BRANCH_COST(optimize_function_for_speed_p(cfun), false) >= 2 &&
      !FLOAT_TYPE_P(TREE_TYPE(rl_arg)) && simple_operand_p(rl_arg) &&
      simple_operand_p(rr_arg)) {
    /* Convert (a != 0) || (b != 0) into (a | b) != 0.  */
    if (code == TRUTH_OR_EXPR && lcode == NE_EXPR && integer_zerop(lr_arg) &&
    ...
    /* Convert (a == 0) && (b == 0) into (a | b) == 0.  */
    if (code == TRUTH_AND_EXPR && lcode == EQ_EXPR && integer_zerop(lr_arg) &&
  ...
  volatilep = 0;
  ll_inner =
      decode_field_reference(ll_arg, &ll_bitsize, &ll_bitpos, &ll_mode,
                             &ll_unsignedp, &volatilep, &ll_mask, &ll_and_mask);
  lr_inner =
      decode_field_reference(lr_arg, &lr_bitsize, &lr_bitpos, &lr_mode,
                             &lr_unsignedp, &volatilep, &lr_mask, &lr_and_mask);
  rl_inner =
      decode_field_reference(rl_arg, &rl_bitsize, &rl_bitpos, &rl_mode,
                             &rl_unsignedp, &volatilep, &rl_mask, &rl_and_mask);
  rr_inner =
      decode_field_reference(rr_arg, &rr_bitsize, &rr_bitpos, &rr_mode,
                             &rr_unsignedp, &volatilep, &rr_mask, &rr_and_mask);
  ...
  lnmode = get_best_mode(end_bit - first_bit, first_bit,
                         TYPE_ALIGN(TREE_TYPE(ll_inner)), word_mode, volatilep);
  if (lnmode == VOIDmode)
    return 0;
  ...
  if (l_const) {
    ...
    if (!integer_zerop(const_binop(BIT_AND_EXPR, l_const,
                                   fold_build1(BIT_NOT_EXPR, lntype, ll_mask),
                                   0))) {
  ...
  if (r_const) {
    ...
    if (!integer_zerop(const_binop(BIT_AND_EXPR, r_const,
                                   fold_build1(BIT_NOT_EXPR, lntype, rl_mask),
                                   0))) {
      warning(0, "comparison is always %d", wanted_code == NE_EXPR);
  ...
  if (l_const == 0) {
    ...
    rnmode =
        get_best_mode(end_bit - first_bit, first_bit,
                      TYPE_ALIGN(TREE_TYPE(lr_inner)), word_mode, volatilep);
    if (rnmode == VOIDmode)
      return 0;
...
static tree optimize_minmax_comparison(enum tree_code code, tree type, tree op0,
                                       tree op1) {
  tree arg0 = op0;
  ...
  if ((op_code != MIN_EXPR && op_code != MAX_EXPR) ||
      TREE_CODE(comp_const) != INTEGER_CST || TREE_OVERFLOW(comp_const) ||
      TREE_CODE(minmax_const) != INTEGER_CST || TREE_OVERFLOW(minmax_const))
    return NULL_TREE;
  ...
  switch (code) {
    ...
    case GE_EXPR:
      return fold_build2(
          ...
          optimize_minmax_comparison(EQ_EXPR, type, arg0, comp_const),
          optimize_minmax_comparison(GT_EXPR, type, arg0, comp_const));
...
static tree extract_muldiv(tree t, tree c, enum tree_code code, tree wide_type,
                           bool *strict_overflow_p) {
...
static tree extract_muldiv_1(tree t, tree c, enum tree_code code,
                             tree wide_type, bool *strict_overflow_p) {
  tree type = TREE_TYPE(t);
  ...
  switch (tcode) {
    case INTEGER_CST:
      ...
      if (code == MULT_EXPR ||
        ...
        return const_binop(code, fold_convert(ctype, t), fold_convert(ctype, c),
                           0);
      break;
    ...
    case NON_LVALUE_EXPR:
      ...
      if ((COMPARISON_CLASS_P(op0) || UNARY_CLASS_P(op0) ||
           BINARY_CLASS_P(op0) || VL_EXP_CLASS_P(op0) ||
          ...
          && ((TYPE_OVERFLOW_WRAPS(TREE_TYPE(op0)) &&
              ...
              || (code != MULT_EXPR &&
                  (TYPE_UNSIGNED(ctype) != TYPE_UNSIGNED(TREE_TYPE(op0))))
      ...
      if ((t2 = fold_convert(TREE_TYPE(op0), c)) != 0 &&
          TREE_CODE(t2) == INTEGER_CST && !TREE_OVERFLOW(t2) &&
    ...
    case RSHIFT_EXPR:
      ...
      if (TREE_CODE(op1) == INTEGER_CST &&
          ...
          0 != (t1 = fold_convert(
                    ctype, const_binop(LSHIFT_EXPR, size_one_node, op1, 0))) &&
          !TREE_OVERFLOW(t1))
        return extract_muldiv(
            build2(tcode == LSHIFT_EXPR ? MULT_EXPR : FLOOR_DIV_EXPR, ctype,
    ...
    case MINUS_EXPR:
      ...
      if (tree_int_cst_sgn(op1) < 0 || tree_int_cst_sgn(c) < 0) {
        if (code == CEIL_DIV_EXPR)
          ...
          code = CEIL_DIV_EXPR;
        else if (code != MULT_EXPR && code != CEIL_MOD_EXPR &&
                 code != FLOOR_MOD_EXPR)
          break;
      }
      ...
      if (code == MULT_EXPR)
        return fold_build2(tcode, ctype,
                           fold_build2(code, ctype, fold_convert(ctype, op0),
    ...
    case EXACT_DIV_EXPR:
      ...
      if (same_p && (t1 = extract_muldiv(op0, c, code, wide_type,
                                         strict_overflow_p)) != 0)
        return fold_build2(tcode, ctype, fold_convert(ctype, t1),
      ...
      if (tcode == code &&
          ...
          0 != (t1 = force_fit_type_double(
                    ctype, TREE_INT_CST_LOW(t1), TREE_INT_CST_HIGH(t1),
                    (TYPE_UNSIGNED(ctype) && tcode != MULT_EXPR) ? -1 : 1,
                    TREE_OVERFLOW(t1))) &&
          !TREE_OVERFLOW(t1))
      ...
      if ((TYPE_OVERFLOW_UNDEFINED(ctype) ||
          ...
          ((code == MULT_EXPR && tcode == EXACT_DIV_EXPR) ||
           (tcode == MULT_EXPR && code != TRUNC_MOD_EXPR &&
            code != CEIL_MOD_EXPR && code != FLOOR_MOD_EXPR &&
            code != ROUND_MOD_EXPR && code != MULT_EXPR))) {
        if (integer_zerop(const_binop(TRUNC_MOD_EXPR, op1, c, 0))) {
          if (TYPE_OVERFLOW_UNDEFINED(ctype))
            *strict_overflow_p = true;
          return fold_build2(
              tcode, ctype, fold_convert(ctype, op0),
              fold_convert(ctype, const_binop(TRUNC_DIV_EXPR, op1, c, 0)));
        } else if (integer_zerop(const_binop(TRUNC_MOD_EXPR, c, op1, 0))) {
          if (TYPE_OVERFLOW_UNDEFINED(ctype))
            *strict_overflow_p = true;
          return fold_build2(
              code, ctype, fold_convert(ctype, op0),
              fold_convert(ctype, const_binop(TRUNC_DIV_EXPR, c, op1, 0)));
        }
      }
      break;
...
static tree fold_binary_op_with_conditional_arg(enum tree_code code, tree type,
                                                tree op0, tree op1, tree cond,
...
bool fold_real_zero_addition_p(const_tree type, const_tree addend, int negate) {
...
static tree fold_mathfn_compare(enum built_in_function fcode,
                                enum tree_code code, tree type, tree arg0,
                                tree arg1) {
  REAL_VALUE_TYPE c;
  ...
  if (BUILTIN_SQRT_P(fcode)) {
    ...
    if (REAL_VALUE_NEGATIVE(c)) {
      ...
      return fold_build2(code, type, arg, build_real(TREE_TYPE(arg), c2));
      ...
      if (REAL_VALUE_ISINF(c2)) {
        ...
        return fold_build2(
            ...
            fold_build2(GE_EXPR, type, arg,
                        build_real(TREE_TYPE(arg), dconst0)),
            fold_build2(NE_EXPR, type, arg, build_real(TREE_TYPE(arg), c2)));
      }
      ...
      if (lang_hooks.decls.global_bindings_p() == 0 &&
        ...
        return fold_build2(
            ...
            fold_build2(GE_EXPR, type, arg,
                        build_real(TREE_TYPE(arg), dconst0)),
            fold_build2(code, type, arg, build_real(TREE_TYPE(arg), c2)));
      }
    }
  }
...
static tree fold_inf_compare(enum tree_code code, tree type, tree arg0,
                             tree arg1) {
  enum machine_mode mode;
  REAL_VALUE_TYPE max;
  tree temp;
  ...
  switch (code) {
    ...
    case GE_EXPR:
      ...
      return fold_build2(neg ? LT_EXPR : GT_EXPR, type, arg0,
                         build_real(TREE_TYPE(arg0), max));

    case LT_EXPR:
      /* x < +Inf is always equal to x <= DBL_MAX.  */
      real_maxval(&max, neg, mode);
      return fold_build2(neg ? GE_EXPR : LE_EXPR, type, arg0,
                         build_real(TREE_TYPE(arg0), max));
    ...
    case NE_EXPR:
      ...
      temp = fold_build2(neg ? LT_EXPR : GT_EXPR, type, arg0,
                         build_real(TREE_TYPE(arg0), max));
...
static tree fold_div_compare(enum tree_code code, tree type, tree arg0,
                             tree arg1) {
  tree prod, tmp, hi, lo;
  tree arg00 = TREE_OPERAND(arg0, 0);
  tree arg01 = TREE_OPERAND(arg0, 1);
  ...
  overflow = mul_double_with_sign(
      TREE_INT_CST_LOW(arg01), TREE_INT_CST_HIGH(arg01), TREE_INT_CST_LOW(arg1),
      TREE_INT_CST_HIGH(arg1), &lpart, &hpart, unsigned_p);
  prod = force_fit_type_double(TREE_TYPE(arg00), lpart, hpart, -1, overflow);
  neg_overflow = false;

  if (unsigned_p) {
    tmp = int_const_binop(MINUS_EXPR, arg01, build_int_cst(TREE_TYPE(arg01), 1),
                          0);
    lo = prod;

    /* Likewise hi = int_const_binop (PLUS_EXPR, prod, tmp, 0).  */
    overflow = add_double_with_sign(
        TREE_INT_CST_LOW(prod), TREE_INT_CST_HIGH(prod), TREE_INT_CST_LOW(tmp),
        TREE_INT_CST_HIGH(tmp), &lpart, &hpart, unsigned_p);
    hi = force_fit_type_double(TREE_TYPE(arg00), lpart, hpart, -1,
                               overflow | TREE_OVERFLOW(prod));
    ...
    tmp = int_const_binop(PLUS_EXPR, arg01, build_int_cst(TREE_TYPE(arg01), 1),
                          0);
    switch (tree_int_cst_sgn(arg1)) {
      case -1:
...
static tree fold_single_bit_test_into_sign_test(enum tree_code code, tree arg0,
                                                tree arg1, tree result_type) {
  /* If this is testing a single bit, we can optimize the test.  */
  if ((code == NE_EXPR || code == EQ_EXPR) && TREE_CODE(arg0) == BIT_AND_EXPR &&
      integer_zerop(arg1) && integer_pow2p(TREE_OPERAND(arg0, 1))) {
    ...
    if (arg00 != NULL_TREE
      ...
      return fold_build2(code == EQ_EXPR ? GE_EXPR : LT_EXPR, result_type,
                         fold_convert(stype, arg00), build_int_cst(stype, 0));
    }
  }
...
tree fold_single_bit_test(enum tree_code code, tree arg0, tree arg1,
                          tree result_type) {
  /* If this is testing a single bit, we can optimize the test.  */
  if ((code == NE_EXPR || code == EQ_EXPR) && TREE_CODE(arg0) == BIT_AND_EXPR &&
      integer_zerop(arg1) && integer_pow2p(TREE_OPERAND(arg0, 1))) {
    ...
    /* First, see if we can fold the single bit test into a sign-bit
       test.  */
    tem = fold_single_bit_test_into_sign_test(code, arg0, arg1, result_type);
    if (tem)
      return tem;
    ...
    ops_unsigned =
        (LOAD_EXTEND_OP(operand_mode) == SIGN_EXTEND && !flag_syntax_only) ? 0
                                                                           : 1;
#else
    ops_unsigned = 1;
#endif
    ...
    if (bitnum != 0)
      inner = build2(RSHIFT_EXPR, intermediate_type, inner, size_int(bitnum));
...
static bool reorder_operands_p(const_tree arg0, const_tree arg1) {
...
bool tree_swap_operands_p(const_tree arg0, const_tree arg1, bool reorder) {
  STRIP_SIGN_NOPS(arg0);
...
static tree fold_widened_comparison(enum tree_code code, tree type, tree arg0,
                                    tree arg1) {
  ...
  if ((code == EQ_EXPR || code == NE_EXPR ||
      ...
      (TREE_TYPE(arg1_unw) == shorter_type ||
       ((TYPE_PRECISION(shorter_type) >= TYPE_PRECISION(TREE_TYPE(arg1_unw))) &&
        (TYPE_UNSIGNED(shorter_type) == TYPE_UNSIGNED(TREE_TYPE(arg1_unw)))) ||
  ...
  above = integer_nonzerop(fold_relational_const(LT_EXPR, type, max, arg1_unw));
  below = integer_nonzerop(fold_relational_const(LT_EXPR, type, arg1_unw, min));
...
static tree fold_sign_changed_comparison(enum tree_code code, tree type,
                                         tree arg0, tree arg1) {
  tree arg0_inner;
  ...
  if (TREE_CODE(arg1) != INTEGER_CST &&
      !(CONVERT_EXPR_P(arg1) && TREE_TYPE(TREE_OPERAND(arg1, 0)) == inner_type))
    return NULL_TREE;
  ...
  if (TREE_CODE(arg1) == INTEGER_CST)
    arg1 =
        force_fit_type_double(inner_type, TREE_INT_CST_LOW(arg1),
                              TREE_INT_CST_HIGH(arg1), 0, TREE_OVERFLOW(arg1));
  else
...
static tree try_move_mult_to_index(tree addr, tree op1) {
  tree s, delta, step;
    ...
    if (TREE_CODE(ref) == ARRAY_REF) {
      ...
      if (mdim) {
        ...
        if (TREE_CODE(TREE_OPERAND(ref, 1)) != INTEGER_CST ||
            !INTEGRAL_TYPE_P(itype) || !TYPE_MAX_VALUE(itype) ||
            TREE_CODE(TYPE_MAX_VALUE(itype)) != INTEGER_CST)
          continue;

        tmp = fold_binary(PLUS_EXPR, itype,
                          fold_convert(itype, TREE_OPERAND(ref, 1)),
  ...
  TREE_OPERAND(pos, 1) =
      fold_build2(PLUS_EXPR, itype, fold_convert(itype, TREE_OPERAND(pos, 1)),
                  fold_convert(itype, delta));
...
static tree fold_plusminus_mult_expr(enum tree_code code, tree type, tree arg0,
                                     tree arg1) {
  ...
  if (TREE_CODE(arg1) == MULT_EXPR) {
    ...
    if (TREE_INT_CST_HIGH(arg1) == -1 && negate_expr_p(arg1) &&
        code == PLUS_EXPR) {
  ...
  /* No identical multiplicands; see if we can find a common
    ...
    if (exact_log2(abs(int11)) > 0 && int01 % int11 == 0) {
      alt0 = fold_build2(MULT_EXPR, TREE_TYPE(arg00), arg00,
                         build_int_cst(TREE_TYPE(arg00), int01 / int11));
      alt1 = arg10;
      same = maybe_same;
      if (swap)
  ...
  if (same)
    return fold_build2(MULT_EXPR, type,
                       fold_build2(code, type, fold_convert(type, alt0),
...
static int native_encode_int(const_tree expr, unsigned char *ptr, int len) {
...
static int native_encode_real(const_tree expr, unsigned char *ptr, int len) {
  tree type = TREE_TYPE(expr);
  int total_bytes = GET_MODE_SIZE(TYPE_MODE(type));
...
static int native_encode_complex(const_tree expr, unsigned char *ptr, int len) {
  int rsize, isize;
  tree part;
...
static int native_encode_vector(const_tree expr, unsigned char *ptr, int len) {
...
static int native_encode_string(const_tree expr, unsigned char *ptr, int len) {
  tree type = TREE_TYPE(expr);
...
int native_encode_expr(const_tree expr, unsigned char *ptr, int len) {
...
static tree native_interpret_int(tree type, const unsigned char *ptr, int len) {
  ...
  for (byte = 0; byte < total_bytes; byte++) {
    ...
    if (bitpos < HOST_BITS_PER_WIDE_INT)
      ...
      hi |= (unsigned HOST_WIDE_INT)value << (bitpos - HOST_BITS_PER_WIDE_INT);
  }
...
static tree native_interpret_real(tree type, const unsigned char *ptr,
                                  int len) {
...
static tree native_interpret_complex(tree type, const unsigned char *ptr,
                                     int len) {
...
static tree native_interpret_vector(tree type, const unsigned char *ptr,
                                    int len) {
...
static tree build_fold_addr_expr_with_type_1(tree t, tree ptrtype,
                                             bool in_fold) {
  ...
  if (TREE_CODE(t) == INDIRECT_REF || TREE_CODE(t) == MISALIGNED_INDIRECT_REF) {
    t = TREE_OPERAND(t, 0);
...
tree build_fold_addr_expr(tree t) {
  return build_fold_addr_expr_with_type_1(t, build_pointer_type(TREE_TYPE(t)),
                                          false);
}
...
static tree fold_addr_expr(tree t) {
...
tree fold_unary(enum tree_code code, tree type, tree op0) {
  ...
  arg0 = op0;
  if (arg0) {
    if (CONVERT_EXPR_CODE_P(code) || code == FLOAT_EXPR || code == ABS_EXPR) {
  ...
  if (TREE_CODE_CLASS(code) == tcc_unary) {
    if (TREE_CODE(arg0) == COMPOUND_EXPR)
      return build2(
          COMPOUND_EXPR, type, TREE_OPERAND(arg0, 0),
          fold_build1(code, type,
                      fold_convert(TREE_TYPE(op0), TREE_OPERAND(arg0, 1))));
    else if (TREE_CODE(arg0) == COND_EXPR) {
      tree arg01 = TREE_OPERAND(arg0, 1);
      tree arg02 = TREE_OPERAND(arg0, 2);
      if (!VOID_TYPE_P(TREE_TYPE(arg01)))
        arg01 = fold_build1(code, type, fold_convert(TREE_TYPE(op0), arg01));
      if (!VOID_TYPE_P(TREE_TYPE(arg02)))
        arg02 = fold_build1(code, type, fold_convert(TREE_TYPE(op0), arg02));
      tem = fold_build3(COND_EXPR, type, TREE_OPERAND(arg0, 0), arg01, arg02);
      ...
      if ((CONVERT_EXPR_CODE_P(code) || code == NON_LVALUE_EXPR) &&
        ...
        tem = build1(
            code, type,
            build3(COND_EXPR, TREE_TYPE(TREE_OPERAND(TREE_OPERAND(tem, 1), 0)),
                   TREE_OPERAND(tem, 0), TREE_OPERAND(TREE_OPERAND(tem, 1), 0),
      ...
      if (TREE_CODE(type) == BOOLEAN_TYPE) {
        ...
        return fold_build3(COND_EXPR, type, arg0,
                           ...
                           fold_build1(code, type, integer_zero_node));
    }
  }
  ...
  switch (code) {
    case PAREN_EXPR:
      ...
      if (CONSTANT_CLASS_P(op0) || TREE_CODE(op0) == PAREN_EXPR)
    ...
    case FIX_TRUNC_EXPR:
      ...
      if (CONVERT_EXPR_P(op0)) {
        ...
        if (((inter_int && inside_int) || (inter_float && inside_float) ||
            ...
            (inter_float || inter_vec || inter_unsignedp == inside_unsignedp) &&
            !(final_prec != GET_MODE_BITSIZE(TYPE_MODE(type)) &&
              TYPE_MODE(type) == TYPE_MODE(inter_type)) &&
            !final_ptr && (!final_vec || inter_prec == inside_prec))
          return fold_build1(code, type, TREE_OPERAND(op0, 0));
        ...
        if (!inside_float && !inter_float && !final_float && !inside_vec &&
            !inter_vec && !final_vec &&
      ...
      if (TREE_CODE(type) == INTEGER_TYPE && TREE_CODE(op0) == BIT_AND_EXPR &&
        ...
        if (TYPE_UNSIGNED(TREE_TYPE(and)) ||
            (TYPE_PRECISION(type) <= TYPE_PRECISION(TREE_TYPE(and))))
          change = 1;
        else if (TYPE_PRECISION(TREE_TYPE(and1)) <= HOST_BITS_PER_WIDE_INT &&
                 host_integerp(and1, 1)) {
          unsigned HOST_WIDE_INT cst;

          cst = tree_low_cst(and1, 1);
          cst &= (HOST_WIDE_INT)-1 << (TYPE_PRECISION(TREE_TYPE(and1)) - 1);
          change = (cst == 0);
#ifdef LOAD_EXTEND_OP
          if (change && !flag_syntax_only &&
              (LOAD_EXTEND_OP(TYPE_MODE(TREE_TYPE(and0))) == ZERO_EXTEND)) {
        ...
        if (change) {
          tem = force_fit_type_double(type, TREE_INT_CST_LOW(and1),
                                      ...
                                      TREE_OVERFLOW(and1));
          return fold_build2(BIT_AND_EXPR, type, fold_convert(type, and0), tem);
        }
      }
      ...
      if (POINTER_TYPE_P(type) && TREE_CODE(arg0) == POINTER_PLUS_EXPR &&
      ...
      if (INTEGRAL_TYPE_P(type) && TREE_CODE(op0) == MULT_EXPR &&
          INTEGRAL_TYPE_P(TREE_TYPE(op0)) &&
          TYPE_PRECISION(type) < TYPE_PRECISION(TREE_TYPE(op0))) {
    ...
    case VIEW_CONVERT_EXPR:
      ...
      if ((INTEGRAL_TYPE_P(type) || POINTER_TYPE_P(type)) &&
          (INTEGRAL_TYPE_P(TREE_TYPE(op0)) || POINTER_TYPE_P(TREE_TYPE(op0))) &&
          TYPE_PRECISION(type) == TYPE_PRECISION(TREE_TYPE(op0))
          /* Do not muck with VIEW_CONVERT_EXPRs that convert from
             a sub-type to its base type as generated by the Ada FE.  */
          && !(INTEGRAL_TYPE_P(TREE_TYPE(op0)) && TREE_TYPE(TREE_TYPE(op0))))
        return fold_convert(type, op0);
      ...
      /* Convert fabs((double)float) into (double)fabsf(float).  */
        ...
        if (targ0 != arg0)
          return fold_convert(type,
                              fold_build1(ABS_EXPR, TREE_TYPE(targ0), targ0));
      }
    ...
    case REALPART_EXPR:
      ...
      if (TREE_CODE(arg0) == PLUS_EXPR || TREE_CODE(arg0) == MINUS_EXPR) {
        ...
        tem = fold_build2(
            TREE_CODE(arg0), itype,
            fold_build1(REALPART_EXPR, itype, TREE_OPERAND(arg0, 0)),
            fold_build1(REALPART_EXPR, itype, TREE_OPERAND(arg0, 1)));
        return fold_convert(type, tem);
      }
      if (TREE_CODE(arg0) == CONJ_EXPR) {
        tree itype = TREE_TYPE(TREE_TYPE(arg0));
    ...
    case IMAGPART_EXPR:
      ...
      if (TREE_CODE(arg0) == PLUS_EXPR || TREE_CODE(arg0) == MINUS_EXPR) {
        ...
        tem = fold_build2(
            TREE_CODE(arg0), itype,
            fold_build1(IMAGPART_EXPR, itype, TREE_OPERAND(arg0, 0)),
            fold_build1(IMAGPART_EXPR, itype, TREE_OPERAND(arg0, 1)));
        return fold_convert(type, tem);
      }
      if (TREE_CODE(arg0) == CONJ_EXPR) {
        tree itype = TREE_TYPE(TREE_TYPE(arg0));
...
tree fold_unary_ignore_overflow(enum tree_code code, tree type, tree op0) {
  tree res = fold_unary(code, type, op0);
  if (res && TREE_CODE(res) == INTEGER_CST && TREE_CODE(op0) == INTEGER_CST &&
...
static tree maybe_canonicalize_comparison_1(enum tree_code code, tree type,
                                            tree arg0, tree arg1,
                                            bool *strict_overflow_p) {
  ...
  t = int_const_binop(sgn0 == -1 ? PLUS_EXPR : MINUS_EXPR, cst0,
...
static tree maybe_canonicalize_comparison(enum tree_code code, tree type,
                                          tree arg0, tree arg1) {
  tree t;
  bool strict_overflow_p;
  const char *const warnmsg =
...
static bool pointer_may_wrap_p(tree base, tree offset, HOST_WIDE_INT bitpos) {
  ...
  if (add_double_with_sign(offset_low, offset_high, bitpos / BITS_PER_UNIT, 0,
                           &total_low, &total_high, true))
    return true;
...
static tree fold_comparison(enum tree_code code, tree type, tree op0,
                            tree op1) {
  tree arg0, arg1, tem;
  ...
  if ((TREE_CODE(arg0) == PLUS_EXPR || TREE_CODE(arg0) == MINUS_EXPR) &&
    ...
    lhs = fold_build2(lhs_add ? PLUS_EXPR : MINUS_EXPR, TREE_TYPE(arg1), const2,
                      const1);
    ...
    if (TREE_CODE(lhs) == INTEGER_CST && TREE_OVERFLOW(lhs)) {
      ...
      if (code2 == LT_EXPR || code2 == LE_EXPR || code2 == EQ_EXPR)
        return omit_one_operand(type, boolean_false_node, variable);
      else if (code2 == NE_EXPR || code2 == GE_EXPR || code2 == GT_EXPR)
        return omit_one_operand(type, boolean_true_node, variable);
    }

    if (TREE_CODE(lhs) == TREE_CODE(arg1) &&
        (TREE_CODE(lhs) != INTEGER_CST || !TREE_OVERFLOW(lhs))) {
  ...
  if (POINTER_TYPE_P(TREE_TYPE(arg0)) &&
      (TREE_CODE(arg0) == ADDR_EXPR || TREE_CODE(arg1) == ADDR_EXPR ||
    ...
    if (TREE_CODE(arg0) == ADDR_EXPR) {
      base0 =
          get_inner_reference(TREE_OPERAND(arg0, 0), &bitsize, &bitpos0,
                              &offset0, &mode, &unsignedp, &volatilep, false);
    ...
    if (TREE_CODE(arg1) == ADDR_EXPR) {
      base1 =
          get_inner_reference(TREE_OPERAND(arg1, 0), &bitsize, &bitpos1,
                              &offset1, &mode, &unsignedp, &volatilep, false);
    ...
    if (indirect_base0 == indirect_base1 && operand_equal_p(base0, base1, 0)) {
      /* We can fold this expression to a constant if the non-constant
         offset parts are equal.  */
      if ((offset0 == offset1 ||
           (offset0 && offset1 && operand_equal_p(offset0, offset1, 0))) &&
          (code == EQ_EXPR || code == NE_EXPR ||
           POINTER_TYPE_OVERFLOW_UNDEFINED))

      {
        if (code != EQ_EXPR && code != NE_EXPR && bitpos0 != bitpos1 &&
      ...
      else if (bitpos0 == bitpos1 && ((code == EQ_EXPR || code == NE_EXPR) ||
                                      POINTER_TYPE_OVERFLOW_UNDEFINED)) {
    ...
    /* For non-equal bases we can simplify if they are addresses
       of local binding decls or constants.  */
             ...
             && base0 != base1 && TREE_CODE(arg0) == ADDR_EXPR &&
             TREE_CODE(arg1) == ADDR_EXPR &&
             (((TREE_CODE(base0) == VAR_DECL ||
                TREE_CODE(base0) == PARM_DECL) &&
               (targetm.binds_local_p(base0) || CONSTANT_CLASS_P(base1))) ||
              CONSTANT_CLASS_P(base0)) &&
             (((TREE_CODE(base1) == VAR_DECL ||
                TREE_CODE(base1) == PARM_DECL) &&
               (targetm.binds_local_p(base1) || CONSTANT_CLASS_P(base0))) ||
    ...
    /* For equal offsets we can simplify to a comparison of the
       base addresses.  */
             ...
             (indirect_base0 ? base0 != TREE_OPERAND(arg0, 0)
                             : base0 != arg0) &&
             (indirect_base1 ? base1 != TREE_OPERAND(arg1, 0)
                             : base1 != arg1) &&
             ((offset0 == offset1) ||
              (offset0 && offset1 && operand_equal_p(offset0, offset1, 0)))) {
      if (indirect_base0)
  ...
  if (TYPE_OVERFLOW_UNDEFINED(TREE_TYPE(arg0)) &&
    ...
    cst = int_const_binop(
        TREE_CODE(arg0) == TREE_CODE(arg1) ? MINUS_EXPR : PLUS_EXPR, const2,
        const1, 0);
    if (!TREE_OVERFLOW(cst) &&
      ...
      return fold_build2(
          code, type, variable1,
          fold_build2(TREE_CODE(arg1), TREE_TYPE(arg1), variable2, cst));
    }

    cst = int_const_binop(
        TREE_CODE(arg0) == TREE_CODE(arg1) ? MINUS_EXPR : PLUS_EXPR, const1,
        const2, 0);
    if (!TREE_OVERFLOW(cst) &&
      ...
      return fold_build2(
          code, type,
          fold_build2(TREE_CODE(arg0), TREE_TYPE(arg0), variable1, cst),
          variable2);
    }
  }
  ...
  if (FLOAT_TYPE_P(TREE_TYPE(arg0))) {
    ...
    if (TREE_CODE(arg1) == REAL_CST) {
      ...
      if (TREE_CODE(arg0) == NEGATE_EXPR)
        return fold_build2(swap_tree_comparison(code), type,
                           TREE_OPERAND(arg0, 0),
                           build_real(TREE_TYPE(arg1), REAL_VALUE_NEGATE(cst)));
      ...
      if (REAL_VALUE_ISNAN(cst) && !HONOR_SNANS(TYPE_MODE(TREE_TYPE(arg1)))) {
    ...
    if (flag_unsafe_math_optimizations && TREE_CODE(arg1) == REAL_CST &&
        (TREE_CODE(arg0) == PLUS_EXPR || TREE_CODE(arg0) == MINUS_EXPR) &&
        TREE_CODE(TREE_OPERAND(arg0, 1)) == REAL_CST &&
        0 != (tem = const_binop(
                  TREE_CODE(arg0) == PLUS_EXPR ? MINUS_EXPR : PLUS_EXPR, arg1,
    ...
    if (flag_associative_math && TREE_CODE(arg1) == REAL_CST &&
        TREE_CODE(arg0) == MINUS_EXPR &&
        TREE_CODE(TREE_OPERAND(arg0, 0)) == REAL_CST &&
        0 != (tem = const_binop(MINUS_EXPR, TREE_OPERAND(arg0, 0), arg1, 0)) &&
        !TREE_OVERFLOW(tem))
      return fold_build2(swap_tree_comparison(code), type,
                         TREE_OPERAND(arg0, 1), tem);

    /* Fold comparisons against built-in math functions.  */
    if (TREE_CODE(arg1) == REAL_CST && flag_unsafe_math_optimizations &&
        !flag_errno_math) {
  ...
  if (TREE_CODE(TREE_TYPE(arg0)) == INTEGER_TYPE && CONVERT_EXPR_P(arg0)) {
  ...
  if (TREE_CODE(arg1) == INTEGER_CST && TREE_CODE(arg0) != INTEGER_CST) {
    ...
    if (twoval_comparison_p(arg0, &cval1, &cval2, &save_p)
        ...
        INTEGRAL_TYPE_P(TREE_TYPE(cval1)) && TYPE_MAX_VALUE(TREE_TYPE(cval1)) &&
      ...
      tree high_result = fold_build2(
          code, type, eval_subst(arg0, cval1, maxval, cval2, minval), arg1);
      tree equal_result = fold_build2(
          code, type, eval_subst(arg0, cval1, maxval, cval2, maxval), arg1);
      tree low_result = fold_build2(
          code, type, eval_subst(arg0, cval1, minval, cval2, maxval), arg1);
      ...
      if (TREE_CODE(high_result) == INTEGER_CST &&
        ...
        switch ((integer_onep(high_result) * 4) +
                (integer_onep(equal_result) * 2) + integer_onep(low_result)) {
          case 0:
            /* Always false.  */
            return omit_one_operand(type, integer_zero_node, arg0);
          case 1:
  ...
  if ((TREE_CODE(arg0) == TRUNC_DIV_EXPR ||
      ...
      !TREE_OVERFLOW(TREE_OPERAND(arg0, 1)) && !TREE_OVERFLOW(arg1)) {
  ...
  if (TREE_CODE(arg0) == BIT_NOT_EXPR && TREE_CODE(arg1) == BIT_NOT_EXPR) {
  ...
  if (TREE_CODE(arg0) == BIT_NOT_EXPR && TREE_CODE(arg1) == INTEGER_CST) {
    tree cmp_type = TREE_TYPE(TREE_OPERAND(arg0, 0));
    return fold_build2(
        swap_tree_comparison(code), type, TREE_OPERAND(arg0, 0),
        fold_build1(BIT_NOT_EXPR, cmp_type, fold_convert(cmp_type, arg1)));
  }
...
static tree fold_mult_zconjz(tree type, tree expr) {
  ...
  tem =
      fold_build2(PLUS_EXPR, itype, fold_build2(MULT_EXPR, itype, rpart, rpart),
  ...
  *residue = 0;
  ...
  if (code == ADDR_EXPR) {
    ...
    if (handled_component_p(expr)) {
      ...
      expr = get_inner_reference(expr, &bitsize, &bitpos, &offset, &mode,
                                 &unsignedp, &volatilep, false);
...
tree fold_binary(enum tree_code code, tree type, tree op0, tree op1) {
  ...
  if ((TREE_CODE(arg0) == INTEGER_CST && TREE_CODE(arg1) == INTEGER_CST) ||
    ...
    if (kind == tcc_binary) {
      ...
      gcc_assert(TYPE_SATURATING(type) == TYPE_SATURATING(TREE_TYPE(arg0)));
  ...
  if (commutative_tree_code(code) && tree_swap_operands_p(arg0, arg1, true))
  ...
  if ((code == BIT_AND_EXPR || code == BIT_IOR_EXPR || code == EQ_EXPR ||
       code == NE_EXPR) &&
    ...
    tem = fold_build2(code == BIT_AND_EXPR   ? TRUTH_AND_EXPR
                      : code == BIT_IOR_EXPR ? TRUTH_OR_EXPR
                                             : TRUTH_XOR_EXPR,
                      boolean_type_node, fold_convert(boolean_type_node, arg0),
  ...
  if (TREE_CODE_CLASS(code) == tcc_binary ||
    ...
    if (TREE_CODE(arg1) == COMPOUND_EXPR &&
      ...
      return build2(
          COMPOUND_EXPR, type, TREE_OPERAND(arg1, 0),
          fold_build2(code, type, op0,
                      fold_convert(TREE_TYPE(op1), TREE_OPERAND(arg1, 1))));

    if (TREE_CODE(arg0) == COND_EXPR || COMPARISON_CLASS_P(arg0)) {
      tem =
          fold_binary_op_with_conditional_arg(code, type, op0, op1, arg0, arg1,
                                              /*cond_first_p=*/1);
      if (tem != NULL_TREE)
        return tem;
    }
    ...
    if (TREE_CODE(arg1) == COND_EXPR || COMPARISON_CLASS_P(arg1)) {
      tem =
          fold_binary_op_with_conditional_arg(code, type, op0, op1, arg1, arg0,
                                              /*cond_first_p=*/0);
      if (tem != NULL_TREE)
        return tem;
    }
  ...
  switch (code) {
    case POINTER_PLUS_EXPR:
      ...
      if (INTEGRAL_TYPE_P(TREE_TYPE(arg1)) && INTEGRAL_TYPE_P(TREE_TYPE(arg0)))
        return fold_convert(
            type, fold_build2(PLUS_EXPR, sizetype, fold_convert(sizetype, arg1),
                              fold_convert(sizetype, arg0)));

      /* index +p PTR -> PTR +p index */
      if (POINTER_TYPE_P(TREE_TYPE(arg1)) && INTEGRAL_TYPE_P(TREE_TYPE(arg0)))
        return fold_build2(POINTER_PLUS_EXPR, type, fold_convert(type, arg1),
                           fold_convert(sizetype, arg0));
      ...
      if (TREE_CODE(arg0) == POINTER_PLUS_EXPR) {
        ...
        tree arg00 = TREE_OPERAND(arg0, 0);
        inner = fold_build2(PLUS_EXPR, sizetype, arg01,
    ...
    case PLUS_EXPR:
      ...
      if (TREE_CODE(arg0) == NEGATE_EXPR &&
        ...
        return fold_build2(MINUS_EXPR, type, fold_convert(type, arg1),
                           fold_convert(type, TREE_OPERAND(arg0, 0)));

      if (INTEGRAL_TYPE_P(type)) {
        /* Convert ~A + 1 to -A.  */
        if (TREE_CODE(arg0) == BIT_NOT_EXPR && integer_onep(arg1))
          return fold_build1(NEGATE_EXPR, type,
                             fold_convert(type, TREE_OPERAND(arg0, 0)));

        /* ~X + X is -1.  */
        if (TREE_CODE(arg0) == BIT_NOT_EXPR && !TYPE_OVERFLOW_TRAPS(type)) {
        ...
        /* X + ~X is -1.  */
        if (TREE_CODE(arg1) == BIT_NOT_EXPR && !TYPE_OVERFLOW_TRAPS(type)) {
        ...
        if (TREE_CODE(arg1) == MULT_EXPR &&
          ...
          if (sum && integer_zerop(sum))
            return fold_convert(
                type, fold_build2(TRUNC_MOD_EXPR, TREE_TYPE(arg0), arg0, cst0));
        }
      }
      ...
      if (!FLOAT_TYPE_P(type)) {
        ...
        if (TREE_CODE(arg0) == BIT_AND_EXPR &&
            ...
            TREE_CODE(TREE_OPERAND(arg0, 1)) == INTEGER_CST &&
            TREE_CODE(TREE_OPERAND(arg1, 1)) == INTEGER_CST &&
            integer_zerop(const_binop(BIT_AND_EXPR, TREE_OPERAND(arg0, 1),
        ...
        if (((TREE_CODE(arg0) == PLUS_EXPR || TREE_CODE(arg0) == MINUS_EXPR) &&
          ...
          if (TREE_CODE(parg0) == MULT_EXPR && TREE_CODE(parg1) != MULT_EXPR)
            return fold_build2(
                pcode, type,
                fold_build2(PLUS_EXPR, type, fold_convert(type, parg0),
                            fold_convert(type, marg)),
                fold_convert(type, parg1));
          if (TREE_CODE(parg0) != MULT_EXPR && TREE_CODE(parg1) == MULT_EXPR)
            return fold_build2(
                PLUS_EXPR, type, fold_convert(type, parg0),
                fold_build2(pcode, type, fold_convert(type, marg),
                            fold_convert(type, parg1)));
        }
      } else {
        ...
        if (operand_equal_p(arg0, arg1, 0) && SCALAR_FLOAT_TYPE_P(type))
          return fold_build2(MULT_EXPR, type, arg0, build_real(type, dconst2));

        /* Convert a + (b*c + d*e) into (a + b*c) + d*e.
           We associate floats only if the user has specified
           -fassociative-math.  */
        if (flag_associative_math && TREE_CODE(arg1) == PLUS_EXPR &&
            TREE_CODE(arg0) != MULT_EXPR) {
        ...
        /* Convert (b*c + d*e) + a into b*c + (d*e +a).
           We associate floats only if the user has specified
           -fassociative-math.  */
        if (flag_associative_math && TREE_CODE(arg0) == PLUS_EXPR &&
            TREE_CODE(arg1) != MULT_EXPR) {
        ...
        if (((code0 == RSHIFT_EXPR && code1 == LSHIFT_EXPR) ||
          ...
          if (code01 == INTEGER_CST && code11 == INTEGER_CST &&
            ...
            if (TREE_CODE(tree110) == INTEGER_CST &&
                0 == compare_tree_int(tree110, TYPE_PRECISION(TREE_TYPE(
                                                   TREE_OPERAND(arg0, 0)))) &&
                operand_equal_p(tree01, tree111, 0))
              return build2(
                  (code0 == LSHIFT_EXPR ? LROTATE_EXPR : RROTATE_EXPR), type,
            ...
            if (TREE_CODE(tree010) == INTEGER_CST &&
                0 == compare_tree_int(tree010, TYPE_PRECISION(TREE_TYPE(
                                                   TREE_OPERAND(arg0, 0)))) &&
    ...
    associate:
      ...
      if ((!FLOAT_TYPE_P(type) || flag_associative_math) &&
        ...
        if (ok && (2 < ((var0 != 0) + (var1 != 0) + (con0 != 0) + (con1 != 0) +
                        (lit0 != 0) + (lit1 != 0) + (minus_lit0 != 0) +
                        (minus_lit1 != 0)))) {
          ...
          if (minus_lit0) {
            if (con0 == 0)
              return fold_convert(
                  type, associate_trees(var0, minus_lit0, MINUS_EXPR, type));
            else {
              con0 = associate_trees(con0, minus_lit0, MINUS_EXPR, type);
              return fold_convert(type,
                                  associate_trees(var0, con0, PLUS_EXPR, type));
            }
          }

          con0 = associate_trees(con0, lit0, code, type);
          return fold_convert(type, associate_trees(var0, con0, code, type));
        }
      }
    ...
    case MINUS_EXPR:
      ...
      if (POINTER_TYPE_P(TREE_TYPE(arg1)) && POINTER_TYPE_P(TREE_TYPE(arg0))) {
        ...
        /* (PTR0 p+ A) - PTR1 -> (PTR0 - PTR1) + A, assuming PTR0 - PTR1
          ...
          tree arg01 = fold_convert(type, TREE_OPERAND(arg0, 1));
          tree tmp =
              fold_binary(MINUS_EXPR, type, arg00, fold_convert(type, arg1));
          if (tmp)
      ...
      if (TREE_CODE(arg0) == NEGATE_EXPR &&
          (FLOAT_TYPE_P(type) || INTEGRAL_TYPE_P(type)) &&
      ...
      if (INTEGRAL_TYPE_P(type) && TREE_CODE(arg0) == NEGATE_EXPR &&
          integer_onep(arg1) && !TYPE_OVERFLOW_TRAPS(type))
        return fold_build1(BIT_NOT_EXPR, type,
                           fold_convert(type, TREE_OPERAND(arg0, 0)));

      /* Convert -1 - A to ~A.  */
      if (INTEGRAL_TYPE_P(type) && integer_all_onesp(arg0))
        return fold_build1(BIT_NOT_EXPR, type, op1);

      /* X - (X / CST) * CST is X % CST.  */
      if (INTEGRAL_TYPE_P(type) && TREE_CODE(arg1) == MULT_EXPR &&
          TREE_CODE(TREE_OPERAND(arg1, 0)) == TRUNC_DIV_EXPR &&
          operand_equal_p(arg0, TREE_OPERAND(TREE_OPERAND(arg1, 0), 0), 0) &&
          operand_equal_p(TREE_OPERAND(TREE_OPERAND(arg1, 0), 1),
                          TREE_OPERAND(arg1, 1), 0))
        return fold_convert(type, fold_build2(TRUNC_MOD_EXPR, TREE_TYPE(arg0),
      ...
      if (!FLOAT_TYPE_P(type)) {
        ...
        if (TREE_CODE(arg0) == BIT_AND_EXPR &&
          ...
          if (operand_equal_p(tem, mask1, 0)) {
            tem = fold_build2(BIT_XOR_EXPR, type, TREE_OPERAND(arg0, 0), mask1);
      ...
      if (negate_expr_p(arg1) &&
        ...
        return fold_build2(PLUS_EXPR, type, fold_convert(type, arg0),
                           fold_convert(type, negate_expr(arg1)));
      ...
      if (TREE_CODE(arg0) == ADDR_EXPR &&
        ...
        if (operand_equal_p(TREE_OPERAND(aref0, 0), TREE_OPERAND(aref1, 0),
                            0)) {
          ...
          return fold_build2(MULT_EXPR, type, diff, fold_convert(type, esz));
        }
      }
    ...
    case MULT_EXPR:
      ...
      if (!FLOAT_TYPE_P(type)) {
        ...
        if (TREE_CODE(arg1) == INTEGER_CST && tree_int_cst_sgn(arg1) == -1 &&
            negate_expr_p(arg0) && (tem = negate_expr(arg1)) != arg1 &&
            !TREE_OVERFLOW(tem))
          return fold_build2(MULT_EXPR, type,
        ...
        if (TREE_CODE(arg1) == LSHIFT_EXPR &&
          ...
          return fold_build2(LSHIFT_EXPR, type, op0, TREE_OPERAND(arg1, 1));
        if (TREE_CODE(arg0) == LSHIFT_EXPR &&
            integer_onep(TREE_OPERAND(arg0, 0)))
          return fold_build2(LSHIFT_EXPR, type, op1, TREE_OPERAND(arg0, 1));

        /* (A + A) * C -> A * 2 * C  */
        if (TREE_CODE(arg0) == PLUS_EXPR && TREE_CODE(arg1) == INTEGER_CST &&
            operand_equal_p(TREE_OPERAND(arg0, 0), TREE_OPERAND(arg0, 1), 0))
          return fold_build2(
              MULT_EXPR, type,
              omit_one_operand(type, TREE_OPERAND(arg0, 0),
                               TREE_OPERAND(arg0, 1)),
              fold_build2(MULT_EXPR, type, build_int_cst(type, 2), arg1));
        ...
        if (!HONOR_NANS(TYPE_MODE(TREE_TYPE(arg0))) &&
            !HONOR_SIGNED_ZEROS(TYPE_MODE(TREE_TYPE(arg0))) && real_zerop(arg1))
          return omit_one_operand(type, arg1, arg0);
        ...
        if (!HONOR_SNANS(TYPE_MODE(TREE_TYPE(arg0))) && real_minus_onep(arg1))
        ...
        if (flag_associative_math && TREE_CODE(arg0) == RDIV_EXPR &&
            TREE_CODE(arg1) == REAL_CST &&
            TREE_CODE(TREE_OPERAND(arg0, 0)) == REAL_CST) {
          tree tem = const_binop(MULT_EXPR, TREE_OPERAND(arg0, 0), arg1, 0);
          if (tem)
            return fold_build2(RDIV_EXPR, type, tem, TREE_OPERAND(arg0, 1));
        }
        ...
        if (!HONOR_NANS(TYPE_MODE(TREE_TYPE(arg0))) &&
            !HONOR_SIGNED_ZEROS(TYPE_MODE(TREE_TYPE(arg0))) &&
            COMPLEX_FLOAT_TYPE_P(TREE_TYPE(arg0)) &&
            TREE_CODE(arg1) == COMPLEX_CST && real_zerop(TREE_REALPART(arg1))) {
          tree rtype = TREE_TYPE(TREE_TYPE(arg0));
          if (real_onep(TREE_IMAGPART(arg1)))
            return fold_build2(
                COMPLEX_EXPR, type,
                negate_expr(fold_build1(IMAGPART_EXPR, rtype, arg0)),
                fold_build1(REALPART_EXPR, rtype, arg0));
          else if (real_minus_onep(TREE_IMAGPART(arg1)))
            return fold_build2(
                COMPLEX_EXPR, type, fold_build1(IMAGPART_EXPR, rtype, arg0),
                negate_expr(fold_build1(REALPART_EXPR, rtype, arg0)));
        }
        ...
        if (flag_unsafe_math_optimizations) {
          ...
          if (fcode0 == fcode1 && BUILTIN_ROOT_P(fcode0)) {
            ...
            if (BUILTIN_SQRT_P(fcode0) && operand_equal_p(arg00, arg10, 0) &&
          ...
          if (fcode0 == fcode1 && BUILTIN_EXPONENT_P(fcode0)) {
            ...
            tree arg = fold_build2(PLUS_EXPR, type, CALL_EXPR_ARG(arg0, 0),
          ...
          if (((fcode0 == BUILT_IN_TAN && fcode1 == BUILT_IN_COS) ||
              ...
              operand_equal_p(CALL_EXPR_ARG(arg0, 0), CALL_EXPR_ARG(arg1, 0),
                              0)) {
          ...
          if (fcode1 == BUILT_IN_POW || fcode1 == BUILT_IN_POWF ||
              fcode1 == BUILT_IN_POWL) {
            tree arg10 = CALL_EXPR_ARG(arg1, 0);
            tree arg11 = CALL_EXPR_ARG(arg1, 1);
            if (TREE_CODE(arg11) == REAL_CST && !TREE_OVERFLOW(arg11) &&
          ...
          if (fcode0 == BUILT_IN_POW || fcode0 == BUILT_IN_POWF ||
              fcode0 == BUILT_IN_POWL) {
            tree arg00 = CALL_EXPR_ARG(arg0, 0);
            tree arg01 = CALL_EXPR_ARG(arg0, 1);
            if (TREE_CODE(arg01) == REAL_CST && !TREE_OVERFLOW(arg01) &&
    ...
    bit_ior:
      ...
      if (TREE_CODE(arg0) == BIT_AND_EXPR && TREE_CODE(arg1) == INTEGER_CST &&
        ...
        if ((~(hi1 | hi2) & mhi) == 0 && (~(lo1 | lo2) & mlo) == 0)
          return fold_build2(BIT_IOR_EXPR, type, TREE_OPERAND(arg0, 0), arg1);
        ...
        hi3 = hi1 & ~hi2;
        lo3 = lo1 & ~lo2;
        for (w = BITS_PER_UNIT; w <= width && w <= HOST_BITS_PER_WIDE_INT;
             w <<= 1) {
          unsigned HOST_WIDE_INT mask =
              (unsigned HOST_WIDE_INT) - 1 >> (HOST_BITS_PER_WIDE_INT - w);
          if (((lo1 | lo2) & mask) == mask && (lo1 & ~mask) == 0 && hi1 == 0) {
            hi3 = 0;
            lo3 = mask;
            break;
          }
        ...
        if (hi3 != hi1 || lo3 != lo1)
          return fold_build2(
              BIT_IOR_EXPR, type,
              fold_build2(BIT_AND_EXPR, type, TREE_OPERAND(arg0, 0),
                          build_int_cst_wide(type, lo3, hi3)),
              arg1);
      }
    ...
    case BIT_XOR_EXPR:
      ...
      if (TREE_CODE(arg0) == BIT_IOR_EXPR &&
        ...
        t1 = fold_build1(BIT_NOT_EXPR, TREE_TYPE(arg1), arg1);
      ...
      if (TREE_CODE(arg1) == BIT_IOR_EXPR &&
        ...
        t1 = fold_build1(BIT_NOT_EXPR, TREE_TYPE(arg0), arg0);
        t1 = fold_build2(BIT_AND_EXPR, type, fold_convert(type, t2),
                         fold_convert(type, t1));
        return t1;
      }
      ...
      /* Fold (X & 1) ^ 1 as (X & 1) == 0.  */
      if (TREE_CODE(arg0) == BIT_AND_EXPR &&
          integer_onep(TREE_OPERAND(arg0, 1)) && integer_onep(arg1))
      ...
      if (TREE_CODE(arg1) == BIT_AND_EXPR &&
        ...
        return fold_build2(BIT_AND_EXPR, type, fold_convert(type, arg0),
                           fold_build1(BIT_NOT_EXPR, type, tem));
      }
      /* Fold X ^ (Y & X) as ~Y & X.  */
      if (TREE_CODE(arg1) == BIT_AND_EXPR &&
    ...
    case BIT_AND_EXPR:
      ...
      if (TREE_CODE(arg0) == BIT_IOR_EXPR && TREE_CODE(arg1) == INTEGER_CST &&
          TREE_CODE(TREE_OPERAND(arg0, 1)) == INTEGER_CST) {
        ...
        return fold_convert(type, fold_build2(BIT_IOR_EXPR, type, tmp2, tmp3));
      }
      ...
      if (TREE_CODE(arg1) == INTEGER_CST && TREE_CODE(arg0) == NOP_EXPR &&
        ...
        unsigned int prec = TYPE_PRECISION(TREE_TYPE(TREE_OPERAND(arg0, 0)));

        if (prec < BITS_PER_WORD && prec < HOST_BITS_PER_WIDE_INT &&
            (~TREE_INT_CST_LOW(arg1) & (((HOST_WIDE_INT)1 << prec) - 1)) == 0)
          return fold_convert(type, TREE_OPERAND(arg0, 0));
      }
      ...
      if ((TREE_CODE(arg0) == LSHIFT_EXPR || TREE_CODE(arg0) == RSHIFT_EXPR) &&
        ...
        if (newmask != mask && (newmask & (newmask + 1)) == 0) {
          ...
          /* Only do the transformation if NEWMASK is some integer
             mode's mask.  */
          for (prec = BITS_PER_UNIT; prec < HOST_BITS_PER_WIDE_INT; prec <<= 1)
          ...
          if (prec < HOST_BITS_PER_WIDE_INT ||
            ...
            if (shift_type != TREE_TYPE(arg0)) {
              tem = fold_build2(TREE_CODE(arg0), shift_type,
                                fold_convert(shift_type, TREE_OPERAND(arg0, 0)),
                                TREE_OPERAND(arg0, 1));
              tem = fold_convert(type, tem);
            } else
              tem = op0;
            return fold_build2(BIT_AND_EXPR, type, tem,
                               build_int_cst_type(TREE_TYPE(op1), newmask));
          }
        }
      }
    ...
    case RDIV_EXPR:
      ...
      if (TREE_CODE(arg1) == REAL_CST &&
          !MODE_HAS_INFINITIES(TYPE_MODE(TREE_TYPE(arg1))) && real_zerop(arg1))
        return NULL_TREE;
      ...
      if (TREE_CODE(arg0) == NEGATE_EXPR && negate_expr_p(arg1))
        return fold_build2(RDIV_EXPR, type, TREE_OPERAND(arg0, 0),
                           negate_expr(arg1));
      if (TREE_CODE(arg1) == NEGATE_EXPR && negate_expr_p(arg0))
        return fold_build2(RDIV_EXPR, type, negate_expr(arg0),
                           TREE_OPERAND(arg1, 0));
      ...
      if (TREE_CODE(arg1) == REAL_CST) {
        ...
        if (optimize) {
          ...
          if (exact_real_inverse(TYPE_MODE(TREE_TYPE(arg0)), &r)) {
            ...
            return fold_build2(MULT_EXPR, type, fold_convert(type, arg0), tem);
          }
        }
      }
      /* Convert A/B/C to A/(B*C).  */
      if (flag_reciprocal_math && TREE_CODE(arg0) == RDIV_EXPR)
        return fold_build2(
            RDIV_EXPR, type, TREE_OPERAND(arg0, 0),
            fold_build2(MULT_EXPR, type, TREE_OPERAND(arg0, 1), arg1));

      /* Convert A/(B/C) to (A/B)*C.  */
      if (flag_reciprocal_math && TREE_CODE(arg1) == RDIV_EXPR)
        return fold_build2(
            MULT_EXPR, type,
            fold_build2(RDIV_EXPR, type, arg0, TREE_OPERAND(arg1, 0)),
            TREE_OPERAND(arg1, 1));

      /* Convert C1/(X*C2) into (C1/C2)/X.  */
      if (flag_reciprocal_math && TREE_CODE(arg1) == MULT_EXPR &&
          TREE_CODE(arg0) == REAL_CST &&
          TREE_CODE(TREE_OPERAND(arg1, 1)) == REAL_CST) {
        tree tem = const_binop(RDIV_EXPR, arg0, TREE_OPERAND(arg1, 1), 0);
        if (tem)
          return fold_build2(RDIV_EXPR, type, tem, TREE_OPERAND(arg1, 0));
      }
      ...
      if (flag_unsafe_math_optimizations) {
        ...
        if (((fcode0 == BUILT_IN_COS && fcode1 == BUILT_IN_SIN) ||
          ...
          if (tanfn != NULL_TREE) {
            ...
            return fold_build2(RDIV_EXPR, type, build_real(type, dconst1), tmp);
          }
        }
        ...
        if (((fcode0 == BUILT_IN_TAN && fcode1 == BUILT_IN_SIN) ||
          ...
          if (!HONOR_NANS(TYPE_MODE(TREE_TYPE(arg00))) &&
            ...
            if (cosfn != NULL_TREE) {
              ...
              return fold_build2(RDIV_EXPR, type, build_real(type, dconst1),
                                 tmp);
            }
          }
        }
    ...
    case FLOOR_DIV_EXPR:
      ...
      if (TREE_CODE(arg1) == LSHIFT_EXPR &&
        ...
        if (integer_pow2p(sval) && tree_int_cst_sgn(sval) > 0) {
          ...
          sh_cnt = fold_build2(PLUS_EXPR, TREE_TYPE(sh_cnt), sh_cnt,
                               build_int_cst(NULL_TREE, pow2));
          return fold_build2(RSHIFT_EXPR, type, fold_convert(type, arg0),
                             sh_cnt);
        }
      }
    ...
    case EXACT_DIV_EXPR:
      ...
      if (integer_zerop(arg1))
        return NULL_TREE;
      /* X / -1 is -X.  */
      if (!TYPE_UNSIGNED(type) && TREE_CODE(arg1) == INTEGER_CST &&
      ...
      /* Convert -A / -B to A / B when the type is signed and overflow is
         undefined.  */
      if ((!INTEGRAL_TYPE_P(type) || TYPE_OVERFLOW_UNDEFINED(type)) &&
          TREE_CODE(arg0) == NEGATE_EXPR && negate_expr_p(arg1)) {
        if (INTEGRAL_TYPE_P(type))
          fold_overflow_warning(("assuming signed overflow does not occur "
      ...
      if ((!INTEGRAL_TYPE_P(type) || TYPE_OVERFLOW_UNDEFINED(type)) &&
          TREE_CODE(arg1) == NEGATE_EXPR && negate_expr_p(arg0)) {
        if (INTEGRAL_TYPE_P(type))
          fold_overflow_warning(("assuming signed overflow does not occur "
                                 "when distributing negation across "
                                 "division"),
                                WARN_STRICT_OVERFLOW_MISC);
        return fold_build2(code, type, fold_convert(type, negate_expr(arg0)),
                           fold_convert(type, TREE_OPERAND(arg1, 0)));
      }
    ...
    case TRUNC_MOD_EXPR:
      ...
      if (code == TRUNC_MOD_EXPR && !TYPE_UNSIGNED(type) &&
          TREE_CODE(arg1) == INTEGER_CST && !TREE_OVERFLOW(arg1) &&
          TREE_INT_CST_HIGH(arg1) < 0 &&
      ...
      if (code == TRUNC_MOD_EXPR && !TYPE_UNSIGNED(type) &&
          TREE_CODE(arg1) == NEGATE_EXPR && !TYPE_OVERFLOW_TRAPS(type))
        return fold_build2(code, type, fold_convert(type, arg0),
                           fold_convert(type, TREE_OPERAND(arg1, 0)));
    ...
    shift:
      ...
      if (TREE_CODE(op0) == code && host_integerp(arg1, false) &&
        ...
        HOST_WIDE_INT low =
            (TREE_INT_CST_LOW(TREE_OPERAND(arg0, 1)) + TREE_INT_CST_LOW(arg1));
      ...
      if (((code == LSHIFT_EXPR && TREE_CODE(arg0) == RSHIFT_EXPR) ||
           (TYPE_UNSIGNED(type) && code == RSHIFT_EXPR &&
      ...
      if (code == LROTATE_EXPR && TREE_CODE(arg1) == INTEGER_CST) {
        tree tem = build_int_cst(TREE_TYPE(arg1), TYPE_PRECISION(type));
      ...
      if ((code == LSHIFT_EXPR || code == RSHIFT_EXPR) &&
          TREE_CODE(arg0) == BIT_AND_EXPR && TREE_CODE(arg1) == INTEGER_CST &&
          TREE_CODE(TREE_OPERAND(arg0, 1)) == INTEGER_CST) {
        tree mask = fold_build2(
            code, type, fold_convert(type, TREE_OPERAND(arg0, 1)), arg1);
        tree shift = fold_build2(
            code, type, fold_convert(type, TREE_OPERAND(arg0, 0)), arg1);
      ...
      if (TREE_CODE(arg0) == TREE_CODE(arg1) &&
        ...
        /* This case if tricky because we must either have commutative
          ...
          return fold_build2(TREE_CODE(arg0), type,
                             fold_build2(code, type, a00, a10), a01);
      }
    ...
    case NE_EXPR:
      ...
      if (TREE_CODE(arg0) == ADDR_EXPR &&
          ...
          !lookup_attribute("alias", DECL_ATTRIBUTES(TREE_OPERAND(arg0, 0))) &&
          !DECL_EXTERNAL(TREE_OPERAND(arg0, 0)) &&
          TREE_CODE(arg1) == ADDR_EXPR &&
          VAR_OR_FUNCTION_DECL_P(TREE_OPERAND(arg1, 0)) &&
          !DECL_WEAK(TREE_OPERAND(arg1, 0)) &&
          !lookup_attribute("alias", DECL_ATTRIBUTES(TREE_OPERAND(arg1, 0))) &&
        ...
        return constant_boolean_node(equal ? code == EQ_EXPR : code != EQ_EXPR,
                                     type);
      }
      ...
      if (TREE_CODE(arg1) == INTEGER_CST &&
          ...
          TREE_CODE(TREE_OPERAND(arg0, 1)) == INTEGER_CST &&
          0 != (tem = const_binop(
                    TREE_CODE(arg0) == PLUS_EXPR ? MINUS_EXPR : PLUS_EXPR,
                    fold_convert(TREE_TYPE(arg0), arg1), TREE_OPERAND(arg0, 1),
                    0)) &&
          !TREE_OVERFLOW(tem))
        return fold_build2(code, type, TREE_OPERAND(arg0, 0), tem);

      /* Similarly for a NEGATE_EXPR.  */
      if (TREE_CODE(arg0) == NEGATE_EXPR && TREE_CODE(arg1) == INTEGER_CST &&
          0 != (tem = negate_expr(arg1)) && TREE_CODE(tem) == INTEGER_CST &&
          !TREE_OVERFLOW(tem))
        return fold_build2(code, type, TREE_OPERAND(arg0, 0), tem);

      /* Similarly for a BIT_XOR_EXPR;  X ^ C1 == C2 is X == (C1 ^ C2).  */
      if (TREE_CODE(arg0) == BIT_XOR_EXPR && TREE_CODE(arg1) == INTEGER_CST &&
          TREE_CODE(TREE_OPERAND(arg0, 1)) == INTEGER_CST)
        return fold_build2(code, type, TREE_OPERAND(arg0, 0),
      ...
      if ((TREE_CODE(arg0) == PLUS_EXPR || TREE_CODE(arg0) == MINUS_EXPR) &&
        ...
        if (code == EQ_EXPR && !integer_zerop(cst))
      ...
      if (TREE_CODE(arg0) == MINUS_EXPR && integer_zerop(arg1))
        return fold_build2(code, type, TREE_OPERAND(arg0, 0),
                           TREE_OPERAND(arg0, 1));
      ...
      if (TREE_CODE(arg0) == BIT_AND_EXPR && integer_zerop(arg1)) {
        ...
        if (TREE_CODE(arg00) == LSHIFT_EXPR &&
          ...
          tree tem = fold_build2(RSHIFT_EXPR, TREE_TYPE(arg00), arg01,
                                 TREE_OPERAND(arg00, 1));
          tem = fold_build2(BIT_AND_EXPR, TREE_TYPE(arg0), tem,
                            build_int_cst(TREE_TYPE(arg0), 1));
          return fold_build2(code, type, fold_convert(TREE_TYPE(arg1), tem),
                             arg1);
        } else if (TREE_CODE(arg01) == LSHIFT_EXPR &&
                   integer_onep(TREE_OPERAND(arg01, 0))) {
          tree tem = fold_build2(RSHIFT_EXPR, TREE_TYPE(arg01), arg00,
                                 TREE_OPERAND(arg01, 1));
          tem = fold_build2(BIT_AND_EXPR, TREE_TYPE(arg0), tem,
                            build_int_cst(TREE_TYPE(arg0), 1));
          return fold_build2(code, type, fold_convert(TREE_TYPE(arg1), tem),
                             arg1);
      ...
      if (integer_zerop(arg1) && !TYPE_UNSIGNED(TREE_TYPE(arg0)) &&
        ...
        tree newmod = fold_build2(TREE_CODE(arg0), newtype,
                                  fold_convert(newtype, TREE_OPERAND(arg0, 0)),
                                  fold_convert(newtype, TREE_OPERAND(arg0, 1)));

        return fold_build2(code, type, newmod, fold_convert(newtype, arg1));
      }
      ...
      if (TREE_CODE(arg0) == BIT_AND_EXPR &&
          ...
          TREE_CODE(TREE_OPERAND(TREE_OPERAND(arg0, 0), 1)) == INTEGER_CST &&
          integer_pow2p(TREE_OPERAND(arg0, 1)) && integer_zerop(arg1)) {
        tree itype = TREE_TYPE(arg0);
        unsigned HOST_WIDE_INT prec = TYPE_PRECISION(itype);
        tree arg001 = TREE_OPERAND(TREE_OPERAND(arg0, 0), 1);

        /* Check for a valid shift count.  */
        if (TREE_INT_CST_HIGH(arg001) == 0 && TREE_INT_CST_LOW(arg001) < prec) {
          tree arg01 = TREE_OPERAND(arg0, 1);
          ...
          /* Otherwise, of unsigned (logical) shifts,
            ...
            return omit_one_operand(
                type, code == EQ_EXPR ? integer_one_node : integer_zero_node,
                arg000);
        }
      }
      ...
      if (TREE_CODE(arg0) == BIT_AND_EXPR &&
          integer_pow2p(TREE_OPERAND(arg0, 1)) &&
          operand_equal_p(TREE_OPERAND(arg0, 1), arg1, 0))
        return fold_build2(code == EQ_EXPR ? NE_EXPR : EQ_EXPR, type, arg0,
      ...
      if (TREE_CODE(arg0) == BIT_AND_EXPR && TREE_CODE(arg1) == INTEGER_CST &&
          TREE_CODE(TREE_OPERAND(arg0, 1)) == INTEGER_CST) {
        tree notc = fold_build1(BIT_NOT_EXPR, TREE_TYPE(TREE_OPERAND(arg0, 1)),
                                TREE_OPERAND(arg0, 1));
        tree dandnotc = fold_build2(BIT_AND_EXPR, TREE_TYPE(arg0), arg1, notc);
      ...
      if (TREE_CODE(arg0) == RSHIFT_EXPR && integer_zerop(arg1) &&
        ...
        if (TREE_INT_CST_HIGH(arg01) == 0 &&
          ...
          return fold_build2(code == EQ_EXPR ? GE_EXPR : LT_EXPR, type, arg00,
                             build_int_cst(itype, 0));
        }
      }

      /* (X ^ Y) == 0 becomes X == Y, and (X ^ Y) != 0 becomes X != Y.  */
      if (integer_zerop(arg1) && TREE_CODE(arg0) == BIT_XOR_EXPR)
        return fold_build2(code, type, TREE_OPERAND(arg0, 0),
                           TREE_OPERAND(arg0, 1));
      ...
      if (TREE_CODE(arg0) == BIT_AND_EXPR &&
        ...
        tem = fold_build2(BIT_AND_EXPR, TREE_TYPE(arg0),
                          ...
                          TREE_OPERAND(arg0, 1));
        return fold_build2(code == EQ_EXPR ? NE_EXPR : EQ_EXPR, type, tem,
                           arg1);
      }
      ...
      if (TREE_CODE(arg0) == BIT_XOR_EXPR &&
        ...
        tree arg00 = TREE_OPERAND(arg0, 0);
        return fold_build2(code == EQ_EXPR ? NE_EXPR : EQ_EXPR, type, arg00,
                           build_int_cst(TREE_TYPE(arg00), 0));
      }
      ...
      if (TREE_CODE(arg0) == BIT_AND_EXPR &&
          ...
          operand_equal_p(TREE_OPERAND(TREE_OPERAND(arg0, 0), 1),
                          TREE_OPERAND(arg0, 1), OEP_ONLY_CONST)) {
        tree arg000 = TREE_OPERAND(TREE_OPERAND(arg0, 0), 0);
        tem = fold_build2(BIT_AND_EXPR, TREE_TYPE(arg000), arg000,
                          TREE_OPERAND(arg0, 1));
        return fold_build2(code == EQ_EXPR ? NE_EXPR : EQ_EXPR, type, tem,
                           build_int_cst(TREE_TYPE(tem), 0));
      }

      if (integer_zerop(arg1) && tree_expr_nonzero_p(arg0)) {
      ...
      if (TREE_CODE(arg0) == BIT_AND_EXPR && TREE_CODE(arg1) == BIT_AND_EXPR) {
        ...
        if (operand_equal_p(arg01, arg11, 0))
          return fold_build2(
              ...
              fold_build2(BIT_AND_EXPR, itype,
                          fold_build2(BIT_XOR_EXPR, itype, arg00, arg10),
                          arg01),
              build_int_cst(itype, 0));
        ...
        if (operand_equal_p(arg01, arg10, 0))
          return fold_build2(
              code, type,
              fold_build2(BIT_AND_EXPR, itype,
                          fold_build2(BIT_XOR_EXPR, itype, arg00, arg11),
                          arg01),
              build_int_cst(itype, 0));
        ...
        if (operand_equal_p(arg00, arg11, 0))
          return fold_build2(
              code, type,
              fold_build2(BIT_AND_EXPR, itype,
                          fold_build2(BIT_XOR_EXPR, itype, arg01, arg10),
                          arg00),
              build_int_cst(itype, 0));
        ...
        if (operand_equal_p(arg00, arg10, 0))
          return fold_build2(
              code, type,
              fold_build2(BIT_AND_EXPR, itype,
                          fold_build2(BIT_XOR_EXPR, itype, arg01, arg11),
                          arg00),
              build_int_cst(itype, 0));
      }
      ...
      if ((TREE_CODE(arg0) == COMPLEX_EXPR || TREE_CODE(arg0) == COMPLEX_CST) &&
        ...
        if (rcond && TREE_CODE(rcond) == INTEGER_CST) {
          if (integer_zerop(rcond)) {
            if (code == EQ_EXPR)
              return omit_two_operands(type, boolean_false_node, imag0, imag1);
            return fold_build2(NE_EXPR, type, imag0, imag1);
          } else {
            if (code == NE_EXPR)
              return omit_two_operands(type, boolean_true_node, imag0, imag1);
        ...
        if (icond && TREE_CODE(icond) == INTEGER_CST) {
          if (integer_zerop(icond)) {
            if (code == EQ_EXPR)
              return omit_two_operands(type, boolean_false_node, real0, real1);
            return fold_build2(NE_EXPR, type, real0, real1);
          } else {
            if (code == NE_EXPR)
              return omit_two_operands(type, boolean_true_node, real0, real1);
    ...
    case GE_EXPR:
      ...
      if ((TREE_CODE(arg0) == PLUS_EXPR || TREE_CODE(arg0) == MINUS_EXPR) &&
        ...
        /* (X - c) > X becomes false.  */
        if (code == GT_EXPR && ((code0 == MINUS_EXPR && is_positive >= 0) ||
        ...
        if (code == LT_EXPR && ((code0 == PLUS_EXPR && is_positive >= 0) ||
        ...
        if (!HONOR_NANS(TYPE_MODE(TREE_TYPE(arg1))) && code == LE_EXPR &&
            ((code0 == MINUS_EXPR && is_positive >= 0) ||
             (code0 == PLUS_EXPR && is_positive <= 0))) {
          if (TREE_CODE(arg01) == INTEGER_CST &&
              TYPE_OVERFLOW_UNDEFINED(TREE_TYPE(arg1)))
        ...
        /* Convert (X + c) >= X to true.  */
        if (!HONOR_NANS(TYPE_MODE(TREE_TYPE(arg1))) && code == GE_EXPR &&
            ((code0 == PLUS_EXPR && is_positive >= 0) ||
             (code0 == MINUS_EXPR && is_positive <= 0))) {
          if (TREE_CODE(arg01) == INTEGER_CST &&
              TYPE_OVERFLOW_UNDEFINED(TREE_TYPE(arg1)))
        ...
        if (TREE_CODE(arg01) == INTEGER_CST) {
          ...
          if (code == GT_EXPR && ((code0 == PLUS_EXPR && is_positive > 0) ||
                                  (code0 == MINUS_EXPR && is_positive < 0))) {
          ...
          if (code == LT_EXPR && ((code0 == MINUS_EXPR && is_positive > 0) ||
                                  (code0 == PLUS_EXPR && is_positive < 0))) {
            if (TYPE_OVERFLOW_UNDEFINED(TREE_TYPE(arg1)))
              fold_overflow_warning(("assuming signed overflow does "
                                     "not occur when assuming that "
          ...
          if (code == LE_EXPR && ((code0 == PLUS_EXPR && is_positive > 0) ||
                                  (code0 == MINUS_EXPR && is_positive < 0))) {
            if (TYPE_OVERFLOW_UNDEFINED(TREE_TYPE(arg1)))
              fold_overflow_warning(("assuming signed overflow does "
                                     "not occur when assuming that "
          ...
          if (code == GE_EXPR && ((code0 == MINUS_EXPR && is_positive > 0) ||
                                  (code0 == PLUS_EXPR && is_positive < 0))) {
            if (TYPE_OVERFLOW_UNDEFINED(TREE_TYPE(arg1)))
              fold_overflow_warning(("assuming signed overflow does "
                                     "not occur when assuming that "
         ...
         the specified precision will have known values.  */
      {
        ...
        if (TREE_CODE(arg1) == INTEGER_CST &&
          ...
          if (width <= HOST_BITS_PER_WIDE_INT) {
            signed_max_lo = ((unsigned HOST_WIDE_INT)1 << (width - 1)) - 1;
            signed_max_hi = 0;
            max_hi = 0;
            ...
            signed_max_lo = -1;
            signed_max_hi = ((unsigned HOST_WIDE_INT)1 << (width - 1)) - 1;
            max_lo = -1;
            min_lo = 0;
          ...
          if ((unsigned HOST_WIDE_INT)TREE_INT_CST_HIGH(arg1) == max_hi &&
          ...
          else if ((unsigned HOST_WIDE_INT)TREE_INT_CST_HIGH(arg1) == max_hi &&
            ...
            switch (code) {
              case GT_EXPR:
                ...
                return fold_build2(EQ_EXPR, type,
                                   fold_convert(TREE_TYPE(arg1), arg0), arg1);
              case LE_EXPR:
                arg1 = const_binop(PLUS_EXPR, arg1,
                                   build_int_cst(TREE_TYPE(arg1), 1), 0);
          ...
          else if ((unsigned HOST_WIDE_INT)TREE_INT_CST_HIGH(arg1) == min_hi &&
            ...
            if (code == LE_EXPR || code == GT_EXPR) {
              ...
              return fold_build2(code == LE_EXPR ? GE_EXPR : LT_EXPR, type,
                                 fold_convert(st, arg0), build_int_cst(st, 0));
            }
          }
        }
      }
      ...
      if (code == LE_EXPR && TREE_CODE(arg1) == INTEGER_CST &&
          TREE_CODE(arg0) == ABS_EXPR && !TREE_SIDE_EFFECTS(arg0) &&
          (0 != (tem = negate_expr(arg1))) && TREE_CODE(tem) == INTEGER_CST &&
          !TREE_OVERFLOW(tem))
      ...
      if (code == GE_EXPR &&
          (integer_zerop(arg1) ||
           (!HONOR_NANS(TYPE_MODE(TREE_TYPE(arg0))) && real_zerop(arg1))) &&
      ...
      strict_overflow_p = false;
      if (code == LT_EXPR && (integer_zerop(arg1) || real_zerop(arg1)) &&
          tree_expr_nonnegative_warnv_p(arg0, &strict_overflow_p)) {
        if (strict_overflow_p)
          fold_overflow_warning(("assuming signed overflow does not occur "
                                 "when simplifying comparison of "
      ...
      if ((code == LT_EXPR || code == GE_EXPR) &&
          ...
          integer_onep(TREE_OPERAND(arg1, 0)))
        return build2(
            code == LT_EXPR ? EQ_EXPR : NE_EXPR, type,
            build2(RSHIFT_EXPR, TREE_TYPE(arg0), arg0, TREE_OPERAND(arg1, 1)),
            build_int_cst(TREE_TYPE(arg0), 0));

      if ((code == LT_EXPR || code == GE_EXPR) &&
          TYPE_UNSIGNED(TREE_TYPE(arg0)) && CONVERT_EXPR_P(arg1) &&
    ...
    case LTGT_EXPR:
      ...
      if (TREE_CODE(arg0) == REAL_CST &&
        ...
        t1 = (code == ORDERED_EXPR || code == LTGT_EXPR) ? integer_zero_node
                                                         : integer_one_node;
        return omit_one_operand(type, t1, arg1);
      }
      ...
      if (code == LTGT_EXPR && !flag_trapping_math &&
          operand_equal_p(arg0, arg1, 0))
        return constant_boolean_node(0, type);
...
static tree contains_label_1(tree *tp, int *walk_subtrees,
...
tree fold_ternary(enum tree_code code, tree type, tree op0, tree op1,
                  tree op2) {
  tree tem;
  tree arg0 = NULL_TREE, arg1 = NULL_TREE;
  enum tree_code_class kind = TREE_CODE_CLASS(code);

  gcc_assert(IS_EXPR_CODE_CLASS(kind) && TREE_CODE_LENGTH(code) == 3);
  ...
  switch (code) {
    ...
    case COND_EXPR:
      ...
      if (TREE_CODE(arg0) == INTEGER_CST) {
        ...
        if ((!TREE_SIDE_EFFECTS(unused_op) || !contains_label_p(unused_op)) &&
            (!VOID_TYPE_P(TREE_TYPE(tem)) || VOID_TYPE_P(type)))
          return pedantic_non_lvalue(tem);
        return NULL_TREE;
      }
      ...
      if (integer_zerop(op1) && integer_onep(op2) &&
          truth_value_p(TREE_CODE(arg0)))
        return pedantic_non_lvalue(fold_convert(type, invert_truthvalue(arg0)));

      /* A < 0 ? <sign bit of A> : 0 is simply (A & <sign bit of A>).  */
      if (TREE_CODE(arg0) == LT_EXPR && integer_zerop(TREE_OPERAND(arg0, 1)) &&
          integer_zerop(op2) &&
        ...
        if (TYPE_PRECISION(TREE_TYPE(tem)) < TYPE_PRECISION(TREE_TYPE(arg1)) &&
            TYPE_PRECISION(TREE_TYPE(tem)) < TYPE_PRECISION(type)) {
      ...
      if (TREE_CODE(arg0) == BIT_AND_EXPR &&
          integer_onep(TREE_OPERAND(arg0, 1)) && integer_zerop(op2) &&
          integer_pow2p(arg1)) {
        tree tem = TREE_OPERAND(arg0, 0);
        STRIP_NOPS(tem);
        if (TREE_CODE(tem) == RSHIFT_EXPR &&
          ...
          return fold_build2(BIT_AND_EXPR, type, TREE_OPERAND(tem, 0), arg1);
      }
      ...
      if (integer_zerop(op2) && TREE_CODE(arg0) == NE_EXPR &&
          integer_zerop(TREE_OPERAND(arg0, 1)) && integer_pow2p(arg1) &&
          TREE_CODE(TREE_OPERAND(arg0, 0)) == BIT_AND_EXPR &&
          operand_equal_p(TREE_OPERAND(TREE_OPERAND(arg0, 0), 1), arg1,
                          OEP_ONLY_CONST))
        return pedantic_non_lvalue(fold_convert(type, TREE_OPERAND(arg0, 0)));

      /* Convert A ? B : 0 into A && B if A and B are truth values.  */
      if (integer_zerop(op2) && truth_value_p(TREE_CODE(arg0)) &&
          truth_value_p(TREE_CODE(arg1)))
        return fold_build2(TRUTH_ANDIF_EXPR, type, fold_convert(type, arg0),
                           arg1);

      /* Convert A ? B : 1 into !A || B if A and B are truth values.  */
      if (integer_onep(op2) && truth_value_p(TREE_CODE(arg0)) &&
          truth_value_p(TREE_CODE(arg1))) {
        /* Only perform transformation if ARG0 is easily inverted.  */
        tem = fold_truth_not_expr(arg0);
        if (tem)
          return fold_build2(TRUTH_ORIF_EXPR, type, fold_convert(type, tem),
                             arg1);
      }

      /* Convert A ? 0 : B into !A && B if A and B are truth values.  */
      if (integer_zerop(arg1) && truth_value_p(TREE_CODE(arg0)) &&
          truth_value_p(TREE_CODE(op2))) {
        /* Only perform transformation if ARG0 is easily inverted.  */
        tem = fold_truth_not_expr(arg0);
        if (tem)
          return fold_build2(TRUTH_ANDIF_EXPR, type, fold_convert(type, tem),
                             op2);
      }

      /* Convert A ? 1 : B into A || B if A and B are truth values.  */
      if (integer_onep(arg1) && truth_value_p(TREE_CODE(arg0)) &&
          truth_value_p(TREE_CODE(op2)))
        return fold_build2(TRUTH_ORIF_EXPR, type, fold_convert(type, arg0),
                           op2);
    ...
    case BIT_FIELD_REF:
      if ((TREE_CODE(arg0) == VECTOR_CST ||
        ...
        if (width != 0 && simple_cst_equal(arg1, TYPE_SIZE(type)) == 1 &&
            (idx % width) == 0 &&
            (idx = idx / width) < TYPE_VECTOR_SUBPARTS(TREE_TYPE(arg0))) {
...
static
  ...
  switch (code) {
    case ARRAY_REF: {
      ...
      if (TREE_CODE(op1) == INTEGER_CST && TREE_CODE(op0) == CONSTRUCTOR &&
        ...
        while (begin != end) {
          ...
          if (TREE_CODE(index) == INTEGER_CST && tree_int_cst_lt(index, op1))
            begin = middle + 1;
...
void print_fold_checksum(const_tree expr) {
  struct md5_ctx ctx;
...
static void fold_check_failed(const_tree expr ATTRIBUTE_UNUSED,
                              const_tree ret ATTRIBUTE_UNUSED) {
  internal_error("fold check: original tree changed by fold");
}

static void fold_checksum_tree(const_tree expr, struct md5_ctx *ctx,
                               htab_t ht) {
  const void **slot;
  enum tree_code code;
  union tree_node buf;
...
recursive_label:
  ...
  *slot = expr;
  ...
  if (TREE_CODE_CLASS(code) != tcc_type &&
      TREE_CODE_CLASS(code) != tcc_declaration && code != TREE_LIST &&
      code != SSA_NAME)
  ...
  switch (TREE_CODE_CLASS(code)) {
    case tcc_constant:
      switch (code) {
        case STRING_CST:
          md5_process_bytes(TREE_STRING_POINTER(expr), TREE_STRING_LENGTH(expr),
                            ctx);
          break;
        case COMPLEX_CST:
    ...
    case tcc_type:
      ...
      if (INTEGRAL_TYPE_P(expr) || SCALAR_FLOAT_TYPE_P(expr)) {
...
tree fold_build2_stat(enum tree_code code, tree type, tree op0,
                      tree op1 MEM_STAT_DECL) {
  tree tem;
#ifdef ENABLE_FOLD_CHECKING
  unsigned char checksum_before_op0[16], checksum_before_op1[16],
      checksum_after_op0[16], checksum_after_op1[16];
  struct md5_ctx ctx;
  htab_t ht;
...
tree fold_build3_stat(enum tree_code code, tree type, tree op0, tree op1,
                      tree op2 MEM_STAT_DECL) {
  tree tem;
#ifdef ENABLE_FOLD_CHECKING
  unsigned char checksum_before_op0[16], checksum_before_op1[16],
      checksum_before_op2[16], checksum_after_op0[16], checksum_after_op1[16],
...
tree fold_build_call_array(tree type, tree fn, int nargs, tree *argarray) {
  tree tem;
#ifdef ENABLE_FOLD_CHECKING
  unsigned char checksum_before_fn[16], checksum_before_arglist[16],
      checksum_after_fn[16], checksum_after_arglist[16];
  struct md5_ctx ctx;
  htab_t ht;
  int i;
...
tree fold_build2_initializer(enum tree_code code, tree type, tree op0,
                             tree op1) {
  tree result;
  START_FOLD_INIT;
...
tree fold_build3_initializer(enum tree_code code, tree type, tree op0, tree op1,
                             tree op2) {
  tree result;
  START_FOLD_INIT;
...
tree fold_build_call_array_initializer(tree type, tree fn, int nargs,
                                       tree *argarray) {
  tree result;
  START_FOLD_INIT;
...
int multiple_of_p(tree type, const_tree top, const_tree bottom) {
  ...
  switch (TREE_CODE(top)) {
    ...
    case LSHIFT_EXPR:
      if (TREE_CODE(TREE_OPERAND(top, 1)) == INTEGER_CST) {
        ...
        /* const_binop may not detect overflow correctly,
           so check for it explicitly here.  */
        if (TYPE_PRECISION(TREE_TYPE(size_one_node)) > TREE_INT_CST_LOW(op1) &&
            TREE_INT_CST_HIGH(op1) == 0 &&
            0 != (t1 = fold_convert(
                      type, const_binop(LSHIFT_EXPR, size_one_node, op1, 0))) &&
            !TREE_OVERFLOW(t1))
    ...
    case INTEGER_CST:
      if (TREE_CODE(bottom) != INTEGER_CST || integer_zerop(bottom) ||
          (TYPE_UNSIGNED(type) &&
           (tree_int_cst_sgn(top) < 0 || tree_int_cst_sgn(bottom) < 0)))
        return 0;
      return integer_zerop(int_const_binop(TRUNC_MOD_EXPR, top, bottom, 0));
...
static bool tree_simple_nonnegative_warnv_p(enum tree_code code, tree type) {
  if ((TYPE_PRECISION(type) != 1 || TYPE_UNSIGNED(type)) && truth_value_p(code))
...
bool tree_unary_nonnegative_warnv_p(enum tree_code code, tree type, tree op0,
                                    bool *strict_overflow_p) {
  if (TYPE_UNSIGNED(type))
    return true;
...
bool tree_binary_nonnegative_warnv_p(enum tree_code code, tree type, tree op0,
  ...
  switch (code) {
    ...
    case PLUS_EXPR:
      ...
      if (TREE_CODE(type) == INTEGER_TYPE && TREE_CODE(op0) == NOP_EXPR &&
        ...
        if (TREE_CODE(inner1) == INTEGER_TYPE && TYPE_UNSIGNED(inner1) &&
          ...
          unsigned int prec =
              MAX(TYPE_PRECISION(inner1), TYPE_PRECISION(inner2)) + 1;
    ...
    case COND_EXPR:
      return (
          tree_expr_nonnegative_warnv_p(TREE_OPERAND(t, 1),
                                        strict_overflow_p) &&
          tree_expr_nonnegative_warnv_p(TREE_OPERAND(t, 2), strict_overflow_p));
    default:
...
bool tree_call_nonnegative_warnv_p(tree type, tree fndecl, tree arg0, tree arg1,
                                   bool *strict_overflow_p) {
  if (fndecl && DECL_BUILT_IN_CLASS(fndecl) == BUILT_IN_NORMAL)
    ...
    CASE_FLT_FN(BUILT_IN_ACOS)
        ...
        : CASE_INT_FN(BUILT_IN_POPCOUNT) : case BUILT_IN_BSWAP32:
    case BUILT_IN_BSWAP64:
      ...
      CASE_FLT_FN(BUILT_IN_SQRT)
          : /* sqrt(-0.0) is -0.0.  */
            if (!HONOR_SIGNED_ZEROS(TYPE_MODE(type))) return true;
      return tree_expr_nonnegative_warnv_p(arg0, strict_overflow_p);
      ...
      CASE_FLT_FN(BUILT_IN_FMAX)
          : /* True if the 1st OR 2nd arguments are nonnegative.  */
            return (tree_expr_nonnegative_warnv_p(arg0, strict_overflow_p) ||
                    (tree_expr_nonnegative_warnv_p(arg1, strict_overflow_p)));
      ...
      CASE_FLT_FN(BUILT_IN_COPYSIGN)
          : /* True if the 2nd argument is nonnegative.  */
            return tree_expr_nonnegative_warnv_p(arg1, strict_overflow_p);

      CASE_FLT_FN(BUILT_IN_POWI)
          : /* True if the 1st argument is nonnegative or the second
               argument is an even integer.  */
            if (TREE_CODE(arg1) == INTEGER_CST &&
                (TREE_INT_CST_LOW(arg1) & 1) == 0) return true;
      return tree_expr_nonnegative_warnv_p(arg0, strict_overflow_p);
...
bool tree_invalid_nonnegative_warnv_p(tree t, bool *strict_overflow_p) {
  ...
  switch (code) {
    case TARGET_EXPR: {
      ...
      if (TREE_CODE(t) == MODIFY_EXPR && TREE_OPERAND(t, 0) == temp)
    ...
    case CALL_EXPR: {
      ...
      return tree_call_nonnegative_warnv_p(TREE_TYPE(t), get_callee_fndecl(t),
                                           arg0, arg1, strict_overflow_p);
    }
    case COMPOUND_EXPR:
    case MODIFY_EXPR:
      return tree_expr_nonnegative_warnv_p(TREE_OPERAND(t, 1),
...
bool tree_expr_nonnegative_warnv_p(tree t, bool *strict_overflow_p) {
  enum tree_code code;
  ...
  switch (TREE_CODE_CLASS(code)) {
    ...
    case tcc_comparison:
      return tree_binary_nonnegative_warnv_p(
          TREE_CODE(t), TREE_TYPE(t), TREE_OPERAND(t, 0), TREE_OPERAND(t, 1),
          strict_overflow_p);

    case tcc_unary:
      return tree_unary_nonnegative_warnv_p(
          TREE_CODE(t), TREE_TYPE(t), TREE_OPERAND(t, 0), strict_overflow_p);
...
bool tree_unary_nonzero_warnv_p(enum tree_code code, tree type, tree op0,
                                bool *strict_overflow_p) {
  switch (code) {
    case ABS_EXPR:
      return tree_expr_nonzero_warnv_p(op0, strict_overflow_p);
...
bool tree_binary_nonzero_warnv_p(enum tree_code code, tree type, tree op0,
                                 tree op1, bool *strict_overflow_p) {
  bool sub_strict_overflow_p;
  switch (code) {
    case POINTER_PLUS_EXPR:
    case PLUS_EXPR:
      if (TYPE_OVERFLOW_UNDEFINED(type)) {
        /* With the presence of negative values it is hard
           to say something.  */
        sub_strict_overflow_p = false;
        if (!tree_expr_nonnegative_warnv_p(op0, &sub_strict_overflow_p) ||
            !tree_expr_nonnegative_warnv_p(op1, &sub_strict_overflow_p))
          return false;
    ...
    case MULT_EXPR:
      if (TYPE_OVERFLOW_UNDEFINED(type)) {
        if (tree_expr_nonzero_warnv_p(op0, strict_overflow_p) &&
            tree_expr_nonzero_warnv_p(op1, strict_overflow_p)) {
          *strict_overflow_p = true;
          return true;
        }
    ...
    case MIN_EXPR:
      sub_strict_overflow_p = false;
      if (tree_expr_nonzero_warnv_p(op0, &sub_strict_overflow_p) &&
          tree_expr_nonzero_warnv_p(op1, &sub_strict_overflow_p)) {
        if (sub_strict_overflow_p)
          *strict_overflow_p = true;
      }
      break;
    ...
    case MAX_EXPR:
      ...
      if (tree_expr_nonzero_warnv_p(op0, &sub_strict_overflow_p)) {
        ...
        if (tree_expr_nonzero_warnv_p(op1, strict_overflow_p))
          return true;
      ...
      else if (tree_expr_nonzero_warnv_p(op1, &sub_strict_overflow_p) &&
               tree_expr_nonnegative_warnv_p(op1, &sub_strict_overflow_p)) {
        if (sub_strict_overflow_p)
          *strict_overflow_p = true;
        return true;
    ...
    case BIT_IOR_EXPR:
      return (tree_expr_nonzero_warnv_p(op1, strict_overflow_p) ||
              tree_expr_nonzero_warnv_p(op0, strict_overflow_p));
...
bool tree_expr_nonzero_warnv_p(tree t, bool *strict_overflow_p) {
  ...
  switch (TREE_CODE_CLASS(code)) {
    ...
    case tcc_binary:
    case tcc_comparison:
      return tree_binary_nonzero_warnv_p(code, type, TREE_OPERAND(t, 0),
                                         TREE_OPERAND(t, 1), strict_overflow_p);
    case tcc_constant:
    case tcc_declaration:
    case tcc_reference:
  ...
  switch (code) {
    ...
    case MODIFY_EXPR:
    case BIND_EXPR:
      return tree_expr_nonzero_warnv_p(TREE_OPERAND(t, 1), strict_overflow_p);

    case SAVE_EXPR:
      return tree_expr_nonzero_warnv_p(TREE_OPERAND(t, 0), strict_overflow_p);
...
tree fold_binary_to_constant(enum tree_code code, tree type, tree op0,
                             tree op1) {
...
tree fold_read_from_constant_string(tree exp) {
  if ((TREE_CODE(exp) == INDIRECT_REF || TREE_CODE(exp) == ARRAY_REF) &&
    ...
    if (string &&
        ...
        TREE_CODE(string) == STRING_CST && TREE_CODE(index) == INTEGER_CST &&
        compare_tree_int(index, TREE_STRING_LENGTH(string)) < 0 &&
        (GET_MODE_CLASS(TYPE_MODE(TREE_TYPE(TREE_TYPE(string)))) == MODE_INT) &&
...
static tree fold_negate_const(tree arg0, tree type) {
  tree t = NULL_TREE;
  ...
  switch (TREE_CODE(arg0)) {
    case INTEGER_CST: {
      unsigned HOST_WIDE_INT low;
      HOST_WIDE_INT high;
      int overflow = neg_double(TREE_INT_CST_LOW(arg0), TREE_INT_CST_HIGH(arg0),
                                &low, &high);
      t = force_fit_type_double(
          type, low, high, 1,
    ...
    case FIXED_CST: {
      FIXED_VALUE_TYPE f;
      bool overflow_p =
          fixed_arithmetic(&f, NEGATE_EXPR, &(TREE_FIXED_CST(arg0)), NULL,
                           TYPE_SATURATING(type));
    ...
    case INTEGER_CST:
      ...
      /* If the value is negative, then the absolute value is
        ...
        int overflow = neg_double(TREE_INT_CST_LOW(arg0),
                                  TREE_INT_CST_HIGH(arg0), &low, &high);
        t = force_fit_type_double(type, low, high, -1,
...
static tree fold_not_const(tree arg0, tree type) {
  tree t = NULL_TREE;

  gcc_assert(TREE_CODE(arg0) == INTEGER_CST);

  t = force_fit_type_double(type, ~TREE_INT_CST_LOW(arg0),
                            ~TREE_INT_CST_HIGH(arg0), 0, TREE_OVERFLOW(arg0));
...
static tree fold_relational_const(enum tree_code code, tree type, tree op0,
                                  tree op1) {
  int result, invert;
  ...
  if (TREE_CODE(op0) == COMPLEX_CST && TREE_CODE(op1) == COMPLEX_CST) {
    tree rcond = fold_relational_const(code, type, TREE_REALPART(op0),
                                       TREE_REALPART(op1));
    tree icond = fold_relational_const(code, type, TREE_IMAGPART(op0),
                                       TREE_IMAGPART(op1));
    if (code == EQ_EXPR)
...
tree fold_indirect_ref_1(tree type, tree op0) {
  ...
  if (TREE_CODE(sub) == ADDR_EXPR) {
    ...
    else if (TREE_CODE(optype) == ARRAY_TYPE && type == TREE_TYPE(optype)) {
    ...
    else if (TREE_CODE(optype) == COMPLEX_TYPE && type == TREE_TYPE(optype))
      return fold_build1(REALPART_EXPR, type, op);
    /* *(foo *)&vectorfoo => BIT_FIELD_REF<vectorfoo,...> */
    else if (TREE_CODE(optype) == VECTOR_TYPE && type == TREE_TYPE(optype)) {
...
tree round_up(tree value, int divisor) {
  ...
  if (divisor == (divisor & -divisor)) {
    if (TREE_CODE(value) == INTEGER_CST) {
      ...
      return force_fit_type_double(TREE_TYPE(value), low, high, -1, overflow_p);
    } else {
      tree t;
...
static tree split_address_to_core_and_offset(tree exp, HOST_WIDE_INT *pbitpos,
                                             tree *poffset) {
  tree core;
  enum machine_mode mode;
  int unsignedp, volatilep;
  ...
  if (TREE_CODE(exp) == ADDR_EXPR) {
    core = get_inner_reference(TREE_OPERAND(exp, 0), &bitsize, pbitpos, poffset,
...
bool ptr_difference_const(tree e1, tree e2, HOST_WIDE_INT *diff) {
  ...
  if (bitpos1 % BITS_PER_UNIT != 0 || bitpos2 % BITS_PER_UNIT != 0 ||
