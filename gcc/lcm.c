...
static void compute_laterin(struct edge_list *, sbitmap *, sbitmap *, sbitmap *,
                            sbitmap *);
...
static void compute_antinout_edge(sbitmap *antloc, sbitmap *transp,
...
static void compute_earliest(struct edge_list *edge_list, int n_exprs,
                             sbitmap *antin, sbitmap *antout, sbitmap *avout,
                             sbitmap *kill, sbitmap *earliest) {
  ...
  for (x = 0; x < num_edges; x++) {
    ...
    if (pred == ENTRY_BLOCK_PTR)
      ...
      if (succ == EXIT_BLOCK_PTR)
        ...
        sbitmap_difference(difference, antin[succ->index], avout[pred->index]);
...
static void compute_laterin(struct edge_list *edge_list, sbitmap *earliest,
  ...
  while (qlen) {
    ...
    if (sbitmap_union_of_diff_cg(later[(size_t)e->aux],
                                 ...
                                 laterin[e->src->index], antloc[e->src->index])
  ...
  FOR_EACH_EDGE(e, ei, EXIT_BLOCK_PTR->preds)
  sbitmap_a_and_b(laterin[last_basic_block], laterin[last_basic_block],
                  later[(size_t)e->aux]);
...
static void compute_insert_delete(struct edge_list *edge_list, sbitmap *antloc,
                                  sbitmap *later, sbitmap *laterin,
                                  sbitmap *insert, sbitmap *del) {
  int x;
  basic_block bb;

  FOR_EACH_BB(bb)
  sbitmap_difference(del[bb->index], antloc[bb->index], laterin[bb->index]);
...
struct edge_list *pre_edge_lcm(int n_exprs, sbitmap *transp, sbitmap *avloc,
                               sbitmap *antloc, sbitmap *kill, sbitmap **insert,
                               sbitmap **del) {
...
#ifdef LCM_DEBUG_INFO
  if (dump_file) {
    dump_sbitmap_vector(dump_file, "laterin", "", laterin,
                        last_basic_block + 1);
...
void compute_available(sbitmap *avloc, sbitmap *kill, sbitmap *avout,
                       sbitmap *avin) {
  edge e;
  basic_block *worklist, *qin, *qout, *qend, bb;
  unsigned int qlen;
...
static void compute_farthest(struct edge_list *edge_list, int n_exprs,
                             sbitmap *st_avout, sbitmap *st_avin,
                             sbitmap *st_antin, sbitmap *kill,
                             sbitmap *farthest) {
  sbitmap difference, temp_bitmap;
  int x, num_edges;
  basic_block pred, succ;
  ...
  for (x = 0; x < num_edges; x++) {
    ...
    if (succ == EXIT_BLOCK_PTR)
      ...
      if (pred == ENTRY_BLOCK_PTR)
        ...
        sbitmap_a_and_b_or_c(farthest[x], difference, kill[succ->index],
                             temp_bitmap);
      }
    }
  }
...
static void compute_nearerout(struct edge_list *edge_list, sbitmap *farthest,
                              sbitmap *st_avloc, sbitmap *nearer,
                              sbitmap *nearerout) {
  int num_edges, i;
  edge e;
  ...
  while (tos != worklist) {
    ...
    if (sbitmap_union_of_diff_cg(
            nearer[(size_t)e->aux], farthest[(size_t)e->aux],
            nearerout[e->dest->index], st_avloc[e->dest->index])
  ...
  FOR_EACH_EDGE(e, ei, ENTRY_BLOCK_PTR->succs)
  sbitmap_a_and_b(nearerout[last_basic_block], nearerout[last_basic_block],
                  nearer[(size_t)e->aux]);
...
static void compute_rev_insert_delete(struct edge_list *edge_list,
                                      sbitmap *st_avloc, sbitmap *nearer,
                                      sbitmap *nearerout, sbitmap *insert,
                                      sbitmap *del) {
  int x;
  basic_block bb;

  FOR_EACH_BB(bb)
  sbitmap_difference(del[bb->index], st_avloc[bb->index], nearerout[bb->index]);
...
struct edge_list *pre_edge_rev_lcm(int n_exprs, sbitmap *transp,
                                   sbitmap *st_avloc, sbitmap *st_antloc,
                                   sbitmap *kill, sbitmap **insert,
                                   sbitmap **del) {
  ...
  if (dump_file) {
    ...
    dump_sbitmap_vector(dump_file, "st_antout", "", st_antout,
                        last_basic_block);
  ...
  compute_farthest(edge_list, n_exprs, st_avout, st_avin, st_antin, kill,
                   farthest);
  ...
  *insert = sbitmap_vector_alloc(num_edges, n_exprs);
  *del = sbitmap_vector_alloc(last_basic_block, n_exprs);
  compute_rev_insert_delete(edge_list, st_avloc, nearer, nearerout, *insert,
                            *del);
