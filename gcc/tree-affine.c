...
double_int double_int_ext_for_comb(double_int cst, aff_tree *comb) {
...
void aff_combination_const(aff_tree *comb, tree type, double_int cst) {
...
void aff_combination_scale(aff_tree *comb, double_int scale) {
  unsigned i, j;
  ...
  for (i = 0, j = 0; i < comb->n; i++) {
    ...
    new_coef = double_int_ext_for_comb(
        double_int_mul(scale, comb->elts[i].coef), comb);
...
void aff_combination_add_elt(aff_tree *comb, tree elt, double_int scale) {
  unsigned i;
  tree type;
  ...
  if (comb->rest)
    comb->rest = fold_build2(PLUS_EXPR, type, comb->rest, elt);
  else
    comb->rest = elt;
}
...
static void aff_combination_add_cst(aff_tree *c, double_int cst) {
...
void tree_to_aff_combination(tree expr, tree type, aff_tree *comb) {
  ...
  switch (code) {
    ...
    case ADDR_EXPR:
      core =
          get_inner_reference(TREE_OPERAND(expr, 0), &bitsize, &bitpos,
                              &toffset, &mode, &unsignedp, &volatilep, false);
...
static tree add_elt_to_tree(tree expr, tree type, tree elt, double_int scale,
                            aff_tree *comb) {
  enum tree_code code;
  tree type1 = type;
  if (POINTER_TYPE_P(type))
  ...
  if (!expr)
    return fold_convert(type, fold_build2(MULT_EXPR, type1, elt,
  ...
  elt = fold_build2(MULT_EXPR, type1, elt, double_int_to_tree(type1, scale));
  if (POINTER_TYPE_P(type)) {
    if (code == MINUS_EXPR)
      elt = fold_build1(NEGATE_EXPR, type1, elt);
    return fold_build2(POINTER_PLUS_EXPR, type, expr, elt);
...
tree aff_combination_to_tree(aff_tree *comb) {
  ...
  return add_elt_to_tree(expr, type, double_int_to_tree(type1, off), sgn, comb);
}
...
static void aff_combination_add_product(aff_tree *c, double_int coef, tree val,
                                        aff_tree *r) {
  unsigned i;
  tree aval, type;
  ...
  for (i = 0; i < c->n; i++) {
    ...
    if (val) {
      type = TREE_TYPE(aval);
      aval = fold_build2(MULT_EXPR, type, aval, fold_convert(type, val));
    }

    aff_combination_add_elt(r, aval, double_int_mul(coef, c->elts[i].coef));
  }
  ...
  if (val)
    aff_combination_add_elt(r, val, double_int_mul(coef, c->offset));
  else
...
static struct aff_comb_elt *aff_combination_find_elt(aff_tree *comb, tree val,
                                                     unsigned *idx) {
  unsigned i;
...
void aff_combination_expand(aff_tree *comb ATTRIBUTE_UNUSED,
  ...
  for (i = 0; i < comb->n; i++) {
    ...
    if (TREE_CODE(e) == NOP_EXPR &&
        (TYPE_PRECISION(type) >= TYPE_PRECISION(TREE_TYPE(TREE_OPERAND(e, 0)))))
    ...
    if (code != SSA_NAME && !IS_EXPR_CODE_CLASS(TREE_CODE_CLASS(code)) &&
    ...
    if (!exp) {
      ...
      *slot = exp;
      ...
      /* Convert (T1)(X *+- CST) into (T1)X *+- (T1)CST if T1 is wider
         ...
         undefined.  */
      if (e != name && INTEGRAL_TYPE_P(type) &&
        ...
        rhs =
            fold_build2(code, type, fold_convert(type, gimple_assign_rhs1(def)),
...
void tree_to_aff_combination_expand(tree expr, tree type, aff_tree *comb,
...
static bool free_name_expansion(const void *key ATTRIBUTE_UNUSED, void **value,
...
void free_affine_expand_cache(struct pointer_map_t **cache) {
  if (!*cache)
    return;
...
static bool double_int_constant_multiple_p(double_int val, double_int div,
...
bool aff_combination_constant_multiple_p(aff_tree *val, aff_tree *div,
                                         double_int *mult) {
  bool mult_set = false;
  unsigned i;
  ...
  if (!double_int_constant_multiple_p(val->offset, div->offset, &mult_set,
                                      mult))
    return false;
  ...
  for (i = 0; i < div->n; i++) {
    ...
    if (!elt)
      return false;
    if (!double_int_constant_multiple_p(elt->coef, div->elts[i].coef, &mult_set,
                                        mult))
      return false;
  }
...
void get_inner_reference_aff(tree ref, aff_tree *addr, double_int *size) {
  HOST_WIDE_INT bitsize, bitpos;
  tree toff;
  enum machine_mode mode;
  int uns, vol;
  aff_tree tmp;
  tree base = get_inner_reference(ref, &bitsize, &bitpos, &toff, &mode, &uns,
                                  &vol, false);
