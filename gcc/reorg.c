...
#define unfilled_slots_base ((rtx *)obstack_base(&unfilled_slots_obstack))
...
static rtx steal_delay_list_from_target(rtx, rtx, rtx, rtx, struct resources *,
                                        struct resources *, struct resources *,
                                        int, int *, int *, rtx *);
static rtx steal_delay_list_from_fallthrough(rtx, rtx, rtx, rtx,
                                             struct resources *,
                                             struct resources *,
                                             struct resources *, int, int *,
                                             int *);
...
static rtx fill_slots_from_thread(rtx, rtx, rtx, rtx, int, int, int, int, int *,
                                  rtx);
...
static int stop_search_p(rtx insn, int labels_p) {
  if (insn == 0)
    return 1;
...
static int resource_conflicts_p(struct resources *res1,
                                struct resources *res2) {
  if ((res1->cc && res2->cc) || (res1->memory && res2->memory) ||
      (res1->unch_memory && res2->unch_memory) || res1->volatil ||
      res2->volatil)
    return 1;
...
static int insn_references_resource_p(rtx insn, struct resources *res,
...
static int insn_sets_resource_p(rtx insn, struct resources *res,
                                int include_delayed_effects) {
...
static rtx find_end_label(void) {
  ...
  if (BARRIER_P(insn) && JUMP_P(PREV_INSN(insn)) &&
    ...
    while (insn && !(JUMP_P(insn) && (GET_CODE(PATTERN(insn)) == RETURN)))
      insn = PREV_INSN(insn);
    if (insn) {
      insn = PREV_INSN(insn);
      ...
      if (crtl->epilogue_delay_list == NULL && HAVE_return) {
...
static rtx emit_delay_sequence(rtx insn, rtx list, int length) {
  int i = 1;
  rtx li;
  int had_barrier = 0;
  ...
  if (PREV_INSN(seq_insn) && NONJUMP_INSN_P(PREV_INSN(seq_insn)) &&
    ...
    NEXT_INSN(XVECEXP(PATTERN(PREV_INSN(seq_insn)), 0,
                      XVECLEN(PATTERN(PREV_INSN(seq_insn)), 0) - 1)) = seq_insn;
...
static rtx add_to_delay_list(rtx insn, rtx delay_list) {
...
static rtx delete_from_delay_slot(rtx insn) {
  ...
  for (trial = insn; PREV_INSN(NEXT_INSN(trial)) == trial;
...
static void delete_scheduled_jump(rtx insn) {
  ...
  if (reg_mentioned_p(cc0_rtx, insn)) {
    ...
    if (note) {
      ...
      if (sets_cc0_p(PATTERN(trial)) != 1 || FIND_REG_INC_NOTE(trial, NULL_RTX))
        return;
...
static int num_filled_delays[NUM_REORG_FUNCTIONS][MAX_DELAY_HISTOGRAM + 1]
                            [MAX_REORG_PASSES];

static int reorg_pass_number;

static void note_delay_statistics(int slots_filled, int index) {
...
static rtx optimize_skip(rtx insn) {
  ...
  if (trial == 0 || !NONJUMP_INSN_P(trial) ||
      GET_CODE(PATTERN(trial)) == SEQUENCE || recog_memoized(trial) < 0 ||
  ...
  if ((next_trial == next_active_insn(JUMP_LABEL(insn)) &&
      ...
      (next_trial != 0 && JUMP_P(next_trial) &&
       JUMP_LABEL(insn) == JUMP_LABEL(next_trial) &&
       (simplejump_p(next_trial) || GET_CODE(PATTERN(next_trial)) == RETURN))) {
...
static int get_jump_flags(rtx insn, rtx label) {
  int flags;
  ...
  /* get_jump_flags can be passed any insn with delay slots, these may
     ...
     direction.  */
  if (JUMP_P(insn) && (condjump_p(insn) || condjump_in_parallel_p(insn)) &&
      INSN_UID(insn) <= max_uid && label != 0 && INSN_UID(label) <= max_uid)
    flags = (uid_to_ruid[INSN_UID(label)] > uid_to_ruid[INSN_UID(insn)])
                ? ATTR_FLAG_forward
                : ATTR_FLAG_backward;
...
static int rare_destination(rtx insn) {
  int jump_count = 0;
  rtx next;
...
static int mostly_true_jump(rtx jump_insn, rtx condition) {
  ...
  return (
      target_label == 0 || INSN_UID(jump_insn) > max_uid ||
      INSN_UID(target_label) > max_uid ||
      (uid_to_ruid[INSN_UID(jump_insn)] > uid_to_ruid[INSN_UID(target_label)]));
}
...
static rtx get_branch_condition(rtx insn, rtx target) {
  ...
  if (GET_CODE(src) == LABEL_REF && XEXP(src, 0) == target)
    ...
    if (rev != UNKNOWN)
      return gen_rtx_fmt_ee(rev, GET_MODE(XEXP(src, 0)), XEXP(XEXP(src, 0), 0),
                            XEXP(XEXP(src, 0), 1));
  }
...
static int condition_dominates_p(rtx condition, rtx insn) {
...
static int redirect_with_delay_slots_safe_p(rtx jump, rtx newlabel, rtx seq) {
  int flags, i;
  rtx pat = PATTERN(seq);
    ...
    if (!(
            ...
            (INSN_ANNULLED_BRANCH_P(jump) &&
             INSN_FROM_TARGET_P(XVECEXP(pat, 0, i)))
                ? eligible_for_annul_false(jump, i - 1, XVECEXP(pat, 0, i),
                                           flags)
            :
#endif
            ...
            (INSN_ANNULLED_BRANCH_P(jump) &&
             !INSN_FROM_TARGET_P(XVECEXP(pat, 0, i)))
                ? eligible_for_annul_true(jump, i - 1, XVECEXP(pat, 0, i),
                                          flags)
                :
#endif
...
static int redirect_with_delay_list_safe_p(rtx jump, rtx newlabel,
                                           rtx delay_list) {
  int flags, i;
  rtx li;
  ...
  for (li = delay_list, i = 0; li; li = XEXP(li, 1), i++)
    if (!(
#ifdef ANNUL_IFFALSE_SLOTS
            (INSN_ANNULLED_BRANCH_P(jump) && INSN_FROM_TARGET_P(XEXP(li, 0)))
...
static int check_annul_list_true_false(int annul_true_p, rtx delay_list) {
  rtx temp;
...
static rtx steal_delay_list_from_target(rtx insn, rtx condition, rtx seq,
  ...
  for (i = 1; i < XVECLEN(seq, 0); i++) {
    ...
    if (!must_annul && ((condition == const_true_rtx ||
...
static rtx steal_delay_list_from_fallthrough(
    rtx insn, rtx condition, rtx seq, rtx delay_list, struct resources *sets,
    struct resources *needed, struct resources *other_needed, int slots_to_fill,
    int *pslots_filled, int *pannul_p) {
  int i;
  int flags;
  int must_annul = *pannul_p;
  int used_annul = 0;
...
static void try_merge_delay_insns(rtx insn, rtx thread) {
  ...
  /* See if we stopped on a filled insn.  If we did, try to see if its
     delay slots match.  */
  if (slot_number != num_slots && trial && NONJUMP_INSN_P(trial) &&
    ...
    for (i = 1; i < XVECLEN(pat, 0); i++) {
      ...
      if (!insn_references_resource_p(dtrial, &set, 1) &&
        ...
        if (!annul_p) {
          ...
          merged_insns = gen_rtx_INSN_LIST(SImode, dtrial, merged_insns);
...
static rtx redundant_insn(rtx insn, rtx target, rtx delay_list) {
  ...
  for (trial = PREV_INSN(target), insns_to_search = MAX_DELAY_SLOT_INSN_SEARCH;
...
static int own_thread_p(rtx thread, rtx label, int allow_fallthrough) {
  rtx active_insn;
  rtx insn;
  ...
  for (insn = prev_nonnote_insn(thread); insn == 0 || !BARRIER_P(insn);
...
static void update_block(rtx insn, rtx where) {
...
static int reorg_redirect_jump(rtx jump, rtx nlabel) {
...
static void update_reg_dead_notes(rtx insn, rtx delayed_insn) {
  rtx p, link, next;
...
static void fix_reg_dead_note(rtx start_insn, rtx stop_insn) {
  rtx p, link, next;
...
static void update_reg_unused_notes(rtx insn, rtx redundant_insn) {
  rtx link, next;
  ...
  for (link = REG_NOTES(insn); link; link = next) {
    ...
    if (!find_regno_note(redundant_insn, REG_UNUSED, REGNO(XEXP(link, 0))))
...
static rtx get_label_before(rtx insn) {
  rtx label;
...
static void fill_simple_delay_slots(int non_jumps_p) {
  ...
  for (i = 0; i < num_unfilled_slots; i++) {
    ...
    if ((trial = next_active_insn(insn)) && JUMP_P(trial) &&
        simplejump_p(trial) &&
      ...
      for (trial = prev_nonnote_insn(insn); !stop_search_p(trial, 1);
        ...
        if (!insn_references_resource_p(trial, &set, 1) &&
          ...
          if (eligible_for_delay(insn, slots_filled, trial, flags)) {
            ...
            delay_list = gen_rtx_INSN_LIST(VOIDmode, trial, delay_list);
            update_block(trial, trial);
            delete_related_insns(trial);
...
#if defined(ANNUL_IFFALSE_SLOTS) || defined(ANNUL_IFTRUE_SLOTS)
    if (slots_filled != slots_to_fill && delay_list == 0 && JUMP_P(insn) &&
    ...
    if (slots_filled != slots_to_fill
        ...
        (!JUMP_P(insn) || ((condjump_p(insn) || condjump_in_parallel_p(insn)) &&
      ...
      if (target == 0)
        for (trial = next_nonnote_insn(insn); trial; trial = next_trial) {
          ...
          if (CALL_P(trial_delay) || JUMP_P(trial_delay))
            maybe_never = 1;
        }
      ...
      if (slots_to_fill != slots_filled && trial && JUMP_P(trial) &&
          simplejump_p(trial) && (target == 0 || JUMP_LABEL(trial) == target) &&
        ...
        if (new_label) {
          delay_list = add_to_delay_list(copy_rtx(next_trial), delay_list);
          slots_filled++;
    ...
    /* If this is an unconditional jump, then try to get insns from the
       target of the jump.  */
    if (JUMP_P(insn) && simplejump_p(insn) && slots_filled != slots_to_fill)
      delay_list = fill_slots_from_thread(
          insn, const_true_rtx, next_active_insn(JUMP_LABEL(insn)), NULL, 1, 1,
          own_thread_p(JUMP_LABEL(insn), JUMP_LABEL(insn), 0), slots_to_fill,
    ...
    if (!insn_references_resource_p(trial, &set, 1) &&
      ...
      if (ELIGIBLE_FOR_EPILOGUE_DELAY(trial, slots_filled)) {
        ...
        crtl->epilogue_delay_list =
            gen_rtx_INSN_LIST(VOIDmode, trial, crtl->epilogue_delay_list);
...
static rtx follow_jumps(rtx label) {
  ...
  for (depth = 0;
       (depth < 10 && (insn = next_active_insn(value)) != 0 && JUMP_P(insn) &&
        ((JUMP_LABEL(insn) != 0 && any_uncondjump_p(insn) &&
          onlyjump_p(insn)) ||
         GET_CODE(PATTERN(insn)) == RETURN) &&
        (next = NEXT_INSN(insn)) && BARRIER_P(next));
       depth++) {
    rtx tem;
...
static rtx fill_slots_from_thread(rtx insn, rtx condition, rtx thread,
                                  rtx opposite_thread, int likely,
                                  int thread_if_true, int own_thread,
                                  int slots_to_fill, int *pslots_filled,
                                  rtx delay_list) {
  rtx new_thread;
  ...
  for (trial = thread;
    ...
    if (!insn_references_resource_p(trial, &set, 1) &&
      ...
      if (!must_annul && (condition == const_true_rtx ||
        ...
        if (thread == old_trial)
          thread = trial;
        pat = PATTERN(trial);
        if ((must_annul || delay_list == NULL) &&
            (thread_if_true ? check_annul_list_true_false(0, delay_list) &&
                                  eligible_for_annul_false(insn, *pslots_filled,
                                                           trial, flags)
                            : check_annul_list_true_false(1, delay_list) &&
                                  eligible_for_annul_true(insn, *pslots_filled,
                                                          trial, flags))) {
          rtx temp;
        ...
        winner:
          ...
          if (own_thread) {
            ...
            for (note = REG_NOTES(trial); note != NULL_RTX;
                 note = XEXP(note, 1))
          ...
          if (slots_to_fill == ++(*pslots_filled)) {
            ...
            while (
                new_thread && !own_thread &&
                !insn_sets_resource_p(new_thread, &set, 1) &&
                !insn_sets_resource_p(new_thread, &needed, 1) &&
                !insn_references_resource_p(new_thread, &set, 1) &&
                (prior_insn = redundant_insn(new_thread, insn, delay_list))) {
    ...
    if (NONJUMP_INSN_P(trial) && GET_CODE(pat) == SET && REG_P(SET_SRC(pat)) &&
        REG_P(SET_DEST(pat)) &&
        !reg_overlap_mentioned_p(SET_DEST(pat), SET_SRC(pat))) {
      rtx next = next_nonnote_insn(trial);

      if (next && NONJUMP_INSN_P(next) && GET_CODE(PATTERN(next)) != USE &&
          !reg_set_p(SET_DEST(pat), next) && !reg_set_p(SET_SRC(pat), next) &&
  ...
  if (trial && NONJUMP_INSN_P(trial) && GET_CODE(PATTERN(trial)) == SEQUENCE &&
      JUMP_P(XVECEXP(PATTERN(trial), 0, 0))) {
    /* If this is the `true' thread, we will want to follow the jump,
       so we can only do this if we have taken everything up to here.  */
    if (thread_if_true && trial == new_thread) {
      delay_list = steal_delay_list_from_target(
          insn, condition, PATTERN(trial), delay_list, &set, &needed,
      ...
      delay_list = steal_delay_list_from_fallthrough(
          insn, condition, PATTERN(trial), delay_list, &set, &needed,
          &opposite_needed, slots_to_fill, pslots_filled, &must_annul);
  }
  ...
  if (delay_list == 0 && likely && new_thread && NONJUMP_INSN_P(new_thread) &&
    ...
    if (!NONJUMP_INSN_P(trial) || GET_CODE(pat) != SET ||
        !eligible_for_delay(insn, 0, trial, flags) || can_throw_internal(trial))
      return 0;
    ...
    if ((GET_CODE(src) == PLUS || GET_CODE(src) == MINUS) &&
        ...
        !reg_overlap_mentioned_p(dest, XEXP(src, 1)) && !side_effects_p(pat)) {
      ...
      ninsn = emit_insn_after(gen_rtx_SET(VOIDmode, dest, new_arith), insn);
  ...
  if (new_thread != thread) {
    ...
    if (new_thread && JUMP_P(new_thread) &&
        (simplejump_p(new_thread) || GET_CODE(PATTERN(new_thread)) == RETURN) &&
        redirect_with_delay_list_safe_p(insn, JUMP_LABEL(new_thread),
                                        delay_list))
...
static void fill_eager_delay_slots(void) {
  ...
  for (i = 0; i < num_unfilled_slots; i++) {
    ...
    insn = unfilled_slots_base[i];
    if (insn == 0 || INSN_DELETED_P(insn) || !JUMP_P(insn) ||
        !(condjump_p(insn) || condjump_in_parallel_p(insn)))
      continue;
    ...
    if (prediction > 0) {
      delay_list = fill_slots_from_thread(
          insn, condition, insn_at_target, fallthrough_insn, prediction == 2, 1,
      ...
      if (delay_list == 0 && own_fallthrough) {
        ...
        delay_list = fill_slots_from_thread(
            insn, condition, fallthrough_insn, insn_at_target, 0, 0,
            own_fallthrough, slots_to_fill, &slots_filled, delay_list);
      }
    } else {
      if (own_fallthrough)
        delay_list = fill_slots_from_thread(
      ...
      if (delay_list == 0)
        delay_list = fill_slots_from_thread(
            insn, condition, insn_at_target, next_active_insn(insn), 0, 1,
            own_target, slots_to_fill, &slots_filled, delay_list);
    }
...
static void delete_prior_computation(rtx note, rtx insn) {
  ...
  for (our_prev = prev_nonnote_insn(insn);
       our_prev && (NONJUMP_INSN_P(our_prev) || CALL_P(our_prev));
    ...
    if (CALL_P(our_prev) &&
        (!RTL_CONST_CALL_P(our_prev) || GET_CODE(pat) != SET ||
...
static void delete_computation(rtx insn) {
  ...
  if (reg_referenced_p(cc0_rtx, PATTERN(insn))) {
    ...
    if (prev && NONJUMP_INSN_P(prev) && sets_cc0_p(PATTERN(prev))) {
      if (sets_cc0_p(PATTERN(prev)) > 0 && !side_effects_p(PATTERN(prev)))
...
static void delete_jump(rtx insn) {
...
static void relax_delay_slots(rtx first) {
  rtx insn, next, pat;
  ...
  for (insn = first; insn; insn = next) {
    ...
    if (JUMP_P(insn) && (condjump_p(insn) || condjump_in_parallel_p(insn)) &&
      ...
      /* See if this jump conditionally branches around an unconditional
         ...
         second jump.  */
      if (next && JUMP_P(next) && any_condjump_p(insn) &&
    ...
    if (JUMP_P(insn) &&
        ...
        (other = prev_active_insn(insn)) != 0 && any_condjump_p(other) &&
        no_labels_between_p(other, insn) &&
        0 > mostly_true_jump(other,
                             get_branch_condition(other, JUMP_LABEL(other)))) {
    ...
    if (optimize_function_for_size_p(cfun) &&
        GET_CODE(PATTERN(delay_insn)) == RETURN && next && JUMP_P(next) &&
    ...
    if (target_label) {
      ...
      if (trial && GET_CODE(PATTERN(trial)) != SEQUENCE &&
          redundant_insn(trial, insn, 0) && !can_throw_internal(trial)) {
      ...
      if (trial && GET_CODE(PATTERN(trial)) == SEQUENCE &&
        ...
        if (target_label &&
            redirect_with_delay_slots_safe_p(delay_insn, target_label, insn)) {
          reorg_redirect_jump(delay_insn, target_label);
          next = insn;
          continue;
        }
    ...
    if (!INSN_ANNULLED_BRANCH_P(delay_insn) &&
        ...
        && !find_reg_note(XVECEXP(pat, 0, XVECLEN(pat, 0) - 1), REG_CC_USER,
                          NULL_RTX)
#endif
    ) {
      rtx after;
    ...
    if (next && NONJUMP_INSN_P(next) &&
        ...
        simplejump_p(insn) && XVECLEN(pat, 0) == 2 &&
    ...
    /* See if this jump (with its delay slots) conditionally branches
       ...
       a RETURN here.  */
    if (!INSN_ANNULLED_BRANCH_P(delay_insn) && any_condjump_p(delay_insn) &&
        next && JUMP_P(next) &&
        (simplejump_p(next) || GET_CODE(PATTERN(next)) == RETURN) &&
        next_active_insn(target_label) == next_active_insn(next) &&
        no_labels_between_p(insn, next)) {
      ...
      if (label && no_labels_between_p(insn, next) &&
...
static void make_return_insns(rtx first) {
  ...
  for (insn = first; insn; insn = NEXT_INSN(insn)) {
    ...
    if (!reorg_redirect_jump(jump_insn, NULL_RTX)) {
      /* Make sure redirecting the jump will not invalidate the delay
         slot insns.  */
      if (redirect_with_delay_slots_safe_p(jump_insn, real_return_label, insn))
...
void dbr_schedule(rtx first) {
  ...
  for (reorg_pass_number = 0; reorg_pass_number < MAX_REORG_PASSES;
       reorg_pass_number++) {
    fill_simple_delay_slots(1);
    fill_simple_delay_slots(0);
  ...
  if (dump_file) {
    ...
    for (insn = first; insn; insn = NEXT_INSN(insn)) {
      if (!INSN_DELETED_P(insn) && NONJUMP_INSN_P(insn) &&
...
static bool gate_handle_delay_slots(void) {
#ifdef DELAY_SLOTS
...
static unsigned int rest_of_handle_delay_slots(void) {
#ifdef DELAY_SLOTS
...
static bool gate_handle_machine_reorg(void) {
  return targetm.machine_dependent_reorg != 0;
}

static unsigned int rest_of_handle_machine_reorg(void) {
