...
#define UNION_OR_STRUCT_P(x)                              \
  ((x)->kind == TYPE_UNION || (x)->kind == TYPE_STRUCT || \
...
static void oprintf(outf_p o, const char *S, ...) ATTRIBUTE_PRINTF_2;
...
static lang_bitmap get_lang_bitmap(const char *gtfile) {
  if (gtfile == this_file)
...
static void set_lang_bitmap(char *gtfile, lang_bitmap n) {
  int i;
...
/* Read one input line from LIST to HEREP (which is updated).  A
   ...
   diagnostics.  */
static bool read_input_line(FILE *list, char **herep, char **linep,
                            struct fileloc *pos) {
  char *here = *herep;
  char *line;
  int c = getc(list);
          ...
          if (strcmp(gt_files[i], line) == 0) {
            ...
            if (bmap & curlangs)
              error_at_line(&epos,
                            ...
                            "for language %s",
                            line,
                            langno == 0 ? "(all)" : lang_dir_names[langno - 1]);
      ...
      if (slashpos) {
        ...
        for (l = 0; l < num_lang_dirs; l++)
          if ((size_t)(slashpos - basename) == strlen(lang_dir_names[l]) &&
              memcmp(basename, lang_dir_names[l], strlen(lang_dir_names[l])) ==
                  0) {
            if (!(bitmap & (1 << l)))
              error(
                  "%s is in language directory '%s' but is not "
...
static struct type string_type = {TYPE_STRING, 0, 0, GC_USED, {0}};

/* The two and only TYPE_SCALARs.  Their u.scalar_is_char flags are
   set to appropriate values at the beginning of main.  */

static struct type scalar_nonchar = {TYPE_SCALAR, 0, 0, GC_USED, {0}};
static struct type scalar_char = {TYPE_SCALAR, 0, 0, GC_USED, {0}};
...
type_p new_structure(const char *name, int isunion, struct fileloc *pos,
...
static type_p find_param_structure(type_p t, type_p param[NUM_PARAM]) {
  ...
  for (res = param_structs; res; res = res->next)
    if (res->u.param_struct.stru == t &&
        memcmp(res->u.param_struct.param, param, sizeof(type_p) * NUM_PARAM) ==
            0)
      break;
  if (res == NULL) {
...
static pair_p create_field_all(pair_p next, type_p type, const char *name,
                               options_p opt, const char *file, int line) {
  pair_p field;
...
pair_p create_field_at(pair_p next, type_p type, const char *name,
                       options_p opt, struct fileloc *pos) {
  return create_field_all(next, adjust_field_type(type, opt), name, opt,
                          pos->file, pos->line);
}
...
static pair_p create_optional_field_(pair_p next, type_p type, const char *name,
...
static void write_rtx_next(void) {
  ...
  for (i = 0; i < NUM_RTX_CODE; i++)
    if (rtx_next_new[i] == -1)
      ...
      oprintf(f, "  RTX_HDR_SIZE + %d * sizeof (rtunion),\n", rtx_next_new[i]);
  oprintf(f, "};\n");
}
...
static type_p adjust_field_rtx_def(type_p t, options_p ARG_UNUSED(opt)) {
  ...
  if (t->kind != TYPE_UNION) {
    error_at_line(&lexer_line, "special `rtx_def' must be applied to a union");
    return &string_type;
  }
  ...
  /* Create a type to represent the various forms of SYMBOL_REF_DATA.  */
  {
    ...
    symbol_union_tp = new_structure("rtx_def_symbol_subunion", 1, &lexer_line,
                                    sym_flds, NULL);
  }
  for (i = 0; i < NUM_RTX_CODE; i++) {
    ...
    for (aindex = 0; aindex < strlen(rtx_format[i]); aindex++) {
      ...
      subfields = create_field(
          subfields, t,
          xasprintf(".fld[%lu].%s", (unsigned long)aindex, subname));
      subfields->opt = nodot;
      if (t == note_union_tp)
        subfields->opt =
            create_option(subfields->opt, "desc", "NOTE_KIND (&%0)");
...
static type_p adjust_field_tree_exp(type_p t, options_p opt ATTRIBUTE_UNUSED) {
  pair_p flds;
  options_p nodot;
  ...
  flds->opt =
      create_option(nodot, "length", "TREE_OPERAND_LENGTH ((tree) &%0)");
...
type_p adjust_field_type(type_p t, options_p opt) {
  ...
  for (; opt; opt = opt->next)
    if (strcmp(opt->name, "length") == 0)
      length_p = 1;
             ...
             (strncmp(opt->name, "param", 5) == 0 && ISDIGIT(opt->name[5]) &&
      ...
      if (!UNION_OR_STRUCT_P(t) &&
        ...
        error_at_line(&lexer_line,
                      "option `%s' may only be applied to structures or "
                      "structure pointers",
                      opt->name);
        return t;
      }
  ...
  if (!length_p && pointer_p && t->u.p->kind == TYPE_SCALAR &&
      t->u.p->u.scalar_is_char)
    return &string_type;
  if (t->kind == TYPE_ARRAY && t->u.a.p->kind == TYPE_POINTER &&
      t->u.a.p->u.p->kind == TYPE_SCALAR && t->u.a.p->u.p->u.scalar_is_char)
...
static void process_gc_options(options_p opt, enum gc_used_enum level,
                               int *maybe_undef, int *pass_param, int *length,
                               int *skip, type_p *nested_ptr) {
  options_p o;
...
static void set_gc_used_type(type_p t, enum gc_used_enum level,
                             type_p param[NUM_PARAM]) {
  if (t->gc_used >= level)
    return;
  ...
  switch (t->kind) {
    ...
    case TYPE_UNION: {
      ...
      for (f = t->u.s.fields; f; f = f->next) {
        ...
        process_gc_options(f->opt, level, &maybe_undef, &pass_param, &length,
                           &skip, &nested_ptr);
    ...
    case TYPE_PARAM_STRUCT: {
      ...
      set_gc_used_type(t->u.param_struct.stru, level, t->u.param_struct.param);
      break;
...
static outf_p create_file(const char *name, const char *oname) {
...
static void open_base_files(void) {
  ...
  /* gtype-desc.c is a little special, so we create it here.  */
  {
    ...
    static const char *const ifiles[] = {"config.h",      "system.h",
                                         "coretypes.h",   "tm.h",
                                         "varray.h",      "hashtab.h",
                                         "splay-tree.h",  "obstack.h",
                                         "bitmap.h",      "input.h",
                                         "tree.h",        "rtl.h",
                                         "function.h",    "insn-config.h",
                                         "expr.h",        "hard-reg-set.h",
                                         "basic-block.h", "cselib.h",
                                         "insn-addr.h",   "optabs.h",
                                         "libfuncs.h",    "debug.h",
                                         "ggc.h",         "cgraph.h",
                                         "tree-flow.h",   "reload.h",
                                         "cpp-id-data.h", "tree-chrec.h",
                                         "cfglayout.h",   "except.h",
                                         "output.h",      "gimple.h",
                                         "cfgloop.h",     NULL};
...
static const char *get_file_realbasename(const char *f) {
...
static const char *get_file_srcdir_relative_path(const char *f) {
  if (strlen(f) > srcdir_len && IS_DIR_SEPARATOR(f[srcdir_len]) &&
...
static const char *get_file_basename(const char *f) {
...
static int get_prefix_langdir_index(const char *f) {
  ...
  for (lang_index = 0; lang_index < num_lang_dirs; lang_index++) {
    ...
    if (f_len > langdir_len && IS_DIR_SEPARATOR(f[langdir_len]) &&
...
static const char *get_file_langdir(const char *f) {
...
static const char *get_file_gtfilename(const char *f) {
  ...
  char *result = (langdir ? xasprintf("gt-%s-%s", langdir, basename)
...
static void output_escaped_param(struct walk_type_data *d, const char *,
                                 const char *);
...
static void write_types(type_p structures, type_p param_structs,
                        const struct write_types_data *wtd);
static void write_types_local_process_field(type_p f,
                                            const struct walk_type_data *d);
static void write_local_func_for_structure(type_p orig_s, type_p s,
                                           type_p *param);
...
static void output_mangled_typename(outf_p of, const_type_p t) {
  if (t == NULL)
    oprintf(of, "Z");
  else
    switch (t->kind) {
...
static void output_escaped_param(struct walk_type_data *d, const char *param,
                                 const char *oname) {
  const char *p;
  ...
  for (p = param; *p; p++)
    if (*p != '%')
      ...
      switch (*++p) {
        ...
        default:
          error_at_line(d->line, "`%s' option contains bad escape %c%c", oname,
                        '%', *p);
      }
}
...
static void walk_type(type_p t, struct walk_type_data *d) {
  ...
  for (oo = d->opt; oo; oo = oo->next)
    if (strcmp(oo->name, "length") == 0)
      ...
      ;
    else if (strncmp(oo->name, "param", 5) == 0 && ISDIGIT(oo->name[5]) &&
  ...
  if (use_param_num != -1) {
    if (d->param != NULL && d->param[use_param_num] != NULL) {
      ...
      d->needs_cast_p = (t->kind != TYPE_POINTER &&
                         (nt->kind == TYPE_POINTER || nt->kind == TYPE_STRING));
      t = nt;
    } else
  ...
  if (maybe_undef_p &&
      (t->kind != TYPE_POINTER || !UNION_OR_STRUCT_P(t->u.p))) {
    error_at_line(d->line, "field `%s' has invalid option `maybe_undef_p'\n",
                  d->val);
    return;
  }
  ...
  switch (t->kind) {
    ...
    case TYPE_POINTER: {
      ...
      if (!length) {
        if (!UNION_OR_STRUCT_P(t->u.p) && t->u.p->kind != TYPE_PARAM_STRUCT) {
          error_at_line(d->line, "field `%s' is pointer to unimplemented type",
                        d->val);
          break;
        }
        ...
        if (nested_ptr_d) {
          ...
          oprintf(d->of, "%*s%s %s * %s%s =\n", d->indent, "",
                  (nested_ptr_d->type->kind == TYPE_UNION ? "union" : "struct"),
                  nested_ptr_d->type->u.s.tag,
                  d->fn_wants_lvalue ? "" : "const ", d->val);
          oprintf(d->of, "%*s", d->indent + 2, "");
          output_escaped_param(d, nested_ptr_d->convert_from, "nested_ptr");
          oprintf(d->of, ";\n");

          d->process_field(nested_ptr_d->type, d);

          if (d->fn_wants_lvalue) {
            oprintf(d->of, "%*s%s = ", d->indent, "", d->prev_val[2]);
            d->prev_val[2] = d->val;
            output_escaped_param(d, nested_ptr_d->convert_to, "nested_ptr");
            oprintf(d->of, ";\n");
          }
    ...
    case TYPE_ARRAY: {
      ...
      d->indent += 2;
      oprintf(d->of, "%*ssize_t i%d;\n", d->indent, "", loopcounter);
      oprintf(d->of, "%*ssize_t l%d = (size_t)(", d->indent, "", loopcounter);
      if (length)
        output_escaped_param(d, length, "length");
      else
    ...
    case TYPE_UNION: {
      ...
      for (f = t->u.s.fields; f; f = f->next) {
        ...
        if (union_p && tagid) {
          ...
          error_at_line(d->line, "can't use `%s' outside a union on field `%s'",
                        default_p ? "default" : "tag", f->name);
        else if (union_p && !(default_p || tagid) &&
                 f->type->kind == TYPE_SCALAR) {
          fprintf(stderr,
                  "%s:%d: warning: field `%s' is missing `tag' or `default' "
                  "option\n",
...
static void write_types_process_field(type_p f,
  ...
  switch (f->kind) {
    case TYPE_POINTER:
      oprintf(d->of, "%*s%s (%s%s", d->indent, "", wtd->subfield_marker_routine,
              cast, d->val);
      if (wtd->param_prefix) {
      ...
      if (d->reorder_fn && wtd->reorder_note_routine)
        oprintf(d->of, "%*s%s (%s%s, %s, %s);\n", d->indent, "",
                wtd->reorder_note_routine, cast, d->val, d->prev_val[3],
                d->reorder_fn);
      break;
      ...
      for (opt = s->u.s.opt; opt; opt = opt->next)
        if (strcmp(opt->name, "ptr_alias") == 0) {
          ...
          if (t->kind == TYPE_STRUCT || t->kind == TYPE_UNION ||
            ...
            oprintf(header_file, "#define gt_%sx_%s gt_%sx_%s\n", wtd->prefix,
                    s->u.s.tag, wtd->prefix, t->u.s.tag);
          else
            error_at_line(&s->u.s.line, "structure alias is not a structure");
          break;
        }
      if (opt)
        continue;
...
static const struct write_types_data ggc_wtd = {
    "ggc_m",    NULL,
    "ggc_mark", "ggc_test_and_set_mark",
    NULL,       "GC marker procedures.  ",
    FALSE};
...
static void finish_root_table(struct flist *flp, const char *pfx,
                              const char *lastname, const char *tname,
                              const char *name) {
  struct flist *fli2;
  ...
  for (fli2 = flp; fli2; fli2 = fli2->next)
    if (fli2->started_p) {
      ...
      for (fnum = 0; bitmap != 0; fnum++, bitmap >>= 1)
        if (bitmap & 1) {
          oprintf(base_files[fnum], "extern const struct %s gt_%s_", tname,
                  pfx);
...
static void write_root(outf_p f, pair_p v, type_p type, const char *name,
  ...
  switch (type->kind) {
      ...
      for (fld = type->u.s.fields; fld; fld = fld->next) {
        ...
        if (skip_p)
          ...
          for (ufld = fld->type->u.s.fields; ufld; ufld = ufld->next) {
            ...
            if (validf != NULL)
              error_at_line(
                  line, "both `%s.%s.%s' and `%s.%s.%s' have tag `%s'", name,
                  fld->name, validf->name, name, fld->name, ufld->name, tag);
            validf = ufld;
          }
          if (validf != NULL) {
            char *newname;
            newname = xasprintf("%s.%s.%s", name, fld->name, validf->name);
            write_root(f, v, validf->type, newname, 0, line, if_marked);
            free(newname);
          }
        } else if (desc)
          error_at_line(line,
    ...
    case TYPE_POINTER: {
      ...
      if (!has_length && UNION_OR_STRUCT_P(tp)) {
        ...
        error_at_line(line, "global `%s' is pointer to unimplemented type",
                      name);
      }
      if (if_marked)
        oprintf(f, ",\n    &%s", if_marked);
...
static void write_roots(pair_p variables) {
  pair_p v;
  ...
  for (v = variables; v; v = v->next) {
    ...
    for (o = v->opt; o; o = o->next)
      if (strcmp(o->name, "length") == 0)
        ...
        ;
      else if (strncmp(o->name, "param", 5) == 0 && ISDIGIT(o->name[5]) &&
               strcmp(o->name + 6, "_is") == 0)
        ;
      else if (strcmp(o->name, "if_marked") == 0)
        ;
      else
        error_at_line(&v->line, "global `%s' has unknown option `%s'", v->name,
                      o->name);
    ...
    if (!deletable_p && length && v->type->kind == TYPE_POINTER &&
  ...
  for (v = variables; v; v = v->next) {
    ...
    write_root(f, v, v->type->u.p->u.param_struct.param[0], v->name, length_p,
               &v->line, if_marked);
  }
...
void note_def_vec(const char *type_name, bool is_scalar, struct fileloc *pos) {
  pair_p fields;
  type_p t;
  options_p o;
...
*/
void note_def_vec_alloc(const char *type, const char *astrat,
                        struct fileloc *pos) {
  const char *astratname = concat("VEC_", type, "_", astrat, (char *)0);
  const char *basename = concat("VEC_", type, "_base", (char *)0);

  pair_p field =
      create_field_at(0, resolve_typedef(basename, pos), "base", 0, pos);
