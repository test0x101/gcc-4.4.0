...
static struct mem_sym_stats_d *get_mem_sym_stats_for(tree var) {
  void **slot;
...
static mem_sym_stats_t mem_sym_stats(struct function *fn, tree var) {
  void **slot;
...
static inline void set_memory_partition(tree sym, tree mpt) {
#if defined ENABLE_CHECKING
  if (mpt)
    gcc_assert(TREE_CODE(mpt) == MEMORY_PARTITION_TAG && !is_gimple_reg(sym));
#endif
...
static int sort_tags_by_id(const void *pa, const void *pb) {
...
static void init_transitive_clobber_worklist(VEC(tree, heap) * *worklist,
...
static void add_to_worklist(tree alias, VEC(tree, heap) * *worklist,
...
static void mark_aliases_call_clobbered(tree tag, VEC(tree, heap) * *worklist,
                                        VEC(int, heap) * *worklist2,
                                        bitmap on_worklist) {
  bitmap aliases;
  bitmap_iterator bi;
  ...
  EXECUTE_IF_SET_IN_BITMAP(aliases, 0, i, bi) {
    ...
    if (!unmodifiable_var_p(entry)) {
      add_to_worklist(entry, worklist, worklist2, ta->escape_mask, on_worklist);
...
static void compute_tag_properties(void) {
  referenced_var_iterator rvi;
  tree tag;
  bool changed = true;
  ...
  qsort(VEC_address(tree, taglist), VEC_length(tree, taglist), sizeof(tree),
        sort_tags_by_id);
...
static void set_initial_properties(struct alias_info *ai) {
  unsigned int i;
  referenced_var_iterator rvi;
  tree var;
  tree ptr;
  ...
  FOR_EACH_REFERENCED_VAR(var, rvi) {
    ...
    } else if (TREE_CODE(var) == PARM_DECL && gimple_default_def(cfun, var) &&
  ...
  for (i = 0; VEC_iterate(tree, ai->processed_ptrs, i, ptr); i++) {
    ...
    if (pi->value_escapes_p && pi->escape_mask & ~ESCAPE_TO_RETURN) {
    ...
    if (pi->name_mem_tag && tag && is_call_clobbered(pi->name_mem_tag))
      mark_call_clobbered(tag, pi->escape_mask);
    ...
    if ((pi->pt_global_mem || pi->pt_anything) && pi->memory_tag_needed &&
        pi->name_mem_tag) {
...
static void compute_call_clobbered(struct alias_info *ai) {
...
static void dump_memory_partitions(FILE *file) {
  unsigned i, npart;
  unsigned long nsyms;
  tree mpt;
  ...
  for (i = 0, npart = 0, nsyms = 0;
       VEC_iterate(tree, gimple_ssa_operands(cfun)->mpt_table, i, mpt); i++) {
    if (mpt) {
...
static inline bool need_to_partition_p(struct mem_ref_stats_d *stats) {
...
static void count_mem_refs(long *num_vuses_p, long *num_vdefs_p,
...
static inline long mem_sym_score(mem_sym_stats_t mp) {
...
void dump_mem_ref_stats(FILE *file) {
  ...
  fprintf(file, "Estimated number of loads:       %ld (%ld/stmt)\n",
          stats->num_vuses,
          (stats->num_mem_stmts) ? CEIL(stats->num_vuses, stats->num_mem_stmts)
                                 : 0);
  fprintf(file, "Actual number of loads:          %ld (%ld/stmt)\n",
          actual_num_vuses,
          (stats->num_mem_stmts) ? CEIL(actual_num_vuses, stats->num_mem_stmts)
                                 : 0);
  ...
  fprintf(file, "Estimated number of stores:      %ld (%ld/stmt)\n",
          stats->num_vdefs,
          (stats->num_mem_stmts) ? CEIL(stats->num_vdefs, stats->num_mem_stmts)
                                 : 0);
  fprintf(file, "Actual number of stores:         %ld (%ld/stmt)\n",
          actual_num_vdefs,
          (stats->num_mem_stmts) ? CEIL(actual_num_vdefs, stats->num_mem_stmts)
                                 : 0);
...
static void dump_mem_sym_stats(FILE *file, tree var) {
...
void dump_mem_sym_stats_for_var(FILE *file, tree var) {
  mem_sym_stats_t stats = mem_sym_stats(cfun, var);
...
static void dump_all_mem_sym_stats(FILE *file) {
  referenced_var_iterator rvi;
  tree sym;
...
static void dump_mp_info(FILE *file, VEC(mem_sym_stats_t, heap) * mp_info) {
  unsigned i;
...
void update_mem_sym_stats_from_stmt(tree var, gimple stmt,
  ...
  stats->frequency_reads +=
      ((long)gimple_bb(stmt)->frequency * num_direct_reads);

  stats->num_direct_writes += num_direct_writes;
  stats->frequency_writes +=
      ((long)gimple_bb(stmt)->frequency * num_direct_writes);
}
...
static inline int compare_mp_info_entries(mem_sym_stats_t mp1,
                                          mem_sym_stats_t mp2) {
...
static int mp_info_cmp(const void *p, const void *q) {
...
static inline void sort_mp_info(VEC(mem_sym_stats_t, heap) * list) {
  ...
  qsort(VEC_address(mem_sym_stats_t, list), VEC_length(mem_sym_stats_t, list),
        sizeof(mem_sym_stats_t), mp_info_cmp);
}

/* Return the memory partition tag (MPT) associated with memory
   symbol SYM.  */

static tree get_mpt_for(tree sym) {
  tree mpt;
...
static tree find_partition_for(mem_sym_stats_t mp_p) {
  unsigned i;
  ...
  for (i = 0; VEC_iterate(tree, mpt_table, i, mpt); i++) {
    ...
    if (mp_p->parent_tags && mpt_stats->parent_tags &&
...
static void rewrite_alias_set_for(tree tag, bitmap new_aliases) {
  bitmap_iterator bi;
  unsigned i;
  tree mpt, sym;
...
static void estimate_vop_reduction(struct mem_ref_stats_d *mem_ref_stats,
  ...
  if (is_call_clobbered(mp_p->var)) {
    if (mpt_stats->has_call_clobbered_vars)
      mem_ref_stats->num_vdefs -=
          mem_ref_stats->num_call_sites + mem_ref_stats->num_asm_sites;
    else
...
static void update_reference_counts(struct mem_ref_stats_d *mem_ref_stats) {
  unsigned i;
  bitmap_iterator bi;
  ...
  for (i = 1; i < num_ssa_names; i++) {
    ...
    if (ptr && POINTER_TYPE_P(TREE_TYPE(ptr)) &&
        (pi = SSA_NAME_PTR_INFO(ptr)) != NULL && pi->memory_tag_needed) {
      unsigned j;
      bitmap_iterator bj;
      tree tag;
...
static void build_mp_info(struct mem_ref_stats_d *mem_ref_stats,
  ...
  FOR_EACH_REFERENCED_VAR(var, rvi) {
    ...
    /* Add VAR's reference info to MP_INFO.  Note that the only
       ...
       call-clobbered.  */
    if (sym_stats->num_indirect_reads > 0 || sym_stats->num_indirect_writes > 0)
...
static void compute_memory_partitions(void) {
  tree tag;
  unsigned i;
  mem_sym_stats_t mp_p;
...
static tree count_ptr_derefs(tree *tp, int *walk_subtrees, void *data) {
...
void count_uses_and_derefs(tree ptr, gimple stmt, unsigned *num_uses_p,
  ...
  *num_stores_p = 0;
  ...
  if (is_gimple_assign(stmt) || gimple_code(stmt) == GIMPLE_RETURN ||
      gimple_code(stmt) == GIMPLE_ASM || is_gimple_call(stmt)) {
...
void delete_mem_ref_stats(struct function *fn) {
...
static void init_mem_ref_stats(void) {
  struct mem_ref_stats_d *mem_ref_stats = gimple_mem_ref_stats(cfun);

  mem_sym_stats_pool =
      create_alloc_pool("Mem sym stats", sizeof(struct mem_sym_stats_d), 100);
...
static void delete_alias_info(struct alias_info *ai) {
  size_t i;
...
static int eq_ptr_info(const void *p1, const void *p2) {
...
static hashval_t ptr_info_hash(const void *p) {
...
static void create_name_tags(void) {
  size_t i;
  ...
  for (i = 1; i < num_ssa_names; i++) {
    ...
    if (!ptr || !POINTER_TYPE_P(TREE_TYPE(ptr)) || !SSA_NAME_PTR_INFO(ptr))
      continue;
...
static void union_alias_set_into(tree tag, bitmap set) {
...
static void compute_flow_sensitive_aliasing(struct alias_info *ai) {
  size_t i;
  tree ptr;
...
static bool have_common_aliases_p(bitmap tag1aliases, bitmap tag2aliases) {
...
static void compute_flow_insensitive_aliasing(struct alias_info *ai) {
  referenced_var_iterator rvi;
  tree var;
  size_t i;
...
static void update_alias_info_1(gimple stmt, struct alias_info *ai) {
  bitmap addr_taken;
  use_operand_p use_p;
  ssa_op_iter iter;
  ...
  FOR_EACH_PHI_OR_STMT_USE(use_p, stmt, iter, SSA_OP_USE) {
    ...
    if (is_gimple_assign(stmt) && gimple_assign_rhs_code(stmt) == ADDR_EXPR &&
        !is_gimple_val(gimple_assign_rhs1(stmt))) {
      /* If the RHS if of the form &PTR->FLD and PTR == OP, then
         this represents a potential dereference of PTR.  */
      tree rhs = gimple_assign_rhs1(stmt);
      tree base = get_base_address(TREE_OPERAND(rhs, 0));
      if (TREE_CODE(base) == INDIRECT_REF && TREE_OPERAND(base, 0) == op)
        num_loads++;
    }
    ...
    if (stmt_escape_type != NO_ESCAPE && num_loads + num_stores < num_uses) {
      ...
      if (is_gimple_call(stmt) || stmt_escape_type == ESCAPE_STORED_IN_GLOBAL) {
        pointer_set_insert(ai->dereferenced_ptrs, var);
        pi->memory_tag_needed = 1;
      }
    }
  ...
  if (gimple_references_memory_p(stmt)) {
    ...
    if (!stmt_dereferences_ptr_p && stmt_escape_type != ESCAPE_TO_CALL &&
        stmt_escape_type != ESCAPE_TO_PURE_CONST &&
...
static void update_alias_info(struct alias_info *ai) {
  basic_block bb;
...
static void setup_pointers_and_addressables(struct alias_info *ai) {
  ...
  FOR_EACH_REFERENCED_VAR_SAFE(var, varvec, srvi) {
    ...
    if (TREE_ADDRESSABLE(var)) {
      if (!bitmap_bit_p(gimple_addressable_vars(cfun), DECL_UID(var)) &&
          TREE_CODE(var) != RESULT_DECL && !is_global_var(var)) {
...
static void maybe_create_global_var(void) {
  ...
  if (gimple_global_var(cfun) == NULL_TREE) {
    ...
    if (bitmap_empty_p(gimple_call_clobbered_vars(cfun)) &&
        stats->num_call_sites > 0 && stats->num_pure_const_call_sites > 0 &&
...
bool may_alias_p(tree ptr, alias_set_type mem_alias_set, tree var,
                 alias_set_type var_alias_set, bool alias_set_only) {
  tree mem;
  ...
  if (mem_alias_set == 0 || DECL_POINTER_ALIAS_SET(ptr) == 0 ||
      TYPE_REF_CAN_ALIAS_ALL(TREE_TYPE(ptr)) || DECL_NO_TBAA_P(ptr)) {
...
bool may_point_to_global_var(tree ptr) {
  struct ptr_info_def *pi = SSA_NAME_PTR_INFO(ptr);
...
static void add_may_alias(tree var, tree alias) {
...
enum escape_type is_escape_site(gimple stmt) {
  if (is_gimple_call(stmt)) {
    ...
    if (DECL_P(lhs) && !is_global_var(lhs))
      return NO_ESCAPE;
...
tree create_tag_raw(enum tree_code code, tree type, const char *prefix) {
  tree tmp_var;
...
static tree get_nmt_for(tree ptr) {
  struct ptr_info_def *pi = get_ptr_info(ptr);
...
static tree get_smt_for(tree ptr, struct alias_info *ai) {
  size_t i;
  tree tag;
  ...
  if (tag == NULL_TREE) {
    ...
    if (tag == NULL_TREE || tag_set != get_alias_set(tag))
...
static void create_global_var(void) {
  tree global_var =
      build_decl(VAR_DECL, get_identifier(".GLOBAL_VAR"), void_type_node);
...
static void dump_alias_stats(FILE *file) {
  const char *funcname =
      lang_hooks.decl_printable_name(current_function_decl, 2);
...
void dump_points_to_info(FILE *file ATTRIBUTE_UNUSED) {
  basic_block bb;
  gimple_stmt_iterator si;
  ssa_op_iter iter;
  const char *fname = lang_hooks.decl_printable_name(current_function_decl, 2);
  referenced_var_iterator rvi;
  tree var;
  ...
  FOR_EACH_BB(bb) {
    ...
    for (si = gsi_start_bb(bb); !gsi_end_p(si); gsi_next(&si)) {
      ...
      if (TREE_CODE(def) == SSA_NAME && POINTER_TYPE_P(TREE_TYPE(def)))
        dump_points_to_info_for(file, def);
    }
  }
...
bool may_be_aliased(tree var) {
  ...
  /* Globally visible variables can have their addresses taken by other
     translation units.  */
  if (MTAG_P(var) && MTAG_GLOBAL(var))
    return true;
  else if (!MTAG_P(var) && (DECL_EXTERNAL(var) || TREE_PUBLIC(var)))
    return true;
...
static tree add_may_alias_for_new_tag(tree tag, tree var) {
  ...
  /* Case 1: |aliases| == 1  */
  if (aliases && bitmap_single_bit_set_p(aliases)) {
...
void new_type_alias(tree ptr, tree var, tree expr) {
  tree tag_type = TREE_TYPE(TREE_TYPE(ptr));
  tree tag;
  tree ali = NULL_TREE;
...
static unsigned int reset_cc_flags(void) {
  tree var;
  referenced_var_iterator rvi;
