...
void create_iv(tree base, tree step, tree var, struct loop *loop,
...
static void add_exit_phis_edge(basic_block exit, tree use) {
...
static void add_exit_phis_var(tree var, bitmap livein, bitmap exits) {
  bitmap def;
  unsigned index;
...
static void add_exit_phis(bitmap names_to_rename, bitmap *use_blocks,
                          bitmap loop_exits) {
  unsigned i;
  bitmap_iterator bi;
...
static bitmap get_loops_exits(void) {
...
static void find_uses_to_rename_use(basic_block bb, tree use,
...
static void find_uses_to_rename_stmt(gimple stmt, bitmap *use_blocks,
                                     bitmap need_phis) {
  ssa_op_iter iter;
  tree var;
...
static void find_uses_to_rename_bb(basic_block bb, bitmap *use_blocks,
                                   bitmap need_phis) {
...
static void find_uses_to_rename(bitmap changed_bbs, bitmap *use_blocks,
                                bitmap need_phis) {
  basic_block bb;
  unsigned index;
  bitmap_iterator bi;
...
static void check_loop_closed_ssa_use(basic_block bb, tree use) {
  gimple def;
  basic_block def_bb;
  ...
  gcc_assert(!def_bb || flow_bb_inside_loop_p(def_bb->loop_father, bb));
}

/* Checks invariants of loop closed ssa form in statement STMT in BB.  */

static void check_loop_closed_ssa_stmt(basic_block bb, gimple stmt) {
  ssa_op_iter iter;
  tree var;
...
void verify_loop_closed_ssa(void) {
  ...
  FOR_EACH_BB(bb) {
    for (bsi = gsi_start_phis(bb); !gsi_end_p(bsi); gsi_next(&bsi)) {
      phi = gsi_stmt(bsi);
      FOR_EACH_EDGE(e, ei, bb->preds)
      check_loop_closed_ssa_use(e->src, PHI_ARG_DEF_FROM_EDGE(phi, e));
    }
...
basic_block ip_end_pos(struct loop *loop) {
  return loop->latch;
}
...
basic_block ip_normal_pos(struct loop *loop) {
  gimple last;
  basic_block bb;
  edge exit;
...
void standard_iv_increment_position(struct loop *loop,
  ...
  if (!bb || (last && gimple_code(last) != GIMPLE_LABEL)) {
...
bool gimple_duplicate_loop_to_header_edge(struct loop *loop, edge e,
  ...
  if (!duplicate_loop_to_header_edge(loop, e, ndupl, wont_exit, orig, to_remove,
                                     flags))
    return false;
  ...
  *exit_bound = NULL_TREE;
  ...
  if (!integer_zerop(desc->may_be_zero))
    cond = fold_build2(TRUTH_AND_EXPR, boolean_type_node,
                       invert_truthvalue(desc->may_be_zero), cond);

  bigstep =
      fold_build2(MULT_EXPR, type, step, build_int_cst_type(type, factor));
  delta = fold_build2(MINUS_EXPR, type, bigstep, step);
  if (cmp == LT_EXPR)
    assum = fold_build2(GE_EXPR, boolean_type_node, bound,
                        fold_build2(PLUS_EXPR, type, min, delta));
  else
    assum = fold_build2(LE_EXPR, boolean_type_node, bound,
...
static void scale_dominated_blocks_in_loop(struct loop *loop, basic_block bb,
                                           int num, int den) {
  basic_block son;
...
void tree_transform_and_unroll_loop(struct loop *loop, unsigned factor,
                                    edge exit, struct tree_niter_desc *desc,
                                    transform_callback transform, void *data) {
  gimple exit_if;
  ...
  determine_exit_conditions(loop, desc, factor, &enter_main_cond, &exit_base,
                            &exit_step, &exit_cmp, &exit_bound);
  ...
  scale_dominated_blocks_in_loop(loop, exit->src, REG_BR_PROB_BASE,
                                 REG_BR_PROB_BASE - exit->probability);

  bsi = gsi_last_bb(exit_bb);
  exit_if = gimple_build_cond(EQ_EXPR, integer_zero_node, integer_zero_node,
                              NULL_TREE, NULL_TREE);
  ...
  ok = gimple_duplicate_loop_to_header_edge(loop, loop_latch_edge(loop),
                                            factor - 1, wont_exit, new_exit,
                                            &to_remove, DLTHE_FLAG_UPDATE_FREQ);
  free(wont_exit);
  gcc_assert(ok);
  ...
  if (new_nonexit->count < 0)
    new_nonexit->count = 0;
  if (prob > 0)
    scale_bbs_frequencies_int(&loop->latch, 1, new_nonexit->probability, prob);

  /* Finally create the new counter for number of iterations and add the new
     exit instruction.  */
  bsi = gsi_last_bb(exit_bb);
  exit_if = gsi_stmt(bsi);
  create_iv(exit_base, exit_step, NULL_TREE, loop, &bsi, false, &ctr_before,
            &ctr_after);
...
void tree_unroll_loop(struct loop *loop, unsigned factor, edge exit,
                      struct tree_niter_desc *desc) {
  tree_transform_and_unroll_loop(loop, factor, exit, desc, NULL, NULL);
}
