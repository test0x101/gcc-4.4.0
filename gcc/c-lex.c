...
static enum integer_type_kind narrowest_unsigned_type(unsigned HOST_WIDE_INT,
                                                      unsigned HOST_WIDE_INT,
                                                      unsigned int);
static enum integer_type_kind narrowest_signed_type(unsigned HOST_WIDE_INT,
                                                    unsigned HOST_WIDE_INT,
                                                    unsigned int);
...
struct c_fileinfo *get_fileinfo(const char *name) {
  ...
  if (!file_info_tree)
    file_info_tree = splay_tree_new((splay_tree_compare_fn)strcmp, 0,
  ...
  splay_tree_insert(file_info_tree, (splay_tree_key)name, (splay_tree_value)fi);
  return fi;
}

static void update_header_times(const char *name) {
...
static int dump_one_header(splay_tree_node n, void *ARG_UNUSED(dummy)) {
...
static void cb_ident(cpp_reader *ARG_UNUSED(pfile),
...
static void cb_line_change(cpp_reader *ARG_UNUSED(pfile),
                           const cpp_token *token, int parsing_args) {
  if (token->type != CPP_EOF && !parsing_args)
    input_location = token->src_loc;
}

void fe_file_change(const struct line_map *new_map) {
  if (new_map == NULL)
    return;
...
static void cb_def_pragma(cpp_reader *pfile, source_location loc) {
  ...
  if (warn_unknown_pragmas > in_system_header) {
    ...
    warning(OPT_Wunknown_pragmas, "%Hignoring #pragma %s %s", &fe_loc, space,
            name);
  }
}

/* #define callback for DWARF and DWARF2 debug info.  */
static void cb_define(cpp_reader *pfile, source_location loc,
                      cpp_hashnode *node) {
...
static void cb_undef(cpp_reader *ARG_UNUSED(pfile), source_location loc,
                     cpp_hashnode *node) {
  const struct line_map *map = linemap_lookup(line_table, loc);
  (*debug_hooks->undef)(SOURCE_LINE(map, loc), (const char *)NODE_NAME(node));
}
...
enum cpp_ttype c_lex_with_flags(tree *value, location_t *loc,
...
static enum integer_type_kind narrowest_unsigned_type(
    unsigned HOST_WIDE_INT low, unsigned HOST_WIDE_INT high,
    unsigned int flags) {
  int itk;
...
static enum integer_type_kind narrowest_signed_type(unsigned HOST_WIDE_INT low,
...
static tree interpret_integer(const cpp_token *token, unsigned int flags) {
  tree value, type;
  ...
  if (itk == itk_none)
    ...
    type = ((flags & CPP_N_UNSIGNED) ? widest_unsigned_literal_type_node
                                     : widest_integer_literal_type_node);
  else
    type = integer_types[itk];

  if (itk > itk_unsigned_long && (flags & CPP_N_WIDTH) != CPP_N_LARGE &&
...
/* Interpret TOKEN, a floating point number with FLAGS as classified
   by cpplib.  */
static tree interpret_float(const cpp_token *token, unsigned int flags) {
  tree type;
  tree value;
  ...
  if (REAL_VALUE_ISINF(real)) {
    if (!MODE_HAS_INFINITIES(TYPE_MODE(type)))
      pedwarn(input_location, 0, "floating constant exceeds range of %qT",
              type);
    else
...
static tree interpret_fixed(const cpp_token *token, unsigned int flags) {
  tree type;
  tree value;
...
static enum cpp_ttype lex_string(const cpp_token *tok, tree *valp,
                                 bool objc_string, bool translate) {
  tree value;
  size_t concats = 0;
...
retry:
  ...
  if ((translate ? cpp_interpret_string : cpp_interpret_string_notranslate)(
    ...
    switch (type) {
      ...
      case CPP_STRING16:
        value = build_string(
            TYPE_PRECISION(char16_type_node) / TYPE_PRECISION(char_type_node),
            "\0"); /* char16_t is 16 bits */
        break;
      case CPP_STRING32:
        value = build_string(
            TYPE_PRECISION(char32_type_node) / TYPE_PRECISION(char_type_node),
            "\0\0\0"); /* char32_t is 32 bits */
        break;
      case CPP_WSTRING:
        value = build_string(
            TYPE_PRECISION(wchar_type_node) / TYPE_PRECISION(char_type_node),
...
static tree lex_charconst(const cpp_token *token) {
  cppchar_t result;
  tree type, value;
