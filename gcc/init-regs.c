    ...
    FOR_BB_INSNS(bb, insn) {
      ...
      for (use_rec = DF_INSN_UID_USES(uid); *use_rec; use_rec++) {
        ...
        if (bitmap_bit_p(lr, regno) && (!bitmap_bit_p(ur, regno))) {
          ...
          if (dump_file)
            fprintf(dump_file,
                    "adding initialization in %s of reg %d at in block %d for "
                    "insn %d.\n",
...
static bool gate_initialize_regs(void) {
  return optimize > 0;
}

static unsigned int rest_of_handle_initialize_regs(void) {
  initialize_uninitialized_regs();
  return 0;
}

struct rtl_opt_pass pass_initialize_regs = {{
    RTL_PASS, "init-regs",          /* name */
