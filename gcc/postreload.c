...
static int reload_cse_simplify_set(rtx set, rtx insn) {
  ...
  if (MEM_P(src) && GET_MODE_BITSIZE(GET_MODE(src)) < BITS_PER_WORD &&
  ...
  if (MEM_P(src))
    ...
    old_cost =
        REGISTER_MOVE_COST(GET_MODE(src), REGNO_REG_CLASS(REGNO(src)), dclass);
  else
  ...
  for (l = val->locs; l; l = l->next) {
...
#endif
        this_cost = REGISTER_MOVE_COST(
            GET_MODE(this_rtx), REGNO_REG_CLASS(REGNO(this_rtx)), dclass);
    } else
      continue;
    ...
    if (this_cost < old_cost ||
        (this_cost == old_cost && REG_P(this_rtx) && !REG_P(SET_SRC(set)))) {
#ifdef LOAD_EXTEND_OP
      if (GET_MODE_BITSIZE(GET_MODE(SET_DEST(set))) < BITS_PER_WORD &&
          extend_op != UNKNOWN
#ifdef CANNOT_CHANGE_MODE_CLASS
          && !CANNOT_CHANGE_MODE_CLASS(GET_MODE(SET_DEST(set)), word_mode,
...
static int reload_cse_simplify_operands(rtx insn, rtx testreg) {
  ...
  for (i = 0; i < recog_data.n_operands; i++) {
    ...
    mode = GET_MODE(op);
#ifdef LOAD_EXTEND_OP
    if (MEM_P(op) && GET_MODE_BITSIZE(mode) < BITS_PER_WORD &&
      ...
      else if (REG_P(SET_DEST(set)) && recog_data.n_operands == 2 &&
               SET_SRC(set) == op &&
               SET_DEST(set) == recog_data.operand[1 - i]) {
        validate_change(insn, recog_data.operand_loc[i],
                        gen_rtx_fmt_e(LOAD_EXTEND_OP(mode), word_mode, op), 1);
        validate_change(insn, recog_data.operand_loc[1 - i],
                        gen_rtx_REG(word_mode, REGNO(SET_DEST(set))), 1);
...
static void reload_combine(void) {
  ...
  for (insn = get_last_insn(); insn; insn = PREV_INSN(insn)) {
    ...
    if (set != NULL_RTX && REG_P(SET_DEST(set)) &&
        (hard_regno_nregs[REGNO(SET_DEST(set))][GET_MODE(SET_DEST(set))] ==
         1) &&
        GET_CODE(SET_SRC(set)) == PLUS && REG_P(XEXP(SET_SRC(set), 1)) &&
      ...
      /* Check that PREV_SET is indeed (set (REGX) (CONST_INT)) and that
         ...
         create.  */
      if (prev_set != 0 && GET_CODE(SET_SRC(prev_set)) == CONST_INT &&
    ...
    } else if (JUMP_P(insn) && GET_CODE(PATTERN(insn)) != RETURN) {
...
static void reload_combine_note_store(rtx dst, const_rtx set,
  ...
  if (GET_CODE(dst) == SUBREG) {
    regno =
        subreg_regno_offset(REGNO(SUBREG_REG(dst)), GET_MODE(SUBREG_REG(dst)),
...
static void reload_cse_move2add(rtx first) {
  ...
  for (insn = first; insn; insn = NEXT_INSN(insn), move2add_luid++) {
      ...
      if (reg_set_luid[regno] > move2add_last_label_luid &&
        ...
        */
        ...
        if (GET_CODE(src) == CONST_INT && reg_base_reg[regno] < 0) {
          rtx new_src =
              gen_int_mode(INTVAL(src) - reg_offset[regno], GET_MODE(reg));
          bool speed = optimize_bb_for_speed_p(BLOCK_FOR_INSN(insn));
          ...
          if (new_src == const0_rtx) {
            ...
            for (narrow_mode = GET_CLASS_NARROWEST_MODE(MODE_INT);
                 narrow_mode != VOIDmode && narrow_mode != GET_MODE(reg);
                 narrow_mode = GET_MODE_WIDER_MODE(narrow_mode)) {
              if (have_insn_for(STRICT_LOW_PART, narrow_mode) &&
                  ((reg_offset[regno] & ~GET_MODE_MASK(narrow_mode)) ==
                   (INTVAL(src) & ~GET_MODE_MASK(narrow_mode)))) {
                rtx narrow_reg = gen_rtx_REG(narrow_mode, REGNO(reg));
                rtx narrow_src = gen_int_mode(INTVAL(src), narrow_mode);
                rtx new_set = gen_rtx_SET(
                    VOIDmode, gen_rtx_STRICT_LOW_PART(VOIDmode, narrow_reg),
                    narrow_src);
                if (validate_change(insn, &PATTERN(insn), new_set, 0))
                  break;
              }
            }
          }
        ...
        /* Try to transform (set (REGX) (REGY))
           ...
           to
                 ...
                 MODES_OK_FOR_MOVE2ADD(GET_MODE(reg), reg_mode[REGNO(src)])) {
          rtx next = next_nonnote_insn(insn);
          rtx set = NULL_RTX;
          if (next)
            set = single_set(next);
          if (set && SET_DEST(set) == reg && GET_CODE(SET_SRC(set)) == PLUS &&
            ...
            rtx new_src = gen_int_mode(
                added_offset + base_offset - regno_offset, GET_MODE(reg));
            bool success = false;
            bool speed = optimize_bb_for_speed_p(BLOCK_FOR_INSN(insn));
            ...
            if (new_src == const0_rtx)
              ...
              success = validate_change(next, &SET_SRC(set), reg, 0);
                     ...
                     have_add2_insn(reg, new_src)) {
              rtx newpat = gen_rtx_SET(
                  VOIDmode, reg, gen_rtx_PLUS(GET_MODE(reg), reg, new_src));
              success = validate_change(next, &PATTERN(next), newpat, 0);
            }
            if (success)
              delete_insn(insn);
            insn = next;
            reg_mode[regno] = GET_MODE(reg);
            reg_offset[regno] =
                trunc_int_for_mode(added_offset + base_offset, GET_MODE(reg));
            continue;
          }
        }
      }
    ...
    if (any_condjump_p(insn)) {
      ...
      if (cnd != NULL_RTX && GET_CODE(cnd) == NE && REG_P(XEXP(cnd, 0)) &&
        ...
        rtx implicit_set = gen_rtx_SET(VOIDmode, XEXP(cnd, 0), XEXP(cnd, 1));
...
static void move2add_note_store(rtx dst, const_rtx set,
                                void *data ATTRIBUTE_UNUSED) {
  ...
  if (SCALAR_INT_MODE_P(GET_MODE(dst)) && nregs == 1 && GET_CODE(set) == SET &&
        ...
        if (REG_P(XEXP(src, 0))) {
          ...
          if (GET_CODE(XEXP(src, 1)) == CONST_INT)
            offset = INTVAL(XEXP(src, 1));
                   ...
                   (MODES_OK_FOR_MOVE2ADD(dst_mode,
                                          reg_mode[REGNO(XEXP(src, 1))]))) {
            if (reg_base_reg[REGNO(XEXP(src, 1))] < 0)
              offset = reg_offset[REGNO(XEXP(src, 1))];
            /* Maybe the first register is known to be a
               constant.  */
            else if (reg_set_luid[REGNO(base_reg)] > move2add_last_label_luid &&
                     (MODES_OK_FOR_MOVE2ADD(dst_mode,
                                            reg_mode[REGNO(XEXP(src, 1))])) &&
    ...
    reg_offset[regno] =
        trunc_int_for_mode(offset + reg_offset[base_regno], dst_mode);
  } else {
