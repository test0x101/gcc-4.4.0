...
vn_ssa_aux_t VN_INFO(tree name) {
  vn_ssa_aux_t res =
      VEC_index(vn_ssa_aux_t, vn_ssa_aux_table, SSA_NAME_VERSION(name));
  gcc_assert(res);
  return res;
}
...
static inline void VN_INFO_SET(tree name, vn_ssa_aux_t value) {
  VEC_replace(vn_ssa_aux_t, vn_ssa_aux_table, SSA_NAME_VERSION(name), value);
}

/* Initialize the value numbering info for a given SSA name.
   This should be called just once for every SSA name.  */

vn_ssa_aux_t VN_INFO_GET(tree name) {
  ...
  VEC_replace(vn_ssa_aux_t, vn_ssa_aux_table, SSA_NAME_VERSION(name), newinfo);
  return newinfo;
}
...
tree vn_get_expr_for(tree name) {
  ...
  /* If the value number is a default-definition or a PHI result
     use it directly.  */
  if (gimple_nop_p(def_stmt) || gimple_code(def_stmt) == GIMPLE_PHI)
    return vn->valnum;
    ...
    case tcc_unary:
      expr =
          fold_build1(gimple_assign_rhs_code(def_stmt),
                      gimple_expr_type(def_stmt), gimple_assign_rhs1(def_stmt));
      break;

    case tcc_binary:
      expr = fold_build2(
          gimple_assign_rhs_code(def_stmt), gimple_expr_type(def_stmt),
...
static void free_reference(void *vp) {
...
static int vn_constant_eq(const void *p1, const void *p2) {
...
static hashval_t vn_constant_hash(const void *p1) {
  const struct vn_constant_s *vc1 = (const struct vn_constant_s *)p1;
  return vc1->hashcode;
}
...
unsigned int get_constant_value_id(tree constant) {
  ...
  slot = htab_find_slot_with_hash(constant_to_value_id, &vc, vc.hashcode,
                                  NO_INSERT);
  if (slot)
...
unsigned int get_or_alloc_constant_value_id(tree constant) {
  ...
  vc->constant = constant;
  slot =
      htab_find_slot_with_hash(constant_to_value_id, vc, vc->hashcode, INSERT);
  if (*slot) {
    free(vc);
    return ((vn_constant_t)*slot)->value_id;
  }
...
static hashval_t vn_reference_op_compute_hash(const vn_reference_op_t vro1) {
  hashval_t result = 0;
  if (vro1->op0)
...
static hashval_t vn_reference_hash(const void *p1) {
...
int vn_reference_eq(const void *p1, const void *p2) {
  tree v;
  int i;
  vn_reference_op_t vro;
  ...
  if (vr1->vuses == vr2->vuses && vr1->operands == vr2->operands)
    return true;
...
static inline void vuses_to_vec(gimple stmt, VEC(tree, gc) * *result) {
  ssa_op_iter iter;
  tree vuse;
...
static VEC(tree, gc) * copy_vuses_from_stmt(gimple stmt) {
...
static inline void vdefs_to_vec(gimple stmt, VEC(tree, gc) * *result) {
  ssa_op_iter iter;
  tree vdef;
...
static VEC(tree, gc) * copy_vdefs_from_stmt(gimple stmt) {
...
VEC(tree, gc) * shared_vuses_from_stmt(gimple stmt) {
...
void copy_reference_ops_from_ref(tree ref,
  ...
  while (ref) {
    ...
    switch (temp.opcode) {
      ...
      case COMPONENT_REF:
        ...
        /* If this is a reference to a union member, record the union
           ...
           confused with this.  */
        if (may_insert && TREE_OPERAND(ref, 2) == NULL_TREE &&
    ...
    if (REFERENCE_CLASS_P(ref) ||
        (TREE_CODE(ref) == ADDR_EXPR && !is_gimple_min_invariant(ref)))
...
static tree get_ref_from_reference_ops(VEC(vn_reference_op_s, heap) * ops) {
  ...
  for (i = 0; VEC_iterate(vn_reference_op_s, ops, i, op); ++i) {
    switch (op->opcode) {
      ...
      case MISALIGNED_INDIRECT_REF:
        *op0_p = build2(MISALIGNED_INDIRECT_REF, op->type, NULL_TREE, op->op0);
        op0_p = &TREE_OPERAND(*op0_p, 0);
        break;

      case BIT_FIELD_REF:
        *op0_p = build3(BIT_FIELD_REF, op->type, NULL_TREE, op->op0, op->op1);
        op0_p = &TREE_OPERAND(*op0_p, 0);
        break;

      case COMPONENT_REF:
        *op0_p = build3(COMPONENT_REF, TREE_TYPE(op->op0), NULL_TREE, op->op0,
                        op->op1);
        op0_p = &TREE_OPERAND(*op0_p, 0);
        break;
      ...
      case ARRAY_REF:
        *op0_p =
            build4(op->opcode, op->type, NULL_TREE, op->op0, op->op1, op->op2);
        op0_p = &TREE_OPERAND(*op0_p, 0);
        break;
...
static VEC(vn_reference_op_s, heap) * create_reference_ops_from_ref(tree ref) {
...
static VEC(vn_reference_op_s, heap) * shared_reference_ops_from_ref(tree ref) {
  if (!ref)
    return NULL;
...
static VEC(tree, gc) * valueize_vuses(VEC(tree, gc) * orig) {
...
static gimple get_def_ref_stmt_vuses(tree ref, VEC(tree, gc) * vuses) {
  gimple def_stmt;
  tree vuse;
  unsigned int i;
...
cont:
  while (def_stmt && is_gimple_assign(def_stmt) &&
...
static tree vn_reference_lookup_1(vn_reference_t vr, vn_reference_t *vnresult) {
  ...
  hash = vr->hashcode;
  slot =
      htab_find_slot_with_hash(current_info->references, vr, hash, NO_INSERT);
  if (!slot && current_info == optimistic_info)
    slot =
        htab_find_slot_with_hash(valid_info->references, vr, hash, NO_INSERT);
  if (slot) {
    if (vnresult)
...
tree vn_reference_lookup_pieces(VEC(tree, gc) * vuses,
  ...
  if (!result && maywalk && vr1.vuses && VEC_length(tree, vr1.vuses) >= 1) {
    tree ref = get_ref_from_reference_ops(operands);
    gimple def_stmt;
    if (ref && (def_stmt = get_def_ref_stmt_vuses(ref, vr1.vuses)) &&
...
tree vn_reference_lookup(tree op, VEC(tree, gc) * vuses, bool maywalk,
...
vn_reference_t vn_reference_insert(tree op, tree result,
                                   VEC(tree, gc) * vuses) {
  void **slot;
  vn_reference_t vr1;
...
vn_reference_t vn_reference_insert_pieces(VEC(tree, gc) * vuses,
...
inline hashval_t vn_nary_op_compute_hash(const vn_nary_op_t vno1) {
  hashval_t hash = 0;
  unsigned i;
  ...
  if (vno1->length == 2 && commutative_tree_code(vno1->opcode) &&
...
static hashval_t vn_nary_op_hash(const void *p1) {
...
int vn_nary_op_eq(const void *p1, const void *p2) {
  const_vn_nary_op_t const vno1 = (const_vn_nary_op_t)p1;
...
tree vn_nary_op_lookup_pieces(unsigned int length, enum tree_code code,
                              tree type, tree op0, tree op1, tree op2, tree op3,
...
tree vn_nary_op_lookup(tree op, vn_nary_op_t *vnresult) {
  void **slot;
  struct vn_nary_op_s vno1;
  unsigned i;
...
tree vn_nary_op_lookup_stmt(gimple stmt, vn_nary_op_t *vnresult) {
  ...
  if (vno1.opcode == REALPART_EXPR || vno1.opcode == IMAGPART_EXPR ||
...
vn_nary_op_t vn_nary_op_insert_pieces(unsigned int length, enum tree_code code,
                                      tree type, tree op0, tree op1, tree op2,
                                      tree op3, tree result,
                                      unsigned int value_id) {
  void **slot;
  vn_nary_op_t vno1;

  vno1 = (vn_nary_op_t)obstack_alloc(
      &current_info->nary_obstack,
      (sizeof(struct vn_nary_op_s) - sizeof(tree) * (4 - length)));
...
vn_nary_op_t vn_nary_op_insert(tree op, tree result) {
...
vn_nary_op_t vn_nary_op_insert_stmt(gimple stmt, tree result) {
...
static inline hashval_t vn_phi_compute_hash(vn_phi_t vp1) {
  hashval_t result = 0;
  int i;
  tree phi1op;
  tree type;
...
static hashval_t vn_phi_hash(const void *p1) {
...
static int vn_phi_eq(const void *p1, const void *p2) {
  const_vn_phi_t const vp1 = (const_vn_phi_t)p1;
...
static vn_phi_t vn_phi_insert(gimple phi, tree result) {
  void **slot;
  ...
  slot =
      htab_find_slot_with_hash(current_info->phis, vp1, vp1->hashcode, INSERT);
...
static void print_scc(FILE *out, VEC(tree, heap) * scc) {
  tree var;
  unsigned int i;
...
static inline bool set_ssa_val_to(tree from, tree to) {
  tree currval;

  if (from != to && TREE_CODE(to) == SSA_NAME &&
      SSA_NAME_OCCURS_IN_ABNORMAL_PHI(to))
    to = from;

  /* The only thing we allow as value numbers are VN_TOP, ssa_names
     and invariants.  So assert that here.  */
  gcc_assert(to != NULL_TREE && (to == VN_TOP || TREE_CODE(to) == SSA_NAME ||
...
static bool visit_copy(tree lhs, tree rhs) {
  ...
  while (TREE_CODE(rhs) == SSA_NAME && SSA_VAL(rhs) != rhs)
    rhs = SSA_VAL(rhs);
...
static bool visit_unary_op(tree lhs, gimple stmt) {
  bool changed = false;
...
static bool visit_binary_op(tree lhs, gimple stmt) {
  bool changed = false;
  tree result = vn_nary_op_lookup_stmt(stmt, NULL);
...
static bool visit_reference_op_call(tree lhs, gimple stmt) {
  bool changed = false;
  struct vn_reference_s vr1;
  tree result;
  ...
  if (result) {
    changed = set_ssa_val_to(lhs, result);
    if (TREE_CODE(result) == SSA_NAME && VN_INFO(result)->has_constants)
    ...
    vr2->result = lhs;
    slot = htab_find_slot_with_hash(current_info->references, vr2,
                                    vr2->hashcode, INSERT);
    if (*slot)
      free_reference(*slot);
    *slot = vr2;
...
static bool visit_reference_op_load(tree lhs, tree op, gimple stmt) {
  bool changed = false;
  tree result =
      vn_reference_lookup(op, shared_vuses_from_stmt(stmt), true, NULL);

  /* We handle type-punning through unions by value-numbering based
     on offset and size of the access.  Be prepared to handle a
     type-mismatch here via creating a VIEW_CONVERT_EXPR.  */
  if (result && !useless_type_conversion_p(TREE_TYPE(result), TREE_TYPE(op))) {
    /* We will be setting the value number of lhs to the value number
       of VIEW_CONVERT_EXPR <TREE_TYPE (result)> (result).
       So first simplify and lookup this expression to see if it
       is already available.  */
    tree val = fold_build1(VIEW_CONVERT_EXPR, TREE_TYPE(op), result);
    if ((CONVERT_EXPR_P(val) || TREE_CODE(val) == VIEW_CONVERT_EXPR) &&
        TREE_CODE(TREE_OPERAND(val, 0)) == SSA_NAME) {
      tree tem = valueize_expr(vn_get_expr_for(TREE_OPERAND(val, 0)));
      if ((CONVERT_EXPR_P(tem) || TREE_CODE(tem) == VIEW_CONVERT_EXPR) &&
          (tem =
               fold_unary_ignore_overflow(TREE_CODE(val), TREE_TYPE(val), tem)))
        val = tem;
    }
    result = val;
    if (!is_gimple_min_invariant(val) && TREE_CODE(val) != SSA_NAME)
...
static bool visit_reference_op_store(tree lhs, tree op, gimple stmt) {
  ...
  result = vn_reference_lookup(lhs, shared_vuses_from_stmt(stmt), false, NULL);
  ...
  if (!result || !resultsame) {
    ...
    if (is_gimple_min_invariant(op) || is_gimple_reg(op))
...
static bool visit_phi(gimple phi) {
  bool changed = false;
  tree result;
  tree sameval = VN_TOP;
  bool allsame = true;
...
static bool stmt_has_constants(gimple stmt) {
...
static tree simplify_binary_expression(gimple stmt) {
  ...
  if (op0 == gimple_assign_rhs1(stmt) && op1 == gimple_assign_rhs2(stmt))
    return NULL_TREE;
  ...
  fold_undefer_overflow_warnings(result && valid_gimple_rhs_p(result), stmt, 0);
...
static tree simplify_unary_expression(gimple stmt) {
  tree result = NULL_TREE;
  ...
  if (VN_INFO(op0)->has_constants)
    ...
    if (UNARY_CLASS_P(tem) || BINARY_CLASS_P(tem) ||
        TREE_CODE(tem) == VIEW_CONVERT_EXPR || TREE_CODE(tem) == SSA_NAME ||
...
static bool visit_use(tree use) {
  bool changed = false;
  ...
  if (SSA_NAME_IS_DEFAULT_DEF(use))
    ...
    if (gimple_code(stmt) == GIMPLE_PHI)
    ...
    else if (!gimple_has_lhs(stmt) || gimple_has_volatile_ops(stmt) ||
      ...
      } else if (stmt_has_constants(stmt) && TREE_CODE(lhs) == SSA_NAME)
        VN_INFO(lhs)->has_constants = true;
      ...
      if ((TREE_CODE(lhs) == SSA_NAME
           ...
           !(simplified && is_gimple_min_invariant(simplified)) &&
      ...
      if (stmt_has_constants(stmt) && TREE_CODE(lhs) == SSA_NAME)
        VN_INFO(lhs)->has_constants = true;
      else if (TREE_CODE(lhs) == SSA_NAME) {
        /* We reset expr and constantness here because we may
           have been value numbering optimistically, and
      ...
      if (TREE_CODE(lhs) == SSA_NAME && SSA_NAME_OCCURS_IN_ABNORMAL_PHI(lhs))
        changed = defs_to_varying(stmt);
...
static int compare_ops(const void *pa, const void *pb) {
...
static void sort_scc(VEC(tree, heap) * scc) {
  qsort(VEC_address(tree, scc), VEC_length(tree, scc), sizeof(tree),
        compare_ops);
}

/* Process a strongly connected component in the SSA graph.  */

static void process_scc(VEC(tree, heap) * scc) {
...
static bool extract_and_process_scc_for_name(tree name) {
  ...
  if (VEC_length(tree, scc) > (unsigned)PARAM_VALUE(PARAM_SCCVN_MAX_SCC_SIZE)) {
    if (dump_file)
      fprintf(dump_file,
...
start_over:
  ...
  while (1) {
    ...
    if (op_iter_done(&iter)) {
      ...
      memcpy(&iter, VEC_last(ssa_op_iter, itervec), sizeof(ssa_op_iter));
    ...
    if (TREE_CODE(use) == SSA_NAME) {
      ...
      continue_walking:
        VN_INFO(name)->low = MIN(VN_INFO(name)->low, VN_INFO(use)->low);
      }
      if (VN_INFO(use)->dfsnum < VN_INFO(name)->dfsnum &&
          VN_INFO(use)->on_sccstack) {
        VN_INFO(name)->low = MIN(VN_INFO(use)->dfsnum, VN_INFO(name)->low);
      }
    }
...
static void allocate_vn_table(vn_tables_t table) {
  ...
  table->references =
      htab_create(23, vn_reference_hash, vn_reference_eq, free_reference);

  gcc_obstack_init(&table->nary_obstack);
  table->phis_pool = create_alloc_pool("VN phis", sizeof(struct vn_phi_s), 30);
  table->references_pool =
      create_alloc_pool("VN references", sizeof(struct vn_reference_s), 30);
}
...
static void init_scc_vn(void) {
  ...
  constant_to_value_id =
      htab_create(23, vn_constant_hash, vn_constant_eq, free);
...
void free_scc_vn(void) {
  ...
  for (i = 0; i < num_ssa_names; i++) {
    tree name = ssa_name(i);
    if (name && VN_INFO(name)->needs_insertion)
...
static void set_hashtable_value_ids(void) {
  ...
  FOR_EACH_HTAB_ELEMENT(valid_info->nary, vno, vn_nary_op_t, hi) {
    if (vno->result) {
  ...
  FOR_EACH_HTAB_ELEMENT(valid_info->phis, vp, vn_phi_t, hi) {
    if (vp->result) {
  ...
  FOR_EACH_HTAB_ELEMENT(valid_info->references, vr, vn_reference_t, hi) {
    if (vr->result) {
...
bool run_scc_vn(bool may_insert_arg) {
  size_t i;
  tree param;
  bool changed = true;
  ...
  for (i = 1; i < num_ssa_names; ++i) {
    tree name = ssa_name(i);
    if (name && VN_INFO(name)->visited == false && !has_zero_uses(name))
      if (!DFS(name)) {
        free_scc_vn();
        may_insert = false;
        return false;
  ...
  while (changed) {
    ...
    for (i = 1; i < num_ssa_names; ++i) {
      ...
      if (!name)
        continue;
      info = VN_INFO(name);
      if (TREE_CODE(info->valnum) == SSA_NAME && info->valnum != name &&
  ...
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    ...
    for (i = 0; i < num_ssa_names; i++) {
      tree name = ssa_name(i);
      if (name && VN_INFO(name)->visited && SSA_VAL(name) != name) {
...
bool expressions_equal_p(tree e1, tree e2) {
  ...
  if (TREE_CODE(e1) == TREE_CODE(e2) && operand_equal_p(e1, e2, OEP_PURE_SAME))
    return true;
...
void sort_vuses(VEC(tree, gc) * vuses) {
  if (VEC_length(tree, vuses) > 1)
    qsort(VEC_address(tree, vuses), VEC_length(tree, vuses), sizeof(tree),
          operand_build_cmp);
}
...
void sort_vuses_heap(VEC(tree, heap) * vuses) {
  if (VEC_length(tree, vuses) > 1)
    qsort(VEC_address(tree, vuses), VEC_length(tree, vuses), sizeof(tree),
          operand_build_cmp);
}
...
bool vn_nary_may_trap(vn_nary_op_t nary) {
  ...
  rhs2 = nary->op[1];
  ret = operation_could_trap_helper_p(nary->opcode, fp_operation, honor_trapv,
                                      honor_nans, honor_snans, rhs2, &handled);
  if (handled && ret)
    return true;
