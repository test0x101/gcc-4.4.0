...
static tree create_tmp_var_for(struct nesting_info *info, tree type,
                               const char *prefix) {
  tree tmp_var;
...
tree build_addr(tree exp, tree context) {
  tree base = exp;
  tree save_context;
  tree retval;
...
static tree get_frame_type(struct nesting_info *info) {
  ...
  if (!type) {
    ...
    name = concat("FRAME.", IDENTIFIER_POINTER(DECL_NAME(info->context)), NULL);
...
static tree lookup_field_for_decl(struct nesting_info *info, tree decl,
...
static tree init_tmp_var_with_call(struct nesting_info *info,
                                   gimple_stmt_iterator *gsi, gimple call) {
  tree t;
...
static tree init_tmp_var(struct nesting_info *info, tree exp,
...
static tree gsi_gimplify_val(struct nesting_info *info, tree exp,
                             gimple_stmt_iterator *gsi) {
...
static tree save_tmp_var(struct nesting_info *info, tree exp,
                         gimple_stmt_iterator *gsi) {
  tree t;
  gimple stmt;
...
static tree get_trampoline_type(void) {
  unsigned align, size;
  tree t;
...
static tree lookup_tramp_for_decl(struct nesting_info *info, tree decl,
                                  enum insert_option insert) {
  void **slot;
...
static void walk_body(walk_stmt_fn callback_stmt, walk_tree_fn callback_op,
...
static inline void walk_function(walk_stmt_fn callback_stmt,
...
static void walk_gimple_omp_for(gimple for_stmt, walk_stmt_fn callback_stmt,
                                walk_tree_fn callback_op,
                                struct nesting_info *info) {
  struct walk_stmt_info wi;
  gimple_seq seq;
  ...
  walk_body(callback_stmt, callback_op, info,
  ...
  for (i = 0; i < gimple_omp_for_collapse(for_stmt); i++) {
    wi.val_only = false;
    walk_tree(gimple_omp_for_index_ptr(for_stmt, i), callback_op, &wi, NULL);
    wi.val_only = true;
    wi.is_lhs = false;
    walk_tree(gimple_omp_for_initial_ptr(for_stmt, i), callback_op, &wi, NULL);

    wi.val_only = true;
    wi.is_lhs = false;
    walk_tree(gimple_omp_for_final_ptr(for_stmt, i), callback_op, &wi, NULL);
...
static struct nesting_info *create_nesting_tree(struct cgraph_node *cgn) {
...
static tree get_static_chain(struct nesting_info *info, tree target_context,
                             gimple_stmt_iterator *gsi) {
...
static tree get_frame_field(struct nesting_info *info, tree target_context,
...
static tree convert_nonlocal_reference_op(tree *tp, int *walk_subtrees,
                                          void *data) {
  ...
  switch (TREE_CODE(t)) {
    ...
    case ADDR_EXPR: {
      ...
      if (wi->changed) {
        ...
        if (save_val_only)
          *tp = gsi_gimplify_val((struct nesting_info *)wi->info, t, &wi->gsi);
      }
    } break;
    ...
    case BIT_FIELD_REF:
      ...
      for (; handled_component_p(t); tp = &TREE_OPERAND(t, 0), t = *tp) {
        if (TREE_CODE(t) == COMPONENT_REF)
          ...
          walk_tree(&TREE_OPERAND(t, 1), convert_nonlocal_reference_op, wi,
                    NULL);
          walk_tree(&TREE_OPERAND(t, 2), convert_nonlocal_reference_op, wi,
                    NULL);
          walk_tree(&TREE_OPERAND(t, 3), convert_nonlocal_reference_op, wi,
                    NULL);
  ...
  for (clause = *pclauses; clause; clause = OMP_CLAUSE_CHAIN(clause)) {
    switch (OMP_CLAUSE_CODE(clause)) {
      ...
      case OMP_CLAUSE_NUM_THREADS:
        wi->val_only = true;
        wi->is_lhs = false;
        convert_nonlocal_reference_op(&OMP_CLAUSE_OPERAND(clause, 0), &dummy,
                                      wi);
        break;
...
static tree get_local_debug_decl(struct nesting_info *info, tree decl,
                                 tree field) {
  tree x, new_decl;
  void **slot;
...
static tree convert_local_reference_op(tree *tp, int *walk_subtrees,
                                       void *data) {
  struct walk_stmt_info *wi = (struct walk_stmt_info *)data;
  struct nesting_info *const info = (struct nesting_info *)wi->info;
  tree t = *tp, field, x;
  ...
  *walk_subtrees = 0;
  switch (TREE_CODE(t)) {
    ...
    case BIT_FIELD_REF:
      ...
      for (; handled_component_p(t); tp = &TREE_OPERAND(t, 0), t = *tp) {
        if (TREE_CODE(t) == COMPONENT_REF)
          walk_tree(&TREE_OPERAND(t, 2), convert_local_reference_op, wi, NULL);
        else if (TREE_CODE(t) == ARRAY_REF || TREE_CODE(t) == ARRAY_RANGE_REF) {
          walk_tree(&TREE_OPERAND(t, 1), convert_local_reference_op, wi, NULL);
          walk_tree(&TREE_OPERAND(t, 2), convert_local_reference_op, wi, NULL);
          walk_tree(&TREE_OPERAND(t, 3), convert_local_reference_op, wi, NULL);
        } else if (TREE_CODE(t) == BIT_FIELD_REF) {
          walk_tree(&TREE_OPERAND(t, 1), convert_local_reference_op, wi, NULL);
          walk_tree(&TREE_OPERAND(t, 2), convert_local_reference_op, wi, NULL);
        }
    ...
    switch (OMP_CLAUSE_CODE(clause)) {
      ...
      do_decl_clause:
        ...
        if (decl_function_context(decl) == info->context &&
          ...
          if (field) {
            bitmap_set_bit(new_suppress, DECL_UID(decl));
            OMP_CLAUSE_DECL(clause) = get_local_debug_decl(info, decl, field);
            need_frame = true;
          }
        }
        break;
      ...
      case OMP_CLAUSE_NUM_THREADS:
        wi->val_only = true;
        wi->is_lhs = false;
        convert_local_reference_op(&OMP_CLAUSE_OPERAND(clause, 0), &dummy, wi);
        break;
  ...
  if (need_stmts)
    for (clause = *pclauses; clause; clause = OMP_CLAUSE_CHAIN(clause))
      switch (OMP_CLAUSE_CODE(clause)) {
        case OMP_CLAUSE_REDUCTION:
          if (OMP_CLAUSE_REDUCTION_PLACEHOLDER(clause)) {
            ...
            walk_body(convert_local_reference_stmt, convert_local_reference_op,
                      info, OMP_CLAUSE_REDUCTION_GIMPLE_INIT(clause));
            walk_body(convert_local_reference_stmt, convert_local_reference_op,
                      info, OMP_CLAUSE_REDUCTION_GIMPLE_MERGE(clause));
            DECL_CONTEXT(OMP_CLAUSE_REDUCTION_PLACEHOLDER(clause)) =
                old_context;
          }
          break;
        ...
        case OMP_CLAUSE_LASTPRIVATE:
          walk_body(convert_local_reference_stmt, convert_local_reference_op,
                    info, OMP_CLAUSE_LASTPRIVATE_GIMPLE_SEQ(clause));
          break;
...
static tree convert_local_reference_stmt(gimple_stmt_iterator *gsi,
  ...
  switch (gimple_code(stmt)) {
    ...
    case GIMPLE_OMP_TASK:
      save_suppress = info->suppress_expansion;
      if (convert_local_omp_clauses(gimple_omp_taskreg_clauses_ptr(stmt), wi)) {
        tree c;
        (void)get_frame_type(info);
...
static tree convert_tramp_reference_op(tree *tp, int *walk_subtrees,
                                       void *data) {
  struct walk_stmt_info *wi = (struct walk_stmt_info *)data;
  struct nesting_info *const info = (struct nesting_info *)wi->info, *i;
...
static tree convert_tramp_reference_stmt(gimple_stmt_iterator *gsi,
  ...
  switch (gimple_code(stmt)) {
    case GIMPLE_CALL: {
      ...
      for (i = 0; i < nargs; i++)
        walk_tree(gimple_call_arg_ptr(stmt, i), convert_tramp_reference_op, wi,
                  NULL);
...
static tree convert_gimple_call(gimple_stmt_iterator *gsi, bool *handled_ops_p,
                                struct walk_stmt_info *wi) {
  struct nesting_info *const info = (struct nesting_info *)wi->info;
  ...
  switch (gimple_code(stmt)) {
    case GIMPLE_CALL:
      ...
      if (target_context && !DECL_NO_STATIC_CHAIN(decl)) {
        gimple_call_set_chain(stmt,
                              get_static_chain(info, target_context, &wi->gsi));
    ...
    case GIMPLE_OMP_TASK:
      ...
      for (i = 0; i < 2; i++) {
        ...
        for (c = gimple_omp_taskreg_clauses(stmt); c; c = OMP_CLAUSE_CHAIN(c))
          if ((OMP_CLAUSE_CODE(c) == OMP_CLAUSE_FIRSTPRIVATE ||
               OMP_CLAUSE_CODE(c) == OMP_CLAUSE_SHARED) &&
              OMP_CLAUSE_DECL(c) == decl)
            break;
        if (c == NULL) {
          c = build_omp_clause(i ? OMP_CLAUSE_FIRSTPRIVATE : OMP_CLAUSE_SHARED);
          OMP_CLAUSE_DECL(c) = decl;
          OMP_CLAUSE_CHAIN(c) = gimple_omp_taskreg_clauses(stmt);
          gimple_omp_taskreg_set_clauses(stmt, c);
        }
    ...
    case GIMPLE_OMP_FOR:
      walk_body(convert_gimple_call, NULL, info, gimple_omp_for_pre_body(stmt));
      /* FALLTHRU */
    case GIMPLE_OMP_SECTIONS:
    case GIMPLE_OMP_SECTION:
    case GIMPLE_OMP_SINGLE:
...
static void finalize_nesting_tree_1(struct nesting_info *root) {
  ...
  if (root->any_parm_remapped) {
    ...
    for (p = DECL_ARGUMENTS(context); p; p = TREE_CHAIN(p)) {
      ...
      y = build3(COMPONENT_REF, TREE_TYPE(field), root->frame_decl, field,
                 NULL_TREE);
      ...
      if (!is_gimple_reg(x) && is_gimple_reg_type(TREE_TYPE(x))) {
  ...
  if (root->any_tramp_created) {
    ...
    for (i = root->inner; i; i = i->next) {
      ...
      x = build3(COMPONENT_REF, TREE_TYPE(field), root->frame_decl, field,
                 NULL_TREE);
  ...
  if (root->new_local_var_chain)
    declare_vars(root->new_local_var_chain,
                 gimple_seq_first_stmt(gimple_body(root->context)), false);
  if (root->debug_var_chain)
    declare_vars(root->debug_var_chain,
                 gimple_seq_first_stmt(gimple_body(root->context)), true);
...
void lower_nested_functions(tree fndecl) {
  ...
  walk_all_functions(convert_nonlocal_reference_stmt,
                     convert_nonlocal_reference_op, root);
  walk_all_functions(convert_local_reference_stmt, convert_local_reference_op,
                     root);
