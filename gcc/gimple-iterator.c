...
static inline void update_modified_stmt(gimple stmt) {
...
static void update_modified_stmts(gimple_seq seq) {
...
static void update_bb_for_stmts(gimple_seq_node first, basic_block bb) {
  gimple_seq_node n;
...
void gsi_insert_seq_before(gimple_stmt_iterator *i, gimple_seq seq,
                           enum gsi_iterator_update mode) {
...
void gsi_insert_seq_after(gimple_stmt_iterator *i, gimple_seq seq,
                          enum gsi_iterator_update mode) {
  update_modified_stmts(seq);
...
void gsi_replace(gimple_stmt_iterator *gsi, gimple stmt, bool update_eh_info) {
  int eh_region;
...
void gsi_insert_before_without_update(gimple_stmt_iterator *i, gimple stmt,
                                      enum gsi_iterator_update m) {
  gimple_seq_node n;
...
void gsi_insert_before(gimple_stmt_iterator *i, gimple stmt,
                       enum gsi_iterator_update m) {
  update_modified_stmt(stmt);
...
void gsi_insert_after_without_update(gimple_stmt_iterator *i, gimple stmt,
                                     enum gsi_iterator_update m) {
  gimple_seq_node n;
...
void gsi_insert_after(gimple_stmt_iterator *i, gimple stmt,
                      enum gsi_iterator_update m) {
  update_modified_stmt(stmt);
...
void gsi_remove(gimple_stmt_iterator *i, bool remove_permanently) {
...
restart:
  if (single_pred_p(dest) && !phi_nodes(dest) && dest != EXIT_BLOCK_PTR) {
  ...
  src = e->src;
  if ((e->flags & EDGE_ABNORMAL) == 0 && single_succ_p(src) &&
      src != ENTRY_BLOCK_PTR) {
    *gsi = gsi_last_bb(src);
    if (gsi_end_p(*gsi))
      return true;
...
basic_block gsi_insert_seq_on_edge_immediate(edge e, gimple_seq stmts) {
  gimple_stmt_iterator gsi;
  basic_block new_bb = NULL;
...
gimple_stmt_iterator gsi_start_phis(basic_block bb) {
