...
#define INDIRECT_REF_P(CODE)                                                   \
  (TREE_CODE(CODE) == INDIRECT_REF || TREE_CODE(CODE) == ALIGN_INDIRECT_REF || \
...
#define VL_EXP_CLASS_P(CODE) (TREE_CODE_CLASS(TREE_CODE(CODE)) == tcc_vl_exp)
...
#include "builtins.def"
  ...
  BUILT_IN_COMPLEX_MUL_MAX = BUILT_IN_COMPLEX_MUL_MIN + MAX_MODE_COMPLEX_FLOAT -
                             MIN_MODE_COMPLEX_FLOAT,

  BUILT_IN_COMPLEX_DIV_MIN,
  BUILT_IN_COMPLEX_DIV_MAX = BUILT_IN_COMPLEX_DIV_MIN + MAX_MODE_COMPLEX_FLOAT -
                             MIN_MODE_COMPLEX_FLOAT,
...
#define BUILTIN_EXP10_P(FN)                             \
  ((FN) == BUILT_IN_EXP10 || (FN) == BUILT_IN_EXP10F || \
   (FN) == BUILT_IN_EXP10L || (FN) == BUILT_IN_POW10 || \
   (FN) == BUILT_IN_POW10F || (FN) == BUILT_IN_POW10L)

#define BUILTIN_EXPONENT_P(FN)                                                 \
  (BUILTIN_EXP10_P(FN) || (FN) == BUILT_IN_EXP || (FN) == BUILT_IN_EXPF ||     \
   (FN) == BUILT_IN_EXPL || (FN) == BUILT_IN_EXP2 || (FN) == BUILT_IN_EXP2F || \
   (FN) == BUILT_IN_EXP2L)
...
#define TREE_NOT_CHECK(T, CODE)                                                \
  __extension__({                                                              \
    __typeof(T) const __t = (T);                                               \
    if (TREE_CODE(__t) == (CODE))                                              \
      tree_not_check_failed(__t, __FILE__, __LINE__, __FUNCTION__, (CODE), 0); \
    __t;                                                                       \
  })
...
#define TREE_CHECK2(T, CODE1, CODE2)                                    \
  __extension__({                                                       \
    ...
    if (TREE_CODE(__t) != (CODE1) && TREE_CODE(__t) != (CODE2))         \
      tree_check_failed(__t, __FILE__, __LINE__, __FUNCTION__, (CODE1), \
                        (CODE2), 0);                                    \
    __t;                                                                \
  })
...
#define TREE_NOT_CHECK2(T, CODE1, CODE2)                                    \
  __extension__({                                                           \
    __typeof(T) const __t = (T);                                            \
    if (TREE_CODE(__t) == (CODE1) || TREE_CODE(__t) == (CODE2))             \
      tree_not_check_failed(__t, __FILE__, __LINE__, __FUNCTION__, (CODE1), \
                            (CODE2), 0);                                    \
    __t;                                                                    \
  })
...
#define TREE_CHECK4(T, CODE1, CODE2, CODE3, CODE4)                      \
  __extension__({                                                       \
    __typeof(T) const __t = (T);                                        \
    if (TREE_CODE(__t) != (CODE1) && TREE_CODE(__t) != (CODE2) &&       \
        TREE_CODE(__t) != (CODE3) && TREE_CODE(__t) != (CODE4))         \
      tree_check_failed(__t, __FILE__, __LINE__, __FUNCTION__, (CODE1), \
                        (CODE2), (CODE3), (CODE4), 0);                  \
    __t;                                                                \
  })

#define NON_TREE_CHECK4(T, CODE1, CODE2, CODE3, CODE4)                      \
  __extension__({                                                           \
    __typeof(T) const __t = (T);                                            \
    if (TREE_CODE(__t) == (CODE1) || TREE_CODE(__t) == (CODE2) ||           \
        TREE_CODE(__t) == (CODE3) || TREE_CODE(__t) == (CODE4))             \
...
#define TREE_RANGE_CHECK(T, CODE1, CODE2)                                     \
  __extension__({                                                             \
    __typeof(T) const __t = (T);                                              \
    if (TREE_CODE(__t) < (CODE1) || TREE_CODE(__t) > (CODE2))                 \
      tree_range_check_failed(__t, __FILE__, __LINE__, __FUNCTION__, (CODE1), \
                              (CODE2));                                       \
    __t;                                                                      \
  })
...
#define OMP_CLAUSE_SUBCODE_CHECK(T, CODE)                                      \
  __extension__({                                                              \
    ...
    if (TREE_CODE(__t) != OMP_CLAUSE)                                          \
      tree_check_failed(__t, __FILE__, __LINE__, __FUNCTION__, OMP_CLAUSE, 0); \
    if (__t->omp_clause.code != (CODE))                                        \
      omp_clause_check_failed(__t, __FILE__, __LINE__, __FUNCTION__, (CODE));  \
    __t;                                                                       \
  })
...
#define OMP_CLAUSE_RANGE_CHECK(T, CODE1, CODE2)                                \
  __extension__({                                                              \
    ...
    if ((int)__t->omp_clause.code < (int)(CODE1) ||                            \
      ...
      omp_clause_range_check_failed(__t, __FILE__, __LINE__, __FUNCTION__,     \
                                    (CODE1), (CODE2));                         \
    __t;                                                                       \
  })
...
#define TREE_VEC_ELT_CHECK(T, I)                                             \
  __extension__(*({                                                          \
    ...
    if (TREE_CODE(__t) != TREE_VEC)                                          \
      tree_check_failed(__t, __FILE__, __LINE__, __FUNCTION__, TREE_VEC, 0); \
    if (__i < 0 || __i >= __t->vec.length)                                   \
      tree_vec_elt_check_failed(__i, __t->vec.length, __FILE__, __LINE__,    \
                                __FUNCTION__);                               \
    &__t->vec.a[__i];                                                        \
  }))
...
#define TREE_OPERAND_CHECK(T, I)                                             \
  __extension__(*({                                                          \
    ...
    if (__i < 0 || __i >= TREE_OPERAND_LENGTH(__t))                          \
      tree_operand_check_failed(__i, __t, __FILE__, __LINE__, __FUNCTION__); \
...
#define TREE_RTL_OPERAND_CHECK(T, CODE, I)                                   \
  __extension__(*(rtx *)({                                                   \
    __typeof(T) const __t = (T);                                             \
    const int __i = (I);                                                     \
    if (TREE_CODE(__t) != (CODE))                                            \
...
extern void omp_clause_check_failed(const_tree, const char *, int, const char *,
                                    enum omp_clause_code) ATTRIBUTE_NORETURN;
...
#define VECTOR_FLOAT_TYPE_P(TYPE) \
  (TREE_CODE(TYPE) == VECTOR_TYPE && TREE_CODE(TREE_TYPE(TYPE)) == REAL_TYPE)
...
#define PREDICT_EXPR_PREDICTOR(NODE)                                          \
  ((enum br_predictor)tree_low_cst(TREE_OPERAND(PREDICT_EXPR_CHECK(NODE), 0), \
                                   0))
...
#define MOVE_NONTEMPORAL(NODE) (EXPR_CHECK(NODE)->base.static_flag)
...
#define DECL_UNSIGNED(NODE) (DECL_COMMON_CHECK(NODE)->base.unsigned_flag)
...
#define FOR_EACH_CONSTRUCTOR_VALUE(V, IX, VAL)                                 \
  for (IX = 0; (IX >= VEC_length(constructor_elt, V))                          \
                   ? false                                                     \
                   : ((VAL = VEC_index(constructor_elt, V, IX)->value), true); \
       (IX)++)
...
#define FOR_EACH_CONSTRUCTOR_ELT(V, IX, INDEX, VAL)                      \
  for (IX = 0;                                                           \
       (IX >= VEC_length(constructor_elt, V))                            \
           ? false                                                       \
           : ((VAL = VEC_index(constructor_elt, V, IX)->value),          \
              (INDEX = VEC_index(constructor_elt, V, IX)->index), true); \
       (IX)++)
...
#define EXPR_LOCUS(NODE)                                            \
  (EXPR_P(NODE) ? CONST_CAST(source_location *, &(NODE)->exp.locus) \
...
#define TYPE_ORIG_SIZE_TYPE(NODE)                                         \
  (INTEGER_TYPE_CHECK(NODE)->type.values ? TREE_TYPE((NODE)->type.values) \
                                         : NULL_TREE)
...
#define TYPE_MODE(NODE)                                                \
  (TREE_CODE(TYPE_CHECK(NODE)) == VECTOR_TYPE ? vector_type_mode(NODE) \
                                              : (NODE)->type.mode)
#define SET_TYPE_MODE(NODE, MODE) (TYPE_CHECK(NODE)->type.mode = (MODE))
...
#define SSA_VAR_P(DECL)                                           \
  (TREE_CODE(DECL) == VAR_DECL || TREE_CODE(DECL) == PARM_DECL || \
   TREE_CODE(DECL) == RESULT_DECL || MTAG_P(DECL) ||              \
...
#define DECL_NO_TBAA_P(DECL) DECL_COMMON_CHECK(DECL)->decl_common.no_tbaa_flag
...
#define COPY_DECL_ASSEMBLER_NAME(DECL1, DECL2)                            \
  (DECL_ASSEMBLER_NAME_SET_P(DECL1)                                       \
       ? (void)SET_DECL_ASSEMBLER_NAME(DECL2, DECL_ASSEMBLER_NAME(DECL1)) \
       : (void)0)
...
struct tree_statement_list_node GTY((chain_next("%h.next"),
                                     chain_prev("%h.prev"))) {
...
struct tree_statement_list GTY(()) {
  struct tree_common common;
...
extern tree build4_stat(enum tree_code, tree, tree, tree, tree,
                        tree MEM_STAT_DECL);
#define build4(c, t1, t2, t3, t4, t5) \
  build4_stat(c, t1, t2, t3, t4, t5 MEM_STAT_INFO)
extern tree build5_stat(enum tree_code, tree, tree, tree, tree, tree,
                        tree MEM_STAT_DECL);
#define build5(c, t1, t2, t3, t4, t5, t6) \
  build5_stat(c, t1, t2, t3, t4, t5, t6 MEM_STAT_INFO)
extern tree build7_stat(enum tree_code, tree, tree, tree, tree, tree, tree,
...
extern tree build_int_cst_wide_type(tree, unsigned HOST_WIDE_INT,
                                    HOST_WIDE_INT);
...
struct attribute_spec {
  ...
  tree (*const handler)(tree *node, tree name, tree args, int flags,
                        bool *no_add_attrs);
};
...
extern tree get_inner_reference(tree, HOST_WIDE_INT *, HOST_WIDE_INT *, tree *,
                                enum machine_mode *, int *, int *, bool);
...
extern tree fold_build3_stat(enum tree_code, tree, tree, tree,
                             tree MEM_STAT_DECL);
#define fold_build3(c, t1, t2, t3, t4) \
  fold_build3_stat(c, t1, t2, t3, t4 MEM_STAT_INFO)
...
extern void lshift_double(unsigned HOST_WIDE_INT, HOST_WIDE_INT, HOST_WIDE_INT,
                          unsigned int, unsigned HOST_WIDE_INT *,
                          HOST_WIDE_INT *, int);
extern void rshift_double(unsigned HOST_WIDE_INT, HOST_WIDE_INT, HOST_WIDE_INT,
                          unsigned int, unsigned HOST_WIDE_INT *,
                          HOST_WIDE_INT *, int);
extern void lrotate_double(unsigned HOST_WIDE_INT, HOST_WIDE_INT, HOST_WIDE_INT,
                           unsigned int, unsigned HOST_WIDE_INT *,
                           HOST_WIDE_INT *);
extern void rrotate_double(unsigned HOST_WIDE_INT, HOST_WIDE_INT, HOST_WIDE_INT,
                           unsigned int, unsigned HOST_WIDE_INT *,
                           HOST_WIDE_INT *);
...
extern hashval_t iterative_hash_exprs_commutative(const_tree, const_tree,
                                                  hashval_t);
...
static inline bool more_const_call_expr_args_p(
    const const_call_expr_arg_iterator *iter) {
  return (iter->i < iter->n);
}
