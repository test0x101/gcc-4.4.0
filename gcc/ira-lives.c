...
/* The function processing birth of register REGNO.  It updates living
   ...
   necessary.  */
static void make_regno_born(int regno) {
  unsigned int i;
  ira_allocno_t a;
  ...
  if ((p = ALLOCNO_LIVE_RANGES(a)) == NULL ||
    ...
    ALLOCNO_LIVE_RANGES(a) = ira_create_allocno_live_range(
        a, curr_point, -1, ALLOCNO_LIVE_RANGES(a));
}

/* Update ALLOCNO_EXCESS_PRESSURE_POINTS_NUM for allocno A.  */
static void update_allocno_pressure_excess_length(ira_allocno_t a) {
  int start, i;
...
/* Process the death of register REGNO.  This updates hard_regs_live
   ...
   REGNO.  */
static void make_regno_dead(int regno) {
  ira_allocno_t a;
  allocno_live_range_t p;
...
/* Mark allocno A as currently living and update current register
   ...
   A.  */
static void set_allocno_live(ira_allocno_t a) {
  int i;
  enum reg_class cover_class, cl;
...
static void clear_allocno_live(ira_allocno_t a) {
  int i;
  unsigned int j;
  enum reg_class cover_class, cl;
  bool set_p;
  ...
  if (sparseset_bit_p(allocnos_live, ALLOCNO_NUM(a))) {
    ...
    if (set_p) {
      ...
      for (i = 0; (cl = ira_reg_class_super_classes[cover_class][i]) !=
                  LIM_REG_CLASSES;
           i++)
        if (high_pressure_start_point[cl] >= 0 &&
            curr_reg_pressure[cl] <= ira_available_class_regs[cl])
...
/* Mark the register REG as live.  Store a 1 in hard_regs_live or
   ...
   needs.  */
static void mark_reg_live(rtx reg) {
  int i, regno;
...
static void mark_reg_dead(rtx reg) {
  int regno;
...
static bool make_pseudo_conflict(rtx reg, enum reg_class cl, rtx dreg,
                                 bool advance_p) {
  ira_allocno_t a;
...
/* Check and make if necessary conflicts for pseudo DREG of class
   ...
   program point.  */
static bool check_and_make_def_use_conflict(rtx dreg, enum reg_class def_cl,
  ...
  advance_p =
      make_pseudo_conflict(recog_data.operand[use], use_cl, dreg, advance_p);
  ...
  if (use < recog_data.n_operands + 1 && recog_data.constraints[use][0] == '%')
    advance_p = make_pseudo_conflict(recog_data.operand[use + 1], use_cl, dreg,
                                     advance_p);
  if (use >= 1 && recog_data.constraints[use - 1][0] == '%')
    advance_p = make_pseudo_conflict(recog_data.operand[use - 1], use_cl, dreg,
                                     advance_p);
  return advance_p;
}
...
static void check_and_make_def_conflict(int alt, int def,
  ...
  for (use = 0; use < recog_data.n_operands; use++) {
    ...
    advance_p =
        check_and_make_def_use_conflict(dreg, def_cl, use, use_cl, advance_p);
...
static bool mark_hard_reg_early_clobbers(rtx insn, bool live_p) {
  df_ref *def_rec;
  bool set_p = false;
...
static enum reg_class single_reg_class(const char *constraints, rtx op,
                                       rtx equiv_const) {
  int ignore_p;
  ...
  cl = NO_REGS;
  for (ignore_p = false; (c = *constraints);
      ...
      switch (c) {
        ...
        case 'P':
          if ((GET_CODE(op) == CONST_INT &&
              ...
              (equiv_const != NULL_RTX && GET_CODE(equiv_const) == CONST_INT &&
               CONST_OK_FOR_CONSTRAINT_P(INTVAL(equiv_const), c, constraints)))
            return NO_REGS;
          break;
        ...
        case 'H':
          if ((GET_CODE(op) == CONST_DOUBLE &&
              ...
              (equiv_const != NULL_RTX &&
               GET_CODE(equiv_const) == CONST_DOUBLE &&
               CONST_DOUBLE_OK_FOR_CONSTRAINT_P(equiv_const, c, constraints)))
            return NO_REGS;
          /* ??? what about memory */
        case 'r':
        case 'a':
        ...
        case '8':
        case '9':
          next_cl = single_reg_class(recog_data.constraints[c - '0'],
...
static void process_single_reg_class_operands(bool in_p, int freq) {
  ...
  for (i = 0; i < recog_data.n_operands; i++) {
    ...
    if (REG_P(operand) && (regno = REGNO(operand)) >= FIRST_PSEUDO_REGISTER) {
      ...
      if (ira_class_subset_p[cl][cover_class] &&
          ...
          (ira_class_hard_reg_index[cover_class][ira_class_hard_regs[cl][0]]) >=
              0 &&
          reg_class_size[cl] <= (unsigned)CLASS_MAX_NREGS(cl, mode)) {
        /* ??? FREQ */
        cost = freq * (in_p ? ira_register_move_cost[mode][cover_class][cl]
                            : ira_register_move_cost[mode][cl][cover_class]);
        ira_allocate_and_set_costs(&ALLOCNO_CONFLICT_HARD_REG_COSTS(operand_a),
                                   cover_class, 0);
    ...
    EXECUTE_IF_SET_IN_SPARSESET(allocnos_live, px) {
      ...
      if (a != operand_a) {
        ...
        IOR_HARD_REG_SET(ALLOCNO_CONFLICT_HARD_REGS(a), reg_class_contents[cl]);
  ...
  if (bb != NULL) {
    ...
    for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
      if (TEST_HARD_REG_BIT(hard_regs_live, i)) {
        ...
        for (j = 0; (cl = ira_reg_class_super_classes[cover_class][j]) !=
                    LIM_REG_CLASSES;
             j++) {
          curr_reg_pressure[cl]++;
          if (curr_bb_node->reg_pressure[cl] < curr_reg_pressure[cl])
            curr_bb_node->reg_pressure[cl] = curr_reg_pressure[cl];
          ira_assert(curr_reg_pressure[cl] <= ira_available_class_regs[cl]);
        }
      }
    ...
    FOR_BB_INSNS_REVERSE(bb, insn) {
      ...
      if (internal_flag_ira_verbose > 2 && ira_dump_file != NULL)
        fprintf(ira_dump_file, "   Insn %u(l%d): point = %d\n", INSN_UID(insn),
                loop_tree_node->parent->loop->num, curr_point);
...
static void create_start_finish_chains(void) {
  ...
  ira_start_point_ranges = (allocno_live_range_t *)ira_allocate(
      ira_max_point * sizeof(allocno_live_range_t));
  memset(ira_start_point_ranges, 0,
         ira_max_point * sizeof(allocno_live_range_t));
  ira_finish_point_ranges = (allocno_live_range_t *)ira_allocate(
      ira_max_point * sizeof(allocno_live_range_t));
...
void ira_print_live_range_list(FILE *f, allocno_live_range_t r) {
  for (; r != NULL; r = r->next)
...
void ira_debug_live_range_list(allocno_live_range_t r) {
...
void ira_create_allocno_live_ranges(void) {
  ...
  curr_point = 0;
  last_call_num = 0;
  allocno_saved_at_call = (int *)ira_allocate(ira_allocnos_num * sizeof(int));
  memset(allocno_saved_at_call, 0, ira_allocnos_num * sizeof(int));
  ira_traverse_loop_tree(true, ira_loop_tree_root, NULL, process_bb_node_lives);
