...
void *ira_reallocate(void *ptr, size_t len) {
  void *res;
...
void ira_free(void *addr ATTRIBUTE_UNUSED) {
#ifndef IRA_NO_OBSTACK
  /* do nothing */
#else
  free(addr);
...
void ira_free_bitmap(bitmap b ATTRIBUTE_UNUSED) {
  /* do nothing */
}
    ...
    if (!hard_reg_set_empty_p(temp_hard_regset)) {
      ...
      for (j = 0; j < ira_reg_class_cover_size; j++) {
        ...
        if (cl == ira_reg_class_cover[j]) {
          ...
          set_p = true;
          break;
        } else if (hard_reg_set_equal_p(temp_hard_regset, temp_hard_regset2))
          eq_p = true;
        else if (hard_reg_set_subset_p(temp_hard_regset, temp_hard_regset2))
          set_p = true;
      }
      if (set_p && !eq_p) {
...
static void setup_class_translate(void) {
  ...
  if (flag_ira_algorithm == IRA_ALGORITHM_PRIORITY)
    for (cl = 0; cl < LIM_REG_CLASSES; cl++) {
      ...
      for (i = 0; i < ira_reg_class_cover_size; i++) {
        ...
        COPY_HARD_REG_SET(temp_hard_regset2, reg_class_contents[cover_class]);
        AND_COMPL_HARD_REG_SET(temp_hard_regset2, no_unit_alloc_regs);
        if (hard_reg_set_equal_p(temp_hard_regset, temp_hard_regset2))
  ...
  for (i = 0; i < ira_reg_class_cover_size; i++) {
    ...
    if (flag_ira_algorithm != IRA_ALGORITHM_PRIORITY)
      for (cl_ptr = &alloc_reg_class_subclasses[cover_class][0];
           (cl = *cl_ptr) != LIM_REG_CLASSES; cl_ptr++) {
  ...
  for (cl = 0; cl < N_REG_CLASSES; cl++) {
    ...
    for (i = 0; i < ira_reg_class_cover_size; i++) {
      cover_class = ira_reg_class_cover[i];
      COPY_HARD_REG_SET(temp_hard_regset, reg_class_contents[cover_class]);
      ...
      if (hard_reg_set_empty_p(temp_hard_regset) &&
        ...
        for (i = 0;; i++) {
          ...
          if (cl3 == LIM_REG_CLASSES)
            break;
          if (reg_class_subset_p(ira_reg_class_intersect[cl1][cl2], cl3))
        ...
        if (hard_reg_set_subset_p(temp_hard_regset, union_set)) {
          ...
          if (ira_reg_class_union[cl1][cl2] == NO_REGS ||
              (hard_reg_set_subset_p(temp_set2, temp_hard_regset)
               ...
               &&
               (!hard_reg_set_equal_p(temp_set2, temp_hard_regset)
...
static void print_class_cover(FILE *f) {
...
/* Output all cover classes and the translation map into
   stderr.  */
void ira_debug_class_cover(void) {
...
static void setup_hard_regno_cover_class(void) {
  int i, j;
  enum reg_class cl;
    ...
    for (j = 0; j < NUM_MACHINE_MODES; j++) {
      ...
      for (k = ira_class_hard_regs_num[cl] - 1; k >= 0; k--) {
        ...
        if (!HARD_REGNO_MODE_OK(hard_regno, j))
          SET_HARD_REG_BIT(prohibited_class_mode_regs[cl][j], hard_regno);
      }
    }
  }
}
...
/* Allocate and initialize IRA_REGISTER_MOVE_COST,
   ...
   not done yet.  */
void ira_init_register_move_cost(enum machine_mode mode) {
  int cl1, cl2;
...
static int insn_contains_asm_1(rtx *loc, void *data ATTRIBUTE_UNUSED) {
  if (!*loc)
    return FALSE;
...
static bool insn_contains_asm(rtx insn) {
  return for_each_rtx(&insn, insn_contains_asm_1, NULL);
}

/* Set up regs_asm_clobbered.  */
static void compute_regs_asm_clobbered(char *regs_asm_clobbered) {
  basic_block bb;
      ...
      if (insn_contains_asm(insn))
        for (def_rec = DF_INSN_DEFS(insn); *def_rec; def_rec++) {
          ...
          if (dregno < FIRST_PSEUDO_REGISTER) {
            ...
            unsigned int end = dregno + hard_regno_nregs[dregno][mode] - 1;
...
static void setup_eliminable_regset(void) {
  ...
  int need_fp =
      (!flag_omit_frame_pointer || (cfun->calls_alloca && EXIT_IGNORE_STACK) ||
       crtl->accesses_prior_frames || crtl->stack_realign_needed ||
  ...
  for (i = 0; i < (int)ARRAY_SIZE(eliminables); i++) {
    ...
    if (!regs_asm_clobbered[eliminables[i].from]) {
      ...
      error("%s cannot be used in asm here", reg_names[eliminables[i].from]);
    else
    ...
    for (list = reg_equiv_init[i]; list != NULL_RTX; list = XEXP(list, 1)) {
      ...
      if (!function_invariant_p(x) ||
        ...
        if (memory_operand(x, VOIDmode))
          ...
          if (GET_CODE(x) == PLUS || x == frame_pointer_rtx ||
              x == arg_pointer_rtx)
            invariant_p = true;
          else
            constant = x;
...
static void setup_allocno_assignment_flags(void) {
  ...
  FOR_EACH_ALLOCNO(a, ai) {
    ...
    ALLOCNO_ASSIGNED_P(a) =
        (hard_regno >= 0 || ALLOCNO_MEM_OPTIMIZED_DEST_P(a) ||
         (ALLOCNO_MEMORY_COST(a) - ALLOCNO_COVER_CLASS_COST(a)) < 0);
    ira_assert(
        hard_regno < 0 ||
        !ira_hard_reg_not_in_set_p(hard_regno, ALLOCNO_MODE(a),
...
static void calculate_allocation_cost(void) {
  ...
  FOR_EACH_ALLOCNO(a, ai) {
    ...
    if (hard_regno < 0) {
      ...
      cost = (ALLOCNO_HARD_REG_COSTS(
          a)[ira_class_hard_reg_index[ALLOCNO_COVER_CLASS(a)][hard_regno]]);
      ira_reg_cost += cost;
    } else {
  ...
  if (internal_flag_ira_verbose > 0 && ira_dump_file != NULL) {
    fprintf(ira_dump_file,
            ...
            ira_overall_cost, ira_reg_cost, ira_mem_cost, ira_load_cost,
            ira_store_cost, ira_shuffle_cost);
  ...
  if (reg_equiv_init_size < max_regno) {
    ...
    for (i = FIRST_PSEUDO_REGISTER; i < reg_equiv_init_size; i++)
      for (prev = NULL_RTX, x = reg_equiv_init[i]; x != NULL_RTX; x = next) {
        ...
        if (REG_P(SET_DEST(set)) && ((int)REGNO(SET_DEST(set)) == i ||
    ...
    for (cp = ALLOCNO_COPIES(a); cp != NULL; cp = next_cp)
      if (cp->first == a)
        ...
        if (internal_flag_ira_verbose > 4 && ira_dump_file != NULL &&
            cp->insn != NULL_RTX && ALLOCNO_HARD_REGNO(cp->first) == hard_regno)
          fprintf(ira_dump_file, "        Redundant move from %d(freq %d):%d\n",
  ...
  for (i = start; i < max_regno; i++) {
    ...
    if (internal_flag_ira_verbose > 2 && ira_dump_file != NULL)
      fprintf(ira_dump_file,
              ...
              reg_class_names[reg_preferred_class(old_regno)],
...
static void expand_reg_info(int old_size) {
  int i;
...
static void validate_equiv_mem_from_store(rtx dest,
  ...
  if ((REG_P(dest) && reg_overlap_mentioned_p(dest, equiv_mem)) ||
      (MEM_P(dest) && true_dependence(dest, VOIDmode, equiv_mem, rtx_varies_p)))
...
static int equiv_init_movable_p(rtx x, int regno) {
  int i, j;
  const char *fmt;
...
static int contains_replace_regs(rtx x) {
  int i, j;
  const char *fmt;
  enum rtx_code code = GET_CODE(x);
  ...
  switch (code) {
    ...
    case REG:
      return (reg_equiv[REGNO(x)].replacement &&
              memref_referenced_p(memref, reg_equiv[REGNO(x)].replacement));
...
static void no_equiv(rtx reg, const_rtx store ATTRIBUTE_UNUSED,
                     void *data ATTRIBUTE_UNUSED) {
  int regno;
  rtx list;
...
static int update_equiv_regs(void) {
  rtx insn;
  basic_block bb;
  int loop_depth;
  bitmap cleared_regs;
  ...
  FOR_EACH_BB(bb) {
    ...
    for (insn = BB_HEAD(bb); insn != NEXT_INSN(BB_END(bb));
      ...
      if (!REG_P(dest) || (regno = REGNO(dest)) < FIRST_PSEUDO_REGISTER ||
          reg_equiv[regno].init_insns == const0_rtx ||
          (CLASS_LIKELY_SPILLED_P(reg_preferred_class(regno)) && MEM_P(src) &&
           !reg_equiv[regno].is_arg_equivalence)) {
      ...
      if (DF_REG_DEF_COUNT(regno) != 1 &&
          (!note || rtx_varies_p(XEXP(note, 0), 0) ||
           (reg_equiv[regno].replacement &&
            !rtx_equal_p(XEXP(note, 0), reg_equiv[regno].replacement)))) {
      ...
      if (DF_REG_DEF_COUNT(regno) == 1 && note &&
      ...
      if (note == 0 && REG_BASIC_BLOCK(regno) >= NUM_FIXED_BLOCKS &&
          MEM_P(SET_SRC(set)) && validate_equiv_mem(insn, dest, SET_SRC(set)))
      ...
      if (note) {
        ...
        if (REG_LIVE_LENGTH(regno) >= 0 && optimize) {
          ...
          if (REG_N_REFS(regno) == 2 &&
              (rtx_equal_p(x, src) || !equiv_init_varies_p(src)) &&
              NONJUMP_INSN_P(insn) &&
  ...
  for (insn = get_insns(); insn; insn = NEXT_INSN(insn)) {
    ...
    if (MEM_P(dest) && REG_P(src) &&
        ...
        DF_REG_DEF_COUNT(regno) == 1 && reg_equiv[regno].init_insns != 0 &&
        reg_equiv[regno].init_insns != const0_rtx &&
        !find_reg_note(XEXP(reg_equiv[regno].init_insns, 0), REG_EQUIV,
                       NULL_RTX) &&
      ...
      if (validate_equiv_mem(init_insn, src, dest) &&
        ...
        /* This insn makes the equivalence, not the one initializing
           the register.  */
        reg_equiv_init[regno] = gen_rtx_INSN_LIST(VOIDmode, insn, NULL_RTX);
  ...
  FOR_EACH_BB_REVERSE(bb) {
    loop_depth = bb->loop_depth;
    for (insn = BB_END(bb); insn != PREV_INSN(BB_HEAD(bb));
         insn = PREV_INSN(insn)) {
      rtx link;
      ...
      for (link = REG_NOTES(insn); link; link = XEXP(link, 1)) {
        if (REG_NOTE_KIND(link) == REG_DEAD
          ...
          if (asm_noperands(PATTERN(equiv_insn)) < 0 &&
            ...
            reg_equiv[regno].init_insns = XEXP(reg_equiv[regno].init_insns, 1);
...
static void print_insn_chain(FILE *file, struct insn_chain *c) {
...
static void build_insn_chain(void) {
  ...
  FOR_EACH_BB_REVERSE(bb) {
    ...
    EXECUTE_IF_SET_IN_BITMAP(df_get_live_out(bb), FIRST_PSEUDO_REGISTER, i,
                             bi) {
            ...
            if ((regno < FIRST_PSEUDO_REGISTER || reg_renumber[regno] >= 0 ||
              ...
              if (GET_CODE(reg) == SUBREG &&
                ...
                if (!DF_REF_FLAGS_IS_SET(def, DF_REF_STRICT_LOW_PART)) {
                  ...
                  last = ((last + UNITS_PER_WORD - 1) / UNITS_PER_WORD *
                          UNITS_PER_WORD);
                }
        ...
        if (INSN_P(insn))
          for (use_rec = DF_INSN_UID_USES(uid); *use_rec; use_rec++) {
            ...
            if (regno < FIRST_PSEUDO_REGISTER ||
              ...
              if (GET_CODE(reg) == SUBREG &&
                  !DF_REF_FLAGS_IS_SET(
                      use, DF_REF_SIGN_EXTRACT | DF_REF_ZERO_EXTRACT)) {
                unsigned int start = SUBREG_BYTE(reg);
                unsigned int last = start + GET_MODE_SIZE(GET_MODE(reg));
    ...
    while (insn &&
           (BARRIER_P(insn) || NOTE_P(insn) || BLOCK_FOR_INSN(insn) == bb))
      insn = PREV_INSN(insn);
...
static void ira(FILE *f) {
  ...
  if (optimize) {
    ...
    ira_reg_equiv_invariant_p = (bool *)ira_allocate(max_regno * sizeof(bool));
  ...
  loops_p = ira_build(optimize && (flag_ira_region == IRA_REGION_ALL ||
  ...
  if (ira_conflicts_p) {
    ...
    ira_spilled_reg_stack_slots =
        ((struct ira_spilled_reg_stack_slot *)ira_allocate(
