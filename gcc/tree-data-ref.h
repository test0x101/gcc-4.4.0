...
#define AM_GET_SUBSCRIPT_ACCESS_VECTOR(M, I) \
  VEC_index(lambda_vector, AM_MATRIX(M), I)
#define AM_GET_ACCESS_MATRIX_ELEMENT(M, I, J) \
  AM_GET_SUBSCRIPT_ACCESS_VECTOR(M, I)[J]

/* Return the column in the access matrix of LOOP_NUM.  */

static inline int am_vector_index_for_loop(struct access_matrix *access_matrix,
                                           int loop_num) {
  int i;
  loop_p l;
...
#define DDR_NUM_DIST_VECTS(DDR) (VEC_length(lambda_vector, DDR_DIST_VECTS(DDR)))
#define DDR_NUM_DIR_VECTS(DDR)  (VEC_length(lambda_vector, DDR_DIR_VECTS(DDR)))
#define DDR_DIR_VECT(DDR, I)    VEC_index(lambda_vector, DDR_DIR_VECTS(DDR), I)
#define DDR_DIST_VECT(DDR, I)   VEC_index(lambda_vector, DDR_DIST_VECTS(DDR), I)
...
static inline bool same_access_functions(
...
static inline bool ddr_is_anti_dependent(ddr_p ddr) {
  return (DDR_ARE_DEPENDENT(ddr) == NULL_TREE && DR_IS_READ(DDR_A(ddr)) &&
          !DR_IS_READ(DDR_B(ddr)) && !same_access_functions(ddr));
}

/* Return true when DEPENDENCE_RELATIONS contains an anti-dependence.  */

static inline bool ddrs_have_anti_deps(VEC(ddr_p, heap) *
...
static inline int index_in_loop_nest(int var, VEC(loop_p, heap) * loop_nest) {
  struct loop *loopi;
  int var_index;
...
static inline bool rdg_has_similar_memory_accesses(struct graph *rdg, int v1,
                                                   int v2) {
  return have_similar_memory_accesses(RDG_STMT(rdg, v1), RDG_STMT(rdg, v2));
}
