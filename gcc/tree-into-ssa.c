...
static inline bool rewrite_uses_p(gimple stmt) {
  return gimple_visited_p(stmt);
}

/* Set the rewrite marker on STMT to the value given by REWRITE_P.  */

static inline void set_rewrite_uses(gimple stmt, bool rewrite_p) {
...
static inline bool register_defs_p(gimple stmt) {
  return gimple_plf(stmt, GF_PLF_1) != 0;
}

/* If REGISTER_DEFS_P is true, mark STMT to have its DEFs registered.  */

static inline void set_register_defs(gimple stmt, bool register_defs_p) {
  gimple_set_plf(stmt, GF_PLF_1, register_defs_p);
}

/* Get the information associated with NAME.  */

static inline ssa_name_info_p get_ssa_name_ann(tree name) {
...
static inline enum need_phi_state get_phi_state(tree var) {
...
static inline void set_phi_state(tree var, enum need_phi_state state) {
  if (TREE_CODE(var) == SSA_NAME)
...
void compute_global_livein(bitmap livein ATTRIBUTE_UNUSED,
  ...
  *tos++ = BASIC_BLOCK(i);
  ...
  while (tos != worklist) {
    ...
    FOR_EACH_EDGE(e, ei, bb->preds) {
      ...
      if (pred != ENTRY_BLOCK_PTR && !bitmap_bit_p(livein, pred_index) &&
...
static inline struct def_blocks_d *get_def_blocks_for(tree var) {
...
static inline bool symbol_marked_for_renaming(tree sym) {
  return bitmap_bit_p(syms_to_rename, DECL_UID(sym));
}

/* Return true if NAME is in OLD_SSA_NAMES.  */

static inline bool is_old_name(tree name) {
  unsigned ver = SSA_NAME_VERSION(name);
...
static inline bool is_new_name(tree name) {
  unsigned ver = SSA_NAME_VERSION(name);
...
static inline bitmap names_replaced_by(tree new_tree) {
  struct repl_map_d m;
  void **slot;
...
static inline void add_to_repl_tbl(tree new_tree, tree old) {
...
static unsigned find_dfsnum_interval(struct dom_dfsnum *defs, unsigned n,
                                     unsigned s) {
  unsigned f = 0, t = n, m;
...
static inline struct def_blocks_d *find_def_blocks_for(tree var) {
...
static inline tree get_default_def_for(tree sym) {
...
static void mark_phi_for_rewrite(basic_block bb, gimple phi) {
  gimple_vec phis;
...
static void insert_phi_nodes_for(tree var, bitmap phi_insertion_points,
                                 bool update_p) {
  unsigned bb_index;
  edge e;
  gimple phi;
...
static void rewrite_initialize_block(
...
static void rewrite_finalize_block(struct dom_walk_data *walk_data
                                       ATTRIBUTE_UNUSED,
                                   basic_block bb ATTRIBUTE_UNUSED) {
...
static int debug_def_blocks_r(void **slot, void *data) {
  FILE *file = (FILE *)data;
...
void dump_def_blocks(FILE *file) {
...
static inline void register_new_update_single(tree new_name, tree old_name) {
...
static inline void register_new_update_set(tree new_name, bitmap old_names) {
  bitmap_iterator bi;
  unsigned i;
...
static void rewrite_update_init_block(
    struct dom_walk_data *walk_data ATTRIBUTE_UNUSED, basic_block bb) {
  edge e;
  edge_iterator ei;
...
static void rewrite_update_fini_block(struct dom_walk_data *walk_data
                                          ATTRIBUTE_UNUSED,
                                      basic_block bb ATTRIBUTE_UNUSED) {
  while (VEC_length(tree, block_defs_stack) > 0) {
...
static inline void maybe_replace_use(use_operand_p use_p) {
  tree rdef = NULL_TREE;
...
static inline void maybe_register_def(def_operand_p def_p, gimple stmt) {
...
static void init_ssa_renamer(void) {
  ...
  def_blocks = htab_create(num_referenced_vars, def_blocks_hash, def_blocks_eq,
                           def_blocks_free);
...
static void mark_def_interesting(tree var, gimple stmt, basic_block bb,
                                 bool insert_phi_p) {
  gcc_assert(bitmap_bit_p(blocks_to_update, bb->index));
  set_register_defs(stmt, true);
...
static inline void mark_use_interesting(tree var, gimple stmt, basic_block bb,
                                        bool insert_phi_p) {
...
void register_new_name_mapping(tree new_Tree ATTRIBUTE_UNUSED,
...
bool name_registered_for_update_p(tree n ATTRIBUTE_UNUSED) {
  if (!need_ssa_update_p())
    return false;
...
static void insert_updated_phi_nodes_for(tree var, bitmap *dfs, bitmap blocks,
...
void update_ssa(unsigned update_flags) {
  ...
  if (!bitmap_empty_p(syms_to_rename)) {
    ...
    start_bb =
        nearest_common_dominator_for_set(CDI_DOMINATORS, blocks_to_update);
  }
