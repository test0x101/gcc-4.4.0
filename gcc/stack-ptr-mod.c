...
static void notice_stack_pointer_modification_1(rtx x,
  ...
  if (x == stack_pointer_rtx
      ...
      || (MEM_P(x) && GET_RTX_CLASS(GET_CODE(XEXP(x, 0))) == RTX_AUTOINC &&
...
static void notice_stack_pointer_modification(void) {
  ...
  FOR_BB_INSNS(bb, insn) {
    if (INSN_P(insn)) {
      ...
      note_stores(PATTERN(insn), notice_stack_pointer_modification_1, NULL);
      if (!current_function_sp_is_unchanging)
        return;
    }
  }
...
static unsigned int rest_of_handle_stack_ptr_mod(void) {
