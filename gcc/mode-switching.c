...
static struct seginfo *new_seginfo(int mode, rtx insn, int bb,
                                   HARD_REG_SET regs_live) {
  struct seginfo *ptr;
...
static void add_seginfo(struct bb_info *head, struct seginfo *info) {
  struct seginfo *ptr;
...
static void make_preds_opaque(basic_block b, int j) {
  edge e;
  edge_iterator ei;
...
static void reg_dies(rtx reg, HARD_REG_SET *live) {
  int regno;
...
static void reg_becomes_live(rtx reg, const_rtx setter ATTRIBUTE_UNUSED,
                             void *live) {
  int regno;
...
static basic_block create_pre_exit(int n_entities, int *entity_map,
                                   const int *num_modes) {
  edge eg;
  edge_iterator ei;
    ...
    if (EDGE_COUNT(EXIT_BLOCK_PTR->preds) == 1 &&
      ...
      do {
        ...
        if (INSN_P(return_copy)) {
          ...
          switch (GET_CODE(return_copy_pat)) {
            case USE:
              ...
              if (GET_CODE(XEXP(return_copy_pat, 0)) == REG &&
                  (FUNCTION_VALUE_REGNO_P(REGNO(XEXP(return_copy_pat, 0))))) {
          ...
          if (copy_start >= ret_start && copy_start + copy_num <= ret_end)
            nregs -= copy_num;
          else if (!maybe_builtin_apply || !FUNCTION_VALUE_REGNO_P(copy_start))
            break;
          last_insn = return_copy;
        }
      ...
      gcc_assert(!nregs || forced_late_switch || short_block ||
                 ...
                 ||
                 (GET_MODE_CLASS(GET_MODE(ret_reg)) != MODE_INT && nregs != 1));

      if (INSN_P(last_insn)) {
        before_return_copy = emit_note_before(NOTE_INSN_DELETED, last_insn);
...
static int optimize_mode_switching(void) {
  rtx insn;
  int e;
  basic_block bb;
  int need_commit = 0;
...
#define N_ENTITIES ARRAY_SIZE(num_modes)
  ...
  for (j = n_entities - 1; j >= 0; j--) {
    ...
    FOR_EACH_BB(bb) {
      ...
      for (insn = BB_HEAD(bb); insn != NULL && insn != NEXT_INSN(BB_END(bb));
           insn = NEXT_INSN(insn)) {
        if (INSN_P(insn)) {
  ...
  for (i = 0; i < max_num_modes; i++) {
    ...
    edge_list =
        pre_edge_lcm(n_entities, transp, comp, antic, kill, &insert, &del);
...
static bool gate_mode_switching(void) {
#ifdef OPTIMIZE_MODE_SWITCHING
  return true;
#else
  return false;
...
static unsigned int rest_of_handle_mode_switching(void) {
#ifdef OPTIMIZE_MODE_SWITCHING
