...
static inline struct loop *gbb_loop(struct graphite_bb *gbb) {
...
static inline int gbb_nb_loops(const struct graphite_bb *gb) {
...
static inline loop_p gbb_loop_at_index(graphite_bb_p gb, int index) {
  return VEC_index(loop_p, GBB_LOOPS(gb), index);
}

/* Returns the index of LOOP in the loop nest around GB.  */

static inline int gbb_loop_index(graphite_bb_p gb, loop_p loop) {
  int i;
  loop_p l;
...
static inline tree oldiv_for_loop(scop_p scop, loop_p loop) {
  int i;
  name_tree iv;
...
static inline int scop_dim_domain(scop_p scop) {
  return scop_nb_loops(scop) + scop_nb_params(scop) + 1;
}

/* Return the dimension of the domains for GB.  */

static inline int gbb_dim_domain(graphite_bb_p gb) {
...
static inline int loop_domain_dim(unsigned int loop_num, scop_p scop) {
...
static inline int loop_iteration_vector_dim(unsigned int loop_num,
                                            scop_p scop) {
...
static inline int scop_loop_index(scop_p scop, struct loop *loop) {
  unsigned i;
  struct loop *l;
...
static inline int gbb_inner_most_loop_index(scop_p scop, graphite_bb_p gb) {
...
static struct loop *outer_most_loop_1(scop_p scop, struct loop *loop,
                                      struct loop *current_outer) {
...
static struct loop *outer_most_loop(scop_p scop, struct loop *loop) {
...
static inline int gbb_outer_most_loop_index(scop_p scop, graphite_bb_p gb) {
  return scop_loop_index(scop, outer_most_loop(scop, gbb_loop(gb)));
}

/* Return the loop depth of LOOP in SCOP.  */

static inline unsigned int scop_gimple_loop_depth(scop_p scop, loop_p loop) {
