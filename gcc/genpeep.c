...
static void gen_peephole(rtx peep) {
  ...
  printf(
      "  PATTERN (ins1) = gen_rtx_PARALLEL (VOIDmode, gen_rtvec_v (%d, "
      "operands));\n",
      n_operands);
...
from the machine description file `md'.  */\n\n");
  ...
  printf("rtx\npeephole (rtx ins1)\n{\n");
  printf(
      "  rtx insn ATTRIBUTE_UNUSED, x ATTRIBUTE_UNUSED, pat "
      "ATTRIBUTE_UNUSED;\n\n");
  ...
  while (1) {
    ...
    if (GET_CODE(desc) == DEFINE_INSN || GET_CODE(desc) == DEFINE_EXPAND ||
        GET_CODE(desc) == DEFINE_SPLIT || GET_CODE(desc) == DEFINE_PEEPHOLE2) {
      insn_code_number++;
    }
  }
