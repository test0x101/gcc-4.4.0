...
static bool check_range(gimple swtch) {
...
static bool check_process_case(tree cs) {
  tree ldecl;
...
static bool check_final_bb(void) {
      ...
      if (bb == info.switch_bb ||
        ...
        if ((flag_pic && reloc != null_pointer_node) ||
          ...
          if (reloc)
            ...
            info.reason = "   Value from a case is not a valid initializer\n";
          return false;
        }
      }
    }
...
static void create_temp_arrays(void) {
  int i;

  info.default_values = (tree *)xcalloc(info.phi_count, sizeof(tree));
  info.constructors =
      (VEC(constructor_elt, gc) **)xcalloc(info.phi_count, sizeof(tree));
  info.target_inbound_names = (tree *)xcalloc(info.phi_count, sizeof(tree));
  info.target_outbound_names = (tree *)xcalloc(info.phi_count, sizeof(tree));
...
static void build_constructors(gimple swtch) {
  ...
  for (i = 1; i < branch_num; i++) {
    ...
    while (tree_int_cst_lt(pos, CASE_LOW(cs))) {
      ...
      for (k = 0; k < info.phi_count; k++) {
        ...
        elt = VEC_quick_push(constructor_elt, info.constructors[k], NULL);
    ...
    for (gsi = gsi_start_phis(info.final_bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
      ...
      do {
        ...
        elt = VEC_quick_push(constructor_elt, info.constructors[j], NULL);
        elt->index = int_const_binop(MINUS_EXPR, pos, info.range_min, 0);
        elt->value = val;
...
static void build_one_array(gimple swtch, int num, tree arr_index_type,
                            gimple phi, tree tidx) {
  ...
  fetch = build4(ARRAY_REF, value_type, decl, tidx, NULL_TREE, NULL_TREE);
...
static void build_arrays(gimple swtch) {
  tree arr_index_type;
  tree tidx, sub;
  gimple stmt;
  gimple_stmt_iterator gsi;
  ...
  sub = force_gimple_operand_gsi(&gsi, fold_convert(arr_index_type, sub), false,
  ...
  for (gsi = gsi_start_phis(info.final_bb), i = 0; !gsi_end_p(gsi);
       gsi_next(&gsi), i++)
...
static gimple gen_def_assigns(gimple_stmt_iterator *gsi) {
  ...
  for (i = 0; i < info.phi_count; i++) {
    tree name = make_ssa_name(SSA_NAME_VAR(info.target_inbound_names[i]), NULL);
...
static void prune_bbs(basic_block bbd, basic_block final) {
  edge_iterator ei;
  edge e;
...
static void fix_phi_nodes(edge e1f, edge e2f, basic_block bbf) {
  gimple_stmt_iterator gsi;
  int i;

  for (gsi = gsi_start_phis(bbf), i = 0; !gsi_end_p(gsi); gsi_next(&gsi), i++) {
    gimple phi = gsi_stmt(gsi);
...
static void gen_inbound_check(gimple swtch) {
  ...
  minus =
      force_gimple_operand_gsi(&gsi, minus, false, NULL, true, GSI_SAME_STMT);
...
static bool process_switch(gimple swtch) {
...
static unsigned int do_switchconv(void) {
  basic_block bb;
