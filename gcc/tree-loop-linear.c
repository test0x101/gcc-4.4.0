...
static void gather_interchange_stats(
    VEC(ddr_p, heap) * dependence_relations ATTRIBUTE_UNUSED,
  ...
  *access_strides = double_int_zero;
  ...
  for (i = 0; VEC_iterate(ddr_p, dependence_relations, i, ddr); i++) {
    ...
    if (DDR_ARE_DEPENDENT(ddr) == chrec_dont_know ||
        DDR_ARE_DEPENDENT(ddr) == chrec_known || DDR_NUM_DIST_VECTS(ddr) == 0)
      continue;

    for (j = 0; j < DDR_NUM_DIST_VECTS(ddr); j++) {
      int dist =
          DDR_DIST_VECT(ddr, j)[loop_depth(loop) - loop_depth(first_loop)];
  ...
  for (i = 0; VEC_iterate(data_reference_p, datarefs, i, dr); i++) {
    ...
    for (it = 0; it < DR_NUM_DIMENSIONS(dr); it++, ref = TREE_OPERAND(ref, 0)) {
      ...
      if (array_size == NULL_TREE || TREE_CODE(array_size) != INTEGER_CST)
        continue;
...
static lambda_trans_matrix try_interchange_loops(
    lambda_trans_matrix trans, unsigned int depth,
    VEC(ddr_p, heap) * dependence_relations,
    VEC(data_reference_p, heap) * datarefs, struct loop *first_loop) {
  bool res;
  struct loop *loop_i;
  struct loop *loop_j;
  ...
  for (loop_j = first_loop->inner; loop_j; loop_j = loop_j->inner)
    for (loop_i = first_loop; loop_depth(loop_i) < loop_depth(loop_j);
         loop_i = loop_i->inner) {
      gather_interchange_stats(dependence_relations, datarefs, loop_i,
                               first_loop, &dependence_steps_i,
                               &nb_deps_not_carried_by_i, &access_strides_i);
      gather_interchange_stats(dependence_relations, datarefs, loop_j,
                               first_loop, &dependence_steps_j,
                               &nb_deps_not_carried_by_j, &access_strides_j);
      ...
      */
      ...
      res = cmp < 0 ? estimated_loop_iterations(loop_j, false, &nb_iter)
      ...
      if (dependence_steps_i < dependence_steps_j ||
          nb_deps_not_carried_by_i > nb_deps_not_carried_by_j || cmp < 0) {
        ...
        if (!lambda_transform_legal_p(trans, depth, dependence_relations))
          lambda_matrix_row_exchange(
              LTM_MATRIX(trans), loop_depth(loop_i) - loop_depth(first_loop),
              loop_depth(loop_j) - loop_depth(first_loop));
      }
    }
...
unsigned int perfect_loop_nest_depth(struct loop *loop_nest) {
  struct loop *temp;
...
void linear_transform_loops(void) {
  ...
  FOR_EACH_LOOP(li, loop_nest, 0) {
    ...
    trans = try_interchange_loops(trans, depth, dependence_relations, datarefs,
                                  loop_nest);

    if (lambda_trans_matrix_id_p(trans)) {
      if (dump_file)
        fprintf(dump_file,
                "Won't transform loop. Optimal transform is the identity "
                "transform\n");
      goto free_and_continue;
    }
    ...
    lambda_loopnest_to_gcc_loopnest(loop_nest, oldivs, invariants, &remove_ivs,
