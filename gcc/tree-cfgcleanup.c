...
static bool cleanup_control_expr_graph(basic_block bb,
  ...
  if (!single_succ_p(bb)) {
    ...
    for (ei = ei_start(bb->succs); (e = ei_safe_edge(ei));) {
      if (e != taken_edge) {
        if (!warned) {
          fold_undefer_overflow_warnings(true, stmt,
...
static bool cleanup_control_flow_bb(basic_block bb) {
  ...
  if (gimple_code(stmt) == GIMPLE_COND || gimple_code(stmt) == GIMPLE_SWITCH)
    retval |= cleanup_control_expr_graph(bb, gsi);
           ...
           (TREE_CODE(TREE_OPERAND(gimple_goto_dest(stmt), 0)) == LABEL_DECL)) {
  ...
  /* Check for indirect calls that have been turned into
     noreturn calls.  */
  else if (is_gimple_call(stmt) && gimple_call_noreturn_p(stmt) &&
...
static inline bool has_abnormal_incoming_edge_p(basic_block bb) {
  edge e;
  edge_iterator ei;
...
static bool phi_alternatives_equal(basic_block dest, edge e1, edge e2) {
  int n1 = e1->dest_idx;
  int n2 = e2->dest_idx;
  gimple_stmt_iterator gsi;
...
static bool remove_forwarder_block(basic_block bb) {
  ...
  if (label && gimple_code(label) == GIMPLE_LABEL &&
...
static bool split_bbs_on_noreturn_calls(void) {
  bool changed = false;
  gimple stmt;
  basic_block bb;
  ...
  if (cfun->gimple_df)
    while (VEC_length(gimple, MODIFIED_NORETURN_CALLS(cfun))) {
      ...
      if (bb == NULL || bb->index < NUM_FIXED_BLOCKS ||
          bb->index >= n_basic_blocks || BASIC_BLOCK(bb->index) != bb ||
          last_stmt(bb) == stmt || !gimple_call_noreturn_p(stmt))
        continue;
...
static bool cleanup_omp_return(basic_block bb) {
  gimple stmt = last_stmt(bb);
  basic_block control_bb;

  if (stmt == NULL || gimple_code(stmt) != GIMPLE_OMP_RETURN ||
      !single_pred_p(bb))
    return false;
...
static bool cleanup_tree_cfg_bb(basic_block bb) {
  ...
  /* Forwarder blocks can carry line number information which is
     ...
     optimizing.  */
  if (optimize > 0 && tree_forwarder_block_p(bb, false) &&
  ...
  /* Merging the blocks may create new opportunities for folding
     ...
     nodes).  */
  if (single_succ_p(bb) && can_merge_blocks_p(bb, single_succ(bb))) {
...
static bool cleanup_tree_cfg_1(void) {
  bool retval = false;
  basic_block bb;
  unsigned i, n;
...
static bool cleanup_tree_cfg_noloop(void) {
  bool changed;
...
bool cleanup_tree_cfg(void) {
  ...
  if (current_loops != NULL && loops_state_satisfies_p(LOOPS_NEED_FIXUP))
...
static unsigned int merge_phi_nodes(void) {
