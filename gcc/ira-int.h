...
static inline void ira_allocno_set_iter_init(ira_allocno_set_iterator *i,
                                             IRA_INT_TYPE *vec, int min,
                                             int max) {
  i->vec = vec;
  i->word_num = 0;
...
/* Return TRUE if we have more allocnos to visit, in which case *N is
   ...
   FALSE.  */
static inline bool ira_allocno_set_iter_cond(ira_allocno_set_iterator *i,
                                             int *n) {
...
static inline void ira_allocno_set_iter_next(ira_allocno_set_iterator *i) {
  i->word >>= 1;
  i->bit_num++;
}
...
extern HARD_REG_SET ira_reg_mode_hard_regset[FIRST_PSEUDO_REGISTER]
                                            [NUM_MACHINE_MODES];
...
extern HARD_REG_SET prohibited_class_mode_regs[N_REG_CLASSES]
                                              [NUM_MACHINE_MODES];
...
extern allocno_live_range_t ira_create_allocno_live_range(ira_allocno_t, int,
                                                          int,
                                                          allocno_live_range_t);
extern allocno_live_range_t ira_copy_allocno_live_range_list(
    allocno_live_range_t);
extern allocno_live_range_t ira_merge_allocno_live_ranges(allocno_live_range_t,
                                                          allocno_live_range_t);
...
extern ira_copy_t ira_create_copy(ira_allocno_t, ira_allocno_t, int, bool, rtx,
...
static inline void ira_allocno_conflict_iter_init(
    ira_allocno_conflict_iterator *i, ira_allocno_t allocno) {
  ...
  if (i->allocno_conflict_vec_p)
    ...
    if (ALLOCNO_MIN(allocno) > ALLOCNO_MAX(allocno))
      ...
      i->size = ((ALLOCNO_MAX(allocno) - ALLOCNO_MIN(allocno) + IRA_INT_BITS) /
                 IRA_INT_BITS) *
                sizeof(IRA_INT_TYPE);
    i->bit_num = 0;
...
/* Return TRUE if we have more conflicting allocnos to visit, in which
   ...
   FALSE.  */
static inline bool ira_allocno_conflict_iter_cond(
    ira_allocno_conflict_iterator *i, ira_allocno_t *a) {
...
static inline void ira_allocno_conflict_iter_next(
    ira_allocno_conflict_iterator *i) {
  if (i->allocno_conflict_vec_p)
    i->word_num++;
  else {
    i->word >>= 1;
...
/* The function returns TRUE if hard registers starting with
   ...
   HARD_REGSET.  */
static inline bool ira_hard_reg_not_in_set_p(int hard_regno,
                                             enum machine_mode mode,
...
static inline void ira_allocate_and_set_costs(int **vec,
                                              enum reg_class cover_class,
                                              int val) {
  int i, *reg_costs;
  int len;
...
static inline void ira_allocate_and_copy_costs(int **vec,
                                               enum reg_class cover_class,
                                               int *src) {
  int len;
...
/* Allocate cost vector *VEC for hard registers of COVER_CLASS and
   ...
   (if SRC is null).  */
static inline void ira_allocate_and_set_or_copy_costs(
    int **vec, enum reg_class cover_class, int val, int *src) {
  int i, *reg_costs;
  int len;
