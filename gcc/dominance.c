...
static void init_dom_info(struct dom_info *di, enum cdi_direction dir) {
...
static unsigned int dom_convert_dir_to_idx(enum cdi_direction dir) {
...
static void calc_dfs_tree_nonrec(struct dom_info *di, basic_block bb,
                                 bool reverse) {
...
static void calc_dfs_tree(struct dom_info *di, bool reverse) {
...
static inline TBB eval(struct dom_info *di, TBB v) {
...
static void calc_idoms(struct dom_info *di, bool reverse) {
  TBB v, w, k, par;
  basic_block en_block;
  edge_iterator ei, einext;
...
inline void set_immediate_dominator(enum cdi_direction dir, basic_block bb,
...
VEC(basic_block, heap) * get_dominated_by_region(enum cdi_direction dir,
                                                 basic_block *region,
                                                 unsigned n_region) {
  unsigned i;
  basic_block dom;
  ...
  for (i = 0; i < n_region; i++)
    for (dom = first_dom_son(dir, region[i]); dom; dom = next_dom_son(dir, dom))
...
void redirect_immediate_dominators(enum cdi_direction dir, basic_block bb,
                                   basic_block to) {
  unsigned int dir_index = dom_convert_dir_to_idx(dir);
...
basic_block nearest_common_dominator(enum cdi_direction dir, basic_block bb1,
                                     basic_block bb2) {
  unsigned int dir_index = dom_convert_dir_to_idx(dir);
...
bool dominated_by_p(enum cdi_direction dir, const_basic_block bb1,
...
void verify_dominators(enum cdi_direction dir) {
  ...
  FOR_EACH_BB(bb) {
    ...
    if (imm_bb != imm_bb_correct) {
      error("dominator of %d should be %d, not %d", bb->index,
            imm_bb_correct->index, imm_bb->index);
      err = 1;
    }
  }
...
static void prune_bbs_to_update_dominators(VEC(basic_block, heap) * bbs,
                                           bool conservative) {
  unsigned i;
  bool single;
...
static void determine_dominators_for_sons(struct graph *g,
                                          VEC(basic_block, heap) * bbs, int y,
                                          int *son, int *brother) {
  bitmap gprime;
  int i, a, nc;
  VEC(int, heap) * *sccs;
...
void iterate_fix_dominators(enum cdi_direction dir,
                            VEC(basic_block, heap) * bbs, bool conservative) {
  unsigned i;
  basic_block bb, dom;
  struct graph *g;
  int n, y;
...
static void debug_dominance_tree_1(enum cdi_direction dir, basic_block root,
