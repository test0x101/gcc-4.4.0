...
rtx compare_and_jump_seq(rtx op0, rtx op1, enum rtx_code comp, rtx label,
                         int prob, rtx cinsn) {
  rtx seq, jump, cond;
...
static rtx may_unswitch_on(basic_block bb, struct loop *loop, rtx *cinsn) {
  ...
  stest = simplify_gen_relational(GET_CODE(test), SImode, mode, op[0], op[1]);
  if (stest == const0_rtx || stest == const_true_rtx)
    return stest;

  return canon_condition(gen_rtx_fmt_ee(GET_CODE(test), SImode, op[0], op[1]));
}

/* Reverses CONDition; returns NULL if we cannot.  */
rtx reversed_condition(rtx cond) {
  enum rtx_code reversed;
  reversed = reversed_comparison_code(cond, NULL);
  if (reversed == UNKNOWN)
    return NULL_RTX;
  else
    return gen_rtx_fmt_ee(reversed, GET_MODE(cond), XEXP(cond, 0),
                          XEXP(cond, 1));
}
...
static void unswitch_single_loop(struct loop *loop, rtx cond_checked, int num) {
  basic_block *bbs;
  struct loop *nloop;
  unsigned i;
  ...
  do {
    ...
    if (cond != const0_rtx && cond != const_true_rtx) {
...
static struct loop *unswitch_loop(struct loop *loop, basic_block unswitch_on,
  ...
  if (!duplicate_loop_to_header_edge(loop, entry, 1, NULL, NULL, NULL, 0))
    return NULL;
  ...
  seq = compare_and_jump_seq(XEXP(cond, 0), XEXP(cond, 1), GET_CODE(cond),
                             block_label(true_edge->dest), prob, cinsn);
  ...
  nloop = loopify(latch_edge, single_pred_edge(get_bb_copy(loop->header)),
                  switch_bb, BRANCH_EDGE(switch_bb), FALLTHRU_EDGE(switch_bb),
