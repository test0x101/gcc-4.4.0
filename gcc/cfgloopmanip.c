...
static void remove_bbs(basic_block *bbs, int nbbs) {
  int i;
...
static int find_path(edge e, basic_block **bbs) {
  ...
  *bbs = XCNEWVEC(basic_block, n_basic_blocks);
  return dfs_enumerate_from(e->dest, 0, rpe_enum_p, *bbs, n_basic_blocks,
                            e->dest);
}
...
static bool fix_loop_placement(struct loop *loop) {
  unsigned i;
  edge e;
...
static void fix_bb_placements(basic_block from, bool *irred_invalidated) {
  sbitmap in_queue;
...
bool remove_path(edge e) {
  ...
  while (loop_outer(e->src->loop_father) &&
         dominated_by_p(CDI_DOMINATORS, e->src->loop_father->latch, e->dest))
  ...
  for (i = 0; i < n_bord_bbs; i++) {
    ...
    for (ldom = first_dom_son(CDI_DOMINATORS, bb); ldom;
...
static void place_new_loop(struct loop *loop) {
...
void add_loop(struct loop *loop, struct loop *outer) {
  basic_block *bbs;
  int i, n;
  struct loop *subloop;
  edge e;
...
void scale_loop_frequencies(struct loop *loop, int num, int den) {
  basic_block *bbs;
...
static void update_dominators_in_loop(struct loop *loop) {
  ...
  for (i = 0; i < loop->num_nodes; i++) {
    ...
    for (ldom = first_dom_son(CDI_DOMINATORS, body[i]); ldom;
         ldom = next_dom_son(CDI_DOMINATORS, ldom))
...
edge create_empty_if_region_on_edge(edge entry_edge, tree condition) {
  ...
  simple_cond = force_gimple_operand_gsi(&gsi, condition, true, NULL, false,
                                         GSI_NEW_STMT);
...
struct loop *create_empty_loop_on_edge(edge entry_edge, tree initial_value,
                                       tree stride, tree upper_bound, tree iv,
                                       tree *iv_before, struct loop *outer) {
  ...
  create_iv(initial_value, stride, iv, loop, &gsi, insert_after, iv_before,
            NULL);
  ...
  upper_bound_gimplified = force_gimple_operand_gsi(&gsi, upper_bound, true,
                                                    NULL, false, GSI_NEW_STMT);
  gsi = gsi_last_bb(exit_e->src);

  cond_expr = gimple_build_cond(LE_EXPR, *iv_before, upper_bound_gimplified,
                                NULL_TREE, NULL_TREE);

  exit_test = gimple_cond_lhs(cond_expr);
  exit_test = force_gimple_operand_gsi(&gsi, exit_test, true, NULL, false,
                                       GSI_NEW_STMT);
...
struct loop *loopify(edge latch_edge, edge header_edge, basic_block switch_bb,
                     edge true_edge, edge false_edge, bool redirect_all_edges,
                     unsigned true_scale, unsigned false_scale) {
...
static void unloop(struct loop *loop, bool *irred_invalidated) {
  basic_block *body;
  struct loop *ploop;
  unsigned i, n;
...
static void fix_loop_placements(struct loop *loop, bool *irred_invalidated) {
  struct loop *outer;
  ...
  while (loop_outer(loop)) {
    ...
    fix_bb_placements(loop_preheader_edge(loop)->src, irred_invalidated);
    loop = outer;
  }
}
...
struct loop *duplicate_loop(struct loop *loop, struct loop *target) {
  struct loop *cloop;
  cloop = alloc_loop();
...
static void duplicate_subloops(struct loop *loop, struct loop *target) {
...
static void copy_loops_to(struct loop **copied_loops, int n,
                          struct loop *target) {
  struct loop *aloop;
  int i;
...
static void loop_redirect_edge(edge e, basic_block dest) {
  if (e->dest == dest)
    return;
...
static void set_zero_probability(edge e) {
...
bool duplicate_loop_to_header_edge(struct loop *loop, edge e,
...
#define SE_ORIG  1
  ...
  if (flags & DLTHE_FLAG_UPDATE_FREQ) {
    ...
    if (orig && REG_BR_PROB_BASE - orig->probability != 0) {
    ...
    for (i = 1; i <= ndupl; i++)
      scale_step[i - 1] =
          TEST_BIT(wont_exit, i) ? prob_pass_wont_exit : prob_pass_thru;
    ...
    if (flags & DLTHE_FLAG_COMPLETTE_PEEL) {
      ...
      prob_pass_main =
          TEST_BIT(wont_exit, 0) ? prob_pass_wont_exit : prob_pass_thru;
      p = prob_pass_main;
  ...
  for (j = 0; j < ndupl; j++) {
    ...
    /* Copy bbs.  */
    copy_bbs(bbs, n, new_bbs, spec_edges, 2, new_spec_edges, loop, place_after);
    ...
    if (add_irreducible_flag) {
      ...
      for (i = 0; i < n; i++) {
        ...
        if ((ae->dest->flags & BB_DUPLICATED) &&
            (ae->src->loop_father == target || ae->dest->loop_father == target))
    ...
    if (is_latch) {
      ...
      redirect_edge_and_branch_force(new_spec_edges[SE_LATCH], loop->header);
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; VEC_iterate(basic_block, dom_bbs, j, dominated); j++) {
      if (flow_bb_inside_loop_p(loop, dominated))
        continue;
      dom_bb = nearest_common_dominator(CDI_DOMINATORS, first_active[i],
                                        first_active_latch);
...
static bool has_preds_from_loop(basic_block block, struct loop *loop) {
  edge e;
  edge_iterator ei;
...
basic_block create_preheader(struct loop *loop, int flags) {
  edge e, fallthru;
  basic_block dummy;
  int nentry = 0;
  bool irred = false;
  ...
  if (nentry == 1) {
    ...
    if (single_entry->src == ENTRY_BLOCK_PTR)
      ...
      if ((flags & CP_SIMPLE_PREHEADERS) && !single_succ_p(single_entry->src))
        need_forwarder_block = true;
...
static basic_block lv_adjust_loop_entry_edge(basic_block first_head,
                                             basic_block second_head, edge e,
                                             void *cond_expr,
                                             unsigned then_prob) {
...
struct loop *loop_version(struct loop *loop, void *cond_expr,
                          basic_block *condition_bb, unsigned then_prob,
  ...
  /* Duplicate loop.  */
  if (!cfg_hook_duplicate_loop_to_header_edge(loop, entry, 1, NULL, NULL, NULL,
                                              0))
    return NULL;
  ...
  cond_bb = lv_adjust_loop_entry_edge(first_head, second_head, entry, cond_expr,
                                      then_prob);
  if (condition_bb)
  ...
  nloop =
      loopify(latch_edge, single_pred_edge(get_bb_copy(loop->header)), cond_bb,
              true_edge, false_edge, false /* Do not redirect all edges.  */,
