...
static tree add_to_dst_predicate_list(struct loop *loop, edge, tree, tree,
                                      gimple_stmt_iterator *);
static void clean_predicate_lists(struct loop *loop);
static basic_block find_phi_replacement_condition(struct loop *loop,
                                                  basic_block, tree *,
                                                  gimple_stmt_iterator *);
static void replace_phi_with_cond_gimple_assign_stmt(gimple, tree, basic_block,
                                                     gimple_stmt_iterator *);
...
static bool tree_if_conversion(struct loop *loop, bool for_vectorizer) {
  basic_block bb;
...
static tree tree_if_convert_stmt(struct loop *loop, gimple t, tree cond,
...
static void tree_if_convert_cond_stmt(struct loop *loop, gimple stmt, tree cond,
                                      gimple_stmt_iterator *gsi) {
  tree c, c2;
...
static bool if_convertible_phi_p(struct loop *loop, basic_block bb,
                                 gimple phi) {
  if (dump_file && (dump_flags & TDF_DETAILS)) {
    fprintf(dump_file, "-------------------------\n");
...
static bool if_convertible_gimple_assign_stmt_p(struct loop *loop,
                                                basic_block bb, gimple stmt) {
  tree lhs;
  ...
  if (stmt_ends_bb_p(stmt) || gimple_has_volatile_ops(stmt) ||
      (TREE_CODE(lhs) == SSA_NAME && SSA_NAME_OCCURS_IN_ABNORMAL_PHI(lhs)) ||
  ...
  if (bb != loop->header && gimple_assign_rhs_could_trap_p(stmt)) {
    if (dump_file && (dump_flags & TDF_DETAILS))
      fprintf(dump_file, "tree could trap...\n");
    return false;
  }

  if (TREE_CODE(lhs) != SSA_NAME && bb != loop->header &&
...
static bool if_convertible_stmt_p(struct loop *loop, basic_block bb,
                                  gimple stmt) {
...
static bool if_convertible_bb_p(struct loop *loop, basic_block bb,
                                basic_block exit_bb) {
  edge e;
  edge_iterator ei;
  ...
  if (exit_bb) {
    if (bb != loop->latch) {
      ...
      return false;
    } else if (bb == loop->latch && bb != exit_bb &&
...
static bool if_convertible_loop_p(struct loop *loop,
...
static void add_to_predicate_list(basic_block bb, tree new_cond) {
  tree cond = (tree)bb->aux;

  if (cond)
    cond = fold_build2(TRUTH_OR_EXPR, boolean_type_node, unshare_expr(cond),
                       new_cond);
  else
    cond = new_cond;
...
static tree add_to_dst_predicate_list(struct loop *loop, edge e, tree prev_cond,
  ...
  if (prev_cond == boolean_true_node || !prev_cond)
    ...
    prev_cond = force_gimple_operand_gsi(gsi, unshare_expr(prev_cond), true,
                                         NULL, true, GSI_SAME_STMT);

    cond = force_gimple_operand_gsi(gsi, unshare_expr(cond), true, NULL, true,
                                    GSI_SAME_STMT);
    ...
    tmp = build2(TRUTH_AND_EXPR, boolean_type_node, unshare_expr(prev_cond),
                 cond);
...
static basic_block find_phi_replacement_condition(struct loop *loop,
  ...
  if (first_edge->src == loop->header ||
      dominated_by_p(CDI_DOMINATORS, second_edge->src, first_edge->src)) {
    *cond = (tree)(second_edge->src)->aux;

    /* If there is a condition on an incoming edge,
       AND it with the incoming bb predicate.  */
    if (second_edge->aux)
      *cond = build2(TRUTH_AND_EXPR, boolean_type_node, *cond,
                     (tree)second_edge->aux);
  ...
  /* Create temp. for the condition. Vectorizer prefers to have gimple
     ...
     compare's result.  */
  *cond = force_gimple_operand_gsi(gsi, unshare_expr(*cond), false, NULL_TREE,
                                   true, GSI_SAME_STMT);
...
static void replace_phi_with_cond_gimple_assign_stmt(
    gimple phi, tree cond, basic_block true_bb, gimple_stmt_iterator *gsi) {
  gimple new_stmt;
  basic_block bb;
  tree rhs;
  tree arg_0, arg_1;
  ...
  rhs = build3(COND_EXPR, TREE_TYPE(PHI_RESULT(phi)), unshare_expr(cond),
               unshare_expr(arg_0), unshare_expr(arg_1));
...
static void process_phi_nodes(struct loop *loop) {
  basic_block bb;
...
static void combine_blocks(struct loop *loop) {
  ...
  if (exit_bb && exit_bb != loop->header &&
...
static gimple ifc_temp_var(tree type, tree exp) {
...
static basic_block *get_loop_body_in_if_conv_order(const struct loop *loop) {
  ...
  while (index < loop->num_nodes) {
    ...
    if (!bitmap_bit_p(visited, bb->index)) {
      if (pred_blocks_visited_p(bb, &visited) || bb == loop->header) {
    ...
    index++;
    if (index == loop->num_nodes && visited_count != loop->num_nodes) {
      /* Not done yet.  */
      index = 0;
    }
  }
...
static bool bb_with_exit_edge_p(struct loop *loop, basic_block bb) {
  edge e;
  edge_iterator ei;
...
static bool gate_tree_if_conversion(void) {
  return flag_tree_vectorize != 0;
}

struct gimple_opt_pass pass_if_conversion = {{
    GIMPLE_PASS, "ifcvt",             /* name */
