...
#define MATCH(prefix, string)                        \
  ((strncmp(prefix, string, sizeof prefix - 1) == 0) \
       ? ((string += sizeof prefix - 1), 1)          \
       : 0)

void set_struct_debug_option(const char *spec) {
  ...
  if (*spec == ',')
    ...
    /* No more -femit-struct-debug-detailed specifications.
       Do final checks. */
    if (*spec != '\0')
      error("argument %qs to %<-femit-struct-debug-detailed%> unknown", spec);
    if (debug_struct_ordinary[DINFO_USAGE_DIR_USE] <
      ...
      error(
          "%<-femit-struct-debug-detailed=dir:...%> must allow at least"
...
static int base_of_path(const char *path, const char **base_out) {
...
static int matches_main_base(const char *path) {
...
static int dump_struct_debug(tree type, enum debug_info_usage usage,
  ...
  fprintf(stderr, "	struct %d %s %s %s %s %d %p %s\n", criterion,
          ...
          matches ? "bas" : "hdr", generic ? "gen" : "ord",
          usage == DINFO_USAGE_DFN       ? ";"
          : usage == DINFO_USAGE_DIR_USE ? "."
                                         : "*",
          result, (void *)type_decl, name);
  return result;
}
...
bool should_emit_struct_debug(tree type, enum debug_info_usage usage) {
...
static bool flag_predictive_commoning_set, flag_unswitch_loops_set,
...
static int integral_argument(const char *arg) {
  const char *p = arg;
...
static char *write_langs(unsigned int mask) {
...
static void complain_wrong_lang(const char *text,
  ...
  warning(0, "command line option \"%s\" is valid for %s but not for %s", text,
          ok_langs, bad_lang);
...
static unsigned int handle_option(const char **argv, unsigned int lang_mask) {
  ...
  if (!(option->flags & (lang_mask | CL_COMMON | CL_TARGET))) {
    ...
    goto done;
  } else if ((option->flags & CL_TARGET) && (option->flags & CL_LANG_ALL) &&
  ...
  if (option->flag_var)
    switch (option->var_type) {
      ...
      case CLVC_EQUAL:
        *(int *)option->flag_var =
            (value ? option->var_value : !option->var_value);
        break;
...
static void add_comma_separated_to_vector(VEC(char_p, heap) * *pvec,
                                          const char *arg) {
  char *tmp;
  char *r;
  char *w;
...
bool flag_instrument_functions_exclude_p(tree fndecl) {
  if (VEC_length(char_p, flag_instrument_functions_exclude_functions) > 0) {
    ...
    for (i = 0;
         VEC_iterate(char_p, flag_instrument_functions_exclude_functions, i, s);
         ++i) {
...
/* Decode and handle the vector of command line options.  LANG_MASK
   ...
   language.  */
static void handle_options(unsigned int argc, const char **argv,
                           unsigned int lang_mask) {
  unsigned int n, i;
...
/* Parse command line options and set default flag values.  Do minimal
   options processing.  */
void decode_options(unsigned int argc, const char **argv) {
  ...
  if (!targetm.ira_cover_classes && flag_ira_algorithm == IRA_ALGORITHM_CB) {
    inform(input_location,
...
static void wrap_help(const char *help, const char *item,
                      unsigned int item_width, unsigned int columns) {
  ...
  do {
    ...
    if (room < len) {
      ...
      for (i = 0; help[i]; i++) {
        ...
        if (help[i] == ' ')
          len = i;
        else if ((help[i] == '-' || help[i] == '/') && help[i + 1] != ' ' &&
...
static void print_filtered_help(unsigned int include_flags,
                                ...
                                unsigned int any_flags, unsigned int columns) {
  unsigned int i;
  const char *help;
      ...
      if (option->flag_var != NULL) {
        if (option->flags & CL_JOINED) {
          if (option->var_type == CLVC_STRING) {
            ...
            sprintf(new_help + strlen(new_help), "%#x",
                    *(int *)option->flag_var);
        } else
          strcat(new_help,
                 option_enabled(i) ? _("[enabled]") : _("[disabled]"));
      }
  ...
  if (!found) {
    ...
    if (langs == 0)
      ...
      for (i = 0; (1U << i) < CL_LANG_ALL; i++)
        if ((1U << i) & langs)
          printf(_(" None found.  Use --help=%s to show *all* the options "
                   "supported by the %s front-end\n"),
                 lang_names[i], lang_names[i]);
    }

  } else if (!displayed)
    printf(
        _(" All options with the desired characteristics have already been "
          "displayed\n"));
  ...
  switch (code) {
    ...
    case OPT__help_: {
      ...
      while (*a != 0) {
        static struct {
          const char *string;
          unsigned int flag;
        } specifics[] = {{"optimizers", CL_OPTIMIZATION},
                         {"target", CL_TARGET},
                         {"warnings", CL_WARNING},
            ...
            *pflags |= specific_flag;
            ...
            if (strncasecmp(a, "c", len) == 0)
              *pflags |= lang_flag;
              ...
              fnotice(stderr,
                      "warning: --help argument %.*s is ambiguous, please be "
                      "more specific\n",
                      len, a);
          }
        } else if (lang_flag != 0)
    ...
    case OPT_Wstrict_overflow:
      warn_strict_overflow =
          (value ? (int)WARN_STRICT_OVERFLOW_CONDITIONAL : 0);
      break;
    ...
    case OPT_fpack_struct_:
      if (value <= 0 || (value & (value - 1)) || value > 16)
        error("structure alignment must be a small power of two, not %d",
              value);
      else {
    ...
    case OPT_fprofile_use:
      ...
      if (!flag_ipa_cp_set)
        flag_ipa_cp = value;
      if (!flag_ipa_cp_clone_set && value && flag_ipa_cp)
    ...
    case OPT_fstack_check_:
      if (!strcmp(arg, "no"))
        ...
        flag_stack_check = STACK_CHECK_BUILTIN ? FULL_BUILTIN_STACK_CHECK
                                               : GENERIC_STACK_CHECK;
...
static void handle_param(const char *carg) {
  char *equal, *arg;
  int value;
...
/* Handle -W and -Wextra.  */
static void set_Wextra(int setting) {
...
void set_Wstrict_aliasing(int onoff) {
...
void set_unsafe_math_optimizations_flags(int set) {
...
bool fast_math_flags_set_p(void) {
  return (!flag_trapping_math && flag_unsafe_math_optimizations &&
          flag_finite_math_only && !flag_signed_zeros && !flag_errno_math);
}

/* Return true iff flags are set as if -ffast-math but using the flags stored
   in the struct cl_optimization structure.  */
bool fast_math_flags_struct_set_p(struct cl_optimization *opt) {
  return (!opt->flag_trapping_math && opt->flag_unsafe_math_optimizations &&
          opt->flag_finite_math_only && !opt->flag_signed_zeros &&
          !opt->flag_errno_math);
}
...
bool get_option_state(int option, struct cl_option_state *state) {
...
void enable_warning_as_error(const char *arg, int value,
                             unsigned int lang_mask) {
  char *new_option;
  int option_index;
  ...
  if (option_index == N_OPTS) {
    ...
    if (cl_options[option_index].var_type == CLVC_BOOLEAN &&
        cl_options[option_index].flag_var && kind == DK_ERROR)
