...
static void lambda_matrix_get_column(lambda_matrix, int, int, lambda_vector);

/* Allocate a matrix of M rows x  N cols.  */

lambda_matrix lambda_matrix_new(int m, int n) {
  lambda_matrix mat;
  int i;
...
void lambda_matrix_copy(lambda_matrix mat1, lambda_matrix mat2, int m, int n) {
  int i;
...
void lambda_matrix_id(lambda_matrix mat, int size) {
  int i, j;
...
bool lambda_matrix_id_p(lambda_matrix mat, int size) {
  int i, j;
  for (i = 0; i < size; i++)
    for (j = 0; j < size; j++) {
      if (i == j) {
...
void lambda_matrix_negate(lambda_matrix mat1, lambda_matrix mat2, int m,
                          int n) {
  int i;
...
void lambda_matrix_transpose(lambda_matrix mat1, lambda_matrix mat2, int m,
                             int n) {
  int i, j;
...
void lambda_matrix_add(lambda_matrix mat1, lambda_matrix mat2,
...
void lambda_matrix_mult(lambda_matrix mat1, lambda_matrix mat2,
...
static void lambda_matrix_get_column(lambda_matrix mat, int n, int col,
                                     lambda_vector vec) {
  int i;
...
void lambda_matrix_delete_rows(lambda_matrix mat, int rows, int from, int to) {
  int i;
  int dist;
  dist = to - from;
...
void lambda_matrix_row_exchange(lambda_matrix mat, int r1, int r2) {
  lambda_vector row;
...
void lambda_matrix_row_add(lambda_matrix mat, int n, int r1, int r2,
                           int const1) {
  int i;
...
void lambda_matrix_row_negate(lambda_matrix mat, int n, int r1) {
  lambda_vector_negate(mat[r1], mat[r1], n);
}

/* Multiply row R1 of matrix MAT with N columns by CONST1.  */

void lambda_matrix_row_mc(lambda_matrix mat, int n, int r1, int const1) {
  int i;
...
void lambda_matrix_col_exchange(lambda_matrix mat, int m, int col1, int col2) {
  int i;
  int tmp;
  for (i = 0; i < m; i++) {
    tmp = mat[i][col1];
...
void lambda_matrix_col_add(lambda_matrix mat, int m, int c1, int c2,
                           int const1) {
  int i;
...
void lambda_matrix_col_negate(lambda_matrix mat, int m, int c1) {
  int i;
...
void lambda_matrix_col_mc(lambda_matrix mat, int m, int c1, int const1) {
  int i;
...
int lambda_matrix_inverse(lambda_matrix mat, lambda_matrix inv, int n) {
  if (n == 2) {
    int a, b, c, d, det;
    a = mat[0][0];
    b = mat[1][0];
...
static int lambda_matrix_inverse_hard(lambda_matrix mat, lambda_matrix inv,
                                      int n) {
  lambda_vector row;
  lambda_matrix temp;
  int i, j;
...
void lambda_matrix_hermite(lambda_matrix mat, int n, lambda_matrix H,
                           lambda_matrix U) {
  lambda_vector row;
...
void lambda_matrix_right_hermite(lambda_matrix A, int m, int n, lambda_matrix S,
                                 lambda_matrix U) {
  int i, j, i0 = 0;
...
void lambda_matrix_left_hermite(lambda_matrix A, int m, int n, lambda_matrix S,
                                lambda_matrix V) {
  int i, j, i0 = 0;
...
int lambda_matrix_first_nz_vec(lambda_matrix mat, int rowsize, int colsize,
                               int startrow) {
  int j;
  bool found = false;
...
void lambda_matrix_project_to_null(lambda_matrix B, int rowsize, int colsize,
                                   int k, lambda_vector x) {
...
void lambda_matrix_vector_mult(lambda_matrix matrix, int m, int n,
...
void print_lambda_matrix(FILE* outfile, lambda_matrix matrix, int m, int n) {
  int i;
