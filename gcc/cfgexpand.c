...
static tree gimple_cond_pred_to_tree(gimple stmt) {
...
static tree set_expr_location_r(tree *tp, int *ws ATTRIBUTE_UNUSED,
                                void *data) {
...
static tree gimple_to_tree(gimple stmt) {
  ...
  switch (gimple_code(stmt)) {
    ...
    case GIMPLE_RETURN: {
      ...
      if (retval && retval != error_mark_node) {
        ...
        if (retval != result) {
          ...
          retval = build2(MODIFY_EXPR, TREE_TYPE(result), result, retval);
        }
      }
    ...
    case GIMPLE_ASM: {
      ...
      t = build4(ASM_EXPR, void_type_node, build_string(strlen(s), s), out, in,
                 cl);
    ...
    case GIMPLE_CALL: {
      ...
      if ((fn = gimple_call_fndecl(stmt)) != NULL_TREE && DECL_BUILT_IN(fn)) {
    ...
    case GIMPLE_SWITCH: {
      ...
      t = build3(SWITCH_EXPR, void_type_node, gimple_switch_index(stmt), NULL,
                 label_vec);
    } break;
...
void add_reg_br_prob_note(rtx last, int probability) {
  ...
  for (last = NEXT_INSN(last); last && NEXT_INSN(last); last = NEXT_INSN(last))
    if (JUMP_P(last)) {
      ...
      if (!any_condjump_p(last) || !JUMP_P(NEXT_INSN(last)) ||
          !simplejump_p(NEXT_INSN(last)) || !NEXT_INSN(NEXT_INSN(last)) ||
      ...
      add_reg_note(last, REG_BR_PROB, GEN_INT(REG_BR_PROB_BASE - probability));
      return;
    }
...
static unsigned int get_decl_align_unit(tree decl) {
  unsigned int align;
...
static HOST_WIDE_INT alloc_stack_frame_space(HOST_WIDE_INT size,
                                             HOST_WIDE_INT align) {
...
static void add_stack_var(tree decl) {
  if (stack_vars_num >= stack_vars_alloc) {
    if (stack_vars_alloc)
      stack_vars_alloc = stack_vars_alloc * 3 / 2;
    else
      stack_vars_alloc = 32;
    stack_vars = XRESIZEVEC(struct stack_var, stack_vars, stack_vars_alloc);
  }
...
static size_t triangular_index(size_t i, size_t j) {
  if (i < j) {
    size_t t;
    t = i, i = j, j = t;
  }
...
static void resize_stack_vars_conflict(size_t n) {
...
static void add_stack_var_conflict(size_t x, size_t y) {
...
static bool stack_var_conflict_p(size_t x, size_t y) {
  size_t index = triangular_index(x, y);
  gcc_assert(index < stack_vars_conflict_alloc);
...
static bool aggregate_contains_union_type(tree type) {
  tree field;
...
static void add_alias_set_conflicts(void) {
...
static void union_stack_vars(size_t a, size_t b, HOST_WIDE_INT offset) {
  size_t i, last;
...
static void dump_stack_var_partition(void) {
  ...
  for (si = 0; si < n; ++si) {
    ...
    fprintf(dump_file,
            "Partition %lu: size " HOST_WIDE_INT_PRINT_DEC " align %u\n",
            (unsigned long)i, stack_vars[i].size, stack_vars[i].alignb);
...
static void expand_one_stack_var_at(tree decl, HOST_WIDE_INT offset) {
  HOST_WIDE_INT align;
  rtx x;
...
static void expand_stack_vars(bool (*pred)(tree)) {
  size_t si, i, j, n = stack_vars_num;
  ...
  for (si = 0; si < n; ++si) {
    ...
    offset = alloc_stack_frame_space(stack_vars[i].size, stack_vars[i].alignb);
...
static HOST_WIDE_INT account_stack_vars(void) {
...
static bool defer_stack_allocation(tree var, bool toplevel) {
...
static HOST_WIDE_INT expand_one_var(tree var, bool toplevel,
                                    bool really_expand) {
  if (SUPPORTS_STACK_ALIGNMENT && TREE_TYPE(var) != error_mark_node &&
...
static void expand_used_vars_for_block(tree block, bool toplevel) {
...
static bool stack_protect_decl_phase_1(tree decl) {
  return stack_protect_decl_phase(decl) == 1;
}

static bool stack_protect_decl_phase_2(tree decl) {
...
static void add_stack_protection_conflicts(void) {
  size_t i, j, n = stack_vars_num;
  unsigned char *phase;
...
static HOST_WIDE_INT account_used_vars_for_block(tree block, bool toplevel) {
  size_t i, j, old_sv_num, this_sv_num, new_sv_num;
  tree t;
  HOST_WIDE_INT size = 0;
...
static void expand_used_vars(void) {
  ...
  if (flag_stack_protect == 2 ||
      (flag_stack_protect && (cfun->calls_alloca || has_protected_decls)))
    create_stack_guard();
...
static void maybe_dump_rtl_for_gimple_stmt(gimple stmt, rtx since) {
...
static rtx label_rtx_for_bb(basic_block bb ATTRIBUTE_UNUSED) {
...
static basic_block expand_gimple_cond(basic_block bb, gimple stmt) {
...
static basic_block expand_gimple_tailcall(basic_block bb, gimple stmt,
                                          bool *can_fallthru) {
  rtx last2, last;
  edge e;
  edge_iterator ei;
...
static void construct_exit_block(void) {
  ...
  while (NEXT_INSN(head) && NOTE_P(NEXT_INSN(head)))
    head = NEXT_INSN(head);
  exit_block =
      create_basic_block(NEXT_INSN(head), end, EXIT_BLOCK_PTR->prev_bb);
...
static tree discover_nonconstant_array_refs_r(tree *tp, int *walk_subtrees,
    ...
    *walk_subtrees = 0;
    ...
    while (((TREE_CODE(t) == ARRAY_REF || TREE_CODE(t) == ARRAY_RANGE_REF) &&
           ...
           (TREE_CODE(t) == COMPONENT_REF &&
            (!TREE_OPERAND(t, 2) ||
             is_gimple_min_invariant(TREE_OPERAND(t, 2)))) ||
           TREE_CODE(t) == BIT_FIELD_REF || TREE_CODE(t) == REALPART_EXPR ||
           TREE_CODE(t) == IMAGPART_EXPR || TREE_CODE(t) == VIEW_CONVERT_EXPR ||
           CONVERT_EXPR_P(t))
      t = TREE_OPERAND(t, 0);
...
static void expand_stack_alignment(void) {
  ...
  if (cfun->calls_alloca || cfun->has_nonlocal_label || crtl->has_nonlocal_goto)
    crtl->need_drap = true;

  gcc_assert(crtl->stack_alignment_needed <= crtl->stack_alignment_estimated);
...
static unsigned int gimple_expand_cfg(void) {
  ...
  {
    tree parent;
    for (parent = DECL_CONTEXT(current_function_decl); parent != NULL_TREE;
