...
/* Print out a wrapper macro for a function which corrects the number
   ...
   the end.  */
static void gen_macro(const char *name, int real, int expect) {
  int i;
...
static void gen_proto(rtx insn) {
  ...
  if (name[0] == 'c' || name[0] == 's') {
    if (!strcmp(name, "call") || !strcmp(name, "call_pop") ||
        !strcmp(name, "sibcall") || !strcmp(name, "sibcall_pop"))
      gen_macro(name, num, 4);
    else if (!strcmp(name, "call_value") || !strcmp(name, "call_value_pop") ||
