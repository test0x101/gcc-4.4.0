...
enum reg_note ds_to_dk(ds_t ds) {
  if (ds & DEP_TRUE)
    return REG_DEP_TRUE;
...
ds_t dk_to_ds(enum reg_note dk) {
  switch (dk) {
    case REG_DEP_TRUE:
      return DEP_TRUE;
...
void init_dep_1(dep_t dep, rtx pro, rtx con, enum reg_note type, ds_t ds) {
  DEP_PRO(dep) = pro;
  DEP_CON(dep) = con;
  DEP_TYPE(dep) = type;
  DEP_STATUS(dep) = ds;
...
void init_dep(dep_t dep, rtx pro, rtx con, enum reg_note kind) {
  ds_t ds;
...
static void copy_dep(dep_t to, dep_t from) {
...
#define DUMP_DEP_ALL \
  (DUMP_DEP_PRO | DUMP_DEP_CON | DUMP_DEP_TYPE | DUMP_DEP_STATUS)
...
static void dump_dep(FILE *dump, dep_t dep, int flags) {
  if (flags & 1)
    flags |= DUMP_DEP_ALL;
...
/* Attach L to appear after link X whose &DEP_LINK_NEXT (X) is given by
   PREV_NEXT_P.  */
static void attach_dep_link(dep_link_t l, dep_link_t *prev_nextp) {
  dep_link_t next = *prev_nextp;

  gcc_assert(DEP_LINK_PREV_NEXTP(l) == NULL && DEP_LINK_NEXT(l) == NULL);
...
static void add_to_deps_list(dep_link_t link, deps_list_t l) {
...
static void detach_dep_link(dep_link_t l) {
...
static void remove_from_deps_list(dep_link_t link, deps_list_t list) {
...
static void move_dep_link(dep_link_t link, deps_list_t from, deps_list_t to) {
  remove_from_deps_list(link, from);
  add_to_deps_list(link, to);
}

/* Return true of LINK is not attached to any list.  */
static bool dep_link_is_detached_p(dep_link_t link) {
...
/* Create a dep_node.  */
static dep_node_t create_dep_node(void) {
...
static void delete_dep_node(dep_node_t n) {
...
static bool deps_list_empty_p(deps_list_t l) {
  return DEPS_LIST_N_LINKS(l) == 0;
}

/* Create a new deps_list.  */
static deps_list_t create_deps_list(void) {
...
/* Free deps_list L.  */
static void free_deps_list(deps_list_t l) {
...
static void clear_deps_list(deps_list_t l) {
  do {
...
static void add_dependence_list_and_free(struct deps *, rtx, rtx *, int,
                                         enum reg_note);
...
static int deps_may_trap_p(const_rtx mem) {
...
static rtx sched_get_condition_with_rev(const_rtx insn, bool *rev) {
...
static bool sched_has_condition_p(const_rtx insn) {
  return !!sched_get_condition_with_rev(insn, NULL);
}

/* Return nonzero if conditions COND1 and COND2 can never be both true.  */
static int conditions_mutex_p(const_rtx cond1, const_rtx cond2, bool rev1,
                              bool rev2) {
  if (COMPARISON_P(cond1) && COMPARISON_P(cond2) &&
      GET_CODE(cond1) == (rev1 == rev2 ? reversed_comparison_code(cond2, NULL)
                                       : GET_CODE(cond2)) &&
      XEXP(cond1, 0) == XEXP(cond2, 0) && XEXP(cond1, 1) == XEXP(cond2, 1))
    return 1;
  return 0;
}
...
bool sched_insns_conditions_mutex_p(const_rtx insn1, const_rtx insn2) {
  rtx cond1, cond2;
...
bool sched_insn_is_legitimate_for_speculation_p(const_rtx insn, ds_t ds) {
     ...
     instruction, cannot be speculatively scheduled along.  */
  {
    ...
    if ((ds & BE_IN_DATA) && sched_has_condition_p(insn))
...
void sd_next_list(const_rtx insn, sd_list_types_def *types_ptr,
...
int sd_lists_size(const_rtx insn, sd_list_types_def list_types) {
  int size = 0;
...
bool sd_lists_empty_p(const_rtx insn, sd_list_types_def list_types) {
  return sd_lists_size(insn, list_types) == 0;
}

/* Initialize data for INSN.  */
void sd_init_insn(rtx insn) {
...
static dep_t sd_find_dep_between_no_cache(rtx pro, rtx con, bool resolved_p,
...
dep_t sd_find_dep_between(rtx pro, rtx con, bool resolved_p) {
...
static enum DEPS_ADJUST_RESULT maybe_add_or_update_dep_1(dep_t dep,
                                                         bool resolved_p,
                                                         rtx mem1, rtx mem2) {
...
static enum DEPS_ADJUST_RESULT ask_dependency_caches(dep_t dep) {
  int elem_luid = INSN_LUID(DEP_PRO(dep));
  int insn_luid = INSN_LUID(DEP_CON(dep));

  gcc_assert(true_dependency_cache != NULL && output_dependency_cache != NULL &&
             anti_dependency_cache != NULL);
...
static void set_dependency_caches(dep_t dep) {
  int elem_luid = INSN_LUID(DEP_PRO(dep));
  int insn_luid = INSN_LUID(DEP_CON(dep));
...
/* Type of dependence DEP have changed from OLD_TYPE.  Update dependency
   caches accordingly.  */
static void update_dependency_caches(dep_t dep, enum reg_note old_type) {
  int elem_luid = INSN_LUID(DEP_PRO(dep));
  int insn_luid = INSN_LUID(DEP_CON(dep));
...
static void change_spec_dep_to_hard(sd_iterator_def sd_it) {
  ...
  if (true_dependency_cache != NULL)
    ...
    bitmap_clear_bit(&spec_dependency_cache[INSN_LUID(insn)], INSN_LUID(elem));
}
...
static enum DEPS_ADJUST_RESULT update_dep(
    dep_t dep, dep_t new_dep, sd_iterator_def sd_it ATTRIBUTE_UNUSED,
  ...
  /* Update DEP_STATUS.  */
  {
    ...
    /* Either existing dep or a dep we're adding or both are
       speculative.  */
    {
      if (!(ds & SPECULATIVE) || !(dep_status & SPECULATIVE))
  ...
  if (true_dependency_cache != NULL && res == DEP_CHANGED)
...
static enum DEPS_ADJUST_RESULT add_or_update_dep_1(dep_t new_dep,
  ...
  if (maybe_present_p) {
    ...
    present_dep = sd_find_dep_between_no_cache(
        DEP_PRO(new_dep), DEP_CON(new_dep), resolved_p, &sd_it);
...
static void get_back_and_forw_lists(dep_t dep, bool resolved_p,
...
void sd_add_dep(dep_t dep, bool resolved_p) {
...
enum DEPS_ADJUST_RESULT sd_add_or_update_dep(dep_t dep, bool resolved_p) {
...
void sd_resolve_dep(sd_iterator_def sd_it) {
  dep_node_t node = DEP_LINK_NODE(*sd_it.linkp);
  dep_t dep = DEP_NODE_DEP(node);
  rtx pro = DEP_PRO(dep);
  rtx con = DEP_CON(dep);
...
void sd_copy_back_deps(rtx to, rtx from, bool resolved_p) {
...
void sd_delete_dep(sd_iterator_def sd_it) {
  ...
  get_back_and_forw_lists(dep, sd_it.resolved_p, &con_back_deps,
                          &pro_forw_deps);
...
static void dump_lists(FILE *dump, rtx insn, sd_list_types_def types,
                       int flags) {
  sd_iterator_def sd_it;
  dep_t dep;
  int all;
...
/* Dump all information about deps_lists of INSN specified by TYPES
   to STDERR.  */
void sd_debug_lists(rtx insn, sd_list_types_def types) {
...
static void add_dependence_list(rtx insn, rtx list, int uncond,
...
static void add_dependence_list_and_free(struct deps *deps, rtx insn,
                                         rtx *listp, int uncond,
                                         enum reg_note dep_type) {
  rtx list, next;
...
static int remove_from_dependence_list(rtx insn, rtx *listp) {
  int removed = 0;
...
static int remove_from_both_dependence_lists(rtx insn, rtx *listp, rtx *exprp) {
  int removed = 0;
...
static void delete_all_dependences(rtx insn) {
  sd_iterator_def sd_it;
  dep_t dep;
...
static void fixup_sched_groups(rtx insn) {
  sd_iterator_def sd_it;
  dep_t dep;
  rtx prev_nonnote;
...
static void add_insn_mem_dependence(struct deps *deps, bool read_p, rtx insn,
                                    rtx mem) {
  rtx *insn_list;
  rtx *mem_list;
  rtx link;
...
static void flush_pending_lists(struct deps *deps, rtx insn, int for_read,
                                int for_write) {
  if (for_write) {
    add_dependence_list_and_free(deps, insn, &deps->pending_read_insns, 1,
                                 REG_DEP_ANTI);
    if (!deps->readonly) {
  ...
  add_dependence_list_and_free(deps, insn, &deps->last_pending_memory_flush, 1,
                               for_read ? REG_DEP_ANTI : REG_DEP_OUTPUT);
  if (!deps->readonly) {
...
static void haifa_start_insn(rtx insn) {
  gcc_assert(insn && !cur_insn);

  cur_insn = insn;
}

static void haifa_finish_insn(void) {
  cur_insn = NULL;
}

void haifa_note_reg_set(int regno) {
  SET_REGNO_REG_SET(reg_pending_sets, regno);
}

void haifa_note_reg_clobber(int regno) {
  SET_REGNO_REG_SET(reg_pending_clobbers, regno);
}

void haifa_note_reg_use(int regno) {
  SET_REGNO_REG_SET(reg_pending_uses, regno);
}

static void haifa_note_mem_dep(rtx mem, rtx pending_mem, rtx pending_insn,
                               ds_t ds) {
  if (!(ds & SPECULATIVE)) {
    mem = NULL_RTX;
...
static void haifa_note_dep(rtx elem, ds_t ds) {
  dep_def _dep;
  dep_t dep = &_dep;
...
static void extend_deps_reg_info(struct deps *deps, int regno) {
  ...
  if (max_regno > deps->max_reg) {
    ...
    memset(&deps->reg_last[deps->max_reg], 0,
           (max_regno - deps->max_reg) * sizeof(struct deps_reg));
    deps->max_reg = max_regno;
  }
}
...
void maybe_extend_reg_info_p(void) {
  ...
  if ((unsigned int)max_regno - 1 >= reg_info_p_size) {
    ...
    reg_info_p = (struct reg_info_t *)xrecalloc(
        reg_info_p, new_reg_info_p_size, reg_info_p_size, sizeof(*reg_info_p));
...
static void sched_analyze_reg(struct deps *deps, int regno,
                              enum machine_mode mode, enum rtx_code ref,
                              rtx insn) {
  ...
  /* ??? Reload sometimes emits USEs and CLOBBERs of pseudos that
    ...
    if (REG_N_CALLS_CROSSED(regno) == 0) {
      if (!deps->readonly && ref == USE)
        deps->sched_before_next_call =
            alloc_INSN_LIST(insn, deps->sched_before_next_call);
      else
        add_dependence_list(insn, deps->last_function_call, 1, REG_DEP_ANTI);
    }
  }
}
...
static void sched_analyze_1(struct deps *deps, rtx x, rtx insn) {
    ...
    for (i = XVECLEN(dest, 0) - 1; i >= 0; i--)
      if (XEXP(XVECEXP(dest, 0, i), 0) != 0)
        sched_analyze_1(deps,
                        gen_rtx_CLOBBER(VOIDmode, XEXP(XVECEXP(dest, 0, i), 0)),
                        insn);
  ...
  while (GET_CODE(dest) == STRICT_LOW_PART || GET_CODE(dest) == SUBREG ||
    ...
    if (GET_CODE(dest) == STRICT_LOW_PART || GET_CODE(dest) == ZERO_EXTRACT ||
    ...
    if (!deps->readonly &&
      ...
      while (pending) {
        if (anti_dependence(XEXP(pending_mem, 0), t) &&
          ...
          note_mem_dep(t, XEXP(pending_mem, 0), XEXP(pending, 0), DEP_ANTI);
      ...
      while (pending) {
        if (output_dependence(XEXP(pending_mem, 0), t) &&
            !sched_insns_conditions_mutex_p(insn, XEXP(pending, 0)))
          note_mem_dep(t, XEXP(pending_mem, 0), XEXP(pending, 0), DEP_OUTPUT);
...
static void sched_analyze_2(struct deps *deps, rtx x, rtx insn) {
  ...
  switch (code) {
    ...
    case MEM: {
      ...
      pending_mem = deps->pending_write_mems;
      while (pending) {
        if (true_dependence(XEXP(pending_mem, 0), VOIDmode, t, rtx_varies_p) &&
            !sched_insns_conditions_mutex_p(insn, XEXP(pending, 0)))
      ...
      for (u = deps->last_pending_memory_flush; u; u = XEXP(u, 1)) {
        if (!JUMP_P(XEXP(u, 0)))
          ...
          if ((sched_deps_info->generate_spec_deps) && sel_sched_p() &&
            ...
            ds_t ds = set_dep_weak(DEP_ANTI, BEGIN_CONTROL, MAX_DEP_WEAK);
...
static void sched_analyze_insn(struct deps *deps, rtx x, rtx insn) {
  RTX_CODE code = GET_CODE(x);
  rtx link;
  unsigned i;
  reg_set_iterator rsi;

  can_start_lhs_rhs_p = (NONJUMP_INSN_P(insn) && code == SET);
  ...
  if (reg_pending_barrier) {
    ...
    if (JUMP_P(insn) && !(sel_sched_p() && sel_insn_is_speculation_check(insn)))
      flush_pending_lists(deps, insn, true, true);
  ...
  if (deps->in_post_call_group_p) {
    ...
    if (GET_CODE(tmp) == SUBREG)
      tmp = SUBREG_REG(tmp);
    if ((GET_CODE(tmp) == PLUS || GET_CODE(tmp) == MINUS) &&
        REG_P(XEXP(tmp, 0)) && REGNO(XEXP(tmp, 0)) == STACK_POINTER_REGNUM &&
    ...
    if (src_regno < FIRST_PSEUDO_REGISTER ||
      ...
      if (!deps->readonly && deps->in_post_call_group_p == post_call_initial)
...
void deps_analyze_insn(struct deps *deps, rtx insn) {
  ...
  if (NONJUMP_INSN_P(insn) || JUMP_P(insn)) {
    ...
    add_dependence_list_and_free(deps, insn, &deps->sched_before_next_call, 1,
                                 REG_DEP_ANTI);
...
void deps_start_bb(struct deps *deps, rtx head) {
  gcc_assert(!deps->readonly);
...
static void delete_dep_nodes_in_back_deps(rtx insn, bool resolved_p) {
  sd_iterator_def sd_it;
  dep_t dep;
...
/* Delete (RESOLVED_P) dependencies between HEAD and TAIL together with
   deps_lists.  */
void sched_free_deps(rtx head, rtx tail, bool resolved_p) {
  rtx insn;
...
/* Remove INSN from dependence contexts DEPS.  Caution: reg_conditional_sets
   is not handled.  */
void remove_from_deps(struct deps *deps, rtx insn) {
  int removed;
  unsigned i;
  reg_set_iterator rsi;
...
static void init_deps_data_vector(void) {
  int reserve =
      (sched_max_luid + 1 - VEC_length(haifa_deps_insn_data_def, h_d_i_d));
  if (reserve > 0 && !VEC_space(haifa_deps_insn_data_def, h_d_i_d, reserve))
...
void sched_deps_init(bool global_p) {
...
void extend_dependency_caches(int n, bool create_p) {
  if (create_p || true_dependency_cache) {
    ...
    true_dependency_cache =
        XRESIZEVEC(bitmap_head, true_dependency_cache, luid);
    output_dependency_cache =
        XRESIZEVEC(bitmap_head, output_dependency_cache, luid);
    anti_dependency_cache =
        XRESIZEVEC(bitmap_head, anti_dependency_cache, luid);

    if (current_sched_info->flags & DO_SPECULATION)
      spec_dependency_cache =
          XRESIZEVEC(bitmap_head, spec_dependency_cache, luid);
...
dw_t estimate_dep_weak(rtx mem1, rtx mem2) {
  rtx r1, r2;
  ...
  if (r1 == r2 || (REG_P(r1) && REG_P(r2) && REGNO(r1) == REGNO(r2)))
...
dw_t get_dep_weak_1(ds_t ds, ds_t type) {
  ds = ds & type;

  switch (type) {
    case BEGIN_DATA:
      ds >>= BEGIN_DATA_BITS_OFFSET;
      break;
    case BE_IN_DATA:
      ds >>= BE_IN_DATA_BITS_OFFSET;
      break;
    case BEGIN_CONTROL:
      ds >>= BEGIN_CONTROL_BITS_OFFSET;
      break;
    case BE_IN_CONTROL:
      ds >>= BE_IN_CONTROL_BITS_OFFSET;
      break;
    default:
      gcc_unreachable();
  }
...
ds_t set_dep_weak(ds_t ds, ds_t type, dw_t dw) {
  ...
  switch (type) {
    case BEGIN_DATA:
      ...
      break;
    case BE_IN_DATA:
      ds |= ((ds_t)dw) << BE_IN_DATA_BITS_OFFSET;
      break;
    case BEGIN_CONTROL:
      ds |= ((ds_t)dw) << BEGIN_CONTROL_BITS_OFFSET;
      break;
    case BE_IN_CONTROL:
      ds |= ((ds_t)dw) << BE_IN_CONTROL_BITS_OFFSET;
      break;
    default:
      gcc_unreachable();
  }
...
static ds_t ds_merge_1(ds_t ds1, ds_t ds2, bool max_p) {
  ds_t ds, t;
...
/* Return the join of two dep_statuses DS1 and DS2.  */
ds_t ds_full_merge(ds_t ds, ds_t ds2, rtx mem1, rtx mem2) {
  ds_t new_status = ds | ds2;

  if (new_status & SPECULATIVE) {
    if ((ds && !(ds & SPECULATIVE)) || (ds2 && !(ds2 & SPECULATIVE)))
...
/* Return the join of DS1 and DS2.  Use maximum instead of multiplying
   probabilities.  */
ds_t ds_max_merge(ds_t ds1, ds_t ds2) {
  if (ds1 == 0 && ds2 == 0)
    return 0;
...
ds_t ds_get_speculation_types(ds_t ds) {
  if (ds & BEGIN_DATA)
    ds |= BEGIN_DATA;
  if (ds & BE_IN_DATA)
    ds |= BE_IN_DATA;
...
/* Return a dep status that contains maximal weakness for each speculation
   type present in DS.  */
ds_t ds_get_max_dep_weak(ds_t ds) {
  if (ds & BEGIN_DATA)
...
static void dump_ds(FILE *f, ds_t s) {
  fprintf(f, "{");
...
static void check_dep(dep_t dep, bool relaxed_p) {
  ...
  if (dt == REG_DEP_TRUE)
    ...
    gcc_assert((ds & DEP_OUTPUT) && !(ds & DEP_TRUE));
  else
    gcc_assert((dt == REG_DEP_ANTI) && (ds & DEP_ANTI) &&
