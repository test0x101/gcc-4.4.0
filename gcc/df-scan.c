...
static void df_ref_record(enum df_ref_class, struct df_collection_rec *, rtx,
                          rtx *, basic_block, struct df_insn_info *,
                          enum df_ref_type, enum df_ref_flags, int, int,
                          enum machine_mode);
static void df_def_record_1(struct df_collection_rec *, rtx, basic_block,
                            struct df_insn_info *, enum df_ref_flags);
static void df_defs_record(struct df_collection_rec *, rtx, basic_block,
                           struct df_insn_info *, enum df_ref_flags);
static void df_uses_record(enum df_ref_class, struct df_collection_rec *, rtx *,
                           enum df_ref_type, basic_block, struct df_insn_info *,
...
static df_ref df_ref_create_structure(enum df_ref_class,
                                      ...
                                      enum df_ref_type, enum df_ref_flags, int,
                                      int, enum machine_mode);

static void df_insn_refs_collect(struct df_collection_rec *, basic_block,
                                 struct df_insn_info *);
...
static bool df_insn_refs_verify(struct df_collection_rec *, basic_block, rtx,
                                bool);
...
static void df_scan_set_bb_info(unsigned int index,
...
/* Allocate the problem data for the scanning problem.  This should be
   ...
   be rescanned.  */
void df_scan_alloc(bitmap all_blocks ATTRIBUTE_UNUSED) {
  struct df_scan_problem_data *problem_data;
  ...
  df_scan->block_pool = create_alloc_pool(
      "df_scan_block pool", sizeof(struct df_scan_bb_info), block_size);

  problem_data = XNEW(struct df_scan_problem_data);
  df_scan->problem_data = problem_data;
  df_scan->computed = true;

  problem_data->ref_base_pool = create_alloc_pool(
      "df_scan ref base", sizeof(struct df_base_ref), block_size);
  problem_data->ref_artificial_pool = create_alloc_pool(
      "df_scan ref artificial", sizeof(struct df_artificial_ref), block_size);
  problem_data->ref_regular_pool = create_alloc_pool(
      "df_scan ref regular", sizeof(struct df_regular_ref), block_size);
  problem_data->ref_extract_pool = create_alloc_pool(
      "df_scan ref extract", sizeof(struct df_extract_ref), block_size);
  problem_data->insn_pool = create_alloc_pool(
      "df_scan insn", sizeof(struct df_insn_info), block_size);
  problem_data->reg_pool =
      create_alloc_pool("df_scan reg", sizeof(struct df_reg_info), block_size);
  problem_data->mw_reg_pool = create_alloc_pool(
      "df_scan mw_reg", sizeof(struct df_mw_hardreg), block_size);
...
static void df_scan_start_dump(FILE *file ATTRIBUTE_UNUSED) {
  int i;
  int dcount = 0;
  int ucount = 0;
  int ecount = 0;
  ...
  fprintf(file,
          "\n;;    total ref usage %d{%dd,%du,%de} in %d{%d regular + %d call} "
          "insns.\n",
          dcount + ucount + ecount, dcount, ucount, ecount, icount + ccount,
          icount, ccount);
}
...
void df_grow_reg_info(void) {
  ...
  if (df->regs_size < new_size) {
    ...
    df->eq_use_regs =
        XRESIZEVEC(struct df_reg_info *, df->eq_use_regs, new_size);
...
df_ref df_ref_create(rtx reg, rtx *loc, rtx insn, basic_block bb,
...
static void df_ref_compress_rec(df_ref **vec_ptr, df_ref ref) {
  df_ref *vec = *vec_ptr;
...
void df_ref_remove(df_ref ref) {
  ...
  if (DF_REF_REG_DEF_P(ref)) {
    if (DF_REF_IS_ARTIFICIAL(ref)) {
      struct df_scan_bb_info *bb_info = df_scan_get_bb_info(DF_REF_BBNO(ref));
...
struct df_insn_info *df_insn_create_insn_record(rtx insn) {
  struct df_scan_problem_data *problem_data =
      (struct df_scan_problem_data *)df_scan->problem_data;
...
static void df_ref_chain_delete_du_chain(df_ref *ref_rec) {
  while (*ref_rec) {
    df_ref ref = *ref_rec;
...
static void df_ref_chain_delete(df_ref *ref_rec) {
...
void df_insn_delete(basic_block bb, unsigned int uid) {
...
static void df_free_collection_rec(struct df_collection_rec *collection_rec) {
  struct df_scan_problem_data *problem_data =
      (struct df_scan_problem_data *)df_scan->problem_data;
  df_ref *ref;
...
static unsigned int df_count_refs(bool include_defs, bool include_uses,
...
static void df_reorganize_refs_by_reg(struct df_ref_info *ref_info,
                                      ...
                                      bool include_eq_uses) {
  if (df->analyze_subset)
    df_reorganize_refs_by_reg_by_insn(ref_info, include_defs, include_uses,
                                      include_eq_uses);
  else
    df_reorganize_refs_by_reg_by_reg(ref_info, include_defs, include_uses,
                                     include_eq_uses);
}
...
static unsigned int df_reorganize_refs_by_insn_bb(
    basic_block bb, unsigned int offset, struct df_ref_info *ref_info,
...
static void df_reorganize_refs_by_insn(struct df_ref_info *ref_info,
  ...
  if (df->blocks_to_analyze) {
    ...
    EXECUTE_IF_SET_IN_BITMAP(df->blocks_to_analyze, 0, index, bi) {
      offset = df_reorganize_refs_by_insn_bb(BASIC_BLOCK(index), offset,
                                             ref_info, include_defs,
    ...
    FOR_ALL_BB(bb)
    offset = df_reorganize_refs_by_insn_bb(bb, offset, ref_info, include_defs,
                                           include_uses, include_eq_uses);
    ref_info->table_size = offset;
  }
}
...
void df_insn_change_bb(rtx insn, basic_block new_bb) {
...
static void df_ref_change_reg_with_loc_1(struct df_reg_info *old_df,
  ...
  while (the_ref) {
    if ((!DF_REF_IS_ARTIFICIAL(the_ref)) && (DF_REF_LOC(the_ref)) &&
...
void df_ref_change_reg_with_loc(int old_regno, int new_regno, rtx loc) {
      ...
      switch (REG_NOTE_KIND(note)) {
        ...
        case REG_EQUAL:
          df_uses_record(DF_REF_REGULAR, &collection_rec, &XEXP(note, 0),
                         DF_REF_REG_USE, bb, insn_info, DF_REF_IN_NOTE, -1, -1,
                         0);
        default:
          break;
      }
...
static int df_ref_compare(const void *r1, const void *r2) {
...
static void df_swap_refs(df_ref *ref_vec, int i, int j) {
...
static unsigned int df_sort_and_compress_refs(df_ref *ref_vec,
                                              unsigned int count) {
  unsigned int i;
  unsigned int dist = 0;
...
static bool df_mw_equal_p(struct df_mw_hardreg *mw1,
  ...
  if (!mw2)
    return false;
  return (mw1 == mw2) ||
         (mw1->mw_reg == mw2->mw_reg && mw1->type == mw2->type &&
          mw1->flags == mw2->flags && mw1->start_regno == mw2->start_regno &&
          mw1->end_regno == mw2->end_regno);
}

/* Compare MW1 and MW2 for sorting.  */

static int df_mw_compare(const void *m1, const void *m2) {
  const struct df_mw_hardreg *const mw1 =
      *(const struct df_mw_hardreg *const *)m1;
  const struct df_mw_hardreg *const mw2 =
      *(const struct df_mw_hardreg *const *)m2;
...
static unsigned int df_sort_and_compress_mws(struct df_mw_hardreg **mw_vec,
                                             unsigned int count) {
  struct df_scan_problem_data *problem_data =
      (struct df_scan_problem_data *)df_scan->problem_data;
  unsigned int i;
...
static void df_canonize_collection_rec(
    struct df_collection_rec *collection_rec) {
  if (collection_rec->def_vec)
    collection_rec->next_def = df_sort_and_compress_refs(
        collection_rec->def_vec, collection_rec->next_def);
  if (collection_rec->use_vec)
    collection_rec->next_use = df_sort_and_compress_refs(
        collection_rec->use_vec, collection_rec->next_use);
  if (collection_rec->eq_use_vec)
    collection_rec->next_eq_use = df_sort_and_compress_refs(
        collection_rec->eq_use_vec, collection_rec->next_eq_use);
  if (collection_rec->mw_vec)
    collection_rec->next_mw = df_sort_and_compress_mws(collection_rec->mw_vec,
                                                       collection_rec->next_mw);
}

/* Add the new df_ref to appropriate reg_info/ref_info chains.  */

static void df_install_ref(df_ref this_ref, struct df_reg_info *reg_info,
                           struct df_ref_info *ref_info, bool add_to_table) {
...
static df_ref *df_install_refs(basic_block bb, df_ref *old_vec,
                               unsigned int count,
                               struct df_reg_info **reg_info,
                               struct df_ref_info *ref_info, bool is_notes) {
  if (count) {
    unsigned int i;
    ...
    for (i = 0; i < count; i++) {
      ...
      new_vec[i] = this_ref;
      df_install_ref(this_ref, reg_info[DF_REF_REGNO(this_ref)], ref_info,
                     add_to_table);
    }
...
static struct df_mw_hardreg **df_install_mws(struct df_mw_hardreg **old_vec,
                                             unsigned int count) {
  if (count) {
    struct df_mw_hardreg **new_vec = XNEWVEC(struct df_mw_hardreg *, count + 1);
    memcpy(new_vec, old_vec, sizeof(struct df_mw_hardreg *) * (count + 1));
    return new_vec;
  } else
...
static void df_refs_add_to_chains(struct df_collection_rec *collection_rec,
  ...
  if (insn) {
    ...
    if (collection_rec->def_vec) {
      ...
      insn_rec->defs =
          df_install_refs(bb, collection_rec->def_vec, collection_rec->next_def,
                          df->def_regs, &df->def_info, false);
    }
    if (collection_rec->use_vec) {
      df_scan_free_ref_vec(insn_rec->uses);
      insn_rec->uses =
          df_install_refs(bb, collection_rec->use_vec, collection_rec->next_use,
                          df->use_regs, &df->use_info, false);
    }
    if (collection_rec->eq_use_vec) {
      df_scan_free_ref_vec(insn_rec->eq_uses);
      insn_rec->eq_uses = df_install_refs(bb, collection_rec->eq_use_vec,
                                          collection_rec->next_eq_use,
                                          df->eq_use_regs, &df->use_info, true);
    }
    if (collection_rec->mw_vec) {
      df_scan_free_mws_vec(insn_rec->mw_hardregs);
      insn_rec->mw_hardregs =
          df_install_mws(collection_rec->mw_vec, collection_rec->next_mw);
    }
  } else {
    struct df_scan_bb_info *bb_info = df_scan_get_bb_info(bb->index);
...
static df_ref df_ref_create_structure(enum df_ref_class cl,
                                      struct df_collection_rec *collection_rec,
                                      rtx reg, rtx *loc, basic_block bb,
                                      struct df_insn_info *info,
                                      enum df_ref_type ref_type,
                                      enum df_ref_flags ref_flags, int width,
                                      int offset, enum machine_mode mode) {
  ...
  if ((regno < FIRST_PSEUDO_REGISTER) && (!DF_REF_IS_ARTIFICIAL(this_ref))) {
...
static void df_ref_record(enum df_ref_class cl,
                          struct df_collection_rec *collection_rec, rtx reg,
                          rtx *loc, basic_block bb,
                          struct df_insn_info *insn_info,
                          enum df_ref_type ref_type,
                          enum df_ref_flags ref_flags, int width, int offset,
  ...
  if (regno < FIRST_PSEUDO_REGISTER) {
    ...
    for (i = regno; i < endregno; i++) {
      ref = df_ref_create_structure(cl, collection_rec, regno_reg_rtx[i], loc,
                                    bb, insn_info, ref_type, ref_flags, width,
                                    offset, mode);
...
bool df_read_modify_subreg_p(rtx x) {
  unsigned int isize, osize;
  if (GET_CODE(x) != SUBREG)
    return false;
  isize = GET_MODE_SIZE(GET_MODE(SUBREG_REG(x)));
  osize = GET_MODE_SIZE(GET_MODE(x));
  return isize > osize && isize > REGMODE_NATURAL_SIZE(GET_MODE(SUBREG_REG(x)));
}
...
static void df_def_record_1(struct df_collection_rec *collection_rec, rtx x,
  ...
  if (GET_CODE(dst) == PARALLEL) {
    ...
    for (i = XVECLEN(dst, 0) - 1; i >= 0; i--) {
      ...
      if (GET_CODE(temp) == EXPR_LIST || GET_CODE(temp) == CLOBBER ||
          GET_CODE(temp) == SET)
        df_def_record_1(
            collection_rec, temp, bb, insn_info,
            GET_CODE(temp) == CLOBBER ? flags | DF_REF_MUST_CLOBBER : flags);
    }
    return;
  }
  ...
  if (REG_P(dst)) {
    df_ref_record(cl, collection_rec, dst, loc, bb, insn_info, DF_REF_REG_DEF,
                  flags, width, offset, mode);

    /* We want to keep sp alive everywhere - by making all
       writes to sp also use of sp. */
    if (REGNO(dst) == STACK_POINTER_REGNUM)
      df_ref_record(DF_REF_BASE, collection_rec, dst, NULL, bb, insn_info,
                    DF_REF_REG_USE, flags, width, offset, mode);
...
static void df_defs_record(struct df_collection_rec *collection_rec, rtx x,
                           basic_block bb, struct df_insn_info *insn_info,
                           enum df_ref_flags flags) {
  ...
  if (code == SET || code == CLOBBER) {
    ...
    df_defs_record(collection_rec, COND_EXEC_CODE(x), bb, insn_info,
                   DF_REF_CONDITIONAL);
...
static void df_uses_record(enum df_ref_class cl,
                           struct df_collection_rec *collection_rec, rtx *loc,
                           enum df_ref_type ref_type, basic_block bb,
                           struct df_insn_info *insn_info,
                           enum df_ref_flags flags, int width, int offset,
                           enum machine_mode mode) {
  RTX_CODE code;
  rtx x;
  ...
  switch (code) {
    ...
    case CLOBBER:
      /* If we are clobbering a MEM, mark any registers inside the address
         as being used.  */
      if (MEM_P(XEXP(x, 0)))
        df_uses_record(cl, collection_rec, &XEXP(XEXP(x, 0), 0),
                       DF_REF_REG_MEM_STORE, bb, insn_info, flags, width,
                       offset, mode);
    ...
    case MEM:
      df_uses_record(cl, collection_rec, &XEXP(x, 0), DF_REF_REG_MEM_LOAD, bb,
                     insn_info, flags & DF_REF_IN_NOTE, width, offset, mode);
      return;
    ...
    case REG:
      df_ref_record(cl, collection_rec, x, loc, bb, insn_info, ref_type, flags,
                    width, offset, mode);
      return;
    ...
    case ZERO_EXTRACT: {
      ...
      if (GET_CODE(XEXP(x, 1)) == CONST_INT &&
        ...
        df_uses_record(DF_REF_EXTRACT, collection_rec, &XEXP(x, 0), ref_type,
                       bb, insn_info, flags, width, offset, mode);
        return;
      }
    } break;
    ...
    case SET: {
      ...
      df_uses_record(cl, collection_rec, &SET_SRC(x), DF_REF_REG_USE, bb,
                     insn_info, flags, width, offset, mode);

      switch (GET_CODE(dst)) {
        case SUBREG:
          if (df_read_modify_subreg_p(dst)) {
            df_uses_record(cl, collection_rec, &SUBREG_REG(dst), DF_REF_REG_USE,
                           bb, insn_info,
                           flags | DF_REF_READ_WRITE | DF_REF_SUBREG, width,
                           offset, mode);
            break;
          }
          /* Fall through.  */
        ...
        case STRICT_LOW_PART: {
          ...
          df_uses_record(cl, collection_rec,
                         ...
                         DF_REF_READ_WRITE | DF_REF_STRICT_LOW_PART, width,
                         offset, mode);
        } break;
        case ZERO_EXTRACT: {
          if (GET_CODE(XEXP(dst, 1)) == CONST_INT &&
            ...
            df_uses_record(DF_REF_EXTRACT, collection_rec, &XEXP(dst, 0),
                           DF_REF_REG_USE, bb, insn_info,
                           DF_REF_READ_WRITE | DF_REF_ZERO_EXTRACT, width,
                           offset, mode);
          } else {
            df_uses_record(cl, collection_rec, &XEXP(dst, 1), DF_REF_REG_USE,
                           bb, insn_info, flags, width, offset, mode);
            df_uses_record(cl, collection_rec, &XEXP(dst, 2), DF_REF_REG_USE,
                           bb, insn_info, flags, width, offset, mode);
            df_uses_record(cl, collection_rec, &XEXP(dst, 0), DF_REF_REG_USE,
                           bb, insn_info,
                           DF_REF_READ_WRITE | DF_REF_ZERO_EXTRACT, width,
                           offset, mode);
          }
      ...
      if (code == ASM_OPERANDS) {
        ...
        for (j = 0; j < ASM_OPERANDS_INPUT_LENGTH(x); j++)
          df_uses_record(cl, collection_rec, &ASM_OPERANDS_INPUT(x, j),
                         DF_REF_REG_USE, bb, insn_info, flags, width, offset,
                         mode);
        return;
      }
      break;
    ...
    case POST_MODIFY:
      ...
      df_ref_record(cl, collection_rec, XEXP(x, 0), &XEXP(x, 0), bb, insn_info,
                    DF_REF_REG_DEF,
                    flags | DF_REF_READ_WRITE | DF_REF_PRE_POST_MODIFY, width,
                    offset, mode);
  ...
  /* Recursively scan the operands of this expression.  */
  {
    ...
    for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--) {
      if (fmt[i] == 'e') {
        ...
        df_uses_record(cl, collection_rec, &XEXP(x, i), ref_type, bb, insn_info,
                       flags, width, offset, mode);
      } else if (fmt[i] == 'E') {
        int j;
        for (j = 0; j < XVECLEN(x, i); j++)
          df_uses_record(cl, collection_rec, &XVECEXP(x, i, j), ref_type, bb,
                         insn_info, flags, width, offset, mode);
      }
    }
  }
...
static void df_get_conditional_uses(struct df_collection_rec *collection_rec) {
  unsigned int i;
  for (i = 0; i < collection_rec->next_def; i++) {
    ...
    if (DF_REF_FLAGS_IS_SET(ref, DF_REF_CONDITIONAL)) {
      ...
      use = df_ref_create_structure(
          DF_REF_CLASS(ref), collection_rec, DF_REF_REG(ref), DF_REF_LOC(ref),
          DF_REF_BB(ref), DF_REF_INSN_INFO(ref), DF_REF_REG_USE,
          DF_REF_FLAGS(ref) & ~DF_REF_CONDITIONAL, width, offset, mode);
...
static void df_get_call_refs(struct df_collection_rec *collection_rec,
    ...
    if (GET_CODE(XEXP(note, 0)) == USE)
      ...
      if (REG_P(XEXP(XEXP(note, 0), 0))) {
        ...
        if (!bitmap_bit_p(defs_generated, regno))
          df_defs_record(collection_rec, XEXP(note, 0), bb, insn_info, flags);
      } else
        df_uses_record(DF_REF_REGULAR, collection_rec, &XEXP(note, 0),
                       DF_REF_REG_USE, bb, insn_info, flags, -1, -1, 0);
    }
  ...
  df_ref_record(DF_REF_BASE, collection_rec,
                regno_reg_rtx[STACK_POINTER_REGNUM], NULL, bb, insn_info,
                DF_REF_REG_USE, DF_REF_CALL_STACK_USAGE | flags, -1, -1, 0);

  /* Calls may also reference any of the global registers,
     so they are recorded as used.  */
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
    if (global_regs[i]) {
      df_ref_record(DF_REF_BASE, collection_rec, regno_reg_rtx[i], NULL, bb,
                    insn_info, DF_REF_REG_USE, flags, -1, -1, 0);
      df_ref_record(DF_REF_BASE, collection_rec, regno_reg_rtx[i], NULL, bb,
                    insn_info, DF_REF_REG_DEF, flags, -1, -1, 0);
    }

  is_sibling_call = SIBLING_CALL_P(insn_info->insn);
  EXECUTE_IF_SET_IN_BITMAP(regs_invalidated_by_call_regset, 0, ui, bi) {
    if (!global_regs[ui] && (!bitmap_bit_p(defs_generated, ui)) &&
        (!is_sibling_call || !bitmap_bit_p(df->exit_block_uses, ui) ||
         refers_to_regno_p(ui, ui + 1, crtl->return_rtx, NULL)))
      df_ref_record(DF_REF_BASE, collection_rec, regno_reg_rtx[ui], NULL, bb,
                    insn_info, DF_REF_REG_DEF, DF_REF_MAY_CLOBBER | flags, -1,
                    -1, 0);
  }
...
static void df_insn_refs_collect(struct df_collection_rec *collection_rec,
  ...
  for (note = REG_NOTES(insn_info->insn); note; note = XEXP(note, 1)) {
    switch (REG_NOTE_KIND(note)) {
      ...
      case REG_NON_LOCAL_GOTO:
        ...
        df_ref_record(DF_REF_BASE, collection_rec,
                      regno_reg_rtx[FRAME_POINTER_REGNUM], NULL, bb, insn_info,
                      DF_REF_REG_USE, 0, -1, -1, 0);
#if FRAME_POINTER_REGNUM != HARD_FRAME_POINTER_REGNUM
        df_ref_record(DF_REF_BASE, collection_rec,
                      regno_reg_rtx[HARD_FRAME_POINTER_REGNUM], NULL, bb,
                      insn_info, DF_REF_REG_USE, 0, -1, -1, 0);
#endif
        break;
      default:
        break;
  ...
  df_uses_record(DF_REF_REGULAR, collection_rec, &PATTERN(insn_info->insn),
                 DF_REF_REG_USE, bb, insn_info, 0, -1, -1, 0);
...
static bool df_need_static_chain_reg(struct function *fun) {
  tree fun_context = decl_function_context(fun->decl);
  return fun_context && DECL_NO_STATIC_CHAIN(fun_context) == false;
}

/* Collect all artificial refs at the block level for BB and add them
   to COLLECTION_REC.  */

static void df_bb_refs_collect(struct df_collection_rec *collection_rec,
                               basic_block bb) {
  collection_rec->next_def = 0;
  collection_rec->next_use = 0;
  collection_rec->next_eq_use = 0;
  ...
  if (bb_has_eh_pred(bb)) {
    ...
    for (i = 0;; ++i) {
      ...
      if (regno == INVALID_REGNUM)
        break;
      df_ref_record(DF_REF_ARTIFICIAL, collection_rec, regno_reg_rtx[regno],
                    NULL, bb, NULL, DF_REF_REG_DEF, DF_REF_AT_TOP, -1, -1, 0);
    }
  }
#endif
  ...
  if (bb->flags & BB_NON_LOCAL_GOTO_TARGET)
    df_ref_record(DF_REF_ARTIFICIAL, collection_rec, hard_frame_pointer_rtx,
                  NULL, bb, NULL, DF_REF_REG_DEF, DF_REF_AT_TOP, -1, -1, 0);
  ...
  if (bb->index >= NUM_FIXED_BLOCKS) {
    bitmap_iterator bi;
    unsigned int regno;
    bitmap au = bb_has_eh_pred(bb) ? df->eh_block_artificial_uses
                                   : df->regular_block_artificial_uses;
...
/* Record all the refs within the basic block BB_INDEX and scan the instructions
 * if SCAN_INSNS.  */
...
static void df_get_entry_block_def_set(bitmap entry_block_defs) {
  rtx r;
  int i;
...
static void df_entry_block_defs_collect(
    struct df_collection_rec *collection_rec, bitmap entry_block_defs) {
  unsigned int i;
  bitmap_iterator bi;
...
static void df_get_exit_block_use_set(bitmap exit_block_uses) {
  unsigned int i;
  ...
  if ((!HAVE_epilogue || !epilogue_completed) && crtl->calls_eh_return) {
...
static void df_exit_block_uses_collect(struct df_collection_rec *collection_rec,
  ...
  /* It is deliberate that this is not put in the exit block uses but
     I do not know why.  */
  if (reload_completed && !bitmap_bit_p(exit_block_uses, ARG_POINTER_REGNUM) &&
      bb_has_eh_pred(EXIT_BLOCK_PTR) && fixed_regs[ARG_POINTER_REGNUM])
    df_ref_record(DF_REF_ARTIFICIAL, collection_rec,
                  regno_reg_rtx[ARG_POINTER_REGNUM], NULL, EXIT_BLOCK_PTR, NULL,
                  DF_REF_REG_USE, 0, -1, -1, 0);
#endif
...
static bool df_refs_verify(df_ref *new_rec, df_ref *old_rec,
                           bool abort_if_fail) {
...
static bool df_mws_verify(struct df_mw_hardreg **new_rec,
...
static bool df_insn_refs_verify(struct df_collection_rec *collection_rec,
                                basic_block bb, rtx insn, bool abort_if_fail) {
...
static bool df_bb_verify(basic_block bb) {
  ...
  df_bb_refs_collect(&collection_rec, bb);
  df_refs_verify(collection_rec.def_vec, df_get_artificial_defs(bb->index),
                 true);
  df_refs_verify(collection_rec.use_vec, df_get_artificial_uses(bb->index),
                 true);
  df_free_collection_rec(&collection_rec);
...
void df_scan_verify(void) {
  ...
  gcc_assert(bitmap_equal_p(regular_block_artificial_uses,
                            df->regular_block_artificial_uses));
  gcc_assert(
      bitmap_equal_p(eh_block_artificial_uses, df->eh_block_artificial_uses));
