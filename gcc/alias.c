...
#define REG_BASE_VALUE(X)                         \
  (REGNO(X) < VEC_length(rtx, reg_base_value)     \
       ? VEC_index(rtx, reg_base_value, REGNO(X)) \
       : 0)
...
static inline alias_set_entry get_alias_set_entry(alias_set_type alias_set) {
...
static inline int mems_in_disjoint_alias_sets_p(const_rtx mem1,
                                                const_rtx mem2) {
...
bool alias_set_subset_of(alias_set_type set1, alias_set_type set2) {
  ...
  if (ase != 0 && ((ase->has_zero_child && set1 == 0) ||
...
static int walk_mems_2(rtx *x, rtx mem) {
  if (MEM_P(*x)) {
...
static int walk_mems_1(rtx *x, rtx *pat) {
  if (MEM_P(*x)) {
 ...
 * sets*/
bool insn_alias_sets_conflict_p(rtx insn1, rtx insn2) {
...
bool component_uses_parent_alias_set(const_tree t) {
  while (1) {
...
alias_set_type get_alias_set(tree t) {
  ...
  if (!flag_strict_aliasing || t == error_mark_node ||
      (!TYPE_P(t) && (TREE_TYPE(t) == 0 || TREE_TYPE(t) == error_mark_node)))
    return 0;
  ...
  if (!TYPE_P(t)) {
    ...
    if (INDIRECT_REF_P(inner)) {
      ...
      if (decl && DECL_POINTER_ALIAS_SET_KNOWN_P(decl)) {
        ...
        if (DECL_POINTER_ALIAS_SET(decl) == -2) {
          ...
          /* No two restricted pointers can point at the same thing.
             ...
             decl.  */
          alias_set_type pointed_to_alias_set = get_alias_set(pointed_to_type);
      ...
      /* If we have an INDIRECT_REF via a void pointer, we don't
         ...
         pointer is marked that way.  */
               ...
               (TYPE_REF_CAN_ALIAS_ALL(TREE_TYPE(TREE_OPERAND(inner, 0)))))
        return 0;
    }
    ...
    if (TREE_CODE(t) == VAR_DECL && DECL_RTL_SET_P(t) && MEM_P(DECL_RTL(t)))
...
void record_alias_subset(alias_set_type superset, alias_set_type subset) {
  ...
  if (subset == 0)
    ...
    splay_tree_insert(superset_entry->children, (splay_tree_key)subset, 0);
  }
}
    ...
    case QUAL_UNION_TYPE:
      ...
      if (TYPE_BINFO(type)) {
        ...
        for (binfo = TYPE_BINFO(type), i = 0;
          ...
          record_alias_subset(superset, get_alias_set(BINFO_TYPE(base_binfo)));
      }
  ...
  switch (GET_CODE(src)) {
    ...
    case MEM:
      /* Check for an argument passed in memory.  Only record in the
         ...
         otherwise.  */
      if (copying_arguments && (XEXP(src, 0) == arg_pointer_rtx ||
    ...
    case MINUS: {
      ...
      /* If either base is named object or a special address
         ...
         base term.  */
      if (src_0 != 0 &&
          (GET_CODE(src_0) == SYMBOL_REF || GET_CODE(src_0) == LABEL_REF ||
           (GET_CODE(src_0) == ADDRESS && GET_MODE(src_0) != VOIDmode)))
        return src_0;

      if (src_1 != 0 &&
          (GET_CODE(src_1) == SYMBOL_REF || GET_CODE(src_1) == LABEL_REF ||
           (GET_CODE(src_1) == ADDRESS && GET_MODE(src_1) != VOIDmode)))
        return src_1;
...
static void record_set(rtx dest, const_rtx set, void *data ATTRIBUTE_UNUSED) {
  unsigned regno;
  rtx src;
  int n;
...
static int rtx_equal_for_memref_p(const_rtx x, const_rtx y) {
  int i;
  int j;
  enum rtx_code code;
  const char *fmt;
  ...
  if (COMMUTATIVE_P(x)) {
    ...
    return (
        rtx_equal_for_memref_p(canon_rtx(XEXP(x, 0)), canon_rtx(XEXP(y, 0))) &&
        rtx_equal_for_memref_p(canon_rtx(XEXP(x, 1)), canon_rtx(XEXP(y, 1))));
  } else if (UNARY_P(x))
    return rtx_equal_for_memref_p(canon_rtx(XEXP(x, 0)), canon_rtx(XEXP(y, 0)));
  ...
  switch (GET_CODE(x)) {
    ...
    case MINUS: {
      ...
      /* If either base term is named object or a special address
         ...
         base term.  */
      if (tmp1 != 0 &&
          (GET_CODE(tmp1) == SYMBOL_REF || GET_CODE(tmp1) == LABEL_REF ||
           (GET_CODE(tmp1) == ADDRESS && GET_MODE(tmp1) != VOIDmode)))
        return tmp1;

      if (tmp2 != 0 &&
          (GET_CODE(tmp2) == SYMBOL_REF || GET_CODE(tmp2) == LABEL_REF ||
           (GET_CODE(tmp2) == ADDRESS && GET_MODE(tmp2) != VOIDmode)))
        return tmp2;
...
static int base_alias_check(rtx x, rtx y, enum machine_mode x_mode,
...
static int memrefs_conflict_p(int xsize, rtx x, int ysize, rtx y,
                              HOST_WIDE_INT c) {
  if (GET_CODE(x) == VALUE)
    x = get_addr(x);
  if (GET_CODE(y) == VALUE)
  ...
  if (GET_CODE(x) == GET_CODE(y))
    switch (GET_CODE(x)) {
      case MULT: {
        ...
        if (rtx_equal_for_memref_p(x0, y0))
          return (xsize == 0 || ysize == 0 || (c >= 0 && xsize > c) ||
                  (c < 0 && ysize + c > 0));
  ...
  if (CONSTANT_P(x)) {
    ...
    if (GET_CODE(x) == CONST) {
      if (GET_CODE(y) == CONST)
        return memrefs_conflict_p(xsize, canon_rtx(XEXP(x, 0)), ysize,
...
static const_rtx fixed_scalar_and_varying_struct_p(
    const_rtx mem1, const_rtx mem2, rtx mem1_addr, rtx mem2_addr,
    bool (*varies_p)(const_rtx, bool)) {
  if (!flag_strict_aliasing)
    return NULL_RTX;

  if (MEM_ALIAS_SET(mem2) && MEM_SCALAR_P(mem1) && MEM_IN_STRUCT_P(mem2) &&
      !varies_p(mem1_addr, 1) && varies_p(mem2_addr, 1))
    /* MEM1 is a scalar at a fixed address; MEM2 is a struct at a
       varying address.  */
    return mem1;

  if (MEM_ALIAS_SET(mem1) && MEM_IN_STRUCT_P(mem1) && MEM_SCALAR_P(mem2) &&
      varies_p(mem1_addr, 1) && !varies_p(mem2_addr, 1))
...
static int aliases_everything_p(const_rtx mem) {
...
static bool nonoverlapping_component_refs_p(const_tree x, const_tree y) {
  ...
  found:
    ...
    x = TREE_OPERAND(x, 0);
    y = TREE_OPERAND(y, 0);
  } while (x && y && TREE_CODE(x) == COMPONENT_REF &&
...
int nonoverlapping_memrefs_p(const_rtx x, const_rtx y) {
  ...
  if (TREE_CODE(exprx) == COMPONENT_REF) {
    if (TREE_CODE(expry) == VAR_DECL && POINTER_TYPE_P(TREE_TYPE(expry))) {
  ...
  if (TREE_CODE(expry) == COMPONENT_REF) {
    if (TREE_CODE(exprx) == VAR_DECL && POINTER_TYPE_P(TREE_TYPE(exprx))) {
  ...
  if (!rtx_equal_p(basex, basey))
    return (
        (CONSTANT_P(basex) && CONSTANT_P(basey)) ||
        (CONSTANT_P(basex) && REG_P(basey) &&
         REGNO_PTR_FRAME_P(REGNO(basey))) ||
        (CONSTANT_P(basey) && REG_P(basex) && REGNO_PTR_FRAME_P(REGNO(basex))));
...
int true_dependence(const_rtx mem, enum machine_mode mem_mode, const_rtx x,
  ...
  if (base && (GET_CODE(base) == LABEL_REF ||
               (GET_CODE(base) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P(base))))
    return 0;
  ...
  if (!memrefs_conflict_p(GET_MODE_SIZE(mem_mode), mem_addr, SIZE_FOR_MODE(x),
                          x_addr, 0))
    return 0;
  ...
  return !fixed_scalar_and_varying_struct_p(mem, x, mem_addr, x_addr, varies);
}
...
int canon_true_dependence(const_rtx mem, enum machine_mode mem_mode,
...
static int write_dependence_p(const_rtx mem, const_rtx x, int writep) {
  rtx x_addr, mem_addr;
  ...
  if (!memrefs_conflict_p(SIZE_FOR_MODE(mem), mem_addr, SIZE_FOR_MODE(x),
                          x_addr, 0))
    return 0;

  fixed_scalar = fixed_scalar_and_varying_struct_p(mem, x, mem_addr, x_addr,
                                                   rtx_addr_varies_p);
...
static void memory_modified_1(rtx x, const_rtx pat ATTRIBUTE_UNUSED,
                              void *data) {
  if (MEM_P(x)) {
    if (anti_dependence(x, (const_rtx)data) ||
        output_dependence(x, (const_rtx)data))
  ...
  do {
    ...
    for (insn = get_insns(); insn; insn = NEXT_INSN(insn)) {
      if (INSN_P(insn)) {
        ...
        if (GET_CODE(PATTERN(insn)) == SET && REG_NOTES(insn) != 0 &&
        ...
        if (set != 0 && REG_P(SET_DEST(set)) &&
          ...
          if (note != NULL_RTX && GET_CODE(XEXP(note, 0)) != EXPR_LIST &&
              !rtx_varies_p(XEXP(note, 0), 1) &&
              !reg_overlap_mentioned_p(SET_DEST(set), XEXP(note, 0))) {
            set_reg_known_value(regno, XEXP(note, 0));
            set_reg_known_equiv_p(regno, REG_NOTE_KIND(note) == REG_EQUIV);
          } else if (DF_REG_DEF_COUNT(regno) == 1 && GET_CODE(src) == PLUS &&
                     REG_P(XEXP(src, 0)) &&
          ...
          } else if (DF_REG_DEF_COUNT(regno) == 1 && !rtx_varies_p(src, 1)) {
