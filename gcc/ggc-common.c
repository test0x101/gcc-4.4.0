...
void *ggc_realloc_stat(void *x, size_t size MEM_STAT_DECL) {
  ...
  if (size <= old_size) {
    ...
    VALGRIND_DISCARD(
        VALGRIND_MAKE_MEM_NOACCESS((char *)x + size, old_size - size));
...
void ggc_splay_dont_free(void *x ATTRIBUTE_UNUSED, void *nl) {
  gcc_assert(!nl);
}
...
void ggc_print_common_statistics(FILE *stream ATTRIBUTE_UNUSED,
...
int gt_pch_note_object(void *obj, void *note_ptr_cookie,
                       gt_note_pointers note_ptr_fn, enum gt_types_enum type) {
  struct ptr_data **slot;

  if (obj == NULL || obj == (void *)1)
    return 0;

  slot = (struct ptr_data **)htab_find_slot_with_hash(
      saving_htab, obj, POINTER_HASH(obj), INSERT);
  if (*slot != NULL) {
...
void gt_pch_note_reorder(void *obj, void *note_ptr_cookie,
                         gt_handle_reorder reorder_fn) {
  ...
  data = (struct ptr_data *)htab_find_with_hash(saving_htab, obj,
                                                POINTER_HASH(obj));
...
static hashval_t saving_htab_hash(const void *p) {
  return POINTER_HASH(((const struct ptr_data *)p)->obj);
}

static int saving_htab_eq(const void *p1, const void *p2) {
...
static int call_count(void **slot, void *state_p) {
...
static int call_alloc(void **slot, void *state_p) {
  struct ptr_data *d = (struct ptr_data *)*slot;
  struct traversal_state *state = (struct traversal_state *)state_p;

  d->new_addr = ggc_pch_alloc_object(state->d, d->obj, d->size,
                                     d->note_ptr_fn == gt_pch_p_S, d->type);
...
static int compare_ptr_data(const void *p1_p, const void *p2_p) {
...
static void relocate_ptrs(void *ptr_p, void *state_p) {
  ...
  result = (struct ptr_data *)htab_find_with_hash(saving_htab, *ptr,
                                                  POINTER_HASH(*ptr));
  gcc_assert(result);
  *ptr = result->new_addr;
}
      ...
      for (i = 0; i < rti->nelt; i++) {
        ...
        if (ptr == NULL || ptr == (void *)1) {
          ...
          new_ptr = (struct ptr_data *)htab_find_with_hash(saving_htab, ptr,
                                                           POINTER_HASH(ptr));
          if (fwrite(&new_ptr->new_addr, sizeof(void *), 1, state->f) != 1)
            fatal_error("can't write PCH file: %m");
        }
      }
}
  ...
  for (i = 0; i < state.count; i++) {
    ...
    if (state.ptrs[i]->reorder_fn != NULL)
      state.ptrs[i]->reorder_fn(state.ptrs[i]->obj,
                                state.ptrs[i]->note_ptr_cookie, relocate_ptrs,
                                &state);
...
void gt_pch_restore(FILE *f) {
  ...
  for (rt = gt_ggc_rtab; *rt; rt++)
    for (rti = *rt; rti->base != NULL; rti++)
      for (i = 0; i < rti->nelt; i++)
        if (fread((char *)rti->base + rti->stride * i, sizeof(void *), 1, f) !=
            1)
          fatal_error("can't read PCH file: %m");
...
void *default_gt_pch_get_address(size_t size ATTRIBUTE_UNUSED,
...
int default_gt_pch_use_address(void *base, size_t size, int fd ATTRIBUTE_UNUSED,
...
void *mmap_gt_pch_get_address(size_t size, int fd) {
  void *ret;
...
int mmap_gt_pch_use_address(void *base, size_t size, int fd, size_t offset) {
  void *addr;
  ...
  addr = mmap((caddr_t)base, size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd,
              offset);
...
static int eq_descriptor(const void *p1, const void *p2) {
...
/* Hash table helpers functions.  */
static hashval_t hash_ptr(const void *p) {
  const struct ptr_hash_entry *const d = (const struct ptr_hash_entry *)p;

  return htab_hash_pointer(d->ptr);
}

static int eq_ptr(const void *p1, const void *p2) {
...
static struct loc_descriptor *loc_descriptor(const char *name, int line,
                                             const char *function) {
  struct loc_descriptor loc;
...
void ggc_record_overhead(size_t allocated, size_t overhead, void *ptr,
  ...
  slot =
      htab_find_slot_with_hash(ptr_hash, ptr, htab_hash_pointer(ptr), INSERT);
  gcc_assert(!*slot);
  *slot = p;
...
static int ggc_prune_ptr(void **slot, void *b ATTRIBUTE_UNUSED) {
...
static int final_cmp_statistic(const void *loc1, const void *loc2) {
...
/* Helper for qsort; sort descriptors by amount of memory consumed.  */
static int cmp_statistic(const void *loc1, const void *loc2) {
  const struct loc_descriptor *const l1 =
      *(const struct loc_descriptor *const *)loc1;
  const struct loc_descriptor *const l2 =
      *(const struct loc_descriptor *const *)loc2;
...
static int add_statistics(void **slot, void *b) {
  int *n = (int *)b;
...
#endif
void dump_ggc_loc_statistics(bool final ATTRIBUTE_UNUSED) {
#ifdef GATHER_STATISTICS
  int nentries = 0;
  char s[4096];
  ...
  for (i = 0; i < nentries; i++) {
    ...
    if (d->allocated) {
      ...
      s[48] = 0;
      fprintf(stderr,
              "%-48s %10li:%4.1f%% %10li:%4.1f%% %10li:%4.1f%% %10li:%4.1f%% "
              "%10li\n",
              s, (long)d->collected, (d->collected) * 100.0 / collected,
              (long)d->freed, (d->freed) * 100.0 / freed,
              ...
              (long)d->overhead, d->overhead * 100.0 / overhead,
              (long)d->times);
    }
  }
  fprintf(stderr,
