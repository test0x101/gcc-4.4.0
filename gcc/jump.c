...
static void init_label_info(rtx f) {
  rtx insn;
...
static void mark_all_labels(rtx f) {
  ...
  for (insn = f; insn; insn = NEXT_INSN(insn))
    if (INSN_P(insn)) {
      ...
      if (!INSN_DELETED_P(insn) && JUMP_P(insn) && JUMP_LABEL(insn) == NULL) {
        rtx label_note = NULL;
        rtx pc = pc_set(insn);
        ...
        if (label_note != NULL && pc_src != NULL) {
...

          {
            ...
            gcc_assert(JUMP_LABEL(insn) == XEXP(SET_SRC(label_set), 0));
          }
        }
...
enum rtx_code reversed_comparison_code_parts(enum rtx_code code, const_rtx arg0,
  ...
  if (GET_CODE(arg0) == CONST_INT ||
      (GET_MODE(arg0) != VOIDmode && GET_MODE_CLASS(mode) != MODE_CC &&
       !HONOR_NANS(mode)))
    return reverse_condition(code);
...
enum rtx_code reversed_comparison_code(const_rtx comparison, const_rtx insn) {
  if (!COMPARISON_P(comparison))
    return UNKNOWN;
  return reversed_comparison_code_parts(
      GET_CODE(comparison), XEXP(comparison, 0), XEXP(comparison, 1), insn);
}

/* Return comparison with reversed code of EXP.
   Return NULL_RTX in case we fail to do the reversal.  */
rtx reversed_comparison(const_rtx exp, enum machine_mode mode) {
  enum rtx_code reversed_code = reversed_comparison_code(exp, NULL_RTX);
  if (reversed_code == UNKNOWN)
    return NULL_RTX;
  else
    return simplify_gen_relational(reversed_code, mode, VOIDmode, XEXP(exp, 0),
                                   XEXP(exp, 1));
}
...
int simplejump_p(const_rtx insn) {
...
int condjump_p(const_rtx insn) {
  ...
  if (GET_CODE(x) == LABEL_REF)
    ...
    return (
        GET_CODE(x) == IF_THEN_ELSE &&
        ((GET_CODE(XEXP(x, 2)) == PC && (GET_CODE(XEXP(x, 1)) == LABEL_REF ||
                                         GET_CODE(XEXP(x, 1)) == RETURN)) ||
         (GET_CODE(XEXP(x, 1)) == PC && (GET_CODE(XEXP(x, 2)) == LABEL_REF ||
...
int condjump_in_parallel_p(const_rtx insn) {
  const_rtx x = PATTERN(insn);
...
int any_uncondjump_p(const_rtx insn) {
...
int any_condjump_p(const_rtx insn) {
  const_rtx x = pc_set(insn);
  enum rtx_code a, b;
...
rtx condjump_label(const_rtx insn) {
  rtx x = pc_set(insn);
...
static int returnjump_p_1(rtx *loc, void *data ATTRIBUTE_UNUSED) {
  rtx x = *loc;

  return x &&
         (GET_CODE(x) == RETURN || (GET_CODE(x) == SET && SET_IS_RETURN_P(x)));
}
...
int only_sets_cc0_p(const_rtx x) {
  if (!x)
    return 0;
...
static void mark_jump_label_1(rtx x, rtx insn, bool in_mem, bool is_target) {
  ...
  switch (code) {
    ...
    case SEQUENCE:
      for (i = 0; i < XVECLEN(x, 0); i++)
        mark_jump_label(PATTERN(XVECEXP(x, 0, i)), XVECEXP(x, 0, i), 0);
      return;
    ...
    case ADDR_DIFF_VEC:
      if (!INSN_DELETED_P(insn)) {
        ...
        for (i = 0; i < XVECLEN(x, eltnum); i++)
          mark_jump_label_1(XVECEXP(x, eltnum, i), NULL_RTX, in_mem, is_target);
      }
      return;
  ...
  for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--) {
    if (fmt[i] == 'e')
      ...
      for (j = XVECLEN(x, i) - 1; j >= 0; j--)
        mark_jump_label_1(XVECEXP(x, i, j), insn, in_mem, is_target);
    }
  }
}
...
rtx delete_related_insns(rtx insn) {
  ...
  if (was_code_label && NEXT_INSN(insn) != 0 && JUMP_P(NEXT_INSN(insn)) &&
...
static void redirect_exp_1(rtx *loc, rtx olabel, rtx nlabel, rtx insn) {
  ...
  if (code == SET && nlabel == 0 && SET_DEST(x) == pc_rtx &&
      GET_CODE(SET_SRC(x)) == LABEL_REF && XEXP(SET_SRC(x), 0) == olabel) {
...
void redirect_jump_2(rtx jump, rtx olabel, rtx nlabel, int delete_unused,
                     int invert) {
  rtx note;
...
static int invert_exp_1(rtx x, rtx insn) {
  RTX_CODE code = GET_CODE(x);
  ...
  if (code == IF_THEN_ELSE) {
    ...
    if (reversed_code != UNKNOWN) {
      validate_change(insn, &XEXP(x, 0),
                      gen_rtx_fmt_ee(reversed_code, GET_MODE(comp),
...
int rtx_renumbered_equal_p(const_rtx x, const_rtx y) {
  int i;
    ...
    if (code == SUBREG) {
      ...
      if (reg_renumber[reg_x] >= 0) {
        if (!subreg_offset_representable_p(reg_renumber[reg_x],
                                           ...
                                           GET_MODE(x)))
          return 0;
        reg_x = subreg_regno_offset(
            reg_renumber[reg_x], GET_MODE(SUBREG_REG(x)), byte_x, GET_MODE(x));
        byte_x = 0;
      }
    } else {
      reg_x = REGNO(x);
    ...
    if (GET_CODE(y) == SUBREG) {
      ...
      if (reg_renumber[reg_y] >= 0) {
        if (!subreg_offset_representable_p(reg_renumber[reg_y],
                                           ...
                                           GET_MODE(y)))
          return 0;
        reg_y = subreg_regno_offset(
            reg_renumber[reg_y], GET_MODE(SUBREG_REG(y)), byte_y, GET_MODE(y));
        byte_y = 0;
      }
    } else {
      reg_y = REGNO(y);
