...
static void extract_attribute_substring(struct substring *str) {
...
static hashval_t hash_attr(const void *p) {
...
static int eq_attr(const void *p, const void *q) {
  const struct attribute_spec *const spec = (const struct attribute_spec *)p;
...
static void init_attributes(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(attribute_tables); i++) {
    ...
    for (j = 0; attribute_tables[i][j].name != NULL; j++) {
      ...
      gcc_assert(!(name[0] == '_' && name[1] == '_' && name[len - 1] == '_' &&
                   name[len - 2] == '_'));
  ...
  for (i = 0; i < ARRAY_SIZE(attribute_tables); i++) {
    ...
    for (j = 0; attribute_tables[i][j].name != NULL; j++)
      for (k = j + 1; attribute_tables[i][k].name != NULL; k++)
        gcc_assert(
            strcmp(attribute_tables[i][j].name, attribute_tables[i][k].name));
  }
    ...
    for (j = i + 1; j < ARRAY_SIZE(attribute_tables); j++)
      for (k = 0; attribute_tables[i][k].name != NULL; k++)
        for (l = 0; attribute_tables[j][l].name != NULL; l++)
          gcc_assert(
              strcmp(attribute_tables[i][k].name, attribute_tables[j][l].name));
  }
#endif
  ...
  for (i = 0; i < ARRAY_SIZE(attribute_tables); i++)
    for (k = 0; attribute_tables[i][k].name != NULL; k++) {
      ...
      slot = (const void **)htab_find_slot_with_hash(
          attribute_hash, &str, substring_hash(str.str, str.length), INSERT);
      gcc_assert(!*slot);
...
const struct attribute_spec *lookup_attribute_spec(tree name) {
  struct substring attr;

  attr.str = IDENTIFIER_POINTER(name);
  attr.length = IDENTIFIER_LENGTH(name);
  extract_attribute_substring(&attr);
  return (const struct attribute_spec *)htab_find_with_hash(
      attribute_hash, &attr, substring_hash(attr.str, attr.length));
}
...
tree decl_attributes(tree *node, tree attributes, int flags) {
  ...
  if (TREE_CODE(*node) == FUNCTION_DECL && current_target_pragma &&
  ...
  for (a = attributes; a; a = TREE_CHAIN(a)) {
    ...
    if (spec->type_required && DECL_P(*anode)) {
      ...
      if (!(TREE_CODE(*anode) == TYPE_DECL &&
            *anode == TYPE_NAME(TYPE_MAIN_VARIANT(TREE_TYPE(*anode)))))
    ...
    if (TYPE_P(*anode) && (flags & (int)ATTR_FLAG_TYPE_IN_PLACE) &&
    ...
    if (spec->handler != NULL)
      returned_attrs =
          chainon((*spec->handler)(anode, name, args, flags, &no_add_attrs),
                  returned_attrs);
    ...
    if (!no_add_attrs) {
      ...
      for (a = lookup_attribute(spec->name, old_attrs); a != NULL_TREE;
        ...
        if (DECL_P(*anode))
          ...
          if (*anode == TYPE_MAIN_VARIANT(*anode)) {
            ...
            for (variant = *anode; variant;
              ...
              else if (!lookup_attribute(spec->name, TYPE_ATTRIBUTES(variant)))
                TYPE_ATTRIBUTES(variant) =
                    tree_cons(name, args, TYPE_ATTRIBUTES(variant));
            }
          }
        } else
          *anode = build_type_attribute_variant(
              *anode, tree_cons(name, args, old_attrs));
      }
    }
