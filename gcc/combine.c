...
static rtx reg_nonzero_bits_for_combine(const_rtx, enum machine_mode, const_rtx,
                                        ...
                                        unsigned HOST_WIDE_INT *);
static rtx reg_num_sign_bit_copies_for_combine(const_rtx, enum machine_mode,
...
static rtx make_extraction(enum machine_mode, rtx, HOST_WIDE_INT, rtx,
                           unsigned HOST_WIDE_INT, int, int, int);
...
static rtx combine_split_insns(rtx pattern, rtx insn) {
  rtx ret;
  unsigned int nregs;
...
static rtx *find_single_use_1(rtx dest, rtx *loc) {
  ...
  switch (code) {
    ...
    case SET:
      ...
      if (GET_CODE(SET_DEST(x)) != CC0 && GET_CODE(SET_DEST(x)) != PC &&
          !REG_P(SET_DEST(x)) &&
          !(GET_CODE(SET_DEST(x)) == SUBREG && REG_P(SUBREG_REG(SET_DEST(x))) &&
  ...
  for (i = GET_RTX_LENGTH(code) - 1; i >= 0; i--) {
    if (fmt[i] == 'e') {
      if (dest == XEXP(x, i) || (REG_P(dest) && REG_P(XEXP(x, i)) &&
      ...
      for (j = XVECLEN(x, i) - 1; j >= 0; j--) {
        if (XVECEXP(x, i, j) == dest ||
            (REG_P(dest) && REG_P(XVECEXP(x, i, j)) &&
...
static rtx *find_single_use(rtx dest, rtx insn, rtx *ploc) {
  rtx next;
  rtx *result;
  rtx link;
  ...
  if (dest == cc0_rtx) {
    next = NEXT_INSN(insn);
    if (next == 0 || (!NONJUMP_INSN_P(next) && !JUMP_P(next)))
      return 0;
  ...
  for (next = next_nonnote_insn(insn); next != 0 && !LABEL_P(next);
...
static void do_SUBST_MODE(rtx *into, enum machine_mode newval) {
  struct undo *buf;
...
static bool combine_validate_cost(rtx i1, rtx i2, rtx i3, rtx newpat,
  ...
  if (newi2pat) {
    ...
    new_cost =
        (new_i2_cost > 0 && new_i3_cost > 0) ? new_i2_cost + new_i3_cost : 0;
  } else {
  ...
  if (old_cost > 0 && new_cost > old_cost) {
    if (dump_file) {
      if (i1) {
        fprintf(dump_file, "rejecting combination of insns %d, %d and %d\n",
                INSN_UID(i1), INSN_UID(i2), INSN_UID(i3));
        fprintf(dump_file, "original costs %d + %d + %d = %d\n", i1_cost,
                i2_cost, i3_cost, old_cost);
      } else {
        fprintf(dump_file, "rejecting combination of insns %d and %d\n",
...
static void delete_noop_moves(void) {
  rtx insn, next;
  basic_block bb;
      ...
      for (def_vec = DF_INSN_DEFS(insn); *def_vec; def_vec++) {
        ...
        if (BLOCK_FOR_INSN(use_insn) == bb) {
          ...
          if (regno >= FIRST_PSEUDO_REGISTER ||
            ...
            if (!links)
              LOG_LINKS(use_insn) = alloc_INSN_LIST(insn, LOG_LINKS(use_insn));
          }
        }
...
static void clear_log_links(void) {
  rtx insn;
...
static int combine_instructions(rtx f, unsigned int nregs) {
  rtx insn, next;
#ifdef HAVE_cc0
  rtx prev;
#endif
  ...
  FOR_EACH_BB(this_basic_block) {
    ...
    if (INSN_P(insn) && BLOCK_FOR_INSN(insn)) {
      ...
      note_stores(PATTERN(insn), set_nonzero_bits_and_sign_copies, insn);
      record_dead_and_set_regs(insn);

#ifdef AUTO_INC_DEC
      for (links = REG_NOTES(insn); links; links = XEXP(links, 1))
        if (REG_NOTE_KIND(links) == REG_INC)
          set_nonzero_bits_and_sign_copies(XEXP(links, 0), NULL_RTX, insn);
#endif

      /* Record the current insn_rtx_cost of this instruction.  */
      if (NONJUMP_INSN_P(insn))
        INSN_COST(insn) =
            insn_rtx_cost(PATTERN(insn), optimize_this_for_speed_p);
      if (dump_file)
  ...
  FOR_EACH_BB(this_basic_block) {
    ...
    for (insn = BB_HEAD(this_basic_block);
      ...
      if (INSN_P(insn)) {
        ...
        if (JUMP_P(insn) && (prev = prev_nonnote_insn(insn)) != 0 &&
            NONJUMP_INSN_P(prev) && sets_cc0_p(PATTERN(prev))) {
          if ((next = try_combine(insn, prev, NULL_RTX, &new_direct_jump_p)) !=
              0)
            goto retry;
        ...
        if (NONJUMP_INSN_P(insn) && (prev = prev_nonnote_insn(insn)) != 0 &&
            NONJUMP_INSN_P(prev) && sets_cc0_p(PATTERN(prev)) &&
        ...
        for (links = LOG_LINKS(insn); links; links = XEXP(links, 1))
          if (NONJUMP_INSN_P(XEXP(links, 0)) &&
              ...
              NONJUMP_INSN_P(prev) && sets_cc0_p(PATTERN(prev)) &&
              (next = try_combine(insn, XEXP(links, 0), prev,
                                  &new_direct_jump_p)) != 0)
            goto retry;
#endif
        ...
        for (links = LOG_LINKS(insn); links; links = XEXP(links, 1))
          for (nextlinks = XEXP(links, 1); nextlinks;
               nextlinks = XEXP(nextlinks, 1))
            if ((next = try_combine(insn, XEXP(links, 0), XEXP(nextlinks, 0),
                                    &new_direct_jump_p)) != 0)
              goto retry;
        ...
        for (links = LOG_LINKS(insn); links; links = XEXP(links, 1)) {
          ...
          if ((set = single_set(temp)) != 0 &&
            ...
            next = try_combine(insn, i2mod, NULL_RTX, &new_direct_jump_p);
            i2mod = NULL_RTX;
            if (next)
              goto retry;
            SET_SRC(set) = orig;
...
static void setup_incoming_promotions(rtx first) {
  tree arg;
  ...
  for (arg = DECL_ARGUMENTS(current_function_decl); arg;
    ...
    if (mode1 != mode3 && mode3 == mode4 && uns1 == uns3 &&
...
static void set_nonzero_bits_and_sign_copies(rtx x, const_rtx set, void *data) {
  ...
  if (REG_P(x) &&
      ...
      && !REGNO_REG_SET_P(DF_LR_IN(ENTRY_BLOCK_PTR->next_bb), REGNO(x)) &&
    ...
    if (insn && reg_referenced_p(x, PATTERN(insn)) &&
    ...
    if (SET_DEST(set) == x ||
      ...
      if (GET_MODE_BITSIZE(GET_MODE(x)) < BITS_PER_WORD &&
          ...
          0 != (INTVAL(src) &
                ((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(GET_MODE(x)) - 1))))
      ...
      if (rsp->sign_bit_copies == 0 || rsp->sign_bit_copies > num)
...
static int can_combine_p(rtx insn, rtx i3, rtx pred ATTRIBUTE_UNUSED, rtx succ,
                         rtx *pdest, rtx *psrc) {
  int i;
  const_rtx set = 0;
  rtx src, dest;
...
#endif
  int all_adjacent =
      (succ ? (next_active_insn(insn) == succ && next_active_insn(succ) == i3)
  ...
  if (dest == stack_pointer_rtx
      ...
      || (CALL_P(i3) && (find_reg_fusage(i3, USE, dest) ||
  ...
  if (REG_P(dest)) {
    ...
    if (REG_P(src) && ((REGNO(dest) < FIRST_PSEUDO_REGISTER &&
                        !HARD_REGNO_MODE_OK(REGNO(dest), GET_MODE(dest)))
                       /* Don't extend the life of a hard register unless it is
                          user variable (if we have few registers) or it can't
                          fit into the desired register (meaning something
                          special is going on). Also avoid substituting a return
                          register into I3, because reload can't handle a
                          conflict with constraints of other inputs.  */
  ...
  if (GET_CODE(src) == ASM_OPERANDS && REG_P(dest) &&
  ...
  for (link = REG_NOTES(insn); link; link = XEXP(link, 1))
    if (REG_NOTE_KIND(link) == REG_INC &&
        (JUMP_P(i3) || reg_used_between_p(XEXP(link, 0), insn, i3) ||
         (pred != NULL_RTX &&
...
static int combinable_i3pat(rtx i3, rtx *loc, rtx i2dest, rtx i1dest,
  ...
  if (GET_CODE(x) == SET) {
    ...
    if ((inner_dest != dest &&
         (!MEM_P(inner_dest) || rtx_equal_p(i2dest, inner_dest) ||
        ...
        || (REG_P(inner_dest) && REGNO(inner_dest) < FIRST_PSEUDO_REGISTER &&
    ...
    if (pi3dest_killed && REG_P(subdest) &&
        ...
        && (REGNO(subdest) != ARG_POINTER_REGNUM || !fixed_regs[REGNO(subdest)])
#endif
...
static int cant_combine_insn_p(rtx insn) {
  ...
  if (REG_P(src) && REG_P(dest) &&
      ((REGNO(src) < FIRST_PSEUDO_REGISTER && !fixed_regs[REGNO(src)] &&
        CLASS_LIKELY_SPILLED_P(REGNO_REG_CLASS(REGNO(src)))) ||
       (REGNO(dest) < FIRST_PSEUDO_REGISTER && !fixed_regs[REGNO(dest)] &&
...
static int likely_spilled_retval_p(rtx insn) {
...
static void adjust_for_new_dest(rtx insn) {
...
static bool can_change_dest_mode(rtx x, int added_sets,
  ...
  return (REG_N_SETS(regno) == 1 && !added_sets && !REG_USERVAR_P(x));
}
...
static bool reg_subword_p(rtx x, rtx reg) {
  ...
  return GET_CODE(x) == SUBREG && SUBREG_REG(x) == reg &&
...
static rtx try_combine(rtx i3, rtx i2, rtx i1, int *new_direct_jump_p) {
  ...
  /* Exit early if one of the insns involved can't be used for
     combinations.  */
  if (cant_combine_insn_p(i3) || cant_combine_insn_p(i2) ||
      (i1 && cant_combine_insn_p(i1)) || likely_spilled_retval_p(i3))
    return 0;
  ...
  if (i1 == 0 && (temp = single_set(i2)) != 0 &&
    ...
    if (offset >= 0 && (GET_MODE_BITSIZE(GET_MODE(SET_DEST(temp))) <=
      ...
      if (width < HOST_BITS_PER_WIDE_INT) {
        ...
        mhi =
            ((unsigned HOST_WIDE_INT)1 << (width - HOST_BITS_PER_WIDE_INT)) - 1;
        mlo = -1;
      } else {
        mlo = -1;
        mhi = -1;
  ...
  if (!combinable_i3pat(i3, &PATTERN(i3), i2dest, i1dest,
                        i1 && i2dest_in_i1src && i1_feeds_i3, &i3dest_killed)) {
    undo_all();
    return 0;
  }
  ...
  if (i1 == 0 && added_sets_2 && GET_CODE(PATTERN(i3)) == SET &&
    ...
    if (undobuf.other_insn == 0 &&
        ...
        ((compare_mode =
              SELECT_CC_MODE(GET_CODE(*cc_use), i2src, const0_rtx)) !=
         GET_MODE(SET_DEST(newpat)))) {
      if (can_change_dest_mode(SET_DEST(newpat), added_sets_2, compare_mode)) {
...
#endif
  {
    ...
    subst_low_luid = DF_INSN_LUID(i2);
    newpat =
        subst(PATTERN(i3), i2dest, i2src, 0, !i1_feeds_i3 && i1dest_in_i1src);
    substed_i2 = 1;
  ...
  if (i1 && GET_CODE(newpat) != CLOBBER) {
    ...
    if ((FIND_REG_INC_NOTE(i1, NULL_RTX) != 0 && !i1_feeds_i3 &&
         dead_or_set_p(i2, i1dest) && !reg_overlap_mentioned_p(i1dest, newpat))
...
validate_replacement:
  ...
  if (insn_code_number < 0 && !(added_sets_2 && i1 == 0) &&
      GET_CODE(newpat) == PARALLEL && XVECLEN(newpat, 0) == 2 &&
      GET_CODE(XVECEXP(newpat, 0, 0)) == SET &&
      GET_CODE(XVECEXP(newpat, 0, 1)) == SET && asm_noperands(newpat) < 0) {
  ...
  if (i1 && insn_code_number < 0 && GET_CODE(newpat) == SET &&
    ...
    if (m_split == 0 && !reg_overlap_mentioned_p(i2dest, newpat)) {
      ...
      parallel = gen_rtx_PARALLEL(
          VOIDmode, gen_rtvec(2, newpat, gen_rtx_CLOBBER(VOIDmode, i2dest)));
      m_split = combine_split_insns(parallel, i3);

      /* If that didn't work, try changing the mode of I2DEST if
         we can.  */
      if (m_split == 0 && new_mode != GET_MODE(i2dest) &&
          new_mode != VOIDmode &&
        ...
        if (m_split == 0 && REGNO(i2dest) >= FIRST_PSEUDO_REGISTER) {
          struct undo *buf;
    ...
    if (m_split && NEXT_INSN(m_split) == NULL_RTX) {
      ...
      if (insn_code_number >= 0) {
        ...
        if (REG_P(new_i3_dest) && REG_P(new_i2_dest) &&
    ...
    if (insn_code_number < 0 && (split = find_split_point(&newpat, i3)) != 0
      ...
      if (split_code == MULT && GET_CODE(XEXP(*split, 1)) == CONST_INT &&
        ...
        SUBST(*split, gen_rtx_ASHIFT(split_mode, XEXP(*split, 0), GEN_INT(i)));
      ...
      if (split_code == SUBREG && MEM_P(SUBREG_REG(*split))) {
        ...
        /* Or as a SIGN_EXTEND if LOAD_EXTEND_OP says that that's
           what it really is.  */
        if (LOAD_EXTEND_OP(GET_MODE(SUBREG_REG(*split))) == SIGN_EXTEND)
          SUBST(*split, gen_rtx_SIGN_EXTEND(split_mode, SUBREG_REG(*split)));
        else
#endif
          SUBST(*split, gen_rtx_ZERO_EXTEND(split_mode, SUBREG_REG(*split)));
      }
#endif
      ...
      if (BINARY_P(SET_SRC(newpat)) &&
        ...
        /* Split "((P op Q) op R) op S" where op is PLUS or MULT.  */
        else if ((code == PLUS || code == MULT) && GET_CODE(src_op0) == code &&
          ...
          if ((rtx_equal_p(p, r) && rtx_equal_p(q, s)) ||
            ...
            newi2pat = gen_rtx_SET(VOIDmode, newdest, XEXP(src_op0, 0));
            SUBST(XEXP(setsrc, 0), newdest);
            SUBST(XEXP(setsrc, 1), newdest);
            subst_done = true;
          }
  ...
  /* Check for a case where we loaded from memory in a narrow mode and
     ...
     for a SUBREG of such a register.  */
           ...
           !(temp = SET_DEST(XVECEXP(newpat, 0, 1)),
             (REG_P(temp) &&
              VEC_index(reg_stat_type, reg_stat, REGNO(temp))->nonzero_bits !=
                  0 &&
    ...
    SUBST(SET_SRC(newpat), gen_lowpart(GET_MODE(SET_SRC(newpat)), ni2dest));
    i2_code_number = recog_for_combine(&newi2pat, i2, &new_i2_notes);
     ...
     they are adjacent to each other or not.  */
  {
    ...
    if (p && p != i2 && NONJUMP_INSN_P(p) && newi2pat && sets_cc0_p(newi2pat)) {
      undo_all();
      return 0;
    }
  }
  ...
  if (undobuf.other_insn) {
    ...
    distribute_notes(new_other_notes, undobuf.other_insn, undobuf.other_insn,
...

  {
    ...
    rtx elim_i2 = ((newi2pat && reg_set_p(i2dest, newi2pat)) ||
                           i2dest_in_i2src || i2dest_in_i1src || !i2dest_killed
                       ? 0
                       : i2dest);
    rtx elim_i1 =
        (i1 == 0 || i1dest_in_i1src ||
                 (newi2pat && reg_set_p(i1dest, newi2pat)) || !i1dest_killed
             ? 0
             : i1dest);
    ...
    if (i3notes)
      distribute_notes(i3notes, i3, i3, newi2pat ? i2 : NULL_RTX, elim_i2,
                       elim_i1);
    if (i2notes)
      distribute_notes(i2notes, i2, i3, newi2pat ? i2 : NULL_RTX, elim_i2,
                       elim_i1);
    if (i1notes)
      distribute_notes(i1notes, i1, i3, newi2pat ? i2 : NULL_RTX, elim_i2,
                       elim_i1);
    if (midnotes)
    ...
    if (i3dest_killed) {
      if (newi2pat && reg_set_p(i3dest_killed, newi2pat))
        distribute_notes(gen_rtx_EXPR_LIST(REG_DEAD, i3dest_killed, NULL_RTX),
                         NULL_RTX, i2, NULL_RTX, elim_i2, elim_i1);
      else
        distribute_notes(gen_rtx_EXPR_LIST(REG_DEAD, i3dest_killed, NULL_RTX),
                         NULL_RTX, i3, newi2pat ? i2 : NULL_RTX, elim_i2,
                         elim_i1);
    }
    ...
    if (i2dest_in_i2src) {
      if (newi2pat && reg_set_p(i2dest, newi2pat))
        ...
        distribute_notes(gen_rtx_EXPR_LIST(REG_DEAD, i2dest, NULL_RTX),
                         NULL_RTX, i3, newi2pat ? i2 : NULL_RTX, NULL_RTX,
                         NULL_RTX);
    }
    ...
    if (returnjump_p(i3) || any_uncondjump_p(i3)) {
      *new_direct_jump_p = 1;
      ...
      if ((temp = next_nonnote_insn(i3)) == NULL_RTX || !BARRIER_P(temp))
    ...
    /* An NOOP jump does not need barrier, but it does need cleaning up
       of CFG.  */
    if (GET_CODE(newpat) == SET && SET_SRC(newpat) == pc_rtx &&
  ...
  switch (code) {
    ...
    case MEM:
      ...
      if (GET_CODE(XEXP(x, 0)) == CONST || GET_CODE(XEXP(x, 0)) == SYMBOL_REF) {
        SUBST(XEXP(x, 0), gen_rtx_LO_SUM(Pmode, gen_rtx_HIGH(Pmode, XEXP(x, 0)),
                                         XEXP(x, 0)));
      ...
      if (GET_CODE(XEXP(x, 0)) == PLUS &&
        ...
        rtx seq = combine_split_insns(gen_rtx_SET(VOIDmode, reg, XEXP(x, 0)),
                                      subst_insn);
        ...
        if (seq && NEXT_INSN(seq) != NULL_RTX &&
            NEXT_INSN(NEXT_INSN(seq)) == NULL_RTX && NONJUMP_INSN_P(seq) &&
            GET_CODE(PATTERN(seq)) == SET && SET_DEST(PATTERN(seq)) == reg &&
            !reg_mentioned_p(reg, SET_SRC(PATTERN(seq))) &&
            NONJUMP_INSN_P(NEXT_INSN(seq)) &&
            GET_CODE(PATTERN(NEXT_INSN(seq))) == SET &&
            SET_DEST(PATTERN(NEXT_INSN(seq))) == reg &&
            memory_address_p(GET_MODE(x), SET_SRC(PATTERN(NEXT_INSN(seq))))) {
    ...
    case SET:
      ...
      if (SET_DEST(x) == cc0_rtx && GET_CODE(SET_SRC(x)) != COMPARE &&
          GET_CODE(SET_SRC(x)) != ZERO_EXTRACT && !OBJECT_P(SET_SRC(x)) &&
          !(GET_CODE(SET_SRC(x)) == SUBREG && OBJECT_P(SUBREG_REG(SET_SRC(x)))))
        return &SET_SRC(x);
#endif
      ...
      if (GET_CODE(SET_DEST(x)) == ZERO_EXTRACT &&
        ...
        if (src == mask)
          SUBST(SET_SRC(x), simplify_gen_binary(IOR, mode, dest, or_mask));
        else {
          rtx negmask = gen_int_mode(~(mask << pos), mode);
          SUBST(SET_SRC(x),
                simplify_gen_binary(
                    IOR, mode, simplify_gen_binary(AND, mode, dest, negmask),
                    or_mask));
        }
      ...
      switch (code) {
        case AND:
          ...
          if (GET_CODE(XEXP(SET_SRC(x), 1)) == CONST_INT &&
            ...
            rtx extraction =
                make_extraction(GET_MODE(SET_DEST(x)), XEXP(SET_SRC(x), 0), pos,
                                NULL_RTX, 1, 1, 0, 0);
            if (extraction != 0) {
        ...
        case NE:
          ...
          if (STORE_FLAG_VALUE == -1 && XEXP(SET_SRC(x), 1) == const0_rtx &&
              ...
              1 <= (pos = exact_log2(nonzero_bits(
                        XEXP(SET_SRC(x), 0), GET_MODE(XEXP(SET_SRC(x), 0)))))) {
            enum machine_mode mode = GET_MODE(XEXP(SET_SRC(x), 0));

            SUBST(SET_SRC(x),
                  gen_rtx_NEG(mode, gen_rtx_LSHIFTRT(mode, XEXP(SET_SRC(x), 0),
                                                     GEN_INT(pos))));
      ...
      if (len && pos >= 0 && pos + len <= GET_MODE_BITSIZE(GET_MODE(inner))) {
        ...
        if (unsignedp && len <= 8) {
          SUBST(SET_SRC(x),
                gen_rtx_AND(mode,
                            gen_rtx_LSHIFTRT(mode, gen_lowpart(mode, inner),
                                             GEN_INT(pos)),
      ...
      if (BINARY_P(SET_SRC(x)) && CONSTANT_P(XEXP(SET_SRC(x), 1)) &&
    ...
    case IOR:
      ...
      if (GET_CODE(XEXP(x, 0)) == NOT && GET_CODE(XEXP(x, 1)) == NOT) {
        SUBST(*loc, gen_rtx_NOT(GET_MODE(x),
                                gen_rtx_fmt_ee(code == IOR ? AND : IOR,
                                               GET_MODE(x), XEXP(XEXP(x, 0), 0),
                                               XEXP(XEXP(x, 1), 0))));
        return find_split_point(loc, insn);
      }
...
#define COMBINE_RTX_EQUAL_P(X, Y)                                 \
  ((X) == (Y) || (REG_P(X) && REG_P(Y) && REGNO(X) == REGNO(Y) && \
  ...
  if (code == PARALLEL && GET_CODE(XVECEXP(x, 0, 0)) == SET &&
    ...
    for (i = XVECLEN(x, 0) - 1; i >= 1; i--) {
      ...
      if (!REG_P(dest) && GET_CODE(dest) != CC0 && GET_CODE(dest) != PC) {
    ...
    if (code == SET && (REG_P(SET_DEST(x)) || GET_CODE(SET_DEST(x)) == CC0 ||
      ...
      if (fmt[i] == 'E') {
        ...
        for (j = XVECLEN(x, i) - 1; j >= 0; j--) {
          if (COMBINE_RTX_EQUAL_P(XVECEXP(x, i, j), from)) {
            ...
            new_rtx = subst(XVECEXP(x, i, j), from, to, 0, unique_copy);
        ...
        new_rtx = XEXP(x, i);
        if (in_dest && i == 0 &&
            (((code == SUBREG || code == ZERO_EXTRACT) && REG_P(new_rtx)) ||
          ...
          /* If we are in a SET_DEST, suppress most cases unless we
             ...
             SET_DEST.  */
          new_rtx =
              subst(XEXP(x, i), from, to,
                    (((in_dest && (code == SUBREG || code == STRICT_LOW_PART ||
                                   code == ZERO_EXTRACT)) ||
                      code == SET) &&
                     i == 0),
                    unique_copy);
        ...
        if (GET_CODE(x) == SUBREG && (GET_CODE(new_rtx) == CONST_INT ||
                                      GET_CODE(new_rtx) == CONST_DOUBLE)) {
          enum machine_mode mode = GET_MODE(x);

          x = simplify_subreg(GET_MODE(x), new_rtx, GET_MODE(SUBREG_REG(x)),
                              SUBREG_BYTE(x));
          if (!x)
            x = gen_rtx_CLOBBER(mode, const0_rtx);
        } else if (GET_CODE(new_rtx) == CONST_INT &&
                   GET_CODE(x) == ZERO_EXTEND) {
          x = simplify_unary_operation(ZERO_EXTEND, GET_MODE(x), new_rtx,
                                       GET_MODE(XEXP(x, 0)));
          gcc_assert(x);
        } else
  ...
  if (GET_CODE(x) == FLOAT_EXTEND && MEM_P(XEXP(x, 0)) &&
...
static rtx combine_simplify_rtx(rtx x, enum machine_mode op0_mode,
                                int in_dest) {
  enum rtx_code code = GET_CODE(x);
  enum machine_mode mode = GET_MODE(x);
  rtx temp;
  ...
  if ((BINARY_P(x) &&
       ((!OBJECT_P(XEXP(x, 0)) && !(GET_CODE(XEXP(x, 0)) == SUBREG &&
                                    OBJECT_P(SUBREG_REG(XEXP(x, 0))))) ||
        (!OBJECT_P(XEXP(x, 1)) && !(GET_CODE(XEXP(x, 1)) == SUBREG &&
    ...
    if (cond != 0
      ...
      if (general_operand(true_rtx, VOIDmode) &&
        ...
        if (true_rtx == const_true_rtx && false_rtx == const0_rtx)
          x = simplify_gen_relational(cond_code, mode, VOIDmode, cond, cop1);
        else if (true_rtx == const0_rtx && false_rtx == const_true_rtx &&
                 ((reversed = reversed_comparison_code_parts(
                       cond_code, cond, cop1, NULL)) != UNKNOWN))
          x = simplify_gen_relational(reversed, mode, VOIDmode, cond, cop1);
        ...
        /* Likewise, we can make the negate of a comparison operation
          ...
          x = simplify_gen_unary(
              NEG, mode,
              simplify_gen_relational(cond_code, mode, VOIDmode, cond, cop1),
              mode);
          ...
          x = simplify_gen_unary(
              NEG, mode,
              simplify_gen_relational(reversed, mode, VOIDmode, cond, cop1),
              mode);
        else
  ...
  switch (GET_RTX_CLASS(code)) {
    ...
    case RTX_COMM_COMPARE: {
      ...
      temp = simplify_relational_operation(code, mode, cmp_mode, XEXP(x, 0),
                                           XEXP(x, 1));
    } break;
    case RTX_COMM_ARITH:
    case RTX_BIN_ARITH:
  ...
  if ((code == PLUS || code == MINUS || code == MULT || code == DIV ||
       code == AND || code == IOR || code == XOR || code == SMAX ||
       code == SMIN || code == UMAX || code == UMIN) &&
    ...
    if (GET_CODE(XEXP(x, 0)) == code) {
      ...
      if (inner == 0 && COMMUTATIVE_ARITH_P(x)) {
        ...
        inner = simplify_binary_operation(code, mode, XEXP(XEXP(x, 0), 0),
                                          XEXP(x, 1));
      }
  ...
  switch (code) {
    ...
    case SUBREG:
      ...
      {
        rtx temp;
        temp = simplify_subreg(mode, SUBREG_REG(x), op0_mode, SUBREG_BYTE(x));
        if (temp)
          return temp;
      }
    ...
    case NEG:
      ...
      if (!REG_P(temp) &&
          !(GET_CODE(temp) == SUBREG && REG_P(SUBREG_REG(temp))) &&
    ...
    case TRUNCATE:
      ...
      if (GET_MODE_BITSIZE(mode) <= HOST_BITS_PER_WIDE_INT &&
          ...
          (temp = get_last_value(XEXP(x, 0))) && COMPARISON_P(temp))
    ...
    case PLUS:
      ...
      /* If only the low-order bit of X is possibly nonzero, (plus x -1)
         ...
         "a = (b & 8) == 0;"  */
      if (XEXP(x, 1) == constm1_rtx && !REG_P(XEXP(x, 0)) &&
          !(GET_CODE(XEXP(x, 0)) == SUBREG && REG_P(SUBREG_REG(XEXP(x, 0)))) &&
      ...
      if (GET_MODE_BITSIZE(mode) <= HOST_BITS_PER_WIDE_INT &&
          (nonzero_bits(XEXP(x, 0), mode) & nonzero_bits(XEXP(x, 1), mode)) ==
              0) {
    ...
    case MULT:
      ...
      if (FLOAT_MODE_P(mode) && flag_associative_math &&
        ...
        rtx tem = simplify_binary_operation(MULT, mode, XEXP(XEXP(x, 0), 0),
                                            XEXP(x, 1));
        if (tem)
    ...
    case ORDERED:
      ...
      if (GET_CODE(XEXP(x, 0)) == COMPARE ||
        ...
        if (STORE_FLAG_VALUE == 1 && new_code == NE &&
            GET_MODE_CLASS(mode) == MODE_INT && op1 == const0_rtx &&
            mode == GET_MODE(op0) && nonzero_bits(op0, mode) == 1)
          return gen_lowpart(mode, expand_compound_operation(op0));

        else if (STORE_FLAG_VALUE == 1 && new_code == NE &&
                 GET_MODE_CLASS(mode) == MODE_INT && op1 == const0_rtx &&
                 mode == GET_MODE(op0) &&
                 (num_sign_bit_copies(op0, mode) == GET_MODE_BITSIZE(mode))) {
          op0 = expand_compound_operation(op0);
          return simplify_gen_unary(NEG, mode, gen_lowpart(mode, op0), mode);
        }

        else if (STORE_FLAG_VALUE == 1 && new_code == EQ &&
                 GET_MODE_CLASS(mode) == MODE_INT && op1 == const0_rtx &&
                 mode == GET_MODE(op0) && nonzero_bits(op0, mode) == 1) {
          op0 = expand_compound_operation(op0);
          return simplify_gen_binary(XOR, mode, gen_lowpart(mode, op0),
                                     const1_rtx);
        }
        ...
        if (STORE_FLAG_VALUE == -1 && new_code == NE &&
          ...
          return simplify_gen_unary(NEG, mode, gen_lowpart(mode, op0), mode);
                 ...
                 (num_sign_bit_copies(op0, mode) == GET_MODE_BITSIZE(mode))) {
          op0 = expand_compound_operation(op0);
          return simplify_gen_unary(NOT, mode, gen_lowpart(mode, op0), mode);
        }
    ...
    case ROTATERT:
      ...
      if (GET_CODE(XEXP(x, 1)) == CONST_INT)
        ...
        SUBST(XEXP(x, 1), force_to_mode(XEXP(x, 1), GET_MODE(XEXP(x, 1)),
                                        ((HOST_WIDE_INT)1 << exact_log2(
...
static rtx simplify_if_then_else(rtx x) {
  ...
  if (comparison_p && true_rtx == const_true_rtx && false_rtx == const0_rtx)
    return simplify_gen_relational(true_code, mode, VOIDmode, XEXP(cond, 0),
                                   XEXP(cond, 1));

  /* Also when the truth value has to be reversed.  */
  if (comparison_p && true_rtx == const0_rtx && false_rtx == const_true_rtx &&
  ...
  if (comparison_p &&
    ...
    if (reg_mentioned_p(from, true_rtx))
      true_rtx =
          subst(known_cond(copy_rtx(true_rtx), true_code, from, true_val),
                pc_rtx, pc_rtx, 0, 0);
    if (reg_mentioned_p(from, false_rtx))
      false_rtx =
          subst(known_cond(copy_rtx(false_rtx), false_code, from, false_val),
                pc_rtx, pc_rtx, 0, 0);
  ...
  if (comparison_p && reversed_comparison_code(cond, NULL) != UNKNOWN &&
      (true_rtx == pc_rtx ||
       (CONSTANT_P(true_rtx) && GET_CODE(false_rtx) != CONST_INT &&
        false_rtx != pc_rtx) ||
       true_rtx == const0_rtx || (OBJECT_P(true_rtx) && !OBJECT_P(false_rtx)) ||
  ...
  if (true_code == EQ && !side_effects_p(cond) && !HONOR_NANS(mode) &&
      rtx_equal_p(XEXP(cond, 0), false_rtx) &&
      rtx_equal_p(XEXP(cond, 1), true_rtx))
    return false_rtx;
  else if (true_code == NE && !side_effects_p(cond) && !HONOR_NANS(mode) &&
  ...
  if (GET_MODE_CLASS(mode) == MODE_INT && comparison_p &&
      XEXP(cond, 1) == const0_rtx && GET_CODE(false_rtx) == NEG &&
    ...
    switch (true_code) {
      ...
      case LE:
        return simplify_gen_unary(
            NEG, mode, simplify_gen_unary(ABS, mode, true_rtx, mode), mode);
      default:
        break;
    }
  ...
  if ((!FLOAT_MODE_P(mode) || flag_unsafe_math_optimizations) && comparison_p &&
      rtx_equal_p(XEXP(cond, 0), true_rtx) &&
  ...
  if ((STORE_FLAG_VALUE == 1 || STORE_FLAG_VALUE == -1) && comparison_p &&
    ...
    if ((GET_CODE(t) == PLUS || GET_CODE(t) == MINUS || GET_CODE(t) == IOR ||
    ...
    /* If an identity-zero op is commutative, check whether there
      ...
      c1 = XEXP(t, 0), op = GET_CODE(t), z = f;
             ...
             (num_sign_bit_copies(f, GET_MODE(f)) >
              (unsigned int)(GET_MODE_BITSIZE(mode) -
                             GET_MODE_BITSIZE(
                                 GET_MODE(XEXP(XEXP(t, 0), 0)))))) {
      c1 = XEXP(XEXP(t, 0), 1);
      z = f;
      ...
      m = GET_MODE(XEXP(t, 0));
               ...
               ((nonzero_bits(f, GET_MODE(f)) &
                 ~GET_MODE_MASK(GET_MODE(XEXP(XEXP(t, 0), 0)))) == 0)) {
      c1 = XEXP(XEXP(t, 0), 1);
      z = f;
      op = GET_CODE(XEXP(t, 0));
      ...
      m = GET_MODE(XEXP(t, 0));
               ...
               rtx_equal_p(SUBREG_REG(XEXP(XEXP(t, 0), 1)), f) &&
               ((nonzero_bits(f, GET_MODE(f)) &
                 ~GET_MODE_MASK(GET_MODE(XEXP(XEXP(t, 0), 1)))) == 0)) {
      c1 = XEXP(XEXP(t, 0), 0);
      z = f;
      op = GET_CODE(XEXP(t, 0));
      extend_op = ZERO_EXTEND;
    ...
    if (z) {
      temp = subst(
          simplify_gen_relational(true_code, m, VOIDmode, cond_op0, cond_op1),
          pc_rtx, pc_rtx, 0, 0);
      temp = simplify_gen_binary(
          MULT, m, temp, simplify_gen_binary(MULT, m, c1, const_true_rtx));
  ...
  if (true_code == NE && XEXP(cond, 1) == const0_rtx &&
      ...
      ((1 == nonzero_bits(XEXP(cond, 0), mode) &&
       ...
       ((num_sign_bit_copies(XEXP(cond, 0), mode) == GET_MODE_BITSIZE(mode)) &&
        (i = exact_log2(-INTVAL(true_rtx))) >= 0)))
    return simplify_shift_const(NULL_RTX, ASHIFT, mode,
...
static rtx simplify_set(rtx x) {
  ...
  if ((GET_MODE_CLASS(mode) == MODE_CC || GET_CODE(src) == COMPARE ||
    ...
    tmp = simplify_relational_operation(old_code, compare_mode, VOIDmode, op0,
                                        op1);
    if (!tmp)
      new_code = old_code;
    ...
    if (new_code != old_code) {
      ...
      SUBST(*cc_use,
            gen_rtx_fmt_ee(new_code, GET_MODE(*cc_use), dest, const0_rtx));
      other_changed = 1;
      ...
      if (((old_code == NE && new_code == EQ) ||
        ...
        if ((recog_for_combine(&pat, other_insn, &note) < 0 &&
          ...
          *cc_use = old_cc_use;
          ...
          op0 = simplify_gen_binary(XOR, GET_MODE(op0), op0, GEN_INT(mask));
        }
      }
    }
...
#ifdef CANNOT_CHANGE_MODE_CLASS
      && !(REG_P(dest) && REGNO(dest) < FIRST_PSEUDO_REGISTER &&
           REG_CANNOT_CHANGE_MODE_P(REGNO(dest), GET_MODE(SUBREG_REG(src)),
                                    GET_MODE(src)))
#endif
      &&
      (REG_P(dest) || (GET_CODE(dest) == SUBREG && REG_P(SUBREG_REG(dest))))) {
    SUBST(SET_DEST(x), gen_lowpart(GET_MODE(SUBREG_REG(src)), dest));
  ...
  /* If we have (set (cc0) (subreg ...)), we try to remove the subreg
     in SRC.  */
  if (dest == cc0_rtx && GET_CODE(src) == SUBREG && subreg_lowpart_p(src) &&
  ...
  if (GET_CODE(src) == SUBREG && subreg_lowpart_p(src) &&
    ...
    SUBST(SET_SRC(x), gen_rtx_fmt_e(LOAD_EXTEND_OP(GET_MODE(SUBREG_REG(src))),
  ...
  if (GET_CODE(dest) != PC && GET_CODE(src) == IF_THEN_ELSE &&
    ...
    rtx false_rtx =
        (GET_CODE(XEXP(src, 0)) == NE ? XEXP(src, 2) : XEXP(src, 1));
    ...
    term2 = simplify_gen_binary(AND, GET_MODE(src), XEXP(XEXP(src, 0), 0),
                                true_rtx);
    term3 = simplify_gen_binary(
        AND, GET_MODE(src),
        simplify_gen_unary(NOT, GET_MODE(src), XEXP(XEXP(src, 0), 0),
                           GET_MODE(src)),
        false_rtx);
    ...
    SUBST(SET_SRC(x),
          simplify_gen_binary(
              IOR, GET_MODE(src),
              simplify_gen_binary(IOR, GET_MODE(src), term1, term2), term3));
...
static rtx expand_compound_operation(rtx x) {
  ...
  switch (GET_CODE(x)) {
    ...
    case SIGN_EXTRACT:
      ...
      if (GET_CODE(XEXP(x, 1)) != CONST_INT ||
          GET_CODE(XEXP(x, 2)) != CONST_INT || GET_MODE(XEXP(x, 0)) == VOIDmode)
        return x;
  ...
  if (GET_CODE(x) == SIGN_EXTEND &&
      (GET_MODE_BITSIZE(GET_MODE(x)) <= HOST_BITS_PER_WIDE_INT &&
       ((nonzero_bits(XEXP(x, 0), GET_MODE(XEXP(x, 0))) &
         ~(((unsigned HOST_WIDE_INT)GET_MODE_MASK(GET_MODE(XEXP(x, 0)))) >>
           1)) == 0))) {
  ...
  if (GET_CODE(x) == ZERO_EXTEND) {
    ...
    if (GET_CODE(XEXP(x, 0)) == TRUNCATE &&
        ...
        (GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) <= HOST_BITS_PER_WIDE_INT) &&
    ...
    if (GET_CODE(XEXP(x, 0)) == SUBREG &&
        GET_MODE(SUBREG_REG(XEXP(x, 0))) == GET_MODE(x) &&
        subreg_lowpart_p(XEXP(x, 0)) && COMPARISON_P(SUBREG_REG(XEXP(x, 0))) &&
        (GET_MODE_BITSIZE(GET_MODE(XEXP(x, 0))) <= HOST_BITS_PER_WIDE_INT) &&
        ((HOST_WIDE_INT)STORE_FLAG_VALUE &
         ~GET_MODE_MASK(GET_MODE(XEXP(x, 0)))) == 0)
      return SUBREG_REG(XEXP(x, 0));
  ...
  if (modewidth + len >= pos) {
    ...
    if (!tem || GET_CODE(tem) == CLOBBER)
      return x;
    tem = simplify_shift_const(NULL_RTX, ASHIFT, mode, tem,
                               modewidth - pos - len);
    tem = simplify_shift_const(NULL_RTX, unsignedp ? LSHIFTRT : ASHIFTRT, mode,
                               tem, modewidth - len);
  } else if (unsignedp && len < HOST_BITS_PER_WIDE_INT)
    tem = simplify_and_const_int(
        NULL_RTX, GET_MODE(x),
...
static const_rtx expand_field_assignment(const_rtx x) {
  ...
  while (1) {
    if (GET_CODE(SET_DEST(x)) == STRICT_LOW_PART &&
      ...
      if (BITS_BIG_ENDIAN) {
        if (GET_CODE(pos) == CONST_INT)
          pos = GEN_INT(GET_MODE_BITSIZE(GET_MODE(inner)) - len - INTVAL(pos));
          ...
          pos = simplify_gen_binary(
              MINUS, GET_MODE(pos),
              GEN_INT(GET_MODE_BITSIZE(GET_MODE(inner)) - len), pos);
      }
    }
       ...
       can be done by moving the SUBREG to the source.  */
             ...
             (((GET_MODE_SIZE(GET_MODE(SET_DEST(x))) + (UNITS_PER_WORD - 1)) /
              ...
              ((GET_MODE_SIZE(GET_MODE(SUBREG_REG(SET_DEST(x)))) +
               ...
               UNITS_PER_WORD))) {
      x = gen_rtx_SET(
          VOIDmode, SUBREG_REG(SET_DEST(x)),
          gen_lowpart(GET_MODE(SUBREG_REG(SET_DEST(x))), SET_SRC(x)));
      continue;
    } else
      break;
    ...
    cleared = simplify_gen_binary(
        AND, compute_mode,
        simplify_gen_unary(NOT, compute_mode,
                           simplify_gen_binary(ASHIFT, compute_mode, mask, pos),
                           compute_mode),
        inner);
    masked = simplify_gen_binary(
        ASHIFT, compute_mode,
        simplify_gen_binary(AND, compute_mode,
                            gen_lowpart(compute_mode, SET_SRC(x)), mask),
        pos);

    x = gen_rtx_SET(VOIDmode, copy_rtx(inner),
                    simplify_gen_binary(IOR, compute_mode, cleared, masked));
  }
...
static rtx make_extraction(enum machine_mode mode, rtx inner, HOST_WIDE_INT pos,
                           rtx pos_rtx, unsigned HOST_WIDE_INT len,
  ...
  if (GET_CODE(inner) == SUBREG && subreg_lowpart_p(inner)) {
    ...
    inner = SUBREG_REG(inner);
             ...
             GET_CODE(XEXP(inner, 1)) == CONST_INT && pos_rtx == 0 &&
             pos == 0 && len > (unsigned HOST_WIDE_INT)INTVAL(XEXP(inner, 1))) {
    /* We're extracting the least significant bits of an rtx
       (ashift X (const_int C)), where LEN > C.  Extract the
       least significant (LEN - C) bits of X, giving an rtx
       whose mode is MODE, then shift it left C times.  */
    new_rtx = make_extraction(mode, XEXP(inner, 0), 0, 0,
                              len - INTVAL(XEXP(inner, 1)), unsignedp, in_dest,
                              in_compare);
    if (new_rtx != 0)
  ...
  if (tmode != BLKmode &&
      ((pos_rtx == 0 && (pos % BITS_PER_WORD) == 0 && !MEM_P(inner) &&
        (inner_mode == tmode || !REG_P(inner) ||
         TRULY_NOOP_TRUNCATION(GET_MODE_BITSIZE(tmode),
                               GET_MODE_BITSIZE(inner_mode)) ||
         reg_truncated_to_mode(tmode, inner)) &&
        (!in_dest ||
         (REG_P(inner) && have_insn_for(STRICT_LOW_PART, tmode)))) ||
       (MEM_P(inner) && pos_rtx == 0 &&
        (pos %
         (STRICT_ALIGNMENT ? GET_MODE_ALIGNMENT(tmode) : BITS_PER_UNIT)) == 0
        /* We can't do this if we are widening INNER_MODE (it
           may not be aligned, for one thing).  */
        && GET_MODE_BITSIZE(inner_mode) >= GET_MODE_BITSIZE(tmode) &&
        (inner_mode == tmode || (!mode_dependent_address_p(XEXP(inner, 0)) &&
      ...
      if (tmode != inner_mode) {
        ...
        if (pos || in_dest) {
          ...
          if (WORDS_BIG_ENDIAN && GET_MODE_SIZE(inner_mode) > UNITS_PER_WORD)
            final_word = ((GET_MODE_SIZE(inner_mode) - GET_MODE_SIZE(tmode)) /
                          UNITS_PER_WORD) -
                         final_word;
    ...
    if (flag_expensive_optimizations &&
        (GET_MODE_BITSIZE(tmode) <= HOST_BITS_PER_WIDE_INT &&
         ((nonzero_bits(new_rtx, tmode) &
           ~(((unsigned HOST_WIDE_INT)GET_MODE_MASK(tmode)) >> 1)) == 0))) {
    ...
    return (
        gen_rtx_fmt_e(unsignedp ? ZERO_EXTEND : SIGN_EXTEND, mode, new_rtx));
  }
  ...
  if (BITS_BIG_ENDIAN) {
    ...
    int width = (MEM_P(inner) ? GET_MODE_BITSIZE(is_mode)
                              : GET_MODE_BITSIZE(wanted_inner_mode));

    if (pos_rtx == 0)
      pos = width - len - pos;
    else
      pos_rtx = gen_rtx_MINUS(GET_MODE(pos_rtx), GEN_INT(width - len), pos_rtx);
  ...
  /* If INNER has a wider mode, and this is a constant extraction, try to
     ...
     the value.  */
  if (wanted_inner_mode != VOIDmode && inner_mode != wanted_inner_mode &&
      !pos_rtx && GET_MODE_SIZE(wanted_inner_mode) < GET_MODE_SIZE(is_mode) &&
      MEM_P(inner) && !mode_dependent_address_p(XEXP(inner, 0)) &&
      !MEM_VOLATILE_P(inner)) {
    int offset = 0;
    ...
    if (BYTES_BIG_ENDIAN && GET_MODE_SIZE(inner_mode) < GET_MODE_SIZE(is_mode))
    ...
    if (BYTES_BIG_ENDIAN != BITS_BIG_ENDIAN && is_mode != wanted_inner_mode)
      offset =
          (GET_MODE_SIZE(is_mode) - GET_MODE_SIZE(wanted_inner_mode) - offset);
  ...
  /* If INNER is not memory, we can always get it into the proper mode.  If we
    ...
    inner = force_to_mode(
        inner, wanted_inner_mode,
        pos_rtx || len + orig_pos >= HOST_BITS_PER_WIDE_INT
            ? ~(unsigned HOST_WIDE_INT)0
            : ((((unsigned HOST_WIDE_INT)1 << len) - 1) << orig_pos),
        0);
  }
    ...
    if (flag_expensive_optimizations &&
        (GET_MODE_BITSIZE(GET_MODE(pos_rtx)) <= HOST_BITS_PER_WIDE_INT &&
         ((nonzero_bits(pos_rtx, GET_MODE(pos_rtx)) &
           ~(((unsigned HOST_WIDE_INT)GET_MODE_MASK(GET_MODE(pos_rtx))) >>
             1)) == 0))) {
...
static rtx make_compound_operation(rtx x, enum rtx_code in_code) {
  ...
  next_code =
      (code == MEM || code == PLUS || code == MINUS ? MEM
       : ((code == COMPARE || COMPARISON_P(x)) && XEXP(x, 1) == const0_rtx)
           ? COMPARE
       : in_code == COMPARE ? SET
                            : in_code);
  ...
  switch (code) {
    case ASHIFT:
      ...
      if (in_code == MEM && GET_CODE(XEXP(x, 1)) == CONST_INT &&
        ...
        new_rtx = gen_rtx_MULT(mode, new_rtx,
                               GEN_INT((HOST_WIDE_INT)1 << INTVAL(XEXP(x, 1))));
      }
      break;
    ...
    case AND:
      ...
      /* Same as previous, but for (subreg (lshiftrt ...)) in first op.  */
               ...
               (i = exact_log2(INTVAL(XEXP(x, 1)) + 1)) >= 0) {
        new_rtx =
            make_compound_operation(XEXP(SUBREG_REG(XEXP(x, 0)), 0), next_code);
        new_rtx = make_extraction(GET_MODE(SUBREG_REG(XEXP(x, 0))), new_rtx, 0,
                                  XEXP(SUBREG_REG(XEXP(x, 0)), 1), i, 1, 0,
                                  in_code == COMPARE);
      }
      /* Same as previous, but for (xor/ior (lshiftrt...) (lshiftrt...)).  */
        ...
        new_rtx =
            gen_rtx_fmt_ee(GET_CODE(XEXP(x, 0)), mode,
      ...
      /* If we are have (and (rotate X C) M) and C is larger than the number
        ...
        new_rtx = make_extraction(
            mode, new_rtx,
            (GET_MODE_BITSIZE(mode) - INTVAL(XEXP(XEXP(x, 0), 1))), NULL_RTX, i,
            1, 0, in_code == COMPARE);
      }
      ...
      /* On machines without logical shifts, if the operand of the AND is
        ...
        if ((INTVAL(XEXP(x, 1)) & ~mask) == 0)
          SUBST(
              XEXP(x, 0),
              gen_rtx_ASHIFTRT(
                  mode, make_compound_operation(XEXP(XEXP(x, 0), 0), next_code),
                  XEXP(XEXP(x, 0), 1)));
      }
      ...
      /* If the constant is one less than a power of two, this might be
        ...
        new_rtx = make_extraction(
            mode, make_compound_operation(XEXP(x, 0), next_code), 0, NULL_RTX,
      ...
      /* If we are in a comparison and this is an AND with a power of two,
        ...
        new_rtx = make_extraction(
            mode, make_compound_operation(XEXP(x, 0), next_code), i, NULL_RTX,
            1, 1, 0, 1);
    ...
    case LSHIFTRT:
      /* If the sign bit is known to be zero, replace this with an
         arithmetic shift.  */
      if (have_insn_for(ASHIFTRT, mode) && !have_insn_for(LSHIFTRT, mode) &&
          mode_width <= HOST_BITS_PER_WIDE_INT &&
          (nonzero_bits(XEXP(x, 0), mode) & (1 << (mode_width - 1))) == 0) {
        new_rtx = gen_rtx_ASHIFTRT(
            mode, make_compound_operation(XEXP(x, 0), next_code), XEXP(x, 1));
        break;
      }
    ...
    case ASHIFTRT:
      ...
      if (GET_CODE(rhs) == CONST_INT && GET_CODE(lhs) == ASHIFT &&
          ...
          INTVAL(rhs) >= INTVAL(XEXP(lhs, 1)) && INTVAL(rhs) < mode_width) {
        new_rtx = make_compound_operation(XEXP(lhs, 0), next_code);
        new_rtx = make_extraction(
            mode, new_rtx, INTVAL(rhs) - INTVAL(XEXP(lhs, 1)), NULL_RTX,
      ...
      if (!OBJECT_P(lhs) &&
          !(GET_CODE(lhs) == SUBREG && (OBJECT_P(SUBREG_REG(lhs)))) &&
          GET_CODE(rhs) == CONST_INT && INTVAL(rhs) < HOST_BITS_PER_WIDE_INT &&
          INTVAL(rhs) < mode_width &&
          (new_rtx = extract_left_shift(lhs, INTVAL(rhs))) != 0)
        new_rtx = make_extraction(
            mode, make_compound_operation(new_rtx, next_code), 0, NULL_RTX,
            mode_width - INTVAL(rhs), code == LSHIFTRT, 0, in_code == COMPARE);
    ...
    case SUBREG:
...

      {
        rtx simplified;
        simplified =
            simplify_subreg(GET_MODE(x), tem, GET_MODE(tem), SUBREG_BYTE(x));
...
/* If X refers to a register that equals REG in value, replace these
   references with REG.  */
static rtx canon_reg_for_combine(rtx x, rtx reg) {
  rtx op0, op1, op2;
  const char *fmt;
  int i;
  bool copied;
  ...
  switch (GET_RTX_CLASS(code)) {
    case RTX_UNARY:
      ...
      if (op0 != XEXP(x, 0))
        return simplify_gen_unary(GET_CODE(x), GET_MODE(x), op0, GET_MODE(reg));
      break;
    ...
    case RTX_COMM_COMPARE:
      ...
      op1 = canon_reg_for_combine(XEXP(x, 1), reg);
      if (op0 != XEXP(x, 0) || op1 != XEXP(x, 1))
        return simplify_gen_relational(GET_CODE(x), GET_MODE(x), GET_MODE(op0),
                                       op0, op1);
      break;
    ...
    case RTX_BITFIELD_OPS:
      ...
      if (op0 != XEXP(x, 0) || op1 != XEXP(x, 1) || op2 != XEXP(x, 2))
        return simplify_gen_ternary(GET_CODE(x), GET_MODE(x), GET_MODE(op0),
                                    op0, op1, op2);
...
static rtx gen_lowpart_or_truncate(enum machine_mode mode, rtx x) {
...
static rtx force_to_mode(rtx x, enum machine_mode mode,
                         unsigned HOST_WIDE_INT mask, int just_select) {
  enum rtx_code code = GET_CODE(x);
  ...
  switch (code) {
    ...
    case SUBREG:
      if (subreg_lowpart_p(x)
          ...
          && ((GET_MODE_SIZE(GET_MODE(x)) <
              ...
              (0 == (mask & GET_MODE_MASK(GET_MODE(x)) &
    ...
    case AND:
      ...
      if (GET_CODE(XEXP(x, 1)) == CONST_INT) {
        ...
        if (GET_CODE(x) == AND && GET_CODE(XEXP(x, 1)) == CONST_INT &&
            ((INTVAL(XEXP(x, 1)) & GET_MODE_MASK(GET_MODE(x))) == mask))
          x = XEXP(x, 0);
        ...
        if (GET_CODE(x) == AND && GET_CODE(XEXP(x, 1)) == CONST_INT &&
          ...
          y = simplify_gen_binary(AND, GET_MODE(x), XEXP(x, 0), GEN_INT(cval));
...

      {
        ...
        if (GET_CODE(XEXP(x, 1)) == CONST_INT && exact_log2(-smask) >= 0 &&
          ...
          return force_to_mode(
              plus_constant(XEXP(x, 0), (INTVAL(XEXP(x, 1)) & smask)), mode,
              smask, next_select);
      }
    ...
    case MINUS:
      ...
      if (GET_CODE(XEXP(x, 0)) == CONST_INT &&
        ...
        x = simplify_gen_unary(NEG, GET_MODE(x), XEXP(x, 1), GET_MODE(x));
        return force_to_mode(x, mode, mask, next_select);
      }
      ...
      if (GET_CODE(XEXP(x, 0)) == CONST_INT &&
          ((INTVAL(XEXP(x, 0)) | (HOST_WIDE_INT)fuller_mask) ==
           INTVAL(XEXP(x, 0)))) {
        x = simplify_gen_unary(NOT, GET_MODE(x), XEXP(x, 1), GET_MODE(x));
        return force_to_mode(x, mode, mask, next_select);
      }
    ...
    case XOR:
      ...
      if (GET_CODE(XEXP(x, 0)) == LSHIFTRT &&
        ...
        temp =
            GEN_INT((INTVAL(XEXP(x, 1)) & mask) << INTVAL(XEXP(XEXP(x, 0), 1)));
    ...
    binop:
      ...
      op0 = gen_lowpart_or_truncate(
          op_mode, force_to_mode(XEXP(x, 0), mode, mask, next_select));
      op1 = gen_lowpart_or_truncate(
          op_mode, force_to_mode(XEXP(x, 1), mode, mask, next_select));
    ...
    case ASHIFT:
      ...
      op0 = gen_lowpart_or_truncate(
          op_mode, force_to_mode(XEXP(x, 0), op_mode, mask, next_select));
    ...
    case LSHIFTRT:
      ...
      if (GET_CODE(x) == LSHIFTRT &&
          ...
          && ((int)num_sign_bit_copies(XEXP(x, 0), GET_MODE(XEXP(x, 0))) >=
              exact_log2(mask + 1)))
        x = simplify_gen_binary(
            LSHIFTRT, GET_MODE(x), XEXP(x, 0),
    ...
    case ASHIFTRT:
      ...
      if (GET_MODE_BITSIZE(GET_MODE(x)) <= HOST_BITS_PER_WIDE_INT &&
          (mask ==
           ((unsigned HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(GET_MODE(x)) - 1))))
      ...
      if (GET_CODE(XEXP(x, 1)) == CONST_INT && INTVAL(XEXP(x, 1)) >= 0 &&
        ...
        if (GET_MODE_BITSIZE(GET_MODE(x)) > HOST_BITS_PER_WIDE_INT) {
          ...
          if (GET_MODE_BITSIZE(GET_MODE(x)) - INTVAL(XEXP(x, 1)) <
              HOST_BITS_PER_WIDE_INT)
            nonzero >>= INTVAL(XEXP(x, 1)) + HOST_BITS_PER_WIDE_INT -
                        GET_MODE_BITSIZE(GET_MODE(x));
        } else {
        ...
        if ((mask & ~nonzero) == 0) {
          x = simplify_shift_const(NULL_RTX, LSHIFTRT, GET_MODE(x), XEXP(x, 0),
                                   INTVAL(XEXP(x, 1)));
      ...
      if (mask == 1)
        x = simplify_gen_binary(LSHIFTRT, GET_MODE(x), XEXP(x, 0), XEXP(x, 1));
    ...
    shiftrt:
      ...
      if ((GET_CODE(x) == LSHIFTRT || GET_CODE(x) == ASHIFTRT) &&
          ...
          GET_CODE(XEXP(x, 0)) == ASHIFT && XEXP(XEXP(x, 0), 1) == XEXP(x, 1))
      ...
      if (GET_CODE(XEXP(x, 1)) == CONST_INT && INTVAL(XEXP(x, 1)) >= 0) {
        temp =
            simplify_binary_operation(code == ROTATE ? ROTATERT : ROTATE,
                                      GET_MODE(x), GEN_INT(mask), XEXP(x, 1));
        if (temp && GET_CODE(temp) == CONST_INT)
          SUBST(XEXP(x, 0), force_to_mode(XEXP(x, 0), GET_MODE(x), INTVAL(temp),
                                          next_select));
      }
      break;
    ...
    case NOT:
      ...
      if (GET_CODE(XEXP(x, 0)) == LSHIFTRT &&
          ...
          (INTVAL(XEXP(XEXP(x, 0), 1)) + floor_log2(mask) <
           GET_MODE_BITSIZE(GET_MODE(x))) &&
          INTVAL(XEXP(XEXP(x, 0), 1)) < HOST_BITS_PER_WIDE_INT) {
        temp = gen_int_mode(mask << INTVAL(XEXP(XEXP(x, 0), 1)), GET_MODE(x));
        temp = simplify_gen_binary(XOR, GET_MODE(x), XEXP(XEXP(x, 0), 0), temp);
        x = simplify_gen_binary(LSHIFTRT, GET_MODE(x), temp,
                                XEXP(XEXP(x, 0), 1));
    ...
    case IF_THEN_ELSE:
      ...
      SUBST(XEXP(x, 1), gen_lowpart_or_truncate(
                            GET_MODE(x), force_to_mode(XEXP(x, 1), mode, mask,
                                                       next_select)));
      SUBST(XEXP(x, 2), gen_lowpart_or_truncate(
                            GET_MODE(x), force_to_mode(XEXP(x, 2), mode, mask,
                                                       next_select)));
      break;
...
static rtx if_then_else_cond(rtx x, rtx *ptrue, rtx *pfalse) {
  ...
  /* If this is a unary operation whose operand has one of two values, apply
    ...
    *pfalse = simplify_gen_unary(code, mode, false0, GET_MODE(XEXP(x, 0)));
    return cond0;
  }
  ...
  /* If this is a binary operation, see if either side has only one of two
    ...
    if ((cond0 != 0 || cond1 != 0) &&
      ...
      if (COMPARISON_P(x)) {
        *ptrue = simplify_gen_relational(code, mode, VOIDmode, true0, true1);
        *pfalse = simplify_gen_relational(code, mode, VOIDmode, false0, false1);
      } else {
    ...
    if ((STORE_FLAG_VALUE == 1 || STORE_FLAG_VALUE == -1) &&
      ...
      if (COMPARISON_P(cond0) && COMPARISON_P(cond1) &&
        ...
        *pfalse = simplify_gen_binary(
            MULT, mode,
            (code == MINUS ? simplify_gen_unary(NEG, mode, op1, mode) : op1),
            const_true_rtx);
        return cond0;
      }
    }
  ...
  /* If X is a SUBREG, we can narrow both the true and false values
    ...
    true0 = simplify_gen_subreg(mode, true0, GET_MODE(SUBREG_REG(x)),
                                SUBREG_BYTE(x));
    false0 = simplify_gen_subreg(mode, false0, GET_MODE(SUBREG_REG(x)),
                                 SUBREG_BYTE(x));
    if (true0 && false0) {
      *ptrue = true0;
      *pfalse = false0;
...
static rtx known_cond(rtx x, enum rtx_code cond, rtx reg, rtx val) {
  ...
  if (cond == EQ && rtx_equal_p(x, reg) && !FLOAT_MODE_P(GET_MODE(x)) &&
  ...
  if (code == ABS && rtx_equal_p(XEXP(x, 0), reg) && val == const0_rtx)
    switch (cond) {
      ...
      case LT:
      case LE:
        return simplify_gen_unary(NEG, GET_MODE(XEXP(x, 0)), XEXP(x, 0),
                                  GET_MODE(XEXP(x, 0)));
      default:
        break;
    }
  ...
  /* The only other cases we handle are MIN, MAX, and comparisons if the
    ...
    if (SUBREG_REG(x) != r) {
      /* We must simplify subreg here, before we lose track of the
         original inner_mode.  */
      new_rtx = simplify_subreg(GET_MODE(x), r, inner_mode, SUBREG_BYTE(x));
      if (new_rtx)
        return new_rtx;
      else
  ...
  /* We don't have to handle SIGN_EXTEND here, because even in the
    ...
    if (XEXP(x, 0) != r) {
      ...
      new_rtx =
          simplify_unary_operation(ZERO_EXTEND, GET_MODE(x), r, inner_mode);
      if (new_rtx)
        return new_rtx;
      else
        SUBST(XEXP(x, 0), r);
...
static int rtx_equal_for_field_assignment_p(rtx x, rtx y) {
  ...
  if (MEM_P(y) && GET_CODE(x) == SUBREG && MEM_P(SUBREG_REG(x)) &&
      rtx_equal_p(SUBREG_REG(x), gen_lowpart(GET_MODE(SUBREG_REG(x)), y)))
    return 1;
...
static rtx make_field_assignment(rtx x) {
  ...
  if (GET_CODE(src) == AND && GET_CODE(XEXP(src, 0)) == ROTATE &&
    ...
    assign =
        make_extraction(VOIDmode, dest, 0, XEXP(XEXP(src, 0), 1), 1, 1, 1, 0);
    if (assign != 0)
  ...
  if (GET_CODE(src) == AND && GET_CODE(XEXP(src, 0)) == SUBREG &&
    ...
    assign = make_extraction(VOIDmode, dest, 0,
                             XEXP(SUBREG_REG(XEXP(src, 0)), 1), 1, 1, 1, 0);
    if (assign != 0)
      return gen_rtx_SET(VOIDmode, assign, const0_rtx);
    return x;
  }
  ...
  /* If DEST is already a field assignment, i.e. ZERO_EXTRACT, and the
     ...
     the AND.  */
  if (GET_CODE(dest) == ZERO_EXTRACT && GET_CODE(XEXP(dest, 1)) == CONST_INT &&
  ...
  mode = (GET_CODE(assign) == STRICT_LOW_PART ? GET_MODE(XEXP(assign, 0))
                                              : GET_MODE(assign));

  /* Shift OTHER right POS places and make it the source, restricting it
     to the proper length and mode.  */

  src = canon_reg_for_combine(
      simplify_shift_const(NULL_RTX, LSHIFTRT, GET_MODE(src), other, pos),
      dest);
  ...
  switch (inner_code) {
    ...
    case SUBREG:
      ...
      if (GET_MODE(SUBREG_REG(lhs)) != GET_MODE(SUBREG_REG(rhs)) ||
          SUBREG_BYTE(lhs) != SUBREG_BYTE(rhs) || !subreg_lowpart_p(lhs) ||
...
static rtx distribute_and_simplify_rtx(rtx x, int n) {
  enum machine_mode mode;
  ...
  tmp = apply_distributive_law(
      simplify_gen_binary(inner_code, mode, new_op0, new_op1));
...
static rtx simplify_and_const_int_1(enum machine_mode mode, rtx varop,
  ...
  if (GET_CODE(varop) == IOR || GET_CODE(varop) == XOR)
    return gen_lowpart(mode,
                       apply_distributive_law(simplify_gen_binary(
...
static rtx simplify_and_const_int(rtx x, enum machine_mode mode, rtx varop,
                                  unsigned HOST_WIDE_INT constop) {
...
static rtx reg_nonzero_bits_for_combine(
    const_rtx x, enum machine_mode mode, const_rtx known_x ATTRIBUTE_UNUSED,
  ...
  if (rsp->last_set_value != 0 &&
      ...
      ((rsp->last_set_label >= label_tick_ebb_start &&
       ...
       (REGNO(x) >= FIRST_PSEUDO_REGISTER && REG_N_SETS(REGNO(x)) == 1 &&
        !REGNO_REG_SET_P(DF_LR_IN(ENTRY_BLOCK_PTR->next_bb), REGNO(x))))) {
  ...
  if (tem) {
    ...
    if (GET_MODE_BITSIZE(GET_MODE(x)) < GET_MODE_BITSIZE(mode) &&
        GET_CODE(tem) == CONST_INT && INTVAL(tem) > 0 &&
        0 != (INTVAL(tem) &
              ((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(GET_MODE(x)) - 1))))
...
static rtx reg_num_sign_bit_copies_for_combine(
    const_rtx x, enum machine_mode mode, const_rtx known_x ATTRIBUTE_UNUSED,
    enum machine_mode known_mode ATTRIBUTE_UNUSED,
    unsigned int known_ret ATTRIBUTE_UNUSED, unsigned int *result) {
  rtx tem;
  reg_stat_type *rsp;

  rsp = VEC_index(reg_stat_type, reg_stat, REGNO(x));
  if (rsp->last_set_value != 0 && rsp->last_set_mode == mode &&
      ((rsp->last_set_label >= label_tick_ebb_start &&
        rsp->last_set_label < label_tick) ||
       (rsp->last_set_label == label_tick &&
        DF_INSN_LUID(rsp->last_set) < subst_low_luid) ||
...
unsigned int extended_count(const_rtx x, enum machine_mode mode,
                            int unsignedp) {
...
/* This function is called from `simplify_shift_const' to merge two
   ...
   If *POP0 or OP1 are UNKNOWN, it means no operation is required.  Only NEG,
   PLUS, IOR, XOR, and AND are supported.  We may set *POP0 to SET if the proper
...
static int merge_outer_ops(enum rtx_code *pop0, HOST_WIDE_INT *pconst0,
  ...
  if (const0 == 0 && (op0 == IOR || op0 == XOR || op0 == PLUS))
    ...
    op0 = SET;
  else if ((unsigned HOST_WIDE_INT)const0 == GET_MODE_MASK(mode) && op0 == AND)
    op0 = UNKNOWN;
...
static rtx simplify_shift_const_1(enum rtx_code code,
  ...
  while (count != 0) {
    ...
    shift_mode =
        (code == ASHIFTRT || code == LSHIFTRT || code == ROTATE ? result_mode
                                                                : mode);
    ...
    /* An arithmetic right shift of a quantity known to be -1 or 0
       is a no-op.  */
    if (code == ASHIFTRT && (num_sign_bit_copies(varop, shift_mode) ==
    ...
    if (code == ASHIFTRT && (count + num_sign_bit_copies(varop, shift_mode) >=
                             GET_MODE_BITSIZE(shift_mode)))
      count = GET_MODE_BITSIZE(shift_mode) - 1;
    ...
    if (GET_MODE_BITSIZE(shift_mode) <= HOST_BITS_PER_WIDE_INT &&
        ...
        ((nonzero_bits(varop, shift_mode) &
          ((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(shift_mode) - 1))) == 0))
      code = LSHIFTRT;
    ...
    switch (GET_CODE(varop)) {
      ...
      case MEM:
        ...
        if ((code == ASHIFTRT || code == LSHIFTRT) &&
            ...
            (tmode = mode_for_size(GET_MODE_BITSIZE(mode) - count, MODE_INT,
                                   1)) != BLKmode) {
          new_rtx = adjust_address_nv(
              varop, tmode, BYTES_BIG_ENDIAN ? 0 : count / BITS_PER_UNIT);

          varop = gen_rtx_fmt_e(code == ASHIFTRT ? SIGN_EXTEND : ZERO_EXTEND,
                                mode, new_rtx);
          count = 0;
          continue;
        }
      ...
      case MULT:
        ...
        if (GET_CODE(XEXP(varop, 1)) == CONST_INT &&
          ...
          varop =
              simplify_gen_binary(ASHIFT, GET_MODE(varop), XEXP(varop, 0),
      ...
      case UDIV:
        ...
        if (GET_CODE(XEXP(varop, 1)) == CONST_INT &&
            exact_log2(INTVAL(XEXP(varop, 1))) >= 0) {
          varop =
              simplify_gen_binary(LSHIFTRT, GET_MODE(varop), XEXP(varop, 0),
                                  GEN_INT(exact_log2(INTVAL(XEXP(varop, 1)))));
          continue;
        }
        break;
      ...
      case ROTATE:
        ...
        if (GET_CODE(XEXP(varop, 1)) == CONST_INT &&
          ...
          if (code == ASHIFT && first_code == ASHIFTRT &&
            ...
            varop = simplify_and_const_int(NULL_RTX, result_mode,
                                           XEXP(varop, 0), mask);
            varop = simplify_shift_const(NULL_RTX, ASHIFT, result_mode, varop,
                                         count);
            count = first_count;
            code = ASHIFTRT;
            continue;
          ...
          mask_rtx = simplify_const_binary_operation(code, result_mode,
                                                     mask_rtx, GEN_INT(count));

          /* Give up if we can't compute an outer operation to use.  */
          if (mask_rtx == 0 || GET_CODE(mask_rtx) != CONST_INT ||
              !merge_outer_ops(&outer_op, &outer_const, AND, INTVAL(mask_rtx),
          ...
          /* If COUNT is positive, the new shift is usually CODE,
             ...
             always be used  */
          if (count > 0 && ((first_code == ROTATE && code == ASHIFT) ||
        ...
        /* If we have (A << B << C) for any shift, we can convert this to
          ...
          rtx new_rtx = simplify_const_binary_operation(
              code, mode, XEXP(varop, 0), GEN_INT(count));
      ...
      case NOT:
        ...
        varop = gen_rtx_XOR(mode, XEXP(varop, 0), GEN_INT(GET_MODE_MASK(mode)));
        continue;
      ...
      case XOR:
        ...
        if (GET_CODE(varop) == IOR && GET_CODE(XEXP(varop, 0)) == PLUS &&
          ...
          count = 0;
          varop = gen_rtx_LE(GET_MODE(varop), XEXP(varop, 1), const0_rtx);
        ...
        if (GET_CODE(XEXP(varop, 1)) == CONST_INT
            ...
            && !(code == ASHIFTRT && GET_CODE(varop) == XOR &&
                 0 > trunc_int_for_mode(INTVAL(XEXP(varop, 1)), shift_mode)) &&
            (new_rtx = simplify_const_binary_operation(
                 code, result_mode, XEXP(varop, 1), GEN_INT(count))) != 0 &&
        ...
        if (GET_CODE(XEXP(varop, 1)) == CONST_INT &&
          ...
          varop = simplify_gen_binary(GET_CODE(varop), shift_mode, lhs, rhs);
      ...
      case EQ:
        ...
        if (code == LSHIFTRT && XEXP(varop, 1) == const0_rtx &&
            ...
            merge_outer_ops(&outer_op, &outer_const, XOR, (HOST_WIDE_INT)1,
                            result_mode, &complement_p)) {
          varop = XEXP(varop, 0);
          count = 0;
          continue;
      ...
      case NEG:
        ...
        if (code == ASHIFT &&
            merge_outer_ops(&outer_op, &outer_const, NEG, (HOST_WIDE_INT)0,
                            result_mode, &complement_p)) {
          varop = XEXP(varop, 0);
          continue;
        }
      ...
      case PLUS:
        ...
        if (code == ASHIFT && GET_CODE(XEXP(varop, 1)) == CONST_INT &&
            (new_rtx = simplify_const_binary_operation(
                 ASHIFT, result_mode, XEXP(varop, 1), GEN_INT(count))) != 0 &&
            GET_CODE(new_rtx) == CONST_INT &&
            merge_outer_ops(&outer_op, &outer_const, PLUS, INTVAL(new_rtx),
                            result_mode, &complement_p)) {
          varop = XEXP(varop, 0);
          continue;
        }
        ...
        if (code == LSHIFTRT && GET_CODE(XEXP(varop, 1)) == CONST_INT &&
            mode_signbit_p(result_mode, XEXP(varop, 1)) &&
            (new_rtx = simplify_const_binary_operation(
                 code, result_mode, XEXP(varop, 1), GEN_INT(count))) != 0 &&
            GET_CODE(new_rtx) == CONST_INT &&
            merge_outer_ops(&outer_op, &outer_const, XOR, INTVAL(new_rtx),
                            result_mode, &complement_p)) {
          varop = XEXP(varop, 0);
          continue;
        }
      ...
      case MINUS:
        ...
        if ((STORE_FLAG_VALUE == 1 || STORE_FLAG_VALUE == -1) &&
            ...
            rtx_equal_p(XEXP(XEXP(varop, 0), 0), XEXP(varop, 1))) {
          count = 0;
          varop = gen_rtx_GT(GET_MODE(varop), XEXP(varop, 1), const0_rtx);
      ...
      case TRUNCATE:
        ...
        if (code == LSHIFTRT && GET_CODE(XEXP(varop, 0)) == LSHIFTRT &&
          ...
          varop_inner =
              gen_rtx_LSHIFTRT(GET_MODE(varop_inner), XEXP(varop_inner, 0),
  ...
  if (outer_op == UNKNOWN && orig_code == code && orig_count == count &&
      varop == orig_varop && shift_mode == GET_MODE(varop))
    return NULL_RTX;
  ...
  if (outer_op != UNKNOWN) {
    ...
    if (outer_op == AND)
      ...
      x = simplify_gen_binary(outer_op, result_mode, x, GEN_INT(outer_const));
  }
...
static rtx simplify_shift_const(rtx x, enum rtx_code code,
                                enum machine_mode result_mode, rtx varop,
                                int count) {
...
static int recog_for_combine(rtx *pnewpat, rtx insn, rtx *pnotes) {
  ...
  if (num_clobbers_to_add) {
    rtx newpat = gen_rtx_PARALLEL(
        VOIDmode, rtvec_alloc(GET_CODE(pat) == PARALLEL
    ...
    for (i = XVECLEN(newpat, 0) - num_clobbers_to_add; i < XVECLEN(newpat, 0);
         i++) {
      ...
      if (GET_CODE(XEXP(XVECEXP(newpat, 0, i), 0)) != SCRATCH) {
        ...
        notes = gen_rtx_EXPR_LIST(REG_UNUSED, XEXP(XVECEXP(newpat, 0, i), 0),
                                  notes);
      }
    }
    pat = newpat;
...
static rtx gen_lowpart_for_combine(enum machine_mode omode, rtx x) {
  ...
  if (omode == Pmode && (GET_CODE(x) == CONST || GET_CODE(x) == SYMBOL_REF ||
...
static enum rtx_code simplify_comparison(enum rtx_code code, rtx *pop0,
                                         rtx *pop1) {
  rtx op0 = *pop0;
  rtx op1 = *pop1;
  rtx tem, tem1;
  ...
  while (1) {
    ...
    if (code != GTU && code != GEU && code != LTU && code != LEU &&
        ...
        GET_CODE(XEXP(op0, 0)) == ASHIFT && GET_CODE(XEXP(op1, 0)) == ASHIFT &&
        GET_CODE(XEXP(XEXP(op0, 0), 0)) == SUBREG &&
        GET_CODE(XEXP(XEXP(op1, 0), 0)) == SUBREG &&
        (GET_MODE(SUBREG_REG(XEXP(XEXP(op0, 0), 0))) ==
         GET_MODE(SUBREG_REG(XEXP(XEXP(op1, 0), 0)))) &&
        GET_CODE(XEXP(op0, 1)) == CONST_INT && XEXP(op0, 1) == XEXP(op1, 1) &&
        XEXP(op0, 1) == XEXP(XEXP(op0, 0), 1) &&
        XEXP(op0, 1) == XEXP(XEXP(op1, 0), 1) &&
        (INTVAL(XEXP(op0, 1)) ==
         (GET_MODE_BITSIZE(GET_MODE(op0)) -
          (GET_MODE_BITSIZE(GET_MODE(SUBREG_REG(XEXP(XEXP(op0, 0), 0)))))))) {
  ...
  while (GET_CODE(op1) == CONST_INT) {
    ...
    if (GET_MODE_CLASS(mode) != MODE_INT &&
        !(mode == VOIDmode && (GET_CODE(op0) == COMPARE || COMPARISON_P(op0))))
      break;
    ...
    /* If we are comparing against a constant power of two and the value
       ...
       with zero.  */
    if (const_op &&
        (code == EQ || code == NE || code == GE || code == GEU || code == LT ||
         code == LTU) &&
        mode_width <= HOST_BITS_PER_WIDE_INT && exact_log2(const_op) >= 0 &&
    ...
    if (const_op == -1 &&
        (code == EQ || code == NE || code == GT || code == LE || code == GEU ||
         code == LTU) &&
    ...
    switch (code) {
      ...
      case LE:
        ...
        else if (const_op == 0 && mode_width <= HOST_BITS_PER_WIDE_INT &&
    ...
    if (sign_bit_comparison_p &&
        GET_MODE_BITSIZE(mode) <= HOST_BITS_PER_WIDE_INT)
      op0 = force_to_mode(
          op0, mode, ((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(mode) - 1)), 0);
    ...
    switch (GET_CODE(op0)) {
      case ZERO_EXTRACT:
        ...
        if (!SHIFT_COUNT_TRUNCATED && GET_CODE(XEXP(op0, 0)) == CONST_INT &&
            XEXP(op0, 1) == const1_rtx && equality_comparison_p &&
            const_op == 0 && (i = exact_log2(INTVAL(XEXP(op0, 0)))) >= 0) {
          if (BITS_BIG_ENDIAN) {
      ...
      case ROTATE:
        ...
        if (equality_comparison_p && GET_CODE(XEXP(op0, 1)) == CONST_INT &&
            (tem = simplify_binary_operation(ROTATERT, mode, op1,
                                             XEXP(op0, 1))) != 0) {
          op0 = XEXP(op0, 0);
          op1 = tem;
          continue;
        ...
        if (const_op == 0 && sign_bit_comparison_p &&
          ...
          op0 = simplify_and_const_int(
              NULL_RTX, mode, XEXP(op0, 0),
              ((HOST_WIDE_INT)1 << (mode_width - 1 - INTVAL(XEXP(op0, 1)))));
          code = (code == LT ? NE : EQ);
          continue;
        }
      ...
      case SUBREG:
        ...
        if (mode_width <= HOST_BITS_PER_WIDE_INT && subreg_lowpart_p(op0) &&
          ...
          if ((c1 > 0 &&
               (unsigned HOST_WIDE_INT)c1 < (unsigned HOST_WIDE_INT)1
                                                << (mode_width - 1) &&
               (equality_comparison_p || unsigned_comparison_p)
               /* (A - C1) zero-extends if it is positive and sign-extends
                  if it is negative, C2 both zero- and sign-extends.  */
               && ((0 == (nonzero_bits(a, inner_mode) & ~GET_MODE_MASK(mode)) &&
                    const_op >= 0)
      ...
      case PLUS:
        /* (eq (plus X A) B) -> (eq X (minus B A)).  We can only do
           ...
           overflows.  */
        if (equality_comparison_p &&
            0 != (tem = simplify_binary_operation(MINUS, mode, op1,
                                                  XEXP(op0, 1)))) {
          op0 = XEXP(op0, 0);
          op1 = tem;
          continue;
      ...
      case MINUS:
        ...
        /* (eq (minus A B) C) -> (eq A (plus B C)) or
           ...
           overflows.  */
        if (equality_comparison_p &&
            0 != (tem = simplify_binary_operation(PLUS, mode, XEXP(op0, 1),
                                                  op1))) {
          op0 = XEXP(op0, 0);
          op1 = tem;
          continue;
        ...
        if (equality_comparison_p &&
            0 != (tem = simplify_binary_operation(MINUS, mode, XEXP(op0, 0),
                                                  op1))) {
          op0 = XEXP(op0, 1);
          op1 = tem;
          continue;
      ...
      case XOR:
        ...
        if (equality_comparison_p && 0 != (tem = simplify_binary_operation(
                                               XOR, mode, XEXP(op0, 1), op1))) {
          op0 = XEXP(op0, 0);
          op1 = tem;
          continue;
        }
      ...
      case ORDERED:
        ...
        if (const_op != 0 || CC0_P(XEXP(op0, 0)) ||
        ...
        if (code == NE || code == EQ ||
            (GET_MODE_BITSIZE(GET_MODE(op0)) <= HOST_BITS_PER_WIDE_INT &&
             ...
             (STORE_FLAG_VALUE &
              (((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(GET_MODE(op0)) - 1)))) &&
             (code == LT || code == GE))) {
          enum rtx_code new_code;
          if (code == LT || code == NE)
      ...
      case AND:
        ...
        if (const_op == 0 && equality_comparison_p &&
          ...
          op0 = simplify_and_const_int(
              NULL_RTX, mode,
              gen_rtx_LSHIFTRT(mode, XEXP(op0, 1), XEXP(XEXP(op0, 0), 1)),
              (HOST_WIDE_INT)1);
          continue;
        }
        ...
        if ((equality_comparison_p || unsigned_comparison_p) &&
            GET_CODE(XEXP(op0, 1)) == CONST_INT &&
            (i = exact_log2((INTVAL(XEXP(op0, 1)) & GET_MODE_MASK(mode)) +
                            1)) >= 0 &&
            const_op >> i == 0 &&
        ...
        if (GET_CODE(XEXP(op0, 0)) == SUBREG) {
          ...
          if (SCALAR_INT_MODE_P(tmode)
              ...
              (c1 & ~GET_MODE_MASK(tmode)) == 0 && c1 != mask &&
              c1 != GET_MODE_MASK(tmode)) {
            op0 = simplify_gen_binary(AND, tmode, SUBREG_REG(XEXP(op0, 0)),
        ...
        if (const_op == 0 && equality_comparison_p &&
            XEXP(op0, 1) == const1_rtx && GET_CODE(XEXP(op0, 0)) == NOT) {
          op0 = simplify_and_const_int(NULL_RTX, mode, XEXP(XEXP(op0, 0), 0),
                                       (HOST_WIDE_INT)1);
          code = (code == NE ? EQ : NE);
          continue;
        }
        ...
        if (const_op == 0 && equality_comparison_p &&
            XEXP(op0, 1) == const1_rtx && GET_CODE(XEXP(op0, 0)) == LSHIFTRT) {
          ...
          if (GET_CODE(shift_op) == NOT ||
              (GET_CODE(shift_op) == XOR &&
               ...
               GET_MODE_BITSIZE(mode) <= HOST_BITS_PER_WIDE_INT &&
               (INTVAL(XEXP(shift_op, 1)) == (HOST_WIDE_INT)1
                                                 << INTVAL(shift_count)))) {
            op0 = simplify_and_const_int(
                NULL_RTX, mode,
      ...
      case ASHIFT:
        ...
        if (GET_CODE(XEXP(op0, 1)) == CONST_INT && INTVAL(XEXP(op0, 1)) >= 0 &&
            ((INTVAL(XEXP(op0, 1)) + !equality_comparison_p) <
             HOST_BITS_PER_WIDE_INT) &&
            ((const_op & (((HOST_WIDE_INT)1 << INTVAL(XEXP(op0, 1))) - 1)) ==
             0) &&
            mode_width <= HOST_BITS_PER_WIDE_INT &&
            (nonzero_bits(XEXP(op0, 0), mode) &
             ~(mask >> (INTVAL(XEXP(op0, 1)) + !equality_comparison_p))) == 0) {
        ...
        if (const_op == 0 && equality_comparison_p &&
            GET_CODE(XEXP(op0, 1)) == CONST_INT &&
            (unsigned HOST_WIDE_INT)INTVAL(XEXP(op0, 1)) == mode_width - 1) {
      ...
      case ASHIFTRT:
        ...
        if (equality_comparison_p && const_op == 0 &&
            GET_CODE(XEXP(op0, 1)) == CONST_INT) {
          op0 = simplify_shift_const(NULL_RTX, LSHIFTRT, mode, XEXP(op0, 0),
                                     INTVAL(XEXP(op0, 1)));
          continue;
        }
        ...
        if (!unsigned_comparison_p && GET_CODE(XEXP(op0, 1)) == CONST_INT &&
            GET_CODE(XEXP(op0, 0)) == ASHIFT &&
            XEXP(op0, 1) == XEXP(XEXP(op0, 0), 1) &&
            (tmode = mode_for_size(mode_width - INTVAL(XEXP(op0, 1)), MODE_INT,
                                   1)) != BLKmode &&
            (((unsigned HOST_WIDE_INT)const_op + (GET_MODE_MASK(tmode) >> 1) +
              1) <= GET_MODE_MASK(tmode))) {
        ...
        if (!unsigned_comparison_p && GET_CODE(XEXP(op0, 1)) == CONST_INT &&
          ...
          op0 = simplify_gen_binary(PLUS, tmode, gen_lowpart(tmode, inner),
                                    new_const);
          continue;
        }
        ...
        if (GET_CODE(XEXP(op0, 1)) == CONST_INT && INTVAL(XEXP(op0, 1)) >= 0 &&
            ...
            (((unsigned HOST_WIDE_INT)const_op +
              (GET_CODE(op0) != LSHIFTRT
                   ? ((GET_MODE_MASK(mode) >> INTVAL(XEXP(op0, 1)) >> 1) + 1)
  ...
  mode = GET_MODE(op0);
  if (mode != VOIDmode && GET_MODE_CLASS(mode) == MODE_INT &&
      GET_MODE_SIZE(mode) < UNITS_PER_WORD && !have_insn_for(COMPARE, mode))
    for (tmode = GET_MODE_WIDER_MODE(mode);
      ...
      if (have_insn_for(COMPARE, tmode)) {
        ...
        if (zero_extended || ((num_sign_bit_copies(op0, tmode) >
          ...
          if (GET_CODE(op0) == AND && !have_insn_for(AND, mode))
            op0 = simplify_gen_binary(AND, tmode,
                                      gen_lowpart(tmode, XEXP(op0, 0)),
                                      gen_lowpart(tmode, XEXP(op0, 1)));
        ...
        if (op1 == const0_rtx && (code == LT || code == GE) &&
            GET_MODE_BITSIZE(mode) <= HOST_BITS_PER_WIDE_INT) {
          op0 = simplify_gen_binary(
              AND, tmode, gen_lowpart(tmode, op0),
              GEN_INT((HOST_WIDE_INT)1 << (GET_MODE_BITSIZE(mode) - 1)));
          code = (code == LT) ? NE : EQ;
          break;
        }
      }
...
static void record_value_for_reg(rtx reg, rtx insn, rtx value) {
  ...
  if (value && insn && reg_overlap_mentioned_p(reg, value)) {
    ...
    if (tem) {
      if (ARITHMETIC_P(tem) && GET_CODE(XEXP(tem, 0)) == CLOBBER &&
  ...
  for (i = regno; i < endregno; i++) {
    rsp = VEC_index(reg_stat_type, reg_stat, i);
    rsp->last_set_label = label_tick;
    if (!insn || (value && rsp->last_set_table_tick >= label_tick_ebb_start))
  ...
  if (value && !get_last_value_validate(&value, insn, rsp->last_set_label, 0)) {
    value = copy_rtx(value);
    if (!get_last_value_validate(&value, insn, rsp->last_set_label, 1))
      value = 0;
  }
  ...
  if (value) {
    ...
    rsp->last_set_sign_bit_copies = num_sign_bit_copies(value, GET_MODE(reg));
  }
}
...
static bool reg_truncated_to_mode(enum machine_mode mode, const_rtx x) {
  reg_stat_type *rsp = VEC_index(reg_stat_type, reg_stat, REGNO(x));
  enum machine_mode truncated = rsp->truncated_to_mode;

  if (truncated == 0 || rsp->truncation_label < label_tick_ebb_start)
    return false;
...
static int record_truncated_value(rtx *p, void *data ATTRIBUTE_UNUSED) {
  rtx x = *p;
  ...
  if (rsp->truncated_to_mode == 0 ||
      ...
      (GET_MODE_SIZE(truncated_mode) < GET_MODE_SIZE(rsp->truncated_to_mode))) {
...
static void record_truncated_values(rtx *x, void *data ATTRIBUTE_UNUSED) {
    ...
    for (j = regno; j < endregno; j++) {
      ...
      if (rsp->last_set_invalid
          ...
          (!(regno >= FIRST_PSEUDO_REGISTER && REG_N_SETS(regno) == 1 &&
             (!REGNO_REG_SET_P(DF_LR_IN(ENTRY_BLOCK_PTR->next_bb), regno))) &&
  ...
  for (i = 0; i < len; i++) {
    if (fmt[i] == 'e') {
      ...
      if (i == 1 && ARITHMETIC_P(x)) {
        ...
        if (ARITHMETIC_P(x1) && (x0 == XEXP(x1, 0) || x0 == XEXP(x1, 1)))
          return get_last_value_validate(&XEXP(x1, x0 == XEXP(x1, 0) ? 1 : 0),
                                         insn, tick, replace);
      }

      if (get_last_value_validate(&XEXP(x, i), insn, tick, replace) == 0)
        return 0;
    } else if (fmt[i] == 'E')
      for (j = 0; j < XVECLEN(x, i); j++)
        if (get_last_value_validate(&XVECEXP(x, i, j), insn, tick, replace) ==
            0)
          return 0;
  }
...
static rtx get_last_value(const_rtx x) {
  unsigned int regno;
  rtx value;
  reg_stat_type *rsp;
...
static int use_crosses_set_p(const_rtx x, int from_luid) {
  const char *fmt;
  int i;
  enum rtx_code code = GET_CODE(x);
  ...
  if (code == REG) {
    ...
    for (; regno < endreg; regno++) {
      ...
      if (rsp->last_set && rsp->last_set_label == label_tick &&
    ...
    if (fmt[i] == 'E') {
      ...
      for (j = XVECLEN(x, i) - 1; j >= 0; j--)
        if (use_crosses_set_p(XVECEXP(x, i, j), from_luid))
          return 1;
    } else if (fmt[i] == 'e' && use_crosses_set_p(XEXP(x, i), from_luid))
      return 1;
  }
  return 0;
}
...
static void reg_dead_at_p_1(rtx dest, const_rtx x,
...
static void move_deaths(rtx x, rtx maybe_kill_insn, int from_luid, rtx to_insn,
                        rtx *pnotes) {
  const char *fmt;
  int len, i;
  enum rtx_code code = GET_CODE(x);
  ...
  if (code == REG) {
    ...
    if (where_dead && DF_INSN_LUID(where_dead) >= from_luid &&
      ...
      /* If we didn't find any note, or if we found a REG_DEAD note that
         ...
         their own REG_DEAD notes lying around.  */
                ...
                (note != 0 && (GET_MODE_SIZE(GET_MODE(XEXP(note, 0))) <
                               GET_MODE_SIZE(GET_MODE(x))))) &&
               regno < FIRST_PSEUDO_REGISTER &&
        ...
        for (i = regno + offset; i < ourend; i++)
          move_deaths(regno_reg_rtx[i], maybe_kill_insn, from_luid, to_insn,
                      &oldnotes);
      }
    ...
    if (GET_CODE(dest) == ZERO_EXTRACT || GET_CODE(dest) == STRICT_LOW_PART ||
        (GET_CODE(dest) == SUBREG &&
         (((GET_MODE_SIZE(GET_MODE(dest)) + UNITS_PER_WORD - 1) /
           UNITS_PER_WORD) ==
    ...
    if (MEM_P(dest))
      move_deaths(XEXP(dest, 0), maybe_kill_insn, from_luid, to_insn, pnotes);
    return;
  }
  ...
  for (i = 0; i < len; i++) {
    if (fmt[i] == 'E') {
      int j;
      for (j = XVECLEN(x, i) - 1; j >= 0; j--)
        move_deaths(XVECEXP(x, i, j), maybe_kill_insn, from_luid, to_insn,
                    pnotes);
    } else if (fmt[i] == 'e')
...
static void distribute_notes(rtx notes, rtx from_insn, rtx i3, rtx i2,
                             rtx elim_i2, rtx elim_i1) {
  rtx note, next_note;
  rtx tem;
  ...
  for (note = notes; note; note = next_note) {
    ...
    switch (REG_NOTE_KIND(note)) {
      ...
      case REG_DEAD:
        ...
        if (from_insn && from_insn == i2mod &&
            !reg_overlap_mentioned_p(XEXP(note, 0), i2mod_new_rhs))
          tem = from_insn;
        else {
          if (from_insn && CALL_P(from_insn) &&
            ...
            place = i2;
                    ...
                    !(i2mod &&
                      reg_overlap_mentioned_p(XEXP(note, 0), i2mod_old_rhs))) ||
          ...
          for (tem = PREV_INSN(tem); place == 0; tem = PREV_INSN(tem)) {
            ...
            if ((REGNO(XEXP(note, 0)) >= FIRST_PSEUDO_REGISTER ||
              ...
              if (set != 0 && !side_effects_p(SET_SRC(set)) &&
                ...
                distribute_notes(old_notes, tem, tem, NULL_RTX, NULL_RTX,
                                 NULL_RTX);
                ...
                if (cc0_setter) {
                  ...
                  distribute_notes(old_notes, cc0_setter, cc0_setter, NULL_RTX,
                                   NULL_RTX, NULL_RTX);
              ...
              /* If we are doing a 3->2 combination, and we have a
                 ...
                 i2.  */
              if (i2 && DF_INSN_LUID(place) > DF_INSN_LUID(i2) && from_insn &&
            ...
            if (!all_used) {
              ...
              for (i = regno; i < endregno;
                ...
                if (!dead_or_set_p(place, piece) &&
                  ...
                  rtx new_note = gen_rtx_EXPR_LIST(REG_DEAD, piece, NULL_RTX);

                  distribute_notes(new_note, place, place, NULL_RTX, NULL_RTX,
                                   NULL_RTX);
                } else if (!refers_to_regno_p(i, i + 1, PATTERN(place), 0) &&
                           !find_regno_fusage(place, USE, i))
                  for (tem = PREV_INSN(place);; tem = PREV_INSN(tem)) {
                    if (!INSN_P(tem)) {
                      if (tem == BB_HEAD(bb))
                        break;
                      continue;
                    ...
                    if (dead_or_set_p(tem, piece) ||
                        reg_bitfield_target_p(piece, PATTERN(tem))) {
    ...
    if (place2)
      REG_NOTES(place2) = gen_rtx_fmt_ee(GET_CODE(note), REG_NOTE_KIND(note),
...
static void distribute_links(rtx links) {
  ...
  for (link = links; link; link = next_link) {
    ...
    if (NOTE_P(XEXP(link, 0)) || (set = single_set(XEXP(link, 0))) == 0)
      continue;
...
static int unmentioned_reg_p_1(rtx *loc, void *expr) {
  rtx x = *loc;

  if (x != NULL_RTX && (REG_P(x) || MEM_P(x)) && !reg_mentioned_p(x, (rtx)expr))
    return 1;
  return 0;
}
...
static bool unmentioned_reg_p(rtx equiv, rtx expr) {
...
static bool gate_handle_combine(void) {
  return (optimize > 0);
}

/* Try combining insns through substitution.  */
static unsigned int rest_of_handle_combine(void) {
...
struct rtl_opt_pass pass_combine = {{
    ...
    0,                      /* todo_flags_start */
    TODO_dump_func | TODO_df_finish | TODO_verify_rtl_sharing |
