...
static tree lhs_of_dominating_assert(tree op, basic_block bb, gimple stmt) {
  ...
  FOR_EACH_IMM_USE_FAST(use_p, imm_iter, op) {
    ...
    if (use_stmt != stmt && gimple_assign_single_p(use_stmt) &&
...
static void record_temporary_equivalence(tree x, tree y,
                                         VEC(tree, heap) * *stack) {
...
static bool record_temporary_equivalences_from_phis(edge e,
  ...
  for (gsi = gsi_start_phis(e->dest); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    /* If the desired argument is not the same as this PHI's result
       ...
       through E->dest.  */
    if (src != dst && TREE_CODE(src) == SSA_NAME &&
...
static tree fold_assignment_stmt(gimple stmt) {
...
static gimple record_temporary_equivalences_from_stmts_at_dest(
    edge e, VEC(tree, heap) * *stack, tree (*simplify)(gimple, gimple)) {
  gimple stmt = NULL;
  gimple_stmt_iterator gsi;
  int max_stmt_count;
  ...
  for (gsi = gsi_start_bb(e->dest); !gsi_end_p(gsi); gsi_next(&gsi)) {
    ...
    if (gimple_assign_single_p(stmt) &&
      ...
      if (!cached_lhs || (TREE_CODE(cached_lhs) != SSA_NAME &&
...
static tree simplify_control_stmt_condition(edge e, gimple stmt,
                                            gimple dummy_cond,
  ...
  if (code == GIMPLE_COND) {
    ...
    fold_undefer_overflow_warnings(
        (cached_lhs && is_gimple_min_invariant(cached_lhs)), stmt,
...
void thread_across_edge(gimple dummy_cond, edge e,
                        bool handle_dominating_asserts,
                        VEC(tree, heap) * *stack,
                        tree (*simplify)(gimple, gimple)) {
  gimple stmt;
  ...
  /* If we stopped at a COND_EXPR or SWITCH_EXPR, see if we know which arm
     will be taken.  */
  if (gimple_code(stmt) == GIMPLE_COND || gimple_code(stmt) == GIMPLE_GOTO ||
