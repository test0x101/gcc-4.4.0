...
static void read_counts_file(void) {
  ...
  if (!gcov_magic(gcov_read_unsigned(), GCOV_DATA_MAGIC)) {
    ...
    warning(0, "%qs is version %q.*s, expected version %q.*s", da_file_name, 4,
            v, 4, e);
    gcov_close();
    return;
  }
  ...
  counts_hash = htab_create(10, htab_counts_entry_hash, htab_counts_entry_eq,
    ...
    if (tag == GCOV_TAG_FUNCTION) {
      ...
      if (!entry) {
        *slot = entry = XCNEW(counts_entry_t);
        ...
        error(
            "coverage mismatch for function %u while reading execution "
            "counters",
            fn_ident);
...
gcov_type *get_coverage_counts(unsigned counter, unsigned expected,
  ...
  if (!entry) {
    warning(0, "no coverage for function %qs found",
            IDENTIFIER_POINTER(DECL_ASSEMBLER_NAME(current_function_decl)));
    return NULL;
  }

  checksum = compute_checksum();
  if (entry->checksum != checksum || entry->summary.num != expected) {
    static int warned = 0;
    const char *id =
        IDENTIFIER_POINTER(DECL_ASSEMBLER_NAME(current_function_decl));
    ...
    if (warn_coverage_mismatch)
      ...
      error("coverage mismatch for function %qs while reading counter %qs", id,
            ctr_names[counter]);

    if (!inhibit_warnings) {
      if (entry->checksum != checksum)
        inform(input_location, "checksum is %x instead of %x", entry->checksum,
               checksum);
      else
    ...
    if (warn_coverage_mismatch && !inhibit_warnings && !warned++) {
      inform(input_location,
...
int coverage_counter_alloc(unsigned counter, unsigned num) {
  ...
  if (!tree_ctr_tables[counter]) {
    ...
    tree gcov_type_array_type = build_array_type(gcov_type_node, NULL_TREE);
...
static tree build_fn_info_value(const struct function_list *function,
  ...
  /* ident */
  value = tree_cons(
      fields, build_int_cstu(get_gcov_unsigned_t(), function->ident), value);
  fields = TREE_CHAIN(fields);

  /* checksum */
  value = tree_cons(
      fields, build_int_cstu(get_gcov_unsigned_t(), function->checksum), value);
  fields = TREE_CHAIN(fields);
  ...
  for (ix = 0; ix != GCOV_COUNTERS; ix++)
    if (prg_ctr_mask & (1 << ix)) {
      tree counters =
          build_int_cstu(get_gcov_unsigned_t(), function->n_ctrs[ix]);
  ...
  array_value =
      build_constructor_from_list(TREE_TYPE(fields), nreverse(array_value));
...
static tree build_ctr_info_type(void) {
  ...
  /* merge */
  gcov_merge_fn_type = build_function_type_list(
      void_type_node, gcov_ptr_type, get_gcov_unsigned_t(), NULL_TREE);
  field =
      build_decl(FIELD_DECL, NULL_TREE, build_pointer_type(gcov_merge_fn_type));
  TREE_CHAIN(field) = fields;
  fields = field;
...
static tree build_ctr_info_value(unsigned int counter, tree type) {
  ...
  value = tree_cons(fields,
                    build_int_cstu(get_gcov_unsigned_t(), prg_n_ctrs[counter]),
                    value);
  fields = TREE_CHAIN(fields);
  ...
  if (prg_n_ctrs[counter]) {
    ...
    array_type = build_int_cstu(get_gcov_unsigned_t(), prg_n_ctrs[counter] - 1);
    array_type = build_index_type(array_type);
    array_type = build_array_type(TREE_TYPE(TREE_TYPE(fields)), array_type);
    ...
    value = tree_cons(
        fields, build1(ADDR_EXPR, TREE_TYPE(fields), tree_ctr_tables[counter]),
        value);
  } else
    value = tree_cons(fields, null_pointer_node, value);
  fields = TREE_CHAIN(fields);

  fn = build_decl(FUNCTION_DECL, get_identifier(ctr_merge_functions[counter]),
                  TREE_TYPE(TREE_TYPE(fields)));
  DECL_EXTERNAL(fn) = 1;
  TREE_PUBLIC(fn) = 1;
  DECL_ARTIFICIAL(fn) = 1;
  TREE_NOTHROW(fn) = 1;
  value = tree_cons(fields, build1(ADDR_EXPR, TREE_TYPE(fields), fn), value);
...
static tree build_gcov_info(void) {
  ...
  fields = field;
  value =
      tree_cons(field, build_int_cstu(TREE_TYPE(field), GCOV_VERSION), value);
  ...
  TREE_CHAIN(field) = fields;
  fields = field;
  value = tree_cons(field, build_int_cstu(TREE_TYPE(field), local_tick), value);
  ...
  TREE_TYPE(filename_string) = build_array_type(
      char_type_node,
      build_index_type(build_int_cst(NULL_TREE, da_file_name_len)));
  value =
      tree_cons(field, build1(ADDR_EXPR, string_type, filename_string), value);

  /* Build the fn_info type and initializer.  */
  fn_info_type = build_fn_info_type(n_ctr_types);
  fn_info_ptr_type =
      build_pointer_type(build_qualified_type(fn_info_type, TYPE_QUAL_CONST));
  for (fn = functions_head, n_fns = 0; fn; fn = fn->next, n_fns++)
    fn_info_value = tree_cons(NULL_TREE, build_fn_info_value(fn, fn_info_type),
                              fn_info_value);
  if (n_fns) {
    tree array_type;
    ...
    /* FIXME: use build_constructor directly.  */
    fn_info_value =
        build_constructor_from_list(array_type, nreverse(fn_info_value));
  ...
  TREE_CHAIN(field) = fields;
  fields = field;
  value = tree_cons(field, build_int_cstu(get_gcov_unsigned_t(), n_fns), value);
  ...
  TREE_CHAIN(field) = fields;
  fields = field;
  value = tree_cons(field, build_int_cstu(get_gcov_unsigned_t(), prg_ctr_mask),
                    value);

  /* counters */
  ctr_info_type = build_ctr_info_type();
  ctr_info_ary_type = build_index_type(build_int_cst(NULL_TREE, n_ctr_types));
  ctr_info_ary_type = build_array_type(ctr_info_type, ctr_info_ary_type);
  for (ix = 0; ix != GCOV_COUNTERS; ix++)
    if (prg_ctr_mask & (1 << ix))
      ctr_info_value = tree_cons(
          NULL_TREE, build_ctr_info_value(ix, ctr_info_type), ctr_info_value);
  /* FIXME: use build_constructor directly.  */
  ctr_info_value =
      build_constructor_from_list(ctr_info_ary_type, nreverse(ctr_info_value));
...
void coverage_init(const char *filename) {
  ...
  /* Name of da file.  */
  da_file_name = XNEWVEC(char, len + strlen(GCOV_DATA_SUFFIX) + prefix_len + 1);
