...
static inline void set_depth(struct et_occ *occ, int d) {
  if (!occ)
    return;
...
static inline void set_depth_add(struct et_occ *occ, int d) {
  if (!occ)
    return;
...
static inline void set_prev(struct et_occ *occ, struct et_occ *t) {
#ifdef DEBUG_ET
  gcc_assert(occ != t);
#endif
...
static inline void set_next(struct et_occ *occ, struct et_occ *t) {
#ifdef DEBUG_ET
  gcc_assert(occ != t);
#endif
...
static inline void et_recomp_min(struct et_occ *occ) {
...
static void et_check_occ_sanity(struct et_occ *occ) {
  if (!occ)
    return;
  ...
  gcc_assert(!occ->parent || occ->parent->prev == occ ||
             occ->parent->next == occ);
}

/* Checks whether tree rooted at OCC is sane.  */

static void et_check_sanity(struct et_occ *occ) {
...
static void et_check_tree_sanity(struct et_occ *occ) {
  while (occ->parent)
    occ = occ->parent;
...
static int record_path_before_1(struct et_occ *occ, int depth) {
  int mn, m;
...
static void record_path_before(struct et_occ *occ) {
  while (occ->parent)
    occ = occ->parent;
...
static int check_path_after_1(struct et_occ *occ, int depth) {
  int mn, m;
...
static void check_path_after(struct et_occ *occ) {
  while (occ->parent)
    occ = occ->parent;
