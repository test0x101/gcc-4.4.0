...
static hashval_t hash_invariant_expr_1(rtx insn, rtx x) {
  enum rtx_code code = GET_CODE(x);
  int i, j;
  const char *fmt;
  hashval_t val = code;
...
static bool invariant_expr_equal_p(rtx insn1, rtx e1, rtx insn2, rtx e2) {
...
static hashval_t hash_invariant_expr(const void *e) {
...
static int eq_invariant_expr(const void *e1, const void *e2) {
...
static struct invariant *find_or_insert_inv(htab_t eq, rtx expr,
                                            enum machine_mode mode,
                                            struct invariant *inv) {
...
static void find_identical_invariants(htab_t eq, struct invariant *inv) {
  unsigned depno;
  bitmap_iterator bi;
...
static void compute_always_reached(struct loop *loop, basic_block *body,
...
static void find_exits(struct loop *loop, basic_block *body, bitmap may_exit,
                       bitmap has_exit) {
  unsigned i;
  edge_iterator ei;
  edge e;
  ...
  for (i = 0; i < loop->num_nodes; i++) {
    if (body[i]->loop_father == loop) {
      FOR_BB_INSNS(body[i], insn) {
        if (CALL_P(insn) && (RTL_LOOPING_CONST_OR_PURE_CALL_P(insn) ||
...
static bool may_assign_reg_p(rtx x) {
  return (GET_MODE(x) != VOIDmode && GET_MODE(x) != BLKmode &&
...
static void find_defs(struct loop *loop, basic_block *body) {
  unsigned i;
...
static struct invariant *create_new_invariant(struct def *def, rtx insn,
                                              bitmap depends_on,
                                              bool always_executed) {
  ...
  if (dump_file) {
    fprintf(dump_file, "Set in insn %d is invariant (%d), cost %d, depends on ",
...
static bool check_dependencies(rtx insn, bitmap depends_on) {
...
static void find_invariant_insn(rtx insn, bool always_reached,
                                bool always_executed) {
  df_ref ref;
  struct def *def;
  bitmap depends_on;
  ...
  if (!may_assign_reg_p(SET_DEST(set)) || !check_maybe_invariant(SET_SRC(set)))
    return;
...
static void find_invariants_insn(rtx insn, bool always_reached,
                                 bool always_executed) {
...
static void find_invariants_bb(basic_block bb, bool always_reached,
                               bool always_executed) {
  rtx insn;
...
static void find_invariants_body(struct loop *loop, basic_block *body,
                                 bitmap always_reached,
                                 bitmap always_executed) {
  unsigned i;

  for (i = 0; i < loop->num_nodes; i++)
    find_invariants_bb(body[i], bitmap_bit_p(always_reached, i),
...
static void get_inv_cost(struct invariant *inv, int *comp_cost,
  ...
  *comp_cost = 0;
  *regs_needed = 0;
  if (inv->move || inv->stamp == actual_stamp)
    return;
...
#ifdef STACK_REGS
  {
    ...
    if (set && IS_STACK_MODE(GET_MODE(SET_SRC(set))) &&
  ...
  EXECUTE_IF_SET_IN_BITMAP(inv->depends_on, 0, depno, bi) {
    ...
    if (aregs_needed
        ...
        && dep->always_executed && !dep->def->uses->next) {
...
static int gain_for_invariant(struct invariant *inv, unsigned *regs_needed,
  ...
  size_cost =
      (estimate_reg_pressure_cost(new_regs + *regs_needed, regs_used, speed) -
...
static int best_gain_for_invariant(struct invariant **best,
                                   unsigned *regs_needed, unsigned new_regs,
                                   unsigned regs_used, bool speed) {
  struct invariant *inv;
  int gain = 0, again;
  ...
  for (invno = 0; VEC_iterate(invariant_p, invariants, invno, inv); invno++) {
    ...
    again = gain_for_invariant(inv, &aregs_needed, new_regs, regs_used, speed);
    if (again > gain) {
      gain = again;
      *best = inv;
      *regs_needed = aregs_needed;
...
static void find_invariants_to_move(bool speed) {
  ...
  new_regs = 0;
  while (best_gain_for_invariant(&inv, &regs_needed, new_regs, regs_used,
                                 speed) > 0) {
...
static bool move_invariant_reg(struct loop *loop, unsigned invno) {
  struct invariant *inv = VEC_index(invariant_p, invariants, invno);
