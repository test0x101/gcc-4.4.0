...
static char *safe_concat(char *buf, char *cur, const char *str) {
...
static void print_exp(char *buf, const_rtx x, int verbose) {
  char tmp[BUF_LEN];
  const char *st[4];
  char *cur = buf;
...
void print_value(char *buf, const_rtx x, int verbose) {
  char t[BUF_LEN];
  char *cur = buf;

  switch (GET_CODE(x)) {
    case CONST_INT:
      sprintf(t, HOST_WIDE_INT_PRINT_HEX, (unsigned HOST_WIDE_INT)INTVAL(x));
    ...
    case CONST_DOUBLE:
      if (FLOAT_MODE_P(GET_MODE(x)))
        ...
        sprintf(t, "<" HOST_WIDE_INT_PRINT_HEX "," HOST_WIDE_INT_PRINT_HEX ">",
...
void print_pattern(char *buf, const_rtx x, int verbose) {
...
void print_insn(char *buf, const_rtx x, int verbose) {
  char t[BUF_LEN];
  const_rtx insn = x;
  ...
  switch (GET_CODE(x)) {
...
#ifdef INSN_SCHEDULING
      if (verbose && current_sched_info)
        sprintf(buf, "%s: jump %s", (*current_sched_info->print_insn)(x, 1), t);
      else
#endif
        sprintf(buf, " %4d %s", INSN_UID(x), t);
      break;
    ...
    case BARRIER:
      ...
      break;
    case NOTE:
      sprintf(buf, " %4d %s", INSN_UID(x), GET_NOTE_INSN_NAME(NOTE_KIND(x)));
      break;
    default:
      sprintf(buf, "i%4d  <What %s?>", INSN_UID(x), GET_RTX_NAME(GET_CODE(x)));
  }
} /* print_insn */
...
void print_rtl_slim_with_bb(FILE *f, rtx first, int flags) {
  print_rtl_slim(f, first, NULL, -1, flags);
}

/* Same as above, but stop at LAST or when COUNT == 0.
   If COUNT < 0 it will stop only at LAST or NULL rtx.  */
void print_rtl_slim(FILE *f, rtx first, rtx last, int count, int flags) {
  basic_block current_bb = NULL;
  rtx insn, tail;

  tail = last ? NEXT_INSN(last) : NULL_RTX;
  for (insn = first; (insn != NULL) && (insn != tail) && (count != 0);
       insn = NEXT_INSN(insn)) {
    if ((flags & TDF_BLOCKS) && (INSN_P(insn) || GET_CODE(insn) == NOTE) &&
        BLOCK_FOR_INSN(insn) && !current_bb) {
    ...
    if ((flags & TDF_BLOCKS) && current_bb && insn == BB_END(current_bb)) {
...
void debug_bb_slim(struct basic_block_def *bb) {
  print_rtl_slim(stderr, BB_HEAD(bb), BB_END(bb), -1, 32);
}

void debug_bb_n_slim(int n) {
