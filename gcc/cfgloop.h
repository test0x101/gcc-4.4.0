...
extern bool duplicate_loop_to_header_edge(struct loop *, edge, unsigned,
                                          sbitmap, edge, VEC(edge, heap) **,
                                          int);
extern struct loop *loopify(edge, edge, basic_block, edge, edge, bool, unsigned,
                            unsigned);
struct loop *loop_version(struct loop *, void *, basic_block *, unsigned,
...
static inline struct loop *get_loop(unsigned num) {
...
static inline VEC(loop_p, gc) * get_loops(void) {
  if (!current_loops)
    return NULL;
...
static inline unsigned number_of_loops(void) {
  if (!current_loops)
    return 0;
...
static inline void fel_next(loop_iterator *li, loop_p *loop) {
  int anum;
...
static inline void fel_init(loop_iterator *li, loop_p *loop, unsigned flags) {
  struct loop *aloop;
  unsigned i;
  int mn;
  ...
  if (flags & LI_ONLY_INNERMOST) {
    ...
    for (aloop = current_loops->tree_root; aloop->inner != NULL;
         aloop = aloop->inner)
      continue;
    ...
    while (1) {
      ...
      if (aloop->next) {
        for (aloop = aloop->next; aloop->inner != NULL; aloop = aloop->inner)
          continue;
...
#define FOR_EACH_LOOP(LI, LOOP, FLAGS) \
  for (fel_init(&(LI), &(LOOP), FLAGS); (LOOP); fel_next(&(LI), &(LOOP)))
