...
void dump_graph(FILE *f, struct graph *g) {
  ...
  for (i = 0; i < g->n_vertices; i++) {
    if (!g->vertices[i].pred && !g->vertices[i].succ)
      continue;
...
static inline int dfs_edge_src(struct graph_edge *e, bool forward) {
...
static inline int dfs_edge_dest(struct graph_edge *e, bool forward) {
...
static inline struct graph_edge *foll_in_subgraph(struct graph_edge *e,
                                                  bool forward,
                                                  bitmap subgraph) {
  int d;
...
static inline struct graph_edge *dfs_fst_edge(struct graph *g, int v,
...
static inline struct graph_edge *dfs_next_edge(struct graph_edge *e,
                                               bool forward, bitmap subgraph) {
  return foll_in_subgraph(forward ? e->succ_next : e->pred_next, forward,
                          subgraph);
}
...
int graphds_dfs(struct graph *g, int *qs, int nq, VEC(int, heap) * *qt,
                bool forward, bitmap subgraph) {
  ...
  for (i = 0; i < nq; i++) {
    ...
    while (1) {
      while (e) {
        if (g->vertices[dfs_edge_dest(e, forward)].component == -1)
          break;
...
int graphds_scc(struct graph *g, bitmap subgraph) {
...
void for_each_edge(struct graph *g, graphds_edge_callback callback) {
  struct graph_edge *e;
  int i;
...
void graphds_domtree(struct graph *g, int entry, int *parent, int *son,
                     int *brother) {
  VEC(int, heap) *postorder = NULL;
