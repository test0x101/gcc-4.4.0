...
static bool allocnos_have_intersected_live_ranges_p(ira_allocno_t a1,
                                                    ira_allocno_t a2) {
  if (a1 == a2)
    return false;
  if (ALLOCNO_REG(a1) != NULL && ALLOCNO_REG(a2) != NULL &&
      (ORIGINAL_REGNO(ALLOCNO_REG(a1)) == ORIGINAL_REGNO(ALLOCNO_REG(a2))))
    return false;
...
static bool pseudos_have_intersected_live_ranges_p(int regno1, int regno2) {
  ira_allocno_t a1, a2;
...
static void initiate_cost_update(void) {
  ...
  update_cost_queue_elems = (struct update_cost_queue_elem *)ira_allocate(size);
...
static inline void queue_update_cost(ira_allocno_t allocno, int divisor) {
...
/* Try to remove the first element from update_cost_queue.  Return false
   ...
   the removed element.  */
static inline bool get_next_update_cost(ira_allocno_t *allocno, int *divisor) {
  struct update_cost_queue_elem *elem;
...
static void update_copy_costs(ira_allocno_t allocno, bool decr_p) {
  ...
  do {
    ...
    for (cp = ALLOCNO_COPIES(allocno); cp != NULL; cp = next_cp) {
      ...
      ira_allocate_and_set_or_copy_costs(
          ...
          cover_class, 0, ALLOCNO_CONFLICT_HARD_REG_COSTS(another_allocno));
      i = ira_class_hard_reg_index[cover_class][hard_regno];
      ira_assert(i >= 0);
  ...
  while (get_next_update_cost(&allocno, &divisor))
    for (cp = ALLOCNO_COPIES(allocno); cp != NULL; cp = next_cp) {
      ...
      if (!ira_reg_classes_intersect_p[cover_class][another_cover_class] ||
          ALLOCNO_ASSIGNED_P(another_allocno) ||
          ALLOCNO_MAY_BE_SPILLED_P(
              ALLOCNO_FIRST_COALESCED_ALLOCNO(another_allocno)))
        continue;
      ...
      conflict_costs = ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS(another_allocno);
      ...
      if (cont_p && divisor <= (COST_HOP_DIVISOR * COST_HOP_DIVISOR *
...
static void print_coalesced_allocno(ira_allocno_t allocno) {
  ira_allocno_t a;
...
static bool assign_hard_reg(ira_allocno_t allocno, bool retry_p) {
  ...
  for (a = ALLOCNO_NEXT_COALESCED_ALLOCNO(allocno);;
    ...
    IOR_HARD_REG_SET(conflicting_regs, ALLOCNO_TOTAL_CONFLICT_HARD_REGS(a));
    ira_allocate_and_copy_costs(&ALLOCNO_UPDATED_HARD_REG_COSTS(a), cover_class,
                                ALLOCNO_HARD_REG_COSTS(a));
    a_costs = ALLOCNO_UPDATED_HARD_REG_COSTS(a);
#ifdef STACK_REGS
    no_stack_reg_p = no_stack_reg_p || ALLOCNO_TOTAL_NO_STACK_REG_P(a);
#endif
    for (cost = ALLOCNO_UPDATED_COVER_CLASS_COST(a), i = 0; i < class_size; i++)
      if (a_costs != NULL) {
        costs[i] += a_costs[i];
    ...
    if (retry_p || bitmap_bit_p(consideration_allocno_bitmap,
      ...
      ira_assert(
          ira_reg_classes_intersect_p[cover_class][conflict_cover_class]);
      if (allocno_coalesced_p) {
      ...
      if (ALLOCNO_ASSIGNED_P(conflict_allocno)) {
        if ((hard_regno = ALLOCNO_HARD_REGNO(conflict_allocno)) >= 0 &&
          ...
          IOR_HARD_REG_SET(conflicting_regs,
                           ira_reg_mode_hard_regset[hard_regno][ALLOCNO_MODE(
                               conflict_allocno)]);
          if (hard_reg_set_subset_p(reg_class_contents[cover_class],
                                    conflicting_regs))
            goto fail;
        }
      } else if (!ALLOCNO_MAY_BE_SPILLED_P(
        ...
        if (conflict_costs != NULL)
          for (j = class_size - 1; j >= 0; j--) {
            ...
            ira_assert(hard_regno >= 0);
            k = (ira_class_hard_reg_index[conflict_cover_class][hard_regno]);
            if (k < 0)
              continue;
  ...
  if (min_full_cost > mem_cost) {
    if (!retry_p && internal_flag_ira_verbose > 3 && ira_dump_file != NULL)
      fprintf(ira_dump_file, "(memory is more profitable %d vs %d) ", mem_cost,
              min_full_cost);
    best_hard_regno = -1;
  }
fail:
  if (flag_ira_algorithm != IRA_ALGORITHM_PRIORITY && best_hard_regno < 0 &&
    ...
    qsort(sorted_allocnos, j, sizeof(ira_allocno_t), allocno_cost_compare_func);
    for (i = 0; i < j; i++) {
      a = sorted_allocnos[i];
...
/* Add ALLOCNO to bucket *BUCKET_PTR.  ALLOCNO should be not in a bucket
   before the call.  */
static void add_allocno_to_bucket(ira_allocno_t allocno,
...
static void get_coalesced_allocnos_attributes(ira_allocno_t allocno, int *freq,
                                              int *num) {
  ira_allocno_t a;
...
static void sort_bucket(ira_allocno_t *bucket_ptr) {
  ...
  if (n <= 1)
    return;
  qsort(sorted_allocnos, n, sizeof(ira_allocno_t), bucket_allocno_compare_func);
  head = NULL;
  for (n--; n >= 0; n--) {
    a = sorted_allocnos[n];
...
static void add_allocno_to_ordered_bucket(ira_allocno_t allocno,
  ...
  for (before = *bucket_ptr, after = NULL; before != NULL;
...
/* Put ALLOCNO onto the coloring stack without removing it from its
   ...
   one.  */
static void push_allocno_to_stack(ira_allocno_t allocno) {
    ...
    FOR_EACH_ALLOCNO_CONFLICT(a, conflict_allocno, aci) {
      ...
      if (bitmap_bit_p(coloring_allocno_bitmap,
        ...
        if (ALLOCNO_IN_GRAPH_P(conflict_allocno) &&
          ...
          conflict_size = (ira_reg_class_nregs[cover_class]
                                              [ALLOCNO_MODE(conflict_allocno)]);
          ira_assert(ALLOCNO_LEFT_CONFLICTS_NUM(conflict_allocno) >= size);
          ...
          conflicts_num = ALLOCNO_LEFT_CONFLICTS_NUM(conflict_allocno) - size;
          if (uncolorable_allocnos_splay_tree[cover_class] != NULL &&
              !ALLOCNO_SPLAY_REMOVED_P(conflict_allocno) &&
              USE_SPLAY_P(cover_class)) {
            ira_assert(
                splay_tree_lookup(uncolorable_allocnos_splay_tree[cover_class],
...
/* Puts ALLOCNO chosen for potential spilling onto the coloring
   stack.  */
static void push_allocno_to_spill(ira_allocno_t allocno) {
  delete_allocno_from_bucket(allocno, &uncolorable_allocno_bucket);
  ALLOCNO_MAY_BE_SPILLED_P(allocno) = true;
  if (internal_flag_ira_verbose > 3 && ira_dump_file != NULL)
...
int ira_loop_edge_freq(ira_loop_tree_node_t loop_node, int regno, bool exit_p) {
  ...
  if (!exit_p) {
    ...
    if (e->src != loop_node->loop->latch &&
        (regno < 0 || (bitmap_bit_p(DF_LR_OUT(e->src), regno) &&
                       bitmap_bit_p(DF_LR_IN(e->dest), regno))))
      freq += EDGE_FREQUENCY(e);
  } else {
    edges = get_loop_exit_edges(loop_node->loop);
    for (i = 0; VEC_iterate(edge, edges, i, e); i++)
      if (regno < 0 || (bitmap_bit_p(DF_LR_OUT(e->src), regno) &&
                        bitmap_bit_p(DF_LR_IN(e->dest), regno)))
        freq += EDGE_FREQUENCY(e);
...
static int allocno_spill_priority_compare(splay_tree_key k1,
                                          splay_tree_key k2) {
  int pri1, pri2, diff;
...
/* Allocate data of SIZE for the splay trees.  We allocate only spay
   ...
   the function.  */
static void *splay_tree_allocate(int size, void *data ATTRIBUTE_UNUSED) {
...
/* Free data NODE for the splay trees.  We allocate and free only spay
   tree roots or splay tree nodes.  If you change this, please rewrite
   the function.  */
static void splay_tree_free(void *node, void *data ATTRIBUTE_UNUSED) {
  int i;
  enum reg_class cover_class;
...
static void push_allocnos_to_stack(void) {
  ...
  for (allocno = uncolorable_allocno_bucket; allocno != NULL;
  ...
  for (num = i = 0; i < ira_reg_class_cover_size; i++) {
    ...
    if (USE_SPLAY_P(cover_class))
      uncolorable_allocnos_splay_tree[cover_class] =
          splay_tree_new_with_allocator(allocno_spill_priority_compare, NULL,
                                        NULL, splay_tree_allocate,
  ...
  for (allocno = uncolorable_allocno_bucket; allocno != NULL;
    ...
    if ((cover_class = ALLOCNO_COVER_CLASS(allocno)) != NO_REGS) {
      cover_class_allocnos[cover_class]
                          [cover_class_allocnos_num[cover_class]++] = allocno;
      if (uncolorable_allocnos_splay_tree[cover_class] != NULL)
  ...
  for (;;) {
    ...
    /* Potential spilling.  */
    ira_assert(ira_reg_class_nregs[cover_class][ALLOCNO_MODE(allocno)] > 0);
    if (USE_SPLAY_P(cover_class)) {
      for (; VEC_length(ira_allocno_t, removed_splay_allocno_vec) != 0;) {
        ...
        if (ALLOCNO_LEFT_CONFLICTS_NUM(allocno) +
            ...
            ALLOCNO_AVAILABLE_REGS_NUM(allocno))
          splay_tree_insert(uncolorable_allocnos_splay_tree[rclass],
                            (splay_tree_key)allocno, (splay_tree_value)allocno);
      }
      allocno = ((ira_allocno_t)splay_tree_min(
                     uncolorable_allocnos_splay_tree[cover_class])
                     ->key);
      splay_tree_remove(uncolorable_allocnos_splay_tree[cover_class],
...
static void pop_allocnos_from_stack(void) {
  ...
  for (; VEC_length(ira_allocno_t, allocno_stack_vec) != 0;) {
    ...
    if (cover_class == NO_REGS) {
      ...
      ira_assert(ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS(allocno) == NULL);
      if (internal_flag_ira_verbose > 3 && ira_dump_file != NULL)
...
static void setup_allocno_available_regs_num(ira_allocno_t allocno) {
...
static void setup_allocno_left_conflicts_num(ira_allocno_t allocno) {
    ...
    for (a = ALLOCNO_NEXT_COALESCED_ALLOCNO(allocno);;
      ...
      FOR_EACH_ALLOCNO_CONFLICT(a, conflict_allocno, aci) {
        ...
        if (bitmap_bit_p(consideration_allocno_bitmap,
          ...
          if (!ALLOCNO_ASSIGNED_P(conflict_allocno))
            conflict_allocnos_size +=
                (ira_reg_class_nregs[cover_class]
                                    [ALLOCNO_MODE(conflict_allocno)]);
          else if ((hard_regno = ALLOCNO_HARD_REGNO(conflict_allocno)) >= 0) {
            int last =
                (hard_regno +
                 hard_regno_nregs[hard_regno][ALLOCNO_MODE(conflict_allocno)]);
...
/* The major function for aggressive allocno coalescing.  For the
   ...
   allocno_coalesced_p.  */
static void coalesce_allocnos(bool reload_p) {
  ira_allocno_t a;
  ...
  sorted_copies =
      (ira_copy_t *)ira_allocate(ira_copies_num * sizeof(ira_copy_t));
  cp_num = 0;
  /* Collect copies.  */
  EXECUTE_IF_SET_IN_BITMAP(coloring_allocno_bitmap, 0, j, bi) {
    ...
    if ((!reload_p && ALLOCNO_ASSIGNED_P(a)) ||
        (reload_p && (!ALLOCNO_ASSIGNED_P(a) || ALLOCNO_HARD_REGNO(a) >= 0 ||
    ...
    for (cp = ALLOCNO_COPIES(a); cp != NULL; cp = next_cp) {
      if (cp->first == a) {
        ...
        /* For priority coloring we coalesce allocnos only with
           ...
           simplicity.  */
        if ((reload_p || (ALLOCNO_COVER_CLASS(cp->second) == cover_class &&
...
static void setup_allocno_priorities(ira_allocno_t *consideration_allocnos,
  ...
  for (i = 0; i < n; i++) {
    ...
    allocno_priorities[ALLOCNO_NUM(a)] = priority =
        (mult * (ALLOCNO_MEMORY_COST(a) - ALLOCNO_COVER_CLASS_COST(a)) *
         ira_reg_class_nregs[ALLOCNO_COVER_CLASS(a)][ALLOCNO_MODE(a)]);
    if (priority < 0)
      priority = -priority;
...
static void print_loop_title(ira_loop_tree_node_t loop_tree_node) {
  ...
  fprintf(
      ...
      (loop_tree_node->parent == NULL ? -1 : loop_tree_node->parent->loop->num),
      loop_tree_node->loop->header->index, loop_depth(loop_tree_node->loop));
  for (subloop_node = loop_tree_node->children; subloop_node != NULL;
       subloop_node = subloop_node->next)
    if (subloop_node->bb != NULL) {
      fprintf(ira_dump_file, " %d", subloop_node->bb->index);
      FOR_EACH_EDGE(e, ei, subloop_node->bb->succs)
      if (e->dest != EXIT_BLOCK_PTR &&
          ((dest_loop_node = IRA_BB_NODE(e->dest)->parent) != loop_tree_node))
        fprintf(ira_dump_file, "(->%d:l%d)", e->dest->index,
                dest_loop_node->loop->num);
    }
  fprintf(ira_dump_file, "\n    all:");
...
static void color_pass(ira_loop_tree_node_t loop_tree_node) {
  ...
  /* Update costs of the corresponding allocnos (not caps) in the
     subloops.  */
  for (subloop_node = loop_tree_node->subloops; subloop_node != NULL;
    ...
    EXECUTE_IF_SET_IN_BITMAP(consideration_allocno_bitmap, 0, j, bi) {
      ...
      if (subloop_allocno == NULL || ALLOCNO_CAP(subloop_allocno) != NULL)
        continue;
      ...
      if (ira_reg_equiv_invariant_p[regno] ||
        ...
        ira_allocate_and_set_or_copy_costs(
            ...
            cover_class, 0, ALLOCNO_CONFLICT_HARD_REG_COSTS(subloop_allocno));
        ALLOCNO_UPDATED_HARD_REG_COSTS(subloop_allocno)[index] -= cost;
        ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS(subloop_allocno)[index] -= cost;
...
static void do_coloring(void) {
  ...
  splay_tree_node_pool = create_alloc_pool(
      "splay tree nodes", sizeof(struct splay_tree_node_s), 100);
...
static void move_spill_restore(void) {
  ...
  for (;;) {
    ...
    FOR_EACH_ALLOCNO(a, ai) {
      regno = ALLOCNO_REGNO(a);
      loop_node = ALLOCNO_LOOP_TREE_NODE(a);
      if (ALLOCNO_CAP_MEMBER(a) != NULL || ALLOCNO_CAP(a) != NULL ||
      ...
      ira_assert(index >= 0);
      cost =
          (ALLOCNO_MEMORY_COST(a) - (ALLOCNO_HARD_REG_COSTS(a) == NULL
                                         ? ALLOCNO_COVER_CLASS_COST(a)
                                         : ALLOCNO_HARD_REG_COSTS(a)[index]));
      for (subloop_node = loop_node->subloops; subloop_node != NULL;
           subloop_node = subloop_node->subloop_next) {
        ira_assert(subloop_node->bb == NULL);
        subloop_allocno = subloop_node->regno_allocno_map[regno];
      ...
      if ((parent = loop_node->parent) != NULL &&
        ...
        if ((hard_regno2 = ALLOCNO_HARD_REGNO(parent_allocno)) < 0)
          ...
          cost += (ira_memory_move_cost[mode][rclass][1] * exit_freq +
                   ira_memory_move_cost[mode][rclass][0] * enter_freq);
          if (hard_regno2 != hard_regno)
            cost -= (ira_register_move_cost[mode][rclass][rclass] *
                     (exit_freq + enter_freq));
...
static void update_curr_costs(ira_allocno_t a) {
  ...
  for (cp = ALLOCNO_COPIES(a); cp != NULL; cp = next_cp) {
    if (cp->first == a) {
      ...
      gcc_unreachable();
    if (!ira_reg_classes_intersect_p[cover_class]
    ...
    if (i < 0)
      continue;
    cost = (cp->first == a ? ira_register_move_cost[mode][rclass][cover_class]
                           : ira_register_move_cost[mode][cover_class][rclass]);
    ira_allocate_and_set_or_copy_costs(&ALLOCNO_UPDATED_HARD_REG_COSTS(a),
                                       cover_class, ALLOCNO_COVER_CLASS_COST(a),
                                       ALLOCNO_HARD_REG_COSTS(a));
    ira_allocate_and_set_or_copy_costs(
        &ALLOCNO_UPDATED_CONFLICT_HARD_REG_COSTS(a), cover_class, 0,
...
void ira_reassign_conflict_allocnos(int start_regno) {
  ...
  FOR_EACH_ALLOCNO(a, ai) {
    ...
    FOR_EACH_ALLOCNO_CONFLICT(a, conflict_a, aci) {
      ira_assert(ira_reg_classes_intersect_p[cover_class]
...
static int collect_spilled_coalesced_allocnos(
    int *pseudo_regnos, int n, ira_allocno_t *spilled_coalesced_allocnos) {
  int i, num, regno;
  ira_allocno_t allocno;
...
static void setup_slot_coalesced_allocno_live_ranges(ira_allocno_t allocno) {
  ...
  for (a = ALLOCNO_NEXT_COALESCED_ALLOCNO(allocno);;
    ...
    slot_coalesced_allocnos_live_ranges[n] = ira_merge_allocno_live_ranges(
...
static bool coalesce_spill_slots(ira_allocno_t *spilled_coalesced_allocnos,
  ...
  slot_coalesced_allocnos_live_ranges = (allocno_live_range_t *)ira_allocate(
    ...
    if (j >= i) {
      ...
      if (internal_flag_ira_verbose > 3 && ira_dump_file != NULL)
        fprintf(ira_dump_file,
                ...
                ALLOCNO_NUM(allocno), ALLOCNO_REGNO(allocno), ALLOCNO_NUM(a),
                ALLOCNO_REGNO(a));
  ...
  for (i = 0; i < ira_allocnos_num; i++)
    ira_finish_allocno_live_range_list(slot_coalesced_allocnos_live_ranges[i]);
...
/* Sort pseudo-register numbers in array PSEUDO_REGNOS of length N for
   ...
   reload.  */
void ira_sort_regnos_for_alter_reg(int *pseudo_regnos, int n,
  ...
  ira_free_bitmap(coloring_allocno_bitmap);
  regno_coalesced_allocno_cost = (int *)ira_allocate(max_regno * sizeof(int));
  regno_coalesced_allocno_num = (int *)ira_allocate(max_regno * sizeof(int));
  ...
  spilled_coalesced_allocnos =
      (ira_allocno_t *)ira_allocate(ira_allocnos_num * sizeof(ira_allocno_t));
  ...
  for (i = 0; i < num; i++) {
    ...
    for (a = ALLOCNO_NEXT_COALESCED_ALLOCNO(allocno);;
      ...
      if (internal_flag_ira_verbose > 3 && ira_dump_file != NULL)
        fprintf(ira_dump_file, " a%dr%d(%d,%d)", ALLOCNO_NUM(a),
...
void ira_mark_allocation_change(int regno) {
    ...
    cost =
        -(ALLOCNO_HARD_REG_COSTS(a) == NULL
              ...
              : ALLOCNO_HARD_REG_COSTS(
                    a)[ira_class_hard_reg_index[cover_class][old_hard_regno]]);
  ...
  if (hard_regno < 0) {
    ...
    cost += (ALLOCNO_HARD_REG_COSTS(a) == NULL
                 ? ALLOCNO_COVER_CLASS_COST(a)
                 : ALLOCNO_HARD_REG_COSTS(
                       a)[ira_class_hard_reg_index[cover_class][hard_regno]]);
    update_copy_costs(a, true);
  } else
...
void ira_mark_memory_move_deletion(int dst_regno, int src_regno) {
  ...
  ira_assert(dst != NULL && src != NULL && ALLOCNO_HARD_REGNO(dst) < 0 &&
...
static bool allocno_reload_assign(ira_allocno_t a,
...
/* Try to allocate hard registers to SPILLED_PSEUDO_REGS (there are
   ...
   iteration.  */
bool ira_reassign_pseudos(int *spilled_pseudo_regs, int num,
  ...
  for (m = i = 0; i < num; i++) {
    ...
    if (internal_flag_ira_verbose > 3 && ira_dump_file != NULL)
      fprintf(ira_dump_file, "      Spill %d(a%d), cost=%d", regno,
              ALLOCNO_NUM(a),
  ...
  for (i = n = 0; i < m; i++) {
    ...
    if (ALLOCNO_HARD_REGNO(conflict_a) < 0 &&
        ...
        !bitmap_bit_p(consideration_allocno_bitmap, ALLOCNO_NUM(conflict_a))) {
      sorted_allocnos[n++] = conflict_a;
      bitmap_set_bit(consideration_allocno_bitmap, ALLOCNO_NUM(conflict_a));
    }
  }
  if (n != 0) {
    setup_allocno_priorities(sorted_allocnos, n);
    ...
    for (i = 0; i < n; i++) {
      ...
      IOR_HARD_REG_SET(forbidden_regs, pseudo_previous_regs[regno]);
      if (internal_flag_ira_verbose > 3 && ira_dump_file != NULL)
        fprintf(ira_dump_file, "        Try assign %d(a%d), cost=%d", regno,
                ALLOCNO_NUM(a),
                ALLOCNO_MEMORY_COST(a) - ALLOCNO_COVER_CLASS_COST(a));
...
rtx ira_reuse_stack_slot(int regno, unsigned int inherent_size,
  ...
  ira_assert(inherent_size == PSEUDO_REGNO_BYTES(regno) &&
             inherent_size <= total_size && ALLOCNO_HARD_REGNO(allocno) < 0);
  if (!flag_ira_share_spill_slots)
    return NULL_RTX;
  ...
  if (slot_num != -1) {
    ...
    /* It means that the pseudo was spilled in the reload pass, try
       to reuse a slot.  */
    for (slot_num = 0; slot_num < ira_spilled_reg_stack_slots_num; slot_num++) {
      slot = &ira_spilled_reg_stack_slots[slot_num];
      ...
      EXECUTE_IF_SET_IN_BITMAP(&slot->spilled_regs, FIRST_PSEUDO_REGISTER, i,
                               bi) {
        another_allocno = ira_regno_allocno_map[i];
        if (allocnos_have_intersected_live_ranges_p(allocno, another_allocno))
          goto cont;
      }
      for (cost = 0, cp = ALLOCNO_COPIES(allocno); cp != NULL; cp = next_cp) {
        if (cp->first == allocno) {
          next_cp = cp->next_first_allocno_copy;
          another_allocno = cp->second;
        } else if (cp->second == allocno) {
        ...
        if (cp->insn == NULL_RTX)
          continue;
        if (bitmap_bit_p(&slot->spilled_regs, ALLOCNO_REGNO(another_allocno)))
          cost += cp->freq;
      }
      if (cost > best_cost) {
        best_cost = cost;
  ...
  if (x != NULL_RTX) {
    ...
    if (internal_flag_ira_verbose > 3 && ira_dump_file) {
      fprintf(ira_dump_file, "      Assigning %d(freq=%d) slot %d of", regno,
              REG_FREQ(regno), slot_num);
      EXECUTE_IF_SET_IN_BITMAP(&slot->spilled_regs, FIRST_PSEUDO_REGISTER, i,
                               bi) {
        if ((unsigned)regno != i)
...
void ira_mark_new_stack_slot(rtx x, int regno, unsigned int total_size) {
...
static int calculate_spill_cost(int *regnos, rtx in, rtx out, rtx insn,
                                int *excess_pressure_live_length, int *nrefs,
                                int *call_used_count, int *first_hard_regno) {
  ...
  for (length = count = cost = i = 0;; i++) {
    ...
    *nrefs += REG_N_REFS(regno);
    ...
    if ((in_p || out_p) &&
      ...
      if (in_p)
        saved_cost +=
            ira_memory_move_cost[ALLOCNO_MODE(a)][ALLOCNO_COVER_CLASS(a)][1];
      if (out_p)
        saved_cost +=
            ira_memory_move_cost[ALLOCNO_MODE(a)][ALLOCNO_COVER_CLASS(a)][0];
...
/* Return TRUE if spilling pseudo-registers whose numbers are in array
   ...
   decisions.  */
bool ira_better_spill_reload_regno_p(int *regnos, int *other_regnos, rtx in,
                                     rtx out, rtx insn) {
  int cost, other_cost;
  ...
  cost = calculate_spill_cost(regnos, in, out, insn, &length, &nrefs,
                              &call_used_count, &hard_regno);
  other_cost = calculate_spill_cost(other_regnos, in, out, insn, &other_length,
                                    &other_nrefs, &other_call_used_count,
                                    &other_hard_regno);
...
static void color(void) {
  ...
  removed_splay_allocno_vec = VEC_alloc(ira_allocno_t, heap, ira_allocnos_num);
...
static void fast_allocation(void) {
  ...
  used_hard_regs =
      (HARD_REG_SET *)ira_allocate(sizeof(HARD_REG_SET) * ira_max_point);
