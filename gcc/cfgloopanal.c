...
bool just_once_each_iteration_p(const struct loop *loop, const_basic_block bb) {
...
static void check_irred(struct graph *g, struct graph_edge *e) {
...
void mark_irreducible_loops(void) {
  ...
  FOR_EACH_EDGE(e, ei, act->succs) {
    ...
    if (!flow_bb_inside_loop_p(act->loop_father, e->dest)) {
      depth = 1 + loop_depth(
                      find_common_loop(act->loop_father, e->dest->loop_father));
...
int num_loop_insns(const struct loop *loop) {
  basic_block *bbs, bb;
  unsigned i, ninsns = 0;
  rtx insn;
...
int average_num_loop_insns(const struct loop *loop) {
  basic_block *bbs, bb;
...
gcov_type expected_loop_iterations_unbounded(const struct loop *loop) {
  edge e;
  edge_iterator ei;
...
unsigned expected_loop_iterations(const struct loop *loop) {
...
unsigned get_loop_level(const struct loop *loop) {
...
static unsigned seq_cost(const_rtx seq, bool speed) {
  unsigned cost = 0;
  rtx set;
...
unsigned estimate_reg_pressure_cost(unsigned n_new, unsigned n_old,
                                    bool speed) {
  unsigned cost;
...
void mark_loop_exit_edges(void) {
  ...
  FOR_EACH_BB(bb) {
    ...
    FOR_EACH_EDGE(e, ei, bb->succs) {
      if (loop_outer(bb->loop_father) && loop_exit_edge_p(bb->loop_father, e))
