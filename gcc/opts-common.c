...
static bool cancel_option(int opt_idx, int next_opt_idx,
...
void prune_options(int *argcp, char ***argvp) {
  int argc = *argcp;
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (opt_index == cl_options_count &&
        (opt[1] == 'W' || opt[1] == 'f' || opt[1] == 'm') && opt[2] == 'n' &&
        opt[3] == 'o' && opt[4] == '-') {
      char *dup;
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (opt_idx) {
      ...
      for (j = i + 1; j < argc; j++) {
        ...
        if (next_opt_idx && cancel_option(opt_idx, next_opt_idx, next_opt_idx))
          break;
      }
    } else
      goto keep;
