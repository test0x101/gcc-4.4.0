...
rtx doloop_condition_get(rtx doloop_pat) {
  rtx cmp;
  rtx inc;
  rtx reg;
  rtx inc_src;
  ...
  if (GET_CODE(pattern) != PARALLEL) {
    ...
    if ((PREV_INSN(doloop_pat) == NULL_RTX) || !INSN_P(PREV_INSN(doloop_pat)))
      return 0;
  ...
  if (GET_CODE(inc_src) != PLUS || XEXP(inc_src, 0) != reg ||
  ...
  /* Check for (set (pc) (if_then_else (condition)
                                       (label_ref (label))
                                       (pc))).  */
  if (GET_CODE(cmp) != SET || SET_DEST(cmp) != pc_rtx ||
  ...
  if ((GET_CODE(condition) != GE && GET_CODE(condition) != NE) ||
      (XEXP(condition, 1) != const0_rtx && XEXP(condition, 1) != const1_rtx))
    return 0;

  if ((XEXP(condition, 0) == reg) || (GET_CODE(XEXP(condition, 0)) == PLUS &&
...
static bool doloop_valid_p(struct loop *loop, struct niter_desc *desc) {
...
static void doloop_modify(struct loop *loop, struct niter_desc *desc,
  ...
  switch (GET_CODE(condition)) {
    ...
    case GE:
      ...
      if (desc->niter_max <=
          ((unsigned HOST_WIDEST_INT)1 << (GET_MODE_BITSIZE(mode) - 1)))
        nonneg = 1;
      break;
  ...
  if (zero_extend_p)
    count = simplify_gen_unary(ZERO_EXTEND, word_mode, count, from_mode);
  ...
  if (desc->noloop_assumptions) {
    ...
    if (EDGE_COUNT(set_zero->preds) == 0) {
      ...
      set_immediate_dominator(CDI_DOMINATORS, set_zero,
                              recompute_dominator(CDI_DOMINATORS, set_zero));
    }
...
#ifdef HAVE_doloop_begin
  {
    ...
    init = gen_doloop_begin(counter_reg,
                            ...
                            GEN_INT(desc->niter_max), GEN_INT(level));
    if (init) {
      start_sequence();
      emit_insn(init);
      sequence = get_insns();
  ...
  if (true_prob_val) {
    ...
    add_reg_note(jump_insn, REG_BR_PROB, GEN_INT(desc->in_edge->probability));
  }
}
...
static bool doloop_optimize(struct loop *loop) {
  enum machine_mode mode;
  ...
  if (est_niter < 3) {
    if (dump_file)
      fprintf(dump_file, "Doloop: Too few iterations (%u) to be profitable.\n",
              est_niter);
    return false;
  }

  max_cost = COSTS_N_INSNS(PARAM_VALUE(PARAM_MAX_ITERATIONS_COMPUTATION_COST));
  ...
  word_mode_max = ((unsigned HOST_WIDE_INT)1 << (word_mode_size - 1) << 1) - 1;
  if (!doloop_seq &&
      mode != word_mode
    ...
    if (word_mode_size > GET_MODE_BITSIZE(mode)) {
      zero_extend_p = true;
      iterations = simplify_gen_unary(ZERO_EXTEND, word_mode, iterations, mode);
      iterations_max =
          simplify_gen_unary(ZERO_EXTEND, word_mode, iterations_max, mode);
    } else {
  ...
  if (!doloop_seq) {
    if (dump_file)
      fprintf(dump_file, "Doloop: Target unwilling to use doloop pattern!\n");
    return false;
  }
  ...
  doloop_modify(loop, desc, doloop_seq, condition, count, zero_extend_p, mode);
  return true;
}
