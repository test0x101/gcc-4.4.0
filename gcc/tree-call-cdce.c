...
static inp_domain get_domain(int lb, bool has_lb, bool lb_inclusive, int ub,
                             bool has_ub, bool ub_inclusive) {
  inp_domain domain;
  domain.lb = lb;
...
static bool check_target_format(tree arg) {
  ...
  if ((mode == SFmode &&
      ...
      || (mode != SFmode && mode != DFmode &&
          (rfmt == &ieee_quad_format || rfmt == &mips_quad_format ||
...
static bool check_pow(gimple pow_call) {
  tree base, expn;
...
static bool check_builtin_call(gimple bcall) {
  tree arg;
...
static bool is_call_dce_candidate(gimple call) {
  tree fn;
  ...
  if (!fn || !DECL_BUILT_IN(fn) || (DECL_BUILT_IN_CLASS(fn) != BUILT_IN_NORMAL))
    return false;
...
static void gen_one_condition(tree arg, int lbub, enum tree_code tcode,
                              const char *temp_name1, const char *temp_name2,
                              VEC(gimple, heap) * conds, unsigned *nconds) {
  ...
  stmt2 = gimple_build_assign(
      tempc, fold_build2(tcode, boolean_type_node, tempn, lbub_real_cst));
...
static void gen_conditions_for_domain(tree arg, inp_domain domain,
                                      VEC(gimple, heap) * conds,
                                      unsigned *nconds) {
  if (domain.has_lb)
    gen_one_condition(arg, domain.lb,
                      (domain.is_lb_inclusive ? LT_EXPR : LE_EXPR),
                      "DCE_COND_LB", "DCE_COND_LB_TEST", conds, nconds);
  ...
  if (domain.has_ub) {
    ...
    gen_one_condition(arg, domain.ub,
                      (domain.is_ub_inclusive ? GT_EXPR : GE_EXPR),
                      "DCE_COND_UB", "DCE_COND_UB_TEST", conds, nconds);
  }
}
...
static void gen_conditions_for_pow_cst_base(tree base, tree expn,
                                            VEC(gimple, heap) * conds,
                                            unsigned *nconds) {
  ...
  gen_conditions_for_domain(expn, exp_domain, conds, nconds);
}
...
static void gen_conditions_for_pow_int_base(tree base, tree expn,
                                            VEC(gimple, heap) * conds,
                                            unsigned *nconds) {
  gimple base_def;
  ...
  gcc_assert(bit_sz > 0 && bit_sz <= MAX_BASE_INT_BIT_SIZE);
...
static void gen_conditions_for_pow(gimple pow_call, VEC(gimple, heap) * conds,
                                   unsigned *nconds) {
  tree base, expn;
  enum tree_code bc, ec;
...
static inp_domain get_no_error_domain(enum built_in_function fnc) {
  switch (fnc) {
    ...
    CASE_FLT_FN(BUILT_IN_ACOS)
        : CASE_FLT_FN(BUILT_IN_ASIN)
        : return get_domain(-1, true, true, 1, true, true);
    /* Hyperbolic functions.  */
    CASE_FLT_FN(BUILT_IN_ACOSH)
        : /* acosh: [1, +inf)  */
          return get_domain(1, true, true, 1, false, false);
    CASE_FLT_FN(BUILT_IN_ATANH)
        : /* atanh: (-1, +1)  */
          return get_domain(-1, true, false, 1, true, false);
    case BUILT_IN_COSHF:
    case BUILT_IN_SINHF:
      /* coshf: (-89, +89)  */
      return get_domain(-89, true, false, 89, true, false);
    case BUILT_IN_COSH:
    case BUILT_IN_SINH:
    case BUILT_IN_COSHL:
    case BUILT_IN_SINHL:
      /* cosh: (-710, +710)  */
      return get_domain(-710, true, false, 710, true, false);
      /* Log functions: (0, +inf)  */
      CASE_FLT_FN(BUILT_IN_LOG)
          : CASE_FLT_FN(BUILT_IN_LOG2)
          : CASE_FLT_FN(BUILT_IN_LOG10)
          : return get_domain(0, true, false, 0, false, false);
      CASE_FLT_FN(BUILT_IN_LOG1P)
          : return get_domain(-1, true, false, 0, false, false);
    /* Exp functions.  */
    case BUILT_IN_EXPF:
    case BUILT_IN_EXPM1F:
      /* expf: (-inf, 88)  */
      return get_domain(-1, false, false, 88, true, false);
    case BUILT_IN_EXP:
    case BUILT_IN_EXPM1:
    case BUILT_IN_EXPL:
    case BUILT_IN_EXPM1L:
      /* exp: (-inf, 709)  */
      return get_domain(-1, false, false, 709, true, false);
    case BUILT_IN_EXP2F:
      /* exp2f: (-inf, 128)  */
      return get_domain(-1, false, false, 128, true, false);
    case BUILT_IN_EXP2:
    case BUILT_IN_EXP2L:
      /* exp2: (-inf, 1024)  */
      return get_domain(-1, false, false, 1024, true, false);
    case BUILT_IN_EXP10F:
    case BUILT_IN_POW10F:
      /* exp10f: (-inf, 38)  */
      return get_domain(-1, false, false, 38, true, false);
    case BUILT_IN_EXP10:
    case BUILT_IN_POW10:
    case BUILT_IN_EXP10L:
    case BUILT_IN_POW10L:
      /* exp10: (-inf, 308)  */
      return get_domain(-1, false, false, 308, true, false);
      /* sqrt: [0, +inf)  */
      CASE_FLT_FN(BUILT_IN_SQRT)
          : return get_domain(0, true, true, 0, false, false);
    default:
      gcc_unreachable();
  }
...
static bool shrink_wrap_one_built_in_call(gimple bi_call) {
  ...
  join_tgt_in_edge_fall_thru =
      make_edge(guard_bb0, join_tgt_bb, EDGE_FALSE_VALUE);
...
static bool shrink_wrap_conditional_dead_built_in_calls(VEC(gimple, heap) *
                                                        calls) {
  bool changed = false;
  unsigned i = 0;
...
static unsigned int tree_call_cdce(void) {
  basic_block bb;
  ...
  FOR_EACH_BB(bb) {
    ...
    for (i = gsi_start_bb(bb); !gsi_end_p(i); gsi_next(&i)) {
      ...
      if (is_gimple_call(stmt) && is_call_dce_candidate(stmt)) {
        if (dump_file && (dump_flags & TDF_DETAILS)) {
...
static bool gate_call_cdce(void) {
